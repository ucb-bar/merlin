...
static iree_status_t pick_dispatch_entry_function(
	iree_vm_module_t *module, iree_vm_function_t *out_fn) {
	*out_fn = (iree_vm_function_t){0};
	...
	if (sig.export_function_count == 0) {
		return iree_make_status(
			IREE_STATUS_NOT_FOUND, "module has no exported functions");
	}
...
static iree_status_t prepare_node(iree_runtime_instance_t *instance,
	iree_hal_device_t *device, const char *vmfb_path, int32_t dispatch_iters,
	iree_allocator_t host_allocator, xpurt_iree_node_t *node) {
	node->session = NULL;
	...
	if (nmod == 0) {
		...
		node->session = NULL;
		return iree_make_status(
			IREE_STATUS_FAILED_PRECONDITION, "no modules in session");
	}
	...
	IREE_RETURN_IF_ERROR(iree_vm_list_create(
		iree_vm_make_undefined_type_def(), 1, host_allocator, &node->inputs));
...
static iree_status_t execute_node(xpurt_iree_node_t *node) {
	return iree_runtime_session_call(
		node->session, &node->entry_fn, node->inputs, NULL);
}
...
xpurt_status_t xpurt_run_dispatch_graph(
	...
	if (!json_path || !driver_name)
		return XPURT_STATUS_ERROR;

	xpurt_graph_t *graph = NULL;
	if (xpurt_graph_load(json_path, &graph) != XPURT_STATUS_OK)
		return XPURT_STATUS_ERROR;
	...
	xpurt_iree_node_t *prepared =
		(xpurt_iree_node_t *)calloc(node_count, sizeof(xpurt_iree_node_t));
	...
	status =
		iree_runtime_instance_create(&inst_opts, host_allocator, &instance);
	...
	for (size_t i = 0; i < node_count; ++i) {
		...
		status = prepare_node(
			instance, device, vmfb_path, 1, host_allocator, &prepared[idx]);
		if (!iree_status_is_ok(status)) {
			iree_status_ignore(status);
			result = XPURT_STATUS_ERROR;
			goto cleanup_prepared;
