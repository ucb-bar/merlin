...
static int json_skip_value(json_parser_t *jp) {
	...
	if (jp->p + 4 <= jp->end && strncmp(jp->p, "null", 4) == 0) {
		jp->p += 4;
		return 1;
	}
	return 0;
...
static int json_parse_deps(
	json_parser_t *jp, char ***out_deps, size_t *out_count) {
	*out_deps = NULL;
	*out_count = 0;
	if (!json_consume(jp, '['))
		return 0;
...
static int json_parse_dispatches(json_parser_t *jp, const char *json_dir,
	...
	*out_count = 0;
	...
	dispatch_node_core_t *nodes =
		(dispatch_node_core_t *)calloc(cap, sizeof(dispatch_node_core_t));
	if (!nodes)
		return 0;
	for (;;) {
		char *key = NULL;
		...
		if (count >= cap) {
			cap *= 2;
			dispatch_node_core_t *n = (dispatch_node_core_t *)realloc(
				nodes, cap * sizeof(dispatch_node_core_t));
			if (!n)
				goto fail_one;
...
static int find_node_idx(
	dispatch_node_core_t *nodes, size_t n, const char *key) {
	for (size_t i = 0; i < n; ++i)
		if (nodes[i].key && strcmp(nodes[i].key, key) == 0)
			return (int)i;
	return -1;
}

static int topo_sort(
	dispatch_node_core_t *nodes, size_t node_count, int **out_order) {
	*out_order = NULL;
	if (node_count == 0)
		return 0;
	...
	*out_graph = NULL;
	...
	while (1) {
		...
		if (strcmp(key, "dispatches") == 0) {
			...
			size_t node_count = 0;
			if (!json_parse_dispatches(&jp, json_dir, &nodes, &node_count))
				break;
			int *order = NULL;
