...
#ifndef IREE_HAL_DRIVERS_CUDA_TILE_NCCL_STATUS_UTIL_H_
#define IREE_HAL_DRIVERS_CUDA_TILE_NCCL_STATUS_UTIL_H_
...
#define IREE_NCCL_RESULT_TO_STATUS(syms, expr, ...)                          \
  iree_hal_cuda_tile_nccl_result_to_status((syms), ((syms)->expr), __FILE__, \
                                           __LINE__)
...
#define IREE_NCCL_RETURN_IF_ERROR(syms, expr, ...)                           \
  IREE_RETURN_IF_ERROR(iree_hal_cuda_tile_nccl_result_to_status(             \
...
#define IREE_NCCL_RETURN_AND_END_ZONE_IF_ERROR(zone_id, syms, expr, ...)      \
  IREE_RETURN_AND_END_ZONE_IF_ERROR(                                          \
      zone_id,                                                                \
      iree_hal_cuda_tile_nccl_result_to_status((syms), ((syms)->expr),        \
                                               __FILE__, __LINE__),           \
      __VA_ARGS__)
...
#define IREE_NCCL_IGNORE_ERROR(syms, expr)                    \
  IREE_IGNORE_ERROR(iree_hal_cuda_tile_nccl_result_to_status( \
      (syms), ((syms)->expr), __FILE__, __LINE__))

// Converts a ncclResult_t to an iree_status_t object.
iree_status_t iree_hal_cuda_tile_nccl_result_to_status(
    const iree_hal_cuda_tile_nccl_dynamic_symbols_t* syms, ncclResult_t result,
...
#endif  // IREE_HAL_DRIVERS_CUDA_TILE_NCCL_STATUS_UTIL_H_
