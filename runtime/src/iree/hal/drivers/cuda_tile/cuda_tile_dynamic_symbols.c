...
static const char* iree_hal_cuda_tile_dylib_names[] = {
...
static iree_status_t iree_hal_cuda_tile_dynamic_symbols_resolve_all(
...
#include "iree/hal/drivers/cuda_tile/cuda_tile_dynamic_symbol_table.h"  // IWYU pragma: keep
...
iree_status_t iree_hal_cuda_tile_dynamic_symbols_initialize(
    ...
    iree_hal_cuda_tile_dynamic_symbols_t* out_syms) {
  ...
  iree_status_t status = iree_dynamic_library_load_from_files(
      IREE_ARRAYSIZE(iree_hal_cuda_tile_dylib_names), iree_hal_cuda_tile_dylib_names,
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_dynamic_symbols_resolve_all(out_syms);
  }
  if (!iree_status_is_ok(status)) {
    iree_hal_cuda_tile_dynamic_symbols_deinitialize(out_syms);
  }
