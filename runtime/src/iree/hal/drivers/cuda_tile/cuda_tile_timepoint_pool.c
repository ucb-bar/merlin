...
//===----------------------------------------------------------------------===//
// iree_hal_cuda_tile_timepoint_t
//===----------------------------------------------------------------------===//

static iree_status_t iree_hal_cuda_tile_timepoint_allocate(
    iree_hal_cuda_tile_timepoint_pool_t* pool, iree_allocator_t host_allocator,
    iree_hal_cuda_tile_timepoint_t** out_timepoint) {
  IREE_ASSERT_ARGUMENT(pool);
  IREE_ASSERT_ARGUMENT(out_timepoint);
  *out_timepoint = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);

  iree_hal_cuda_tile_timepoint_t* timepoint = NULL;
...
static void iree_hal_cuda_tile_timepoint_clear(
    iree_hal_cuda_tile_timepoint_t* timepoint) {
  iree_allocator_t host_allocator = timepoint->host_allocator;
  iree_hal_cuda_tile_timepoint_pool_t* pool = timepoint->pool;
  memset(timepoint, 0, sizeof(*timepoint));
  timepoint->host_allocator = host_allocator;
  timepoint->pool = pool;
}

static void iree_hal_cuda_tile_timepoint_free(iree_hal_cuda_tile_timepoint_t* timepoint) {
  iree_allocator_t host_allocator = timepoint->host_allocator;
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_ASSERT(timepoint->kind == IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_NONE);
...
//===----------------------------------------------------------------------===//
// iree_hal_cuda_tile_timepoint_pool_t
//===----------------------------------------------------------------------===//

struct iree_hal_cuda_tile_timepoint_pool_t {
  ...
  iree_hal_cuda_tile_timepoint_t* available_list[] IREE_GUARDED_BY(timepoint_mutex);
};
// + Additional inline allocation for holding timepoints up to the capacity.

iree_status_t iree_hal_cuda_tile_timepoint_pool_allocate(
    iree_event_pool_t* host_event_pool,
    iree_hal_cuda_tile_event_pool_t* device_event_pool,
    iree_host_size_t available_capacity, iree_allocator_t host_allocator,
    iree_hal_cuda_tile_timepoint_pool_t** out_timepoint_pool) {
  ...
  *out_timepoint_pool = NULL;
  ...
  for (iree_host_size_t i = 0; i < available_capacity; ++i) {
    status = iree_hal_cuda_tile_timepoint_allocate(
  ...
  if (iree_status_is_ok(status)) {
    *out_timepoint_pool = timepoint_pool;
    ...
    iree_hal_cuda_tile_timepoint_pool_free(timepoint_pool);
  }
  IREE_TRACE_ZONE_END(z0);
  return status;
}

void iree_hal_cuda_tile_timepoint_pool_free(
  ...
  for (iree_host_size_t i = 0; i < timepoint_pool->available_count; ++i) {
    iree_hal_cuda_tile_timepoint_free(timepoint_pool->available_list[i]);
  }
...
// payload values.
static iree_status_t iree_hal_cuda_tile_timepoint_pool_acquire_internal(
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
    iree_host_size_t timepoint_count,
    iree_hal_cuda_tile_timepoint_t** out_timepoints) {
  ...
  // new iree_hal_cuda_tile_timepoint_t objects.
  ...
  if (remaining_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < remaining_count; ++i) {
      ...
      if (!iree_status_is_ok(status)) {
        ...
        iree_hal_cuda_tile_timepoint_pool_release(
...
iree_status_t iree_hal_cuda_tile_timepoint_pool_acquire_host_wait(
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
    iree_host_size_t timepoint_count,
    iree_hal_cuda_tile_timepoint_t** out_timepoints) {
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_internal(
              timepoint_pool, timepoint_count, out_timepoints));
  for (iree_host_size_t i = 0; i < timepoint_count; ++i) {
    out_timepoints[i]->kind = IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_HOST_WAIT;
...
iree_status_t iree_hal_cuda_tile_timepoint_pool_acquire_device_signal(
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
    iree_host_size_t timepoint_count,
    iree_hal_cuda_tile_timepoint_t** out_timepoints) {
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  // timepoints to avoid nested locks.
  iree_hal_cuda_tile_event_t** device_events = iree_alloca(
      timepoint_count * sizeof((*out_timepoints)->timepoint.device_signal));
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_event_pool_acquire(timepoint_pool->device_event_pool,
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_internal(
              timepoint_pool, timepoint_count, out_timepoints));
  for (iree_host_size_t i = 0; i < timepoint_count; ++i) {
    out_timepoints[i]->kind = IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_SIGNAL;
...
iree_status_t iree_hal_cuda_tile_timepoint_pool_acquire_device_wait(
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
    iree_host_size_t timepoint_count,
    iree_hal_cuda_tile_timepoint_t** out_timepoints) {
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_internal(
              timepoint_pool, timepoint_count, out_timepoints));
  for (iree_host_size_t i = 0; i < timepoint_count; ++i) {
    out_timepoints[i]->kind = IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_WAIT;
...
void iree_hal_cuda_tile_timepoint_pool_release(
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
    iree_host_size_t timepoint_count, iree_hal_cuda_tile_timepoint_t** timepoints) {
  IREE_ASSERT_ARGUMENT(timepoint_pool);
  ...
  for (iree_host_size_t i = 0; i < timepoint_count; ++i) {
    switch (timepoints[i]->kind) {
      case IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_HOST_WAIT:
        iree_event_pool_release(timepoint_pool->host_event_pool, 1,
                                &timepoints[i]->timepoint.host_wait);
        break;
      case IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_SIGNAL:
        iree_hal_cuda_tile_event_release(timepoints[i]->timepoint.device_signal);
        break;
      case IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_WAIT:
        iree_hal_cuda_tile_event_release(timepoints[i]->timepoint.device_wait);
        break;
      default:
        break;
    }
  ...
  if (to_pool_count > 0) {
    for (iree_host_size_t i = 0; i < to_pool_count; ++i) {
      iree_hal_cuda_tile_timepoint_clear(timepoints[i]);
    }
  ...
  if (remaining_count > 0) {
    ...
    for (iree_host_size_t i = 0; i < remaining_count; ++i) {
      iree_hal_cuda_tile_timepoint_clear(timepoints[to_pool_count + i]);
      iree_hal_cuda_tile_timepoint_free(timepoints[to_pool_count + i]);
    }
    IREE_TRACE_ZONE_END(z1);
  }
  IREE_TRACE_ZONE_END(z0);
