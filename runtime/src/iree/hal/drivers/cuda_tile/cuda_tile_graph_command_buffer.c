...
// barriers.
#define IREE_HAL_CUDA_TILE_MAX_CONCURRENT_GRAPH_NODE_COUNT 32
...
typedef struct iree_hal_cuda_tile_graph_command_buffer_t {
  ...
  CUgraphNode
      cu_graph_nodes[IREE_HAL_CUDA_TILE_MAX_CONCURRENT_GRAPH_NODE_COUNT];
...
} iree_hal_cuda_tile_graph_command_buffer_t;

static const iree_hal_command_buffer_vtable_t
    iree_hal_cuda_tile_graph_command_buffer_vtable;

static iree_status_t
iree_hal_cuda_tile_graph_command_buffer_execution_barrier_internal(
    iree_hal_cuda_tile_graph_command_buffer_t* command_buffer);

static iree_hal_cuda_tile_graph_command_buffer_t*
iree_hal_cuda_tile_graph_command_buffer_cast(
    iree_hal_command_buffer_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value,
                        &iree_hal_cuda_tile_graph_command_buffer_vtable);
  return (iree_hal_cuda_tile_graph_command_buffer_t*)base_value;
}

#if IREE_TRACING_FEATURES & IREE_TRACING_FEATURE_INSTRUMENTATION_DEVICE

static void iree_cuda_tile_graph_command_buffer_trace_zone_begin_external(
    iree_hal_cuda_tile_graph_command_buffer_t* command_buffer,
...
static void iree_cuda_tile_graph_command_buffer_trace_zone_end(
    iree_hal_cuda_tile_graph_command_buffer_t* command_buffer,
    int32_t verbosity) {
  // Make sure there are no new nodes after the last barrier.
...
#define IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_BEGIN_EXTERNAL( \
    command_buffer, verbosity, file_name, file_name_length, line,      \
    function_name, function_name_length, name, name_length)            \
  iree_cuda_tile_graph_command_buffer_trace_zone_begin_external(       \
      command_buffer, verbosity, file_name, file_name_length, line,    \
      function_name, function_name_length, name, name_length)
#define IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_BEGIN(command_buffer, \
                                                             verbosity)      \
  IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_BEGIN_EXTERNAL(             \
      command_buffer, verbosity, /*file_name=*/NULL, 0, /*line=*/0,          \
      __FUNCTION__, strlen(__FUNCTION__), /*name=*/NULL, 0)
#define IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_END(command_buffer, \
                                                           verbosity)      \
  iree_cuda_tile_graph_command_buffer_trace_zone_end(command_buffer, verbosity)
...
iree_status_t iree_hal_cuda_tile_graph_command_buffer_create(
  ...
  *out_command_buffer = NULL;
  ...
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer = NULL;
  ...
  iree_hal_command_buffer_initialize(
      ...
      &iree_hal_cuda_tile_graph_command_buffer_vtable, &command_buffer->base);
...
static void iree_hal_cuda_tile_graph_command_buffer_destroy(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
  ...
  if (command_buffer->cu_graph != NULL) {
    IREE_CUDA_TILE_IGNORE_ERROR(command_buffer->symbols,
...
bool iree_hal_cuda_tile_graph_command_buffer_isa(
    iree_hal_command_buffer_t* command_buffer) {
  return iree_hal_resource_is(&command_buffer->resource,
                              &iree_hal_cuda_tile_graph_command_buffer_vtable);
}

CUgraphExec iree_hal_cuda_tile_graph_command_buffer_handle(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
...
void iree_hal_cuda_tile_graph_tracing_notify_submitted_commands(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
...
static iree_status_t
iree_hal_cuda_tile_graph_command_buffer_flush_collectives(
    iree_hal_cuda_tile_graph_command_buffer_t* command_buffer) {
  ...
  //  iree_hal_cuda_tile_nccl_submit_batch(command_buffer->context,
  //                                  &command_buffer->collective_batch,
  //                                  nccl_stream);
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_begin(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
  ...
  IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_BEGIN(
      command_buffer, IREE_HAL_STREAM_TRACING_VERBOSITY_COARSE);

  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_graph_command_buffer_end(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
  ...
  IREE_CUDA_TILE_GRAPH_COMMAND_BUFFER_TRACE_ZONE_END(
      command_buffer, IREE_HAL_STREAM_TRACING_VERBOSITY_COARSE);
...
static iree_status_t
iree_hal_cuda_tile_graph_command_buffer_begin_debug_group(
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_end_debug_group(
    iree_hal_command_buffer_t* base_command_buffer) {
  iree_hal_cuda_tile_graph_command_buffer_t* command_buffer =
      iree_hal_cuda_tile_graph_command_buffer_cast(base_command_buffer);
  (void)command_buffer;
...
static iree_status_t
iree_hal_cuda_tile_graph_command_buffer_execution_barrier(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_signal_event(
    iree_hal_command_buffer_t* base_command_buffer, iree_hal_event_t* event,
    iree_hal_execution_stage_t source_stage_mask) {
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "event not yet supported");
}

static iree_status_t iree_hal_cuda_tile_graph_command_buffer_reset_event(
    iree_hal_command_buffer_t* base_command_buffer, iree_hal_event_t* event,
    iree_hal_execution_stage_t source_stage_mask) {
  return iree_make_status(IREE_STATUS_UNIMPLEMENTED, "event not yet supported");
}

static iree_status_t iree_hal_cuda_tile_graph_command_buffer_wait_events(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_advise_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
...
static uint32_t iree_hal_cuda_tile_splat_pattern(const void* pattern,
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_fill_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_graph_command_buffer_flush_collectives(
              command_buffer));
  ...
  uint32_t pattern_4byte =
      iree_hal_cuda_tile_splat_pattern(pattern, pattern_length);
  ...
  if (command_buffer->graph_node_count >=
      IREE_HAL_CUDA_TILE_MAX_CONCURRENT_GRAPH_NODE_COUNT) {
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_update_buffer(
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_copy_buffer(
    iree_hal_command_buffer_t* base_command_buffer,
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_collective(
...
static iree_status_t iree_hal_cuda_tile_graph_command_buffer_dispatch(
    iree_hal_command_buffer_t* base_command_buffer,
  ...
  // cuda_tile dispatch: block_dims always {1,1,1}, shared memory managed
  // by cubin.
...
static const iree_hal_command_buffer_vtable_t
    iree_hal_cuda_tile_graph_command_buffer_vtable = {
        .destroy = iree_hal_cuda_tile_graph_command_buffer_destroy,
        .begin = iree_hal_cuda_tile_graph_command_buffer_begin,
        .end = iree_hal_cuda_tile_graph_command_buffer_end,
        .begin_debug_group =
            iree_hal_cuda_tile_graph_command_buffer_begin_debug_group,
        .end_debug_group =
            iree_hal_cuda_tile_graph_command_buffer_end_debug_group,
        .execution_barrier =
            iree_hal_cuda_tile_graph_command_buffer_execution_barrier,
        .signal_event = iree_hal_cuda_tile_graph_command_buffer_signal_event,
        .reset_event = iree_hal_cuda_tile_graph_command_buffer_reset_event,
        .wait_events = iree_hal_cuda_tile_graph_command_buffer_wait_events,
        .advise_buffer = iree_hal_cuda_tile_graph_command_buffer_advise_buffer,
        .fill_buffer = iree_hal_cuda_tile_graph_command_buffer_fill_buffer,
        .update_buffer = iree_hal_cuda_tile_graph_command_buffer_update_buffer,
        .copy_buffer = iree_hal_cuda_tile_graph_command_buffer_copy_buffer,
        .collective = iree_hal_cuda_tile_graph_command_buffer_collective,
        .dispatch = iree_hal_cuda_tile_graph_command_buffer_dispatch,
};
