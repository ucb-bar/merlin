...
static const char* IREE_HAL_CUDA_TILE_ALLOCATOR_ID = "CUDA unpooled";
#endif  // IREE_TRACING_FEATURE_ALLOCATION_TRACKING

typedef struct iree_hal_cuda_tile_allocator_t {
...
} iree_hal_cuda_tile_allocator_t;

static const iree_hal_allocator_vtable_t iree_hal_cuda_tile_allocator_vtable;

static iree_hal_cuda_tile_allocator_t* iree_hal_cuda_tile_allocator_cast(
    iree_hal_allocator_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_allocator_vtable);
  return (iree_hal_cuda_tile_allocator_t*)base_value;
}

iree_status_t iree_hal_cuda_tile_allocator_create(
    iree_hal_device_t* parent_device,
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols, CUdevice device,
    CUstream stream, iree_hal_cuda_tile_memory_pools_t* pools,
  ...
  iree_hal_cuda_tile_allocator_t* allocator = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*allocator),
                                (void**)&allocator));

  iree_hal_resource_initialize(&iree_hal_cuda_tile_allocator_vtable,
                               &allocator->resource);
...
static void iree_hal_cuda_tile_allocator_destroy(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  IREE_ASSERT_ARGUMENT(base_allocator);
  iree_hal_cuda_tile_allocator_t* allocator =
      iree_hal_cuda_tile_allocator_cast(base_allocator);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_allocator_t iree_hal_cuda_tile_allocator_host_allocator(
    const iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  iree_hal_cuda_tile_allocator_t* allocator =
      (iree_hal_cuda_tile_allocator_t*)base_allocator;
  return allocator->host_allocator;
}

static iree_status_t iree_hal_cuda_tile_allocator_trim(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator) {
  return iree_ok_status();
}

static void iree_hal_cuda_tile_allocator_query_statistics(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  IREE_STATISTICS({
    ...
    if (allocator->pools) {
      iree_hal_cuda_tile_memory_pools_merge_statistics(allocator->pools,
                                                  out_statistics);
    }
  });
}

static iree_status_t iree_hal_cuda_tile_allocator_query_memory_heaps(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
...
iree_hal_cuda_tile_allocator_query_buffer_compatibility(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
...
static void iree_hal_cuda_tile_buffer_release_callback(void* user_data,
                                                  iree_hal_buffer_t* buffer);

static iree_status_t iree_hal_cuda_tile_allocator_allocate_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  iree_hal_cuda_tile_buffer_type_t buffer_type = IREE_HAL_CUDA_TILE_BUFFER_TYPE_DEVICE;
  void* host_ptr = NULL;
  CUdeviceptr device_ptr = 0;
  IREE_TRACE_ZONE_BEGIN_NAMED(z0, "iree_hal_cuda_tile_buffer_allocate");
  ...
  if (iree_all_bits_set(compat_params.type,
    ...
    if (iree_all_bits_set(compat_params.type,
      ...
      buffer_type = IREE_HAL_CUDA_TILE_BUFFER_TYPE_DEVICE;
    ...
    // Host local cases.
    buffer_type = IREE_HAL_CUDA_TILE_BUFFER_TYPE_HOST;
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_buffer_release_callback_t callback = {
        .fn = iree_hal_cuda_tile_buffer_release_callback,
        .user_data = (void*)base_allocator};
    status = iree_hal_cuda_tile_buffer_wrap(
  ...
  if (iree_status_is_ok(status)) {
    IREE_TRACE_ALLOC_NAMED(IREE_HAL_CUDA_TILE_ALLOCATOR_ID,
                           (void*)iree_hal_cuda_tile_buffer_device_pointer(buffer),
                           allocation_size);
    ...
    *out_buffer = buffer;
    ...
    if (!buffer && (device_ptr || host_ptr)) {
      iree_hal_cuda_tile_buffer_free(allocator->symbols, buffer_type, device_ptr,
                                host_ptr);
    } else {
...
static void iree_hal_cuda_tile_allocator_deallocate_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
  ...
  const iree_hal_cuda_tile_buffer_type_t buffer_type =
      iree_hal_cuda_tile_buffer_type(base_buffer);
  ...
  iree_hal_cuda_tile_buffer_free(allocator->symbols, buffer_type,
                            iree_hal_cuda_tile_buffer_device_pointer(base_buffer),
                            iree_hal_cuda_tile_buffer_host_pointer(base_buffer));

  switch (buffer_type) {
    case IREE_HAL_CUDA_TILE_BUFFER_TYPE_DEVICE:
    case IREE_HAL_CUDA_TILE_BUFFER_TYPE_HOST: {
      IREE_TRACE_FREE_NAMED(
          IREE_HAL_CUDA_TILE_ALLOCATOR_ID,
          (void*)iree_hal_cuda_tile_buffer_device_pointer(base_buffer));
...
static void iree_hal_cuda_tile_buffer_release_callback(void* user_data,
                                                  iree_hal_buffer_t* buffer) {
  iree_hal_cuda_tile_allocator_t* allocator = (iree_hal_cuda_tile_allocator_t*)user_data;

  const iree_hal_cuda_tile_buffer_type_t buffer_type =
      iree_hal_cuda_tile_buffer_type(buffer);

  iree_hal_cuda_tile_buffer_free(allocator->symbols, buffer_type,
                            iree_hal_cuda_tile_buffer_device_pointer(buffer),
                            iree_hal_cuda_tile_buffer_host_pointer(buffer));

  switch (buffer_type) {
    case IREE_HAL_CUDA_TILE_BUFFER_TYPE_DEVICE:
    case IREE_HAL_CUDA_TILE_BUFFER_TYPE_HOST: {
      IREE_TRACE_FREE_NAMED(IREE_HAL_CUDA_TILE_ALLOCATOR_ID,
                            (void*)iree_hal_cuda_tile_buffer_device_pointer(buffer));
      IREE_STATISTICS(iree_hal_allocator_statistics_record_free(
...
static iree_status_t iree_hal_cuda_tile_allocator_import_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
    const iree_hal_buffer_params_t* IREE_RESTRICT params,
  ...
  switch (external_buffer->type) {
    case IREE_HAL_EXTERNAL_BUFFER_TYPE_HOST_ALLOCATION: {
      ...
      buffer_type = IREE_HAL_CUDA_TILE_BUFFER_TYPE_HOST_REGISTERED;
    ...
    case IREE_HAL_EXTERNAL_BUFFER_TYPE_DEVICE_ALLOCATION: {
      buffer_type = IREE_HAL_CUDA_TILE_BUFFER_TYPE_EXTERNAL;
...
static iree_status_t iree_hal_cuda_tile_allocator_export_buffer(
    iree_hal_allocator_t* IREE_RESTRICT base_allocator,
    iree_hal_buffer_t* IREE_RESTRICT buffer,
    iree_hal_external_buffer_type_t requested_type,
    iree_hal_external_buffer_flags_t requested_flags,
    iree_hal_external_buffer_t* IREE_RESTRICT out_external_buffer) {
  iree_hal_cuda_tile_buffer_type_t buffer_type = iree_hal_cuda_tile_buffer_type(buffer);
...
static const iree_hal_allocator_vtable_t iree_hal_cuda_tile_allocator_vtable = {
    .destroy = iree_hal_cuda_tile_allocator_destroy,
    .host_allocator = iree_hal_cuda_tile_allocator_host_allocator,
    .trim = iree_hal_cuda_tile_allocator_trim,
    .query_statistics = iree_hal_cuda_tile_allocator_query_statistics,
    .query_memory_heaps = iree_hal_cuda_tile_allocator_query_memory_heaps,
    .query_buffer_compatibility =
        iree_hal_cuda_tile_allocator_query_buffer_compatibility,
    .allocate_buffer = iree_hal_cuda_tile_allocator_allocate_buffer,
    .deallocate_buffer = iree_hal_cuda_tile_allocator_deallocate_buffer,
    .import_buffer = iree_hal_cuda_tile_allocator_import_buffer,
    .export_buffer = iree_hal_cuda_tile_allocator_export_buffer,
};
