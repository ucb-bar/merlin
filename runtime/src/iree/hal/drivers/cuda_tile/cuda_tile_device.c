...
//
// CUDA device for cuda_tile HAL driver.
// Forked from upstream CUDA HAL with cuda_tile dispatch semantics
// (block_dims={1,1,1}, CTL1 format). Full feature set:
//   - CUDA graph and stream command buffers
//   - NCCL collective channels
//   - Async memory pools
...
#include "iree/hal/drivers/cuda_tile/cuda_tile_allocator.h"
#include "iree/hal/drivers/cuda_tile/cuda_tile_dynamic_symbols.h"
#include "iree/hal/drivers/cuda_tile/cuda_tile_event_pool.h"
#include "iree/hal/drivers/cuda_tile/cuda_tile_event_semaphore.h"
#include "iree/hal/drivers/cuda_tile/cuda_tile_graph_command_buffer.h"
#include "iree/hal/drivers/cuda_tile/cuda_tile_memory_pools.h"
...
#include "iree/hal/drivers/cuda_tile/cuda_tile_nop_executable_cache.h"
...
#include "iree/hal/drivers/cuda_tile/cuda_tile_timepoint_pool.h"
...
typedef struct iree_hal_cuda_tile_device_t {
  ...
  iree_hal_cuda_tile_device_params_t params;
  ...
  // CUDA uses a deferred work queue, so submission order is causal order.
  iree_async_axis_t axis;
  iree_atomic_int64_t epoch;

  // Device event pool, used for backing semaphore timepoints.
  iree_hal_cuda_tile_event_pool_t* device_event_pool;
  // Timepoint pools, shared by various semaphores.
  iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool;
  ...
  iree_hal_cuda_tile_memory_pools_t memory_pools;
...
static const iree_hal_device_vtable_t iree_hal_cuda_tile_device_vtable;
static const iree_hal_deferred_work_queue_device_interface_vtable_t
    iree_hal_cuda_tile_deferred_work_queue_device_interface_vtable;
...
typedef struct iree_hal_cuda_tile_deferred_work_queue_device_interface_t {
  ...
  iree_allocator_t host_allocator;
  const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols;
} iree_hal_cuda_tile_deferred_work_queue_device_interface_t;

static void iree_hal_cuda_tile_deferred_work_queue_device_interface_destroy(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
...
iree_hal_cuda_tile_deferred_work_queue_device_interface_bind_to_thread(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
          base_device_interface);
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_wait_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_create_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_record_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_native_event_t event) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_synchronize_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_native_event_t event) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_destroy_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_native_event_t event) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
...
static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_semaphore_acquire_timepoint_device_signal_native_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    struct iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_hal_deferred_work_queue_native_event_t* out_event) {
  return iree_hal_cuda_tile_event_semaphore_acquire_timepoint_device_signal(
      semaphore, value, (CUevent*)out_event);
}

static bool
iree_hal_cuda_tile_deferred_work_queue_device_interface_acquire_host_wait_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    struct iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_hal_deferred_work_queue_host_device_event_t* out_event) {
  return iree_hal_cuda_tile_semaphore_acquire_event_host_wait(
      semaphore, value, (iree_hal_cuda_tile_event_t**)out_event);
}

static void
iree_hal_cuda_tile_deferred_work_queue_device_interface_release_wait_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_host_device_event_t wait_event) {
  iree_hal_cuda_tile_event_release(wait_event);
}

static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_device_wait_on_host_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_host_device_event_t wait_event) {
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
  ...
  return IREE_CURESULT_TO_STATUS(
      ...
      cuStreamWaitEvent(
          ...
          iree_hal_cuda_tile_event_handle(
              (iree_hal_cuda_tile_event_t*)wait_event),
          0),
      "cuStreamWaitEvent");
}

static void*
iree_hal_cuda_tile_deferred_work_queue_device_interface_native_event_from_wait_event(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
    iree_hal_deferred_work_queue_host_device_event_t event) {
  return iree_hal_cuda_tile_event_handle((iree_hal_cuda_tile_event_t*)event);
}

static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_create_stream_command_buffer(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
  ...
  iree_hal_cuda_tile_deferred_work_queue_device_interface_t* device_interface =
      (iree_hal_cuda_tile_deferred_work_queue_device_interface_t*)(
          base_device_interface);
  return iree_hal_cuda_tile_device_create_stream_command_buffer(
      device_interface->device, mode, categories, 0, out);
}

static iree_status_t
iree_hal_cuda_tile_deferred_work_queue_device_interface_submit_command_buffer(
    iree_hal_deferred_work_queue_device_interface_t* base_device_interface,
  ...
  if (iree_hal_cuda_tile_stream_command_buffer_isa(command_buffer)) {
    // Stream command buffer — just notify submitted.
    iree_hal_cuda_tile_stream_notify_submitted_commands(command_buffer);
  } else {
    // Graph command buffer — launch the captured graph.
    CUgraphExec exec =
        iree_hal_cuda_tile_graph_command_buffer_handle(command_buffer);
    ...
    if (IREE_LIKELY(iree_status_is_ok(status))) {
      iree_hal_cuda_tile_graph_tracing_notify_submitted_commands(
          command_buffer);
    }
  }
  return status;
...
//===----------------------------------------------------------------------===//
// Tracing device interface
//===----------------------------------------------------------------------===//

typedef struct iree_hal_cuda_tile_tracing_device_interface_t {
  ...
  iree_allocator_t host_allocator;
  const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols;
} iree_hal_cuda_tile_tracing_device_interface_t;
static const iree_hal_stream_tracing_device_interface_vtable_t
    iree_hal_cuda_tile_tracing_device_interface_vtable_t;

void iree_hal_cuda_tile_tracing_device_interface_destroy(
    iree_hal_stream_tracing_device_interface_t* base_device_interface) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
  iree_allocator_free(device_interface->host_allocator, device_interface);
}

iree_status_t
iree_hal_cuda_tile_tracing_device_interface_synchronize_native_event(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
...
iree_status_t
iree_hal_cuda_tile_tracing_device_interface_create_native_event(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
    iree_hal_stream_tracing_native_event_t* base_event) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
...
iree_status_t
iree_hal_cuda_tile_tracing_device_interface_query_native_event(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
    iree_hal_stream_tracing_native_event_t base_event) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
...
void iree_hal_cuda_tile_tracing_device_interface_event_elapsed_time(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
    float* relative_millis, iree_hal_stream_tracing_native_event_t start_event,
    iree_hal_stream_tracing_native_event_t end_event) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
  IREE_CUDA_TILE_IGNORE_ERROR(
      device_interface->cuda_symbols,
      cuEventElapsedTime(relative_millis, (CUevent)start_event,
                         (CUevent)end_event));
}

void iree_hal_cuda_tile_tracing_device_interface_destroy_native_event(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
    iree_hal_stream_tracing_native_event_t base_event) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
  IREE_CUDA_TILE_IGNORE_ERROR(device_interface->cuda_symbols,
                               cuEventDestroy((CUevent)base_event));
}

iree_status_t
iree_hal_cuda_tile_tracing_device_interface_record_native_event(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
    iree_hal_stream_tracing_native_event_t base_event) {
  iree_hal_cuda_tile_tracing_device_interface_t* device_interface =
      (iree_hal_cuda_tile_tracing_device_interface_t*)base_device_interface;
  return IREE_CURESULT_TO_STATUS(
      device_interface->cuda_symbols,
      cuEventRecord((CUevent)base_event,
                    device_interface->dispatch_cu_stream));
}

iree_status_t
iree_hal_cuda_tile_tracing_device_interface_add_graph_event_record_node(
    iree_hal_stream_tracing_device_interface_t* base_device_interface,
...
static iree_hal_cuda_tile_device_t* iree_hal_cuda_tile_device_cast(
    iree_hal_device_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_device_vtable);
  return (iree_hal_cuda_tile_device_t*)base_value;
}

static iree_hal_cuda_tile_device_t* iree_hal_cuda_tile_device_cast_unsafe(
    iree_hal_device_t* base_value) {
  return (iree_hal_cuda_tile_device_t*)base_value;
}

// Advances the frontier tracker epoch for the device. The CUDA work queue is
// FIFO ordered, so enqueue order is the causal ordering we expose.
static void iree_hal_cuda_tile_device_advance_frontier(
    iree_hal_cuda_tile_device_t* device) {
...
static iree_status_t iree_hal_cuda_tile_device_create_internal(
    iree_hal_driver_t* driver, iree_string_view_t identifier,
    const iree_hal_cuda_tile_device_params_t* params, CUdevice cu_device,
    CUstream dispatch_stream, CUcontext context,
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols,
    const iree_hal_cuda_tile_nccl_dynamic_symbols_t* nccl_symbols,
    iree_allocator_t host_allocator, iree_hal_device_t** out_device) {
  iree_hal_cuda_tile_device_t* device = NULL;
  iree_host_size_t total_size = iree_sizeof_struct(*device) + identifier.size;
  IREE_RETURN_IF_ERROR(
      iree_allocator_malloc(host_allocator, total_size, (void**)&device));

  iree_hal_resource_initialize(&iree_hal_cuda_tile_device_vtable,
                               &device->resource);
  ...
  iree_status_t status = iree_allocator_malloc(
      host_allocator,
      sizeof(iree_hal_cuda_tile_deferred_work_queue_device_interface_t),
  ...
  // Enable tracing for the stream — no-op if disabled.
  if (iree_status_is_ok(status) && device->params.stream_tracing) {
    ...
    iree_hal_cuda_tile_tracing_device_interface_t* tracing_device_interface =
        NULL;
    status = iree_allocator_malloc(
        host_allocator,
        sizeof(iree_hal_cuda_tile_tracing_device_interface_t),
...
iree_status_t iree_hal_cuda_tile_device_create(
  ...
  iree_status_t status = iree_hal_cuda_tile_device_check_params(params);
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_device_create_internal(
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_cuda_tile_device_t* cuda_device =
        iree_hal_cuda_tile_device_cast(*out_device);
  ...
  iree_hal_cuda_tile_event_pool_t* device_event_pool = NULL;
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_event_pool_allocate(
        cuda_symbols, params->event_pool_capacity, host_allocator,
        &device_event_pool);
  }

  iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool = NULL;
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_timepoint_pool_allocate(
  ...
  if (iree_status_is_ok(status)) {
    ...
    if (timepoint_pool) iree_hal_cuda_tile_timepoint_pool_free(timepoint_pool);
    if (device_event_pool)
      iree_hal_cuda_tile_event_pool_release(device_event_pool);
...
CUcontext iree_hal_cuda_tile_device_context(iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast_unsafe(base_device);
  return device->cu_context;
}

const iree_hal_cuda_tile_dynamic_symbols_t*
iree_hal_cuda_tile_device_dynamic_symbols(iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast_unsafe(base_device);
  return device->cuda_symbols;
}

static void iree_hal_cuda_tile_device_destroy(
    iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  iree_allocator_t host_allocator =
      iree_hal_device_host_allocator(base_device);
  const iree_hal_cuda_tile_dynamic_symbols_t* symbols = device->cuda_symbols;
  IREE_TRACE_ZONE_BEGIN(z0);
  ...
  iree_hal_cuda_tile_memory_pools_deinitialize(&device->memory_pools);
  ...
  if (device->timepoint_pool) {
    iree_hal_cuda_tile_timepoint_pool_free(device->timepoint_pool);
  }
  if (device->device_event_pool) {
    iree_hal_cuda_tile_event_pool_release(device->device_event_pool);
  }

  IREE_CUDA_TILE_IGNORE_ERROR(symbols,
                               cuStreamDestroy(device->dispatch_cu_stream));
  IREE_CUDA_TILE_IGNORE_ERROR(symbols,
...
static iree_string_view_t iree_hal_cuda_tile_device_id(
    iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
...
static iree_allocator_t iree_hal_cuda_tile_device_host_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
...
static iree_hal_allocator_t* iree_hal_cuda_tile_device_allocator(
    iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
...
static void iree_hal_cuda_tile_replace_device_allocator(
...
static void iree_hal_cuda_tile_replace_channel_provider(
...
static iree_status_t iree_hal_cuda_tile_device_trim(
    iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  ...
  if (device->supports_memory_pools) {
    IREE_RETURN_IF_ERROR(iree_hal_cuda_tile_memory_pools_trim(
        &device->memory_pools, &device->params.memory_pools));
  }
  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_device_query_attribute(
    iree_hal_cuda_tile_device_t* device, CUdevice_attribute attribute,
    int64_t* out_value) {
  int value = 0;
  IREE_CUDA_TILE_RETURN_IF_ERROR(
      device->cuda_symbols,
...
static iree_status_t iree_hal_cuda_tile_device_query_i64(
...
static iree_status_t iree_hal_cuda_tile_device_query_capabilities(
    iree_hal_device_t* base_device,
...
iree_hal_cuda_tile_device_topology_info(iree_hal_device_t* base_device) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  return &device->topology_info;
}

static iree_status_t iree_hal_cuda_tile_device_refine_topology_edge(
    iree_hal_device_t* src_device, iree_hal_device_t* dst_device,
    iree_hal_topology_edge_t* edge) {
  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_device_assign_topology_info(
    iree_hal_device_t* base_device,
...
//===----------------------------------------------------------------------===//
// Command buffer / executable cache creation
//===----------------------------------------------------------------------===//

static iree_status_t iree_hal_cuda_tile_device_create_channel(
  ...
  if (!device->nccl_symbols || !device->nccl_symbols->dylib) {
    return iree_make_status(
        ...
        "NCCL runtime library not available; "
        "ensure installed and the shared library (libnccl.so) "
  ...
  iree_hal_cuda_tile_nccl_id_t id;
  memset(&id, 0, sizeof(id));
  if (iree_const_byte_span_is_empty(params.id)) {
    if (!device->channel_provider) {
      return iree_make_status(
          IREE_STATUS_INVALID_ARGUMENT,
          "default collective channel ID requested but no channel provider "
          "has been set on the device to provide it");
    }
    if (params.rank == 0) {
      IREE_RETURN_IF_ERROR(
          iree_hal_cuda_tile_nccl_get_unique_id(device->nccl_symbols, &id),
    ...
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "NCCL ID must be %zu bytes but caller provided "
                            "%zu bytes",
  ...
  if (iree_hal_cuda_tile_nccl_id_is_empty(&id)) {
    return iree_make_status(IREE_STATUS_INVALID_ARGUMENT,
                            "no default NCCL ID specified (all zeros)");
  }

  return iree_hal_cuda_tile_nccl_channel_create(
...
iree_status_t iree_hal_cuda_tile_device_create_stream_command_buffer(
  ...
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  return iree_hal_cuda_tile_stream_command_buffer_create(
      iree_hal_device_allocator(base_device), device->cuda_symbols,
      device->nccl_symbols, device->tracing_context, mode,
      command_categories, binding_capacity, device->dispatch_cu_stream,
      &device->block_pool, device->host_allocator, out_command_buffer);
}

static iree_status_t iree_hal_cuda_tile_device_create_command_buffer(
    iree_hal_device_t* base_device, iree_hal_command_buffer_mode_t mode,
    iree_hal_command_category_t command_categories,
  ...
  switch (device->params.command_buffer_mode) {
    case IREE_HAL_CUDA_TILE_COMMAND_BUFFER_MODE_GRAPH: {
      if (binding_capacity > 0) {
        ...
        return iree_hal_cuda_tile_graph_command_buffer_create(
            iree_hal_device_allocator(base_device), device->cuda_symbols,
    ...
    case IREE_HAL_CUDA_TILE_COMMAND_BUFFER_MODE_STREAM: {
      return iree_hal_deferred_command_buffer_create(
          iree_hal_device_allocator(base_device), mode, command_categories,
          queue_affinity, binding_capacity, &device->block_pool,
          iree_hal_device_host_allocator(base_device), out_command_buffer);
...
static iree_status_t iree_hal_cuda_tile_device_create_event(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
...
static iree_status_t iree_hal_cuda_tile_device_create_executable_cache(
    iree_hal_device_t* base_device, iree_string_view_t identifier,
    iree_hal_executable_cache_t** out_executable_cache) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  return iree_hal_cuda_tile_nop_executable_cache_create(
      identifier, device->cuda_symbols, device->cu_device,
      device->host_allocator, out_executable_cache);
}

static iree_status_t iree_hal_cuda_tile_device_import_file(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
...
static iree_status_t iree_hal_cuda_tile_device_create_semaphore(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    uint64_t initial_value, iree_hal_semaphore_flags_t flags,
    iree_hal_semaphore_t** out_semaphore) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  return iree_hal_cuda_tile_event_semaphore_create(
...
iree_hal_cuda_tile_device_query_semaphore_compatibility(
...
static iree_status_t iree_hal_cuda_tile_device_queue_alloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
  ...
  if (device->supports_memory_pools &&
    ...
    status = iree_hal_cuda_tile_memory_pools_alloca(
  ...
  if (iree_status_is_ok(status)) {
    iree_hal_cuda_tile_device_advance_frontier(device);
  } else {
...
static iree_status_t iree_hal_cuda_tile_device_queue_dealloca(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
  ...
  iree_status_t status = iree_ok_status();
  if (device->supports_memory_pools) {
    status = iree_hal_cuda_tile_memory_pools_dealloca(
...
static iree_status_t iree_hal_cuda_tile_device_queue_read(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
...
static iree_status_t iree_hal_cuda_tile_device_queue_write(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
...
static void iree_hal_cuda_tile_device_collect_tracing_context(
    void* user_data) {
  iree_hal_stream_tracing_context_collect(
      (iree_hal_stream_tracing_context_t*)user_data);
}

static iree_status_t iree_hal_cuda_tile_device_queue_execute(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity,
    const iree_hal_semaphore_list_t wait_semaphore_list,
    const iree_hal_semaphore_list_t signal_semaphore_list,
    iree_hal_command_buffer_t* command_buffer,
  ...
  iree_status_t status = iree_hal_deferred_work_queue_enqueue(
      device->work_queue, iree_hal_cuda_tile_device_collect_tracing_context,
...
static iree_status_t iree_hal_cuda_tile_device_queue_flush(
    iree_hal_device_t* base_device, iree_hal_queue_affinity_t queue_affinity) {
  iree_hal_cuda_tile_device_t* device =
      iree_hal_cuda_tile_device_cast(base_device);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_hal_cuda_tile_device_profiling_begin(
    iree_hal_device_t* base_device,
    const iree_hal_device_profiling_options_t* options) {
  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_device_profiling_flush(
    iree_hal_device_t* base_device) {
  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_device_profiling_end(
    iree_hal_device_t* base_device) {
  return iree_ok_status();
}
...
static const iree_hal_device_vtable_t iree_hal_cuda_tile_device_vtable = {
    .destroy = iree_hal_cuda_tile_device_destroy,
    .id = iree_hal_cuda_tile_device_id,
    .host_allocator = iree_hal_cuda_tile_device_host_allocator,
    .device_allocator = iree_hal_cuda_tile_device_allocator,
    .replace_device_allocator = iree_hal_cuda_tile_replace_device_allocator,
    .replace_channel_provider = iree_hal_cuda_tile_replace_channel_provider,
    .trim = iree_hal_cuda_tile_device_trim,
    .query_i64 = iree_hal_cuda_tile_device_query_i64,
    .query_capabilities = iree_hal_cuda_tile_device_query_capabilities,
    .topology_info = iree_hal_cuda_tile_device_topology_info,
    .refine_topology_edge = iree_hal_cuda_tile_device_refine_topology_edge,
    .assign_topology_info = iree_hal_cuda_tile_device_assign_topology_info,
    .create_channel = iree_hal_cuda_tile_device_create_channel,
    .create_command_buffer = iree_hal_cuda_tile_device_create_command_buffer,
    .create_event = iree_hal_cuda_tile_device_create_event,
    .create_executable_cache =
        iree_hal_cuda_tile_device_create_executable_cache,
    .import_file = iree_hal_cuda_tile_device_import_file,
    .create_semaphore = iree_hal_cuda_tile_device_create_semaphore,
    .query_semaphore_compatibility =
        iree_hal_cuda_tile_device_query_semaphore_compatibility,
    .queue_alloca = iree_hal_cuda_tile_device_queue_alloca,
    .queue_dealloca = iree_hal_cuda_tile_device_queue_dealloca,
    .queue_fill = iree_hal_device_queue_emulated_fill,
    .queue_update = iree_hal_device_queue_emulated_update,
    .queue_copy = iree_hal_device_queue_emulated_copy,
    .queue_read = iree_hal_cuda_tile_device_queue_read,
    .queue_write = iree_hal_cuda_tile_device_queue_write,
    .queue_host_call = iree_hal_device_queue_emulated_host_call,
    .queue_dispatch = iree_hal_device_queue_emulated_dispatch,
    .queue_execute = iree_hal_cuda_tile_device_queue_execute,
    .queue_flush = iree_hal_cuda_tile_device_queue_flush,
    .profiling_begin = iree_hal_cuda_tile_device_profiling_begin,
    .profiling_flush = iree_hal_cuda_tile_device_profiling_flush,
    .profiling_end = iree_hal_cuda_tile_device_profiling_end,
};

static const iree_hal_deferred_work_queue_device_interface_vtable_t
    iree_hal_cuda_tile_deferred_work_queue_device_interface_vtable = {
        .destroy =
            iree_hal_cuda_tile_deferred_work_queue_device_interface_destroy,
        .bind_to_thread =
            iree_hal_cuda_tile_deferred_work_queue_device_interface_bind_to_thread,
        .wait_native_event =
            iree_hal_cuda_tile_deferred_work_queue_device_interface_wait_native_event,
...
static const iree_hal_stream_tracing_device_interface_vtable_t
    iree_hal_cuda_tile_tracing_device_interface_vtable_t = {
        .destroy = iree_hal_cuda_tile_tracing_device_interface_destroy,
        .synchronize_native_event =
            iree_hal_cuda_tile_tracing_device_interface_synchronize_native_event,
        .create_native_event =
            iree_hal_cuda_tile_tracing_device_interface_create_native_event,
        .query_native_event =
            iree_hal_cuda_tile_tracing_device_interface_query_native_event,
        .event_elapsed_time =
            iree_hal_cuda_tile_tracing_device_interface_event_elapsed_time,
        .destroy_native_event =
            iree_hal_cuda_tile_tracing_device_interface_destroy_native_event,
        .record_native_event =
            iree_hal_cuda_tile_tracing_device_interface_record_native_event,
