...
#define IREE_HAL_CUDA_TILE_MAX_DEVICE_NAME_LENGTH 128
...
typedef struct iree_hal_cuda_tile_driver_t {
...
static const iree_hal_driver_vtable_t iree_hal_cuda_tile_driver_vtable;

static iree_hal_cuda_tile_driver_t* iree_hal_cuda_tile_driver_cast(
    iree_hal_driver_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_driver_vtable);
  return (iree_hal_cuda_tile_driver_t*)base_value;
}
...
static iree_status_t iree_hal_cuda_tile_driver_create_internal(
    iree_string_view_t identifier,
    const iree_hal_cuda_tile_driver_options_t* options,
    const iree_hal_cuda_tile_device_params_t* device_params,
    iree_allocator_t host_allocator, iree_hal_driver_t** out_driver) {
  iree_hal_cuda_tile_driver_t* driver = NULL;
  iree_host_size_t total_size = iree_sizeof_struct(*driver) + identifier.size;
  IREE_RETURN_IF_ERROR(
      iree_allocator_malloc(host_allocator, total_size, (void**)&driver));

  iree_hal_resource_initialize(&iree_hal_cuda_tile_driver_vtable, &driver->resource);
...
IREE_API_EXPORT iree_status_t iree_hal_cuda_tile_driver_create(
  ...
  iree_status_t status = iree_hal_cuda_tile_driver_create_internal(
...
static void iree_hal_cuda_tile_driver_destroy(iree_hal_driver_t* base_driver) {
  IREE_ASSERT_ARGUMENT(base_driver);

  iree_hal_cuda_tile_driver_t* driver = iree_hal_cuda_tile_driver_cast(base_driver);
...
static iree_status_t iree_hal_cuda_tile_init(iree_hal_cuda_tile_driver_t* driver) {
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_hal_cuda_tile_populate_device_info(
    CUdevice device, iree_hal_cuda_tile_dynamic_symbols_t* syms, uint8_t* buffer_ptr,
    uint8_t** out_buffer_ptr, iree_hal_device_info_t* out_device_info) {
  *out_buffer_ptr = buffer_ptr;

  char device_name[IREE_HAL_CUDA_TILE_MAX_DEVICE_NAME_LENGTH];
...
static bool iree_hal_cuda_tile_is_valid_device(iree_hal_cuda_tile_driver_t* driver,
                                          CUdevice device) {
  return true;
}

static iree_status_t iree_hal_cuda_tile_driver_query_available_devices(
  ...
  IREE_RETURN_AND_END_ZONE_IF_ERROR(z0, iree_hal_cuda_tile_init(driver));
  ...
  iree_host_size_t total_size =
      device_count * (sizeof(iree_hal_device_info_t) +
                      IREE_HAL_CUDA_TILE_MAX_DEVICE_NAME_LENGTH * sizeof(char));
  iree_status_t status =
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t i = 0; i < device_count; ++i) {
      ...
      if (!iree_hal_cuda_tile_is_valid_device(driver, device)) continue;
      status = iree_hal_cuda_tile_populate_device_info(
...
static iree_status_t iree_hal_cuda_tile_driver_dump_device_info(
...
static iree_status_t iree_hal_cuda_tile_driver_select_default_device(
    iree_hal_driver_t* base_driver, iree_hal_cuda_tile_dynamic_symbols_t* syms,
    int default_device_index, iree_allocator_t host_allocator,
    CUdevice* out_device) {
  iree_hal_device_info_t* device_infos = NULL;
  iree_host_size_t device_count = 0;
  IREE_RETURN_IF_ERROR(iree_hal_cuda_tile_driver_query_available_devices(
...
static iree_status_t iree_hal_cuda_tile_driver_create_device_by_id(
    iree_hal_driver_t* base_driver, iree_hal_device_id_t device_id,
  ...
  if (device_id == IREE_HAL_DEVICE_ID_DEFAULT) {
    IREE_RETURN_AND_END_ZONE_IF_ERROR(
        z0, iree_hal_cuda_tile_driver_select_default_device(
  ...
  iree_string_view_t device_name = iree_make_cstring_view("cuda_tile");
...
static iree_status_t iree_hal_cuda_tile_driver_create_device_by_uuid(
  ...
  // Ensure CUDA is initialized before querying it.
  IREE_RETURN_IF_ERROR(iree_hal_cuda_tile_init(driver));
  ...
  iree_status_t status = iree_hal_cuda_tile_driver_create_device_by_id(
...
static iree_status_t iree_hal_cuda_tile_driver_create_device_by_index(
    iree_hal_driver_t* base_driver, iree_string_view_t driver_name,
...
static iree_status_t iree_hal_cuda_tile_driver_create_device_by_path(
    iree_hal_driver_t* base_driver, iree_string_view_t driver_name,
  ...
  if (iree_string_view_is_empty(device_path)) {
    return iree_hal_cuda_tile_driver_create_device_by_id(
  ...
  if (iree_string_view_consume_prefix(&device_path, IREE_SV("GPU-"))) {
    ...
    return iree_hal_cuda_tile_driver_create_device_by_uuid(
  ...
  if (iree_string_view_atoi_int32(device_path, &device_index)) {
    return iree_hal_cuda_tile_driver_create_device_by_index(
...
static const iree_hal_driver_vtable_t iree_hal_cuda_tile_driver_vtable = {
    .destroy = iree_hal_cuda_tile_driver_destroy,
    .query_available_devices = iree_hal_cuda_tile_driver_query_available_devices,
    .dump_device_info = iree_hal_cuda_tile_driver_dump_device_info,
    .create_device_by_id = iree_hal_cuda_tile_driver_create_device_by_id,
    .create_device_by_path = iree_hal_cuda_tile_driver_create_device_by_path,
};
