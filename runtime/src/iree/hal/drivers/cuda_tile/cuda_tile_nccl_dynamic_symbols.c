...
static const char* iree_hal_cuda_tile_nccl_dylib_names[] = {
...
static iree_status_t iree_hal_cuda_tile_nccl_dynamic_symbols_resolve_all(
    iree_hal_cuda_tile_nccl_dynamic_symbols_t* syms) {
...
#include "iree/hal/drivers/cuda_tile/cuda_tile_nccl_dynamic_symbol_table.h"  // IWYU pragma: keep
#undef IREE_NCCL_PFN_DECL
#undef IREE_NCCL_PFN_DECL_STR_RETURN
  return iree_ok_status();
}

static iree_status_t iree_hal_cuda_tile_nccl_check_version(
...
iree_status_t iree_hal_cuda_tile_nccl_dynamic_symbols_initialize(
    iree_allocator_t host_allocator,
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_library,
    iree_hal_cuda_tile_nccl_dynamic_symbols_t* out_syms) {
  ...
  iree_status_t status = iree_dynamic_library_load_from_files(
      IREE_ARRAYSIZE(iree_hal_cuda_tile_nccl_dylib_names),
      iree_hal_cuda_tile_nccl_dylib_names, IREE_DYNAMIC_LIBRARY_FLAG_NONE,
  ...
  if (iree_status_is_ok(status)) {
    ...
    // time of resolving.
    status = iree_hal_cuda_tile_nccl_check_version(out_syms->dylib);
  }

  // Resolve all symbols; this will fail if any required symbols are missing.
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_nccl_dynamic_symbols_resolve_all(out_syms);
  }
...
void iree_hal_cuda_tile_nccl_dynamic_symbols_deinitialize(
    iree_hal_cuda_tile_nccl_dynamic_symbols_t* syms) {
  IREE_TRACE_ZONE_BEGIN(z0);
