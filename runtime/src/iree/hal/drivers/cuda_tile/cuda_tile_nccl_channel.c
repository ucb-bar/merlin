...
typedef struct iree_hal_cuda_tile_nccl_channel_t {
...
} iree_hal_cuda_tile_nccl_channel_t;

static const iree_hal_channel_vtable_t iree_hal_cuda_tile_nccl_channel_vtable;

static iree_hal_cuda_tile_nccl_channel_t*
iree_hal_cuda_tile_nccl_channel_cast(iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_nccl_channel_vtable);
  return (iree_hal_cuda_tile_nccl_channel_t*)base_value;
}

static const iree_hal_cuda_tile_nccl_channel_t*
iree_hal_cuda_tile_nccl_channel_const_cast(
    const iree_hal_channel_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_nccl_channel_vtable);
  return (const iree_hal_cuda_tile_nccl_channel_t*)base_value;
}
...
static uint64_t iree_hal_cuda_tile_nccl_hash_id(
    const iree_hal_cuda_tile_nccl_id_t* id) {
...
iree_status_t iree_hal_cuda_tile_nccl_get_unique_id(
    const iree_hal_cuda_tile_nccl_dynamic_symbols_t* symbols,
    iree_hal_cuda_tile_nccl_id_t* out_id) {
...
iree_status_t iree_hal_cuda_tile_nccl_channel_create(
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols,
    const iree_hal_cuda_tile_nccl_dynamic_symbols_t* nccl_symbols,
    const iree_hal_cuda_tile_nccl_id_t* id, int rank, int count,
  ...
  *out_channel = NULL;
  IREE_TRACE(
      const uint64_t id_hash = iree_hal_cuda_tile_nccl_hash_id(id));
  ...
  iree_hal_cuda_tile_nccl_channel_t* channel = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*channel),
                                (void**)&channel));

  iree_hal_resource_initialize(&iree_hal_cuda_tile_nccl_channel_vtable,
                               &channel->resource);
...
static void iree_hal_cuda_tile_nccl_channel_destroy(
    iree_hal_channel_t* base_channel) {
  iree_hal_cuda_tile_nccl_channel_t* channel =
      iree_hal_cuda_tile_nccl_channel_cast(base_channel);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_hal_cuda_tile_nccl_channel_split(
  ...
  if (iree_status_is_ok(status)) {
    status = IREE_NCCL_RESULT_TO_STATUS(
        channel->nccl_symbols, ncclCommCount(split_comm, &split_count),
        "ncclCommCount");
  }

  // Wrap the split communicator in a new channel.
  iree_hal_cuda_tile_nccl_channel_t* split_channel = NULL;
  if (iree_status_is_ok(status)) {
    status =
...
static void iree_hal_cuda_tile_nccl_channel_query_rank_and_count(
    const iree_hal_channel_t* base_channel, int32_t* out_rank,
    int32_t* out_count) {
  IREE_ASSERT_ARGUMENT(base_channel);
  IREE_ASSERT_ARGUMENT(out_count);
  const iree_hal_cuda_tile_nccl_channel_t* channel =
      iree_hal_cuda_tile_nccl_channel_const_cast(base_channel);
...
static ncclComm_t iree_hal_cuda_tile_nccl_channel_comm(
    iree_hal_channel_t* base_channel) {
  IREE_ASSERT_ARGUMENT(base_channel);
  iree_hal_cuda_tile_nccl_channel_t* channel =
      iree_hal_cuda_tile_nccl_channel_cast(base_channel);
...
static iree_status_t iree_hal_cuda_tile_get_nccl_data_type(
...
static iree_status_t iree_hal_cuda_tile_get_nccl_reduction_type(
...
static iree_status_t iree_hal_cuda_tile_nccl_submit_batch_entry(
    const iree_hal_collective_batch_entry_t* entry, CUstream stream) {
  IREE_ASSERT_ARGUMENT(entry);
  IREE_ASSERT_ARGUMENT(stream);

  iree_hal_cuda_tile_nccl_channel_t* channel =
      iree_hal_cuda_tile_nccl_channel_cast(entry->channel);
  const iree_hal_cuda_tile_nccl_dynamic_symbols_t* symbols =
      channel->nccl_symbols;
  ncclComm_t comm = iree_hal_cuda_tile_nccl_channel_comm(entry->channel);

  ncclDataType_t datatype;
  IREE_RETURN_IF_ERROR(
      iree_hal_cuda_tile_get_nccl_data_type(entry->op.element_type, &datatype));

  switch (entry->op.kind) {
    case IREE_HAL_COLLECTIVE_KIND_ALL_GATHER: {
      CUdeviceptr sendbuff =
          iree_hal_cuda_tile_buffer_device_pointer(
    ...
    case IREE_HAL_COLLECTIVE_KIND_ALL_REDUCE: {
      ...
      CUdeviceptr recvbuff =
          ...
          entry->recv_binding.offset;
      ncclRedOp_t redop;
      IREE_RETURN_IF_ERROR(iree_hal_cuda_tile_get_nccl_reduction_type(
    ...
    case IREE_HAL_COLLECTIVE_KIND_ALL_TO_ALL: {
      ...
      // These calls are already grouped by
      // iree_hal_cuda_tile_nccl_submit_batch.
...
iree_status_t iree_hal_cuda_tile_nccl_submit_batch(
    const iree_hal_cuda_tile_nccl_dynamic_symbols_t* symbols,
  ...
  for (iree_host_size_t i = 0; i < batch->count; ++i) {
    IREE_RETURN_IF_ERROR(
        iree_hal_cuda_tile_nccl_submit_batch_entry(&batch->entries[i],
                                                   stream));
  }
...
static const iree_hal_channel_vtable_t
    iree_hal_cuda_tile_nccl_channel_vtable = {
        .destroy = iree_hal_cuda_tile_nccl_channel_destroy,
        .split = iree_hal_cuda_tile_nccl_channel_split,
        .query_rank_and_count =
            iree_hal_cuda_tile_nccl_channel_query_rank_and_count,
};
