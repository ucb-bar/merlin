...
typedef struct iree_hal_cuda_tile_semaphore_t {
...
} iree_hal_cuda_tile_semaphore_t;

static const iree_hal_semaphore_vtable_t iree_hal_cuda_tile_semaphore_vtable;

static iree_hal_cuda_tile_semaphore_t* iree_hal_cuda_tile_semaphore_cast(
    iree_hal_semaphore_t* base_value) {
  IREE_HAL_ASSERT_TYPE(base_value, &iree_hal_cuda_tile_semaphore_vtable);
  return (iree_hal_cuda_tile_semaphore_t*)base_value;
}

iree_status_t iree_hal_cuda_tile_event_semaphore_create(
    uint64_t initial_value, const iree_hal_cuda_tile_dynamic_symbols_t* symbols,
    iree_hal_cuda_tile_timepoint_pool_t* timepoint_pool,
  ...
  iree_hal_cuda_tile_semaphore_t* semaphore = NULL;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_allocator_malloc(host_allocator, sizeof(*semaphore),
                                (void**)&semaphore));

  iree_hal_semaphore_initialize(&iree_hal_cuda_tile_semaphore_vtable,
                                &semaphore->base);
...
static void iree_hal_cuda_tile_semaphore_destroy(
    iree_hal_semaphore_t* base_semaphore) {
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(base_semaphore);
...
static iree_status_t iree_hal_cuda_tile_semaphore_query(
...
static iree_status_t iree_hal_cuda_tile_semaphore_signal(
...
static void iree_hal_cuda_tile_semaphore_fail(iree_hal_semaphore_t* base_semaphore,
                                         iree_status_t status) {
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(base_semaphore);
  IREE_TRACE_ZONE_BEGIN(z0);
...
static iree_status_t iree_hal_cuda_tile_semaphore_timepoint_host_wait_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_timepoint_t* timepoint = (iree_hal_cuda_tile_timepoint_t*)user_data;
...
static iree_status_t iree_hal_cuda_tile_semaphore_acquire_timepoint_host_wait(
    iree_hal_cuda_tile_semaphore_t* semaphore, uint64_t min_value,
    iree_timeout_t timeout, iree_hal_cuda_tile_timepoint_t** out_timepoint) {
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_host_wait(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_cuda_tile_semaphore_timepoint_host_wait_callback,
...
bool iree_hal_cuda_tile_semaphore_acquire_event_host_wait(
    iree_hal_semaphore_t* base_semaphore, uint64_t min_value,
    iree_hal_cuda_tile_event_t** out_event) {
  *out_event = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(base_semaphore);
  ...
  for (iree_hal_semaphore_timepoint_t* tp = semaphore->base.timepoint_list.head;
       tp != NULL; tp = tp->next) {
    iree_hal_cuda_tile_timepoint_t* signal_timepoint =
        (iree_hal_cuda_tile_timepoint_t*)tp;
    if (signal_timepoint->kind == IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_SIGNAL &&
        signal_timepoint->base.minimum_value >= min_value) {
      *out_event = signal_timepoint->timepoint.device_signal;
      iree_hal_cuda_tile_event_retain(*out_event);
      break;
    }
  }
...
static iree_status_t iree_hal_cuda_tile_semaphore_try_wait_or_acquire_wait_timepoint(
  ...
  *out_timepoint = NULL;
  ...
  iree_status_t status = iree_hal_cuda_tile_semaphore_acquire_timepoint_host_wait(
...
static iree_status_t iree_hal_cuda_tile_semaphore_wait(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
  ...
  iree_hal_cuda_tile_timepoint_t* timepoint;
  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_semaphore_try_wait_or_acquire_wait_timepoint(
  ...
  iree_hal_cuda_tile_timepoint_pool_release(semaphore->timepoint_pool, 1,
                                       &timepoint);
  if (!iree_status_is_ok(status)) {
    IREE_TRACE_ZONE_END(z0);
    return status;
...
iree_status_t iree_hal_cuda_tile_semaphore_multi_wait(
  ...
  iree_hal_cuda_tile_timepoint_t** timepoints = NULL;
  ...
  if (iree_status_is_ok(status)) {
    ...
    for (iree_host_size_t i = 0; i < semaphore_list.count && needs_wait; ++i) {
      iree_hal_cuda_tile_timepoint_t* timepoint;
      status = iree_hal_cuda_tile_semaphore_try_wait_or_acquire_wait_timepoint(
  ...
  for (iree_host_size_t i = 0; i < timepoint_count; ++i) {
    iree_hal_cuda_tile_timepoint_t* timepoint = timepoints[i];
    ...
    iree_hal_cuda_tile_timepoint_pool_release(timepoint->pool, 1, &timepoint);
  }
  ...
  for (iree_host_size_t i = 0; i < semaphore_list.count; ++i) {
    iree_hal_cuda_tile_semaphore_t* semaphore =
        iree_hal_cuda_tile_semaphore_cast(semaphore_list.semaphores[i]);
    iree_slim_mutex_lock(&semaphore->mutex);
    if (semaphore->current_value >= IREE_HAL_SEMAPHORE_FAILURE_VALUE) {
      iree_slim_mutex_unlock(&semaphore->mutex);
      status = iree_make_status(IREE_STATUS_ABORTED);
...
static iree_status_t iree_hal_cuda_tile_semaphore_timepoint_device_signal_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_timepoint_t* timepoint = (iree_hal_cuda_tile_timepoint_t*)user_data;
...
// device.
iree_status_t iree_hal_cuda_tile_event_semaphore_acquire_timepoint_device_signal(
    iree_hal_semaphore_t* base_semaphore, uint64_t to_value,
    CUevent* out_event) {
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(base_semaphore);
  iree_hal_cuda_tile_timepoint_t* signal_timepoint = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_device_signal(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_cuda_tile_semaphore_timepoint_device_signal_callback,
          .user_data = signal_timepoint,
      },
      &signal_timepoint->base);
  iree_hal_cuda_tile_event_t* event = signal_timepoint->timepoint.device_signal;
  ...
  for (iree_hal_semaphore_timepoint_t* tp = semaphore->base.timepoint_list.head;
       tp != NULL; tp = tp->next) {
    iree_hal_cuda_tile_timepoint_t* wait_timepoint = (iree_hal_cuda_tile_timepoint_t*)tp;
    if (wait_timepoint->kind == IREE_HAL_CUDA_TILE_TIMEPOINT_KIND_DEVICE_WAIT &&
        wait_timepoint->timepoint.device_wait == NULL &&
        wait_timepoint->base.minimum_value <= to_value) {
      iree_hal_cuda_tile_event_retain(event);
      wait_timepoint->timepoint.device_wait = event;
    }
  }
  iree_slim_mutex_unlock(&semaphore->base.timepoint_mutex);

  *out_event = iree_hal_cuda_tile_event_handle(event);
  IREE_TRACE_ZONE_END(z0);
  return iree_ok_status();
}
...
//
// Note that this callback is invoked by the a host thread.
static iree_status_t iree_hal_cuda_tile_semaphore_timepoint_device_wait_callback(
    void* user_data, iree_hal_semaphore_t* semaphore, uint64_t value,
    iree_status_code_t status_code) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_timepoint_t* timepoint = (iree_hal_cuda_tile_timepoint_t*)user_data;
...
iree_status_t iree_hal_cuda_tile_event_semaphore_acquire_timepoint_device_wait(
    iree_hal_semaphore_t* base_semaphore, uint64_t min_value,
    CUevent* out_event) {
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(base_semaphore);
  iree_hal_cuda_tile_timepoint_t* wait_timepoint = NULL;
  IREE_TRACE_ZONE_BEGIN(z0);

  IREE_RETURN_AND_END_ZONE_IF_ERROR(
      z0, iree_hal_cuda_tile_timepoint_pool_acquire_device_wait(
  ...
  iree_hal_semaphore_acquire_timepoint(
      ...
      (iree_hal_semaphore_callback_t){
          .fn = iree_hal_cuda_tile_semaphore_timepoint_device_wait_callback,
          .user_data = wait_timepoint,
      },
      &wait_timepoint->base);

  iree_hal_cuda_tile_event_t* wait_event = NULL;
  if (iree_hal_cuda_tile_semaphore_acquire_event_host_wait(&semaphore->base,
...
static iree_status_t iree_hal_cuda_tile_semaphore_import_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
...
static iree_status_t iree_hal_cuda_tile_semaphore_export_timepoint(
    iree_hal_semaphore_t* base_semaphore, uint64_t value,
    iree_hal_queue_affinity_t queue_affinity,
...
static const iree_hal_semaphore_vtable_t iree_hal_cuda_tile_semaphore_vtable = {
    .destroy = iree_hal_cuda_tile_semaphore_destroy,
    .query = iree_hal_cuda_tile_semaphore_query,
    .signal = iree_hal_cuda_tile_semaphore_signal,
    .fail = iree_hal_cuda_tile_semaphore_fail,
    .wait = iree_hal_cuda_tile_semaphore_wait,
    .import_timepoint = iree_hal_cuda_tile_semaphore_import_timepoint,
    .export_timepoint = iree_hal_cuda_tile_semaphore_export_timepoint,
};
