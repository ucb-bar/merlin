...
iree_status_t iree_hal_cuda_tile_event_semaphore_create(
  ...
  iree_async_semaphore_initialize(
      (const iree_async_semaphore_vtable_t*)&iree_hal_cuda_tile_semaphore_vtable,
...
static void iree_hal_cuda_tile_semaphore_destroy(
  ...
  iree_hal_cuda_tile_semaphore_t* semaphore =
      iree_hal_cuda_tile_semaphore_cast(iree_hal_semaphore_cast(base_semaphore));
...
static uint64_t iree_hal_cuda_tile_semaphore_query(
    iree_async_semaphore_t* base_semaphore) {
...
static void iree_hal_cuda_tile_semaphore_on_fail(
...
static void iree_hal_cuda_tile_semaphore_timepoint_device_signal_callback(
    void* user_data, iree_async_semaphore_timepoint_t* async_timepoint,
    iree_status_t status) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_timepoint_t* timepoint =
      (iree_hal_cuda_tile_timepoint_t*)async_timepoint;
...
iree_status_t iree_hal_cuda_tile_event_semaphore_acquire_timepoint_device_signal(
  ...
  signal_timepoint->base.callback =
      iree_hal_cuda_tile_semaphore_timepoint_device_signal_callback;
  signal_timepoint->base.user_data = NULL;
  iree_async_semaphore_acquire_timepoint(&semaphore->async, to_value,
                                         &signal_timepoint->base);
  // signal_timepoint is now owned by the semaphore — do not access it.
...
// the target value. Releases the timepoint (and its CUDA event) back to the
// pool. Fires without the semaphore lock held.
static void iree_hal_cuda_tile_semaphore_timepoint_device_wait_callback(
    void* user_data, iree_async_semaphore_timepoint_t* async_timepoint,
    iree_status_t status) {
  IREE_TRACE_ZONE_BEGIN(z0);
  iree_hal_cuda_tile_timepoint_t* timepoint =
    ...
    .async =
        {
            ...
            .on_fail = iree_hal_cuda_tile_semaphore_on_fail,
        },
