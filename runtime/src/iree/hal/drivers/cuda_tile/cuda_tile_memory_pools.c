...
static const char* IREE_HAL_CUDA_TILE_DEVICE_LOCAL_POOL_RESERVED_ID =
    "CUDA pool: device-local reserved";
static const char* IREE_HAL_CUDA_TILE_OTHER_POOL_RESERVED_ID =
    "CUDA pool: other reserved";
#endif  // IREE_TRACING_FEATURE_ALLOCATION_TRACKING

static iree_status_t iree_hal_cuda_tile_create_memory_pool(
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols,
    CUdevice cu_device, iree_hal_cuda_tile_memory_pool_params_t params,
...
iree_status_t iree_hal_cuda_tile_memory_pools_initialize(
    iree_hal_device_t* parent_device,
    const iree_hal_cuda_tile_dynamic_symbols_t* cuda_symbols,
    CUdevice cu_device,
    const iree_hal_cuda_tile_memory_pooling_params_t* pooling_params,
    iree_allocator_t host_allocator,
    iree_hal_cuda_tile_memory_pools_t* IREE_RESTRICT out_pools) {
  ...
  if (iree_status_is_ok(status)) {
    status = iree_hal_cuda_tile_create_memory_pool(
        cuda_symbols, cu_device, pooling_params->device_local,
...
void iree_hal_cuda_tile_memory_pools_deinitialize(
...
static void iree_hal_cuda_tile_memory_pool_track_alloc(
    iree_hal_cuda_tile_memory_pools_t* pools, iree_hal_buffer_t* buffer) {
  ...
  IREE_TRACE_ALLOC_NAMED(
      is_device_local ? IREE_HAL_CUDA_TILE_DEVICE_LOCAL_POOL_RESERVED_ID
                      : IREE_HAL_CUDA_TILE_OTHER_POOL_RESERVED_ID,
      (void*)iree_hal_cuda_tile_buffer_device_pointer(buffer), allocation_size);
  IREE_STATISTICS({
...
static void iree_hal_cuda_tile_memory_pool_track_free(
    iree_hal_cuda_tile_memory_pools_t* pools, iree_hal_buffer_t* buffer) {
  bool is_device_local = iree_all_bits_set(iree_hal_buffer_memory_type(buffer),
                                           IREE_HAL_MEMORY_TYPE_DEVICE_LOCAL);
  (void)is_device_local;
...
void iree_hal_cuda_tile_memory_pools_merge_statistics(
    iree_hal_cuda_tile_memory_pools_t* pools,
...
iree_status_t iree_hal_cuda_tile_memory_pools_trim(
    iree_hal_cuda_tile_memory_pools_t* pools,
    const iree_hal_cuda_tile_memory_pooling_params_t* pooling_params) {
...
static void iree_hal_cuda_tile_async_buffer_release_callback(
    void* user_data, iree_hal_buffer_t* buffer) {
  iree_hal_cuda_tile_memory_pools_t* pools =
      (iree_hal_cuda_tile_memory_pools_t*)user_data;
  IREE_TRACE_ZONE_BEGIN(z0);

  CUdeviceptr device_ptr = iree_hal_cuda_tile_buffer_device_pointer(buffer);
  IREE_CUDA_IGNORE_ERROR(pools->cuda_symbols, cuMemFree(device_ptr));
  iree_hal_cuda_tile_memory_pool_track_free(pools, buffer);

  IREE_TRACE_ZONE_END(z0);
}

iree_status_t iree_hal_cuda_tile_memory_pools_alloca(
    iree_hal_cuda_tile_memory_pools_t* pools, CUstream stream,
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_buffer_release_callback_t release_callback = {
        .fn = iree_hal_cuda_tile_async_buffer_release_callback,
        .user_data = pools,
    };
    status = iree_hal_cuda_tile_buffer_wrap(
        placement, params.type, params.access, params.usage, allocation_size,
        /*byte_offset=*/0,
        /*byte_length=*/allocation_size, IREE_HAL_CUDA_TILE_BUFFER_TYPE_ASYNC,
  ...
  if (iree_status_is_ok(status)) {
    ...
    iree_hal_cuda_tile_memory_pool_track_alloc(pools, buffer);
    *out_buffer = buffer;
  } else if (buffer) {
...
iree_status_t iree_hal_cuda_tile_memory_pools_dealloca(
    iree_hal_cuda_tile_memory_pools_t* pools, CUstream stream,
  ...
  // it asynchronously.
  iree_status_t status = iree_ok_status();
  if (iree_hal_cuda_tile_buffer_type(buffer) ==
      IREE_HAL_CUDA_TILE_BUFFER_TYPE_ASYNC) {
    ...
    if (iree_status_is_ok(status)) {
      ...
      // is destroyed.
      iree_hal_cuda_tile_buffer_drop_release_callback(buffer);
