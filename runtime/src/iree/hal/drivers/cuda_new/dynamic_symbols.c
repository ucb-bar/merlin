...
static const char *iree_hal_cuda_new_dylib_names[] = {
...
#define IREE_CUDA_NEW_DRIVER_API_VERSION 11030

static iree_status_t iree_hal_cuda_new_dynamic_symbols_resolve_all(
	iree_hal_cuda_new_dynamic_symbols_t *syms) {
	IREE_RETURN_IF_ERROR(iree_dynamic_library_lookup_symbol(
		syms->dylib, "cuGetProcAddress",
		(void **)&syms->cuGetProcAddress));
#define IREE_CU_PFN_DECL(cuda_symbol_name, ...)                            \
	{                                                                       \
		static const char *name = #cuda_symbol_name;                        \
		IREE_CUDA_NEW_RETURN_IF_ERROR(                                      \
			syms,                                                           \
			cuGetProcAddress(name, (void **)&syms->cuda_symbol_name,        \
				IREE_CUDA_NEW_DRIVER_API_VERSION,                           \
				CU_GET_PROC_ADDRESS_DEFAULT),                               \
			"when resolving " #cuda_symbol_name " using cuGetProcAddress"); \
	}
#include "dynamic_symbol_table.h" // IWYU pragma: keep
...
iree_status_t iree_hal_cuda_new_dynamic_symbols_initialize(
	...
	iree_status_t status = iree_dynamic_library_load_from_files(
		IREE_ARRAYSIZE(iree_hal_cuda_new_dylib_names),
		iree_hal_cuda_new_dylib_names, IREE_DYNAMIC_LIBRARY_FLAG_NONE,
		host_allocator, &out_syms->dylib);
	if (iree_status_is_not_found(status)) {
		iree_status_ignore(status);
		status = iree_make_status(IREE_STATUS_UNAVAILABLE,
			"CUDA driver library 'libcuda.so'/'nvcuda.dll' not available; "
			"ensure installed and in dynamic library search path");
	}
	if (iree_status_is_ok(status)) {
		status = iree_hal_cuda_new_dynamic_symbols_resolve_all(out_syms);
	}
	if (!iree_status_is_ok(status)) {
		iree_hal_cuda_new_dynamic_symbols_deinitialize(out_syms);
	}
