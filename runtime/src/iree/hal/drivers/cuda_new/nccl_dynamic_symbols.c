...
static const char *iree_hal_cuda_new_nccl_dylib_names[] = {
...
// Resolves all NCCL dynamic symbols from the symbol table.
static iree_status_t iree_hal_cuda_new_nccl_dynamic_symbols_resolve_all(
...
#include "nccl_dynamic_symbol_table.h" // IWYU pragma: keep
#undef IREE_NCCL_PFN_DECL
#undef IREE_NCCL_PFN_DECL_STR_RETURN
	return iree_ok_status();
}

// Checks the NCCL version is compatible with what we were compiled against.
static iree_status_t iree_hal_cuda_new_nccl_check_version(
	...
	if (major != NCCL_MAJOR || nccl_version < required_minimum_version) {
		return iree_make_status(
			IREE_STATUS_UNAVAILABLE,
			"NCCL version is %d.%d.%d, but >=%d.%d and <%d is required",
			major, minor, patch, NCCL_MAJOR, NCCL_MINOR, NCCL_MAJOR + 1);
	}
...
iree_status_t iree_hal_cuda_new_nccl_dynamic_symbols_initialize(
	...
	if (!cuda_library->dylib) {
		return iree_make_status(
			...
			"CUDA dynamic symbols must be resolved prior to loading NCCL "
			"symbols");
	}
	...
	iree_status_t status = iree_dynamic_library_load_from_files(
		IREE_ARRAYSIZE(iree_hal_cuda_new_nccl_dylib_names),
		iree_hal_cuda_new_nccl_dylib_names, IREE_DYNAMIC_LIBRARY_FLAG_NONE,
	...
	if (iree_status_is_ok(status)) {
		status = iree_hal_cuda_new_nccl_check_version(out_syms->dylib);
	}

	if (iree_status_is_ok(status)) {
		status = iree_hal_cuda_new_nccl_dynamic_symbols_resolve_all(out_syms);
	}
