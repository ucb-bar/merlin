    ...
    for i in range(n):
        ...
        try:
            ...
            print(f"  [{tag}] run {i}: wall_ns={m.get('wall_ns')} cos={cos:.7f} "
                  f"n_xnn={n_xnn} matmul_ns={None if mb is None else round(mb)} "
...
def main() -> None:
    ...
    ap.add_argument("--model", default="output/openvla_fp32_consistent")
    ...
    ap.add_argument("--configs", default="ours_wholemodel_vf,xnnpack_kernels,baseline",
                    help="ours_wholemodel_vf,ours_wholemodel,xnnpack_kernels,baseline")
    ap.add_argument("--out", default="output/rvv_bench/dispatch_breakdown.json")
