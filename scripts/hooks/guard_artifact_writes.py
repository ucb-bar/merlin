...
Source edits (merlin/, scripts/, experiments/*/scripts/, tests/, *.md docs, etc.) are always allowed.
Escape hatch: env MERLIN_ALLOW_ARTIFACT_WRITE=1, or list a path prefix in
scripts/hooks/artifact_allowlist.txt. Contract: exit 0 = allow; exit 2 + stderr message = deny.
"""
...
def _repo_root() -> Path:
    ...
    if env:
        return Path(env)
    # hook lives at <repo>/scripts/hooks/guard_artifact_writes.py
    return Path(__file__).resolve().parents[2]


def _allowlisted(rel: str, root: Path) -> bool:
    f = root / "scripts" / "hooks" / "artifact_allowlist.txt"
    if not f.exists():
        return False
