...
def main():
    parser = argparse.ArgumentParser(description="Quantize a PyTorch model and export to MLIR via multiple paths.")
    parser.add_argument(
        "--model",
        type=str,
        default="mobilenet",
    ...
    parser.add_argument(
        "--output_dir", type=str, default="report_pt_export", help="Directory to save the exported files and report."
    )
    ...
    if model_quant_ao_static:
        ...
        try_torch_onnx_export(
            model_quant_ao_static, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
        try_torch_mlir_export(
            model_quant_ao_static, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
        try_turbine_aot_export(
            model_quant_ao_static, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
    else:
    ...
    if model_quant_ao_dynamic:
        ...
        try_torch_onnx_export(
            model_quant_ao_dynamic, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
        try_torch_mlir_export(
            model_quant_ao_dynamic, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
    ...
    if model_quant_torchao:
        ...
        try_turbine_aot_export(
            model_quant_torchao, sample_inputs, args.model, model_type_str, model_output_dir, results
        )
    else:
        print(f"\nSkipping export steps for '{model_type_str}' model because quantization failed.")
        add_failure_placeholders(model_type_str, results, f"Skipped ({model_type_str} Quant failed)")
