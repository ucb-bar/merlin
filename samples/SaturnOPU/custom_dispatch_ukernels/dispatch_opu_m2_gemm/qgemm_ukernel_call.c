...
// This function signature matches the complex memref ABI that IREE will
// generate. Note: We only use the base pointers (the first argument for each
// memref).
void qgemm_i8_bias_i32_workgroup(
	// memref %at (A)
	const int8_t *restrict at_base, const int8_t *restrict at_aligned,
	size_t at_offset, size_t at_size_m, size_t at_size_k, size_t at_stride_m,
	size_t at_stride_k,
	// memref %b (B)
	const int8_t *restrict b_base, const int8_t *restrict b_aligned,
	size_t b_offset, size_t b_size_k, size_t b_size_n, size_t b_stride_k,
	size_t b_stride_n,
	// memref %bias
