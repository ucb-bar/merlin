...
class LSTMNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.lstm = LSTM(input_size=665, hidden_size=395, num_layers=2, dropout=0.15, bias=False)
...
class LSTMNetVIT(nn.Module):
    ...
    def forward(self, image, velocity, quaternion, h_in, c_in):
        X = [image, velocity, quaternion]
        ...
        # Add sequence dimension (L=1) for LSTM
        out = out.unsqueeze(1)

        # --- NO IF-STATEMENT ---
        # Always use the 5 inputs
...
class UNetConvLSTMNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.unet_upconv1 = nn.ConvTranspose2d(
            16,
            8,
            kernel_size=2,
            stride=2,
        ...
        self.unet_upconv2 = nn.ConvTranspose2d(
            8,
            4,
            kernel_size=3,
            stride=3,
    ...
    def forward(self, X):
        ...
        unet_dec1 = torch.relu(
            self.unet_d12(torch.relu(self.unet_d11(torch.cat([self.unet_upconv1(y_e3), y_e2], dim=1))))
        )
        unet_dec2 = torch.relu(
            self.unet_d22(torch.relu(self.unet_d21(torch.cat([self.unet_upconv2(unet_dec1), y_e1], dim=1))))
        )
