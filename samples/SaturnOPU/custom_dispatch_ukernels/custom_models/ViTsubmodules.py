...
class EfficientSelfAttention(nn.Module):
    def __init__(self, channels, reduction_ratio, num_heads):
        ...
        self.cn1 = nn.Conv2d(
            in_channels=channels, out_channels=channels, kernel_size=reduction_ratio, stride=reduction_ratio
        )
...
class MixTransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        self._attn = nn.ModuleList(
            [EfficientSelfAttention(out_channels, reduction_ratio, num_heads) for _ in range(n_layers)]
        )
