...
class DispatchNode:
    ...
    concurrent_group: int | None = None  # Stage number where this dispatch can run
...
class MLIRStreamParser:
    ...
    def load_file(self):
        ...
        with open(self.mlir_file_path) as f:
            self.content = f.read()
    ...
    def parse_dispatches(self):
        ...
        for match in matches:
            ...
            node = DispatchNode(name=full_name, dispatch_id=dispatch_id)
    ...
    def parse_dependencies(self):
        ...
        if missing_dispatches:
            print(
                f"Warning: {len(missing_dispatches)} dispatches not found in execution stages: {sorted(missing_dispatches)}"
            )
        ...
        while i < len(lines):
            ...
            if "stream.cmd.concurrent" in line:
                ...
                if current_stage_dispatches:
                    print(
                        f"[DEBUG] Concurrent stage {execution_stage}: dispatches {sorted(current_stage_dispatches)}; deps from prev stage {sorted(previous_stage_dispatches)}"
                    )
            ...
            # Check for standalone dispatch (sequential)
                ...
                if dispatch_id is not None and dispatch_id in self.dispatches:
                    print(
                        f"[DEBUG] Sequential stage {execution_stage}: dispatch {dispatch_id}; deps from prev stage {sorted(previous_stage_dispatches)}"
                    )
    ...
    def _extract_dispatch_id(self, line: str) -> int | None:
    ...
    def export_to_json(self, output_path: str):
        ...
        for edge in self.graph.edges():
            data["edges"].append({"from": f"dispatch_{edge[0]}", "to": f"dispatch_{edge[1]}"})
        ...
        for group_idx, group in enumerate(self.concurrent_groups):
            data["concurrent_groups"].append(
                {"group": group_idx, "dispatches": [f"dispatch_{d}" for d in sorted(group)]}
            )
...
def main():
    ...
    parser.add_argument("mlir_file", type=str, help="Path to MLIR stream file")
    parser.add_argument("--visualize", "-v", type=str, help="Output path for visualization (PNG)", default=None)
    parser.add_argument("--export-json", "-j", type=str, help="Export graph to JSON file", default=None)
