...
static iree_status_t SubmitAsyncInvocation(iree_runtime_session_t *session,
	...
	uint64_t signal_epoch, iree_allocator_t host_allocator,
	...
	*out_outputs = nullptr;
...

	{
		...
		// Create an empty fence (0 capacity) to represent "no wait
		// dependencies"
		status = iree_hal_fence_create(
			/*capacity=*/0, host_allocator, &empty_wait_fence);
		if (!iree_status_is_ok(status))
			goto cleanup;
	...
	status = iree_hal_fence_create_at(signal_semaphore, signal_epoch,
		iree_hal_device_host_allocator(device), &signal_fence);
	if (!iree_status_is_ok(status))
		goto cleanup;
...
static iree_status_t SubmitModelFrame(SharedRuntimeState *state,
	...
	iree_vm_list_t *outputs = nullptr;
	status =
		SubmitAsyncInvocation(model->session, model->function_name, input_view,
			model->device, model->timeline, epoch, host_allocator, &outputs);
...
static iree_status_t WaitTimelineWithTimeout(
	iree_hal_semaphore_t *semaphore, uint64_t epoch, iree_timeout_t timeout) {
	IREE_ASSERT_ARGUMENT(semaphore);
...
static void ReaperMain(SharedRuntimeState *state,
	...
	while (true) {
		...
		if (!dronet_ready && !mlp_ready) {
			...
			iree_status_t status = WaitTimelineWithTimeout(wait_timeline,
				wait_epoch, iree_make_timeout_ns(kReaperWaitTimeoutNs));
			if (!iree_status_is_ok(status)) {
				StoreFatalStatusIfFirst(
					state, status, "[reaper] timed wait on timeline failed");
				break;
			}
			continue;
		}
