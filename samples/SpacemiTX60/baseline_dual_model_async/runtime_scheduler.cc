...
static void StoreFatalStatusIfFirst(
	SharedRuntimeState *state, iree_status_t status, const char *context) {
...
static bool FunctionUsesCoarseFencesAbi(const iree_vm_function_t *function) {
	...
	// Many VMFBs (especially embedded/perf builds) don't retain reflection
	// attrs. If it's missing, we can't validate here, so don't fail.
	if (iree_string_view_is_empty(model))
		return true;
...
static iree_status_t CreateF32InputViewFromData(iree_hal_device_t *device,
	iree_hal_allocator_t *device_allocator, const iree_hal_dim_t *shape,
	iree_host_size_t shape_rank, const float *input_data,
	iree_host_size_t element_count, iree_hal_buffer_view_t **out_view) {
	...
	*out_view = nullptr;
	...
	return iree_hal_buffer_view_allocate_buffer_copy(device, device_allocator,
		shape_rank, shape, IREE_HAL_ELEMENT_TYPE_FLOAT_32,
		IREE_HAL_ENCODING_TYPE_DENSE_ROW_MAJOR, buffer_params,
		iree_make_const_byte_span(input_data, element_count * sizeof(float)),
		out_view);
}

static iree_status_t ValidateOutputList(
	iree_vm_list_t *outputs, iree_host_size_t expected_outputs) {
	...
	if (output_count != expected_outputs) {
		return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
			"expected %" PRIu64 " outputs from invocation but got %" PRIu64,
	...
	for (iree_host_size_t i = 0; i < output_count; ++i) {
		...
		if (!view) {
			return iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
				"output %" PRIu64 " was not a HAL buffer view", (uint64_t)i);
		}
	}
	return iree_ok_status();
}

static iree_status_t SubmitAsyncInvocation(iree_runtime_session_t *session,
	const char *function_name, iree_hal_buffer_view_t *input_view,
	iree_hal_device_t *device, iree_hal_semaphore_t *signal_semaphore,
	uint64_t signal_epoch, iree_host_size_t expected_outputs,
	iree_allocator_t host_allocator, iree_hal_fence_t **out_signal_fence,
	...
	*out_outputs = nullptr;
	...
	iree_allocator_t device_host_allocator =
	...
	status = iree_vm_list_create(iree_vm_make_undefined_type_def(),
		/*capacity=*/expected_outputs, host_allocator, &outputs);
	if (!iree_status_is_ok(status))
		goto cleanup;
	...
	status = iree_hal_fence_create_at(
		signal_semaphore, signal_epoch, device_host_allocator, &signal_fence);
	if (!iree_status_is_ok(status))
		goto cleanup;

	{
		iree_vm_ref_t signal_fence_ref =
			iree_hal_fence_retain_ref(signal_fence);
...
static iree_status_t SubmitModelFrame(SharedRuntimeState *state,
	ModelSubmitContext *model, iree_allocator_t host_allocator) {
	...
	iree_status_t status =
		CreateF32InputViewFromData(model->device, model->device_allocator,
			model->sensor->shape().data(), model->sensor->shape().size(),
			model->host_input.data(), model->host_input.size(), &input_view);
	if (!iree_status_is_ok(status))
		return status;
	...
	status = SubmitAsyncInvocation(model->session, model->function_name,
		input_view, model->device, model->timeline, epoch,
		model->expected_outputs, host_allocator, &signal_fence, &outputs);
...
static iree_status_t PollTimelineReached(
	iree_hal_semaphore_t *semaphore, uint64_t epoch, bool *out_reached) {
...
static void ReleasePendingOutputsUnlocked(
...
static void ReaperMain(SharedRuntimeState *state,
	iree_hal_semaphore_t *dronet_timeline, iree_hal_semaphore_t *mlp_timeline) {
	while (true) {
...

		{
			...
			state->pending_cv.wait_for(
				lock, std::chrono::milliseconds(1), [&]() {
					return HasFatalStatus(state) ||
						state->submission_done.load(
							std::memory_order_relaxed) ||
						!state->dronet_pending.empty() ||
						!state->mlp_pending.empty();
				});
		...
		if (has_dronet) {
			iree_status_t status = PollTimelineReached(
				dronet_timeline, dronet_epoch, &dronet_ready);
			if (!iree_status_is_ok(status)) {
				StoreFatalStatusIfFirst(
					state, status, "[dronet/reaper] timeline wait failed");
				break;
			}
		}
		...
		if (has_mlp) {
			...
			if (!iree_status_is_ok(status)) {
				StoreFatalStatusIfFirst(
					state, status, "[mlp/reaper] timeline wait failed");
				break;
			}
		}

		if (!dronet_ready && !mlp_ready) {
			// Avoid timed semaphore waits (some embedded builds/drivers return
			// ABORTED).
		...
		const bool pop_dronet = dronet_ready &&
			(!mlp_ready || dronet_submit_time <= mlp_submit_time);
		InflightInvocation invocation;
		{
			std::lock_guard<std::mutex> lock(state->pending_mutex);
			std::deque<InflightInvocation> *queue =
		...
		if (!iree_status_is_ok(status)) {
			StoreFatalStatusIfFirst(
				state, status, "[reaper] invocation outputs were invalid");
			break;
		}
		...
		if (invocation.model_id == ModelId::kDronet) {
			...
			state->dronet_total_latency_us.fetch_add(
				latency_us, std::memory_order_relaxed);
			state->dronet_completed_epoch.store(
				invocation.epoch, std::memory_order_relaxed);
		} else {
			state->mlp_invocations.fetch_add(1, std::memory_order_relaxed);
			state->mlp_total_latency_us.fetch_add(
				latency_us, std::memory_order_relaxed);
			state->mlp_completed_epoch.store(
				invocation.epoch, std::memory_order_relaxed);
		}
	}
...
extern "C" int merlin_dual_model_runtime_run(
	...
	if (!config || !config->dronet_vmfb_path || !config->mlp_vmfb_path ||
		...
		!config->driver_name) {
		fprintf(stderr,
			"Invalid config passed to merlin_dual_model_runtime_run.\n");
		return 1;
	}
	...
	do {
		...
		status = iree_runtime_session_create_with_device(instance,
			&session_options, device,
		...
		status = iree_runtime_session_lookup_function(session,
			iree_make_cstring_view(config->dronet_function), &dronet_function);
		if (!iree_status_is_ok(status))
			break;

		iree_vm_function_t mlp_function;
		status = iree_runtime_session_lookup_function(session,
			iree_make_cstring_view(config->mlp_function), &mlp_function);
		if (!iree_status_is_ok(status))
			break;

		auto dump_model = [](const char *tag, const iree_vm_function_t *f) {
			iree_string_view_t v = iree_vm_function_lookup_attr_by_name(
				f, IREE_SV("iree.abi.model"));
		...
		// TODO: Figure out what is the problem with the naming of the different
		// modules
		// const bool dronet_async =
		// FunctionUsesCoarseFencesAbi(&dronet_function); const bool mlp_async =
		// FunctionUsesCoarseFencesAbi(&mlp_function);
		...
		if (!dronet_async || !mlp_async) {
			status = iree_make_status(IREE_STATUS_FAILED_PRECONDITION,
				"both functions must be compiled with "
				"--iree-execution-model=async-external "
				"(iree.abi.model=coarse-fences) and you must call the $async "
				"exports");
			break;
		}
		...
		status = iree_hal_semaphore_create(device, IREE_HAL_QUEUE_AFFINITY_ANY,
			/*initial_value=*/0ull, IREE_HAL_SEMAPHORE_FLAG_DEFAULT,
			&dronet_timeline);
		if (!iree_status_is_ok(status))
			break;
		...
		dronet_sensor = std::make_unique<PeriodicTensorSensor>("dronet_sensor",
			dronet_shape, config->dronet_sensor_frequency_hz,
			/*base_value=*/0.01f, /*amplitude=*/1.0f);
		fprintf(stdout, "Created dronet sensor.\n");

		mlp_sensor = std::make_unique<PeriodicTensorSensor>("mlp_sensor",
			mlp_shape, config->mlp_sensor_frequency_hz,
		...
		dronet_submit.device_allocator =
		...
		const auto report_period =
			PeriodFromFrequencyHz(config->report_frequency_hz);
		...
		while (!HasFatalStatus(&state)) {
			...
			while (now >= next_mlp_release + mlp_period) {
				state.mlp_deadline_misses.fetch_add(
			...
			if (now >= next_mlp_release) {
				...
				if (mlp_inflight < kMlpMaxInFlight) {
					status =
						SubmitModelFrame(&state, &mlp_submit, host_allocator);
					if (!iree_status_is_ok(status)) {
						StoreFatalStatusIfFirst(
							&state, status, "[mlp] async submission failed");
						break;
					}
					submitted_any = true;
				} else {
			...
			while (!HasFatalStatus(&state)) {
				const uint64_t dronet_inflight =
					state.dronet_submitted_epoch.load(
						std::memory_order_relaxed) -
					state.dronet_completed_epoch.load(
						std::memory_order_relaxed);
				if (dronet_inflight >= kDronetMaxInFlight)
					break;

				status =
					SubmitModelFrame(&state, &dronet_submit, host_allocator);
				if (!iree_status_is_ok(status)) {
					StoreFatalStatusIfFirst(
						&state, status, "[dronet] async submission failed");
					break;
				}
				submitted_any = true;
			}
			...
			if (now >= next_report) {
				...
				const double dronet_hz =
					(dt_s > 0.0) ? ((double)dronet_delta / dt_s) : 0.0;
				const double mlp_hz =
					(dt_s > 0.0) ? ((double)mlp_delta / dt_s) : 0.0;
				...
				const uint64_t dronet_generated =
					dronet_sensor->generated_count();
				...
				fprintf(stdout,
					...
					dronet_hz, mlp_hz, misses, dronet_now, mlp_now,
					dronet_inflight, mlp_inflight, dronet_fresh, mlp_fresh,
					dronet_generated, mlp_generated);
				fflush(stdout);
		...
		const double dronet_avg_ms = (dronet_total > 0)
			? ((double)dronet_latency_us / 1000.0) / (double)dronet_total
			: 0.0;
		const double mlp_avg_ms = (mlp_total > 0)
			? ((double)mlp_latency_us / 1000.0) / (double)mlp_total
			: 0.0;
