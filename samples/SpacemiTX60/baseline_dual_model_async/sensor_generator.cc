...
PeriodicTensorSensor::PeriodicTensorSensor(const std::string &name,
	const std::vector<iree_hal_dim_t> &shape, double frequency_hz,
	float base_value, float amplitude)
	: name_(name), shape_(shape), frequency_hz_(frequency_hz),
	  base_value_(base_value), amplitude_(amplitude) {
	element_count_ = 1;
...
uint64_t PeriodicTensorSensor::Snapshot(
...
void PeriodicTensorSensor::ThreadMain() {
	...
	while (!stop_requested_.load(std::memory_order_relaxed)) {
		...
		if (next_release > now) {
			...
			// If generation is late, continue immediately and re-anchor the
			// next release to avoid unbounded drift.
			next_release = now;
		}
	}
}

void PeriodicTensorSensor::GenerateSample(
	uint64_t sample_index, std::vector<float> *out) const {
