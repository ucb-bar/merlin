...
class PeriodicTensorSensor {
 public:
  ...
  const std::vector<iree_hal_dim_t>& shape() const { return shape_; }
  iree_host_size_t element_count() const { return element_count_; }
  double frequency_hz() const { return frequency_hz_; }

  // Copies the latest generated tensor into |out_data| and returns the latest
  // sequence number.
  uint64_t Snapshot(std::vector<float>* out_data,
