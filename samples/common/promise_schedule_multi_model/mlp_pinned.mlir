...
module attributes {stream.affinity.default = #hal.device.affinity<@device_a>} {
  ...
  util.initializer {
    ...
    %did_map, %result = stream.resource.try_map on(#hal.device.affinity<@device_a>) %buffer_cst[%c0] : !util.buffer -> i1, !stream.resource<constant>{%c5952}
    cf.cond_br %did_map, ^bb2(%0, %result : !stream.timepoint, !stream.resource<constant>), ^bb1
  ^bb1:  // pred: ^bb0
    %1 = stream.resource.alloc uninitialized on(#hal.device.affinity<@device_a>) : !stream.resource<constant>{%c5952}
    %file = stream.file.constant on(#hal.device.affinity<@device_a>) %buffer_cst[%c0 for %c5952] : !util.buffer{%c5952} -> !stream.file
    %2 = stream.file.read on(#hal.device.affinity<@device_a>) await(%0) => %file[%c0_i64], %1[%c0], %c5952 : !stream.file -> !stream.resource<constant>{%c5952} => !stream.timepoint
    cf.br ^bb2(%2, %1 : !stream.timepoint, !stream.resource<constant>)
  ^bb2(%3: !stream.timepoint, %4: !stream.resource<constant>):  // 2 preds: ^bb0, ^bb1
    %result_0, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@device_a>) : !stream.resource<constant>{%c6592} => !stream.timepoint
    %5 = stream.timepoint.join max(%3, %result_timepoint) => !stream.timepoint
    %6 = stream.cmd.execute once on(#hal.device.affinity<@device_a>) await(%5) => with(%4 as %arg0: !stream.resource<constant>{%c5952}, %result_0 as %arg1: !stream.resource<constant>{%c6592}) {
  ...
  util.func public @main_graph$async(%arg0: !hal.buffer_view, %arg1: !hal.fence, %arg2: !hal.fence) -> !hal.buffer_view attributes {inlining_policy = #util.inline.never, iree.abi.model = "coarse-fences", iree.abi.stub} {
    ...
    %2 = stream.tensor.import on(#hal.device.affinity<@device_a>) %arg0 : !hal.buffer_view -> tensor<?x10xf32>{%0} in !stream.resource<external>{%1}
    %3 = stream.timepoint.import on(#hal.device.affinity<@device_a>) %arg1 : (!hal.fence) => !stream.timepoint
    %4 = arith.ceildivsi %0, %c32 : index
    %5 = arith.muli %4, %c1280 : index
    %6 = arith.muli %4, %c4096 : index
    %7 = arith.muli %0, %c8 : index
    %result, %result_timepoint = stream.resource.alloca uninitialized on(#hal.device.affinity<@device_a>) await(%3) => !stream.resource<external>{%7} => !stream.timepoint
    %8 = arith.addi %5, %6 : index
    %9 = arith.addi %8, %6 : index
    %result_0, %result_timepoint_1 = stream.resource.alloca uninitialized on(#hal.device.affinity<@device_a>) await(%3) => !stream.resource<transient>{%9} => !stream.timepoint
    ...
    %23 = stream.cmd.execute on(#hal.device.affinity<@device_a>) await(%10) => with(%2 as %arg3: !stream.resource<external>{%1}, %__hoisted_tensor_32x10xf32__encoded as %arg4: !stream.resource<constant>{%c6592}, %result as %arg5: !stream.resource<external>{%7}, %result_0 as %arg6: !stream.resource<transient>{%9}) {
    ...
    %24 = stream.resource.dealloca on(#hal.device.affinity<@device_a>) await(%23) => %result_0 : !stream.resource<transient>{%9} => !stream.timepoint
    stream.timepoint.chain_external on(#hal.device.affinity<@device_a>) %24 => (%arg2 : !hal.fence)
    %25 = stream.tensor.export on(#hal.device.affinity<@device_a>) %result : tensor<?x2xf32>{%0} in !stream.resource<external>{%7} -> !hal.buffer_view
