...
void iree_task_topology_initialize_from_mask(
	iree_task_topology_t *out_topology, uint64_t mask) {
	iree_task_topology_initialize(out_topology);

	for (int core_id = 0; core_id < 64; ++core_id) {
		if ((mask >> core_id) & 1) {
			iree_task_topology_group_t group;
			iree_task_topology_group_initialize(
				out_topology->group_count, &group);
			...
			iree_status_t status =
				iree_task_topology_push_group(out_topology, &group);
...
iree_status_t create_device_with_mask(iree_allocator_t host_allocator,
	uint64_t core_mask, const char *label, iree_hal_device_t **out_device) {
	...
	options.worker_local_memory_size =
		64 * 1024; // 64KB local memory per worker

	iree_task_executor_t *executor = NULL;
	IREE_RETURN_IF_ERROR(iree_task_executor_create(
		options, &topology, host_allocator, &executor));

	iree_hal_task_device_params_t params =
		iree_hal_task_device_params_default();

	// Create the device using the pinned executor
	// Note: queue_count=1 is sufficient; the executor handles the thread pool
	// width internally.
	iree_status_t status =
		iree_hal_task_device_create(iree_make_cstring_view(label), &params,
			executor, 1, NULL, host_allocator, out_device);
...
int main(int argc, char **argv) {
	...
	IREE_CHECK_OK(iree_runtime_instance_create(
		&instance_opts, host_allocator, &instance));
	...
	IREE_CHECK_OK(
		create_device_with_mask(host_allocator, 1, "device_a", &device_a));

	// 2. Create Core 1 Device (device_b)
	iree_hal_device_t *device_b = NULL;
	IREE_CHECK_OK(
		create_device_with_mask(host_allocator, 2, "device_b", &device_b));

	// 3. Create Cluster Device (device_ab)
	iree_hal_device_t *device_ab = NULL;
	IREE_CHECK_OK(
		create_device_with_mask(host_allocator, 3, "device_ab", &device_ab));
	...
	IREE_CHECK_OK(iree_runtime_session_create(
		instance, &session_opts, host_allocator, &session));
	...
	IREE_CHECK_OK(
		iree_hal_module_create(iree_runtime_instance_vm_instance(instance),
			IREE_ARRAYSIZE(devices), devices, IREE_HAL_MODULE_FLAG_NONE,
			iree_runtime_instance_host_allocator(instance), &hal_module));
	...
	IREE_CHECK_OK(iree_vm_list_create(
		iree_vm_make_undefined_type_def(), 1, host_allocator, &inputs));

	iree_vm_list_t *outputs = NULL;
	IREE_CHECK_OK(iree_vm_list_create(
		iree_vm_make_undefined_type_def(), 1, host_allocator, &outputs));
	...
	IREE_CHECK_OK(iree_hal_buffer_view_allocate_buffer_copy(device_a,
		iree_hal_device_allocator(device_a), IREE_ARRAYSIZE(shape), shape,
	...
	// but iree_hal_buffer_map_read handles mapping regardless of device
	// location for local-task devices (shared memory).
	...
	fprintf(stdout, "Result: [%.1f, %.1f, %.1f, %.1f]\n", result_data[0],
		result_data[1], result_data[2], result_data[3]);
