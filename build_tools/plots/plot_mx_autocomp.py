...
def fig_cum_spend_per_iter():
    ...
    for k, (run, lab) in enumerate(CURATED_SPEND):
        ...
        axc.plot(x, cum_c, ls=ls, color=col, lw=2.2, marker="o", ms=5, mfc=col, mec=INK, mew=0.8, zorder=4)
        axt.plot(x, cum_t, ls=ls, color=col, lw=2.2, marker="o", ms=5, mfc=col, mec=INK, mew=0.8, zorder=4)
        legend_h.append(Line2D([0], [0], color=col, ls=ls, lw=2.2, marker="o", mfc=col, mec=INK, label=f"{lab}"))
        if cum_c[-1] > pricey[0]:
    ...
    pipe_h = [
        Line2D([0], [0], color=PIPE[p]["c"], lw=3, label=PIPE[p]["lab"]) for p in ("sonnet", "geminiqwen", "geminipro")
    ]
    axc.legend(
        handles=pipe_h, loc="upper left", fontsize=10, title="model pipeline", title_fontproperties={"weight": "bold"}
    )
    axt.legend(handles=legend_h, loc="upper left", fontsize=9.0, ncol=1)
    emph(axc, pricey[1] - 0.15, pricey[0], f"${pricey[0]:.2f}", color=GOLD, fs=11, ha="right", va="bottom")
...
def fig_total_by_model():
    ...
    groups = [
        (
            "Sonnet-4.6",
            bym.get("us.anthropic.claude-sonnet-4-6", 0) + bym.get("global.anthropic.claude-sonnet-4-6", 0),
            NAVY,
        ),
    ...
    fig, (axd, axr) = plt.subplots(1, 2, figsize=(14.5, 6.2), gridspec_kw={"width_ratios": [1.15, 1]})
    style_ax(axd, grid="y")
    for xi, v, c in zip(x, vals, cols):
        vbars(axd, [xi], [v], c, width=0.62)
    for xi, v in zip(x, vals):
        axd.text(xi, v + 1.6, f"${v:.1f}", ha="center", va="bottom", fontsize=10.5, color=INK, fontweight="bold")
    axd.set_xticks(x)
    axd.set_xticklabels(labels, fontsize=9.5, rotation=12, ha="right")
    ...
    axr.set_xlim(0, len(cum) * 1.04)
    axr.set_ylim(0, max(cum) * 1.08)
    title(axr, "Cumulative project spend")
    emph(axr, len(cum) * 0.92, max(cum) * 0.99, f"${max(cum):.2f}", color=GOLD, fs=12, ha="right", va="top")
...
def fig_baseline_vs_final():
    ...
    blocks = [
        ("RTL (silicon truth)", [(l, s) for l, _, _, s in RTL_ROWS], "RTL"),
    ...
    for x, lab, sp, tier, bname in xpos:
        ...
        ax.text(x, sp + 0.03, f"{sp:.2f}×", ha="center", va="bottom", fontsize=9.5, color=INK, fontweight="bold")
    ...
    for bname, rows, tier in blocks:
        ...
        ax.text(np.mean(xs), 2.40, bname, ha="center", va="top", fontsize=10.5, color=TIER[tier], fontweight="bold")
    # hero
    ...
    leg = [
        Patch(facecolor=TIER["RTL"], edgecolor=INK, label="RTL — silicon truth"),
...
def fig_model_compare():
    ...
    for r in rows:
        ...
        ax.scatter(cost, sp, s=150, c=col, marker=mk, edgecolors=INK, linewidths=1.1, zorder=5, alpha=0.95)
        pts.append((cost, sp, r["prob_type"], pipe, r["metric"]))
        seen_pipe.add(pipe)
        seen_tier.add(r["metric"])
    ...
    pipe_h = [
        Line2D([0], [0], marker="o", ls="", mfc=PIPE[p]["c"], mec=INK, ms=11, label=PIPE[p]["lab"])
        for p in ("sonnet", "geminiqwen", "geminipro")
        if p in seen_pipe
    ]
    tier_h = [
        Line2D([0], [0], marker="o", ls="", mfc="white", mec=INK, ms=11, label="spike latency"),
        Line2D([0], [0], marker="D", ls="", mfc="white", mec=INK, ms=10, label="faithful cycles"),
    ]
    l1 = ax.legend(
        handles=pipe_h, loc="upper right", fontsize=9.5, title="model pipeline", title_fontproperties={"weight": "bold"}
    )
    ax.add_artist(l1)
    ax.legend(handles=tier_h, loc="lower right", fontsize=9.5, title="metric", title_fontproperties={"weight": "bold"})
    fig.tight_layout(rect=[0, 0, 1, 0.95])
...
def fig_outcome_funnel():
    ...
    ocol = {"improved": NAVY, "correct_no_gain": SLATE, "regressed": SAGE, "incorrect": MAUVE, "compile_error": BLUE}
    olab = {
        "improved": "improved",
    ...
    for o in order:
        ...
        ax.bar(x, vals, width, bottom=bottoms, color=ocol[o], edgecolor=INK, linewidth=1.1, zorder=3, label=olab[o])
        bottoms += vals
    for xi, tot in zip(x, totals):
        ax.text(
            xi,
            tot + max(totals) * 0.015,
            f"{tot} attempts",
            ha="center",
            va="bottom",
            fontsize=9.5,
    ...
    emph(
        ...
        x[-1] + 0.05,
        max(totals) * 1.10,
        f"{tot_improved}/{tot_all} improved",
        color=GOLD,
        fs=11,
        ha="right",
        va="top",
...
def fig_gain_by_iter():
    ...
    for run, lab, col, ls in runs:
        ...
        ax.plot(xs, ys, ls=ls, color=col, lw=2.4, marker="o", ms=6, mfc=col, mec=INK, mew=0.8, zorder=4, label=lab)
    ax.set_xlabel("optimization iteration")
    ax.set_ylabel("best speedup found so far")
    ax.set_xlim(-0.2, 7.2)
    ax.set_ylim(0.95, maxsp * 1.08)
    ax.set_xticks(range(0, 8))
