...
for xi, mv, xv in zip(x, muon_v, mx_v):
    ...
    if mv > 8:
        ax.annotate(
            f"${mv:.1f}", (xi, mv / 2), ha="center", va="center", fontsize=9.5, color=BG, fontweight="bold", zorder=5
        )
    if xv > 8:
        ax.annotate(
            f"${xv:.1f}",
...
emph(
    ax,
    len(models) - 0.5,
    max(combined[m] for m in models) * 1.07,
    f"combined ${gtot:.0f} · {gcalls:,} calls",
    color=GOLD,
    fs=12,
    ha="right",
    va="center",
...
ax.legend(
    handles=[
        Patch(fc=NAVY, ec=INK, label=f"Muon / radiance  (${muon_tot:.0f})"),
