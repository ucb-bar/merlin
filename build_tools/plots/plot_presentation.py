...
def ours_key(s):
    cands = [
        k
        for k in ("ours_wholemodel_vf", "ours_v3", "ours_wholemodel", "ours_tiled")
...
def fig_fourway():
    ...
    def _lat(ser, m):
        s = data[m]
        key = ours_key(s) if ser == "ours" else keymap.get(ser, ser)
    ...
    ax.legend(
        handles=[
            Patch(facecolor=SERIES[s]["c"], edgecolor=INK, hatch=SERIES[s]["h"], label=SERIES[s]["lab"]) for s in order
        ],
        fontsize=14,
        ncol=2,
        loc="upper left",
    ...
    for j, m in enumerate(models):
        ...
        if all(r > 1 for r in rs):  # both experts slower than ours → we win
            tag = f"OURS WINS\nup to {max(rs):.2f}× faster"
            col = GOLD
        else:  # an expert is faster than ours → we lose
            tag = f"experts win\nup to {1 / min(rs):.2f}× faster"
            col = BLUE
        ax.text(
            j,
            max(max(rs), 1.0) + 0.10,
    ...
    ax.legend(
        handles=[
            Patch(facecolor=SERIES[s]["c"], edgecolor=INK, hatch=SERIES[s]["h"], label=SERIES[s]["lab"]) for s in zser
        ],
        fontsize=14,
        loc="upper right",
    )
...
def fig_progression():
    ...
    ax.set_ylim(0, v3 * 1.18)
    ax.set_ylabel("whole-model speedup vs baseline  (×)")
    ...
    style_ax(ax)
    k = [
        ("baseline", 22430926, MAUVE),
        ("vfmacc\ncontraction", 123094, SLATE),
        ("tiled\n(bounded)", 1328219, SAGE),
        ("v3 compute\nkernel", 53207, NAVY),
    ]
    xs = np.arange(len(k))
    vals = [s[1] for s in k]
    ...
    for yv, lab, col, mult in [
        (101705, "XNNPACK", SLATE, 1.85),
        (84483, "OpenBLAS", SAGE, 1.0),
    ...
    ax.set_ylim(3e4, 4e8)
    ax.set_xticks(xs)
    ax.set_xticklabels([s[0] for s in k], fontsize=10.5)
    ...
    caption(
        fig,
        "Left — whole-model bitvla on K1: attention-vfmacc → tiled → accumulator-resident v3, the final "
...
def fig_gap_attribution():
    ...
    vv = pick("ours_wholemodel")
    vf = pick("ours_wholemodel_vf")
    xn = pick("xnnpack")
    MR4, OB = 1.25, 1.06
    ...
    caption(
        fig,
        "Static decode of the emitted RVV asm.  Left: the iteration-2 .vf form eliminates the 8-op "
...
def fig_dispatch():
    ...
    panels = [
        (nm, k)
        for nm, k in (("openvla", "openvla_fp32_consistent"), ("rdt2", "rdt2_fp32_consistent"))
    ...
    for ax, (nm, key) in zip(axes, panels):
        ...
        for yi, (lab, mmms, dms) in zip(y, rows):
            ...
            emph(ax, mmms + dms + xmax * 0.012, yi, f"{mmms + dms:.0f} ms", color=INK, fs=12, va="center", ha="left")
        ax.set_yticks(y)
        ...
        ax.annotate(
            f"matmul  ours {omm:.0f} ms  vs  XNN {xmm:.0f} ms",
            xy=(omm, y[0]),
        ...
        ax.annotate(
            "", xy=(omm, y[0] - 0.30), xytext=(xmm, y[0] - 0.30), arrowprops=dict(arrowstyle="<->", color=GOLD, lw=1.8)
        )
        emph(
            ax,
            (omm + xmm) / 2,
    ...
    axes[0].legend(
        handles=[
            Patch(facecolor=MATMUL, edgecolor=INK, label="matmul kernel (MEASURED both arms)"),
    ...
    caption(
        fig,
        "K1 board, per-dispatch rdtime split on BOTH arms (cos >= 0.99999), SAME baseline runtime — only "
        ...
        "(our small-M GEMM behind XNNPACK's), not the runtime.",
        y=0.0,
    )
...
def fig_crossover():
    ...
    for name, ys, col, ls, mk in series:
        xs = [s for s, v in zip(shapes, ys) if v]
    ...
    ax.set_xticklabels([f"{s}³" for s in shapes], fontsize=12)
    ...
    caption(
        fig,
        "K1 inner-compute ticks.  A compiler-emitted register-blocked kernel (ours-intrinsic; dashed = "
...
def fig_beam_tree():
    ...
    STC = {"root": SLATE, "top": NAVY, "winner": NAVY, "survive": SAGE, "mid": SAGE, "prune": SLATE, "kill": MAUVE}
    ...
    def node(x, y, n, r_in):
        ...
        col = STC[n["st"]]
        circ = Ellipse(
            (x, y), 2 * rx, 2 * ry, linewidth=1.8, edgecolor=INK, facecolor=col, alpha=1.0, zorder=4
        )  # full opacity → off-white stays legible
        circ.set_path_effects([SHADOW, pe.Normal()])
        ax.add_patch(circ)
        if n["st"] == "winner":  # winner gets a gold ring
            ring = Ellipse((x, y), 2 * rx + 0.22, 2 * ry + 0.30, fill=False, linewidth=2.8, edgecolor=GOLD, zorder=3)
            ax.add_patch(ring)
        ...
        ax.text(
            ...
            va="center",
            color=PAPER,
            fontsize=24 if n["perf"] is not None else 26,
            fontweight="bold",
            zorder=6,
        )
        ax.text(x, y - ry * 0.46, n["mark"], ha="center", va="center", color=PAPER, fontsize=13, zorder=6)
    ...
    # legend
    leg = [
        Patch(facecolor=NAVY, edgecolor=INK, label="survivor / winner (top-k, expanded)"),
...
def fig_cca():
    ...
    ax.text(
        5.10,
        CARDY + CARDH - 0.95 - ((CARDH - 0.95) / 11) * 1.85,
        "↔",
        ha="center",
        va="center",
        color=GOLD,
    ...
    ax.text(
        5.10,
        CARDY + CARDH - 0.95 - ((CARDH - 0.95) / 11) * 2.85,
        "↔",
        ha="center",
        va="center",
        color=GOLD,
    ...
    ax.text(
        5.10,
        3.55,
        "cca_compare  →  Divergence  (only differing fields)",
        ha="center",
        color=BLUE,
        fontfamily=SANS,
        fontweight="bold",
    ...
    for j, (kfull, ev, ov) in enumerate(
        [("compute.contraction_form", "fused_fma", "mul_add"), ("compute.accumulator_resident", "true", "false")]
    ):
        ry = 2.85 - j * 0.72
        ax.text(0.70, ry, kfull, ha="left", color=INK, fontsize=10.2, fontfamily="monospace")
        ax.text(
            5.00, ry, f"expert = {ev}", ha="left", color=NAVY, fontsize=10.2, fontweight="bold", fontfamily="monospace"
        )
        ax.text(
            7.65, ry, f"ours = {ov}", ha="left", color=MAUVE, fontsize=10.2, fontweight="bold", fontfamily="monospace"
        )
    ax.text(
        5.10,
        1.18,
        "agreement gate: PASS — 6/8 facet fields reconstruct identically across levels",
        ha="center",
        color=SAGE,
        fontsize=9.2,
        fontweight="bold",
    ...
    ax.text(
        5.10,
        0.74,
        "evidence kernels: xnnpack_rvv_gemm · openblas_rvv_gemm   (asm-lifted, vtype-resolved — no regex)",
        ha="center",
        color=INK,
        fontsize=8.8,
        fontstyle="italic",
    ...
    arr = FancyArrowPatch(
        (10.05, 2.2), (11.05, 2.2), arrowstyle="-|>", mutation_scale=26, linewidth=2.4, color=GOLD, zorder=4
    )
    ax.add_patch(arr)
    ax.text(10.55, 2.66, "action_catalog\nroute", ha="center", va="center", color=GOLD, fontsize=9.2, fontweight="bold")
    ...
    ax.add_patch(act)
    hb = FancyBboxPatch(
        (AX, 8.87), AW, 0.62, boxstyle="round,pad=0.0,rounding_size=0.0", linewidth=0, facecolor=BLUE, zorder=4
    )
    ax.add_patch(hb)
    ax.text(
        AX + AW / 2,
    ...
    ax.text(AX + 0.30, 8.45, "action_class", ha="left", va="top", color=INK, fontsize=9.4, fontfamily="monospace")
    for j, cls in enumerate(["FLAG", "HEURISTIC", "PASS", "KNOB"]):
        ...
        chip = FancyBboxPatch(
            ...
            linewidth=1.3,
            edgecolor=(GOLD if on else "#cbb9a3"),
            facecolor=(GOLD if on else "white"),
            alpha=1.0 if on else 0.9,
            zorder=5,
        )
        ax.add_patch(chip)
        ax.text(
    ...
    afields = [
        ...
        (
            "change",
            "form vector.contract → emit\na fused vfmacc (one MAC) instead\nof separate vfmul.vv + vfadd.vv",
            INK,
        ),
    ...
    for i, s in enumerate(stages):
        ...
        chip = FancyBboxPatch(
            ...
            linewidth=1.4,
            edgecolor=INK,
            facecolor=(BLUE if on else "white"),
            alpha=1.0 if on else 0.92,
            zorder=5,
        )
        ax.add_patch(chip)
        ax.text(
        ...
        if i < n - 1:
            ax.annotate(
                "",
                xy=(cx + cw + gap - 0.04, 10.04),
                xytext=(cx + cw + 0.04, 10.04),
...
def fig_beam_gates():
    ...
    for i, ((head, sub, krung), chk, y) in enumerate(zip(gates, checks, gy)):
        ...
        ax.text(
            CX + 0.74, ky + 0.20, head, ha="left", va="center", color="white", fontfamily=SERIF, fontsize=13.5, zorder=5
        )
        ax.text(CX + 0.34, ky - 0.26, sub, ha="left", va="center", color="white", fontsize=9.6, zorder=5)
        ax.text(
            CX + CW - 0.18,
            y + BH - 0.24,
            krung,
    ...
    for cy in gate_cys:
        ...
        ax.text(CX - 0.18, cy, "✗", ha="right", va="center", color=MAUVE, fontsize=10.5, fontweight="bold", zorder=5)
    arrow((BUSX, RY + RH / 2), (RX + RW + 0.06, RY + RH / 2), color=MAUVE, lw=1.8, ls=(0, (5, 3)), scale=13, z=1)
    ...
    ax.text(KX + 0.06, OY + OH - 0.16, "Then ask:", ha="left", color=BLUE, fontsize=10.6, fontweight="bold")
    ax.text(
        KX + 0.06,
        OY + 0.54,
        "• does it generalize to other shapes?",
        ha="left",
        color=INK,
        fontsize=10.0,
        fontstyle="italic",
    ...
    ax.text(
        KX + 0.06,
        OY + 0.16,
        "• does it translate to E2E model performance?",
        ha="left",
        color=INK,
        fontsize=10.0,
        fontstyle="italic",
...
def fig_beam_candidates():
    ...
    if not runs:
        print("beam_candidates: no beam_rvv_v2 run; skipping")
        return
    run = runs[-1]
    ...
    for ax, M in zip(axes, ("bitvla", "openvla")):
        ...
        for i, x in enumerate(rows):
            sp = x["speedup"]
            scalar = x["lowering"] == "scalar_fallback"
            if scalar:
                col = MAUVE
            elif i == best_i:
                col = NAVY
            ...
            util = 100 * sp / ceil
            if scalar:
                note = f"{sp:.2f}×  ·  scalar fallback (0% VPU)"
            elif util >= 100:
                note = f"{sp:.2f}×  ·  {util:.0f}% — beats ceiling"
            else:
                note = f"{sp:.2f}×  ·  {util:.0f}% of ceiling"
            emph(ax, sp + topv * 0.015, i, note, color=(MAUVE if scalar else INK), fs=9.2, va="center", ha="left")
...
def _node(ax, x, y, w, h, kind, title, sub=None, ts=11.0, ss=7.6, tag=None, z=4):
    ...
    ax.text(x + w / 2, ty, title, ha="center", va="center", color=tc, fontfamily=SERIF, fontsize=ts, zorder=z + 1)
    if sub:
        ax.text(
            x + w / 2,
            y + h * 0.24,
...
def _golds(ax, gx, cy, w, items, chip_h=0.40, gap=0.085, fs=8.4, z=4):
    ...
    for i, it in enumerate(items):
        ...
        ax.add_patch(b)
        ax.text(gx + w / 2, yy + chip_h / 2, it, ha="center", va="center", color=INK, fontsize=fs, zorder=z + 1)
...
def _draw_shared_stack(ax):
    ...
    cx = MX + MW / 2
    GX, GW = 9.95, 2.90
    wl = _node(ax, MX, 7.95, MW, 0.66, "input", "Interesting Workload", "ML Framework · PyTorch · TorchAO", ts=11)
    ...
    core = _node(
        ...
        0.66,
        "ours2",
        "Core MLIR Dialects Infrastructure",
        "linalg · scf · quant_ext · interface",
        ts=10.5,
    )
    copt = _node(ax, MX, 4.05, MW, 0.78, "ours", "Compiler Optimization", "schedule / interface passes", ts=12)
    passo = _node(ax, MX, 2.70, MW, 0.74, "tofill", "Passes Optimization", "target→LLVM passes", ts=12)
    tdial = _node(ax, MX, 1.50, MW, 0.66, "tofill", "Target Dialect", "OOT dialect · targetgen / IRDL", ts=12)
    outp = _node(
        ax,
        MX,
        0.40,
        ...
        0.66,
        "output",
        "Output · Application Binary · Runtime",
        "rv64 elf · spike / K1 / FireSim",
        ts=10.5,
    )
    # gold side-stacks of mined improvements (front-end vs lowering)
    _golds(
        ax, GX, m2m["cy"], GW, ["New Quantization Schemes", "New PyTorch Ops", "New IR", "New Graph Transformations"]
    )
    _golds(
        ax,
        GX,
        passo["cy"],
        GW,
        ["Feedback (CHECK-DAG)", "Informed Heuristics", "Introduce new Instructions", "Automate MLIR to LLVM"],
    )
    ...
    _narrow(ax, (lib["l"] - 0.02, lib["cy"]), (copt["r"] + 0.02, copt["cy"]), color=SAGE, lw=1.7, ls=(0, (5, 3)))
    # spine arrows
...
def _dual_legend(ax, y=-0.42):
    items = [
        ("input", "input"),
        ("input_opt", "optional input"),
        ("ours", "our compiler stack"),
        ("gold", "mined improvement"),
        ("tofill", "your dialect (OOT / targetgen)"),
...
def fig_dual_compiler():
    ...
    gen = _node(ax, 3.00, 4.05, 2.30, 0.86, "ours", "MLIR Dialect & Passes Gen", "targetgen scaffold", ts=10.5)
    ext = _node(ax, 3.00, 2.45, 2.30, 0.70, "external", "ModelBlaster / Autocomp", "autotuner", ts=10.5, tag="external")
    okp = _node(ax, 0.60, 1.15, 2.55, 0.80, "ours2", "Optimized Kernels & Opt Path", "mined result", ts=10.5)
    ...
    _narrow(ax, (ext["cx"], ext["t"] + 0.02), (gen["cx"], gen["b"] - 0.02), color=SLATE, lw=1.7, ls=(0, (4, 3)))
...
def fig_dual_dse():
    ...
    dse = _node(
        ...
        0.86,
        "external",
        "DSE HW Accelerator Tool",
        "external DSE engine",
        ts=10.5,
        tag="external",
    )
    props = _node(ax, 0.45, 5.30, 2.25, 0.72, "ours2", "DSE Propositions", "candidate HW points", ts=11)
    phw = _node(ax, 3.05, 5.30, 2.00, 0.72, "input", "Proposed HW", "candidate accelerator", ts=11)
    gen = _node(ax, 3.05, 4.05, 2.00, 0.86, "ours", "MLIR Dialect & Passes Gen", "targetgen scaffold", ts=10.5)
    behav = _node(ax, 0.45, 2.55, 2.25, 0.72, "ours2", "Compiled Behavioral Model", "functional", ts=10.5)
    perf = _node(ax, 0.45, 1.40, 2.25, 0.72, "ours2", "Compiled Perf Model", "cycle / cost model", ts=10.5)
    ext = _node(ax, 0.55, 0.50, 2.05, 0.62, "external", "Autocomp", "autotuner", ts=10, tag="external")
    # DSE tool ↔ propositions
    _narrow(ax, (dse["cx"], dse["b"] - 0.02), (props["cx"], props["t"] + 0.02), color=SLATE, lw=1.8, ls=(0, (4, 3)))
    ...
    _narrow(ax, (ext["cx"], ext["t"] + 0.02), (perf["cx"], perf["b"] - 0.02), color=SLATE, lw=1.7, ls=(0, (4, 3)))
...
# ============================================================================
def _code_card(ax, x, y, w, h, header, headcol, lines, sub=None, hfs=11.0, lfs=9.0, lh=0.30, z=4):
    ...
    ax.add_patch(box)
    hb = FancyBboxPatch(
        (x, y + h - 0.52), w, 0.52, boxstyle="round,pad=0,rounding_size=0", linewidth=0, facecolor=headcol, zorder=z + 1
    )
    ax.add_patch(hb)
    ax.text(
        x + 0.20,
...
def fig_kernel_input():
    ...
    _code_card(ax, 2.55, 0.75, 7.5, 4.55, "input.mlir", NAVY, lines, sub="linalg-on-tensors", lfs=9.6, lh=0.40)
    # green "input" pill
    pill = FancyBboxPatch(
        (2.55, 5.34),
        1.35,
    ...
    ax.add_patch(pill)
    ax.text(
        2.55 + 0.675, 5.55, "INPUT", ha="center", va="center", color="white", fontsize=9.5, fontweight="bold", zorder=7
    )
    ax.text(
        10.0,
        5.55,
        "out/artifacts/rvv_workloads/matmul_f32_64x64x64/model.mlir",
        ha="right",
        va="center",
        color=SLATE,
        fontsize=8.4,
    ...
    ax.text(
        6.3,
        0.30,
        "one op, one shape — we follow THIS kernel through the whole loop",
        ha="center",
        va="center",
        color=BLUE,
        fontsize=10.5,
...
def fig_asm_diff():
    ...
    _code_card(
        ...
        6.0,
        5.7,
        "Baseline  —  our compiler today",
        MAUVE,
        base,
        sub="separate mul + add",
        lfs=9.6,
        lh=0.44,
    )
    _code_card(ax, 8.75, 0.55, 6.0, 5.7, "Fused vfmacc  —  our fork", NAVY, impr, sub="impr_rvv_v5", lfs=9.6, lh=0.44)
    ...
    ax.add_patch(cb)
    ax.text(7.6, 4.05, "what\ndiffers", ha="center", va="center", color=BLUE, fontfamily=SERIF, fontsize=12, zorder=6)
    ax.text(
        7.6,
        3.25,
        "8 ops\n↓\n4 fused\nMACs",
    ...
    ax.text(
        7.6,
        0.22,
        "same numbers (cos = 1.0)  ·  fuse multiply+add into one vfmacc  ·  "
        "XNNPACK & OpenBLAS emit the same fused form — the structure we mined",
        ha="center",
        va="center",
        color=BLUE,
        fontsize=9.6,
...
def fig_journey_strip():
    ...
    for i, (lab, sub, fc, tc) in enumerate(steps):
        ...
        if i < n - 1:
            _narrow(ax, (cx + cw + 0.04, cy + ch / 2), (cx + cw + gap - 0.04, cy + ch / 2), lw=2.2, scale=14)
    ax.text(
        7.8,
        0.55,
        "one kernel, followed the whole way — each step is a committed artifact",
        ha="center",
        va="center",
        color=BLUE,
        fontsize=10.5,
...
def fig_why_we_win():
    ...
    axL.set_xticklabels(mdl, fontsize=10)
    axL.set_ylim(0, 15.5)
    ...
    axR.set_xticklabels(mdl, fontsize=10)
    axR.set_ylim(0, 2.0)
    ...
    fig.text(
        0.5,
        0.93,
        "The kernel is not the system — our GEMM is slower, our whole-model bitVLA still wins",
        ha="center",
        color=INK,
        fontfamily=SERIF,
        fontsize=15.5,
    ...
    fig.text(
        0.5,
        0.355,
        "We mine the experts' structural decisions into compiler passes — but the "
        "measured payoff is whole-model scheduling of the NON-matmul path on bitVLA, not a faster "
        "GEMM (our kernel loses everywhere).",
        ha="center",
        color=BLUE,
        fontsize=10.2,
        fontstyle="italic",
    ...
    mech = [
        (
            "1.  kernel: we lose (measured)",
            "ours ÷ XNNPACK matmul = 3.2× / 7.9× / 13.6×\nslower. register_block:null → MR never mined",
        ),
        (
            "2.  bitVLA whole-model: we win",
            "148 ms vs XNNPACK 167 (1.13×) /\nOpenBLAS 180 (1.22×) — the vf schedule, not GEMM",
        ),
    ...
    for i, (h, body) in enumerate(mech):
        ...
        card = FancyBboxPatch(
            ...
            linewidth=1.5,
            edgecolor=GOLD if i == 1 else "#d9cfc0",
    ...
    fig.text(
        0.5,
        0.018,
        "fairness: same-pass · each expert's BEST kernel (XNNPACK 7x4v · OpenBLAS "
...
def fig_action_classes():
    ...
    rows = [
        (
            ...
            "tried first (cheapest) — clang did NOT fuse (vfmacc = 0) → escalate",
            MAUVE,
        ),
        (
            "KNOB",
            ...
            "forkable today via schedule.mlir   ·   ~1.05× on K1",
            SAGE,
        ),
        (
            "HEURISTIC",
            SAGE,
            "compute.nr_is_vsetvlmax  /  mr_adapts_to_m",
            "schedule:NR=vsetvlmax   ·   MR=min(MR,M)",
            "forkable today   ·   small-N & M=1 matmuls now vectorize",
            SAGE,
        ),
        (
            "PASS",
            ...
            "the WINNER — a new lowering pattern   ·   7.9× on K1",
            GOLD,
        ),
    ]
    RX, RW = 1.30, 13.30
    ...
    ax.text(
        0.30,
        (ys[0] + ys[-1]) / 2,
        "escalate:  cheaper  →  more invasive",
        rotation=90,
        ha="center",
        va="center",
        color=SLATE,
    ...
    ax.text(
        7.7,
        0.62,
        "FLAG · KNOB · HEURISTIC are forkable TODAY (schedule.mlir + cflags);  "
        "PASS needs compiler code (a default-off impr_feature).  "
        "A 5th tier — CODEGEN — is a deferred dedicated micro-kernel.",
        ha="center",
        va="center",
        color=BLUE,
        fontsize=9.6,
