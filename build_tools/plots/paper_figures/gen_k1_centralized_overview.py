...
MANIFEST = json.loads((HERE / "k1_centralized_overview_20260907.json").read_text(encoding="utf-8"))
...
def by_model(rows: list[dict], name: str) -> dict | None:
    ...
    return next((row for row in rows if row["model"].lower().replace("_", "") == folded), None)


def decorate_panel(ax: plt.Axes, number: int, heading: str, subtitle: str) -> None:
    bbox = ax.get_position()
    card = FancyBboxPatch(
        (bbox.x0 - 0.012, bbox.y0 - 0.035),
        bbox.width + 0.024,
        bbox.height + 0.085,
        ...
        linewidth=0.8,
        zorder=-5,
        path_effects=[pe.SimplePatchShadow(offset=(5, -5), shadow_rgbFace="#9C8E82", alpha=0.18), pe.Normal()],
    )
    ...
    ax.text(
        0.0,
        1.045,
        f"\N{BLACK RIGHT-POINTING TRIANGLE} {subtitle}",
        transform=ax.transAxes,
        ha="left",
        va="center",
        color=ACCENT,
...
fig, axes = plt.subplots(1, 4, figsize=(18.0, 9.0), sharey=True, gridspec_kw={"width_ratios": [2.25, 1.25, 1.85, 1.25]})
...
for y0, row in zip(y, rows):
    ...
    ax.plot([lo, hi], [y0, y0], color="#A8A29B", linewidth=2.0, linestyle="--" if lower_bound else "-", zorder=1)
    ax.scatter(
        row["merlin_ms"],
        y0,
        marker=">" if lower_bound else "s",
        s=76,
        color=MERLIN,
        edgecolor="white",
        linewidth=0.8,
    ...
    ax.scatter(
        row["peer_ms"],
        y0,
        marker="D" if row["peer"] == "ExecuTorch" else "o",
        s=65,
        color=peer_color,
        edgecolor="white",
        linewidth=0.8,
    ...
    merlin_label = (
        (">" if lower_bound else "") + format_ms(row["merlin_ms"]) + ("\N{DOUBLE DAGGER}" if lower_bound else "")
    )
    ax.text(
        row["merlin_ms"] * 1.12,
        y0 + 0.16,
    ...
    ax.text(430000, y0, missing_label, ha="left", va="center", color=missing_color, fontsize=7.2, fontstyle="italic")
ax.set_xscale("log")
...
for y0, row in zip(y, rows):
    ...
    ax.barh(
        y0, max(ratio - 1.0, 0.015), left=1.0, height=0.52, color=color, edgecolor="#574F47", linewidth=0.6, hatch=hatch
    )
    prefix = ">" if row["merlin_kind"] == "lower_bound" else ""
    suffix = (
        "\N{DAGGER}"
        if row["model"] == "SmallLLaMA"
        else ("\N{DOUBLE DAGGER}" if row["merlin_kind"] == "lower_bound" else "")
    )
    ax.text(
        ratio * 1.07,
        y0,
        f"{prefix}{ratio:.2f}\N{MULTIPLICATION SIGN}{suffix}",
        ha="left",
        va="center",
        color=INK,
        fontsize=9,
...
for category, color in zip(categories, colors):
    ...
    bars = ax.barh(y, values, left=left, height=0.54, color=color, edgecolor="white", linewidth=0.7)
...
for y0, row in zip(y, rows):
    if row["profile"] is None:
        ...
        ax.text(
            50,
            y0,
            "current matched profile not measured",
            ha="center",
            va="center",
            color=MUTED,
            fontsize=7.5,
...
for y0, row in zip(y, rows):
    ...
    if (
        one is None
        or eight is None
        or one.get("merlin_ms") is None
        or eight.get("merlin_ms") is None
        or one.get("executorch_ms") is None
        or eight.get("executorch_ms") is None
    ):
        ax.text(50, y0, "not measured", ha="center", va="center", color=MUTED, fontsize=8, fontstyle="italic")
        continue
    ...
    ax.text(
        merlin_eff,
        y0 + 0.20,
        f"{merlin_speedup:.2f}\N{MULTIPLICATION SIGN}",
        ha="center",
        va="bottom",
        fontsize=8,
        color=MERLIN,
    ...
    ax.text(
        peer_eff,
        y0 - 0.20,
        f"{peer_speedup:.2f}\N{MULTIPLICATION SIGN}",
        ha="center",
        va="top",
        fontsize=8,
        color=EXECUTORCH,
...
fig.text(0.5, 0.015, MANIFEST["footnote"], ha="center", va="bottom", color="#625D57", fontsize=8.8, linespacing=1.35)
