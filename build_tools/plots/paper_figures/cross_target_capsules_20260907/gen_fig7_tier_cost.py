...
axes[0].text(-0.13, 1.04, "(a) functional tier", transform=axes[0].transAxes, fontweight="bold", color=BLUE)
...
for i, t in enumerate(TARGETS):
    ...
    if n:
        ...
        axes[1].text(i, 0.12, "N/A", ha="center", color=GOLD, fontweight="bold")
axes[1].set_yscale("log")
axes[1].set_ylim(0.02, max(v[1] for v in l3 if v[2]) * 2.0)
axes[1].set_ylabel("Timed L3 adapter wall time (s, log)")
axes[1].set_xticks(x, [LABELS[t] for t in TARGETS])
axes[1].text(-0.13, 1.04, "(b) RTL tier attempts", transform=axes[1].transAxes, fontweight="bold", color=BLUE)
...
axes[2].bar(x, fresh, 0.58, color=SAGE, edgecolor=INK, linewidth=1.3, zorder=3, label="fresh pass")
axes[2].bar(x, carried, 0.58, bottom=fresh, color=SLATE, edgecolor=INK, linewidth=1.3, zorder=3, label="carried pass")
axes[2].bar(
    x,
    unspecified,
    0.58,
    bottom=fresh + carried,
    color=GOLD,
    edgecolor=INK,
    linewidth=1.3,
    zorder=3,
    label="origin unspecified",
)
...
axes[2].text(-0.13, 1.04, "(c) evidence provenance", transform=axes[2].transAxes, fontweight="bold", color=BLUE)
