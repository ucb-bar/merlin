...
axes[0].text(
    -0.03, 1.025, "(a) pass rate in shared coarse strata", transform=axes[0].transAxes, color=BLUE, fontweight="bold"
)
c0 = fig.colorbar(im0, ax=axes[0], fraction=0.045, pad=0.03)
c0.set_label("Pass fraction")

positive = times[np.isfinite(times) & (times > 0)]
norm = matplotlib.colors.LogNorm(vmin=max(positive.min(), 0.03), vmax=positive.max())
...
for y in range(len(shared)):
    for x in range(3):
        ...
        axes[1].text(
            x,
            y,
            "—" if not np.isfinite(val) else f"{val:.1f}s",
            ha="center",
            va="center",
            fontweight="bold",
            fontsize=9,
...
axes[1].text(-0.03, 1.025, "(b) median L2 adapter time", transform=axes[1].transAxes, color=BLUE, fontweight="bold")
c1 = fig.colorbar(im1, ax=axes[1], fraction=0.045, pad=0.03)
