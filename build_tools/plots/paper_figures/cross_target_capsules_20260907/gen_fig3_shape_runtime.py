...
for ti, target in enumerate(TARGETS):
    ...
    for regime in regimes:
        vals = [
            r["l2_wall_s"] for r in caps if r["target"] == target and r["shape_regime"] == regime and r["l2_wall_s"]
        ]
        medians.append(np.median(vals) if vals else np.nan)
    ax.scatter(
        np.arange(len(regimes)) + offsets[ti],
        medians,
        s=62,
...
ax.text(-0.08, 1.04, "(a) shape regime", transform=ax.transAxes, fontweight="bold", color=BLUE)
...
for target in TARGETS:
    ...
    for status in sorted({r["status"] for r in rows}):
        ...
        ax.scatter(
            [r["input_elements"] for r in subset],
            [r["l2_wall_s"] for r in subset],
            s=34 if status == "pass" else 54,
            marker=target_markers[target] if status == "pass" else markers.get(status, "X"),
            facecolors=COLORS[target] if status == "pass" else "none",
            edgecolors=COLORS[target],
            linewidths=1.2,
            alpha=0.8,
            label=LABELS[target] if status == "pass" else None,
            zorder=3,
        )
ax.set_xscale("log")
ax.set_yscale("log")
...
ax.legend(fontsize=9)
ax.text(0.02, 0.98, "Open × = non-pass", transform=ax.transAxes, va="top", color=GOLD, fontweight="bold")
ax.text(-0.08, 1.04, "(b) problem size", transform=ax.transAxes, fontweight="bold", color=BLUE)
