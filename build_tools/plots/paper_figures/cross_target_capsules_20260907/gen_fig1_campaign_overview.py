...
for i, t in enumerate(TARGETS):
    vbars(axes[0], [i], [vals[i]], COLORS[t], width=0.58)
...
axes[0].text(-0.13, 1.04, "(a) coverage", transform=axes[0].transAxes, fontweight="bold", color=BLUE)
...
axes[1].bar(x, think, 0.58, color=GOLD, edgecolor=INK, linewidth=1.3, zorder=3, label="think + generate")
axes[1].bar(x, tool, 0.58, bottom=think, color=SLATE, edgecolor=INK, linewidth=1.3, zorder=3, label="tools + wait")
for i, total in enumerate(think + tool):
    block_shadow(axes[1], i - 0.29, 0, 0.58, total, z=2.4)
    axes[1].text(i, total + 0.25, f"{total:.1f} h", ha="center", fontweight="bold")
...
axes[1].text(-0.13, 1.04, "(b) activity", transform=axes[1].transAxes, fontweight="bold", color=BLUE)
...
axes[2].bar(x, cached, 0.58, color=SLATE, edgecolor=INK, linewidth=1.3, zorder=3, label="cached input")
axes[2].bar(x, fresh, 0.58, bottom=cached, color=GOLD, edgecolor=INK, linewidth=1.3, zorder=3, label="fresh input")
axes[2].bar(x, out, 0.58, bottom=cached + fresh, color=SAGE, edgecolor=INK, linewidth=1.3, zorder=3, label="output")
for i, total in enumerate(cached + fresh + out):
    block_shadow(axes[2], i - 0.29, 0, 0.58, total, z=2.4)
    axes[2].text(
        i,
        total + 5,
        f"{rows[TARGETS[i]]['cache_read_share'] * 100:.1f}% cached",
        ha="center",
        fontweight="bold",
        fontsize=8.5,
    )
...
axes[2].text(-0.13, 1.04, "(c) telemetry", transform=axes[2].transAxes, fontweight="bold", color=BLUE)
