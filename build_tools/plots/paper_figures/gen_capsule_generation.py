...
def derive() -> dict:
    ...
    for target in _targets():
        ...
        spec_path = REPO / "experiments" / "reference-data" / "phase0" / "conformance" / f"{target}.yaml"
...
def _json_path() -> Path:
    ...
    if existing:
        return existing[-1]
    return HERE / f"{STEM}_{datetime.now(UTC):%Y%m%d}.json"
...
def main(argv=None) -> int:
    ...
    if a.derive:
        data = derive()
        validate(data)
        path = HERE / f"{STEM}_{datetime.now(UTC):%Y%m%d}.json"
