...
REPO = repo_root()
BALANCE = (
    REPO / "out/artifacts/measurements/gsim_GemminiRocketConfig/gemmini/"
    "movement_balance_v1_20260908T184840Z_939a075/movement_balance.json"
)
BUFFERS = (
    REPO / "out/artifacts/perf-bench/gemmini/development_phase2_global_encoding_20260908/validation/canonical/default"
)
...
def collect() -> dict:
    ...
    for name in WORKLOADS:
        ...
        verdict = arithmetic_intensity(
            rep.routed_macs,
            cost.traffic_bytes,
            machine_macs_per_byte=balance["ridge_point"]["macs_per_byte_upper_bound"],
        )
        rows.append(
            {
                "model": name,
                "routed_macs": rep.routed_macs,
...
def render(data: dict, out_stem: Path) -> None:
    ...
    ax0.plot(
        grid,
        [base + slope * g for g in grid],
        color=PALETTE[1],
        lw=1.8,
        label=f"{fit['peak_bytes_per_cycle']:.3f} B/cycle",
    )
    ax0.axhline(base, color=PALETTE[4], ls=":", lw=1.4, label=f"fixed cost {base:.1f} cyc")
    ax0.set_xlabel("bytes moved")
    ax0.set_ylabel("cycles")
    ax0.set_title(f"Movement balance, measured\nn={fit['n_samples']} sizes, $r^2$={fit['r_squared']:.5f}")
    ax0.legend(loc="upper left", framealpha=0.9)
    ax0.annotate(
        "the intercept dominates this domain,\nso the slope is a MARGINAL rate",
        xy=(0.97, 0.06),
    ...
    ax1.barh(
        list(ypos),
        [r["macs_per_byte"] for r in order],
        color=colors,
        height=0.6,
    ...
    ax1.axvline(ridge, color=PALETTE[1], lw=1.8, label=f"ridge $\\leq$ {ridge:.2f} MACs/B (UPPER bound)")
    ...
    for y, row in zip(ypos, order):
        if not row["macs_per_byte"]:
            ax1.annotate(
                "0 routed MACs — all-host placement",
                xy=(0, y),
                xytext=(8, 0),
    ...
    ax1.set_yticklabels([r["model"] for r in order])
    ...
    idx = range(len(rows))
    ax2.bar(
        [i - 0.2 for i in idx], [r["on_unit"] for r in rows], width=0.4, color=PALETTE[0], label="contractions ON unit"
    )
    ax2.bar(
        [i + 0.2 for i in idx],
        [r["off_unit"] for r in rows],
        width=0.4,
        color=PALETTE[1],
        hatch="//",
        edgecolor="#8a7d68",
    ...
    twin = ax2.twinx()
    twin.plot(
        list(idx), [r["host_regions"] for r in rows], color=PALETTE[3], marker="o", lw=1.6, label="host-lane regions"
    )
    twin.set_ylabel("host-lane regions")
    twin.set_yscale("log")
    twin.grid(False)
    ...
    ax2.legend(
        handles + h2, texts + t2, loc="upper center", framealpha=0.9, bbox_to_anchor=(0.5, -0.16), ncol=3, fontsize=8
    )

    commit = (bal.get("provenance", {}).get("merlin", {}) or {}).get("commit", "")[:7]
    fig.suptitle(
        f"Gemmini phase-2 instruments, measured off the FPGA "
