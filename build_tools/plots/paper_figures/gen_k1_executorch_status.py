...
fig, grid = plt.subplots(2, 2, figsize=(11.0, 6.4), gridspec_kw={"wspace": 0.33, "hspace": 0.42})
axes = grid.ravel()
...
for x0, name, best, color in zip(xpos, names, values, colors):
    ...
    axes[0].scatter(x0 + jitter, runs, s=33, color=color, edgecolor="black", linewidth=0.45, zorder=3)
...
ratios = [
    (
        row["executorch_ms"] / (row["merlin_ms"] or row.get("merlin_lower_bound_ms"))
        if row["executorch_ms"] is not None and (row["merlin_ms"] is not None or row.get("merlin_lower_bound_ms"))
        else np.nan
    )
    for row in rows
]
...
for label, values, source_rows, offset, marker, color in (
    ...
    for y0, value in zip(y, values):
        if not np.isnan(value):
            ...
            axes[1].text(value + 0.025, y0 + offset, value_text, ha="left", va="center", fontsize=8.0)
...
config_markers = ["o", "^", "s"]
for offset, config, color, marker in zip((-0.14, 0.0, 0.14), scaling["merlin_configs"], config_colors, config_markers):
    medians = []
...
axes[2].set_xticks(x, [f"{core} core" if core == 1 else f"{core} cores" for core in scaling["cores"]])
axes[2].set_ylabel("Latency (ms; lower is better)")
all_merlin = [
    v for config in scaling["merlin_configs"] for sessions in config["sessions_ms"].values() for v in sessions
]
axes[2].set_ylim(0, max(all_merlin) * 1.42)
axes[2].legend(frameon=False, loc="upper right", fontsize=7.5)
axes[2].text(0.02, 0.98, "(c)", transform=axes[2].transAxes, ha="left", va="top", fontweight="bold")
...
axes[3].set_xticks(x, [f"{core} core" if core == 1 else f"{core} cores" for core in lstm["cores"]])
axes[3].set_yscale("log")
axes[3].set_ylabel("Latency (ms, log scale)")
axes[3].legend(frameon=False, loc="upper right", fontsize=7.5)
axes[3].text(0.02, 1.02, "(d)", transform=axes[3].transAxes, ha="left", va="bottom", fontweight="bold")
