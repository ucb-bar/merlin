...
def load_cost_by_problem():
    ...
    for r in csv.DictReader(open(IDX / "cost_by_problem.csv")):
        raw[(int(r["prob"]), r["run"])].append(
            (int(r["iteration"]), float(r["usd"]), int(r["input_tokens"]) + int(r["output_tokens"]))
        )
    series = {}
...
def fig1_spend_per_kernel():
    ...
    axL.legend(loc="upper left", fontsize=9, ncol=1, handlelength=2.4, borderpad=0.7, labelspacing=0.5)
...
def fig8_spend_per_kernel_with_total():
    ...
    fig, (axL, axR) = plt.subplots(1, 2, figsize=(15, 6.2), gridspec_kw=dict(width_ratios=[1.32, 1]))
    keys = sorted(SERIES, key=lambda k: (k[0], k[1]))
    ...
    for xi, c in zip(x[:-1], cum[:-1]):
        axR.annotate(
            f"${c:.0f}", (xi, c), xytext=(0, 9), textcoords="offset points", ha="center", fontsize=9, color=INK
        )
    axR.set_xticks(x)
...
def fig2_spend_by_model():
    ...
    colors = [
        MAUVE
        if k == "us.anthropic.claude-sonnet-4-6"
        else (NAVY if k == "gemini-3.5-flash" else (BLUE if k == "qwen.qwen3-coder-480b-a35b-v1:0" else SLATE))
        for k, _ in items
    ]
    ...
    emph(
        ...
        len(items) - 0.5,
        max(usd) * 1.08,
        f"total ${SPEND['total_usd']:.0f} · {SPEND['calls']:,} calls",
        color=GOLD,
        fs=12,
        ha="right",
        va="center",
...
def fig3_baseline_vs_final():
    ...
    axN.annotate(
        "matmul best = the golden SMEM\ntechnique (RTL-confirmed)",
        (0 + w / 2, net[0][1]),
        xytext=(0.46, net[0][0] * 0.86),
        fontsize=9,
        color=BLUE,
        fontweight="bold",
        ha="left",
        va="center",
        arrowprops=dict(arrowstyle="->", color=BLUE, lw=1.3, connectionstyle="arc3,rad=-0.2"),
    )
    from matplotlib.patches import Patch

    axT.legend(
        handles=[
            Patch(fc=MAUVE, ec=INK, label="baseline (Claude Code)"),
...
def fig3b_speedup_all():
    ...
    ax.text(
        len(sp) - 1,
        max(vals) * 1.06,
        "bars measured from 1.0× (no change)",
        ha="right",
        va="center",
        fontsize=9,
        color=INK,
    ...
    for xi, (p, v) in zip(x, sp):
        ...
        emph(ax, xi, v, f"{v:.2f}×", color=col, fs=11.5, ha="center", va="bottom") if PROB[p]["sig"] else ax.annotate(
            f"{v:.3f}×", (xi, v), xytext=(0, 4), textcoords="offset points", ha="center", fontsize=9.5, color=INK
        )
    ax.annotate(
        "P3 attn96, P4 swiglu, P9 gelu landed exactly 1.000× (unchanged)",
        (0, max(vals) * 1.06),
        fontsize=9,
        color=INK,
        ha="left",
...
def fig4_yield_funnel():
    ...
    axF.set_yticks(y)
    axF.set_yticklabels([s[0] for s in stages], fontsize=10.5)
    ...
    emph(
        ...
        total * 0.5,
        y[1],
        f"{100 * compcorr / total:.0f}% compile+correct",
        color=GOLD,
        fs=12,
        ha="center",
        va="center",
    ...
    style_ax(axB, grid="x")
    order = [("compile_error", MAUVE), ("regressed", SLATE), ("correct_no_gain", SAGE), ("improved", NAVY)]
    ...
    axB.set_yticks(yb)
    axB.set_yticklabels([k for k, _ in order], fontsize=10.5)
    axB.set_xlim(0, max(oc.values()) * 1.32)
    axB.set_xlabel(f"candidates (labeled subset, n={labeled})")
    title(axB, "Outcome breakdown*")
    axB.annotate(
        "compile errors dominated by\nregister oversubscription\n(>256 physical registers)",
        (oc["compile_error"], yb[0]),
        xytext=(max(oc.values()) * 0.95, 1.5),
        fontsize=9,
        color=BLUE,
        fontweight="bold",
        ha="right",
        va="center",
        arrowprops=dict(arrowstyle="->", color=BLUE, lw=1.2, connectionstyle="arc3,rad=0.25"),
    )
    fig.text(
        0.5,
        0.02,
        "* outcome labels exist only for the transform-ledger subset (probs 1,2,3,7); "
        "generated/correct counts span all 835 attempts.",
        ha="center",
        fontsize=8.3,
        color=INK,
        fontstyle="italic",
...
def fig5_cost_vs_gain():
    ...
    LBL = {0: (10, 12), 1: (-10, 14), 2: (8, 12), 3: (8, -18), 6: (10, 10), 10: (0, 14)}
    ...
    for p, usd, sp, tok in pts:
        ...
        size = 130 + tok / 1e5
        ax.scatter([usd], [sp], s=size, color=c, edgecolor=INK, linewidth=1.3, zorder=5, alpha=0.92)
        ax.annotate(
            f"P{p} {PROB[p]['short']}",
            (usd, sp),
            xytext=LBL[p],
    ...
    ax.text(
        ax.get_xlim()[1] * 0.98, 1.18, "bubble area ~ tokens", ha="right", fontsize=8.7, color=INK, fontstyle="italic"
    )
...
def fig6_utilization():
    ...
    emph(
        ax, (max(vals) + 100) / 2, (len(rows) - 1) / 2, "headroom to peak", color=GOLD, fs=12, ha="center", va="center"
    )
...
def fig7_cumulative_total():
    ...
    for xi, c in zip(x[:-1], cum[:-1]):  # last point labelled by the gold total callout
        ax.annotate(
            f"${c:.0f}", (xi, c), xytext=(0, 9), textcoords="offset points", ha="center", fontsize=9.5, color=INK
        )
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=10)
    ...
    ax.annotate(
        "late hybrid runs (Gemini-plan +\nQwen-code) drove the final climb",
        (x[-2] + 0.5, (cum[-2] + cum[-1]) / 2),
        xytext=(x[-3] - 0.2, cum[-1] * 0.82),
        fontsize=9,
        color=BLUE,
        fontweight="bold",
        ha="left",
