...
e2e = [
    ("bitvla", 2.525, 0.274, 9.22, "fused_vfmacc_tiled"),
...
labels = {
    "baseline": "ours-baseline (compiler)",
    "tiled": "ours-tiled (compiler)",
    "OpenBLAS": "OpenBLAS (expert)",
    "XNNPACK": "XNNPACK (expert)",
    "intrinsic": "ours-intrinsic (hand ceiling)",
}
for k in order:
    xs = [s for s, v in zip(shapes, k1[k]) if v is not None]
    ys = [v for v in k1[k] if v is not None]
    ax.plot(xs, ys, marker="o", ms=5, lw=2.2, color=C[k], ls="--" if k == "intrinsic" else "-", label=labels[k])
...
ax.set_ylabel("rdtime ticks  (lower = faster)")
...
ax.set_ylim(1e4, 5e8)
ax.set_xticks(x)
ax.set_xticklabels([f"{s}³" for s in sp_shapes])
...
ax.legend(
    handles=[
        Patch(color=C_OURS, label="ours-best (compiler)"),
...
fig.suptitle(
    "RVV GEMM — every measured comparison  (baseline · OpenBLAS · XNNPACK · ours)",
    fontsize=13,
    fontweight="bold",
    y=1.02,
)
...
def plot_op_coverage():
    ...
    col = {
        "xnnpack": C["XNNPACK"],
        "ours_scalar": C["baseline"],
        "ours_vectorize_nofeature": C["tiled"],
        "ours_vectorized": C["vfmacc"],
        "ours_f32_baseline": C["baseline"],
        "ours_int8_w8a8": C["intrinsic"],
        "ours_conv_baseline": C["baseline"],
        "ours_conv_vfmacc": C["vfmacc"],
        "ours_bmm_baseline": C["baseline"],
        "ours_bmm_vfmacc": C["vfmacc"],
        "ours_depthwise": C["tiled"],
    }
    nice = {
        "xnnpack": "XNNPACK",
        "ours_scalar": "ours-scalar",
        "ours_vectorize_nofeature": "ours-vec (no-feat)",
        "ours_vectorized": "ours-vectorized (poly)",
        "ours_f32_baseline": "ours-f32",
        "ours_int8_w8a8": "ours-int8-W8A8",
        "ours_conv_baseline": "ours-baseline",
        "ours_conv_vfmacc": "ours-vfmacc",
        "ours_bmm_baseline": "ours-baseline",
        "ours_bmm_vfmacc": "ours-vfmacc",
        "ours_depthwise": "ours-depthwise",
    }
    op_titles = {
        "gelu": "GELU (f32)",
        "sigmoid": "sigmoid (f32)",
        "int8_gemm": "int8 GEMM (qd8/W8A8)",
        "dwconv": "depthwise conv (f32 3x3)",
        "conv2d": "conv2d (f32, im2col→GEMM)",
        "attention_bmm": "attention bmm (ours-vs-ours)",
    }
    ops = [
        o
        for o in ["gelu", "sigmoid", "int8_gemm", "dwconv", "conv2d", "attention_bmm"]
    ...
    for idx, op in enumerate(ops):
        ...
        for si, s in enumerate(srcs):
            ...
            for hx in hatch_x:
                ...
                axh.text(hx, 1.3, "not_run", rotation=90, fontsize=6, ha="center", va="bottom", color="#999")
        axh.set_yscale("log")
        axh.set_xticks(xg)
        axh.set_xticklabels(sizes_seen, fontsize=8)
    ...
    figh.suptitle(
        "Cross-framework op coverage on real K1 silicon — GEMM + GELU + sigmoid + "
