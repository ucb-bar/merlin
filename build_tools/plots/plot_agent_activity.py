...
BAND = {"thinking": (BLUE_ := "#5C5C7A"), "reading": SLATE, "writing": GOLD, "bash": SAGE, "tool_wait": MAUVE}
...
def render(run_dir: Path, out: Path, bins: int) -> int:
    ...
    if not usable:
        ...
        if empty:
            why = (
                "every round parsed but produced no completed tool call to place on a timeline "
    ...
    ax.stackplot(xs, *[share[a] for a in ACTIVITIES], colors=[BAND[a] for a in ACTIVITIES], alpha=0.55, linewidth=0)
    ...
    if len(merged.tokens) < _MIN_RATE_SAMPLES:
        notes.append(
            f"token rate unavailable: only {len(merged.tokens)} usage sample(s) — this "
    ...
    if len(merged.tokens) >= _MIN_RATE_SAMPLES:
        ...
        for idx, (colour, lab) in ((1, (SLATE, "rate input")), (2, (NAVY, "rate output"))):
            ...
            deltas = [
                max(b - a, 0) / max(t2 - t1, 1e-6) for (a, b), (t1, t2) in zip(zip(series, series[1:]), zip(tm, tm[1:]))
            ]
    ...
    tot = merged.totals()
    ax.set_title(
        f"{run_dir.name}   {merged.wall_s / 60:.0f} min measured   "
        f"thinking {100 * tot['thinking'] / max(merged.wall_s, 1):.0f}%   "
        f"tool wait {100 * tot['tool_wait'] / max(merged.wall_s, 1):.0f}%",
        loc="left",
    )
    dropped = [t for t in parts if not t.measured]
    if dropped:
        notes.append(
            f"{len(dropped)} of {len(parts)} round(s) omitted — {dropped[0].basis}: "
    ...
    handles += [
        Line2D([0], [0], color=SLATE, lw=2, label="rate input"),
    ...
    print(
        f"wrote {out}  ({len(merged.spans)} spans over {merged.wall_s / 60:.1f} min, "
