...
_BASE_CAPABILITIES: dict[str, tuple[str, str, list[str]]] = {
    ...
    "chia": ("WS-B/D", "Chia public integration API", ["MERLIN_CHIA_PYTHON", "merlin-experiments[chia]"]),
...
def _probe(key: str) -> tuple[str, str]:
    ...
    try:
        ...
        if key == "chia":
            # Import-only probe in the selected environment: a Python executable alone
            # does not establish Chia compatibility. Never initialize Ray here.
            from merlin.benchharness.chia_bridge import chia_python

            chia_py = chia_python()
            probe = subprocess.run(
                [chia_py, "-I", "-c", "from merlin.benchharness.chia_bridge import require_chia; require_chia()"],
                capture_output=True,
                text=True,
                timeout=20,
            )
            return (
                "available" if probe.returncode == 0 else "unavailable",
                chia_py if probe.returncode == 0 else f"{chia_py}: {probe.stderr.strip()[-2000:]}",
            )
        if key == "llm_api":
...
def _interpreters() -> dict[str, str]:
    ...
    out: dict[str, str] = {}
    try:
        from merlin.benchharness.chia_bridge import chia_python, driver_python

        for label, resolve in (
            ("driver (MERLIN_EXPERIMENT_PYTHON)", driver_python),
            ("chia (MERLIN_CHIA_PYTHON)", chia_python),
        ):
            try:
                out[label] = resolve()
            except RuntimeError as exc:
                out[label] = f"MISSING: {exc}"
    except ImportError:
        out["experiments"] = "MISSING: install merlin-experiments for workflow interpreters"
