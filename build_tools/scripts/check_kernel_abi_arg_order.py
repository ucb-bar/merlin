...
WHAT IT PROVES, AND WHAT IT DOES NOT. Default mode proves the *document* and the *emitted harness*
agree, per shape, for a buffer of that shape. ``--static`` only validates contract rows, probe shapes,
and token resolution; it does not certify any harness. It does not claim the orders are alike — they are not, and the
...
def _probe_movement() -> dict:
    return {
        "abi_version": "0.1",
        "tensors": {
            "S": {"shape": [4, 4], "dtype": "i8", "role": "input"},
            "D": {"shape": [4, 4], "dtype": "i32", "role": "output"},
        },
        "commands": [
            {"opcode": "MOVEMENT", "operands": {"src": "S", "dst": "D"}, "attributes": {"output_dtype": "i32"}}
        ],
    }
...
def _probe_whole_program() -> dict:
    ...
    return {
        ...
        "commands": [
            ...
            {"opcode": "MATMUL_RESIDENT", "operands": {"lhs": "A0", "rhs": "R", "dst": "acc0"}},
            {
                "opcode": "COMMIT",
                "operands": {"src": "acc0", "dst": "mid"},
                "attributes": {"epilogue": [], "output_dtype": "i8"},
            },
            {"opcode": "MATMUL_RESIDENT", "operands": {"lhs": "mid", "rhs": "R", "dst": "acc1"}},
            {
                "opcode": "COMMIT",
...
def _probe_native_whole_op(opcode: str) -> dict | None:
    ...
    if opcode == "CONV2D":
        return {
            "abi_version": "0.1",
            "tensors": {
                "IFM": {"shape": [1, 4, 4, 4], "dtype": "i8", "role": "input"},
    ...
    if opcode in ("ATTENTION_QK", "ATTENTION_PV"):
        ...
        return {
            ...
            "commands": [
                {"opcode": opcode, "operands": {a: "A", b: "B", "dst": "Y0"}, "attributes": {"output_dtype": "i32"}}
            ],
        }
    if opcode == "BATCHED_MATMUL":
        return {
            "abi_version": "0.1",
            "tensors": {
                "A": {"shape": [2, 3, 5], "dtype": "i8", "role": "input"},
...
def _probe_resident_matmul() -> dict:
    ...
    t = {
        "W0": {"shape": [16, 16], "dtype": "i8", "role": "weight"},
    ...
    cmds: list[dict] = [
        {"opcode": "RES_PACK", "operands": {"src": "W0", "dst": "R0"}, "attributes": {"layout": "packed_rhs"}},
        {"opcode": "RES_PACK", "operands": {"src": "W1", "dst": "R1"}, "attributes": {"layout": "packed_rhs"}},
    ]
...
def _resident_groups(cb: dict) -> list[tuple[str, list[tuple[str, str]]]]:
    ...
    commit_of = {
        (cmd.get("operands") or {}).get("src"): (cmd.get("operands") or {}).get("dst")
        for cmd in cb.get("commands", [])
...
def resolve_token(token: str, cb: dict) -> list[str]:
    ...
    if token == "interface_external_tensors_in_declaration_order":
        return [name for name, spec in (cb.get("tensors") or {}).items() if (spec or {}).get("role") in _EXTERNAL_ROLES]
    ...
    raise Unresolvable(
        f"unknown arg_order token {token!r} — this gate cannot certify a shape whose "
...
def _renderers() -> list[tuple[str, object, object]]:
    """``[(target, defining_module, render_harness), ...]`` for explicitly selected OOT backends.
...
def check(verbose: bool = False, *, static: bool = False) -> list[str]:
    ...
    if not isinstance(rows, list) or not rows or not symbol_pattern:
        return [
            f"{_CONTRACT.name}: kernel_abi.arg_order_by_command_shape / .symbol could not be read "
    ...
    if any(not isinstance(row, dict) for row in rows):
        return [f"{_CONTRACT.name}: a shape row is not a mapping — NOT certified"]
    declared_tokens = {t for row in rows for t in (row.get("order") or [])}
    undocumented = sorted(declared_tokens - set(tokens_doc))
    if undocumented:
        problems.append(
            f"{_CONTRACT.name}: arg_order token(s) {undocumented} are used by a shape row "
            f"but absent from arg_order_tokens"
        )
    orphan = sorted(set(tokens_doc) - declared_tokens)
    if orphan:
        problems.append(
            f"{_CONTRACT.name}: arg_order_tokens define(s) {orphan}, which no shape row "
    ...
    for row in rows:
        ...
        if (
            ...
            or not order
            or any(not isinstance(t, str) or not t for t in order)
        ):
            problems.append(f"{_CONTRACT.name}: a shape row is missing `shape` or `order`: {row!r}")
            continue
        if shape in by_shape:
            problems.append(f"{_CONTRACT.name}: duplicate arg_order row for {shape!r}")
        by_shape[shape] = row
    required_shapes = ("whole_program", "movement", "native_whole_op", "resident_matmul")
    for required in required_shapes:
        if required not in by_shape:
            problems.append(
                f"{_CONTRACT.name}: no arg_order row for the {required!r} command shape — "
                f"a renderer dispatches to it, so it cannot go undocumented"
            )

    for shape in sorted(set(by_shape) - set(required_shapes)):
        problems.append(f"{_CONTRACT.name}: no probe exists for command shape {shape!r} — NOT certified")

    # Probe and resolve every declared shape even during the unconditional static gate. Static
    # success only establishes that the contract is inspectable, never that a renderer matches it.
    ...
    if "native_whole_op" in by_shape:
        opcodes = by_shape["native_whole_op"].get("opcodes")
        if not isinstance(opcodes, list) or not opcodes or len(set(opcodes)) != len(opcodes):
            problems.append(f"{_CONTRACT.name}: native_whole_op.opcodes must be a nonempty unique list")
        else:
            for opcode in opcodes:
                probe = _probe_native_whole_op(str(opcode))
                if probe is None:
                    problems.append(
                        f"the contract declares whole-op opcode {opcode!r} that this gate has "
                        "no probe buffer for — NOT certified"
                    )
                    continue
    ...
    expected_by_probe: list[tuple[str, dict, list[str]]] = []
    for shape, cb in probes:
        expected: list[str] = []
        try:
            for token in by_shape[shape]["order"]:
                expected.extend(resolve_token(token, cb))
        except Unresolvable as exc:
            problems.append(
                f"{shape}: the contract's order could not be resolved against the probe buffer ({exc}) — NOT certified"
            )
            continue
        produced = {
            str((cmd.get("operands") or {}).get("dst"))
            for cmd in cb.get("commands", [])
            if cmd.get("opcode") == "COMMIT"
        }
        if not expected or any(name not in cb.get("tensors", {}) and name not in produced for name in expected):
            problems.append(
                f"{shape}: resolved pointer order {expected} is empty or names an undeclared tensor — NOT certified"
            )
            continue
        expected_by_probe.append((shape, cb, expected))
        if static and verbose:
            print(f"  static {shape}: {', '.join(expected)}")
    if static:
        return problems
    ...
    if not renderers:
        problems.append(
            "no explicitly selected OOT backend exposes a render_harness; select a reviewed "
            "provider via MERLIN_TARGET_PATH — dynamic gate did NOT run and does NOT "
            "report success"
        )
        return problems
    ...
    for target, backend, render in renderers:
        ...
        if row is not None:
            if code_ops is None:
                problems.append(
                    f"{target}: backend exposes no {_WHOLE_OP_ATTR}, so the whole-op opcode "
        ...
        for shape, cb, expected in expected_by_probe:
            ...
            if got is None:
                problems.append(
                    f"{target}/{shape}: no parseable {symbol}(...) call in the rendered harness — NOT certified"
                )
                continue
            if got != expected:
                problems.append(
                    f"{target}/{shape}: harness passes {got}, contract declares {expected} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if problems:
        ...
        return 1
    if a.static:
        print("kernel ABI static contract/probe checks OK; OOT renderer comparison NOT run")
    else:
        print("kernel ABI argument order OK: every selected OOT harness matches the contract")
    return 0
