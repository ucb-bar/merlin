...
def elf_census(elf: Path) -> dict:
    ...
    return {
        ...
        "forward_families": fam_fwd,
        "whole_elf_families": fam_all,
...
def build(bundle: Path, pkg, work: Path, honor: bool, max_session_steps: int | None) -> Path:
    ...
    return k1.build_k1_binary(
        bundle, work, pkg, fallback_policy="forbid", max_session_steps=max_session_steps, honor_pkg_cflags=honor
    )
...
def main(argv=None) -> int:
    ...
    for tag, honor in arms.items():
        try:
            ...
            report["arms"][tag] = {"honor_pkg_cflags": honor, "build_error": f"{type(e).__name__}: {e}"}
            continue
        elves[tag] = elf
        report["arms"][tag] = {"honor_pkg_cflags": honor, "elf": str(elf), "static": elf_census(elf)}
    ...
    if len(elves) == 2:
        ...
        report["static_delta"] = {
            ...
            "ours_share_of_baseline_vector_pct": round(100.0 * fv / bv, 2) if bv else None,
            "clang_share_of_baseline_vector_pct": round(100.0 * (bv - fv) / bv, 2) if bv else None,
            "forward_families": {"clang_on": b["forward_families"], "honoured": f["forward_families"]},
            "libm_call_sites": {"clang_on": b["libm_call_sites"], "honoured": f["libm_call_sites"]},
    ...
    if not a.build_only and len(elves) == 2:
        ...
        if any(d is None for d in b_d + f_d) or not b_d or not f_d:
            report["correctness"] = {
                "decided": False,
                "reason": "a run produced no output digest; a missing digest is refused as firmly as a mismatch",
            }
        else:
            report["correctness"] = {
                "decided": True,
                ...
                "baseline_digests": sorted(set(b_d)),
                "honoured_digests": sorted(set(f_d)),
            }
        ...
        if all(walls.values()):
            ...
            report["wall"] = {
                ...
                "note": "NOT a speed claim. A ratio inside the board's own noise floor is not a result.",
            }
