...
def _rvv_coverage(binary: Path) -> dict | None:
    ...
    try:
        ...
        return {
            "symbol": "forward",
            "vector_insns": fwd.vector,
            "scalar_compute_insns": fwd.scalar_compute,
            "static_vector_coverage": fwd.coverage,
            "note": (
                "STATIC instruction coverage of the emitted `forward` (one inlined "
...
def gate_run(
    prefix, golden_refs: dict, *, cos_th: float | None, rel_th: float | None, tag: str
    ...
    ok = bool(g.get("ok"))
    detail = f"tiers={g.get('tiers')} tier_ok={g.get('tier_ok')} cos={g.get('cos')} rel={g.get('rel')}"
    ...
    if not g.get("comparison_complete", True):
        ...
        print(
            f"  [{tag}] PARTIAL OUTPUT: the gate compared {g.get('n_compared')} of "
...
def run_profiled(
    ...
    for i in range(n):
        ...
        try:
            ...
            ok, why, gate = gate_run(res["prefix"], golden_refs, cos_th=cos_th, rel_th=rel_th, tag=f"on {i}")
            gates.append(gate)
            if not ok:
                blocker = why
                continue
            ...
            print(
                f"  [on {i}] wall_ns={m['wall_ns']} execs={execs} "
    ...
    if table is None or gated == 0:
        return {"ok": False, "blocker": blocker or "no gated run", "n_gated": 0, "gates": gates}
    ...
    return {
        ...
        "executions_per_timed_iteration": opf.executions_per_iteration(execs_per_launch, max(1, iters), warmup),
...
def run_unprofiled(
    ...
    for i in range(n):
        ...
        try:
            ...
            ok, why, gate = gate_run(res["prefix"], golden_refs, cos_th=cos_th, rel_th=rel_th, tag=f"off {i}")
            gates.append(gate)
            if not ok:
                blocker = why
                continue
...
def build_breakdown(model_dir: Path, prof: dict, unprof: dict, noise_pct: float) -> dict:
    ...
    wall_ms = (
        _ticks_to_ms(wall_ticks_per_exec) if wall_ticks_per_exec and coverage["runtime_shares_reportable"] else None
    )
    cov_scalar = coverage["profiler_coverage"]

    by_family = opf.rollup(table, lambda r: r["family_resolved"], "family", wall_ms=wall_ms, coverage=cov_scalar)
    ...
    by_category = opf.rollup(table, lambda r: r["category"], "category", wall_ms=wall_ms, coverage=cov_scalar)
    by_op = opf.rollup(table, lambda r: r.get("mlir_op") or "(none)", "mlir_op", wall_ms=wall_ms, coverage=cov_scalar)
    by_key = opf.rollup(table, lambda r: r["join_key"], "join_key", wall_ms=wall_ms, coverage=cov_scalar)
    by_aten = opf.rollup(table, lambda r: r.get("aten") or "(none)", "aten", wall_ms=wall_ms, coverage=cov_scalar)
    ...
    if pon and poff:
        ...
        pert = {
            "profiled_median_wall_ns": pon,
            "unprofiled_median_wall_ns": poff,
            "delta_pct": round(delta_pct, 3),
            "noise_floor_pct": noise_pct,
            "perturbation_ok": delta_pct <= noise_pct,
            "note": (
                "delta = profiled/unprofiled-1. Instrumentation can only add cost, so the "
    ...
    quantize_vs_contraction = {
        ...
        "note": (
            "Measured ms, not instructions. Both sides are LOWER BOUNDS in opposite ways: an "
    ...
    return {
        ...
        "wall_ms_per_execution": (round(_ticks_to_ms(wall_ticks_per_exec), 6) if wall_ticks_per_exec else None),
        "executions_per_timed_iteration": per_iter,
        "coverage": coverage,
        ...
        "vectorized_flag_meaning": (
            "`vectorized`/`contraction_intended_ms` reflect the pipeline's "
...
def main() -> None:
    ...
    ap.add_argument("-n", type=int, default=3, help="separate build+deploy launches; the min across them is reported")
    ap.add_argument(
        "--iters",
        type=int,
        default=1,
    ...
    if base.is_int8:
        ...
        if not w8a8.is_file():
            raise SystemExit(
                f"{a.baseline} is an int8 package but {md.name} ships no "
    ...
    print(f"[features] {feats or '<baseline>'} ({'from the package' if a.features is None else 'from --features'})")
    ...
    summary: dict = {
        ...
        "max_session_steps": a.max_session_steps,
        "dump_cap": a.dump_cap,
        "golden_tiers": sorted(golden_refs),
        "method": (
            "Per-op rdtime marks interleaved between the top-level ops of @forward "
    ...
    out = (
        Path(a.out)
        if a.out
        else (artifacts_dir() / "measurements" / "k1_spacemit" / md.name / f"op_profile_{ftag}.json")
    )
    ...
    if b:
        ...
        print(f"\n=== WHERE THE WALL GOES (measured ms, share {denom}, profiler_coverage={c['profiler_coverage']}) ===")
        for row in b["by_category"]:
            share = row["share_of_runtime"] if ok else row["share_of_attributed"]
            print(f"  {row['category']:<24} {row['ms']:8.2f} ms  {100 * (share or 0):5.1f}% n_ops={row['n_ops']}")
        print(
            "  (categories this profiler CANNOT attribute: "
            + ", ".join(sorted(b["categories_not_attributable"]))
            + " — see the artifact)"
        )
        ...
        for row in b["by_family"][:12]:
            ...
            print(
                f"  {row['family']:<20} {row['ms']:8.2f} ms  {100 * (share or 0):5.1f}% "
        ...
        print(
            f"attributed={b['total_attributed_ms']:.2f}ms/execution over "
        ...
        if ok:
            print(
                f"scalar={b['scalar_ms']:.1f}ms ({b['scalar_share_of_runtime'] * 100:.1f}% of "
            ...
            print(
                f"   shares above are of ATTRIBUTED op time ({b['total_attributed_ms']:.2f} ms), "
                f"NOT of the {b['measured_wall_ms']} ms wall — do not quote them as runtime %."
            )
            print(
                f"scalar={b['scalar_ms']:.1f}ms "
        ...
        if c["executions_exceed_timed_iterations"]:
            ...
            print(
                f"   note: the shim counted {c['executions_per_launch']} @forward executions "
        ...
        if b["contraction_is_a_lower_bound"]:
            print(
                f"   contraction is a LOWER BOUND: {b['unclassified_generic_ms']:.2f} ms sits in "
