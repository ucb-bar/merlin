...
def _default_corpus_target() -> str:
    ...
    Found by elimination -- the one declared profile alias with no corpus directory -- so this file names no
    ...
    names = [item.profile for item in all_declarations()]
    if not names:
        raise ValueError("cert-affordability requires a nonempty Phase 0 declaration inventory")
...
def _resolved_target(label: str) -> str:
    ...
    """
    if any(label in (item.target, item.profile) for item in all_declarations()):
        # Only an absent declaration licenses the historical registry path. Ambiguous selectors
        # and malformed catalog entries must propagate instead of borrowing another provider.
        return for_target(label).target
    try:
...
def _engine_fit(target: str, cache: dict):
    ...
    if target not in cache:
        selected_target = _resolved_target(target)
        try:
            cache[target] = CA.fits_for(selected_target)
...
def _cheaper_tier(label: str, cache: dict):
    ...
    ``corpus.admission.constructed_oracle_tiers`` -- so this cannot name a tier the run does not have, and so
    no tier name is spelled here. Unresolvable is ``_TIER_UNKNOWN``, never a cap.
    """
    if label not in cache:
        try:
            from merlin_experiments.corpus.admission import constructed_oracle_tiers

            tiers = [str(t) for t in (constructed_oracle_tiers(label) or ())]
