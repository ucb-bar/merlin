...
def split_forward_by_mark(dis_text: str, *, symbol: str = "forward", mark_sym: str = MARK_SYM) -> dict:
    ...
    unresolved = 0
    found = False
    regions: dict = defaultdict(lambda: {"vector": 0, "scalar": 0, "total": 0, "vsetvl": 0, "mark_overhead": 0})
...
def structural_class(op_text: list[str], *, mlir_op: str, result_type: str | None, lanes: int) -> dict:
...
def strip_marks(instrumented_text: str, *, mark_sym: str = MARK_SYM) -> str:
    ...
    for line in instrumented_text.splitlines():
        s = line.strip()
        if (
            s.startswith("%prof_id_")
            or s.startswith(f"call @{mark_sym}")
...
def classify_prepared(instrumented_mlir: Path, *, lanes: int) -> dict:
    ...
    for n, o in enumerate(ops):
        out[o["id"]] = structural_class(
            lines[bounds[n] : bounds[n + 1]], mlir_op=o["mlir_op"], result_type=o["result_type"], lanes=lanes
        )
    return out
...
def _object_view_fraction(obj: Path) -> dict | None:
    ...
    return {
        "object_forward_coverage": fwd.coverage,
        ...
        "fraction_of_body_visible_as_forward": ((fwd.vector + fwd.scalar_compute) / body if body else None),
    }
...
def _elf_forward(binary: Path) -> dict:
    ...
    return {
        ...
        "vector": fwd.vector,
        "scalar_compute": fwd.scalar_compute,
        "scalar_int": fwd.scalar_int,
        "scalar_float": fwd.scalar_float,
        "vsetvl": fwd.vsetvl,
        "total": fwd.total,
    }
...
def analyse(
    model_dir: Path, pkg, work_root: Path, *, objdump: str | None, max_drift: float, lanes: int, reuse: bool = False
) -> dict:
    ...
    on = work_root / "on"
    off = work_root / "off"
    have = (on / "merlin_k1").is_file() and (off / "merlin_k1").is_file() and (on / "opprof_table.json").is_file()
    if reuse and have:
        bin_on, bin_off = on / "merlin_k1", off / "merlin_k1"
    else:
        bin_on = k1.build_k1_binary(model_dir, on, replace(pkg, run_id=f"sr_on_{model_dir.name}"), op_profile=True)
        bin_off = k1.build_k1_binary(model_dir, off, replace(pkg, run_id=f"sr_off_{model_dir.name}"), op_profile=False)
    ...
    attributable = (
        split["found"]
        and split["monotonic"]
        and split["dense_from_zero"]
        and not split["duplicate_ids"]
        and not split["unresolved_marks"]
        and len(split["order"]) == len(table) + 1
    )
    out = {
        "model": str(model_dir),
        "package": getattr(pkg, "run_id", None),
        "compiler_features": sorted(pkg.compiler_features or []),
        "binary_instrumented": str(bin_on),
        "binary_control": str(bin_off),
        "forward_control": fwd_off,
        "forward_instrumented": fwd_on,
        "object_view_control": _object_view_fraction(off / "model.o"),
        "marks": len(split["order"]),
        "ops_in_table": len(table),
        "unresolved_marks": split["unresolved_marks"],
        "layout_monotonic": split["monotonic"],
        "duplicate_ids": split["duplicate_ids"],
        "ids_dense_from_zero": split["dense_from_zero"],
        "perturbation_coverage_delta": drift,
        "perturbation_ok": drift <= max_drift,
        "max_coverage_drift": max_drift,
        "attributable": attributable,
        "refusal": None
        if attributable
        else "layout/id check failed -- the recovered mark ids are not exactly 0..N "
...
def _print(res: dict) -> None:
    ...
    print(
        f"  forward (LINKED ELF, shipped build): vector={c['vector']} scalar={c['scalar_compute']} "
        f"coverage={c['coverage']:.4f}"
    )
    ov = res.get("object_view_control")
    if ov:
        print(
            f"  forward (UNRELOCATED OBJECT view): coverage={ov['object_forward_coverage']:.4f} on "
    ...
    print(
        f"  instrumented forward coverage={i['coverage']:.4f} "
    ...
    for axis, title in (
        ...
        for row in rows[:14]:
            print(
                f"  {row[axis][:38]:38s} {row['n_ops']:6d} {row['scalar']:8d} "
                f"{row['share_of_scalar_static']:6.1%} {row['share_of_scalar_elems']:9.1%} "
                f"{row['vector']:8d}  {row['n_all_scalar_ops']}/{row['n_ops']}"
            )
...
def main() -> None:
    ...
    ap.add_argument("--model", action="append", required=True, help="recapture bundle dir (repeatable)")
    ap.add_argument("--package", default="out/artifacts/targets/rvv/hand_v0_int8")
    ap.add_argument(
        "--features",
        default=None,
        help="comma-separated compiler features; omitted = the package's own, empty string = the frozen baseline",
    )
    ...
    ap.add_argument(
        "--reuse-builds", action="store_true", help="reuse a complete pair of binaries already in the work dir"
    )
    ...
    for md in a.model:
        ...
        res = analyse(
            md, pkg, wr, objdump=a.objdump, max_drift=a.max_coverage_drift, lanes=a.lanes, reuse=a.reuse_builds
        )
        _print(res)
        results.append(res)
    ...
    prod = (
        ...
        if a.out
            ...
            "scalar-remainder", version=1, notes="static per-op attribution of forward's instruction stream"
        ).path
    )
    prod.mkdir(parents=True, exist_ok=True)
    (prod / "scalar_remainder.json").write_text(
        json.dumps({"package": a.package, "compiler_features": feats, "models": results}, indent=1)
    )
