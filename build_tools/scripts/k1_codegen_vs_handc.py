...
def _spread(walls):
    ...
    return {
        "min_ns": ws[0],
        "max_ns": ws[-1],
        "median_ns": ws[len(ws) // 2],
        "stdev_ns": round(std),
        "range_pct": round(100.0 * (ws[-1] - ws[0]) / ws[0], 2),
        "n": len(ws),
    }


def run_arm(
    model_dir: Path, base_pkg, golden, n: int, tag: str, features: list[str], kernel_backend: str | None
) -> dict:
    ...
    for i in range(n):
        ...
        try:
            ...
            print(
                f"  [{tag}] run {i}: wall={runs[-1]['wall_ns']} "
    ...
    return {
        "tag": tag,
        "run_id": pkg.run_id,
        "compiler_features": list(features),
        "kernel_backend": kernel_backend,
        "dispatch_timing": dispatch_timing,
        "n_routed": n_routed,
        "min_wall_ns": walls[0] if walls else None,
        "spread": _spread(walls),
        "matmul_bucket_ns": mm_ns,
        "dispatch_bucket_ns": disp_ns,
...
def _attribute(r: dict) -> dict:
    ...
    out = {
        ...
        "shim_matmul_reproducibility_pct": (round(100.0 * abs(mD - mC) / mC, 2) if (mC and mD) else None),
        "control_vs_C_pct": (round(100.0 * (ctrl - C) / C, 2) if (ctrl and C) else None),
        "note": (
            "codegen_matmul = B.wall - D.dispatch_bucket: D's non-matmul == B's non-matmul "
    ...
    if gap:
        out["share_non_matmul_pct"] = round(100.0 * non_mm / gap, 1) if non_mm is not None else None
        out["share_matmul_pct"] = (
            round(100.0 * (cg_mm - mC) / gap, 1) if (cg_mm is not None and mC is not None) else None
        )
    return out
...
def run_workload(
    model_dir,
    baseline_pkg="out/artifacts/targets/rvv/hand_v0",
    n=3,
    arms="A_base,B_vf,C_base_shim,D_vf_shim,ctrl",
    out=None,
    ...
    summary = {
        ...
        "design": (
            "2x2 factorial: {baseline, vf} lowering x {our codegen, hand-C shim} matmul. "
