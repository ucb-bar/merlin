...
def _emitted_facts(model_o: Path, keep: Path) -> dict:
    ...
    try:
        p = subprocess.run(
            [objdump_bin(), "-d", "--mattr=+v", str(model_o)], capture_output=True, text=True, timeout=180
        )
...
def _run_ours(S: int, reps: int, tag: str, keep: Path, feats: list[str]) -> dict:
    ...
    binp = keep / f"{tag}.elf"
    srcs = [
        str(X.HERE / "ours_gemm_f16_driver.c"),
...
def _run_xnn_f16(S: int, reps: int, tag: str, keep: Path) -> dict:
    ...
    cmd = [
        ...
        *cflags,
        ...
        "-o",
        str(binp),
        str(X.HERE / "xnnpack_gemm_f16_driver_7x4v.c"),
        "-lm",
    ]
    try:
        p = subprocess.run(cmd, capture_output=True, text=True, timeout=600)
...
def main() -> int:
    ...
    ap.add_argument(
        "--control", action="store_true", help="also run the SAME binary a second batch for the noise-floor control"
    )
    ...
    for S in shapes:
        ...
        if a.control and r["status"] == "pass":
            ...
            rc["control_of"] = tag
            print("   ", rc["status"], "ticks=", rc.get("ticks"), "spread%=", rc.get("run_spread_pct"), flush=True)
            rows.append(rc)
