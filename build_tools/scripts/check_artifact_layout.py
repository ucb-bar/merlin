...
# The gate may run without an editable install. Only the lightweight core is
# needed for the same storage declaration/validation used by product writers.
sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "src"))

from merlin.common.artifacts import declared_product_roots  # noqa: E402
from merlin.common.yaml import load_yaml  # noqa: E402
...
def _force_added_into_ignored(root: Path) -> list[str]:
    ...
    added = subprocess.run(
        ["git", "diff", "--cached", "--name-only", "--diff-filter=AR", "--", "out/"],
        cwd=root,
        capture_output=True,
        text=True,
        check=True,
...
def _product_versions(root: Path):
    """Inspect declared product homes and one optional target axis, never recurse.

    Keep the legacy immediate-home scan for pre-contract products. Nested homes
    come from the writer's declaration, not another list of special topic names.
    """
    art = root / "out/artifacts"
    contract_path = root / "merlin/contract/storage.yaml"
    declared = load_yaml(contract_path) if contract_path.is_file() else {}
    if not isinstance(declared, dict):
        raise ValueError(f"{contract_path}: storage declaration must be a mapping")
    homes = {art / name for name in declared.get("concerns", {})}
    homes.update(art / relative for relative in declared_product_roots(declared).values())
    versions = set(art.glob("*/v*"))
    for home in homes:
        versions.update(home.glob("v*"))
        versions.update(home.glob("*/v*"))
    seen = set()
    for path in sorted(versions):
        if path.is_dir() and path.name[1:].isdigit() and path.resolve() not in seen:
...
def check(root: Path, staged: bool) -> list[str]:
    ...
    art = root / "out" / "artifacts"
    if art.is_dir():
        for v in _product_versions(root):
...
def main(argv: list[str]) -> int:
    ...
    except (OSError, ValueError, YAMLError, subprocess.CalledProcessError) as exc:
        reason = (
