...
_CAMPAIGN_SOURCES = (
    "build_tools/scripts/k1_int8_et_campaign.py",
    INSTRUMENT,
    "packages/merlin-analysis/src/merlin/compare/et_campaign.py",
    "packages/merlin-analysis/src/merlin/compare/executorch_column.py",
)
...
def _write_summary(outdir: Path, ledger: Path, product) -> dict:
    ...
    try:
        summary["provenance"] = _prov.record(sources=[str(repo_root() / s) for s in _CAMPAIGN_SOURCES])
...
def main() -> int:
    ...
    ap.add_argument("--models", default=",".join(DEFAULT_MODELS), help="comma-separated cells, cheapest-first")
    ...
    ap.add_argument(
        "--out-dir", default=None, help="resume into an existing campaign product dir; omitted = create a new one"
    )
    ap.add_argument(
        "--dry-run",
        action="store_true",
        help="resolve and price every cell, print what WOULD run, touch no board and write no artifact",
    )
    ...
    ap.add_argument(
        "--board-usable-bytes",
        type=int,
        default=ec.DEFAULT_BOARD_USABLE_BYTES,
    ...
    if a.dry_run:
        ...
        for p in plans:
            ...
            print(
                f"    ours bundle     : {p.ours_bundle_id}"
                + (f"  [layout-only rewrite of {p.reference_bundle_id}]" if p.layout_equivalence else "")
            )
            print(f"    goldens         : {p.goldens}")
            print(f"    w8a8 reference  : {p.w8a8_reference['status']} (independent={p.w8a8_reference['independent']})")
            fp = p.footprint
            print(
                f"    resident >= {fp['resident_lower_bound_bytes'] / 1e9:.2f} GB vs budget "
    ...
    if a.out_dir:
        ...
        if not (outdir / "manifest.yaml").is_file():
            print(
                f"ERROR: --out-dir {outdir} carries no manifest.yaml, so it is not a campaign "
    ...
    done = set() if a.force else ec.completed_models(ec.read_ledger(ledger), retry_refused=a.retry_refused)
    if done:
    ...
    for plan in plans:
        ...
        try:
            with log.open("w", encoding="utf-8") as lf:
                got = subprocess.run(
                    full, cwd=str(repo_root()), stdout=lf, stderr=subprocess.STDOUT, timeout=a.cell_timeout
                )
            rc = got.returncode
        except subprocess.TimeoutExpired:
            rc = None
            refusal = (
                f"the instrument did not finish within --cell-timeout={a.cell_timeout}s; no "
        ...
        if not refusal:
            if cell_json.is_file():
                ...
                refusal = (
                    f"the instrument exited {rc} without writing a record; nothing to read a "
        ...
        if v.get("status") == "measured":
            print(
                f"MEASURED {plan.model}: ours={v['ours_ns'] / 1e6:.3f} ms  et_warm="
                f"{v['executorch_warm_ns'] / 1e6:.3f} ms  speedup="
                f"{v['speedup_vs_executorch']:.3f}x  beats={v['beats_executorch']}",
                flush=True,
            )
        else:
