    ...
    for root in roots:
        ...
        for child in sorted(root.iterdir()):
            if not child.is_dir():
                continue
            if (
                (child / "agent" / "tools.jsonl").is_file()
                or (child / "control").is_dir()
                or (child / "global_control").is_dir()
            ):
                yield child
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--ledger", type=Path, default=None, help="the campaign's optimization ledger (JSON)")
    parser.add_argument("--root", type=Path, action="append", default=None, help="a run root to walk; repeatable")
    parser.add_argument("--out", type=Path, required=True)
    args = parser.parse_args(argv)

    roots = args.root or [
        runs_dir() / args.target / "perf-bench" / "agent_stages",
        artifacts_dir() / "perf-bench" / args.target,
    ]
    ledger_path = args.ledger
    if ledger_path is None:
        candidates = sorted((artifacts_dir() / "perf-bench" / args.target).glob("*_ledger_*/optimization_ledger.json"))
        if not candidates:
    ...
    analysis = build_analysis(
        ...
        point_events=points,
        broker_calls=calls,
        ledger=read_ledger(ledger_path, target=args.target),
    ...
    (out / "campaign_analysis.json").write_text(json.dumps(document, indent=2) + "\n", encoding="utf-8")

    budget = analysis.budget
    print(f"stages with telemetry     {analysis.stages}")
    print(f"tool spans                {analysis.tool_spans:,}  (+{analysis.point_events:,} point events excluded)")
    print(f"brokered calls            {budget.calls:,}  wall {budget.wall_seconds / 3600:.2f} h")
    print(
        f"  refused                 {budget.refused:,} calls "
    ...
    for tier, row in budget.by_tier().items():
        print(
            f"  tier {tier:8s}         {int(row['calls']):5d} calls  "
            f"{row['wall_seconds'] / 3600:6.2f} h  {int(row['refused']):4d} refused"
        )
    print(f"attempts                  {analysis.attempts.total}  {analysis.attempts.by_verdict}")
    print(f"  scopes reached          {analysis.attempts.scopes_reached} of {len(analysis.attempts.by_scope)}")
