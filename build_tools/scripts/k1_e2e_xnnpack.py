    ...
    *,
    ...
    for i in range(n):
        ...
        try:
            ...
            print(
                f"  [{tag}] run {i}: wall_ns={runs[-1]['wall_ns']} cos={cos:.6f} vlen={res.get('vlen')} n_xnn={n_xnn}"
            )
...
def run_workload(
    model_dir: str | Path,
    baseline_pkg: str = "out/artifacts/targets/rvv/hand_v0",
    n: int = 3,
    configs: str = "baseline,ours_tiled,ours_v3,ours_wholemodel,ours_wholemodel_vf,xnnpack_kernels,openblas_kernels",
    out: str | Path | None = None,
    *,
    iters: int = 1,
    warmup: int = 0,
    ours_best_features: list[str] | None = None,
    hybrid_graph_features: list[str] | None = None,
) -> dict:
    ...
    ours_wm = replace(base, run_id="ours_wholemodel", compiler_features=["accumulator_resident_wholemodel"])
    # ours-wholemodel-vf = wholemodel tail clamps + v3's .vf scalarize (kills the .vv broadcast ladder).
    ours_wm_vf = replace(base, run_id="ours_wholemodel_vf", compiler_features=["accumulator_resident_wholemodel_vf"])
    ...
    BEST_GRAPH_FEATURES = list(hybrid_graph_features or ["fuse_transpose_b", "erase_self_copy"])
    OURS_BEST_FEATURES = list(ours_best_features or ["accumulator_resident_wholemodel_vf_mrpad", *BEST_GRAPH_FEATURES])
    ...
    ours_best_tag = max(
        (t for t, r in ours_cands.items() if valid_wall(r)),
    ...
    summary = {
        ...
        "protocol": {
            ...
            "timed_iters_per_run": iters,
            "warmup_iters_per_run": warmup,
            "statistic": "min of per-iteration means",
        },
        "note": (
            "Same-pass head-to-head vs the SAME baseline in ONE pass. xnnpack-kernels routes f32 "
        ...
        "ours_v3": rv,
        "ours_wholemodel": rw,
        "ours_wholemodel_vf": rwv,
        "ours_best": rbest,
        "xnnpack_kernels": rx,
        "openblas_kernels": rob,
...
def main() -> None:
    ...
    ap.add_argument("--iters", type=int, default=1, help="timed in-process iterations per independent run")
    ap.add_argument("--warmup", type=int, default=0, help="untimed in-process warmup iterations per independent run")
    ap.add_argument(
        "--ours-best-features",
        default=None,
        help="comma-separated compiler features for the ours_best arm; defaults to the current stacked compiler recipe",
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--configs",
        default="baseline,ours_tiled,ours_v3,xnnpack_kernels",
    ...
    ours_best_features = a.ours_best_features.split(",") if a.ours_best_features is not None else None
    hybrid_graph_features = a.hybrid_graph_features.split(",") if a.hybrid_graph_features is not None else None
...
def _fmt(v, prec=4):
    return (
        "—" if v is None else (f"{v:,}" if isinstance(v, int) else f"{v:.{prec}f}" if isinstance(v, float) else str(v))
    )


def _write_md(path: Path, s: dict) -> None:
    rows = [
        ("baseline (hand_v0)", s["baseline"], None),
        ...
        (
            "ours-wholemodel-vf (.vf, no broadcast ladder)",
            s.get("ours_wholemodel_vf", {}),
            s.get("speedup_ours_wholemodel_vf"),
        ),
    ...
    verdict = "ours FASTER than XNNPACK" if (bx and bx > 1) else "XNNPACK faster than ours" if bx else "—"
    lines += [
        "",
