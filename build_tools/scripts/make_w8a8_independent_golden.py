...
class SafeTensors(Mapping):
    ...
    def __init__(self, path: Path):
        ...
        self._meta = {
            name: meta for name, meta in header.items() if name != "__metadata__" and meta["dtype"] in _ST_DTYPE
        }
...
def quantized_weight_diff(bundle_weights, fresh_weights, *, expected_quantized: int | None = None) -> dict:
    ...
    mismatched = [
        k
        for k in integer
        if bundle_weights[k].shape != fresh_weights[k].shape or not np.array_equal(bundle_weights[k], fresh_weights[k])
    ]
    missing = sorted(k for k in bundle_weights if k not in fresh_weights)
    coverage_complete = expected_quantized is None or (expected_quantized > 0 and len(integer) >= expected_quantized)
    return {
...
def flatten_quantized_parameters(model) -> dict[str, np.ndarray]:
    ...
    def walk(obj, prefix: str) -> None:
        ...
        if callable(flatten):
            ...
            flat[prefix] = tensor.float().numpy() if ("float8" in text or "bfloat16" in text) else tensor.numpy()

    for name, parameter in model.named_parameters():
        if type(parameter).__name__ not in ("Parameter", "Tensor") or hasattr(parameter, "__tensor_flatten__"):
            walk(parameter, name)
    return flat
...
def bundle_quantized_parameters(bundle: Path) -> dict[str, np.ndarray]:
    ...
    with np.load(path) as data:
        return {k[len("qinner::") :]: np.asarray(data[k]) for k in data.files if k.startswith("qinner::")}
...
def int8_bundles() -> list[Path]:
    ...
    if not root.is_dir():
        return []
    return sorted(
        d for d in root.iterdir() if d.is_dir() and "_int8" in d.name and (d / "weights.safetensors").is_file()
    )
...
def capture_environment(root: Path, model: str, overrides: dict[str, str] | None = None) -> dict[str, str]:
    ...
    """
    env = dict(os.environ)
    env.update({k: str(v) for k, v in (_load_capture_toml(root / "workloads" / model).get("env", {}) or {}).items()})
...
def generate(
    ...
    *,
    ...
    return True, (
        f"wrote {payload['path']} shape={payload['shape']} in {time.time() - t0:.0f}s; "
        ...
        + (
            ...
            if payload.get("cos_vs_self_golden") is not None
            else ""
        )
    )
...
def _inner(bundle: Path, model: str, root: Path, out_name: str, scheme: str) -> int:
    ...
    if callable(getattr(mdl, "write_bundle", None)):
        ...
        emit(
            {
                "ok": False,
                "reason": f"{model} is a multi-program (session) capture; this tool references a single forward",
            }
        )
        return 5
    mdl.eval()
    ...
    if len(recorded) != len(seeded):
        ...
        return 3
    inputs = tuple(torch.from_numpy(np.array(a)).to(t.dtype) for a, t in zip(recorded, seeded_inputs))
    seed_reproduces_inputs = all(a.shape == b.shape and np.array_equal(a, b) for a, b in zip(recorded, seeded))
    ...
    work = Path(tempfile.mkdtemp(prefix=f"indep_w8a8_{bundle.name}_", dir=os.environ.get("TMPDIR") or None))
    try:
        ...
        if not report["n_quantized"]:
            ...
            report = quantized_weight_diff(
                bundle_quantized_parameters(bundle),
        ...
        emit(
            {
                ...
                "weights": report,
                "reason": (
                    "no integer weight tensor is shared with the bundle (unmeasured, not agreement)"
                    if not report["n_quantized"]
                    else (
                        f"quantization coverage is partial: shared "
                        ...
                        else f"{report['n_mismatched']} quantized weight tensor(s) differ from the bundle's"
                    )
                ),
            }
        )
    ...
    if arr.shape != fp32_golden.shape:
        emit({"ok": False, "weights": report, "reason": f"shape {arr.shape} != fp32 golden {fp32_golden.shape}"})
        return 4
    ...
    provenance = {
        ...
        "decides": (
            f"whether merlin's arithmetic under {scheme} is correct"
    ...
    (bundle / (out_name + ".provenance.json")).write_text(json.dumps(provenance, indent=2) + "\n", encoding="utf-8")
    emit(
        {
            "ok": True,
            "path": str(target),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--list", action="store_true", help="report which int8 bundles have an independent reference")
    parser.add_argument("--force", action="store_true", help="regenerate even if one exists")
    parser.add_argument(
        "--out-name", default=DEFAULT_OUT_NAME, help=f"file written beside golden_w8a8.npy (default {DEFAULT_OUT_NAME})"
    )
    ...
    if args.inner:
        return _inner(Path(args.bundles[0]), args.model, Path(args.m2m_root), args.out_name, args.scheme)
