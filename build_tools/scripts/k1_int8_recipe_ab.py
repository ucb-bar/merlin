...
def run(shapes: list[int], reps: int, prefix: str, dtype: str) -> list[dict]:
    ...
    rows: list[dict] = []
    for S in shapes:
        bundle = workloads.gen_matmul_f32(REPO / "out" / "artifacts" / "cache" / "rvv_workloads", M=S, N=S, K=S)
        ...
        if dtype == "int8":
            ...
            r = _build_run_xnn(f"{prefix}qd8_xnn_{S}", HERE / "xnnpack_qd8_gemm_driver.c", defs, reps=reps, base=base)
            ...
            r = _build_run_xnn(f"{prefix}f32_xnn_{S}", HERE / "xnnpack_gemm_driver_7x4v.c", defs, reps=reps, base=base)
            print("   ", r["status"], r.get("ticks"), r.get("blocker", ""), flush=True)
            rows.append(r)
        ...
        for feats, sid in arms:
            base = {
                **common,
                "op": f"{dtype}_gemm",
                "dtype": ("i8xi8->i32" if int8 else "f32"),
                "source": sid,
                "int8_compute": int8,
            ...
            r = _build_run_ours(
                f"{prefix}{sid}_{S}", bundle, driver, defs, sid, feats, int8=int8, vectorize=True, reps=reps, base=base
            )
            print("   ", r["status"], r.get("ticks"), r.get("instret"), r.get("blocker", ""), flush=True)
            rows.append(r)
    return rows
...
def main() -> int:
    ...
    out = Path(a.out) if a.out else (artifacts_dir() / "perf-bench" / "rvv" / f"int8_recipe_ab_{a.dtype}.jsonl")
    ...
    print(f"\nwrote {out}")
    for r in rows:
        print(f"  {r['source']:24s} {r.get('M')}^3  ticks={r.get('ticks')}  instret={r.get('instret')}  {r['status']}")
    return 0
