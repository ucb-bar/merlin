...
NOTHING IS DELETED. A retired entry moves beside its declared recipe to ``retired/<profile>.v0.yaml``,
tracked, no longer read by the generator, with ``RETIRED.md`` recording what covers each one. The repo's practice is to
...
def _targets() -> list[str]:
    return sorted(item.profile for item in all_declarations())
...
def _binding(target: str):
    ...
    prof = yaml.safe_load(declaration.recipe.read_text())
...
def _on_disk(target: str) -> dict:
    import yaml

    from merlin.targetgen.target_experiment import load_target_experiment

    experiment = load_target_experiment(for_target(target).descriptor)
    paths = sorted({path for root in experiment.graded_roots() for path in root.rglob("capsule.yaml")})
    out = {}
    for p in paths:
        try:
...
def classify(target: str) -> dict:
    ...
    synth_path = for_target(target).synth_profile
    synth = yaml.safe_load(synth_path.read_text()) if synth_path is not None and synth_path.is_file() else {}
    disk = _on_disk(target)
...
def main(argv=None) -> int:
    ...
    for t, names in sorted(retire.items()):
        if not names:
            continue
        declaration = for_target(t)
        prof_path = declaration.recipe
        out_dir = prof_path.parent / "retired"
        out_dir.mkdir(parents=True, exist_ok=True)
        lines = records.setdefault(out_dir, list(header))
        ...
        (out_dir / f"{declaration.profile}.v0.yaml").write_text(
            "# DERIVED-OUT: entries retired from " + str(prof_path) + " because a derived capsule\n"
        ...
        lines.append("")
    for out_dir, lines in records.items():
        (out_dir / "RETIRED.md").write_text("\n".join(lines))
    print(f"\n  retired {sum(len(v) for v in retire.values())} entr(y/ies) to {', '.join(map(str, records))}")
    print(
