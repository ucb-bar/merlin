...
def _run_cfg(model_dir, pkg, golden, n, tag, kernel_backend, ours_mr):
    ...
    for i in range(n):
        ...
        try:
            res = k1.run_on_k1(
                model_dir, work, pkg, timeout=1800, kernel_backend=kernel_backend, dispatch_timing=True, ours_mr=ours_mr
            )
...
def main():
    ...
    results["xnnpack_kernels"] = _run_cfg(
        md, replace(base, run_id="xnnpack_kernels"), golden, a.n, "xnnpack_kernels", "xnnpack", 4
    )
        ...
        sweep.append(
            {
                ...
                "ours_wall_ns": r.get("min_wall_ns"),
                "xnnpack_wall_ns": xn.get("min_wall_ns"),
    ...
    summary = {
        ...
        "method": (
            "ours_board shim MR-configurable (-DOURS_MR); matmul bucket measured via rdtime in "
    ...
    for s in sweep:
        ...
        print(
            f"  MR={s['ours_mr']}: ours_matmul={None if s['ours_matmul_bucket_ns'] is None else round(s['ours_matmul_bucket_ns'] / 1e6, 1)}ms "
