...
import torchao
from torchao.quantization.quant_primitives import MappingType, choose_qparams_affine, quantize_affine
...
for name, x in keep.items():
    ...
    scale, zp = choose_qparams_affine(
        t, MappingType.SYMMETRIC, bs, torch.int8, quant_min=-127, quant_max=127, eps=1e-5, scale_dtype=None
    )
    ...
    q_et = qd.quantize_per_token(t, s_t, torch.zeros_like(s_t, dtype=torch.int64), -127, 127, torch.int8)
    ...
    json.dumps(
        {
            ...
            "call": (
                "MappingType.SYMMETRIC, block_size=per-token, target_dtype=torch.int8, "
            ...
            "cases": {
                "ties": "values landing exactly on .5 after scaling (round-half-to-EVEN)",
