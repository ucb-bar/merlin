...
the explicitly selected matrix-lowering provider's contract and encoding accessors.
...
def main(argv: "list[str] | None" = None) -> int:
    ...
    ap.add_argument(
        "--matrix-support-target", help="explicit OOT support provider target; required for routing and audit"
    )
    ap.add_argument(
    ...
    kw = (
        dict(matrix=zm.MatrixRouting(unit=a.unit, config=a.config, support_target=a.matrix_support_target))
        if routed
        else {}
    )
