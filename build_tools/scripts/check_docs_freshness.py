...
Usage:
  ...
  check_docs_freshness.py --ratchet --current-only
                                   # gate maintained docs; report draft drift as unverified
...
def _last_commit_date(path: str) -> str | None:
    """Newest date touching either the historical reference or its current physical source.

    A compatibility symlink must not freeze freshness at the last edit of the old pathname while
    the implementation under src keeps changing. Query both paths, preserving pre-move history.
    """
    paths = [path]
    try:
        physical = (ROOT / path).resolve().relative_to(ROOT.resolve()).as_posix()
    except ValueError:
        physical = path
    if physical != path:
        paths.append(physical)
    r = subprocess.run(
        ["git", "-C", str(ROOT), "log", "-1", "--format=%cs", "--", *paths], capture_output=True, text=True
    )
    d = (r.stdout or "").strip()
    return d or None
...
def _is_current(doc: str) -> bool:
    path = DOCS / doc
    if not path.is_file():
        return False
    fm = parse_front_matter(path.read_text(encoding="utf-8"))
    return bool(fm and fm.get("status") == "current")


def ratchet(*, current_only: bool = False) -> int:
    all_drifted = {d["doc"]: d for d in drift()}
    if current_only:
        drifted = {name: item for name, item in all_drifted.items() if _is_current(name)}
        allowed = {name for name in _ratchet_entries() if _is_current(name)}
        print(
            f"docs freshness: current-only gate; {len(all_drifted) - len(drifted)} "
            "draft/superseded drift(s) remain UNVERIFIED (inspect --json or strict --ratchet)"
        )
    else:
        drifted = all_drifted
...
def main(argv: list[str]) -> int:
    if "--ratchet" in argv:
        return ratchet(current_only="--current-only" in argv)
    if "--current-only" in argv:
        sys.stderr.write("--current-only requires --ratchet\n")
        return 2
    if "--check" in argv:
        errs = schema_errors()
        if errs:
