...
def _rows(files: list[str]) -> list[tuple[str, int, str, str]]:
    ...
    for d in sorted(counts):
        ...
        if len(parts) == 1:
            wanted.append(d)
        elif len(parts) == 2 and parts[0] in ("merlin", "build_tools", "docs", ".claude", ".github"):
            wanted.append(d)
...
def render() -> str:
    ...
    out = [
        ...
        "",
        "Generated by `build_tools/scripts/gen_repo_map.py` from `git ls-files` and each directory's",
        "own `AGENT.md`. **Do not edit by hand** — run the generator (`check_docs.py` `--check`s it).",
        "",
