...
def _race_summary(op: str, by_op: dict[str, list[dict]]) -> dict:
    ...
    for sh in shapes:
        ...
        blockers = sorted({r.get("blocker", "")[:120] for r in srows if r.get("status") == "not_run"} - {""})
        cell = {
            "shape": sh,
...
def main() -> None:
    ...
    if a.results:
        ...
        rpaths = [mdir / "cross_framework_ops_broadened_brd.jsonl", mdir / "cross_framework_ops_k1.jsonl"]
    ...
    # ---- markdown ----
    md = [
        "# Kernel coverage matrix — model relevance vs the XNNPACK expert surface",
        "",
    ...
    for m in matrix:
        ...
        if race.get("raced") and race.get("shapes"):
            ...
            ratio_str = m["note"]
        md.append(
            f"| {m['family']} | {m['mean_work_share'] * 100:.2f} | "
    ...
    for m in matrix:
        if not m["race"].get("raced"):
            md.append(
                f"- **{m['family']}** ({m['mean_work_share'] * 100:.2f}% work, "
