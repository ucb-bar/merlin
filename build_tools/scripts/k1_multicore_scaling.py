...
def main() -> int:
    ...
    if not k1.available():
        print(
            "k1 board unavailable — this measurement needs real silicon; "
    ...
    out_path = (
        Path(a.out)
        if a.out
        else (artifacts_dir() / "perf-bench" / "rvv" / f"multicore_scaling_{a.model}_{a.dtype}.jsonl")
    )
    ...
    print(f"building composed vector+omp binary (parallel_harts={max(threads)}) pkg={Path(pkg_dir).name} …", flush=True)
    t0 = time.time()
    binary = k1.build_k1_binary(mdir, work, pkg, inputs_npz=mdir / "inputs.npz", parallel_harts=max(threads))
    ...
    for t in threads:
        ...
        for i in range(a.n):
            try:
                ...
                if w:
                    walls.append(int(w))
                print(f"  T={t} run {i}: wall_ns={w} gate_ok={gate.get('ok')} cos={gate.get('cos')}", flush=True)
            except Exception as e:  # noqa: BLE001
                print(f"  T={t} run {i}: BLOCKED — {type(e).__name__}: {str(e).splitlines()[0][:200]}", flush=True)
                break
    ...
    for r in rows:
        print(
            f"{r['threads']:>3} {str(r['median_wall_ns']):>14} "
