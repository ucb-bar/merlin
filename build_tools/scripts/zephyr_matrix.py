...
DEFAULT = [
    "small_consistent",
    "tiny_consistent",
    "rdt2_fp32_consistent",
    "groot_n1d7_fp32_consistent",
    "molmoact_fp32_consistent",
]
...
print(f"available={zm.available()}  backend={backend}  n_harts={n_harts}  iters={iters}  bundles={bundles}", flush=True)
for name in bundles:
    ...
    try:
        ...
        print(
            f"ZRESULT {name} cos={res.get('cos', -1):.7f} rel={res.get('rel', -1):.3e} "
