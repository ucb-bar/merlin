...
specific target OR to a specific workload, what each weld costs, and what would remove it. A
hand-maintained debt list rots, so this gate does not trust it: it re-measures the live tree with the
same scan the name gate uses and compares.

A WORKLOAD weld is the one this register could not express until now, and that is how one survived in
plain sight. ``compute_groups.ABSORBABLE`` listed a conv block's epilogue -- bias, scale, round,
clamp, relu, cast, quantize, pool -- as "the stages a contraction's group may carry out", in
target-agnostic library code, derived from no target and named after no workload, so neither the
name gate nor this register could see it. It was scanning for the wrong kind of literal: a target
name is a string a scan can find, and a workload assumption is a plausible-looking SET. Entries of
``kind: workload_assumption`` are declared by hand for that reason -- no scan finds them -- and are
exempt from the stale check below for the same reason.
...
KINDS = ("target_name", "target_coupling", "runtime_concession", "dead_seam", "workload_assumption")
...
def _coupled_files() -> set[str]:
    ...
    return {_source_layout.policy_path(line.split(":", 1)[0]) for line in gate.coupling_inventory()}
...
def main(argv: list[str] | None = None) -> int:
    ...
    for entry in entries:
        for item in entry.get("items") or []:
            declared[_source_layout.policy_path(str(item))] = entry.get("id", "?")
    ...
    scan_visible = {
        _source_layout.policy_path(str(item))
        for entry in entries
        if entry.get("kind") in scanned_kinds
        for item in (entry.get("items") or [])
    }
