...
def _reference(model: str, dtype: str, root: Path, ours_bundle_id: str | None = None) -> dict:
    ...
    if dtype == "fp32":
        return {
            "wall_ns": _xnnpack_kernels_wall_ns(model),
            "kind": "xnnpack_kernels_in_runtime",
            "dtype": "fp32",
            "label": XNNPACK_KERNELS_LABEL,
        }
    et = executorch_cell(model, dtype, root=root, ours_bundle_id=ours_bundle_id)
    if et.get("executorch_status") == "measured":
        return {
            "wall_ns": et["executorch_wall_ns"],
            "kind": "executorch_external",
            "dtype": dtype,
            "label": EXECUTORCH_LABEL,
            "note": (
                f"non-fp32 cell: reference = TRUE external ExecuTorch {dtype} wall "
    ...
    return {
        ...
        "dtype": dtype,
        "note": (
            f"non-fp32 cell: no dtype-appropriate reference (attain_vs_xnn=None) — NEVER "
...
def _manual_best(model: str) -> dict:
    ...
    return {
        "features": d.get("ours_best_features"),
        "wall_ns": ob.get("min_wall_ns"),
...
def run_cell(dtype: str, model: str, *, width: int, depth: int, top_k: int) -> dict:
    ...
    comparability = {
        "xnnpack_kernels": XNNPACK_KERNELS_LABEL,
        "executorch": EXECUTORCH_LABEL,
    ...
    xnn = (
        ExpertBaseline(
            wall_ns=float(ref["wall_ns"]),
            workload=ours_bundle_id,
            dtype=ref.get("dtype"),
            substrate="k1_spacemit",
            note=ref.get("kind") or "",
        )
        if ref.get("wall_ns")
        else None
    )
    ...
    try:
        res = run_instrumented_beam(
            seed_pkg="out/artifacts/targets/rvv/hand_v0",
            model_dir=bundle,
            expert_objdump=obj,
            op="matmul",
            dtype=dtype,
...
def main() -> int:
    ...
    ap.add_argument(
        "--cells",
        default="fp32:bitvla,fp32:openvla,fp32:rdt2",
    ...
    out_path = (
        Path(a.out)
        if a.out
        else (artifacts_dir() / "kernel-mining" / "rvv" / "bench" / "autonomous_beam_experiment.json")
    )
    ...
    for c, info in audit.items():
        print(
            f"[bundle] {'OK  ' if info['present'] else 'MISS'} {c} -> {info['bundle'] or info['looked_for']}",
            flush=True,
        )
    if not any(i["present"] for i in audit.values()):
        print(
            "[bundle] no requested cell has a capture bundle on disk — refusing to run a search "
            "that can only report not_run for every cell",
            flush=True,
        )
        return 2
    results = []
    for c in cells:
        ...
        et_str = (
            f"{et.get('executorch_wall_ns') / 1e6:.0f}ms"
            if et.get("executorch_wall_ns")
            else f"not_measured({et.get('reason', '')[:60]})"
        )
        print(
            f"[{c}] {r.get('status')} :: beam={r.get('beam_discovered_features')} "
