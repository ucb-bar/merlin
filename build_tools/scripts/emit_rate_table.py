...
def derived_peak(target: str) -> tuple[float, dict]:
    ...
    if not usable:
        raise SystemExit(
            f"{target}: RTL facts declare no compute array with an instance count, so "
    ...
    return float(chosen["instances"]), {
        "array": chosen.get("name"),
        "rows": chosen.get("rows"),
        "cols": chosen.get("cols"),
        "instances": chosen.get("instances"),
        "corroborated": chosen.get("corroborated"),
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--check", action="store_true", help="exit non-zero unless every held-out program is contained")
    ...
    print(
        f"target {args.target}: peak {peak:g} MACs/cycle "
    ...
    for name, rate in sorted(table.rates.items()):
        print(
            f"    {name:18s} slowest={rate.slowest_macs_per_cycle:9.4f} n={rate.n_programs:3d} "
            f"cycles {rate.cycles_min:.0f}..{rate.cycles_max:.0f}"
        )
    rate_pct = gate["containment_rate"]
    print(
        f"  containment   : {gate['contained']}/{gate['n_decided']} "
    ...
    print(
        f"  median width  : {'n/a' if width is None else f'{width:.1f}x'} "
    ...
    if args.check:
        if not gate["n_decided"]:
            print("REFUSED: no held-out program could be decided, so the bound is untested", file=sys.stderr)
            return 2
        if rate_pct is not None and rate_pct < 1.0:
            print(
                f"FAILED: {gate['below_floor']} below the floor and {gate['above_ceiling']} above "
