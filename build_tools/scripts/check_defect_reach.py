...
def main(argv=None) -> int:
    ...
    if not targets:
        raise ValueError("defect-reach requires a nonempty Phase 0 declaration inventory")
    subtrees = {t for t in targets if (root / t).is_dir()}
    if args.target:
        targets = [for_target(args.target).profile]
    ...
    for t in targets:
        ...
        sto = store_overflow_reach(caps, target=for_target(t).target)
