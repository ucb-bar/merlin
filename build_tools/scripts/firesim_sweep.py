...
def main() -> int:
    ...
    if bool(args.matrix_unit) != bool(args.matrix_config):
        ap.error(
            "--matrix-unit and --matrix-config go together: the tile edge comes from the "
            "configuration, so a unit without one would be routed at a guessed geometry"
        )
    matrix = zm.MatrixRouting(unit=args.matrix_unit, config=args.matrix_config) if args.matrix_unit else None
