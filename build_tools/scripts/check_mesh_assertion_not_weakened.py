...
Modes, mirroring the sibling gates here:
  ...
  --allow-unresolved   CI source-only mode: still FAIL on new weakened assertions, but report missing
                       generated contracts as UNQUALIFIED rather than failing the job. The normal
                       full gate remains fail-closed, and a misspelled --target always fails.
...
def audit(targets=()) -> dict:
    ...
    return {
        ...
        "unmatched_requested_targets": sorted(set(targets) - seen_targets),
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--allow-unresolved",
        action="store_true",
        help="source-only CI: missing generated contracts remain UNQUALIFIED, but new weakenings still fail",
    )
    ap.add_argument(
        "--stop-hook",
        action="store_true",
    ...
    if a.stop_hook:
        ...
        if rep["unmatched_requested_targets"]:
            print(
                json.dumps(
                    {
                        "decision": "block",
                        "reason": "mesh assertion: requested target(s) have no capsules: "
                        + ", ".join(rep["unmatched_requested_targets"]),
                    }
                )
            )
        elif rep["unresolved_targets"]:
            print(
                json.dumps(
                    {
                        "decision": "block",
    ...
    if a.json:
        ...
        if rep["unresolved_targets"]:
            ...
            for t, why in rep["unresolved_targets"].items():
                print(f"     ? {t}: {why[:90]}")
        if rep["unmatched_requested_targets"]:
            print("   UNMATCHED REQUESTED TARGETS: " + ", ".join(rep["unmatched_requested_targets"]))

    if rep["unmatched_requested_targets"]:
        print(
            "CANNOT DECIDE: requested target(s) have no capsules: " + ", ".join(rep["unmatched_requested_targets"]),
            file=sys.stderr,
        )
        return 2
    if a.allow_unresolved and rep["n_admitted_capsules_checked"] == 0:
        print(
            "CANNOT DECIDE: source-only mode resolved no admitted capsules; "
            "a missing contract set cannot pass vacuously",
            file=sys.stderr,
        )
        return 2
    ...
    if rep["unresolved_targets"]:
        ...
        if not (a.advisory or a.allow_unresolved):
            return 2
    if new:
        print(
    ...
    if rep["unresolved_targets"] and a.allow_unresolved:
        print(
            "PARTIAL: resolved targets passed; unresolved targets remain UNQUALIFIED. "
            "Run the full gate with generated contracts to qualify them.",
            file=sys.stderr,
        )
    return 0
