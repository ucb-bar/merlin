...
# --- loading --------------------------------------------------------------------------------------
def _generator():
    """Use the authoritative Phase 0 sweep implementation.
...
def _descriptor_for(target: str) -> Path | None:
    cand = for_target(target).descriptor
...
def targets_with_holdouts() -> list[str]:
    return sorted(
        {d.profile for d in all_declarations() if d.hidden_profile is not None and d.hidden_profile.is_file()}
    )
...
def audit(target: str) -> dict:
    ...
    public_path = declaration.recipe
    hidden_path = declaration.hidden_profile
    ...
    if hidden_path is None or not hidden_path.is_file():
