...
SCAN_ROOTS = (*_source_layout.SOURCE_SCAN_ROOTS, "merlin/targets", "build_tools/scripts")
...
def _iter_targets(staged: bool) -> list[str]:
    return [
        rel.as_posix()
        for rel in _source_layout.scan_python_paths(ROOT, SCAN_ROOTS, staged=staged)
        if not any(fragment in f"/{rel.as_posix()}" for fragment in EXCLUDE_FRAGMENTS)
    ]
...
def findings(staged: bool = False, ratchet_path: Path | None = None) -> list[dict]:
    ...
    for rel in _iter_targets(staged):
        for rec in scan_file(ROOT / rel, rel):
            key = (_source_layout.policy_path(rec["path"]), rec["source"])
