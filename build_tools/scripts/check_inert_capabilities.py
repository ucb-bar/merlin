...
DEFAULT_SCAN_ROOTS = (*_source_layout.SOURCE_SCAN_ROOTS, "build_tools/scripts")
#: Trees consulted only to answer "does anything PRODUCE this value / READ this knob?".
DEFAULT_WITNESS_ROOTS = (
    *_source_layout.SOURCE_SCAN_ROOTS,
    "build_tools/scripts",
    "merlin/contract",
    "merlin/tests",
    "merlin/schemas",
    ...
    for r in roots:
        if r.is_file() and r.suffix == ".py":
            if r.resolve() not in seen:
                ...
                out.append(r)
            continue
        scan_roots = sorted(r.glob("*/src")) if r.name == "packages" else [r]
        for scan_root in scan_roots:
            for p in _safe_rglob(scan_root, ".py"):
...
class Finding:
    ...
    @property
    def key(self) -> str:
        return f"{self.kind} {_source_layout.policy_path(self.ident)}"
