...
point back to canonical ``merlin_experiments.phase0`` owners and ``corpus_spec`` for
...
class HoldoutSourceContext:
    """Explicit declaration and implementation owners for commit/reveal replay.

    Package roots name Python namespace directories, not installation prefixes.
    Their complete membership and loaded origins are verified on every use.
    source_root resolves descriptor-relative resources; it is not a code fallback.
    """

    source_root: Path
    catalog_path: Path
    core_package_root: Path
    experiments_package_root: Path
    experiments_namespace_root: Path
...
def _fresh_parent(path: Path, *, label: str) -> Path:
    ...
    parent = path.parent
    if parent.is_symlink() or any(ancestor.is_symlink() for ancestor in parent.parents) or not parent.is_dir():
...
def _dtype_bytes(token: str) -> int:
    aliases = {"int8": 1, "i8": 1, "uint8": 1, "u8": 1, "int32": 4, "i32": 4, "uint32": 4, "u32": 4}
    try:
...
def _resolve_extent(token: object, tile: int) -> int:
    ...
    try:
        return int(resolve_extent(token, tile))
...
def _pk_sweep(profile_path: Path) -> dict[str, Any]:
    ...
    sweeps = [row for row in (document.get("sweeps") or []) if isinstance(row, dict) and row.get("id") == "PK"]
    if len(sweeps) != 1:
    ...
    if (
        base.get("op") != "matmul"
        or performance.get("family") != "PK"
        or performance.get("claim") != "PREDICTS"
        or (performance.get("emitter") or {}).get("entry") != "merlin.targetgen.corpus_spec.build"
    ):
...
def _source_paths(
    target: str, rtl_facts: Path, perf_profile: Path, *, context: HoldoutSourceContext
) -> dict[str, Path]:
    import importlib
    ...
    expected_owners = {
        "merlin_experiments.phase0.generation": context.experiments_package_root / "phase0/generation.py",
        "merlin.targetgen.corpus_spec": context.core_package_root / "targetgen/corpus_spec.py",
        "merlin.targetgen.capsule_runner": context.experiments_namespace_root / "targetgen/capsule_runner.py",
    }
    for module_name, expected in expected_owners.items():
        if module_source_path(module_name).resolve() != expected:
            raise HoldoutError(f"declared source package root differs from loaded owner: {module_name}")
    ...
    declaration = for_target(target, catalog_path=catalog_path)
    # Load the actual generation owners before checking their origins. A checkout
    # inventory alone cannot attest a writer imported from another installation.
    for name in ("profiles", "provenance", "sweeps", "writer"):
        importlib.import_module(f"merlin_experiments.phase0.{name}")
    paths = {
        "rtl_circt_facts": rtl_facts,
        "shared_perf_contract": perf_profile,
        "generate_corpus_py": Path(generation.__file__).resolve(),
        "corpus_spec_py": Path(corpus_spec.__file__).resolve(),
        "derivation_definition": declaration.definition,
        "derivation_catalog": catalog_path,
        "target_experiment": declaration.descriptor,
        "target_public_profile": declaration.recipe,
        "declared_performance_template": declaration.performance_template,
        "holdout_implementation": Path(__file__).resolve(),
    }
    # Conservative source closure: include membership, not merely the former facade's bytes.
    # External dependencies/tools are not attested by this repository-source commitment.
    module_owners = {}
    for owner, namespace, root in (
        ("core", "merlin", context.core_package_root),
        ("experiments", "merlin_experiments", context.experiments_package_root),
        ("experiments_namespace", "merlin", context.experiments_namespace_root),
    ):
        members = python_members(root)
        paths.update({f"implementation/{owner}/{name}": path for name, path in members.items()})
        for name, path in members.items():
            parts = list(Path(name).with_suffix("").parts)
            if parts[-1] == "__init__":
                parts.pop()
            module_name = ".".join([namespace, *parts])
            module_owners[module_name] = path
    for module_name, path in module_owners.items():
        loaded = sys.modules.get(module_name)
        if loaded is None:
            continue
        origin = getattr(loaded, "__file__", None)
        # Identity-preserving compatibility modules point at a canonical module
        # object. Both the alias source and canonical source must be inventoried.
        canonical = getattr(loaded, "__name__", module_name)
        expected = module_owners.get(canonical)
        if (
            origin is None
            or expected is None
            ...
            raise HoldoutError(f"loaded source origin differs from committed owner: {module_name}")
    paths["experiment_schema"] = context.experiments_package_root / "resources/experiment.schema.json"
    actual_schema = Path(str(files("merlin_experiments").joinpath("resources/experiment.schema.json")))
    if actual_schema.resolve() != paths["experiment_schema"].resolve():
        raise HoldoutError("loaded experiment schema differs from committed owner")
    for label in ("synth_profile", "smt_profile"):
        path = getattr(declaration, label)
    ...
    if descriptor.is_file() and not descriptor.is_symlink():
        ...
        if contract:
            contract_path = (context.source_root / str(contract)).resolve()
...
def verify_rtl_facts_provenance(
    ...
    *,
    target: str,
    fact_builder: Any = None,
    core_hw_resolver: Any = None,
    ...
    if (
        generator.get("name") != "merlin.targetgen.rtl.circt_introspect"
        or facts.get("target") != target
    ...
    if extractor_path is None:
        raise HoldoutError("injected provenance replay requires an explicit extractor path")
    extractor = _assert_plain_file(extractor_path, label="CIRCT facts extractor")
    extractor_sha = _sha256_file(extractor)
    recorded_extractor = inputs.get("extractor_sha")
    if (
        not isinstance(recorded_extractor, str)
        or len(recorded_extractor) < 16
    ...
    if not isinstance(recorded_fir, str) or len(recorded_fir) < 16 or fir_sha[: len(recorded_fir)] != recorded_fir:
    ...
    try:
        ...
        raise HoldoutError(f"CIRCT RTL facts could not be replayed ({type(exc).__name__})") from exc
...
def derive_domain(
    rtl_facts_path: Path, perf_profile_path: Path, *, target: str, context: HoldoutSourceContext
) -> dict[str, Any]:
    ...
    perf_profile_path = _assert_frozen_file(perf_profile_path, label="frozen shared performance profile")
    facts_document = _load_json(rtl_facts_path, label="frozen RTL/CIRCT facts")
    generator = facts_document.get("generator") or {}
    if not str(generator.get("name") or "").startswith("merlin.targetgen.rtl.circt_introspect"):
    ...
    if facts.get("target") != target:
        raise HoldoutError(f"RTL/CIRCT facts target {facts.get('target')!r} does not match {target!r}")
    ...
    if len(legal) < MIN_MEMBERS:
        raise HoldoutError(f"RTL-derived PK domain has only {len(legal)} unseen legal point(s); need {MIN_MEMBERS}")

    sources = _source_paths(target, rtl_facts_path, perf_profile_path, context=context)
    positive_depths = [
        int(row["pipeline_depth"])
        for row in (facts.get("timing") or [])
        if isinstance(row, Mapping) and isinstance(row.get("pipeline_depth"), int) and row["pipeline_depth"] > 0
    ]
    ...
    if len(legal_shapes) < MIN_MEMBERS:
        raise HoldoutError("RTL/public-contract-derived M/N/K domain has too few legal unseen shapes")
    ...
    return {
        ...
        "legal_k": {
            "minimum": min(legal),
            "maximum": max(legal),
            "cardinality": len(legal),
            "excluded_public_dev": dev_k,
        },
        "bounds": {
        ...
        "generalization": {
            ...
            "selection_domain": {
                ...
                "excluded_public_shapes": [{"M": pm, "N": pn, "K": pk} for pm, pn, pk in sorted(public_shapes)],
...
def select_members(seed: bytes, domain: Mapping[str, Any], count: int) -> list[int]:
    ...
    ranked = sorted(
        values,
        key=lambda value: (hashlib.sha256(seed + b"\0" + domain_digest + value.to_bytes(8, "big")).digest(), value),
    )
...
def _legal_generalization_shapes(domain: Mapping[str, Any]) -> list[tuple[int, int, int]]:
    ...
    excluded = {
        (int(row["M"]), int(row["N"]), int(row["K"])) for row in (selection.get("excluded_public_shapes") or [])
    }
...
def select_generalization_members(seed: bytes, domain: Mapping[str, Any], count: int) -> list[dict[str, int]]:
    ...
    if count > len(values):
        raise HoldoutError(f"generalization count {count} exceeds legal domain size {len(values)}")
    digest = hashlib.sha256(_canonical_json(domain)).digest()
    ranked = sorted(
        values,
        key=lambda shape: (
            hashlib.sha256(seed + b"\0mnk\0" + digest + b"".join(value.to_bytes(8, "big") for value in shape)).digest(),
            shape,
        ),
    )
    ...
    def coverage(shapes: Sequence[tuple[int, int, int]]) -> tuple[bool, ...]:
        ...
        return (
            ...
            any(m <= rows for m in m_values),
            any(m > rows for m in m_values),
            any(n <= cols for n in n_values),
            any(n > cols for n in n_values),
            any(m % rows for m in m_values),
            any(n % cols for n in n_values),
        )
    ...
    while len(selected) < count:
        ...
        best = max(
            enumerate(remaining),
            key=lambda indexed: (sum(coverage([*selected, indexed[1]])) - before, -indexed[0]),
        )
        selected.append(best[1])
        remaining.pop(best[0])
    if len(selected) != count or not all(coverage(selected)):
        raise HoldoutError("seeded cohort cannot satisfy precommitted M/N/K mesh-boundary coverage")
...
def commit_holdout(
    ...
    *,
    rtl_facts_path: Path,
    perf_profile_path: Path,
    target: str,
    context: HoldoutSourceContext,
    ...
    if isinstance(count, bool) or not isinstance(count, int) or count < MIN_MEMBERS:
        raise HoldoutError(f"holdout needs at least {MIN_MEMBERS} members")
    if (
        isinstance(generalization_count, bool)
    ...
    domain = {
        **derive_domain(rtl_facts_path, perf_profile_path, target=target, context=context),
    ...
    selected_generalization = select_generalization_members(seed, domain, generalization_count)
    public = {
        "algorithm": ALGORITHM,
        "version": ALGORITHM_VERSION,
        "domain": domain,
        "cohort_counts": {"PK_predictor": count, "PK_MNK_generalization": generalization_count},
    ...
    private = {
        ...
        "source_paths": {
            ...
            for label, path in sorted(
                _source_paths(
                    target, Path(rtl_facts_path).resolve(), Path(perf_profile_path).resolve(), context=context
                ).items()
            )
        },
    ...
    try:
        ...
        raise
    return HoldoutPaths(
        public_commitment.resolve(), host_private_dir.resolve(), seed_path.resolve(), state_path.resolve()
    )
...
def _verify_current_generation_sources(
    public: Mapping[str, Any], private: Mapping[str, Any], *, context: HoldoutSourceContext
) -> None:
    """Old receipts remain inspectable, but cannot authorize newly extracted implementation."""
    recorded = public["domain"].get("source_sha256") or {}
    paths = private.get("source_paths") or {}
    if "holdout_implementation" not in recorded or not any(k.startswith("implementation/") for k in recorded):
        raise HoldoutError("legacy holdout commitment lacks canonical implementation closure; prepare a new frozen run")
    current = _source_paths(
        str(public["domain"]["target"]),
        Path(paths["rtl_circt_facts"]),
        Path(paths["shared_perf_contract"]),
        context=context,
    )
    if set(current) != set(recorded):
        raise HoldoutError("canonical holdout source membership changed; prepare a new frozen run")
    for label, path in current.items():
        if _sha256_file(path) != recorded[label]:
            raise HoldoutError(f"current holdout implementation/input differs from commitment: {label}")


def _verify_candidate_seals(expected_ids: Sequence[str], candidate_seals: Mapping[str, Path]) -> list[dict[str, str]]:
    ...
    for candidate_id in expected_ids:
        path = _assert_frozen_file(Path(candidate_seals[candidate_id]), label=f"candidate seal {candidate_id}")
        ...
        if (
            document.get("state") != "sealed"
            or candidate.get("read_only") is not True
            or admission.get("consumable") is not True
        ...
        receipts.append(
            {"candidate_id": candidate_id, "record_sha256": _sha256_file(path), "candidate_sha256": observed}
        )
    return receipts


def _binding(target: str, domain: Mapping[str, Any], *, context: HoldoutSourceContext):
    from merlin.targetgen import corpus_spec
    from merlin.targetgen.target_experiment import load_target_experiment
    from merlin_experiments.phase0.declarations import for_target
    from merlin_experiments.phase0.profiles import load_profile

    declaration = for_target(target, catalog_path=context.catalog_path)
    target_experiment = load_target_experiment(declaration.descriptor, source_root=context.source_root)
    public_profile = load_profile(declaration.profile, include_holdouts=False, **declaration.profile_inputs())
    binding = corpus_spec.derive_binding(target_experiment, public_profile.get("datapath", {}))
    bounds = domain["bounds"]
    if (
        binding.tile_dim != bounds["mesh"]["rows"]
...
def _tree_record(root: Path) -> dict[str, Any]:
    ...
    for path in sorted(root.rglob("*")):
        ...
        if path.is_file():
            rows.append(
                {"path": path.relative_to(root).as_posix(), "bytes": path.stat().st_size, "sha256": _sha256_file(path)}
            )
...
def reveal_and_materialize(
    ...
    *,
    ...
    _verify_current_generation_sources(public, private, context=context)
    ...
    selected_generalization = select_generalization_members(seed, public["domain"], counts["PK_MNK_generalization"])
    ...
    if selected_generalization != private.get("selected_generalization_shapes"):
        raise HoldoutError("host-private generalization members do not follow the committed algorithm")
    ...
    candidate_receipts = _verify_candidate_seals(private.get("candidate_ids") or [], candidate_seals)
    ...
    try:
        from merlin_experiments.phase0 import provenance, sweeps, writer

        binding = _binding(str(public["domain"]["target"]), public["domain"], context=context)
        ...
        for entry in entries:
            path = writer._write_capsule(entry, binding, output_dir)
            if path is None:
                raise HoldoutError(f"shared generator skipped holdout member {entry.get('name')}")
            provenance._scrub_capsule_dir(path)
            predictor_written.append(Path(path))
        predictor_names = [path.name for path in predictor_written]
        if len(predictor_names) != len(set(predictor_names)) or len(predictor_written) != len(selected):
        ...
        for index, shape in enumerate(selected_generalization):
            ...
            entry.update(shape)
            entry["name"] = f"PKG{index:02d}_m{shape['M']}k{shape['K']}n{shape['N']}"
            ...
            sweeps._materialize_performance_entry(entry, binding)
            path = writer._write_capsule(entry, binding, output_dir)
            if path is None:
                raise HoldoutError(f"shared generator skipped generalization member {entry['name']}")
            provenance._scrub_capsule_dir(path)
        ...
        if len(generalization_written) != len(selected_generalization) or len(all_names) != len(set(all_names)):
        ...
        manifest = {
            ...
            "cohorts": {
                "PK_predictor": {
                    ...
                    "claim": "PREDICTS",
                    "varied_axes": ["K"],
                    "member_count": len(predictor_names),
                },
                ...
                + [
                    {
                        ...
                        "source_family": "PK",
                        "descriptor_contract_family": "PK",
                        "cohort": "PK_MNK_generalization",
                        **shape,
                    }
                    for name, shape in zip(generalization_names, selected_generalization, strict=True)
                ]
            ),
            ...
            "corpus": capsules,
            "generator": {
                "entry": "merlin_experiments.phase0.writer._write_capsule",
