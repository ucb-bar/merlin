...
_TIMING_ENGINES = {
    ...
    "verilator": {"oracle_kind": "rtl_verilator", "dependent_metric": "verilator_L3_cycles"},
}

# This mirrors the reviewed declaration in experiments/templates/phase0/performance.yaml.
...
def _refused(reason: str, *, preflight: Mapping[str, Any] | None = None) -> dict[str, Any]:
    declaration = (
        copy.deepcopy(preflight.get("declaration"))
    ...
    return {
        ...
        "cohort": (
            copy.deepcopy(preflight.get("cohort"))
            if preflight is not None and preflight.get("cohort") is not None
            else None
        ),
        "evidence": None,
        "fit": None,
...
def _descriptor_point(raw: object) -> dict[str, Any]:
    ...
    if not isinstance(name, str) or not name or any(character not in _SIMPLE_NAME_CHARS for character in name):
    ...
    if comparand.get("kind") != "fitted_prediction" or comparand.get("against") != "measured_cycles_same_member":
    ...
    if (
        falsifier.get("observation") != "residual_cycles_by_K"
    ...
    m, lhs_k = (_positive_extent(lhs_shape[0], f"{name}.M"), _positive_extent(lhs_shape[1], f"{name}.lhs_K"))
    weight_k, n = (
        _positive_extent(weight_shape[0], f"{name}.weight_K"),
...
def _validate_descriptors(descriptors: object) -> tuple[list[dict[str, Any]], dict[str, Any], dict[str, Any]]:
    ...
    fixed_fields = ("M", "N", "operation", "operand_dtype", "accum_dtype", "epilogue", "acceptance", "timing_simulator")
    ...
    if sorted(by_k) != expected_k:
        raise _Refusal(f"PK cohort must contain exactly K={expected_k}, observed {sorted(by_k)}")
    ordered = [by_k[k] for k in expected_k]
    for index, point in enumerate(ordered):
        expected_name = f"PK{index:02d}_k{point['K']}"
        if point["capsule"] != expected_name:
            raise _Refusal(f"PK K={point['K']} member is {point['capsule']!r}, expected {expected_name!r}")
...
def _validate_results(
    results: object, points: Sequence[Mapping[str, Any]], acceptance: Mapping[str, Any]
) -> list[dict[str, Any]]:
    ...
    if len(pk_rows) != expected_count:
        raise _Refusal(f"PK requires exactly {expected_count} result rows, observed {len(pk_rows)}")
        ...
        for replicate in tuple(_ACCEPTANCE_BASE["replicates"]["identities"]):
            for simulator, tier, purpose, citable in (
                ...
                if simulator == "spike":
                    ...
                    if (
                        oracle.get("kind") != timing_oracle_kind
                        or oracle.get("derived_from_rtl") is not True
                        or oracle.get("fidelity") != "elaborated_rtl"
                    ):
                        raise _Refusal(f"PK result {key!r} does not prove its selected elaborated-RTL oracle")
                    timing_identity = _mapping(row.get("timing_identity"), f"PK result {key!r} timing identity")
                    ...
                    if set(timing_identity) != expected_identity_fields:
                        raise _Refusal(f"PK result {key!r} timing identity does not contain the exact fields")
                    if (
                        ...
                        raise _Refusal(f"PK result {key!r} timing identity names a different engine or fidelity")
...
def _fit(observations: Sequence[Mapping[str, Any]], timing_simulator: str) -> tuple[dict[str, Any], dict[str, bool]]:
    ...
    details = []
    for row, residual, bound, accepted in zip(observations, residuals, allowed, within, strict=True):
        details.append(
            {
                "capsule": row["capsule"],
    ...
    fit = {
        "form": "affine",
        "method": "ordinary_least_squares_all_L3_replicates",
        "equation": (f"{timing_simulator}_L3_cycles = rate_cycles_per_K_element * K + intercept_cycles"),
        "n_observations": n,
...
def analyze_pk_claim(descriptors: object, results: object) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence": {
            ...
            "l2_correctness_rows_validated": (len(points) * len(acceptance["replicates"]["identities"])),
            ...
            "timing_fidelity": "elaborated_rtl",
            "simulator_binary_sha256": observations[0]["timing_identity"]["simulator_binary_sha256"],
            "elaborated_firrtl_sha256": observations[0]["timing_identity"]["elaborated_firrtl_sha256"],
