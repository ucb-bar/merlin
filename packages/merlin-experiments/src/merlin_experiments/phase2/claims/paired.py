...
def analyze_paired_claim(descriptors: object, results: object, per_unit: object = None) -> dict[str, Any]:
    ...
    for d in descriptors:
        ...
        if perf.get("claim") != "DIFFERENTIAL":
            return _fail("this procedure decides DIFFERENTIAL claims only", observed_claim=perf.get("claim"))
    ...
    if predicted is None or (predicted != EITHER and predicted not in roles):
        return _fail(
            "the contract does not predict which role is cheaper, nor that they merely "
            "differ, so no measurement could contradict it",
            roles=roles,
        )
    ...
    for capsule in capsules:
        ...
        rows.append({"capsule": capsule, roles[0]: a, roles[1]: b, "delta_cycles": delta, "replicate_band": band})
    ...
    if breaches:
        return {
            ...
            "reason": (
                "the negative control, whose arms are the same program, shows a cycle "
    ...
    if predicted == EITHER:
        indistinguishable = [r["capsule"] for r in judged if abs(r["delta_cycles"]) <= r["replicate_band"]]
        ...
        if losers:
            outcome = {
                ...
                "reason": (
                    ...
                    f"their measured replicate band"
                ),
                "members": losers[:8],
            }
        else:
            outcome = {
                ...
                "rows": rows,
                "reason": (
                    f"{predicted!r} is cheaper on all {len(judged)} member(s), beyond each member's own replicate band"
                ),
            }
    return _apply_growth_falsifier(outcome, falsifier, judged, roles, per_unit)


def _apply_growth_falsifier(
    ...
    if not isinstance(per_unit, Mapping) or not per_unit:
        return _fail(
            ...
            declared_observation=falsifier.get("observation"),
            direction_verdict=outcome["verdict"],
    ...
    if missing:
        return _fail(
            "a member has no per-unit count, so its saving cannot be attributed",
            members=missing[:8],
            rows=outcome.get("rows"),
        )
...
def _validated_declaration(
    descriptors: object, replicates: Sequence[str]
    ...
    for index, raw in enumerate(descriptors):
        ...
        performance = _mapping_or_refuse(descriptor.get("performance"), f"descriptor {name!r} performance")
    ...
    if schedule.identities and tuple(schedule.identities) != identities:
        raise _Refusal(
            f"the contract freezes replicates {list(schedule.identities)} and the run offers {list(identities)}"
        )
        ...
        unresolved.append(
            {
                ...
                "detail": (
                    "the family names its negative control in prose and the contract binds it to "
    ...
    if growth:
        if len(members) < 2:
            raise _Refusal(
                "the falsifier asserts the saving GROWS with the count removed, and one paired point cannot show growth"
            )
        unresolved.append(
            {
                ...
                "detail": (
                    "the decision needs how many units the cheap arm removed on each member; "
    ...
    resolved = {
        ...
        "cohort": {
            ...
            "negative_control_capsule": (str(control_capsule) if control_capsule is not None else None),
            "roles": roles,
...
def preflight_paired_claim(descriptors: object, *, replicates: Sequence[str]) -> dict[str, Any]:
