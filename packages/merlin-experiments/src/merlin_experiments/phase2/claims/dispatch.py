...
def declaration_module(declaration: Mapping[str, Any], family: str) -> Any:
    ...
    try:
        ...
        raise StageGateError(f"the sealed {family} declaration names an unusable analyzer: {exc}") from exc
    if identity is None:
        raise StageGateError(f"the sealed {family} declaration names no acceptance.analyzer, so nothing decides it")
    try:
        return load_analyzer_module(identity.module)
    except Exception as exc:  # noqa: BLE001
        raise StageGateError(f"the sealed {family} analyzer {identity.declared!r} is unavailable: {exc}") from exc


def supported_acceptance(module: Any, declaration: Mapping[str, Any], family: str):
    ...
    parameters = [
        parameter
        for parameter in inspect.signature(supported).parameters.values()
...
def verify_supported_acceptance(module: Any, declaration: Mapping[str, Any], family: str) -> None:
    expected = supported_acceptance(module, declaration, family)
    if expected is not None and _canonical_json(declaration) != _canonical_json(expected):
        raise StageGateError(f"frozen {family} acceptance differs from the supported claim contract")
...
# Serialized declarations retain their reviewed identity and version. Only the
# implementation location changes; other explicitly declared modules resolve as
# before. No global import aliases or guesses based on the family name.
_MODULE_OWNERS = {
    "perf_pk_claim": "merlin_experiments.phase2.claims.pk",
...
def load_analyzer_module(declared_module: str):
    """Load the canonical owner without changing the frozen analyzer identity."""
    return importlib.import_module(_MODULE_OWNERS.get(declared_module, declared_module))
