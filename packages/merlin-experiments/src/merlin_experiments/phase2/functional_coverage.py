...
def derive(
    cases: Sequence[tuple[str, Path]], *, reference_cost_model: Mapping | None = None, pins: Mapping | None = None
) -> dict[str, Any]:
    ...
    for key, stratum in sorted(strata.items()):
        ...
        scores = {identity: costs[identity] if measured else elements[identity] for identity in sorted(elements)}
        representative = min(scores, key=lambda identity: (scores[identity], identity))
        selected.append(representative)
        rows.append(
            {
                **stratum,
                "workload_sha256": sorted(elements),
...
def verify(
    coverage: Any, cases: Sequence[tuple[str, Path]], members: set[str], *, pins: Mapping | None = None
) -> dict[str, Any]:
    ...
    if coverage is None:
        if members != full:
            raise CoverageError(
                "functional certificate is not the exact admitted public+hidden cohort: "
