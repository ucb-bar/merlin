...
class PinnedProxyConfigError(RuntimeError):
    """A bound configuration must not silently fall back to ambient model routing."""
...
def proxy_config_path() -> Path:
    ...
    explicit = os.environ.get("MERLIN_PROXY_CONFIG")
    if explicit:
        path = Path(explicit).expanduser().absolute()
        expected = os.environ.get("MERLIN_PROXY_CONFIG_SHA256")
        if expected:
            try:
                if _config_digest(path) != expected:
                    raise PinnedProxyConfigError("pinned proxy configuration changed")
            except OSError as exc:
                raise PinnedProxyConfigError("pinned proxy configuration is unavailable") from exc
        return path
    checkout = checkout_root()
    if checkout is None:
        raise RuntimeError("installed proxy bridge requires MERLIN_PROXY_CONFIG")
    return checkout / "merlin/experiments/capsule_bench/proxy/litellm_config.yaml"


def _config_digest(path: Path) -> str:
    if not stat.S_ISREG(path.lstat().st_mode):
        raise PinnedProxyConfigError("proxy configuration must be an ordinary file")
    return hashlib.sha256(path.read_bytes()).hexdigest()


def bind_frozen_proxy_config(snapshot: Path, verified_receipt: dict, environment: dict) -> dict | None:
    """Bind a caller-verified snapshot resource, retaining explicit operator authority.

    The caller verifies the snapshot first and persists the returned host-only identity.
    An absent resource is harmless for native providers; bridge use still requires an
    explicit config in installed mode. Inherited digest pins are never refreshed.
    """
    explicit = environment.get("MERLIN_PROXY_CONFIG")
    relative = "merlin/experiments/capsule_bench/proxy/litellm_config.yaml"
    if explicit:
        path = Path(explicit).expanduser().absolute()
        if not path.exists():
            if environment.get("MERLIN_PROXY_CONFIG_SHA256"):
                raise PinnedProxyConfigError("pinned proxy configuration is unavailable")
            return None
        expected = environment.get("MERLIN_PROXY_CONFIG_SHA256")
        role = "operator_override"
        if path == (snapshot / relative).absolute() and expected == verified_receipt["files"].get(relative):
            # Descendants inherit the selected path and pin, not a fresh override.
            if expected is not None:
                role = "verified_snapshot"
    else:
        expected = verified_receipt["files"].get(relative)
        if expected is None:
            return None
        path = snapshot / relative
        if path.resolve() != path.absolute():
            raise RuntimeError("frozen proxy configuration must not traverse links")
        role = "verified_snapshot"
    digest = _config_digest(path)
    if expected is not None and digest != expected:
        raise RuntimeError("pinned proxy configuration changed")
    environment["MERLIN_PROXY_CONFIG"] = str(path)
    environment["MERLIN_PROXY_CONFIG_SHA256"] = digest
    return {"path": str(path), "sha256": digest, "role": role}


def proxy_venv_python() -> Path:
    from merlin.common.paths import checkout_root

    explicit = os.environ.get("MERLIN_PROXY_EXECUTABLE")
    if explicit:
        return Path(explicit).expanduser().absolute()
    checkout = checkout_root()
    if checkout is None:
        raise RuntimeError("installed proxy bridge requires MERLIN_PROXY_EXECUTABLE")
    return checkout / "build" / "proxy-venv" / "bin" / "litellm"
