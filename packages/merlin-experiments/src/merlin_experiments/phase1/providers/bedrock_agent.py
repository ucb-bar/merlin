...
def _tools(target: str) -> dict:
    """Build fresh tool schemas for the invocation target, without shared mutation."""
    return {
        "tools": [
            {
                "toolSpec": {
                    "name": "run_bash",
                    "description": "Run a bash command in your isolated workspace (cwd = workspace root). "
                    "For build/test/inspect — "
            ...
            },
            {
                "toolSpec": {
                    "name": "write_file",
                    "description": "Create/overwrite a text file at a path relative to the workspace root (e.g. "
                    f"submission/mlir_oot/{target}_opt.py). Content written verbatim — no shell escaping. Use for all "
                    "source files.",
                    "inputSchema": {
                        "json": {
                            "type": "object",
            ...
            },
            {
                "toolSpec": {
                    ...
                    "or an allowed contract path). Answer files "
                    "are masked.",
                    "inputSchema": {
            ...
            },
            {
                "toolSpec": {
                    "name": "self_check",
                    "description": "Grade your CURRENT submission/ against the REAL target oracle "
                    "and get REDACTED feedback: per-tier "
                    "pass/fail, decoded trace, instruction counts, and NUMERIC DIFF STATS (how far off your output is) "
                    "— everything EXCEPT the withheld reference output values (there is no answer key). "
                    "This is your only mid-round signal for whether the "
                    "numerics are correct; call it after each build and iterate until capsules pass. `capsules` = "
                    "comma-separated ids or 'all'.",
                    "inputSchema": {
                        "json": {"type": "object", "properties": {"capsules": {"type": "string"}}, "required": []}
                    },
                }
            },
        ]
...
_DELEGATE_TOOL = {
    "toolSpec": {
        "name": "delegate",
        "description": "Delegate a well-scoped, self-contained sub-task to a cheaper/faster model "
        "to save your own turns "
        ...
        "inputSchema": {
            "json": {
                "type": "object",
                "properties": {"subtask": {"type": "string"}, "context": {"type": "string"}},
                "required": ["subtask"],
            }
        },
    }
...
def _extract_output(
    ...
    *,
    ...
    if mid:
        if signal:
            parts.append(
                f"[… {len(mid)} middle lines elided; the {len(signal)} error/grade-signal line(s) "
                f"below are kept verbatim — re-run with grep/sed/tail to see any other part …]"
            )
            parts.extend(signal)
        else:
            parts.append(
                f"[… {len(mid)} middle lines elided (no error/grade signal detected); re-run with "
...
def _write_file(ws: Path, rel: str, content: str) -> str:
    from ..runtime_environment import RETIRED_READONLY_ENV, RETIRED_READONLY_MESSAGE

    if os.environ.get(RETIRED_READONLY_ENV, "").strip() == "1":
        return f"[refused] {RETIRED_READONLY_MESSAGE}"
    p = (ws / rel).resolve()
...
def _run_subagent(
    ...
    *,
    ...
    sub_tools = {
        "tools": [
            t for t in _tools(te.target)["tools"] if t["toolSpec"]["name"] in ("run_bash", "write_file", "read_file")
        ]
    }
    sys = [
        {
            "text": "You are a focused sub-agent. Do EXACTLY the delegated sub-task and nothing more, then "
    ...
    msgs = [
        {"role": "user", "content": [{"text": f"SUBTASK:\n{subtask}\n\nCONTEXT:\n{context}".strip()}, _CACHE_POINT]}
    ]
    final: list[str] = []
...
def run_round(
    ...
    *,
    ...
    from merlin_experiments.phase1.providers import model_tiers as _MT
    ...
    round_tools = _tools(te.target)["tools"] + ([_DELEGATE_TOOL] if delegate_enabled else [])
    ...
    system = [
        {
            "text": "You are an autonomous compiler engineer with a LIMITED number of tool turns. Build the target "
            ...
            f"package (e.g. `mlir_oot/{te.target}_opt.py`) and each command's argv should reference it as `{{tool}}` — "
            ...
            f"at runtime; your OWN code is only the {te.target}-SPECIFIC lowering. Also browse the full RTL repo "
    ...
    cli = boto3.client(
        ...
        config=_BotoConfig(read_timeout=300, connect_timeout=15, retries={"max_attempts": 5, "mode": "adaptive"}),
    )
    emit(
        {
            "type": "system",
            ...
            "subagent_model": sub_mid if delegate_enabled else None,
        }
    )
    ...
    def _transient(exc: Exception) -> bool:
        ...
        return any(
            w in s
            for w in ("readtimeout", "connect", "timeout", "throttl", "too many", "503", "500", "serviceunavailable")
        )
        ...
        for it in range(max_iters):
            ...
            for attempt in range(4):  # extra loop-level retries on transient errors the SDK didn't absorb
                ...
                try:
                    ...
                    _tc = {"tools": round_tools + [_CACHE_POINT]} if cache_enabled else {"tools": round_tools}
                    resp = cli.converse(
                        modelId=mid,
                        system=_sys,
                        messages=messages,
            ...
            emit(
                {
                    ...
                    "message": {
                        "content": [
                            {
                                "type": "tool_result",
                                "tool_use_id": r["toolResult"]["toolUseId"],
                                "content": r["toolResult"]["content"][0]["text"],
                            }
                            for r in results
                        ]
                    },
