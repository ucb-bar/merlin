...
def _formal_model_simulator(target: str) -> dict:
    ...
    if FORMAL_REQUIRED_TIER not in cfg.rtl_tiers:
        raise RuntimeError(f"formal tier {FORMAL_REQUIRED_TIER} is not an RTL tier for target {target}")
    ...
    if not selected.get("engine") or selected.get("fidelity") != "elaborated_rtl":
        raise RuntimeError(f"formal simulator selection for {target} did not resolve to elaborated_rtl: {selected!r}")
...
def _install_formal_model_simulator(target: str) -> tuple[dict, str | None]:
    ...
    if required is not None and selected.get("engine") != required:
        raise RuntimeError(f"formal model engine {selected.get('engine')!r} differs from required {required!r}")
...
def phase_completion(score: Mapping | None, *, required_tier: str = FORMAL_REQUIRED_TIER) -> tuple[bool, list[str]]:
    ...
    if not isinstance(scope, Mapping):
        ...
        if any(not isinstance(v, int) or isinstance(v, bool) or v < 0 for v in (n_trace, n_model)):
            ...
            if isinstance(n_capsules, int) and not isinstance(n_capsules, bool) and n_trace + n_model != n_capsules:
    ...
    if not isinstance(admission, Mapping):
        ...
        if policy not in {
            "all_discovered",
            "frozen_target_capability_operand_dtype",
        ...
        if any(
            ...
            for v in (source_n, admitted_n, capability_excluded_n, resource_excluded_n, search_excluded_n)
        ):
            reasons.append("cohort_admission_counts_malformed")
        else:
            if source_n <= 0 or admitted_n <= 0:
                reasons.append("cohort_admission_vacuous")
            if source_n != (admitted_n + capability_excluded_n + resource_excluded_n + search_excluded_n):
                reasons.append("cohort_admission_arithmetic_invalid")
            if isinstance(n_capsules, int) and not isinstance(n_capsules, bool) and admitted_n != n_capsules:
...
def _phase_manifest(score: Mapping, *, required_tier: str) -> dict:
    ...
    return {
        "passed": score.get("public_passed")
        or score.get("hidden_passed")
...
def main(argv: list[str] | None = None, *, context: InvocationContext | None = None) -> int:
    ...
    ap.add_argument(
        "--contract", default=None, help="optional frozen contract root used to resolve capsule inheritance"
    )
    ...
    if a.capsules is None:
        ...
        a.capsules = str(context.repo / "merlin/contract/capsules")
    if not legacy_context:
        if a.contract is None:
            ap.error("installed formal grading requires explicit --contract")
    ...
    try:
        _formal_mesh_selection, _inherited_mesh_sim = _install_formal_model_simulator(context.target)
    ...
    (it / "capsule_status_after.yaml").write_text(
        yaml.safe_dump({c["capsule"]: c["status"] for c in pub.get("per_capsule", [])}, sort_keys=True)
    )
    ...
    manifest = {
        ...
        "process": {
            "wall_time_seconds": ctt.get("wall_time_seconds"),
        ...
        "oracle_mode": "no_oracle(L0/L1/trace)"
        if a.no_oracle
        else f"contract-routed({'+'.join(sorted(CR.oracle_adapters(context.target)))})",
        ...
        "gradeable_reason": (
            None if not a.no_oracle else "numeric oracle unavailable — structural (L0/L1/trace) tiers only"
        ),
        "model_mesh_sim": {
            "required_tier": FORMAL_REQUIRED_TIER,
    ...
    fr = [
        f"# {run_dir.name} — final report ({a.arm})",
        "",
        ...
        f"- hidden: {manifest['hidden']['passed']} (after freeze "
        f"{frozen['submission_sha256'][:12] if frozen['submission_sha256'] else 'n/a'})",
    ...
    print(
        f"graded {run_dir.name}: public functional_pass={pub.get('functional_pass')} "
