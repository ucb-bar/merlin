...
accelerator name, no golden); the structural AST scanner is shared with Merlin core.
...
def _load_scan_file():
    """Load the shared structural scanner, never repository-local exemption policy."""
...
def _submission_regex_evidence(sub_dir: Path) -> tuple[list[dict], list[dict]]:
    ...
    if not callable(scan):
        raise RuntimeError("Python regex scanner unavailable; install the matching Merlin core")
    if not sub_dir.is_dir():
        raise RuntimeError("Python regex scan unavailable: submission directory is absent")
    hits: list[dict] = []
    vendored: list[dict] = []
    allowed_hashes = _vendored_xdsl_hashes()
    for py in sorted(sub_dir.rglob("*.py")):
        relative_path = py.relative_to(sub_dir)
        if "__pycache__" in relative_path.parts or "build" in relative_path.parts:
            continue
        try:
            ...
            if hashlib.sha256(py.read_bytes()).hexdigest() in allowed_hashes:
                vendored.append(
                    {"file": relative, "n_hits": len(file_hits), "attribution": "byte_identical_granted_xdsl"}
                )
                continue
            for line, kind in file_hits:
                hits.append({"file": relative, "line": line, "kind": kind})
        except Exception as exc:  # noqa: BLE001 — inability to inspect is not clean evidence
            raise RuntimeError(
                f"Python regex scan failed for {py.relative_to(sub_dir)}: {type(exc).__name__}: {exc}"
            ) from exc
...
def _tool_calls(tpath: Path) -> list[ToolCall]:
    ...
    driver = next(
        (str(e.get("driver") or "") for e in events if e.get("type") == "system" and e.get("subtype") == "init"), ""
    )
    ...
    for e in events:
        ...
        for b in message.get("content", []):
            if isinstance(b, dict) and b.get("type") == "tool_use":
                ...
                succeeded = bool(
                    result is not None
                    and (explicit_error is False or (driver != "codex" and explicit_error is not True))
                )
                out.append(
                    ToolCall(
...
def _exec_str(call: ToolCall) -> str:
    ...
    cmd = call.input.get("command") or call.input.get("cmd") or call.input.get("_raw") or ""
...
def _python_fragments(call: ToolCall) -> tuple[str, ...]:
    ...
    while i < len(lines):
        ...
        if not tokens and lines[i].strip():
            ...
            span = next((j for j in range(i + 1, len(lines) + 1) if _shell_tokens("\n".join(lines[i:j]))), None)
            if span is None:
                i += 1
                continue
        ...
        if "<<" in tokens:
            ...
            end = next((j for j in range(i + 1, len(lines)) if lines[j].strip() == delimiter), len(lines))
...
def _rtl_facts_evidence(call: ToolCall) -> bool:
    ...
    return quoted("target") and any(quoted(key) for key in ("arrays", "memories", "datapaths", "interfaces"))


def _scaffold_generator_evidence(call: ToolCall) -> bool:
    names = _python_call_names(call)
    generators = (
        "mlir_scaffold.generate",
        "xdsl.generate",
        "llvm_plan.generate",
        "target_repo.generate_skeleton",
        "target_repo.emit_contracts",
        "generate_skeleton",
    )
...
def _submission_mutation(call: ToolCall) -> bool:
    ...
    target_text = " ".join(
        str(call.input.get(k) or "") for k in ("file_path", "path", "patch", "content", "new_string", "_raw")
    )
    ...
    names = _python_call_names(call)
    if any(
        name.endswith((".write_text", ".write_bytes", ".mkdir", ".unlink", ".rename")) or name == "open"
        for name in names
    ):
        return True
...
def _rtl_checks_read(calls: list[ToolCall]) -> bool:
    ...
    for call in calls:
        ...
        resolved = next((r for r in _resolved_commands(command) if r[0] in readers), None) if command else None
        ...
        if shell_read and resolved[0] in {"python", "python3"}:
            ...
            if not any(name == "open" or name.endswith((".read_text", ".read_bytes")) for name in calls_in_source):
                continue
        ...
        if (
            ...
            and resolved[0] in {"jq", "awk", "python", "python3"}
            and "rtl_checks" in ex
...
def compute(
    tpath: Path, sub_dir: Path, arm: str, endpoint_kind: str, *, resolved_tools: Iterable[str] | None = None
) -> dict:
    ...
    assisted = (
        arm in _ASSISTED
        if tools is None
        else bool(tools & {"merlin_infra", "xdsl_kit", "cca_spine", "isa_tools", "cca_tools"})
    )
    ...
    regex_scan_error = None
    if xdsl:
        try:
            regex_hits, vendored_regex = _submission_regex_evidence(sub_dir)
        except Exception as exc:  # noqa: BLE001 — an applicable but unavailable gate cannot pass
            regex_scan_error = f"{type(exc).__name__}: {exc}"
    ...
    checks["no_regex_ok"] = False if regex_scan_error else (not regex_hits) if (xdsl and has_py) else None
    checks["isa_tools_used"] = (
        _any(
            calls,
            "isa_tools.py asm",
            "isa_tools.py disasm",
    ...
    if arm4:
        ...
        checks["arm4_discovery_before_submission_mutation"] = all(i is not None for i in discovery) and (
            mutation_idx is None or max(i for i in discovery if i is not None) < mutation_idx
        )
    else:
...
def main(argv=None) -> int:
    ...
    if resolved_tools is None:
        try:
            ...
            resolved_tools = None
    verdict = compute(tpath, run_dir / "submission", arm, endpoint, resolved_tools=resolved_tools)
