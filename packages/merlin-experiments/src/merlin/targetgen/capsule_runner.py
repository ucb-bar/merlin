...
from .runner_config import (  # noqa: F401 — shared config and stable legacy identities
    CONVENTIONAL_RTL_TIERS as _RTL_TIERS,
)
from .runner_config import (
    CONVENTIONAL_TIER_SIM as _TIER_SIM,  # noqa: F401 — stable legacy export
)
from .runner_config import (
    _config_for_target,
    _default_config,  # noqa: F401 — stable legacy export
)

# Core owns metadata. This optional evaluator supplies concrete adapters through
...
from .capsule_common import (  # noqa: F401
...
def _out_of_range_reason(sim: str, idx, size) -> str:
    ...
    return (
        f"the emitted kernel indexed an on-chip resource OUT OF BOUNDS on {sim}: index {idx} with "
...
from .elf_lanes import PROGRAM_ARTIFACT_SUFFIX  # noqa: E402 -- historical public identity
...
def _tier_certificate_key(capsule_name: str, tier: str, *, target, generated, shas, from_rtl: bool):
    ...
    if identity is None:
        return None, None, _TC.execution_identity_reason(target=target, executables=executables, toolchain_shas=shas)
    ...
    if instrument is None:
        return (
            ...
            None,
            (
                f"the instrument for tier {tier} could not be established (the grading "
...
def carried_tier_result(capsule_name: str, tier: str, mandatory: bool, *, target, generated, shas, from_rtl: bool):
    ...
    identity, instrument, why = _tier_certificate_key(
        capsule_name, tier, target=target, generated=generated, shas=shas, from_rtl=from_rtl
    )
    if identity is None:
        return None, why
    ...
    carried["earned_evidence"] = {k: stored.pop(k) for k in _TC.ARTIFACTS_OF_THE_EARNING_RUN if k in stored} or None
    ...
    return TierResult(
        ...
        mandatory,
        reason=(
            "verdict carried: this tier was NOT executed in this run; the same "
...
def record_tier_certificate(
    ...
    *,
    ...
    try:
        ...
        identity, instrument, _why = _tier_certificate_key(
            capsule_name, tier, target=target, generated=generated, shas=shas, from_rtl=from_rtl
        )
        if identity is None:
            return
        _TC.record(
            capsule_name, tier, identity, instrument, status=result.status, tier_result=result.to_dict(), run_id=run_id
        )
...
def suppressed_tier_result(tier: str, mandatory: bool, failed_tier: str, *, from_rtl: bool = False):
    ...
    return TierResult(
        ...
        mandatory,
        reason=(
            f"not run: mandatory tier {failed_tier} already failed, so this deeper tier "
...
def _sim_engine_adapters(sim_via: str, target: str) -> dict[str, Callable]:
    ...
    if sim_via == "chipyard":
        ...
        if plan.selection is None:
            ...
            print(f"[oracle] {target}: no elaborated-RTL engine for L3 — {plan.unavailable_reason}", flush=True)
            return adapters
        sel = plan.selection
        ...
        adapters["L3"] = simulator_adapter(sel["engine"], target, selection=sel)
        return adapters
    return {}
...
def oracle_adapters(target: str, sim_via: str | None = None) -> dict[str, Callable]:
    ...
    if endpoint_kind == "external_backend":
        ...
        plan = _oracle_policy.program_tier_plan(target, model_ext)
        from .program_oracle import program_functional_adapter, program_oracle_adapter, program_verilator_adapter
...
def oracle_available(target: str, sim_via: str | None = None) -> tuple[bool, str]:
    ...
    if endpoint_kind == "external_backend":
        if not mlc_bridge.arc_available(target):
            return False, (
                f"external_backend target {target!r}: mlc arc cosim unavailable "
                f"(set MERLIN_MLC_DIR and build the arc model)"
            )
        if not model_ext:
            return False, (
                f"external_backend target {target!r}: contract declares no runner.model_ext — "
...
def _arc_answers_a_buffer(target: str) -> tuple[bool, str]:
    ...
    if any(got.get(k) != v for k, v in want.items()):
        return False, (
            f"a {tile}x{tile} probe buffer disagreed with the reference — the model answers, "
...
def codegen_smoke(target: str, *, backend_target: str | None = None) -> tuple[bool | None, str]:
    """Check production code generation through the selected support backend.

    True means the probe ran and passed, False refuses admission, and None records
    unavailable coverage, never success. An experiment may declare backend identity;
    simulator names and ISA properties never select a compiler. The hook receives
    the original experiment target.
    """
    from ..runtime.backends import base as backends

    selected = target if backend_target is None else backend_target
    if not isinstance(selected, str) or not selected.strip():
        return False, "codegen smoke requires a non-empty backend target"
    try:
        backend = backends.get_backend(selected)
    except KeyError as exc:
        # Failed plugin loads also raise KeyError; only exact absence permits a skip.
        if backend_target is None and exc.args == (selected,):
            return None, f"n/a (no codegen backend registered for {selected!r})"
        return False, f"codegen backend {selected!r} unavailable: {str(exc)[-200:]}"
    except Exception as exc:  # noqa: BLE001 — selected backend failure refuses admission
        return False, f"codegen backend {selected!r} failed to load: {type(exc).__name__}: {str(exc)[-200:]}"
    hook = getattr(backend, "preflight_codegen_smoke", None)
    if hook is None:
        return None, f"n/a (backend {selected!r} provides no production codegen smoke)"
    if not callable(hook):
        return False, f"backend for {selected!r} exposes a non-callable preflight_codegen_smoke"
    try:
        result = hook(target=target)
    except Exception as exc:  # noqa: BLE001 — a broken production probe refuses admission
        return False, f"production codegen smoke raised {type(exc).__name__}: {str(exc)[-200:]}"
    if (
        not isinstance(result, tuple)
        or len(result) != 2
        or (result[0] is not None and type(result[0]) is not bool)
        or not isinstance(result[1], str)
    ):
        return False, f"backend for {selected!r} returned a malformed preflight_codegen_smoke result"
    return result
...
def _resolve_oracle_adapters(target: str) -> dict[str, Callable]:
    ...
    else arc), never a fixed target or experiment policy. (Named to dodge the ``run_capsule``
...
from .elf_lanes import ACCELERATOR_LANE as _ACCELERATOR_LANE  # noqa: E402
...
from .elf_lanes import (  # noqa: E402 -- historical public identities
    EXECUTED_LANE_EVIDENCE,
    WHOLE_PROGRAM_COMPLETION_EVIDENCE,
)
...
def whole_program_completion_lane_report(
    ...
    command_buffer: dict,
    *,
    global_plan_proof: dict,
    numeric: dict,
    tiers: dict,
    decoded_trace: dict | None,
) -> dict | None:
    ...
    if not required and not forbidden:
        return None
    if (
        (command_buffer.get("kernel_abi") or {}).get("kind") != "whole_program"
    ...
    def _tier_passed(value) -> bool:
        ...
        mandatory = value.get("mandatory") if isinstance(value, dict) else getattr(value, "mandatory", False)
    ...
    if not isinstance(tasks, list) or not tasks or any(not isinstance(row, dict) for row in tasks):
        return None
    completed: list[int] = []
...
def lane_report(
    ...
    * **A FORBIDDEN LANE IS JUDGED ONLY ON EXECUTION.** "The router planned no work here" is not "the
    ...
    agg = {lane: _measured(_records.get(rec, {}).get(key)) for lane, (rec, key) in _LANE_COUNTERS.items()}
    ...
    out = {
        "required": req,
        "observed": sorted(carried),
        "unexercised": unexercised,
        # The lanes this report is ABOUT. `carried` is only ever filled from the declared set above,
        # so `observed` is silent about a lane the capsule never named -- it is not a claim that no
        # such lane ran. The grader cross-checks this report against the dispatch ledger, which sees
        # every lane; without the scope stated here it compared a declared-only set to a ledger-wide
        # one and any capsule that dispatched on more lanes than it declared could never agree.
        "scope": sorted({*req, *forbid}),
    ...
    if plan_only:
        out["caveat"] = (
            f"no execution accounting was recorded for {plan_only}, so those lanes report "
...
def _tier_field(record, key, default=None):
    """One field of a tier record, whichever form it is in -- a :class:`TierResult` or its ``to_dict``."""
    if isinstance(record, dict):
        return record.get(key, default)
    return getattr(record, key, default)


def carried_verdict_is_attributed(record, execution_identity) -> bool:
    """Does this CARRIED tier verdict belong to the bytes being graded right now?

    A carried verdict is not a guess and not stale by construction. :mod:`merlin.targetgen.tier_cache`
    keys a certificate on :func:`tier_cache.execution_identity` -- a content digest of the executables
    this run produced, the target, and the toolchain pins -- and ``lookup`` re-verifies the stored
    record's own copy of that key before returning it, so a hit is a statement about exactly these
    bytes on exactly this instrument. ``tier_promote.record_cert`` resolves against the same identity
    and never re-hashes, so a certificate is never re-attributed to bytes edited since.

    That is why this asks the question rather than refusing outright: per-capsule promotion is the
    designed behaviour in continuous mode, so most graded capsules CARRY their expensive tier, and
    refusing every carried verdict would make a lane contract flap on cache state rather than on
    compiler quality -- the same class of defect the lane evidence exists to remove.

    It is asked INDEPENDENTLY, though: ``execution_identity`` is recomputed here from the bytes on disk
    (see :func:`graded_execution_identity`) and compared to the identity the carried block records. A
    mismatch, a malformed digest, or no identity at all is refused -- fail closed, because an
    unattributable verdict is exactly the stale-certificate case the strict rule was protecting against.
    """
    from .oracle_schedule import valid_execution_digest

    carried = _tier_field(record, "carried")
    if not isinstance(carried, dict):
        return False
    earned = carried.get("execution_identity")
    if not valid_execution_digest(earned) or not valid_execution_digest(execution_identity):
        return False
    return str(earned) == str(execution_identity)


def graded_execution_identity(*, target, generated, shas) -> "str | None":
    """The content identity of what this run would execute, or ``None`` when it cannot be formed.

    The same function the certificate cache keys on, called on the bytes present NOW, so comparing it
    with a carried record's identity is an independent check rather than a restatement of the cache's
    own claim. ``None`` on any refusal (no readable executable, an unidentifiable device), which callers
    must treat as "not attributable".
    """
    from . import tier_cache as _TC

    try:
        return _TC.execution_identity(target=target, executables=run_executables(generated), toolchain_shas=shas)
    except Exception:  # noqa: BLE001 -- an identity we cannot form credits nothing
        return None


def executed_program_tiers(tiers: dict, execution_tiers, *, execution_identity=None) -> list[str]:
    """The oracle tiers that certify THESE bytes on an instrument, and passed.

    ``execution_tiers`` is the runner's own set of tiers that execute the program on an instrument
    (the target's ``tier_sim`` ladder plus any injected adapter) -- the L0/L1 command-buffer
    interpretation is never in it, because interpreting a buffer we built is not running the program.
    A tier is counted only when its record says it passed and that verdict is about this program:

    * ``not_applicable`` / ``budget_deferred`` -- the tier never ran; the first is honestly N/A and the
      second was deliberately not bought. Either way nothing executed, for these bytes or any others.
    * ``carried`` -- executed in an EARLIER run. Counted only when
      :func:`carried_verdict_is_attributed` matches the certificate's identity against
      ``execution_identity``, the identity recomputed from the bytes under grade; refused otherwise, and
      refused outright when the caller supplies no identity to compare against.

    Accepts either a :class:`TierResult` or its ``to_dict`` form, because the model path stores the
    second and the operator path the first.
    """
    out: list[str] = []
    for tier in sorted(str(t) for t in (execution_tiers or ())):
        record = (tiers or {}).get(tier)
        if record is None or _tier_field(record, "status") != "pass":
            continue
        if _tier_field(record, "not_applicable", False) or _tier_field(record, "budget_deferred", False):
            continue
        if _tier_field(record, "carried") and not carried_verdict_is_attributed(record, execution_identity):
            continue
        out.append(tier)
    return out


def operator_path_lane_execution(
    capsule: dict,
    *,
    tiers: dict,
    execution_tiers,
    ...
    """Per-lane execution counts the OPERATOR path can honestly hand :func:`lane_report`, or ``None``.

    The op tier ladder owns no routing plan and no dispatch ledger, which is why a capsule requiring a
    lane went ungraded there (``LANE_CONTRACT_NOT_EVALUATED``). It does own the two halves a positive
    claim actually needs, and this function credits the accelerator lane only when it holds BOTH:

    * **Something ran.** An oracle tier executed this capsule's emitted program on an instrument and
      passed -- see :func:`executed_program_tiers`. A skipped, deferred or unavailable tier is not an
      execution, and neither is our own L0/L1 interpretation of the command buffer. A verdict CARRIED
      from an earlier run counts only when its certificate is attributed to the same
      ``execution_identity`` as the bytes under grade.
    * **What ran drove the accelerator.** The decoded instruction stream carries instructions of THIS
      target's accelerator classes, counted by :func:`merlin.targetgen.trace_check.accelerator_instruction_count`
      -- membership is the target's own RTL decode table claiming the opcode, never a list written here.

    Neither half alone says anything. A passing oracle tier with no accelerator instruction is a program
    the host computed; accelerator instructions with no oracle tier are a stream nobody executed. Only
    together do they say that the accelerator lane carried work, which is exactly what the required-lane
    assertion claims. With either half missing this returns ``None`` and the capsule stays unmeasured --
    ``lane_report`` is never handed a fabricated count, and a routing plan is never consulted.
    """
    from . import trace_check as _TCK

    lanes = capsule.get("lanes") or {}
    if _ACCELERATOR_LANE not in [str(x) for x in (lanes.get("require") or ())]:
        return None
    if not executed_program_tiers(tiers, execution_tiers, execution_identity=execution_identity):
        return None
    if not isinstance(decoded_trace, dict):
        return None
    try:
        n = _TCK.accelerator_instruction_count(decoded_trace)
    except Exception:  # noqa: BLE001 -- an undecodable trace measures nothing, and credits nothing
        return None
    return {_ACCELERATOR_LANE: n} if n > 0 else None


def credit_executed_required_lanes(report: dict | None, capsule: dict, lane_execution: dict | None) -> dict | None:
    """Fold executed-lane evidence into ``report`` for the REQUIRED lanes it settles, leaving the rest alone.

    ``report`` is whatever the negative half already produced -- a linked-ELF or declared-program scan,
    which can refute a ``forbid`` and can never credit a ``require``. That half is copied through
    untouched: its ``violated``/``unmeasured_forbidden``/scan blocks are the artifact-level evidence and
    nothing here may weaken them. What is added is the positive half, and only for lanes that
    :func:`lane_report` itself judged on an executed rung, so the evidence vocabulary has one owner.
    """
    if not lane_execution:
        return report
    positive = lane_report(capsule, None, lane_execution=lane_execution)
    if positive is None:
        return report
    credited = [
        ln
        for ln in positive["required"]
        if ln in positive["observed"] and positive["evidence"].get(ln) in EXECUTED_LANE_EVIDENCE
    ]
    if not credited:
        return report
    # Stated beside the report because `judged_by` is a single label for the whole report and this
    # report now has two halves judged on different rungs -- the per-lane `evidence` mapping is the
    # authority, and this says which lanes the new positive rung answered for, and on how much work.
    provenance = {
        "lane_counts": dict(lane_execution),
        "credited_lanes": sorted(credited),
        "source": "executed oracle tier + decoded instruction trace",
    }
    if report is None:
        # Nothing measured the negative half (no scannable artifact). The positive report stands on its
        # own; any forbidden lane in it is still reported at its own rung and stays unjudged.
        out = dict(positive)
        out["execution_evidence"] = provenance
        return out
    out = dict(report)
    out["evidence"] = {**(report.get("evidence") or {}), **{ln: positive["evidence"][ln] for ln in credited}}
    out["observed"] = sorted(set(report.get("observed") or []) | set(credited))
    out["unexercised"] = [ln for ln in (report.get("unexercised") or []) if ln not in credited]
    still_unmeasured = [ln for ln in (report.get("unmeasured_required") or []) if ln not in credited]
    if report.get("unmeasured_required") is not None:
        if still_unmeasured:
            out["unmeasured_required"] = still_unmeasured
        else:
            out.pop("unmeasured_required", None)
            out.pop("caveat", None)
    out["execution_evidence"] = provenance
    return out
...
def dispatch_boundary_report(mesh_execution: dict | None) -> dict:
    ...
    return {
        "status": "pass" if sequence and not unresolved else "fail",
...
def _absent_outputs(nrep: dict) -> list[str]:
    ...
    return [
        nm
        for nm, d in po.items()
        if isinstance(d, dict)
        and d.get("status") == "fail"
...
def _unwritten_output_detail(nrep: dict, sim_name: str) -> str | None:
    ...
    return (
        f"on {sim_name}, output(s) {shown} came back as a SINGLE CONSTANT value while the expected "
...
def _absent_output_detail(nrep: dict, sim_name: str, expected: dict, observed: dict) -> str | None:
    ...
    return (
        f"on {sim_name}, your emitted artifact never wrote output(s) {', '.join(absent)} — "
...
def _encoding_divergence_hint(
    trace_check_res: dict | None,
    independent_float: bool,
    cb: dict | None = None,
    ...
    if independent_float:
        hint = (
            " Decode your OWN emitted artifact (the disassembler / instruction_trace.json) and verify "
...
def _unexercised_note(unexercised: list[str], exercised: dict) -> dict:
    ...
    return {
        t: (
            "no whole-model analogue: this tier grades a per-op command buffer, which a model "
...
def _numeric_when_not_accelerated(st, gate, verify: dict, cos: float, engine: str, measured_on: str) -> dict:
    ...
    return {
        "status": ("pass" if (st == "verified" and gate) else "fail"),
        "engine": engine,
        "gate": verify or None,
        "cos": (cos if cos else None),
        "measured_on": measured_on,
        "note": (
            "arithmetic only -- this capsule is NOT a pass; see the failure block for what "
...
def _numeric_not_compared(engine: str, measured_on: str, verify: dict | None, detail: str) -> dict:
...
def _batched_tiers_of(target: str | None) -> frozenset[str]:
    """The tiers ``target``'s OWN descriptor declares as batched (measured N windows per job).

    Read from the descriptor rather than named here on purpose: the runner must not know that some
    particular tier means an FPGA behind a shared queue. A target that declares nothing yields the
    empty set and every existing path is unchanged.
    """
    try:
        from .target_experiment import batched_oracle_tiers

        return batched_oracle_tiers(target)
    except Exception:  # noqa: BLE001 — unresolvable descriptor
        return frozenset()
...
def _model_tier_map(
    ...
    *,
    ...
    for t in declared:
        if t in ("L0", "L1"):
            tiers[t] = TierResult(
                t, "skipped", True, not_applicable=True, reason="a whole model has no command buffer to interpret"
            )
    # A BATCHED TIER IS NEVER DERIVED FROM THE MODEL'S OWN COUNTERS. The counters below say which
    # layers reached the accelerator in the run that produced them -- a statement about a SIMULATOR
    # run, not about the FPGA. A tier the target declares batched was measured elsewhere or not at
    # all, so it reads `unavailable` until a measurement is handed in, and `not_run_is_not_pass`
    # turns that into `incomplete`. It is recorded rather than omitted for the reason every other
    # tier is: a tier with no record is not evidence, and an absent key reads as though the capsule
    # was never graded at that tier at all.
    _batched = _batched_tiers_of(target)
    for t in declared:
        if t not in _batched or t in tiers:
            continue
        seen = (measurement or {}).get(t)
        if not isinstance(seen, dict) or not seen.get("status"):
            tiers[t] = TierResult(
                t,
                "unavailable",
                True,
                reason=f"{t} is a batched tier on this target: a capsule accumulates into a "
                "round-group and the group is measured as one queue job. No measurement "
                "for this capsule was handed to the grade, so whether it runs on the "
                "device is UNKNOWN",
            )
            continue
        tiers[t] = TierResult(
            ...
            str(seen["status"]),
            True,
            reason=str(seen.get("reason") or f"{t} measured as one window of a batched job"),
            cycles=seen.get("cycles"),
            derived_from_rtl=bool(seen.get("derived_from_rtl", True)),
            cycle_accurate=bool(seen.get("cycle_accurate", False)),
    ...
    _rtl = model_citable_rtl_tier([t for t in declared if t not in _batched], target)
    ...
    if on is None:
        ...
        tiers[tier] = TierResult(tier, "fail", True, reason=f"{int(fb)} matmul layer(s) fell back to the host kernel")
    elif int(on) == 0:
        tiers[tier] = TierResult(tier, "skipped", True, reason="no matmul layer executed on the accelerator")
    else:
        tiers[tier] = TierResult(tier, "pass", True, reason=f"{int(on)} matmul layer(s) executed on the accelerator")
    return tiers
...
_CHILD_PREAMBLE = _CHILD_GUARD + ("__import__('merlin.targetgen.capsule_runner', fromlist=['main']).main(sys.argv[1:])")
...
def _grade_model_capsule_unlocked(
    capsule: dict,
    *,
    target: str | None = None,
    timeout: int,
    ...
    with _tf.TemporaryDirectory(prefix="model_grade_") as _td:
        ...
        cmd = [sys.executable, "-c", _CHILD_PREAMBLE, "--model-grade", str(spec_p), "--model-grade-out", str(out_p)]
        ...
        if _child_env.get("MERLIN_PASS_LOG"):
            ...
            _child_env["MERLIN_PASS_LOG_REQUIREMENTS"] = json.dumps(list(capsule.get("pass_requirements") or []))
        proc = _sp.Popen(cmd, stdout=_sp.PIPE, stderr=_sp.STDOUT, text=True, env=_child_env, start_new_session=True)
        try:
            ...
            return _stopped(
                ...
                model_budget_s=budget,
                elapsed_s=round(elapsed, 1),
            )
        if not out_p.is_file():
            return _stopped(f"whole-model grade process exited {proc.returncode} without a result: {tail[-400:]}")
        try:
...
def _resolve_model_host_lane(target: str, dtype: str):
    ...
    descriptor = _model_frozen_input(descriptor_path(target), snapshot_identity)
    ...
    if experiment.target != target:
        raise ValueError(f"descriptor {descriptor} names target {experiment.target!r}, not {target!r}")
...
def _resolve_model_numeric_policy(target: str | None) -> tuple[dict | None, dict | None]:
    policy, identity, _ = resolve_numeric_policy(target)
    return policy, identity


def resolve_numeric_policy(
    target: str | None, *, descriptor: Path | None = None
) -> tuple[dict | None, dict | None, Any]:
    """Return policy, byte identity and its descriptor through the already-bound snapshot.

    Consumers deriving a binding must use the returned experiment, not reload its mutable original
    path after policy verification. This does not attest external capability facts or tools.
    """
    if target is None:
    ...
    _, snapshot = _verified_model_host_lane_snapshot()
    source = Path(descriptor) if descriptor is not None else descriptor_path(target)
    experiment = load_target_experiment(_model_frozen_input(source, snapshot))
    if experiment.target != target:
        raise ValueError("model numerical-policy descriptor names a different target")
    original_repo = Path(snapshot["source_repo"]) if snapshot is not None else repo_root()
    declared = numeric_profile_path(experiment.numeric_profile, repo=original_repo)
    frozen = None
...
def _model_frozen_input(source: Path, snapshot: dict | None) -> Path:
    """Map only declared frozen bytes after rechecking the host's expected marker."""
    if snapshot is None:
        return source
    from .sandbox import bwrap as BW
    ...
    if hashlib.sha256(raw).hexdigest() != snapshot["manifest_sha256"]:
        raise ValueError("model host-lane snapshot marker changed during input resolution")
    bundle = _model_snapshot_bundle(json.loads(raw))
    [frozen] = BW.snapshot_input_paths(root.parent / "workspace", bundle, [source], repo=Path(snapshot["source_repo"]))
    return frozen


def _model_snapshot_bundle(manifest: dict) -> dict:
    """Declaration membership from a marker already bound to host-owned provenance."""
    return {key: [{"path": path} for path in manifest.get(key, [])] for key in ("allowed", "host_inputs")}
...
def _verified_model_host_lane_snapshot() -> tuple[Path | None, dict[str, Any] | None]:
    ...
    if raw_required not in ("", "0", "false", "no", "off", "1", "true", "yes", "on"):
        raise ValueError(f"{_MODEL_HOST_SNAPSHOT_REQUIRED_ENV} has invalid boolean value {raw_required!r}")
    required = raw_required in ("1", "true", "yes", "on")
    raw_root = os.environ.get(_MODEL_HOST_SNAPSHOT_ROOT_ENV, "").strip()
    if not raw_root:
        if required or os.environ.get(_MODEL_HOST_SNAPSHOT_RECORD_ENV):
            raise ValueError(
                f"bwrap model grading requires {_MODEL_HOST_SNAPSHOT_ROOT_ENV}; no verified run snapshot was provided"
            )
        return None, None

    lexical_root = Path(raw_root)
    if not lexical_root.is_absolute() or lexical_root.is_symlink():
        raise ValueError(f"{_MODEL_HOST_SNAPSHOT_ROOT_ENV} must name a non-symlink absolute directory")
    try:
    ...
    BW.require_snapshot_ownership(manifest)
    try:
        expected = json.loads(os.environ.get(_MODEL_HOST_SNAPSHOT_RECORD_ENV, ""))
        source_repo = Path(manifest["repo"])
        if not source_repo.is_absolute() or ".." in source_repo.parts:
            raise ValueError("invalid original snapshot repository")
        BW.verify_snapshot_binding(
            root.parent / "workspace", _model_snapshot_bundle(manifest), expected, repo=source_repo
        )
    except (KeyError, TypeError, ValueError, RuntimeError) as exc:
        raise ValueError("model host-lane snapshot lacks matching host-owned provenance") from exc

    # Preserve the bound aggregate identity here. Mapping declaration/profile bytes through
    # snapshot_input_paths verifies the payload; HostLane.resolve additionally checks the selected
    # compiler package before and after compilation.
    ...
    if (
        ...
        or not isinstance(n_files, int)
        or n_files < 1
        or not isinstance(n_bytes, int)
        or n_bytes < 1
    ):
...
def _model_runtime_bundle(capsule: dict, *, timeout: int):
    ...
    def _materialized():
        ...
        interface_names = [str(x) for x in (capsule.get("linalg_mlir"), capsule.get("interface_mlir")) if x]
        if not interface_names or len(set(interface_names)) != 1:
            raise ValueError("model capsule linalg_mlir/interface_mlir declarations are absent or inconsistent")
        ...
        manifest_asset = _model_asset(root, manifest_name, "capture-time weight manifest") if manifest_name else None
        dependency_name = op_attrs.get("loader_dependencies")
        dependency_root = None
        if dependency_name:
            dependency_lexical = root / str(dependency_name)
            if (
                Path(str(dependency_name)).is_absolute()
                or ".." in Path(str(dependency_name)).parts
                or dependency_lexical.is_symlink()
        ...
        source_files = {
            "capsule_declaration": capsule_yaml,
            "loader": loader,
            "linalg_interface": interface,
        ...
        if len(output_signature) != len(output_names):
            raise ValueError(
                f"capsule declares {len(output_names)} outputs but interface returns {len(output_signature)}"
            )
        ...
        for output_name, (out_shape, out_dtype) in zip(output_names, output_signature, strict=True):
            ...
            if output_array.size != int(np.prod(out_shape, dtype=np.int64)):
                raise ValueError(f"frozen golden {output_name!r} element count disagrees with interface shape")
        ...
        try:
            ...
            (bundle / "output_order.json").write_text(json.dumps(output_names, separators=(",", ":")), encoding="utf-8")
            if manifest_asset is not None:
            ...
            request = {
                ...
                "signature": [{"shape": list(shape), "dtype": dtype} for shape, dtype in signature],
                "output_signature": [{"shape": list(shape), "dtype": dtype} for shape, dtype in output_signature],
            ...
            run = subprocess.run(
                [str(torch_python), str(worker), "--request", str(request_path)],
                cwd=str(root),
                env=env,
                capture_output=True,
                text=True,
            ...
            line = next((ln[len(marker) :] for ln in run.stdout.splitlines() if ln.startswith(marker)), None)
            if run.returncode != 0 or line is None:
                detail = (run.stderr or run.stdout or "no diagnostic")[-1200:]
                raise ValueError(
                    f"frozen loader/weights/interface/golden validation failed (rc={run.returncode}): {detail}"
                )
            report = json.loads(line)
            if (
                report.get("loader_sha256") != source_before["artifacts"]["loader"]["sha256"]
                or report.get("weights_sha256") != source_before["artifacts"]["external_weights"]["sha256"]
            ):
            ...
            provenance = {
                ...
                "construction": (
                    "frozen_capsule_assets_v2" if manifest_asset is not None else "frozen_capsule_assets_v1"
                ),
                ...
                "validation": {
                    ...
                    for k in (
                        "torch_export",
                        "capture_manifest_validated",
                        "golden_validated",
                        "golden_value_replay",
                        "weights_validated_exact",
                        "loader_input_count",
                    )
                },
                "tool": {
                    "worker_sha256": _file_identity(worker)["sha256"],
...
def _grade_model_capsule_inline(
    ...
    *,
    ...
    result: dict = {
        ...
        "kind": "model",
        "label": capsule.get("label"),
        "operation": {"op": "model", "model": model, "dtype": dtype, "run": run_where, "target": target},
        "contract_version": CONTRACT_VERSION,
    }
    if not model:
        result.update(
        ...
        try:
            ...
            result.update(
                ...
                failure={
                    ...
                    "detail": f"frozen host lane unavailable: {type(e).__name__}: {str(e)[:300]}",
                },
            )
            return result
    if numeric_policy is _MODEL_NUMERIC_UNSET:
        numeric_policy = None
    if numeric_policy_identity is not None:
        result["numeric_policy_input"] = numeric_policy_identity
    ...
    _requires_boundary_plane = MODEL_BOUNDARY_CAPSTONE in (capsule.get("pass_requirements") or ())
    if _requires_boundary_plane and linalg_path is None:
        result.update(
            status="incomplete",
            failure={
                "plane": "pass_obligations",
                "category": "NOT_RUN_IS_NOT_PASS",
    ...
    try:
        ...
        if _requires_boundary_plane:
            ...
            # WITH the target, so a dispatch is a whole compute group. Without it this plane took the
            # single-op baseline while `compile_cli` formed groups over the same model.
            _plane = run_dialect_plane(parse_mlir_file(linalg_path), n_harts=1, target=target)
    ...
    if _host_experiment is not None and _host_identity is not None:
        try:
            ...
            if _snapshot_before is not None:
                if any(
                    _snapshot_after.get(key) != _snapshot_before.get(key)
                    for key in ("root", "content_sha256", "manifest_sha256")
                ):
            ...
            _, _host_after = _host_experiment.resolve_host_lane(root=_snapshot_repo_after, dtype=dtype)
        except Exception as e:  # noqa: BLE001
            result.update(
                status="incomplete",
                failure={
                    "plane": "model",
                    "category": "NOT_RUN_IS_NOT_PASS",
                    "detail": f"frozen host lane became unreadable during grading: {type(e).__name__}: {str(e)[:300]}",
                },
            )
            return result
    ...
    if run_where == "host":
        result["host_reference"] = {
            "status": st,
            "gate": _v,
            "engine": engine,
            "note": "merlin's own dispatch runtime — a diagnostic, never an oracle for a submitted backend",
        }
        result.update(
    ...
    _lane_rep = lane_report(capsule, result.get("routing_plan"), model_exec, out.get("host_execution"))
    _lane_ok = True
    if _lane_rep is not None:
        ...
        if _lane_rep.get("violated"):
            ...
            result.update(
                ...
                failure={
                    "plane": "model",
                    "category": "ACCELERATED_A_FORBIDDEN_LANE",
    ...
    if n_tiles:
        ...
        _tile_ok = (
            bool(_ok) if _ok is not None else (_failed == 0 and _unavail == 0 and _unsynth == 0 and _passed == n_tiles)
        )
        ...
        if model_exec:
            ...
            _model_ok = _on is not None and int(_on) > 0 and not int(model_exec.get("matmul_layers_host_fallback") or 0)
    ...
    if _delegated:
        ...
        result["contract_obligations"] = {
            "capacity_fit": {
                ...
                "tiled_by": (
                    "declared_primitive_tile" if "declared_primitive_tile" in _tb else (_tb[0] if _tb else None)
                ),
                "n_layers": len(_delegated),
                "layers": _delegated[:8],
                "detail": (
                    "the target backend did not satisfy capacity_fit at these extents; the "
    ...
    if mesh_exec:
        result["tile_evidence"] = {
            "n_tiles": n_tiles,
            "n_passed": mesh_exec.get("n_passed"),
            "note": "synthesized tiles OF THIS MODEL'S SHAPES — evidence that the shapes run, not that this model ran",
        }
    result["op_coverage"] = {
        "note": "the op-pass fraction this capsule was gated on is OP COVERAGE, not a verdict on the model"
    }
    ...
    if (capsule.get("semantic") or {}).get("must_accelerate") and model_exec:
        ...
        if int(_on) == 0 and _unavail and not _fb:
            result.update(
                status="incomplete",
                numeric=_numeric_when_not_accelerated(st, gate, _v, _cos, engine, measured_on="host_lane_unmeasured"),
                failure={
                    "plane": "model",
                    "category": "NOT_RUN_IS_NOT_PASS",
        ...
        if int(_on) == 0 and not _fb and not n_tiles:
            ...
            result.update(
                ...
                numeric=_numeric_not_compared(
                    ...
                    run_where,
                    _v,
                    "nothing was compared ON THE ACCELERATOR: no declared tier ran, so the host-side number is not a verdict about this backend. A comparison that did not happen is not a passing comparison.",
                ),
                failure={
                    "plane": "model",
                    "category": "NOT_RUN_IS_NOT_PASS",
    ...
    _failed_tiers = sorted(
        {t for t, v in exercised.items() if v != "pass"} | {k for k, v in _model_tiers.items() if v.status == "fail"}
    )
    if _failed_tiers:
        result.update(
            status="fail",
            numeric=_numeric_when_not_accelerated(st, gate, _v, _cos, engine, measured_on=run_where),
            failure={
                "plane": "model",
    ...
    if st == "verified" and gate:
        ...
        result.update(
            ...
            numeric={
                ...
                "evidence": (
                    "per-element-guarded"
                    if _guarded
                    else "AGGREGATE ONLY — the cosine-only tier carried this verdict; no per-element bound was applied"
                ),
            },
        )
        if unexercised:
        ...
        result.update(
            ...
            note=(
                f"quantized ({dtype}) whole-model output within quant tolerance of the fp32 "
...
def _finalize_capsule_result(
    *,
    ...
    # binary is not one that ran). See `merlin.targetgen.elf_lanes`.
    #
    # It can prove the POSITIVE half too, but from execution rather than from the artifact, and only by
    # holding both halves of the claim at once: an oracle tier RAN this program on an instrument and
    # passed -- freshly, or on a certificate attributed to these exact bytes -- AND the decoded stream
    # it ran carries instructions of this target's own accelerator classes. See
    # `operator_path_lane_execution` for why neither half alone says anything.
    # Without both, nothing is credited and the capsule stays incomplete -- which is what the four
    # conv-epilogue capsules (`lanes.require: [on_mesh]`, the only ones asserting that a conv's
    # requant/bias/relu rides the mesh store path) hit on every grade before this existed.
    #
    ...
    if _decl_lanes.get("require") or _decl_lanes.get("forbid"):
        ...
        if _lane_rep is None:
            try:
                _lane_rep = _EL.lane_report_from_elf(capsule, paths.generated / _EL.PACKAGE_ELF_NAME, target=eff_target)
            ...
            if _EL.unjudged_lanes(_lane_rep, _decl_lanes):
                ...
                if len(_programs) == 1:
                    try:
                        _program_rep = _EL.lane_report_from_declared_program(capsule, _programs[0], target=eff_target)
                        if len(_EL.unjudged_lanes(_program_rep, _decl_lanes)) < len(
                            _EL.unjudged_lanes(_lane_rep, _decl_lanes)
                        ):
                            _lane_rep = _program_rep
                    except Exception as _pe:  # noqa: BLE001 -- scanner limits are not submission defects
                        import sys as _sys

                        _sys.stderr.write(f"WARNING: declared-program lane scan failed (unmeasured): {_pe}\n")
            # The positive half. Folded in AFTER the artifact scans so the negative half is whatever
            # they concluded, unweakened -- this only ever adds evidence for a REQUIRED lane, and only
            # on a rung `lane_report` itself counts as executed.
            _exec_tiers = execution_tiers if execution_tiers is not None else getattr(cfg, "oracle_tiers", ())
            # Recomputed from the bytes on disk, and ONLY when some tier record actually claims to have
            # been carried -- forming it hashes every executable, and a run in which nothing was carried
            # has nothing to attribute. `None` here refuses every carried verdict, which is the
            # fail-closed direction.
            _graded_identity = None
            if any(_tier_field(tiers.get(t), "carried") for t in _exec_tiers or ()):
                _graded_identity = graded_execution_identity(
                    target=eff_target, generated=paths.generated, shas=toolchain_shas(eff_target)
                )
            _lane_rep = credit_executed_required_lanes(
                _lane_rep,
                capsule,
                operator_path_lane_execution(
                    capsule,
                    tiers=tiers,
                    execution_tiers=_exec_tiers,
                    decoded_trace=decoded_trace,
                    execution_identity=_graded_identity,
                ),
            )
            if _lane_rep is not None:
        ...
        if _lane_rep is not None and _lane_rep.get("violated"):
            ...
            _artifact_kind = "linked executable" if _lane_rep.get("elf_scan") is not None else "oracle-declared program"
            _identity = _scan.get("opcode_source") or "the target's discovered mesh-compute ISA role"
            status = "fail"
            failure = {
                "plane": "lanes",
                "category": "ACCELERATED_A_FORBIDDEN_LANE",
                "detail": (
                    f"capsule forbids lane(s) {_lane_rep['violated']}, and the "
            ...
            if failure is None:
                failure = {
                    "plane": "lanes",
                    "category": "LANE_CONTRACT_NOT_EVALUATED",
                    "detail": (
                        f"capsule declares lanes {dict(_decl_lanes)}; lane(s) {_unjudged} "
    ...
    if status == "pass" and any(getattr(t, "budget_deferred", False) for t in tiers.values()):
        ...
        failure = {
            ...
            "detail": (
                f"passed the screen tier; certify tier(s) {', '.join(_def)} not purchased "
            ...
            if tr is None or tr.status in ("unavailable", "skipped"):
                if no_oracle:
                    ...
                    if failure is None:
                        ...
                        failure = {
                            "plane": "oracle_unavailable",
                            "category": "NOT_RUN_IS_NOT_PASS",
                            "tier": tier,
                            "tier_status": _st,
                            "tier_reason": _why or None,
                            "detail": (
                                f"{_why} (mandatory tier {tier}, status {_st})"
                                if _why
    ...
    if status == "pass" and not no_oracle:
        ran_required = [
            t
            for t in required
            if tiers.get(t) is not None and not getattr(tiers[t], "not_applicable", False) and tiers[t].status == "pass"
        ]
        if not ran_required:
            status = "incomplete"
            if failure is None:
                ...
                failure = {
                    "plane": "declared_tier_unreachable" if _unreach else "oracle_unavailable",
                    "category": "NOT_RUN_IS_NOT_PASS",
                    ...
                    "detail": (
                        ...
                        f"declared."
                        if _unreach
                        else f"no runnable required oracle tier certified this capsule "
    ...
    result = {
        ...
        "trace_check": trace_check_res,
        "numeric": numeric,
        "failure": failure,
        "toolchain_shas": toolchain_shas(eff_target),
    }
...
def run_capsule(
    ...
    *,
    ...
    dtype: str = "i8xi8_i32",
    config=None,
    perf_extractor: Callable | None = None,
    no_oracle: bool = False,
    workers: int | None = None,
    adapter_managed_tiers: tuple[str, ...] = (),
) -> dict:
    ...
    # contract — never a fixed experiment's simulator pair. `{}` stays honest no-oracle (L0/L1/trace only).
    ...
    if capsule.get("kind") == "model":
        ...
        (paths.run_path / "model_grade_started.json").write_text(
            json.dumps(
                {
                    "capsule": name,
                    "started_at": _dt.datetime.now(_dt.timezone.utc).isoformat(),
                    "model_budget_s": _budget,
                    "step_timeout_s": timeout,
                    "note": "whole-model capsule; absent capsule_result.json means it is still running or was killed",
                },
                indent=2,
            ),
            encoding="utf-8",
    ...
    try:
        ...
        pkg, cb, llvm_text = run_entrypoints(
            pkg,
            package_dir,
            capsule,
            paths,
        ...
        if _lowering.get("status") in _LOB.REFUSING_STATUSES:
            raise CertFailure("lowering_obligation", _cat("PROTOCOL_VIOLATION"), str(_lowering.get("detail")))
        ...
        if _declared:
            _epi = _EPI.assess(
                cb,
                [
                    _EPI.ReadoutCapability(
                        str(r["selector"]),
                        frozenset(str(x) for x in r.get("applies") or ()),
            ...
            if _epi.refusing:
                raise CertFailure("epilogue_applicability", _cat("PROTOCOL_VIOLATION"), str(_epi.detail))
        else:
            epilogue_applicability = {
                "schema": "merlin_epilogue_applicability_v1",
                "status": "unavailable",
                "detail": (
                    f"target {eff_target!r} declares no readout epilogue capability, so "
        ...
        if _vals:
            ...
            if not (set(_vals) & set(_tensors)):
                ...
                if _leaves is None:
                    _leaves = [n for n, s in _tensors.items() if s.get("role") in ("input", "weight", "bias")]
        ...
        if independent_float:
            ...
            numeric = {
                ...
                "golden_source": gsource,
                "note": (
                    "command-buffer reference/simulate are inapplicable to an explicitly declared "
            ...
            numeric = {
                "status": nrep["status"],
                "policy": nrep["policy"],
                "max_abs_diff": nrep["max_abs_error"],
                "max_rel_error": nrep["max_rel_error"],
                "mismatch_count": nrep["mismatch_count"],
                "first_mismatch": nrep["first_mismatch"],
            ...
            tiers["L0"] = TierResult(
                "L0",
                "pass" if nrep["status"] == "pass" else "fail",
                mandatory="L0" in required or True,
                reason=(
                    None if nrep["status"] == "pass" else "your command buffer does not compute the declared operation"
                ),
                evidence="numeric_report.yaml",
            )
            if nrep["status"] != "pass":
                _absent_cb = _absent_output_detail(nrep, "command_buffer", gold, ref) or _unwritten_output_detail(
                    nrep, "command_buffer"
                )
                raise CertFailure(
                    "numeric_golden",
                    _cat("FUNCTIONAL_MISMATCH"),
                    _absent_cb
                    or (
            ...
            if not l1_ok:
                raise CertFailure(
                    "command_buffer_reference", _cat("FUNCTIONAL_MISMATCH"), "reference(cb) != simulate(cb)"
                )

        # Bound whether or not the trace gate runs: `None` means "nobody parsed these bytes here",
        # which the plan evidence answers by parsing them itself.
        _parsed_lowered_shared = None
        ...
        if cfg.trace_gate == "rocc_insn":
            # PARSED ONCE, then shared. This is the first of up to three full xDSL parses of the same
            # bytes in one grade; on a 29 MB artifact each costs about 44 s plus 5 s to verify, so the
            # plan evidence below is handed THIS module instead of parsing the text again. The decoder
            # keeps its own text path for an artifact that is not a parseable module.
            _parsed_lowered_shared = RD._parse_module(llvm_text)
            trace = (
                RD.decode_module(
                    _parsed_lowered_shared, target=eff_target, source=str(paths.generated / cfg.fourth_output_name)
                )
                if _parsed_lowered_shared is not None
                else RD.decode_text(llvm_text, source=str(paths.generated / cfg.fourth_output_name), target=eff_target)
            )
            ...
            (paths.generated / "instruction_trace.json").write_text(json.dumps(trace, indent=2), encoding="utf-8")
            ...
            try:
                from merlin.liveness import Program as _LProg
                from merlin.liveness import assess as _lassess
            ...
            trace_check_res = TCK.check(
                trace,
                capsule.get("expected", {}),
                cb=cb,
                address_model="pointer_args",
            ...
            # `trace_all_pass`, which gates the run's formal verdict, and a program that moves one tile
            # per transfer is slow, not wrong. Reported under its own key so the author can see the
            # declared transfer width going unused without a performance note being able to fail a
            # correct capsule. Silent unless the target declares a payload bound.
            _movement = TCK.movement_findings(trace, TCK.movement_bound_for(eff_target))
            if _movement:
                trace_check_res["movement"] = _movement
        ...
        # Selected support providers express infrastructure absence through the
        # shared OracleUnavailable identity, retaining their own subclasses and
        # messages. Grading must not import an unrelated target to classify errors.
        ...
        if _execution_ceiling is not None:
            ...
            _available_tiers = {
                t for t, index in _tier_indices.items() if index is not None and index <= _ceiling_index
            }
        ...
        for tier in _tier_seq:
            ...
            if tier != _screen_tier:
                # THE CAPSULE'S OWN DECLARED CEILING, honoured HERE and not only in promotion.
                # `tier_policy.oracle_ceiling` was called by `tier_promote` (which enqueues cert jobs
                # between grades) and by nothing inside the grade itself, even though this TierResult
                # already carries an `oracle_ceiling` field to record it. So a capsule declaring
                # `max_oracle_tier: L2` with a VERIFIED `extends` still had the cert tier run on it
                # during every grade. MEASURED on one run: six such capsules, each declaring L2 and each
                # resting on a sibling that did earn L3, ran the cert tier to the 1200s per-capsule
                # timeout and failed there -- 7230s of adapter wall, about half the grade, spent
                # re-certifying claims their own declarations said they were not making.
                _ceil = _tier_policy.oracle_ceiling(
                    str(cfg.target or target or ""),
                    capsule,
                    tier,
                ...
                if not _ceil.allowed:
                    tiers[tier] = TierResult(
                        ...
                        oracle_ceiling=_ceil.record,
                    )
                    continue
                _may, _why = _tier_policy.may_certify(str(cfg.target or target or ""), capsule)
                if not _may:
                    tiers[tier] = TierResult(
                        tier, "skipped", mand, reason=_why, budget_deferred=True, derived_from_rtl=tier in cfg.rtl_tiers
                    )
                    continue
            ...
            if adapter is None:
                if mand:
                    ...
                    tiers[tier] = TierResult(
                        ...
                        True,
                        reason=(
                            f"this capsule declares required oracle tier {tier} "
            ...
            if _executed_here and tier not in adapter_managed_tiers:
                _carried, _why_not = carried_tier_result(
                    name,
                    tier,
                    mand,
                    target=eff_target,
            ...
            try:
                ...
                _l2_cycles = _stage.get("predecessor_l2_cycles") if isinstance(_stage, dict) else None
                if _l2_cycles is not None:
                    if not isinstance(_l2_cycles, int) or isinstance(_l2_cycles, bool) or _l2_cycles <= 0:
                        raise ValueError("evaluation_stage.predecessor_l2_cycles must be a positive integer")
                    oracle_cb["_oracle_l2_cycles"] = _l2_cycles
                    _cycle_policy = _stage.get("cycle_budget")
                    if not isinstance(_cycle_policy, dict):
                        raise ValueError("evaluation_stage with predecessor cycles requires cycle_budget policy")
                ...
                tiers[tier] = TierResult(
                    ...
                    "fail",
                    mand,
                    reason=_did_not_halt_reason(str(e)),
                    derived_from_rtl=tier in cfg.rtl_tiers,
                ...
                if _is_readback_refusal(e):
                    ...
                    if mand:
                        raise InfraFailure(
                            INFRASTRUCTURE_PLANE,
                            InfraCategory.READBACK_TRANSPORT_REFUSED,
                            ...
                            f"submission -- the kernel may be correct.",
                        ) from e
                    continue
                ...
                # WALL-CLOCK, not cycle-cap. `subprocess.TimeoutExpired` stringifies as "Command '...'
                # timed out after N seconds"; `did not halt` is the other, opposite condition and wins.
                _timed_out = ("timed out after" in _msg) and not _did_not_halt
                ...
                tiers[tier] = TierResult(
                    ...
                    reason=(
                        ...
                        if _did_not_halt
                        else _oracle_timeout_reason(_sim, _msg)
                        if _timed_out
                        else f"kernel faulted at runtime: {_clip(_msg, 260)}"
                        if _trapped
                        else _out_of_range_reason(_sim, _oob_idx, _oob_size)
                        if _oob
                        else f"the emitted package did not LINK (your build defect, not the "
                        f"simulator's): {_clip(_msg, 600)}"
                        if _link_failed
                ...
                if mand:
                    ...
                    if _trapped:
                        raise CertFailure(
                            ...
                            f"layout, never a baked 0 or a guessed address): {_clip(_msg, 300)}",
                        ) from e
                    ...
                    if _oob:
                        raise CertFailure(
                            _sim, _cat("FUNCTIONAL_MISMATCH"), _out_of_range_reason(_sim, _oob_idx, _oob_size)
                        ) from e
                    if _link_failed:
                        raise CertFailure(
                            _sim,
                            ...
                            f"not a simulator fault: {_clip(_msg, 600)}",
                        ) from e
                    if _timed_out:
            ...
            if res.get("executability") is not None:
                ...
                tiers[tier] = TierResult(
                    tier,
                    "pass" if ex.get("legal") else "fail",
                    mandatory=False,
            ...
            if res.get("completion_only"):
                ...
                if perf_extractor is not None:
                    ...
                    _cu = {k: v for k, v in _cperf.items() if k not in ("gflops", "pct_fp_peak", "flops")} or None
            ...
            if onboard_numeric is not None:
                ...
                if mand or numeric.get("status") == "skipped":
                    numeric = {
                        ...
                        "policy": onrep["policy"],
                        "golden_source": gsource,
                        "max_abs_diff": onrep["max_abs_error"],
                ...
                okt = (
                    _match_by_policy(res["outputs"], gold, policy)
            ...
            if perf_extractor is not None:
                ...
                _util = {k: v for k, v in _perf.items() if k not in ("gflops", "pct_fp_peak", "flops")} or None
            ...
            _absent_detail = (
                (
                    _absent_output_detail(onrep, sim_name, gold, res["outputs"])
                    or _unwritten_output_detail(onrep, sim_name)
                )
                if direct_oracle_golden and onrep is not None
                else None
            )
            ...
            _derived_from_rtl = (
                _oracle_meta.get("derived_from_rtl", tier in cfg.rtl_tiers)
                if isinstance(_oracle_meta, dict)
                else (tier in cfg.rtl_tiers)
            )
            ...
            if not okt and mand:
                _cf = CertFailure(
                    ...
                    _cat("FUNCTIONAL_MISMATCH"),
                    _mismatch_reason
                    + _encoding_divergence_hint(
                        trace_check_res, direct_oracle_golden, cb=cb, capsule=capsule, trace=decoded_trace
                    ),
                )
                    ...
                    for _later in _tier_seq[_tier_seq.index(tier) + 1 :]:
                        if _later not in tiers:
                            tiers[_later] = suppressed_tier_result(
                                _later, _later in required, tier, from_rtl=_later in cfg.rtl_tiers
                            )
    ...
    _work_volume, _cb_artifact = _cb_evidence(cb, compiler_provenance="submission command-buffer contract entrypoint")
    _lane_extra: dict[str, Any] = {}
    if (
        whole_program
        and cb is not None
        and ((capsule.get("lanes") or {}).get("require") or (capsule.get("lanes") or {}).get("forbid"))
    ):
        try:
            ...
            _plan_proof = verify_compiler_global_plan(
                ...
                lowered_text=llvm_text,
                command_buffer=cb,
                candidate_sha256=_candidate_digest,
                # The module parsed from these exact bytes above, in this invocation, unmutated --
                # the hook's own precondition. Verification still runs; only the reparse is saved.
                parsed_lowered_module=_parsed_lowered_shared,
                command_buffer_sha256=_hashlib.sha256(
                    json.dumps(cb, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()
                ).hexdigest(),
            )
            _lane_extra["global_plan_verification"] = _plan_proof
            _completed_lane_report = whole_program_completion_lane_report(
                capsule, cb, global_plan_proof=_plan_proof, numeric=numeric, tiers=tiers, decoded_trace=decoded_trace
            )
            ...
            _lane_extra["global_plan_verification"] = {
                "status": "UNKNOWN",
                "reason": (f"whole-program lane proof unavailable: {type(_lane_exc).__name__}: {_lane_exc}"),
            }
    ...
    _movement_volume = _mv_evidence(cb, compiler_provenance="submission command-buffer contract entrypoint")
    return _finalize_capsule_result(
        ...
        failure=failure,
        tiers=tiers,
        trace_check_res=trace_check_res,
        numeric=numeric,
        required=required,
        no_oracle=no_oracle,
        ...
        epilogue_applicability=epilogue_applicability,
        declined=declined,
        decoded_trace=decoded_trace,
        execution_tiers=_available_tiers,
        extra={
...
def _run_suite(
    ...
    *,
    ...
    def _one(cap: dict, workers: int = 1) -> dict:
        ...
        with _pass_recorder.pass_run_context(str(cap["name"]), cap.get("pass_requirements") or ()):
            return run_capsule(
                cap,
                package_dir,
                runs_root=runs_root,
    ...
    if _cover:
        print(
            f"  tier plan: {len(_cover)}/{len(op_caps)} eligible capsule(s) form the derived covering "
            f"set (certified first); budget="
            f"{_tier_policy.budget_seconds() or 'unlimited'}",
            flush=True,
            file=sys.stderr,
        )
    ...
    if ungradeable:
        print(
            f"  {len(ungradeable)} capsule(s) NOT GRADED — outside this target's declared capability: "
            f"{', '.join(r['capsule'] for r in ungradeable[:6])}"
            f"{' ...' if len(ungradeable) > 6 else ''}",
            flush=True,
            file=sys.stderr,
        )
    if not model_caps:
    ...
    graded = [r for r in op_results if r.get("status") not in NOT_MEASURED_STATUSES]
    ...
    if len(eligible) != len(graded):
        print(
            f"  model gate: {len(graded) - len(eligible)} graded op capsule(s) excluded from the gate "
    ...
    if _screened:
        print(
            f"  model gate: denominator is the {len(denom)} CERTIFIED capsule(s); "
            ...
            f"what was certified, not over the whole suite",
            flush=True,
            file=sys.stderr,
        )
    model_results = []
            ...
            model_results.append(
                {
                    ...
                    "failure": {
                        "plane": "gate",
                        "category": "GATED",
                        "detail": f"whole-model capsule deferred: op pass fraction {frac:.2f} < gate {thr}",
                    },
                }
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not caps:
        ...
        print(
            f"no capsule discovered under {str(where)!r}{why} -- refusing to report a verdict over "
            f"an empty cohort (capsule discovery does not descend into symlinked directories)",
            file=sys.stderr,
        )
        return 2
    results = run_suite(caps, a.package, runs_root=a.runs_root, contract=a.contract, timeout=a.timeout, target=a.target)
    ...
    if not results:
        print(
            f"{len(caps)} capsule(s) discovered but the suite returned no result row -- nothing was measured",
            file=sys.stderr,
        )
        return 2
