...
def parse_agent_transcript(
    path: str | Path, *, driver: str, model: str = "", billing_mode: str = METERED, include_cost: bool = True
) -> dict:
    ...
    ``include_cost=False`` returns usage evidence only, without consulting ambient prices or
    the process-global price cache. Callers with a separately pinned price authority own pricing.
    """
    p = Path(path)
    if not p.is_file():
        return {"available": False, "reason": f"transcript not found: {p}"}
