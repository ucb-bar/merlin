...
def probe(
    ...
    *,
    ...
    try:
        ...
        te = load_target_experiment(selected.descriptor)
        if selected.target != target or te.target != target:
        ...
        binding = corpus_spec.derive_binding(te, profile.get("datapath", {}))
    ...
    for m, k, n in shapes:
        ...
        try:
            ...
            built = writer._write_capsule(entry, binding, root / f"m{m}k{k}n{n}")
