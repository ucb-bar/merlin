"""Experiment-owned bounded reduced-source A/B observations, not model-cost calibration."""
...
def _record_task_route_presence(result, *, candidate, probes, pair, prepared):
    """Add scoped relevance without altering raw numerical/runtime observations."""
    from .source_contraction_preparation import task_route_feedback, task_route_presence_for_preparation

    comparison = task_route_presence_for_preparation(candidate=candidate, probes=probes, pair=pair, prepared=prepared)
    result.update(
        ...
        prepared_task_route_presence_matches=(
            document_digest(comparison) == prepared.get("task_route_presence_sha256")
...
def _validated_inputs(prepared, pair):
    ...
    if (
        prepared.get("schema") != "source_contraction_preparation_v1"
    ...
    source, extraction = extract_source_contraction(
        pair.source, prepared["source_op_index"], entry=prepared["entry"], **bounds
    )
    ...
    if (
        oracle.get("schema") != "source_contraction_prepared_oracle_v1"
        or oracle.get("probe_source_sha256") != text_digest(source)
        or any(
            oracle.get(key) != extraction[key]
            for key in ("input_shapes", "input_dtypes", "output_shape", "output_dtype")
        )
        or not isinstance(oracle.get("cases"), list)
        or not oracle["cases"]
    ):
        raise ValueError("prepared typed oracle has a different source boundary")
    case = oracle["cases"][0]
    arrays, payloads = [], []
    for values, shape, dtype, expected_sha in zip(
        case["inputs"], oracle["input_shapes"], oracle["input_dtypes"], case["input_le_sha256"], strict=True
    ):
...
class SourceProgramPairProvider:
    ...
    def __call__(self, *, candidate, probes, prepared, timeout_s=60):
        ...
        if probes.analysis.session.inputs.target != self.target:
        ...
        result = {
            "schema": "complete_source_pair_execution_v1",
            "status": "UNKNOWN",
            "arms": {},
            "preparation_sha256": document_digest(prepared),
            "workdir": str(work),
            "full_model_executed": False,
            "full_model_recompiled": False,
            "global_speedup_proven": False,
            "global_cost_calibration": False,
            "selected_task_route_correspondence": "UNPROVEN",
        ...
        try:
            ...
            portfolio_member = (
                comparison_binding.get("selection")
                if comparison_binding.get("schema") == "selected_portfolio_source_program_pair_v1"
                else None
            )
            ...
            _record_task_route_presence(result, candidate=candidate, probes=probes, pair=pair, prepared=prepared)
            ...
            result.update(
                full_model_artifact_binding=_binding(pair),
                source_sha256=pair.source_sha256,
                probe_source_sha256=text_digest(source),
                logical_dispatch_digest=pair.graph_sha256,
                selected_case=case["case"],
                unexecuted_cases=[c["case"] for c in oracle["cases"][1:]],
                expected_le_sha256=case["expected_le_sha256"],
                input_le_sha256=case["input_le_sha256"],
            )
            ...
            for arm in ("before", "after"):
                ...
                cb = json.loads(
                    _read(entry["command_buffer_path"], entry["command_buffer_canonical_sha256"], 2 * 1024 * 1024)
                )
                if (
                    entry["source_sha256"] != text_digest(source)
                    ...
                    or entry["source_task_cfg_proof"].get("plan_digest")
                    != document_digest(cb["params"]["global_program_plan"])
                ...
                if proof.get("status") != "verified":
                    ...
                    admission = numerical_probe_admission(
                        source_text=source,
                        lowered_text=lowered,
                        command_buffer=cb,
                        declared_proof=proof,
                        candidate_sha256=entry["compiler_sha256"],
                        source_op_index=prepared["probe_source_op_index"],
                    )
                    if admission.get("status") != "source_bound_numerical_probe" or document_digest(
                        admission
                    ) != document_digest(entry.get("short_execution_admission")):
                        raise ValueError(
                            "refused declared plan lacks an exact separate short numerical-probe admission"
                        )
                ...
                arm_work = work / arm
                arm_work.mkdir()
                policy = probes.native_probe_policy(candidate, arm_work)
                environment = self.adapter.short_program_environment(policy)
                build = self.adapter.prepare_short_program_build(
                    source_text=source,
                    lowered_text=lowered,
                    command_buffer=cb,
                    logical_payloads=dict(zip(inputs, payloads, strict=True)),
                    source_evidence={
                        "probe_source_sha256": text_digest(source),
                        "lowered_sha256": text_digest(lowered),
                        "selected_source_op_index": prepared["source_op_index"],
                        "compiler_sha256": entry["compiler_sha256"],
                        "declared_source_plan": proof,
                        "short_execution_admission": admission,
                    },
                    workdir=arm_work / "build_worker",
                    python_executable=environment["python_executable"],
                    build_namespace_root=environment["build_namespace_root"],
                    build_path_bindings=environment["build_path_bindings"],
                )
                dependencies = build.dependencies
                cap = HostBuildDependencies(
                    source_root=str(repo_root()),
                    namespace_root=str(environment["build_namespace_root"]),
                    argv=build.argv,
                    file_pins=tuple(json.loads(build.pins_json).items()),
                    source_grants=tuple(
                        (d["source"], d["destination"], d["sha256"]) for d in dependencies if d["kind"] == "python"
                    ),
                    request_path=build.request_path,
                    worker_path=str(arm_work / "build_worker/worker.py"),
                    format_data=tuple(
                        (d["source"], d["destination"], d["sha256"], d["role"])
                        for d in dependencies
                        if d["kind"] == "format_data"
                    ),
                )

                def bounded_build(argv, *, timeout_s):
                    execution = probes.run_native_probe(
                        candidate, arm_work, argv, timeout_s=min(timeout_s, remaining()), _build_dependencies=cap
                    )
                ...
                result["arms"][arm] = {
                    ...
                    "declared_plan_status": proof["status"],
                    "short_execution_admission": admission,
                }
                ...
                command = self.adapter.prepare_short_program_execution(
                    Path(built["elf_path"]),
                ...
                try:
                    ...
                    for name in ("stdout", "stderr"):
                        ...
                        if raw is not None:
                            (arm_work / ("runtime." + name)).write_text(
                ...
                remaining()
            final_pair = bind_source_program_pair(
                candidate=candidate,
                probes=probes,
                comparison_arm=prepared["comparison_arm"],
                portfolio_member=portfolio_member,
            ...
            if (
                _binding(final_pair) != _binding(pair)
                or document_digest(final_pair.comparison_binding) != initial
                or probes.analysis.session.current_probe_binding(candidate) != initial_controller_binding
            ...
            result.update(
                ...
                limitations=[
                    "other prepared oracle cases not executed",
                    "full-shape numerical equivalence UNPROVEN",
                    "selected changed task/route correspondence UNPROVEN",
                    "no extrapolation to model cost",
