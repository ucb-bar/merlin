...
def prove_implicit_zero_initializer(*, source_text: str, command_buffer: dict, source_op_index: int) -> dict:
    ...
    """
    result = {
        "schema": "implicit_zero_initializer_evidence_v1",
        "status": "UNKNOWN",
        "source_sha256": text_digest(source_text),
        "command_buffer_sha256": document_digest(command_buffer),
        "declared_plan_modified": False,
        "emitted_machine_initialization": "UNPROVEN",
        "numeric_equivalence": "UNPROVEN",
        "scope": "source initializer to abstract command-buffer zero seed only",
    }
    try:
        ...
        _, dimensions = _contract(selected)
        if dimensions["batch_shape"] or dimensions["output_rank"] != 2:
            raise ValueError("initializer admission requires an unbatched rank-2 contraction")
        m, k, n = (dimensions[name] for name in ("m", "k", "n"))
        if m * k * n > 20000:
            raise ValueError("initializer admission is only for a bounded short contraction")
        fill = selected.operands[2].owner
        if (
            not isinstance(fill, Operation)
            or fill.name != "linalg.fill"
            or fill not in ops
        ...
        if (
            len(fill.regions) != 1
            or len(fill.regions[0].blocks) != 1
            or len(scalar) != 1
            or scalar[0].name != "linalg.yield"
            or len(scalar[0].operands) != 1
        ...
        if (
            ...
            or len(plan["tasks"]) != 1
            or task is None
            or task.get("kind") != "contraction"
            or task["source_op_indices"] != [source_op_index]
        ):
        ...
        if len(entries) != len(block.args) or len(set(entries)) != len(entries) or abi["kind"] != "whole_program":
        ...
        values = [
            row for row in plan["source_values"] if row["op_index"] == source_op_index and row["result_index"] == 0
        ]
        if (
            len(values) != 1
            or plan["output_bindings"] != [values[0]["tensor"]]
            or abi["outputs"] != plan["output_bindings"]
            or task["reads"] != [lhs, rhs]
        ):
        ...
        if (
            set(pack_ops) != {"src", "dst"}
            or pack_ops["src"] != rhs
            or pack.get("attributes", {}) != {"layout": "packed_rhs"}
            or set(mm_ops) != {"lhs", "rhs", "dst"}
            or mm_ops["lhs"] != lhs
            or mm_ops["rhs"] != pack_ops["dst"]
            or matmul.get("attributes", {})
            or mm_ops["dst"] in set(entries) | set(command_buffer["tensors"]) | {pack_ops["dst"]}
            or set(commit_ops) != {"src", "dst"}
            or commit_ops["src"] != mm_ops["dst"]
        ):
        ...
        if (
            set(attrs) != {"epilogue", "output_dtype"}
            or attrs["epilogue"] != []
        ...
        if commit_ops["dst"] != destination and (
            commit_ops["dst"] != temporary
            or not any(
                row.get("tensor") == temporary
                and row.get("purpose") == "accumulator_readout"
                and row.get("source_op_index") == source_op_index
        ...
        semantics = module_source_path("merlin.runtime.tensor")
        result.update(
            status="proved",
            implicit_source_op_indices=implicit,
            owning_source_op_index=source_op_index,
            owning_task_index=task["task_index"],
            source_inputs={"lhs": lhs, "rhs": rhs},
            command_indices=[0, 1, 2],
            fresh_accumulator=mm_ops["dst"],
            command_semantics="fresh MATMUL_RESIDENT dot product, no prior accumulator operand",
            shared_semantics_source={
                "path": str(semantics),
...
def numerical_probe_admission(
    *,
    ...
    evidence = prove_implicit_zero_initializer(
        source_text=source_text, command_buffer=command_buffer, source_op_index=source_op_index
    )
    result = {
        "schema": "short_initializer_execution_admission_v1",
        "status": "UNKNOWN",
        "source_sha256": text_digest(source_text),
        "lowered_sha256": text_digest(lowered_text),
        "command_buffer_sha256": document_digest(command_buffer),
        "candidate_sha256": candidate_sha256,
        "declared_proof_sha256": document_digest(declared_proof),
        "initializer_closure": evidence,
        "declared_plan_modified": False,
    ...
    if len(lowered_text.encode()) > 2 * 1024 * 1024:
        ...
        return result
    actual = verify_compiler_global_plan(
    ...
    if (
        document_digest(actual) != document_digest(declared_proof)
        or actual.get("status") != "refused"
    ...
    result.update(
        ...
        runtime_requirements=[
            "bounded complete source, no full model",
            "exact source inputs including initializer",
