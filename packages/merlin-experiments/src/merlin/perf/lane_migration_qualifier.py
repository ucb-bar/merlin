...
def contraction_lane_migrations(
    source_text: str, before: Mapping[str, Any], after: Mapping[str, Any]
) -> list[dict[str, Any]]:
    ...
    for task in _tasks(after):
        ...
        if all(owner is not None and owner.get("kind") == "contraction" for owner in old):
            continue
        if (
            any(owner is None or owner.get("kind") != "host" for owner in old)
        ...
        batching = (
            "batched" if geometry["lhs_batched"] else "broadcast",
...
def representative_migrations(
    rows: list[Mapping[str, Any]], *, maximum: int = 4
    ...
    remaining_batching = {tuple(row["operand_batching"][side] for side in ("lhs", "rhs")) for row in rows}
    ...
    if (
        coverage["observed_iteration_ranks"] != coverage["qualified_iteration_ranks"]
...
class LaneMigrationContractionQualifier:
    ...
    def __init__(
        ...
        *,
        target: str,
        runtime_provider: Callable[..., Mapping[str, Any]],
    ...
    def __call__(
        ...
        record: dict[str, Any] = {
            "schema": "lane_migration_contraction_qualification_v1",
            "status": "UNKNOWN",
        ...
        try:
            if not callable(getattr(probes.analysis.session, "selected_changed_portfolio_context", None)):
            ...
            if (
                prior_source.read_text() != source_text
                or text_digest(source_text) != member_binding["current"]["source_sha256"]
                or member_binding["previous"]["source_sha256"] != member_binding["current"]["source_sha256"]
            ):
            ...
            representatives, coverage = representative_migrations(rows, maximum=self.maximum_representatives)
            record.update(
                migrations=rows,
            ...
            entry = next(
                op.sym_name.data for op in parse_mlir_text(source_text).body.block.ops if op.name == "func.func"
            )
            for ordinal, row in enumerate(representatives):
                remaining()
                prepared = prepare_source_contraction(
                    candidate=candidate,
                    probes=probes,
                    comparison_arm="previous",
                    source_op_index=row["source_op_index"],
                    entry=entry,
                    max_m=2,
                    max_n=2,
                    max_k=3,
                    ...
                    portfolio_member=selected["selection"],
                    output=work / f"prepare_{ordinal}",
                ...
                if prepared.get("status") != "prepared":
                    raise ValueError(
                        "lane-migration representative preparation refused: " + str(prepared.get("reason", "unknown"))
                    )
                ...
                if (
                    any(extraction.get(key) != value for key, value in expected_extraction.items())
                    or prepared.get("expected_route_transition") != ["host", "contraction"]
                    or (prepared.get("arms") or {}).get("after", {}).get("declared_route") != "contraction"
                    or (
                        math.prod(row["source_batch_shape"]) > 1
                ...
                execution = dict(
                    self.runtime_provider(
                        candidate=candidate, probes=probes, prepared=prepared, timeout_s=min(60.0, remaining())
                    )
                )
                representative["execution"] = execution
                if (
                    execution.get("status") != "passed"
                    or execution.get("correct") is not True
                    or any(
                        (execution.get("arms") or {}).get(arm, {}).get("correct") is not True
                        for arm in ("before", "after")
                    )
                ):
                    raise ValueError(
                        "lane-migration representative differs from the source oracle: "
                        + str(execution.get("reason", "unknown"))
                    )
                record["bounded_programs_executed"] += 2
            verified = probes.analysis.session.selected_changed_portfolio_context(candidate, selected["selection"])
            actual_binding = {
            ...
            record.update(
                status="passed_reduced_lane_migration_witness",
                numerical_pass=True,
                bounded_runtime_executed=True,
        ...
        except (ValueError, KeyError, TypeError, AttributeError, RuntimeError, OSError, TimeoutError) as error:
