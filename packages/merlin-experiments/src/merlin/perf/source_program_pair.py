...
def artifact_record(artifact: Mapping[str, Any], *, baseline: bool = False) -> dict[str, Any]:
    return {
        "interface": Path(artifact["interface"]),
...
def check_artifact_proof(
    artifact: Mapping[str, Any], proof: Mapping[str, Any], source_sha: str, graph_sha: str
) -> None:
    raw_buffer = artifact["command_buffer_text"]
    if (
        not isinstance(raw_buffer, str)
        or text_digest(raw_buffer) != artifact["command_buffer_sha256"]
    ...
    *,
    ...
    if portfolio_member is not None:
        ...
        if not callable(getattr(probes.analysis.session, "selected_changed_portfolio_context", None)):
            raise ValueError("portfolio source pair requires member-aware probes accessors")
        ...
        after_raw = probes.analysis.session.current_artifacts(candidate)
        diag = probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"]
    graph_sha = diag["captured_logical_graph"]["logical_dispatch_digest"]
    if comparison_arm == "optimization_baseline":
        before_raw = probes.analysis.session.optimization_baseline_artifacts(candidate)
        binding = probes.analysis.session.optimization_baseline_artifact_binding(candidate)
        compile_before = probes.compile_optimization_baseline_probe_candidate
        ...
        expected = {
            ...
            "command_buffer_sha256": binding["command_buffer_sha256"],
            "compiler_sha256": binding["compiler_sha256"],
            "host_verifier_policy_sha256": probes.analysis.session.inputs.host_policy["sha256"],
            "evidence_sha256": document_digest(before_proof),
        }
        if (
            not isinstance(proof_binding, Mapping)
            ...
            raise ValueError(
                "optimization baseline lacks exact cached host-policy-bound plan proof; "
                "run static proof preparation, not a probe-time full verification"
            )
    else:
        if selected is None and len(probes.analysis.session.journal.iterations) < 2:
            raise ValueError("previous comparison requires a preceding analyzed iteration")
        before_raw = (
            before_context["artifacts"]
            if selected is not None
            else probes.analysis.session.previous_artifacts(candidate)
        )
        binding = (
            {
            ...
            if selected is not None
            else probes.analysis.session.previous_probe_binding(candidate).to_dict()
        )
        compile_before = probes.compile_previous_probe_candidate
        before_proof = (
            (before_context["analysis"].get("diagnostics") or {}).get("verified_global_plan_emission")
            if selected is not None
            else probes.analysis.session.journal.iterations[-2]["analysis"]["diagnostics"].get(
                "verified_global_plan_emission"
            )
        )
    artifacts = {
        "before": artifact_record(before_raw, baseline=comparison_arm == "optimization_baseline"),
    ...
    owners = {arm: source_owners(program_plan(artifact["command_buffer"])) for arm, artifact in artifacts.items()}
