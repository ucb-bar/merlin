...
from .source_contraction_witness import evaluate_source_contraction, extract_source_contraction
from .source_program_pair import (
    bind_source_program_pair,
    check_artifact_proof,
    document_digest,
    program_plan,
    source_owners,
    text_digest,
...
def task_route_feedback(comparison):
    ...
    return {
        ...
        "next_action": (
            "The reduced probe does not reproduce the observed full-task instruction classes; retain its numerical/timing result only for that reduced program."
            if status == "observed_known_class_presence_mismatch"
            else "Resolve missing source/artifact-bound task observations before attributing this probe to a full-model lowering change."
            if status == "UNKNOWN"
            else "Known class presence is scoped evidence only; descriptor/address equivalence and global cost remain unresolved."
        ),
        "descriptor_semantic_equivalence": "UNKNOWN",
        "emitted_address_equivalence": "UNKNOWN",
        "timing_calibration_admissible": False,
        "global_cost_validated": False,
    }


def task_route_presence_for_preparation(*, candidate, probes, pair, prepared):
    ...
    """
    from .task_instruction_evidence import target_instruction_facts, task_instruction_binding
    from .task_route_presence import compare_task_route_presence

    full, short = {}, {}
    extraction = prepared.get("extraction", {})
    try:
        facts = target_instruction_facts(probes.analysis.session.inputs.target)
        policy = probes.analysis.session.inputs.host_policy["sha256"]
        current = probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"]
        before = (
            current
            if prepared["comparison_arm"] == "optimization_baseline"
            else probes.analysis.session.journal.iterations[-2]["analysis"]["diagnostics"]
        )
        short_source = Path(prepared["workdir"]) / "interface.mlir"
        with short_source.open("rb") as stream:
            raw_source = stream.read(256 * 1024 + 1)
        if (
            len(raw_source) > 256 * 1024
            or text_digest(raw_source.decode()) != extraction["probe_source_sha256"]
        ...
        for arm, diag, summary_key, proof_key in (
            (
                "before",
                before,
                "baseline" if prepared["comparison_arm"] == "optimization_baseline" else "candidate",
                "verified_baseline_global_plan_emission"
                if prepared["comparison_arm"] == "optimization_baseline"
                else "verified_global_plan_emission",
            ),
            ...
            full[arm] = {
                "summary": summary,
                "summary_sha256": document_digest(summary),
                "expected_binding": task_instruction_binding(
                    source_text=pair.source,
                    lowered_text=artifact["lowered_text"],
                    command_buffer_text=artifact["command_buffer_text"],
                    verified_plan=proof,
                    target_facts=facts,
                    host_policy_sha256=policy,
                ),
            }
            item = prepared["arms"][arm]
            texts = {}
            for field, pin in (
                ("lowered_path", "lowered_sha256"),
            ...
            expected = {
                "source_sha256": extraction["probe_source_sha256"],
            ...
            if (
                any(short_proof.get(key) != value for key, value in expected.items())
            ...
            short[arm] = {
                "summary": item["task_instruction_evidence"],
                "summary_sha256": item["task_instruction_evidence_sha256"],
                "expected_binding": task_instruction_binding(
                    source_text=raw_source.decode(),
                    lowered_text=texts["lowered_path"],
                    command_buffer_text=texts["command_buffer_path"],
                    verified_plan=short_proof,
                    target_facts=facts,
                    host_policy_sha256=policy,
                ),
                "short_execution_admission": item.get("short_execution_admission"),
            }
        return compare_task_route_presence(
            full=full,
            short=short,
            extraction=extraction,
            extraction_sha256=document_digest(extraction),
            short_source_op_index=prepared["probe_source_op_index"],
        )
    except (ValueError, KeyError, TypeError, AttributeError, OSError, RuntimeError) as exc:
        result = compare_task_route_presence(
            full={},
            short={},
            extraction=extraction,
            extraction_sha256=document_digest(extraction),
            short_source_op_index=prepared.get("probe_source_op_index"),
        )
...
def _short_source_index(text, entry):
    ...
    selected = [
        index
        for index, op in enumerate(op for op in functions[0].body.block.ops if op.name != "func.return")
...
def _typed_cases(probe, extraction, remaining):
    ...
    for mode in ("signed_coordinate_pattern", "negative_extremes", "modular_overflow"):
        ...
        for index, (shape, dtype) in enumerate(
            zip(extraction["input_shapes"], extraction["input_dtypes"], strict=True)
        ):
            width = int(dtype[1:])
            if mode == "signed_coordinate_pattern":
                ...
                value = (
                    -(1 << (width - 1)) if mode == "negative_extremes" and index % 2 == 0 else (1 << (width - 1)) - 1
                )
        ...
        cases.append(
            {
                "case": mode,
                "inputs": [array.tolist() for array in arrays],
                "expected": expected.tolist(),
                "input_le_sha256": [
                    hashlib.sha256(array.astype(array.dtype.newbyteorder("<")).tobytes()).hexdigest()
                    for array in arrays
                ],
                "expected_le_sha256": hashlib.sha256(
                    expected.astype(expected.dtype.newbyteorder("<")).tobytes()
                ).hexdigest(),
            }
        )
    return {
        "schema": "source_contraction_prepared_oracle_v1",
        "probe_source_sha256": text_digest(probe),
        "input_shapes": extraction["input_shapes"],
        "input_dtypes": extraction["input_dtypes"],
        "output_shape": extraction["output_shape"],
        "output_dtype": extraction["output_dtype"],
        "cases": cases,
        "semantics": extraction["arithmetic"],
...
def prepare_source_contraction(
    *,
    ...
    if (
        ...
        or type(max_batch_extent) is not int
        or not 1 <= max_batch_extent <= 16
        or type(max_source_bytes) is not int
        or not 1 <= max_source_bytes <= 16_000_000
        or type(max_macs) is not int
        or not 1 <= max_macs <= 100000
    ):
        raise ValueError("source contraction selection and bounds must be explicit and valid")
    if expected_route_transition is not None and (
        type(expected_route_transition) is not tuple
        or len(expected_route_transition) != 2
        or any(route not in {"host", "contraction"} for route in expected_route_transition)
    ...
    if expected_short_route_transition is not None and (
        type(expected_short_route_transition) is not tuple
        or len(expected_short_route_transition) != 2
        or any(route not in {None, "host", "contraction"} for route in expected_short_route_transition)
    ):
    ...
    record = {
        ...
        "full_model_recompiled": False,
        "full_model_proofs_recomputed": False,
    ...
    try:
        ...
        initial_controller_binding = probes.analysis.session.current_probe_binding(candidate)
        remaining()
        full_owners = {arm: pair.owners[arm].get(source_op_index, {}) for arm in ("before", "after")}
        expected_routes = dict(
            zip(("before", "after"), expected_route_transition or ("contraction", "contraction"), strict=True)
        )
        expected_short_routes = dict(
            zip(("before", "after"), expected_short_route_transition or tuple(expected_routes.values()), strict=True)
        )
        ...
        record.update(
            ...
            logical_dispatch_digest=pair.graph_sha256,
            comparison_binding=pair.comparison_binding,
            full_model_source_owner=full_owners,
            full_model_artifact_binding={
                arm: {key: artifact[key] for key in ("compiler_sha256", "lowered_sha256", "command_buffer_sha256")}
                for arm, artifact in pair.artifacts.items()
            },
        )
        probe, extraction = extract_source_contraction(
            pair.source,
            source_op_index,
            entry=entry,
            max_m=max_m,
        ...
        for arm, compile_arm in (("before", pair.compile_before), ("after", probes.compile_probe_candidate)):
            result = compile_arm(
                candidate, source_path, work / (arm + "_compile"), timeout_s=remaining(), emit_command_buffer=True
            )
            ...
            if (
                lowered.returncode
                or not lowered.stdout
                or emitted is None
                or emitted.returncode
                or not isinstance(cb, dict)
                or cb.get("declined") is not None
            ):
            ...
            proof = verify_compiler_global_plan(
                source_text=probe,
                lowered_text=lowered.stdout,
                command_buffer=cb,
            ...
            if proof.get("status") == "refused":
                ...
                short_admission = numerical_probe_admission(
                    source_text=probe,
                    lowered_text=lowered.stdout,
                    command_buffer=cb,
                    declared_proof=json.loads(json.dumps(proof)),
                    candidate_sha256=pair.artifacts[arm]["compiler_sha256"],
                    source_op_index=short_index,
                )
                ...
                if short_admission.get("status") == "source_bound_numerical_probe":
                    expected_admission = {
                        ...
                        "source_sha256": text_digest(probe),
                        "lowered_sha256": text_digest(lowered.stdout),
                        "command_buffer_sha256": cb_sha,
                        "candidate_sha256": pair.artifacts[arm]["compiler_sha256"],
                    ...
                    if any(short_admission.get(key) != value for key, value in expected_admission.items()):
                        raise ValueError(
                            "short initializer execution admission has stale source/artifact/proof bindings"
                        )
            actual_route = owner.get("kind")
            if (
                (expected_short_routes[arm] is not None and actual_route != expected_short_routes[arm])
            ...
            if (
                len(entries) != len(extraction["inputs"])
            ...
            record["arms"][arm] = {
                "compiler_sha256": pair.artifacts[arm]["compiler_sha256"],
                "source_sha256": text_digest(probe),
                "lowered_sha256": text_digest(lowered.stdout),
                "command_buffer_canonical_sha256": cb_sha,
                "command_buffer_sha256": cb_sha,
                "command_buffer_serialization": "canonical host serialization saved byte-exactly, not original compiler raw formatting",
                "lowered_path": str(lowered_path),
                "command_buffer_path": str(cb_path),
                "source_task_cfg_proof": proof,
                "source_owner": owner,
                "declared_route": actual_route,
                "full_model_declared_route": expected_routes[arm],
                "input_tensors": entries,
            ...
            try:
                ...
                trace = target_decode.decode_module(parsed_short, target=probes.analysis.session.inputs.target)
                summary = summarize_task_instructions(
                    source_text=probe,
                    lowered_text=lowered.stdout,
                    command_buffer_text=raw_cb,
                    command_buffer=cb,
                    verified_plan=proof,
                    parsed_module=parsed_short,
                    decoded_trace=trace,
                    target_facts=target_instruction_facts(probes.analysis.session.inputs.target),
                    host_policy_sha256=probes.analysis.session.inputs.host_policy["sha256"],
                    decode_module=lambda module: target_decode.decode_module(
                        module, target=probes.analysis.session.inputs.target
                    ),
        ...
        current_pair = bind_source_program_pair(
            candidate=candidate,
            probes=probes,
            comparison_arm=comparison_arm,
            portfolio_member=portfolio_member,
        ...
        if (
            probes.analysis.session.current_probe_binding(candidate) != initial_controller_binding
            ...
            or any(
                current_pair.artifacts[arm][key] != pair.artifacts[arm][key]
                for arm in ("before", "after")
                for key in ("compiler_sha256", "lowered_sha256", "command_buffer_sha256")
            )
        ):
        ...
        record.update(
            status="prepared",
            independent_oracle=str(oracle_path),
            independent_oracle_sha256=hashlib.sha256(oracle_path.read_bytes()).hexdigest(),
        )
        comparison = task_route_presence_for_preparation(
            candidate=candidate, probes=probes, pair=current_pair, prepared=record
        )
        record.update(
            task_route_presence=comparison,
