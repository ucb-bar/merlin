...
from .source_convolution_witness import evaluate_source_convolution, extract_source_convolution
from .source_program_pair import bind_source_program_pair
from .source_program_pair import program_plan as _plan
from .source_program_pair import source_owners as _owners
...
def prepare_source_convolution(
    *, candidate: Path, probes, comparison_arm: str, entry: str, output: Path, timeout_s: float = 60
) -> dict:
    ...
    record = {
        "schema": "source_convolution_preparation_v1",
        "status": "UNKNOWN",
        ...
        "target_instruction_semantics": "UNVERIFIED",
        "global_speedup_proven": False,
        "scope": "complete reduced source programs prepared; not executed or full-shape qualified",
        "arms": {},
    }
    work = None
    ...
    try:
        pair = bind_source_program_pair(candidate=candidate, probes=probes, comparison_arm=comparison_arm)
        ...
        for _, shape in observe_model_macs(source, entry=entry):
            ...
            if (
                shape.status == "derived"
                and owners["before"].get(index, {}).get("kind") == "host"
        ...
        short_mac = next(
            shape.source_op_index for _, shape in observe_model_macs(probe, entry=entry) if shape.status == "derived"
        )
        record.update(
            source_sha256=source_sha,
            extraction=extraction,
            skipped_source_options=errors,
            comparison_binding=binding,
            workdir=str(work),
            full_model_source_owner={arm: owners[arm][index] for arm in owners},
        )
        initial_current_binding = probes.analysis.session.current_probe_binding(candidate)
        for arm, compile_method in (("before", compile_before), ("after", probes.compile_probe_candidate)):
            result = compile_method(
                candidate, probe_path, work / (arm + "_compile"), timeout_s=remaining(), emit_command_buffer=True
            )
            lowered, buffer_result, cb = result["lowered"], result["command_buffer_emission"], result["command_buffer"]
            if (
                lowered.returncode
                or buffer_result is None
                or buffer_result.returncode
            ...
            proof = verify_compiler_global_plan(
                ...
                command_buffer=cb,
                candidate_sha256=artifacts[arm]["compiler_sha256"],
            ...
            plan = _plan(cb)
            if (
                len(plan["entry_bindings"]) != 2
                or len(set(plan["entry_bindings"])) != 2
                or len(plan["output_bindings"]) != 1
            ...
            record["arms"][arm] = {
                "compiler_sha256": artifacts[arm]["compiler_sha256"],
                "lowered_sha256": _sha(lowered.stdout),
                "command_buffer_canonical_sha256": cb_sha,
                "source_task_cfg_proof": proof,
                "declared_route": expected_kind,
                "source_owner": task,
                "input_tensors": plan["entry_bindings"],
        ...
        if probes.analysis.session.current_probe_binding(candidate) != initial_current_binding:
        ...
        oracle = {
            "schema": "source_convolution_prepared_oracle_v1",
            "probe_source_sha256": _sha(probe),
            "cases": cases,
            "expected_output_shape": extraction["output_shape"],
            "input_dtypes": ["i8", "i8"],
            "output_dtype": "i32",
        }
