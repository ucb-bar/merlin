...
class IsolatedAnalysisWorker:
    ...
    def __init__(
        ...
        *,
        ...
        contract_root: Path,
        sandbox_factory: Callable[..., Mapping[str, Any]],
        output: Path,
        python_command: Callable[[list[str]], list[str]] | None = None,
    ):
        self.analysis_source = Path(analysis_source).resolve()
        self.contract_root = Path(contract_root).resolve()
        self.sandbox_factory = sandbox_factory
        self.output = Path(output).resolve()
        # The host may supply a frozen-source transport without coupling core to
        # an experiment engine. Candidate commands retain their existing sandbox.
    ...
    def __call__(
        ...
        **kwargs: Any,
        ...
        request = {
            "analysis_source": str(self.analysis_source),
        ...
        process = None
        status = "failed"
        native_argv = [sys.executable, "-m", __name__, str(request_path)]
        transport_argv = None
        try:
            transport_argv = self.python_command(list(native_argv)) if self.python_command else list(native_argv)
            # Command preparation (including source verification) spends the
            # same bounded host budget as request and sandbox preparation.
            remaining = timeout_s - (time.monotonic() - started)
            if remaining <= 0:
                raise TimeoutError("analysis preparation exhausted its wall-clock budget")
            result_grace = min(_RESULT_TRANSPORT_GRACE_SECONDS, max(0.05, remaining * 0.1))
            ...
            if analysis_timeout <= 0:
                raise TimeoutError("analysis preparation left no child execution budget")
            request["timeout_s"] = analysis_timeout
            request_path.write_text(json.dumps(request, default=_json_default))
...
def _load_analysis(analysis_source: Path) -> Any:
    """Execute fresh canonical analyzer source through the active source-verifying finder."""
    fullname = "merlin_experiments.phase2.emission_analysis"
    analysis_source = analysis_source.resolve()
    parent = importlib.import_module(fullname.rpartition(".")[0])
    spec = None
    for finder in sys.meta_path:
        spec = finder.find_spec(fullname, parent.__path__)
        if spec is not None:
            break
    if spec is None or spec.loader is None or not spec.origin:
        raise ValueError("host analyzer module is unavailable")
    if Path(spec.origin).resolve() != analysis_source:
        raise ValueError("host analyzer origin differs from the requested analysis source")
    get_source = getattr(spec.loader, "get_source", None)
    source = get_source(fullname) if get_source is not None else None
    if source is None:
        raise ValueError("host analyzer source is unavailable")
    analysis = importlib.util.module_from_spec(spec)
    sys.modules[fullname] = analysis
    exec(compile(source, str(analysis_source), "exec", dont_inherit=True), analysis.__dict__)
    return analysis


def _worker(request_path: Path) -> int:
    request = json.loads(request_path.read_text())
    analysis_module = _load_analysis(Path(request["analysis_source"]))
    started = time.monotonic()
    ...
    try:
        ...
        analysis = analysis_module.analyze_whole_model_emission(
            Path(request["baseline"]),
            Path(request["candidate"]),
            INPUTS.StageE2ESentinel(**request["sentinel"]),
