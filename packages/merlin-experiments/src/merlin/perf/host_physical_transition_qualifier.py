...
def _declarations(artifact):
    return (
        artifact.get("command_buffer", {})
        .get("params", {})
        .get("global_program_plan", {})
        .get("physical_transitions", [])
    )
...
def _edge(row):
    ...
    return tuple(
        edge[key]
        for key in ("producer_op_index", "producer_result_index", "consumer_op_index", "consumer_operand_index")
    )
...
def _mechanism(row):
    ...
    return (
        ...
        row["destination"]["placement"],
        _orientation(row["source_layout"]),
...
def _bounded_native_module(module, *, expected_symbol, argument_count):
    ...
    if (
        len(functions) != 1
        or list(module.body.block.ops) != functions
    ...
    if (
        functions[0].sym_name.data != expected_symbol
    ...
    if (
        ...
        or any(type(n) is not int or n < 0 for n in dynamic.values())
        or sum(dynamic.values()) > 100_000
        or type(loads) is not int
        or type(stores) is not int
        or loads < 0
        or stores < 0
        or loads + stores > 1024 * 1024
    ):
        raise ValueError("actual emitted short witness exceeds allocation/work bound or has unknown work")
    return {
        "allocation_bytes": allocation,
        "dynamic_ir_operations": sum(dynamic.values()),
...
class ChangedRegionQualifierDispatch:
    def __init__(self, *, physical, legacy, lane_migration=None):
        ...
        if lane_migration is not None and lane_migration.abi_provenance != self.abi_provenance:
            raise ValueError("changed-region providers disagree on host ABI provenance")

    def __call__(self, *, candidate, probes, timeout_s, portfolio_member=None):
        selected = None
        if callable(getattr(probes.analysis.session, "selected_changed_portfolio_context", None)):
            ...
            artifacts = (
                probes.analysis.session.previous_artifacts(candidate),
                probes.analysis.session.current_artifacts(candidate),
            )
...
class HostPhysicalTransitionQualifier:
    ...
    def __call__(self, *, candidate, probes, timeout_s, portfolio_member=None):
        ...
        record = {
            "schema": "host_physical_transition_qualification_v1",
            "status": "UNKNOWN",
        ...
        try:
            ...
            if callable(getattr(probes.analysis.session, "selected_changed_portfolio_context", None)):
                ...
                artifacts = {
                    "before": selected_context["previous"]["artifacts"],
                    "after": selected_context["current"]["artifacts"],
                }
                analyses = {
                    "before": selected_context["previous"]["analysis"],
                ...
                if portfolio_member is not None:
                    raise ValueError("portfolio member selection requires member-aware probes accessors")
                artifacts = {
                    "before": probes.analysis.session.previous_artifacts(candidate),
                    "after": probes.analysis.session.current_artifacts(candidate),
                }
                analyses = dict(
                    zip(
                        artifacts,
                        (row["analysis"] for row in probes.analysis.session.journal.iterations[-2:]),
                        strict=True,
                    )
                )
            sources = {}
            ...
            for arm, analysis in analyses.items():
                ...
                if (
                    ...
                    or plan["source_sha256"] != _sha(sources[arm])
                ...
                remaining()
                full[arm] = verify_physical_transitions(
                    source_text=sources[arm],
                    lowered_text=artifact["lowered_text"],
                    command_buffer=artifact["command_buffer"],
                )
            ...
            changed = sorted(
                edge
                for edge in set(by_arm["before"]) | set(by_arm["after"])
            ...
            edge = changed[0]
            text, extraction = extract_pointwise_chain(
                sources["after"], [edge[0], edge[2]], max_extent=3, max_operations=2
            )
            ...
            for arm, compile_method in (
                ...
                result = compile_method(
                    candidate, source, work / (arm + "_compile"), timeout_s=remaining(), emit_command_buffer=True
                )
                lowered, cb = result["lowered"], result["command_buffer"]
                if (
                    lowered.returncode
                    or result["command_buffer_emission"] is None
                    or result["command_buffer_emission"].returncode
                ...
                record["arms"][arm] = {
                    "physical_evidence": proof,
                    "source_edge_correspondence": list(short_edge),
                    "mechanisms": [_mechanism(row) for row in declarations],
                    "emitted_bounds": bounds,
                }
            for arm, (lowered, cb) in compiled.items():
                record["arms"][arm].update(
                    self._native(candidate, probes, work / arm, lowered, cb, text, extraction, remaining)
                )
            remaining()
            if selected_context is not None:
                selected_after = probes.analysis.session.selected_changed_portfolio_context(
    ...
    def _native(self, candidate, probes, work, lowered, cb, source, extraction, remaining):
        import numpy as np
        ...
        for argv in (
            [str(translator), "--mlir-to-llvmir", str(lowered_path), "-o", str(ll)],
            [str(translator.with_name("clang")), "-shared", "-fPIC", "-O2", str(ll), "-o", str(library)],
        ):
            result = probes.run_native_probe(candidate, work, argv, timeout_s=remaining())
        ...
        for seed in range(3):
            ...
            for spec in inputs:
                ...
                pool = (
                    [-0.0, 0.5, -0.5, 1.0, -1.0, 2**-24, -(2**-24)]
                    if spec["dtype"] == "f32"
                    else [-128, -127, -1, 0, 1, 126, 127]
                )
                values.append(
                    np.asarray([pool[(i + seed) % len(pool)] for i in range(prod(spec["shape"]))], dtype=dtype).reshape(
                        spec["shape"]
                    )
                )
            ...
            for arg in abi["args"]:
                ...
                if not (
                    0 < layout["rows"] * layout["cols"] <= 4096
            ...
            request.write_text(
                json.dumps({"library": str(library), "symbol": self.expected_symbol, "arguments": arguments})
            )
            result = probes.run_native_probe(
                candidate, work, ["python3", str(runner), str(request)], timeout_s=remaining()
            )
            if result.returncode:
        ...
        return {
            "cases": cases,
            "native_binary_sha256": _sha(library.read_bytes()),
