...
_OPB = {
    ...
    "batched_matmul": ("batched_weight_or_state", ["weight_stationary", "output_stationary", "input_stationary"]),
    ...
    return {
        "shape": {
            ...
            "data-spaces": [
                ...
                {"name": "Outputs", "projection": [[["M"]], [["N"]]], "read-write": True, "operand_identity": "output"},
            ],
        },
        ...
        rows.append(
            {
                "workload": s["workload"],
                "op_class": s["op_class"],
                "shape_id": f"{s['M']}x{s['N']}x{s['K']}",
                "M": s["M"],
                "N": s["N"],
                "K": s["K"],
                "multiplicity": s["multiplicity"],
                "macs_per_instance": s["M"] * s["N"] * s["K"],
                "operand_b_identity": opb_id,
                "dataflow_candidates": "|".join(dataflow),
                "legal_spatial_axes": "M|N|K_with_reduction",
                "legal_temporal_axes": "M|N|K",
                "bypass_candidates": "Weights|Inputs|Outputs",
                "k_reduction_needs_partial_sum": "True",
                "fqn": s["fqn"],
            }
        )
...
_LOOP_SEED_COLS = [
    ...
    "outer_loop_K",
    "K_source",
    "instances_per_replan",
    "macs_per_instance",
    "macs_per_replan",
    "operand_b_identity",
    "dataflow_candidates",
    "reduction_reuse_scope",
    "kv_state_bytes_workload",
    "k_reduction_needs_partial_sum",
    "evidence",
]
...
def loop_aware_seed_rows(cs_dir) -> list[dict]:
    ...
    rows = []
    for (w, oc, role, M, N, Kd), a in sorted(
        acc.items(), key=lambda kv: (kv[0][0], kv[0][2], -kv[0][3] * kv[0][4] * kv[0][5])
    ):
        ...
        rows.append(
            {
                ...
                "N": N,
                "K": Kd,
                "layers_in_capture": layers,
                "outer_loop_K": outer,
                "K_source": "recovered_from_ir"
                if (repeated and f["K_present"])
                else ("not_repeated" if not repeated else "assumed_or_config"),
                "instances_per_replan": layers * outer,
                "macs_per_instance": mac_inst,
                "macs_per_replan": mac_inst * layers * outer,
                "operand_b_identity": opb_id,
                "dataflow_candidates": "|".join(dataflow),
                "reduction_reuse_scope": ("across_K (resident-eligible)" if repeated else "once_per_replan"),
                "kv_state_bytes_workload": f["kv_bytes"],
                "k_reduction_needs_partial_sum": "True",
...
def loop_aware_problem_shapes(cs_dir) -> dict:
        ...
        wobj["shapes"].append(
            {
                ...
                "problem": {
                    "shape": {"name": "matmul", "dimensions": ["M", "N", "K"]},
    ...
    return {
        "loop_aware_mapspace_seeds": {
            ...
            "count_workloads": len(workloads),
            "count_shapes": len(rows),
            "workloads": workloads,
        }
    }
