...
def state_records(topo, attribution) -> list[StateRecord]:
    ...
    for c in topo.state_crossing_boundaries():
        _upsert(
            ...
            consumed_by=c.get("consumed_by"),
            reused=c.get("reused_times"),
        )
    ...
    return sorted(by_state.values(), key=lambda r: (-_SCOPE_RANK[r.lifetime_scope], r.state))
