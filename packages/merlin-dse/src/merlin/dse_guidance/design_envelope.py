...
def derive(
    ...
    *,
    ...
    dispatches_evidence: str,
    captured_dtype: str,
    region: str = "repeated_head",
    ...
    reqs: list[Requirement] = [
        Requirement("macs_per_replan", float(macs_per_replan), "MAC", E_IR, f"macs_per_step x K={K}"),
        Requirement(
        ...
        Requirement(
            "weight_reload_bytes_per_replan",
            float(weight_reload_per_replan),
            "B",
            E_IR,
        ...
        Requirement(
            ...
            float(dispatches_per_replan),
            "dispatch",
            dispatches_evidence,
            f"dispatches_per_step x K={K}",
        ),
    ]
    ...
    if deadline_s and deadline_s > 0:
        reqs += [
            Requirement(
                "required_compute_rate", macs_per_replan / deadline_s, "MAC/s", E_DERIVED, "macs_per_replan / deadline"
            ),
            Requirement(
                "required_weight_bandwidth",
                weight_reload_per_replan / deadline_s,
                "B/s",
                E_DERIVED,
                "naive reload bandwidth if non-resident",
            ),
            Requirement(
                "required_activation_bandwidth",
                int(activation_bytes_per_step) * K / deadline_s,
                "B/s",
                E_DERIVED,
            ...
            Requirement(
                ...
                dispatches_per_replan / deadline_s,
                "dispatch/s",
                E_DERIVED,
                "host command issue rate to meet deadline",
            ),
        ]
    else:
        for n, u in (
            ("required_compute_rate", "MAC/s"),
            ("required_weight_bandwidth", "B/s"),
            ("required_activation_bandwidth", "B/s"),
            ("required_command_rate", "dispatch/s"),
        ):
    ...
    env = DesignEnvelope(
        ...
        requirements=reqs,
        capacity_by_dtype_B=capacity,
        candidate_axes=_candidate_axes(K, weight_bytes, avoidable_reload, capacity, dispatches_per_replan),
    )
    if design:
        env.feasibility = _feasibility(env, design, macs_per_replan, weight_reload_per_replan, dispatches_per_replan)
    return env
            ...
            axes.append(
                {
                    ...
                    "needed_to_quantify": [
                        "target memory hierarchy + bandwidth model",
                        "packed-layout support",
        ...
        axes.append(
            {
                ...
                "requirement": {"bounded_loop": True, "loop_carried_state": True, "device_dependency_tracking": True},
...
def _feasibility(
    env: DesignEnvelope, design: dict, macs_per_replan: int, weight_reload_per_replan: int, dispatches_per_replan: int
) -> dict:
    ...
    if "compute_bound_s" in out or "memory_bound_s" in out:
        out["latency_lower_bound_s"] = max(out.get("compute_bound_s", 0.0), out.get("memory_bound_s", 0.0))
    if cap:
        # Feasible if SOME candidate dtype's resident set fits; report the smallest that fits.
        cap_B = cap * 1e6
        fits = {f: env.capacity_by_dtype_B[f] for f in env.capacity_by_dtype_B if env.capacity_by_dtype_B[f] <= cap_B}
    ...
    out["command_feasible"] = (dispatches_per_replan * submit_ns * 1e-9 <= d) if (submit_ns and d) else E_NA
    return out
...
def from_recovered(
    ...
    *,
    ...
    deadline_s = topology.replan_deadline_ms / 1000.0 if topology.replan_deadline_ms else None
    return derive(
        workload=topology.workload,
        K=topology.K,
        deadline_s=deadline_s,
        deadline_evidence=E_ASSUMED,
        macs_per_step=int(f.get("macs_per_invocation", 0)),
        ...
        dispatches_evidence=dispatches_evidence,
        captured_dtype=captured_dtype,
        design=design,
    )
...
def to_yaml_obj(env: DesignEnvelope) -> dict:
    return {
        "design_envelope": {
            ...
            "requirements": [
                {"name": r.name, "value": r.value, "unit": r.unit, "evidence": r.evidence, "note": r.note}
...
def markdown(env: DesignEnvelope) -> str:
    ...
    L.append(
        f"- K = {env.K}  ·  replan deadline = {d} ({env.deadline_evidence})  ·  captured dtype = {env.captured_dtype}\n"
    )
    L.append("## Requirements\n")
    L.append("| requirement | value | unit | evidence |")
    L.append("|-------------|-------|------|----------|")
    for r in env.requirements:
        val = "n/a" if r.value is None else (f"{r.value:.3e}" if r.unit != "B" else _human_bytes(r.value))
        ...
        rows.append(
            {
                "workload": env.workload,
                "region": env.region,
                "requirement": f"resident_capacity_{f}",
                "value": round(env.capacity_by_dtype_B[f]),
                "unit": "B",
                "evidence": E_IR,
                "note": "dtype-scaled from captured element count",
            }
        )
    return rows
