...
CANDIDATE_CATALOG: dict[str, dict] = {
    "resident_action_head_weights": {
        ...
        "measurements": [
            "action_head_weight_bytes",
            "weight_reload_bytes_per_step",
            "pack_cost_per_step",
            "resident_capacity_required",
            "measured memory bandwidth",
        ],
        "could_be_wrong_if": [
            "weights exceed resident capacity",
            "packing already hoisted out of the loop",
            "DMA/weight traffic is not the bottleneck",
            "host dispatch dominates total latency",
        ],
        "benefit_unquantified": "head/backbone cost attribution or measured weight traffic is unavailable",
    },
    "resident_prefix_kv": {
        ...
        "could_be_wrong_if": [
            "prefix/KV exceeds resident capacity",
    ...
    "command_batching": {
        ...
        "measurements": ["dispatches_per_replan", "host_submit_ns", "command_encode_ns", "batched_submit_ns"],
        "could_be_wrong_if": [
    ...
    "autonomous_K_loop": {
        ...
        "measurements": ["K", "per_step_host_dispatch_ns", "per_step_sync_ns", "loop_carried_state_bytes"],
        "could_be_wrong_if": [
    ...
    "backbone_head_partition": {
        ...
        "measurements": ["backbone_cost", "head_cost_per_step", "state_crossing_bytes", "deadline_slack"],
        "could_be_wrong_if": [
            "backbone and head costs are comparable (no clear slow/fast split)",
    ...
    "decode_kv_cache_path": {
        ...
        "could_be_wrong_if": [
            "the head is not autoregressive",
    ...
    "accumulator_commit": {
        ...
        "could_be_wrong_if": [
            "the intermediate is already fused",
    ...
    "async_chunk_overlap": {
        ...
        "could_be_wrong_if": [
            "the next replan depends on executed-chunk feedback",
...
_HEAD_ATTRIBUTED_AXES = ("resident_action_head_weights", "resident_prefix_kv", "packed_layout_preservation")
...
def _make(topo: VlaRuntimeTopology, axis: str, signal_type: str, evidence: dict, reason: str) -> DseCandidate:
...
def _detect_temporal_reuse(topo, facts):
    ...
    ev = {"K": topo.K, "loop_invariant_state": sorted(inv), "head_phases": [r.name for r in topo.head_phases()]}
...
def _detect_rate_mismatch(topo, facts):
    ...
    if topo.backbone_phases() and topo.head_phases() and topo.control_rate_hz:
        ev = {
            "backbone_phases": [r.name for r in topo.backbone_phases()],
        ...
        out.append(
            (
                "async_chunk_overlap",
                "rate_mismatch",
                {"H": topo.H, "control_rate_hz": topo.control_rate_hz, "replan_deadline_ms": topo.replan_deadline_ms},
...
def _detect_prefix_kv(topo, facts):
    ...
    kv = [
        c
        for c in crossings
        if "kv" in c["state"].lower() or "prefix" in c["state"].lower() or "feature" in c["state"].lower()
    ]
    if kv:
        out.append(
            (
...
_DETECTORS = (
    _detect_temporal_reuse,
    _detect_prefix_kv,
    _detect_rate_mismatch,
    _detect_cpu_coupling,
    _detect_layout_packing,
    _detect_accumulator_epilogue,
    _detect_dynamic_loop,
)


def detect(topo: VlaRuntimeTopology, capture_facts=None, attribution=None) -> list[DseCandidate]:
...
def markdown(topo: VlaRuntimeTopology, candidates: list[DseCandidate]) -> str:
    ...
    L.append(
        f"- workload class: **{topo.workload_class}**  ·  K={topo.K}, H={topo.H}, "
        f"control_rate={topo.control_rate_hz} Hz\n"
    )
    L.append(
        "> These are **structural** candidates derived from the recovered workload contract. "
    ...
    if not candidates:
        L.append(
            "_No DSE candidates: the workload exposes no multi-rate reuse, rate mismatch, or epilogue pressure._\n"
        )
        return "\n".join(L)
    for c in candidates:
        L.append(f"## {c.axis}")
        ...
        L.append(
            f"- **status**: legality={c.legality}, benefit={c.benefit} (blocked by: {c.quantification_blocked_by})"
        )
