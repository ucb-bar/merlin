...
_CAP_COLS = ["workload", "region", "resident_f32_B", "resident_bf16_B", "resident_int8_B", "invocations", "k_basis"]
...
def locality_rows(cs_dir: Path) -> list[dict]:
    ...
    for r in _rows(Path(cs_dir) / "data_movement_table.csv"):
        ...
        wb, ab, ob, kv = (
            _int(r.get("weight_bytes")),
            _int(r.get("activation_input_bytes")),
            _int(r.get("output_bytes")),
            _int(r.get("kv_bytes")),
        )
        out.append(
            {
                ...
                "k_dependent": "yes" if across else "no",
        ...
        out.append(
            {
                "workload": wl,
                "region": reg,
                "operand": "activation_in",
                "bytes": ab,
                "reuse_scope": "within_op",
                "resident_candidate": "no",
                "cache_or_scratchpad": "cache/streamed",
                "k_dependent": "no",
                "evidence": "data_movement_table.csv",
            }
        )
            ...
            out.append(
                {
                    ...
                    "operand": "kv_state",
                    "bytes": kv,
                    "reuse_scope": "across_decode",
                    "resident_candidate": "yes (KV cache)",
                    "cache_or_scratchpad": "scratchpad/HAL_resident_object",
                    "k_dependent": "yes",
