        ...
        for c in p.get("cands", []):
            ...
            if row is None:
                by_axis[c.axis] = ProofRow(
                    ...
                    compiler_proof_needed=c.compiler_proof,
                    status=st,
                    status_evidence=_STATUS_EVIDENCE[st],
                    workloads_suggesting=[case.workload],
                )
            else:
...
def compiler_proof_csv(packages) -> str:
    ...
    rows = [
        {
            "axis": r.axis,
            "system_abstraction": r.system_abstraction,
            "compiler_proof_needed": r.compiler_proof_needed,
            "status": r.status,
    ...
    return _csv(
        rows,
        ["axis", "system_abstraction", "compiler_proof_needed", "status", "status_evidence", "workloads_suggesting"],
    )
