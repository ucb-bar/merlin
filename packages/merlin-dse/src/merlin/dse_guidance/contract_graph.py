...
def _shardable(op_kind: str | None) -> dict:
    ...
    return {
        "shardable_dim": "N",
        "shardable_evidence": E_DERIVED,
        "region_overlap": "unknown",
        "overlap_evidence": E_NA,
    }
...
def build_graph(case, shapes, nc, state_records, dependencies=None) -> ContractGraph:
    ...
    for r in topo.temporal.regions:
        ...
        nodes.append(
            Node(
                ...
                state_summary={
                    "loop_invariant_state": list(r.loop_invariant_state),
                    "loop_carried_state": list(r.loop_carried_state),
                    "produces": list(r.produces),
                    "consumes": list(r.consumes),
                },
                evidence=E_FQN,
            )
        )
                ...
                numerical_contract_summary=(
                    {
                        "dtype": ncs["dtype"],
                        "n_matmuls": ncs["n"],
                        "accumulator_dtype": (nc.accumulator_dtype if nc else None),
                        "evidence": E_IR,
                    }
                    if ncs
                    else {"dtype": "unavailable", "evidence": E_NA}
                ),
    ...
    for s in shapes:
        nodes.append(
            Node(
                id=f"{wl}:op:{s.op_index}",
                workload=wl,
                kind=KIND_OPERATOR,
                region_role=s.region_role,
                parent_region=f"{wl}:region:{s.region_role}",
                shape_summary={
                    "op_kind": s.op_kind,
                    "M": s.M,
                    "N": s.N,
                    ...
                    "rhs_weight_bytes": s.rhs_weight_bytes,
                    "output_bytes": s.output_bytes,
                    "dtype": s.dtype,
                    "attention_or_conv": "unavailable",
                    "primitive_coverage_ref": "tile_waste_table.csv",
                    "evidence": E_IR,
                },
    ...
    if head and head.attribution_status == "attributed" and K > 1:
        nodes.append(
            Node(
                id=f"{wl}:loop:repeated_head",
                workload=wl,
                kind=KIND_LOOP_BODY,
                region_role="repeated_head",
                parent_region=phase_by_role.get("repeated_head"),
                operator_refs=[f"{wl}:op:{i}" for i in head.matmul_indices],
                rate={
                    "cadence": PR.classify_cadence(
                        "repeated_head", topo.workload_class, head.facts.get("invocations"), K
                    ),
                    "trip_count": K,
                    "trip_count_evidence": E_CONFIG,
                },
                work_summary={
                    "macs_per_invocation": head.facts.get("macs_per_invocation"),
                    "macs_per_replan": head.facts.get("macs_total"),
                    "evidence": E_IR,
                },
                evidence=E_FQN,
            )
    ...
    for st in state_records:
        ...
        nodes.append(
            Node(
                id=sid,
                workload=wl,
                kind=KIND_STATE_OBJECT,
                state_summary={
                    "lifetime_scope": st.lifetime_scope,
                    "bytes": st.bytes,
                    "bytes_evidence": st.bytes_evidence,
                    "reused_times": st.reused_times,
                    "implied_abstraction": st.implied_abstraction,
                    "produced_by": st.produced_by,
                    "consumed_by": st.consumed_by,
                },
        ...
        if st.lifetime_scope == SCOPE_INVARIANT:
            ...
            edges.append(
                Edge(
                    ...
                    tensor=st.state,
                    bytes=st.bytes,
                    lifetime="crosses_boundary",
                    can_pipeline="unknown",
                    evidence=st.scope_evidence,
    ...
    return ContractGraph(workload=wl, workload_class=topo.workload_class, rate_model=rm, nodes=nodes, edges=edges)
...
def _node_obj(n: Node) -> dict:
    d = {
        "id": n.id,
        "kind": n.kind,
        "region_role": n.region_role,
        "parent_region": n.parent_region,
        "evidence": n.evidence,
    }
    for k in (
        "operator_refs",
...
def _edge_obj(e: Edge) -> dict:
    d = {"source": e.source, "target": e.target, "kind": e.kind, "can_pipeline": e.can_pipeline, "evidence": e.evidence}
    ...
    return {
        "workload_contract_graph": {
            ...
            "graphs": [
                {
                    "workload": g.workload,
                    "workload_class": g.workload_class,
...
def _counts(g: ContractGraph) -> dict:
    ...
    return {
        ...
        "edge_kinds": dict(ek),
        "edges_known": known,
        "edges_unknown": len(g.edges) - known,
    }
            ...
            rows.append(
                {
                    ...
                    "period_s": r.get("period_s") if r.get("period_s") is not None else "unavailable",
                    "period_evidence": r.get("period_evidence"),
                    "cadence_evidence": r.get("cadence_evidence"),
                }
            )
    return _csv(
        rows, ["workload", "phase", "role", "cadence", "invocations", "period_s", "period_evidence", "cadence_evidence"]
    )
...
def multi_rate_contract_yaml(graphs: list[ContractGraph]) -> dict:
    ...
    for g in graphs:
        ...
        crossings = [
            {"state": e.tensor, "lifetime": e.lifetime, "bytes": e.bytes, "evidence": e.evidence}
            for e in g.edges
...
def summary_md(graphs: list[ContractGraph]) -> str:
    ...
    L.append(
        "| workload | class | nodes | edges | phase | region | operator | loop_body | "
    ...
    for g in graphs:
        ...
        nk = c["node_kinds"]
        L.append(
            f"| {g.workload} | {g.workload_class} | {c['nodes']} | {c['edges']} | "
    ...
    for g in graphs:
        ...
        L.append(f"| {g.workload} | {'yes' if loop else 'no'} | {cad} | {K} | {', '.join(inv) if inv else '—'} |")
    L.append("")
    L.append("## What downstream DSE phases can now consume\n")
    L.append(
        "- **Phase/rate scheduling:** per-phase cadence + the rate model (K/H/control rate, "
    ...
    L.append("- **Partition:** region nodes carry per-region MACs/bytes and the recovered backbone/head split.")
    L.append("- **Primitive sizing:** operator nodes link to the P5 shape classes / coverage table.")
    L.append(
        "- **Scheduling/overlap:** real `data_dependency` edges (from the SSA use-def graph) "
...
def rate_mismatch_report_md(graphs: list[ContractGraph]) -> str:
    ...
    for g in graphs:
        ...
        L.append(
            f"| {g.workload} | {'once_per_replan' if has_bb else '—'} | "
    ...
    L.append(
        "- **`recovered_from_ir`:** region roles' MACs/bytes, operator shapes, loop-invariant "
        "weight bytes, and the **operator data-dependency edges** (from the SSA use-def graph)."
    )
    L.append(
        "- **`recovered_from_prov_fqn`:** the region roles and the once-vs-repeated cadence "
        "split (backbone once, head repeated)."
    )
    L.append(
        "- **`recovered_from_model_config`:** K, H, control_rate_hz (the model's published "
        "architecture constants, from the model registry)."
    )
    L.append(
        "- **`derived_requirement`:** the replan deadline (= H / control_rate) and the "
    ...
    for g in graphs:
        ...
        L.append(
            f"- **{g.workload}:** {ek.get(EDGE_DATA, 0)} `data_dependency` edges recovered from "
