    ...
    for axis, dim in sizes.items():
        ...
        out.append(
            ShardAxis(
                ...
                semantic_class=shape.semantic_class,
                axis=axis,
                dim_size=dim,
                ...
                comm_category=comm,
                per_extra_shard_bytes=extra[axis],
                required_abstractions=absts,
            )
        )
    return out
        ...
        rows.append(
            {
                ...
                "axis": a.axis,
                "dim_size": a.dim_size,
                "shardable_2": a.shardable[2],
                "shardable_4": a.shardable[4],
                "shardable_8": a.shardable[8],
                "tail_2": a.has_tail[2],
                "tail_4": a.has_tail[4],
                "tail_8": a.has_tail[8],
                "reduction_required": a.reduction_required,
                "comm_category": a.comm_category,
...
def report_md(by_workload: dict, axes_all: list[ShardAxis]) -> str:
    ...
    L.append("## Findings\n")
    L.append(
        f"- **Reduction-free sharding dominates:** {nored} (op,axis) M/N opportunities split "
        f"without any cross-shard reduction — only `weight_broadcast`/`activation_multicast` + "
        f"`output_partition_commit`."
    )
    L.append(
        f"- **K-sharding is the high-communication mode:** {kshard} (op,axis) opportunities "
        f"would need a `partial_sum_object` + `accumulator_merge`; the partial-sum bytes are "
        f"in `sharding_table.csv`."
    )
    L.append(
        "- **Attention / conv sharding:** `unavailable` — that structure is lowered into the "
