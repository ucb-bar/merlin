...
REAL_GEOMETRY: dict[str, RealGeometry] = {
    "pi05": RealGeometry(
        "pi05",
        "extracted from Pi0Config(pi05=True) instantiated model",
        stacks=[
            Stack(
                "paligemma_lm_gemma_2b", 18, 2048, 16384, 8, 1, 256, "prefix_once", carries_decode_kv=True
            ),  # prefix KV reused (invariant) by the expert
    ...
    "smolvla": RealGeometry(
        "smolvla",
        "extracted from SmolVLA instantiated model config",
        stacks=[
            Stack("vlm_vision", 12, 768, 3072, 12, 12, 64, "prefix_once"),
            Stack("vlm_text_smollm2", 32, 960, 2560, 15, 5, 64, "prefix_once", carries_decode_kv=True),
    ...
    "openvla": RealGeometry(
        "openvla",
        "published Llama-2-7B LM (recapture reduces to 2L/128)",
        stacks=[
            Stack("llama2_7b_lm", 32, 4096, 11008, 32, 32, 128, "decode_lm", carries_decode_kv=True),
        ],
        vocab=32064,
        embed_hidden=4096,
        decode_seq=263,
    ...
    "tiny_llama": RealGeometry(
        "tiny_llama",
        "extracted from AutoConfig TinyLlama/TinyLlama-1.1B-Chat-v1.0",
    ...
    "molmoact": RealGeometry(
        "molmoact",
        "extracted from MolmoActLlmConfig() defaults",
    ...
    "small_llama": RealGeometry(
        "small_llama",
        "synthetic toy config (fully known; not a deployment-scale model)",
    ...
    "rdt2": RealGeometry(
        "rdt2",
        "RDT2 post_train.yaml (hidden 1024, multiple_of 256, depth 14); per-layer GEMM params "
        ...
        stacks=[
            Stack(
                ...
                128,
                "repeated_head",
                carries_decode_kv=False,
                raw_layer_gemm_params=33_816_576,
            )
        ],
        vocab=0,
        embed_hidden=0,
...
def magnitude_rows() -> list[dict]:
    ...
    for w, g in sorted(REAL_GEOMETRY.items()):
        ...
        per_tok = sum(s.layer_params() * s.n_layers for s in g.stacks if s.role in ("repeated_head", "decode_lm"))
        rows.append(
            {
                "workload": w,
                "source": g.source,
                "n_stacks": len(g.stacks),
    ...
    for w, g in sorted(REAL_GEOMETRY.items()):
        ...
        if lr is not None:
            ...
            if kvs:
                ...
                check = (
                    (f"formula matches IR iter_arg ({recomputed}=={lr.kv_cache_bytes})")
                    if recomputed == lr.kv_cache_bytes
        ...
        rows.append(
            {
                "workload": w,
                "decode_stack": s.name,
                "n_kv_heads": s.kv_heads,
                "head_dim": s.head_dim,
                "seq": g.decode_seq,
                "n_layers": s.n_layers,
                "kv_bytes_f32": int(elems * 4),
                "kv_bytes_bf16": int(elems * 2),
                "kv_bytes_int8": int(elems * 1),
                "loop_carried_in_ir": carried,
                "ir_formula_check": check,
