...
def tile_coverage(shape, name: str, TM: int, TN: int) -> OpPrimitiveCoverage:
    ...
    return OpPrimitiveCoverage(
        ...
        op_index=shape.op_index,
        shape_class=shape.shape_class,
        region_role=shape.region_role,
        primitive=name,
        primitive_kind="tile",
        applicable=True,
        true_macs=true_macs,
        ...
        covered_under_5pct=c5,
        covered_under_10pct=c10,
        covered_under_25pct=c25,
    )
...
def gemv_coverage(shape, name: str, L: int) -> OpPrimitiveCoverage:
    ...
    if vec_dim is None:  # not a vector-like shape: lane does not apply
        return OpPrimitiveCoverage(
            ...
            padded_macs=None,
            padding_waste=None,
            tile_utilization=None,
            tail_fraction=None,
            covered_under_5pct=False,
            covered_under_10pct=False,
            covered_under_25pct=False,
        )
    ...
    return OpPrimitiveCoverage(
        ...
        applicable=True,
        true_macs=true_macs,
        padded_M=(pvec if vec_dim == "M" else M),
        padded_N=(pvec if vec_dim == "N" else N),
        padded_K=K,
        padded_macs=padded_macs,
        padding_waste=round(waste, 6) if waste is not None else None,
        tile_utilization=round(util, 6) if util is not None else None,
...
def aggregate_by_primitive_workload(cov: list[OpPrimitiveCoverage]) -> list[PrimitiveWorkloadAgg]:
    ...
    for (prim, kind, w), cs in groups.items():
        ...
        out.append(
            PrimitiveWorkloadAgg(
                primitive=prim,
                primitive_kind=kind,
                workload=w,
                op_count=len(cs),
                applicable_ops=len(appl),
...
def aggregate_regret(cov: list[OpPrimitiveCoverage], per_wl: list[PrimitiveWorkloadAgg]) -> list[PrimitiveRegret]:
    ...
    for prim, cs in by_prim.items():
        ...
        overfit = (
            sorted(a.workload for a in wls if a.coverage_under_10pct >= 0.9 and worst < 0.5) if best >= 0.9 else []
        )
        out.append(
            PrimitiveRegret(
                primitive=prim,
                ...
                macs_total=macs_total,
                mac_weighted_utilization=round(util, 6) if util is not None else None,
                coverage_under_5pct=round(cov5, 6),
                coverage_under_10pct=round(cov10, 6),
                coverage_under_25pct=round(cov25, 6),
                worst_workload_coverage_10=round(worst, 6),
                best_workload_coverage_10=round(best, 6),
                average_workload_coverage_10=round(avg, 6),
                max_regret=round(best - worst, 6),
                clusters_poorly_served="; ".join(poor),
                workloads_where_overfit="; ".join(overfit),
            )
        )
...
def primitive_regret_csv(regret: list[PrimitiveRegret]) -> str:
    ...
    cols = [
        ...
        "coverage_under_5pct",
        "coverage_under_10pct",
        "coverage_under_25pct",
        "worst_workload_coverage_10",
        "best_workload_coverage_10",
        "average_workload_coverage_10",
        "max_regret",
        "clusters_poorly_served",
        "workloads_where_overfit",
    ]
...
def coverage_report_md(per_wl: list[PrimitiveWorkloadAgg], regret: list[PrimitiveRegret]) -> str:
    ...
    for r in regret:
        ...
        L.append(
            f"| {r.primitive} | {r.primitive_kind} | {util} | {r.coverage_under_5pct:.1%} | "
...
def cross_workload_report_md(regret: list[PrimitiveRegret], per_wl: list[PrimitiveWorkloadAgg]) -> str:
    L = [
    ...
    for r in regret:
        L.append(
            f"| {r.primitive} | {r.average_workload_coverage_10:.0%} | "
    ...
    if regret:
        ...
        L.append("## Findings\n")
        L.append(
            f"- **Widest average structural coverage:** `{widest.primitive}` at "
        ...
        L.append(
            f"- **Worst cross-workload regret:** `{worst_regret.primitive}` "
        ...
        if overfit:
            o = overfit[0]
            L.append(
                f"- **Overfit primitives:** `{o.primitive}` covers "
