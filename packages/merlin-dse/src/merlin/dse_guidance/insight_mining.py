...
_VERIFIED_ARTIFACTS = {
    ...
    "tile_waste_table.csv",
    "primitive_coverage_matrix.csv",
    "primitive_regret_table.csv",
    "shape_summary_by_workload.csv",
    "operator_cluster_table.csv",
    "workload_contract_graph.yaml",
    "critical_path_table.csv",
    "traffic_table.csv",
    "data_movement_table.csv",
    "dtype_capacity_table.csv",
    "dma_stream_table.csv",
    "buffer_requirement_table.csv",
    "sharding_table.csv",
    "region_attribution.yaml",
    "accuracy_gated_dtype_candidates.csv",
    "dispatch_granularity_table.csv",
    "epilogue_pattern_table.csv",
    "accumulator_contract_table.csv",
    "hw_sw_boundary_matrix.csv",
    "boundary_candidate_contracts.yaml",
    "responsibility_split_matrix.csv",
    "dse_search_space_knobs.yaml",
    "dse_contract.json",
    "resident_state_table.csv",
}

# expected root artifacts -> source phase (P13-a inventory). Per-workload subdir artifacts handled
# separately.
_ARTIFACT_PHASE = {
    "cross_workload_provenance.csv": "P1",
    "workload_contract_table.csv": "P2",
    "requirements_table.csv": "P3",
    "dse_readiness_summary.csv": "P4",
    "abstraction_pressure_table.csv": "P4",
    "abstraction_pressure_ranking.csv": "P4",
    "compiler_proof_matrix.csv": "P4",
    "resident_state_table.csv": "P4",
    "workload_family_table.csv": "P4",
    "measurement_priority_table.csv": "P4",
    "numerical_contract_fidelity_report.md": "P4",
    "accuracy_gate_results.csv": "P4",
    "operator_shape_table.csv": "P5",
    "operator_cluster_table.csv": "P5",
    "shape_summary_by_workload.csv": "P5",
    "shape_summary_by_region.csv": "P5",
    "operator_geometry.yaml": "P5",
    "tile_waste_table.csv": "P5",
    "primitive_coverage_matrix.csv": "P5",
    "primitive_regret_table.csv": "P5",
    "workload_contract_graph.yaml": "P6",
    "phase_rate_table.csv": "P6",
    "multi_rate_contract.yaml": "P6",
    "critical_path_table.csv": "P7",
    "concurrency_windows.csv": "P7",
    "sharding_table.csv": "P7",
    "sharding_opportunities.yaml": "P7",
    "operator_cluster_to_hierarchy.csv": "P7",
    "resource_pressure_table.csv": "P7",
    "processing_unit_candidates.yaml": "P7",
    "pipeline_envelope.yaml": "P8",
    "pipeline_stage_table.csv": "P8",
    "pipeline_candidates.yaml": "P8",
    "buffering_requirement_table.csv": "P8",
    "processing_unit_guidance.yaml": "P8",
    "data_movement_table.csv": "P9",
    "memory_hierarchy_envelope.yaml": "P9",
    "reuse_lifetime_table.csv": "P9",
    "dma_stream_table.csv": "P9",
    "buffer_requirement_table.csv": "P9",
    "dtype_capacity_table.csv": "P9",
    "traffic_table.csv": "P9",
    "epilogue_pattern_table.csv": "P10",
    "accumulator_contract_table.csv": "P10",
    "numerical_epilogue_candidates.yaml": "P10",
    "lost_numerical_contracts.csv": "P10",
    "dse_search_space_knobs.yaml": "P11",
    "accuracy_gated_dtype_candidates.csv": "P6",
    "hw_sw_boundary_matrix.csv": "P12",
    "boundary_candidate_contracts.yaml": "P12",
    "responsibility_split_matrix.csv": "P12",
    "boundary_dse_knobs.yaml": "P12",
    "isa_candidate_primitives.yaml": "P12",
    "runtime_object_candidates.yaml": "P12",
    "command_isa_candidates.yaml": "P12",
    "dse_contract.json": "P12",
...
SIGNAL_METRICS = {
    ...
    "max_regret",
    "accuracy_int8_w8a8",
    "measured_dispatch_ratio",
    "matmul_bias_epilogues",
    "dominant_shape_class",
    "mac_fraction_dense_gemm",
    "mac_fraction_skinny_gemm_or_gemv",
    "compiler_proofs_proven_for_workload",
    "compiler_proofs_assumed",
    "compiler_proofs_unknown",
    "clean_8way_mn_shards",
    "head_cadence",
    "accumulator_dtype",
    "compute_dtype",
    "lowbit_storage_dequantized_finding",
    "overlap_candidates_yes",
...
_METRIC_QUESTION = {
    "total_macs": Q_PRIMITIVES,
    "n_matmuls": Q_PRIMITIVES,
    "coverage_under_10pct": Q_PRIMITIVES,
    "max_regret": Q_PRIMITIVES,
    "dominant_shape_class": Q_PRIMITIVES,
    "available_parallelism": Q_HETERO,
    "serialization": Q_HETERO,
    "mac_fraction_dense_gemm": Q_HETERO,
    "mac_fraction_skinny_gemm_or_gemv": Q_HETERO,
    "clean_8way_mn_shards": Q_HETERO,
    "unit_multiplicity_implication": Q_HETERO,
    "head_weight_bytes": Q_RESIDENCY,
    "avoidable_weight_reload": Q_RESIDENCY,
    "resident_int8_B": Q_RESIDENCY,
    "measured_dispatch_ratio": Q_COMMAND,
    "head_cadence": Q_COMMAND,
    "overlap_candidates_yes": Q_COMMAND,
    "accuracy_int8_w8a8": Q_LOWBIT,
    "accuracy_gate_report_present": Q_LOWBIT,
    "matmul_bias_epilogues": Q_LOWBIT,
    "accumulator_dtype": Q_LOWBIT,
    "compute_dtype": Q_LOWBIT,
    "lowbit_storage_dequantized_finding": Q_LOWBIT,
    "boundary_pressure_score": Q_BOUNDARY,
    "compiler_proofs_proven_for_workload": Q_BOUNDARY,
    "compiler_proofs_assumed": Q_BOUNDARY,
...
def _file_meta(path: Path) -> dict:
    ...
    schema_valid = True
    has_evidence = False
    counts = {lbl: 0 for lbl in ("unavailable", "unknown", "assumed_reference", "derived_requirement", "measured")}
    if path.is_file():
    ...
    return {
        ...
        "has_evidence_fields": has_evidence,
        **counts,
    }
        ...
        rows.append(
            {
                ...
                "exists": p.is_file(),
                "source_phase": phase,
                "checked_by_verify": name in _VERIFIED_ARTIFACTS,
                **m,
            }
        )
...
def _workloads(cs_dir: Path) -> list[str]:
    ...
    return sorted({r["workload"] for r in _csv_rows(cs_dir / "critical_path_table.csv") if r.get("workload")})
...
def _fact(
    ...
    *,
    ...
    return {
        ...
        "derivation_type": _derivation(evidence),
        "evidence_tier": tier,
        ...
        "verifying_check": _METRIC_CHECK.get(metric, ""),
        "corroborated_by": corroborated_by,
        "caveat": caveat,
        "dse_implication": implication,
        ...
        "presentation_candidate": (metric in SIGNAL_METRICS and tier in ("A", "B") and bool(implication)),
    }
...
def unified_facts(cs_dir: Path, scope: str) -> list[dict]:
    ...
    acc_m = {r.get("model", r.get("workload", "")): r for r in _csv_rows(cs_dir / "accuracy_gate_results.csv")}
    ...
    def _ops_weight(w):
        return sum(
            int(r["rhs_weight_bytes"]) for r in ops if r["workload"] == w and r["region_role"] == "repeated_head"
        )
    ...
    for w in workloads:
        ...
        add(
            workload=w,
            metric="ready_quantitative_dse",
            value=d.get("ready_quantitative_dse"),
            unit="bool",
            artifact="dse_readiness_summary.csv",
            phase="P4",
            evidence=_DERIVED,
        ...
        add(
            workload=w,
            metric="accuracy_int8_w8a8",
            value=acc,
            unit="band",
            artifact="accuracy_gated_dtype_candidates.csv",
            phase="P6",
        ...
        if c:
            add(
                workload=w,
                metric="available_parallelism",
                value=c["available_parallelism"],
                unit="work/span",
                artifact="critical_path_table.csv",
                phase="P7",
                evidence=_DERIVED,
            ...
            add(
                ...
                phase="P7",
                evidence="recovered_from_ir",
                corroborated_by=2 if ops else 1,
                caveat=_SMALL,
                implication="per-replan compute volume",
            )
        cw = next((r for r in _csv_rows(cs_dir / "concurrency_windows.csv") if r["workload"] == w), None)
        if cw:
            mw = max(
                (int(r["ready_ops"]) for r in _csv_rows(cs_dir / "concurrency_windows.csv") if r["workload"] == w),
                default=0,
            )
            add(
                workload=w,
                metric="max_ready_width",
                value=mw,
                unit="ops",
                artifact="concurrency_windows.csv",
                phase="P7",
                evidence=_DERIVED,
        ...
        if nm:
            add(
                ...
                value=nm,
                unit="ops",
                artifact="operator_shape_table.csv",
                phase="P5",
                evidence="recovered_from_ir",
                corroborated_by=2 if c else 1,
                implication="operator count to cover",
            )
        ...
        if ws:
            ...
            add(
                workload=w,
                metric="dominant_shape_class",
                value=top["shape_class"],
                unit="",
        ...
        if any(r["workload"] == w for r in shr):
            add(
                workload=w,
                metric="n_region_shape_groups",
                value=sum(1 for r in shr if r["workload"] == w),
                unit="groups",
        ...
        if any(r["workload"] == w for r in tile):
            add(
                workload=w,
                metric="n_tile_evaluations",
                value=sum(1 for r in tile if r["workload"] == w),
                unit="op*primitive",
                artifact="tile_waste_table.csv",
                phase="P5",
                evidence=_DERIVED,
        ...
        if wc:
            ...
            add(
                workload=w,
                abstraction=best["primitive"],
                metric="coverage_under_10pct",
                value=best["coverage_under_10pct"],
                unit="MAC-fraction",
        ...
        if dm_head:
            ...
            tr_head = next((r for r in tr if r["workload"] == w and r["region"] == "repeated_head"), None)
            corr_av = 1 + (1 if tr_head else 0)
            add(
                workload=w,
                region="repeated_head",
                metric="avoidable_weight_reload",
                value=dm_head["avoidable_weight_reload"],
                unit="bytes",
                artifact="data_movement_table.csv",
                phase="P9",
                evidence=_DERIVED,
                corroborated_by=corr_av,
                caveat=_SMALL,
                implication="resident_weight_object residency benefit (bytes), no bandwidth claim",
            )
            corr_r = 1 + (1 if (w in dcap and _eqish(dcap[w].get("int8_B"), dm_head["resident_int8_B"])) else 0)
            add(
                workload=w,
                region="repeated_head",
                metric="resident_int8_B",
                value=dm_head["resident_int8_B"],
                unit="bytes",
                artifact="data_movement_table.csv",
                phase="P9",
                evidence=_DERIVED,
                corroborated_by=corr_r,
        ...
        if any(r["workload"] == w for r in dma):
            ...
            add(
                workload=w,
                metric="byte_carrying_streams",
                value=nbs,
                unit="streams",
                artifact="dma_stream_table.csv",
                phase="P9",
                evidence="recovered_from_ir",
        ...
        if any(r["workload"] == w for r in buf):
            ...
            add(
                workload=w,
                metric="min_input_buffers",
                value=bf["min_input_buffer_count"],
                unit="buffers",
                artifact="buffer_requirement_table.csv",
                phase="P9",
                evidence=_DERIVED,
                implication="double-buffering to overlap DMA with compute",
            )
        if any(r["workload"] == w for r in bufing):
            yes = sum(1 for r in bufing if r["workload"] == w and r["can_overlap"] == "yes")
            add(
                workload=w,
                metric="overlap_candidates_yes",
                value=yes,
                unit="candidates",
        ...
        if any(r["workload"] == w for r in reuse):
            add(
                workload=w,
                metric="n_resident_objects",
                value=sum(1 for r in reuse if r["workload"] == w),
                unit="objects",
                artifact="reuse_lifetime_table.csv",
                phase="P9",
                evidence="recovered_from_ir",
        ...
        if we:
            ...
            add(
                workload=w,
                metric="matmul_bias_epilogues",
                value=n_bias,
                unit="ops",
                artifact="epilogue_pattern_table.csv",
                phase="P10",
                evidence="recovered_from_ir",
        ...
        if wa:
            add(
                workload=w,
                region=wa[0]["region"],
                metric="accumulator_dtype",
                value=wa[0]["accumulator_dtype"],
                unit="dtype",
        ...
        if any(r["workload"] == w for r in shard):
            clean = sum(
                ...
                if r["workload"] == w
                and r["axis"] in ("M", "N")
                and r["shardable_8"] == "True"
                and r["tail_8"] == "False"
            )
            add(
                workload=w,
                metric="clean_8way_mn_shards",
                value=clean,
                unit="op*axis",
                artifact="sharding_table.csv",
                phase="P7",
                evidence=_DERIVED,
        ...
        if any(r["workload"] == w for r in pstage):
            ...
            add(
                workload=w,
                metric="phase_cadences",
                value="; ".join(cads),
                unit="",
                artifact="pipeline_stage_table.csv",
                phase="P8",
                evidence="recovered_from_prov_fqn",
                corroborated_by=2 if prate else 1,
                implication="multi-rate phase structure",
            )
        if w in wct:
            add(
                workload=w,
                metric="recovered_roles",
                value=wct[w].get("roles", ""),
                unit="",
                artifact="workload_contract_table.csv",
                phase="P2",
                evidence="recovered_from_prov_fqn",
                implication="region roles recovered from prov.fqn",
            )
        # dtype capacity (source; corroborates resident_int8) + traffic (corroborates avoidable)
        if w in dcap:
            add(
                workload=w,
                metric="int8_capacity_B",
                value=dcap[w].get("int8_B"),
                unit="bytes",
                artifact="dtype_capacity_table.csv",
                phase="P9",
                evidence=_DERIVED,
        ...
        if trh:
            add(
                workload=w,
                region="repeated_head",
                metric="weight_traffic_if_nonresident",
                value=trh["weight_traffic_if_nonresident"],
                unit="bytes",
                artifact="traffic_table.csv",
                phase="P9",
                evidence=_DERIVED,
        ...
        if wp:
            ...
            add(
                workload=w,
                metric="head_cadence",
                value=head.get("cadence", ""),
                unit="",
                artifact="phase_rate_table.csv",
                phase="P6",
                evidence="recovered_from_prov_fqn",
                corroborated_by=2 if pstage else 1,
                implication="repeated-head cadence (rate class)",
            )
        ...
        if isinstance(nc, dict):
            ...
            add(
                ...
                value=str(ncb.get("compute_dtype", "f32")),
                unit="dtype",
                artifact="numerical_contract.yaml",
                phase="P10",
                evidence="recovered_from_ir",
                implication="storage/compute dtype contract",
            )
        ...
        if isinstance(ra, dict):
            ...
            if attr_mm:
                add(
                    workload=w,
                    metric="attributed_matmuls",
                    value=attr_mm,
                    unit="ops",
                    artifact="region_attribution.yaml",
                    phase="P1",
                    evidence="recovered_from_prov_fqn",
                    corroborated_by=2 if nm else 1,
                    implication="matmuls attributed to roles",
                )
...
def _all_facts(cs_dir: Path, man: dict, add) -> None:
    ...
    for b in sorted(bm, key=lambda r: -int(r["boundary_pressure_score"]))[:8]:
        strong = [
            ...
            for lv in (
                "compiler_transform",
                "runtime_hal_object",
                "command_buffer_or_command_isa",
                "accelerator_isa",
                "device_microcode_or_controller",
                "fixed_hardware_datapath",
            )
        ...
        add(
            ...
            level=";".join(strong),
            metric="boundary_pressure_score",
            value=b["boundary_pressure_score"],
            unit="evidence",
            artifact="hw_sw_boundary_matrix.csv",
            phase="P12",
            evidence=_DERIVED,
            corroborated_by=2 if bc_n else 1,
    ...
    count(
        ...
        "P12",
        "n_runtime_objects",
        "objects that could cross the HAL boundary",
        sub="candidates",
    )
    count(
        "command_isa_candidates.yaml",
        "command_isa_candidates",
        "P12",
        "n_command_candidates",
        "command-ISA abstractions structurally suggested",
        sub="candidates",
    )
    count(
        "isa_candidate_primitives.yaml",
        "isa_candidate_primitives",
        "P12",
        "n_isa_primitives",
        "accelerator-ISA primitives structurally suggested",
        sub="primitives",
    )
    count(
        ...
        "P12",
        "n_boundary_knobs",
        "DSE knobs created by boundary placement",
        sub="knobs",
    )
    ...
    if resp:
        add(
            workload="ALL",
            metric="compiler_owned_functions",
            value=sum(1 for r in resp if r["compiler"] == "owns"),
            unit="functions",
    ...
    if isinstance(knobs, dict):
        ...
        add(
            workload="ALL",
            metric="enabled_knob_groups",
            value=sum(1 for g in grp if g.get("enabled")),
            unit="groups",
            artifact="dse_search_space_knobs.yaml",
            phase="P11",
            evidence=_DERIVED,
    ...
    if cpm:
        for st in ("proven_for_workload", "assumed", "unknown"):
            add(
                workload="ALL",
                metric=f"compiler_proofs_{st}",
                value=sum(1 for r in cpm if r["status"] == st),
                unit="axes",
                artifact="compiler_proof_matrix.csv",
                phase="P4",
                evidence=_DERIVED,
                implication=f"abstraction axes with compiler proof status = {st}",
            )
    count(
        "abstraction_pressure_ranking.csv",
        None,
        "P4",
        "n_abstraction_axes_ranked",
        "abstraction axes by cross-workload pressure",
    )
    count(
        "abstraction_pressure_table.csv",
        None,
        "P4",
        "n_abstraction_pressure_rows",
        "per-workload abstraction pressure rows",
    )
    count("workload_family_table.csv", None, "P4", "n_workload_families", "workloads clustered into families")
    count("resident_state_table.csv", None, "P4", "n_resident_states", "states with a residency lifetime")
    count("requirements_table.csv", None, "P3", "n_requirements", "hardware-independent requirements derived")
    count("cross_workload_provenance.csv", None, "P1", "n_provenance_rows", "flat-vs-recovered provenance items")
    count(
        "operator_cluster_table.csv",
        None,
        "P5",
        "n_shape_clusters",
        "cross-workload geometry clusters",
        evidence="recovered_from_ir",
    )
    count(
        "operator_cluster_to_hierarchy.csv",
        None,
        "P7",
        "n_cluster_hierarchy_maps",
        "shape-cluster -> hierarchy-option mappings",
    )
    count(
        "multi_rate_contract.yaml",
        "multi_rate_contract",
        "P6",
        "n_multi_rate_workloads",
        "workloads with a recovered multi-rate contract",
        sub="workloads",
    )
    count(
        "numerical_epilogue_candidates.yaml",
        "numerical_epilogue_candidates",
        "P10",
        "n_fused_candidates",
        "fused-epilogue abstraction candidates",
        sub="candidates",
    )
    count(
        "lost_numerical_contracts.csv",
        None,
        "P10",
        "n_erased_numerical_contracts",
        "numerical contracts erased by the capture (scoped limitation)",
        evidence="unavailable",
    )
    count(
        "processing_unit_candidates.yaml",
        "processing_unit_candidates",
        "P7",
        "n_unit_candidates",
    ...
    count(
        "boundary_candidate_contracts.yaml",
        "boundary_candidate_contracts",
        "P12",
        "n_boundary_certificates",
        "HW/SW boundary-placement certificates",
        sub="certificates",
    )
    count(
        "pipeline_candidates.yaml",
        "pipeline_candidates",
        "P8",
        "n_pipeline_candidate_workloads",
        "workloads with pipeline overlap candidates",
        sub="workloads",
    )
    count(
        "pipeline_envelope.yaml",
        "pipeline_envelope",
        "P8",
        "n_pipeline_envelope_workloads",
        "workloads with a multi-rate phase model",
        sub="workloads",
        evidence="recovered_from_prov_fqn",
    )
    ...
    if isinstance(mh, dict):
        regs = sum(len(x.get("regions", [])) for x in mh.get("memory_hierarchy_envelope", {}).get("workloads", []))
        add(
            workload="ALL",
            metric="n_memory_regions",
            value=regs,
            unit="regions",
            artifact="memory_hierarchy_envelope.yaml",
            phase="P9",
            evidence="recovered_from_ir",
    ...
    if isinstance(so, dict):
        ...
        add(
            workload="ALL",
            metric="n_sharding_abstractions",
            value=len(ra),
            unit="abstractions",
            artifact="sharding_opportunities.yaml",
            phase="P7",
            evidence=_DERIVED,
    ...
    if (cs_dir / "accuracy_gate_report.md").is_file():
        ...
        add(
            workload="ALL",
            metric="accuracy_gate_report_present",
            value="yes",
            unit="",
            artifact="accuracy_gate_report.md",
            phase="P4",
            evidence="measured",
            corroborated_by=2,
            implication="measured int8 accuracy summary (real measurement)",
        )
    # operator_geometry total operators (corroborates operator_shape_table)
    og = _yaml(cs_dir / "operator_geometry.yaml")
    if isinstance(og, dict):
        tot = sum(x.get("n_operators", 0) for x in og.get("operator_geometry", {}).get("workloads", []))
        add(
            workload="ALL",
            metric="total_operators",
            value=tot,
            unit="ops",
            artifact="operator_geometry.yaml",
            phase="P5",
            evidence="recovered_from_ir",
    ...
    if isinstance(wcg, dict):
        nodes = sum(len(g.get("nodes", [])) for g in wcg.get("workload_contract_graph", {}).get("graphs", []))
        add(
            workload="ALL",
            metric="contract_graph_nodes",
            value=nodes,
            unit="nodes",
            artifact="workload_contract_graph.yaml",
            phase="P6",
            evidence="recovered_from_ir",
    ...
    if rp:
        for rc in ("dense_gemm", "skinny_gemm_or_gemv"):
            if rc in rp:
                add(
                    workload="ALL",
                    metric=f"mac_fraction_{rc}",
                    value=rp[rc]["mac_fraction"],
                    unit="MAC-fraction",
                    artifact="resource_pressure_table.csv",
                    phase="P7",
                    evidence="recovered_from_ir",
    ...
    if isinstance(pug, dict):
        ...
        add(
            workload="ALL",
            metric="unit_multiplicity_implication",
            value="heterogeneous" if "heterogeneous" in impl.lower() else "see report",
            unit="",
            artifact="processing_unit_guidance.yaml",
            phase="P8",
            evidence=_DERIVED,
    ...
    for r in sorted(reg, key=lambda x: -float(x["coverage_under_10pct"]))[:2]:
        add(
            workload="ALL",
            abstraction=r["primitive"],
            metric="coverage_under_10pct",
            value=r["coverage_under_10pct"],
            unit="MAC-fraction",
            artifact="primitive_regret_table.csv",
            phase="P5",
            evidence=_DERIVED,
    ...
    mp = sorted(_csv_rows(cs_dir / "measurement_priority_table.csv"), key=lambda r: -int(r["n_candidates_unblocked"]))[
        :5
    ]
    for r in mp:
        add(
            workload="ALL",
            metric="measurement_unblocks",
            value=r["n_candidates_unblocked"],
            unit="candidates",
            artifact="measurement_priority_table.csv",
            phase="P4",
            evidence=_DERIVED,
            caveat=r["measurement"],
    ...
    for r in _csv_rows(cs_dir / "accuracy_gate_results.csv"):
        ...
        add(
            ...
            value=st,
            unit="band",
            artifact="accuracy_gate_results.csv",
            phase="P4",
            evidence="measured" if st else "unavailable",
            corroborated_by=2 if st else 1,
    ...
    if disp:
        add(
            workload="ALL",
            metric="measured_dispatch_ratio",
            value=disp[0].get(disp[0] and list(disp[0])[-1], "see file"),
            unit="ratio",
            artifact="dispatch_coupling.csv",
            phase="P4",
            evidence="measured",
    ...
    if fid.is_file():
        add(
            workload="ZOO",
            metric="lowbit_storage_dequantized_finding",
            value="present",
            unit="",
            artifact="numerical_contract_fidelity_report.md",
            phase="P4",
            evidence="recovered_from_ir",
...
_QUERIES = [
    (
        "primitive_shapes_to_include",
        "What compute primitive shapes should a DSE search space include?",
        ["primitive_coverage_matrix.csv", "primitive_regret_table.csv"],
    ),
    (
        "primitives_broadly_useful",
        "Which primitive shapes are broadly useful across workloads?",
        ["primitive_regret_table.csv"],
    ),
    (
        "primitives_workload_specific",
        "Which primitive shapes are workload-specific / high-regret?",
        ["primitive_regret_table.csv"],
    ),
    (
        "heterogeneous_units",
        "Which workloads suggest heterogeneous processing units?",
        ["processing_unit_guidance.yaml", "resource_pressure_table.csv"],
    ),
    (
        "bounded_loop_commands",
        "Which workloads suggest bounded-loop commands?",
        ["workload_contract_graph.yaml", "boundary_candidate_contracts.yaml"],
    ),
    (
        "strong_abstractions",
        "Which abstractions are most strongly supported across workloads?",
        ["abstraction_pressure_ranking.csv", "hw_sw_boundary_matrix.csv"],
    ),
    (
        "family_specific_abstractions",
        "Which abstractions are family-specific?",
        ["abstraction_pressure_table.csv", "workload_family_table.csv"],
    ),
    (
        "boundary_resident_weights",
        "Which boundary placements are plausible for resident weights?",
        ["boundary_candidate_contracts.yaml", "hw_sw_boundary_matrix.csv"],
    ),
    (
        "boundary_kloop",
        "Which boundary placements are plausible for K-loop execution?",
        ["boundary_candidate_contracts.yaml"],
    ),
    (
        "boundary_packed_lowbit",
        "Which boundary placements are plausible for packed low-bit tensors?",
        ["boundary_candidate_contracts.yaml"],
    ),
    ("hal_objects", "Which objects should potentially cross the HAL boundary?", ["runtime_object_candidates.yaml"]),
    ("command_isa", "Which command-ISA abstractions are structurally suggested?", ["command_isa_candidates.yaml"]),
    (
        "accelerator_isa",
        "Which accelerator-ISA primitives are structurally suggested?",
        ["isa_candidate_primitives.yaml"],
    ),
    ("blocked_by_proof", "Which candidates are blocked by missing compiler proof?", ["compiler_proof_matrix.csv"]),
    (
        "blocked_by_measurement",
        "Which candidates are blocked by missing measurements?",
        ["dse_readiness_summary.csv", "measurement_priority_table.csv"],
    ),
    (
        "measurements_unblock_most",
        "Which measurements unblock the most candidates?",
        ["measurement_priority_table.csv"],
    ),
    ("assumption_heavy", "Which findings rely heavily on assumptions?", ["dse_contract.json"]),
    (
        "shallow_analyses",
        "Which analyses are currently shallow or incomplete?",
        ["lost_numerical_contracts.csv", "boundary_candidate_contracts.yaml"],
    ),
    (
        "plots_for_presentation",
        "Which plots should be generated for presentation?",
        ["operator_shape_table.csv", "critical_path_table.csv", "hw_sw_boundary_matrix.csv", "data_movement_table.csv"],
    ),
    (
        "safe_claims",
        "Which claims are safe to present without quantitative performance measurements?",
...
_LIMIT_QUERIES = {
    "boundary_packed_lowbit": "weak",
    "assumption_heavy": "partial",
    "shallow_analyses": "partial",
    "blocked_by_proof": "partial",
    "family_specific_abstractions": "partial",
    "primitives_workload_specific": "partial",
}
...
def usefulness(cs_dir: Path, scope: str, facts: list[dict]) -> list[dict]:
    ...
    for key, q, arts in _QUERIES:
        ...
        strong_facts = [
            f
            for f in related
            if f["evidence_tier"] in ("A", "B") and (f["corroborated_by"] >= 2 or f["verifying_check"])
        ]
        ...
        use = {"strong": "main", "partial": "backup", "weak": "backup", "unavailable": "do_not_show"}[status]
        answers.append(
            {
                "key": key,
                "query": q,
                "status": status,
                "supporting_artifacts": present,
                ...
                "caveats": (
                    "packed low-bit / scales erased in the capture (see required_inputs_manifest)"
                    if key == "boundary_packed_lowbit"
                    else "structural only; not a performance claim"
                ),
    ...
    for metric, fs in by_metric.items():
        ...
        for v in spread.values():
            try:
                ...
                pass
        spread_summary = (
            f"range [{min(vals):g}, {max(vals):g}] across {len(spread)} workloads"
            if len(vals) >= 2
            else (f"{len(spread)} workload(s)" if spread else "cross-workload")
        )
        ...
        findings.append(
            {
                ...
                "claim": f"{metric}: {impl}",
                "evidence_tier": tier,
                "supporting_artifacts": arts,
                "supporting_workloads": wls,
                "per_workload_values": spread,
                "per_workload_spread": spread_summary,
                "max_corroborated_by": max_corr,
                "relevant_metrics": [metric],
                "dse_implication": impl,
                "caveats": caveats,
                "dse_question": _METRIC_QUESTION.get(metric, ""),
        ...
        findings.append(
            {
                ...
                "evidence_tier": "D",
                "supporting_artifacts": ["lost_numerical_contracts.csv", "boundary_candidate_contracts.yaml"],
                "supporting_workloads": ["ALL"],
                "relevant_metrics": ["unavailable"],
...
_PLOTS = [
    (
        ...
        "unified_fact_table.csv",
        ["workload", "evidence_type"],
        "workload",
        "count",
        "evidence_type",
        "stacked_bar",
    ...
    (
        ...
        "shape_summary_by_workload.csv",
        ["workload", "shape_class", "mac_fraction"],
        "workload",
        "mac_fraction",
        "shape_class",
        "stacked_bar",
    ...
    (
        "shape_class_opcount_share",
        "Shape-class op-count share by workload",
        "shape_summary_by_workload.csv",
        ["workload", "shape_class", "op_count"],
        "workload",
        "op_count",
        "shape_class",
        "stacked_bar",
    ...
    (
        "primitive_coverage_heatmap",
        "Primitive x workload structural coverage",
        "primitive_coverage_matrix.csv",
        ["primitive", "workload", "coverage_under_10pct"],
        "workload",
        "primitive",
        "coverage_under_10pct",
        "heatmap",
    ...
    (
        ...
        "primitive_regret_table.csv",
        ["primitive", "coverage_under_10pct", "max_regret"],
        "primitive",
        "coverage_under_10pct",
        "max_regret",
        "grouped_bar",
    ...
    (
        "abstraction_pressure_bar",
        "Abstraction pressure (workloads supporting)",
        "abstraction_pressure_ranking.csv",
        ["system_abstraction", "n_workloads"],
        "system_abstraction",
        "n_workloads",
        "",
        "bar",
    ...
    (
        ...
        "data_movement_table.csv",
        ["workload", "region", "resident_int8_B", "resident_bf16_B"],
        "workload_region",
        "bytes",
        "dtype",
        "grouped_bar",
    ...
    (
        ...
        "data_movement_table.csv",
        ["workload", "region", "avoidable_weight_reload"],
        "workload_region",
        "avoidable_weight_reload",
        "",
        "bar",
    ...
    (
        ...
        "measurement_priority_table.csv",
        ["measurement", "n_candidates_unblocked"],
        "measurement",
        "n_candidates_unblocked",
        "",
        "bar",
        "main",
    ),
    ...
    (
        "epilogue_pattern_counts",
        "Epilogue patterns by workload",
        "epilogue_pattern_table.csv",
        ["workload", "pattern"],
        "workload",
        "count",
    ...
    (
        ...
        "data_movement_table.csv",
        ["workload", "region", "weight_bytes", "invocations"],
        "loop_count",
        "bytes_moved",
        "policy",
        "decision_curve",
    ...
    (
        ...
        "sharding_table.csv",
        ["axis", "shardable_2", "shardable_4", "shardable_8", "per_extra_shard_bytes"],
        "shard_count",
        "extra_bytes",
        "axis",
        "decision_bar",
    ...
    (
        ...
        "tile_waste_table.csv",
        ["workload", "op_index", "primitive", "covered_under_10pct"],
        "mean_coverage",
        "worst_coverage",
        "set",
        "scatter",
    ...
    (
        ...
        "sharding_table.csv",
        ["workload", "op_index", "axis", "per_extra_shard_bytes"],
        "top_op",
        "extra_over_output",
        "axis",
        "decision_bar",
    ...
    (
        ...
        ["workload", "region", "requirement", "value"],
        "deadline_ms",
        "required_GMAC_per_s",
        "workload",
        "line",
    ...
    (
        ...
        "work_coverage_table.csv",
        ["workload", "linear_gemm_macs", "attention_macs"],
        "workload",
        "recovered_macs",
        "op_class",
        "stacked_bar",
    ...
    (
        ...
        "work_coverage_table.csv",
        ["workload", "visible_linear_fraction"],
        "workload",
        "visible_linear_fraction",
        "",
        "bar",
...
def canonical_signal_table(facts: list[dict]) -> list[dict]:
    ...
    for f in facts:
        ...
        strength = (
            "measured"
            if f.get("metric_class") == "measured"
            else (
                "verified+corroborated"
                if (f.get("verifying_check") and int(cb or 1) >= 2)
                else "verified"
                if f.get("verifying_check")
                else "corroborated x%d" % int(cb or 1)
                if int(cb or 1) >= 2
                else "single-source"
            )
        )
        ...
        rows.append(
            {
                "dse_question": f.get("dse_question", ""),
                "metric": f["metric_name"],
                "workload": f.get("workload", ""),
                "entity": entity,
                "value": f.get("metric_value", ""),
                "unit": f.get("metric_unit", ""),
...
def per_operator_hotspots(cs_dir: Path, scope: str = "all", k: int = 10) -> dict:
    ...
    by_pad = sorted(
        [r for r in enriched if r["best_tile_padding_waste"] is not None], key=lambda r: -r["best_tile_padding_waste"]
    )[:k]
    ...
    reload_rows = [
        {
            ...
            "weight_bytes": int(r["weight_bytes"]),
        }
        for r in by_reload
    ]
    ...
    for tag, rows_ in (("macs", by_macs), ("weight_bytes", by_weight), ("padding_waste", by_pad)):
        for i, r in enumerate(rows_, 1):
            flat.append({"ranking": tag, "rank": i, **r, "region": "", "avoidable_weight_reload": ""})
    for i, r in enumerate(reload_rows, 1):
        flat.append(
            {
                "ranking": "avoidable_reload",
                "rank": i,
                "workload": r["workload"],
                "op_index": "",
                "prov_fqn": "",
                ...
                "K": "",
                "macs": "",
                "mac_fraction_of_workload": "",
                "rhs_weight_bytes": r["weight_bytes"],
                "best_tile_padding_waste": "",
                "is_tail_heavy": "",
                "region": r["region"],
    ...
    return {
        "n_ops": len(ops),
        "by_macs": by_macs,
        "by_weight_bytes": by_weight,
        "by_padding_waste": by_pad,
        "by_avoidable_reload": reload_rows,
...
def abstraction_coverage(cs_dir: Path) -> list[dict]:
    ...
    for b in bm:
        ...
        rows.append(
            {
                "abstraction": b["abstraction"],
                "workloads_supporting": "; ".join(supp),
                "n_workloads": nwl,
...
def workload_family_summary(cs_dir: Path, findings: list[dict] | None = None) -> dict:
    ...
    for w in wls:
        f = _family_of(w)
        fam.setdefault(f, {"workloads": [], "total_macs": 0, "dominant_shape_class": "", "parallelism": [], "K": []})
    ...
    for f, d in fam.items():
        ...
        d["parallelism_range"] = (
            [round(min(d["parallelism"]), 2), round(max(d["parallelism"]), 2)] if d["parallelism"] else []
        )
    ...
    for fd in findings or []:
        ...
        rec = {"claim": fd.get("title", ""), "families": sorted(fams), "dse_question": fd.get("dse_question", "")}
        (fam_specific if len(fams) == 1 else cross).append(rec)
    return {"families": fam, "family_specific_findings": fam_specific, "cross_family_findings": cross}
...
def corpus_expansion_plan(cs_dir: Path) -> dict:
    ...
    for m in missing:
        ...
        by_family.setdefault(a.family, []).append(
            {"model": m, "loop_kind": a.loop_kind, "reference_K": a.loop_count, "note": a.note or ""}
        )
    return {
        "captured_models": sorted(captured),
        "captured_families": sorted({MODEL_ARCH[b].family for b in captured if b in MODEL_ARCH}),
        "missing_by_family": by_family,
...
def _signal_by_workload(cs_dir: Path):
    ...
    for w in wls:
        ...
        skinny_gemm = sum(float(r["mac_fraction"]) for r in srows if r["shape_class"] in ("wide_skinny", "tall_skinny"))
        ...
        sig[w] = {
            ...
            "dense_mac": round(dense, 4),
            "gemv_mac": round(gemv, 4),
            "true_gemv_mac": round(true_gemv, 4),
            "skinny_gemm_mac": round(skinny_gemm, 4),
            ...
            "k_shard": any(
                r["axis"] == "K" and r["reduction_required"] == "True" and r.get("shardable_8") == "True"
                for r in shrows
            ),
            "mn_shard": any(
                r["axis"] in ("M", "N") and r["reduction_required"] == "False" and r.get("shardable_8") == "True"
                for r in shrows
            ),
            "epi_scale_act": any(r["has_scale"] == "True" and r["has_activation"] == "True" for r in erows),
            "epi_bias": any(r["has_bias"] == "True" for r in erows),
            "has_backbone": any(o["region_role"] == "backbone_once" for o in ops if o["workload"] == w),
            "has_repeated": any(o["region_role"] == "repeated_head" for o in ops if o["workload"] == w),
        }
    return sig, wls
...
def _classify_abstraction(spec: dict, w: str, sig: dict) -> tuple:
    ...
    has_control = s["family"] in ("flow_matching", "diffusion", "autoregressive_vla") and bool(s["control_rate"])
    ...
    if name == "resident_weight_object":
        if s["invocations"] > 1 and s["weight_bytes"] > 1_000_000 and s["avoidable_reload"] > s["weight_bytes"]:
            return "necessary", (
                "repeated weight reuse (K>1), resident weights >1MB, "
    ...
    if sup == "dense":
        d = s["dense_mac"]
        return (
            ("necessary", "dense (squareish_gemm) MAC fraction >0.5")
            if d > 0.5
            else ("useful", "dense MAC fraction in (0.1, 0.5]")
            if d > 0.1
    ...
    if sup == "gemv":
        ...
        return (
            ("necessary", "gemv/skinny MAC fraction >0.5 (true_gemv+skinny; split in predicate_audit)")
            if g > 0.5
            else ("useful", "gemv/skinny MAC fraction in (0.1, 0.5]")
            if g > 0.1
    ...
    if sup == "k_loop":  # bounded_loop_command / loop_carried_state_handle
        if s["K"] > 1:
            return "useful", (
                "K>1 cadence (configured/reference, not IR-recovered; needs a "
...
def abstraction_necessity(cs_dir: Path) -> dict:
    ...
    for spec in catalog_rows():
        ...
        reason = next(
            (r for w, (c, r) in per.items() if c == "necessary"),
            next(
                (r for w, (c, r) in per.items() if c == "useful"),
                next((r for w, (c, r) in per.items() if c == "possible"), next(iter(per.values()))[1]),
            ),
        )
        row = {
            "abstraction": spec["abstraction"],
            "support_tag": spec["support"],
            "macro_class": macro,
            "predicate": reason,
            "capture_gate": _capture_gate(spec),
        }
        for w in wls:
            row[w] = classes[w]
...
def _frontier_stats(pset, op_macs, op_cover, drop=None):
    ...
    micro = (
        sum(
            _set_coverage(pset, op_macs, op_cover, drop)[w] * sum(m for (ww, _), m in op_macs.items() if ww == w)
            for w in cov
        )
        / tot_macs
    )
    ...
    return {
        ...
        "micro": round(micro, 4),
        "worst": round(worst, 4),
        "max_regret": round(max(vals) - min(vals), 4),
        "per_workload": cov,
    }
...
def robustness(cs_dir: Path) -> dict:
    ...
    findings.append(
        {
            "finding": "best_2_primitive_set",
            "all": list(full[1]),
            "all_worst": full[2]["worst"],
            "all_macro": full[2]["macro"],
    ...
    dense_micro_loo = {
        w: (sum(sig[x]["dense_mac"] * tot_mac[x] for x in wls if x != w) / (corpus_mac - tot_mac[w] or 1.0))
        for w in wls
    }
    drop_to_low = sorted(w for w in wls if dense_micro_loo[w] < 0.2 <= dense_micro)
    findings.append(
        {
            "finding": "dense_gemm_mac_dominance",
            "macro": round(dense_macro, 4),
            "micro": round(dense_micro, 4),
            "micro_loo": {w: round(v, 4) for w, v in dense_micro_loo.items()},
            "collapses_if_removed": drop_to_low,
            "note": (
                "MAC-dominant (micro) but corpus-narrow (macro)"
                if dense_micro - dense_macro > 0.2
                else "consistent across views"
            ),
        }
    )
    ...
    findings.append(
        {
            "finding": "residency_pressure_rank",
            "all": rank,
...
_FIDELITY_FEATURES = [
    ("op_shapes_MNK", "strong"),
    ("region_roles", "strong"),
    ("dtype_information", "strong"),
    ...
    ("attention_bmm_qkT_attnV", "recovered_attn"),
    ("softmax", "recovered_softmax"),
    ("normalization", "recovered_norm"),
    ("K_or_decode_loop", "loop"),
    ("kv_cache_state", "kv"),
    ("loop_carried_state", "loop_carried"),
    ("packed_lowbit_layout", "lowbit"),
    ("scale_metadata", "lowbit"),
    ("host_dispatch_count", "measured"),
    ...
    for w in wls:
        ...
        if mp.is_file():
            ...
            if lr.present:
                lr_by_w[w] = lr
    fam_to_class = {
        "flow_matching": TOP.CLASS_FLOW_MATCHING,
        "diffusion": TOP.CLASS_FLOW_MATCHING,
...
def decision_scorecard(bundle: dict) -> list[dict]:
    ...
    q.append(
        {
            "q": "Q1 best single primitive (worst-workload coverage)?",
            "answer": (f"{s1['set'][0]} -> worst {s1['worst']:.2f}, macro {s1['macro']:.2f}" if s1 else "n/a"),
    ...
    q.append(
        {
            "q": "Q2 best 2-primitive set?",
            "answer": (
                f"{'+'.join(s2['set'])} -> worst {s2['worst']:.2f} (vs {s1['worst']:.2f} single)"
                if s2 and s1
                else "n/a"
            ),
    ...
    q.append(
        {
            "q": "Q3 capacity x dtype residency thresholds?",
    ...
    q.append(
        {
            "q": "Q4 sharding axis for top-MAC ops?",
    ...
    q.append(
        {
            "q": "Q5 which abstractions are NECESSARY (not just possible)?",
            "answer": (
                f"{necd.get('necessary', 0)} necessary, {necd.get('useful', 0)} useful, "
    ...
    q.append(
        {
            "q": "Q6 which conclusions are driven by one workload (RDT)?",
            "answer": (
                f"dense-MAC dominance macro {dd.get('macro', '?')} vs micro "
    ...
    q.append(
        {
            "q": "Q7 which claims depend on configured K (capture fidelity)?",
...
def _predicate_io(spec: dict, w: str, sig: dict) -> tuple:
    ...
    if name == "resident_weight_object":
        return (
            f"K={s['invocations']}, weight_bytes={s['weight_bytes']}, avoidable_reload={s['avoidable_reload']}",
            "K>1 & weight_bytes>1e6 & avoidable_reload>weight_bytes",
            "yes",
            "yes",
        )
    if sup == "dense":
    ...
    if sup == "gemv":
        return (
            f"gemv_mac={s['gemv_mac']} (true_gemv={s['true_gemv_mac']}, skinny_gemm={s['skinny_gemm_mac']})",
            ">0.5 necessary; >0.1 useful",
            "no",
            "no",
        )
    if name in ("partial_sum_object", "accumulator_merge"):
        return (f"k_shard={s['k_shard']}, mn_shard={s['mn_shard']}", "k_shard & not mn_shard -> necessary", "no", "no")
    if sup == "epilogue":
        return (
            f"epi_scale_act={s['epi_scale_act']}, epi_bias={s['epi_bias']}",
            "scale+act -> necessary; bias -> useful",
            "no",
            "no",
        )
    if sup == "k_loop":
...
def predicate_audit(cs_dir: Path) -> dict:
    ...
    for spec in catalog_rows():
        ...
        has_neg = any(c in ("possible", "blocked", "not_applicable") for c in classes) and any(
            c in ("necessary", "useful") for c in classes
        )
        for w in wls:
            cls, reason = per[w]
            ...
            rows.append(
                {
                    ...
                    "thresholds": thr,
                    "uses_configured_K": usesK,
                    "uses_erased_capture": erased,
...
_UNCOV_COLS = ["threshold_pct", "set_size", "primitive_set", "workload", "op_index", "status", "true_macs"]
...
def primitive_frontier_robustness(cs_dir: Path, max_size: int = 4) -> dict:
    ...
    cs_dir = Path(cs_dir)
    base = sorted({r["primitive"] for r in _csv_rows(cs_dir / "tile_waste_table.csv") if r.get("applicable") == "True"})
    ...
    def stats(pset, t, drop=None):
        ...
        return {
            "macro": sum(vals) / len(vals),
            "micro": sum(num.values()) / tot,
            "worst": min(vals),
            "max_regret": max(vals) - min(vals),
            "per": per,
        }
            ...
            rows.append(
                {
                    ...
                    "set_size": size,
                    "primitive_set": "+".join(combo),
                    "worst": round(st["worst"], 4),
                    "macro": round(st["macro"], 4),
                    "micro": round(st["micro"], 4),
...
_INFLUENCE_COLS = [
    ...
    "best",
    "most_influential",
    "loo_micro_at_influential",
    "max_loo_micro_delta",
    "winner_stable_magnitude_unstable",
]
...
def macro_micro_influence(cs_dir: Path) -> dict:
    ...
    for label, key in _INFLUENCE_METRICS:
        ...
        loo = {w: (sum(vals[x] * tot_mac[x] for x in wls if x != w) / ((corpus - tot_mac[w]) or 1.0)) for w in wls}
        ...
        rows.append(
            {
                ...
                "worst_workload": worst_w,
                "worst": round(vals[worst_w], 4),
                "best_workload": best_w,
                "best": round(vals[best_w], 4),
                "most_influential": infl,
                "loo_micro_at_influential": round(loo[infl], 4),
    ...
    return {"workloads": wls, "rows": rows, "loo_rows": loo_rows, "residency_pressure_rank": reload_rank}
...
_AUDIT_COLS = [
    ...
    "supporting_metric",
    "metric_class",
    "depends_on_configured_K",
    "depends_on_flat_capture",
    "survives_loo",
    "survives_macro_vs_micro",
    "survives_threshold",
    "driven_by",
    "decision_or_fact",
    "verdict",
...
def adversarial_audit(bundle: dict) -> dict:
    ...
    add(
        conclusion="primitive-set frontier: a 2-primitive set covers the corpus where one fails",
        ...
        depends_on_configured_K="no",
        depends_on_flat_capture="no",
        ...
        survives_threshold=(
            "yes (5/10/20%)"
            if thr_robust
            else ("not computed" if thr_robust is None else f"no (winner varies: {frob.get('two_winners')})")
        ),
        driven_by=("specific pair shifts with finer tiles/threshold" if not thr_robust else "none"),
        ...
        verdict=(
            "robust"
            if two.get("robust") and thr_robust
            else "claim robust (a 2-set suffices); specific pair is threshold/LOO-sensitive"
        ),
    )
    dd = rob.get("dense_gemm_mac_dominance", {})
    di = infl.get("dense_gemm_mac_fraction", {})
    add(
        conclusion="dense-GEMM is corpus-narrow (skinny/GEMV dominates the MAC mass)",
        ...
        depends_on_flat_capture="no",
        survives_loo=(
            f"magnitude NOT stable (max micro swing {di.get('max_loo_micro_delta', '?')})"
            if di.get("winner_stable_magnitude_unstable") == "yes"
            else "yes"
        ),
        survives_macro_vs_micro="winner yes; magnitude no (macro/micro differ widely)",
        survives_threshold="n/a",
        driven_by=di.get("most_influential", "pi05"),
        decision_or_fact="decision (do not size only for square GEMMs)",
        verdict="winner robust, magnitude fragile",
    )
    add(
        conclusion="resident_weight_object is necessary (K-loop weight residency is a search axis)",
        ...
        metric_class="derived (weight bytes) + assumed (K)",
        depends_on_configured_K="yes",
        depends_on_flat_capture="yes (K-loop erased)",
        survives_loo="n/a (per-workload predicate)",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="configured/reference K",
        decision_or_fact="decision (residency knob)",
        verdict="blocked-by-capture (needs a loop-preserving capture to confirm K)",
    )
    add(
        conclusion="skinny_gemm_or_gemv_engine is necessary/useful across the corpus",
        supporting_artifact="abstraction_necessity_table.csv; predicate_audit_table.csv",
        supporting_metric="gemv/skinny MAC fraction (true_gemv + skinny split in predicate_audit)",
        metric_class="derived",
        depends_on_configured_K="no",
        depends_on_flat_capture="no",
        survives_loo="see predicate_audit per-workload",
        survives_macro_vs_micro="yes",
        survives_threshold="n/a",
        driven_by="skinny-GEMM projections (not true GEMV)",
        decision_or_fact="decision (engine serves skinny GEMM, not only M/N<=4 GEMV)",
        verdict="robust (name now honest: skinny OR gemv)",
    )
    add(
        conclusion="low-bit abstractions are blocked (cannot be evaluated)",
        ...
        depends_on_configured_K="no",
        depends_on_flat_capture="yes (packed layout + scales erased)",
        survives_loo="n/a",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="f32 fake-quant capture",
        decision_or_fact="decision-blocking (needs a low-bit recapture)",
        verdict="blocked-by-capture",
    )
    add(
        conclusion="attention/KV abstractions are blocked (attention lowered)",
        supporting_artifact="abstraction_necessity_table.csv; capture_fidelity_matrix.csv",
        supporting_metric="KV abstractions classified blocked/not_applicable",
        metric_class="unavailable",
        depends_on_configured_K="no",
        depends_on_flat_capture="yes (q.kT / attn.v lowered)",
        survives_loo="n/a",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="flat capture",
        decision_or_fact="decision-blocking (needs attention-preserving capture)",
        verdict="blocked-by-capture",
    )
    add(
        conclusion="bounded_loop_command / loop_carried_state_handle are useful",
        supporting_artifact="abstraction_necessity_table.csv; predicate_audit_table.csv",
        supporting_metric="K>1 cadence (configured/reference)",
        metric_class="assumed",
        depends_on_configured_K="yes",
        depends_on_flat_capture="yes (loops not in IR)",
        survives_loo="n/a",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="configured/reference K",
    ...
    add(
        conclusion="capture fidelity is the limiting factor (flat captures erase loop/KV/low-bit)",
        ...
        metric_class="derived",
        depends_on_configured_K="no",
        depends_on_flat_capture="yes",
        survives_loo="uniform across corpus",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="capture level (uniform)",
        decision_or_fact="methodology result (the central finding)",
        verdict="robust",
    )
    add(
        conclusion="HW/SW boundary placement is itself a DSE search axis",
        ...
        metric_class="derived",
        depends_on_configured_K="no",
        depends_on_flat_capture="partly",
        survives_loo="n/a",
        survives_macro_vs_micro="n/a",
        survives_threshold="n/a",
        driven_by="structural enumeration",
        decision_or_fact="framing (search axis, not a choice)",
...
_ENV_COLS = [
    ...
    "deadline_ms",
    "deadline_basis",
    "required_compute_MAC_per_s",
    "required_weight_B_per_s_nonresident",
    "required_weight_B_per_s_resident",
    "required_command_rate_per_s",
    "command_rate_basis",
    "resident_capacity_bf16_B",
    "resident_capacity_int8_B",
]
...
def timing_requirement_envelope(cs_dir: Path) -> dict:
    ...
    cs_dir = Path(cs_dir)
    req = {
        (r["workload"], r["region"], r["requirement"]): r["value"] for r in _csv_rows(cs_dir / "requirements_table.csv")
    }
    ...
    for w in wls:
        ...
        macs_replan, weight_bytes, disp_replan = (
            g("macs_per_replan"),
        ...
        cmd_basis = (
            "measured_dispatch_available"
            if _base_model(w) in _MEASURED_DISPATCH
            else "proxy_only (matmul-count, ~12x undercount; NOT a hardware command rate)"
        )
        scen = [(float(dl), "sweep") for dl in _DEADLINE_MS]
        if arch and arch.control_rate_hz and arch.action_horizon:
            scen.append(
                (
                    1000.0 * arch.action_horizon / arch.control_rate_hz,
                    f"derived_H/control_rate (H={arch.action_horizon}, {arch.control_rate_hz}Hz; configured/reference)",
                )
            )
...
_OMIT_COLS = [
    ...
    "elementwise_ops",
    "kv_state_recovered",
    "lowbit_packed_recovered",
    "scale_metadata_recovered",
    "still_erased",
    "capture_level_needed",
]
        ...
        rows.append(
            {
                ...
                "kv_state_recovered": (
                    "no (KV state across decode loop erased)" if is_ar else "n/a (non-autoregressive)"
                ),
        ...
        rows.append(
            {
                "workload": d.name,
                "n_scf_loops": n_loops,
                "n_linalg_matmul": n_mm,
                "n_linalg_generic": n_gen,
                "lowbit_int_types_present": lowbit,
                "loops_preserved": n_loops > 0,
                "evidence": (
                    f"{n_loops} scf loops, {n_gen} generics vs {n_mm} named matmuls, "
...
def per_family_summary(cs_dir: Path) -> dict:
    ...
    for f, a in sorted(agg.items()):
        ...
        rows.append(
            {
                "family": f,
                "n_workloads": len(a["workloads"]),
                "workloads": "; ".join(sorted(a["workloads"])),
                "linear_gemm_macs": a["lin_macs"],
                "attention_macs": a["attn_macs"],
                "visible_linear_fraction": round(a["lin_macs"] / tot, 4) if tot else 1.0,
                "n_attention_ops": a["n_attention_ops"],
                "n_softmax": a["n_softmax"],
                "n_norm": a["n_norm"],
            }
        )
    return {"rows": rows}
...
_ABLATION_AXES = [
    "attention_primitive",
    "softmax_reduction_unit",
    "packed_lowbit_weights",
    "scale_metadata",
        ...
        unlock.append(
            {
                "workload": w,
                "attention_primitive": (
                    "named (linalg_ext) @high_level" if hl.get("linalg_ext_softmax") else "recovered (generic) @flat"
                ),
                "softmax_reduction_unit": (
                    "named @high_level" if hl.get("linalg_ext_softmax") else "recovered (generic) @flat"
                ),
                "packed_lowbit_weights": (
                    "decidable @quant_qdq" if qd.get("quant_ext_dequantize") else "blocked (no qdq capture)"
                ),
                "scale_metadata": (
                    "decidable @quant_qdq" if qd.get("quant_ext_dequantize") else "blocked (no qdq capture)"
                ),
...
def plot_manifest(cs_dir: Path, scope: str) -> list[dict]:
    ...
    for pid, title, art, cols, x, y, series, kind, rec in _PLOTS:
        ...
        if present and art.endswith(".csv") and (cs_dir / art).is_file():
            ...
            have_cols = all(
                c in header for c in cols if c not in ("level", "count", "workload_region", "bytes", "dtype")
            )
        ...
        out.append(
            {
                ...
                "series": series,
                "plot_type": kind,
                "available": bool(present and have_cols and n_rows > 0),
...
def consistency_checks(cs_dir: Path, inventory, facts, findings, plots, answers) -> list[tuple]:
    ...
    chk(
        all(all(a in exist for a in f["supporting_artifacts"]) for f in findings),
        "every finding references existing artifacts",
    )
    # 2 every main finding tier A/B
    chk(
        all(f["evidence_tier"] in ("A", "B") for f in findings if f["presentation_placement"] == "main"),
        "every main finding is tier A or B",
    )
    # 3 plot manifest references existing columns/artifacts (for available plots)
    chk(
        all(p["available"] for p in plots if p["recommendation"] != "omit"),
        "every non-omit plot references an existing artifact + columns",
    )
    # 4 no main finding purely assumed
    chk(
        not any(f["presentation_placement"] == "main" and f["evidence_tier"] == "C" for f in findings),
        "no main finding is based only on assumed_reference",
    )
    ...
    chk(
        all(all(p in allowed_lv for p in f["boundary_level"].split(";") if p) for f in facts if f["boundary_level"]),
    ...
    chk(
        meas_q is not None and meas_q["status"] in ("strong", "partial"),
    ...
    chk(
        all("exists" in r for r in inventory) and (art_names == art_names),
        "artifact inventory records exists for every artifact",
    )
    # 10 usefulness answers use the allowed status vocabulary
    chk(
        all(a["status"] in ("strong", "partial", "weak", "unavailable") for a in answers),
...
_INHERENT_LIMITS = [
    {
        "limit": "real deployment K + control rate",
        "currently": "K/control are published config reference values (recovered_from_model_config)",
        "evidence_today": "Tier C",
        "required_input": "a deployment/runtime trace giving actual loop counts + control frequency",
    },
    {
        "limit": "per-unit throughput / latency / area / energy",
        "currently": "Merlin refuses to estimate these for unbuilt hardware",
        "evidence_today": "unavailable",
        "required_input": "a candidate design YAML (unit shapes + a cost model); then the future DSE "
        "tool computes them",
    },
    {
        "limit": "KV / attention structure + true data deps at loop level",
        "currently": "attention is lowered into matmul projections; KV unavailable",
        "evidence_today": "unavailable",
        "required_input": "a Level-2 loop-preserving, attention-not-lowered capture",
    },
    {
        "limit": "packed low-bit layout + scales for the recaptured models",
        "currently": "the recaptures are dequantized f32; the quantized zoo has low-bit storage for OTHER models",
    ...
    {
        "limit": "fp8 / int4 accuracy gates",
        "currently": "int8 W8A8 is measured (real); fp8/int4 are unavailable",
        "evidence_today": "int8 measured",
        "required_input": "per-format accuracy runs (W8A8 already done) for fp8 / int4",
    },
    {
        "limit": "real-magnitude weights",
        "currently": "captures are small random-init instances (structure real, magnitudes small)",
        "evidence_today": "structure recovered_from_ir",
...
def gap_audit(cs_dir: Path, scope: str, bundle: dict | None = None) -> list[dict]:
    ...
    inv, facts, findings, plots, answers = (b["inventory"], b["facts"], b["findings"], b["plots"], b["usefulness"])
    gaps = []
    k = 0
    ...
    if scope == "all":
        for r in inv:
            if r["exists"] and r["source_phase"] != "other" and r["artifact"] not in used:
                g("unused_artifact", r["artifact"], "present artifact contributes 0 mined facts", True, "open")
    ...
    for f in findings:
        ...
        if not backed:
            g(
                "fake_or_unbacked_finding",
                f["title"],
                "main finding has no per-metric-verified or >=2-corroborated fact",
                True,
                "open",
            )
    ...
    for a in answers:
        if a["status"] == "strong" and a.get("n_backing_strong_facts", 0) == 0:
            g("heuristic_status", a["key"], "'strong' status with no backing tier-A/B fact", True, "open")
    # 4 coarse_verification: a tier-A/B fact whose 'verified' is neither a check nor >=2-corroboration
    for f in facts:
        if (
            f["evidence_tier"] in ("A", "B")
            ...
            g(
                "coarse_verification",
                f["fact_id"],
                "verified tier-A/B fact lacks a specific check and <2 corroboration",
                True,
                "open",
            )
    ...
    for f in facts:
        if f["presentation_candidate"] and (f["metric_name"] not in SIGNAL_METRICS or f["evidence_tier"] in ("C", "D")):
            g("uncorroborated_fact", f["fact_id"], "candidate is not a tier-A/B signal metric", True, "open")
    ...
    for f in findings:
        if len(f["supporting_workloads"]) > 1 and not f.get("per_workload_spread"):
            g("aggregated_finding", f["title"], "multi-workload finding hides per-workload spread", True, "open")
...
def mine(cs_dir, scope: str) -> dict:
    ...
    bundle["family_summary"] = (
        workload_family_summary(cs_dir, findings)
        if is_all
        else {"families": {}, "family_specific_findings": [], "cross_family_findings": []}
    )
    ...
    bundle["primitive_frontier_robustness"] = (
        primitive_frontier_robustness(cs_dir) if is_all else {"workloads": [], "rows": [], "uncovered_rows": []}
    )
    bundle["macro_micro_influence"] = (
        macro_micro_influence(cs_dir) if is_all else {"workloads": [], "rows": [], "loo_rows": []}
    )
    ...
    bundle["timing_envelope"] = timing_requirement_envelope(cs_dir) if is_all else {"workloads": [], "rows": []}
    ...
    bundle["capture_ablation"] = capture_level_ablation(cs_dir) if is_all else {"rows": [], "unlock": []}
    return bundle
...
def _inventory_md(inv) -> str:
    ...
    L = [
        "# Artifact inventory\n",
        f"{present}/{len(inv)} expected artifacts present. Missing artifacts are recorded explicitly (partial mode).\n",
    ...
    for r in sorted(inv, key=lambda x: (not x["exists"], x["source_phase"], x["artifact"])):
        L.append(
            f"| {r['artifact']} | {r['exists']} | {r['source_phase']} | {r['file_type']} | "
...
def _strength_md(s, scope) -> str:
    L = [
        f"# Evidence-strength report ({scope})\n",
    ...
    L.append(
        f"- high-confidence (A+B): {s['high_confidence_findings']} · "
...
def _scorecard_md(answers, scope) -> str:
    L = [
        f"# DSE-usefulness scorecard ({scope})\n",
    ...
    for i, a in enumerate(answers, 1):
        L.append(
            f"| {i} | {a['query']} | **{a['status']}** | {a['recommended_presentation_use']} | "
...
def _findings_md(findings, scope) -> str:
    L = [
        f"# Presentation candidate findings ({scope})\n",
    ...
    for place in ("main", "backup"):
        ...
        for f in fs:
            L.append(
                f"- **{f['title']}** [tier {f['evidence_tier']}] — {f['claim']}  "
...
def _plot_md(plots, scope, rendered) -> str:
    ...
    for p in plots:
        L.append(
            f"| {p['plot_id']} | {p['title']} | {p['plot_type']} | {p['source_artifact']} | "
...
def _readme_md(bundle, rendered) -> str:
    ...
    return (
        f"# Insight-mining run — scope: {bundle['scope']}\n\n"
...
def _gap_audit_md(gaps, open_av, scope) -> str:
    ...
    L = [
        f"# Devil's-advocate gap audit ({scope})\n",
...
def _required_inputs_md(ri, scope) -> str:
    L = [
        f"# Required-inputs manifest ({scope})\n",
...
def _hotspots_md(h, scope) -> str:
    ...
    if d:
        L.append(
            f"**Dominant op (by MACs):** `{d['prov_fqn']}` in {d['workload']} — "
    ...
    for r in h["by_macs"]:
        L.append(
            f"| {r['workload']} | {r['prov_fqn']} | {r['M']}×{r['N']}×{r['K']} | "
    ...
    for r in h["by_avoidable_reload"]:
        L.append(f"| {r['workload']} | {r['region']} | {r['avoidable_weight_reload']:,} | {r['weight_bytes']:,} |")
    return "\n".join(L) + "\n"


def _coverage_md(rows, scope) -> str:
    L = [
        f"# Abstraction coverage ({scope})\n",
    ...
    for r in rows:
        L.append(
            f"| {r['abstraction']} | {r['n_workloads']} ({r['workload_coverage']:.0%}) | "
...
def _family_md(fs, scope) -> str:
    L = [
        f"# Workload-family summary ({scope})\n",
...
def _corpus_md(cp, scope) -> str:
    ...
    for fam, models in sorted(cp["missing_by_family"].items()):
        for m in models:
            L.append(f"| {fam} | {m['model']} | {m['loop_kind']} | {m['reference_K']} | {m['note']} |")
...
def _signal_report_md(bundle, scope) -> str:
    ...
    L = [
        f"# Signal findings report ({scope})\n",
    ...
    for q in DSE_QUESTIONS:
        ...
        for f in fs:
            L.append(
                f"- **{f['title']}** [tier {f['evidence_tier']}] — {f['dse_implication']}  "
    ...
    L.append(
        "**Robust (structural, independent of magnitudes):** shape-class distribution, the "
    ...
    L.append(
        "- The 4 recaptures are small, random-init f32 instances — structure and provenance "
    ...
    L.append(
        "- low-bit / KV / attention structure is erased or lowered in the capture, so those "
        "candidates are blocked, not measured (see What remains unclaimed)."
    )
    if bundle.get("scope") == "all":
        L.append(
            f"- {n_fam_specific} family-specific findings recovered: the corpus is small and "
...
def _plots_index_md(plots, scope, rendered, folder) -> str:
    L = [
        f"# Presentation plots index ({scope})\n",
...
def _necessity_cols(wls):
    return (
        ["abstraction", "support_tag", "macro_class"]
        + list(wls)
        + [f"n_{k}" for k in _NEC_CLASSES]
        + ["predicate", "capture_gate"]
    )
...
def _necessity_md(nec, scope) -> str:
    ...
    for row in nec["rows"]:
        L.append(
            f"| {row['abstraction']} | **{row['macro_class']}** | "
...
def _frontier_rows(fr):
    ...
    for size, b in sorted(fr["best_by_size"].items()):
        rows.append(
            {
                "set_size": size,
                "primitive_set": " + ".join(b["set"]),
                "macro_coverage": b["macro"],
                "micro_coverage": b["micro"],
                "worst_workload_coverage": b["worst"],
...
def _frontier_md(fr, scope) -> str:
    L = [
        f"# Primitive-set frontier ({scope})\n",
    ...
    for r in _frontier_rows(fr):
        L.append(
            f"| {r['set_size']} | {r['primitive_set']} | {r['worst_workload_coverage']:.2f} | "
...
def _pareto_md(par, scope) -> str:
    ts = par["thresholds"]
    L = [
        f"# Operator Pareto hotspots ({scope})\n",
        ...
        "| workload | n_ops | " + " | ".join(f"k@{int(t * 100)}%MAC" for t in ts) + " | top-op MAC share |",
        "|---|---|" + "|".join(["---"] * len(ts)) + "|---|",
    ]
    for r in par["rows"]:
        L.append(
            f"| {r['workload']} | {r['n_ops']} | "
...
def _robustness_md(rob, scope) -> str:
    L = [
        f"# Macro/micro + leave-one-workload-out robustness ({scope})\n",
...
def _capture_fidelity_md(cf, scope) -> str:
    wls = cf["workloads"]
    L = [
        f"# Capture-fidelity matrix ({scope})\n",
    ...
    for w in wls:
        d = cf["per_workload"][w]
        L.append(
            f"- **{w}** ({d['family']}, severity {d['severity']}): lost "
...
def _scorecard16_md(qs, scope) -> str:
    L = [
        f"# DSE decision-question scorecard ({scope})\n",
...
def _slides_md(bundle, scope) -> str:
    ...
    s1, s2 = fr.get(1), fr.get(2)
    L = [
        f"# Presentation slide candidates ({scope})\n",
    ...
    slides = [
        (
            "What Merlin does",
            "flat capture -> workload contract -> DSE search-space axes (it "
            "enumerates the search space; it does not choose a design)",
            "dse_search_space_knobs",
        ...
        (
            ...
            "capture_fidelity_matrix",
            "the central next-step result, not a side note",
        ),
    ]
...
def _findings_digest_md(bundle: dict, cs_dir: Path, rendered: list[str]) -> str:
    ...
    canon = bundle["canonical_signal"]
    L = [
        f"# DSE findings digest — scope: {scope}\n",
    ...
    for q in DSE_QUESTIONS:
        ...
        for r in qrows:
            L.append(
                f"| {r['metric']} | {r['entity']} | {r['value']} | {r['unit']} | "
    ...
    L.append(
        "Structural what-if curves (bytes / coverage / counts — never latency or speedup). "
    ...
    L.append(
        "- Every headline row traces through `unified_fact_table.csv` "
        "(`metric_name -> source_artifact -> verifying_check`)."
    )
    L.append(
        "- The numbers are recomputed independently by "
        "`merlin/benchmarks/dse_guidance/verify_implementation.py` (run it; exit 0 = all checks "
        "pass)."
    )
    L.append(
        "- Regenerate this whole folder with `merlin-dse-guidance --insight-mining` "
...
def _predicate_audit_md(pa, scope) -> str:
    ...
    L = [
        ...
        f"**{len(susp)} suspicious cells** (necessity resting on configured K, or a non-discriminating predicate).\n",
    ...
    for r in rows:
        L.append(
            f"| {r['abstraction']} | {r['workload']} | {r['classification']} | "
...
def _adversarial_audit_md(aud, scope) -> str:
    L = [
        f"# Adversarial audit of the headline conclusions ({scope})\n",
    ...
    for r in aud["rows"]:
        L.append(
            f"| {r['conclusion']} | {r['metric_class']} | {r['survives_loo']} | "
...
def _frontier_robustness_md(fro, scope) -> str:
    L = [
        f"# Primitive-set frontier robustness ({scope})\n",
    ...
    for r in fro["rows"]:
        L.append(
            f"| {r['threshold_pct']}% | {r['set_size']} | {r['primitive_set']} | "
            f"{r['worst']:.3f} | {r['macro']:.3f} | {r['micro']:.3f} | {r['max_regret']:.3f} |"
        )
    nunc = len(fro["uncovered_rows"])
    L.append(f"\nUncovered ops as the set grows (10% threshold): `uncovered_ops_by_primitive_set.csv` ({nunc} rows).\n")
    return "\n".join(L) + "\n"


def _influence_md(inf, scope) -> str:
    L = [
        f"# Workload influence / magnitude stability ({scope})\n",
    ...
    for r in inf["rows"]:
        L.append(
            f"| {r['metric']} | {r['macro']:.3f} | {r['micro']:.3f} | {r['macro_micro_gap']:.3f}"
...
def _deadline_sensitivity_md(env, scope) -> str:
    ...
    L = [
        f"# Deadline-sensitivity of the requirement envelope ({scope})\n",
    ...
    for w in env["workloads"]:
        d = by.get(w, {})
        L.append(f"| {w} | {d.get('K', '?')} | " + " | ".join(f"{d.get(dl, 0) / 1e9:.2f} GMAC/s" for dl in dls) + " |")
    L.append(
...
def _residency_tradeoff_md(env, scope) -> str:
    L = [
        f"# Residency vs deadline tradeoff ({scope})\n",
    ...
    for r in env["rows"]:
        ...
        L.append(
            f"| {w} | {r['K']} | {_fmt_bytes(nr)}/s | {_fmt_bytes(rs)}/s | {ratio:.0f}x | "
...
def _omitted_op_md(om, scope) -> str:
    L = [
        f"# Operator recovery accounting ({scope})\n",
    ...
    for r in om["rows"]:
        L.append(
            f"| {r['workload']} | {r['linear_gemm_ops']} | {r['linear_gemm_macs']} | "
...
def _erasure_evidence_md(ce, scope) -> str:
    L = [
        f"# Capture-erasure IR evidence ({scope})\n",
    ...
    for r in ce["rows"]:
        L.append(
            f"| {r['workload']} | {r['n_scf_loops']} | {r['n_linalg_matmul']} | "
...
def _per_family_md(pf, scope) -> str:
    L = [
        f"# Per-family decomposition ({scope})\n",
    ...
    for r in pf["rows"]:
        L.append(
            f"| {r['family']} | {r['n_workloads']} | {r['linear_gemm_macs']} | "
...
def _ablation_md(ab, scope) -> str:
    L = [
        f"# Capture-level ablation ({scope})\n",
    ...
    for r in ab["rows"]:
        L.append(
            f"| {r['workload']} | {r['level']} | {r['available']} | {r['linalg_matmul']} | "
...
def _p17_findings_md(bundle, scope) -> str:
    ...
    L = [
        f"# P17 final report — what is robust, what is blocked ({scope})\n",
...
def emit_run(bundle: dict, run_dir, rendered: list[str]) -> None:
    ...
    yaml_artifact(
        "unified_fact_table.yaml",
        {"unified_fact_table": {"scope": scope, "facts": facts}},
        header=f"unified_fact_table: {scope} (no performance claim)",
    ).write(run_dir)
    s = bundle["evidence_strength"]
    Artifact(
        "evidence_strength_table.csv",
        _csv_text([{"metric": k, "value": v} for k, v in s.items() if not isinstance(v, dict)], ["metric", "value"]),
    ).write(run_dir)
    ...
    yaml_artifact(
        "dse_usefulness_answers.yaml",
        {"dse_usefulness_answers": {"scope": scope, "answers": bundle["usefulness"]}},
        header=f"dse_usefulness_answers: {scope}",
    ).write(run_dir)
    Artifact("presentation_candidate_findings.md", _findings_md(bundle["findings"], scope)).write(run_dir)
    Artifact(
        "presentation_candidate_findings.csv",
        _csv_text(
            [
                {
                    **f,
                    "supporting_artifacts": "; ".join(f["supporting_artifacts"]),
    ...
    Artifact(
        "gap_audit_report.csv",
        _csv_text(
            gaps, ["gap_id", "scope", "category", "target", "description", "avoidable", "status", "required_input"]
        ),
    ).write(run_dir)
    Artifact("gap_audit_report.md", _gap_audit_md(gaps, oa, scope)).write(run_dir)
    ri = bundle.get("required_inputs", [])
    yaml_artifact(
        "required_inputs_manifest.yaml",
    ...
    yaml_artifact(
        "plot_manifest.yaml",
        {"plot_manifest": {"scope": scope, "plots": bundle["plots"], "rendered": rendered}},
        header=f"plot_manifest: {scope}",
    ).write(run_dir)
    ...
    Artifact("canonical_signal_table.csv", _csv_text(bundle["canonical_signal"], _CANON_COLS)).write(run_dir)
    h = bundle["hotspots"]
    ...
    if bundle["abstraction_coverage"]:
        Artifact("abstraction_coverage_table.csv", _csv_text(bundle["abstraction_coverage"], _COVER_COLS)).write(
            run_dir
        )
        Artifact("abstraction_coverage.md", _coverage_md(bundle["abstraction_coverage"], scope)).write(run_dir)
    ...
    Artifact("abstraction_necessity_table.csv", _csv_text(nec["rows"], _necessity_cols(nec["workloads"]))).write(
        run_dir
    )
    Artifact("abstraction_necessity.md", _necessity_md(nec, scope)).write(run_dir)
    Artifact(
        "primitive_set_frontier.csv",
        _csv_text(
            _frontier_rows(bundle["primitive_frontier"]),
            ["set_size", "primitive_set", "worst_workload_coverage", "macro_coverage", "micro_coverage", "max_regret"],
        ),
    ).write(run_dir)
    Artifact("primitive_set_frontier.md", _frontier_md(bundle["primitive_frontier"], scope)).write(run_dir)
    ...
    Artifact("capture_fidelity_matrix.csv", _csv_text(cf["matrix"], ["feature"] + cf["workloads"])).write(run_dir)
    Artifact("capture_fidelity_matrix.md", _capture_fidelity_md(cf, scope)).write(run_dir)
    Artifact("decision_question_scorecard.md", _scorecard16_md(bundle["decision_scorecard"], scope)).write(run_dir)
    ...
    if fro.get("rows"):
        ...
        Artifact("uncovered_ops_by_primitive_set.csv", _csv_text(fro["uncovered_rows"], _UNCOV_COLS)).write(run_dir)
    inf = bundle.get("macro_micro_influence", {})
    if inf.get("rows"):
        Artifact("macro_micro_influence_table.csv", _csv_text(inf["rows"], _INFLUENCE_COLS)).write(run_dir)
        Artifact("leave_one_out_delta_table.csv", _csv_text(inf["loo_rows"], _LOO_DELTA_COLS)).write(run_dir)
    ...
    if env.get("rows"):
        ...
        Artifact("residency_vs_deadline_tradeoff.md", _residency_tradeoff_md(env, scope)).write(run_dir)
    ...
    Artifact("DSE_FINDINGS.md", _findings_digest_md(bundle, bundle.get("cs_dir", run_dir), rendered)).write(run_dir)
    Artifact("presentation_plots_index.md", _plots_index_md(bundle["plots"], scope, rendered, "generated_plots")).write(
        run_dir
    )
