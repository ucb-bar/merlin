...
_CAP_KEY = {"int8_w8a8": "int8", "fp8_w8a8": "fp8", "int4_weight_only": "int4", "int4_weight_fp8_activation": "int4"}
...
_COMPILER_PROOFS = {
    ...
    "native_lowbit_compute": [
        "low-bit operands legal for the matmul",
        "accumulator width (i32/fp32) sufficient",
        "requant placement known",
        "scale metadata preserved",
    ],
}
...
def _status_for(workload: str, fmt: str) -> tuple[str, str | None, str]:
    ...
    if s == "pass":
        return "measured_pass", "docs/results.md (W8A8 accuracy table)", "accuracy_legal_structural_candidate"
    if s == "fail":
        ...
        for fmt in _FORMAT_LADDER:
            ...
            out.append(
                DtypeCertificate(
                    ...
                    required_hw_abstractions=abstraction.get(
                        "system_abstraction", getattr(cand, "required_hw_support", "")
                    ),
    ...
    return {
        "numerical_candidate_certificates": {
            ...
            "certificates": [
                {
                    "region": c.region,
                    "candidate": c.candidate_axis,
                    "dtype": c.dtype,
                    "accuracy": {
                        "status": c.accuracy_status,
                        "source": c.accuracy_source,
                        "evidence": (E_MEASURED if c.accuracy_status.startswith("measured") else E_NA),
                    },
                    "resident_capacity_at_format": {
                        "bytes": c.resident_capacity_at_format_B,
                        "evidence": (E_DERIVED if c.resident_capacity_at_format_B is not None else E_NA),
                    },
            ...
            rows.append(
                {
                    "workload": c.workload,
                    "region": c.region,
                    "candidate_axis": c.candidate_axis,
                    "dtype": c.dtype,
                    "accuracy_status": c.accuracy_status,
                    "dse_status": c.dse_status,
                    "resident_capacity_at_format_B": (
                        round(c.resident_capacity_at_format_B)
