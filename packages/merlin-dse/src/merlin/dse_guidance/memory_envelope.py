...
from merlin.dse_guidance.design_envelope import CAPACITY_FORMATS, E_ASSUMED, E_DERIVED, E_IR, E_NA, ELEMENT_BYTES
            ...
            "regions": [
                {
                    ...
                    "weight_traffic_if_nonresident": {"value": r.weight_traffic_if_nonresident, "evidence": E_DERIVED},
                    "avoidable_weight_reload": {"value": r.avoidable_weight_reload, "evidence": E_DERIVED},
                    "activation_bytes_per_invocation": {"value": r.activation_bytes_per_invocation, "evidence": E_IR},
...
def traffic_csv(packages) -> str:
    ...
    return _csv(
        ...
        [
            ...
            "invocations",
            "weight_bytes",
            "weight_traffic_if_nonresident",
            "avoidable_weight_reload",
            "activation_bytes_per_invocation",
            "activation_traffic_total",
            "reuse_factor",
        ],
    )
    ...
    for role, ss in by_role.items():
        ...
        out.append(
            RegionMemory(
                ...
                invocations=inv,
                weight_bytes=weight,
                activation_input_bytes=act_in,
                output_bytes=outp,
                intermediate_bytes=None,
                scale_bytes=None,
                kv_bytes=None,
                read_bytes_resident=read_res,
                write_bytes=write,
                read_write_ratio=(round(read_res / write, 3) if write else None),
                reuse_factor=inv,
                avoidable_weight_reload=weight * max(inv - 1, 0),
...
def reuse_lifetime(region_mem: list[RegionMemory]) -> list[ReuseRecord]:
    ...
    for rm in region_mem:
        ...
        out.append(
            ReuseRecord(
                object=f"{rm.region}.weights",
                producer_region=rm.region,
                consumer_regions=[rm.region],
                lifetime=across,
                reload_behavior=(RB_LOAD_ONCE_K if rm.invocations > 1 else RB_RESIDENT),
                required_abstraction="resident_weight_object",
                bytes=rm.weight_bytes,
                evidence=E_IR,
            )
        )
        out.append(
            ReuseRecord(
                object=f"{rm.region}.activations",
                producer_region=rm.region,
                consumer_regions=[rm.region],
                lifetime=LT_WITHIN_REGION,
                reload_behavior=RB_RELOAD_EACH,
                required_abstraction="resident_activation_object",
                bytes=rm.activation_input_bytes,
                evidence=E_IR,
            )
        )
    return out
                    ...
                    "regions": [
                        {
                            ...
                            "avoidable_weight_reload": {"value": rm.avoidable_weight_reload, "evidence": E_DERIVED},
                            "resident_by_dtype": rm.resident_by_dtype,
                            "lifetime": rm.lifetime,
                        }
                        for rm in mems
                    ],
                }
            ...
            rows.append(
                {
                    ...
                    "region": rm.region,
                    "invocations": rm.invocations,
                    "weight_bytes": rm.weight_bytes,
                    "activation_input_bytes": rm.activation_input_bytes,
                    "output_bytes": rm.output_bytes,
                    "intermediate_bytes": "unavailable",
                    "scale_bytes": "unavailable",
                    "kv_bytes": "unavailable",
                    "read_bytes_resident": rm.read_bytes_resident,
                    "write_bytes": rm.write_bytes,
                    "read_write_ratio": rm.read_write_ratio if rm.read_write_ratio is not None else "unavailable",
                    "avoidable_weight_reload": rm.avoidable_weight_reload,
                    "resident_int8_B": rm.resident_by_dtype.get("int8"),
                    "resident_bf16_B": rm.resident_by_dtype.get("bf16"),
                    "lifetime": rm.lifetime,
                }
            )
    return _csv(
            ...
            rows.append(
                {
                    "workload": wl,
                    "object": r.object,
                    "producer_region": r.producer_region,
                    "consumer_regions": "; ".join(r.consumer_regions),
                    "lifetime": r.lifetime,
                    "reload_behavior": r.reload_behavior,
                    "required_abstraction": r.required_abstraction,
                    "bytes": r.bytes,
                    "evidence": r.evidence,
                }
            )
    ...
    return {
        "memory_abstraction_candidates": {
            ...
            "candidates": [
                {
                    "abstraction": a,
                    "implied_by_n_objects": n,
                    "evidence": E_IR if a in ("resident_weight_object", "resident_activation_object") else E_NA,
                }
...
def memory_envelope_report_md(mem_by_workload: dict) -> str:
    ...
    for wl, rm in allregs[:8]:
        dom = max(
            (
                ("weight", rm.weight_bytes),
                ("activation", rm.activation_input_bytes * rm.invocations),
                ("output", rm.output_bytes * rm.invocations),
            ),
            key=lambda kv: kv[1],
        )[0]
        L.append(
            f"| {wl} | {rm.region} | {rm.weight_bytes:,} | {rm.activation_input_bytes:,} | "
    ...
    L.append("## Findings\n")
    L.append(
        f"- **Memory pressure is {dom}-dominated** across the recaptured workloads "
        f"(weights {tot_w:,} B, activations {tot_a:,} B, outputs {tot_o:,} B)."
    )
    top = allregs[0]
    L.append(
        f"- **Top avoidable-reload candidate:** `{top[0]}/{top[1].region}` — "
        f"{top[1].avoidable_weight_reload:,} B avoidable if weights are made resident "
        f"(= weight_bytes × (reuse − 1))."
    )
    L.append(
        "- **Repeatedly implied abstractions:** `resident_weight_object` (weights reused "
    ...
    L.append(
        "- intermediate-materialization, scale/zero-point, and KV/prefix bytes (`unavailable` "
        "in a flat dequantized capture);"
    )
    L.append(
        "- a target memory hierarchy (capacities, bandwidths) — supplied via a design YAML, "
