...
from merlin.dse_guidance import aet_ingest, loader, synth
...
from merlin.dse_guidance.study import discover_model_specs, discover_specs, run_model_study, run_study
...
def _single(args) -> int:
    ...
    if args.baseline_cost:
        ...
        print(
            "note: no baseline cost — emitting structural outputs only (topology, capture "
    ...
    out = Path(args.out) if args.out else (paths.artifacts_dir() / "dse-guidance" / wl)
    write_artifacts(result, out)

    for wmsg in result.warnings:
        print(f"warning: {wmsg}")
    print(
        f"workload={wl}  class={result.topology.workload_class}  capture-fidelity severity={result.fidelity.severity}"
    )
...
def _top_names(tr: dict, n: int = 3) -> list[str]:
    return [r["axis"] for r in tr["axes"] if r["priority_score"] and r["legality"]][:n]
...
def _design_envelope(args) -> int:
    ...
    from merlin.dse_guidance.corpus import _recap_dir, available_models

    design = load_yaml(args.design_candidate) if args.design_candidate else None
    out = Path(args.out) if args.out else (paths.artifacts_dir() / "dse-guidance" / "design_envelope")
    ...
    for env in envs:
        yaml_artifact(
            f"{env.workload}/design_envelope.yaml", DE.to_yaml_obj(env), header=f"design_envelope: {env.workload}"
        ).write(out)
    ...
    print(
        f"design envelopes (requirements, not calibration) for {len(envs)} workload(s); "
...
def _study(args) -> int:
    ...
    out = Path(args.out) if args.out else (paths.artifacts_dir() / "dse-guidance" / default_sub)
    summary = run_model_study(out) if args.models else run_study(specs, out)
    print(f"studied {summary['n_workloads']} workloads ({kind}): {', '.join(summary['workloads'])}")
...
def _insight_mining(args) -> int:
    ...
    cs_dir = case_study_dir(args.case_study_dir)
    if not (cs_dir / "dse_contract.json").is_file() and not (cs_dir / "critical_path_table.csv").is_file():
        raise SystemExit(f"no case-study artifacts under {cs_dir}; select input with --case-study-dir")
    ...
    for scope in scopes:
        ...
        rendered = PP.render_plots(bundle["plots"], cs_dir, bundle["facts"], run_dir / "generated_plots")
        ...
        print(
            f"               facts={s['total_facts']} tiers(A/B/C/D)="
...
def _devils_advocate(args) -> int:
    ...
    if not run_dir.is_dir() or not (run_dir / "DSE_FINDINGS.md").is_file():
        raise SystemExit(f"{run_dir} is not an insight-mining run (no DSE_FINDINGS.md); run --insight-mining first")
    try:
    ...
    print(
        f"critic: {len(result['accepted'])} grounded over-claims kept / "
...
def _query(args) -> int:
    ...
    # --out remains a backwards-compatible input alias in query mode only.
    base = case_study_dir(args.case_study_dir or args.out)
    manifest_path = base / "dse_contract.json"
    if not manifest_path.is_file():
        raise SystemExit(f"no dse_contract.json under {base}; select input with --case-study-dir")
    ...
    if topic == "summary":
        ...
        for w, d in sorted(m["per_workload"].items()):
            print(
                f"  {w}: class={d['class']} K={d['K']} roles={d['roles']} "
        ...
        for g in m["search_space_knob_groups"]:
            print(f"  [{g['source_phase']:6s}] {g['group']:28s} enabled={g['enabled']} n_knobs={g['n_knobs']}")
    elif topic == "boundary":
        bp = m["boundary_placement"]
        if arg:
            from merlin.common.yaml import load_yaml

            certs = load_yaml(base / "boundary_candidate_contracts.yaml")["boundary_candidate_contracts"][
                "certificates"
            ]
            ...
            print(
                f"{c['abstraction']}  (pressure/evidence={c['boundary_pressure_score']}, "
                f"workloads={c['supporting_workloads']})"
            )
            print(f"  compiler proof: {c['required_compiler_proof']} [{c['compiler_proof_status']}]")
            ...
            for t in bp["top_by_evidence_breadth"]:
                print(f"  {t['abstraction']:28s} score={t['boundary_pressure_score']} strong@={t['strong_levels']}")
        ...
        raise SystemExit("query must be one of: summary | knobs | boundary[:<abstraction>] | missing | index")
    return 0


def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(prog="merlin-dse-guidance", description=__doc__)
    ap.add_argument("--workload", default=None, help="synthetic name or semantic_memory benchmark name")
    ap.add_argument("--region-yaml", default=None, help="explicit workload_region YAML path")
    ap.add_argument(
        "--temporal-metadata", default=None, help="temporal_workload_metadata YAML (else synthesized from region)"
    )
    ap.add_argument(
        "--baseline-cost", default=None, help="baseline_cost YAML (else analytical baseline synthesized from region)"
    )
    ap.add_argument("--cpu-coupling", default=None, help="cpu_coupling measurements YAML")
    ap.add_argument("--aet-run", default=None, help="aet run dir with metrics/summary_metrics.json (measured coupling)")
    ap.add_argument("--H", type=int, default=16, help="action horizon (synthetic region loader)")
    ap.add_argument(
        "--control-rate-hz", type=float, default=30.0, help="control rate for synthesized temporal metadata"
    )
    ap.add_argument("--study", action="store_true", help="run the exhaustive cross-workload study")
    ap.add_argument(
        "--models",
        action="store_true",
        help="with --study: study the real model zoo (captures under output/) instead of the semantic_memory regions",
    )
    ap.add_argument(
        "--design-envelope",
        action="store_true",
    ...
    ap.add_argument("--capture-dir", default=None, help="a dir with model.mlir to attribute (for --design-envelope)")
    ap.add_argument("--design-candidate", default=None, help="optional candidate-design YAML for roofline feasibility")
    ap.add_argument(
        "--case-study",
        action="store_true",
    ...
    ap.add_argument(
        "--case-study-dir",
        default=None,
        help="case-study input for mining/query (default: committed experiments/reference-data/dse/case_study)",
    )
    ap.add_argument(
        "--query",
        default=None,
        ...
        "--case-study-dir, legacy --out, or the committed reference dir)",
    )
    ap.add_argument(
        "--devils-advocate",
        default=None,
    ...
    if args.case_study:
        ...
        out = Path(args.out) if args.out else (paths.artifacts_dir() / "dse-guidance" / "case_study")
        summary = run_case_study(out)
        print(
            f"cross-workload case study over {len(summary['workloads'])} real captures: "
