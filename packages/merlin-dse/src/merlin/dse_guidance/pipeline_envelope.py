...
ALLOWED_ABSTRACTIONS = {
    ...
    "double_buffered_action_chunk",
    "prefix_state_object",
    "loop_carried_state_handle",
...
def phase_model(graph, shapes) -> list[Phase]:
    ...
    for n in graph.nodes:
        ...
        phases.append(
            Phase(
                workload=wl,
                phase_id=n.id.split(":")[-1],
                phase_class=_phase_class(role, graph.workload_class),
                role=role,
                cadence=rate.get("cadence", "unknown"),
                invocations=rate.get("invocations"),
                period_s=rate.get("period_s"),
...
def overlap_candidates(graph, phases, has_control_loop: bool = False) -> list[OverlapCandidate]:
    ...
    out.append(
        OverlapCandidate(
            source_phase="backbone(next replan)",
            target_phase="action_execution(current chunk)",
            dependency_type="cross_replan_no_shared_state",
            can_overlap=("yes" if has_backbone_compute else "unknown"),
            why=(
                "backbone of replan n+1 shares no loop-carried state with replan n's head, so the rates decouple"
                if has_backbone_compute
                else "no backbone compute is recovered in this capture (the capture is the head only) — "
                "the overlap target is not present"
            ),
            required_abstractions=["double_buffered_action_chunk", "async_queue", "event_token", "prefix_state_object"],
            required_buffer_count=(2 if has_backbone_compute else "unavailable"),
            event_queue_support=["async_queue", "event_token"],
            missing_evidence=(
                "per-phase timing (to size the overlap)"
                if has_backbone_compute
                else "backbone compute is not in this capture"
            ),
    ...
    out.append(
        OverlapCandidate(
            source_phase="dma_prefetch(resident weights)",
            target_phase=head_id,
            dependency_type="read_only_resident_state",
            can_overlap="yes",
            why="loop-invariant weights are read-only across the K-loop, so the next "
            "tile can be prefetched while the current step computes",
            required_abstractions=["resident_weight_object", "async_queue"],
            required_buffer_count=2,
            event_queue_support=["async_queue"],
            missing_evidence="DRAM bandwidth (to size prefetch depth)",
            evidence=E_FQN,
        )
    )
    ...
    out.append(
        OverlapCandidate(
            source_phase=head_id,
            target_phase=head_id,
            dependency_type="bounded_loop",
            can_overlap="yes",
            ...
            required_abstractions=["bounded_loop_command", "loop_carried_state_handle", "resident_weight_object"],
            required_buffer_count=1,
            event_queue_support=["region_level_dispatch"],
    ...
    out.append(
        OverlapCandidate(
            source_phase="control_tick_consumer",
            target_phase="replan_inference(next)",
            dependency_type="different_cadence_producer_consumer",
            can_overlap=("yes" if has_control_loop else "unknown"),
            why=(
                "the control loop consumes H actions at the control rate while the next replan "
                "computes — different cadences, no data dependency"
                if has_control_loop
                else "this workload has no real-time control loop (not a VLA action head) — no control tick to decouple"
            ),
            required_abstractions=["double_buffered_action_chunk", "producer_consumer_queue", "event_token"],
            required_buffer_count=(2 if has_control_loop else "unavailable"),
            event_queue_support=["producer_consumer_queue", "event_token"],
            missing_evidence=(
                "the control-tick consumer is a runtime phase not in the model-forward "
                "capture; its timing is unavailable"
                if has_control_loop
                else "no control loop in this workload's architecture"
            ),
    ...
    out.append(
        OverlapCandidate(
            source_phase="decode_token_step",
            target_phase="kv_cache_movement",
            dependency_type="kv_dependency",
            can_overlap="unknown",
            ...
            required_buffer_count="unavailable",
            event_queue_support=[],
                    ...
                    "phases": [
                        {
                            ...
                            "state_inputs": p.state_inputs,
                            "state_outputs": p.state_outputs,
                            "candidate_resource_class": p.resource_class,
                            "evidence": p.evidence,
                            "missing": p.missing,
                        }
                        for p in phases
                    ...
                    "candidates": [
                        {
                            ...
                            "why": c.why,
                            "required_abstractions": c.required_abstractions,
            ...
            rows.append(
                {
                    ...
                    "required_buffer_count": (
                        c.required_buffer_count if c.required_buffer_count != "unavailable" else "unavailable"
                    ),
...
def overlap_report_md(phase_by_workload: dict, overlap_by_workload: dict) -> str:
    ...
    for c in any_cands:
        L.append(
            f"| {c.source_phase} ‖ {c.target_phase} | {c.can_overlap} | "
    ...
    L.append("\n## Findings\n")
    L.append(
        "- **Backbone/head and control/inference decouple by cadence** — the once-per-replan "
    ...
    L.append(
        "- **The K-loop is representable as a bounded device-side loop** (loop-invariant "
        "weights + bounded trip count) — `requires bounded_loop_command`."
    )
    L.append(
        "- **`double_buffered_action_chunk` / `async_queue` recur** as the abstractions overlap "
        "needs — `requires event/queue abstraction`."
    )
    L.append(
        "- **Blocked by missing timing/dependency evidence:** per-phase wall-clock periods, "
