...
ALLOWED_DMA_ABSTRACTIONS = {
    "multi_stream_dma_descriptor",
    "prefetch_descriptor",
    "prefetch_weight_once",
    "double_buffered_activation_tile",
    "scale_sideband_stream",
    "output_commit_stream",
    "kv_stream_handle",
    "packed_weight_store",
    "activation_ring_buffer",
}
...
def region_streams(rm) -> list[DmaStream]:
    ...
    return [
        DmaStream(
            ...
            rm.reuse_factor,
            prefetchable="yes",
            can_overlap_compute="yes",
            sideband=False,
            candidate_abstraction="prefetch_weight_once",
            evidence=E_IR,
        ),
        DmaStream(
            rm.region,
            "activation_input",
            rm.activation_input_bytes,
            "read",
            1,
            prefetchable="unknown",
            can_overlap_compute="yes",
            sideband=False,
            candidate_abstraction="double_buffered_activation_tile",
            evidence=E_IR,
        ),
        DmaStream(
            rm.region,
            ...
            can_overlap_compute="yes",
            sideband=False,
            candidate_abstraction="output_commit_stream",
            evidence=E_IR,
        ),
        DmaStream(
            rm.region,
            ...
            "unavailable",
            prefetchable="unknown",
            can_overlap_compute="unknown",
            sideband=True,
            candidate_abstraction="scale_sideband_stream",
            evidence=E_NA,
        ),
        DmaStream(
            rm.region,
            ...
            can_overlap_compute="unknown",
            sideband=False,
            candidate_abstraction="kv_stream_handle",
            evidence=E_NA,
        ),
        DmaStream(
            rm.region,
            "intermediate_writeback",
            "unavailable",
            "unavailable",
            "unavailable",
            prefetchable="unknown",
            can_overlap_compute="unknown",
            sideband=False,
            candidate_abstraction="activation_ring_buffer",
            evidence=E_NA,
        ),
        DmaStream(
            rm.region,
            ...
            can_overlap_compute="unknown",
            sideband=True,
            candidate_abstraction="multi_stream_dma_descriptor",
            evidence=E_NA,
        ),
    ]
    ...
    for rm in region_mem:
        ...
        out.append(
            BufferRequirement(
                ...
                input_buffer_bytes=rm.activation_input_bytes,
                output_buffer_bytes=rm.output_bytes,
            ...
            rows.append(
                {
                    ...
                    "can_overlap_compute": s.can_overlap_compute,
                    "sideband": s.sideband,
                    "candidate_abstraction": s.candidate_abstraction,
                    "evidence": s.evidence,
                }
            )
    return _csv(
            ...
            rows.append(
                {
                    ...
                    "buffer_bytes_bf16": b.buffer_bytes_by_dtype.get("bf16"),
                    "evidence": b.evidence,
                }
            )
    return _csv(
        ...
        [
            "workload",
            "region",
            "min_input_buffer_count",
            "min_output_buffer_count",
            "double_buffering_needed",
            "producer_consumer_queue_candidate",
            "input_buffer_bytes",
            "output_buffer_bytes",
...
def dma_pressure_report_md(stream_by_workload: dict, region_mem_by_workload: dict) -> str:
    ...
    for s in seen.values():
        ...
        L.append(
            f"| {s.stream} | {known} | {s.direction} | {s.candidate_abstraction} | "
    ...
    L.append("## Findings\n")
    L.append(
        "- **Three byte-carrying streams per region** (weight read, activation read, output "
        "write) structurally suggest a `multi_stream_dma_descriptor` with independent channels."
    )
    L.append(
        "- **The weight stream is prefetchable and reused** (`prefetch_weight_once`); the "
        "activation stream structurally suggests a `double_buffered_activation_tile`."
    )
    L.append(
        "- **Scale-sideband, KV, and intermediate streams are `unavailable`** — the capture is "
    ...
    L.append(
        "- per-stream bandwidth and a target memory hierarchy (a design YAML) — absent here; "
