...
def read_tool_spans(stage_dir: Path) -> tuple[SpanSet, int]:
    ...
    if out.spans:
        ...
        # by `merlin_experiments.phase2.telemetry.finalize`, which is called by the measured-claim
        # authoring stage; the global launcher never calls it. The raw driver stream IS on disk at
        ...
        out.availability.set(
            ...
            unavailable(
                ...
                + (
                    ...
                    f"not tool spans, and phase2.telemetry.finalize (which writes the spans) is called "
                    f"only from the measured-claim authoring stage"
                    if raw
                    else ""
                )
            ),
