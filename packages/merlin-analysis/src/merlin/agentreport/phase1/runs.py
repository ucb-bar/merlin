...
# job is to be exact about what was and was not measured.
from merlin.targetgen.capsule_common import NOT_MEASURED_STATUSES


@dataclass(frozen=True)
class ReportInputs:
    """Explicit target and artifact roots; constructing a reader discovers nothing."""
...
def add_input_arguments(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--target", help="target identity for default output layout and report labels")
    parser.add_argument("--descriptor", type=Path, help="explicit target descriptor (alternative to --target)")
    parser.add_argument("--runs-root", type=Path, help="capsule-bench root containing arm directories")
    parser.add_argument("--reports-root", type=Path, help="input audit and default report output root")


def resolve_inputs(parser, options, defaults: ReportInputs | None = None) -> ReportInputs:
    from merlin.common.paths import artifacts_dir, runs_root

    target = options.target
    if options.descriptor is not None:
        from merlin.targetgen.target_experiment import load_target_experiment

        declared = load_target_experiment(options.descriptor).target
        if target and target != declared:
            parser.error("--target disagrees with --descriptor")
        target = declared
    if not target:
        if defaults is None:
            parser.error("provide --target or --descriptor")
        target = defaults.target
    if target in {".", ".."} or Path(target).name != target or "\\" in target:
        parser.error("target must be a single path component")
    # Native defaults apply only to the same target; explicit target selection
    # must not accidentally read another target's cached native roots.
    selected = defaults if defaults is not None and defaults.target == target else None
    return ReportInputs(
        target,
        options.runs_root or (selected.runs_root if selected else runs_root(target, "capsule-bench")),
        options.reports_root or (selected.reports_root if selected else artifacts_dir() / "capsule-bench" / target),
    )
...
def collect_comparable_runs(select: tuple[str, ...] = (), *, inputs: ReportInputs) -> tuple[dict, dict]:
...
def cohort_main(select: tuple[str, ...], *, inputs: ReportInputs) -> int:
    ...
    runs, skipped = collect_comparable_runs(select, inputs=inputs)
    if not runs:
...
def main(argv: list[str] | None = None, *, defaults: ReportInputs | None = None):
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    add_input_arguments(parser)
    parser.add_argument("--cohort", nargs="*", help="read-only common-cohort comparison, filtered by substrings")
    options = parser.parse_args(argv)
    inputs = resolve_inputs(parser, options, defaults)
    ...
    if options.cohort is not None:
        toks = tuple(t for t in ",".join(options.cohort).split(",") if t)
        return cohort_main(toks, inputs=inputs)
    ...
    inputs.reports_root.mkdir(parents=True, exist_ok=True)
    p = inputs.reports_root / "agentic_results.json"
