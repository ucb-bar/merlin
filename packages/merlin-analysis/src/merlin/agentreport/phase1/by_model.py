...
def collect(tag: str | None, arm_filter: str | None, *, inputs: AAR.ReportInputs) -> list[dict]:
    """One record per run directory under the active target's capsule-bench run root."""
    audit_p = inputs.reports_root / "full_suite_audit.json"
...
def markdown(models: dict, rows: list[dict], arm: str | None, *, target: str) -> str:
    L = [f"# capsule-bench by model - target `{target}`" + (f", arm `{arm}`" if arm else ""), ""]
    L += [
...
def by_cell(rows: list[dict], *, runs_root: Path) -> dict:
    ...
    for r in rows:
        ...
        c["bridged"] = c["bridged"] if c["bridged"] is not None else _bridge_of(r["run_id"], runs_root=runs_root)
    return out


def _bridge_of(run_id: str, *, runs_root: Path) -> bool | None:
    """Read the run's bridge.json, written whenever a (model, harness) pairing needed the proxy."""
    for sub in AAR.RUN_DIRS:
        p = runs_root / sub / run_id / "bridge.json"
        if p.is_file():
            try:
...
def main(argv=None, *, defaults: AAR.ReportInputs | None = None) -> int:
    ...
    rows = collect(a.tag, a.arm, inputs=inputs)
    models = by_model(rows)
    cells = by_cell(rows, runs_root=inputs.runs_root)
    ...
    md = markdown(models, rows, a.arm, target=inputs.target) + "\n" + matrix_markdown(cells)
