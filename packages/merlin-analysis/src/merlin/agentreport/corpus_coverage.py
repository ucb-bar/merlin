...
* **roster** -- the explicitly supplied declared corpus files. This is the
...
def read_roster(
    ...
    roster, reason = read_roster_paths((profile_dir / f"{target}{suffix}.yaml", lane) for suffix, lane in lanes.items())
    return roster, f"no roster file for {target!r} under {profile_dir}" if reason else ""


def read_roster_paths(paths: Iterable[tuple[Path, str]]) -> tuple[dict[str, str], str]:
    """Read explicitly ordered roster files, without discovering siblings or current definitions.

    Paths may describe current declarations or historical snapshots; that choice belongs to the
    caller. First declaration wins. Sweeps are not expanded into a second, invented denominator.
    """
    import yaml
    ...
    if not out:
        return {}, "no capsule roster could be read from the explicitly supplied paths"
    return out, ""
