...
def collect_detection(
    *, m: int = 4, k: int = 4, n: int = 4, reuse: int = 2, timeout_ms: int = 60_000
) -> dict[str, Any]:
...
def collect_scaling(
    shapes: tuple[tuple[int, int, int], ...] = DEFAULT_SCALING_SHAPES,
    *,
    reuse: int = 2,
...
def _caption(fig, text: str, *, y: float = -0.02, width: int = 150) -> None:
    fig.text(0.5, y, textwrap.fill(text, width), ha="center", va="top", fontsize=8.6, color=paper.INK)
...
def fig_detection_matrix(record: dict[str, Any]):
    ...
    paper.card(
        ax,
        f"Which layer catches which defect  "
        f"(shape {record['shape']['m']}x{record['shape']['k']}x{record['shape']['n']}, "
        f"reuse {record['shape']['reuse']}" + (f", solver bound {_bound / 1000:.0f}s)" if _bound else ")"),
    )
    ...
    for row, fault in enumerate(faults):
        ...
        for col, layer in enumerate(columns):
            ...
            if hit is None:
                ...
                face, hatch, label = (ABSTAINED, ABSTAINED_HATCH, f"ABSTAIN\n{_fmt_seconds(hit['seconds'])}")
            else:
                face, hatch, label = MISSED, None, f"miss\n{_fmt_seconds(hit['seconds'])}"
            ax.add_patch(
                Rectangle(
                    (col, y), 1.0, 1.0, facecolor=face, edgecolor=paper.CARD_EC, linewidth=1.4, hatch=hatch, zorder=3
                )
            )
            if label:
                ...
                bbox = dict(boxstyle="round,pad=0.28", fc="white", ec=paper.CARD_EC, lw=1.0) if hatch else None
                ax.text(
                    col + 0.5,
                    y + 0.5,
                    label,
    ...
    ax.set_xticklabels(
        [c if c in layers else f"{c.upper()}\n(not measured)" for c in columns], fontsize=10.5, fontweight="bold"
    )
    ...
    handles = [
        Patch(facecolor=DETECTED, edgecolor=paper.CARD_EC, label="detected"),
        Patch(facecolor=MISSED, edgecolor=paper.CARD_EC, label="missed"),
    ]
    if any(c.get("outcome") == "abstained" for c in cells.values()):
        handles.append(
            Patch(facecolor=ABSTAINED, edgecolor=paper.CARD_EC, hatch=ABSTAINED_HATCH, label="abstained (no verdict)")
        )
    handles.append(Patch(facecolor=UNMEASURED, edgecolor=paper.CARD_EC, hatch=UNMEASURED_HATCH, label="not measured"))
    ax.legend(
        handles=handles, loc="lower right", bbox_to_anchor=(1.0, 0.0), ncol=len(handles), fontsize=9.5, frameon=False
    )

    fp = [d for d in record.get("false_positives", []) if d["detected"]]
    fp_text = (
        f"{len(fp)} layer(s) FLAGGED the unmutated program -- the matrix above is not usable"
        if fp
        else "no layer flagged the unmutated program, so every DETECTED cell above is a true positive"
    )
    unmeasured_text = "  ".join(f"{name.upper()}: {why}" for name, why in unmeasured.items())
    bound = record.get("timeout_ms")
    bound_text = (
        f" Solver bound {bound / 1000:.0f} s per formal attempt."
        if bound
        else " Solver bound NOT RECORDED (pre-v2 record): abstentions are indistinguishable from misses in this data."
    )
    ab = [c for c in cells.values() if c.get("outcome") == "abstained"]
    ab_text = (
        f" {len(ab)} attempt(s) ABSTAINED (no verdict within the bound); an abstention is not "
    ...
    _caption(
        fig,
        f"Figure F1: {len(faults)} seeded faults x {len(layers)} measured layers, "
...
def fig_cost_to_detect(record: dict[str, Any]):
    ...
    for layer in layers:
        ...
        abst = sum(1 for (_, l), d in cells.items() if l == layer and d.get("outcome") == "abstained")
        stats.append(
            {
                "layer": layer,
    ...
    paper.card(
        ax,
        f"What each layer charges to catch what it catches  "
        f"(shape {_sh['m']}x{_sh['k']}x{_sh['n']}" + (f", solver bound {_b / 1000:.0f}s)" if _b else ")"),
    )
    ...
    for yi, row in zip(y, rows):
        if row["mean"] is None:
            ax.barh(
                ...
                height=0.6,
                color=UNMEASURED,
                edgecolor=paper.CARD_EC,
                linewidth=1.5,
                hatch=UNMEASURED_HATCH,
                zorder=3,
            )
            ax.text(
                float(np.sqrt(left * right)),
                yi,
                "NOT MEASURED -- " + row["why"],
                va="center",
                ha="center",
                fontsize=9.4,
                fontweight="bold",
    ...
    ax.set_yticks(y)
    ax.set_yticklabels([r["layer"] if r["mean"] is not None else r["layer"].upper() for r in rows], fontsize=11)
    ...
    _abs_note = (
        f" {_abs_total} attempt(s) ABSTAINED within the bound and are included in the mean "
    ...
    _caption(
        fig,
        f"Figure F2: mean wall cost per detection attempt across the seeded fault corpus "
...
def fig_obligation_coverage(record: dict[str, Any]):
    ...
    ax.bar(
        ...
        linewidth=1.5,
        zorder=3,
        label="checked by the static suite",
    )
    ax.bar(
        x,
        omitted,
        ...
        hatch="//",
        zorder=3,
        label="omitted, reason recorded",
    )
    for xi, (c, o) in zip(x, zip(checked, omitted)):
        if c:
            ax.text(
                xi, c / 2, str(c), ha="center", va="center", fontsize=10.5, fontweight="bold", color=paper.INK, zorder=4
            )
        if o:
            ax.text(
                xi,
                ...
                color=paper.INK,
                zorder=4,
                bbox=dict(boxstyle="circle,pad=0.22", fc="white", ec=paper.CARD_EC, lw=0.8),
            )
        ax.text(xi, c + o + 0.08, f"{c + o} declared", ha="center", va="bottom", fontsize=9.0, color=paper.INK)
    ...
    paper.callout(
        ax,
        (x[0], checked[0] + omitted[0] + 0.12),
        f"{record['emitted_total']} of {record['declared_total']} declared obligations are checked.\n"
        + textwrap.fill(f"Before this layer the count was {base.get('emitted')}: {base.get('source', '')}", 62),
    ...
    _caption(
        fig,
        f"Figure F3: obligations come from each target's capability manifest, so the "
        f"denominator is derived, not chosen. Omission reasons -- {reasons}",
        y=-0.03,
    )
    fig.tight_layout()
    return fig
...
def fig_formal_scaling(record: dict[str, Any]):
    ...
    for i, tile in enumerate(tiles):
        ...
        pos = (
            (tile["product"] / 9.0, tile["seconds"] * 9.0)
            if i % 2 == 0
            else (tile["product"] * 1.9, tile["seconds"] * 0.20)
        )
        paper.callout(
            ax,
            (tile["product"], tile["seconds"]),
    ...
    _caption(
        fig,
        f"Figure F4: translation validation of the interface plane at {len(points)} "
...
def draw_all(
    records: dict[str, dict[str, Any]], out_dir: Path, *, formats: tuple[str, ...] = ("png", "svg")
) -> list[Path]:
...
def internal_note(records: dict[str, Any]) -> str:
    ...
    tile_lines = [
        f"  {pt['m']}x{pt['k']}x{pt['n']} ({', '.join(pt['mesh_tile_for'])}): {pt['status']} in {pt['seconds']:.2f} s"
        for pt in tiles
    ]
    ...
    return "\n".join(
        [
            ...
            f"**{bound} ms** per formal attempt"
            if bound
            else "- detection matrix: solver bound NOT RECORDED (pre-v2 record)",
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--from", dest="source", type=Path, help="a directory holding the three JSON records; skips re-measuring"
    )
