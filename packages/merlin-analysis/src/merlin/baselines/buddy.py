...
def native_import_available() -> bool:
    ...
    return (
        _torch_venv_python() is not None and buddy_python_packages() is not None and mlir_python_packages() is not None
    )


def native_import(model: str, variant: str, out_dir: Path, *, registry: str = "tosa", timeout: int = 1800) -> Path:
    ...
    if py is None or bpp is None or mpp is None:
        raise BuddyError(
            "native importer unavailable (need torch venv + buddy/MLIR python packages; "
    ...
    merlin_py = os.pathsep.join(str(path) for path in python_import_roots())
    env["PYTHONPATH"] = os.pathsep.join([str(bpp), str(mpp), merlin_py, env.get("PYTHONPATH", "")]).rstrip(os.pathsep)
    ...
    cmd = [
        str(py),
        "-m",
        "merlin.baselines.buddy_native_import",
        "--model",
        model,
        "--variant",
        variant,
    ...
    if r.returncode != 0 or not sub.is_file():
        raise BuddyError(
            f"native import failed for {model}/{variant}:\nSTDOUT:{r.stdout[-1000:]}\nSTDERR:{r.stderr[-1500:]}"
        )
    return sub
...
def _repair_malformed_select_slices(text: str) -> tuple[str, int]:
    ...
    while True:
        ...
        out.append(f"{_OFFSETS_ANCHOR}{offs_text}{_SIZES_ANCHOR}{new_sizes}{_STRIDES_ANCHOR}{strides_text}>{tail}")
        i = end_of_op
...
def lower_to_llvmir(mlir_path: Path, work: Path, *, timeout: int = 1200) -> Path:
    ...
    if opt is None or xlate is None:
        raise BuddyError(
            "buddy-mlir lowering toolchain not built "
...
def compile_rv64gcv_object(ll_path: Path, work: Path, *, timeout: int = 2400) -> Path:
    ...
    if opt is not None:
        ...
        common = [f"-mtriple={triple}", f"-mattr={_RVV_MATTR}", f"--riscv-v-vector-bits-min={_RVV_VBITS}"]
        ...
        passes = ["-passes=function(loop-vectorize,slp-vectorizer,instcombine,simplifycfg)"] if big else ["-O3"]
...
def link_k1_elf(model_dir: Path, obj: Path, work: Path, *, inputs_npz: Path | None = None, timeout: int = 600) -> Path:
    ...
    mmap_weights = weights_bin.stat().st_size >= _BUDDY_MMAP_WEIGHTS_THRESHOLD if weights_bin.is_file() else False
...
def audit_object(obj: Path) -> tuple[float | None, list[ScalarFallback], dict]:
    ...
    *compute-bearing model kernels* that stayed scalar, not the harness plumbing.
    ...
    fallbacks = [
        ScalarFallback(symbol=sym, reason="emitted scalar (no RVV in kernel)", region=_region_of_symbol(sym))
...
def run_model(
    ...
    *,
    ...
    res = BaselineResult(
        ...
        march=k1.K1_MARCH,
        toolchain="buddy-mlir(llvm-fork)+clang23+spacemit-clang",
        framework_commit=buddy_commit(),
...
def run_model_native(
    ...
    *,
    ...
    res = BaselineResult(
        ...
        rel_threshold=rel_thr,
        march=k1.K1_MARCH,
        toolchain="buddy-mlir(native-dynamo)+llvm23",
        framework_commit=buddy_commit(),
        timestamp=artifacts.utc_stamp(),
        notes="native=dynamo-importer",
    )
    ...
    if not native_import_available():
        res.gap_reason = (
            "buddy native importer unavailable (need MLIR python bindings + buddy "
    ...
    try:
        elf = link_native_k1_elf(
            nat,
            objs,
            work,
            iv,
            out_elems=out_elems,
            out_lastdim=out_lastdim,
            in_elems=max(1, len(iv)),
...
def _run_native_on_board(res: BaselineResult, elf: Path, arg0: Path, b: _bundle.CaptureBundle, work: Path) -> None:
    ...
    if res.e2e_rdtime_ticks is not None:
        res.regions = [
            RegionProfile(
                name="other",
                rdtime_ticks=res.e2e_rdtime_ticks,
                cycles=res.e2e_cycles,
...
def _offboard_correctness(b: _bundle.CaptureBundle, work: Path) -> Correctness:
    ...
    return Correctness(
        note="host-JIT correctness driver not yet wired (no fabricated cos/rel); "
...
def run_all_variants(models=DEFAULT_MODELS, variants=VARIANT_ORDER, *, write: bool = True) -> list[BaselineResult]:
...
def _main(argv=None) -> int:
    ...
    ap.add_argument("--variant", default=None, help="single variant (int8|fp32); default runs BOTH, int8 first")
    ...
    for r in results:
        ...
        print(
            f"{r.model}/{r.variant}: {r.status():10s} {cov} {cc} "
