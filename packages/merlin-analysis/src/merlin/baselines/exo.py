...
_GLUE_GAP: dict[str, str] = {
    ...
    "rdt2": "diffusion transformer action head (model.blocks + register_tokens + t_embedder) — not a decoder-only LM",
    "groot_n1d7": "action-head model (head.action_encoder/decoder + state_encoder + diffusion) — not a decoder-only LM",
...
def _exo_dir() -> Path:
    return Path(__file__).with_name("exo_kernels")
...
def compile_exo_gemm(out_dir: Path) -> Path:
    ...
    r = subprocess.run(
        [str(exocc), "-o", str(out_dir), "-p", str(repo_root() / "merlin/python"), "-s", "exo_gemm", str(lib)],
        capture_output=True,
        text=True,
        env=env,
        timeout=180,
...
def compile_glue_ops(out_dir: Path) -> Path:
    ...
    env = dict(os.environ, PYTHONPATH=str(repo_root() / "merlin/python"))
    r = subprocess.run(
        [str(exocc), "-o", str(out_dir), "-p", str(repo_root() / "merlin/python"), "-s", "exo_glue_ops", str(lib)],
        capture_output=True,
        text=True,
        env=env,
        timeout=180,
...
def compile_exo_igemm(out_dir: Path, ku: int = 1, u: int = 1) -> Path:
    ...
    env = dict(os.environ, PYTHONPATH=str(repo_root() / "merlin/python"))
    r = subprocess.run(
        [str(exocc), "-o", str(out_dir), "-p", str(repo_root() / "merlin/python"), "-s", "exo_igemm", str(lib)],
        capture_output=True,
        text=True,
        env=env,
        timeout=180,
...
def compile_autosched_dot(out_dir: Path, nblock: int = 1) -> Path:
    ...
    env = dict(os.environ, PYTHONPATH=str(repo_root() / "merlin/python"))
    r = subprocess.run(
        [str(exocc), "-o", str(out_dir), "-p", str(repo_root() / "merlin/python"), "-s", "autosched_dot", str(lib)],
        capture_output=True,
        text=True,
        env=env,
        timeout=180,
...
def _schema_names(schema: str) -> dict:
    ...
    return dict(
        embed="lm.model.embed_tokens.weight",
        final_norm="lm.model.norm.weight",
        lm_head="lm.lm_head",
        layer_prefix="lm.model.layers.{L}",
        input_ln="{p}.input_layernorm.weight",
        post_ln="{p}.post_attention_layernorm.weight",
        q="{p}.self_attn.q_proj",
        k="{p}.self_attn.k_proj",
        v="{p}.self_attn.v_proj",
        o="{p}.self_attn.o_proj",
        gate="{p}.mlp.gate_proj",
        up="{p}.mlp.up_proj",
...
def detect_llama_config(hdr: dict, variant: str) -> dict | None:
    ...
    for schema in ("hf", "terse"):
        ...
        return dict(NL=NL, H=H, V=V, FF=FF, KV_OUT=kv_out, schema=schema, NH_hint=(4 if schema == "terse" else None))
    return None
...
def emit_weights_header(bundle_root: Path, out_dir: Path, variant: str = "fp32") -> Path:
    ...
    if cfg is None:
        raise RuntimeError(
            "capture is not a runnable Llama-family decoder (glue supports decoder-only q/k/v/o + gate/up/down stacks)"
        )
    ...
    if ivkey in extra.files:
        ...
        HD = H // NH
        half = HD // 2
        inv_freq = (1.0 / (_CFG["THETA"] ** (np.arange(0, half, dtype=np.float32) / half))).astype(np.float32)
    ...
    lines = [
        "/* generated by merlin.baselines.exo.emit_weights_header — do not edit. */",
        "#pragma once",
        "#include <stddef.h>",
        "#include <stdint.h>",
        f"#define NL {NL}",
    ...
    if q8:
        lines += [
            f"#define WOFF_LM_HEAD_W {off(lm_head_name(sfx))}UL",
        ...
        lines += [
            f"#define WOFF_LM_HEAD {off(lm_head_name('.weight'))}UL",
        ...
        for L in range(NL):
            ...
            names = [nm[r].format(p=p) + ("" if r in ("input_ln", "post_ln") else ".weight") for r in roles]
...
def build_glue_binary(
    bundle_root: Path,
    work: Path,
    variant: str = "fp32",
    ku: int = 1,
    u: int = 1,
    autosched: bool = False,
    nblock: int = 1,
) -> tuple[Path, list[str]]:
    ...
    if variant == "int8":
        ...
        extra_c = [
            str(compile_glue_ops(work)),  # residual-add + ewise-mul RVV
            str(_exo_dir() / "fgemm_nk.c"),  # transpose-free weight-only dot (hot path)
            str(compile_exo_igemm(work, ku, u)),
        ]  # EXO int8 vwmacc GEMM (audit only)
...
def autotune_igemm_ku(
    work: Path, *, shape=(8, 4096, 2048), candidates=IGEMM_U_CANDIDATES
...
def autotune_autosched_nblock(
    work: Path, *, shape=(8, 512, 512), candidates=AUTOSCHED_NBLOCK_CANDIDATES
) -> tuple[int, dict[int, int]]:
...
def _scalar_fallbacks(variant: str = "fp32") -> list[ScalarFallback]:
    ...
    return [ScalarFallback(symbol=sym, reason="no EXO RVV kernel — scalar glue", region=reg) for sym, reg in ops]


def run(
    model: str = "tiny_llama", variant: str = "fp32", *, autotune: bool = True, autosched: bool = False
) -> BaselineResult:
    ...
    res = BaselineResult(
        ...
        march=MARCH,
        toolchain="spacemit-clang-19+exo-1.0.0",
    ...
    if autosched and variant != "int8":
        ...
        kdesc = (
            "WEIGHT-ONLY int8 (W8A16), TRANSPOSE-FREE: native [OUT,IN] int8 weights dequantized "
            ...
            if variant == "int8"
            else "f32 vfmacc.vf GEMM (gemm_nt_ref, EXO)"
        )
    if variant == "int8":
        glue_note = (
            "RVV kernels: transpose-free weight-only int8 dot GEMM (fgemm_nk_dot_i8, "
        ...
        glue_note = (
            "The nn.Linear GEMM is the EXO-AUTOSCHEDULED dot (EXO vectorize auto-op, not a "
        ...
        glue_note = (
            "Only the nn.Linear GEMM is an EXO kernel; RMSNorm/RoPE/attention-softmax/"
            "SwiGLU/residual/embed are scalar C glue (labeled)."
        )
    res.notes = (
        f"whole-model = RVV GEMM kernel [{kdesc}] + hand C glue runtime; EXO is a "
    ...
    if model in _GLUE_GAP:
        res.gap_reason = f"EXO glue runtime is a Llama-family decoder; {model} does not fit: {_GLUE_GAP[model]}"
        return res.validate()
    ...
    if variant == "int8" and autotune and k1_exec.board_available():
        try:
            ...
            res.notes += (
                f" | EXO-igemm U autotune (audit only; glue uses dot GEMM): U_ticks={u_ticks} -> best U={u}"
                if u_ticks
                else " | EXO-igemm autotune: no candidate ran, U=8"
            )
    ...
    if autosched and variant != "int8" and autotune and k1_exec.board_available():
        try:
            ...
            res.notes += (
                f" | autosched nblock autotune: ticks={nb_ticks} -> best nblock={nblock}"
                if nb_ticks
                else " | autosched nblock autotune: no candidate ran, nblock=1"
            )
        except Exception as e:  # noqa: BLE001
    ...
    try:
        ...
        ksyms = (
            ("fgemm_nk_dot_i8", "gemm_nt_ref", "igemm_nt_ref")
            if variant == "int8"
        ...
        for ksym in ksyms:
            ...
            if gk is not None and gk.coverage is not None:
                res.notes += f" | {ksym} RVV coverage={gk.coverage:.2f} (vec={gk.vector} scalar={gk.scalar_compute})"
    ...
    try:
        with k1_exec.board_lock():
            ...
            if local_out.exists():
                local_out.unlink()
            proc = k1_exec.run([f"MERLIN_WEIGHTS={remote_w} MERLIN_OUTFILE={remote_out} {remote}"], timeout=1800)
            ...
            for _att in range(2):
                r = subprocess.run(
                    ["scp", *k1_exec._SSH_OPTS, f"{k1.K1_HOST}:{remote_out}", str(local_out)],
                    capture_output=True,
                    timeout=300,
                )
    ...
    if of is not None and scp_ok and local_out.is_file():
        ...
        if raw.size == of[1]:  # exact match only (not >=; guards stale/partial files)
            ...
            res.gap_reason = (
                f"output file truncated: got {raw.size} floats, expected {of[1]} "
    ...
    if out is None:
        res.gap_reason = (
            f"no usable output (rc={proc.returncode}); OUTFILE={of} scp_ok={scp_ok}; console tail: {stdout[-200:]}"
        )
        return res.validate()
    res.ran = True
    ...
    if pred.size == gold.size:
        ...
        if variant == "int8" and b.golden.is_file():
            ...
            if g2.size == pred.size:
                res.notes += (
                    f" | vs golden.npy(weight-only) cos="
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--all":
        ...
        for r in run_all(variant=variant):
            print(
                f"{r.model:12s} {r.variant:4s} {r.status():10s} "
