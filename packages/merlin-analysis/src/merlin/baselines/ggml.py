...
_VLA_SCOPE_REASON: dict[str, str] = {
    ...
    "openvla": (
        "openvla's LLM backbone is Llama-2 (a supported ggml arch), but the captured unit is "
    ...
    "molmoact": (
        "molmoact's captured unit IS a clean causal-LM input_ids->logits forward and its "
    ...
    "rdt": (
        "rdt is an RDT diffusion action expert: the captured forward takes (x, freq, t, lang_c, "
    ...
    "rdt2": (
        "rdt2 is an RDT2 flow-matching action head over cached cross-attention KV (inputs "
        "x,t,state_c,kv_0..27 -> action [1,24,20]) — a diffusion/flow step, not a causal LM; "
        "no llama.cpp arch / no token-in / no vocab logits"
    ),
    "xr0": (
        "xr0 is a diffusion action head (noisy_action,t,state,cos,sin,attn_mask,kv_0..31 -> "
        "[1,30,32]) — a denoising step over cached KV, not a token-in causal LM; no ggml arch"
    ),
    "pi05": (
        "pi05 is a pi-0.5 flow-matching VLA denoiser (multi-image + lang_tokens + state + noise "
        "-> action [1,50,32]) — a diffusion step, not a token-in / vocab-logits causal LM; no "
        "ggml arch"
    ),
    "smolvla": (
        "smolvla is a SmolVLA flow-matching denoiser (img + lang_tokens + state + noise -> "
        "action [1,50,32]) — a diffusion step, not a token-in causal LM; no ggml arch"
    ),
    "groot_n1d7": (
        "groot_n1d7 is a GR00T-N1 action head over precomputed backbone_features (+ state, "
...
def convert_to_gguf(model: str, *, outtype: str = "f16", timeout: int = 1200) -> Path:
    ...
    if snap is None:
        raise GgmlError(
            f"no local HF checkpoint for {model!r} to convert to GGUF "
...
def _region_of_symbol(sym: str) -> str:
    ...
    if any(
        t in s
        for t in ("_add", "_mul", "gelu", "silu", "relu", "_exp", "quantize", "dequantize", "rope", "_scale", "_cpy")
    ):
        return "elementwise"
    return "other"
...
def audit_cpu_so(so: Path) -> RvvAudit:
    ...
    for name, sc in sorted(report.by_symbol.items()):
        ...
        if any(t in name for t in ("ggml_gemm", "ggml_gemv", "ggml_vec_dot")) and any(
            q in name for q in _ACTIVE_QUANT_KERNELS
        ):
            aqv += sc.vector
            aqs += sc.scalar_compute
        if sc.is_scalar_fallback:
            fallbacks.append(
                ScalarFallback(symbol=name[:80], reason="ggml compute kernel emitted fully scalar (no RVV)", region=reg)
            )
    cov_kernels = (kv / (kv + ks)) if (kv + ks) else None
    cov_aq = (aqv / (aqv + aqs)) if (aqv + aqs) else None
    region_cov = {
        r: (region_v[r] / (region_v[r] + region_s.get(r, 0))) for r in region_v if (region_v[r] + region_s.get(r, 0))
    }
    return RvvAudit(
        coverage_overall=report.coverage_overall,
        coverage_kernels=cov_kernels,
...
def _board_sh(cmd: str, *, timeout: int = 900) -> subprocess.CompletedProcess:
    return subprocess.run(["ssh", *_ssh_opts(), k1_exec.K1_HOST, cmd], capture_output=True, text=True, timeout=timeout)
...
def deploy_runtime() -> None:
    ...
    tools = [bindir / t for t in ("llama-bench", "llama-quantize", "llama-completion") if (bindir / t).is_file()]
    for p in libs:
        subprocess.run(
            ["scp", *_ssh_opts(), str(p), f"{k1_exec.K1_HOST}:{_BOARD_DIR}/lib/{p.name}"],
            check=True,
            capture_output=True,
            timeout=300,
        )
    for p in tools:
        subprocess.run(
            ["scp", *_ssh_opts(), str(p), f"{k1_exec.K1_HOST}:{_BOARD_DIR}/{p.name}"],
            check=True,
            capture_output=True,
            timeout=300,
        )
...
def run_on_board(
    gguf_remote: str, *, n_threads: int = 8, pp: int = 64, tg: int = 32, timeout: int = 1200
) -> tuple[dict, str]:
    ...
    bench = f"{ld} {_BOARD_DIR}/llama-bench -m {gguf_remote} -t {n_threads} -p {pp} -n {tg} -r 3"
...
def run_model(
    ...
    *,
    ...
    res = BaselineResult(
        ...
        march=GGML_MARCH,
        toolchain="llama.cpp+ggml(RVV) / spacemit-gcc-14.3",
        framework_commit=ggml_commit(),
    ...
    try:
        ...
        res.notes += (
            f" rvv_int8_cov(Q8_0:q8_0+q8_K)={aud.coverage_int8} "
    ...
    if model == "small_llama":
        ...
        res.built = True
        res.notes += (
            f" gguf_f16={gguf_f16.name}({gguf_f16.stat().st_size // 1024}KB, direct "
    ...
    if model not in _HF_CHECKPOINTS:
        if model == "bitvla":
            ...
            reason = (
                "bitvla's LLM backbone is a genuine BitNetForCausalLM (llama.cpp supports the "
    ...
    if _correctness_bundle(model) is None:
        res.notes += (
            " correctness: cos=None UNCOMPARABLE — no real-checkpoint capture bundle "
...
def _run_bench_on_board(res: BaselineResult, model: str, gguf_f16: Path, quants: tuple[str, ...]) -> None:
    ...
    with k1_exec.board_lock():
        ...
        if have != want:
            subprocess.run(
                ["scp", *_ssh_opts(), str(gguf_f16), f"{k1_exec.K1_HOST}:{remote_f16}"],
                check=True,
                capture_output=True,
                timeout=1800,
            )
        for quant in quants:
            ...
            if "yes" not in _board_sh(f"test -s {remote_q} && echo yes || echo no", timeout=60).stdout:
                qp = _board_sh(f"{ld} {_BOARD_DIR}/llama-quantize {remote_f16} {remote_q} {quant} 2>&1", timeout=1800)
                if "yes" not in _board_sh(f"test -s {remote_q} && echo yes || echo no").stdout:
                    ...
                    why = next((ln.strip() for ln in tail if "not divisible" in ln), "") or next(
                        (
                            ln.strip()
        ...
        if cbundle is not None:
            try:
                ...
                subprocess.run(
                    ["scp", *_ssh_opts(), str(dumper), f"{k1_exec.K1_HOST}:{_BOARD_DIR}/merlin_logits"],
                    check=True,
                    capture_output=True,
                    timeout=120,
                )
                ...
                r = _board_sh(f"{ld} {_BOARD_DIR}/merlin_logits {corr_gguf} {out_remote} {idstr}", timeout=900)
                loc = cache_dir("baselines") / f"logits_{model}_{corr_quant}.f32"
                subprocess.run(
                    ["scp", *_ssh_opts(), f"{k1_exec.K1_HOST}:{out_remote}", str(loc)], capture_output=True, timeout=120
                )
                if loc.is_file() and loc.stat().st_size > 8:
                    cos, rel = _compare_logits_to_golden(loc, cbundle.golden)
                    res.cos, res.rel = cos, rel
                    src = (
                        "real full TinyLlama-1.1B checkpoint"
                        if model == "tiny_llama"
                        else "GGUF built directly from the same capture-bundle weights"
                        if model == "small_llama"
                        else "same-checkpoint GGUF"
                    )
                    res.notes += (
                        f" correctness: COMPARABLE cos={cos:.6f} rel={rel:.6f} vs "
                        f"{cbundle.root.name}/golden.npy via {corr_quant} logits on the "
                        f"SAME seeded input_ids ({src});"
                    )
                else:
                    res.notes += (
                        f" correctness dump produced no logits: {r.stdout.strip()[-100:]} {r.stderr.strip()[-100:]};"
                    )
            except Exception as e:  # noqa: BLE001
    ...
    for quant in quants:
        ...
        if b.get("rc") == 0 and b.get("tps"):
            ran_any = True
            cov = b.get("cov")
            tps_summ.append(f"{quant}[" + ", ".join(f"{t}={v}tok/s" for t, v in b["tps"].items()) + f", rvv={cov}]")
