...
from merlin.common.paths import module_source_path, python_import_roots, repo_root, runtime_dir
    ...
    paths = (
        ...
        *(
            ...
            for module in (
                ...
                "merlin.llvmlower.passes_xdsl",
                "merlin.llvmlower.pipeline",
                "merlin.llvmlower.session_bundle",
                "merlin.compare.paper_merlin_lower_worker",
                "merlin.compare.paper_build_bundle",
                "merlin.compare.paper_session_abi",
                "merlin.compare.paper_session_tracer",
            )
        ),
    )
...
def _load_recipe(
    ...
    for index, raw in enumerate(rows):
        ...
        worker = repo_root() / "packages/merlin-analysis/src/merlin/compare/paper_merlin_lower_worker.py"
...
def _producer_env(authority: MultiToolchainAuthority) -> dict[str, str]:
    return {
        ...
        "PYTHONPATH": ":".join(str(path) for path in python_import_roots()),
        "PYTHONNOUSERSITE": "1",
