...
def _git_provenance(path: Path) -> dict[str, Any]:
    ...
    try:
        sha = subprocess.run(
            ["git", "-C", str(path), "rev-parse", "HEAD"], capture_output=True, text=True, timeout=10, check=True
        ).stdout.strip()
        ...
        dirty_content_sha256 = hashlib.sha256(
            json.dumps(dirty_payload, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
...
def _protocol_design_replacement_valid(
    *,
    ...
    try:
        ...
        if (
            len(replacement["reason_codes"]) != len(reason_codes)
            or set(documents)
            != {
                "frozen_protocol",
                "protocol_claim",
                ...
                "claim_revocation",
                "campaign_exclusion",
                "arm1_terminal_outcome",
                "arm1_grader_result",
                "arm2_terminal_outcome",
                "arm2_grader_result",
                "arm4_run_record",
                "arm4_controller_cancellation",
            }
        ...
        arm1_terminal = _mapping(documents["arm1_terminal_outcome"], "predecessor Arm1 terminal")
        arm2_terminal = _mapping(documents["arm2_terminal_outcome"], "predecessor Arm2 terminal")
        ...
        cancellation = _mapping(documents["arm4_controller_cancellation"], "predecessor Arm4 cancellation")
        ...
        def linked(document: dict[str, Any], key: str, artifact: str, **extra: Any) -> bool:
            ...
            return (
                isinstance(row, dict)
                and row.get("sha256") == artifact_sha256.get(artifact)
                and all(row.get(name) == value for name, value in extra.items())
            )

        def exact_reasons(document: dict[str, Any]) -> bool:
            reasons = document.get("reason_codes")
            return isinstance(reasons, list) and len(reasons) == len(reason_codes) and set(reasons) == reason_codes

        def protocol_failure(terminal: dict[str, Any], grader: dict[str, Any], arm: str, message: str) -> bool:
            ...
            return (
                terminal.get("arm") == arm
                and terminal.get("terminal_class") == "graded_fail"
                and checks.get("agent_success") is True
                and checks.get("workspace_input_audit") is True
                and checks.get("aet_reconciled") is True
                and checks.get("compiler_seal_status") == "sealed"
                and checks.get("grader_status") == "fail"
                and grader.get("status") == "fail"
                and isinstance(records, list)
                and len(records) == 143
                and all(
                    isinstance(row, dict)
                    and row.get("reason") == "compiler invocation failed"
                    and message in str(row.get("stderr_tail", ""))
                    for row in records
                )
            )
        ...
        if (
            frozen.get("status") != "protocol_frozen"
            or not isinstance(freeze, dict)
            or freeze.get("protocol_inputs_sha256") != protocol
        ...
        if actual_names != sorted(expected_receipt_names) or any(
            path.is_symlink() or not path.is_file() for path in cells.iterdir()
        ):
            return False
        receipt_tree = revocation.get("receipt_tree")
        if (
            not isinstance(receipt_tree, dict)
            or set(receipt_tree) != set(expected_receipt_names)
            or any(
                not _digest(digest) or hashlib.sha256((cells / name).read_bytes()).hexdigest() != digest
        ...
        for ordinal, (plan_row, exclusion_row) in enumerate(zip(plan, exclusion_cells)):
            if not isinstance(plan_row, dict) or not isinstance(exclusion_row, dict):
                return False
            expected_name = f"{ordinal:02d}.consumed.json" if ordinal < 3 else f"{ordinal:02d}.cancelled.json"
            receipt = json.loads((cells / expected_name).read_text(encoding="utf-8"))
            if not isinstance(receipt, dict):
                return False
            expected_run = (
                f"{campaign}__{plan_row.get('arm')}__"
                f"r{int(plan_row.get('repeat')):02d}__"
                f"seed{int(plan_row.get('seed')):03d}"
            )
            expected_state = (
                "sealed_protocol_pilot"
                if ordinal < 2
                else "controller_cancelled_during_agent"
                if ordinal == 2
                else "not_started_cancelled"
            )
            if (
                plan_row.get("ordinal") != ordinal
                or receipt.get("version") != 1
                or receipt.get("status") != "authorized"
                or receipt.get("protocol_inputs_sha256") != protocol
                or receipt.get("campaign_run_id") != campaign
                or receipt.get("ordinal") != ordinal
                or receipt.get("arm") != plan_row.get("arm")
                or receipt.get("repeat") != plan_row.get("repeat")
                or receipt.get("seed") != plan_row.get("seed")
                or receipt.get("run_id") != expected_run
                or exclusion_row.get("ordinal") != ordinal
                or exclusion_row.get("arm") != plan_row.get("arm")
                or exclusion_row.get("repeat") != plan_row.get("repeat")
                or exclusion_row.get("seed") != plan_row.get("seed")
                or exclusion_row.get("state") != expected_state
                or exclusion_row.get("arm_outcome") is not False
                or exclusion_row.get("receipt") != expected_name
                or exclusion_row.get("receipt_sha256") != receipt_tree[expected_name]
            ):
                return False
        ...
        if (
            ...
            or len(sealed) != 2
            or [row.get("run_id") for row in sealed if isinstance(row, dict)] != [expected_arm1, expected_arm2]
            or not isinstance(partial, dict)
        ):
            return False
        audit_links_ok = (
            linked(audit, "frozen_protocol", "frozen_protocol")
            and linked(audit, "protocol_claim", "protocol_claim", status="bound")
            and linked(sealed[0], "terminal_outcome", "arm1_terminal_outcome")
            and linked(sealed[0], "grader_result", "arm1_grader_result")
            and linked(sealed[1], "terminal_outcome", "arm2_terminal_outcome")
            and linked(sealed[1], "grader_result", "arm2_grader_result")
            and partial.get("run_record_sha256") == artifact_sha256["arm4_run_record"]
            and partial.get("raw_events_sha256") == artifact_sha256["arm4_raw_events"]
        )

        exclusion_schedule_ok = (
            exclusion.get("version") == 1
            and exclusion.get("authority") == "controller_campaign_exclusion_v1"
            and exclusion.get("campaign_run_id") == campaign
            and exclusion.get("protocol_inputs_sha256") == protocol
            and exclusion.get("classification") == "protocol_invalid_pilot"
            and exclusion.get("confirmatory_eligible") is False
            and exclusion.get("excluded_from_arm_outcomes") is True
            and exclusion.get("excluded_from_promotion") is True
            and exclusion.get("excluded_from_holdout_capture") is True
            and exclusion.get("supersedes_terminal_paper_eligibility") is True
            and exclusion.get("treatment_or_provider_started") is True
            and exact_reasons(exclusion)
            and linked(exclusion, "frozen_protocol", "frozen_protocol")
            and linked(exclusion, "protocol_claim", "protocol_claim", status="bound")
            and linked(exclusion, "design_audit", "design_audit")
            and exclusion.get("non_reuse")
            == {
                "threads": True,
                "workspaces": True,
                "submissions": True,
        ...
        exclusion_partial_ok = (
            isinstance(exclusion_partial, dict)
            and exclusion_partial.get("run_record_sha256") == artifact_sha256["arm4_run_record"]
            and exclusion_partial.get("raw_events_sha256") == artifact_sha256["arm4_raw_events"]
            and exclusion_partial.get("controller_cancellation_sha256")
            == artifact_sha256["arm4_controller_cancellation"]
            and exclusion_partial.get("turn_completed_present") is False
            and exclusion_partial.get("usage_record_present") is False
            and exclusion_partial.get("submission_directory_empty") is True
        )

        revocation_links_ok = (
            revocation.get("version") == 1
            and revocation.get("authority") == "controller_protocol_claim_revocation_v1"
            and revocation.get("campaign_run_id") == campaign
            and revocation.get("protocol_inputs_sha256") == protocol
            and revocation.get("consumed_ordinals") == [0, 1, 2]
            and revocation.get("cancelled_ordinals") == list(range(3, 16))
            and revocation.get("resume_forbidden") is True
            and exact_reasons(revocation)
            and linked(revocation, "claim", "protocol_claim", historical_status="bound")
            and linked(revocation, "campaign_exclusion", "campaign_exclusion")
            and linked(revocation, "design_audit", "design_audit")
        )

        cancellation_links_ok = (
            cancellation.get("version") == 1
            and cancellation.get("run_id") == expected_arm4
            and cancellation.get("campaign_run_id") == campaign
            and cancellation.get("protocol_inputs_sha256") == protocol
            and cancellation.get("classification") == "controller_cancelled_after_protocol_invalidation"
            and cancellation.get("terminal_outcome_present") is False
            and cancellation.get("paper_evidence_eligible") is False
            and cancellation.get("promotion_eligible") is False
            and exact_reasons(cancellation)
            and linked(cancellation, "design_audit", "design_audit")
        )

        return (
            claim.get("version") == 1
            and claim.get("status") == "bound"
            and claim.get("campaign_run_id") == campaign
            and claim.get("protocol_inputs_sha256") == protocol
            and audit.get("version") == 1
            and audit.get("authority") == "post_start_protocol_design_audit"
            and audit.get("campaign_run_id") == campaign
            and audit.get("protocol_inputs_sha256") == protocol
            and decision.get("classification") == "protocol_design_invalid"
            and decision.get("campaign_stopped") is True
            and decision.get("exclude_all_cells_from_arm_outcomes") is True
            and decision.get("exclude_all_cells_from_promotion") is True
            and decision.get("exclude_all_cells_from_holdout_capture") is True
            and exact_reasons(decision)
            and audit_links_ok
            and exclusion_schedule_ok
            and exclusion_partial_ok
            and revocation_links_ok
            and cancellation_links_ok
            and protocol_failure(arm1_terminal, arm1_grader, "arm1_raw_cpp", "missing numeric capsule field: family")
            and protocol_failure(arm2_terminal, arm2_grader, "arm2_cpp_scaffold", "missing family enum")
            and arm4_record.get("run_id") == expected_arm4
            and arm4_record.get("arm") == "arm4_agentic_pass_authoring"
            and arm4_record.get("seed") == 1
            and arm4_raw_size > 0
            and partial.get("run_id") == expected_arm4
            and partial.get("classification") == "controller_cancelled_after_protocol_invalidation"
            and partial.get("terminal_outcome_present") is False
            and partial.get("paper_evidence_eligible") is False
            and partial.get("promotion_eligible") is False
        )
...
def _calibration_sample(rows: list[dict[str, Any]], *, per_family: int, families: list[str]) -> list[dict[str, Any]]:
    ...
    representatives = [min(values, key=lambda row: str(row["sha256"])) for _, values in sorted(buckets.items())]
    selected = []
    for family in sorted(families):
        selected.extend(
            sorted((row for row in representatives if row["family"] == family), key=lambda row: str(row["sha256"]))[
                :per_family
            ]
        )
    return selected
...
def _calibration_private_capsule(row: dict[str, Any], *, nonce: bytes, phase: str) -> dict[str, Any]:
    ...
    for name, value in sorted(shape.items()):
        ...
        delta = 1 + int.from_bytes(hashlib.sha256(nonce + material).digest()[:2], "big") % 13
        shape[name] = value + delta
    if shape == row.get("shape"):
        raise ValueError("calibration private capsule shape did not change")
    private["shape"] = shape
    identity = {key: private[key] for key in ("family", "operation", "dtype", "shape", "layout", "state", "core_count")}
...
def _calibration_k1_state_gates():
    ...
    path = (repo_root() / "merlin/experiments/cpu_host_compiler_v0/grader.py").resolve()
...
def _calibration_space_problem(recorded: Any) -> str | None:
    ...
    """
    expected = (repo_root() / "merlin/experiments/cpu_host_compiler_v0/optimization_space_v1.yaml").resolve()
    try:
        ...
        if hashlib.sha256(path.read_bytes()).hexdigest() != hashlib.sha256(expected.read_bytes()).hexdigest():
...
def _validate_calibration_semantics(
    *,
    ...
    noise_authority_sha256: str | None = None,
) -> list[str]:
    ...
    if label == "noise_calibration":
        expected_checks = {
            "six_families": True,
            "six_valid_pairs_per_family": True,
            "all_correct": True,
            "identical_k1_text": True,
            "no_heldout_argument": True,
        }
        if (
            value.get("version") != 2
            or value.get("kind") != "cpu_host_k1_order_balanced_aa_noise_calibration"
            or value.get("status") != "pass"
            or value.get("checks") != expected_checks
            or value.get("paid_work") is not False
            or value.get("heldout_opened") is not False
            or value.get("protocol_state_mutated") is not False
        ):
        ...
        if any(value.get(field) != source_sha256.get(name) for field, name in source_fields.items()):
        ...
        expected_public_context = {
            ...
            "rows_sha256": hashlib.sha256(
                json.dumps(train_rows or [], sort_keys=True, separators=(",", ":")).encode()
            ).hexdigest(),
        }
        ...
        derivation = (
            "margin=max(0.02,ceil((exp(max(abs(log(pair_ratio)))+0.005)-1)*1000)/1000); lower_bound=1/(1+margin)"
        )
        calibration_protocol = {
            ...
            "private_shape_authority": "trusted_broker_private_capsule_independent_calibration_nonce",
            "public_context": "complete_public_train",
            "search_package_authority": "driver_private_prebuild",
            "derivation": derivation,
        }
        protocol_sha256 = hashlib.sha256(
            json.dumps(calibration_protocol, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
        if (
            value.get("calibration_protocol") != calibration_protocol
            or value.get("calibration_protocol_sha256") != protocol_sha256
        ...
        try:
            package_ok = (
                ...
                and (submission / "manifest.yaml").is_file()
                and hashlib.sha256((submission / "manifest.yaml").read_bytes()).hexdigest()
                == value.get("submission_manifest_sha256")
                and _tree_sha256(submission) == value.get("submission_tree_sha256")
            )
        except OSError:
            package_ok = False
        if not package_ok:
            ...
            if not isinstance(manifest, dict) or manifest.get("build") != {"command": ["/bin/true"]}:
                problems.append("A/A noise calibration did not measure the prebuilt package")
        noise_sources = {name: digest for name, digest in source_sha256.items() if name != "search_space"}
        ...
        if space_problem is not None:
            problems.append(f"A/A noise calibration does not bind the frozen optimization space: {space_problem}")
        prebuild_input = Path(str(value.get("prebuild_input_submission", ""))).resolve()
        receipt = value.get("prebuild_receipt")
        try:
            input_ok = (
                prebuild_input.is_dir()
                and not prebuild_input.is_symlink()
                and prebuild_input != submission
                and hashlib.sha256((prebuild_input / "manifest.yaml").read_bytes()).hexdigest()
                == value.get("prebuild_input_manifest_sha256")
                and _tree_sha256(prebuild_input) == value.get("prebuild_input_tree_sha256")
                and _grader_package_tree_identity(prebuild_input) == value.get("prebuild_input_package_sha256")
            )
            receipt_ok = (
                isinstance(receipt, dict)
                and receipt.get("version") == 1
                and receipt.get("authority") == "driver_private_prebuild"
                and receipt.get("private_build_override") == ["/bin/true"]
                and receipt.get("submitted_manifest_sha256") == value.get("prebuild_input_manifest_sha256")
                and receipt.get("prebuild_tree_sha256") == value.get("prebuild_input_package_sha256")
                and _digest(receipt.get("built_tree_sha256"))
                and receipt.get("built_tree_sha256") != receipt.get("prebuild_tree_sha256")
                and receipt.get("built_tree_sha256") != receipt.get("sealed_prebuilt_tree_sha256")
                and receipt.get("sealed_prebuilt_tree_sha256") == _grader_package_tree_identity(submission)
                and receipt.get("private_manifest_sha256") == value.get("submission_manifest_sha256")
                and value.get("prebuild_receipt_sha256")
        ...
        try:
            ...
            expected_private = [
                _calibration_private_capsule(row, nonce=nonce, phase="confirm") for row in expected_public
            ]
            records = private_authority["records"]
            private_ok = (
                private_authority.get("version") == 1
                and private_authority.get("authority") == "trusted_broker_private_capsule_independent_calibration_nonce"
                and len(nonce) == 32
                and private_authority.get("phase") == "confirm"
                and private_authority.get("split") == "train"
                and private_authority.get("nonce_sha256") == hashlib.sha256(nonce).hexdigest()
                and private_authority.get("records_sha256")
                == hashlib.sha256(json.dumps(records, sort_keys=True, separators=(",", ":")).encode()).hexdigest()
                and records
                == [
                    {"public": public, "private": private}
                    for public, private in zip(expected_public, expected_private, strict=True)
                ]
            )
        ...
        if isinstance(receipt, dict):
            required_tools.update(
                f"prebuild_command_{index}" for index, _ in enumerate(receipt.get("real_build_commands", ()))
            )
            required_tools.update(
                f"private_build_override_{index}" for index, _ in enumerate(receipt.get("private_build_override", ()))
            )
        if (
            not isinstance(toolchain, dict)
            or not required_tools <= set(toolchain)
            or any(
                not isinstance(toolchain[name], dict)
                or not Path(str(toolchain[name].get("path", ""))).resolve().is_file()
                or hashlib.sha256(Path(str(toolchain[name]["path"])).resolve().read_bytes()).hexdigest()
                != toolchain[name].get("sha256")
                or Path(str(toolchain[name]["path"])).resolve().stat().st_mode & 0o777 != toolchain[name].get("mode")
        ...
        def valid_spike_gates(row: dict[str, Any]) -> bool:
            ...
            for gate in gates.values():
                ...
                if (
                    ...
                    or gate.get("compile_ok") is not True
                    or gate.get("k1_compile_ok") is not True
                    or gate.get("passed") is not True
                    or not isinstance(checks, dict)
                    or any(
                        checks.get(name) is not True
                        for name in ("rvv_correctness", "instruction_evidence", "vlen_256", "cycle_measurement")
                    )
                    or not _digest(gate.get("kernel_text_sha256"))
                ):
                    return False
            return True

        def valid_k1_evidence(
            evidence: Any, *, observation: dict[str, Any], side: str, seed: int, elapsed_ns: int, calls: int
        ) -> bool:
            ...
            expected_digest = observation.get("baseline_code_sha256" if side == "parent" else "candidate_code_sha256")
            return (
                evidence.get("capsule") == observation.get("capsule_id")
                and evidence.get("family") == observation.get("family")
                and evidence.get("status") == "pass"
                and evidence.get("seed") == seed
                and isinstance(checks, dict)
                and bool(checks)
                and all(checks.values())
                and isinstance(metrics, dict)
                and metrics.get("wall_ns") == elapsed_ns
                and metrics.get("calls") == calls
                and evidence.get("kernel_text_sha256") == expected_digest
                and isinstance(evidence.get("receipt_nonce"), int)
                and not isinstance(evidence.get("receipt_nonce"), bool)
                and evidence["receipt_nonce"] > 0
                and _digest(evidence.get("local_sha256"))
                and evidence.get("remote_sha256") == evidence.get("local_sha256")
                and isinstance(monitor, dict)
                and monitor.get("returncode") == 0
                and evidence.get("ssh_returncode") == 0
                and _positive_number(evidence.get("board_wall_seconds"))
            )

        def valid_condition(
            condition: Any, *, observation: dict[str, Any], expected_valid: bool, arrays: list[list[int]]
        ) -> bool:
            ...
            if (
                not isinstance(pair_id, int)
                or isinstance(pair_id, bool)
                or not 0 <= pair_id < repeats
                or not isinstance(attempt_id, int)
                or isinstance(attempt_id, bool)
                or attempt_id < 0
                or condition.get("order") != pair_orders[pair_id]
                or condition.get("valid") is not expected_valid
            ):
                return False
            settle = condition.get("settle_probes")
            before, after = condition.get("before"), condition.get("after")
            if (
                not isinstance(settle, list)
                or not settle
                or len(settle) > int(board_environment.get("settle_attempts", 0))
                or any(state_ready(probe, board_environment) for probe in settle[:-1])
                or before != settle[-1]
                or not state_ready(settle[-1], board_environment)
                or state_pair_ok(before, after, board_environment) is not expected_valid
            ):
                return False
            ...
            if (
                ...
                or set(measurements) != {"parent", "candidate"}
            ):
                return False
            for side, elapsed_samples, call_samples in (
                ...
                if (
                    ...
                    return False
                if expected_valid and (elapsed_ns != elapsed_samples[pair_id] or calls != call_samples[pair_id]):
                    return False
            return True
        ...
        if (
            ...
            or len(observations) != len(families)
            or {row.get("family") for row in observations if isinstance(row, dict)} != set(families)
            or any(
                not isinstance(row, dict)
                or row.get("correctness_ok") is not True
                or row.get("baseline_code_sha256") != row.get("candidate_code_sha256")
                or not valid_spike_gates(row)
                or len(row.get("board_condition_pairs", ())) != repeats
        ...
        if raw_observations_ok:
            for observation in observations:
                arrays = [
                    ...
                    for name in ("baseline_elapsed_ns", "baseline_calls", "candidate_elapsed_ns", "candidate_calls")
                ]
                if any(
                    not isinstance(samples, list)
                    or len(samples) != repeats
                    or any(not isinstance(sample, int) or isinstance(sample, bool) or sample <= 0 for sample in samples)
                ...
                if (
                    observation.get("pair_orders") != pair_orders
                    or not isinstance(condition_pairs, list)
                    or not isinstance(excluded_pairs, list)
                    or len(excluded_pairs)
                    > int(board_environment.get("maximum_invalid_pair_replacements_per_capsule", -1))
                    or any(
                        not valid_condition(condition, observation=observation, expected_valid=True, arrays=arrays)
                        or condition.get("pair_id") != pair_index
                        for pair_index, condition in enumerate(condition_pairs)
                    )
                    or any(
                        not valid_condition(condition, observation=observation, expected_valid=False, arrays=arrays)
                        for condition in excluded_pairs
                    )
                ):
                    raw_observations_ok = False
                    break
                ...
                for condition in chronological:
                    ...
                    if condition["valid"]:
                        current_pair_id += 1
                if (
                    sorted(attempt_ids) != list(range(len(all_attempts)))
                    or [condition["attempt_id"] for condition in condition_pairs]
                    != sorted(condition["attempt_id"] for condition in condition_pairs)
                    or [condition["attempt_id"] for condition in excluded_pairs]
                    != sorted(condition["attempt_id"] for condition in excluded_pairs)
                    or not transcript_ok
                    or current_pair_id != repeats
                    or observation.get("k1_program_count") != 2 * len(all_attempts)
                ):
                    raw_observations_ok = False
                    break
        ...
        if not isinstance(pairs, list) or pairs != expected_pair_rows:
            ...
            exact = (
                math.isclose(
                    float(value.get("maximum_absolute_pair_deviation", -1)),
                    max(abs(ratio - 1.0) for ratio in ratios),
                    rel_tol=1e-12,
                    abs_tol=1e-12,
                )
                and math.isclose(
                    float(value.get("maximum_absolute_log_ratio", -1)), maximum_log, rel_tol=1e-12, abs_tol=1e-12
                )
                and math.isclose(float(value.get("padded_log_half_width", -1)), padded, rel_tol=1e-12, abs_tol=1e-12)
                and float(value.get("derived_noise_margin", -1)) == margin
                and float(value.get("upper_speedup_bound", -1)) == 1.0 + margin
                and float(value.get("lower_speedup_bound", -1)) == 1.0 / (1.0 + margin)
                and float(space.get("noise_margin", -1)) == margin
            )
            if not exact:
                problems.append("A/A derived margin/reciprocal bound do not exactly equal optimization_space")
        return problems
    if (
        value.get("version") != 1
        or value.get("paid_work") is not False
        ...
        problems.append(f"{label} calibration must be version 1, public-only, unpaid, and non-mutating")
    ...
    if _calibration_space_problem(value.get("space")) is not None or value.get("space_sha256") != source_sha256.get(
        "search_space"
    ):
        problems.append(f"{label} calibration does not bind the exact frozen optimization space")
    submission = Path(str(value.get("submission", ""))).resolve()
    try:
        measured_manifest = yaml.safe_load((submission / "manifest.yaml").read_text(encoding="utf-8"))
        package_ok = (
            submission.is_dir()
            and not submission.is_symlink()
            and (submission / "manifest.yaml").is_file()
    ...
    elif not isinstance(measured_manifest, dict) or measured_manifest.get("build") != {"command": ["/bin/true"]}:
    ...
    try:
        prebuild_input_ok = (
            ...
            and not prebuild_input.is_symlink()
            and prebuild_input != submission
            and (prebuild_input / "manifest.yaml").is_file()
            and hashlib.sha256((prebuild_input / "manifest.yaml").read_bytes()).hexdigest()
            == value.get("prebuild_input_manifest_sha256")
            and _tree_sha256(prebuild_input) == value.get("prebuild_input_tree_sha256")
            and _grader_package_tree_identity(prebuild_input) == value.get("prebuild_input_package_sha256")
    ...
    receipt = value.get("prebuild_receipt")
    receipt_digest = (
        hashlib.sha256(json.dumps(receipt, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
        if isinstance(receipt, dict)
        else ""
    )
    receipt_ok = (
        ...
        and receipt.get("submitted_manifest_sha256") == value.get("prebuild_input_manifest_sha256")
        and receipt.get("private_manifest_sha256") == value.get("submission_manifest_sha256")
        and receipt.get("prebuild_tree_sha256")
        == (value.get("prebuild_input_package_sha256") if prebuild_input_ok else None)
        and _digest(receipt.get("built_tree_sha256"))
        and receipt.get("built_tree_sha256") != receipt.get("prebuild_tree_sha256")
        and receipt.get("built_tree_sha256") != receipt.get("sealed_prebuilt_tree_sha256")
        and receipt.get("sealed_prebuilt_tree_sha256")
        == (_grader_package_tree_identity(submission) if package_ok else None)
        and isinstance(receipt.get("real_build_commands"), list)
        and bool(receipt.get("real_build_commands"))
        and isinstance(receipt.get("real_build_logs"), list)
        and bool(receipt.get("real_build_logs"))
        and isinstance(receipt.get("built_entrypoint_identity"), list)
        and len(receipt["built_entrypoint_identity"]) == 2
        and isinstance(receipt["built_entrypoint_identity"][0], int)
        and _digest(receipt["built_entrypoint_identity"][1])
        and value.get("prebuild_receipt_sha256") == receipt_digest
    )
    ...
    if isinstance(noise_authority, dict) and _digest(noise_authority_sha256):
        expected_cost_lineage = {
            ...
            "noise_authority": value.get("calibration_lineage", {}).get("noise_authority")
            if isinstance(value.get("calibration_lineage"), dict)
            else None,
            "noise_authority_sha256": noise_authority_sha256,
            "pre_result_protocol_sha256": noise_authority.get("calibration_protocol_sha256"),
            "derived_noise_margin": float(noise_authority.get("derived_noise_margin", -1)),
            "raw_input_tree_sha256": noise_authority.get("prebuild_input_tree_sha256"),
            "raw_input_package_sha256": noise_authority.get("prebuild_input_package_sha256"),
    ...
    if isinstance(lineage, dict):
        try:
            ...
            lineage_path_ok = (
                ...
                and not lineage_path.is_symlink()
                and hashlib.sha256(lineage_path.read_bytes()).hexdigest() == noise_authority_sha256
            )
        except OSError:
            lineage_path_ok = False
    if (
        expected_cost_lineage is None
        or lineage != expected_cost_lineage
        or not lineage_path_ok
        or value.get("prebuild_input_tree_sha256") != noise_authority.get("prebuild_input_tree_sha256")
        or value.get("prebuild_input_package_sha256") != noise_authority.get("prebuild_input_package_sha256")
        or float(space.get("noise_margin", -1)) != float(noise_authority.get("derived_noise_margin", -2))
    ):
        problems.append(f"{label} calibration lacks exact A/A predecessor, final-margin, and raw-tree lineage")
    toolchain = value.get("toolchain_identity")
    expected_tools = {
        "python",
        "bwrap",
        ...
        "spike_objdump",
        *(
            f"prebuild_command_{index}"
            for index, _ in enumerate(receipt.get("real_build_commands", ()) if isinstance(receipt, dict) else ())
        ),
        *(
            f"private_build_override_{index}"
            for index, _ in enumerate(receipt.get("private_build_override", ()) if isinstance(receipt, dict) else ())
        ),
    }
    ...
    if toolchain_ok:
        for name in expected_tools:
            ...
            path = Path(str(identity.get("path", ""))).resolve() if isinstance(identity, dict) else Path("unresolved")
            try:
                valid = (
                    ...
                    and path.stat().st_mode & 0o777 == identity.get("mode")
                )
            except OSError:
                valid = False
            if not valid:
    ...
    per_family = int(space["screen_samples_per_family" if phase == "screen" else "confirmation_samples_per_family"])
    try:
        nonce_hex = private_authority["nonce_hex"]
        nonce = bytes.fromhex(nonce_hex)
        records = private_authority["records"]
        expected_public = _calibration_sample(
            list(train_rows or []), per_family=per_family, families=list(space["confirmation_families"])
        )
        expected_private = [_calibration_private_capsule(row, nonce=nonce, phase=phase) for row in expected_public]
        records_digest = hashlib.sha256(
            json.dumps(records, sort_keys=True, separators=(",", ":")).encode("utf-8")
        ).hexdigest()
        private_ok = (
            isinstance(private_authority, dict)
            and private_authority.get("version") == 1
            and private_authority.get("authority") == "trusted_broker_private_capsule_independent_calibration_nonce"
            and private_authority.get("phase") == phase
            and private_authority.get("split") == "train"
            and len(nonce) == 32
            and private_authority.get("nonce_sha256") == hashlib.sha256(nonce).hexdigest()
            and private_authority.get("records_sha256") == records_digest
            and records
            == [
                {"public": public, "private": private}
                for public, private in zip(expected_public, expected_private, strict=True)
    ...
    if label == "k1_calibration":
        if value.get("kind") != "cpu_host_trusted_search_k1_program_calibration" or value.get("status") != "pass":
            problems.append("K1 calibration has the wrong kind")
        if checks != {"all_passed": True, "max_within_planning_upper": True, "mean_within_expected": True}:
        ...
        expected_declared = {
            ...
            "planning_upper_seconds_per_program": float(budget["planning_upper_seconds_per_k1_program"]),
        }
        ...
        if calibration_capsule != (
            {key: expected_capsule.get(key) for key in ("id", "sha256", "family", "split")}
            if expected_capsule
            else None
        ):
        ...
        trusted_observation = (
            trusted[0] if isinstance(trusted, list) and len(trusted) == 1 and isinstance(trusted[0], dict) else {}
        )
        ...
        maximum_replacements = int(
            space.get("board_environment", {}).get("maximum_invalid_pair_replacements_per_capsule", -1)
        )
        expected_program_count = trusted_observation.get("k1_program_count")
        program_count_bound = (
            isinstance(valid_pairs, list)
            and len(valid_pairs) == int(space["measurement_repeats"])
            and all(isinstance(pair, dict) and pair.get("valid") is True for pair in valid_pairs)
            and isinstance(excluded_pairs, list)
            and len(excluded_pairs) <= maximum_replacements
            and all(isinstance(pair, dict) and pair.get("valid") is False for pair in excluded_pairs)
            and type(expected_program_count) is int
            and expected_program_count == 2 * (len(valid_pairs) + len(excluded_pairs))
        )
        if (
            not isinstance(trusted, list)
            or len(trusted) != 1
            or not isinstance(trusted[0], dict)
            or trusted[0].get("capsule_id") != (expected_capsule or {}).get("id")
            or trusted[0].get("family") != (expected_capsule or {}).get("family")
            or trusted[0].get("correctness_ok") is not True
            or not program_count_bound
        ):
            problems.append("K1 calibration trusted observation is incomplete")
        if (
            not isinstance(programs, list)
            or not program_count_bound
            or len(programs) != expected_program_count
            or not isinstance(statistics, dict)
            or statistics.get("count") != len(programs)
        ):
            problems.append("K1 calibration must retain every valid and board-condition-rejected program")
        elif any(
            not isinstance(row, dict)
            or row.get("index") != index
            or row.get("capsule_id") != (expected_capsule or {}).get("id")
            or row.get("family") != (expected_capsule or {}).get("family")
            or row.get("status") != "pass"
            or not isinstance(row.get("checks"), dict)
            or not row["checks"]
            or not all(row["checks"].values())
            or not isinstance(row.get("start_monotonic_ns"), int)
            or not isinstance(row.get("end_monotonic_ns"), int)
            or row["end_monotonic_ns"] <= row["start_monotonic_ns"]
            or not _positive_number(row.get("total_seconds"))
            or not math.isclose(
            ...
            or row["evidence"].get("capsule") != row.get("capsule_id")
            or row["evidence"].get("family") != row.get("family")
            or row["evidence"].get("status") != "pass"
            or row["evidence"].get("checks") != row.get("checks")
            or row["evidence"].get("metrics") != row.get("metrics")
            or row["evidence"].get("monitor") != row.get("monitor")
            or row["evidence"].get("kernel_text_sha256") != row.get("kernel_text_sha256")
            or row["evidence"].get("seed") != row.get("seed")
            or not isinstance(row["evidence"].get("receipt_nonce"), int)
            or not _digest(row["evidence"].get("local_sha256"))
            or row["evidence"].get("remote_sha256") != row["evidence"].get("local_sha256")
            or not _positive_number((row.get("metrics") or {}).get("wall_ns"))
            or not _positive_number((row.get("metrics") or {}).get("calls"))
            ...
            maximum = max(totals)
            if (
                not math.isclose(float(statistics.get("mean_seconds", -1)), mean, rel_tol=1e-12, abs_tol=1e-12)
                or not math.isclose(float(statistics.get("median_seconds", -1)), median, rel_tol=1e-12, abs_tol=1e-12)
                or not math.isclose(float(statistics.get("p95_seconds", -1)), p95, rel_tol=1e-12, abs_tol=1e-12)
                or not math.isclose(float(statistics.get("max_seconds", -1)), maximum, rel_tol=1e-12, abs_tol=1e-12)
                or mean > expected_declared["expected_seconds_per_program"]
                or maximum > expected_declared["planning_upper_seconds_per_program"]
            ):
                problems.append("K1 calibration statistics do not support the frozen budget")
    elif label == "spike_calibration":
        expected_capsules = int(space["screen_samples_per_family"]) * len(space.get("confirmation_families", ()))
        ...
        if (
            value.get("capsules") != expected_capsules
            or value.get("completed_observations") != expected_capsules
            or value.get("maximum_candidate_evaluations") != maximum_evaluations
            or not isinstance(value.get("start_monotonic_ns"), int)
            or not isinstance(value.get("end_monotonic_ns"), int)
            or value["end_monotonic_ns"] <= value["start_monotonic_ns"]
            or not _positive_number(candidate)
            or not _positive_number(projected)
            or not math.isclose(
                float(candidate),
            ...
            or not math.isclose(float(projected), float(candidate) * maximum_evaluations, rel_tol=1e-12, abs_tol=1e-12)
            or float(projected) > float(budget["expected_spike_screen_seconds"])
        ):
            problems.append("Spike calibration measured counts/projection do not support the budget")
        if (
            not isinstance(observations, list)
            or len(observations) != expected_capsules
            or any(
                not isinstance(row, dict)
                or row.get("correctness_ok") is not True
                or not isinstance(row.get("capsule_id"), str)
                or not row.get("capsule_id")
                or row.get("family") not in space.get("confirmation_families", ())
                for row in observations
            )
        ):
        ...
        expected_declared = {
            "package_build": {
                "expected_seconds": float(budget["expected_seconds_per_confirmation_package_build"]),
                "planning_upper_seconds": float(budget["planning_upper_seconds_per_confirmation_package_build"]),
            },
            "compiler_invocation": {
                "expected_seconds": float(budget["expected_seconds_per_confirmation_compiler_invocation"]),
                "planning_upper_seconds": float(budget["planning_upper_seconds_per_confirmation_compiler_invocation"]),
            },
            "spike_check": {
                "expected_seconds": float(budget["expected_seconds_per_confirmation_spike_check"]),
                "planning_upper_seconds": float(budget["planning_upper_seconds_per_confirmation_spike_check"]),
            },
        }
        ...
        if value.get("declared") != expected_declared:
            problems.append("confirmation-overhead calibration declaration differs from the frozen budget")
        ...
        runtime_capsules = sum(row.get("family") == "runtime_parallel" for row in expected_private)
        stage_counts = {
            "package_build": count,
            "compiler_invocation": count + runtime_capsules * 2,
            "spike_check": count,
        }
        expected_capsule_rows = [{key: row.get(key) for key in ("id", "sha256", "family")} for row in expected_private]
        if (
            not isinstance(capsules, list)
            or capsules != expected_capsule_rows
            or not isinstance(repeats, int)
            or repeats != 2
            or len({row.get("id") for row in capsules if isinstance(row, dict)}) != expected_capsules
            or any(not isinstance(row, dict) or not _digest(row.get("sha256")) for row in capsules)
            or value.get("spike_statuses") != ["pass"] * count
        ):
            problems.append("confirmation-overhead calibration public measurements are incomplete")
        trusted = value.get("trusted_evaluation_observations")
        if (
            not isinstance(trusted, list)
            or len(trusted) != expected_capsules
            or any(
                not isinstance(row, dict)
                or row.get("correctness_ok") is not True
                or row.get("calibration_authority") != "exact_confirmation_pre_k1_stages_without_k1"
                for row in trusted
            )
            or {(row.get("capsule_id"), row.get("family")) for row in trusted}
            != {(row["id"], row["family"]) for row in expected_private}
        ):
        ...
        for name, limits in expected_declared.items():
            ...
            if (
                ...
                or observation.get("count") != stage_counts[name]
                or not _positive_number(observation.get("mean_seconds"))
                or not _positive_number(observation.get("max_seconds"))
                or float(observation["mean_seconds"]) > limits["expected_seconds"]
                or float(observation["max_seconds"]) > limits["planning_upper_seconds"]
            ):
        ...
        if not isinstance(stage_observations, dict) or set(stage_observations) != set(expected_declared):
            problems.append("confirmation-overhead lacks complete raw stage observations")
        else:
            for name, rows in stage_observations.items():
                if (
                    not isinstance(rows, list)
                    or len(rows) != stage_counts[name]
                    or any(
                        not isinstance(row, dict)
                        or row.get("index") != index
                        or row.get("stage") != name
                        or row.get("side") not in {"parent", "candidate"}
                        or row.get("capsule_id") not in {capsule["id"] for capsule in expected_private}
                        or row.get("family")
                        != next(
                            (
                                capsule["family"]
                                for capsule in expected_private
                                if capsule["id"] == row.get("capsule_id")
                            ),
                            None,
                        )
                        or row.get("status") != "pass"
                        ...
                        or not isinstance(row.get("end_monotonic_ns"), int)
                        or row["end_monotonic_ns"] <= row["start_monotonic_ns"]
                        or not _positive_number(row.get("wall_seconds"))
                        or not math.isclose(
                    ...
                    problems.append(f"confirmation-overhead {name} raw observations are incomplete")
                    continue
                expected_membership = (
                    {
                        ...
                        for mode in (("rvv", "rvv_multicore") if capsule["family"] == "runtime_parallel" else ("rvv",))
                    }
                    if name == "compiler_invocation"
                    else {
                        (capsule["id"], side, None) for capsule in expected_private for side in ("parent", "candidate")
                    }
                )
                if {(row["capsule_id"], row["side"], row.get("mode")) for row in rows} != expected_membership:
                    problems.append(f"confirmation-overhead {name} capsule/stage membership differs")
                    continue
                ...
                if (
                    ...
                    or summary.get("count") != len(totals)
                    or not math.isclose(
                        float(summary.get("mean_seconds", -1)), sum(totals) / len(totals), rel_tol=1e-12, abs_tol=1e-12
                    )
                    or not math.isclose(float(summary.get("median_seconds", -1)), median, rel_tol=1e-12, abs_tol=1e-12)
                    or not math.isclose(float(summary.get("p95_seconds", -1)), p95, rel_tol=1e-12, abs_tol=1e-12)
                    or not math.isclose(
                        float(summary.get("max_seconds", -1)), max(totals), rel_tol=1e-12, abs_tol=1e-12
                    )
                ):
                    problems.append(f"confirmation-overhead {name} summary differs from raw observations")
    return problems
...
class HostExperimentSpec:
    ...
    def parse(raw: Any, *, source_path: Path | None = None) -> "HostExperimentSpec":
        ...
        spec = HostExperimentSpec(
            ...
            label=str(raw["label"]),
            target_contract=str(raw["target_contract"]),
            dialect_plan=str(raw.get("dialect_plan", "")),
            ...
            arms=arms,
            agent=dict(_mapping(raw["agent"], "agent")),
            ...
            freeze=dict(_mapping(raw["freeze"], "freeze")),
    ...
    def from_yaml(path: str | Path) -> "HostExperimentSpec":
        ...
        return HostExperimentSpec.parse(yaml.safe_load(source.read_text(encoding="utf-8")), source_path=source)
    ...
    def _validate(self) -> None:
        ...
        if self.replacement:
            expected_replacement = {
                "kind",
                "predecessor_campaign_run_id",
                "predecessor_protocol_inputs_sha256",
                "scope",
                "treatment_or_provider_started",
            ...
            common_valid = (
                set(self.replacement) == expected_replacement
                and self.replacement.get("scope") == "all_sixteen_cells"
                and self.replacement.get("excluded_from_arm_outcomes") is True
                and isinstance(self.replacement.get("predecessor_campaign_run_id"), str)
                and bool(self.replacement.get("predecessor_campaign_run_id"))
                and bool(_digest(self.replacement.get("predecessor_protocol_inputs_sha256")))
            )
            infrastructure = (
                kind == "whole_campaign_infrastructure_replacement"
                and self.replacement.get("treatment_or_provider_started") is False
                and set(self.replacement.get("reason_codes", ()))
                == {
            ...
            protocol_design = (
                kind == "whole_campaign_protocol_design_replacement"
                and self.replacement.get("treatment_or_provider_started") is True
                and set(self.replacement.get("reason_codes", ()))
                == {
            ...
            replacement_evidence = _mapping(self.replacement.get("evidence"), "replacement.evidence")
            expected_evidence = (
                {
                    "frozen_protocol",
                    "protocol_claim",
                    ...
                    "ordinal0_run_result",
                    "ordinal0_stderr",
                    "ordinal0_terminal_outcome",
                    "ordinal0_raw_events",
                    "ordinal0_token_ledger",
                }
                if infrastructure
                else {
                    "frozen_protocol",
            ...
            for name, row in replacement_evidence.items():
                ...
                if set(row) != {"path", "sha256"} or not _resolved(row.get("path")) or not _digest(row.get("sha256")):
        ...
        if int(agent.get("repeats", 0)) != 4 or int(agent.get("active_wall_seconds_per_arm", 0)) < 1:
        ...
        if isinstance(agent.get("launch_seed"), bool) or not isinstance(agent.get("launch_seed"), int):
        ...
        for ordinal, row in enumerate(launch_plan):
            if (
                ...
                or isinstance(row.get("repeat"), bool)
        ...
        if (
            self.environment.get("require_exact_local_before_each_cell") is not True
            or self.environment.get("require_exact_k1_before_each_cell") is not True
        ):
            raise ValueError("environment must require exact local and K1 identity before every cell")
        if (
            self.freeze.get("forbid_model_name_dispatch") is not True
            or self.freeze.get("forbid_post_freeze_tuning") is not True
            or int(self.freeze.get("required_empty_sweeps", 0)) != 1
            or self.freeze.get("require_all_four_arms_complete") is not True
        ):
            raise ValueError(
        ...
        if self.freeze.get("failure_policy") != {
            ...
            raise ValueError("freeze.failure_policy must launch every cell once and forbid retries/fallback")
        ...
        if selection != {
            ...
            raise ValueError("freeze.selection must predeclare arm4/repeat0 without using heldout outcomes")
        search_arms = [arm.id for arm in self.arms if "deterministic_candidate_search" in arm.capabilities]
        if (
            int(self.search.get("required_paired_measurements", 0)) != 6
            or self.search.get("measurement_authority") != "spacemit_k1_elapsed_ns_div_completed_calls"
            or list(self.search.get("trusted_seal_required_for", ())) != search_arms
        ):
            raise ValueError("search arms require six balanced paired trusted K1 measurements and a driver seal")
        ...
        if any(
            ...
            for name in (
                "arm_wall_seconds",
                "reserved_agent_seconds",
                "expected_seconds_per_k1_program",
                "planning_upper_seconds_per_k1_program",
                "expected_spike_screen_seconds",
                "planning_upper_spike_screen_seconds",
        ...
        required_environment = {
            ...
            "governor",
            "frequency_khz",
            "frequency_core_count",
            "frequency_relative_tolerance",
            "maximum_temperature_millic",
            "maximum_pair_temperature_delta_millic",
            "maximum_load_1m",
            "maximum_pair_load_delta",
            "settle_attempts",
        ...
        if (
            environment["online"] != "0-7"
            or environment["governor"] != "performance"
            or int(environment["frequency_core_count"]) != int(self.grading.get("expected_harts", 0))
            or not 0 < float(environment["frequency_relative_tolerance"]) < float(space_config.get("noise_margin", 0))
            or int(environment["settle_attempts"]) * float(environment["settle_interval_seconds"]) > 60
            or int(environment["maximum_invalid_pair_replacements_per_capsule"]) < 0
        ):
    ...
    def preflight(
        self, *, check_environment: bool = True, probe_board: bool = False, require_frozen: bool = False
    ) -> HostPreflight:
        errors: list[str] = []
        blockers: list[str] = []
        warnings: list[str] = []
        ...
        if require_frozen and self.status not in {
            ...
            blockers.append("experiment status is draft; a live campaign requires a frozen protocol")
        ...
        paths = {
            ...
            "host_agent_contract": str(module_source_path("merlin.benchharness.host_agent")),
            "capsule_descriptor_contract": str(module_source_path("merlin.benchharness.capsule_descriptor")),
            "artifact_lifecycle": str(module_source_path("merlin.common.artifacts")),
            "repository_paths": str(module_source_path("merlin.common.paths")),
            "schema_validator": str(module_source_path("merlin.common.schemas")),
            "agent_sandbox_boundary": str(module_source_path("merlin.targetgen.sandbox.bwrap")),
            "corpus_materializer": str(module_source_path("merlin.mining.corpus")),
            "corpus_partition_policy": str(module_source_path("merlin.mining.campaign")),
            "k1_measurement_adapter": str(module_source_path("merlin.mining.k1")),
            "experiment_schema": str(repo_root() / "merlin/schemas/cpu_host_experiment.schema.yaml"),
            "frozen_environment_contract": str(module_source_path("merlin.compare.frozen_environment")),
            ...
            "confirmation_overhead_calibration": (self.search.get("calibration") or {}).get(
                "confirmation_overhead_artifact"
            ),
            "noise_calibration": (self.search.get("calibration") or {}).get("noise_artifact"),
        ...
        for name, row in sorted((self.replacement.get("evidence") or {}).items()):
            ...
            actual = hashlib.sha256(path.read_bytes()).hexdigest() if path is not None and path.is_file() else None
        ...
        if replacement_evidence:
            ...
            if kind == "whole_campaign_infrastructure_replacement":
                ...
                try:
                    launch = _mapping(
                        json.loads(predecessor["launch"].read_text(encoding="utf-8")), "replacement predecessor launch"
                    )
                    claim = _mapping(
                        json.loads(predecessor["protocol_claim"].read_text(encoding="utf-8")),
                        "replacement predecessor claim",
                    )
                    run_result = _mapping(
                        json.loads(predecessor["ordinal0_run_result"].read_text(encoding="utf-8")),
                        "replacement predecessor run result",
                    )
                    attempts = run_result.get("attempts")
                    usage = _mapping(run_result.get("total_usage"), "replacement predecessor usage")
                    results = launch.get("results")
                    planned = launch.get("planned")
                    objective_no_treatment_start = (
                        launch.get("campaign_run_id") == self.replacement.get("predecessor_campaign_run_id")
                        and launch.get("protocol_inputs_sha256")
                        == self.replacement.get("predecessor_protocol_inputs_sha256")
                        and claim.get("protocol_inputs_sha256")
                        == self.replacement.get("predecessor_protocol_inputs_sha256")
                        and isinstance(planned, list)
                        and len(planned) == 16
                        and isinstance(results, list)
                        and len(results) == 16
                        and sum(row.get("attempted") is True for row in results if isinstance(row, dict)) == 1
                        and sum(row.get("cell_status") == "not_started" for row in results if isinstance(row, dict))
                        == 15
                        and run_result.get("thread_id") is None
                        and run_result.get("status") == "failed"
                        and isinstance(attempts, list)
                        and len(attempts) == 1
                        and isinstance(attempts[0], dict)
                        and attempts[0].get("thread_id") is None
                        and usage.get("reported") is False
                        and predecessor["ordinal0_raw_events"].stat().st_size == 0
                        and predecessor["ordinal0_token_ledger"].stat().st_size == 0
                        and "bwrap: setting up uid map: Permission denied"
                        in predecessor["ordinal0_stderr"].read_text(encoding="utf-8")
                    )
                except Exception as exc:
                    errors.append(f"replacement predecessor cannot be objectively audited: {exc}")
                if not objective_no_treatment_start:
                    errors.append("replacement predecessor evidence does not prove zero treatment/provider start")
                ...
                if not protocol_design_invalid:
                    errors.append("replacement predecessor evidence does not prove protocol-design invalidity")
                replacement_verification = {
            ...
            evidence["replacement_predecessor"] = {
                ...
                "protocol_inputs_sha256": self.replacement.get("predecessor_protocol_inputs_sha256"),
                "excluded_from_arm_outcomes": self.replacement.get("excluded_from_arm_outcomes"),
                **replacement_verification,
        ...
        if _resolved(environment_value):
            ...
            try:
                ...
                environment_check = validate_frozen_environment(
                    ...
                    check_local=check_environment,
                    check_board=probe_board,
                )
        ...
        elif self.status in {"protocol_frozen", "campaign_complete", "campaign_complete_unpromoted"} or require_frozen:
        ...
        if not _resolved(materialized):
            ...
            if not materialized_root.is_dir():
                ...
                if not lock_path.is_file():
                    ...
                    try:
                        lock = _mapping(yaml.safe_load(lock_path.read_text(encoding="utf-8")), "corpus lock")
                        ...
                        if any(
                            lock.get(name) != derived[name]
                            for name in ("definition_sha256", "corpus_sha256", "capsule_count", "split_counts", "files")
                        ):
                            errors.append("materialized corpus is not the exact expansion of its frozen definition")
                        files = _mapping(lock.get("files"), "corpus lock files")
                        required = {"public/train.jsonl", "public/validation.jsonl", "sealed/heldout.jsonl"}
        ...
        if materialized_root is not None and materialized_root.is_dir():
            ...
            if train_path.is_file():
                ...
                train_rows = [
                    json.loads(line) for line in train_path.read_text(encoding="utf-8").splitlines() if line.strip()
                ]
                ...
                if configured_noise_path is not None and configured_noise_path.is_file():
                    try:
                        loaded_noise = json.loads(configured_noise_path.read_text(encoding="utf-8"))
                        if isinstance(loaded_noise, dict):
                            configured_noise_value = loaded_noise
                            configured_noise_sha256 = hashlib.sha256(configured_noise_path.read_bytes()).hexdigest()
                    ...
                    try:
                        ...
                        blockers.extend(
                            _validate_calibration_semantics(
                                ...
                                space=self.search_space_config(),
                        ...
                        blockers.append(f"{calibration_name} semantic validation failed closed: {exc}")
        if grader_path and grader_path.is_file():
            if materialized_root is not None and materialized_root.is_dir():
                corpus_check = subprocess.run(
                    [
                        ...
                        "--validate-corpus",
                        "--train",
                        str(materialized_root / "public/train.jsonl"),
                        "--validation",
                        str(materialized_root / "public/validation.jsonl"),
                        "--heldout",
                        str(materialized_root / "sealed/heldout.jsonl"),
                    ],
                    capture_output=True,
                    text=True,
                    timeout=60,
                ...
                try:
                    ...
                    corpus_result = {
                        "ready": False,
                        "error": (
                            corpus_check.stderr.strip()
                            or corpus_check.stdout.strip()
                            or "grader emitted no corpus-check result"
                        )[:1000],
                    }
            ...
            try:
                proc = subprocess.run(
                    [sys.executable, str(grader_path), "--self-check"], capture_output=True, text=True, timeout=30
                )
                ...
                if proc.returncode:
                    ...
                    self_check = json.loads(
                        proc.stdout.strip().splitlines()[-1] if proc.stdout.count("\n") == 0 else proc.stdout
                    )
                    ...
                    if missing_levels:
                        blockers.append(f"deterministic CPU-host grader does not implement levels {missing_levels}")
                    ...
                    if (
                        trusted.get("outside_sandbox_broker_api") is not True
                        or trusted.get("paired_measurements") != 6
                        or trusted.get("heldout_argument") is not False
                    ):
        ...
        for name, provenance in external_provenance.items():
            if (
                ...
                or not _git_oid(provenance.get("git_sha"))
                or not _digest(provenance.get("dirty_content_sha256"))
            ):
                blockers.append(f"{name} source provenance cannot bind commit plus local content")

        arm_workspace_inputs: dict[str, Any] = {}
        workspace_paths = {
            name: resolved_paths.get(name)
            for name in (
                "task",
                "target_contract",
                "dialect_plan",
        ...
        if materialized_root is not None and all(
            ...
            try:
                ...
                with tempfile.TemporaryDirectory(prefix="merlin-host-protocol-workspaces-") as temporary:
                    root = Path(temporary)
        ...
        freeze_methodology = {
            name: self.freeze.get(name)
            for name in (
                "forbid_model_name_dispatch",
                "forbid_post_freeze_tuning",
                "required_empty_sweeps",
                "require_all_four_arms_complete",
                "failure_policy",
                "selection",
            )
        }
        protocol_payload = {
            ...
            "arms": [
                {"id": arm.id, "order": arm.order, "capabilities": sorted(arm.capabilities), "treatment": arm.treatment}
                for arm in self.arms
            ],
            "agent": self.agent,
            "telemetry": self.telemetry,
        ...
        actual_protocol_digest = hashlib.sha256(
            json.dumps(protocol_payload, sort_keys=True, separators=(",", ":"), default=list).encode("utf-8")
        ).hexdigest()
        ...
        if self.status in {"campaign_complete", "campaign_complete_unpromoted"}:
            ...
            if not isinstance(campaign_record, dict):
                ...
                actual_campaign_sha = hashlib.sha256(
                    json.dumps(campaign_record, sort_keys=True, separators=(",", ":")).encode("utf-8")
                ).hexdigest()
                ...
                if (
                    campaign_record.get("completed_run_count") != 16
                    or campaign_record.get("analysis_plan_sha256") != self.analysis.get("sha256")
                    or not isinstance(boundary_receipts, list)
                    or len(boundary_receipts) != 4
                    or selection.get("heldout_outcome_used") is not False
                    or selection.get("selection_outcome_fields_used") != []
                    or selection_run_id != expected_selected
                    or promotion.get("status") != ("promoted" if promoted else "ineligible")
                ):
                    errors.append("completed campaign is not an outcome-independent complete 4x4 campaign")
            ...
            if not promoted and self.freeze.get("selected_compiler_package") != "unresolved":
                ...
                try:
                    from merlin.benchharness.host_agent import _submission_package_digest, _submission_source_digest

                    manifest = _mapping(
                        yaml.safe_load((package / "manifest.yaml").read_text(encoding="utf-8")),
                        "selected compiler manifest",
                    )
                    policy = (package / str(manifest.get("policy", ""))).resolve()
                    if (
                        not policy.is_relative_to(package)
                        or not policy.is_file()
                        or hashlib.sha256(policy.read_bytes()).hexdigest() != self.freeze.get("selected_policy_sha256")
                        or _submission_source_digest(package) != self.freeze.get("compiler_sha256")
                        or _submission_package_digest(package) != self.freeze.get("runtime_sha256")
                    ):
        ...
        confirmation_requests = distinct_incumbents * int(space_config.get("confirmation_width", 0)) * 2
        ...
        runtime_confirmation_capsules = (
            ...
            if "runtime_parallel" in space_config.get("confirmation_families", ())
            else 0
        )
        compiler_invocations = confirmation_requests * (confirmation_capsules + runtime_confirmation_capsules) * 2
        spike_checks = confirmation_requests * confirmation_capsules * 2
        expected_k1_programs = (
            confirmation_requests * confirmation_capsules * 2 * int(space_config.get("measurement_repeats", 0))
        )
        k1_programs = (
            confirmation_requests
            * confirmation_capsules
            * 2
            * (
        ...
        expected_confirmation_overhead = (
            package_builds * float(budget["expected_seconds_per_confirmation_package_build"])
            + compiler_invocations * float(budget["expected_seconds_per_confirmation_compiler_invocation"])
            + spike_checks * float(budget["expected_seconds_per_confirmation_spike_check"])
        )
        planning_upper_confirmation_overhead = (
            package_builds * float(budget["planning_upper_seconds_per_confirmation_package_build"])
            + compiler_invocations * float(budget["planning_upper_seconds_per_confirmation_compiler_invocation"])
            + spike_checks * float(budget["planning_upper_seconds_per_confirmation_spike_check"])
        )
        expected_seconds = (
        ...
        available_seconds = int(self.agent["active_wall_seconds_per_arm"]) - int(budget["reserved_agent_seconds"])
        evidence["trusted_search_budget"] = {
            ...
            "planning_upper_confirmation_overhead_seconds": (planning_upper_confirmation_overhead),
        ...
        if int(budget.get("arm_wall_seconds", -1)) != int(self.agent["active_wall_seconds_per_arm"]):
        ...
        if planning_upper_seconds > available_seconds:
            blockers.append("trusted search planning upper bound exceeds the active arm budget after reserve")
        ...
        if check_environment:
            ...
            if not evidence["codex_binary"]:
                ...
                version = subprocess.run(
                    [evidence["codex_binary"], "--version"], capture_output=True, text=True, timeout=30
                )
                auth = subprocess.run(
                    [evidence["codex_binary"], "login", "status"], capture_output=True, text=True, timeout=30
                )
            ...
            if not evidence["aet_importable"]:
                ...
                evidence["aet_timestamp_sidecar_support"] = "timestamped" in inspect.signature(
                    import_codex_run
                ).parameters or any(
            ...
            if not chia_python.is_file():
                ...
                proc = subprocess.run([str(chia_python), "-c", check], capture_output=True, text=True, timeout=30)
            ...
            try:
                ...
                if not evidence["k1_available"]:
                    ...
                    state_probe = grader_module._probe_k1_state(grader_module._k1_connection())
                    board_environment = _mapping(
                        self.search_space_config().get("board_environment"), "board_environment"
                    )
                    state_ready = grader_module._k1_state_ready(state_probe, board_environment)
    ...
    def _validate_analysis_plan(self, plan: dict[str, Any]) -> None:
        ...
        if (
            design.get("kind") != "balanced_williams_4x4"
            or design.get("repeats") != 4
            or design.get("repeat_identifier") != "paired_block_id"
            or design.get("provider_sampling_seeded") is not False
            or design.get("provider_seed_claim_forbidden") is not True
            or design.get("carryover_scope") != "within_block_transitions_only"
            or design.get("cross_block_transition_policy") != "excluded_after_mandatory_washout_and_requalification"
            or not isinstance(sequences, list)
            or len(sequences) != 4
            or any(not isinstance(sequence, list) or len(sequence) != 4 for sequence in sequences)
        ):
        ...
        carryover = [(left, right) for sequence in sequences for left, right in zip(sequence, sequence[1:])]
        ...
        if (
            estimand.get("population") != "all_sixteen_predeclared_arm_block_cells"
            or estimand.get("policy") != "intention_to_treat"
            or estimand.get("primary_unit") != "scheduled_arm_block_cell"
        ):
        ...
        if (
            not required_endpoints.issubset(set(endpoints.get("primary", ())))
            or endpoints.get("inferential_significance_claims") != "forbidden"
            or endpoints.get("interpretation") != "descriptive_small_n"
        ):
        ...
        if (
            reporting.get("report_order_and_block") is not True
            or reporting.get("report_block_boundary_receipts") is not True
        ):
