...
def _render_environment(record: dict[str, Any], bundle: Path) -> dict[str, str]:
    ...
    rendered = {str(key): str(value).replace("{bundle}", str(bundle)) for key, value in raw.items()}
    ...
    if not any(key.endswith("_PAPER_READY") and value == "1" for key, value in rendered.items()):
...
def _sanitized_environment(exact: dict[str, str]) -> dict[str, str]:
    environment = {key: value for key, value in os.environ.items() if not key.startswith(_ENV_PREFIXES)}
    environment.update(exact)
    return environment


def _source_paths(root: Path, study: PaperStudySpec) -> list[Path]:
    paths = [root / "workloads" / "capture_consistent.py", root / "workloads" / "capture.py", root / "m2m"]
...
def _preflight(
    study: PaperStudySpec, host: HostExperimentSpec, model2mlir: Path, paper_inputs: Path
    ...
    for model in study.models:
        for precision, artifact in model.artifacts.items():
            if str(artifact.get("sha256", "")).lower() != "unresolved":
                errors.append(
                    f"{model.name}/{precision}: capture is already registered; post-registration recapture is forbidden"
                )
    ...
    if not paper_inputs.is_dir():
        ...
        if actual_input_digest != expected_input_digest:
            errors.append(
                f"paper input bundle digest differs: study={expected_input_digest} actual={actual_input_digest}"
            )
        else:
    ...
    evidence = {
        "paper_inputs": {
            "path": str(paper_inputs),
            "expected_sha256": expected_input_digest,
            "actual_sha256": actual_input_digest,
        },
        "host_campaign": {
            "path": str(host.source_path) if host.source_path else None,
            "status": host.status,
            "freeze": dict(host.freeze),
            "preflight": host_preflight.to_dict(),
        },
        "model2mlir": {"path": str(model2mlir), "source_sha256": source_digest, **_git_provenance(model2mlir)},
    }
...
def _validate_output(task: CaptureTask, source: dict[str, Any], elapsed_ns: int) -> dict[str, Any]:
    sidecar = {
        ...
        "command_sha256": _json_sha256(list(task.command)),
    ...
    capture_bundle.CaptureBundle(model=task.model.capture, variant=task.variant, root=task.output).require()
    session, session_errors = validate_capture_session(
        task.output, task.model.session, expected_provenance=task.model.expected_provenance
    )
    ...
    if isinstance(provenance, dict) and provenance.get("checkpoint") != task.model.checkpoint:
        session_errors.append(
            f"session checkpoint differs: study={task.model.checkpoint!r} capture={provenance.get('checkpoint')!r}"
        )
    if session_errors:
    ...
    return {
        ...
        "sha256": sha256_paths([task.output]),
        "session_kind": session.get("kind"),
        "paper_ready": session.get("paper_ready"),
        "measurement_source_receipt": {"path": str(source_receipt), "sha256": _file_sha256(source_receipt)},
    }


def _default_runner(task: CaptureTask, environment: dict[str, str], stdout_path: Path, stderr_path: Path) -> int:
    with stdout_path.open("w", encoding="utf-8") as stdout, stderr_path.open("w", encoding="utf-8") as stderr:
        proc = subprocess.run(
            list(task.command), cwd=Path(task.command[1]).parent.parent, env=environment, stdout=stdout, stderr=stderr
        )
...
def materialize(
    ...
    *,
    ...
    plan: dict[str, Any] = {
        ...
        "study": {
            "path": str(study_path),
            "sha256": _file_sha256(study_path),
            "status": study.status,
            "holdout_models": list(study.holdout_models),
        },
        "host_experiment": {"path": str(host_experiment_path), "sha256": _file_sha256(host_experiment_path)},
        "evidence": evidence,
        "environment_policy": {
    ...
    write_yaml(
        staged_path,
        staged,
        header=("Capture-complete draft; pass this file to merlin-compare --freeze. Do not tune on captures"),
    )
    ...
    _write_json(
        registration_path,
        {
            ...
            "host_campaign_freeze": dict(host.freeze),
            "captures": results,
        },
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--study", type=Path, default=bench_dir() / "rvv_paper" / "study_v2.yaml")
    parser.add_argument(
        "--host-experiment", type=Path, required=True, help="completed, output-sealed CPU-host campaign YAML"
    )
