...
def _cell_timeout_seconds(cell: MatrixCell) -> int:
    ...
    if not _MIN_CELL_TIMEOUT_SECONDS <= value <= _MAX_CELL_TIMEOUT_SECONDS:
        raise ValueError(f"timeout must be in [{_MIN_CELL_TIMEOUT_SECONDS}, {_MAX_CELL_TIMEOUT_SECONDS}] seconds")
    return value
...
def execution_matrix(spec: PaperStudySpec) -> tuple[MatrixCell, ...]:
    ...
    for block in sorted(blocks, key=lambda value: digest("block:" + ":".join(map(str, value)))):
        cells = sorted(
            blocks[block], key=lambda cell: digest("backend:" + ":".join(map(str, block)) + ":" + cell.backend.name)
        )
        ordered.extend(cells)
    if len(ordered) != len(matrix) or {cell.key for cell in ordered} != set(keys):
...
def _external_python(cell: MatrixCell) -> _PinnedPython:
    ...
    try:
        ...
        os.close(fd)
    if actual != expected:
        raise ValueError(f"python executable digest mismatch: expected={expected} actual={actual}")
...
def _stage_external_python(cell: MatrixCell, private_dir: Path) -> tuple[_PinnedPython, Path, int]:
    ...
    try:
        with os.fdopen(os.dup(source_fd), "rb") as source, os.fdopen(os.dup(destination_fd), "wb") as destination:
...
def _framework_package(cell: MatrixCell) -> tuple[str, str]:
    ...
    if not isinstance(row, dict):
        raise ValueError(f"external runtime package is absent for {cell.model.name}/{cell.precision}")
    path, digest = str(row.get("path", "")), str(row.get("sha256", ""))
    if not path or not digest:
        raise ValueError(f"external runtime package path/digest is absent for {cell.model.name}/{cell.precision}")
    return path, digest


def _audit_framework_package(cell: MatrixCell, package_value: str, package_digest: str) -> list[str]:
    ...
    if package_value == "unresolved" or not package_path.is_dir():
        return [f"frozen session package is absent for {cell.model.name}/{cell.precision}: {package_value}"]
    ...
    try:
        ...
        if frozen.model != cell.model.capture or frozen.variant != cell.model.artifacts[cell.precision]["variant"]:
            errors.append(f"session package model/variant differs for {cell.model.name}/{cell.precision}")
        if _is_sha256(artifact_digest) and frozen.capture_sha256 != artifact_digest:
            errors.append(f"session package capture digest differs for {cell.model.name}/{cell.precision}")
        capture_path = Path(str(cell.model.artifacts[cell.precision].get("path", "")))
        if not capture_path.is_absolute():
            capture_path = _resolve_path(str(capture_path))
        expected_identity = session_identity_sha256(
            capture_session_identity(load_yaml(capture_path / "session_contract.yaml"))
        )
        if frozen.capture_session_identity_sha256 != expected_identity:
            errors.append(
                f"session package loader/checkpoint/input identity differs for {cell.model.name}/{cell.precision}"
            )
        expected_source = str(cell.backend.options.get("framework_source_sha256", ""))
        if _is_sha256(expected_source) and frozen.framework_source_sha256 != expected_source:
            errors.append(f"session package source digest differs for {cell.model.name}/{cell.precision}")
        ...
        if not _is_sha256(expected_environment) or frozen.build_environment_sha256 != expected_environment:
            errors.append(f"session package build-environment digest differs for {cell.model.name}/{cell.precision}")
    except Exception as exc:  # noqa: BLE001 — report every preflight blocker
        errors.append(f"invalid frozen session package for {cell.model.name}/{cell.precision}: {exc}")
    return errors


def _capture_contract(
    spec: PaperStudySpec, cell: MatrixCell, *, verify_digest: bool = True
) -> tuple[Path, dict, list[str]]:
    artifact = cell.model.artifacts[cell.precision]
    capture = (
        Path(artifact["path"]) if artifact.get("path") else bundle.resolve(cell.model.capture, artifact["variant"]).root
    )
    errors: list[str] = []
    ...
    session, session_errors = validate_capture_session(
        capture, cell.model.session, expected_provenance=cell.model.expected_provenance
    )
    ...
    if session.get("paper_ready") is True and captured_checkpoint != cell.model.checkpoint:
        errors.append(f"session checkpoint differs: study={cell.model.checkpoint!r} capture={captured_checkpoint!r}")
...
def environment_preflight(spec: PaperStudySpec) -> Preflight:
    ...
    if not paper_inputs.is_dir():
        ...
        if actual_inputs != spec.paper_inputs["sha256"]:
            errors.append(
                f"paper input bundle digest mismatch: spec={spec.paper_inputs['sha256']} actual={actual_inputs}"
            )
        else:
    ...
    if not authority_value or not authority_path.is_file():
        ...
        try:
            ...
            load_toolchain_authority(authority_path, expected_sha256=authority_digest, expected_target=spec.target)
    ...
    for cell in spec.matrix():
        ...
        if cell.backend.adapter == "merlin_compile":
            ...
            elif _is_sha256(options.get("package_sha256")) and sha256_paths([package]) != options["package_sha256"]:
                blockers.append(f"{cell.backend.name}: compiler package digest mismatch")
        elif cell.backend.adapter == "executorch":
            command = options.get("command")
            if not isinstance(command, list) or not command:
                blockers.append(f"{cell.backend.name}: no stateful ExecuTorch session command is frozen")
                ...
                if command[0] != "{python_executable}":
                    blockers.append(
                        f"{cell.backend.name}: command must start with the pinned {{python_executable}} placeholder"
                    )
                if (
                    "merlin.baselines.executorch_session" not in command
                    or "run" not in command
                    or "build" in command
                    or "--session-contract" in command
                ):
                    blockers.append(
                        f"{cell.backend.name}: paper command must use the frozen-package ExecuTorch run-only adapter"
                    )
                present = {
                    field for field in _EXECUTORCH_COMMAND_FIELDS if any(f"{{{field}}}" in part for part in command)
                }
                missing_fields = sorted(_EXECUTORCH_COMMAND_FIELDS - present)
                if missing_fields:
                    blockers.append(f"{cell.backend.name}: stateful command omits placeholders {missing_fields}")
            try:
            ...
            try:
                ...
                if package_key not in checked_framework_packages:
                    ...
                    blockers.extend(
                        f"{cell.backend.name}: {problem}"
                        for problem in _audit_framework_package(cell, package_value, package_digest)
                    )
        else:
        ...
        digest_key = (
            ...
            if cell.backend.kind == "external_runtime"
            else None
        )
        if digest_key is not None:
            if not isinstance(source_values, list) or not source_values:
                ...
                if missing_sources:
                    blockers.append(f"{cell.backend.name}: frozen source paths are absent: {missing_sources}")
...
def _base_result(spec: PaperStudySpec, cell: MatrixCell, run_id: str, timestamp: str, git_sha: str) -> dict:
    artifact = cell.model.artifacts[cell.precision]
    ...
    model_session_identities = (
        session_identities.get(cell.model.name, {}) if isinstance(session_identities, dict) else {}
    )
    capture_session_identity = (
        model_session_identities.get(cell.precision) if isinstance(model_session_identities, dict) else None
    )
    return {
        "schema_version": 2,
        "run_id": run_id,
        ...
        "checkpoint": cell.model.checkpoint,
        "artifact_sha256": artifact["sha256"],
        "fidelity": cell.model.fidelity,
        "backend": cell.backend.name,
        "runtime": cell.backend.runtime,
        "precision": cell.precision,
        "quantization": cell.backend.quantization,
        ...
        "quality": {"gate_ok": False, "metric": cell.model.quality["metric"], "value": None},
        "timing": {
            "unit": "ns",
            "sample_unit": "complete_session",
            "scope": timing_scope,
...
def _merge_external_result(result: dict, external: object, cell: MatrixCell) -> None:
    ...
    for name in _EXTERNAL_IDENTITY & set(external):
        if external[name] != result[name]:
            raise ValueError(
                f"external adapter identity mismatch for {name}: expected={result[name]!r} got={external[name]!r}"
            )
...
def execute_cell(
    ...
    *,
    ...
    deadline_ns = int(deadline_ns) if deadline_ns is not None else time.monotonic_ns() + timeout_seconds * 1_000_000_000
    evidence = evidence_sink if evidence_sink is not None else {}
    evidence.update(
        adapter_output=None, stdout="", stderr="", adapter_command=["paper-cell-adapter", "--cell", cell.key]
    )
    capture, session_contract, capture_errors = _capture_contract(spec, cell)
    if capture_errors:
        result["lifecycle"]["reason"] = "; ".join(capture_errors)
        evidence.update(adapter_output=None, stdout="", stderr="", adapter_command=["capture-preflight", cell.key])
        validate_paper_result(result)
        return result
    validated_v2_session = int(session_contract.get("version", 0) or 0) == 2
    if not validated_v2_session:
        result["lifecycle"]["reason"] = "paper measurement requires an executed, validated version-2 session contract"
        evidence.update(
            adapter_output=None, stdout="", stderr="", adapter_command=["session-contract", "--required-version", "2"]
        )
        validate_paper_result(result)
        return result
    ...
    try:
        if cell.backend.adapter == "merlin_compile":
            ...
            try:
                with contextlib.redirect_stdout(stdout_buffer), contextlib.redirect_stderr(stderr_buffer):
                    compiled = compile_rvv(
                        cell.model.capture,
                        "int8" if cell.precision == "w8a8" else "fp32",
                        run="k1",
                        verify=True,
                        ...
                        harts=cell.core_count,
                        iters=cell.model.session.observations,
            ...
            result["timing"] = {
                ...
                "median": wall_stats.get("median"),
                "p95": wall_stats.get("p95"),
                "drift": wall_stats.get("drift"),
            }
            result["memory"] = {
                "policy": compiled.get("memory_policy"),
            ...
            backend_identity_ok = (
                "kernel_backend" in raw_execution and raw_execution.get("kernel_backend") == expected_backend
            )
            ...
            if cell.backend.kind in {"kernel_swap", "frozen_baseline"}:
                result["provenance"]["kernel_source_sha256"] = options.get("kernel_source_sha256")
        elif cell.backend.adapter == "executorch":
            ...
            pinned, staged, execute_fd = _stage_external_python(cell, Path(private_dir) / "python-runtime")
            command = [
                str(part).format(
                    python_executable=str(pinned.argv0),
            ...
            try:
                ...
                try:
                    proc = subprocess.run(
                        command,
                        executable=f"/proc/self/fd/{execute_fd}",
                        pass_fds=(execute_fd,),
                        capture_output=True,
                        text=True,
                        timeout=_remaining_seconds(deadline_ns),
    ...
    treatment_applied = cell.backend.kind != "kernel_swap" or (
        isinstance(routed, int)
        ...
        and eligible > 0
        and routed == eligible
        and candidates >= eligible
    )
    if cell.backend.kind == "kernel_swap" and not treatment_applied and not result["lifecycle"].get("reason"):
        result["lifecycle"]["reason"] = (
    ...
    expected_worker_source = "extension_threadpool_no_pool_guard" if cell.core_count == 1 else "extension_threadpool"
    worker_threads_ok = cell.backend.kind != "external_runtime" or (
        int(execution.get("worker_threads") or 0) == cell.core_count
        and execution.get("worker_thread_source") == expected_worker_source
    )
    if result["lifecycle"].get("ran") and not worker_threads_ok and not result["lifecycle"].get("reason"):
        result["lifecycle"]["reason"] = "ExecuTorch worker-thread configuration differs from the requested core count"
    board_conditions_ok = _board_conditions_locked(result["provenance"].get("board_conditions"))
    if result["lifecycle"].get("ran") and not board_conditions_ok and not result["lifecycle"].get("reason"):
        result["lifecycle"]["reason"] = (
    ...
    if result["lifecycle"].get("ran") and not semantic_session_ok and not result["lifecycle"].get("reason"):
        result["lifecycle"]["reason"] = (
            "execution evidence does not prove a version-2 semantic session with distinct per-observation inputs"
        )
    passed = (
        result["lifecycle"]["ran"]
        and result["correctness"].get("gate_ok") is True
        and result["quality"].get("gate_ok") is True
        and result["timing"].get("samples")
    ...
    if result["lifecycle"]["status"] != "error":
        result["lifecycle"]["status"] = "pass" if passed else ("fail" if result["lifecycle"]["ran"] else "not_run")
    validate_paper_result(result)
    return result


def _execute_live_cell(spec: PaperStudySpec, cell: MatrixCell, *, index: int, out_dir: Path, parent_handle) -> dict:
    ...
    child_dir = (
        out_dir / "cell-runs" / f"{index:03d}_{cell.model.name}_{cell.backend.name}_{cell.precision}_{cell.core_count}c"
    )
    staging_dir = (
        out_dir / "controller-contracts" / f"{index:03d}_{cell.model.name}_{cell.backend.name}_{cell.precision}_"
        f"{cell.core_count}c"
    )
    from .paper_contract_registry import build_registered_contract

    expected = _base_result(spec, cell, run_id, parent_handle.timestamp, parent_handle.git_sha)
    contract_path = build_registered_contract(
        spec,
        cell,
        run_id=run_id,
        timestamp=parent_handle.timestamp,
        git_sha=parent_handle.git_sha,
        staging_dir=staging_dir,
        base_result=expected,
    )
    ...
    if any(result["provenance"].get(key) != value for key, value in frozen_provenance.items()):
    ...
    matrix = [
        {
            ...
            "session": c.model.session.kind,
        }
        for index, c in enumerate(ordered_cells)
    ]
    write_yaml(out_dir / "matrix.yaml", {"n_cells": len(matrix), "cells": matrix})
    lines = [
        f"# {spec.label}",
        "",
        f"Study SHA-256: `{spec.sha256()}`",
        "",
...
def run(
    spec: PaperStudySpec, *, live: bool = False, environment_audit: bool = False, out_dir: Path | None = None
) -> Path:
    ...
    if live:
        handle = start_run(
            suite="paper-study",
            method="frozen-compiler",
            target=spec.target,
            extra={
                "study_sha256": spec.sha256(),
                "n_cells": len(spec.matrix()),
                "results_dir": (str(explicit_out) if explicit_out else "canonical_aet_run"),
            },
        )
        if out_dir is None:
        ...
        product = new_product(
            "paper-study", version=2, target=spec.target, sources=[str(spec.source_path)] if spec.source_path else []
        )
    ...
    try:
        ...
        for index, cell in enumerate(execution_matrix(spec)):
            result = _execute_live_cell(spec, cell, index=index, out_dir=out_dir, parent_handle=handle)
        ...
        (out_dir / "paper-results.md").write_text(render_markdown(paper_report), encoding="utf-8")
