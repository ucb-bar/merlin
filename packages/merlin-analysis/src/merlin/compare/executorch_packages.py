...
def _external_backend(study: PaperStudySpec) -> BackendSpec:
    ...
    if len(backends) != 1:
        raise ValueError(f"paper packaging requires exactly one external runtime, found {len(backends)}")
    backend = backends[0]
    if (
        backend.name != "executorch_xnnpack"
        or backend.runtime != "executorch"
        or backend.adapter != "executorch"
        or backend.precisions != ("fp32",)
        or backend.options.get("xnnpack") is not True
    ):
        raise ValueError("external runtime must be executorch_xnnpack/executorch, XNNPACK-enabled, FP32-only")
    return backend


def _registration(
    path: Path, study_path: Path, study_sha256: str, study: PaperStudySpec
    ...
    if registered_study != study_path:
        errors.append(f"capture registration names another staged study: {registered_study}")
    expected_study_sha = str(raw.get("study_sha256", ""))
    if not _is_sha256(expected_study_sha) or expected_study_sha != study_sha256:
        errors.append(
            f"capture registration staged-study digest differs: registration={expected_study_sha} actual={study_sha256}"
        )
    ...
    if not isinstance(rows, list):
        return raw, registration_sha256, [*errors, "capture registration captures must be a list"]
    ...
    for model in study.models:
        for precision, artifact in model.artifacts.items():
            ...
            if actual_path != expected_path or row.get("sha256") != artifact.get("sha256"):
                errors.append(f"capture registration path/digest differs for {model.name}/{precision}")
    return raw, registration_sha256, errors


def _paper_input_environments(
    study: PaperStudySpec,
) -> tuple[dict[str, dict[str, str]], list[str], Path, dict[str, Any]]:
    errors: list[str] = []
    ...
    if not _is_sha256(expected_digest) or actual_digest != expected_digest:
        errors.append(f"paper input digest differs: study={expected_digest} actual={actual_digest}")
...
def _compiler_identity(path: Path) -> dict[str, str]:
    ...
    version = subprocess.run([str(path), "--version"], capture_output=True, text=True, timeout=30)
...
def _toolchain_identity(root: str | Path | None = None) -> dict[str, Any]:
    ...
    if not requested:
        raise ValueError("paper package preflight requires explicit MERLIN_K1_TOOLCHAIN or --k1-toolchain")
    candidate = Path(requested).resolve()
    from merlin.baselines import toolchain_locator

    found = toolchain_locator.find_prefix(candidate, "MERLIN_K1_TOOLCHAIN", tools=("clang", "clang++"))
    resolved = found.resolve() if found is not None else None
    if resolved is None:
        raise ValueError(f"explicit SpacemiT toolchain contains no clang/clang++ prefix: {candidate}")
    return {
        "root": str(resolved),
...
def _git_sha(root: Path, where: str) -> str:
    revision = subprocess.run(["git", "-C", str(root), "rev-parse", "HEAD"], capture_output=True, text=True, timeout=30)
...
def _require_external_source_closure(checkout: Path, source_root: Path, source_file: Path) -> None:
    ...
    try:
        ...
        raise ValueError("external model source escapes its declared source root/checkout") from exc
    if not source_root.is_dir() or not source_file.is_file():
        raise FileNotFoundError(f"external model source closure is absent: root={source_root} file={source_file}")
    for path in sorted(source_root.rglob("*"), key=lambda candidate: candidate.as_posix()):
        ...
        try:
            ...
            raise ValueError(f"external model source nested symlink cannot be resolved: {path}") from exc
        try:
            target.relative_to(source_root)
            target.relative_to(checkout)
        except ValueError as exc:
            raise ValueError(
                f"external model source nested symlink escapes its declared source root/checkout: {path} -> {target}"
            ) from exc


def _external_model_sources(
    backend: BackendSpec, environments: dict[str, dict[str, str]], records: dict[str, Any]
    ...
    for model_name, value in declarations.items():
        ...
        if (
            ...
            or str(checkpoint.get("source_file_sha256", "")) != identity["source_file_sha256"]
        ):
            errors.append(f"{model_name}: external model source differs from paper-input provenance")
        identities[model_name] = identity
    return identities, errors


def _exact_environment(
    model_environment: dict[str, str], model2mlir: Path, toolchain: dict[str, Any]
) -> dict[str, str]:
...
def _model2mlir_identity(root: Path, study: PaperStudySpec) -> tuple[dict[str, Any], list[str]]:
    ...
    loaders = {model.name: root / "workloads" / model.capture / "loader.py" for model in study.models}
    if not root.is_dir():
    ...
    return {
        "path": str(root),
        "git_sha": git_sha,
        "capture_source_sha256": (sha256_paths([capture_source]) if capture_source.is_dir() else "unresolved"),
        "loader_sha256": {
            name: sha256_paths([path]) if path.is_file() else "unresolved" for name, path in loaders.items()
        },
    }, errors


def _child_environment(exact: dict[str, str]) -> dict[str, str]:
    inherited = {key: value for key, value in os.environ.items() if key in _ALLOWED_INHERITED_ENV_KEYS}
...
def _tasks(
    ...
    model2mlir: Path,
    product: ProductDir,
    toolchain_root: str | Path | None,
) -> tuple[list[PackageTask], list[str], dict[str, Any]]:
    errors: list[str] = []
    ...
    environments, environment_errors, paper_inputs, paper_input_records = _paper_input_environments(study)
    ...
    try:
        ...
        toolchain_identity = {}
        errors.append(str(exc))
    external_sources, external_source_errors = _external_model_sources(backend, environments, paper_input_records)
    ...
    for model in study.models:
        for precision, artifact in model.artifacts.items():
            ...
            if actual_digest != expected_digest:
                errors.append(
                    f"{model.name}/{precision}: capture digest differs: staged={expected_digest} actual={actual_digest}"
                )
    ...
    for model in study.models:
        ...
        if actual_digest != capture_digest:
            errors.append(f"{model.name}/fp32: capture digest differs: staged={capture_digest} actual={actual_digest}")
        ...
        elif str(package.get("path", "")) != "unresolved" or str(package.get("sha256", "")) != "unresolved":
            errors.append(f"{model.name}/fp32: package is already registered; immutable packages are not rebuilt")
        environment = _child_environment(
            _exact_environment(environments.get(model.name, {}), model2mlir, toolchain_identity)
        )
        ...
        command = (
            sys.executable,
            "-m",
            "merlin.baselines.executorch_session",
            "build",
            "--model",
            model.capture,
            "--variant",
            "fp32",
            "--session-contract",
            str(capture_path / "session_contract.yaml"),
            "--warmups",
            str(model.session.warmups),
            "--observations",
            str(model.session.observations),
            "--measurement-repeats",
            str(model.session.measurement_repeats),
            "--framework-source-sha256",
            framework_digest,
            "--build-invocation-environment-sha256",
            _json_sha256(environment),
            "--external-model-source-spec-json",
            json.dumps(source_declarations.get(model.name), sort_keys=True, separators=(",", ":")),
            "--output",
            str(output),
            "--work",
            str(work),
        ...
        task_model2mlir_identity = {
            ...
            "loader_sha256": str((model2mlir_identity.get("loader_sha256") or {}).get(model.name, "")),
            "capture_source_sha256": str(model2mlir_identity.get("capture_source_sha256", "")),
        }
        tasks.append(
            PackageTask(
                model,
                ...
                identity,
                task_model2mlir_identity,
                copy.deepcopy(toolchain_identity),
...
def _default_runner(task: PackageTask, environment: dict[str, str], stdout_path: Path, stderr_path: Path) -> int:
    with stdout_path.open("w", encoding="utf-8") as stdout, stderr_path.open("w", encoding="utf-8") as stderr:
        proc = subprocess.run(list(task.command), cwd=repo_root(), env=environment, stdout=stdout, stderr=stderr)
...
def _validate_package(task: PackageTask) -> dict[str, Any]:
    ...
    expected = {
        ...
        "capture_session_identity_sha256": (
            package.capture_session_identity_sha256,
            task.capture_session_identity_sha256,
        ),
        "framework_source_sha256": (package.framework_source_sha256, task.framework_source_sha256),
        "build_invocation_environment_sha256": (
            package.build_invocation_environment_sha256,
            _json_sha256(task.environment),
        ),
        ...
        "measurement_repeats": (package.plan.repeats, task.model.session.measurement_repeats),
    }
    mismatches.extend(
    ...
    embedded_identity = environment.get("executorch_identity") if isinstance(environment, dict) else None
    if embedded_identity != task.executorch_identity:
        mismatches.append("embedded ExecuTorch exporter/runtime identity differs from package preflight")
    ...
    if isinstance(environment, dict):
        ...
        if embedded_model2mlir != task.model2mlir_identity:
            mismatches.append("embedded Model2MLIR identity differs from package preflight")
        ...
        if embedded_external != task.external_model_source:
            mismatches.append("embedded external model source differs from package preflight")
        if package.external_model_source != embedded_external:
            mismatches.append("loaded external model source differs from embedded metadata")
    else:
    ...
    producer = executorch_session_resources(task.output / executorch_session.PAPER_COMPILER_INPUT, include_private=True)
    ...
    return {
        ...
        "capture": task.model.capture,
        "precision": "fp32",
        "variant": "fp32",
        "xnnpack": package.xnnpack,
        ...
        "build_invocation_environment_sha256": package.build_invocation_environment_sha256,
...
def _paper_measurement_templates(
    ...
    for model in study.models:
        ...
        receipt_path = product.add_artifact(f"measurement-resources/{model.name}/package-receipt.json")
        ...
        with tempfile.TemporaryDirectory(prefix="merlin-executorch-receipt-") as temporary:
            ...
            regeneration = regenerate_model_object(
                recipe=EXECUTORCH_RECIPE,
                registry_id="executorch_v1",
                target=study.target,
        ...
        receipt = {
            "schema_version": 2,
            "kind": "paper_backend_package_receipt_v2",
            "status": "finalized",
            "registry_id": "executorch_v1",
        ...
        for cores in study.core_counts:
            template_path = product.add_artifact(f"measurement-template-{model.name}-fp32-c{cores}.yaml")
            template = {
                "schema_version": 3,
                "kind": "paper_backend_measurement_template_v3",
                "status": "frozen",
                "registry_id": "executorch_v1",
                "backend_adapter": "executorch",
                "cell": {"model": model.name, "backend": backend.name, "precision": "fp32", "core_count": cores},
                "resources": {
                    "package_receipt": ref(receipt_path),
                    "compiler_input": ref(compiler_input),
                    "model_object": ref(runner),
                    "build_tool": ref(tool),
                    "runtime_artifact": ref(runtime_artifact),
                },
                "environment": {},
                "execution": {"mode": "rvv", "core_ids": list(range(cores)), "require_worker_threads": True},
                "memory_policy": "mmap",
                "timeout_seconds": backend.options["timeout"],
            }
...
def _prepublication_reobserve(
    ...
    if fresh_study_sha256 != study_sha256:
        errors.append(f"study changed before publication: preflight={study_sha256} actual={fresh_study_sha256}")
    try:
        backend = _external_backend(fresh_study)
        fresh_tasks, fresh_errors, fresh_evidence = _tasks(
            fresh_study, fresh_study_sha256, backend, registration_path, model2mlir, product, toolchain_root
        )
    except (OSError, ValueError) as exc:
        return [*errors, f"preflight input changed before publication: {exc}"]
    errors.extend(f"preflight input changed before publication: {error}" for error in fresh_errors)
...
def materialize(
    study_path: str | Path,
    capture_registration: str | Path | None,
    model2mlir_root: str | Path,
    *,
    execute: bool = False,
    ...
    try:
        ...
        tasks, task_errors, evidence = _tasks(
            study, study_sha256, backend, registration_path, model2mlir, product, k1_toolchain
        )
    ...
    plan: dict[str, Any] = {
        ...
        "study": {
            ...
            "sha256": study_sha256,
            "status": study.status,
            "holdout_models": list(study.holdout_models),
        },
        "environment_policy": {
    ...
    reobservation_errors = _prepublication_reobserve(
        study_path, study_sha256, registration_path, model2mlir, product, k1_toolchain, tasks, evidence
    )
    ...
    write_yaml(
        staged_temp, staged, header=("Capture- and ExecuTorch-package-complete draft; pass to merlin-compare --freeze")
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--study", type=Path, required=True, help="capture-complete staged-study.yaml")
    parser.add_argument(
        "--capture-registration", type=Path, default=None, help="defaults to capture-registration.json beside --study"
    )
    parser.add_argument("--model2mlir", type=Path, default=capture_bundle.model2mlir_root())
    parser.add_argument(
        "--k1-toolchain", type=Path, help="exact SpacemiT compiler prefix; otherwise explicit MERLIN_K1_TOOLCHAIN only"
    )
    parser.add_argument(
        "--execute", action="store_true", help="build all five packages; default only writes a timestamped preflight"
    )
    args = parser.parse_args(argv)
    try:
        output = materialize(
            args.study, args.capture_registration, args.model2mlir, execute=args.execute, k1_toolchain=args.k1_toolchain
        )
