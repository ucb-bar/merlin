...
def _controller_pythonpath(remote_controller: str) -> str:
    from merlin.common.access import PYTHON_SOURCE_ROOTS

    roots = ["src"] + [str(Path(source.path).parent) for source in PYTHON_SOURCE_ROOTS]
    return ":".join(f"{remote_controller}/repo/{root}" for root in dict.fromkeys(roots))
...
def _controller_rows(root: Path) -> list[dict[str, Any]]:
    ...
    prefixes = [(Path("merlin/schemas"), False)]
    seen = set()
    for declared in PYTHON_SOURCE_ROOTS:
        source = root / declared.path
        if source.is_dir() and source.resolve() not in seen and any(source.rglob("*.py")):
            # Prefer canonical roots over a compatibility link when both exist.
            if source.is_symlink():
                continue
            prefixes.append((Path(declared.path), True))
    ...
    required = {
        "paper_measurement_controller.py",
        "paper_k1_orchestrator.py",
        "paper_model_abi_runner.c",
        "paper_k1_board_probe.c",
    }
    if not all(any(row["path"].endswith("/compare/" + name) for row in rows) for name in required):
...
def _contract_build_tool(root: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or _sha_file(path) != reference["sha256"]
    ):
    ...
    return {"sha256": reference["sha256"], "size": path.stat().st_size, "elf_machine": machine}


def _runtime_requirements(cells: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    tools = {
        (row["build_tool"]["sha256"], row["build_tool"]["size"], row["build_tool"]["elf_machine"]) for row in cells
    }
    required_cores = max(int(row["cell"]["core_count"]) for row in cells)
    return {
        "required_core_count": required_cores,
        "build_tools": [{"sha256": sha, "size": size, "elf_machine": machine} for sha, size, machine in sorted(tools)],
    }


def prepare_contract_matrix(study_path: str | Path, *, output_dir: str | Path | None = None) -> Path:
    ...
    handle = start_run(
        suite="paper-study",
        method="frozen-contract-preparation",
        target="k1",
        extra={
            "study_sha256": spec.sha256(),
            "study_path": str(source),
            "contract_only": True,
            "n_cells": len(spec.matrix()),
        },
    )
    ...
    try:
        ...
        for index, cell in enumerate(execution_matrix(spec)):
            ...
            name = _safe_name(f"{index:03d}_{cell.model.name}_{cell.backend.name}_{cell.precision}_{cell.core_count}c")
        ...
        plan_path = create_matrix_plan(contract_paths, destination / "k1-matrix-plan.json", controller_root=repo_root())
        plan = _load_plan(plan_path)
        if [row["run_id"] for row in plan["cells"]] != [row["run_id"] for row in rows] or [
            row["execution_index"] for row in plan["cells"]
        ] != list(range(len(rows))):
            raise ValueError("transport plan changed the frozen study execution order")
        _atomic_yaml(
            destination / "prepared-matrix.yaml",
            {
                "schema_version": 1,
                "kind": _PREPARED_KIND,
                "status": "complete",
                "study": {"path": str(retained_study), "file_sha256": source_sha, "canonical_sha256": spec.sha256()},
                "aet_parent": {
                    "run_id": handle.run_id,
                    "timestamp": handle.timestamp,
                    "git_sha": handle.git_sha,
        ...
        status = "ok"
        summary = {"prepared": True, "n_cells": len(rows), "matrix_sha256": plan["matrix_sha256"]}
        return destination
    finally:
...
def create_matrix_plan(
    contract_paths: Sequence[str | Path], output_path: str | Path, *, controller_root: str | Path | None = None
) -> Path:
    ...
    identity = {
        ...
        "cells": [
            {
                key: row[key]
                for key in ("execution_index", "run_id", "cell", "contract_sha256", "tree_sha256", "build_tool")
            }
            for row in cells
        ],
    }
    plan = {
        ...
        "study_sha256": study_sha,
        "matrix_sha256": _canonical_sha(identity),
        "runtime_requirements": runtime_requirements,
        "controller": {
            "root": str(source_root),
            "tree_sha256": identity["controller_tree_sha256"],
...
def _load_plan(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or value["kind"] != _PLAN_KIND
        or value["status"] != "frozen"
        or not _is_sha256_hex(str(value["study_sha256"]))
        or not _is_sha256_hex(str(value["matrix_sha256"]))
        or not isinstance(value["runtime_requirements"], dict)
        or not isinstance(value["controller"], dict)
        or set(value["controller"]) != {"root", "tree_sha256", "tree_files"}
    ...
    if value["controller"]["tree_sha256"] != _tree_sha(controller_rows) or value["controller"]["tree_files"] != len(
        controller_rows
    ):
    ...
    for index, row in enumerate(value["cells"]):
        ...
        if not contract_path.is_relative_to(root) or contract_path.is_symlink() or not contract_path.is_file():
        ...
        if (
            contract["target"] != "k1"
            or contract["study_sha256"] != value["study_sha256"]
            or contract["run_id"] != row["run_id"]
            or dict(cell) != row["cell"]
            or build_tool != row["build_tool"]
            or _sha_file(contract_path) != row["contract_sha256"]
            or _tree_sha(rows) != row["tree_sha256"]
            or len(rows) != row["tree_files"]
        ...
        identity_cells.append(
            {
                ...
                for key_name in ("execution_index", "run_id", "cell", "contract_sha256", "tree_sha256", "build_tool")
            }
        )
    ...
    if (
        _runtime_requirements(verified_cells) != value["runtime_requirements"]
...
def _write_tar(archive: Path, root: Path, rows: Sequence[Mapping[str, Any]], *, prefix: str) -> str:
    archive.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(archive, "w", format=tarfile.PAX_FORMAT) as stream:
        for row in rows:
            source = root / str(row["path"])
            stat = source.stat()
            if _sha_file(source) != row["sha256"] or stat.st_size != row["size"] or stat.st_mode & 0o777 != row["mode"]:
...
class SSHConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or len(remote_root.parts) < 3
            or ".." in remote_root.parts
            or not remote_python.is_absolute()
            or remote_python.as_posix() != self.remote_python
            or ".." in remote_python.parts
        ):
            raise ValueError("K1 SSH execution requires root, absolute remote paths, and a valid port")
...
class RemoteTransport(Protocol):
    def stage(self, archive: Path, *, archive_sha256: str, tree_sha256: str, kind: str) -> str: ...
...
class K1SSHSystemdTransport:
    ...
    def __init__(self, config: SSHConfig, *, runner: Callable[..., subprocess.CompletedProcess[str]] = subprocess.run):
        self.config = config
        self.runner = runner
    ...
    def _ssh(self, argv: Sequence[str], *, timeout: float = 120) -> subprocess.CompletedProcess[str]:
        return self._run([*self._ssh_base(), shlex.join(list(argv))], timeout=timeout)

    def stage(self, archive: Path, *, archive_sha256: str, tree_sha256: str, kind: str) -> str:
        if (
            kind not in {"controller", "contract"}
            or not _is_sha256_hex(archive_sha256)
            or (not _is_sha256_hex(tree_sha256) or _sha_file(archive) != archive_sha256)
        ):
        ...
        self._ssh(["install", "-d", "-m", "0700", str(incoming_dir), str(destination.parent)], timeout=30)
        remote = f"{self.config.user}@{self.config.host}:{incoming}"
        self._run([*self._scp_base(), str(archive), remote], timeout=1800)
        marker = json.dumps(
            {"archive_sha256": archive_sha256, "tree_sha256": tree_sha256}, sort_keys=True, separators=(",", ":")
        )
        script = """
set -eu
...
""".strip()
    ...
    def environment_preflight(
        ...
        *,
        ...
        pythonpath = _controller_pythonpath(remote_controller)
        argv = [
            "/usr/bin/env",
            ...
            "-m",
            "merlin.compare.paper_k1_orchestrator",
            "remote-preflight",
            "--output",
            remote_output,
            "--matrix-sha256",
            matrix_sha256,
            "--controller-tree-sha256",
            controller_tree_sha256,
            "--runtime-requirements-sha256",
            runtime_requirements_sha256,
            "--required-core-count",
            str(required_core_count),
            "--expected-python",
            self.config.remote_python,
        ]
    ...
    def run_cell(
        ...
        *,
        ...
        argv = [
            ...
            "remote-cell",
            "--contract",
            f"{remote_contract}/contract/measurement_contract.yaml",
            "--output-root",
            remote_run_root,
            "--contract-tree-sha256",
...
""".strip()
        completed = self._ssh(["/bin/sh", "-c", script, "retrieve", remote_run_root], timeout=300)
...
def _terminal_document(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or value["kind"] != _TERMINAL_KIND
        or value["status"] != "terminal"
        or not _is_sha256_hex(str(value["contract_tree_sha256"]))
        or not _is_sha256_hex(str(value["contract_sha256"]))
        or not _is_sha256_hex(str(value["issuance_fingerprint"]))
    ):
        raise ValueError("remote terminal marker is malformed")
    for name in ("receipt", "result"):
        ref = value[name]
        if not isinstance(ref, dict) or set(ref) != {"path", "sha256"} or not _is_sha256_hex(str(ref["sha256"])):
...
def _validate_retrieved(
    transport_root: Path, planned: Mapping[str, Any], *, retrieval_sha256: str, final_dir: Path
) -> dict[str, Any]:
    terminal = _terminal_document(transport_root / "terminal.json")
    if (
        terminal["run_id"] != planned["run_id"]
        or terminal["contract_tree_sha256"] != planned["tree_sha256"]
        or terminal["contract_sha256"] != planned["contract_sha256"]
    ):
    ...
    remote_result = _contained(transport_root, terminal["result"]["path"], label="retrieved result")
    if _sha_file(receipt) != terminal["receipt"]["sha256"] or _sha_file(remote_result) != terminal["result"]["sha256"]:
    ...
    if (
        ...
        or any(result.get(name) != value for name, value in planned["cell"].items())
    ):
...
def _validate_local_terminal(cell_dir: Path, planned: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or state["kind"] != _STATE_KIND
        or state["status"] != "terminal"
        or state["run_id"] != planned["run_id"]
        or state["cell"] != planned["cell"]
        or state["contract_sha256"] != planned["contract_sha256"]
        or state["contract_tree_sha256"] != planned["tree_sha256"]
        or any(
            not _is_sha256_hex(str(state[name]))
            for name in (
                "receipt_sha256",
                "result_sha256",
                "remote_result_sha256",
    ...
    if (
        ...
        or _sha_file(result_path) != state["result_sha256"]
        or result.get("run_id") != planned["run_id"]
    ):
    ...
    if (
        receipt_raw.is_symlink()
        or not receipt.is_relative_to((cell_dir / "transport").resolve())
        or not receipt.is_file()
        or _sha_file(receipt) != state["receipt_sha256"]
        or issuance_fingerprint(receipt) != state["issuance_fingerprint"]
    ):
...
def _mapped_runtime_libraries() -> list[dict[str, Any]]:
    ...
    for line in maps.read_text(encoding="utf-8").splitlines():
        ...
        if ".so" in name or name.startswith("ld-linux"):
            ...
            if path.is_file():
                paths.add(path)
    rows = [{"path": str(path), "sha256": _sha_file(path), "size": path.stat().st_size} for path in sorted(paths)]
    names = {Path(row["path"]).name for row in rows}
    if not any(name.startswith("libc.so") for name in names) or not any(name.startswith("ld-linux") for name in names):
...
def create_remote_environment_receipt(
    ...
    *,
    ...
    modules = {public: _remote_module_identity(public, imported) for public, imported in _REMOTE_MODULES.items()}
    ...
    tools = {name: _remote_file_identity(Path(path), label=name) for name, path in _REMOTE_TOOLS.items()}
    ...
    document = {
        ...
        "runtime": {
            ...
            "procfs_task_state": True,
            "systemd_manager": True,
            "systemd_version_sha256": _sha_bytes(systemd_version.stdout + systemd_version.stderr),
            "openssl_ed25519": True,
            "board_probe_source_sha256": _sha_file(source),
            "board_probe_executable_sha256": probe_sha,
            "board_probe": probe,
    ...
    _atomic_bytes(output, (json.dumps(document, sort_keys=True, separators=(",", ":")) + "\n").encode("utf-8"))
    return output


def _validate_environment_receipt_bytes(
    value: bytes,
    *,
    matrix_sha256: str,
    controller_tree_sha256: str,
    ...
    if (
        ...
        or document["kind"] != _ENVIRONMENT_KIND
        or document["status"] != "ready"
        or document["matrix_sha256"] != matrix_sha256
    ...
    if (
        ...
        or set(python) != file_fields | {"version", "implementation"}
        or not _is_sha256_hex(str(python.get("sha256", "")))
        or not isinstance(python.get("size"), int)
        or python["size"] <= 0
    ):
    ...
    for name, row in modules.items():
        if (
            ...
            or row["name"] != name
            or row["import_name"] != _REMOTE_MODULES[name]
    ...
    runtime_fields = {
        ...
        "procfs_task_state",
        "systemd_manager",
        "systemd_version_sha256",
        "openssl_ed25519",
        "board_probe_source_sha256",
    ...
    if (
        ...
        or runtime["procfs_task_state"] is not True
        or runtime["systemd_manager"] is not True
        or runtime["openssl_ed25519"] is not True
        or any(
            not _is_sha256_hex(str(runtime[name]))
            for name in ("systemd_version_sha256", "board_probe_source_sha256", "board_probe_executable_sha256")
        )
    ...
    if (
        ...
        or probe.get("kind") != "merlin_board_probe_v1"
        or probe.get("vlen_source") != "csr"
        or probe.get("governor") != "performance"
    ):
    ...
    if (
        ...
        or len(frequencies) != required_core_count
        or [row.get("core_id") for row in frequencies] != list(range(required_core_count))
        or any(
            row.get("governor") != "performance" or row.get("current_khz") != row.get("max_khz") for row in frequencies
        )
    ):
...
def _default_output(plan: Mapping[str, Any]) -> Path:
    ...
    return artifacts_dir() / "paper-k1-matrix" / "k1" / f"{stamp}_{str(plan['matrix_sha256'])[:8]}"


def _cell_label(planned: Mapping[str, Any]) -> str:
    cell = planned["cell"]
    return _safe_name(
        f"{planned['execution_index']:03d}_{cell['model']}_{cell['backend']}_{cell['precision']}_{cell['core_count']}c"
    )
    ...
    with lock_path.open("a+b") as lock:
        ...
        if environment_path.exists():
            ...
            if existing_cells.exists() and any(existing_cells.iterdir()):
                raise ValueError("existing terminal cells have no retained prerequisite environment receipt")
            try:
                controller_archive = staging / (f"controller-{plan['controller']['tree_sha256']}.tar")
                controller_archive_sha = _write_tar(controller_archive, controller_root, controller_rows, prefix="repo")
                if _tree_sha(_controller_rows(controller_root)) != plan["controller"]["tree_sha256"]:
                    raise ValueError("controller source changed during environment preflight staging")
                remote_controller = transport.stage(
                    controller_archive,
                    archive_sha256=controller_archive_sha,
                    tree_sha256=plan["controller"]["tree_sha256"],
                    kind="controller",
                )
                remote_matrix = str(PurePosixPath(remote_root) / "runs" / plan["matrix_sha256"])
                ...
                _atomic_json(
                    output / "environment-preflight-failure.json",
                    {
                        "schema_version": 1,
                        "status": "blocked_before_first_cell",
                        "matrix_sha256": plan["matrix_sha256"],
        ...
        for planned in plan["cells"]:
            ...
            try:
                if not attempt["board_terminal_before"]:
                    ...
                    contract_archive_sha = _write_tar(contract_archive, contract_root, contract_rows, prefix="contract")
                    ...
                    remote_contract = transport.stage(
                        contract_archive,
                        archive_sha256=contract_archive_sha,
                        tree_sha256=planned["tree_sha256"],
                        kind="contract",
                    )
                    contract, _ = _contract(contract_root / PurePosixPath(planned["contract_path"]))
                    attempt["systemd"] = transport.run_cell(
                        ...
                        unit_name=(
                            "merlin-paper-" + plan["matrix_sha256"][:10] + "-" + f"{planned['execution_index']:03d}"
                        ),
                ...
                verified = _validate_retrieved(
                    transport_root, planned, retrieval_sha256=retrieval_sha, final_dir=cell_dir
                )
                ...
                _atomic_json(
                    output / "matrix-state.json",
                    {
                        ...
                        "status": "incomplete",
                        "matrix_sha256": plan["matrix_sha256"],
                        "terminal_cells": len(fingerprints),
                        "expected_cells": len(plan["cells"]),
                        "environment_preflight_sha256": environment_sha,
                    },
                )
...
def finalize_matrix(
    plan_path: str | Path, run_dir: str | Path, study_path: str | Path, *, output_path: str | Path | None = None
) -> Path:
    ...
    if not retained_plan.is_file() or retained_plan.is_symlink() or _sha_file(retained_plan) != plan_file_sha:
    ...
    if (
        not notary_path.is_file()
        or notary_path.is_symlink()
    ...
    fingerprints = load_issuance_notary(notary_path, expected_study_sha256=spec.sha256())
    ...
    if (
        len(set(expected_labels)) != len(expected_labels)
    ...
    for planned, label in zip(plan["cells"], expected_labels):
        ...
        if (
            set(cell_entries) != {"terminal-state.json", "result.yaml", "transport"}
        ...
        key = (result.get("model"), result.get("backend"), result.get("precision"), result.get("core_count"))
        if (
            not isinstance(identity, Mapping)
            or set(identity) != identity_fields
            or any(result.get(name) != value for name, value in identity.items())
            or dict(contract_cell) != planned["cell"]
            or key
            != (
                planned["cell"]["model"],
                planned["cell"]["backend"],
                planned["cell"]["precision"],
                planned["cell"]["core_count"],
            )
            or result.get("run_id") != run_id
            or result.get("artifact_sha256") != contract.get("artifact_sha256")
            or result.get("session") != contract.get("session")
            or not isinstance(provenance, Mapping)
            or any(provenance.get(name) != value for name, value in contract.get("frozen_provenance", {}).items())
        ):
    ...
    for result in results:
        validate_paper_result(result)
    sealed = seal_results_document(spec, results, trusted_issuance_fingerprints=fingerprints)
    ...
    build_paper_report(spec, sealed, trusted_issuance_fingerprints=fingerprints)
    ...
    if output.is_symlink() or output.exists() or finalization_path.exists() or finalization_path.is_symlink():
    ...
    finalization = {
        "schema_version": 1,
        "kind": _FINALIZATION_KIND,
        "status": "complete",
        "finalized_at_utc": datetime.now(timezone.utc).isoformat(),
        "study_file_sha256": study_file_sha,
        "study_sha256": spec.sha256(),
...
def _validate_remote_terminal(
    root: Path, *, contract: Mapping[str, Any], contract_tree_sha256: str, contract_sha256: str
) -> dict[str, Any]:
    terminal = _terminal_document(root / "terminal.json")
    if (
        terminal["run_id"] != contract["run_id"]
        or terminal["contract_tree_sha256"] != contract_tree_sha256
        or terminal["contract_sha256"] != contract_sha256
    ):
    ...
    if (
        _sha_file(receipt) != terminal["receipt"]["sha256"]
        or _sha_file(result) != terminal["result"]["sha256"]
        or issuance_fingerprint(receipt) != terminal["issuance_fingerprint"]
    ):
    ...
    if (
        ...
        or result_value.get("run_id") != contract["run_id"]
        or any(result_value.get(name) != value for name, value in contract["cell"].items())
    ):
        raise ValueError("existing remote result differs from the frozen contract cell")
    return terminal


def run_remote_cell(contract_path: str | Path, output_root: str | Path, *, contract_tree_sha256: str) -> Path:
    ...
    if (
        root_unresolved.is_symlink()
        or not pure_root.is_absolute()
        or ".." in pure_root.parts
        or "runs" not in pure_root.parts
        or len(pure_root.parts) < 5
    ):
    ...
    if terminal_path.exists():
        _validate_remote_terminal(
            root, contract=contract, contract_tree_sha256=contract_tree_sha256, contract_sha256=_sha_file(contract_path)
        )
    ...
    if result.get("run_id") != contract["run_id"] or any(result.get(name) != value for name, value in cell.items()):
    ...
    terminal = {
        ...
        "result": {"path": result_path.relative_to(root).as_posix(), "sha256": _sha_file(result_path)},
        "issuance_fingerprint": fingerprint,
...
def main(argv: list[str] | None = None) -> int:
    ...
    prepare = actions.add_parser("prepare", help="materialize the frozen matrix contracts without executing them")
    ...
    finalize = actions.add_parser("finalize", help="ingest a complete retrieved matrix and seal canonical results")
    ...
    remote_preflight = actions.add_parser("remote-preflight", help="board-only dependency/environment preflight")
    ...
    if arguments.action == "plan":
        print(create_matrix_plan(arguments.contract, arguments.output, controller_root=arguments.controller_root))
        return 0
    if arguments.action == "finalize":
        print(finalize_matrix(arguments.plan, arguments.run_dir, arguments.study, output_path=arguments.output))
        return 0
    if arguments.action == "remote-cell":
        print(
            run_remote_cell(
                arguments.contract, arguments.output_root, contract_tree_sha256=arguments.contract_tree_sha256
            )
        )
        return 0
    if arguments.action == "remote-preflight":
        print(
            create_remote_environment_receipt(
                arguments.output,
