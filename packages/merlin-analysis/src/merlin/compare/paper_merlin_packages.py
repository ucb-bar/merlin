...
_BUILD_ARGV = ["{tool}", "-O2", "-std=c11", "{source:runner}", "{source:model_object}", "-o", "{output}"]
_HEX = frozenset("0123456789abcdef")


def _compiler_source_identity() -> str:
    """Freeze the split implementation, not just the legacy core symlink.

    This producer already requires a source checkout. Never hash an entire installed
    site-packages directory as an accidental fallback, and never rewrite old receipts.
    """
    root = repo_root().resolve()
    sources = python_import_roots()
    if not (root / "src/merlin/__init__.py").is_file() or any(
        not source.resolve().is_relative_to(root) for source in sources
    ):
        raise ValueError("producer source identity requires the active Merlin source checkout")
    metadata = [root / "pyproject.toml", *sorted((root / "packages").glob("*/pyproject.toml"))]
    return sha256_paths([*sources, *metadata])
...
def issue_toolchain_authority(
    *, output: str | Path, authority_id: str, target: str, build_tool: str | Path
) -> tuple[Path, Path]:
    ...
    authority = write_toolchain_authority(
        output, authority_id=authority_id, target=target, build_tool=Path(build_tool).resolve()
    )
...
def _verify_authority_receipt(authority_path: Path, receipt_path: Path, *, target: str) -> Mapping[str, Any]:
    authority = load_toolchain_authority(authority_path, expected_target=target)
    receipt = _closed(
        _load_json(receipt_path, "toolchain authority issuance receipt"),
        {
            "schema_version",
            "kind",
            "status",
...
def _validate_registered_captures(study: PaperStudySpec, registration: Mapping[str, Any]) -> None:
    ...
    for model in study.models:
        for precision in model.precisions:
            ...
            if (
                ...
                raise ValueError(f"capture registration/study bytes differ for {model.name}/{precision}")


def _validate_promoted_campaign_binding(registration: Mapping[str, Any], promoted: Path) -> None:
    ...
    if (
        ...
        or not policy.is_relative_to(promoted)
...
def _backend_identity(cell: MatrixCell, promoted: Path) -> dict[str, Any]:
    if cell.backend.name == "merlin_frozen":
        ...
        if (
            ...
            or set(manifest) != {"version", "build", "compiler", "policy"}
...
def _producer_receipt_path(root: Path, cell: MatrixCell) -> Path:
    return (
        root
        / _safe_component(cell.backend.name, "backend")
        / _safe_component(cell.model.name, "model")
        / _safe_component(cell.precision, "precision")
        / "producer-input.json"
    )


def _producer_input(
    root: Path, cell: MatrixCell, *, identity: Mapping[str, Any], source_identity: str, runtime_sha256: str
) -> Path:
    receipt_path = _producer_receipt_path(root, cell)
    receipt = _closed(
        _load_json(receipt_path, f"{cell.key} producer input"),
        {
            "schema_version",
            "kind",
            "status",
            ...
            "kernel_backend",
            "promoted_compiler_sha256",
            "promoted_compiler_source_sha256",
        },
    ...
    if (
        not compiler_input.is_relative_to(receipt_path.parent.resolve())
        or compiler_input.is_symlink()
        or not compiler_input.is_file()
    ...
    if (
        receipt["schema_version"] != 1
        or receipt["kind"] != _PRODUCER_INPUT_KIND
        ...
        or receipt["promoted_compiler_source_sha256"] != identity["promoted_compiler_source_sha256"]
    ):
...
def register_backend_producer_input(
    *,
    study_path: str | Path,
    promoted_compiler: str | Path,
    runtime_artifact: str | Path,
    producer_root: str | Path,
    backend: str,
    model: str,
    precision: str,
    compiler_input: str | Path,
) -> Path:
    ...
    matches = [
        cell
        for cell in _required_cells(study)
    ...
    source_identity = _compiler_source_identity()
    ...
    retained = stage_compiler_input(
        Path(compiler_input).resolve(), output.parent / "compiler-input/compiler-input.json", recipe=MERLIN_RECIPE
    )
    ...
    _write_json(
        ...
        {
            "schema_version": 1,
            "kind": _PRODUCER_INPUT_KIND,
            "status": "producer_complete",
            "cell": {"model": model, "backend": backend, "precision": precision},
            "compiler_input": {"path": retained.relative_to(output.parent).as_posix(), "sha256": _sha(retained)},
...
def _build_package(
    ...
    *,
    ...
    regeneration = regenerate_model_object(
        ...
        target="k1",
        compiler_input=staged_input,
        tool=Path(str(authority["tool"]["path"])),
        output=model_object,
    ...
    hashes = {
        "compiler_input": _sha(staged_input),
        "model_object": _sha(model_object),
        "runner": _sha(runner),
        "build_tool": _sha(tool),
    ...
    _write_json(
        receipt,
        {
            ...
            "cell": {"model": cell.model.name, "backend": cell.backend.name, "precision": cell.precision},
            ...
            "build_source_identity_sha256": _canonical_sha(
                {
                    ...
                    "object_builder": builder_sha,
                    "runner": hashes["runner"],
                }
            ),
            "build_argv": _BUILD_ARGV,
            "result_executable_sha256": _sha(executable),
...
def _write_template(package: Path, cell: MatrixCell, resources: Mapping[str, Path]) -> Path:
    ...
    template = {
        ...
        "resources": refs,
        "environment": {},
        "execution": {"mode": "rvv", "core_ids": list(range(core)), "require_worker_threads": core > 1},
...
def _validate_template(
    ...
    *,
    ...
    _package_receipt(
        resolved["package_receipt"],
        registry_id="merlin_compile_v1",
        cell=cell,
        source_identity=source_identity,
        package_identity=str(identity["package_sha256"]),
...
def materialize(
    study_path: str | Path,
    capture_registration_path: str | Path,
    promoted_compiler: str | Path,
    producer_inputs: str | Path,
    runtime_artifact: str | Path,
    toolchain_authority: str | Path,
    ...
    *,
    ...
    try:
        ...
        authority = _verify_authority_receipt(authority_path, authority_receipt_path, target=study.target)
        verify_build_tool(
            str(authority["tool"]["path"]),
            authority_path=authority_path,
            authority_sha256=_sha(authority_path),
            target=study.target,
            expected_identity_sha256=str(authority["tool"]["identity_sha256"]),
    ...
    for cell in cells if not errors else ():
        ...
        try:
            ...
            inputs[key] = _producer_input(
                producer_root, cell, identity=identity, source_identity=source_identity, runtime_sha256=_sha(runtime)
            )
        except (OSError, ValueError) as exc:
            errors.append(str(exc))
    ...
    plan = {
        "schema_version": 1,
        "kind": "paper_merlin_package_plan_v1",
        "mode": "execute" if execute else "preflight",
        "status": "blocked" if errors else "ready",
        "study": {"path": str(study_path), "sha256": _sha(study_path)},
        "capture_registration": {"path": str(registration_path), "sha256": _sha(registration_path)},
        "promoted_compiler": {"path": str(promoted), "sha256": sha256_paths([promoted]) if promoted.exists() else None},
        "producer_inputs": str(producer_root),
        "runtime_artifact": {"path": str(runtime), "sha256": _sha(runtime) if runtime.is_file() else None},
        "toolchain_authority": {
            "path": str(authority_path),
            "sha256": _sha(authority_path) if authority_path.is_file() else None,
        },
        "authority_receipt": {
            "path": str(authority_receipt_path),
            "sha256": (_sha(authority_receipt_path) if authority_receipt_path.is_file() else None),
        },
        "compiler_source_sha256": source_identity,
    ...
    product.notes = f"non-ExecuTorch package plan {plan['status']}; producer inputs={len(inputs)}/{expected_packages}"
    ...
    try:
        for key, base_cell in sorted(package_cells.items()):
            ...
            built[key] = _build_package(
                base_cell,
                root=staging / "packages",
                compiler_input=inputs[key],
                identity=identity,
                source_identity=source_identity,
                runtime_artifact=runtime,
                authority_path=authority_path,
        ...
        for cell in cells:
            ...
            ref = {"path": str(published / template.relative_to(staging)), "sha256": _sha(template)}
            refs.setdefault(cell.backend.name, {}).setdefault(cell.model.name, {}).setdefault(cell.precision, {})[
                str(cell.core_count)
            ] = ref
        ...
        _write_json(
            ...
            {
                "schema_version": 1,
                "kind": _PACKAGE_SET_KIND,
                "complete": True,
                "study": "package-ready-study.yaml",
                "study_sha256": _sha(ready_staging),
    ...
    _write_json(plan_path, plan)
    product.notes = f"complete non-ExecuTorch package set; packages={expected_packages}; templates={expected_templates}"
    product.write_manifest()
    return product.path
...
def main(argv: list[str] | None = None) -> int:
    ...
    authority = commands.add_parser("issue-authority", help="issue the explicit compiler authority and review receipt")
    ...
    packages = commands.add_parser("packages", help="plan or build the complete non-ExecuTorch package set")
    ...
    register = commands.add_parser(
        "register-producer-input", help="deep-retain one completed backend producer's MRLNSES2 graph"
    )
    ...
    if args.command == "issue-authority":
        authority_path, receipt = issue_toolchain_authority(
            output=args.output, authority_id=args.authority_id, target=args.target, build_tool=args.build_tool
        )
    ...
    if args.command == "register-producer-input":
        output = register_backend_producer_input(
            ...
            promoted_compiler=args.promoted_compiler,
            runtime_artifact=args.runtime_artifact,
            producer_root=args.producer_inputs,
    ...
    try:
        output = materialize(
            args.study,
            args.capture_registration,
            args.promoted_compiler,
