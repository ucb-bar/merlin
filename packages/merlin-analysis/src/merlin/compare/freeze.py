...
def _require_external_package_registration(spec: PaperStudySpec, study_sha256: str) -> tuple[Path, str, dict]:
    ...
    expected = {
        ...
        if precision in model.precisions
    }
    ...
    if (
        not isinstance(registered_model2mlir, dict)
        or not isinstance(registered_toolchain, dict)
        or not isinstance(registered_external, dict)
    ):
        raise ValueError("cannot freeze: package registration build/source identities are absent")
    for key, row in indexed.items():
        ...
        if (
            registered_path.resolve() != declared_path.resolve()
            or row.get("sha256") != declared.get("sha256")
            or row.get("build_environment_sha256") != declared.get("build_environment_sha256")
        ...
        expected_model2mlir = {
            ...
            "loader_sha256": (registered_model2mlir.get("loader_sha256", {}) or {}).get(model_name),
            "capture_source_sha256": registered_model2mlir.get("capture_source_sha256"),
        }
        if (
            row.get("model2mlir_identity") != expected_model2mlir
            or row.get("toolchain_identity") != registered_toolchain
            or row.get("external_model_source") != registered_external.get(model_name)
        ):
            raise ValueError(f"cannot freeze: package registration row {key} build/source identities differ")
...
def _git_sha(root: Path) -> str:
    proc = subprocess.run(["git", "-C", str(root), "rev-parse", "HEAD"], capture_output=True, text=True, timeout=30)
...
def freeze_study(
    ...
    *,
    ...
    if snapshot_spec.canonical_dict() != spec.canonical_dict():
        raise ValueError("cannot freeze: supplied paper study differs from its exact source-byte snapshot")
    spec = snapshot_spec
    ...
    registered_package_rows = {
        (str(row["model"]), str(row["precision"])): row for row in package_registration["packages"]
    }
    long_lived_inputs.append(
        ("external package registration", [package_registration_path], package_registration_digest)
    )
    ...
    validate_packages_before_private_io(
        raw, toolchain_authority_path=authority_path, toolchain_authority_sha256=authority_digest
    )
    ...
    for backend in raw["backends"]:
        ...
        if not isinstance(packages, dict):
            raise ValueError(f"cannot freeze external backend {backend['name']}: packages are absent")
        for model in raw["models"]:
            for precision in backend["precisions"]:
                ...
                frozen_package = load_session_package(package_path, expected_sha256=registered_digest)
    ...
    if input_binding_errors:
        raise ValueError("cannot freeze: paper input binding is invalid: " + "; ".join(input_binding_errors))
    ...
    for model_raw, model in zip(raw["models"], spec.models, strict=True):
        for precision, artifact in model_raw["artifacts"].items():
            ...
            if capture_value:
                ...
                capture = (
                    bundle.CaptureBundle(model=model.capture, variant=variant, root=capture.resolve()).require().root
                )
            else:
                capture = bundle.resolve(model.capture, variant).require().root
            session_contract, session_errors = validate_capture_session(
                capture, model.session, expected_provenance=model.expected_provenance
            )
            if session_errors:
            ...
            long_lived_inputs.append((f"capture {model.name}/{precision}", [capture], artifact["sha256"]))
    ...
    for backend in raw["backends"]:
        ...
        if package_value and package_value != "unresolved":
            ...
            options["package"] = str(package.resolve())
            options["package_sha256"] = sha256_paths([package])
            long_lived_inputs.append((f"backend package {backend['name']}", [package], options["package_sha256"]))
        if backend["kind"] == "compiler":
            options["package"] = str(policy)
            options["package_sha256"] = policy_digest
        source_values = options.get("source_paths", ()) or ()
        if source_values:
            ...
            options[digest_key] = sha256_paths(source_paths)
            long_lived_inputs.append((f"backend sources {backend['name']}", source_paths, options[digest_key]))
        ...
        if backend["kind"] == "external_runtime":
            ...
            for model in raw["models"]:
                for precision in backend["precisions"]:
                    ...
                    row = by_precision.get(precision) if isinstance(by_precision, dict) else None
                    ...
                    registered_digest = str(row.get("sha256", ""))
                    if len(registered_digest) != 64 or any(
                        value not in "0123456789abcdef" for value in registered_digest
                    ):
                        raise ValueError(
                            f"cannot freeze {backend['name']} {model['name']}/{precision}: "
                    ...
                    long_lived_inputs.append(
                        (f"external package {model['name']}/{precision}", [package_path], registered_digest)
                    )
                    registered_environment_digest = str(row.get("build_environment_sha256", ""))
                    if len(registered_environment_digest) != 64 or any(
                        value not in "0123456789abcdef" for value in registered_environment_digest
                    ):
                        raise ValueError(
                            f"cannot freeze {backend['name']} {model['name']}/{precision}: "
                    ...
                    if frozen.model != model["capture"] or frozen.variant != artifact["variant"]:
                        raise ValueError(
                            f"cannot freeze {backend['name']} {model['name']}/{precision}: "
                    ...
                    expected_session_identity = capture_session_identities[(model["name"], precision)]
                    ...
                    if (
                        frozen.executorch_identity != registered_row.get("executorch_identity")
                        or frozen.model2mlir_identity != registered_row.get("model2mlir_identity")
                        or frozen.toolchain_identity != registered_row.get("toolchain_identity")
                        or frozen.external_model_source != registered_row.get("external_model_source")
                    ):
                        raise ValueError(
                            f"cannot freeze {backend['name']} {model['name']}/{precision}: "
    ...
    long_lived_inputs.append(("RVV baseline source pins", [baseline_sources], baseline_sources_digest))
    ...
    raw["freeze"] = {
        **raw["freeze"],
        ...
        "capture_session_identity_sha256": {
            model.name: {
                precision: capture_session_identities[(model.name, precision)] for precision in model.precisions
            }
            for model in spec.models
        },
    ...
    for label, paths, expected in long_lived_inputs:
        ...
        if actual != expected:
            raise ValueError(f"cannot freeze: {label} changed before publication: observed={expected} actual={actual}")
    ...
    if hashlib.sha256(package_registration_path.read_bytes()).hexdigest() != package_registration_digest:
