...
def _program_mlir(capture: Path, descriptor: SessionDescriptor) -> tuple[Path, ...]:
    ...
    if descriptor.source_contract_version == 1:
        ...
        paths = tuple(capture / str(row.get("bundle", "")) / "model.mlir" for row in rows if isinstance(row, Mapping))
...
def _abi_blockers(capture: Path) -> list[Blocker]:
    ...
    for index, path in enumerate(programs):
        try:
            ...
            blockers.append(Blocker("capture_mlir_signature_invalid", f"program {index}: {exc}"))
            continue
        covered = {input_index for owner, input_index in external | routes | states if owner == index}
        ...
        if not outputs:
            blockers.append(Blocker("capture_mlir_output_absent", f"program {index} has no compiled output"))
    return blockers
...
def _capture_blockers(cell: MatrixCell, registration: Mapping[str, Any] | None) -> list[Blocker]:
    ...
    if registration is None:
        return [Blocker("capture_registration_absent", "no complete registration authorizes this capture")]
    ...
    matches = (
        [
            ...
            and row.get("precision") == cell.precision
        ]
        if isinstance(rows, list)
        else []
    )
    if len(matches) != 1 or matches[0].get("sha256") != digest:
        return [Blocker("capture_registration_cell_mismatch", "capture registration does not bind this cell")]
...
def _paper_input_prerequisites(study: PaperStudySpec) -> tuple[list[Blocker], dict[str, Any]]:
    ...
    if bundle.is_symlink() or not bundle.is_dir():
        return [
            Blocker("paper_input_bundle_absent", f"frozen paper input bundle is absent or unsafe: {bundle}")
        ], evidence
    ...
    if actual != expected:
        return [
            Blocker(
                "paper_input_bundle_digest_mismatch", f"prepared-input bytes differ: study={expected} actual={actual}"
            )
        ], evidence
    binding_errors = validate_paper_input_binding(bundle, study.models)
    if binding_errors:
        return [Blocker("paper_input_bundle_binding_invalid", detail) for detail in binding_errors], evidence
...
def _global_prerequisites(
    ...
    capture_registration: Path | None,
    promoted_compiler: Path | None,
    ...
    if runtime_artifact is None:
        blockers.append(Blocker("runtime_artifact_absent", "no explicit frozen runtime artifact was supplied"))
    elif runtime_artifact.is_symlink() or not runtime_artifact.is_file():
        blockers.append(
            Blocker("runtime_artifact_invalid", f"runtime artifact is absent or unsafe: {runtime_artifact}")
        )
    else:
...
def materialize(
    study_path: str | Path,
    *,
    capture_registration: str | Path | None = None,
    promoted_compiler: str | Path | None = None,
    runtime_artifact: str | Path | None = None,
    producer_authority: str | Path | None = None,
    execute: bool = False,
    ...
    sources = [
        str(study_path),
        *(str(path) for path in (registration_path, promoted_path, runtime_path, authority_path) if path is not None),
    ]
    product = product or new_product("paper-merlin-producers", version=1, target=study.target, sources=sources)
    ...
    actual_hand = sorted({cell.precision for cell in cells if cell.backend.name == "hand_v0_int8"})
    plan = {
        "schema_version": 1,
        "kind": "paper_merlin_backend_producer_plan_v1",
        "mode": "execute" if execute else "preflight",
        "status": "blocked",
        "study": {"path": str(study_path), "sha256": _sha(study_path)},
        "matrix_contract": {
            "packages": len(cells),
            "templates_after_packaging": len(cells) * len(study.core_counts),
        ...
        "evidence": evidence,
        "capability_sources": {
            "generic_worker": "packages/merlin-analysis/src/merlin/compare/paper_merlin_lower_worker.py",
            "hand_and_kernel_swap_reference": "merlin/python/merlin/mining/k1.py",
            "producer": "packages/merlin-analysis/src/merlin/compare/paper_merlin_mlir_producer.py",
        },
        "cells": rows,
        "summary": {
            "required_packages": len(cells),
            "genuinely_produced": 0,
            "registered": 0,
            "runnable_templates": 0,
            "blocked": len(rows),
            "blocker_counts": {
                code: sum(any(blocker["code"] == code for blocker in row["blockers"]) for row in rows)
                for code in sorted({blocker["code"] for row in rows for blocker in row["blockers"]})
            },
        },
    }
    plan_path = product.add_artifact("producer-plan.json")
    _write_json(plan_path, plan)
    product.notes = f"paper Merlin producers blocked; genuine=0/{len(cells)}; execute={execute}"
    product.write_manifest()
    raise ProducerPlanNotReady(
        [
            f"{row['cell']['model']}/{row['cell']['backend']}/{row['cell']['precision']}: "
            f"{','.join(blocker['code'] for blocker in row['blockers'])}"
            for row in rows
        ],
        product.path,
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        output = materialize(
            args.study,
            capture_registration=args.capture_registration,
