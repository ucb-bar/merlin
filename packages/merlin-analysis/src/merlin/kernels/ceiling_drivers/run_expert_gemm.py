...
from merlin.common.paths import module_source_path, work_dir
...
def measure_one(source: str, *, M: int = 64, N: int = 64, K: int = 64, isa: str = bench_ceiling.DEFAULT_ISA) -> dict:
    ...
    if cycles is None:
        return {**base, "cycles": None, "status": "not_run", "blocker": "no CYCLES line in console"}
    row = {
        **base,
        "cycles": cycles,
        "status": "pass",
        "note": f"{source} f32 GEMM on spike; inner-compute timed (pack outside); verified vs scalar ref",
    }
...
def main() -> int:
    ...
    for source in ("openblas", "xnnpack"):
        ...
        if status == "pass":
            print(f"{source:9s}  cycles={row['cycles']:>8}  instret={row.get('instructions', '?')}  -> appended")
        else:
