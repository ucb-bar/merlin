...
SHADOW = pe.withSimplePatchShadow(offset=(3.0, -3.0), shadow_rgbFace=(0.18, 0.178, 0.173), alpha=0.26, rho=1.0)


def use_merlin_style():
    """Register the house fonts and apply the rcParams. Call once per script."""
    for fp in (
        glob.glob("/usr/share/fonts/opentype/inter/Inter-*.otf")
