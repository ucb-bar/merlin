...
set -uo pipefail
cd $MERLIN_REPO/.claude/worktrees/codex-radiance-smoke
set -a; . ./.env; set +a
...
export TMPDIR=$TMPDIR/codex_runs
mkdir -p "$TMPDIR"
...
export MERLIN_MUON_GSIM_EMU="${MERLIN_MUON_GSIM_EMU:?set MERLIN_MUON_GSIM_EMU in .env}"
...
export MERLIN_MLC_DIR=${MERLIN_MLC_DIR:-${MERLIN_MLC_DIR}/../modeling}
...
CHIA_PY=$MERLIN_REPO/build/chia-venv/bin/python
