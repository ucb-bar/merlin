...
related: [capsule_generation, compiler_verification, capsule_phase_split, radiance_staged_evaluation]
code_refs:
  - merlin/python/merlin/targetgen/capsule_grade.py
  - merlin/python/merlin/targetgen/capsule_runner.py
  - merlin/python/merlin/verify/epilogue_applicability.py
  ...
  - merlin/experiments/capsule_bench/harness/tier_promote.py
  ...
  - merlin/experiments/capsule_bench/harness/codex_agent.py
...
**Gate.** `freeze_run.py` records `repo_sha` and the submission digest. Code snapshots bind the grader
inputs. The mid-batch hazard is not yet gated: it needs a campaign-level pin that refuses to grade
under a revision other than the one the campaign was launched with.
