...
code_refs:
  ...
  - src/merlin/targetgen/corpus_synth.py
  - src/merlin/targetgen/capsule_source.py
  - packages/merlin-experiments/src/merlin/targetgen/capsule_golden.py
  - src/merlin/targetgen/conformance.py
...
| 1 | derive the requirement | `corpus.admission.conformance_spec()` / core `required_cells()` | `conformance/<target>.yaml` — cells + 8 axes |
| 2 | synthesize entries | `corpus_synth` via `synth_capsule_corpus.py --write` | fresh versioned `synth.yaml` under configured `out/artifacts/verification/<target>/`; review before selecting as an input |
...
`load_profile` merges **explicitly selected inputs in a fixed order**. It is *not* a glob, and that has bitten
...
|---|---|---|
| 1 | `recipe`: `examples/<target>/phase0/recipe.yaml` in the catalog | `datapath:` block + `capsules:` + `sweeps:` |
| 2 | `performance_template`: `experiments/templates/phase0/performance.yaml` | the single shared performance template; appends `sweeps`, records path + sha256 |
| 3 | `synth_profile`: a retained reference or reviewed synthesis artifact | `capsules` only — the derived requirement (stage 2) |
| 4 | `smt_profile`: explicitly declared optional sidecar | `capsules` only — SMT counterexamples promoted to capsules |
| 5 | `hidden_profile`: explicitly declared private sidecar | appends `capsules` **and** `sweeps`; never public example content |

Entries append, preserving declaration order. A duplicate `name` raises. Optional sidecars
may be absent; frozen runs bind both presence and bytes. Public-only loading never reads
or stats the hidden path. Legacy explicit `profiles_root` mode retains its directory
convention (`_perf.yaml`, target recipe and dotted sidecars); it is not an implicit checkout
default and cannot be combined with the explicit recipe mode above.
...
- **It does not model residual accelerator state.** No axis expresses what was in the scratchpad
  ...
  fully covered corpus, as a padded-convolution defect demonstrated in September 2026.
