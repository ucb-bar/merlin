...
## ⚠️ Model capsules cannot be graded from a clone

**No capsule weights are in the repository.** `merlin/contract/capsules/.gitignore:23` ignores
`*.safetensors`, and `git ls-files '*.safetensors'` returns **0** against **7** model capsules that
need one. They exist only as untracked files on the machine that generated them.

Measured on a fresh worktree: all six graded model capsules — `M0`, `M1`, `M2`, `M3`, `GX0`,
`SY_micro_model` — come back `incomplete` with
...
including `M3_host_island_seam`, which passes at L3 on a machine that has the file. So a clone can
grade the ISA, layer and model-slice capsules and **none** of the model ones, and the failure is an
absent asset rather than anything about the backend.

If you need the model capsules, get the weights out of band from a machine that has them — they are
regenerated by the capsule's own `capsule.pytorch.py`, so they can also be re-derived, at the cost of
re-deriving the golden alongside them.
