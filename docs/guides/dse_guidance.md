...
- **Level 1 (partially implemented — `attribution.py`)** — attribute *real* per-matmul IR facts
  ...
  **Demonstrated on a real capture (not the toy):** a fresh RDT denoise-step capture (the real
  ...
  `blocked_by: missing_calibration`. Capture: `merlin/benchmarks/dse_guidance/recaptures/rdt/model.mlir`
  (the 20 `prov.fqn`-tagged `linalg.matmul` ops above; region attribution is regenerated by
  `merlin-dse-guidance --case-study`, not stored as a golden). Caveats: depth=2 (real 1B is 28) and random init — the
