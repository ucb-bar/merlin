...
This is a checkout-local reproduction guide for a historical target example, not Merlin's generic
installed Phase-0→1→2 entry point. The pinned emulator digest and campaign receipts identify that
experiment; they do not certify a different OOT backend or a new model invocation.
...
tiering cosmetic — two model strings in the record, one model answering both.) These are archived
campaign selections, not a claim that the same model identifiers are currently available; record
the model actually served for each new run.
