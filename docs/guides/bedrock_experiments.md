...
code_refs: [merlin/experiments/capsule_bench/harness, packages/merlin-experiments/src/merlin/targetgen/aet_bridge.py]
---
...
- Merlin's public Chia trace adapter (`CHIA_AET_SINK=1`) uses AET billing only for explicitly typed
  metadata; native child AET ledgers remain authoritative. Missing/untyped trace costs are unknown.
