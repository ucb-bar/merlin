...
def _emit_fp8_matmul(p, workload, cb, matmul, commit):
    ...
    if (((m + 31) // 32) * ((k + 31) // 32) * ((n + 31) // 32) >= 16
        ...
        if n % 32 == 0:
            return True
        # The compact runtime body handles all complete column tiles.  Keep
        # only the final, shape-derived partial tile in straight-line form.
        full_n = (n // 32) * 32
        tiles = (tile for tile in TileScheduler.choose(m, k, n)
                 if tile.n0 >= full_n)
    else:
...
def _dynamic_store_bf16(p, dram_reg, out_cols, rows, cols):
    ...
    p.li(6, 1024)
    p.emit(e.vstore(3, 6), e.delay(40))
    # Full-width loop tiles map to two aligned 32-byte register halves per
    # row.  Send four rows (eight halves) concurrently instead of spending
    # thousands of scalar LSU cycles compacting them into a second buffer.
    for first in range(0, rows, 4):
        batch_rows = min(4, rows - first)
        issued = 0
        for index in range(batch_rows):
            row = first + index
            p.li(24, row * out_cols * 2)
            p.emit(e.add(18, dram_reg, 24))
            p.li(6, 768 + row * 8)
            p.li(12, 32)
            p.emit(e.dma_store(18, 6, 12, issued), e.addi(0, 0, 0))
            issued += 1
            p.emit(e.addi(18, 18, 32))
            p.li(6, 1024 + row * 8)
            p.li(12, 32)
            p.emit(e.dma_store(18, 6, 12, issued), e.addi(0, 0, 0))
            issued += 1
...
def _emit_fp8_matmul_loop_region(p, lhs, rhs, out, m, k, n,
    ...
    p.li(20, m_tiles)
    p.li(25, lhs["base"] + m_start * k)
    p.li(26, out["base"] + (m_start * n + n_start) * 2)
    p.label(tag + "_m")
    p.li(21, n_tiles)
    p.li(27, rhs["base"] + n_start)
...
def _emit_batched_matmul(p, workload, cb, item):
    ...
    for b in range(batch):
        ...
        if lhs["dtype"] == "fp8_e4m3":
            if not _emit_fp8_matmul(p, workload, local_cb, mm, commit): return False
        elif not _emit_bf16_matmul(p, workload, local_cb, mm, commit): return False
    return True
