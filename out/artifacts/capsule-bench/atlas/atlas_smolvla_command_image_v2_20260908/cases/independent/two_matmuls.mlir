module attributes {merlin_iface.version = "0.1", merlin_iface.target = "atlas", merlin_iface.abi_version = "0.1"} {
  ...
  %W1 = merlin_iface.tensor {name = "W1", role = "weight"} : tensor<32x7xf8E4M3FN>
  %A1 = merlin_iface.tensor {name = "A1", role = "input"} : tensor<3x32xf8E4M3FN>
  %W0_res = merlin_iface.resident_pack %W0 {layout = "packed_rhs"} : (tensor<32x8xf8E4M3FN>) -> !merlin_iface.resident
  %W1_res = merlin_iface.resident_pack %W1 {layout = "packed_rhs"} : (tensor<32x7xf8E4M3FN>) -> !merlin_iface.resident
  ...
  %acc1 = merlin_iface.matmul %A1, %W1_res : (tensor<3x32xf8E4M3FN>, !merlin_iface.resident) -> !merlin_iface.acc<bf16>
  %Y1 = merlin_iface.commit %acc1 {name = "Y1", epilogue = [], output_dtype = "bf16"} : (!merlin_iface.acc<bf16>) -> tensor<3x7xbf16>
