...
# AtlasCore24's DMA assertions derive these bounds directly from the launch:
# transferBeats = size >> 5, vmemLineAddr = vmemAddr >> 3, and
# vmemLineAddr + transferBeats - 1 < 0xc000.  The compiler's local address is
# in 32-bit words, matching the scalar register handed to DMA; the RTL's shift
# converts that address to its 32-byte line index.
DMA_BEAT_BYTES = 32
VMEM_DMA_LINE_CAPACITY = 0xC000


def _check_dma_window(local_words, size):
    if size < DMA_BEAT_BYTES or size % DMA_BEAT_BYTES:
        raise ValueError(
            f"Atlas DMA size must be a positive {DMA_BEAT_BYTES}-byte multiple: {size}"
        )
    beats = size // DMA_BEAT_BYTES
    if local_words < 0 or (local_words >> 3) + beats > VMEM_DMA_LINE_CAPACITY:
        raise ValueError(
            "Atlas DMA VMEM window exceeds hardware capacity: "
            f"base={local_words} beats={beats} capacity={VMEM_DMA_LINE_CAPACITY}"
        )
...
def _dma_load(p, local_words, dram_addr, size, channel=0):
    _check_dma_window(local_words, size)
    p.li(6, local_words)
    p.li(16, dram_addr)
    p.li(12, size)
...
def _dma_load_striped(p, local_words, dram_addr, size):
    ...
    for channel in range(channels):
        ...
        _check_dma_window(local_words + offset // 4, amount)
...
def _dynamic_store_bf16(p, dram_reg, out_cols, rows, cols):
    ...
    p.li(6, 1024)
    p.emit(e.vstore(3, 6), e.delay(40))
    # A row has at most two live register halves.  The row-major destination
    # stride need not be a DMA-beat multiple (N=113 advances by 226 bytes), so
    # even a full 32-byte half can start inside a beat.  Direct DMA would align
    # that address down and shear row r by its r*stride offset.  Route every
    # segment through an aligned RMW window; this also handles sub-beat tails.
    left = min(cols, 16)
    right = max(0, cols - 16)
    segments = tuple(
        (col, local_words, size)
        for col, local_words, size in (
            (0, 768, left * 2), (16, 1024, right * 2))
        if size
    )
    for col, local_words, size in segments:
        # Atlas DMA has no sub-beat transaction: size>>5 must be nonzero.  A
        # tail segment is therefore updated through one conservative 64-byte
        # aligned RMW window.  64 bytes covers any even BF16 offset plus a
        # <=32-byte segment, including a row that straddles a 32-byte beat.
        # Iterate rows in one scalar loop so this fix does not trade a legal
        # DMA for an over-IMEM unrolled image.
        transfer_bytes = 2 * DMA_BEAT_BYTES
        scratch_words = 2048
        _check_dma_window(scratch_words, transfer_bytes)
        tag = "subbeat_rows_" + str(len(p.words))
        p.li(13, col * 2)
        p.emit(e.add(13, dram_reg, 13))
        p.li(14, local_words * 4)
        p.li(15, rows)
        p.li(24, out_cols * 2)
        p.label(tag)
        p.emit(e.andi(23, 13, DMA_BEAT_BYTES - 1),
               e.andi(18, 13, -DMA_BEAT_BYTES))
        p.li(6, scratch_words)
        p.li(12, transfer_bytes)
        p.emit(e.dma_load(6, 18, 12, 0), e.dma_wait(0))
        p.li(8, scratch_words * 4)
        p.emit(e.add(8, 8, 23))
        for halfword in range(size // 2):
            p.emit(e.lhu(7, 14, halfword * 2), e.delay(4),
                   e.sh(7, 8, halfword * 2), e.delay(4))
        p.li(6, scratch_words)
        p.li(12, transfer_bytes)
        p.emit(e.dma_store(18, 6, 12, 0), e.dma_wait(0),
               e.add(13, 13, 24), e.addi(14, 14, 32),
               e.addi(15, 15, -1), e.delay(4))
        p.bne(15, 0, tag)
