...
def _read_iface(module, tensor_ops):
    ...
    for op in module.walk():
        if op.name == "merlin_iface.resident_pack":
            ...
            # A committed tensor is also a legal SSA producer for a later
            # operation in a whole-program interface.  Leaf-only capsules did
            # not expose this omission because no operation consumed a commit
            # result; a two-kernel chain otherwise fails while resolving its
            # second lhs before command-buffer construction even begins.
            names[op.results[0]] = out.name
