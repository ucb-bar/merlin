#!/usr/bin/env python3
"""Offline integrity and falsifiability checks for this L3 evidence pair."""
...
def main() -> int:
    ...
    assert report["selection_is_numeric_qualification"] is False
    assert len(report["decisions"]) == 23
    selected = [d for d in report["decisions"] if d["status"] == "selected"]
    disabled = [d for d in report["decisions"] if d["status"] == "disabled"]
    assert [d["family"] for d in selected] == ["kernels/bias_add"]
    assert {d["family"] for d in disabled} == {
        "kernels/flash_attention_mx_gemma", "kernels/flash_attention_mx_fp6"}
    print("ok: identical compiler artifact passes 256/256 and rejects perturbed-golden control")
    return 0
