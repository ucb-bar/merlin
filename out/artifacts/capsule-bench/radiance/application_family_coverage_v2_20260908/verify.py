#!/usr/bin/env python3
"""Offline fail-closed verifier for the v2 census and multiply L2 receipts."""
...
def verify() -> None:
    ...
    assert census["total_regions"] == 13186 and census["classified_regions"] == 13046
    comparison = coverage["search_comparison"]
    assert len(comparison["pre_wire_14_capsules"]) == 14
    assert len(comparison["after_add_15_capsules"]) == 15
    assert len(comparison["post_wire_16_capsules"]) == 16
    assert comparison["post_wire_family_counts"] == {"contraction": 14, "elementwise_map": 2}
    assert coverage["wired_probes"]["add"]["exact_occurrences_in_declared_captures"] == 99
    assert coverage["wired_probes"]["mul"]["exact_occurrences_in_declared_captures"] == 168
    assert coverage["selection"]["highest_frequency_missing_family"] == {
        "admitted": False, "family": "movement", "occurrences": 3469,
        "reason": "absent from effective capability map",
    }
    effective = coverage["effective_capability"]
    assert effective["family"] == "elementwise_map"
    assert effective["dtypes"] == ["float32"] and effective["ranks"] == []
    assert effective["composed_with"] == [] and effective["providers"] == [["simt_cluster", "simt"]]
    ...
    assert _sha(HERE / receipt["negative_control"]["receipt"]) \
        == receipt["negative_control"]["receipt_sha256"]
    ...
    assert positive["numeric"]["per_output"]["out"]["n_elements"] == 32
    assert negative["numeric"]["per_output"]["out"]["n_elements"] == 32
    ...
    print("PASS: exact multiply census and fail-capable L2 pair verified; no GSIM/physical claim")
