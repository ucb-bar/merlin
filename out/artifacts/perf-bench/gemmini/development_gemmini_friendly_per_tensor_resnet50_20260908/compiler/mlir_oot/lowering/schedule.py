...
def readout_plan(e: Epilogue) -> Readout:
    ...
    """
    normalized = ["bias" if stage == "bias_add" else stage for stage in e.stages]
    bias_positions = [index for index, stage in enumerate(normalized) if stage == "bias"]
    if bias_positions and (e.bias is None or bias_positions != [0]):
        raise LoweringDeclined(
            "a declared bias stage must occur exactly once as accumulator initialization",
            op="epilogue")
    stages = [s for s in normalized if s != "bias"]
    ...
    ordered_native_pre = pre in ([], ["acc_scale"], ["relu"], ["acc_scale", "relu"])
    if (narrowing and e.integer_output_policy != "modular"
            and ordered_native_pre and e.acc_scale > 0.0):
...
class Scheduler:
    ...
    def convolution(self, c: Convolution) -> None:
        ...
        self._convolution_selections.append({
            ...
            "selected": "gemmini_loop_conv_ws" if native else "streamed_row_im2col_loop_ws",
            "reason": _reason,
        })
        if native:
    ...
    def native_convolution_eligibility(self, c: Convolution) -> tuple[bool, str]:
        ...
        activation, weight, dst = self.buf(c.activation), self.buf(c.weight), self.buf(c.dst)
        # The hardware order is accumulator bias initialization, store scaling, activation.
        # Preserve that exact source stage order in the selector input; mere membership is not
        # enough because scale/ReLU do not commute for negative values.
    ...
    def native_convolution(self, c: Convolution) -> None:
        ...
        bias_init = (self.buf(c.bias) if c.bias else self.stage_buffer(
                        ...
                        for krow in range(0, c.kh, krows):
                            for kcol in range(0, c.kw, kcols):
                                for kch in range(0, c.ci, kchs):
                                    ...
                                    self.emit(
                                        "loop_conv_ws",
                                        [c.activation, c.weight, c.dst, bias_init.name],
                                        ...
                                        trans_weight_0132=False, trans_input_3120=False,
                                        max_pixels_per_row=min(
                                            tile_kc, max(1, DIM // tile_kch)), dw=False,
                                        a_spad_id=0, b_spad_id=0,
                                        input_offset=((b * c.hi * c.wi
                                                       + (irow + upad) * c.wi
                                        ...
                                        output_offset=((b * c.ho * c.wo
                                                        + orow * c.wo + ocol)
                                                       * out_stride + poch),
                                        bias_offset=poch * DTYPE_BYTES["i32"],
                                        write_output=final,
                                        accumulator_initialization=(
                                            "i32_bias" if c.bias else "read_only_zero_bias"),
