...
class FnBuilder:
    ...
    def __init__(self, arg_types: list[Attribute]):
        ...
        # Attributes attached to operations created in the current emission scope.  Keeping
        # task provenance here makes attribution O(new operations); rescanning every block after
        # every whole-model task was O(tasks * accumulated blocks).
        self.scoped_attributes: dict[str, Attribute] = {}

    def _decorate(self, op):
        for name, value in self.scoped_attributes.items():
            op.attributes.setdefault(name, value)
        return op
