...
matmuls/attention from ``collapse_overrank_matmul``) into a real integer contraction:
  ...
  * each f32 activation operand is dynamically quantized to i8 (symmetric, per output-row:
    ``s = max(max|x|/127, 1e-5)`` reduced over the operand's contraction dim, then
    ``roundeven(x * (1/s))``, zero-point 0), matching TorchAO's captured arithmetic;
...
def _constant_int(value) -> int | None:
    """Read an integer constant through a rank-0 tensor.splat, if exact."""
    owner = getattr(value, "owner", None)
    if getattr(owner, "name", None) == "tensor.splat" and owner.operands:
...
def _emit_torchao_symmetric_scale(block, amax, denominator):
    """Emit TorchAO's symmetric int8 scale, including its mandatory ``1e-5`` floor.

    ``Int8DynamicActivationInt8WeightConfig`` computes ``amax / 127`` and then clamps the result
    to ``1e-5``.  The former zero-only fallback to one made zero rows finite, but it was not the
    arithmetic captured by model2MLIR and changed every group whose magnitude was below 0.00127.
    Keep an explicit select around the clamp: it is redundant numerically, but gives the
    quantize-round optimizer a local proof that an all-zero group's divisor is nonzero.
    """
    ...
    eps = arith.ConstantOp(FloatAttr(1.0e-5, ftype))
    floored = arith.MaximumfOp(raw.result, eps.results[0])
    is_zero = arith.CmpfOp(amax, zero.results[0], "oeq")
    safe = arith.SelectOp(is_zero.result, eps.results[0], floored.result)
    block.add_ops([raw, zero, eps, floored, is_zero, safe])
    return safe.result


def _emit_torchao_reciprocal_tensor(scale):
    """Materialize one reciprocal per scale group, as TorchAO does before elementwise quantization.

    Besides bit-exactness (``x * (1/s)`` can round differently from ``x/s``), keeping the reciprocal
    in its own generic prevents a floating-point division per activation element.  The number of
    divisions is therefore the number of rows/groups, independent of their reduction width.
    """
    ...
    rank = len(scale_type.get_shape())
    empty = tensor.EmptyOp((), scale_type)
    body = Block(arg_types=[scale_type.element_type, scale_type.element_type])
    one = arith.ConstantOp(FloatAttr(1.0, scale_type.element_type))
    reciprocal = arith.DivfOp(one.results[0], body.args[0])
...
def _zero_f32_splat(value) -> bool:
    """Whether ``value`` is a statically shaped tensor filled with exact f32 zero."""
    owner = getattr(value, "owner", None)
    if getattr(owner, "name", None) != "tensor.splat" or not owner.operands:
        return False
    scalar = getattr(owner.operands[0], "owner", None)
    if getattr(scalar, "name", None) != "arith.constant":
        return False
    attr = getattr(scalar, "value", None)
    raw = getattr(getattr(attr, "value", None), "data", None)
    return isinstance(raw, (int, float)) and float(raw) == 0.0


def _static_per_tensor_i8_layout_chain(value):
    """Commute calibrated symmetric per-tensor QDQ through pure layout operations.

    ResNet weights and activations reach im2col matmuls through reshape, transpose, zero-padding,
    and pure-gather operations. Dequantization commutes exactly with those operations when the
    zero point is zero. Rebuilding the path on the original i8 value avoids dynamic requantization
    and retains the capture's scalar scale. Anything computed or ambiguous refuses.
    """
    from xdsl.dialects import arith, tensor
    ...
    for _ in range(12):
        ...
        if _is_dequant_per_tensor(owner):
            if (len(owner.operands) < 3 or _constant_int(owner.operands[2]) != 0
                    or not isinstance(owner.operands[0].type, TensorType)
                    or owner.operands[0].type.element_type != i8):
                return None
            root = owner
            break
        ...
        if name == "linalg.transpose":
            if len(owner.inputs) != 1:
                return None
            chain.append(owner)
            current = owner.inputs[0]
            continue
        if name == "linalg.generic" and _is_pure_gather(owner):
            chain.append(owner)
            current = owner.inputs[0]
            continue
        if name == "tensor.insert_slice":
            if len(owner.operands) < 2 or not _zero_f32_splat(owner.operands[1]):
                return None
            chain.append(owner)
            current = owner.operands[0]
            continue
    ...
    pre = []
    dead = [root]
    current = root.operands[0]
    for old in reversed(chain):
        result_type = TensorType(i8, list(old.results[0].type.get_shape()))
        if old.name in _RESHAPE_OPS:
            new = type(old).create(
                operands=[current, *list(old.operands[1:])], result_types=[result_type],
                properties=dict(old.properties), attributes=dict(old.attributes))
            pre.append(new)
        elif old.name == "linalg.transpose":
            empty = tensor.EmptyOp((), result_type)
            new = L.TransposeOp(current, empty.results[0], old.permutation, result_type)
            new.attributes.update(dict(old.attributes))
            pre.extend([empty, new])
            old_empty = getattr(old.outputs[0], "owner", None)
            if getattr(old_empty, "name", None) == "tensor.empty":
                dead.append(old_empty)
        elif old.name == "linalg.generic":
            empty = tensor.EmptyOp((), result_type)
            body = Block(arg_types=[i8, i8])
            body.add_op(L.YieldOp(body.args[0]))
            new = L.GenericOp(
                inputs=(current,), outputs=(empty.results[0],), body=Region(body),
                indexing_maps=old.indexing_maps, iterator_types=old.iterator_types,
            ...
            old_base = old.operands[1]
            zero = arith.ConstantOp.from_int_and_width(0, 8)
            base = tensor.SplatOp(zero.results[0], (), result_type)
            new = type(old).create(
                operands=[current, base.results[0], *list(old.operands[2:])],
                result_types=[result_type], properties=dict(old.properties),
                attributes=dict(old.attributes))
            pre.extend([zero, base, new])
            old_splat = getattr(old_base, "owner", None)
            if getattr(old_splat, "name", None) == "tensor.splat":
                dead.append(old_splat)
                old_zero = getattr(old_splat.operands[0], "owner", None)
                if getattr(old_zero, "name", None) == "arith.constant":
                    dead.append(old_zero)
        current = new.results[0]
        dead.append(old)
    return current, root.operands[1], pre, dead
...
def lower_contraction_int8(module, *, named_contraction: bool = False,
    ...
    for op, (ndim, in_maps, out_dims, red_flags) in targets:
        ...
        for oi, operand in enumerate(op.inputs):
            ...
            # A linear layer commonly reaches linalg as
            #   transpose(dequantize_per_channel(i8_weight)).
            # Treating the transpose result as an arbitrary f32 operand would dequantize the
            # whole constant, scan it for a new max, and requantize it on every inference.  The
            # transpose is pure data movement and commutes exactly with dequantization, so retain
            # the calibrated i8 bytes/scales and transpose those bytes instead.  A later target
            ...
            # Preserve target-friendly PT2E qvalues/scales through model2MLIR's pure layout path
            # instead of scanning and dynamically requantizing the expanded f32 im2col matrix.
            static_layout = _static_per_tensor_i8_layout_chain(operand)
            if static_layout is not None:
                i8_value, scalar_scale, layout_ops, layout_dead = static_layout
                pre.extend(layout_ops)
                dead_chain_ops.extend(layout_dead)
                i8_inputs.append(i8_value)
                scale_vals.append((scalar_scale, []))
                _bump(report_out, "static_per_tensor_layout_reused")
                continue
...
def lower_conv_int8(module, *, select=None, report_out: "dict | None" = None) -> int:
    ...
    **THIS PASS IS UNREACHABLE ON EVERY CONVOLUTIONAL MODEL IN THIS FLEET, AND SAYS SO.** Its
    ...
    def static_activation(act):
        """Recover PT2E's calibrated i8 activation, commuting through zero-padding.

        Returns ``(i8_value, scale, ops_to_insert)``. Refuses any non-zero
        zero-point: padding q=0 is exact only for the symmetric W8A8 contract.
        """
        from xdsl.dialects.tensor import InsertSliceOp

        owner = getattr(act, "owner", None)
        pad = None
        if _is_dequant_per_tensor(owner):
            deq = owner
        elif getattr(owner, "name", None) == "tensor.insert_slice":
            source_owner = getattr(owner.source, "owner", None)
            if not _is_dequant_per_tensor(source_owner):
                return None
            deq, pad = source_owner, owner
        else:
            return None
        if len(deq.operands) < 3 or _constant_int(deq.operands[2]) != 0:
            return None
        qvalue, scale = deq.operands[0], deq.operands[1]
        if not (isinstance(qvalue.type, TensorType) and qvalue.type.element_type == i8):
            return None
        if pad is None:
            return qvalue, scale, []

        # The frontend pads real zero using splat+insert_slice. For symmetric
        # quantization zero maps exactly to i8 zero, so form the same view in i8
        # instead of scanning and dynamically requantizing the padded f32 tensor.
        padded_t = TensorType(i8, list(act.type.get_shape()))
        zero = arith.ConstantOp.from_int_and_width(0, 8)
        base = tensor.SplatOp(zero.results[0], [], padded_t)
        inserted = InsertSliceOp.from_static_parameters(
            qvalue,
            base.results[0],
            list(pad.static_offsets.iter_values()),
            list(pad.static_sizes.iter_values()),
            list(pad.static_strides.iter_values()),
        )
        return inserted.results[0], scale, [zero, base, inserted]
    ...
    for op in targets:
        ...
        if recovered is not None:
            q_value, s_a_value, static_ops = recovered
            pre += static_ops
            n_static_activation += 1
        else:
            # Dynamic fallback for legacy weight-only captures. A PT2E static
