...
class Pipeline:
    ...
    def __init__(self, text: str, *, enable_source_conv: bool = False,
                 input_prologue: dict | None = None, requantization_policy=None):
        self.text = text
        ...
        if not _parse.is_merlin_iface(self.module):
            ...
            if wl.mesh_regions or wl.host_regions:
                ...
                try:
                    ...
                    plan = planner.build(
                        self.module, wl,
                        requantization_policy=self.requantization_policy,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.requant_accuracy_policy is not None:
        from mlir_oot.frontend.requantization_contract import AccuracyPolicy
        try:
            requantization_policy = AccuracyPolicy.load(args.requant_accuracy_policy)
        except (OSError, ValueError, TypeError, json.JSONDecodeError) as exc:
            ap.error(f"invalid requantization accuracy policy: {exc}")
    pipe = Pipeline(text, enable_source_conv=(args.source_convolution
                    ...
                    requantization_policy=requantization_policy)
