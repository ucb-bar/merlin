...
from ..xdsl_utils import constant_integer
...
class HostLinalg:
    ...
    def run_segment(self, ops: list[Operation],
        ...
        segment has to leave in DRAM for whatever runs next. Every segment boundary is a real
        memory seam: declared inputs are rebound from their pointers and outputs are committed
        there. Pure captured scalar/splat values may remain cached across segments; memory-backed
        live-ins never rely on an earlier segment's private tensor storage.
        """
