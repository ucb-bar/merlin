...
K_FLUSH = 7
K_LOOP_WS = 8
K_LOOP_WS_CONFIG_BOUNDS = 9
K_LOOP_WS_CONFIG_ADDRS_AB = 10
K_LOOP_WS_CONFIG_ADDRS_DC = 11
K_LOOP_WS_CONFIG_STRIDES_AB = 12
K_LOOP_WS_CONFIG_STRIDES_DC = 13
...
def loop_ws_static(*, rows: int, cols: int, depth: int, row_stride_a: int,
                   row_stride_b: int, row_stride_c: int, row_stride_d: int | None = None,
                   full_c: bool = True,
                   activation: int = NO_ACTIVATION,
                   accumulate: bool = False) -> list[tuple[int, int, int]]:
    """Static LOOP_WS configuration surrounding runtime A/B/C pointers.

    Returns bounds, AB strides, DC strides, and launch records. Address records are emitted by
    codegen because their pointers are runtime SSA values. Extents are logical elements; LOOP_WS
    consumes tile counts plus padding on the final tile exactly as ``gemmini.h`` specifies.
    """
    tiles_i = -(-int(rows) // DIM)
    tiles_j = -(-int(cols) // DIM)
    tiles_k = -(-int(depth) // DIM)
    pad_i = tiles_i * DIM - int(rows)
    pad_j = tiles_j * DIM - int(cols)
    pad_k = tiles_k * DIM - int(depth)
    bounds_rs1 = (pad_k << 32) | (pad_j << 16) | pad_i
    bounds_rs2 = (tiles_k << 32) | (tiles_j << 16) | tiles_i
    launch_rs1 = ((activation & 0xFF) << 8) | ((1 if full_c else 0) << 1) | int(accumulate)
    launch_rs2 = 0
    d_stride = row_stride_c if row_stride_d is None else int(row_stride_d)
    return [
        (K_LOOP_WS_CONFIG_BOUNDS, _u64(bounds_rs1), _u64(bounds_rs2)),
        (K_LOOP_WS_CONFIG_STRIDES_AB, _u64(row_stride_a), _u64(row_stride_b)),
        (K_LOOP_WS_CONFIG_STRIDES_DC,
         _u64(d_stride) if accumulate else 0, _u64(row_stride_c)),
        (K_LOOP_WS, _u64(launch_rs1), _u64(launch_rs2)),
    ]
