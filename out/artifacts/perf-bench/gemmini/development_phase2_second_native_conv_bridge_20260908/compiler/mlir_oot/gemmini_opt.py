...
class Pipeline:
    ...
    def __init__(self, text: str, *, enable_source_conv: bool = False,
                 input_prologue: dict | None = None,
                 diagnostic_transposed_compute_only: bool = False):
        self.text = text
        ...
        if not _parse.is_merlin_iface(self.module):
            ...
            if wl.mesh_regions or wl.host_regions:
                ...
                try:
                    ...
                    plan = (planner.build(
                        self.module, wl,
                        diagnostic_transposed_compute_only=self.diagnostic_transposed_compute_only)
                        if planner is source_conv_model_lane else planner.build(self.module, wl))
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--diagnostic-transposed-nchw-loop-conv", action="store_true",
                    help="retain the exact but FireSim-regressed batch-1 NCHW LOOP_CONV route for diagnostics only")
    ...
    if args.diagnostic_transposed_nchw_loop_conv and not (
            args.source_convolution or args.integer_contract is not None):
        ap.error("diagnostic transposed LOOP_CONV requires source convolution scheduling")
    ...
    pipe = Pipeline(text, enable_source_conv=(args.source_convolution
                    ...
                    diagnostic_transposed_compute_only=(
                        args.diagnostic_transposed_nchw_loop_conv))
