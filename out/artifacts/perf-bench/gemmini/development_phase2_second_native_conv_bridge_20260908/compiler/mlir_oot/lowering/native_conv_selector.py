...
def select_native_loop_conv_compute_only(c, activation, weight, dst) -> tuple[bool, str]:
    ...
    if c.batch < F.DIM and not c.diagnostic_transposed_compute_only:
        # Hardware evidence, not a simulator heuristic: canonical ResNet q545 was 5.1106%
        # slower than q535 although Spike predicted a 1.98% win. In this RTL trans_input_3120
        # maps systolic I to min(batch,DIM) and advances output columns one at a time, so batch
        # one uses 1/16 of the row dimension. Preserve the exact route only behind an explicit
        # diagnostic opt-in; production selection fails closed to streamed im2col + LOOP_WS.
        return False, "hardware_cost_guard_transposed_nchw_underfills_systolic_rows"
    ...
    if activation.pitch < c.wi:
        return False, "trans_input_3120_row_pitch_is_smaller_than_logical_width"
    activation_encoding = activation.storage_encoding
    if activation.pitch != c.wi:
        expected_strides = [c.ci * c.hi * activation.pitch,
                            c.hi * activation.pitch, activation.pitch, 1]
        if (activation_encoding is None
                or activation_encoding.get("schema") != "grouped_axes_storage_v1"
                or activation_encoding.get("logical_shape") != [1, c.ci, c.hi, c.wi]
                or activation_encoding.get("physical_shape") != [1, c.ci, c.hi, c.wi]
                or activation_encoding.get("axis_groups") != [[0], [1], [2], [3]]
                or activation_encoding.get("strides_elements") != expected_strides
                or activation_encoding.get("storage_elements") != c.ci * c.hi * activation.pitch):
            return False, "trans_input_3120_pitched_nchw_encoding_is_not_proven"
