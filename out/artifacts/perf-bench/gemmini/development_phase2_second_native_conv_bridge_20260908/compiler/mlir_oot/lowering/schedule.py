...
class Scheduler:
    ...
    def native_convolution_eligibility(self, c: Convolution) -> tuple[bool, str]:
        ...
        activation, weight, dst = self.buf(c.activation), self.buf(c.weight), self.buf(c.dst)
        if c.native_performance_refusal:
            return False, c.native_performance_refusal
                            ...
                            for kcol in range(0, c.kw, kcols):
                                for kch in range(0, c.ci, kchs):
                                    ...
                                    self.emit(
                                        ...
                                        # In trans_input_3120 mode this field is the physical
                                        # NCHW row stride used by LoopConvLdInput's DRAM address;
                                        # the tile's logical icols remains a separate derived
                                        # parameter. This admits proven padded rows without a
                                        # dense staging copy.
                                        in_col_dim=(in_stride if c.compute_only_native else c.wi),
                                        in_channels=c.ci,
                                        ...
                                        input_offset=((kch * c.hi * in_stride
                                                       + (irow + upad) * in_stride
                                                       + (icol + lpad))
