...
class ComputeOnlyLoopConvTest(unittest.TestCase):
    ...
    def test_hardware_cost_guard_rejects_batch_one_transposed_input_by_default(self):
        plan, conv = self.make()
        conv.diagnostic_transposed_compute_only = False
        selected, reason = Scheduler(plan).native_convolution_eligibility(conv)
        self.assertFalse(selected)
        self.assertEqual(
            reason, "hardware_cost_guard_transposed_nchw_underfills_systolic_rows")

    def test_hardware_regressed_route_requires_explicit_diagnostic_opt_in(self):
        plan, conv = self.make()
        selected, reason = Scheduler(plan).native_convolution_eligibility(conv)
        self.assertTrue(selected)
        self.assertEqual(reason, "selected_compute_only_full_width_mvout")
    ...
    def test_pitched_nchw_is_exactly_addressed_but_remains_diagnostic_only(self):
        pitch = 64
        shape = [1, 64, 56, 56]
        ifm_encoding = {
            "schema": "grouped_axes_storage_v1", "logical_shape": shape,
            "physical_shape": shape, "axis_groups": [[0], [1], [2], [3]],
            "dtype": "i8", "strides_elements": [64 * 56 * pitch, 56 * pitch, pitch, 1],
            "storage_elements": 64 * 56 * pitch, "offset_elements": 0,
        }
        buffers = {
            "ifm": Buffer("ifm", shape, "i8", "intermediate", ifm_encoding),
            "weight": Buffer("weight", [64, 64], "i8", "weight"),
            "dst": encoded_dst(co=64, ho=56, wo=56),
        }
        conv = Convolution(
        ...
        selected, reason = scheduler.native_convolution_eligibility(conv)
        self.assertTrue(selected, reason)
        with patch("mlir_oot.lowering.schedule.loop_conv.auto_tile",
                   return_value=(1, 28, 28, 16, 1, 1, 16)):
            scheduler.native_convolution(conv)
        loops = [ins for ins in scheduler.instrs if ins.kind == "loop_conv_ws"]
        self.assertTrue(loops)
        self.assertTrue(all(ins.attrs["in_col_dim"] == pitch for ins in loops))
        expected = 16 * 56 * pitch + 28 * pitch + 28
        self.assertIn(expected, [ins.attrs["input_offset"] for ins in loops])

    def test_malformed_pitched_nchw_encoding_fails_closed(self):
        plan, conv = self.make()
        activation = plan.buffers["ifm"]
        activation.shape = [1, 3, 16, 15]
        conv.wi = conv.wo = 15
        activation.storage_encoding = {
            "schema": "grouped_axes_storage_v1", "logical_shape": activation.shape,
            "physical_shape": activation.shape, "axis_groups": [[0], [1], [2], [3]],
            "dtype": "i8", "strides_elements": [768, 256, 16, 2],
            "storage_elements": 768, "offset_elements": 0,
        }
        selected, reason = Scheduler(plan).native_convolution_eligibility(conv)
        self.assertFalse(selected)
