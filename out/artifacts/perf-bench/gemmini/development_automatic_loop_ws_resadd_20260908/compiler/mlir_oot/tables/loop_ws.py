...
def loop_ws_resadd_static(*, rows, cols, row_stride_a, row_stride_b, row_stride_c,
                          activation=0):
    """Pack the exact ``gemmini_loop_ws(..., K=0, is_resadd=true)`` descriptor."""
    if min(rows, cols, row_stride_a, row_stride_b, row_stride_c) <= 0:
        raise ValueError("LOOP_WS resadd requires positive static geometry")
    i_tiles, j_tiles = (-(-rows // F.DIM), -(-cols // F.DIM))
    values = {
        "I": i_tiles, "J": j_tiles, "K": 0,
        "pad_I": i_tiles * F.DIM - rows,
        "pad_J": j_tiles * F.DIM - cols,
        "pad_K": 0,
        "A_stride": row_stride_a, "B_stride": row_stride_b,
        "C_stride": row_stride_c, "D_stride": 0,
        "A_transpose": 0, "B_transpose": 0, "full_C": 0, "low_D": 0,
        "ex_accumulate": 0, "act": activation,
        "a_spad_id": 0, "b_spad_id": 0, "is_resadd": 1,
    }
    result = []
    for row in contract()["records"]:
        if row["name"] in {"k_LOOP_WS_CONFIG_ADDRS_AB", "k_LOOP_WS_CONFIG_ADDRS_DC"}:
            continue
        result.append((row["funct"], expression(row["rs1"], values),
                       expression(row["rs2"], values)))
    return result
