...
from ..frontend.gemmini_resadd import (
    IntegerResAdd,
    looks_like_candidate as looks_like_resadd_candidate,
    recognize as recognize_integer_resadd,
)
from ..frontend.residual_epilogue import (
    Refusal as FloatResidualRefusal,
    ResidualEpilogue,
    recognize_all as recognize_float_residuals,
)
from ..tables import rtl_facts as F
from .plan import (BLOCK_DIAGONAL_ELEM_BUDGET, Buffer, Contraction, Epilogue,
                   LoweringDeclined, Plan, ResAdd, kernel_args)
from .resadd_selector import assess_float_residual
...
class MixedBuilder:
    ...
    def __init__(self, module, wl: LinalgWorkload):
        ...
        self.integer_resadds: dict[Operation, IntegerResAdd] = {}
        self.resadd_refusals: list[dict[str, Any]] = []
        self.float_residuals: list[ResidualEpilogue] = []
        self.float_residual_refusals: list[FloatResidualRefusal] = []
        self._scratch = 0
    ...
    def build(self) -> Plan:
        ...
        self.float_residuals, self.float_residual_refusals = recognize_float_residuals(ops)
        for op in ops:
            result = recognize_integer_resadd(op)
            if result is not None:
                self.integer_resadds[op] = result
            elif looks_like_resadd_candidate(op):
                self.resadd_refusals.append({
                    "source_op_index": self.source_indices[op],
                    "reason": "integer add is not the exact identity-scale saturating-i8 "
                              "LOOP_WS contract",
                })
        self.absorbed_by = _exclusive_absorbed(mesh_ops)
        for op, formation in self.integer_resadds.items():
            self.absorbed_by[op] = list(formation.support_operations)
        ...
        resadd_set = set(id(op) for op in self.integer_resadds)
        schedule: list[Any] = []
        for op in ops:
            if op in absorbed:
                continue
            ...
            elif id(op) in resadd_set:
                schedule.append(("host", pending))
                pending = []
                schedule.append(("resadd", op))
            else:
                pending.append(op)
        schedule.append(("host", pending))
        ...
        for formation in self.integer_resadds.values():
            needed_by_mesh[formation.lhs] = None
            needed_by_mesh[formation.rhs] = None
        ...
        for kind, payload in schedule:
            if kind == "mesh":
                ...
                continue
            if kind == "resadd":
                self._resadd(self.integer_resadds[payload], in_dram)
                in_dram.add(payload.results[0])
                continue
    ...
    def _select_exact_native_epilogues(self, mesh_ops: list[Operation]) -> None:
        """Select only source chains proven equal to abstract native i8 narrowing."""
    ...
    def _resadd(self, formation: IntegerResAdd, in_dram: set[SSAValue]) -> None:
        """Place one source-proven identity-scale saturating i8 add on LOOP_WS."""
        for operand in (formation.lhs, formation.rhs):
            if operand not in in_dram:
                raise LoweringDeclined(
                    "a LOOP_WS residual add reads an operand no earlier task left in DRAM",
                    op="resadd", shape=list(formation.shape))
        lhs = self.of_value.get(formation.lhs) or self.buffer_for(formation.lhs)
        rhs = self.of_value.get(formation.rhs) or self.buffer_for(formation.rhs)
        dst = self.of_value.get(formation.output) or self.buffer_for(formation.output)
        rows = prod(formation.shape[:-1]) if len(formation.shape) > 1 else 1
        cols = formation.shape[-1]
        self.commands.append({
            "opcode": "RESADD",
            "operands": {"lhs": lhs, "rhs": rhs, "dst": dst},
            "attributes": {
                "implementation": "LOOP_WS", "is_resadd": True,
                "a_scale": 1.0, "b_scale": 1.0, "c_scale": 1.0,
                "relu": formation.relu,
            },
        })
        task = ResAdd(lhs, rhs, dst, rows, cols, formation.relu)
        task.source_op_indices = [
            *[self.source_indices[item] for item in formation.support_operations],
            self.source_indices[formation.operation],
        ]
    ...
    def _mesh(self, op: Operation, in_dram: set[SSAValue]) -> None:
        ...
        self.commands.append({"opcode": "COMMIT",
                              ...
                              "attributes": {"epilogue": (["relu"] if formation is not None
                                                            and formation.relu else []), "output_dtype":
                                             F.ACCUMULATOR_DTYPE if modular else out_dtype}})
        self.ordered.append(
            Contraction(contraction_lhs, rhs, dst, m, k, n,
                        lhs_row_elems=k, rhs_row_elems=n,
                        epilogue=Epilogue(
                            stages=(["relu"] if formation is not None and formation.relu else []),
    ...
    def _command_buffer(self, segments: list[HostSegment]) -> dict[str, Any]:
        ...
        def task_record(index: int, task: Any) -> dict[str, Any]:
            if isinstance(task, HostSegment):
                return {
                    "task_index": index, "kind": "host",
                    "source_op_indices": list(task.source_op_indices),
                    "reads": [name for _, name in task.inputs],
                    "writes": [name for _, name in task.outputs],
                }
            if isinstance(task, ResAdd):
                return {
                    "task_index": index, "kind": "resadd",
                    "source_op_indices": list(task.source_op_indices),
                    "reads": [task.lhs, task.rhs], "writes": [task.dst],
                    "numeric_contract":
                        "clamp_i8(add_i32(sign_extend_i8(a),sign_extend_i8(b)))",
                    "implementation": "gemmini_loop_ws_is_resadd",
                }
            return {
                "task_index": index, "kind": "contraction",
                "source_op_indices": list(task.source_op_indices),
                "reads": [task.lhs, task.rhs],
                "writes": [task.dst] + ([task.accumulator_temporary]
            ...
            "params": {
                ...
                **({"batched_contractions": list(self.batched_contractions)}
                ...
                "global_program_plan": {
                    ...
                    "tasks": [task_record(i, task) for i, task in enumerate(self.ordered)],
                    "source_values": [
                ...
                "gemmini_resadd_placement": {
                    "schema": "gemmini_exact_loop_ws_resadd_placement_v1",
                    "candidate_count": (len(self.integer_resadds) + len(self.resadd_refusals)
                                        + len(self.float_residuals)
                                        + len(self.float_residual_refusals)),
                    "selected_count": len(self.integer_resadds),
                    "selected_source_op_indices": sorted(
                        self.source_indices[op] for op in self.integer_resadds),
                    "refused": list(self.resadd_refusals),
                    "float_residual_refused": [
                        assess_float_residual(item).receipt(item, self.source_indices)
                        for item in self.float_residuals
                    ] + [
                        {"source_op_index": self.source_indices[item.operation],
                         "selected": False, "reasons": [item.reason]}
                        for item in self.float_residual_refusals
                    ],
                    "hardware_semantics":
                        "clamp_i8(add_i32(rne_clamp_i8(a*A_scale),"
                        "rne_clamp_i8(b*B_scale))*C_scale)",
                    "selected_qparams": {"A_scale": 1.0, "B_scale": 1.0,
                                         "C_scale": 1.0},
                    "approximation": False,
                },
            },
    ...
    def _epilogue_receipt(self) -> dict[str, Any]:
        ...
        for producer, formation in self.native_epilogues.items():
            ...
            row.update({
                "selected_lowering": "native_identity_scale_i8_readout",
        ...
        return {
            ...
            "numeric_contract": "exact_for_every_i32_accumulator",
            "approximation": False,
        }
