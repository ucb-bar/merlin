...
from ..frontend.residual_epilogue import recognize_all as recognize_float_residuals
from ..frontend.linalg_reader import HOST_LANE, MESH_LANE, LinalgWorkload
from ..tables import rtl_facts as F
from .plan import (Buffer, Contraction, Epilogue, LoweringDeclined, Plan, ResAdd,
                   kernel_args, row_pitch)
...
class SourceConvBuilder(MixedBuilder):
    ...
    def build(self) -> Plan:
        ...
        for op, formation in self.integer_resadds.items():
            absorbed_by[op] = list(formation.support_operations)
            absorbed.update(formation.support_operations)
        all_owners = {}
        ...
        resadd_set = set(self.integer_resadds)
        schedule: list[Any] = []
        for op in ops:
            if op in absorbed:
                continue
                    ...
                    "tasks": [
                        {"task_index": i,
                         "kind": ("host" if isinstance(task, HostSegment) else
                                  ...
                                  "resadd" if isinstance(task, ResAdd) else
                                  "contraction"),
                         ...
                         "writes": [n for _, n in task.outputs] if isinstance(task, HostSegment)
                                   else ([task.dst] if isinstance(task, (Convolution, ResAdd)) else
                                         [task.dst] + ([task.accumulator_temporary]
                                                       if task.accumulator_temporary else [])),
                         **({} if isinstance(task, (HostSegment, Convolution, ResAdd)) else {
...
def build(module, wl):
    return SourceConvBuilder(module, wl).build()
