...
class Emitter:
    ...
    def emit_loop_ws_resadd(self, ins: Instr) -> None:
        """Lower one exact identity-scale i8 residual tile with ``is_resadd`` set."""
        a = ins.attrs
        lhs, rhs, dst = ins.bufs
        for record in (
            isa.config_ex(dataflow=isa.WEIGHT_STATIONARY, sys_act=isa.NO_ACTIVATION),
            isa.config_st(
                stride=int(a["c_stride"]),
                acc_act=isa.RELU if bool(a["relu"]) else isa.NO_ACTIVATION,
                acc_scale=1.0),
            isa.config_ld(stride=int(a["a_stride"]), scale=1.0,
                          shrunk=True, load_id=0),
            isa.config_ld(stride=int(a["b_stride"]), scale=1.0,
                          shrunk=True, load_id=1),
        ):
            self.rocc(record[0], self.fb.const(record[1]), self.fb.const(record[2]))
        bounds, strides_ab, strides_dc, launch = loop_ws.loop_ws_resadd_static(
            rows=int(a["rows"]), cols=int(a["cols"]),
            row_stride_a=int(a["a_stride"]), row_stride_b=int(a["b_stride"]),
            row_stride_c=int(a["c_stride"]),
            activation=isa.RELU if bool(a["relu"]) else isa.NO_ACTIVATION)
        self.rocc(bounds[0], self.fb.const(bounds[1]), self.fb.const(bounds[2]))
        self.rocc(loop_ws.opcode("k_LOOP_WS_CONFIG_ADDRS_AB"),
                  self.dram_addr(lhs, int(a["a_offset"])),
                  self.dram_addr(rhs, int(a["b_offset"])))
        self.rocc(loop_ws.opcode("k_LOOP_WS_CONFIG_ADDRS_DC"), self.fb.const(0),
                  self.dram_addr(dst, int(a["c_offset"])))
        for record in (strides_ab, strides_dc, launch):
            self.rocc(record[0], self.fb.const(record[1]), self.fb.const(record[2]))
    ...
    def build(self) -> ModuleOp:
        ...
        for ins in self.instrs:
            ...
            elif ins.kind == "loop_ws_resadd":
                self.emit_loop_ws_resadd(ins)
                loop_ws_state.clear()
