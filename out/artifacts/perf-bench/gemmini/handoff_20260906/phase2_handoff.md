...
FireSim queue job 536 completed at **555,991,472 measured cycles**, with all 1,000 int8 logits
matching the frozen reference. The executable contains 53 Merlin-generated native `LOOP_CONV`
kernels, 3,550 convolution descriptors, and 2,316 i32-bias `LOAD3` descriptors. The hardware run
therefore validates the public native-convolution mechanism and demonstrates sub-billion headroom.

The result is deliberately classified as a hybrid diagnostic rather than a compiler checkpoint.
A TVM-generated runner still owns the activation arena and complete graph, including 20 residual
rescale/add/ReLU blocks, padding/max-pool, average pool/flatten, and dense. The 2.368x ratio against
q535 is not apples-to-apples and must not appear as Merlin's end-to-end speedup.

The experiment controller may mine only general mechanisms from q536: narrow scalar epilogue
emission, correct i32 bias addressing, descriptor/capability checks, and strict warm/reentrant
tests. It must reject the model-specific stitcher, layer-index dispatch, mixed compiler runner, and
TVM arena. Promotion still requires source-semantic ownership, the four-model compile gate, exact
warm/measured validation, and a content-addressed compiler artifact.

Evidence:

- `../q536_native_loopconv_hybrid_diagnostic_20260908.md`
- `../q536_native_loopconv_hybrid_receipt.json`
- full source package:
  `../development_native_loop_conv_int8_epilogue_v1_20260908/firesim_candidate_warm_v1`


## 31. Final combined compiler: experiment output and exact boundary

The complete directly invocable Phase-2 compiler is
`development_phase2_final_combined_exact_residual_global_encoding_20260908`, compiler SHA-256
`48694957d14c9608960f7ac2b6cd08b72b15c55e4a11ff26e1cf952e0cd7607e` over 45 files. Its 2,760-line
review patch has SHA-256 `0e611e3c9c2231196653bfdb906a45843b89f8ee14f4ffbefe683116a9f2477a`.
The release verifier binds the compiler, tests, patch, final receipt, q535 hardware lineage, two
warm/reentrant executions, four full-model compiles, opt-in bridge, and structural audits.

### 31.1 What is assembled

| mechanism | result | purpose in the loop |
| --- | --- | --- |
| standard LLVM scalar conversions | preserved from q534 | delete repeated host scalar work |
| affine segmented im2col | preserved from q535 | delete address/bounds/clamp work over full layers |
| ordered quantized epilogue | implemented, fail closed | expose/delete exact contraction boundaries |
| true second-tensor residual | 15 ResNet sites selected | fuse global branch joins without treating residual as bias |
| global encoding solver | implemented, fail closed | price layout transforms, fanout, lifetime, capacity, and retained encodings |
| native `LOOP_CONV` | capability-selected | delete im2col only when semantic/layout/geometry proof permits |
| dynamic-weight bridge | explicit opt-in | expose transformer contractions without false f32 equivalence |

The integration has no model, layer, or fixed-shape cases. Sixty-five focused tests enforce exact
stage order, unique ownership, real-bias hazards, warm-safe zero-bias initialization, mixed conv and
matmul formation, post-fusion encoding facts, affine spans, native-convolution geometry/capacity,
residual broadcasting, workspace reuse, and bridge gating.

### 31.2 Fast evidence and four-model gate

The exact non-residual witness runs warm then measured, checks 323 outputs, and measures 56 Spike
proxy cycles versus 13,760 before fusion. The encoded two-convolution chain runs warm then measured,
checks 96 outputs, selects two native convolutions with zero im2col tasks, and measures 102 proxy
cycles. These are bounded mechanism proofs, not absolute target performance.

All four complete graphs compile sequentially without L3 or FireSim:

| model | source ops | accelerator tasks | host tasks | maximum RSS |
| --- | ---: | ---: | ---: | ---: |
| ResNet-50 W8A8 | 1,240 | 54 | 51 | 199,688 KiB |
| TinyLLaMA | 718 | 0 | 1 | 53,760 KiB |
| LSTMNetViT | 2,302 | 37 | 38 | 187,412 KiB |
| SmolVLA denoise | 11,910 | 116 | 117 | 308,264 KiB |

The final ResNet target is byte-identical to the residual child. Its exact transferred local A/B is
506,265,226->492,147,976 Spike cycles, 14,117,250 saved (2.7885%; 1.028685x), with 1,000/1,000
logits exact. Tasks fall 109->105, host tasks 55->51, ABI pointers 393->389, and physical
intermediates by 7,340,032 bytes. No final-combined hardware number is claimed; q535 at
1,316,619,699 cycles remains the latest full-Merlin FireSim checkpoint.

### 31.3 What remains and why

The frozen PT2E ResNet selects 0/53 exact native-narrow convolutions. All layers have per-channel
weight scale and FP32 bias that the current one-scalar-scale narrow `LOOP_CONV` store cannot
represent exactly. A naive Conv1 fold mismatches 143/802,816 values before pooling and 33/200,704
after pooling. The solver therefore reports, but does not realize, a conditional output-boundary
reduction from 44,455,936 to 11,113,984 bytes.

The next exact target-neutral contract must describe scale granularity/axis, zero points, bias
domain, ordered FP32 stages, residual domains, rounding, saturation, and endpoint
`{narrow_i8, full_i32, accumulator_handle}`. It can select full-i32/accumulator output plus exact
host/RVV epilogue, narrow output after arithmetic-equivalence proof, or fallback. Native-aligned
W8A8 requires its own calibration, golden, accuracy, and provenance gate. q536's 555,991,472 cycles
remain a hybrid opportunity bound because TVM owns its host graph and arena; its 2.368x arithmetic
ratio versus q535 is not a Merlin speedup.

No final-combined FireSim, L3, or queue action occurred; no sub-billion compiler claim or formal
roofline is made. Phase 1 remains frozen at 92/96.

### 31.4 Downstream invocation and evidence
...
Object/ELF construction remains runner-owned. The final artifact contains `README.md`,
`STATUS.json`, `FINAL_COMBINED.md`, `validation/final_combined_receipt.json`, `verify.py`, and
`final_combined_compiler.patch`. Hardware evidence is in
`q535_affine_im2col_hardware_result_20260908.md`,
`q535_affine_im2col_hardware_receipt.json`, and the sealed q535 bundle. The q536 diagnostic,
standalone epilogue/residual/encoding receipts, and
`universal_narrow_epilogue_recovery_v1_20260908` capability audit provide the remaining lineage.
