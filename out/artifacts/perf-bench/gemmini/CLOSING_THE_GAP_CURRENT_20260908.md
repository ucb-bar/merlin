...
### 24.1 q536 is a native-convolution opportunity bound, not a compiler score

FireSim queue job 536 measured **555,991,472 cycles** with all 1,000 int8 logits exact and the
required queue-only `kill -> infrasetup -> runworkload -> kill` lifecycle. It validates 53
Merlin-generated native `LOOP_CONV` kernels, 3,550 convolution descriptors, and 2,316 i32-bias
`LOAD3` descriptors, demonstrating sub-billion native-convolution headroom.

The executable is hybrid: a TVM-generated runner owns the activation arena, call graph, residual
blocks, padding/max-pool, average-pool/flatten, and dense output. Its 2.368x arithmetic ratio versus
q535 is therefore not a Merlin compiler speedup. Evidence is
...
The directly invocable compiler is:

```text
../development_phase2_final_combined_exact_residual_global_encoding_20260908
```

Its 45-file compiler SHA-256 is
`48694957d14c9608960f7ac2b6cd08b72b15c55e4a11ff26e1cf952e0cd7607e`; its 2,760-line review
patch SHA-256 is `0e611e3c9c2231196653bfdb906a45843b89f8ee14f4ffbefe683116a9f2477a`.
The verifier binds 65 tests, two exact warm/reentrant witnesses, the four-model compile gate,
opt-in bridge, source ownership, ABI, encoding/refusal evidence, final receipt, and q535 lineage.

The artifact contains only general mechanisms: q534 native scalar lowering, q535 affine im2col,
ordered quantized epilogues, true second-tensor residuals, global encoding/fanout/lifetime/capacity
planning, fail-closed native convolution, and the explicit default-off transformer bridge. There
are no model, layer, or fixed-shape dispatches.

The exact epilogue witness checks 323 outputs and measures 13,760->56 Spike proxy cycles; the
encoded two-convolution witness checks 96/96 outputs at 102 cycles with two native convolutions and
zero im2col tasks. ResNet selects 15 exact residuals, reducing tasks 109->105, host tasks 55->51,
ABI pointers 393->389, and physical intermediates by 7,340,032 bytes. Its target is byte-identical
to the residual child, transferring the exact local 506,265,226->492,147,976-cycle A/B (2.7885%;
1.028685x), with 1,000/1,000 logits exact. This changed object has no hardware result; q535 remains
the latest honest full-Merlin FireSim checkpoint at 1,316,619,699 cycles.

Frozen PT2E ResNet admits 0/53 exact native-narrow convolutions. Per-channel scales and FP32 bias
cannot be expressed by the current one-scalar-scale narrow `LOOP_CONV` store. Conv1's naive fold
mismatches 143/802,816 values before pooling and 33/200,704 after pooling; the conditional
44,455,936->11,113,984-byte boundary reduction is opportunity, not realized work. The next exact
contract needs scale granularity/axis, zero points, bias/residual domains, ordered FP32 stages,
rounding/saturation, and `{narrow_i8, full_i32, accumulator_handle}` endpoints. Native-aligned W8A8
is a separate calibration/golden/accuracy/provenance contract.

TinyLLaMA, LSTMNetViT, and SmolVLA compile/non-regress; byte identity is not a performance win.
...
Object/ELF construction remains runner-owned. The artifact's `README.md`, `STATUS.json`,
`FINAL_COMBINED.md`, `validation/final_combined_receipt.json`, `verify.py`, and review patch are the
release index. Standalone exact-epilogue, residual, global-encoding, q535/q536, and
`universal_narrow_epilogue_recovery_v1_20260908` artifacts preserve full lineage.
