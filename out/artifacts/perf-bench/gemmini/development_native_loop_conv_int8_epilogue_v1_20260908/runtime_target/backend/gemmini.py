...
def _flat_matrix_shape(spec: dict, *, name: str) -> tuple[int, int]:
    ...
    if not isinstance(shape, list) or len(shape) < 1 or any(
            not isinstance(dim, int) or isinstance(dim, bool) or dim <= 0 for dim in shape):
        raise CodegenError(f"native interface tensor {name!r} needs a positive rank >= 1 shape, got {shape!r}")
    if len(shape) == 1:
        return 1, shape[0]
    rows = 1
    for dim in shape[:-1]:
...
def _native_interface_harness_c(cb: dict, command: dict, *, inputs: dict | None = None) -> str:
    ...
    if opcode == "ATTENTION_QK":
        ...
        if operands.get("bias") is not None:
            required.append(operands.get("bias"))
    ...
    for name in args:
        ...
        role = spec.get("role")
        dtype = str(spec.get("dtype") or "")
        if not (dtype == "i8" or (role == "bias" and dtype == "i32")):
            raise CodegenError(
                f"native interface input {name!r} must use i8, except a convolution bias "
                f"which must use accumulator i32; got role={role!r}, dtype={dtype!r}")
        ...
        decls.append(container.decl(
            f"T_{name}", prows * pcols, const=True,
            initializer=",".join(str(word) for word in container_words(padded, dtype))))
