...
## 10. Native LOOP_CONV_WS facts used by this candidate

These facts were re-authored from Jack's pinned public Gemmini sources rather than copied from a
compiler-specific model schedule. Paths are outside the artifact only as provenance; the exact
facts and hashes are recorded here so the snapshot remains auditable.

| fact | pinned source and SHA-256 | compiler use |
|---|---|---|
| `UniversalResNet50Config` sets `has_loop_conv = true`, `has_training_convs = true`, `has_max_pool = true`, and `acc_read_full_width = false` | `/scratch/jack/chipyard/generators/gemmini/src/main/scala/gemmini/UniversalResNet50Config.scala` lines 53, 69–75; `cb6a5248cbeeab2d5ba9564d1e6ca5a3be17287512e5050991f2eb9edaffa8f5` | capability facts in `tables/rtl_facts.py`; the selector may consider LOOP_CONV but cannot claim a wide result |
| the elaborated Gemmini configuration fixes `hasIm2Col = false` | `/scratch/jack/chipyard/generators/gemmini/src/main/scala/gemmini/GemminiConfigs.scala` line 206; `06d763189914715ced1a547dc56386f61066d18f9ae2c8d01ca3b19de6fd8416` | prevents describing LOOP_CONV as a separate hardware-im2col datapath |
| `gemmini_loop_conv_ws` issues CONFIG_1 through CONFIG_6, then LOOP_CONV_WS (functs 16–21, 15), with the exact rs1/rs2 bit fields transcribed in `tables/loop_conv.py` | `/scratch/jack/chipyard/generators/gemmini/software/gemmini-rocc-tests/include/gemmini.h` lines 384–401; `2f97540b6ba4378c22572e7d4a41d5d0c95931ac55dcf1fa5246b3923e4ad3ff` | verified target op and LLVM inline-asm emission; a literal-expansion unit test compares every static record |
| `gemmini_extended3_config_ex` names `C_stride` before `A_stride`; the public convolution call supplies `1, stride` | same header lines 260 and 2564 | `schedule.py` emits `c_stride=1`, `a_stride=conv_stride`; the first reduced Spike run caught the initially reversed form |
| `tiled_conv_total_spad_rows` and `tiled_conv_stride_auto` use half the scratchpad and accumulator for double buffering, reduce the largest tile extent, preserve output width utilization, then greedily restore extents | same header lines 1817–1850 and 2664–2820 | `loop_conv.total_rows` and `auto_tile`, restricted to the admitted no-pool, non-transposed, non-downsampled subset |
| `LoopConvSt` constructs both ordinary and pooled MVOUT local addresses with `read_full = false.B` | `/scratch/jack/chipyard/generators/gemmini/src/main/scala/gemmini/LoopConv.scala` lines 1000 and 1009; `8609d9f27ff037ffdeaba53f70e8c0a9b3a994c3871e9eacbee20e58d3dc6c6d` | `native_conv_selector.py` rejects i32/full-width boundaries instead of silently narrowing them |

The local execution checkout has a newer header SHA
`007826db4fdaf35783e5d91ad3f21c2df9a310c1c51d53499f7745fc379974de`, but its
`gemmini_loop_conv_ws` macro is textually identical and its `LoopConv.scala` has the same SHA. The
two exact reduced Spike probes therefore qualify these descriptors against the locally executable
target without replacing Jack's pinned sources as the authoring evidence.

### Output permutation and per-channel epilogue audit

`trans_output_1203` is **not** an NCHW conversion. `LoopConvSt` lines 891–893 compute its address as
`((orow * Wo * N + ocol * N + b) * Co + och)`, i.e. the permutation `(N,H,W,C) -> (H,W,N,C)`.
The non-transposed form is NHWC. Neither expression produces `(N,Co,H,W)`.

A restricted NCHW construction is arithmetically possible but is not enabled here: force
`batches=1` and `pochs=1`, point each launch at one `(batch, channel)` plane, and use
`out_stride=1`. The non-transposed address then walks one contiguous H×W plane. This costs at
least `N*Co` launches and still needs its own exact reduced test and cost evidence.

Bias is descriptor-local (`CONFIG_6` carries its pointer) and could initialize a singleton channel
from an exact acc_t bias, which the current graph does not yet provide. No-pool ReLU and scale are
global CONFIG_ST state: `LoopConvSt` emits MVOUT directly, while the descriptor's activation field
is used only when the pool path reconfigures the store. A uniform activation/scale can be issued
once. Changing CONFIG_ST per singleton-channel launch can race the concurrent loop slots unless
each change is separated by a completion fence. Per-channel BN scale is therefore refused: the
provably exact available form would serialize every channel and defeat this route's overlap.
