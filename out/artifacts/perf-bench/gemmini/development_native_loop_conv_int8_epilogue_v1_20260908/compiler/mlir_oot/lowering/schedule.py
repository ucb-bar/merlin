...
class Scheduler:
                ...
                for m0 in range(0, mt, bm):
                    for n0 in range(0, nt, bn):
                        for k0 in range(0, kt, bk):
                            ...
                            c_offset = (out_base + m0 * DIM * out_plane_pitch + n0 * DIM) * DTYPE_BYTES["i32"]
                            self.emit(
    ...
    def native_convolution(self, c: Convolution) -> None:
        ...
        # CONFIG_ST is global state consumed asynchronously by LoopConvSt.  Do
        # not let a following layer change its scalar while an older slot can
        # still be moving out.  This is conservative and correctness-first.
                            ...
                            for kcol in range(0, c.kw, kcols):
                                for kch in range(0, c.ci, kchs):
                                    ...
                                    self.emit(
                                        ...
                                        no_bias=not (c.bias and first), no_pool=True,
                                        ...
                                        output_offset=((b * c.ho * c.wo
                                                        + orow * c.wo + ocol)
                                                       * out_stride + poch),
                                        # LOAD3 consumes precisely the bias slice for this output-
                                        # channel tile.  A zero offset is only correct for poch=0;
                                        # keeping it for wider layers repeats channel 0's bias.
    ...
    def contraction(self, c: Contraction) -> None:
        ...
        use_loop_ws = (
            not e.bias
            and out_dtype == F.ACCUMULATOR_DTYPE
            and acc_act == isa.NO_ACTIVATION
            and acc_scale == 1.0
        )
        resident_loop_shape = (
                ...
                for n0 in range(0, nt, bn):
                    for k0 in range(0, kt, bk):
                        ...
                        c_offset = ((out_row_offset + m0 * DIM) * out_row_elems
                                    + n0 * DIM) * DTYPE_BYTES[F.ACCUMULATOR_DTYPE]
                        self.emit(
