...
def read(module: ModuleOp) -> LinalgWorkload:
    ...
    for op in module.walk():
        ...
        if region is None:
            ...
            if structural_matmul:
                dtype = _shape_dtype(op.operands[0].type)[1]
            region = Region(rid,
                            family="contraction" if structural_matmul else _s(op, "prov.family"),
                            op=_s(op, "prov.op"),
                            dtype=dtype)
            seen[rid] = region
    ...
    if not wl.regions:
        ...
        wl.regions.append(place(Region("region_0", family="unknown", op="", dtype=dtype)))
    return wl
