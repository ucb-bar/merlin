...
def validate(cb: dict[str, Any]) -> list[str]:
    ...
    if not commands and "declined" not in cb and not cb.get("kernel_abi"):
        problems.append(
            "commands must be non-empty unless the buffer declines or declares a whole-program kernel")
