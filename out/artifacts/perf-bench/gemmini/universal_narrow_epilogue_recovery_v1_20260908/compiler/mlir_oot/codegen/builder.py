...
class FnBuilder:
    ...
    def __init__(self, arg_types: list[Attribute]):
        ...
        self.emission_attributes: dict[str, Attribute] = {}

    def _decorate(self, op):
        op.attributes.update(self.emission_attributes)
        return op

    # -- primitives --------------------------------------------------------------------------
    def add(self, op):
        self.blk.add_op(self._decorate(op))
        return op
    ...
    def for_range(self, count: int, body: Callable[[SSAValue], None]) -> None:
        """Emit the source loop as CFG before scalar code expansion."""
        if count <= 0:
            return
        ...
        self.add(llvm.BrOp(header, self.const(0)))
        self.blk = header
        iv = header.args[0]
        condition = self.add(llvm.ICmpOp(iv, self.const(count), IntegerAttr(2, i64))).results[0]
        self.add(llvm.CondBrOp(condition, loop, [], after, []))
        self.blk = loop
        body(iv)
        self.add(llvm.BrOp(header, self.add_i(iv, self.const(1))))
        self.blk = after
