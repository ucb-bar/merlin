...
class SmolVlaDenoiseStep(nn.Module):
    ...
    def forward(self, img, img_mask, lang_tokens, lang_masks, state, noise):
        ...
        keep = torch.cat(
            [
                img_mask.to(torch.bool)[:, None].expand(-1, NIMG),
                lang_masks.to(torch.bool),
                torch.ones_like(img_mask, dtype=torch.bool)[:, None],
            ],
            dim=1,
        )
        ...
        prefix = prefix + self.pre_attn(self.n_pre1(prefix), rope_pos=pos, add_mask=pre_mask, fused=True)
        ...
        causal = torch.full((CHUNK, CHUNK), -1.0e4).triu(1)[None, None]
