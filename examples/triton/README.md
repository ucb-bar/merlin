...
Everything up to and including a numerically checked command buffer needs a fresh clone, its
venv, and the optional pinned Triton dependency — no external simulator or board. The reference
target package is tracked in the repo; `preflight` reports any missing prerequisite.
