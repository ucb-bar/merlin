# Saturn OPU: workflow map

The single [definition](experiment.yaml) is registered as `saturn-opu-functional` in the
[catalog](../../experiments/catalog.yaml). Its declared runtime target is
`saturn_opu_mxv256d128`; do not replace it with a provider name or another OPU configuration.
...
`merlin experiment inspect saturn-opu-functional --phase 1` inspects configuration without
starting an experiment. `merlin experiment runs --target saturn_opu_mxv256d128` discovers
