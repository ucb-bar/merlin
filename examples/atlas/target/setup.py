...
`--write-env` explicitly appends local configuration. Status checks are not certification.

Usage:
  python examples/atlas/target/setup.py                 # check + report (default)
  python examples/atlas/target/setup.py --write-env     # + append MERLIN_EXT_* to .env
  python examples/atlas/target/setup.py --sync-npu-model
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        "Supplying this option explicitly requests materialization.",
    )
    ap.add_argument(
        "--materialize-target-package", action="store_true", help="derive the OOT definition package (opt-in)"
    )
    ap.add_argument(
        "--no-target-package", action="store_true", help="skip materializing the OOT target-definition package"
    )
    a = ap.parse_args(argv)
    if a.no_target_package and (a.materialize_target_package or a.target_package_dir):
        ap.error("--no-target-package conflicts with an explicit materialization request")
    ...
    # Package presence is a setup check, not proof that the model imports or executes.
    ...
    if ok and (a.materialize_target_package or a.target_package_dir):
        try:
