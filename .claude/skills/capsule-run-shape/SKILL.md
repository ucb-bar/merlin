...
## The engine an unflagged self-check runs on

A tier is a FIDELITY, not a simulator, so *which* elaborated-RTL engine answers is chosen by
`rtl_engine_policy.ENGINE_PRIORITY` over the engines the target actually has. The GRADE has routed
through that policy for a while (`[oracle] <target> L3 engine: gsim [elaborated_rtl] (over vcs)`); the
agent's own self-check did not, and fell back to the historical Verilator binding whenever
`MERLIN_REQUIRED_RTL_ENGINE` was unset. Measured on `gem_p1_cc_20260922_1907`: the grader ran GSIM
while every self-check ran Verilator on the same design, and self-check turnaround went 6 min, 7 min,
75 min, 96 min across five requests.

- `agent_selfcheck._default_sim()` now resolves pin → `capsule_runner.describe_l3_engine` → a
  visible, announced fallback. `spike` is never an implicit RTL default.
- The in-sandbox shim and the broker send **no** `sim` when the caller named none — an explicit `sim`
  in a request is a choice the broker must honour, so a literal there silently overrides discovery.
- The per-edit loop names the cheap tier (`--tiers <screen>`); the certifying run is the flagless one.
