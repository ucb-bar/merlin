// QNN kernel: fp32 elementwise Subtract over shape [1,16].
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
	...
	VALIDATE(getQnnGraphConfigFromInfo(
				 "sub_f32", graphsConfigInfo, numGraphsConfigInfo, gc),
		err);
	VALIDATE(model.initialize(backendHandle, interface, contextHandle,
				 "sub_f32", false, DO_GRAPH_NODE_VALIDATIONS, gc),
		err);
	...
	VALIDATE(
		addOp(model, "sub_op", QNN_OP_ELEMENT_WISE_SUBTRACT, {"a", "b"}, to),
		err);
