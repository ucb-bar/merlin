// QNN kernel: fp32 Tanh over shape [1,16].
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
	...
	VALIDATE(getQnnGraphConfigFromInfo(
				 "tanh_f32", graphsConfigInfo, numGraphsConfigInfo, gc),
		err);
	VALIDATE(model.initialize(backendHandle, interface, contextHandle,
				 "tanh_f32", false, DO_GRAPH_NODE_VALIDATIONS, gc),
		err);
	...
	VALIDATE(addOp(model, "tanh_op", QNN_OP_TANH, {"input"}, to), err);
