// QNN kernel: dronet's first Conv2D layer (the stem). Realistic per-layer
// workload for benchmarking: significantly more MACs than the 1×16
// elementwise kernels we've been using to demonstrate plumbing.
//
// Shape (NHWC layout per QNN convention):
//   input  : [1, 224, 224, 1]   fp32 APP_WRITE
//   weight : [5, 5, 1, 32]      fp32 STATIC (HWCF; 800 floats)
//   bias   : [32]               fp32 STATIC (32 floats)
//   output : [1, 112, 112, 32]  fp32 APP_READ
//
// Conv2D: kernel 5×5, stride 2, pad 2 (SAME). MAC count =
//   112 × 112 × 32 × (5 × 5 × 1) = 10,035,200 ≈ 10M MACs.
//
// fp32 weight/bias are baked in (uniform 0.001 / 0.0). With all-1.0 input,
// each output pre-bias = 5×5×1.0×0.001 = 0.025 (assuming all kernel
// positions inside; edge positions less). Useful as a deterministic
// non-trivial workload, not a faithful dronet weight initialization.
...
namespace {
constexpr uint32_t kKh = 5, kKw = 5, kInC = 1, kOutC = 32;
constexpr uint32_t kInH = 224, kInW = 224;
constexpr uint32_t kOutH = 112, kOutW = 112;

float g_weight[kKh * kKw * kInC * kOutC];
float g_bias[kOutC];

void initialize_weights() {
	for (uint32_t i = 0; i < sizeof(g_weight) / sizeof(float); ++i) {
		g_weight[i] = 0.001f;
	}
...
uint32_t g_pad_amount[4] = {2, 2, 2, 2};
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
	...
	VALIDATE(getQnnGraphConfigFromInfo("conv_layer_dronet_stem",
				 graphsConfigInfo, numGraphsConfigInfo, gc),
		err);
	VALIDATE(
		model.initialize(backendHandle, interface, contextHandle,
			"conv_layer_dronet_stem", false, DO_GRAPH_NODE_VALIDATIONS, gc),
		err);
