// QNN kernel: fp32 Relu6 over shape [1,16].
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
	...
	VALIDATE(getQnnGraphConfigFromInfo(
				 "relu6_f32", graphsConfigInfo, numGraphsConfigInfo, gc),
		err);
	VALIDATE(model.initialize(backendHandle, interface, contextHandle,
				 "relu6_f32", false, DO_GRAPH_NODE_VALIDATIONS, gc),
		err);
	...
	VALIDATE(addOp(model, "relu6_op", QNN_OP_RELU6, {"input"}, to), err);
