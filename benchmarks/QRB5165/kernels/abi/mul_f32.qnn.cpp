// QNN kernel: fp32 elementwise Multiply over shape [1,16].
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
	...
	VALIDATE(getQnnGraphConfigFromInfo(
				 "mul_f32", graphsConfigInfo, numGraphsConfigInfo, gc),
		err);
	VALIDATE(model.initialize(backendHandle, interface, contextHandle,
				 "mul_f32", false, DO_GRAPH_NODE_VALIDATIONS, gc),
		err);
	...
	VALIDATE(
		addOp(model, "mul_op", QNN_OP_ELEMENT_WISE_MULTIPLY, {"a", "b"}, to),
		err);
