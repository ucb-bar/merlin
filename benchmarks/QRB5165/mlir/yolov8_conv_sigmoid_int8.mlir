// NCHW int8 conv + dequant + Sigmoid — exercises the recognizer's
// `_classify_activation_generic` for the closed-form
//   sigmoid(x) = 1 / (1 + exp(-x))
// body shape (negf → exp → addf 1 → divf 1/x). Lowering emits
// `ElementWiseNeuron(Sigmoid)` between the Conv2d and the post-Conv
// Transpose with shared output q-params.
//
// Same conv shape as `yolov8_stem_conv_int8.mlir`. The Sigmoid body
// pattern matches what real yolov8 emits inside its SiLU; the
// Conv+SiLU recognizer (multi-op fusion: ElementWiseNeuron(Sigmoid)
// → ElementWiseMultiply with the conv input) is a follow-up.

module {
  func.func @yolov8_conv_sigmoid_int8(%input: tensor<1x3x8x8xi8>)
      -> tensor<1x16x4x4xf32> {
    ...
    return %sig : tensor<1x16x4x4xf32>
  }
}
