// NCHW int8 conv + dequant + Tanh — single-op `math.tanh` body.
// Same shape as the other yolov8 fixtures; just the activation differs.

module {
  func.func @yolov8_conv_tanh_int8(%input: tensor<1x3x8x8xi8>)
      -> tensor<1x16x4x4xf32> {
    ...
    %tanh_init = tensor.empty() : tensor<1x16x4x4xf32>
    %tanh = linalg.generic {
        indexing_maps = [
          affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>,
          affine_map<(d0, d1, d2, d3) -> (d0, d1, d2, d3)>
        ...
        outs(%tanh_init : tensor<1x16x4x4xf32>) {
      ^bb0(%in: f32, %y: f32):
        %t = math.tanh %in : f32
        linalg.yield %t : f32
    } -> tensor<1x16x4x4xf32>

    return %tanh : tensor<1x16x4x4xf32>
  }
}
