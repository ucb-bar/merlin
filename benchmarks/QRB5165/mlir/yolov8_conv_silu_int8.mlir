// NCHW int8 conv + dequant + SiLU (synthetic shape).
//
// SiLU(x) = x * sigmoid(x). Real yolov8 emits this with requantize-to-i8
// round-trips between sigmoid and multiply (a 5-generic chain); this
// fixture isolates the "ideal" 2-generic shape (sigmoid then multiply)
// to exercise the multi-op fusion plumbing in the recognizer. The real-
// yolov8 SiLU pattern (with the requantize round-trips) lands as a
// follow-up extension that walks past the extra dequant/requant ops.

module {
  func.func @yolov8_conv_silu_int8(%input: tensor<1x3x8x8xi8>)
      -> tensor<1x16x4x4xf32> {
    ...
    // Dequantize (becomes the `x` operand of SiLU)
    ...
    // Sigmoid: 1 / (1 + exp(-x)) — produces sigmoid(x)
    ...
    %silu = linalg.generic {
        ...
        iterator_types = ["parallel", "parallel", "parallel", "parallel"]
      } ins(%deq, %sig : tensor<1x16x4x4xf32>, tensor<1x16x4x4xf32>)
