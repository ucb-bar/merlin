...
**Geomean cycles over the 24 cycle-accurate kernels** (lower = faster; golden is the hand-tuned reference): 1.golden (C lib) = 3084.4, 2.baseline-gen (v0) = 6941.7, 3.merlin-gen (v1) = 5222.5, (extra) merlin-native ref = 6941.7.
...
- **IREE arm (4):** the deprecated-merlin C++ Gemmini dialect genuinely fires (161 `gemmini.*` RoCC instrs in the dispatch) and PASSES on spike; but it currently runs spike-only, so it has no cycle-accurate column yet (IREE-runtime-on-RTL is heavy — deferred to FireSim L5). Its spike PASS confirms functional correctness of the hand-written dialect (verified against an all-ones reference inside the IREE runner: each output == K — data-independent, distinct from the exact-int shared-golden check the other arms use).
...
- **FireSim L5 (pending):** the cycle-accurate numbers for 64³+ and the model/attention shapes — the performance-relevant sizes — require FireSim; tracked as the L5 backfill.
- `to/err` = verilator timeout (>900s) or runner error; `·` = not run.
