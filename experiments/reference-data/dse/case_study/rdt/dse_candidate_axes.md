# Structural DSE candidate axes — rdt

- workload class: **flow_matching_action_head**  ·  K=5, H=64, control_rate=30.0 Hz
...
- **signal**: temporal_reuse — action-head weights are loop-invariant across K=5 steps; a flat capture sees one use and hides this axis
- **evidence**: `{'K': 5, 'loop_invariant_state': ['weights'], 'head_phases': ['head']}`
...
- **attributed IR facts (Level-1)**: `{'matmul_count': 21, 'macs_per_invocation': 39466041344, 'weight_bytes': 393216000, 'activation_bytes_per_invocation': 119139840, 'invocations': 5, 'macs_total': 197330206720, 'activation_bytes_total': 595699200}`
...
## autonomous_K_loop
- **signal**: temporal_reuse — a bounded K=5 loop with loop-carried state can run device-side
- **evidence**: `{'K': 5, 'loop_carried_state': []}`
...
- **evidence**: `{'backbone_phases': ['backbone'], 'head_phases': ['head'], 'K': 5, 'H': 64, 'control_rate_hz': 30.0, 'replan_deadline_ms': 2133.3333333333335}`
...
- **evidence**: `{'H': 64, 'control_rate_hz': 30.0, 'replan_deadline_ms': 2133.3333333333335}`
...
## command_batching
- **signal**: cpu_coupling — the K=5 head steps issue repeated host submits a flat capture hides
- **evidence**: `{'K': 5, 'note': 'K per-step submits are collapsible to one buffer'}`
