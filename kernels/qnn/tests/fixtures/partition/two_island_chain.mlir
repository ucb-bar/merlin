// Two-island fixture: a conv → reshape chain. The partitioner should
// emit ONE conv island + ONE reshape island, with the conv's output
// crossing the boundary as the reshape island's input.

module {
  func.func @two_island_chain(%input: tensor<1x3x8x8xi8>)
      -> tensor<1x256xf32> {
    ...
    // Boundary between conv island and reshape island: the deq result
    // is consumed by the i8 quantize that feeds the reshape, but in
    // this synthetic fixture we directly cast f32 → i8 elementwise.
    ...
    // Convert i8 reshape result to f32 for return.
    %out_init = tensor.empty() : tensor<1x256xf32>
    %out = linalg.generic {
        indexing_maps = [
      ...
      } ins(%reshaped : tensor<1x256xi8>)
        outs(%out_init : tensor<1x256xf32>) {
      ^bb0(%in: i8, %y: f32):
        %f = arith.sitofp %in : i8 to f32
        linalg.yield %f : f32
    } -> tensor<1x256xf32>

    return %out : tensor<1x256xf32>
  }
}
