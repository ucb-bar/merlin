...
_RVV_ROUTES: list[_Route] = [
    ...
    _Route(
        ...
        shape_regimes=("skinny", "square_small"),
    ),  # small-N attention / skinny contractions
    _Route(
        ...
        shape_regimes=("vector", "square_small", "skinny"),
    ),  # small-/skinny-M decode matmuls
    _Route(
        ...
        promise_comparison=lambda d: (
            "exact"
            if (_mr_of(d.expert) is not None and _mr_of(d.ours) is not None and _mr_of(d.expert) < _mr_of(d.ours))
            else "at_least"
        ),
        ...
        change=lambda d: (
            ...
            if not (_mr_of(d.expert) is not None and _mr_of(d.ours) is not None and _mr_of(d.expert) < _mr_of(d.ours))
            else "lower the matmul register-block MR to the expert's, taking reuse from the "
    ...
    _Route(
        ...
        change=lambda d: (
            "widen the vector register group so the emitted vsetvli carries a higher "
            "LMUL -- directly, leaving every tile and vector size unchanged"
            if _is_higher(d)
            else "narrow the vector group to the expert's LMUL so the same elements occupy "
    ...
    _Route(
        ...
        action_class="KNOB",
        target_seam="schedule:dtype_strategy (accumulate-width datapath)",
    ...
    _Route(
        ...
        when=lambda d: bool(d.expert) and d.expert != d.ours,
        action_class="KNOB",
        target_seam="schedule:dtype_strategy (element-width datapath)",
    ...
    _Route(
        ...
        "registered feature does emit 4.9x more vector instructions with output that was "
        "bit-identical on deepjscc and lstmnetvit but WRONG on small_llama -- a "
        "packed vector<Nxi1> store read back one-byte-per-element, fixed in "
        "d4f86238; the original claim generalised one model's digest to all -- "
...
def register_route(backend: str, route: _Route) -> None:
    ...
    if any(
        r.axis == route.axis and r.target_seam == route.target_seam and r.action_class == route.action_class
        for r in routes
    ):
        return
    routes.append(route)
...
REBUILD_SCOPES: tuple[str, ...] = ("none", "schedule", "target-package", "compiler", "runtime", "full")
...
def route(divergence: Divergence) -> CompilerAction | None:
    ...
    cands = [r for r in _ROUTES.get(divergence.backend, []) if r.axis == divergence.axis and r.when(divergence)]
    if not cands:
        return None
    return _action_from_route(min(cands, key=lambda r: _CLASS_ORDER.get(r.action_class, 99)), divergence)
...
SEAMS_NEEDING_A_NEW_MODULE: dict[str, str] = {
    "tile-epilogue-store-once": "eliminate the rank-generic per-tile copy by storing C once. The residual copies are NOT "
    ...
    "rvv-microkernel-emitter": "a register-blocked, accumulator-resident, VL-agnostic RVV micro-kernel emitter. "
    ...
    "fuse-requant-narrowing-store": "fuse the requantize + narrowing (vnclip/vfncvt) into the store epilogue.",
    "vl-polymorphic-tail": "emit a VL-agnostic vsetvli loop with a mask/vl tail instead of fixed vsetivli unrolling.",
    "hoist-loop-invariant-call": "the cheap mitigation for a per-iteration call: hoist it out of the loop, or enlarge the tile "
...
def route_escalated(divergence: Divergence, prior_class: str) -> CompilerAction | None:
    ...
    cands = [
        ...
        if r.axis == divergence.axis and divergence.expert is not None and _CLASS_ORDER.get(r.action_class, 99) > floor
    ]
    if not cands:
        return None
    return _action_from_route(min(cands, key=lambda r: _CLASS_ORDER.get(r.action_class, 99)), divergence)
...
def unvalidated_scope(actions, regime: str) -> tuple[CompilerAction, ...]:
    ...
    would buy the most.
    """
    return tuple(a for a in actions if applies_to_shape(a, regime) and shape_scope(a) == "unspecified")
...
SEAM_FILES: dict[str, _SeamSpec] = {
    "impr_features": (
        ...
        "registered PASS/HEURISTIC/PATTERN feature hook (default-off)",
        False,
    ),
    "schedule": (
        "merlin/python/merlin/mining/from_strategy.py (+ the package knobs.yaml / schedule.mlir)",
        "transform-schedule knob (forkable via schedule.mlir today)",
        False,
    ),
    "quant": (
        "merlin/python/merlin/llvmlower/quant_passes.py",
        "int8 quant-pass registry (register/toggle a QuantPass; reached via dtype_strategy=int8_w8a8"
        " -> pkg.is_int8 -> int8_compute -> apply_quant)",
        False,
    ),
    "cflag": (
        "merlin/python/merlin/runtime/backends/zephyr_model.py (RVV cflags)",
        "compiler flag / march feature",
        False,
    ),
    "pass": (
        "merlin/python/merlin/llvmlower/ (NEW pass module — write it, then register as an impr feature)",
        "new MLIR pass / lowering",
        True,
    ),
}
...
def register_seam(
    prefix: str, seam_file: str, seam_kind: str, needs_new_code: bool, *, backend: str | None = None
) -> None:
...
def seam_location(target_seam: str, *, backend: str | None = None, oot_package: str | None = None) -> dict:
...
def escalation_ladder(axis: str, backend: str = "rvv", *, oot_package: str | None = None) -> list[dict]:
    ...
    rs = sorted(
        (r for r in _ROUTES.get(backend, []) if r.axis == axis), key=lambda r: _CLASS_ORDER.get(r.action_class, 99)
    )
    out = []
    for r in rs:
    ...
    for a in actions:
        for c in a.conflicts:
            if c in names or c in families:
                problems.append(f"{a.divergence_axis}: declares a conflict with {c!r}, which is in this bundle")
...
def lineage_problems(applied, candidate) -> tuple[str, ...]:
    ...
    for c in candidate.conflicts:
        if c in names:
            problems.append(
                f"{candidate.divergence_axis}: declares a conflict with {c!r}, already applied on this parent"
            )
