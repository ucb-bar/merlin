...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--write", action="store_true", help="write a versioned artifact under out/artifacts/optimization-surface/"
    )
    ...
    print(f"target: {a.target}   seams={len(s.entries)}  forkable={len(s.forkable())}  gaps={len(s.gaps())}")
    for e in shown:
        flag = "" if e.forkable_now else "  [GAP: no registrable hook yet]"
        print(f"  {e.seam_id:52s} {e.phase:15s} {e.mechanism:9s} {e.scope:8s} rebuild={e.rebuild_scope}{flag}")
        if e.cca_axes:
    ...
    if s.ungoverned:
        ...
        print(
            f"  {len(s.ungoverned)} seam(s) govern no CCA axis: {', '.join(s.ungoverned[:6])}"
