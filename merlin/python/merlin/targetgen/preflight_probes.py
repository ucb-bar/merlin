...
def _operation_contract_for_experiment(te) -> dict[str, Any]:
    ...
    if verdict == "mismatch":
        raise ProbeProtocolError("descriptor-declared and registry-resolved target contracts disagree")
...
def _unknown_observations(probe, detail: str) -> list[dict[str, Any]]:
    return [
        {**dict(operation), "status": "unknown", "evidence": {"kind": "preflight_adapter", "detail": detail}}
...
def _normalize_result(probe, raw: Any) -> dict[str, Any]:
    ...
    for index, observation in enumerate(observations):
        ...
        if (
            ...
            or not str(evidence.get("kind") or "").strip()
    ...
    if missing or extra:
        raise ProbeProtocolError(f"adapter observations differ from requirements (missing={missing}, extra={extra})")
...
def run_declared_capability_probes(
    te, *, workdir, timeout: int = 600, operation_contract: Mapping[str, Any] | None = None
) -> dict[str, Any]:
    ...
    try:
        declared = merge_operation_contracts(
            operation_contract if operation_contract is not None else _operation_contract_for_experiment(te)
        )
    ...
    for index, probe in enumerate(probes):
        ...
        try:
            required_ids = {_operation_key(operation) for operation in probe.requirements["operations"]}
            ...
            normalized = _normalize_result(probe, adapter(te=te, probe=probe, workdir=probe_dir, timeout=timeout))
        except Exception as exc:  # noqa: BLE001 -- a broken/unavailable probe fails closed with evidence
            detail = f"{type(exc).__name__}: {exc}"
            normalized = {"ok": False, "reason": detail, "observations": _unknown_observations(probe, detail)}
        mergeable_observations.extend(
            observation for observation in normalized["observations"] if _operation_key(observation) in declared_ids
        )
        rows.append(
            {**normalized, "capability": probe.capability, "adapter": probe.adapter, "fixture": dict(probe.fixture)}
        )
    ...
    return {
        "ok": ok,
        "probes": rows,
        "operation_capabilities": observed_contract,
        "reason": ("all declared capability probes passed" if ok else "one or more declared capability probes failed"),
    }
