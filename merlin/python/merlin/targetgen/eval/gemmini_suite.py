...
generate_driver = gemmini.gemmini_codegen.generate_driver
...
def record_gemmini_run(
    ...
    *,
    ...
    spec = RunSpec(
        project="merlin",
        suite=SUITE,
        method=f"{rung}_{simulator}_{codegen_backend}",
        seed=seed,
        run_id=run_id,
    ...
    if result is None:
        if codegen_backend == "mlir_inline_asm_rocc":
            gmlir = gemmini.gemmini_codegen_mlir
            result = gmlir.run_on_spike(cb, workdir=paths.generated, simulator=simulator, timeout=timeout)
        else:
    ...
    if codegen_backend == "mlir_inline_asm_rocc":
        ...
        try:
            ...
            pass
        for fname, kind in (("gemmini_kernel.ll", "llvm_ir"), ("gemmini_kernel.o", "object"), ("harness.c", "harness")):
    ...
    if not correct:
        if simulator == "spike":
            ...
            candidates = (
                "config ordering, fences, block_stride, mvin/mvout layout, accumulator "
        ...
        fr = FailureRecord(
            ...
            failure_id=f"{run_id}-mismatch",
            likely_cause=plane,
            observed=str(result.get("outputs")),
            artifact_refs=[str(cb_path)],
        )
