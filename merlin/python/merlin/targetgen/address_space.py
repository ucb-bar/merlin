...
class AddressSpace:
    ...
    sources: dict[str, str] = field(default_factory=dict)
    #: The facts' ``datapaths`` exactly as read -- kept because WHERE the accumulate type lives (in a
    #: store, or inside the compute element) is answered by them, not by the memory list.
    datapaths: tuple[dict, ...] = ()
...
#: backend addresses by row, distinct from the operand store. ``IN_DATAPATH``: the facts declare an
#: accumulate type but no store it fills -- the accumulator is state inside the compute element, so there
#: are no accumulator rows to address, and its depth (in tiles) is something the facts do not say.
#: ``UNKNOWN_KIND``: neither is derivable, or the facts disagree with themselves.
ADDRESSABLE = "addressable"
IN_DATAPATH = "in_datapath"
UNKNOWN_KIND = "unknown"

#: The facts schema's ROLE for the accumulate datapath -- a key the extractors write
#: (``datapaths[].name == "accumulator"``, or an ``accumulator`` dtype on a datapath), not a spelling of
#: any target's.
_ACCUMULATE_ROLE = "accumulator"


@dataclass(frozen=True)
class AccumulatorKind:
    """Where one target's accumulate results live, derived from its facts, with the evidence.

    ``rows`` is set only for ``ADDRESSABLE``. ``IN_DATAPATH`` deliberately carries ``rows=None`` and an
    ``unknown``: the facts say WHERE the accumulator is (a module, its ports, the type) and nothing about
    how many tiles of it there are, so any number here would be a constant wearing a derivation's clothes.
    """
...
def _declared_accumulate_types(datapaths: Sequence[dict]) -> list[tuple[str, str]]:
    """``(datapath name, accumulate dtype)`` for every accumulate type the facts declare, in either form
    the extractors write: a datapath whose role IS the accumulator, or a datapath carrying an
    ``accumulator`` dtype for what it computes."""
    out: list[tuple[str, str]] = []
    for d in datapaths:
        if str(d.get("name", "")).casefold() == _ACCUMULATE_ROLE and d.get("dtype"):
            out.append((str(d["name"]), str(d["dtype"])))
        elif isinstance(d.get(_ACCUMULATE_ROLE), str) and d.get(_ACCUMULATE_ROLE):
            out.append((str(d.get("name")), str(d[_ACCUMULATE_ROLE])))
    return out


def accumulator_kind(space: AddressSpace) -> AccumulatorKind:
    """ADDRESSABLE, IN_DATAPATH or UNKNOWN -- from the store list AND the datapaths, never width alone.

    Row width can say that two stores are different address spaces; it cannot say that the wider one is
    where results accumulate (an instruction buffer and a scratchpad differ in width too). So an
    ADDRESSABLE verdict needs both: the width-resolved accumulator store, and an accumulate datapath
    structurally linked to it (the store's element type came from that datapath). Declared accumulate
    types that disagree with each other, or with the store they would fill, are UNKNOWN -- a device with
    two datapaths declaring i32 and f32 accumulators has no single accumulate type to schedule for.
    """
