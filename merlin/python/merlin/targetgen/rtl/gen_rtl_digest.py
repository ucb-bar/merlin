...
def generate(facts: dict) -> str:
    ...
    L = []
    # Name the accelerator from the DERIVED RTL source (config/kind CIRCT elaborated), not a baked target
    # literal — a second RoCC target's digest must not be mislabeled with the first target's name.
    _accel = src.get("kind") or src.get("config") or "the accelerator"
    L.append(f"# RTL_DIGEST — {_accel}, distilled from the RTL by CIRCT (read this, not the RTL)")
    L.append("")
