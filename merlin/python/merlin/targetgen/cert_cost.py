...
A capsule sized to an application's real shape is worthless if nobody can afford to certify it, and
"nobody can afford it" does not announce itself -- it presents as a grade that is still running. So
the price is MEASURED, per target, per tier, and per ENGINE, from the ``capsule_result.json`` files
every graded run already writes.

**Why per engine, and not one law per tier.** The cert tier is a fidelity, not a simulator (see
``rtl_engine_policy``): several elaborated-RTL engines answer on the same rung, and their costs differ
by an order of magnitude. Fitted over this checkout's 644 cycle-accurate samples:

======================  =====  ==========================================  =====
engine bucket           n      seconds ~=                                  r2
======================  =====  ==========================================  =====
one RTL engine          290    14.0 + 0.00471 * cycles                     0.85
another RTL engine      354    134.2 + 0.06944 * cycles                    0.51
======================  =====  ==========================================  =====

Pooling those two produces a law that describes neither: the fixed cost differs by ~120 s and the
per-cycle rate by 15x, so a capsule priced off the blend is mispriced by whichever engine actually
runs. The bucket key is read off the tier record itself and kept VERBATIM -- this module never needs
to know which engine a key names, only that samples from different keys are different populations.

**Which independent variable.** Cycles, not shape. An RTL simulator's time is spent advancing cycles;
a tensor's declared extent is a proxy for that and a weak one. Cycles are also obtainable cheaply: the
FUNCTIONAL tier costs milliseconds and reports a cycle count, and the cycle-accurate count tracks it at
a stable-enough ratio to size with (measured here: median 5.25 and 5.89 on the two engine buckets, p10
3.4 / p90 7.8). So a capsule that has only ever run at the screen tier can still be priced for the
cert tier -- which is the whole point, since the decision has to be made BEFORE paying.

**Read it as a sizing instrument, not a stopwatch.** An r2 of 0.51 on one bucket is honest about the
scatter: capsules at an identical cycle count measured 105-154 s. Budget with margin. Every fit reports
its ``r2``, its sample count and the cycle range it was fitted over, so a caller quotes evidence rather
than a number.

THREE REFUSALS, all deliberate, because each of them is a way a cost model lies:

* **No measured history, no fit.** A target/tier/engine nobody has certified yields ``None``, and the
  caller must then refuse rather than size from a default. A default here is a number nobody measured
  driving a decision somebody quotes.
* **No extrapolation.** A fit over 159-28k cycles says nothing about 10 million. Past the measured
  range times :data:`EXTRAPOLATION_MARGIN` the prediction is an absence, not a large number.
* **UNKNOWN is not "affordable" and not "capped".** :class:`Affordability` is three-valued on purpose.
  Collapsing an unpriced capsule into either of the other two is how an unaffordable grade gets
  scheduled on the strength of arithmetic -- or how an affordable one gets silently dropped.
"""
