...
{weights_decl}
{dtiming_decl}{oprof_decl}#define MERLIN_DUMP_CAP {dump_cap}
...
def main_linux_session_c(dump_cap: int = 4096) -> str:
...
static void *worker(void *unused) {{
  ...
  int k = elems < MERLIN_DUMP_CAP ? (int)elems : MERLIN_DUMP_CAP;
  printf("OUT %d", k);
...
def build_k1_session_binary(model_dir: str | Path, work: str | Path, pkg, *,
    ...
    for record in manifest["programs"]:
        ...
        _run([clang23, "--target=riscv64-unknown-linux-gnu", f"-march={codegen_march()}",
              f"-mabi={K1_MABI}", *model_opt, "-Wno-override-module", "-c", lowered.ll_path,
              "-o", model_object])
    ...
    main_c.write_text(main_linux_session_c(), encoding="utf-8")
    ...
    def _build_deploy_run(mode: str, tag: str) -> dict:
        ...
        if multi_program:
            ...
            binary = build_k1_binary(
                ...
                parallel_harts=(parallel_harts if mode == "rvv_openmp" else None),
                fallback_policy=fallback_policy, kernel_backend=kernel_backend,
                dispatch_timing=dispatch_timing, op_profile=op_profile, ours_mr=ours_mr,
