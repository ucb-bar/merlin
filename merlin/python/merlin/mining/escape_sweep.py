...
class EscapeResult:
    ...
    def to_json(self) -> dict[str, Any]:
        return {
            ...
            "depths_reliable": self.depths_reliable,
            "sites": self.sites,
...
def run_cell(cell: EscapeCell, *, timeout: int = 3600) -> EscapeResult:
    ...
    base = EscapeResult(cell=cell.name, kind=cell.kind, dtype=cell.dtype, features=cell.features, status="build_failed")
    ...
    try:
        ...
        if cell.features:
            pkg = replace(pkg, run_id=f"esc_{cell.name}", compiler_features=list(cell.features))
        bwork = work / "build"
        elf = k1.build_k1_binary(bundle, bwork, pkg)
        rep = audit(bwork / "model.o", elf)
        if not rep.readable:
            return replace(base, status="unknown", note="object or ELF unreadable (disassembly/symbol table)")
        ...
        return EscapeResult(
            ...
            status="ok",
            in_loop=rep.in_loop_counts(),
            prologue={
                k: v for k, v in (rep.site_counts() or {}).items() if v - (rep.in_loop_counts() or {}).get(k, 0) > 0
            },
            ...
            sites=[
                {
                    ...
                    "depth_reliable": s.depth_reliable,
                }
                for s in rep.sites
            ],
        )
    except Exception as exc:  # noqa: BLE001 -- a failed cell must be reported
        return replace(base, note=f"{type(exc).__name__}: {exc}", sites=[{"traceback": traceback.format_exc()[-1200:]}])


def run_sweep(cells: list[EscapeCell], *, workers: int = 4, retry_serial: bool = True) -> list[EscapeResult]:
    ...
    *,
    ...
    for dtype in dtypes:
        for feats in features:
            ...
            for op, shape in ops:
                cells.append(
                    EscapeCell(name=f"{op}:{dtype}:{tag}", kind="op", dtype=dtype, features=feats, op=op, shape=shape)
                )
            ...
            for md in (models or {}).get(dtype, []):
                cells.append(
                    EscapeCell(
                        name=f"{Path(md).name}:{dtype}:{tag}", kind="model", dtype=dtype, features=feats, model_dir=md
                    )
                )
    return cells
...
def main(argv: list[str] | None = None) -> int:
    ...
    models = {"f32": [m for m in a.models_f32.split(",") if m], "int8": [m for m in a.models_int8.split(",") if m]}
