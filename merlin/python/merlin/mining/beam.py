...
from .baseline import UNRECORDED as _BASELINE_UNRECORDED
from .baseline import attainment as _baseline_attainment
...
def _escalations(action, achieved_cca, knobs: dict) -> list:
    ...
    for axis in ac.achieved_residual(action, achieved_cca):
        ...
        d = Divergence(
            axis=axis,
            expert=want,
            ours=ac._facet_value(achieved_cca, axis),
...
def _cca_divergences(run_dir: Path, expert_cca, op_key: dict, compare_fn: "Callable | None" = None) -> list:
    ...
    ours = cca.lift_asm(
        rvv.decode_text(objd.read_text()),
        op=str(op_key.get("op", "matmul")),
        source="ours",
...
def _correctness_residual(result: dict) -> float | None:
    ...
    vals = [v for k, v in c.items() if isinstance(v, (int, float)) and not isinstance(v, bool) and "rel" in k]
    return float(max(vals)) if vals else None


def _score(result: dict, run_dir: Path, curated: RvvFingerprint, op_key: dict, target: str | None = None) -> dict:
    ...
    _auth = (
        _meas.authority_for(_target)
        if _target
        else _meas.MeasurementAuthority(
            target="<unspecified>",
            declared=False,
    ...
    objective = _meas.whole_model_objective(cov.get("claimed_mac_fraction"), cov.get("attainment"), numerics_ok=gate_ok)
    ...
    out = {
        ...
        "board_conditions": _cond,
        "measurement_protocol": _proto,
    }
    if _auth_gaps:
...
def _ranked_speedup(speedup: float | None, parent_speedup: float | None, margin: float) -> float | None:
...
def run_beam(
    ...
    *,
    ...
    certify_fn: Callable = certify_rvv,
    proposer: Callable | None = None,
    expert_cca=None,
    compare_fn: Callable | None = None,
    pass_slot_fn: Callable | None = None,
    loader: Callable = load_rvv_package,
    minter: Callable = mint_fork,
    max_workers: int | None = None,
    sweep_fn: Callable = run_sweep,
    ...
    def certify_and_score(
        pkg_dir: Path, run_id: str, parent_rid: str | None, lever: str, evidence: list[str], d: int
    ) -> dict:
        res = certify_fn(
    ...
    seed_node = certify_and_score(Path(seed_pkg), f"{seed.run_id}__beam", None, "seed", ["baseline"], 0)
    ...
    for d in range(1, depth + 1):
        ...
        for esc_pkg, esc_prop, esc_parent_rid in pending_escalations:
            ...
            fork_dir = minter(
                ...
                version=d,
                depth=d,
                timestamp=f"{timestamp}_esc{counter}",
                source_evidence=esc_prop.evidence,
                lever=esc_prop.lever,
                target=target,
                out_root=out_root,
            ...
            jobs.append(
                {
                    "package_dir": str(fork_dir),
                    "model_dir": str(model_dir),
                    "runs_root": str(runs_root),
                    "run_id": fork_dir.name,
                    "targets": targets,
                    "baseline_run_dir": (str(baseline_run_dir) if baseline_run_dir else None),
                }
            )
        ...
        for parent_pkg, parent_node in parents:
            ...
            divs = (
                _cca_divergences(runs_root / parent_node["run_id"], expert_cca, op_key, compare_fn=compare_fn)
                if cca_mode
                else parent_node["divergences"]
            )
        ...
        for (fork_dir, parent_rid, p), res in zip(meta, results):
            ...
            node["inert"] = bool(node["emitted_digest"] and _parent_digest and node["emitted_digest"] == _parent_digest)
            ...
            if action is not None and objd.is_file():
                ...
                achieved_cca = lift_asm(
                    _rvv.decode_text(objd.read_text()),
                    op=str(op_key.get("op", "matmul")),
                    source="fork",
                    undefined_symbols=_undef_syms(runs_root / fork_dir.name),
                )
                step = make_step(action, achieved_cca, correctness_ok=sc["gate_ok"], speedup=node.get("speedup"))
                ...
                if step.residual:
                    ...
                    node["escalations"] = [
                        {
                            "axis": e.targets,
                            "class": getattr(e.action, "action_class", None),
                            "seam": getattr(e.action, "target_seam", None),
                            "forkable": e.forkable,
                        }
                        for e in esc_props
                    ]
                    pending_escalations.extend((fork_pkg, e, fork_dir.name) for e in esc_props if e.forkable)
        ...
        survivors = [n for n in rank_results(gen_nodes) if n["gate_ok"] and not n.get("inert")][:top_k]
    ...
    _starved = sorted(
        {str(_d.get("family") or _d.get("lever") or "?") for _d in deferred if _d.get("reason") == "over_width"}
    )
    deferral_census = {
        "total": len(deferred),
        ...
        "note": (
            "proposals deferred over_width were never built; a non-zero count "
    ...
    effectiveness_census = {
        "nodes": len(nodes),
        "with_emitted_digest": len(_digests),
        "distinct_emitted": _distinct,
        "duplicate_emission_rate": (round(1.0 - _distinct / len(_digests), 3) if _digests else None),
        "inert_nodes": _inert,
        "note": (
            "a high duplicate_emission_rate or inert_nodes count means the levers were not "
    ...
    tree = {
        ...
        "expert_baseline": (_baseline_identity(expert_wall_ns)),
        "noise_margin": margin,
        "two_phase": validate_fn is not None,
        "best": {
            k: best.get(k) for k in ("run_id", "structural_match", "speedup", "attainment_vs_expert", "cycles", "lever")
        }
        if best
        else None,
        "nodes": nodes,
