...
def _finish(
    bundle: Path, mlir: str, manifest: dict, order: dict, inputs: dict[str, np.ndarray], golden: np.ndarray
) -> Path:
...
def gen_matmul_f32(out_root: str | Path, M: int = 64, N: int = 64, K: int = 64, seed: int = 0) -> Path:
...
def gen_batch_matmul_f32(out_root: str | Path, B: int = 4, M: int = 32, N: int = 8, K: int = 32, seed: int = 0) -> Path:
    ...
    manifest = {"0": {"kind": "input", "name": "a"}, "1": {"kind": "input", "name": "b"}}
    order = {"a": 0, "b": 1}
    return _finish(bundle, mlir, manifest, order, {"in0": a, "in1": b}, np.matmul(a, b))


def gen_conv2d_as_matmul_f32(out_root: str | Path, M: int = 64, N: int = 16, K: int = 27, seed: int = 0) -> Path:
...
def gen_matmul_f16(out_root: str | Path, M: int = 128, N: int = 128, K: int = 128, seed: int = 0) -> Path:
...
def gen_reduce_f32(out_root: str | Path, op: str = "sum", M: int = 64, N: int = 4096, seed: int = 0) -> Path:
    ...
    _INIT = {
        "sum": ("%czero", "0.000000e+00", "arith.addf"),
        "max": ("%cneg", "0xFF800000", "arith.maximumf"),
        "min": ("%cpos", "0x7F800000", "arith.minimumf"),
    }
    _NP = {"sum": lambda x: x.sum(axis=1), "max": lambda x: x.max(axis=1), "min": lambda x: x.min(axis=1)}
    if op not in _INIT:
    ...
    x = r.standard_normal((M, N)).astype(np.float32)
    return _finish(
        bundle, mlir, {"0": {"kind": "input", "name": "x"}}, {"x": 0}, {"in0": x}, _NP[op](x).astype(np.float32)
    )


def gen_relu_f32(out_root: str | Path, N: int = 65536, lo: float = 0.0, hi: float = 6.0, seed: int = 0) -> Path:
    ...
    return _finish(bundle, mlir, {"0": {"kind": "input", "name": "x"}}, {"x": 0}, {"in0": x}, golden)
...
def gen_transpose_f32(out_root: str | Path, R: int = 256, C: int = 256, seed: int = 0) -> Path:
    ...
    return _finish(bundle, mlir, {"0": {"kind": "input", "name": "x"}}, {"x": 0}, {"in0": x}, x.T.copy())


_GENERATORS = {
    "matmul_f32": gen_matmul_f32,
    "matmul_f16": gen_matmul_f16,
    "softmax_f32": gen_softmax_f32,
    "batch_matmul_f32": gen_batch_matmul_f32,
    "conv2d_im2col_f32": gen_conv2d_as_matmul_f32,
    "gelu_f32": gen_gelu_f32,
    "sigmoid_f32": gen_sigmoid_f32,
    ...
    "binary_f32": gen_binary_f32,
    "reduce_f32": gen_reduce_f32,
    "relu_f32": gen_relu_f32,
    "transpose_f32": gen_transpose_f32,
}
