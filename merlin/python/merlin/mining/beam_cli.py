...
def run_instrumented_beam(
    *,
    seed_pkg: str | Path,
    model_dir: str | Path,
    expert_objdump: str | Path,
    op: str = "matmul",
    dtype: str = "f32",
    shape_regime: str = "square",
    ...
    validate_model_dir: str | Path | None = None,
    validate_fn=None,
    noise_margin: float | None = None,
    proposer=None,
    teachers: str | None = None,
    pass_slot_turns: int = 0,
    pass_slot_fn=None,
) -> dict[str, Any]:
    ...
    if teachers:
        ...
        fams = None if teachers.strip().lower() == "all" else tuple(f.strip() for f in teachers.split(",") if f.strip())
    ...
    parent = start_run(
        ...
        extra={
            ...
            "workload": Path(model_dir).name,
            "expert_objdump": str(expert_objdump),
            "role": "beam_parent",
    ...
    try:
        res = run_beam(
            ...
            op_key=op_key,
            runs_root=parent.run_dir / "forks",
            out_root=str(parent.run_dir / "targets"),
            width=width,
            depth=depth,
            top_k=top_k,
            target="rvv",
        ...
        if teacher_audit:
            write_yaml(parent.run_dir / "teacher_audit.yaml", {"teachers": teachers, "generations": teacher_audit})
        ...
        parent_summary = {
            ...
            "pass_slot_runs": sum(len(n.get("pass_slot") or ()) for n in res.get("nodes", [])),
            "pass_slot_accepted": sum(
                1 for n in res.get("nodes", []) for r in (n.get("pass_slot") or ()) if r.get("accepted")
            ),
            "teachers": (teachers or "single-expert"),
            "teacher_unanswered_axes": sorted({a for g in teacher_audit for a in g.get("unanswered_axes", ())}),
        }
        status = "ok"
...
def _declared_expert(args):
    ...
    if args.expert_wall_ns is None:
        return None
    missing = [f"--expert-{n}" for n in ("workload", "dtype") if not getattr(args, f"expert_{n}", None)]
    if missing:
        raise SystemExit(
    ...
    return ExpertBaseline(
        wall_ns=float(args.expert_wall_ns),
        workload=args.expert_workload,
        dtype=args.expert_dtype,
        substrate=args.expert_substrate,
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        prog="merlin-rvv-beam", description="aet-instrumented, board-serialized CCA beam search (BB3)."
    )
    ap.add_argument(
        "--seed-pkg",
        default=None,
    ...
    ap.add_argument(
        "--expert-dtype",
        default=None,
        help="the numeric format the expert wall was measured in (fp32/int8/fp16); required with --expert-wall-ns",
    )
    ap.add_argument(
        "--expert-substrate", default="k1_spacemit", help="which device produced the expert wall (default: k1_spacemit)"
    )
    ap.add_argument(
        "--expert-note",
        default="",
    ...
    res = run_instrumented_beam(
        ...
        expert_objdump=args.expert_objdump,
        op=args.op,
        dtype=args.dtype,
        shape_regime=args.shape_regime,
        targets=targets,
        width=args.width,
        depth=args.depth,
        top_k=args.top_k,
        max_workers=args.max_workers,
        expert_wall_ns=_declared_expert(args),
        validate_model_dir=args.validate_model_dir,
