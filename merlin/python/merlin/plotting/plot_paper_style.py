...
def callout(ax, xy, text, xytext, fc="#fff6e0", ec=GOLD):
    ax.annotate(
        ...
        arrowprops=dict(arrowstyle="-", color=ec, lw=1.3, connectionstyle="arc3,rad=0.0"),
    )
...
def fig_e2e():
    ...
    V3 = "#b8742a"
    OB = "#7a9e7a"  # ours accum-resident microkernel (winner) / OpenBLAS
    FEAT = {
        "ours_v3": "accum-resident v3",
        "ours_wholemodel_vf": "wholemodel .vf",
        "ours_wholemodel": "wholemodel",
        "ours_tiled": "tiled vfmacc",
    }
    ...
    card(ax, "bitvla — whole-model latency on K1 silicon")
    b = load("bitvla")
    bk = ours_best(b)
    base = wall(b, "baseline")
    rows = [
    ...
    for yi, (lab, s_, col, fast) in zip(y, rows):
        ...
        ax.text(
            ...
            fontsize=10.5,
            fontweight="bold",
            color=(V3 if fast else INK),
        )
    ax.set_yticks(y)
    ax.set_yticklabels([r[0] for r in rows], fontsize=9.8)
    ax.set_xlim(0, base * 1000 * 1.65)
    ax.set_xlabel("latency (ms / forward) — lower is better")
    xn = wall(b, "xnnpack_kernels")
    over = (xn / wall(b, bk)) if (xn and wall(b, bk)) else None
    callout(
        ax,
        (wall(b, bk) * 1000, y[-1] + 0.32),
        f"compiler-emitted micro-kernel BEATS XNNPACK's\nhand kernel — {over:.2f}× faster, cos 0.99999",
        (base * 1000 * 0.5, y[-1] + 0.30),
        fc="#f7efe2",
        ec=V3,
    )
    ...
    card(ax, "best compiler speedup per model vs the best expert")
    ...
    for yi, m in enumerate(models):
        s = load(m)
        bk = ours_best(s)
        base = wall(s, "baseline")
        if not (s and bk and base):
            continue
        sp = base / wall(s, bk)
        ...
        if exps:  # best-expert reference tick (so "ours vs hand-tuned" is visible in absolute speedup space)
            ...
            ax.text(
                be,
                yi + 0.40,
                ("WINS" if sp >= be else f"{pct}% of expert"),
                ha="center",
                fontsize=7.6,
                fontweight="bold",
                color=(V3 if sp >= be else GREY),
            )
    ax.axvline(1.0, color=GREY, lw=1, ls="--")
    ax.set_yticks(y)
    ax.set_yticklabels(models, fontsize=11)
    ax.set_xlim(0, maxsp * 1.18)
    ax.set_xlabel("whole-model speedup (×) — higher is better\n(▮ = fastest expert)")
...
def fig_crossover():
    ...
    ax.set_xticklabels([f"{s}³" for s in shapes])
    ...
    callout(ax, (384, 8.0e4), "ours-intrinsic beats both\nexperts through 384³", (96, 4.5e5), fc="#eaf4f6", ec=TEAL)
    callout(ax, (512, 1146939), "OpenBLAS retakes\nlead at 512³ (0.82×)", (512, 9.0e4), fc="#eef3ec", ec=SAGE)
    ax.text(40, 1.1e7, "ours-baseline never\nforms vfmacc (~100× off)", fontsize=8.6, color=SALMON, style="italic")
...
def fig_progression():
    ...
    ax = fig.add_subplot(gs[0])
    card(ax, "Beam progression — bitvla whole-model on K1")
    ...
    _base = _b["baseline"]["min_wall_ns"]
    _xnn = _base / _b["xnnpack_kernels"]["min_wall_ns"]
    ...
    for xi, (lab, v, col) in zip(xs, steps):
        ...
        ax.text(
            ...
            fontsize=11,
            fontweight="bold",
            color=col if col != "#e8c98a" else "#9c7415",
        )
    ...
    ax = fig.add_subplot(gs[1])
    card(ax, "Beam progression — single GEMM 64³ (spike instret)")
    k = [
        ("baseline", 22430926, SALMON),
        ("vfmacc\ncontraction", 123094, "#e8c98a"),
        ("tiled\n(bounded)", 1328219, GOLD),
        ("v3 compute\nkernel", 53207, V3),
    ]
    xs = np.arange(len(k))
    vals = [s[1] for s in k]
    ...
    ax.set_yscale("log")
    ax.set_ylim(2.5e4, 2.5e8)
    ax.set_xticks(xs)
    ax.set_xticklabels([s[0] for s in k], fontsize=9)
...
def fig_opt_effects():
    V3 = "#b8742a"
    EX = [("GEMM 64³ (autotune)", "#9aa0aa"), ("bitvla (whole-model)", GOLD), ("openvla (whole-model)", TEAL)]
    ...
    for fi, (fname, vals) in enumerate(feats):
        for ei, v in enumerate(vals):
            ...
            ax.text(
                v + 0.15, yy, f"{v}×{star}", va="center", fontsize=8.3, fontweight="bold", color=(V3 if star else INK)
            )
    ...
    ax.set_yticks(yb)
    ax.set_yticklabels([f[0] for f in feats], fontsize=10)
    ax.set_xlim(0, 19)
    ax.set_xlabel("speedup vs frozen baseline (×) — per driving example")
    ax.set_title(
        "How each mined optimization affects the RVV dialect — and how the ranking depends on the example",
        loc="left",
        color=INK,
        fontsize=12,
        pad=10,
    ...
    ax.legend(
        handles=[Patch(color=c, label=lab) for lab, c in EX],
        fontsize=9,
        loc="lower right",
        framealpha=0.95,
        facecolor="white",
...
def fig_beam_ranking():
    ...
    runs = sorted(glob.glob(str(OUT.parents[2] / "out/artifacts/kernel-mining/rvv/beam_rvv_v2_*")))
    if not runs:
        print("beam_ranking: no beam_rvv_v2 run; skipping")
        return
    run = runs[-1]
    V3 = "#b8742a"
    ...
    for ax, M in zip(axes, ("bitvla", "openvla")):
        ...
        for x in rows:
            ...
            tag = x["tag"]
            if sp is None:
                labs.append(f"{tag} (blocked)")
                vals.append(0.0)
                cols.append("#cfcfcf")
            else:
                labs.append(tag)
                vals.append(sp)
                if x["lowering"] == "scalar_fallback":
                    cols.append(SALMON)
                elif tag == rows[0]["tag"]:
                    cols.append(V3 if "v3" in tag or "accum" in tag else GOLD)
                elif sp <= 1.05:
                    cols.append(GREY)
                else:
                    cols.append(GOLD)
        ...
        for i, (x, v) in enumerate(zip(rows, vals)):
            t = (
                "blocked"
                if x["speedup"] is None
                else (f"{v:.2f}×" + ("  (scalar)" if x["lowering"] == "scalar_fallback" else ""))
            )
            ax.text(
                (v if v else 0) + 0.15,
                i,
        ...
        ax.axvline(1.0, color="#999", lw=1, ls="--")
        ax.set_yticks(yb)
        ax.set_yticklabels(labs, fontsize=8.6)
        ax.invert_yaxis()
    ...
    B = "beam_rvv_v2"
    fig.suptitle(
        f"Faithful whole-model beam ({B}) — every candidate optimization ranked per model on K1",
        fontsize=13,
        fontweight="bold",
        y=1.0,
    )
    ...
    for ax, M in zip(axes, ("bitvla", "openvla")):
        ...
        for i, x in enumerate(rows):
            ...
            ax.text(
                ...
                fontsize=8.3,
                fontweight="bold",
                color=(SALMON if scalar else INK),
            )
        ...
        ax.axvline(1.0, color="#999", lw=1, ls="--")
        ax.set_yticks(yb)
        ax.set_yticklabels([x["tag"] for x in rows], fontsize=8.6)
        ax.set_xlabel("performance — whole-model speedup vs baseline (×)")
        ax.set_xlim(0, ceil * 1.35)
        ax.set_title(
            f"{M}: performance + VPU utilization per beam candidate", loc="left", color=INK, fontsize=11.5, pad=8
        )
    fig.suptitle(
        "Beam candidates — performance (speedup) and utilization (% of ceiling + VPU state), baseline → best",
        fontsize=12.5,
        fontweight="bold",
        y=1.0,
    )
...
def fig_fourway():
    ...
    if not data:
        print("fourway: no k1_4way_*.json yet; skipping")
        return
    models = [m for m in models if m in data]

    def cfg(s, key):  # (wall_s, range_pct) or None
        r = s.get(key) or {}
        if r.get("skipped") or not r.get("min_wall_ns"):
            return None
    ...
    for i, ser in enumerate(series):
        ...
        for m in models:
            s = data[m]
            key = ours_key(s) if ser == "ours" else ser
            c = cfg(s, key)
            ys.append(c[0] if c else 0)
            errs.append((c[0] * c[1] / 100.0) if c else 0)
        ax.bar(
            x + (i - 1.5) * bw,
            ys,
            bw * 0.9,
    ...
    for j, m in enumerate(models):
        s = data[m]
        o = spdup(s, ours_key(s))
        xn = spdup(s, "xnnpack_kernels")
        ob = spdup(s, "openblas_kernels")
        ...
        if o and exps:
            ...
            ax.text(
                ...
                fontsize=8.5,
                fontweight="bold",
                color=(V3 if o[0] >= best_exp else INK),
            )
    ax.set_xticks(x)
    ax.set_xticklabels(models, fontsize=11)
    ...
    fig.suptitle(
        "Whole-model four-way on real K1 silicon — baseline · ours-best · XNNPACK · OpenBLAS (same-pass, cos-gated)",
        fontsize=12.5,
        fontweight="bold",
        y=1.0,
    )
    ...
    for m in models:
        ...
        exps = [e[0] for e in (spdup(data[m], "xnnpack_kernels"), spdup(data[m], "openblas_kernels")) if e]
        if not (o and exps):
            continue
        if o[0] >= max(exps):
            won.append(m)
    ...
    verdict = f"ours BEATS both experts on {', '.join(won)}" if won else "ours trails the experts on every model here"
    trail = f"; reaches {min(trail_pcts)}–{max(trail_pcts)}% of the best expert on the rest" if trail_pcts else ""
    fig.text(
        0.5,
        -0.03,
...
def fig_gap_attribution():
    ...
    ax.bar(xb, [b[1] for b in bars], 0.6, color=[b[2] for b in bars], edgecolor=CARD_EC, linewidth=1.4, zorder=3)
    ...
    for s in ("top", "right"):
        ax.spines[s].set_visible(False)
    rows = [
        ("ours iter-2 (.vf)", vf["loads_per_fma"], V3, False),
    ...
    for xi, (lab, v, c, hatch) in zip(xb, rows):
        ax.bar(xi, v, 0.6, color=c, edgecolor=CARD_EC, linewidth=1.4, zorder=3, hatch="//" if hatch else None)
    ...
    ax.set_xticks(xb)
    ax.set_xticklabels([r[0] for r in rows], fontsize=8.8)
    ...
    fig.suptitle(
        "Where the matmul-kernel residual is — memory-traffic decode (iteration 1→2→3)",
        fontsize=12.5,
        fontweight="bold",
        y=1.0,
    )
...
def fig_dispatch_breakdown():
    ...
    MATMUL = "#6f93b0"
    DISP = "#cf8b7d"  # matmul = steel (shared); dispatch = salmon (the gap)
    ...
    fig, axes = plt.subplots(1, len(panels), figsize=(13.5, 4.8))
    fig.patch.set_facecolor("white")
    if len(panels) == 1:
        axes = [axes]
    for ax, (nm, key) in zip(axes, panels):
        ...
        bars = [
            ("ours .vf", mm, loc["ours_dispatch_bucket_ns"] / 1e6, True),
        ...
        delta = loc["delta_wall_ns"] / 1e6
        over = loc["ours_over_xnnpack"]
        ax.set_xlim(0, (mm + loc["ours_dispatch_bucket_ns"] / 1e6) * 1.22)
        ax.set_title(
            f"{nm} — matmul is {frac:.0f}% of wall; the {over:.2f}× gap is all dispatch",
            loc="left",
            color=INK,
            fontsize=11.5,
            pad=8,
        ...
        ax.annotate(
            "",
            xy=(mm + loc["ours_dispatch_bucket_ns"] / 1e6, y[0] - 0.34),
            xytext=(mm + loc["xnnpack_dispatch_bucket_ns"] / 1e6, y[0] - 0.34),
            arrowprops=dict(arrowstyle="<->", color="#9c4f3f", lw=1.6),
        )
        ax.text(
            (mm + (loc["ours_dispatch_bucket_ns"] + loc["xnnpack_dispatch_bucket_ns"]) / 2e6),
            y[0] - 0.52,
            f"Δ {delta:.0f} ms = 100% dispatch",
            ha="center",
            fontsize=8.4,
            color="#9c4f3f",
            fontweight="bold",
    ...
    axes[0].legend(
        handles=[
            Patch(color=MATMUL, label="matmul kernel (shared, = XNNPACK by decode)"),
    ...
    fig.suptitle(
        "Where the whole-model time goes — matmul kernel vs dispatch (K1 silicon, measured)",
        fontsize=12.5,
        fontweight="bold",
        y=1.0,
    )
