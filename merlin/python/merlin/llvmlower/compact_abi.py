...
def _references_symbol(attribute: Attribute, symbol: str) -> bool:
    ...
    return any(_references_symbol(child, symbol) for child in (*parameters, *children) if isinstance(child, Attribute))
...
def compact_pointer_entry(
    ...
    *,
    ...
    for op in function.walk():
        if any(
            _references_symbol(attr, function.sym_name.data)
    ...
    for old_block, new_block in zip(function.body.blocks, result.body.blocks):
        ...
        if len(old_ops) != len(new_ops) or not all(
            old.is_structurally_equivalent(new, equivalence) for old, new in zip(old_ops, new_ops)
        ):
