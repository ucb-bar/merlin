...
def weight_loader(bundle_dir: str | Path, manifest: dict):
    ...
    def load_arg(i: int):
        ...
        return np.frombuffer(raw, dtype=npdt, count=count, offset=payload_off + begin).reshape(h["shape"])
...
def _resolve_source(module, value, load_arg):
    ...
    while True:
        ...
        if name not in VIEW_OPS:
            raise QuantHoistRefused(f"chain reaches {name!r}, which is not a pure re-view; refusing to reproduce it")
...
def _is_scale(op) -> bool:
    ...
    return (
        ...
        and body in (["arith.divf"], ["arith.cmpf", "arith.divf", "arith.maximumf", "arith.select"])
...
def _quantize_formula(op) -> "str | None":
    ...
    if op.name != "linalg.generic" or any("reduction" in k for k in _iterator_kinds(op)):
        return None
    if body == ["arith.divf", "arith.fptosi", "arith.maximumf", "arith.minimumf", "math.roundeven"]:
        return "divide_scale"
    if body == ["arith.fptosi", "arith.maximumf", "arith.minimumf", "arith.mulf", "math.roundeven"]:
...
def plan(module, activation_args, load_arg):
    ...
    for op in module.walk():
        ...
        if tuple(amax.shape) != want:
            raise QuantHoistRefused(f"recomputed amax shape {tuple(amax.shape)} != IR's {want}; refusing")
        scale = (amax / np.float32(qmax)).astype(np.float32)
        scale_body = sorted(name for name in _body_op_names(scale_op) if name != "arith.constant")
        ...
        auxiliary = tuple(reciprocal_views + ([reciprocal_op] if reciprocal_op else []) + scale_views)
...
def _match_absmean_weight_transpose(op, module, load_arg):
    ...
    mean = getattr(mean_clamp.operands[0], "owner", None) if _aten(mean_clamp) == "aten.clamp.default" else None
    ...
    absolute = (
        getattr(reduction.operands[0], "owner", None) if getattr(reduction, "name", None) == "linalg.reduce" else None
    )
    ...
    if divisor is None or int(divisor) != weight.size:
        raise QuantHoistRefused(f"absmean reduction divisor {divisor} != weight elements {weight.size}; refusing")
    ...
    if weight.ndim != 2 or result_shape != tuple(reversed(weight.shape)):
        raise QuantHoistRefused(f"absmean transpose result {result_shape} is not rank-2 reverse of {weight.shape}")
...
def _plan_absmean_weight_transposes(module, load_arg, start: int):
    ...
    for op in list(module.walk()):
        ...
        value, _root = matched
        arg = HoistedArg(
            f"qhoist::absmean_transpose::{start + len(matches)}", tuple(value.shape), _elem_str(op.results[0].type)
        )
