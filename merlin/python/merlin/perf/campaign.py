...
class FamilyDecision:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {"family": self.family, "state": self.state.value, "reason": self.reason, "details": dict(self.details)}
...
class ReplicaIdentity:
    ...
    def __post_init__(self) -> None:
        for key, value in (
            ...
            if not isinstance(value, str) or not value.strip():
                raise CampaignDeclarationError(f"replica identity {key} must be a non-empty string, got {value!r}")
    ...
    def as_dict(self) -> dict[str, str]:
        return {"family": self.family, "member": self.member, "tier": self.tier, "replica": self.replica}
...
class Measurement:
    ...
    def __post_init__(self) -> None:
        ...
        if isinstance(self.cycles, bool) or not isinstance(self.cycles, int) or self.cycles <= 0:
            raise CampaignEvidenceError(f"measurement {self.identity.label} cycles must be a positive integer")
        if isinstance(self.words, bool) or not isinstance(self.words, int) or self.words < 0:
            raise CampaignEvidenceError(f"measurement {self.identity.label} words must be a non-negative integer")
        if isinstance(self.seconds, bool) or not isinstance(self.seconds, (int, float)):
            raise CampaignEvidenceError(f"measurement {self.identity.label} seconds must be a non-negative number")
        if isinstance(self.concurrency, bool) or not isinstance(self.concurrency, int):
            raise CampaignEvidenceError(f"measurement {self.identity.label} concurrency must be an integer >= 1")
        ...
        try:
            ...
            raise CampaignEvidenceError(f"measurement {self.identity.label} is not a valid cost sample: {exc}") from exc
...
class FalsifierEvidence:
    ...
    @classmethod
    def from_ab_decision(
        cls, identity: ReplicaIdentity, decision: ABDecision, *, negative_control: bool
    ) -> "FalsifierEvidence":
        ...
        return cls(identity=identity, negative_control=negative_control, fired=fired, reason=decision.reason)
...
def validate_declaration(performance: Mapping[str, Any]) -> str:
    ...
    if not isinstance(negative_control, str) or not negative_control.strip():
        raise CampaignDeclarationError(f"family {family!r} falsifier must name its negative-control member")
    ...
    if not isinstance(required, list) or any(not isinstance(name, str) or not name.strip() for name in required):
    ...
    if duplicates:
        raise CampaignDeclarationError(f"family {family!r} repeats gate trait(s) {duplicates}; each fact is asked once")
...
def assess_eligibility(
    performance: Mapping[str, Any], *, traits: Mapping[str, bool | None], emitter_implemented: bool | None
) -> FamilyDecision:
    ...
    for name in required:
        ...
        if value is not None and not isinstance(value, bool):
            raise CampaignEvidenceError(f"derived trait {name!r} must be exactly True, False, or None, got {value!r}")
        states[name] = value
    ...
    return FamilyDecision(family, ELIGIBLE, "every required trait is True and the emitter is implemented", details)
...
def validate_measurements(
    ...
    *,
    ...
    for name in fitted:
        ...
        if absent:
            raise CampaignEvidenceError(f"fitted parameter {name!r} is absent from measurement(s) {absent}")
        distinct: list[Any] = []
        for row in rows:
    ...
    *,
    ...
    try:
        rows = validate_measurements(
            measurements, expected_identities=expected_identities, fitted_parameters=fitted_parameters
        )
    except CampaignEvidenceError as exc:
        return _refused(eligibility, f"screen evidence refused: {exc}", stage="screen")
    wrong_family = [row.identity.label for row in rows if row.identity.family != eligibility.family]
    if wrong_family:
        return _refused(eligibility, f"screen evidence belongs to another family: {wrong_family}", stage="screen")

    evidence = tuple(falsifier_evidence)
    if any(not isinstance(row, FalsifierEvidence) for row in evidence):
        return _refused(eligibility, "falsifier evidence contains a non-evidence record", stage="screen")
    repeated = _duplicates(row.identity for row in evidence)
    if repeated:
        return _refused(eligibility, f"falsifier evidence repeats replica(s) {_labels(repeated)}", stage="screen")
    ...
    if undetermined:
        return _refused(
            eligibility,
            "negative-control falsifier evidence is undetermined: " + "; ".join(row.reason for row in undetermined),
            stage="screen",
    ...
    if not comparable:
        return _refused(
            eligibility,
            f"differential comparison refused: {comparison_reason}",
            stage="screen",
...
def complete_family(
    ...
    *,
    ...
    wrong_family = [row.identity.label for row in rows if row.identity.family != promoted.family]
    if wrong_family:
        return _refused(promoted, f"certification evidence belongs to another family: {wrong_family}", stage="certify")
    if certification_passed is not True:
        state = "failed" if certification_passed is False else "was not established"
        return _refused(
            promoted,
            f"certification {state}: {certification_reason}",
            stage="certify",
    ...
    return FamilyDecision(
        promoted.family,
        COMPLETE,
        f"certification passed: {certification_reason}",
        {
            **dict(promoted.details),
            "stage": "complete",
