...
class LaneMigrationContractionQualifier:
    ...
    def __call__(
        self, *, candidate: Path, experiment: Any, timeout_s: float, portfolio_member: Mapping[str, Any] | None = None
    ) -> dict[str, Any]:
        started = monotonic()
        ...
        try:
            ...
            for ordinal, row in enumerate(representatives):
                ...
                execution = dict(
                    self.runtime_provider(
                        candidate=candidate, experiment=experiment, prepared=prepared, timeout_s=min(60.0, remaining())
                    )
                )
            ...
            verified = experiment.selected_changed_portfolio_context(candidate, selected["selection"])
            actual_binding = {
