...
class Harvest:
    ...
    def series(self, quantity: str, *, status: str | None = None) -> tuple[Observation, ...]:
        ...
        """
        return tuple(o for o in self.observations if o.quantity == quantity and (status is None or o.status == status))
    ...
    def by_workload(self, quantity: str, *, status: str | None = None) -> dict[str, tuple[Observation, ...]]:
...
def spread(values: Sequence[float]) -> dict[str, Any]:
    ...
    return {"n": len(vals), "min": lo, "max": hi, "median": float(median(vals)), "ratio": (hi / lo if lo > 0 else None)}
...
def harvested_term(
    name: str, observations: "Sequence[Observation]", *, unit: str, regime: str, escalate: str = ""
) -> PerformanceTerm:
    ...
    evidence = tuple(sorted({e for o in observations for e in o.evidence})) or ("no citable observation was recovered",)
    prov = Provenance(kind=HARVEST_KIND, evidence=evidence)
    weak = (
        "observed inside running programs that were also doing: "
    ...
    validity = Validity(
        ...
        expected_error=(
            f"observed {stats['n']} time(s) across {len(subs)} submission(s) at verdict "
        ...
        escalate_when=(
            escalate
            or "a different submission, or any change to what else runs "
            "alongside; cycles are a property of the submission, not of the "
            "workload name"
        ),
    )
    if not stats["n"]:
        return assert_contended(
...
def harvest_capsule_result(
    path: "str | Path", *, authority: MeasurementAuthority, submission: str | None = None
    ...
    for stage, record in sorted((doc.get("tiers") or {}).items()):
        ...
        if not tier:
            refusals.append(
                Refusal(
                    f"{workload}@{stage}",
                    "the tier record states neither a "
                    "fidelity nor a provenance flag, so the number reaches no tier "
                    "on the measurement ladder and cannot be placed",
                    str(p),
                )
            )
            continue
        if not citable(authority, tier):
            refusals.append(
                Refusal(
                    ...
                    f"citable" + ("" if authority.declared else " (nothing is declared at all)"),
                    str(p),
                )
            )
            continue
        obs.append(
            Observation(
                ...
                workload=workload,
                stage=stage,
                substrate=substrate or "unnamed oracle",
                tier=tier,
                ...
                concurrent=(
                    "the whole program: this substrate reports one number per run and no "
    ...
    for entry in record.get(TIMING_OBSERVATIONS_KEY) or ():
        ...
        out.append(
            Observation(
                ...
                value=float(value),
                unit=str(entry.get("unit") or "cycles"),
                status=str(record.get("status") or ""),
                ...
                evidence=tuple(
                    sorted(
                        {str(path), f"{TIMING_OBSERVATIONS_KEY}[{quantity}]"}
...
def harvest_score_file(
    path: "str | Path", *, authority: MeasurementAuthority
) -> tuple[list[Observation], list[Refusal]]:
        ...
        for stage, cycles in sorted(entry.items()):
            ...
            refusals.append(
                Refusal(
                    ...
                    "result carries both and is the harvested source",
                    str(p),
                )
            )
    return obs, refusals
...
def discover_roots(target: str) -> tuple[Path, ...]:
    ...
    """
    return tuple(d for d in (runs_dir() / target, artifacts_dir() / "capsule-bench" / target) if d.is_dir())
...
def retro_mine(
    ...
    *,
    ...
    auth = (
        ...
        else authority_for(target, dict(descriptor) if descriptor is not None else None)
    )
    ...
    if not search:
        refusals.append(
            Refusal(
                f"{target} run roots",
                "no graded-run root exists on this host; nothing was executed here to harvest",
                "",
            )
        )
    for root in search:
    ...
    if not auth.declared:
        refusals.append(
            Refusal(
                f"{target} measurement authority", "; ".join(auth.gaps()) or "nothing declared", "capability manifest"
            )
        )
    return Harvest(
        target=target,
        ...
        roots=tuple(str(r) for r in search),
    )
...
def harvest_op_stream(
    ...
    *,
    ...
    if not streams:
        return Harvest(
            target=target, authority=auth, refusals=(Refusal("op streams", "the suite carries none", submission),)
        )
    try:
        delay_mnemonic = derive_delay_mnemonic(streams)
    except ValueError as exc:
        return Harvest(target=target, authority=auth, refusals=(Refusal("delay marker", str(exc), submission),))
    tier = auth.cycles_tier if auth.declared else ""
    if not tier or not citable(auth, tier):
        refusals.append(
            Refusal(
                "scheduled delays",
                f"the corpus's cycle tier {tier or 'UNKNOWN'!r} is not citable for "
                f"{target} (declared {auth.citable_tier!r})",
                submission,
            )
        )
    ...
    for name in sorted(kernels):
        ...
        for i, entry in enumerate(stream):
            ...
            obs.append(
                Observation(
                    submission=submission or "the pinned measured corpus",
                    workload=name,
                    stage=fam,
                    substrate="the corpus's own program schedule",
                    tier=tier,
                    quantity=f"scheduled_delay.{mnemonic}",
                    value=float(gap),
                    unit="cycles",
                    status="pass",
                    concurrent=(
                        f"the same program's {', '.join(others)} ops"
                        if others
                        else "no other op family in this program",
                    ),
                    evidence=(
                        f"{schema.op_stream_key}[{name}]",
...
def fit_points(evidence: AxisEvidence, form: str) -> Fit:
    ...
    if params is None:
        return Fit(
            form=form,
            refusal=f"no fit form named {form!r}; implemented "
    ...
    if n < need_points or evidence.distinct_x < need_x:
        return Fit(
            ...
            refusal=(
                f"{form} fits {k} parameter(s) {list(params)} and needs >={need_points} "
...
def invert_fill_law(law: str, fill: float, *, max_dimension: int = 4096) -> tuple[int | None, str]:
    ...
    if abs(float(fill) - target) > 1e-6:
        return None, (
            f"the fitted fill {fill} is not an integer number of cycles, so no integer "
    ...
    return None, (
        f"no dimension in 1..{max_dimension} gives a {law!r} fill of {target}; the law "
...
def _pair_movement(
    kernels: Mapping[str, Any], buckets: "Sequence[str]", schema: SuiteSchema, policy: Mapping[str, Any]
    ...
    for bucket in buckets:
        pts = [
            Point(
                float(e[schema.activity_key][schema.read_beats_key] + e[schema.activity_key][schema.write_beats_key]),
                float(e[schema.activity_key][bucket]),
                name,
            )
            for name, e in kernels.items()
            if bucket in e[schema.activity_key]
        ]
        fit = fit_points(
            AxisEvidence(axis="movement_probe", x_name="beats", y_name=bucket, y_unit="cycles", points=tuple(pts)),
            "affine",
        )
    ...
    if best_r2 < min_r2:
        return (
            None,
            (
                f"no bucket's busy cycles are explained by the beat counters (best {best} at "
                f"r2={best_r2:.4f} < {min_r2}); this instrument does not identify a movement "
                "engine, so its terms stay unharvested"
            ),
            scores,
        )
    if len(ranked) > 1 and ranked[1][1] > max_runner_up:
        return (
            None,
            (
                f"two buckets look linear in the beat counters ({best} r2={best_r2:.4f}, "
                f"{ranked[1][0]} r2={ranked[1][1]:.4f} > {max_runner_up}); the pairing is "
                "ambiguous and is refused rather than picked"
            ),
            scores,
        )
...
def _pair_compute(
    kernels: Mapping[str, Any], buckets: "Sequence[str]", schema: SuiteSchema, delay_mnemonic: str
    ...
    """
    bucket_support = {b: _support({n: e[schema.activity_key].get(b, 0) for n, e in kernels.items()}) for b in buckets}
    ...
    for bucket, support in sorted(bucket_support.items()):
        if not support:
            refusals.append(
                Refusal(
                    bucket,
                    "the bucket is idle in every program in the corpus, so no op family's support can be matched to it",
                )
            )
            continue
        ...
        if len(matches) != 1:
            refusals.append(
                Refusal(
                    bucket,
                    f"{len(matches)} op family(ies) have exactly this bucket's support "
    ...
    for family, support in families.items():
        ...
        if len(owners) > 1:
            for b in owners:
                paired.pop(b, None)
            refusals.append(
                Refusal(family, f"family drives {len(owners)} buckets {owners} by support; the pairing is not unique")
            )
    return paired, refusals


def axes_from_suite(
    suite: Mapping[str, Any], *, schema: SuiteSchema = SUITE_SCHEMA, movement_policy: Mapping[str, Any] | None = None
    ...
    buckets = sorted(
        {k for e in kernels.values() for k in (e.get(schema.activity_key) or {}) if k not in schema.non_unit_keys}
    )
    ...
    beats = {
        n: float(e[schema.activity_key][schema.read_beats_key] + e[schema.activity_key][schema.write_beats_key])
    ...
    if mover is None:
        ...
        axes["movement_beat_count"] = AxisEvidence(
            axis="movement_beat_count",
            x_name="beats",
            y_name=f"{mover} busy",
            y_unit="cycles",
            points=tuple(Point(beats[n], float(kernels[n][schema.activity_key][mover]), n) for n in sorted(kernels)),
            note=why,
        )

    # --- moved bytes per beat ------------------------------------------------------------------
    foot_pts = [
        Point(beats[n], float(kernels[n][schema.footprint_key]), n)
        for n in sorted(kernels)
        if kernels[n].get(schema.footprint_key) is not None
    ]
    missing = [n for n in sorted(kernels) if kernels[n].get(schema.footprint_key) is None]
    axes["moved_byte_footprint"] = AxisEvidence(
        axis="moved_byte_footprint",
        x_name="beats",
        y_name="operand footprint",
        y_unit="bytes",
    ...
    if not streams:
        refusals.append(Refusal("op streams", "the suite carries none, so no unit's issue structure is observable"))
    ...
    for bucket, family in sorted(paired.items()):
        ...
        for name in sorted(kernels):
            ...
            if composed.cycles is None:
                ...
                packed_pts.append(Point(float(composed.computes) / max(composed.groups, 1), float(busy), label))
                continue
            group_pts.append(Point(float(composed.groups), float(busy) - float(composed.cycles), label))
            packed_excl.append(
                Refusal(
                    label,
                    "one compute op per drained result: this program says nothing about the cost of packing several",
                )
            )
    axes["compute_group_count"] = AxisEvidence(
        axis="compute_group_count",
        x_name="drained result groups",
        y_name="measured busy less the delays the program scheduled",
        y_unit="cycles",
        points=tuple(group_pts),
        excluded=tuple(group_excl),
    ...
    axes["back_to_back_compute"] = AxisEvidence(
        axis="back_to_back_compute",
        x_name="compute ops per drained result",
        y_name="measured busy",
        y_unit="cycles",
        points=tuple(packed_pts),
        excluded=tuple(packed_excl),
    ...
    zero_work = [
        n
        for n in sorted(kernels)
        if not any(str(op[0]) in paired.values() for op in (kernels[n].get(schema.op_stream_key) or []))
        and not (kernels[n].get(schema.op_stream_key) or [])
    ]
    axes["zero_work_program"] = AxisEvidence(
        axis="zero_work_program",
        x_name="issued engine ops",
        y_name="total cycles",
        y_unit="cycles",
        points=tuple(Point(0.0, float(kernels[n][schema.activity_key][schema.total_key]), n) for n in zero_work),
    ...
    axes["working_set_bytes"] = AxisEvidence(
        axis="working_set_bytes",
        x_name="resident bytes",
        y_name="total cycles",
        y_unit="cycles",
    ...
    axes["algorithmic_bytes"] = AxisEvidence(
        axis="algorithmic_bytes",
        x_name="bytes the algorithm needs",
        y_name="bytes moved",
        y_unit="bytes",
    ...
    axes["concurrent_unit_busy"] = AxisEvidence(
        axis="concurrent_unit_busy",
        x_name="summed per-unit busy",
        y_name="observed total",
        y_unit="cycles",
...
def detected_traits(axes: Mapping[str, AxisEvidence], derivation: Mapping[str, Any]) -> dict[str, tuple[bool, str]]:
    ...
    if mover:
        out["explicit_dma"] = (
            True,
            (
                derivation["movement"]["why"] + " -- an engine whose cycles track a beat counter is a data "
    ...
    if pairings:
        out["structural_pipeline_depth"] = (
            True,
            f"{len(pairings)} activity bucket(s) pair uniquely with an op-stream family, so a "
    ...
    if mover and pairings:
        out["multiple_engine_groups"] = (
            True,
            f"a movement engine ({mover}) and {len(pairings)} compute unit(s) carry work in the same corpus",
        )
    axis = axes.get("concurrent_unit_busy")
    if axis is not None and not axis.points:
        out["explicit_completion"] = (
            False,
            "the only activity instrument available partitions the cycle count, so no "
...
class Experiment:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            "rule": self.rule.id,
            "constant": self.rule.constant,
            "term": self.rule.term,
            "axis": self.rule.axis,
            "fit_form": self.rule.fit_form,
...
class Recovery:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "constant": self.constant,
            "value": (None if self.value is UNKNOWN else self.value),
            "unit": self.unit,
    ...
    def validate(self) -> tuple[str, ...]:
        ...
        for r in self.rules:
            ...
            if r.fit_form not in FIT_FORMS:
                problems.append(f"{r.id}: fit form {r.fit_form!r} is not implemented ({sorted(FIT_FORMS)})")
            if r.optimization_family not in self.optimization_families:
                problems.append(f"{r.id}: optimization family {r.optimization_family!r} is not in the vocabulary")
            if r.experiment_family not in self.experiment_families:
                problems.append(f"{r.id}: experiment family {r.experiment_family!r} is not in the vocabulary")
            src = str(r.recover.get("from") or "")
            if src not in FIT_FORMS.get(r.fit_form, ()) and src != "law_inverse":
                problems.append(
                    f"{r.id}: recovers from {src!r}, which is neither a parameter of {r.fit_form!r} nor 'law_inverse'"
                )
            for trait, want in (r.when.get("traits") or {}).items():
                for state in [want] if isinstance(want, str) else list(want):
                    if state not in _TRAIT_STATES:
                        problems.append(
                            f"{r.id}: trait state {state!r} for {trait!r} is not one of {sorted(_TRAIT_STATES)}"
                        )
            if not r.rationale.strip():
                problems.append(f"{r.id}: carries no rationale; a rule nobody can read is a rule nobody can refute")
...
def _as_rule(raw: Mapping[str, Any], source: str) -> Rule:
    return Rule(
        id=str(raw["id"]),
        constant=str(raw["constant"]),
        term=str(raw["term"]),
        axis=str(raw["axis"]),
        fit_form=str(raw["fit_form"]),
        ...
        cross_check=str(raw.get("cross_check") or ""),
        source=source,
    )
...
def load_registry(directory: "str | Path | None" = None) -> RuleRegistry:
    ...
    return RuleRegistry(
        ...
        axes=axes,
        optimization_families=opt,
        experiment_families=exp,
        movement_policy=movement,
        tolerances=tolerances,
    )


def _trait_state(profile: TargetProfile | None, detected: Mapping[str, tuple[bool, str]], name: str) -> tuple[Any, str]:
    ...
    for rule in registry.rules:
        ...
        if axis is None:
            deferrals.append(
                Deferral(
                    rule,
                    f"axis {rule.axis!r} is not carried by any harvested evidence",
            ...
            deferrals.append(
                Deferral(
                    ...
                    (
                        ...
                        f"{len(axis.points)} at {axis.distinct_x}. " + (axis.note or "")
                    ),
                    tuple(
                        [f"{need_points - len(axis.points)} more observation(s) on {rule.axis!r}"]
                        if len(axis.points) < need_points
                        else []
                    )
                    + tuple(
                        [f"{need_levels - axis.distinct_x} more distinct {axis.x_name} level(s)"]
                        if axis.distinct_x < need_levels
                        else []
                    ),
                )
            )
            continue
        experiments.append(Experiment(rule=rule, axis=axis, points_required=need_points, levels_required=need_levels))
    return tuple(experiments), tuple(deferrals)


def run_experiment(
    experiment: Experiment, *, cross_check: Mapping[str, Any] | None = None, tolerance: float = 0.0
) -> Recovery:
    ...
    if not fit.ok:
        return Recovery(experiment=rule.id, constant=rule.constant, value=UNKNOWN, unit=unit, fit=fit, note=fit.refusal)
    ...
    if source == "law_inverse":
        ...
        if raw is None:
            return Recovery(
                ...
                unit=unit,
                fit=fit,
                note=f"the fit carries no parameter {inner!r}",
            )
        ...
        note = (
            f"{rule.constant} = the {source} of a fit of form {rule.fit_form!r} over "
    ...
    if expected is not None and value is not UNKNOWN:
        within = (
            abs(float(value) - float(expected)) <= float(tolerance) * abs(float(expected))
            if float(expected)
            else abs(float(value)) <= float(tolerance)
        )
    return Recovery(
        experiment=rule.id,
        constant=rule.constant,
...
def emit_harvest(
    *,
    ...
    if suite_path is not None:
        ...
        axes, axis_refusals, derivation = axes_from_suite(suite, schema=schema, movement_policy=reg.movement_policy)
        refusals.extend(axis_refusals)
        stream_harvest = harvest_op_stream(
            suite, target=target, authority=harvest.authority, schema=schema, submission=str(suite_path)
        )
    ...
    if prof is None:
        try:
            ...
            harvest.refusals = harvest.refusals + (Refusal("target profile", f"{type(exc).__name__}: {exc}"),)
    ...
    pd = new_product(
        ...
        version=version,
        target=target,
        sources=[str(s) for s in (harvest.roots or ())] + ([str(suite_path)] if suite_path else []),
    ...
    pd.add_artifact("experiments.json").write_text(
        json.dumps(
            {
                "target": target,
...
def main(argv: "Sequence[str] | None" = None) -> int:
    ...
    ap.add_argument(
        "--root", action="append", default=[], help="a run root to mine (default: this target's roots under out/)"
    )
    ap.add_argument("--suite", default=None, help="a measured cycle suite to derive the axes from")
    ap.add_argument("--version", type=int, default=1)
    args = ap.parse_args(list(argv) if argv is not None else None)
    out = emit_harvest(target=args.target, suite_path=args.suite, roots=args.root or None, version=args.version)
    print(out)
    return 0
