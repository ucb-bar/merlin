...
def _semantic_site(node: Mapping[str, Any]) -> tuple[dict[str, Any] | None, str | None]:
    ...
    if kind == "view":
        ...
        if category not in _MATERIALIZATION_CATEGORIES:
            return None, None
        if type(node.get("regions")) is not int or node["regions"] < 0 or not isinstance(node.get("captures"), list):
    ...
    if by_family[0] not in _POINTWISE_CATEGORIES:
        return None, None
    if type(node.get("regions")) is not int or node["regions"] != 0 or node.get("captures") != []:
...
def _refusal_reasons(
    ...
    *,
    uses: Mapping[str, list[int]],
    results: set[str],
    owner: Mapping[int, Mapping[str, Any]],
    relevant: Mapping[int, Mapping[str, Any]],
    ...
    for output in outputs:
        ...
        if not consumers:
            ...
            if owner[consumer]["task_index"] != owner[index]["task_index"]:
                reasons.append(
                    f"buffer {output} crosses task {owner[index]['task_index']}->{owner[consumer]['task_index']}"
                )
...
def inventory_host_epilogue_sites(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    plan = _mapping(
        diagnostics.get("verified_global_plan_emission"), "analysis.diagnostics.verified_global_plan_emission", missing
    )
    graph = _mapping(diagnostics.get("captured_logical_graph"), "analysis.diagnostics.captured_logical_graph", missing)
    program = _mapping(
        graph.get("dispatch_program"), "analysis.diagnostics.captured_logical_graph.dispatch_program", missing
    )
    task_evidence = _mapping(
        diagnostics.get("task_instruction_evidence"), "analysis.diagnostics.task_instruction_evidence", missing
    )
    task_summary = _mapping(
        task_evidence.get("candidate"), "analysis.diagnostics.task_instruction_evidence.candidate", missing
    )
    binding = _mapping(
        task_summary.get("binding"), "analysis.diagnostics.task_instruction_evidence.candidate.binding", missing
    )
    ...
    if (
        task_summary.get("schema") != "task_instruction_evidence_v1"
    ...
    for task in tasks:
        if not isinstance(task, Mapping):
            ...
            continue
        task_index, kind, indices = (
            task.get("task_index"),
            task.get("declared_task_kind"),
    ...
    for source in sorted(relevant):
        for output in nodes[source]["outputs"]:
            ...
            if destination in relevant and owner[source]["task_index"] == owner[destination]["task_index"]:
    ...
    for members in sorted((sorted(value) for value in components.values()), key=lambda row: row[0]):
        ...
        internal = [edge for edge in edges if edge["producer"] in member_set and edge["consumer"] in member_set]
        ...
        candidate = {
            ...
            "input_boundaries": sorted(
                ...
                key=lambda row: (
                    row["buffer"],
                    -1 if row["producer_source_operation_id"] is None else row["producer_source_operation_id"],
                ),
            ),
    ...
    for index, reason in sorted(semantic_refusals.items()):
        refusals.append({"source_operation_id": index, "refusal_class": "semantic", "reasons": [reason]})
        ...
        refusals.append(
            {
                ...
                "reasons": _refusal_reasons(
                    index, nodes[index], uses=uses, results=set(results), owner=owner, relevant=relevant, nodes=nodes
                ),
            }
        )
    ...
    *,
    ...
    for index, member in enumerate(members):
        ...
        for field in ("required_lanes", "required_tiers"):
            ...
            strings = isinstance(values, list) and all(isinstance(value, str) and value for value in values)
    ...
    if (
        ...
        or not _pin(declared_portfolio_sha256)
    ...
    result["status"] = "ready_for_portfolio_source_site_binding" if not inventory_problems else "not_ready"
    result["portfolio_site_inventory_sha256"] = _digest(result)
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = inventory_portfolio_host_epilogue_sites(record, iteration_record_sha256=_raw_sha256(source_bytes))
