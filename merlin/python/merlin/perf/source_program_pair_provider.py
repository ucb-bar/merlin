...
def _record_task_route_presence(result, *, candidate, experiment, pair, prepared):
    ...
    comparison = task_route_presence_for_preparation(
        candidate=candidate, experiment=experiment, pair=pair, prepared=prepared
    )
    result.update(
...
class SourceProgramPairProvider:
    ...
    def __call__(self, *, candidate, experiment, prepared, timeout_s=60):
        ...
        try:
            ...
            _record_task_route_presence(
                result, candidate=candidate, experiment=experiment, pair=pair, prepared=prepared
            )
            ...
            for arm in ("before", "after"):
                ...
                def bounded_build(argv, *, timeout_s):
                    execution = experiment.run_native_probe(
