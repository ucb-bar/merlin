...
def _selected(envelope, source_index, *, short_domain=False):
    ...
    if (
        ...
        or summary.get("schema") != "task_instruction_evidence_v1"
    ...
    if summary["status"] == "short_admitted_static_ownership":
        ...
        if (
            ...
            or any(
                ...
                for key in ("source_sha256", "lowered_sha256", "command_buffer_sha256")
            )
    ...
    if (
        ...
        or not set(unknown).issubset(indices)
        or not isinstance(no_roles, list)
        or not set(no_roles).issubset(indices)
    ):
    ...
    if (
        sum(task["class_counts"].values()) != len(indices)
    ...
    return {
        ...
        "class_counts": dict(task["class_counts"]),
        "role_counts": dict(task["role_counts"]),
        "static_instruction_count": len(indices),
...
def compare_task_route_presence(*, full, short, extraction, extraction_sha256, short_source_op_index):
    ...
    """
    result = {
        "schema": "task_route_presence_comparison_v1",
        "status": "UNKNOWN",
        ...
        "limitations": [
            "instruction counts are static, not work, tile geometry, or latency",
    ...
    try:
        if (
            ...
            or digest(extraction) != extraction_sha256
            or type(extraction.get("source_op_index")) is not int
            or extraction["source_op_index"] < 0
            or type(short_source_op_index) is not int
            or short_source_op_index < 0
        ):
        ...
        for domain, records in (("full", full), ("short", short)):
            ...
            if (
                bindings[domain]["before"]["logical_dispatch_digest"]
                != bindings[domain]["after"]["logical_dispatch_digest"]
            ):
        ...
        def changes(values):
            return {
                domain: {
                    "appeared": sorted(set(arms["after"]) - set(arms["before"])),
        ...
        status = (
            "observed_known_class_presence_change_reproduced"
            if changed and same
            else "no_known_class_presence_change"
            if same
            else "observed_known_class_presence_mismatch"
        )
        result.update(
            status=status,
            bindings=bindings,
            summary_sha256=receipts,
            extraction_sha256=extraction_sha256,
            observations=observations,
            class_presence_changes=changes(classes),
            role_presence_changes=changes(roles),
            known_class_presence_equal_by_arm={
                arm: classes["full"][arm] == classes["short"][arm] for arm in ("before", "after")
            },
            known_role_presence_equal_by_arm={
                arm: roles["full"][arm] == roles["short"][arm] for arm in ("before", "after")
            },
            all_static_classes_decoded=all(
                not row["unknown_instruction_count"] for domain in observations.values() for row in domain.values()
            ),
            selected_owned_payload_changed={
                domain: arms["before"]["owned_instruction_payload_sha256"]
                != arms["after"]["owned_instruction_payload_sha256"]
