...
class HostPhysicalTransitionQualifier:
    ...
    def __call__(self, *, candidate, experiment, timeout_s, portfolio_member=None):
        ...
        try:
            ...
            if callable(getattr(experiment, "selected_changed_portfolio_context", None)):
                selected_context = experiment.selected_changed_portfolio_context(candidate, portfolio_member)
                artifacts = {
                ...
                if portfolio_member is not None:
                    raise ValueError("portfolio member selection requires member-aware experiment accessors")
                artifacts = {
                    "before": experiment.previous_artifacts(candidate),
                    "after": experiment.current_artifacts(candidate),
                }
                analyses = dict(zip(artifacts, (row["analysis"] for row in experiment.iterations[-2:]), strict=True))
            sources = {}
            full = {}
            ...
            for arm, compile_method in (
                ("before", experiment.compile_previous_probe_candidate),
            ...
            for arm, (lowered, cb) in compiled.items():
                record["arms"][arm].update(
                    self._native(candidate, experiment, work / arm, lowered, cb, text, extraction, remaining)
                )
            remaining()
            if selected_context is not None:
                selected_after = experiment.selected_changed_portfolio_context(candidate, selected_context["selection"])
    ...
    def _native(self, candidate, experiment, work, lowered, cb, source, extraction, remaining):
        ...
        for seed in range(3):
            ...
            result = experiment.run_native_probe(
