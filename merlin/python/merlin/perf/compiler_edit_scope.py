...
def inspect_compiler_edits(initial: Path, candidate: Path, contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema": "compiler_submitted_edit_scope_v1",
...
_MECHANISM_SELECTOR_KINDS = frozenset({"imports", "module", "class", "function", "method", "helper"})
...
def _node_body_structure(node: ast.Module | ast.ClassDef) -> tuple[Any, ...]:
    ...
    body = tuple(
        ...
        if not isinstance(item, (ast.Import, ast.ImportFrom, ast.ClassDef, ast.FunctionDef, ast.AsyncFunctionDef))
    )
    return fields, body
...
def _python_semantic_units(before: Path | None, after: Path | None, *, path: str) -> list[dict[str, Any]]:
    ...
    imports = []
    module = []
    definitions = [{} if text is None else _definition_inventory(text) for text in (before_text, after_text)]
    ...
    for kind, values in (("imports", imports), ("module", module)):
        reordered = (
            kind == "module"
            and before_tree is not None
            and after_tree is not None
            and sorted(_definition_order(before_tree)) == sorted(_definition_order(after_tree))
            and _definition_order(before_tree) != _definition_order(after_tree)
        )
        if values[0] != values[1] or reordered:
            if reordered:
                values = [
                    (values[index], _definition_order(tree)) for index, tree in enumerate((before_tree, after_tree))
                ]
            units.append(
                {
                    "kind": kind,
    ...
    for kind, symbol in keys:
        ...
        values = [None if node is None else ast.dump(node, include_attributes=False) for node in nodes]
        ...
        if kind == "class" and nodes[0] is not None and nodes[1] is not None:
            ...
            reordered = sorted(_definition_order(nodes[0])) == sorted(
                _definition_order(nodes[1])
            ) and _definition_order(nodes[0]) != _definition_order(nodes[1])
            if values[0] == values[1] and not reordered:
                continue
            if reordered:
                values = [(values[index], _definition_order(node)) for index, node in enumerate(nodes)]
        units.append(
            {
                "kind": kind,
                "path": path,
...
def validate_mechanism_catalog(
    catalog: Mapping[str, Any], initial: Path, contract: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    if (
        catalog.get("schema") != "compiler_mechanism_catalog_v1"
        ...
        or isinstance(mechanisms, (str, bytes))
        or not mechanisms
    ):
        raise ValueError("mechanism catalog schema, contract, or canonical identity is invalid")
    authority = [(owner["path"], owner["symbol"]) for owner in contract["existing_symbols"]]
    authority_paths = {path for path, _symbol in authority}
    extensions = {_relative(row["directory"]): set(row["surface_ids"]) for row in contract.get("helper_extensions", [])}
    ids: set[str] = set()
        ...
        for selector in selectors:
            ...
            if kind == "helper":
                ...
                if any(
                    ...
                    for other, owner in helper_owners
                    if owner != ident
                ):
                ...
                required = {"kind", "path"} | ({"symbol"} if kind in {"class", "function", "method"} else set())
                ...
                source = initial / str(path)
                if (
                    str(path) not in authority_paths
                ...
                if kind in {"class", "function", "method"}:
                    ...
                    if not any(
                        str(path) == owner_path and (symbol == owner_symbol or symbol.startswith(owner_symbol + "."))
...
def inspect_round_mechanism_edits(
    initial: Path, round_start: Path, candidate: Path, contract: Mapping[str, Any], catalog: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    for path in sorted(before.keys() | after.keys()):
        ...
        for unit in _python_semantic_units(left, right, path=path):
            ...
            helper_code = (
                path not in initial_files
                or unit["symbol"] is not None
                and (unit["kind"], unit["symbol"])
                not in (_definition_inventory(initial_files[path].read_text()) if path in initial_files else {})
            )
            if helper_code:
                owners.update(owner for directory, owner in helpers if directory in relative.parents)
    ...
    if start_scope["status"] != "allowed":
        violations.append(
            {"reason": "round start already exceeds cumulative edit authority", "details": start_scope["violations"]}
        )
    if candidate_scope["status"] != "allowed":
        violations.append(
            {"reason": "candidate exceeds cumulative edit authority", "details": candidate_scope["violations"]}
        )
    if len(mechanisms) > 1:
        violations.append(
            {"reason": "round contains more than one optimization mechanism", "mechanism_ids": sorted(mechanisms)}
        )
    raw_changed = sorted(
        path
        for path in before.keys() | after.keys()
        if path not in before or path not in after or before[path].read_bytes() != after[path].read_bytes()
    )
    return {
        ...
        "semantic_ast_units": units,
        "semantic_noop": not units,
        "formatting_only_noop": bool(raw_changed) and not units,
        "round_changed_paths": raw_changed,
        "violations": violations,
        "cumulative_start_scope": start_scope,
        "cumulative_candidate_scope": candidate_scope,
        "candidate_manifest_grants_authority": False,
