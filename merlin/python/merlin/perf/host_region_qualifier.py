...
from .host_source_witness import (bf16_bits, bf16_values, evaluate_pointwise_source, extract_pointwise_chain,
...
def pointwise_view_chain_emission_evidence(
    ...
    emitted_scalar = lowered_scalar.get(source_scalar)
    names = list(emitted_operation_names)
    activities_derived = all(
        arm.get("status") == "derived" for arm in (before_activity, after_activity))
    operation_counts = {
        category: [(arm.get("dynamic_operations") or {}).get(category)
                   for arm in (before_activity, after_activity)]
        for category in ("floating_arithmetic", "conversion")
    }
    expected = extraction.get("probe_intermediate_payload_bytes")
    deleted = materialization_delta.get("deleted_payload_bytes") or {}
    source_contract = (
        ...
        and set(extraction["view_operations"]) <= {
    ...
    demonstrated = bool(source_contract and activities_derived and direct_output and emitted_scalar
                        and names.count(emitted_scalar) == 1
                        and exact_deletion)
    return {
        "status": ("demonstrated_changed_pointwise_through_view_chain" if demonstrated
                   else "NOT_DEMONSTRATED"),
        ...
        try:
            ...
            for kind, (_, task, payload) in options:
                ...
                try:
                    if kind == "pointwise_view_chain":
                        probe_text, extraction = extract_pointwise_view_chain(
            ...
            if extraction.get("mechanism") == "pointwise_view_chain":
                ...
                if view_evidence["status"] != "demonstrated_changed_pointwise_through_view_chain":
                    raise ValueError(
                        "changed reduced pointwise-through-view-chain mechanism not demonstrated")
            ...
            for seed in range(3):
                ...
                for i, spec in enumerate(input_specs):
                    dtype = np.dtype("float32" if spec["dtype"] in {"f32", "bf16"}
                                     else "int"+spec["dtype"][1:])
                ...
                for arg in abi["args"]:
                    ...
                    if arg["access"] == "read":
                        ...
                        row["values"] = (bf16_bits(values).tolist()
                                         if spec["dtype"] == "bf16" else values.tolist())
                        input_index += 1
                    else:
                ...
                correct_outputs = []
                for output, wanted, output_spec in zip(
                        decoded["outputs"], expected, output_specs):
                    raw_actual = np.asarray(output)
                    actual = (bf16_values(raw_actual.astype(np.uint16)).reshape(wanted.shape)
                    ...
                    if extraction.get("mechanism") in {
                            ...
                            "named_reduction", "insert_slice", "pointwise_concat",
                            "pointwise_view_chain"} and wanted.dtype.kind == "f":
