...
def extract_queued_movement_context(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "decoded_trace_sha256": _digest(trace),
        "binding_method": "host-retained emitted bytes and their decoder output",
        "motifs": [],
        "unsupported": [],
        "considered_primitives": 0,
        "max_commands": max_commands,
        "max_motifs": max_motifs,
        "executed": False,
        "full_model_execution_allowed": False,
    ...
    tasks = {
        ...
        for task in buffer.get("params", {}).get("global_program_plan", {}).get("tasks", [])
    ...
    def config_before(index, subtype):
        return next(
            (
                ...
                if rows[i].get("class") in configs and rows[i].get("decoded", {}).get("subtype") == subtype
            ),
            None,
        )
    ...
    for primitive in initialized_compute_primitives(trace, target=target):
        ...
        if any(
            rows[i].get("class") not in configs and "spad_addr" not in rows[i].get("decoded", {}) for i in indices[:-2]
        ):
        ...
        command_empty = entry is not None and all(rows[i].get("class") in configs for i in range(entry + 1, start))
        ...
        if asm and any(asm[i].parent_block() is not asm[start].parent_block() for i in indices):
            ...
            unexpected = sorted(
                {
                    op.name
                    for op in block_ops[first : last + 1]
                    if op.name != "llvm.inline_asm"
                    and _category(op.name)
                    not in {"constant", "address", "integer_arithmetic", "conversion", "comparison"}
                }
            )
            if unexpected:
        ...
        for index in load_indices:
            ...
            if (
                any(not isinstance(value, int) or isinstance(value, bool) or value <= 0 for value in (count, cols))
            ...
            tensor = (
                abi[argument].get("tensor")
                if isinstance(argument, int) and 0 <= argument < len(abi) and isinstance(abi[argument], Mapping)
                else None
            )
            ...
            transfers.append(
                {
                    ...
                    "physical_rows": [address, address + count],
                    "rows": count,
                    "cols": cols,
                    "external_pointer": dict(external),
                    "tensor": tensor,
                    "dtype": tensor_info.get("dtype"),
                    "load_configuration_index": config,
                    "load_configuration": config_payload,
            ...
            for earlier, slots in extents.items():
                if earlier < index and slots.intersection(extents[index]):
                    dependencies.append(
                        {"before": earlier, "after": index, "kind": "WAW", "state": "decoded operand-store rows"}
                    )
        for item in inputs:
            dependencies.append(
                {
                    "before": item["producer_index"],
                    "after": end,
                    "kind": "RAW",
            ...
            bound = [
                {
                    ...
                    "declared_engine": endpoint.engine,
                    "roles": list(endpoint.roles_of(identity)),
                    "source": endpoint.source,
                }
                for endpoint in endpoints
                if endpoint.roles_of(identity)
            ]
        ...
        semantics = [
            {key: rows[index].get(key) for key in ("class", "funct", "rs1", "rs2", "decoded")} for index in indices
        ]
        motif = {
            "schema": "queued_movement_context_v1",
            "artifact_sha256": actual_sha,
            ...
            "initial_configurations": {
                kind: rows[index] if index is not None else None for kind, index in initial_configs.items()
            },
            "source_entry_completion_index": entry,
            "command_empty_entry_observed": command_empty,
            "source_completion_semantics_proven": False,
            "task_index": task_index,
            "source_op_indices": task.get("source_op_indices") if task else None,
            ...
            "state_missing": sorted(set(state_missing)),
            "context_missing": sorted(set(context_missing)),
            ...
            "calibration_admissible": False,
            "in_context_cycles": None,
            "overlap_observed": None,
            "physical_parallelism_proven": False,
        }
        motif["context_shape_sha256"] = _digest(
            {"instructions": semantics, "initial_configurations": motif["initial_configurations"], "target_isa": isa}
        )
