...
def bind_source_program_pair(
    *,
    ...
    if comparison_arm == "optimization_baseline":
        ...
        before_raw = before_context["artifacts"] if selected is not None else experiment.previous_artifacts(candidate)
        binding = (
            {
            ...
            else experiment.previous_probe_binding(candidate).to_dict()
        )
        ...
        before_proof = (
            ...
            if selected is not None
            else experiment.iterations[-2]["analysis"]["diagnostics"].get("verified_global_plan_emission")
        )
    artifacts = {
