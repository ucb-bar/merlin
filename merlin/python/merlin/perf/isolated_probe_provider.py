...
class IsolatedPrimitiveProbeProvider:
    ...
    def __call__(self, *, candidate: Path, experiment: Any, timeout_s: float | None = None) -> dict[str, Any]:
        from merlin.targetgen import gsim_emulator
        from merlin.targetgen.rocc import decode

        started = monotonic()
        limit = min(600.0, float(experiment.timeout_s), 600.0 if timeout_s is None else float(timeout_s))
