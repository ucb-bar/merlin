...
def _integer(value: Any, label: str, *, positive: bool = False) -> int:
    _require(
        ...
        f"{label} must be a {'positive' if positive else 'nonnegative'} integer",
    )
    return value
...
class CompactBinding:
    ...
    def __init__(
        ...
        *,
        ...
        self._readonly_snapshots = tuple(
            bytes(arena) if access == "read" else None
    ...
    def base_views(self) -> tuple[memoryview, ...]:
        ...
        return tuple(
            memoryview(arena).toreadonly() if access == "read" else memoryview(arena)
    ...
    def original_argument_views(self) -> tuple[memoryview, ...]:
        ...
        return tuple(
            bases[arg.base_index][arg.byte_offset : arg.byte_offset + arg.byte_extent] for arg in self._arguments
        )
    ...
    def validate_runtime_addresses(self, base_addresses: Sequence[int]) -> tuple[int, ...]:
        ...
        spans = []
        for index, (address, arena, alignment) in enumerate(
            zip(base_addresses, self._arenas, self._target_abi.base_alignments, strict=True)
        ):
        ...
        _require(
            all(
                address % alignment == 0
                for address, alignment in zip(result, self._target_abi.argument_alignments, strict=True)
            ),
    ...
    def readback(self, base_payloads: Sequence[bytes] | None = None) -> dict[str, bytes]:
        ...
        for arg in self._arguments:
            ...
            for index in product(*(range(dim) for dim in encoding.logical_shape)):
                offset = (
                    encoding.offset_elements
                    + sum(i * s for i, s in zip(index, encoding.logical_strides_elements, strict=True))
                ) * width
    ...
    def runtime_address_evidence(self, base_addresses: Sequence[int]) -> dict:
        ...
        return {
            "schema": "compact_runtime_addresses_v1",
            "status": "validated",
            ...
            "original_argument_addresses": list(arguments),
            "alignment_and_nonoverlap_checked": True,
            "live_target_allocation_verified": False,
            "entry_invoked": False,
    ...
    *,
    ...
    _require(
        all(
            type(value) is tuple
            for value in (
                target_abi.pointer_index_bits,
                target_abi.base_alignments,
                target_abi.argument_alignments,
                target_abi.base_access,
            )
        ),
        "target ABI facts require immutable ordered tuples",
    )
    _require(isinstance(command_buffer, Mapping), "command buffer must be a mapping")
    _require(
        isinstance(target_abi.provenance, str) and bool(target_abi.provenance.strip()), "target ABI provenance required"
    )
    _require(
        isinstance(compact_contract, Mapping) and compact_contract.get("schema") == "compact_pointer_entry_v1",
        "unsupported compact entry contract",
    )
    for key in ("original_symbol", "compact_symbol", "binding_provenance"):
        _require(
            isinstance(compact_contract.get(key), str) and bool(compact_contract[key].strip()), f"missing compact {key}"
        )
    _require(
        compact_contract["original_symbol"] != compact_contract["compact_symbol"],
        "compact entry must have a distinct symbol",
    )
    _require(
        compact_contract.get("storage_reused") is False
        and compact_contract.get("alignment_or_noalias_added") is False
    ...
    _require(
        isinstance(args, list) and bool(args) and all(isinstance(arg, Mapping) for arg in args),
        "complete original argument list required",
    )
    bases, bindings = compact_contract.get("bases"), compact_contract.get("bindings")
    _require(
        isinstance(bases, list) and bool(bases) and all(isinstance(base, Mapping) for base in bases),
        "compact bases malformed",
    )
    _require(
        isinstance(bindings, list) and all(isinstance(binding, Mapping) for binding in bindings),
        "compact bindings malformed",
    )
    _require(
        type(compact_contract.get("original_argument_count")) is int
        and compact_contract["original_argument_count"] == len(args)
        and type(compact_contract.get("compact_argument_count")) is int
        and compact_contract["compact_argument_count"] == len(bases),
        "compact argument counts disagree with original ABI",
    )
    _require(
        len(target_abi.pointer_index_bits)
        == len(target_abi.base_alignments)
        == len(target_abi.base_access)
        == len(bases)
        and len(target_abi.argument_alignments) == len(args),
        "incomplete target ABI facts",
    )
    ...
    for i, base in enumerate(bases):
        ...
        _require(
            ...
            "compact base names must be unique and nonempty",
        )
        names.add(name)
    ...
    _require(
        [row["argument_index"] for row in ordered] == list(range(len(args))),
        "bindings must cover every original argument exactly once",
    )
    ...
    _require(
        len(access) == len(args)
        and all(isinstance(name, str) and mode in ("read", "write", "readwrite") for name, mode in access.items()),
        "original arguments must be distinct typed tensor pointers",
    )
    _require(
        set(logical_payloads) == {name for name, mode in access.items() if mode != "write"},
    ...
    for index, (arg, binding) in enumerate(zip(args, ordered, strict=True)):
        ...
        _require(
            max(total_format_storage, total_logical_storage) <= max_storage_bytes,
        ...
        _require(
            target_abi.base_access[base] == arg["access"], "tensor access differs from homogeneous base access group"
        )
        alignment = target_abi.argument_alignments[index]
        _require(
            target_abi.base_alignments[base] % alignment == 0 and offset % alignment == 0,
        ...
        if arg["access"] != "write":
            ...
            _require(
                type(payload) is bytes and len(payload) == logical_extent,
                "logical payload differs from declared dtype/shape byte extent",
            )
    ...
    for intervals in spans:
        ...
        _require(
            ...
            "tensor bindings overlap; aliasing/reuse unsupported",
        )
    storage = resolve_storage_bindings(
        command_buffer, inputs, max_storage_bytes=max_storage_bytes, prepack_authorizations=prepack_authorizations
    )
    ...
    evidence = {
        "schema": "compact_caller_binding_v1",
        "compact_symbol": compact_contract["compact_symbol"],
        "original_symbol": compact_contract["original_symbol"],
        "command_buffer_sha256": _digest(command_buffer),
        "compact_contract_sha256": _digest(compact_contract),
        "target_abi": asdict(target_abi),
        "target_abi_sha256": _digest(asdict(target_abi)),
        "base_storage_bytes": extents,
        "original_argument_tensors": [arg.tensor for arg in arguments],
        "logical_input_sha256": {
            name: hashlib.sha256(payload).hexdigest() for name, payload in logical_payloads.items()
        },
        ...
        "numerical_equivalence": "UNPROVEN",
    ...
    return CompactBinding(
        arenas=arenas, arguments=tuple(arguments), outputs=tuple(outputs), target_abi=target_abi, evidence=evidence
    )
