...
def rewrite_matmuls_to_openblas(mlir_text: str) -> tuple[str, int]:
    ...
    unchanged. Raises
    :class:`~merlin.runtime.backends._matmul_routing.MatmulRoutingError` on a ``linalg.matmul``
    the structural reader cannot parse, rather than silently routing nothing.
    """
    return _mr.rewrite_matmuls(mlir_text, "merlin_openblas_gemm_f32")
