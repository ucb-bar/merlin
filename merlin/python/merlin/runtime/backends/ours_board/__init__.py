...
# A routable matmul line (single textual op) — identical matcher to xnnpack_board/openblas_board.
_MM_RE = re.compile(
...
def _is_routable(at: str, bt: str, ct: str, rt: str) -> bool:
    """Plain 2-D f32 matmul (identical to the expert backends' faithful set)."""
...
def rewrite_matmuls_to_ours(mlir_text: str) -> tuple[str, int]:
    ...
    def repl(m: re.Match) -> str:
        ...
        sym = sigs.get(key)
        if sym is None:
            sym = f"merlin_ours_gemm_f32_{len(sigs)}"
            sigs[key] = sym
        n += 1
