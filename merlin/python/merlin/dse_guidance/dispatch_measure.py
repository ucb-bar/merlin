...
def load_measured(path=None) -> list[DispatchMeasurement]:
    ...
    return [
        DispatchMeasurement(
            ...
            matmul_estimate=int(r.get("matmul_estimate", 0)),
            cos=float(r.get("cos", 0.0)),
            exec_wall_s=_f(r, "exec_wall_s"),
            compute_call_ms=_f(r, "compute_call_ms"),
            host_overhead_ms=_f(r, "host_overhead_ms"),
            overhead_frac=_f(r, "overhead_frac"),
        )
...
def measure_host_breakdown(model_dir, workdir=None, cache_dir=None) -> DispatchMeasurement:
    ...
    res = DR.run_model(md, wd, cache_dir=cache, tap=lambda op, val: events.append((op.name, time.perf_counter())))
    ...
    return DispatchMeasurement(
        model=md.name,
        n_kernels=int(res["n_kernels"]),
        n_unique=int(res.get("n_unique_kernels") or 0),
        matmul_estimate=matmuls,
        cos=float(res.get("cos", 0.0)),
        exec_wall_s=round(total, 3),
        compute_call_ms=round(compute * 1e3, 1),
        host_overhead_ms=round(overhead * 1e3, 1),
...
def calibration_rows(measured: list[DispatchMeasurement] | None = None) -> list[dict]:
    ...
    for m in measured or load_measured():
        ...
        rows.append(
            {
                "model": m.model,
                "quantity": "dispatches_per_forward",
                "predicted_matmul_proxy": m.matmul_estimate,
                "measured": m.n_kernels,
...
def report_md(measured: list[DispatchMeasurement] | None = None) -> str:
    ...
    L.append(
        "> The dispatch *count* per forward is measured by running the real captured model "
    ...
    for m in ms:
        ...
        L.append(f"| {m.model} | {m.matmul_estimate} | {m.n_kernels} | {uc} | {m.n_unique} | {m.cos:.4f} |")
    L.append("")
    if ms:
        avg = sum(m.undercount_ratio for m in ms if m.undercount_ratio) / len(ms)
        L.append(
            f"**Finding:** the matmul-count proxy under-counts real dispatch granularity by "
    ...
    if bd:
        ...
        for m in bd:
            L.append(
                f"| {m.model} | {m.n_kernels} | {m.compute_call_ms} | {m.host_overhead_ms} | {m.overhead_frac:.2f} |"
            )
        avgf = sum(m.overhead_frac for m in bd) / len(bd)
        L.append("")
        L.append(
            f"**Finding:** ~{avgf * 100:.0f}% of host time per forward is dispatch/allocation "
...
def to_csv(rows: list[dict]) -> str:
    cols = ["model", "quantity", "predicted_matmul_proxy", "measured", "error_pct", "undercount_ratio", "evidence_type"]
    buf = io.StringIO()
