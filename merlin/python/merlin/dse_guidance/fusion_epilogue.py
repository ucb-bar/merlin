...
_SHAPE_OPS = {"tensor.expand_shape", "tensor.collapse_shape", "tensor.extract_slice", "linalg.transpose"}
...
_CAST = {"arith.sitofp", "arith.fptosi", "arith.truncf", "arith.extf", "arith.fptrunc", "arith.uitofp"}
    ...
    for i, mm in enumerate(matmuls):
        ...
        out.append(
            EpiloguePattern(
                index=i,
                op_kind=op_kind,
                pattern=_pattern_label(flags),
                has_bias=flags["bias"],
                has_activation=flags["activation"],
                has_scale=flags["scale"],
                has_clamp=flags["clamp"],
                has_cast=flags["cast"],
                epilogue_generic_count=n_generic,
                reshape_separated_epilogue=reshape_separated,
                evidence=E_IR,
            )
        )
    return tuple(out)
    ...
    for r in attribution.regions:
        ...
        out.append(
            AccumulatorContract(
                ...
                output_dtype=output,
                scale_dtype="unavailable",
                scale_granularity="unavailable",
                dequant_location=dequant,
                requant_location=requant,
                accumulator_materialization=acc_mat,
                evidence=E_IR,
            )
        )
    return out
...
def certificates(patterns, accum) -> list[FusionCertificate]:
    ...
    def cert(abstraction, present, proof, hw, knobs, acc_need, could_wrong):
        return FusionCertificate(
            ...
            evidence=(E_IR if present else E_NA),
            required_compiler_proof=proof,
            required_hw_runtime_support=hw,
            dse_knobs_exposed=knobs,
            accuracy_measurements_needed=acc_need,
            performance_measurements_needed_later="per-op latency / energy once a backend exists",
            what_is_not_claimed=NOT,
            could_be_wrong_if=could_wrong,
        )
    ...
    return [
        ...
        cert(
            "activation_clamp_unit",
            any_act or any_clamp,
            ...
            rows.append(
                {
                    "workload": wl,
                    "op_index": p.index,
                    "op_kind": p.op_kind or "matmul",
                    "pattern": p.pattern,
                    "has_bias": p.has_bias,
                    "has_activation": p.has_activation,
                    "has_scale": p.has_scale,
                    "has_clamp": p.has_clamp,
                    "has_cast": p.has_cast,
            ...
            rows.append(
                {
                    ...
                    "accumulator_dtype": a.accumulator_dtype,
                    "output_dtype": a.output_dtype,
                    "scale_dtype": a.scale_dtype,
                    "scale_granularity": a.scale_granularity,
...
def lost_contracts_csv(acc_by_workload: dict) -> str:
    ...
    for wl, accs in acc_by_workload.items():
        ...
        for contract, status, reason in [
            (
                "low_bit_weight_storage",
                ("present" if lowbit else "lost"),
                "weights are f32 in the capture (dequantized) — native low-bit storage erased"
                if not lowbit
                else "low-bit storage visible",
            ),
            ("scale_zero_point_metadata", "erased", "scales/zero-points folded away when the capture was dequantized"),
            ("packed_lowbit_layout", "lost", "weights stored in dense f32 layout — packed sub-byte layout erased"),
...
def fusion_report_md(pat_by_workload: dict, acc_by_workload: dict, cert_by_workload: dict) -> str:
    ...
    n_reshape = sum(1 for pats in pat_by_workload.values() for p in pats if p.reshape_separated_epilogue)
    ...
    L.append(
        f"- Of the {n_plain} matmuls with **no directly-fused epilogue**, {n_reshape} are "
    ...
    L.append(
        "- **Preserved (`recovered_from_ir`):** the matmul→bias epilogue (addmm / addf) and the "
        "activation/clamp ops (erf / exp / maximumf) that follow it — the epilogue slot is real."
    )
    L.append(
        "- **Erased (`unavailable` / lost):** dequant-before-matmul, requant, scale/zero-point "
    ...
    L.append(
        f"- **Suggested (IR-supported):** {', '.join('`' + a + '`' for a in sup) or '—'} — the "
        "epilogue slot, accumulator, and activation/clamp unit are backed by detected patterns."
    )
    L.append(
        f"- **Blocked (missing low-bit / scale / sparsity in the capture):** "
    ...
    L.append(
        "- adds an **epilogue-op-set** knob (which ops fuse onto the matmul output: bias / "
        "requant / activation / clamp) — the slot is proven present;"
    )
    L.append("- adds an **accumulator-dtype** knob (i32 for int8 storage, derived);")
    L.append(
        "- gates the **weight-format / dequant-in-load / scale-granularity** knobs behind a "
