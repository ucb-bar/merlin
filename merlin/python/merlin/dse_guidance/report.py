...
_DIFF_COLUMNS = [
    ...
    "replan_deadline_ms",
    "deadline_visible",
    "visible_weight_reuse",
    "visible_prefix_kv_reuse",
...
def triage_md(multirate: dict, flat: dict, baseline: BaselineCost) -> str:
    ...
    L.append(
        "> ⚠️ **QUANTITATIVE — uncalibrated.** These gap_closure / priority numbers are only "
    ...
    if baseline.target_total_ms is not None:
        ...
        L.append(f"- target gap: **{tg:g} ms**" + ("" if (tg or 0) > 0 else "  _(baseline already meets target)_"))
    else:
        L.append("- target total: _none provided_ (reporting baseline share only)")
    L.append("")
    L.append(
        "`gap_closure = (baseline_total - intervention_total) / (baseline_total - "
        "target_total)`  ·  "
    ...
    for label, tr in (("Multi-rate representation", multirate), ("Flat representation", flat)):
        ...
        for r in tr["axes"]:
            ...
            L.append(
                f"| {r['axis']} | {gc} | {ps} | {r['evidence_type']} | "
    ...
    if gained:
        L.append(
            f"- Axes the flat capture **hides**: **{', '.join(gained)}** — these only "
...
def _top_recommended(tr: dict, n: int = 3) -> list[str]:
    out = [
        r["axis"] for r in tr["axes"] if r["priority_score"] is not None and r["priority_score"] > 0 and r["legality"]
    ]
    return out[:n]
...
def negative_control_md(triage_multirate: dict, baseline: BaselineCost) -> str:
    ...
    L.append(
        "A workload with no cross-step reuse (K=1, no loop-invariant weights) must NOT "
    ...
    for axis in checks:
        ...
        gc = "n/a" if r["gap_closure"] is None else f"{r['gap_closure']:.3f}"
        ps = "n/a" if r["priority_score"] is None else f"{r['priority_score']:.4f}"
        deprioritized = (r["legality"] == 0) or (r["priority_score"] in (None, 0)) or (r["gap_closure"] in (None, 0))
        ok = ok and deprioritized
        verdict = "deprioritized ✅" if deprioritized else "RECOMMENDED ❌"
        L.append(f"| {axis} | {r['legality']} | {gc} | {ps} | {verdict} |")
    L.append("")
    L.append(
        f"**Result: {'PASS' if ok else 'FAIL'}** — residency/autonomous-loop features "
