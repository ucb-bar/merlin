    ...
    temporal = T.parse(
        {
            ...
            "regions": [
                ...
                {"name": "head", "role": "repeated_head", "invocation_count": K, "loop_invariant_state": ["weights"]},
            ],
        }
    )
...
def _rows_for(case: WorkloadCase) -> list[dict]:
        ...
        rows.append(
            {
                ...
                "item": "action-head weight reuse",
                "flat_view": "weights used once (0 contract facts)",
                "recovered_view": (
                    f"repeated_head x{case.K}: {f['matmul_count']} matmuls, "
    ...
    if bb and bb.attribution_status == "attributed":
        rows.append(
            {
                "workload": case.workload,
                "item": "backbone vs head split",
                "flat_view": "undifferentiated single forward",
                "recovered_view": (
                    f"backbone_once: {bb.facts['matmul_count']} matmuls; "
    ...
    rows.append(
        {
            "workload": case.workload,
            "item": "host dispatch / sync coupling",
...
def case_study_md(cases: list[WorkloadCase]) -> str:
    ...
    L.append(
        "> Flat model captures are insufficient DSE units. With `prov.fqn` provenance, Merlin "
    ...
    L.append(
        f"Real recaptured workloads: **{len(cases)}** "
    ...
    for c in cases:
        ...
        L.append(
            f"| {c.workload} | weights once, no K-loop | {recovered} | {facts} | {impl} | "
    ...
    for c in cases:
        ...
        head = _role(c, "repeated_head")
        hf = (
            f"{head.facts['matmul_count']} mm, {head.facts['weight_bytes'] / 1e6:.0f} MB, "
            f"{head.facts['macs_per_invocation'] / 1e9:.1f} GMAC/step xK={c.K}"
            if head and head.attribution_status == "attributed"
            else "—"
        )
        L.append(
            f"| {c.workload} | {c.cls} | {c.n_matmuls} | "
    ...
    L.append(
        "- **Flat view:** weights used once, no K-loop, no backbone/head split, no deadline — "
        "so residency / autonomous-loop / partition axes are invisible or illegal."
    )
    L.append(
        "- **Recovered view:** roles from `prov.fqn`, real per-region MACs/bytes, the repeated "
        "head and (for OpenVLA) the vision-backbone/LM split made explicit."
    )
    L.append(
        "- **Honest gate:** every candidate carries real facts but stays "
    ...
    L.append(
        "See `cross_workload_provenance.csv` for the per-item flat-vs-recovered table, and "
...
def _abstraction_pressure_ranking(packages) -> str:
    ...
    for p in packages:
        for c in p["cands"]:
            a = agg.setdefault(
                c.axis, {"sys": c.system_abstraction, "workloads": set(), "n_regions": 0, "blocked": Counter()}
            )
    ...
    return _csv(rows, ["axis", "system_abstraction", "n_workloads", "n_regions", "evidence_strength", "blocked_by"])
...
def _measurement_priority_table(packages) -> str:
    ...
    for p in packages:
        ...
        for m in mn.get("accuracy", []):
            cat_of[m] = "accuracy_measurable_now"
            wls[m].add(p["case"].workload)
        for m in mn.get("runtime_proxy", []):
            cat_of[m] = "proxy_measured"
            wls[m].add(p["case"].workload)
        for m in plan.get("needs_target_design", []):
            cat_of[m] = "target_measured"
            wls[m].add(p["case"].workload)
    ...
    rows = [
        {
            "measurement": m,
            "category": cat,
            "n_candidates_unblocked": unblocks.get(m, 0),
            "workloads": "; ".join(sorted(wls[m])),
        }
...
def _dse_readiness_table(packages) -> str:
    ...
    return _csv(
        ...
        [
            ...
            "accuracy_status",
            "cpu_coupling",
            "ready_structural_DSE",
            "ready_quantitative_DSE",
            "missing_before_ranking",
        ],
    )
        ...
        rows.append(
            {
                ...
                "bf16_B": round(cap["bf16"]),
                "fp8_B": round(cap["fp8"]),
                "int8_B": round(cap["int8"]),
                "fp6_B": round(cap["fp6"]),
                "int4_B": round(cap["int4"]),
                "avoidable_reload_B": _envval(env, "avoidable_weight_reload_bytes"),
            }
        )
    return _csv(
        rows, ["workload", "captured_dtype", "bf16_B", "fp8_B", "int8_B", "fp6_B", "int4_B", "avoidable_reload_B"]
    )
...
def _case_study_summary_md(packages) -> str:
    ...
    L.append(
        "> Flat captures are not DSE-ready workload descriptions. Merlin recovers a temporal/"
    ...
    for p in packages:
        ...
        facts = f"{macs / 1e9:.0f} GMAC/replan, {avoid / 1e9:.2f} GB avoidable reload" if macs and avoid else "—"
        bf16 = f"resident bf16 {env.capacity_by_dtype_B['bf16'] / 1e6:.0f} MB" if env else "—"
        impl = p["cands"][0].system_abstraction if p["cands"] else "—"
        L.append(
            f"| {p['case'].workload} | {_roles_str(p['case'])} | {facts} | {bf16} | {impl} | "
            f"{'; '.join(p['readiness'].missing)} |"
        )
    L.append("")
    L.append(
        "See per-workload `<workload>/workload_contract_report.md` for the full package, "
...
def _dse_contract_manifest(packages, knob_catalog, boundary_certs) -> dict:
    ...
    for p in packages:
        ...
        per_wl[c.workload] = {
            "class": c.cls,
            "K": c.K,
            "K_source": "recovered_from_model_config",
            ...
            "top_by_evidence_breadth": [
                {
                    ...
                    "strong_levels": [b["level"] for b in c.boundary_levels if b["status"] == "strong_candidate"],
                    "supporting_workloads": c.supporting_workloads,
                }
                for c in top_boundary
            ],
        },
...
def _dse_search_space_knobs(
    all_shapes, all_axes, dags, units, overlap_by_wl, pat_by_wl, region_mem_by_wl, boundary_certs=None
) -> dict:
    ...
    clean_mn = sum(1 for a in all_axes if a.axis in ("M", "N") and a.shardable[8] and not a.has_tail[8])
    ...
    pipe_abs = sorted(
        {
            a
            for cands in overlap_by_wl.values()
            for c in cands
            if c.can_overlap == "yes"
            for a in c.required_abstractions
        }
    )
    ...
    groups = [
        ...
        {
            "group": "intra_op_sharding",
            "source_phase": "P7",
            "knobs": [f"shard_axis in {{M,N,K}}", f"shard_count in {list(SH.UNIT_COUNTS)}"],
            "enabled": clean_mn > 0,
            "evidence": DE.E_DERIVED,
            "gated_by": f"{clean_mn} reduction-free M/N shards available; K-sharding needs partial-sum reduction",
        },
        {
            "group": "inter_op_parallelism",
            "source_phase": "P7",
            "knobs": ["num_engines"],
            "enabled": avg_par >= 1.5,
            "evidence": DE.E_DERIVED,
            "gated_by": f"avg inter-op parallelism {avg_par}x (low -> limited; favors intra-op sharding)",
        },
        {
            "group": "processing_unit_set",
            "source_phase": "P7/P8",
            "knobs": units_ir,
            "enabled": True,
            "evidence": DE.E_IR,
        ...
        {
            "group": "pipeline_overlap",
            "source_phase": "P8",
            "knobs": pipe_abs,
            "enabled": bool(pipe_abs),
            "evidence": DE.E_DERIVED,
        ...
        {
            "group": "memory_residency",
            "source_phase": "P9",
            "knobs": ["resident_weight_object", f"weight_dtype in {list(DE.CAPACITY_FORMATS)}", "prefetch_depth"],
            "enabled": True,
            "evidence": DE.E_IR,
        ...
        },
        {
            "group": "dma_streams",
            "source_phase": "P9",
            "knobs": ["multi_stream_dma_descriptor", "double_buffered_activation_tile", "prefetch_weight_once"],
            "enabled": True,
            "evidence": DE.E_IR,
            "gated_by": "3 byte-carrying streams/region (weight/activation/output)",
        },
        {
            "group": "epilogue_fusion",
            "source_phase": "P10",
            "knobs": [f"epilogue_op_set subset of {epi_ops}", "accumulator_dtype", "requant_in_epilogue"],
            "enabled": bool(epi_ops),
            "evidence": DE.E_IR,
    ...
    if boundary_certs:
        ...
        groups.append(
            {
                "group": "hw_sw_boundary_placement",
                "source_phase": "P12",
                "knobs": [f"{c.abstraction}@{{compiler/runtime/command/isa/microcode/datapath}}" for c in top],
                "enabled": True,
                "evidence": DE.E_DERIVED,
...
def _dse_search_space_knobs_md(catalog: dict) -> str:
    ...
    for g in groups:
        ...
        L.append(f"| {g['group']} | {g['source_phase']} | {g['enabled']} | {knobs} | {g['gated_by']} |")
    L.append(
...
def zoo_numerical_audit() -> list:
    ...
    return [c for c in contracts if c.declared_quantization != "none" or c.low_bit_storage] or contracts
    ...
    for c in cases:
        ...
        for r in ATTR.extract_matmuls(cap):
            rows.append(
                {
                    "workload": c.workload,
                    "source": "linalg.matmul",
                    "op_index": r.index,
                    "op_class": "linear_gemm",
                    "prov_op": r.op or "",
                    "prov_fqn": r.fqn or "",
                    "role": ATTR.role_from_fqn(r.fqn) or "",
                    "M": r.M,
                    "K": r.K,
                    "N": r.N,
                    "batch": 1,
...
def work_coverage_csv(cases: list[WorkloadCase]) -> str:
    ...
    for c in cases:
        ...
        rows.append(
            {
                "workload": c.workload,
                "n_linear_matmul": len(mm),
                "linear_gemm_macs": lin,
                "n_attention_ops": cls.get(ATTR.OPC_ATTENTION, 0),
                "attention_macs": attn,
                "n_batched_matmul": cls.get(ATTR.OPC_BATCHED_MATMUL, 0),
                "batched_matmul_macs": bmm,
                "total_recovered_macs": tot,
                "visible_linear_fraction": round(lin / tot, 4) if tot else 1.0,
                "n_softmax": cls.get(ATTR.OPC_SOFTMAX, 0),
                "n_normalization": cls.get(ATTR.OPC_NORM, 0),
                "n_conv": cls.get(ATTR.OPC_CONV, 0),
                ...
                "n_layout": cls.get(ATTR.OPC_LAYOUT, 0),
                "n_other": cls.get(ATTR.OPC_OTHER, 0),
            }
        )
...
_ABLATION_CSV_COLS = ["workload", "level", "available"] + [o.replace(".", "_") for o in _ABLATION_OPS]
...
def capture_level_ablation_csv() -> str:
    ...
    wls = (
        sorted(
            p.name for p in lvl.glob("*") if (p / "model_highlevel.mlir").is_file() or (p / "model_qdq.mlir").is_file()
        )
        if lvl.is_dir()
        else []
    )
        ...
        rows.append(
            {
                ...
                "loop_preserved": True,
                "K": lr.K,
                "K_source": lr.K_source,
                "n_iter_args": lr.n_iter_args,
                "loop_carried_roles": roles,
...
_RESIDENCY_COLS = [
    ...
    "loop_preserved",
    "K",
    "n_loop_invariant_operands",
    "n_loop_carried_iter_args",
    "n_body_defs",
    "resident_proof",
    "evidence",
]
...
def run_case_study(out_dir) -> dict:
    ...
    for c in cases:
        ...
        yaml_artifact(
            f"{c.workload}/region_attribution.yaml",
            ATTR.to_yaml_obj(c.attribution),
            header=f"region_attribution (Level-1, prov.fqn): {c.workload}",
        ).write(out)
        Artifact(f"{c.workload}/dse_candidate_axes.md", CAND.markdown(c.topo, c.candidates)).write(out)
        # state-lifetime / residency view (which tensors persist + the abstraction they imply)
        recs = SL.state_records(c.topo, c.attribution)
        yaml_artifact(
            f"{c.workload}/state_lifetime.yaml",
            SL.to_yaml_obj(recs, c.workload),
            header=f"state_lifetime: {c.workload}",
        ).write(out)
        # memory-traffic / reuse envelope + (honest) command-graph view
        mem = ME.region_traffic(c.attribution)
        yaml_artifact(
            f"{c.workload}/memory_envelope.yaml",
            ME.to_yaml_obj(mem, c.workload),
            header=f"memory_envelope: {c.workload}",
        ).write(out)
        cmd = CG.command_graph(c.topo, c.attribution)
        yaml_artifact(
            f"{c.workload}/command_graph.yaml",
            CG.to_yaml_obj(cmd),
            header=f"command_graph (honest; loop unrolled): {c.workload}",
        ).write(out)
        # numerical contract per recaptured workload (fp32 -> low-bit opportunity)
        nc = NC.audit(
            cap,
            workload=c.workload,
            workload_class=c.cls,
        ...
        yaml_artifact(
            f"{c.workload}/numerical_contract.yaml", NC.to_yaml_obj(nc), header=f"numerical_contract: {c.workload}"
        ).write(out)
        ...
        graphs.append(CGRAPH.build_graph(c, shapes, nc, recs, dependencies=ATTR.matmul_dependencies(cap)))
        # design envelope (hardware-independent requirements)
        env = DE.from_recovered(c.topo, c.attribution, captured_dtype=dtype)
        if env is not None:
            envelopes.append(env)
            yaml_artifact(
                f"{c.workload}/design_envelope.yaml",
                DE.to_yaml_obj(env),
                header=f"design_envelope (requirements, not calibration): {c.workload}",
            ).write(out)
        ...
        yaml_artifact(
            f"{c.workload}/numerical_candidate_certificates.yaml",
            DC.to_yaml_obj(certs, c.workload),
            header=f"numerical_candidate_certificates: {c.workload}",
        ).write(out)
        ...
        readiness = CON.dse_readiness(c.topo, c.attribution, nc, cpu_coupling_available=False, accuracy_status=acc)
        plan = CON.measurement_plan(cands)
        yaml_artifact(
            f"{c.workload}/abstraction_candidates.yaml",
            CON.abstraction_yaml(cands),
            header=f"abstraction_candidates: {c.workload}",
        ).write(out)
        yaml_artifact(
            f"{c.workload}/dse_readiness.yaml", CON.readiness_yaml(readiness), header=f"dse_readiness: {c.workload}"
        ).write(out)
        yaml_artifact(
            f"{c.workload}/measurement_plan.yaml", {"measurement_plan": plan}, header=f"measurement_plan: {c.workload}"
        ).write(out)
        Artifact(
            f"{c.workload}/workload_contract_report.md",
            CON.workload_contract_report_md(c.topo, c.attribution, nc, env, cands, readiness, plan),
        ).write(out)
        pkg = {
            "case": c,
            "env": env,
        ...
        yaml_artifact(
            f"{c.workload}/dse_search_space_template.yaml",
            SS.to_yaml_obj(SS.template_for_workload(pkg)),
            header=f"dse_search_space_template: {c.workload}",
        ).write(out)
        packages.append(pkg)
    if envelopes:
    ...
    Artifact("abstraction_pressure_ranking.csv", _abstraction_pressure_ranking(packages)).write(out)
    Artifact("workload_family_table.csv", WF.workload_family_csv(packages)).write(out)
    Artifact("measurement_priority_table.csv", _measurement_priority_table(packages)).write(out)
    ...
    for fam, axis_set in sorted(WF.family_axis_sets(packages).items()):
        yaml_artifact(
            f"dse_search_space_template_{fam}.yaml",
            SS.to_yaml_obj(SS.template_for_family(fam, axis_set)),
            header=f"dse_search_space_template (family): {fam}",
        ).write(out)
    ...
    Artifact("dataflow_candidate_table.csv", _csv(MS.dataflow_rows(_wl_caps), MS._DATAFLOW_COL)).write(out)
    yaml_artifact(
        "timeloop_problem_shapes.yaml",
        MS.problem_shapes(_wl_caps),
        header="timeloop_problem_shapes (structural search-space seeds; no perf claim)",
    ).write(out)
    ...
    Artifact("mapspace_seeds_loop.csv", _csv(MS.loop_aware_seed_rows(out), MS._LOOP_SEED_COLS)).write(out)
    yaml_artifact(
        "mapspace_seeds_loop.yaml",
        MS.loop_aware_problem_shapes(out),
        header="loop_aware_mapspace_seeds (P21: outer K loop + region split + KV state; "
        "structural search-space seed, no perf claim)",
    ).write(out)
    yaml_artifact(
        "operator_geometry.yaml",
        OG.to_yaml_obj(geom_by_workload, conv_visible),
        header="operator_geometry (structural; no speedup)",
    ).write(out)
    Artifact("shape_summary_by_workload.csv", OG.shape_summary_by_workload_csv(geom_by_workload)).write(out)
    Artifact("shape_summary_by_region.csv", OG.shape_summary_by_region_csv(geom_by_workload)).write(out)
    Artifact("operator_cluster_table.csv", OG.operator_cluster_csv(all_shapes)).write(out)
    Artifact("operator_geometry_report.md", OG.report_md(geom_by_workload, all_shapes)).write(out)
    ...
    Artifact("primitive_coverage_matrix.csv", PC.primitive_coverage_matrix_csv(per_wl)).write(out)
    Artifact("primitive_coverage_report.md", PC.coverage_report_md(per_wl, regret)).write(out)
    Artifact("primitive_regret_table.csv", PC.primitive_regret_csv(regret)).write(out)
    Artifact("cross_workload_coverage_report.md", PC.cross_workload_report_md(regret, per_wl)).write(out)
    # P6 multi-rate workload contract graph (the central IR for later phases; structural only)
    yaml_artifact(
        "workload_contract_graph.yaml",
        CGRAPH.to_yaml_obj(graphs),
        header="workload_contract_graph (multi-rate; no speedup)",
    ).write(out)
    ...
    yaml_artifact(
        ...
        CGRAPH.multi_rate_contract_yaml(graphs),
        header="multi_rate_contract (rate model + phases)",
    ).write(out)
    ...
    yaml_artifact(
        ...
        header="parallel_region_candidates (structural; no speedup)",
    ).write(out)
    ...
    yaml_artifact(
        ...
        header="sharding_opportunities (structural; no speedup)",
    ).write(out)
    ...
    Artifact("operator_cluster_to_hierarchy.csv", RH.cluster_to_hierarchy_csv(clusters)).write(out)
    ...
    yaml_artifact(
        ...
        header="parallel_hierarchy_hints (structural; no speedup)",
    ).write(out)
    ...
    yaml_artifact(
        ...
        header="processing_unit_candidates (structural; no speedup)",
    ).write(out)
    Artifact(
        "processing_unit_parallelism_report.md", RH.processing_unit_report_md(units, pressure, clusters, dags)
    ).write(out)
    ...
    phase_by_wl = {w: PE.phase_model(graph_by_wl[w], geom_by_workload[w]) for w in geom_by_workload}
    overlap_by_wl = {
        w: PE.overlap_candidates(graph_by_wl[w], phase_by_wl[w], has_control_loop=_has_control_loop(w))
        for w in phase_by_wl
    }
    yaml_artifact(
        "pipeline_envelope.yaml",
        PE.pipeline_envelope_yaml(phase_by_wl),
        header="pipeline_envelope (multi-rate phase model; no speedup)",
    ).write(out)
    Artifact("pipeline_stage_table.csv", PE.pipeline_stage_csv(phase_by_wl)).write(out)
    yaml_artifact(
        "pipeline_candidates.yaml",
        PE.pipeline_candidates_yaml(overlap_by_wl),
        header="pipeline_candidates (structural overlap; no speedup)",
    ).write(out)
    Artifact("buffering_requirement_table.csv", PE.buffering_requirement_csv(overlap_by_wl)).write(out)
    Artifact("overlap_opportunities.md", PE.overlap_report_md(phase_by_wl, overlap_by_wl)).write(out)
    pug = PUG.guidance(all_shapes, dags, pressure, all_axes)
    yaml_artifact(
        "processing_unit_guidance.yaml",
        PUG.guidance_yaml(pug),
        header="processing_unit_guidance (evidence only; no selection)",
    ).write(out)
    ...
    region_mem_by_wl = {c.workload: ME.region_memory(c.attribution, geom_by_workload[c.workload]) for c in cases}
    ...
    yaml_artifact(
        ...
        header="memory_hierarchy_envelope (structural; no bandwidth)",
    ).write(out)
    ...
    yaml_artifact(
        ...
        header="memory_abstraction_candidates (structural)",
    ).write(out)
    Artifact("memory_envelope_report.md", ME.memory_envelope_report_md(region_mem_by_wl)).write(out)
    ...
    Artifact("dma_pressure_report.md", DMA.dma_pressure_report_md(stream_by_wl, region_mem_by_wl)).write(out)
    ...
    Artifact("accumulator_contract_table.csv", FE.accumulator_contract_csv(acc_by_wl)).write(out)
    yaml_artifact(
        "numerical_epilogue_candidates.yaml",
        FE.epilogue_candidates_yaml(cert_by_wl),
        header="numerical_epilogue_candidates (structural; no low-bit perf)",
    ).write(out)
    Artifact("lost_numerical_contracts.csv", FE.lost_contracts_csv(acc_by_wl)).write(out)
    Artifact("fusion_opportunity_report.md", FE.fusion_report_md(pat_by_wl, acc_by_wl, cert_by_wl)).write(out)
    ...
    for c in cases:
        ...
        ev_ctx[c.workload] = {
            ...
            "k_loop": c.K > 1,
            "control_loop": _has_control_loop(c.workload),
    ...
    Artifact("hw_sw_boundary_matrix.csv", BP.hw_sw_boundary_matrix_csv(boundary_certs)).write(out)
    yaml_artifact(
        "boundary_candidate_contracts.yaml",
        BP.boundary_candidate_contracts_yaml(boundary_certs),
        header="boundary_candidate_contracts (search space; no speedup)",
    ).write(out)
    Artifact("boundary_placement_report.md", BP.boundary_report_md(boundary_certs, resp_rows)).write(out)
    Artifact("responsibility_split_matrix.csv", BP.responsibility_split_csv(resp_rows)).write(out)
    Artifact("interface_contract_sketches.md", BP.interface_contract_sketches_md(boundary_certs)).write(out)
    yaml_artifact(
        "isa_candidate_primitives.yaml",
        BP.isa_candidate_primitives_yaml(boundary_certs),
        header="isa_candidate_primitives (sketch; no speedup)",
    ).write(out)
    yaml_artifact(
        "runtime_object_candidates.yaml",
        BP.runtime_object_candidates_yaml(boundary_certs),
        header="runtime_object_candidates (sketch; no speedup)",
    ).write(out)
    yaml_artifact(
        "command_isa_candidates.yaml",
        BP.command_isa_candidates_yaml(boundary_certs),
        header="command_isa_candidates (sketch; no speedup)",
    ).write(out)
    yaml_artifact(
        "boundary_dse_knobs.yaml",
        BP.boundary_dse_knobs_yaml(boundary_certs),
        header="boundary_dse_knobs (search-space knobs; no speedup)",
    ).write(out)
    # Consolidated DSE search-space knobs across P5-P12 (the bridge a DSE engine consumes)
    knob_catalog = _dse_search_space_knobs(
        all_shapes, all_axes, dags, units, overlap_by_wl, pat_by_wl, region_mem_by_wl, boundary_certs
    )
    yaml_artifact(
        "dse_search_space_knobs.yaml", knob_catalog, header="dse_search_space_knobs (consolidated P5-P10; no speedup)"
    ).write(out)
    ...
    Artifact("dse_contract.json", _json.dumps(manifest, indent=2, sort_keys=True) + "\n").write(out)
