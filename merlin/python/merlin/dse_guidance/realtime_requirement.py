...
_VLA_HZ = [("VLA 30Hz (real-time baseline)", 30.0), ("VLA 50Hz (high-perf)", 50.0), ("VLA 100Hz (dexterous)", 100.0)]
...
_RT_COLS = [
    ...
    "window",
    "required_GMAC_per_s",
    "required_weight_GBps_resident",
    "required_weight_GBps_reload",
    "amortization",
    "evidence",
]
                ...
                rows.append(
                    {
                        ...
                        "family": arch.family,
                        "regime": regime,
                        "budget_ms": round(window_s * 1e3, 3),
                        "window": f"replan (H={H} actions / {hz:.0f}Hz)",
                ...
                rows.append(
                    {
                        ...
                        "regime": regime,
                        "budget_ms": round(window_s * 1e3, 3),
                        "window": f"per token ({tok:.0f} tok/s)",
            ...
            rows.append(
                {
                    "workload": w,
                    "family": arch.family,
                    "regime": "VLM TTFT < 500ms (prefill)",
                    "budget_ms": _TTFT_MS,
                    "window": "prefill (once-prefix)",
