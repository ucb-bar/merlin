            ...
            rows.append(
                {
                    ...
                    "op_count": len(ss),
                    "macs_total": macs,
                    "mac_fraction": round(macs / total_macs, 4),
                }
            )
            ...
            rows.append(
                {
                    ...
                    "shape_class": cls,
                    "op_count": len(ss),
                    "macs_total": sum(s.macs for s in ss),
                }
            )
...
def operator_cluster_csv(all_shapes: list[OperatorShape]) -> str:
    ...
    return _csv(rows, ["shape_class", "op_count", "macs_total", "mac_fraction", "workloads", "semantic_roles"])
                ...
                "operators": [
                    {
                        ...
                        "is_tail_heavy": s.is_tail_heavy,
                        "is_small_fragment": s.is_small_fragment,
                        "evidence": E_IR,
                    }
                    for s in shapes
                ],
...
def report_md(by_workload: dict[str, list[OperatorShape]], all_shapes: list[OperatorShape]) -> str:
    ...
    L.append(f"Operators extracted: **{len(all_shapes)}** across **{len(by_workload)}** workloads.\n")
    ...
    if tail:
        ex = tail[0]
        L.append(
            f"- **Tail-heavy operators:** {len(tail)} op(s) waste >10% against a 32×32 tile "
    ...
    if frag:
        ex = frag[0]
        L.append(
            f"- **Small dispatch fragments:** {len(frag)} op(s) below "
    ...
    L.append(
        "- **Attention structure** (heads / head_dim / kv_len / mask): `unavailable` — "
