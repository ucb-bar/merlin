    ...
    for c in structural_candidates:
        ...
        out.append(
            AbstractionCandidate(
                axis=c.axis,
                system_abstraction=m["system_abstraction"],
                why_this_exists={
                ...
                compiler_proof=c.required_compiler_proof,
                runtime_hw_support=c.required_hw_support,
                dse_knobs_exposed=m["dse_knobs"],
                measurements_needed=list(c.required_measurements),
        ...
        for k in numerical.candidates:
            ...
            out.append(
                AbstractionCandidate(
                    ...
                    runtime_hw_support=k.required_hw_support,
                    dse_knobs_exposed=m["dse_knobs"],
                    measurements_needed=(
                        list(k.required_accuracy_measurements) + list(k.required_performance_measurements)
                    ),
...
def dse_readiness(
    topo, attribution, numerical, cpu_coupling_available: bool, accuracy_status: str = "unavailable"
) -> DseReadiness:
    ...
    fields = {
        ...
        "role_attribution": {"available": has_role, "source": head.source if has_role else "unavailable"},
        "K_source": {"available": True, "source": "assumed_reference"},
        "deadline_source": {"available": topo.replan_deadline_ms is not None, "source": "assumed_reference"},
        "dtype_contract": {"available": has_dtype, "source": "recovered_from_ir"},
        "state_lifetimes": {"available": bool(topo.loop_invariant_state()), "source": "recovered_from_prov_fqn"},
        "dispatch_graph": {"available": has_role, "source": "recovered_from_ir"},
        "accuracy_constraints": {
            "available": acc_available,
            "source": "measured" if acc_available else "unavailable",
            "int8_status": accuracy_status,
        },
        "cpu_coupling": {
            "available": cpu_coupling_available,
...
_TARGET_HINTS = ("bandwidth", "cycles", "latency", "capacity", "memory", "kernel calibration", "matmul cycles")
...
def measurement_plan(candidates: list[AbstractionCandidate]) -> dict:
    buckets: dict[str, list[str]] = {"accuracy_measurable_now": [], "proxy_measured": [], "target_measured": []}
    seen: set[str] = set()
    for c in candidates:
...
def abstraction_yaml(candidates: list[AbstractionCandidate]) -> dict:
    return {
        "abstraction_candidates": [
            {
                "axis": c.axis,
                "system_abstraction_needed": c.system_abstraction,
                "why_this_exists": c.why_this_exists,
                "compiler_proof": c.compiler_proof,
                "runtime_hw_support": c.runtime_hw_support,
                "dse_knobs_exposed": c.dse_knobs_exposed,
                "measurements_needed": c.measurements_needed,
                "what_is_not_claimed": c.what_is_not_claimed,
                "status": {"legality": c.legality, "quantification_blocked_by": c.quantification_blocked_by},
            }
            for c in candidates
        ]
    }
...
def readiness_yaml(r: DseReadiness) -> dict:
    return {
        "dse_readiness": {
            "workload": r.workload,
            "ready_to_rank_designs": r.ready,
            "fields": r.fields,
            "missing_before_ranking": r.missing,
        }
    }


def workload_contract_report_md(
    topo, attribution, numerical, envelope, cands: list[AbstractionCandidate], readiness: DseReadiness, plan: dict
) -> str:
    ...
    L.append(
        "> Merlin recovers the HW/SW contract a flat capture erases and hands a future DSE "
    ...
    L.append(f"- class: **{topo.workload_class}**  ·  K={topo.K}, H={topo.H}, control_rate={topo.control_rate_hz} Hz\n")

    L.append("## 1. Recovered structure\n")
    if head and head.attribution_status == "attributed":
        f = head.facts
        L.append(
            f"- repeated head ({head.source}): {f['matmul_count']} matmuls, "
    ...
    if numerical is not None:
        ...
        L.append(
            f"- storage **{numerical.weight_storage_dtype}**, compute "
    ...
    if envelope is not None:
        ...
        for name in (
            "macs_per_replan",
            "resident_capacity_required",
            "avoidable_weight_reload_bytes",
            "required_compute_rate",
            "required_weight_bandwidth",
            "required_command_rate",
        ):
            r = envelope.req(name)
            if r and r.value is not None:
                L.append(f"- {name} = {r.value:.3e} {r.unit} ({r.evidence})")
        L.append(
            "- resident capacity by format: "
            + ", ".join(f"{k}={v / 1e6:.0f}MB" for k, v in envelope.capacity_by_dtype_B.items())
        )
        L.append("")
    ...
    for c in cands:
        L.append(
            f"| {c.system_abstraction} | {', '.join(c.dse_knobs_exposed) or '—'} | {c.quantification_blocked_by} |"
        )
    ...
    L.append(f"- measurable now (runtime proxy): {', '.join(plan['measurable_now']['runtime_proxy']) or '—'}")
