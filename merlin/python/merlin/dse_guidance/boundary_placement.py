...
_LEVEL_ROLE = {
    L_COMPILER: dict(
        sw="compiler rewrites the workload; hardware sees ordinary ops",
        hw="none (generic ops)",
        runtime="none",
        isa="none",
    ...
    L_RUNTIME: dict(
        sw="runtime tracks the object's lifetime + layout",
        hw="exploits the declared lifetime / layout",
        runtime="object ABI + lifetime tracking + metadata",
        isa="none",
        hw_support="addressable resident store",
    ),
    L_COMMAND: dict(
        sw="submits higher-level commands instead of ops",
        hw="command processor executes / loops / tracks deps",
        runtime="command encoder + event/queue",
        isa="command opcodes + handles",
        hw_support="device-side command processor",
    ),
    L_ISA: dict(
        sw="compiler targets the semantic ISA op",
        hw="datapath executes the semantic instruction",
        runtime="dispatch only",
        isa="the semantic instruction + operands",
        hw_support="datapath implementing the op",
    ),
    L_MICROCODE: dict(
        sw="submits a bounded region; device iterates",
        hw="controller owns the loop / deps / prefetch / state",
        runtime="bounded-region handoff",
        isa="region / loop descriptor",
        hw_support="sequencer + state + dependency tracking",
    ),
    L_DATAPATH: dict(
        sw="none (absorbed into the unit)",
        hw="hardwired unit",
        runtime="none",
        isa="implicit",
        hw_support="dedicated fixed unit",
    ),
}
...
_ERASED_LEVELS = {
    L_COMPILER: S_POSSIBLE,
    L_RUNTIME: S_BLOCKED,
    L_COMMAND: S_BLOCKED,
    L_ISA: S_BLOCKED,
    L_MICROCODE: S_BLOCKED,
    L_DATAPATH: S_BLOCKED,
}
...
_KV_LEVELS = {
    ...
    L_COMMAND: S_UNAVAIL,
    L_ISA: S_UNAVAIL,
    L_MICROCODE: S_UNAVAIL,
    L_DATAPATH: S_UNAVAIL,
}


def _lv(ct, rh, cmd, isa, mc, fx) -> dict:
    return {L_COMPILER: ct, L_RUNTIME: rh, L_COMMAND: cmd, L_ISA: isa, L_MICROCODE: mc, L_DATAPATH: fx}
...
_BOUNDARY_CATALOG: dict[str, dict] = {
    "resident_weight_object": dict(
        ...
        region_roles=["repeated_head", "backbone_once"],
        support="k_loop",
        cp_axis="resident_action_head_weights",
        erased=False,
        metadata="dtype, shape, layout, size_bytes, lifetime",
        knobs=[
            ("resident_capacity", "weight bytes resident across the K-loop"),
    ...
    "resident_packed_weight_object": dict(
        ...
        region_roles=["repeated_head"],
        support="lowbit",
        cp_axis="resident_packed_lowbit_weights",
        erased=True,
        metadata="packed dtype, pack_format, scale_object_handle, lifetime",
        knobs=[("pack_format", "sub-byte packing layout"), ("resident_capacity", "packed resident bytes")],
        risk="packed residency overfits a weight layout; needs a low-bit capture",
        levels=None,
    ),
    "packed_lowbit_tensor": dict(
        sources=["numerical_contract", "dtype_capacity_table"],
        region_roles=["repeated_head"],
        support="lowbit",
        cp_axis="resident_packed_lowbit_weights",
        erased=True,
        metadata="storage dtype, pack_format, group_size, scale_object_handle",
        knobs=[("weight_format", "int4/fp8/... packed format"), ("group_size", "quant group granularity")],
        risk="format / layout overfit; capture is dequantized f32",
        levels=None,
    ),
    "scale_object": dict(
        sources=["numerical_contract"],
        region_roles=["repeated_head"],
        support="lowbit",
        cp_axis="resident_packed_lowbit_weights",
        erased=True,
        metadata="scale dtype, zero_point, granularity (per-tensor/-channel/-group)",
        knobs=[("scale_granularity", "per-tensor / channel / group"), ("zero_point", "asymmetric vs symmetric")],
        risk="scales folded away in the capture; needs the source quant metadata",
        levels=None,
    ),
    "prefix_kv_object": dict(
        sources=["workload_contract_graph", "state_lifetime"],
        region_roles=["prefix_builder"],
        support="decode",
        cp_axis="decode_kv_cache_path",
        erased=False,
        kv=True,
        metadata="kv dtype, heads, head_dim, kv_len (unavailable — attention lowered)",
        knobs=[("kv_capacity", "resident KV bytes")],
        risk="attention/KV lowered into matmul projections — structure not recovered",
        levels=None,
    ),
    "loop_carried_state_handle": dict(
        ...
        region_roles=["repeated_head"],
        support="k_loop",
        cp_axis="autonomous_K_loop",
        erased=False,
        metadata="state dtype, shape, update site, lifetime=across_K",
        knobs=[("state_capacity", "carried-state bytes"), ("update_in_loop", "in-loop vs host update")],
    ...
    "region_level_dispatch": dict(
        sources=["command_graph", "pipeline_envelope"],
        region_roles=["repeated_head", "backbone_once"],
        support="all",
        cp_axis="command_batching",
        erased=False,
    ...
    "event_token": dict(
        sources=["pipeline_envelope", "command_graph"],
        region_roles=["repeated_head", "backbone_once"],
        support="all",
        cp_axis="async_chunk_overlap",
        erased=False,
    ...
    "async_queue": dict(
        sources=["pipeline_envelope", "dma_stream_table"],
        region_roles=["repeated_head", "backbone_once"],
        support="all",
        cp_axis="async_chunk_overlap",
        erased=False,
    ...
    "producer_consumer_queue": dict(
        sources=["pipeline_envelope"],
        region_roles=["repeated_head", "backbone_once"],
        support="control_loop",
        cp_axis="async_chunk_overlap",
        erased=False,
    ...
    "double_buffered_action_chunk": dict(
        sources=["pipeline_envelope", "phase_rate_table"],
        region_roles=["repeated_head"],
        support="control_loop",
        cp_axis="async_chunk_overlap",
        erased=False,
    ...
    "matrix_engine": dict(
        ...
        cp_axis=None,
        erased=False,
        metadata="tile shape, accumulator dtype",
        knobs=[("tile_M", "output tile rows"), ("tile_N", "output tile cols"), ("pe_array_dims", "PE grid")],
    ...
    "epilogue_requant_unit": dict(
        sources=["fusion_epilogue", "resource_pressure_table"],
        region_roles=["repeated_head", "backbone_once"],
        support="epilogue",
        cp_axis="fused_requant_epilogue",
        erased=False,
    ...
    "dma_engine": dict(
        sources=["memory_hierarchy_envelope", "dma_stream_table"],
        region_roles=["repeated_head", "backbone_once"],
        support="all",
        cp_axis=None,
        erased=False,
    ...
    "multi_stream_dma_descriptor": dict(
        sources=["dma_stream_table"],
        region_roles=["repeated_head"],
        support="all",
        cp_axis=None,
        erased=False,
        metadata="per-stream descriptor (weight/activation/output)",
    ...
    "prefetch_descriptor": dict(
        sources=["dma_stream_table", "memory_hierarchy_envelope"],
        region_roles=["repeated_head"],
        support="all",
        cp_axis=None,
        erased=False,
        metadata="prefetch target, depth, trigger",
    ...
    "partial_sum_object": dict(
        sources=["sharding_opportunities"],
        region_roles=["repeated_head", "backbone_once"],
        support="all",
        cp_axis=None,
        erased=False,
    ...
    "fused_dequant_matmul": dict(
        sources=["numerical_contract", "fusion_epilogue"],
        region_roles=["repeated_head"],
        support="lowbit",
        cp_axis="resident_packed_lowbit_weights",
        erased=True,
        ...
        risk="needs a low-bit capture; the f32 capture only shows compiler-dequant",
        levels=None,
    ),
    "fused_requant_epilogue": dict(
        ...
        metadata="epilogue op set, requant scale, output dtype",
        knobs=[
            ("requant_in_epilogue", "requant onto the matmul output"),
    ...
    "native_lowbit_matmul": dict(
        ...
        risk="accuracy + format overfit; needs a low-bit capture + accuracy gate",
        levels=None,
    ),
    "kv_cache_object": dict(
        sources=["workload_contract_graph"],
        region_roles=["prefix_builder"],
        support="decode",
        cp_axis="decode_kv_cache_path",
        erased=False,
        ...
        knobs=[("kv_capacity", "resident KV bytes")],
        risk="attention/KV not recovered from the flat capture",
        levels=None,
    ),
    "decode_loop_controller": dict(
        ...
        cp_axis="decode_kv_cache_path",
        erased=False,
        metadata="token trip count, kv handle, loop-carried token state",
    ...
    return [
        {
            ...
            "erased": bool(spec.get("erased", False)),
            "kv": bool(spec.get("kv", False)),
...
class BoundaryCertificate:
    ...
    what_is_not_claimed: str = (
        "no speedup, cycles, area, or energy claim; a boundary search-space candidate, not a chosen design"
    )
...
def _missing_for(status, spec) -> str:
    if status == S_BLOCKED:
        return (
            "low-bit capture (packed weights + scales) + per-format accuracy"
            if spec.get("erased")
            else "accuracy / numerical measurement"
        )
    if status == S_UNAVAIL:
        return (
            "attention/KV structure (a loop-preserving capture)"
            if spec.get("kv")
...
def build_certificate(name, spec, evidence_ctx, cp_proofs) -> BoundaryCertificate:
    ...
    for lv in LEVELS:
        ...
        per_proof = "none (generic ops)" if lv == L_COMPILER else proof_str if st not in (S_NA, S_UNAVAIL) else "n/a"
        risk = spec["risk"]
        ...
        blevels.append(
            {
                "level": lv,
                "status": st,
                "software_manages": role["sw"],
                "hardware_manages": role["hw"],
                ...
                "risk": risk,
                "missing_evidence": _missing_for(st, spec),
            }
        )
    ...
    comp = {
        ...
        "crosses_rate_boundary": int(
            "backbone_once" in spec["region_roles"] or spec["support"] in ("control_loop", "decode")
        ),
    ...
    knobs = [
        {"knob": k, "reason": r, "evidence": ("recovered_from_ir" if not erased else "unavailable")}
        for k, r in spec["knobs"]
    ]
    return BoundaryCertificate(
        abstraction=name,
        source_analyses=spec["sources"],
        supporting_workloads=wls,
        region_roles=spec["region_roles"],
        evidence_summary=ev,
        cp_matrix_axis=cp_axis,
        required_compiler_proof=proof_str,
        compiler_proof_status=proof_status,
        dse_knobs=knobs,
        boundary_levels=blevels,
        boundary_pressure_score=score,
        pressure_components=comp,
        erased=erased,
    )


def build_certificates(evidence_ctx, cp_proofs) -> list[BoundaryCertificate]:
    return [build_certificate(n, s, evidence_ctx, cp_proofs) for n, s in _BOUNDARY_CATALOG.items()]
...
_RESP_FUNCTIONS = [
    ...
    "dtype_selection",
    "weight_packing",
    "scale_metadata_management",
    "resident_object_lifetime",
    "K_loop_iteration",
    "loop_carried_state_update",
    "command_dependency_tracking",
    ...
    "safety_action_commit",
]
_RESP_COLUMNS = ["compiler", "runtime_hal", "command_processor", "accelerator_isa", "device_microcode", "datapath"]
...
def hw_sw_boundary_matrix_csv(certs) -> str:
    ...
    return _csv(rows, ["abstraction"] + LEVELS + ["supporting_workloads", "boundary_pressure_score"])
            ...
            "certificates": [
                {
                    ...
                    "source_analyses": c.source_analyses,
                    "supporting_workloads": c.supporting_workloads,
                    "region_roles": c.region_roles,
                    "evidence_summary": c.evidence_summary,
                    "compiler_proof_matrix_axis": c.cp_matrix_axis,
...
def runtime_object_candidates_yaml(certs) -> dict:
    ...
    return {
        "runtime_object_candidates": {
            ...
            "candidates": [
                {
                    "object": c.abstraction,
                    "status": _lvl_status(c, L_RUNTIME),
                    "metadata": next(b["metadata_crossing"] for b in c.boundary_levels if b["level"] == L_RUNTIME),
                    "supporting_workloads": c.supporting_workloads,
...
def command_isa_candidates_yaml(certs) -> dict:
    ...
    return {
        "command_isa_candidates": {
            ...
            "candidates": [
                {
                    "command": c.abstraction,
                    "status": _lvl_status(c, L_COMMAND),
                    "supporting_workloads": c.supporting_workloads,
...
def isa_candidate_primitives_yaml(certs) -> dict:
    ...
    return {
        "isa_candidate_primitives": {
            ...
            "primitives": [
                {
                    "primitive": c.abstraction,
                    "status": _lvl_status(c, L_ISA),
                    "supporting_workloads": c.supporting_workloads,
                    "required_compiler_proof": c.required_compiler_proof,
...
def boundary_dse_knobs_yaml(certs) -> dict:
    ...
    for c in certs:
        strongest = max(
            c.boundary_levels,
            key=lambda b: {"strong_candidate": 3, "possible": 2, "weak_candidate": 1}.get(b["status"], 0),
        )
            ...
            knobs.append(
                {
                    "knob": k["knob"],
                    "abstraction": c.abstraction,
                    "boundary_level": strongest["level"],
                    "reason": k["reason"],
                    "evidence": k["evidence"],
                }
            )
...
def interface_contract_sketches_md(certs) -> str:
    ...
    L.append("- fields: `dtype`, `shape`, `layout`, `lifetime`, `size_bytes`, `scale_object_handle` (if quantized)")
    L.append("- operations: `load`, `pin`, `reuse`, `evict`")
    L.append(
        "- evidence: weights are loop-invariant across the K-loop "
        "(`resident_action_head_weights` = proven_for_workload)\n"
    )
    L.append("## Command ISA sketch — `bounded_loop_command`\n")
    L.append(
        "- fields: `trip_count`, `body_region_handle`, `loop_carried_state_handles`, "
        "`invariant_state_handles`, `event_in`, `event_out`"
    )
    L.append(
        "- evidence: the repeated head is a bounded K-loop with loop-invariant weights "
        "(`autonomous_K_loop` = assumed)\n"
    )
    L.append("## Accelerator ISA primitive sketch — `matmul_packed_lowbit`\n")
    L.append(
        "- fields: `lhs_dtype`, `rhs_storage_dtype`, `accumulator_dtype`, `scale_object`, "
        "`output_dtype`, `tile_shape`, `epilogue_mode`"
    )
    L.append(
        "- evidence: **blocked** — the capture is dequantized f32; this primitive needs a "
    ...
    L.append(
        "- evidence: GEMV/skinny shapes dominate the decode workloads (P5 geometry); a square "
        "matrix engine covers them poorly (P5 regret)\n"
    )
    L.append(
        "These sketches correspond to the `runtime_object_candidates.yaml`, "
...
def boundary_report_md(certs, resp_rows) -> str:
    ...
    for c in certs:
        ...
        if (
            sts[L_COMPILER] in cand
            and sts[L_RUNTIME] in cand
            and sts[L_COMMAND] in cand
    ...
    for c in strong[:10]:
        ...
        L.append(f"| {c.abstraction} | {tops} | {', '.join(c.supporting_workloads)} | {c.boundary_pressure_score} |")
    L.append("")
    ...
    L.append(
        "- `bounded_loop_command` / `region_level_dispatch`: a pure host loop submits a "
    ...
    L.append("- `multi_stream_dma_descriptor`: software-issued per-tile DMA explodes; a descriptor engine batches it.")
    L.append("")
    L.append("## Hardware-only management may hide semantics the compiler knows\n")
    L.append(
        "- `resident_weight_object`: a hardware cache rediscovers reuse the compiler already "
        "proved (loop-invariant weights) — a `resident_weight_object` keeps the lifetime "
        "explicit."
    )
    L.append(
        "- `partial_sum_object` / `accumulator_merge`: hardware-internal K-sharding hides the "
        "reduction; a command/ISA-level shard keeps it visible to the compiler."
    )
    L.append("")
    L.append("## ISA / HAL objects the evidence suggests\n")
    L.append(
        "- runtime objects: see `runtime_object_candidates.yaml`; command ops: "
    ...
    L.append(
        f"- {', '.join('`' + a + '`' for a in blocked)} — packed low-bit / scale / KV structure "
    ...
    L.append(
        "- per-unit throughput / latency / area / energy (a design YAML), per-format low-bit "
