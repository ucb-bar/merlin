...
def _deadline_feasibility(multirate: Representation, baseline: BaselineCost) -> dict:
    ...
    if multirate.replan_deadline_ms is not None and baseline.unit == "ms":
        ...
        out["note"] = "deadline feasibility not evaluated (baseline unit is not ms or no deadline available)"
    return out


def run_guidance(
    temporal: TemporalMetadata,
    baseline: BaselineCost | None = None,
    ...
    if baseline is not None:
        ...
        if coupling is not None:
            coupling_per_replan = coupling.per_replan(
                dispatches_per_step=multirate.dispatches_per_step,
                K=multirate.K,
    ...
    return GuidanceResult(
        ...
        triage_flat=triage_flat,
        triage_multirate=triage_multirate,
        baseline=baseline,
        coupling=coupling,
        coupling_per_replan=coupling_per_replan,
...
def write_artifacts(result: GuidanceResult, out_dir: str | Path) -> list[Path]:
    ...
    artifacts: list[Artifact] = [
        yaml_artifact(
            ...
            TOP.to_report_dict(result.topology),
        ...
        yaml_artifact(
            "dse_candidate_axes.yaml",
            CAND.to_yaml_obj(result.candidate_axes),
            header=f"dse_candidate_axes: {result.workload} (structural; no calibration)",
        ),
        yaml_artifact("flat_report.yaml", to_report_dict(result.flat), header=f"flat_report: {result.workload}"),
        yaml_artifact(
            "multirate_report.yaml", to_report_dict(result.multirate), header=f"multirate_report: {result.workload}"
        ),
        Artifact("flat_vs_multirate_diff.csv", report.flat_vs_multirate_csv(result.flat, result.multirate)),
    ]
    ...
    if result.baseline is not None:
        artifacts += [
            ...
            Artifact("axis_triage.md", report.triage_md(result.triage_multirate, result.triage_flat, result.baseline)),
            Artifact("bottleneck_breakdown.csv", report.bottleneck_breakdown_csv(result.baseline)),
            yaml_artifact(
                "deadline_feasibility.yaml", result.deadline, header=f"deadline_feasibility: {result.workload}"
            ),
        ]
        if result.coupling_per_replan is not None:
            artifacts.append(
                yaml_artifact(
                    "cpu_coupling_result.yaml",
        ...
        if result.calibration_rows:
            artifacts.append(Artifact("calibration_anchor.csv", calibration.anchor_csv(result.calibration_rows)))
        if result.is_negative_control:
            artifacts.append(
                Artifact(
                    "negative_control_report.md", report.negative_control_md(result.triage_multirate, result.baseline)
                )
            )
...
def _maybe_figures(result: GuidanceResult, fig_dir: Path) -> list[Path]:
    ...
    if plots.flat_vs_multirate_plot(result.flat, result.multirate, fig_dir / "flat_vs_multirate.png"):
