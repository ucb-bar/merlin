...
def guidance(all_shapes, dags, pressure, all_axes) -> dict:
    ...
    monolithic = MultiplicityOption(
        ...
        evidence_for=[
            ...
            (
                ...
                if dense and dense.mac_fraction > 0.5
                else "some workloads are dominated by one op"
            ),
        ],
        evidence_against=[
    ...
    replicated = MultiplicityOption(
        ...
        evidence_for=[
            (f"{n_par} workload(s) expose some inter-op parallelism" if n_par else "large ops shard cleanly along M/N"),
    ...
    specialized = MultiplicityOption(
        ...
        evidence_for=[
            (
                ...
                f"skinny/GEMV {skinny.mac_fraction:.0%}"
                if dense and skinny
        ...
        candidate_units=[
            ...
            {"unit": "skinny_gemm_or_gemv_engine", "for": "skinny_gemm_or_gemv", "evidence": "recovered_from_ir"},
            {"unit": "epilogue_requant_unit", "for": "epilogue_or_requant", "evidence": "recovered_from_ir"},
            {"unit": "dma_engine", "for": "dma_or_memory", "evidence": "recovered_from_prov_fqn"},
            {"unit": "loop_controller", "for": "control_or_dispatch", "evidence": "recovered_from_prov_fqn"},
            {"unit": "scalar_control_unit", "for": "control_or_dispatch", "evidence": "recovered_from_prov_fqn"},
            {"unit": "kv_cache_unit", "for": "attention_softmax_or_reduction", "evidence": "unavailable"},
        ],
    )
...
def guidance_yaml(g: dict) -> dict:
    return {
        "processing_unit_guidance": {
            ...
            "avg_inter_op_parallelism": {
                "value": g["avg_inter_op_parallelism"],
                "evidence": E_DERIVED,
    ...
    for opt in ("one_bigger_unit", "multiple_identical_units", "multiple_specialized_units"):
        ...
        if o.evidence_against:
            ...
            for e in o.evidence_against:
                L.append(f"- {e}")
        if o.candidate_units:
            L.append("\n**Candidate units:** " + ", ".join(f"`{u['unit']}`" for u in o.candidate_units))
        L.append("")
