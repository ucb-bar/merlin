...
if HAS_XDSL:
    ...
    from xdsl.ir import Dialect, EnumAttribute, ParametrizedAttribute, SpacedOpaqueSyntaxAttribute, TypeAttribute
        ...
        def verify_(self) -> None:
            for d in self.shape:
                if not isinstance(d, IntegerAttr) or d.value.data <= 0:
                    raise VerifyException("runtime.buffer_view.create shape dims must be positive ints")
    ...
    class CommandBufferCreateOp(IRDLOperation):
        ...
        def verify_(self) -> None:
            if not self.target.data:
                raise VerifyException("runtime.command_buffer.create needs the target name")
        ...
        def verify_(self) -> None:
            ...
            for key, val in self.args.data.items():
                if not isinstance(val, StringAttr) or not val.data:
                    raise VerifyException("runtime.command_buffer.append arg %r must name a tensor" % key)
    ...
    class MetricsReadOp(IRDLOperation):
        ...
        def verify_(self) -> None:
            for entry in self.metrics:
                ...
                if m is None or (m not in KNOWN_METRICS and not m.startswith("target_specific.")):
    ...
    _OP_CLASSES = [
        ...
        HandleCreateOp,
        HandleDestroyOp,
        CommandBufferCreateOp,
        CommandBufferAppendOp,
        SubmitOp,
        WaitOp,
        ProfileRegionOp,
        MetricsReadOp,
    ...
    def build_example():
        ...
        blk = Block()
        dev = DeviceGetOp(
            result_types=[DeviceType()],
            properties={"device": StringAttr("toy_npu0"), "backend": BackendAttr(Backend.SIMULATOR)},
        )
        buf = BufferAllocOp(
            operands=[dev.dev],
            result_types=[BufferType()],
            properties={"bytes": IntegerAttr(4096, 64), "memory": StringAttr("device")},
        )
        view = BufferViewCreateOp(
            operands=[buf.buffer],
            result_types=[BufferViewType()],
            properties={
                "shape": ArrayAttr([IntegerAttr(64, 64), IntegerAttr(64, 64)]),
                "dtype": StringAttr("i8"),
                "layout": StringAttr("row_major"),
            },
        )
        h = HandleCreateOp(
            operands=[dev.dev],
            result_types=[HandleType()],
            properties={"kind": StringAttr("resident_tensor"), "lifetime": StringAttr("region")},
        )
        cb = CommandBufferCreateOp(
            operands=[dev.dev],
            result_types=[CommandBufferType()],
            properties={"target": StringAttr("toy_npu"), "mode": SubmitModeAttr(SubmitMode.BATCHED)},
        )
        ap = CommandBufferAppendOp(
            operands=[cb.cb],
            properties={
        ...
        sub = SubmitOp(
            ...
            result_types=[EventType()],
            properties={"mode": SubmitModeAttr(SubmitMode.BLOCKING)},
        )
        wait = WaitOp(operands=[sub.event])
        prof = ProfileRegionOp(regions=[Region([Block()])], properties={"label": StringAttr("resident_matmul")})
        met = MetricsReadOp(
            operands=[dev.dev],
            result_types=[MetricsType()],
            properties={
                "metrics": ArrayAttr([StringAttr("cycles"), StringAttr("bytes_moved"), StringAttr("resident_hits")])
            },
        )
        tr = TraceEmitOp(
            properties={"event": StringAttr("resident_hit"), "attrs": DictionaryAttr({"object": StringAttr("W_res")})}
        )
