...
KNOWN_PREDICATES = {
    ...
    "capacity_fit",
    "layout_preserved",
    "runtime_persistent_handle",
    "accumulator_not_user_visible",
    "epilogue_consumes_accumulator",
    "event_token_support",
}
...
if HAS_XDSL:
            ...
            if self.candidate_features is not None:
                for entry in self.candidate_features:
                    if not isinstance(entry, StringAttr) or not entry.data:
                        raise VerifyException("contract.region candidate_features must be non-empty strings")
        ...
        def verify_(self) -> None:
            if self.kind.data not in ASSUMPTION_KINDS:
                raise VerifyException(
                    "contract.assume kind %r not registered (known: %s)" % (self.kind.data, sorted(ASSUMPTION_KINDS))
                )
        ...
        def verify_(self) -> None:
            ...
            for entry in self.features:
                ...
                if not f or not f.replace("_", "").isalnum():
                    raise VerifyException("contract.capability feature %r is not a valid identifier" % (f,))
    ...
    _ATTR_CLASSES = [FeatureType, ProofType, CapabilityType, LifetimeAttr, MemoryRoleAttr, LayoutRoleAttr]
    ...
    def build_example():
        ...
        cap = CapabilityOp(
            ...
            properties={
                ...
                "features": ArrayAttr([StringAttr("resident_packed_tensor"), StringAttr("accumulator_commit")]),
                "runtime": ArrayAttr([StringAttr("command_buffer"), StringAttr("metrics")]),
            },
        )
        assume = AssumeOp(
            operands=[w], properties={"kind": StringAttr("immutable"), "lifetime": LifetimeAttr(Lifetime.WITHIN_REGION)}
        )
        fact = FactOp(
            operands=[w],
            properties={
        ...
        check = CheckOp(operands=[w, [prove.proof]], properties={"requirement": StringAttr("rhs_immutable")})
        inner = Block()
        inner.add_ops([])
        region = RegionOp(
