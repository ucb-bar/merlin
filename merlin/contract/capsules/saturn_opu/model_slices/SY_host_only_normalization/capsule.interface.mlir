builtin.module attributes {prov.level = "linalg-on-tensors"} {
  func.func @forward(%0: tensor<16x32xf32>, %1: tensor<32xf32>, %2: tensor<32xf32>) -> tensor<16x32xf32> {
    ...
    %5 = linalg.reduce ins(%0:tensor<16x32xf32>) outs(%4:tensor<16xf32>) dimensions = [1]
    (%6: f32, %7: f32) {
      %8 = arith.addf %6, %7 : f32
      linalg.yield %8 : f32
    }
    %9 = arith.constant {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} 3.200000e+01 : f32
    ...
    %18 = tensor.empty() : tensor<16x32xf32>
    %19 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, 0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%0, %17 : tensor<16x32xf32>, tensor<16x1xf32>) outs(%18 : tensor<16x32xf32>) attrs =  {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} {
    ^bb1(%20: f32, %21: f32, %22: f32):
      %23 = arith.subf %20, %21 : f32
      linalg.yield %23 : f32
    } -> tensor<16x32xf32>
    %24 = tensor.empty() : tensor<16x32xf32>
    %25 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%19, %19 : tensor<16x32xf32>, tensor<16x32xf32>) outs(%24 : tensor<16x32xf32>) attrs =  {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} {
    ...
    %32 = linalg.reduce ins(%25:tensor<16x32xf32>) outs(%31:tensor<16xf32>) dimensions = [1]
    (%33: f32, %34: f32) {
      %35 = arith.addf %33, %34 : f32
      linalg.yield %35 : f32
    }
    %36 = arith.constant {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} 3.200000e+01 : f32
    ...
    %59 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, 0)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%19, %54 : tensor<16x32xf32>, tensor<16x1xf32>) outs(%58 : tensor<16x32xf32>) attrs =  {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} {
    ^bb6(%60: f32, %61: f32, %62: f32):
      %63 = arith.mulf %60, %61 : f32
      linalg.yield %63 : f32
    } -> tensor<16x32xf32>
    %64 = tensor.empty() : tensor<16x32xf32>
    %65 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%59, %1 : tensor<16x32xf32>, tensor<32xf32>) outs(%64 : tensor<16x32xf32>) attrs =  {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} {
    ^bb7(%66: f32, %67: f32, %68: f32):
      %69 = arith.mulf %66, %67 : f32
      linalg.yield %69 : f32
    } -> tensor<16x32xf32>
    %70 = tensor.empty() : tensor<16x32xf32>
    %71 = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%65, %2 : tensor<16x32xf32>, tensor<32xf32>) outs(%70 : tensor<16x32xf32>) attrs =  {prov.region_id = "layer_norm_0", prov.family = "normalization", prov._pattern_hint = "layer_norm", prov.op = "layer_norm", prov.aten = "aten.native_layer_norm.default", prov.orig_dtype = "float32"} {
    ^bb8(%72: f32, %73: f32, %74: f32):
      %75 = arith.addf %72, %73 : f32
      linalg.yield %75 : f32
    } -> tensor<16x32xf32>
    func.return %71 : tensor<16x32xf32>
  }
}
