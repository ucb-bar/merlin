...
module attributes {merlin_iface.version = "0.1", merlin_iface.target = "gemmini", merlin_iface.abi_version = "0.1"} {
  %IFM = merlin_iface.tensor {name = "IFM", role = "input"} : tensor<1x6x6x4xi8>
  ...
  %Y0 = merlin_iface.conv2d %IFM, %W_res {kernel = [3, 3, 4, 16], stride = [1, 1], padding = [1, 1, 1, 1], dilation = [1, 1], name = "Y0", epilogue = ["bias_add", "acc_scale", "relu"], output_dtype = "i8", acc_scale = 0.03125 : f32, bias = "B", layout = "nhwc"} : (tensor<1x6x6x4xi8>, !merlin_iface.resident) -> tensor<36x16xi8>
