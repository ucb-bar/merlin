module attributes {merlin_iface.version = "0.1", merlin_iface.target = "radiance", merlin_iface.abi_version = "0.1"} {
  ...
  %S = merlin_iface.attention_qk %Q, %K {name = "S", block_scale = "e8m0", output_dtype = "bf16"} : (tensor<16x32xf8E4M3FN>, tensor<32x32xf8E4M3FN>) -> tensor<16x32xbf16>
  %P = merlin_iface.softmax %S {name = "P", axis = 1 : i64, scale = 0.17677669529663687 : f32, softcap = 50.0 : f32} : (tensor<16x32xbf16>) -> tensor<16x32xbf16>
  %Y0 = merlin_iface.attention_pv %P, %V {name = "Y0", block_scale = "e8m0", output_dtype = "bf16"} : (tensor<16x32xbf16>, tensor<32x16xf8E4M3FN>) -> tensor<16x16xbf16>
}
