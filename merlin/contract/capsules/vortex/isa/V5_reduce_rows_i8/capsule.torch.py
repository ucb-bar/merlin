"""V5_reduce_rows_i8 — torch source for the capsule (authored for model2MLIR).

row-wise sum reduction (i8 -> i32). Lowering this through model2MLIR's FXImporter backend yields standard-dialect
...
class Capsule(nn.Module):
    def forward(self, a):
        out = a.to(torch.int32).sum(dim=1, dtype=torch.int32)
        return out
