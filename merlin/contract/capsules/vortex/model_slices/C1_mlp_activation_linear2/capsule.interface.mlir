// C1_mlp_activation_linear2: linear (i8 x i8 -> i32) + relu.
...
module attributes {merlin.capsule = "C1_mlp_activation_linear2"} {
  func.func @forward(%A: tensor<16x64xi8> {merlin.role = "input"},
                     %W: tensor<64x64xi8> {merlin.role = "weight"}) -> tensor<16x64xi32> {
    ...
    %relu = linalg.generic {indexing_maps = [#ew, #ew],
         ...
         ins(%mm : tensor<16x64xi32>) outs(%eR : tensor<16x64xi32>) {
    ^bb0(%v: i32, %o: i32):
