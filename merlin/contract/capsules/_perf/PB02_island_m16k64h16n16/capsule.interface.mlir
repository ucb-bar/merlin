builtin.module attributes {prov.level = "linalg-on-tensors"} {
  func.func @forward(%A0: tensor<16x64xi8>, %W0: tensor<64x16xi8>, %W1: tensor<16x16xi8>) -> tensor<16x16xi32> {
    ...
    %acc0 = linalg.generic {indexing_maps = [affine_map<(d0, d1, d2) -> (d0, d2)>, affine_map<(d0, d1, d2) -> (d2, d1)>, affine_map<(d0, d1, d2) -> (d0, d1)>], iterator_types = ["parallel", "parallel", "reduction"]} ins(%A0, %W0 : tensor<16x64xi8>, tensor<64x16xi8>) outs(%acc0_init : tensor<16x16xi32>) attrs = {prov.region_id = "contraction_0", prov.op = "matmul", prov.family = "contraction"} {
    ...
    %host_empty = tensor.empty() : tensor<16x16xi8>
    %xor_mask = arith.constant 1 : i8
    %host = linalg.generic {indexing_maps = [affine_map<(d0, d1) -> (d0, d1)>, affine_map<(d0, d1) -> (d0, d1)>], iterator_types = ["parallel", "parallel"]} ins(%narrow : tensor<16x16xi8>) outs(%host_empty : tensor<16x16xi8>) attrs = {prov.region_id = "host_island_0", prov.op = "xor_low_bit", prov.family = "host_scalar", prov.placement = "host_required"} {
    ^bb1(%x: i8, %unused: i8):
      %mapped = arith.xori %x, %xor_mask : i8
      linalg.yield %mapped : i8
    } -> tensor<16x16xi8>
