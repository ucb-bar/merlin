...
This is the stronger agent claim: derive a kernel for a non-matmul family, including a pattern
(reduction) not shown in the examples.
"""
...
def generate_vector_kernel(*, visible=("VEC0", "VEC1"), heldout=("VEC2",), rounds=4,
    ...
    for r in range(rounds):
        ...
        try:
            ...
            feedback = f"{e}. Output one ```python block."; history.append({"r": r, "stage": "nocode"}); continue
        bad = [t for t in CHEAT if t in code]
        if bad:
            feedback = f"Rejected: forbidden tokens {bad}; compute on hardware only."
            history.append({"r": r, "stage": "cheat", "bad": bad}); continue
        try:
            gen = _load(code, work / f"vk_r{r}.py")
        except Exception as e:
            feedback = f"import failed: {e!r}"; history.append({"r": r, "stage": "load", "e": repr(e)}); continue
        vis, ok = {}, True
        for rung in visible:
            try:
                res = _certify(rung, gen, work / f"run_{rung}_{r}")
            except Exception as e:
                vis[rung] = f"err:{e!r}"; ok = False; feedback = f"{rung} compile/run failed: {e!r}"; break
            vis[rung] = res["correct"]
            if not res["correct"]:
                ok = False
                from ...runtime import reference_outputs
                feedback = f"{rung}: got {res['outputs']} expected {reference_outputs(build(rung))} — fix the vector logic."
                break
        ...
        for rung in heldout:
            try:
                res = _certify(rung, gen, work / f"run_{rung}_{r}")
                held[rung] = res["correct"]; hok = hok and res["correct"]
            except Exception as e:
                held[rung] = f"err:{e!r}"; hok = False
