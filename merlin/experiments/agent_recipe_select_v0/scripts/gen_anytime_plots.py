...
def _sweep_optimum(workload_key: str) -> tuple[int | None, int | None, dict | None]:
    ...
    for prod in sorted(root.glob("v*/*/recipe_sweep.json"), key=lambda p: p.parent.name, reverse=True):
        rows = json.loads(prod.read_text())["rows"]
        rows = [
            r for r in rows if r["workload"] == workload_key and isinstance(r.get("cycles"), int) and r.get("correct")
        ]
...
def main(argv: list[str] | None = None) -> int:
        ...
        for r in s_["history"]:
            if key == "cumulative_tokens":
                ...
                if r.get("cumulative_notional_usd") is not None:
                    ...
                    run_c += float((r.get("accounting") or {}).get("subscription_notional_usd") or 0.0)
    ...
    for ri, s_ in enumerate(summaries):
        ...
        for ci, (key, xlabel, _sc) in enumerate(AXES):
            ...
            ax.step(
                ...
                ys,
                where="post",
                color=PS.COLOR.get("merlin_targetgen", "#E6B84C"),
                lw=2.4,
                marker="o",
                ms=4,
            ...
            if opt and base:
                ...
                ax.text(
                    0.99,
                    base / opt,
                    f"exhaustive optimum {base / opt:.3f}x  ",
                    ha="right",
                    va="bottom",
        ...
        first_best = next(
            (i for i, r in enumerate(s_["history"]) if r.get("best_cycles_so_far") == s_.get("best_cycles")), None
        )
        rows_tbl.append(
            {
                "workload": wkey,
                "baseline_cycles": base,
                "best_cycles": s_["best_cycles"],
                "speedup": round(reached, 4) if reached else None,
                "exhaustive_optimum_cycles": opt,
                "optimum_recipe": orec,
                "found_optimum": (s_["best_cycles"] == opt) if opt else None,
                "candidates": s_["candidates"],
                "invalid": s_["invalid_candidates"],
                "duplicates": s_["duplicate_candidates"],
                "candidate_reaching_best": first_best,
                "tokens_to_best": series(s_, "cumulative_tokens")[first_best] if first_best is not None else None,
                "wall_s_to_best": series(s_, "cumulative_wall_seconds")[first_best] if first_best is not None else None,
                "notional_usd_to_best": series(s_, "cumulative_notional_usd")[first_best]
                if first_best is not None
                else None,
                "tokens_total": t["tokens_total"],
                "tokens_in": t["tokens_input"],
                "tokens_out": t["tokens_output"],
    ...
    print(
        f"{'workload':<13}{'base':>6}{'best':>6}{'sp':>8}{'opt':>6}{'hit':>6}{'cand':>5}"
        f"{'inv':>4}{'dup':>4}{'@cand':>6}{'tok@best':>10}{'wall@best':>10}{'$@best':>8}"
        f"{'tok_tot':>9}{'wall_tot':>9}{'$_tot':>7}"
    )
    for r in rows_tbl:
        print(
            f"{r['workload']:<13}{r['baseline_cycles']:>6}{r['best_cycles']:>6}"
