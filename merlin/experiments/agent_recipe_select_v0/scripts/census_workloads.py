...
def rows_from_census(census_json: Path, pkg: Path, *, budget_s: float) -> list[dict]:
    ...
    for model in payload["models"]:
                ...
                out.append(
                    {
                        ...
                        "mac_share": "",
                        "expressible": "no",
                        "inexpressible_reason": f"rank {len(par)}x{len(red)} contraction: this lowering emits 2-D "
                        f"matmul only ({r.get('op_class')})",
                        "census_verdict": (r.get("legality") or {}).get("verdict", ""),
                        "eval_M": "",
                        "eval_N": "",
                        "eval_K": "",
                        "sizing": "not_expressible",
        ...
        for rank, ((m_, n_, k_, dts), rs) in enumerate(ranked):
            ...
            sizing = (
                "true_shape"
                if (bud_m, bud_n) == (m_, n_)
                else "clamped_capacity+budget"
                if (cap_m, cap_n) != (m_, n_)
                else "clamped_budget"
            )
            ...
            out.append(
                {
                    ...
                    "derived_block_m": plan.bm,
                    "derived_block_n": plan.bn,
                    "derived_block_k": plan.bk,
                    ...
                    "true_N": n_,
                    "true_K": k_,
                    "capacity_fit_M": cap_m,
                    "capacity_fit_N": cap_n,
                    "capacity_fit_failed": cap_fail,
                    "pred_cycles_low": int(lo),
                    "pred_cycles_high": int(hi),
...
def _capability_report(rows: list[dict], census: Path, pkg: Path, digest: str, budget_s: float) -> str:
    ...
    """
    L = [
        "# Model kernel census and the capability it exposes",
        "",
        f"- census: `{census}`",
        ...
        "",
        "| model | distinct shapes | fit whole (pre-blocking) | emittable (with blocking) | MACs covered |",
        "|---|---|---|---|---|",
    ]
    if a.emit_workloads:
        ...
        for r in rows:
            ...
            (wdir / name).write_text(IFACE.format(M=r["eval_M"], N=r["eval_N"], K=r["eval_K"]), encoding="utf-8")
            written.append(name)
    ...
    for model in sorted({r["model_id"] for r in rows}):
        ...
        L.append(
            f"| {model} | {len(mr)} | {len(whole)}/{len(mr)} | {len(expr)}/{len(mr)} | "
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--census",
        default="",
        help="workload_census.json (default: newest under out/artifacts/target-evolution/gemmini/v1)",
    )
    ap.add_argument("--package", default="", help="minted compiler package (default: mint from WORK)")
    ap.add_argument(
        "--budget-s", type=float, default=300.0, help="per-candidate GSIM wall budget that sizing must respect"
    )
    ...
    if dest:
        ...
        with dest.open("w", newline="", encoding="utf-8") as fh:
            ...
            for r in rows:
                w.writerow(r)
        (dest.parent / "census_provenance.json").write_text(
            json.dumps(
                {
                    "census": str(census),
                    "package": pkg.name,
                    ...
                    "budget_s": a.budget_s,
                    "calibration": {
                        "macs_per_cycle_low": MACS_PER_CYCLE_LOW,
                        "macs_per_cycle_high": MACS_PER_CYCLE_HIGH,
                        "eval_fixed_s": EVAL_FIXED_S,
                        "eval_s_per_cycle": EVAL_S_PER_CYCLE,
                        "source": "measured on 172 v0 candidates; NOT the falsified cost model",
                    },
                    "claim_model_rule": (
                        "resnet50_v1_5 and tiny_llama are claim models: this file is a "
    ...
    for model in sorted({r["model_id"] for r in rows}):
        ...
        print(
            f"  predicted GSIM wall        {sum(r['pred_eval_s'] for r in shapes) / 60:.1f} min "
