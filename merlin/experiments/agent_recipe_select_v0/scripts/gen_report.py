...
def _find(name: str):
    ...
    for prod in sorted((p for p in root.glob("v*/*/") if (p / name).exists()), key=lambda p: p.name, reverse=True):
...
def main(argv: list[str] | None = None) -> int:
    ...
    if pin:
        ...
        for cap, per in by.items():
            ...
            a(f"| {cap} | {v.get('cycles')} | {g.get('cycles')} | {v.get('wall_s')}s | {g.get('wall_s')}s |")
        a(
    ...
    if sweep:
        ...
        for res, dr in combos:
            ...
            for w in wls:
                ...
                cells.append(f"{row['cycles']}" + ("" if not d else f" (**{d:+.1f}%**)" if d > 0 else f" ({d:+.1f}%)"))
            tag = f"`{res}`/`{dr}`" + (" **(frozen default)**" if (res, dr) == ("per_tile", "inline") else "")
            a(f"| {tag} | " + " | ".join(cells) + " |")
        a("\nAll cells bit-exact. Two results:\n")
        a(
            "- **`panel` is a monotone, strict win** that scales with the N sweep, because the saving "
            "is `Mt·Kt·(Nt−1)` activation transfers — DERIVED, and matched by the emitted-code delta."
        )
        a(
            "- **`drain` flips sign on both axes**: it helps with `per_tile` at the smaller shapes and "
    ...
    if ho:
        ...
        _resolved = {f"{m}x{n}x{k}": _R.resolve_auto(m, n, k, dim=16, spad_rows=16384) for m, n, k in _probe}
        a(
            "Rule, read from the compiler's own `resolve_auto` rather than from a recorded string: "
        ...
        if ho.get("rule_supersedes"):
            ...
            a(
                "> The product JSON for this run still records the earlier capacity predicate "
        ...
        a(f"Fitted on {sorted(tuple(s) for s in ho['fitting_shapes'])}; tested on shapes never measured before.\n")
        ...
        for r in ho["rows"]:
            ...
            v = (
                "WINS"
                if r["heldout"] in names
                else "TIES"
                if r["heldout"] in ties
                else "N/A (inexpressible)"
                if r["heldout"] in inex
                else "—"
            )
            a(
                f"| {r['heldout']} ({r['M']}×{r['N']}×{r['K']}) | Nt={r['Nt']} | {cell(pt)} "
    ...
    if agent:
        ...
        a(
            "Driver **codex** on the ChatGPT subscription seat (`gpt-5.6-sol`), one `codex exec` per "
        ...
        for q in agent:
            ...
            opt = None
            for prod in sorted(
                (artifacts_dir() / "recipe-select" / T.TARGET).glob("v*/*/recipe_sweep.json"),
                key=lambda x: x.parent.name,
                reverse=True,
            ):
                rr = [
                    r
                    for r in json.loads(prod.read_text())["rows"]
                    if r["workload"] == wkey and isinstance(r.get("cycles"), int) and r.get("correct")
                ]
                if len({json.dumps(r["recipe"], sort_keys=True) for r in rr}) >= 15:
                    opt = min(r["cycles"] for r in rr)
                    break
            t = d["totals"]
            first = next((i for i, r in enumerate(d["history"]) if r.get("best_cycles_so_far") == best), None)
            a(
                f"| {wkey} | {base} | **{best}** | "
        ...
        a("")
        a(
            "**Zero invalid candidates** is the core of the thesis: because the compiler constructs the "
        ...
        for wkey, first, t in detail:
            a(
                f"| {wkey} | candidate {first} | {t['tokens_total']} | "
        ...
        a(
            "⚠️ **Two things the token axis measures that are not the model.** Each candidate is a "
        ...
        a(
            "⚠️ **This does not yet demonstrate search EFFICIENCY.** The space is 20 points and the "
    ...
    ok_by_wl = {}
    for q, d in ac_runs:
        if d.get("arm_completed") and (d.get("invalid_total") or 0) * 2 <= (d.get("candidates_evaluated") or 1):
            ok_by_wl[d.get("workload")] = q.parent.name
    ac_ok = [d for q, d in ac_runs if ok_by_wl.get(d.get("workload")) == q.parent.name]
    ac_void = [(q, d) for q, d in ac_runs if ok_by_wl.get(d.get("workload")) != q.parent.name]
    if ac_runs:
        a("## The AutoComp arm — an LLM rewrites the kernel\n")
        a(
            "AutoComp's own search, prompts and parameters (copied verbatim from its "
        ...
        if ac_ok:
            a(
                "| workload | seed (C library) | AutoComp best | vs its own seed | candidates | "
                "did not compile | compiled but wrong |"
            )
            a("|---|---|---|---|---|---|---|")
            for d in ac_ok:
                a(
                    f"| {d['workload']} | {d['baseline_cycles']} | **{d['best_cycles']}** | "
        ...
        if ac_void:
            a(
                f"⚠️ **{len(ac_void)} run(s) VOIDED and excluded from the table above** — every one by a "
        ...
        a(
            "A second asymmetry, equally deliberate: **one candidate is not the same unit on both "
    ...
    if eff:
        ...
        a(
            "The recipe arm reached the optimum on every workload — but with a 16-evaluation budget "
        ...
        a(
            "| workload | space | optimum | agent reached it at | random's EXPECTED best at that n | "
        ...
        for r in eff["rows"]:
            if "space_size" not in r:
                continue
            a(
                f"| {r['workload']} | {r['space_size']} | {r['optimum']} | "
        ...
        a(
            "That is the load-bearing caveat on the whole ablation, and it points at what a real "
    ...
    a(
        "**The calibrated gemmini cost model.** Predicted vs measured, errors in BOTH directions and "
        "outside its own declared `max_abs_pct` of 34.9%:\n"
    )
    a("| case | predicted | measured | error |")
    a("|---|---|---|---|")
    for name, pred, meas in (
        ("A2_single_tile_matmul", 174, 302),
        ("PK03_k128", 1103, 604),
        ("w1_small 32³", 1056, 780),
        ("w2_medium 64³", 7322, 3466),
    ...
    a(
        '**The capacity predicate for the new default.** The intended rule was "`panel` where both '
    ...
    a(
        "- **Shapes past the operand-store bound** (`Kt·(Mt+Nt) > operand_rows/DIM`) are inexpressible "
        "for every wave-A value. That needs a blocked-residency value, and it is a compiler COVERAGE "
        "gap, not evidence about the rule.\n"
    )
    a(
        "- **The agentic arm.** No model has been in the loop for any number above; LLM spend is a "
    ...
    a(
        "- **AutoComp head-to-head.** The existing bridge gates on provider and cannot be driven by "
