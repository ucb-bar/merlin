...
def main(argv: list[str] | None = None) -> int:
    ...
    # ---- report
    print(
        f"machine (derived): DIM={dim}  scratchpad_rows={spad_rows}  capacity relation Kt*(Mt+Nt) <= {spad_rows // dim}"
    )
    print(
        f"cost model: MAPE band +/-{band_pct:.2f}%  n_points={model.error.get('n_points')}  "
    ...
    for r in visible:
        print(
            f"{r['workload']:<13}{r['recipe_activation_residency']:<10}"
    ...
    for w in WORKLOADS:
        ...
        print(
            f"  {w.wid:<13} {best['recipe_activation_residency']}+{best['recipe_config_policy']}"
            f"  {best['predicted_cycles']:.0f} cyc  ({best['delta_pct_vs_default']:+.1f}% vs default)"
        )

    spread = {}
    for w in WORKLOADS:
        cand = [r["predicted_cycles"] for r in visible if r["workload"] == w.wid and r["fits_operand_store"]]
        if cand:
            spread[w.wid] = 100.0 * (max(cand) - min(cand)) / max(cand)
    over = [k for k, v in spread.items() if v > band_pct]
    print(f"\nrecipe spread per workload (max-min)/max: " + ", ".join(f"{k} {v:.1f}%" for k, v in spread.items()))
    print(
        f"GATE: spread exceeds the {band_pct:.2f}% band on {len(over)}/{len(spread)} workloads "
    ...
    if nofit:
        print(
            f"Frozen default does NOT fit the operand store for: {', '.join(nofit)} "
    ...
    if pins:
        v = pins["gemmini_rtl"]
        print(f"\nprovenance: gemmini_rtl ok={v.ok}" + ("" if v.ok else f" drift={list(v.drift)[:2]}"))
