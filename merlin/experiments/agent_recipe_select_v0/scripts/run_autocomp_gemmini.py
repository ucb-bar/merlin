...
from merlin.common.artifacts import git_sha7, utc_stamp  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--workload", default="", choices=sorted(SHAPES) + [""], help="a named control shape shared with v0"
    )
    ap.add_argument(
        "--workload-mlir",
        default="",
    ...
    ap.add_argument("--budget-candidates", type=int, default=16, help="match the recipe arm's evaluation budget")
        ...
        json.dumps(
            {
                ...
                "tiering_note": (
                    "the seat serves one model, so the tiers differ on reasoning effort; "
                    "per-tier spend is in the run summary's by_tier block"
                ),
                "plan_model": a.plan_model,
                "code_model": a.code_model,
                "engine": a.engine,
    ...
    backend = GsimGemminiEvalBackend(
        M=M, K=K, N=N, workdir=run_dir / "candidates", simulator=a.engine, timeout=a.eval_timeout
    )
    ...
    prob = Prob(
        "gemmini",
        0,
        test_file=run_dir / "seed_kernel.c",
    ...
    code_agent = (
        LLMEnsemble([GemminiLLMAgent(a.code_model, hw, backend)])
        if a.code_model and a.code_model != a.plan_model
        else None
    )
    ...
    if not summary["arm_completed"]:
        summary["do_not_cite"] = (
            "the search did not run to completion, so best_cycles is the SEED and this is not an AutoComp result"
        )
        ...
        json.dumps(
            {
                **json.loads((run_dir / "run_record.json").read_text()),
                ...
                "summary": {"best_cycles": best, "candidates": len(traj), "tokens_total": cx["tokens_total"]},
            },
            indent=1,
        ),
        encoding="utf-8",
