...
AUTOCOMP_PY = (
    Path(os.environ.get("MERLIN_EXT_AUTOCOMP") or HERE.parents[3].parent / "autocomp") / ".venv" / "bin" / "python"
)
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--resume", action="store_true", help="skip shapes a prior campaign of this ARM already finished cleanly"
    )
    ap.add_argument(
        "--resume-from",
        action="append",
    ...
    if a.resume or a.resume_from:
        ...
        for d in prior:
            ...
            for line in mf.read_text(encoding="utf-8").splitlines():
                ...
                rec = json.loads(line)
                arm = rec.get("arm") or (
                    "recipe" if "campaign_recipe" in d.name else "autocomp" if "campaign_autocomp" in d.name else ""
                )
        ...
        while True:
            for i, (proc, path, t0) in enumerate(list(running)):
                if proc.poll() is not None:
                    ...
                    print(
                        f"[{len(done)}/{len(shapes)}] {path.name} rc={proc.returncode} {rec['wall_s']:.0f}s", flush=True
                    )
                    running.pop(i)
                    return True
    ...
    for path in shapes:
        ...
        if a.arm == "recipe":
            ...
            argv_ = [
                str(AUTOCOMP_PY),
                str(HERE / "run_autocomp_gemmini.py"),
                "--workload-mlir",
                str(path),
                "--budget-candidates",
                str(a.budget),
        ...
        proc = subprocess.Popen(
            argv_, stdout=log, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL, env=env, start_new_session=True
        )
