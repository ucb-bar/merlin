...
from merlin.common.artifacts import artifacts_dir, finish_run, start_run  # noqa: E402
...
def _products() -> list[Path]:
    ...
    if not root.exists():
        return []
    return sorted((p for p in root.glob("v*/*/") if (p / "manifest.yaml").exists()), key=lambda p: p.name, reverse=True)
...
def oracle_spend() -> dict:
    ...
    if ho:
        cells = [a for r in ho["rows"] for a in r["arms"].values() if isinstance(a.get("cycles"), int)]
        phases["heldout_generalization"] = {
            "product": str(hp),
            "engine": ho.get("engine"),
...
def agent_spend() -> list[dict]:
    ...
    for f, d in ac_runs:
        ...
        if not d.get("arm_completed"):
            ...
            void = (
                "superseded by a later run; its candidates were rejected by harness defects "
        ...
        out.append(
            {
                ...
                "notional_usd": 0.0,
                "billed_usd": None,
                "agent_seconds": max((d.get("wall_seconds") or 0) - (d.get("oracle_seconds") or 0), 0.0),
        ...
        out.append(
            {
                ...
                "invalid": d.get("invalid_candidates"),
                "tokens_input": t.get("tokens_input", 0),
                "tokens_cached": t.get("tokens_cached", 0),
...
def _log_agent_runs(rows: list[dict]) -> int:
    ...
    for r in rows:
        ...
        try:
            lg = EvalRunLogger.start(
                project="merlin",
                suite=T.SUITE,
                target=T.TARGET,
                method=("autocomp_codex" if r.get("arm") == "autocomp" else "recipe_agent_codex"),
                seed=0,
                run_id=r["run_id"],
                run_path=rd,
                tracking_mode="local",
            ...
            lg.log_token_usage(
                ...
                cache_read_tokens=r["tokens_cached"],
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--method", default="recipe_sweep_deterministic", help="the arm this spend belongs to")
    ...
    for name, p in phases.items():
        print(
            f"  {name:<26} sims={p['sim_invocations']:<4} wall={p['wall_seconds']:>8.1f}s "
    ...
    if ag:
        print(f"  {'arm / workload / model':<52}{'cands':>6}{'inv':>5}{'tokens':>10}{'notional$':>11}")
        for r in ag:
            label = (
                "" if r.get("cited") else "VOID "
            ) + f"{r.get('arm', '?')} / {r['workload']} / {r.get('model') or ''}"
            print(
                f"  {label[:52]:<52}{str(r['candidates']):>6}"
        ...
        print(
            f"  billed: NOTHING -- {AGENT_DRIVER}/{AGENT_MODEL} on a subscription seat is not "
    ...
    h = start_run(
        suite=T.SUITE, method=args.method, target=T.TARGET, seed=0, extra={"phases": list(phases), "llm_in_loop": False}
    )
    try:
        from aet.tracking.run_logger import EvalRunLogger
        ...
        lg.log_token_usage(input_tokens=0, output_tokens=0, cache_creation_tokens=0, cache_read_tokens=0, model="none")
        lg.log_cost(0.0, model="none")
        lg.log_agent_turns(0)
        lg.log_param("llm_in_loop", "false")
        lg.log_param("llm_zero_reason", "deterministic enumeration of a 6-point recipe space; no model was invoked")
    ...
    (h.run_dir / "spend.json").write_text(
        json.dumps(
            {
                "phases": phases,
                "totals": {"sim_invocations": total_sims, "wall_seconds": total_wall, "cycles_simulated": total_cyc},
                "llm": {
                    "in_loop": False,
                    "tokens_total": 0,
                    "turns": 0,
                    "reason": "deterministic enumeration; no model invoked",
                    "planned_driver": AGENT_DRIVER,
                    "planned_model": AGENT_MODEL,
                    "planned_billing_mode": AGENT_BILLING,
                    "dollars_note": (
                        "a subscription seat is not billed per token, so dollars are "
    ...
    finish_run(h, "ok", summary={"sim_invocations": total_sims, "cycles_simulated": total_cyc, "llm_tokens": 0})
