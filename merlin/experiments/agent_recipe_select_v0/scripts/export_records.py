...
COLUMNS = [
    ...
    "macs",
    "candidate_index",
    "recipe_activation_residency",
    "recipe_config_policy",
    "recipe_drain",
    "recipe_block_m",
    "recipe_block_n",
    "recipe_block_k",
    ...
    "n_mvout",
    "n_fence",
    "code_differs_from_default",
    "counts_differ_from_default",
    # result
    "legal",
    "correct",
    "cycles",
...
def _blocks_from_compiler(row: dict) -> None:
    ...
    try:
        ...
        pl = R.blocks(R.Recipe(**rec), m=m, n=n, k=k, dim=DIM, spad_rows=SPAD_ROWS, acc_rows=ACC_ROWS)
        if pl.ok:
...
def _census_shape(row: dict, workload: str) -> None:
    ...
    if _CENSUS is None:
        _CENSUS = {}
        for prod in sorted((T.REPO / "out/artifacts/recipe-select/gemmini/v2").glob("*/kernel_census.csv")):
    ...
    for f in sorted(root.glob("v*/*/heldout_rule.json")):
        ...
        for r in d["rows"]:
            for arm_value, res in (r.get("arms") or {}).items():
                ...
                row.update(
                    {
                        ...
                        "failure": res.get("reason") or res.get("error") or res.get("status") or "",
                ...
                _recipe(row, {"activation_residency": arm_value, "config_policy": "per_mvin", "drain": "inline"})
                out.append(row)
    return out
            ...
            row.update(
                {
                    ...
                    "candidate_index": r.get("index"),
                    "is_default": r.get("index") == 1,
                    "legal": r.get("compiled"),
                    "correct": r.get("correct"),
                    "cycles": r.get("cycles") if r.get("cycles") is not None else "",
                    "speedup_vs_default": round(base / r["cycles"], 4)
                    if (base and isinstance(r.get("cycles"), int) and r.get("correct"))
                    else "",
            ...
            row.update(
                {
                    ...
                    "Kt": tiles.get("Kt", ""),
                    "macs": sh.get("macs", ""),
                    "candidate_index": r["candidate"],
                    "is_default": False,
                    "candidate_id": r.get("candidate_id", ""),
                    "artifact_digest": r.get("artifact_digest", ""),
                    "n_instructions": r.get("n_instructions", ""),
                    "code_differs_from_default": (r.get("vs_default_code") or {}).get("identical") is False
                    if r.get("vs_default_code")
                    else "",
                    "legal": r.get("legal", ""),
                    "correct": r.get("correct", ""),
                    "cycles": r.get("cycles", ""),
                    "speedup_vs_default": round(base / r["cycles"], 4)
                    if (base and isinstance(r.get("cycles"), int))
                    else "",
                    ...
                    "tokens_input": a.get("tokens_input", ""),
                    ...
                    "billed_usd": "" if a.get("estimated_cost_usd") is None else a.get("estimated_cost_usd"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    rows.sort(
        key=lambda r: (
            str(r["arm"]),
            str(r["workload"]),
            int(r["candidate_index"]) if str(r["candidate_index"]).lstrip("-").isdigit() else 0,
        )
    )
