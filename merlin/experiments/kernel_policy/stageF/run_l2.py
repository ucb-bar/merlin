...
def compile_variant(paths, spec, variant: str, n: int, out_dir: Path) -> Path:
    ...
    cmd = [
        ...
        "-fno-common",
        "-fno-builtin-printf",
        f"-DVARIANT_{variant.upper()}",
        f"-D{spec['sweep'][0]}={n}",
        f"-I{paths['rocc']}",
        f"-I{paths['rocc']}/include",
        str(CALIB_SRC / spec["harness"]),
        "-o",
        str(out),
        "-lm",
    ]
...
def run_and_count(paths, spec, binary: Path) -> dict:
    ...
    counts["gemmini_cmds"] = sum(v for k, v in counts.items() if k not in ("instret", "fence"))
    return dict(counts)
    ...
    with tempfile.TemporaryDirectory() as td:
        for n in spec["sweep"][1]:
            for variant in spec["variants"]:
                ...
                rows.append({"insight": insight, "variant": variant, spec["sweep"][0]: n, "events": ev})
                print(
                    f"{insight} {spec['sweep'][0]}={n:>4} {variant:<8} "
...
def decide(insight: str, rows: list[dict]) -> dict:
    ...
    s: dict = {"slate": insight, "fidelity": "L2_event_counts (events, not cycles)", "at": {key: nmax}, "rows": rows}
    if insight == "resident_rhs":
        ...
        s["decision"] = (
            "act" if s["config_fraction_baseline"] > 0.3 and s["commands_removed_fraction"] >= 0.3 else "park"
        )
        ...
        s["tail_overhead_fraction"] = {
            n: round(tail[n]["events"]["instret"] / vla[n]["events"]["instret"] - 1, 4) for n in tail
        }
        worst = max(s["tail_overhead_fraction"].values())
        s["decision"] = (
            "act" if worst >= 0.1 else "park (no measurable instret win at VLEN=128; portability rationale only)"
        )
    return s
...
def main() -> int:
    ...
    ap.add_argument("--insight", default="all", choices=[*INSIGHTS, "all"])
