...
- `emit_target_artifact`: `{tool} --convert-iface-to-radiance --emit-target-artifact {input_mlir}` — lower your target dialect to an **LLVM-dialect MLIR** module (a `.mlir`, NOT textual LLVM-IR) whose command-ISA instructions are `llvm.inline_asm` ops wrapping raw `.insn` directives — with opcode/func3/func7 from the discovered ISA facts, assembled by STOCK clang/LLVM, no forked toolchain. EVERY operand must be an SSA value defined earlier — an immediate as `%c = llvm.mlir.constant(<imm> : i64) : i64`, a pointer via `llvm.ptrtoint` of an arg — then passed by name. Canonical:
