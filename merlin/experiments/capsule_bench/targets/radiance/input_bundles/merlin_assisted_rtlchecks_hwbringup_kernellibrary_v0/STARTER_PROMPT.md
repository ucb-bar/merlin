...
9. Run these four — reading the files instead of CALLING them does not count:
   - `from rtl import facts; facts.load_facts("radiance")` — the RTL fact bundle (arrays / memories / datapaths / interfaces); every ISA, mesh and datapath number comes from this dict.
   - `from rtl_backend import target_profile, derived_levers; derived_levers(target_profile("radiance"))` — the compiler levers this hardware implies; build to the list it returns.
...
`radiance-kernels-pr1/`. Read its `selection.yaml` and `manifest.yaml` before authoring. Mine reusable
