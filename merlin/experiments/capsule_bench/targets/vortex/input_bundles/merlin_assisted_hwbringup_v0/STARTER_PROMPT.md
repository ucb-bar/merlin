...
target-specific set of compiler seams you may modify for `vortex`, so you build the right lever set
...
- `python cca_contract.py check-bijection vortex` — the *what-to-build* checklist: which lever axes
  ...
  phantom. (API form, if you prefer: `from cca_contract import check_bijection; check_bijection("vortex")`.)
- `python action_catalog.py escalation-ladder <axis> vortex` — for one axis, the full
  ...
  `from action_catalog import escalation_ladder; escalation_ladder("<axis>", "vortex")`.)
...
6. Enumerate your lever set: run `python cca_contract.py check-bijection vortex` + `python action_catalog.py escalation-ladder <axis> vortex` (runnable CLIs, like `isa_tools.py`) and build every leverable axis they list.
7. Emit `submission/lowered.llvm.mlir` per the LLVM-dialect MLIR contract above: your xDSL pass pipeline BUILDS the `builtin.module` with `llvm.func @merlin_kernel_body(...)` (one args-struct pointer — operands at `arg->args[i]` in `merlin.arg_table` order; read your coordinate from the CSRs and reconverge divergent control per the ISA spec) that COMPUTES the op and `llvm.return`s — no prints, no `.insn`/`.word`. Verify the module parses/`verify()`s before self_check.
