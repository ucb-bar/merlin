# Task: generate a gemmini_universal MLIR out-of-tree target backend (capsule_bench — FULL SUITE)

You are an autonomous agent. Produce a **non-exempt out-of-tree MLIR target backend package** for the
gemmini_universal accelerator under `submission/`. Your package is graded — through its CLI entrypoints only,
...
```
submission/
  manifest.yaml   # artifact_type: mlir_oot_target_backend; target: gemmini_universal; language: cpp|python;
                  # integrity_exempt: false; (cpp) a build block; 4 required command argv templates
  mlir_oot/       # your OOT sources: input dialect + gemmini_universal target dialect + passes + gemmini_universal-opt
...
- `lower_interface_to_target`: `{tool} --convert-iface-to-gemmini_universal {input_mlir}` — emit gemmini_universal-dialect MLIR
- `emit_command_buffer`: `{tool} --emit-command-buffer={output_json} {input_mlir}` — schema-valid `command_buffer.json`
- `emit_target_artifact`: `{tool} --convert-iface-to-gemmini_universal --emit-target-artifact {input_mlir}` — lower your target dialect to an **LLVM-dialect MLIR** module (a `.mlir`, NOT textual LLVM-IR) whose command-ISA instructions are `llvm.inline_asm` ops wrapping raw `.insn` directives — with opcode/func3/func7 from the discovered ISA facts, assembled by STOCK clang/LLVM, no forked toolchain. EVERY operand must be an SSA value defined earlier — an immediate as `%c = llvm.mlir.constant(<imm> : i64) : i64`, a pointer via `llvm.ptrtoint` of an arg — then passed by name. Canonical:
...
**Shipped gemmini_universal ISA — the source of truth for instruction encodings (derive, never invent):**
The real gemmini_universal ISA is shipped read-only in your bundle. Derive EVERY instruction's
...
- `experiments/capsule_bench/targets/gemmini_universal/contracts/hwbringup_gemmini_universal_v0/isa_include/gemmini.h`
- `experiments/capsule_bench/targets/gemmini_universal/contracts/hwbringup_gemmini_universal_v0/isa_include/gemmini_params.h`
- `experiments/capsule_bench/targets/gemmini_universal/contracts/hwbringup_gemmini_universal_v0/` (also mounted as `gemmini_universal/`) — RTL + ISA headers + README + a WORKED
...
# Target ISA facts: gemmini_universal
...
target-specific set of compiler seams you may modify for `gemmini_universal`, so you build the right lever set
...
- `python cca_contract.py check-bijection gemmini_universal` — the *what-to-build* checklist: which lever axes
  ...
  phantom. (API form, if you prefer: `from cca_contract import check_bijection; check_bijection("gemmini_universal")`.)
- `python action_catalog.py escalation-ladder <axis> gemmini_universal` — for one axis, the full
  ...
  `from action_catalog import escalation_ladder; escalation_ladder("<axis>", "gemmini_universal")`.)
...
5. CLOSE THE LOOP on every edit: change `submission/` -> build -> `agent_selfcheck.py` on the
   changed capsule -> read `qa/verdict.json`. Blocking waits exist for the slow steps: `python await_verdict.py` for the next grade, `python simjob.py wait` for a simulation job.
...
6. Author the backend as an **xDSL pass pipeline** (`xdsl_dialects/`, `targetgen/synthesize/`, `targetgen/generate/`) — structured IR passes, NOT ad-hoc string assembly, and with **NO regular expressions** (`import re` / regex text-matching is prohibited; parse the IR structurally). This is checked on your submission.
7. Enumerate your lever set: run `python cca_contract.py check-bijection gemmini_universal` + `python action_catalog.py escalation-ladder <axis> gemmini_universal` (runnable CLIs, like `isa_tools.py`) and build every leverable axis they list.
...
9. Before every self_check, run `python isa_tools.py lint` and `disasm` on your emitted submission/*.mlir and confirm every instruction decodes (nothing UNKNOWN or ambiguous) and the kernel halts. A clean `lint` is NOT enough — a correctly-NAMED instruction carrying a WRONG FIELD decodes cleanly and still diverges on the hardware plane. So also read the `disasm` operand fields back against what your command buffer declares for that same command — DMA/DRAM address and row pitch, readout dtype / element width, accumulate and dataflow bits, config scale — and reconcile every one that disagrees.
10. RTL-checks arm: DERIVE the ISA / mesh / datapath from the granted RTL-extracted facts (`targetgen/rtl/` + the RTL facts pin) — do not hand-invent them — and run the CIRCT RTL checks on your lowering. Your backend must be a compilation FROM those RTL-derived facts.
11. Run these four — reading the files instead of CALLING them does not count:
   - `from rtl import facts; facts.load_facts("gemmini_universal")` — the RTL fact bundle (arrays / memories / datapaths / interfaces); every ISA, mesh and datapath number comes from this dict.
   - `from rtl_backend import target_profile, derived_levers; derived_levers(target_profile("gemmini_universal"))` — the compiler levers this hardware implies; build to the list it returns.
   - `from generate import mlir_scaffold; mlir_scaffold.generate(<dialect plan>)` — scaffold the dialect/pass skeleton from the derived plan instead of hand-writing it (`llvm_plan.generate` / `target_repo.generate_skeleton` count too).
   - read the `rtl_checks` block of `qa/verdict.json` whenever a new grade lands (wait for one with `python await_verdict.py`, never a poll loop) — that is where this arm's RTL feedback on YOUR lowering arrives; fix what it reports.
