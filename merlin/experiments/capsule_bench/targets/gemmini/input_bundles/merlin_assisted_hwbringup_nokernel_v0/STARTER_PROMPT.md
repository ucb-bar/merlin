...
2. **Make `validate()` your per-round compile-time gate.** Before every sim run:
   `from merlin.targetgen.oot_starterkit.verify import validate` →
   `validate(module, cb, trace, isa_header=gemmini_h, define_prefix="k_")` returns `{ok, findings}`.
   It runs xDSL's native `module.verify()` (broken graph), the cmdbuf schema, and the public-header
   funct legality check. The target's generated encoder or selected RTL checks must enforce
   config-before-use; generic verification does not infer protocol from instruction names.
...
the repo's `.venv/bin/python` (the interpreter that has `xdsl` + `merlin`).
...
> ISA header. This is exactly where the framework's verifier earns its keep: lean on `validate()` and the
> generic `transforms.im2col`/`tile_to_dim` rather than pattern-matching an example.

## B. Spend FEW tokens — derive once, don't spelunk
1. **Do NOT crawl the RTL repo.** Read `gemmini/README.md` + the ISA header **once**. There are no example
   kernels in this condition. The 55 RTL files are reference-of-last-resort, not a starting point.
