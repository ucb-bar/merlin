...
| **Codegen levers (derived)** | `merlin/python/merlin/targetgen/rtl_backend.py` | The generic, derivation-driven backend: it DERIVES the target's compiler levers/routes/codegen-opts from mlc RTL discovery (a discovered mesh implies a dataflow lever, an accumulator implies a residency lever). The seams point at YOUR generated OOT package (OOT-relative), not our in-tree reference. |
