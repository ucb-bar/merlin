...
## Generic C++ OOT backend generators (cpp_merlininfra — the scaffolding you are granted)
You have the project's **generic C++ out-of-tree generators**, so you do not hand-roll the package
plumbing. They are ordinary Python modules — read them and run them with `.venv/bin/python`. They
generate *scaffolding only*: they never contain a lowering, an encoding, or an answer.
- `merlin/python/merlin/targetgen/generate/mlir_scaffold.py` — generates a generic C++ OOT MLIR package
  skeleton from a dialect plan you supply: a target-dialect ODS skeleton (empty ops/types you fill in),
  a `gemmini-opt` driver already wiring the four entrypoints, and the CMake to build against the
  provided LLVM/MLIR. You still define your own dialects and write the lowering — in C++.
- `merlin/python/merlin/targetgen/generate/target_repo.py` — lays out the package tree (directories,
  CMake wiring, entrypoint stubs) so `manifest.yaml`'s `build` block compiles first time.
- `merlin/python/merlin/targetgen/generate/llvm_plan.py` — helps structure the LLVM/RoCC lowering leg
  (how your target dialect reaches `llvm.inline_asm` ops). It plans the shape; the field packing is
  still yours to take from the **Target ISA facts** above.

There is **no** pre-built `merlin_iface` dialect and **no** Python (xDSL) authoring kit here: you write
the dialect and parsing for the frozen input grammar yourself in C++. Use the scaffold to reach a
gradeable package early (see workflow item 4), then spend your time on the dialect and the lowering.
...
5. Stand the package up with the granted **C++ OOT generators** (`generate.mlir_scaffold`,
   `generate.target_repo`, `generate.llvm_plan` — see the section above) rather than hand-writing the
   package plumbing, then fill in your own dialects and lowering in C++. The generators produce the
   skeleton and the CMake/entrypoint wiring; every semantic decision is still yours and must come from
   the Target ISA facts + the capability contract.
