...
Import the starter kit with the repo's own interpreter — `.venv/bin/python` at the repository root,
which is the one that has `xdsl` + `merlin` installed (`python3` on PATH does not):
...
- `from merlin.targetgen.oot_starterkit.verify import validate` —
  `validate(module, cb, trace, isa_header=gemmini_h, define_prefix="k_")` is your per-round generic
  gate (graph verify + cmdbuf schema + public-header funct legality). Run it BEFORE any sim; use the
  generated encoder or selected RTL checks for target-specific instruction ordering.
