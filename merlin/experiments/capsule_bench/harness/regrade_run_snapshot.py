...
def main(argv: list[str] | None = None) -> int:
    ...
    from merlin_experiments.phase1 import run_inputs as RI  # noqa: PLC0415
    from merlin_experiments.phase1.workspaces import workspace_candidates  # noqa: PLC0415
    ...
    for workspace in workspace_candidates(source, target=target, experiment=descriptor.parent):
        candidates.append(workspace.parent / "bundle_inputs" / "repo" / hidden_rel)
    ...
    for candidate in candidates:
        try:
            record = RI.subtree_snapshot_record(candidate)
    ...
    for index in range(args.repeats):
        ...
        if not reused:
            ...
            proc = subprocess.run(inherited_python_command(command), cwd=repo_root(), text=True, capture_output=True)
    ...
    summary = {
        **validation,
        "source_submission_sha256": RI.subtree_snapshot_record(source_submission)["content_sha256"],
        "repeats": repeats,
