...
path, execution.sandbox_command) and have it try to reach + USE every tool it might need, then report. This validates
...
def _smoke_prompt(arm: str, is_rtlchecks: bool, golden_rel: str, facts_rel: str, capsule: str) -> str:
    ...
    if arm != "baseline":
        T.append(
            "`python3 -c \"import xdsl; from xdsl.ir import Dialect; print('xdsl_ok')\"`  (the xDSL spine; do NOT import merlin — that is forbidden and blocked)"
        )
    ...
    n = len(T)
    lines = [
        "You are running inside an isolated sandbox for a smoke test. Do EXACTLY this, then stop.",
        "",
...
def _bedrock_ping(model: str, region: str, profile: str, timeout: int) -> int:
    ...
    print(
        f"=== Bedrock ping — model={model} region={region} {'profile=' + profile if profile else 'env-var creds'} ==="
    )
    try:
        r = subprocess.run(
            ["claude", "--print", "--model", model, "Reply with exactly: BEDROCK_OK"],
            capture_output=True,
            text=True,
            timeout=timeout,
            env=env,
        ...
        print("  TIMEOUT — no response from Bedrock")
        return 1
...
def main(argv=None):
    ...
    print(
        f"=== LIVE smoke agent — arm={a.arm} target={te.target} sim={sim or 'n/a'} model={a.model} "
    ...
    with tempfile.TemporaryDirectory() as td:  # honours TMPDIR; bwrap masks via tmpfs, not a /tmp bind
        ...
        WT.assemble_workspace(bundle, ws, context=C.CONTEXT)
        (ws / "SMOKE.md").write_text(smoke)
        # EXACTLY the driver's flags (bypassPermissions so Bash runs; --add-dir for the workspace)
        inner = (
            f"claude --print --model {a.model} --permission-mode bypassPermissions --add-dir {ws} "
            f"--output-format stream-json --verbose < {ws / 'SMOKE.md'}"
        )
        cmd = EX.sandbox_command(inner, ws, bundle, context=C.CONTEXT)
        broker = FL.start_brokers(
            ws,
            FL.BrokerConfig(
                C.CONTEXT,
                TS.resolved_tools(ARM_BUNDLE[a.arm], bundle_dir=C.BUNDLES / ARM_BUNDLE[a.arm]),
                C.HARNESS / ".oracle_timing.json",
            ),
        )  # same as execution.launch: shim + driver-side broker
        primary_error = None
        try:
            print(f"  launching claude inside bwrap + broker (timeout {a.timeout}s)...")
            try:
                r = subprocess.run(["bash", "-c", cmd], cwd=str(ws), capture_output=True, text=True, timeout=a.timeout)
            except subprocess.TimeoutExpired:
                print("  TIMEOUT — agent did not finish")
                return 1
        ...
        markers = {
            "mlir/cmake/ninja/g++ ran": ("LLVM" in blob or "cmake version" in blob),
            ...
            "self-check via broker returns verdict": (
                "selfcheck_ok" in blob
                or '"n_passed"' in blob
                or '"all_pass"' in blob
