...
def main(argv: list[str] | None = None) -> int:
    ...
    for k in range(a.start_index, a.start_index + a.n):
        ...
        if r:
            results.append(r)
            print(
                f"  -> public={r['public']} hidden={r['hidden']} tier={r['tier']} "
                f"rounds={r['n_rounds']} wall={r['wall_s']}s cost=${r['cost_usd']} "
                f"clean={r['answer_access_clean']}",
                flush=True,
            )

    n_full = sum(1 for r in results if r["public"] == "4/4" and r["hidden"] == "3/3")
    agg = {
        "public_4of4_AND_hidden_3of3": f"{n_full}/{len(results)}",
    ...
    md = [
        f"# Repeatability — {a.arm} ({a.model}), n={len(results)}",
        "",
    ...
    for r in results:
        md.append(
            f"| {r['run_id']} | {r['public']} | {r['hidden']} | {r['tier']} | {r['n_rounds']} | "
