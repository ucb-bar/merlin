...
S.use_style()
COL = {
    "baseline-C++": S.COLOR["golden"],
    "merlin-xDSL": S.COLOR["merlin_targetgen"],
...
def fig_circt():
    ...
    for i in range(2):
        for j in range(2):
            ...
            ax.text(
                j, i, f"{labs[i][j]}\n\n{grid[i, j]}", ha="center", va="center", fontsize=9, fontweight="bold", color=c
            )
    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title("CIRCT static check vs sim outcome\n(119 arm×round×capsule points)", fontsize=11)
    # the decision bar
    ax2.axis("off")
    ...
    S.caption(
        fig,
        "Replayed the EXACT live CIRCT screen (rtl_check_runner.prescreen) over every emitted "
...
def fig_effort():
    ...
    mets = [
        ("cost ($)", "cost_usd", 1),
        ("tokens (M)", "tokens_total", 1e-6),
        ("tool-calls", "tool_calls", 1),
        ("self-checks", "n_self_checks", 1),
    ...
    for ax, (lab, key, sc) in zip(axes, mets):
        ...
        ax.set_xticklabels(
            [a.replace("merlin", "m").replace("baseline-", "") for a in arms], rotation=30, ha="right", fontsize=8
        )
        ax.set_title(lab, fontsize=11)
        for i, v in enumerate(vals):
            ax.text(
                i, v, f"{v:.0f}" if v >= 10 else f"{v:.1f}", ha="center", va="bottom", fontsize=8, fontweight="bold"
            )
    fig.suptitle(
        "Effort to reach 25/25 verilator-correct — all 3 arms converged (N=1/arm)",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
    S.caption(
        fig,
        "All three reach functional+numerical correctness (25/25 incl. hidden). baseline-C++ ≈ "
