...
def _classify_cmd(cmd: str, result: str) -> str:
    ...
    out_of_ws_answer = any(
        any(h in m.lower() for h in _ANSWER_PATH_HINTS) and "_qa_ws/" not in m for m in re.findall(r"/scratch\S+", cmd)
    )
...
# Recovery policy is shared with the functional driver and reports.
from merlin_experiments.phase1.recovery import rounds_rate_limited as _rounds_rate_limited  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    agg = {
        ...
        "n_total": len(results),
        "n_valid": len(valid),
        "n_blocked_rate_limit": len(blocked),
    ...
    md = [
        f"# Repeatability — {a.arm} (claude-opus-4-8), rate-limit-aware",
        "",
        ...
        "",
        f"- **Valid full-pass rate (public 4/4 AND hidden 3/3, blocked runs excluded): {agg['valid_pass_rate']}**",
        ...
        f"- wall(s) valid: {agg['wall_seconds_valid']}",
        f"- cost$ valid: {agg['cost_usd_valid']}",
    ...
    for r in results:
        md.append(
            f"| {r['run_id']} | **{r['outcome']}** | {r['public']} | {r['hidden']} | {r['tier']} "
    ...
    print(
        f"reclassified {len(results)} runs: valid_pass_rate={agg['valid_pass_rate']} "
        f"({agg['n_blocked_rate_limit']} blocked); answer-access leak any={agg['answer_access_leak_any']}"
    )
    for r in results:
        print(
            f"  {r['run_id']}: {r['outcome']} (rej/work {r['rounds_rejected']}/{r['rounds_worked']}, "
