...
def _shim(ws, *args, timeout=60):
    return subprocess.run(
        [PY, str(ws / "simjob.py"), *args], cwd=str(ws), capture_output=True, text=True, timeout=timeout
    )
...
def main():
    if not (REF / "manifest.yaml").exists():
        print("FAIL: reference backend missing")
        return 1
    env = dict(os.environ)
    ...
    with tempfile.TemporaryDirectory() as td:  # honours TMPDIR
        ...
        shutil.copy(module_source_path("merlin_experiments.phase1.tools.simjob"), ws / "simjob.py")
        ...
        try:
            ...
            t0 = time.time()
            r = _shim(ws, "submit", "--sim", "spike", "--capsules", "A1_mvin_mvout")
            dt = time.time() - t0
            ...
            _ok(
                "spike job returns a complete screen-only L2 pass on known-good backend",
                st == "done" and screen_ok,
                f"state={st} {screen_detail}",
            )
            # 5. malicious job ended as error (rejected), not run
            stbad = None
            for _ in range(6):
                pb = _shim(ws, "poll", "--job-id", jbad)
                stbad = json.loads(pb.stdout)["state"]
            ...
            if jevil:
                for _ in range(6):
                    ...
                    time.sleep(1)
            _ok(
                "bad --sim rejected by the trusted broker",
            ...
            vstate = None
            for _ in range(12):
                pv = _shim(ws, "poll", "--job-id", jv)
                vstate = json.loads(pv.stdout)["state"]
            ...
            _ok(
                "verilator job accepted + queued/scheduled without rejection",
                bool(jv) and vstate in ("queued", "running", "done"),
                f"state={vstate}",
            )
        finally:
