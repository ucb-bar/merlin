...
from merlin_experiments.phase1 import workspace_transport as WT  # noqa: E402
...
def run_canary(
    ...
    if arm:
        ...
        bundle = RX._load_bundle(arm, bundle_id=bundle_id)
        assembled = WT.assemble_copy_workspace(bundle, ws, context=R.C.CONTEXT)
        bundle_info.update(
            {
    ...
    try:
        mask = WT.probe(ws, bundle, sandbox, context=R.C.CONTEXT)
    ...
    rc, tpath = CA.run_round(
        ...
        timeout,
        effort="low",
        sandbox_command=partial(EX.sandbox_command, context=R.C.CONTEXT),
