...
(phase1.feedback.certification._l3_barrier_decision) AND simulates the barrier orchestration using it.
Deterministic, no agent, no sim."""

from __future__ import annotations

from merlin_experiments.phase1.feedback import certification as L
...
def main():
    ...
    _ok(
        "recovery: fail,fail,pass -> converged via re-iteration",
        out == "converged" and att == 3 and fr == 2,
    ...
    _ok(
        "happy: first L3 pass -> converged, no fix rounds",
        out == "converged" and att == 1 and fr == 0,
    ...
    _ok(
        "budget: persistent L3 fail -> stop at max_rounds (honest, not terminal-at-first-fail)",
        out == "max_rounds" and rnd == 3 and fr >= 1,
        f"outcome={out} rnd={rnd} fixrounds={fr}",
    )
    ...
    _ok(
        "timeout-then-pass -> recovers (timeout is non-terminal)",
        out == "converged" and fr == 1,
