...
class Journal:
    ...
    def stage(self, name: str, status: str, detail: str, **extra):
        entry = {"stage": name, "status": status, "detail": detail, "at_utc": _utc(), **extra}
    ...
    def _render(self) -> str:
        lines = [
            f"# Overnight chain — {self.doc['tag']} ({self.doc['target']})",
            "",
        ...
        if not self.doc["stages"]:
            ...
            lines += [f"**status: {len(bad)} stage(s) did not complete — {', '.join(s['stage'] for s in bad)}.**", ""]
        else:
...
def stage_preflight(j: Journal, logs: Path, *, strict: bool) -> bool:
    ...
    rc, tail = _run(
        [PY, "build_tools/scripts/check_conformance_coverage.py", "--target", C.TARGET],
        logs / "coverage.log",
        timeout=3600,
    )
    j.stage("preflight/coverage", "ok", f"recorded (exit {rc}); see coverage.log", note=f"```\n{tail}\n```")
...
def stage_functional(j: Journal, logs: Path, a) -> Path | None:
    ...
    if a.dry_run:
        j.stage("functional", "skipped", "dry-run: not launched", note="```\n$ " + " ".join(cmd) + "\n```")
        return run_dir
    t0 = time.time()
...
def stage_grade_and_freeze(j: Journal, logs: Path, run_dir: Path, a) -> bool:
    ...
    j.stage("grade", "ok" if rc == 0 else "failed", f"agg_ab_results exit {rc}", note=f"```\n{tail}\n```")
    ...
    rc2, tail2 = _run(
        [PY, "-m", "merlin_experiments.phase1.feedback.freeze", "--repo", str(REPO), "--run-dir", str(run_dir)],
        logs / "freeze.log",
        timeout=1800,
    )
    j.stage(
        "freeze",
        "ok" if rc2 == 0 else "failed",
...
def stage_calibration(j: Journal, logs: Path, a) -> bool:
    ...
    j.stage("calibration", "ok" if rc == 0 else "failed", f"perf_calibrate exit {rc}", note=f"```\n{tail}\n```")
    return rc == 0
...
def _perf_capsules_exist() -> tuple[bool, str]:
    ...
    try:
        ...
        desc = REPO / "merlin" / "experiments" / "capsule_bench" / "targets" / C.TARGET / "target_experiment.yaml"
    ...
    return False, (
        f"no comparison group in {C.TARGET}'s graded roots has two members (groups seen: "
...
def stage_performance(j: Journal, logs: Path, a) -> bool:
    ...
    if not ready:
        j.stage(
            "performance",
            "refused",
            "the performance capsule families do not exist",
            note=f"{why}\n\n"
        ...
        return False
    j.stage(
        "performance", "refused", "capsules exist but the optimization driver is not wired in this script", note=why
    )
    return False
