...
def _rtl_check_checks(target: str) -> list[dict]:
    ...
    corpus = _target_experiment(target).capsule_corpus
...
def _arm_superset_check(target: str) -> list[dict]:
    ...
    try:
        b = _target_experiment(target).resource_path("input_bundles")
    except (FileNotFoundError, ValueError) as exc:
        return [_ok("arm-4 ⊇ arm-3 tool grants", False, str(exc))]
...
def _target_experiment(target: str):
    ...
    descriptor = C.DESCRIPTOR if target == C.TARGET else descriptor_for(target)
    if descriptor is None or not descriptor.is_file():
        raise FileNotFoundError(f"target descriptor is absent for {target}: {descriptor}")
    experiment = load_target_experiment(descriptor)
    if experiment.target != target:
        raise ValueError(f"selected descriptor declares {experiment.target}, not requested target {target}")
    return experiment
...
def _asm_probe_mnemonic(target: str) -> str:
    ...
    """
    try:
        from merlin_experiments.phase1.brokers import isa_tools as _IB
...
def sandbox_authoring_readiness(target: str, arm: str = "merlin_assisted_rtlchecks") -> dict:
    ...
    try:
        ...
        from merlin_experiments.phase1.workspaces import workspace_parent

        from merlin.common import storage_lifecycle
        from merlin.common.paths import module_source_path

        qa_root = workspace_parent(target, arm)
        qa_root.mkdir(parents=True, exist_ok=True)
        root = Path(tempfile.mkdtemp(prefix="tooling-readiness-", dir=qa_root))
        workspace_lease = storage_lifecycle.acquire(root, owner="phase1-tooling-readiness")
        ws = root / "workspace"
        snapshot = BW.materialize_bundle_inputs(ws, tool_bundle, repo=C.REPO, descriptor=te)
        ...
        for log in logs:
            log.close()
        if root is not None and workspace_lease is not None:
            workspace_lease.close()
            with storage_lifecycle.removal_guard(root, require_terminal=True):
                if ws is not None:
