...
def test_starter_kit():
    ...
    from merlin.targetgen.oot_starterkit.verify import structural_checks
    ...
    # Generic starter-kit verification checks decode legality. Protocol ordering is selected
    # target support's responsibility, never inferred from instruction-name substrings here.
    try:
        legal = {"SETTING": 0, "EXECUTE": 4}
        bad_trace = {"instructions": [{"name": "EXECUTE", "funct": 99}, {"name": "UNKNOWN", "funct": "UNKNOWN"}]}
        clean_trace = {"instructions": [{"name": "SETTING", "funct": 0}, {"name": "EXECUTE", "funct": 4}]}
        caught = structural_checks(bad_trace, legal)
        clean = structural_checks(clean_trace, legal)
        _ok(
            "verify.structural_checks checks declared codes, not target protocol",
        ...
        r = subprocess.run(
            inherited_python_command(
                [
                    PY,
                    "-m",
                    "merlin_experiments.phase1.feedback.selfcheck",
                    "--descriptor",
                    str(C.DESCRIPTOR),
                    "--repo",
                    str(C.REPO),
