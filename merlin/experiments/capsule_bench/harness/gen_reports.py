...
import _common as C
import yaml
from merlin_experiments.phase1.recovery import rounds_rate_limited as _rounds_rate_limited
...
def main() -> int:
    ...
    idx = [
        f"# Run index ({C.TARGET} capsule-bench)",
        "",
        f"{len(rows)} run(s).",
        "",
    ...
    for r in rows:
        idx.append(
            f"| {r['run_id']} | {r['arm']} | {r['model']} | {r.get('integrity_status')} | "
    ...
    cmp = [
        f"# Comparison table ({C.TARGET} capsule-bench)",
        "",
