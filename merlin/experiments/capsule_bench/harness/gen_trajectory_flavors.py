...
ACT_LAB = {
    ...
    "write": "writing code",
    "bash": "bash / shell",
    "tool": "tool wait (verilator·CIRCT)",
}
...
def load_arm(rel):
    ...
    for tp in sorted(glob.glob(str(run / "rounds" / "round_*.transcript.jsonl"))):
        ...
        for l in open(tp):
            ...
            if e.get("type") == "result":
                ...
                tu = next(
                    (c for c in (m.get("content") or []) if isinstance(c, dict) and c.get("type") == "tool_use"), None
                )
                if tu is not None:
                    nm = (tu.get("name") or "").lower()
                    s = json.dumps(tu.get("input", {}))
                    if nm == "read":
                        cat = "read"
    ...
    for ri, r in enumerate(rounds):
        ...
        s_ou = sum(x[4] for x in ms) or 1
        wsum = sum(x[5] for x in ms) or 1
...
def _fine_milestones(run, total):
    ...
    for r in rows:
        if str(r.get("capsules")) != "all":
            continue
        nc = r.get("n_capsules") or 0
        np_ = r.get("n_passed") or 0
        if nc < 20:
            continue
        if np_ > best:
            best = np_
...
def _rounds(ax, d, topax=None, fs=12.5, labels=True):
    ...
    while i < d["n"]:
        ...
        tx.text(
            ...
            0.022,
            lab,
            transform=tx.get_xaxis_transform(),
            ha="center",
            va="bottom",
            fontsize=fs,
            fontweight="bold",
...
def _chip(ax, d, compact=False, y=1.045, fs=12.5):
    ...
    if compact:
        ...
        txt = (
            f"{d['total']:.0f} min active   ·   ${d['cum_cost'][-1]:.0f}   ·   {d['tot_tok'] / 1e6:.0f}M tok   ·   "
...
def _basics(ax, d, lab, last, fs=1.0):
    title(ax, lab, fs=20 * fs, pad=10)
    ax.set_xlim(0, d["total"] * 1.01)
...
def _line_handles(include_spend=True, rate=False):
    ...
    h += [
        Line2D([0], [0], color=L_INPUT, lw=3.2, label=pre + "input"),
...
def fl_A(DATA):
    ...
    for ax, (d, lab) in zip(axes, DATA):
        ...
        ax.set_ylim(0, 1)
        ax.set_ylabel("activity share", fontsize=14)
        axT = ax.twinx()
        axT.spines["top"].set_visible(False)
        axT.spines["right"].set_color(INK)
        _token_lines(axT, d, log=True)
        axT.set_ylabel("cum. tokens (log)", fontsize=14)
        axT.tick_params(labelsize=11)
        _spend_axis(ax, d)
        _rounds(ax, d)
        _milestones(ax, d)
        _chip(ax, d)
        _basics(ax, d, lab, ax is axes[-1])
    h = [Patch(fc=ACT_COL[a], alpha=0.6, label=ACT_LAB[a]) for a in ACTS] + _line_handles()
    _legend(
        fig, h, "Authoring trajectory — activity share + cumulative tokens (by type) + spend", "fig_traj_A_share_spend"
    )
...
def fl_B(DATA):
    ...
    for ax, (d, lab) in zip(axes, DATA):
        ...
        axT.spines["right"].set_color(INK)
        _token_lines(axT, d, log=True)
        axT.set_ylabel("cum. tokens by type (log)", fontsize=14)
        axT.tick_params(labelsize=11)
        _spend_axis(ax, d)
        _rounds(ax, d)
        _milestones(ax, d)
...
def fl_C(DATA):
    ...
    for ax, (d, lab) in zip(axes, DATA):
        ...
        axT.set_yscale("log")
        axT.set_ylabel("token rate (tok/min, log)", fontsize=14)
        axT.tick_params(labelsize=11)
        _spend_axis(ax, d)
        _rounds(ax, d)
        _milestones(ax, d)
...
def fl_F(DATA):
    ...
    for ax, (d, lab) in zip(axes, DATA):
        ...
        axT.set_ylim(0, d["tot_tok"] * 1.18 or 1)
        axT.set_ylabel("cum. tokens (stacked by type)", fontsize=14)
        axT.tick_params(labelsize=11)
        _spend_axis(ax, d)
        _rounds(ax, d)
        _milestones(ax, d)
    ...
    h = [Patch(fc=ACT_COL[a], alpha=0.55, label=ACT_LAB[a]) for a in ACTS] + [
        Patch(fc=L_CACHE, alpha=0.32, label="cache-read"),
        Patch(fc=L_INPUT, alpha=0.32, label="input"),
        Patch(fc=L_OUTPUT, alpha=0.32, label="output"),
        Line2D([0], [0], color=L_TOTAL, lw=3.6, label="total tokens"),
...
def _panel_rate(
    ...
    *,
    ...
    if activity == "stack":
        ...
        ax.set_ylim(0, 1)
        ax.set_ylabel("activity share", fontsize=14 * fs)
    elif activity == "strip":
        frac = 0.16  # confine the activity bands to a slim ribbon along the panel floor
        ax.stackplot(
            g,
            *[sh[a] * frac for a in ACTS],
            colors=[ACT_COL[a] for a in ACTS],
            alpha=min(band_alpha + 0.18, 0.7),
            zorder=1,
        )
        ax.set_ylim(0, 1)
        ax.set_yticks([])
    ...
    axT.set_yscale("log")
    axT.set_ylabel("token rate (tok/min, log)", fontsize=14 * fs)
    axT.tick_params(labelsize=11 * fs)
...
def _rate_option(DATA, fname, subtitle, **panel_kw):
    ...
    h = _rate_legend_handles(activity=panel_kw.get("activity", "stack"), show_spend=panel_kw.get("show_spend", True))
...
def _scale_bar(ax, d, minutes=20, y=-0.185, fs=17, color=BLUE):
    ...
    ax.plot([x0, x1], [y, y], transform=tr, color=color, lw=4.6, zorder=27, solid_capstyle="butt", clip_on=False)
    for xx in (x0, x1):  # end ticks
        ax.plot([xx, xx], [y - 0.032, y + 0.032], transform=tr, color=color, lw=4.6, zorder=27, clip_on=False)
    ax.text(
        x1 + 0.012 * d["total"],
        y,
        f"{minutes} min",
...
def _rate_single(d, lab, fname):
    ...
    fig.legend(
        ...
        facecolor="white",
        edgecolor="#d9cfc0",
        bbox_to_anchor=(0.5, bottom - 0.22),
        columnspacing=1.3,
        handlelength=1.8,
        borderpad=0.7,
    )
...
def _panel_rate_staged(ax, d, lab, last, fs, stage):
    ...
    axT.set_yscale("log")
    axT.set_ylim(1, rmax * 1.35)  # fixed range so the frame is identical every stage
    axT.set_ylabel("token rate (tok/min, log)", fontsize=14 * fs)
    axT.tick_params(labelsize=11 * fs)
...
def _panel_duration(ax, d, lab, grmax, fs=1.0):
    ...
    axT.spines["right"].set_color(INK)
    for arr, col in ((d["cin"], L_INPUT), (d["cou"], L_OUTPUT)):
        axT.plot(d["t"], np.clip(_rate(arr, d["t"]), 1, None), color=col, lw=3.0 * fs, zorder=8, path_effects=LHALO)
    axT.set_yscale("log")
    axT.set_ylim(1, grmax * 1.4)
    axT.set_ylabel("tok/min (log)", fontsize=12 * fs)
    axT.tick_params(labelsize=11 * fs)
    ...
    ax.set_title(
        f"{lab}    ·    {d['total']:.0f} min", loc="left", color=INK, fontsize=15 * fs, fontweight="bold", pad=6
    )
...
def fl_A_rate_combined_scales(DATA):
    ...
    h = [Patch(fc=ACT_COL[a], alpha=0.40, label=ACT_LAB[a]) for a in ACTS] + _rate_legend_handles(
        activity="none", show_spend=False
    )
...
def fl_A_rate_short(DATA):
    ...
    _legend(
        fig,
        h,
        "Authoring trajectory — activity share + token consumption RATE + spend",
        "fig_traj_A_rate_short",
        sup_y=0.962,
        leg_y=0.02,
    )
...
def fl_B_rate(DATA):
    ...
    for ax, (d, lab) in zip(axes, DATA):
        ...
        axT.set_ylim(0, (base.max() * 1.2) or 1)
        axT.set_ylabel("token rate (tok/min, stacked)", fontsize=14)
        axT.tick_params(labelsize=11)
        _spend_axis(ax, d)
        _rounds(ax, d)
        _milestones(ax, d)
        ...
        _basics(ax, d, lab, ax is axes[-1])
    h = [Patch(fc=ACT_COL[a], alpha=0.40, label=ACT_LAB[a]) for a in ACTS] + [
        Patch(fc=L_CACHE, alpha=0.5, label="cache-read rate"),
        Patch(fc=L_INPUT, alpha=0.5, label="input rate"),
        Patch(fc=L_OUTPUT, alpha=0.5, label="output rate"),
        Line2D([0], [0], color=L_TOTAL, lw=2.6, label="total rate"),
        Line2D([0], [0], color=L_SPEND, lw=3.6, ls=(0, (6, 2)), label="spend ($)"),
        Line2D([0], [0], color=GOLD, lw=2.6, ls=(0, (4, 3)), label="milestone"),
    ]
...
def _cmp_handles():
    return [
        Line2D([0], [0], color=c, lw=4.4, ls=ls, marker=m, mfc=c, mec=INK, ms=13, label=n) for n, c, m, ls in RUN_STYLE
    ]
...
def _draw_tests(ax, DATA, fs=1.0):
    ...
    for (d, _l), (name, col, mk, ls) in zip(DATA, RUN_STYLE):
        ...
        ax.scatter([xs[k] for k in ch], [ys[k] for k in ch], s=95, color=col, ec=INK, lw=1.2, zorder=6, marker=mk)
        ...
        ax.text(
            ...
            ha="left",
            zorder=9,
            path_effects=_HALO_TXT(),
        )
    ax.set_xlim(0, xmax * 1.13)
    ax.set_ylim(-0.6, 21.2)
    ax.set_yticks([0, 5, 10, 15, 20])  # integer ticks only — no .5
    ax.set_xlabel("Time (min)", fontsize=23 * fs)
    ax.set_ylabel("tests passing (of 20)", fontsize=23 * fs)
...
def fl_compare_tests_facets(DATA):
    ...
    for i, (ax, (d, _l), (name, col, mk, ls)) in enumerate(zip(axes, DATA, RUN_STYLE)):
        ...
        ax.scatter([xs[k] for k in ch], [ys[k] for k in ch], s=150, color=col, ec=INK, lw=1.6, zorder=6, marker=mk)
...
def _draw_cost(ax, DATA, fs=1.0):
    ...
    for (d, _l), (name, col, mk, ls) in zip(DATA, RUN_STYLE):
        ...
        ax.annotate(
            f"${d['cum_cost'][-1]:.0f} · {d['tot_tok'] / 1e6:.0f}M",
            (xe, ye),
            xytext=(0, 18),
            textcoords="offset points",
            color=col,
            ...
            ha="center",
            zorder=9,
            path_effects=_HALO_TXT(3.8),
        )
    ax.set_xlim(0, xmax * 1.10)
    ax.set_ylim(0, ymax * 1.24)
    ax.set_xlabel("Time (min)", fontsize=23 * fs)
    ax.set_ylabel("cumulative cost (USD)", fontsize=23 * fs)
    ax.tick_params(labelsize=19 * fs)
...
def fl_compare_efficiency(DATA):
    ...
    for (d, _l), (name, col, mk, ls) in zip(DATA, RUN_STYLE):
        ...
        ax.annotate(
            f"{name}\n{tp}/20 tests · ${c:.0f} · {tok:.0f}M tok · {t:.0f} min",
            (t, c),
            xytext=(16, 18),
            textcoords="offset points",
            fontsize=17,
...
def fl_compare_overview(DATA):
    ...
    _draw_tests(axL, DATA, fs=1.12)
    _draw_cost(axR, DATA, fs=1.12)
    fig.legend(
        handles=_cmp_handles(),
        loc="lower center",
        ncol=4,
        fontsize=21,
        frameon=True,
...
def main():
    ...
    for fn in (
        ...
        fl_A_rate_options,
        fl_A_rate_arms,
        fl_A_rate_combined_scales,
        fl_combined_duration,
        fl_run1_buildup,
        fl_compare_tests,
        fl_compare_cost,
        fl_compare_efficiency,
        fl_compare_overview,
