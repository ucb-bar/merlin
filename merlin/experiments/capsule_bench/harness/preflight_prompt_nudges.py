...
TASK = load_target_experiment(C.DESCRIPTOR).resource_path("task/TASK_realistic.md")
CHECKS = {
    "READY precondition (spike clean first)": re.compile(
        r"FULLY clean.*READY_FOR_BARRIER|do \*\*NOT\*\* write `READY_FOR_BARRIER`|premature READY", re.I | re.S
    ),
