...
def _source_masks(root: Path) -> dict[str, Path]:
    ...
    for item in MODULE_ACCESS:
        if item.identity not in required:
            continue
        locations = {path.resolve() for path in module_locations(root, item)}
        if not locations:
            raise SystemExit(f"sandbox preflight cannot probe absent required module {item.identity}")
        for path in sorted(locations):
            masks[f"{item.origin} {item.identity}: {path.relative_to(root.resolve())}"] = path
    return masks
...
def _bwrap(ws: Path, bundle: dict, inner: str) -> str:
    ...
    that assembles its own weaker sandbox is not evidence about the real one; ``execution.sandbox_command`` applies the
    derived answer-mask pass, which skips a surface the base already hides."""
    return EX.sandbox_command(inner, ws, bundle, context=C.CONTEXT)
...
def main(argv=None):
    ...
    with tempfile.TemporaryDirectory() as td:  # honours TMPDIR; bwrap masks via tmpfs, not a /tmp bind
        ...
        masks.update(_source_masks(C.REPO))
        ...
        for src, dstn in (
            (module_source_path("merlin_experiments.phase1.tools.selfcheck"), "agent_selfcheck.py"),
            (module_source_path("merlin_experiments.phase1.tools.simjob"), "simjob.py"),
        ):
            d = ws / dstn
        ...
        broker = subprocess.Popen(
            [
                ...
                str(C.REPO),
                "--timing-file",
                str(SCRIPTS_DIR / ".oracle_timing.json"),
                "--ws",
                str(ws),
            ],
        ...
        try:
            ...
            reference = next(
                path for label, path in _source_masks(C.REPO).items() if "merlin.runtime.reference:" in label
            )
            rc2, oout, _ = _run(
                ws,
                bundle,
                f'head -c1 "{reference}" >/dev/null 2>&1 && echo LEAK || echo masked',
                timeout=60,
            )
