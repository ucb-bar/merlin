...
def main(argv=None):
    ...
    try:
        ...
        provider = EX.ProviderConfig(driver=a.driver, provider=a.provider)
        _agent = (
            f"driver={a.driver} -> resolved={EX.resolve_driver(a.model, config=provider)} model={a.model} "
            f"billing={EX.billing_mode(a.model, config=provider)}"
        )
