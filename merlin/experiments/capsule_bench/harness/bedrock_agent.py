    ...
    "tools": [
        {
            "toolSpec": {
                "name": "run_bash",
                "description": "Run a bash command in your isolated workspace (cwd = workspace root). For build/test/inspect — "
        ...
        },
        {
            "toolSpec": {
                "name": "read_file",
                "description": "Read a text file (relative to the workspace root, or an allowed contract path). Answer files "
                "are masked.",
                "inputSchema": {
        ...
        },
        {
            "toolSpec": {
                "name": "self_check",
                "description": "Grade your CURRENT submission/ against the REAL target oracle and get REDACTED feedback: per-tier "
...
_DELEGATE_TOOL = {
    "toolSpec": {
        "name": "delegate",
        "description": "Delegate a well-scoped, self-contained sub-task to a cheaper/faster model to save your own turns "
...
def _write_file(ws: Path, rel: str, content: str) -> str:
    ...
    if os.environ.get(_BW.PINNED_SUBMISSION_READ_ONLY_ENV, "").strip() == "1" and (
        p == submission or p.is_relative_to(submission)
    ):
...
def _run_subagent(
    ...
    *,
    ...
    sub_tools = {
        "tools": [t for t in _TOOLS["tools"] if t["toolSpec"]["name"] in ("run_bash", "write_file", "read_file")]
    }
    sys = [
        {
