...
ARMS = {
    "rb_abc4": ("raw_baseline", "baseline-C++"),
...
def circt_vs_sim() -> dict:
    ...
    for rid, (_, label) in ARMS.items():
        for rnd, d in round_dirs(rid):
            ...
            cell = (
                "true_neg"
                if (circt_ok and sim_pass)
                else "true_pos"
                if (not circt_ok and not sim_pass)
                else "false_clean"
                if (circt_ok and not sim_pass)
                else "false_alarm"
            )
            conf[cell] += 1
            if cell == "false_clean":
                false_clean_cases.append(
                    {
                        "arm": label,
                        "round": rnd,
                        "capsule": cap,
    ...
    return {
        "confusion": conf,
        "n_points": n,
        "false_clean_count": conf["false_clean"],
...
def main():
    ...
    print("== A. struggle matrix ==")
    M = struggle_matrix()
    (OUT / "struggle_matrix.json").write_text(json.dumps(M, indent=2))
    for label, caps in M.items():
        hard = sorted(
            [(c, i["n_rounds_failing"]) for c, i in caps.items() if i["n_rounds_failing"] > 0], key=lambda x: -x[1]
        )
    ...
    print(
        f"  ★ FALSE-CLEAN (CIRCT-ok but sim-FAIL): {cs['false_clean_count']}  -> "
