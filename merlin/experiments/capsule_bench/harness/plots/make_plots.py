...
ACT_EDGES = {
    T.KIND_READ: S.GREEN_E,
    T.KIND_EDIT: S.SALMON_E,
    T.KIND_BASH: S.BLUE_E,
    T.KIND_THINK: S.LAVENDER_E,
    T.KIND_RESULT: S.AMBER_E,
}
ACT_LABEL = {
    T.KIND_READ: "Read",
    T.KIND_EDIT: "Edit/Write",
    T.KIND_BASH: "Bash",
    T.KIND_THINK: "Thinking",
    T.KIND_RESULT: "Tool result",
}
PLANE_COLORS = {
    "trace_check": S.AMBER,
    "numeric_golden": S.SALMON,
    "numeric": S.SALMON,
    "spike": S.BLUE,
    "command_buffer": S.LAVENDER,
    "verilator": S.GREEN,
    "runner_internal": S.GREY,
    "oracle_unavailable": "#cfcfcf",
    "build": S.MUSTARD,
}
...
def _draw_bg(ax, x, kinds, rounds, tl, sigma, bg: str) -> None:
    ...
    if bg == "rounds":
        ...
        for bi in range(nb):
            ...
            d = ind[sel].mean()
            if d > 0:
                ax.axvspan(edges[bi], edges[bi + 1], color=PHASE_THINK, alpha=float(min(0.7, d * 2.2)), lw=0, zorder=0)
        sep()
    elif bg == "turnphase":
...
def _draw_activity_panel(ax, run: dict, bg: str = "rounds", sigma_frac: float = SIGMA_FRAC):
    ...
    tl = run["timeline"]
    if not tl:
        ax.set_title(f"{run['run_id']} — no transcript yet")
        ax.axis("off")
        return None
    x = _global_x(tl)
    ...
    if bg == "stream":
        ...
        tot = np.sum(list(ys.values()), axis=0)
        tot[tot == 0] = 1.0
        base = np.zeros_like(x, dtype=float)
        for kind, c in ACT_COLORS.items():
            frac = ys[kind] / tot
            ax.fill_between(x, base, base + frac, color=c, alpha=0.85, lw=0, label=ACT_LABEL[kind], zorder=3)
            base = base + frac
        ax.set_ylim(0, 1.0)
        ax.set_ylabel("activity composition", fontsize=9)
    else:
        ymax = 0.0
        for kind, c in ACT_COLORS.items():
            ...
            if ind.sum() == 0:
                continue
            y = _gaussian_smooth(ind, sigma)
            ymax = max(ymax, float(y.max()))
            faint = kind == T.KIND_RESULT
            ax.plot(
                x, y, color=c, lw=1.6 if faint else 2.1, alpha=0.45 if faint else 0.95, label=ACT_LABEL[kind], zorder=4
            )
        ax.set_ylim(0, max(0.45, ymax * 1.18))
        ax.set_ylabel("activity prevalence", fontsize=9)

    ax.set_xlim(0, 100)
    ax.set_xlabel("Progress through transcript (%)", fontsize=9)
    ...
    axr = ax.twinx()
    axr.set_ylim(0, 1.02)
    axr.set_ylabel("capsules passing", fontsize=9, color=OUTCOME)
    axr.tick_params(labelsize=8, colors=OUTCOME)
    ...
    if rmeta:
        ...
        for r in np.unique(rounds):
            ...
            if rr and rr.get("n_passed") is not None and rr.get("n_capsules"):
                ox.append(x[m].max())
                oy.append(rr["n_passed"] / rr["n_capsules"])
        if ox:
...
def _fig1_legend(fig, ncol=4):
    ...
    handles += [
        Line2D([], [], color=OUTCOME, lw=2.8, marker="o", label="Capsules passing (right axis)"),
        Line2D([], [], color=RL_MARK, lw=1.4, ls=":", label="Rate-limit reject"),
        Patch(fc=PHASE_ACT, label="act/round"),
        Patch(fc=PHASE_ENV, label="environment"),
        Patch(fc=PHASE_THINK, label="thinking"),
    ]
    fig.legend(handles=handles, loc="lower center", ncol=ncol, fontsize=9, frameon=False, bbox_to_anchor=(0.5, -0.02))
...
def fig1_activity(runs: list[dict], out: Path, bg: str = "rounds") -> Path:
    ...
    for idx, run in enumerate(runs):
        ...
        ax.set_title(f"{run['run_id']}  ({run['arm']})" + ("  ✓ converged" if conv else ""), fontsize=11, loc="left")
...
def fig1_compare(run: dict, out: Path, modes=("rounds", "reasoning", "turnphase", "stream")) -> Path:
    """Same run rendered under every background mode, side by side, so the options can be compared."""
    titles = {
        "rounds": "A) Round bands — loop iteration #",
...
def fig2_planes(runs: list[dict], out: Path) -> Path:
    ...
    for idx, run in enumerate(runs):
        ...
        if npass:
            ax.plot(
                [rd["round"] for rd in rmeta if rd["n_passed"] is not None],
                npass,
                color=OUTCOME,
                lw=2.2,
                marker="o",
        ...
        ax.set_ylabel("# capsules", fontsize=8)
        ax.set_xticks(xr)
    ...
    for k in range(len(runs), nrow * ncol):
        axes[k // ncol][k % ncol].axis("off")
    fig.suptitle("Failure plane by round — how each arm burns down (or stalls on) failure classes", fontsize=12)
...
def fig3_heatmap(runs: list[dict], out: Path) -> Path:
    ...
    for idx, run in enumerate(runs):
        ...
        if not caps:
            ax.set_title(f"{run['run_id']} — no per-capsule grades yet")
            ax.axis("off")
            continue
        order = sorted(
            caps.keys(),
            key=lambda c: (CLASS_ORDER.index(caps[c]["class"]) if caps[c]["class"] in CLASS_ORDER else 99, c),
        )
        ...
        ax.set_xticklabels([f"R{j}" for j in range(nr)], fontsize=7)
        # class separators
        prev = None
        for i, c in enumerate(order):
            cl = caps[c]["class"]
    ...
    for k in range(len(runs), nrow * ncol):
        axes[k // ncol][k % ncol].axis("off")
    handles = [
        Patch(fc=S.GREEN, ec=S.GREEN_E, label="pass"),
...
def fig4_summary(audit: dict, runs: list[dict], out: Path) -> Path:
    ...
    if cov and backends:
        ...
        ax.set_ylabel("fraction of class passing")
        ax.set_ylim(0, 1.12)
        ...
        ax.set_title(f"Coverage by workload class\n(full_suite_audit.json: {_n} capsules, backends: {_bk})", fontsize=8)
        ...
        if zero:
            S.callout(
                ax,
                "never implemented\n(" + ", ".join(zero) + ")",
                xy=(xpos[classes.index(zero[0])] + w / 2, 0.02),
                xytext=(len(classes) * 0.45, 0.5),
                color=S.SALMON_E,
            )
        S.style_axes(ax)
    else:
        ax.axis("off")
        ax.set_title("coverage: full_suite_audit.json not found", fontsize=9)
    ...
    if mat and backends:
        ...
        ax.set_ylabel("L3 verilator cycles")
        ax.legend()
        ax.set_title("Cycle A/B per capsule (lower = faster RTL)")
        S.style_axes(ax)
    else:
        ax.axis("off")
        ax.set_title("cycles: audit matrix unavailable", fontsize=9)
    ...
    ax.set_ylabel("rounds", color=S.BLUE_E)
    ax2.set_ylabel("est. cost $", color=S.MUSTARD_E)
    ax2.grid(False)
    ...
    if labs:
        ...
        S.callout(
            ax,
            "cache reads ≈ 97% of tokens\n→ cheap on cache, $ from output",
            xy=(xpos[-1], base[-1] * 0.6),
            xytext=(xpos[0] + 0.1, base.max() * 0.78),
            color=S.GREEN_E,
        )
        S.style_axes(ax)
    else:
        ax.axis("off")
        ax.set_title("token economics: needs cost_time_toolcalls.yaml", fontsize=9)
...
def fig5_cards(runs: list[dict], out: Path) -> Path:
    ...
    cards = [
        ...
        (
            "PUBLIC  CAPSULES  PASSING",
            [p[0] or 0 for p in best],
            [f"{p[0] or 0}/{p[1] or '?'}" for p in best],
            "best round, of 20",
        ),
        (
            "COST  PER  RUN",
            [c or 0 for c in costs],
            [f"${c:.0f}" if c else "in-flight" for c in costs],
            "USD (finished runs)",
        ),
    ]
    n = len(cards)
    ...
    for ci, (title, vals, labels, unit) in enumerate(cards):
        ...
        for bi, b in enumerate(bars):
            b.set_color(S.FILLS[bi % len(S.FILLS)])
            b.set_edgecolor(S.INK)
            b.set_linewidth(1.4)
        mx = max(vals) if max(vals) > 0 else 1
        for yi, (v, lab) in enumerate(zip(vals, labels)):
            inside = v > 0.28 * mx
            ax.text(
                v - 0.02 * mx if inside else v + 0.02 * mx,
                y[yi],
                lab,
        ...
        ax.set_xticks([])
        ax.set_title(title, fontsize=12.5, loc="left", fontfamily="monospace", weight="bold", color="#3a3a3a", pad=10)
        for s in ax.spines.values():
            s.set_visible(False)
        ax.grid(False)
        ax.text(
            0.995, 1.06, unit, transform=ax.transAxes, ha="right", va="bottom", fontsize=8, color="#999", style="italic"
        )
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--runs",
        default="raw_baseline/rb_full_01,merlin_assisted/merlin_full_01,"
    ...
    ap.add_argument(
        "--bg", default="rounds", help="fig1 background mode: rounds | reasoning | turnphase | stream | plain"
    )
    ap.add_argument(
        "--compare-run",
        default="raw_baseline/rb_full_01",
    ...
    for fn, name in [
        (lambda o: fig1_activity(runs, o, bg=a.bg), "fig1_activity_trajectory.png"),
