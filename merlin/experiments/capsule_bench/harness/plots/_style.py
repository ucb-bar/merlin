...
def callout(
    ax, text, xy, xytext, *, fc="white", ec="#b9b9b9", fontsize=8.5, color="#333333", weight="normal", arrow=True
):
    ...
    ax.annotate(
        ...
        arrowprops=(dict(arrowstyle="-", color=ec, lw=1.0, connectionstyle="arc3,rad=0.0") if arrow else None),
    )
...
def card(fig, rect, facecolor=CREAM, shadow=True):
    ...
    if shadow:
        ...
        fig.patches.append(
            FancyBboxPatch((x, y), w, h, boxstyle=box, transform=sh, fc="#000000", ec="none", alpha=0.16, zorder=-3)
        )
    fig.patches.append(
        FancyBboxPatch(
            (x, y), w, h, boxstyle=box, transform=fig.transFigure, fc=facecolor, ec="#d8cfb8", lw=1.2, zorder=-1
        )
    )
