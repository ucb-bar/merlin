...
def fig_tier_matrix(data: dict, out: Path) -> None:
    ...
    code = {
        "pass": (GREEN, "passed"),
        "L2": (SALMON, "wrong HW encoding  (L2 spike)"),
        "L1": (SALMON_L, "L1"),
        "L0": (AMBER, "wrong numerics  (L0)"),
        "L3": (BLUE_L, "wrong on RTL  (L3)"),
        "-": (SLATE_L, "no tier ran  (contract plane)"),
    }
    ...
    for j, (c, _) in enumerate(cells):
        ...
        for i, cap in enumerate(caps):
            ...
            ax.add_patch(
                plt.Rectangle(
                    (j, i), 0.94, 0.92, facecolor=code.get(v, (SLATE_L,))[0], edgecolor="white", linewidth=1.6
                )
            )
    ax.set_xlim(-0.05, len(cells))
    ax.set_ylim(-0.1, len(caps))
    ...
    ax.set_yticklabels([c.split("_", 1)[0] + "  " + c.split("_", 1)[1].replace("_", " ") for c in caps], fontsize=11.5)
    ax.invert_yaxis()
    ...
    handles = [Patch(facecolor=code[v][0], label=code[v][1]) for v in ("pass", "L0", "L2", "L3", "-") if v in present]
    ax.legend(handles=handles, loc="upper center", bbox_to_anchor=(0.5, -0.13), ncol=len(handles), fontsize=12.5)
...
def fig_ladder(data: dict, out: Path) -> None:
    ...
    for c, lbl in cells:
        ...
        ax.plot(
            range(len(tiers)),
            ys,
            color=mcol.get(model, SLATE),
            lw=3.4,
            linestyle=hdash.get(harness, (0, ())),
            marker="o",
            markersize=11,
    ...
    for c, lbl in cells:
        ...
        if lbl.startswith("GPT-5.6-sol"):
            ax.annotate(
                f"{lbl.split(chr(10))[1]} \u2192 {y3}/20 on RTL",
                (3, y3),
                textcoords="offset points",
                xytext=(-18, 16) if y3 > 10 else (-6, -34),
                ha="right",
                fontsize=13,
                color=GREEN if y3 > 10 else INK,
                fontweight="bold",
            )
    ...
    ax.text(
        0.06,
        7.4,
        "everyone else computes the right\nanswer and encodes it wrong",
        fontsize=13.5,
        color=SALMON,
        ha="left",
        va="center",
    ...
    handles += [
        plt.Line2D([], [], color=MUTED, lw=2.2, linestyle=v, label=k) for k, v in hdash.items() if k != "Codex (run 3)"
    ]
...
def fig_planes(data: dict, out: Path) -> None:
    ...
    order = [
        ("pass", "passed", GREEN),
        ("compute", "wrong code produced", SALMON),
    ...
    for y, (c, lbl) in zip(ys, cells):
        ...
        for key, _, col in order:
            ...
            if v >= 2:
                ax.text(
                    left + v / 2, y, str(v), ha="center", va="center", color="white", fontsize=13, fontweight="bold"
                )
            left += v
...
def fig_tooling(data: dict, out: Path) -> None:
    ...
    cells = _pick(data)
    checks = [
        ("isa_tools_used", "ran the RTL\nISA tools"),
        ("cca_used", "enumerated the\nlever set"),
        ("no_regex_ok", "no regex in\nits compiler"),
        ("full_selfcheck", "self-checked all\ncapsules"),
    ]
    ...
    for j, (c, lbl) in enumerate(cells):
        ...
        for i, (k, _) in enumerate(checks):
            ...
            if k == "no_regex_ok" and v is False and c["conformance"].get("regex_only_in_vendored"):
                col, mark = AMBER, "!"
            ax.add_patch(plt.Rectangle((i, j), 0.9, 0.82, facecolor=col, edgecolor="white", linewidth=2))
            ax.text(i + 0.45, j + 0.41, mark, ha="center", va="center", color="white", fontsize=17, fontweight="bold")
    ax.set_xlim(-0.05, len(checks))
    ax.set_ylim(-0.1, len(cells))
    ...
    ax2.barh(
        range(len(cells)),
        hid,
        color=[GREEN if h == 5 else (AMBER if h else SLATE_L) for h in hid],
        height=0.62,
        edgecolor="white",
        linewidth=1.2,
    )
...
def fig_architecture(data: dict, out: Path) -> None:
    ...
    cells = _pick(data)
    roles = [
        ("structure", "dialect + lowering passes it wrote", GREEN),
    ...
    for y, (c, lbl) in zip(ys, cells):
        ...
        for key, _, col in roles:
            ...
            left += v
        ax.text(left + 0.22, y, f"{c['submission']['n_lines']:,} lines", va="center", fontsize=12, color=MUTED)
    ax.set_yticks(ys)
    ...
    ax.legend(
        handles=[Patch(facecolor=c, label=l) for _, l, c in roles],
        ncol=3,
        loc="upper center",
        bbox_to_anchor=(0.5, -0.16),
        fontsize=12,
...
def fig_confounds(data: dict, out: Path) -> None:
    ...
    issues = [
        ("harness config delivered", lambda c: c["harness_health"]["config_delivered"] is not False),
        ...
        (
            ...
            lambda c: (
                c["diagnostics"]["verdicts"] == 0 or c["diagnostics"]["redacted"] / c["diagnostics"]["verdicts"] < 0.05
            ),
        ),
    ]
    ...
    for j, (run, lbl) in enumerate(rows):
        ...
        for i, (_, test) in enumerate(issues):
            ok = bool(test(c))
            ax.add_patch(
                plt.Rectangle((i, j), 0.9, 0.82, facecolor=GREEN if ok else SALMON, edgecolor="white", linewidth=2)
            )
            ax.text(
                i + 0.45,
                j + 0.41,
...
def fig_efficiency(data: dict, out: Path) -> None:
    ...
    ax.barh([y + 0.38 for y in ys], cac, color=BLUE_L, height=0.36, label="served from cache", edgecolor="white")
    ax.set_yticks([y + 0.19 for y in ys])
    ax.set_yticklabels(labs, fontsize=12)
    ax.invert_yaxis()
    ax.set_xlabel("million tokens")
    ...
    ax2.barh(
        list(ys),
        tools,
        color=[GREEN if c["n_passed"] == 20 else SLATE for c, _ in cells],
        height=0.62,
        edgecolor="white",
        linewidth=1.2,
    )
...
def fig_natural_experiment(data: dict, out: Path) -> None:
    ...
    ax.text(
        1.56,
        21.4,
        "the command buffer is identical  \u2192  everything diverges here",
        fontsize=13,
        color=MUTED,
        ha="left",
    )
