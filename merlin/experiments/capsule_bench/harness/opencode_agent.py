...
def _system_prompt(te) -> str:
    ...
    return (
        ...
        "MANIFEST FORMAT: the `entrypoints.tool` field must be the BARE script path relative to the package "
        ...
        "MANDATORY — BUILD ON MERLIN'S FRAMEWORK, DO NOT REINVENT IT. This is the `merlin_assisted` arm: your "
        "backend MUST build on the granted oot_starterkit (on PYTHONPATH), NOT hand-rolled. Read its guide "
        "first (`cat oot_starterkit/AGENT.md` and `cat oot_starterkit/scaffold/README.md`), then import + USE "
        "it: `from merlin.targetgen.oot_starterkit import parse_interface, CommandBufferBuilder, transforms, "
        "verify` (also `dialect`). parse_interface parses the fixed merlin_iface grammar — do NOT write your "
        "own parser. CommandBufferBuilder emits a SCHEMA-VALID command_buffer.json by construction — use it "
        "so you PASS the command_buffer_schema plane. dialect.py is the TYPED/VERIFIED xDSL IR; verify.py is "
        "the structural verifier; transforms.py holds generic passes. Your OWN code is only the "
        f"{t}-SPECIFIC lowering on top of these framework pieces.\n"
        "Use the CCA seam menu (`from merlin.kernels import cca_contract, action_catalog` — "
        f"cca_contract.check_bijection('{t}') / action_catalog.escalation_ladder) as the what-to-build "
