...
def check_canary_isolation() -> dict:
    ...
    for bundle_id in bundles_to_check():
        ...
        with tempfile.TemporaryDirectory() as td:  # honours TMPDIR
            ...
            WT.assemble_workspace(bundle, ws, context=C.CONTEXT, _policy_test_live_inputs=True)
...
def check_bundle_hash_repro() -> dict:
    ...
    A grant that resolves nowhere is recorded as ``unresolvable`` and fails the gate. Silently
    omitting it is the failure that hid the gap: a shrinking lock looked like a smaller bundle
    instead of an unpinned one. Host-only inputs participate in both hash passes, but their paths
    and digests never enter this public lock or report: the run's private snapshot receipt binds
    those bytes. Host paths use the snapshot's canonical interpretation, not grant shorthand.
    """
    out = {}
    ...
    for bundle_id in bundles_to_check():
        bundle = load_bundle_by_id(bundle_id)
        paths = [
            str(e.get("path", "")).rstrip("/") or ("/" if e.get("path") == "/" else "")
        ...
        host_paths = bwrap._host_input_paths(bundle)
        host_resolved = {p: (C.REPO / p).absolute() for p in host_paths}
        missing_hosts = sum(bwrap.path_kind(path) == "missing" for path in host_resolved.values())

        def _hash(sources: dict[str, Path], cache: dict[Path, str]) -> dict:
            hashes = {}
            for declared, grant in sources.items():
        ...
        h1, h2 = _hash(resolved, pass1), _hash(resolved, pass2)
        private_h1, private_h2 = _hash(host_resolved, pass1), _hash(host_resolved, pass2)
        out[bundle_id] = {
            "reproducible": not unresolvable and not missing_hosts and h1 == h2 and private_h1 == private_h2,
            "n_paths": len(h1),
            "unresolvable": unresolvable,
            "n_host_inputs": len(host_paths),
            "unresolvable_host_inputs_count": missing_hosts,
        }
