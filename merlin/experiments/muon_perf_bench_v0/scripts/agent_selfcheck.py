...
def grade(submission: str, capsules_root: str, runs_root: str, timeout: int, only: str | None = None) -> dict:
...
def main(argv: list[str] | None = None) -> int:
    ...
    _default_caps = (
        "capsules" if Path("capsules").is_dir() else str(_REPO / "merlin/experiments/muon_perf_bench_v0/kernels")
    )
    ...
    print(
        "\nALL PASS — now push %FP-peak up." if v["all_pass"] else "\nNot all pass yet — fix the failing planes above."
    )
    return 0 if v["all_pass"] else 1
