...
DEFAULT_PUBLIC_TRAIN = repo_root() / "out/artifacts/rvv-development-corpus/k1_cpu/v2/latest/public/train.jsonl"
...
def _retain_input_submission(source: Path, destination: Path) -> Path:
    ...
    links = [path.relative_to(source).as_posix() for path in source.rglob("*") if path.is_symlink()]
    if links:
        raise ValueError(f"qualification submission symlinks are forbidden: {links[:8]}")
    shutil.copytree(source, destination, symlinks=False)
    if _tree_sha256(source) != _tree_sha256(destination) or _package_tree_sha256(source) != _package_tree_sha256(
        destination
    ):
...
def _copy_aa_sealed_prebuilt(*, noise: dict[str, Any], aa_receipt: dict[str, Any], destination: Path) -> Path:
    ...
    if not manifest_path.is_file() or _sha256(manifest_path) != aa_receipt.get("private_manifest_sha256"):
    ...
    first = command[0]
    raw_entrypoint = (
        command[1] if first in {"python3", "/usr/bin/python3", "/usr/bin/python3.12"} and len(command) > 1 else first
    )
    ...
    if (
        ...
        or not entrypoint.is_relative_to(source)
        or raw_entrypoint_path.is_symlink()
    ...
    if (
        not policy.is_relative_to(source)
        or not policy.is_file()
        or raw_policy.is_symlink()
        or _sha256(policy) != aa_receipt.get("policy_sha256")
    ):
...
def _executable_source_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "noise_validator": (repo_root() / "packages/merlin-analysis/src/merlin/compare/host_experiment.py").resolve(),
...
def _verify_inventory_entry(root: Path, entry: dict[str, Any]) -> Path:
    ...
    if (
        ...
        or ".." in relative.parts
        or relative.as_posix() != str(entry.get("relative_path", ""))
    ):
...
def _retain_source_snapshot(root: Path, snapshot: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for name, identity in sorted(snapshot.items()):
        ...
        if entry["sha256"] != identity["sha256"] or entry["size_bytes"] != identity["size_bytes"]:
            raise RuntimeError(f"retained executable source differs: {name}")
        inventory.append(entry)
    return inventory


def _retain_authority_inputs(
    root: Path, *, public_train: Path, space: Path, noise_authority: Path
) -> list[dict[str, Any]]:
    inventory: list[dict[str, Any]] = []
...
def select_public_runtime_capsule(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    candidates = [
        ...
        and row.get("family") == "runtime_parallel"
        and row.get("operation") == "static_partition"
        and isinstance(row.get("state"), dict)
        and row["state"].get("reuse_count") == 1
        and isinstance(row.get("core_count"), int)
        and not isinstance(row.get("core_count"), bool)
        and int(row["core_count"]) > 1
    ]
    if not candidates:
...
def _default_semantic_noise_problems(
    *, noise: dict[str, Any], public_train: Path, space: dict[str, Any], rows: list[dict[str, Any]]
) -> list[str]:
    """Invoke the campaign's full independent A/A receipt validator lazily."""
    host = _load(
        "merlin_multicore_host_preflight",
        repo_root() / "packages/merlin-analysis/src/merlin/compare/host_experiment.py",
    )
    costs = _load("merlin_multicore_cost_sources", HERE / "calibrate_search_costs.py")
    return host._validate_calibration_semantics(
        label="noise_calibration",
        value=noise,
        train_sha256=_sha256(public_train),
        source_sha256=costs._source_sha256(),
        space=space,
        train_rows=rows,
    )
...
def _noise_lineage(
    *,
    ...
    rows: list[dict[str, Any]],
    raw_tree_sha256: str,
    raw_package_sha256: str,
    semantic_validator: Callable[..., list[str]] | None,
    ...
    basic_ok = (
        ...
        and noise.get("version") == 2
        and noise.get("kind") == "cpu_host_k1_order_balanced_aa_noise_calibration"
        and noise.get("status") == "pass"
        and noise.get("checks") == expected_checks
        and noise.get("paid_work") is False
        and noise.get("heldout_opened") is False
        and noise.get("protocol_state_mutated") is False
        and noise.get("public_train_sha256") == _sha256(public_train)
        and isinstance(protocol, dict)
        and protocol.get("measurement_repeats") == len(PAIR_ORDERS)
        and protocol.get("board_environment") == space.get("board_environment")
        and _canonical_sha256(protocol) == noise.get("calibration_protocol_sha256")
        and lineage == expected_lineage
        and noise.get("prebuild_input_tree_sha256") == raw_tree_sha256
        and noise.get("prebuild_input_package_sha256") == raw_package_sha256
        and math.isfinite(margin)
        and margin > 0
        and margin == final_margin
    )
    if not basic_ok:
        raise ValueError(
            "A/A authority does not bind the public split, raw compiler, board gate, and final derived margin"
        )
...
def _retain_compiled_artifacts(receipts: Path, compiled: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    inventory: list[dict[str, Any]] = []
    for mode, record in sorted(compiled.items()):
        ...
        for name, source in sources.items():
            ...
            target.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(source, target)
            inventory.append(_inventory_entry(receipts, target, role="compiled_artifact", mode_name=mode, name=name))
    return inventory


def _retain_k1_build_artifacts(
    *, receipts: Path, run_root: Path, row: dict[str, Any], mode: str, attempt_id: int, evidence: dict[str, Any]
) -> list[dict[str, Any]]:
    ...
    for name in _K1_ARTIFACT_NAMES:
        ...
        entry = _inventory_entry(
            receipts, target, role="k1_build_artifact", mode_name=mode, attempt_id=attempt_id, name=name
        )
        ...
        if name == "kernel.text.bin" and entry["sha256"] != evidence.get("kernel_text_sha256"):
            raise ValueError("retained K1 text differs from the kernel text receipt")
        inventory.append(entry)
    return inventory


def _equivalent_work_contract(row: dict[str, Any], compiled: dict[str, dict[str, Any]], grader) -> dict[str, Any]:
    one, many = compiled["rvv"], compiled["rvv_multicore"]
    harts = int(row["core_count"])
    buffer_plan = _safe(grader._buffer_plan(row))
    invariant = {
        "capsule_sha256": row["sha256"],
        "family": row["family"],
        "operation": row["operation"],
        "dtype": row["dtype"],
        "layout": row["layout"],
        "shape": _safe(row["shape"]),
        "state": _safe(row["state"]),
        "buffer_plan": buffer_plan,
    }
    checks = {
        "same_input_mlir": one.get("input_mlir_sha256") == many.get("input_mlir_sha256")
        and _digest(one.get("input_mlir_sha256")),
        "same_capsule_digest": all(
            record.get("metadata", {}).get("capsule_sha256") == row["sha256"] for record in (one, many)
        ),
        "same_buffer_plan": one.get("buffer_plan") == many.get("buffer_plan") == buffer_plan,
        "exact_modes": (one.get("mode"), many.get("mode")) == ("rvv", "rvv_multicore")
        and one.get("metadata", {}).get("actual_mode") == "rvv"
        and many.get("metadata", {}).get("actual_mode") == "rvv_multicore",
        "exact_harts": one.get("metadata", {}).get("harts") == 1 and many.get("metadata", {}).get("harts") == harts,
        "no_fallback": all(record.get("metadata", {}).get("fallback_used") is False for record in (one, many)),
...
def _physical_process_window_evidence(monitor: dict[str, Any], harts: int) -> dict[str, Any]:
    ...
    try:
        ...
        sets_well_formed = all(
            ...
            and set(values) <= expected_set
            for values in sampled_sets
        )
        checks = {
...
def _logical_shard_evidence(metrics: dict[str, Any], harts: int, output_count: int) -> dict[str, Any]:
    return {
        "version": 1,
...
def _require_complete_k1_evidence(
    *, evidence: dict[str, Any], row: dict[str, Any], mode: str, harts: int, seed: int, grader
    ...
    if not isinstance(evidence, dict):
        ...
        raise K1EvidenceError(f"{mode} measurement lacks the complete passing grader K1 evidence", assessment)
    ...
    grader_checks = {
        ...
        "grader.check_set_exact": (isinstance(checks, dict) and set(checks) == REQUIRED_K1_CHECKS),
        "grader.checks_all_true": (isinstance(checks, dict) and all(value is True for value in checks.values())),
    ...
    trusted_lines = [
        line for line in str(monitor.get("child_stdout", "")).splitlines() if line.startswith("MERLIN_TRUSTED_RESULT ")
    ]
    expected_receipt = f"MERLIN_TRUSTED_RESULT version=1 seed={seed} nonce={receipt_nonce} memory=1 numeric=1"
    physical = _physical_process_window_evidence(monitor, harts)
    output_count = int(grader._buffer_plan(row)["output_count"])
    per_call, attribution = grader._k1_timing_authority(metrics, harts, output_count)
    board_wall = evidence.get("board_wall_seconds")
    board_wall_ok = (
        isinstance(board_wall, (int, float))
        and not isinstance(board_wall, bool)
        and math.isfinite(float(board_wall))
        and float(board_wall) > 0
    )
    replay_checks = {
        "timing.wall_ns_positive_int": (isinstance(wall_ns, int) and not isinstance(wall_ns, bool) and wall_ns > 0),
        "timing.calls_positive_int": (isinstance(calls, int) and not isinstance(calls, bool) and calls > 0),
        "receipt.nonce_positive_int": (
            isinstance(receipt_nonce, int) and not isinstance(receipt_nonce, bool) and receipt_nonce > 0
        ),
        ...
        "binary.upload_digest_match": (evidence.get("remote_sha256") == evidence.get("local_sha256")),
        "ssh.returncode_zero": evidence.get("ssh_returncode") == 0,
        "monitor.returncode_zero": monitor.get("returncode") == 0,
        "monitor.metrics_unique_present_and_exact": (metrics_parse_error is None and parsed_metrics == metrics),
        ...
        **{f"physical_process_window.{name}": passed is True for name, passed in physical["checks"].items()},
    }
    assessment = {
        "version": 1,
        "mode": mode,
...
def _paired_k1(
    *,
    ...
    maximum_replacements = int(board_environment["maximum_invalid_pair_replacements_per_capsule"])
    ...
    with grader._k1_lock(connection):
        while repeat < len(PAIR_ORDERS):
            ...
            modes = ("rvv", "rvv_multicore") if order == "rvv_rvv_multicore" else ("rvv_multicore", "rvv")
            measurements: dict[str, dict[str, Any]] = {}
            for mode in modes:
                run_root = Path(tempfile.mkdtemp(prefix=f"pair_{repeat}_attempt_{pair_attempt}_{mode}_"))
                try:
                    evidence = grader._grade_k1(row, compiled[mode], operation_codes, run_root, seed=seed)
                ...
                try:
                    wall_ns, calls, logical, physical = _require_complete_k1_evidence(
                        evidence=evidence, row=row, mode=mode, harts=harts, seed=seed, grader=grader
                    )
                    ...
                    failure_receipt = _write_receipt(
                        receipts, f"failures/attempt_{pair_attempt:02d}_pair_{repeat:02d}_{mode}.json", failure
                    )
                    raise ValueError(
                        ...
                        f"sha256={failure_receipt['sha256']}"
                    ) from error
            ...
            receipt = _write_receipt(receipts, f"pairs/attempt_{pair_attempt:02d}_pair_{repeat:02d}.json", raw)
    ...
    return {
        ...
        "attempt_count": pair_attempt,
        "k1_program_count": 2 * pair_attempt,
        "kernel_text_sha256": code_digests,
        "raw_receipt_inventory": pair_inventory,
        "k1_build_artifact_inventory": build_inventory,
    }


def _replay_pair_receipts(
    *, receipts: Path, transcript: dict[str, Any], grader, row: dict[str, Any], board_environment: dict[str, Any]
) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if [record.get("attempt_id") for record, _ in disk_records] != list(range(len(disk_records))):
        raise ValueError("raw K1 pair attempts are not one complete chronology")

    expected_by_attempt = {
        int(record["attempt_id"]): record
        for record in [*transcript.get("valid_pairs", []), *transcript.get("excluded_pairs", [])]
    }
    ...
    for raw, receipt_entry in disk_records:
        ...
        if (
            current_pair_id >= len(PAIR_ORDERS)
            or raw.get("version") != 1
            or raw.get("pair_id") != current_pair_id
            or raw.get("order") != PAIR_ORDERS[current_pair_id]
        ):
        ...
        if (
            ...
            or not settle
            or before != settle[-1]
            or any(grader._k1_state_ready(probe, board_environment) for probe in settle[:-1])
            or not grader._k1_state_ready(before, board_environment)
        ):
        ...
        for mode, harts in (("rvv", 1), ("rvv_multicore", int(row["core_count"]))):
            ...
            wall_ns, calls, logical, physical = _require_complete_k1_evidence(
                evidence=measurement.get("evidence"), row=row, mode=mode, harts=harts, seed=seed, grader=grader
            )
            ...
            if (
                measurement.get("logical_shard_evidence") != logical
                or measurement.get("physical_process_window_evidence") != physical
            ):
            ...
            if artifacts != expected_artifacts or {str(entry.get("name")) for entry in expected_artifacts} != set(
                _K1_ARTIFACT_NAMES
            ):
                raise ValueError("raw K1 pair build-artifact inventory differs")
            artifact_paths = {
                str(entry["name"]): _verify_inventory_entry(receipts, entry) for entry in expected_artifacts
            }
            evidence = measurement["evidence"]
            if _sha256(artifact_paths["capsule_k1"]) != evidence.get("local_sha256") or _sha256(
                artifact_paths["kernel.text.bin"]
    ...
    replayed = {
        ...
        "valid_pairs": valid_pairs,
        "excluded_pairs": excluded_pairs,
        "attempt_count": len(disk_records),
        "k1_program_count": 2 * len(disk_records),
        "kernel_text_sha256": code_digests,
...
def _effect_summary(transcript: dict[str, Any], *, margin: float, minimum_directional_pairs: int) -> dict[str, Any]:
    ...
    for record in transcript["valid_pairs"]:
        ...
        direction = (
            "multicore_faster" if speedup > upper else ("multicore_slower" if speedup < lower else "within_aa_noise")
        )
        pairs.append(
            {
                ...
                "multicore_speedup": speedup,
                "margin_classification": direction,
            }
        )
    ...
    return {
        ...
        "lower_bound": lower,
        "upper_bound": upper,
        "minimum_directional_pairs": minimum_directional_pairs,
        "pairs": pairs,
        "multicore_faster_pairs": faster,
        "multicore_slower_pairs": slower,
        ...
        "directionally_consistent_effect": direction in {"multicore_faster", "multicore_slower"},
        "multicore_speedup_qualified": direction == "multicore_faster",
    }


def verify_saved_qualification(value: dict[str, Any] | str | Path, *, grader=None) -> dict[str, Any]:
    ...
    categories = (
        [raw.get("pre_k1")],
        raw.get("compiled_artifacts"),
        raw.get("pairs"),
        raw.get("k1_build_artifacts"),
        raw.get("executable_sources"),
        raw.get("authority_inputs"),
    )
    entries: list[dict[str, Any]] = []
    for category in categories:
        if not isinstance(category, list) or any(not isinstance(entry, dict) for entry in category):
    ...
    if not isinstance(snapshot, dict) or _canonical_sha256(snapshot) != value.get("source_snapshot_sha256"):
        raise ValueError("qualification executable source snapshot is malformed")
    source_entries = {str(entry.get("source_role")): entry for entry in raw["executable_sources"]}
    ...
    for name, identity in snapshot.items():
        ...
        if entry.get("sha256") != identity.get("sha256") or entry.get("size_bytes") != identity.get("size_bytes"):
            raise ValueError(f"retained executable source identity differs: {name}")

    authority_entries = {str(entry.get("authority_role")): entry for entry in raw["authority_inputs"]}
    ...
    if set(authority_entries) != set(expected_authorities) or any(
        authority_entries[role].get("sha256") != digest for role, digest in expected_authorities.items()
    ):
    ...
    if (
        not isinstance(pre_k1, dict)
        or pre_k1.get("row") != value.get("selected_public_capsule")
        or pre_k1.get("work_contract") != value.get("equivalent_work_contract")
        or pre_k1.get("spike_gate") != value.get("spike_gate")
    ):
        raise ValueError("qualification pre-K1 receipt differs from the published gates")
    compiled_receipts = pre_k1.get("compiled")
    if not isinstance(compiled_receipts, dict) or set(compiled_receipts) != {"rvv", "rvv_multicore"}:
        raise ValueError("qualification pre-K1 receipt lacks both compiled modes")
    compiled_entries = {
        (str(entry.get("mode_name")), str(entry.get("name"))): entry for entry in raw["compiled_artifacts"]
    }
    for mode, record in compiled_receipts.items():
        ...
        metadata_path = _verify_inventory_entry(root, compiled_entries[(mode, "metadata.json")])
    ...
    recomputed_effect = _effect_summary(
        replayed,
        margin=float(effect["noise_margin"]),
...
def qualify(
    *,
    ...
    public_train: Path,
    space_path: Path,
    noise_authority: Path,
    prebuilt_destination: Path,
    receipts_destination: Path,
    grader=None,
    runner=None,
    semantic_noise_validator: Callable[..., list[str]] | None = None,
) -> dict[str, Any]:
    submission, public_train, space_path = (submission.resolve(), public_train.resolve(), space_path.resolve())
    ...
    before = (
        _sha256(public_train),
        _sha256(space_path),
        raw_tree,
        raw_package,
        _sha256(noise_authority.resolve()),
        _canonical_sha256(source_before),
    )
    noise, lineage = _noise_lineage(
        noise_path=noise_authority,
        public_train=public_train,
        space_path=space_path,
        space=space,
        rows=rows,
        raw_tree_sha256=raw_tree,
        raw_package_sha256=raw_package,
        semantic_validator=semantic_noise_validator,
    )
    authority_input_inventory = _retain_authority_inputs(
        receipts_destination, public_train=public_train, space=space_path, noise_authority=noise_authority.resolve()
    )
    ...
    exact_prebuild_fields = (
        ...
        "real_build_commands",
        "prebuild_tree_sha256",
        "built_tree_sha256",
        "sealed_prebuilt_tree_sha256",
        "submitted_entrypoint_identity",
        "built_entrypoint_identity",
        "private_build_override",
        "policy_sha256",
    )
    ...
    _copy_aa_sealed_prebuilt(noise=noise, aa_receipt=aa_prebuild, destination=prebuilt_destination)
    ...
    lineage.update(
        {
            ...
            "qualification_sealed_prebuilt_tree_sha256": prebuild_receipt["sealed_prebuilt_tree_sha256"],
    ...
    if semantic_noise_validator is None:
        ...
        current_toolchain = costs._toolchain(grader=grader, kind="k1-program", prebuild_receipt=prebuild_receipt)
    ...
    with tempfile.TemporaryDirectory(prefix="merlin-host-multicore-effect-") as temporary:
        ...
        if not all(spike_gate[key] for key in ("rvv_spike_correct", "rvv_multicore_l0_correct")):
            raise ValueError("pre-K1 Spike/L0 gate failed")
        pre_k1_receipt = _write_receipt(
            ...
            "pre_k1.json",
            {
                ...
                "compiled": {
                    mode: _safe({key: value for key, value in record.items() if key != "_kernel_path"})
    ...
    current = (
        _sha256(public_train),
        _sha256(space_path),
        _tree_sha256(submission),
        _package_tree_sha256(submission),
        _sha256(noise_authority.resolve()),
        _canonical_sha256(source_after),
    )
    if (
        current != before
        or _package_tree_sha256(prebuilt_destination) != prebuild_receipt["sealed_prebuilt_tree_sha256"]
    ):
        raise RuntimeError("qualification mutated a sealed input, executable source, or prebuilt compiler package")
    ...
    effect = _effect_summary(
        transcript,
        margin=float(noise["derived_noise_margin"]),
        minimum_directional_pairs=int(space["selection"]["minimum_pairwise_wins"]),
    )
    checks = {
        "public_train_only": row["split"] == "train",
        "aa_authority_bound": lineage["noise_authority_sha256"] == _sha256(noise_authority.resolve()),
        "equivalent_semantic_work_per_call": work_contract["equivalent"],
        "both_modes_l0_pass": all(work_contract["checks"].values()),
        "pre_k1_spike_gate": spike_gate["rvv_spike_correct"] and spike_gate["rvv_multicore_l0_correct"],
        "six_valid_pairs": len(transcript["valid_pairs"]) == len(PAIR_ORDERS),
        "balanced_order": transcript["pair_orders"].count("rvv_rvv_multicore") == 3
        and transcript["pair_orders"].count("rvv_multicore_rvv") == 3,
        "complete_k1_evidence": transcript["k1_program_count"] == 2 * transcript["attempt_count"],
        ...
        "logical_shards_balanced": all(
            ...
            for measurement in pair["measurements"].values()
        ),
        ...
        "frozen_board_condition_gate": all(pair["valid"] is True for pair in transcript["valid_pairs"]),
    ...
    result = {
        ...
        "selection_rule": ("static_partition_reuse1_then_max_harts_then_max_work_items_then_lexical_sha256"),
        ...
        "source_sha256": {name: identity["sha256"] for name, identity in source_before.items()},
    }
...
def main(argv: list[str] | None = None) -> int:
    ...
    handle = start_run(
        suite="cpu-host-compiler",
        method="k1-multicore-effect-qualification",
        target="k1_cpu",
    ...
    try:
        try:
            ...
            result = qualify(
                ...
                prebuilt_destination=prebuilt,
                receipts_destination=receipts,
            )
            ...
            result = {
                ...
                "source_sha256": {
                    "qualifier": _sha256(Path(__file__)),
        ...
        finish_run(
            handle,
            status=("ok" if result and result["status"] == "pass" else "error"),
            summary={
                "ready": bool(result and result["status"] == "pass"),
                "effect_direction": (result or {}).get("effect", {}).get("effect_direction"),
                "multicore_speedup_qualified": (result or {})
                .get("effect", {})
                .get("multicore_speedup_qualified", False),
