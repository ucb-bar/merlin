...
def _numeric_inputs(target: str):
    """Read the experiment's explicit recipe, never a core target-name profile lookup."""
    ...
    return load_declared_numeric_policy(load_target_experiment(descriptor_path(target)), repo=repo_root())
...
def probe(target: str, tier: str, workdir: Path, *, force: bool = False) -> dict:
    ...
    numeric_policy, numeric_source = _numeric_inputs(target)
    if p.is_file() and not force:
        cached = json.loads(p.read_text())
        if cached.get("numeric_policy_source") == numeric_source:
            return cached
    ...
    settle = WG.probe_settle(facts, ops, cf, run_matmul, operands=(A0, W0), numeric_policy=numeric_policy)
    out = {
        "target": target,
        "tier": tier,
        "numeric_policy_source": numeric_source,
        "control_flow": {
...
def build_plan(target: str, tier: str, shape: str, workdir: Path, *, with_operands: bool = True) -> WG.MatmulPlan:
    ...
    numeric_policy, _ = _numeric_inputs(target) if with_operands else (None, None)
    return WG.plan_matmul(
        facts, ops, m=m, k=k, n=n, control_flow=cf, settle=settle, A=A, W=W, numeric_policy=numeric_policy
    )
