...
def latest_sweep(target: str) -> dict:
    bases = [root / target / "v2" for root in (A.product_root("perf-ledger"), A.artifacts_dir() / "perf-ledger")]
    unique = {}
    for base in bases:
        for path in base.glob("*/cycle_sweep.json"):
            unique.setdefault(path.resolve(), path)
    # Product names retain the historical timestamp ordering across both homes;
    # the directory migration is not evidence that a measurement is newer.
    files = sorted(unique.values(), key=lambda path: (path.parent.name, str(path)))
    if not files:
        raise SystemExit(f"no sweep product under {bases}; run `cycle_sweep.py run` first")
