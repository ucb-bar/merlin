...
Numerical-input ownership prerequisite: **PARTIAL**. The layer workload edge passes the descriptor's
explicit public numeric recipe into golden generation and settle probing; cache reuse binds that
recipe's declared-source identity. Synthetic regressions cover unchanged policy propagation and
recipe-change invalidation. These declarations are not hardware verification; no hardware probes
were rerun for this structural migration.
