...
def prior_runs(target: str) -> list:
    ...
    """
    bases = [root / target for root in (A.product_root("perf-workload"), A.artifacts_dir() / "perf-workload")]
    unique = {}
    for base in bases:
        for path in base.glob("v*/*/workload_record.json"):
            unique.setdefault(path.resolve(), path)
    out = []
    # Keep version/product-name ordering, independent of canonical versus legacy
    # directory spelling. Organizer symlinks must not double-count measurements.
    records = sorted(unique.values(), key=lambda path: (path.parent.parent.name, path.parent.name, str(path)))
