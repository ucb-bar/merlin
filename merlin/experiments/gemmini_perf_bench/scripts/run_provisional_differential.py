...
_CORPUS_SECTIONS = ("golden_kernels", "model_kernels", "attention_kernels", "conv_kernels", "movement_kernels")
_DIGEST_RE = re.compile(r"[0-9a-fA-F]{64}")
_RUN_ID_RE = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]*")
_MAX_TIMEOUT_S = 3600
_TREE_HASH_EXCLUSIONS = {"build", "__pycache__", ".git"}
_CLAIM_SCOPE = "Provisional pinned baseline/candidate differential only; not the full agentic Phase-P contract."
_LIMITATIONS = (
...
def _excluded_source_paths(root: Path) -> list[str]:
    ...
    return sorted(path.relative_to(root).as_posix() for path in root.rglob("*") if path.name in _TREE_HASH_EXCLUSIONS)
...
def _materialize_hashed_tree(source: Path, snapshot: Path, label: str) -> str:
    ...
    if source_after != before:
        raise ProvisionalExperimentError(f"{label} source changed during materialization: {before} != {source_after}")
    if observed != before:
        raise ProvisionalExperimentError(f"{label} frozen copy changed during materialization: {before} != {observed}")
...
def inspect_package(path: Path, expected_sha256: str, label: str) -> dict:
    ...
    if manifest.get("artifact_type") != "mlir_oot_target_backend":
        raise ProvisionalExperimentError(f"{label} package is not an mlir_oot_target_backend")
    if manifest.get("target") != PB.TARGET:
        raise ProvisionalExperimentError(f"{label} package targets {manifest.get('target')!r}, not {PB.TARGET!r}")
    if manifest.get("integrity_exempt") is not False:
        raise ProvisionalExperimentError(f"{label} package must explicitly declare integrity_exempt: false")
...
def _normalize_spike_tier(tier: object) -> dict:
    ...
    provenance_valid = row.get("derived_from_rtl") is not True and row.get("cycle_accurate") is not True
    return {
        "simulator": "spike",
        "tier": "L2",
...
def _run_cell(
    ...
    record = {"kernel": kernel_id, "lane": lane, "package_tree_sha256": package_digest, "valid": False}
    try:
...
def _write_reports(run_dir: Path, document: dict, kernel_ids: Sequence[str]) -> None:
    ...
    document["completion"] = {
        ...
        "complete": (
            len(document.get("cells") or []) == expected and valid_cells == expected and not document.get("refusals")
        ),
    }
...
def _validate_config(config: ExperimentConfig) -> None:
    ...
    if not 1 <= config.timeout_s <= _MAX_TIMEOUT_S:
        raise ProvisionalExperimentError(f"cell timeout must be between 1 and {_MAX_TIMEOUT_S} seconds")
    ...
    if config.counter_unit is not None:
        ...
        if not config.hardware_counters or not unit or any(not (char.isalnum() or char == "_") for char in unit):
            raise ProvisionalExperimentError("counter_unit requires hardware counters and must be one identifier token")
...
def run_experiment(config: ExperimentConfig) -> tuple[int, Path, dict]:
    ...
    try:
        ...
        # Preserve this experiment's declared Spike screen and Verilator measurement.
        defaults = {
            "L2": CR.simulator_adapter("spike", PB.TARGET),
            "L3": CR.simulator_adapter("verilator", PB.TARGET),
        }
        adapters = defaults
            ...
            for kernel in corpus:
                for lane in ("baseline", "candidate"):
                    ...
                    document["cells"].append(
                        _run_cell(
                            package_snapshots[lane],
                            source_packages[lane]["snapshot_tree_sha256"],
                            lane,
                            kernel,
                            workload_snapshot,
                            cells_root,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--kernels", required=True, help="explicit comma-separated kernel ids; 'all' is intentionally refused"
    )
    parser.add_argument(
        "--run-id", required=True, help="fresh output directory name under the canonical perf-bench run root"
    )
    parser.add_argument(
        "--timeout", type=int, default=900, help=f"timeout per package/kernel cell (1..{_MAX_TIMEOUT_S}s)"
    )
    parser.add_argument(
        "--nice-increment",
        type=int,
    ...
    parser.add_argument(
        "--counter-unit", help="select a unit family from the shipped counter header (default: occupancy)"
    )
    args = parser.parse_args(argv)
    try:
        code, run_dir, document = run_experiment(
            ExperimentConfig(
