...
def iree_compile(name, M, K, N, workdir: Path) -> tuple[Path, str, tuple]:
    ...
    cmd = [str(IREE), str(fx), *GEMMINI_FLAGS, f"--iree-hal-dump-executable-files-to={dump}", "-o", str(vmfb)]
    ...
    gp = subprocess.run([_OBJCOPY, f"--globalize-symbol={sym}", str(obj), str(gobj)], capture_output=True, text=True)
    if gp.returncode != 0:
...
def build_iree_elf(name, M, K, N, obj: Path, sym: str, wgc: tuple, workdir: Path) -> Path:
    ...
    src.write_text(
        _HARNESS_C.format(
            ...
            K=K,
            N=N,
            seed_x=BMC.det_seed("X"),
            seed_w=BMC.det_seed("W"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    for k in corpus:
        sims = (
            (["spike", "verilator"] if k.get("sim_hint") == "L2+L3" else ["spike"])
            if a.sims == "auto"
            else [s.strip() for s in a.sims.split(",")]
        )
        print(f"\n=== IREE {k['id']} ({k['M']}x{k['K']}x{k['N']}, sims={sims}) ===", flush=True)
        r = run_one(k, sims, out / "_work" / k["id"], a.timeout)
        results.append({"kernel": k["id"], "iree": r})
        print(
            f"  build={r.get('ok_build')} sym={r.get('sym', '-')} "
