...
def _declaration(
    *,
    ...
    cohort: ORCH.FunctionalGradeCohort,
    ...
    return {
        ...
        "cohort": {
            ...
            "same_elf_certificate_descriptors": len(ORCH._functional_gsim_cases(cohort)),
    ...
    *,
    ...
    if lowerer is HQUAL.lower_with_functional_baseline:
        member = HQUAL.RevealedMember(
            ...
            workload=PRODUCER.derive_workload(case.manifest),
            workload_sha256=case.identity,
        )
...
def _capture_case(
    *,
    ...
    try:
        ...
        if document.get("workload_sha256") != case.identity or document.get("workload") != PRODUCER.derive_workload(
            case.manifest
        ):
