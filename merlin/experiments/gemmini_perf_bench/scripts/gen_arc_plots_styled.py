...
def fig_checks():
    ...
    axm.set_xlim(-0.05, 2.05)
    axm.set_ylim(-0.05, 2.05)
    axm.invert_yaxis()
    axm.axis("off")
    ...
    for r, c, txt, fc, tc in cells:
        axm.add_patch(Rectangle((c + 0.04, r + 0.04), 0.92, 0.92, fc=fc, ec=INK, lw=1.4, zorder=3))
        axm.text(
            c + 0.5,
            r + 0.5,
            txt,
    ...
    axm.text(1.0, 2.16, "check verdict  →   accept / reject", ha="center", va="top", fontsize=10.5, color=INK)
    axm.text(-0.14, 1.0, "oracle   pass / fail", rotation=90, va="center", ha="center", fontsize=10.5, color=INK)
    ...
    for i, (v, n) in enumerate(zip(vals, [91, 124])):
        ...
        axr.text(i, v - 5.5, f"{v:.0f}%", ha="center", fontsize=12, fontweight="bold", color="white")
    emph(axr, 1, vals[1] + 8.5, "+23 pts", color=GOLD, fs=12, ha="center")
    axr.set_xticks(xb)
    axr.set_xticklabels(["base checks", "+ fence / preload\n+ config order"], fontsize=10.5)
    axr.set_ylim(0, 105)
...
def fig_landscape():
    ...
    pts = [
        ...
        (
            "arc middle-tier (ours)",
            arc_thru,
            3.2,
            NAVY,
            f"RTL datapath: bit-exact numerics · cycles @ ideal-mem · {ARC_WALL_MED * 1e3:.0f} ms",
            "B",
        ),
    ...
    ax.set_xscale("log")
    ax.set_xlim(veri_thru / 4, 1e4)
    ax.set_ylim(0.4, 5.0)
    # RTL-faithful band + right-edge label (clear of every bubble)
    ax.axhspan(2.9, 4.7, color=NAVY, alpha=0.05, zorder=0)
    ax.text(
        7e3, 4.55, "RTL-derived\ntiers", ha="right", va="top", fontsize=10, color="#9a8f78", style="italic", zorder=1
    )
    # bubbles
    ...
    ax.annotate(
        "",
        xy=(arc_thru, 3.05),
        xytext=(veri_thru, 3.5),
        arrowprops=dict(arrowstyle="-|>", color=GOLD, lw=2.4, alpha=0.95, connectionstyle="arc3,rad=-0.18"),
        zorder=4,
    )
