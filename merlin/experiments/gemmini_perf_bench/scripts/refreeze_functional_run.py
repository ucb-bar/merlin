...
``bundle_input_snapshot`` V4 record (a real read-only on-disk tree with its own ``snapshot.json``),
...
# ------------------------------------------------------------------------------ snapshot staging
def materialize_snapshot(ws: Path, bundle: Mapping, repo: Path, *, descriptor=None) -> tuple[Path, dict]:
    """Build (or re-verify) the immutable V4 bundle-input snapshot through real bwrap machinery."""
    from merlin.targetgen.sandbox import bwrap as BW

    ws.mkdir(parents=True, exist_ok=True)
    BW.materialize_bundle_inputs(ws, dict(bundle), repo=repo, descriptor=descriptor)
    ...
    if record.get("version") != 4:
        raise RefreezeError(f"verified refreeze requires a new V4 bundle snapshot: {record}")
    return root, record
...
def stage_public_cohort(te, snapshot_root: Path, repo: Path, names: Sequence[str], dest: Path) -> list[str]:
    ...
    roots = [BW._snapshot_path(snapshot_root, te.capsule_corpus, repo)]
    roots += [BW._snapshot_path(snapshot_root, repo / rel.rstrip("/"), repo) for rel in te.corpus_siblings()]
...
def stage_hidden_cohort(te, snapshot_root: Path, repo: Path, names: Sequence[str], dest: Path) -> list[str]:
    ...
    source = BW._snapshot_path(snapshot_root, repo / hidden_rel.rstrip("/"), repo)
...
def run_official_grade(
    ...
    *,
    ...
    env["MERLIN_MODEL_HOST_LANE_SNAPSHOT_RECORD"] = json.dumps(environment["bundle_input_snapshot"], sort_keys=True)
    print(f"[refreeze] grading: {' '.join(cmd)}", flush=True)
    from merlin_experiments.frozen_python import inherited_python_command

    return subprocess.run(inherited_python_command(cmd), cwd=str(repo), env=env).returncode
...
def main(argv: list[str] | None = None) -> int:
    ...
    # This retained CLI still uses the native target-selector default. Formal grading and
    # freezing are package-owned and receive explicit context in the child command.
    ...
    snapshot_root, snapshot = materialize_snapshot(ws, bundle, repo, descriptor=te)
    print(
        f"[refreeze] snapshot: {snapshot_root} "
        f"({snapshot['n_files']} files, {snapshot['n_bytes']} bytes, "
        f"{(_dt.datetime.now(_dt.UTC) - started).total_seconds():.1f}s)",
        flush=True,
    )
    ...
    if snapshot["version"] == 4:
        # Preserve the source authoring manifest carried above. This separately archived bundle
        # describes the new re-grade, and must never be reconstructed from a later live checkout.
        import hashlib

        frozen_manifest = run_dir / "refreeze_input_bundle_manifest.yaml"
        shutil.copy2(bundle_manifest_path, frozen_manifest)
        if _load_yaml(frozen_manifest) != bundle:
            raise RefreezeError("re-freeze input bundle changed while staging")
        env["bundle_manifest_file"] = frozen_manifest.name
        env["bundle_manifest_sha256"] = hashlib.sha256(frozen_manifest.read_bytes()).hexdigest()
    ...
    grade_started = _dt.datetime.now(_dt.UTC)
    ...
    elapsed = (_dt.datetime.now(_dt.UTC) - grade_started).total_seconds()
