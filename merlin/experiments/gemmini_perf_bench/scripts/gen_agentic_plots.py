...
def _strip(ax, getval, sc=1.0):
    ...
    for a in ARMS:
        ...
        if not vs:
            continue
        x = XPOS[a] + np.random.uniform(-0.05, 0.05, len(vs))
        ax.scatter(x, vs, s=90, color=COLOR[a], edgecolor=S.INK, lw=1, zorder=5)
        ax.hlines(np.median(vs), XPOS[a] - 0.18, XPOS[a] + 0.18, color=S.INK, lw=1.5, zorder=6)
    _xlabels(ax)
...
def fig_effort():
    metrics = [
        ("cost (USD)", lambda r: r.get("cost_usd"), 1),
        ("tokens (M)", lambda r: r.get("tokens_total"), 1e-6),
        ("tool calls", lambda r: r.get("tool_calls"), 1),
        ("wall (min)", lambda r: r.get("wall_s"), 1 / 60),
    ...
    fig.suptitle(
        "Authoring effort — baseline vs merlin vs merlin+CIRCT (dot=run, bar=median)",
        fontsize=15,
        fontweight="bold",
        y=1.02,
    )
    S.caption(
        fig,
        "Each dot = one valid converged run; bar = per-arm median. N per arm on the axis. "
...
def fig_convergence():
    ...
    for a in ARMS:
        for r in _valid(a):
            ...
            if xs and ys and len(xs) == len(ys):
                ...
                seen.add(a)
                ax.step(xs, ys, where="post", color=COLOR[a], lw=2, alpha=0.85, marker="o", label=lab)
    ax.set_xlabel("agent round")
    ax.set_ylabel("pilot capsules passing (of 4)")
    ...
    S.caption(
        fig,
        "Pilot 4-capsule QA loop; converge when all 4 pass. One line per valid run, coloured by "
...
def fig_completeness():
    ...
    for a in ARMS:
        ...
        ax.text(
            XPOS[a], pub + hid + 0.3, f"{pub + hid}/{n_total}", ha="center", va="bottom", fontsize=10, fontweight="bold"
        )
    ...
    _xlabels(ax)
    ax.set_ylabel("capsules passed (RTL oracle L2+L3)")
    ax.set_ylim(0, n_total * 1.12)
    from matplotlib.patches import Patch

    ax.legend(
        handles=[
            Patch(fc="#CCC", ec=S.INK, label="public (20)"),
            Patch(fc="#CCC", ec=S.INK, hatch="//", label="hidden (5)"),
        ],
        fontsize=8,
        loc="upper left",
    )
    ...
    S.caption(
        fig,
        "Each frozen dialect re-graded on the ENTIRE 25-capsule corpus (20 public + 5 hidden) on "
...
def fig_coverage():
    ...
    for j, a in enumerate(ARMS):
        ...
        if not any(vals):
            continue
        ax.bar(x + (j - (len(ARMS) - 1) / 2) * w, vals, w, color=COLOR[a], edgecolor=S.INK, lw=0.8, label=f"{LABEL[a]}")
    ax.plot(x, [cov[c]["n"] for c in classes], "k_", markersize=14, label="capsules in class")
    ax.set_xticks(x)
    ax.set_xticklabels([c.replace("matmul+", "+") for c in classes], rotation=30, ha="right", fontsize=8)
    ax.set_ylabel("capsules passed")
    ax.legend(fontsize=8)
    ax.set_title("Capability coverage by op-class (full suite, best run/arm)", pad=10)
    S.caption(
        fig,
        "Per-op-class capsules passed on the full suite, best valid run per arm. Black ticks = "
...
def fig_per_capsule_effort():
    ...
    _strip(a1, lambda r: (r.get("cost_usd") or 0) / 4)
    a1.set_title("cost per pilot capsule ($)", fontsize=11)
    _strip(a2, lambda r: (r.get("tokens_total") or 0) / 4, 1e-6)
    a2.set_title("tokens per pilot capsule (M)", fontsize=11)
    fig.suptitle(
        "Downstream efficiency — authoring effort per pilot capsule passed", fontsize=14, fontweight="bold", y=1.02
    )
    S.caption(
        fig,
        "Total effort ÷ 4 (all valid runs converged 4/4 pilot capsules). dot=run, bar=median, "
