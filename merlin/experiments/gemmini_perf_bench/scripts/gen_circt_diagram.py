...
from merlin_plotstyle import BG, GOLD, INK, MAUVE, NAVY, SAGE, SANS, SERIF, SLATE, suptitle, use_merlin_style
...
def alabel(ax, x, y, s, *, fs=9.5, color=INK, weight="normal"):
    ax.text(
        ...
        fontfamily=SANS,
        linespacing=1.0,
        bbox=dict(boxstyle="round,pad=0.18", fc=BG, ec="#d9cfc0", lw=0.7),
    )
...
def main():
    ...
    for i, lab in enumerate(albls):
        ...
        alabel(ax, (x0 + x1) / 2, cy + 0.04, lab, fs=8.7, color=(NAVY if i < 4 else "#7a6a40"), weight="bold")

    # ---------- detail card 1: HW-dialect — what we read ----------
    lx, ty, cwd = panel(ax, 0.45, 4.15, 3.75, 2.75, "HW-dialect MLIR — what we read")
    rows = [
        ("module port widths", "i8 · i32"),
        ("systolic array", "16 × 16"),
        ("scratchpad / acc mem", "256 KB"),
        ("ISA funct table", "legal funct"),
    ]
    ...
    finds = [
        ("funct opcode", "in legal table", "0x1f", "illegal"),
    ...
    ax.text(
        lx,
        ty + 0.02,
        "compiled assertions  ·  run by the LLVM FileCheck binary",
        ha="left",
        va="top",
        color=NAVY,
        fontsize=11,
    ...
    def pill(px, py, w, txt, active):
        ...
        ax.text(
            ...
            ha="center",
            va="center",
            color=("white" if active else "#8a8174"),
            fontsize=10,
            fontfamily=SANS,
            fontweight="bold",
            zorder=6,
    ...
    bullets = [
        "grounded in the real RTL — not documentation",
    ...
    fig.text(
        0.5,
        0.93,
        "it compiles the real Gemmini RTL into machine-checkable assertions that grade the agent's kernel each round",
        ha="center",
        va="center",
        fontsize=13,
        color=INK,
