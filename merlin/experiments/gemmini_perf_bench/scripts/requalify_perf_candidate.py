...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--output-record", type=Path, required=True, help="record path beneath a fresh output directory"
    )
    parser.add_argument(
        "--descriptor", type=Path, help="target descriptor; defaults to the exact descriptor named by the source record"
    )
    ...
    try:
        ...
        record = VERIFY.requalify_audit_only_candidate(args.source_record, args.output_record, target)
    except (OSError, KeyError, ValueError, StageGateError) as exc:
        print(f"NO-GO: {exc}")
        return 2
