...
def _verify_chia_launch_receipt() -> dict[str, Any]:
    ...
    command_matches = (
        ...
        and len(recorded_command) == len(current_command)
    ...
    expected_command_artifacts = [
        {
            "index": index,
            "path": str(Path(current_command[index]).resolve()),
    ...
    if (
        receipt.get("schema") != "merlin.chia-agentic-perf-launch.v1"
        ...
        or any(
            not isinstance(assigned.get(name), (int, float))
            or isinstance(assigned.get(name), bool)
            or float(assigned[name]) < 1.0
...
def _verify_resume_chia_identity(saved: Mapping[str, Any], current: Mapping[str, Any]) -> None:
    ...
    """
    stable_keys = ("plan_sha256", "required_resources", "wrapper", "chia_trace", "command", "command_artifacts")
...
def _telemetry_treatment_identity(preflight: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        or any(not isinstance(source, Mapping) or not _is_sha(source.get("sha256")) for source in sources.values())
    ):
    ...
    if (
        not isinstance(model_resolution, Mapping)
...
def _verify_trial_treatments(handoffs: Mapping[str, PAS.VerifiedCandidateHandoff], expected: Mapping[str, Any]) -> None:
...
def _verify_live_agent_treatment(config: Config, expected: Mapping[str, Any]) -> None:
    ...
    try:
        current = PAS.telemetry_preflight(
            model=config.model, price_table=config.telemetry_price_table, codex_binary=config.codex_binary
        )
...
def _verify_trial_contract(trial: str, handoff: PAS.VerifiedCandidateHandoff, expected: Mapping[str, Any]) -> None:
...
def subprocess_runner(argv: Sequence[str], cwd: Path, environment: Mapping[str, str]) -> CommandResult:
    completed = subprocess.run(list(argv), cwd=cwd, env=dict(environment), capture_output=True, text=True)
...
def _score_cohort_boundary(
    ...
    if (
        ...
        or admission.get("excluded_name_set_sha256") != _name_set_sha256(tuple(set(source) - set(admitted)))
    ):
        raise ExperimentError(f"frozen {label} cohort-admission record does not close")
    return admitted, dict(admission)


def _functional_grade_cohort_from_run(target: object, functional: PAS.StageFunctionalRun) -> FunctionalGradeCohort:
...
def _verify_functional_certificate(
    certificate: GATE.CertificateRecord, cohort: FunctionalGradeCohort
) -> dict[str, Any]:
...
def _functional_regrade_inputs(target: object, cohort: FunctionalGradeCohort) -> tuple[str, str]:
    ...
    materialized_public = discover_capsules(public_root, labels={"public", "dev"}, contract=PB.REPO / "merlin/contract")
...
def _frozen_functional_regrade_inputs(root: Path, cohort: FunctionalGradeCohort) -> tuple[str, str, Path]:
    ...
    for capsule in (*cohort.public, *cohort.hidden):
        if (
            ...
            or GATE.workload_sha256(PAIRED.CERTPROD.derive_workload(capsule.manifest)) != capsule.workload_sha256
        ):
    ...
    _score_cohort_boundary(
        {
            ...
            "cohort_admission": {
                ...
                "admitted_name_set_sha256": _name_set_sha256(tuple(capsule.name for capsule in cohort.hidden)),
...
def _verify_tuning_certificate(
    ...
    corpus = PAS.discover_performance_corpus(target, capsules=capsule_selection, families=family_selection)
    ...
    for member in corpus.capsules:
        identity = GATE.workload_sha256(PAIRED.CERTPROD.derive_workload(member.source_dir / "capsule.yaml"))
    ...
    if selected:
        full_identities = {
            GATE.workload_sha256(PAIRED.CERTPROD.derive_workload(member.source_dir / "capsule.yaml"))
...
def _verify_functional_certificate_provenance(
    ...
    if (
        declaration.get("schema") != "merlin.functional-gsim-qualification.v1"
    ...
    if (
        completion.get("schema") != "merlin.functional-gsim-qualification.v1"
...
def _resolve_chia_python(explicit: Path | None) -> Path | None:
    candidates = [
        explicit,
        Path(os.environ["MERLIN_CHIA_PYTHON"]) if os.environ.get("MERLIN_CHIA_PYTHON") else None,
...
def _chia_canary(python: Path | None) -> dict[str, Any]:
    ...
    completed = subprocess.run(
        [str(resolved), "-c", program], cwd=PB.REPO, env=environment, capture_output=True, text=True, timeout=30
    )
...
def preflight(config: Config, *, heldout_certificate_provider_available: bool = False) -> dict[str, Any]:
    ...
    oversized = {
        name: value for name, value in bounded_simulator_deadlines.items() if value > PAS.ITERATION_MAX_SECONDS
    }
    if oversized:
        raise ExperimentError(
...
def _verify_saved_file(saved: Mapping[str, Any], expected: Path, *, label: str) -> Path:
    ...
    if not path.is_file() or path.resolve() != expected.resolve() or _sha_file(path) != saved.get("sha256"):
...
def _handoff(path: Path, target: object) -> PAS.VerifiedCandidateHandoff:
    return PAS.verify_candidate_handoff(path, verify_authoring_tools=False, target_experiment=target)
...
def _verify_measurement_manifest(
    ...
    *,
    ...
    if manifest.get("measurement_plan_sha256") != PAIRED._sha256_bytes(PAIRED._canonical_bytes(plan)):
        raise ExperimentError("paired measurement plan digest is invalid")
    try:
        expected_results = tuple(PAIRED.ResultIdentity(**row) for row in plan.get("expected_results") or [])
    ...
    if (
        certificate.get("sha256") != certificate_sha256
...
def _author_candidates(
    ...
    *,
    ...
    for trial in TRIALS:
        ...
        _verify_trial_contract(trial, handoff, declaration["trial_contracts"][trial])
        if handoff.telemetry_evidence.get("preflight_sha256") != _sha_bytes(_canonical(declaration["agent_telemetry"])):
    ...
    *,
    ...
    for cell in cells:
        ...
        if (
            ...
            command = [
                sys.executable,
                str(HERE / "run_paired_perf_bench.py"),
                "--functional-run-id",
                config.functional_run_id,
                "--functional-submission-sha256",
                config.functional_submission_sha256,
            ...
            for predicate in config.waive_functional_gate or ():
                command += ["--waive-functional-gate", predicate]
            launch = partial(
                _run_checked, command_runner, command, environment=child_environment(config, cell.certificate)
            )
...
def run(
    ...
    *,
    ...
    if config.functional_gsim_certificate is not None and config.functional_gsim_certificate_sha256 is not None:
        ...
        _require_same_gsim_build(tuning_certificate, functional_certificate, label="functional GSIM certificate")
        _verify_functional_certificate_provenance(functional_certificate, tuning_certificate, functional.digest)
        _verify_functional_certificate(functional_certificate, functional_cohort)
        functional_descriptor, _functional_descriptor_binding = _functional_qualification_descriptor(
            functional_certificate, functional_cohort
        )
    ...
    public_roots, hidden_roots, frozen_contract = _frozen_functional_regrade_inputs(root, functional_cohort)
    regrades = {}
    for trial, handoff in handoffs.items():
        ...
        if saved is None:
            ...
            if _uncheckpointed_state(grade_dir, manifest_path, label=f"functional regrade {trial}") == "absent":
                ...
                regrade_environment = child_environment(config, functional_certificate or tuning_certificate)
    ...
    final = {
        ...
        "measurement_manifests": [
            {"trial": trial, "path": str(path), "sha256": _sha_file(path)} for trial, path in measurement_manifests
        ],
