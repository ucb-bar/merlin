...
def score(
    records: Sequence[Mapping], *, target: str, peak_macs_per_cycle: float, rates_by_class: Mapping[str, float]
) -> dict:
    ...
    return {
        ...
        "programs_undecided": len(undecided),
        ...
        "worst_workload": (min(per_workload.items(), key=lambda kv: kv[1]["rate"])[0] if per_workload else None),
        "rows": sorted(rows, key=lambda row: (row["workload"], row["digest"])),
        "undecided": undecided[:40],
        "licence": (
            "a band may ELIMINATE a candidate and may never certify one; this rate says only "
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--root", action="append", default=[], help="a run tree to harvest (repeatable)")
    parser.add_argument("--run-prefix", default="", help="run-directory name prefix stripped when naming the workload")
    parser.add_argument(
        "--peak-macs-per-cycle",
        type=float,
        default=None,
    ...
    if peak is None:
        ...
        if len(peaks) != 1:
            ...
            print(
                f"refusing: {len(peaks)} compute peak(s) derived for {args.target!r} ({names}); "
                f"pass --peak-macs-per-cycle to name the one this corpus runs on",
                file=sys.stderr,
            )
            return 2
    ...
    if not records:
        print(
            f"refusing: no run under {[str(r) for r in roots]} carries both a {BUFFER_NAME} and a "
            f"console with a {CYCLE_METRIC} metric",
            file=sys.stderr,
        )
        return 2
    ...
    report["rates_fitted_on_training_workloads_only"] = {
        ...
        "held_out_workloads": len(held_out),
        "by_compute_class": provenance,
    }
    report["in_sample"] = score(train, target=args.target, peak_macs_per_cycle=peak, rates_by_class=rates)[
        "containment"
    ]

    out = Path(args.out) if args.out else (artifacts_dir() / "perf-bench" / args.target / ARTIFACT_NAME)
    ...
    print(
        f"{report['programs_decided']} program(s) over {report['workloads']} workload(s); "
        f"{report['programs_undecided']} undecided"
    )
    print(
        f"contained {containment['contained']}  below-floor {containment['below_floor']}  "
        f"above-ceiling {containment['above_ceiling']}" + (f"  rate {rate:.3f}" if rate is not None else "")
    )
