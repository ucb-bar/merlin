...
def matching_processes(run_root: Path, source_snapshot: str | None, *, proc: Path = Path("/proc")) -> list[dict]:
    ...
    allowed_scripts = (
        {
            str(
                (
                    ...
                    / "merlin/experiments/gemmini_perf_bench/scripts/launch_global_agent_experiment.py"
                ).absolute()
            )
        }
        if source_snapshot
    ...
    for entry in proc.iterdir():
        ...
        try:
            ...
            scripts = [token for token in argv if token.rsplit("/", 1)[-1] == "launch_global_agent_experiment.py"]
...
def compare_portfolio_iterations(seed: dict[str, Any], retained: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for (identity, left), (_, right) in zip(before, after, strict=True):
            ...
            changed = any(
                value.get("delta") not in (None, 0)
                for value in (comparison.get("metrics") or {}).values()
                if isinstance(value, dict)
            )
            rows.append({"identity": identity, "comparison": comparison, "has_nonzero_structural_delta": changed})
        return {
            "schema": "phase2_portfolio_structural_comparison_v1",
            "status": "compared",
            "members": rows,
            "changed_members": sum(row["has_nonzero_structural_delta"] for row in rows),
...
def collect_status(
    run_root: Path,
    *,
    milestones: Path | None = None,
    source_snapshot: Path | None = None,
    process_reader=matching_processes,
) -> dict[str, Any]:
    ...
    continuations = [
        read(path)
        for path in sorted(folder.glob("continuation_*.json"))
    ...
    if not checkpoints and (folder / "initial_seed_candidate.json").is_file():
        ...
        checkpoints = [
            {
                "path": str(folder / "initial_seed_candidate.json"),
                "round": -1,
    ...
    portfolio_comparison = {
        "schema": "phase2_portfolio_structural_comparison_v1",
        "status": "UNKNOWN",
        "reason": "no bound retained checkpoint and seed portfolio",
        "members": [],
        "aggregation": "none; each model is compared only with itself",
    }
    ...
    measured = {
        "full_model_speedup": "UNPROVEN",
        "scope": "short mechanism receipts only; not model timing",
    ...
    return {
        "schema": "phase2_read_only_status_v1",
        ...
        "source_snapshot_origin": "launch_receipt"
        if launch.get("source_snapshot")
        else "explicit_host_startup_path"
        if snapshot
        else "UNKNOWN",
        ...
        "round": {
            "latest_prepared_workspace": prepared[-1].name if prepared else None,
            "latest_terminal_audit": {key: audits[-1].get(key) for key in ("round", "status", "agent_exit_code")}
            if audits
            else None,
            "note": "prepared workspace is not proof that authoring started",
            "completed_audits": len(audits),
            "failed_continuations": len(failures),
        },
        "iterations": {
            "processed": len(iterations),
            "latest_index": latest.get("iteration"),
            ...
            "latest_analysis_execution": (
                "exact_static_analysis_reuse"
                if latest.get("analysis_reuse")
                else "full_graph_compilation"
                if latest
                else None
            ),
        ...
        "retained_iteration_binding_verified": retained is not None,
        "budget": {
            "total_authoring_seconds": launch.get("total_authoring_seconds"),
            "round_seconds": launch.get("round_seconds"),
            "iteration_seconds": launch.get("iteration_seconds"),
        ...
        "development_gaps": {
            "full_model_numerical_qualification": "UNPROVEN",
            "full_model_speedup": "UNPROVEN",
            "phase1_passed": phase1.get("public_passed"),
            "phase1_total": phase1.get("public_total"),
            "frozen_phase1_gap_ids": phase1.get("known_functional_gap_ids", []),
...
def render_markdown(status: dict) -> str:
    ...
    lines = [
        "# Phase-2 progress",
        "",
        f"Updated: {status['refreshed_at']}",
        "",
        f"Run: `{status['run_root']}`",
    ...
    if changed:
        ...
        lines += [
            f"| {capsule} | {name} | {value.get('before')} | {value.get('after')} | {value.get('delta')} |"
    ...
    lines += [
        "",
        "Full-model speedup: **UNPROVEN**. Short probe results do not establish full-model timing.",
        "Authored checkpoints are not numerical/performance promotion. Frozen Phase-1 qualification is unchanged.",
        "",
        "## Development notes",
        "",
        "```json",
        json.dumps(status["milestones"], indent=2),
        "```",
        "",
...
def refresh_loop(
    refresh, *, watch_seconds: float = 0, interval_seconds: float = 30, monotonic=time.monotonic, sleep=time.sleep
) -> None:
    ...
    while True:
        ...
        first = False
        status = refresh()
        if watch_seconds == 0 or (status["terminal_receipt_present"] and not status["live_processes"]):
            return
...
def main() -> int:
    ...
    parser.add_argument(
        "--milestones", type=Path, help="Separate host-authored JSON notes; never a qualification receipt"
    )
    parser.add_argument(
        "--source-snapshot", type=Path, help="Exact host-known startup source path; must match launch.json when present"
    )
    parser.add_argument(
        "--watch-seconds", type=float, default=0, help="Bounded refresh duration, maximum 3600; default one shot"
    )
