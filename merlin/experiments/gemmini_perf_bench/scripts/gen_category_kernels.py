...
MOVE = [
    ("K_move_16x16", 16, 16, "single DIM tile movement (capsule A1 shape)"),
...
ATTN = [
    ("K_attn_qk_64x64x64", "attn_qk", 64, 64, 64, "Q", "Kt", "QK^T (seq64, head64)"),
    ("K_attn_pv_64x64x64", "attn_pv", 64, 64, 64, "P", "V", "PV (seq64, head64)"),
    ("K_attn_qk_128x64x128", "attn_qk", 128, 64, 128, "Q", "Kt", "QK^T (seq128, head64)"),
]

_CLASSES = ["FLUSH", "CONFIG_EX", "CONFIG_LD", "MVIN", "CONFIG_ST", "PRELOAD", "COMPUTE_PRELOADED", "MVOUT"]
...
def conv_capsule(kid, N, H, W, Ci, Kh, Kw, Co, s, note):
    ...
    cap = {
        ...
        "source_reference": "bareMetalC/conv.c",
        "label": "dev",
        "interface_mlir": "capsule.interface.mlir",
        "inputs": [
            {"name": "W", "role": "weight", "shape": [Krow, Co], "dtype": "i8"},
        ...
        "expected": {
            "instruction_classes": _CLASSES,
            "modes": {"conv2d": True, "i8": False, "relu": False, "acc_scale": False, "k_accumulate": Krow > PB.DIM},
        },
    ...
    entry = {
        ...
        "op": "conv2d",
        "dtype": "i8",
        "shape": f"{N}x{H}x{W}x{Ci}_k{Kh}x{Kw}x{Co}s{s}",
        "macs": macs,
        "sim_hint": "L2+L3" if macs <= 2_000_000 else "L2_only",
        "source": "baremetalc:conv.c",
        "note": note,
    }
...
def move_capsule(kid, M, N, note):
    cap = {
        ...
        "kind": "isa",
        "source_role": "uplifted_from_bareMetalC",
        "source_reference": "bareMetalC/mvin_mvout.c",
        "label": "dev",
        "interface_mlir": "capsule.interface.mlir",
        "inputs": [{"name": "X", "role": "input", "shape": [M, N], "dtype": "i8"}],
        "operation": {"op": "movement", "attributes": {"src": "X", "out": "Y0", "semantic": "mvin_mvout"}},
        "numeric_policy": {"compare": "exact_int", "dtype": "i8"},
        "expected": {
            "instruction_classes": ["FLUSH", "CONFIG_LD", "MVIN", "CONFIG_ST", "MVOUT"],
    ...
    entry = {
        ...
        "macs": 0,
        "sim_hint": "L2+L3",
        "source": "baremetalc:mvin_mvout.c",
        "note": note,
    }
    return cap, mlir, entry
...
def _write(cap, mlir):
    ...
    (d / "golden.yaml").write_text(
        yaml.safe_dump({"golden_source": "merlin_tensor_int", "outputs": gold}, sort_keys=False)
    )
