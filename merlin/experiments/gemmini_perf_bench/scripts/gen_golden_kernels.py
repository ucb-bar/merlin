...
def main(argv: list[str] | None = None) -> int:
    ...
    for e in corpus:
        print(
            f"  {e['id']:30s} {e['M']}x{e['K']}x{e['N']} {e['output_dtype']:3s} "
    ...
    for e in corpus:
        cap = MSE.make_matmul_capsule(
            name=e["id"],
            semantic=f"golden_matmul_{e['source']}",
            M=e["M"],
            K=e["K"],
            N=e["N"],
            lhs="X",
            ...
            acc_scale=e.get("acc_scale"),
            label="dev",
            source_reference=e["source"],
        )
