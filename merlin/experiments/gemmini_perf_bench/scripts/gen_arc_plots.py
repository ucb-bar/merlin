...
def fig_landscape():
    ...
    pts = [  # name, throughput(kernels/s), fidelity, color, note, (label dx,dy), (note dx,dy)
        (
            ...
            1.0,
            S.COLOR["baseline"],
            "fast, NOT faithful (cyc plateau ~120)",
            (0, 26),
            (0, -26),
        ),
        (
            ...
            3e2,
            2.0,
            S.COLOR["iree_dialect"],
            "~3 ms; structural only (no numerics)",
            (0, 26),
            (0, -26),
        ),
        (
            "arc middle-tier (this work)",
            arc_thru,
            3.2,
            S.COLOR["merlin_targetgen"],
            f"RTL numerics+cycles · NO boot · {ARC_WALL_MED * 1e3:.0f} ms median",
            (0, 28),
            (0, -28),
        ),
        (
            ...
            3.7,
            S.COLOR["golden"],
            f"RTL-faithful, boot-dominated (~{VERILATOR_WALL:.0f} s median)",
            (78, 14),
            (78, -2),
        ),
        (
            ...
            fsim_thru,
            4.4,
            S.COLOR["merlin_native"],
            f"RTL + FPGA (~{FIRESIM_WALL:.0f} s/run)",
            (78, 14),
            (78, -2),
        ),
    ]
    ...
    ax.annotate(
        "",
        xy=(arc_thru, 3.25),
        xytext=(veri_thru, 3.6),
        arrowprops=dict(arrowstyle="->", color=S.COLOR["merlin_targetgen"], lw=2, alpha=0.8),
    )
    ax.text(
        arc_thru**0.5 * veri_thru**0.5,
        3.66,
        f"~{ratio:,.0f}× faster\nsame RTL fidelity",
        ha="center",
        fontsize=8.5,
        fontweight="bold",
        color=S.COLOR["merlin_targetgen"],
    )
    ax.set_xscale("log")
    ax.set_xlabel(
        "← slower            throughput = 1 / measured wall  (kernels/s, log)            faster →", fontsize=10
    )
    ax.set_yticks([1.0, 2.0, 3.2, 3.7, 4.4])
    ax.set_yticklabels(
        ["functional\nnumerics", "structural\n(ISA-legal)", "RTL numerics\n+ cycles", "RTL (full)", "RTL + FPGA"],
        fontsize=8,
    )
    ax.set_ylabel("fidelity  →", fontsize=10)
    ax.set_ylim(0.3, 4.9)
    ax.set_xlim(min(veri_thru, fsim_thru) / 3, 1e4)
    ax.set_title("Where each Gemmini oracle sits — the arc middle-tier fills the spike↔verilator gap", pad=12)
    S.caption(
        fig,
        f"MEASURED: arc wall median {ARC_WALL_MED * 1e3:.1f} ms (20 capsules); verilator wall "
...
def fig_speed():
    ...
    bars = ax.bar(
        ...
        arc_wall,
        0.62,
        label="arc middle-tier (measured)",
        color=S.COLOR["merlin_targetgen"],
        edgecolor=S.INK,
        lw=0.8,
    )
    if VERILATOR_WALL:
        ax.axhline(
            VERILATOR_WALL,
            color=S.COLOR["golden"],
            lw=2,
            ls="--",
            label=f"verilator median (measured, {VERILATOR_WALL:.0f} s)",
        )
    if FIRESIM_WALL:
        ax.axhline(
            FIRESIM_WALL,
            color=S.COLOR["merlin_native"],
            lw=2,
            ls=":",
            label=f"FireSim per-run (measured, {FIRESIM_WALL:.0f} s)",
        )
    ax.set_yscale("log")
    ax.set_xticks(x)
    ...
    if ratio:
        S.badge(
            ax,
            len(caps) * 0.5,
            (ARC_WALL_MED * VERILATOR_WALL) ** 0.5,
            f"~{ratio:,.0f}× faster\nthan verilator",
            color=S.COLOR["merlin_targetgen"],
            fontsize=9,
        )
    S.caption(
        fig,
        f"arc wall = MEASURED min-of-5 per capsule (isolated @Gemmini, ideal memory). verilator/"
...
def fig_bitexact():
    ...
    for i, c in enumerate(caps):
        ...
        axg.text(
            0.29, i + 0.5, "✓" if c["bitexact"] else "✗", ha="center", va="center", color="white", fontweight="bold"
        )
    ...
    axc.set_yticks(range(len(cc)))
    axc.set_yticklabels([c["capsule"][:22] for c in cc], fontsize=7)
    axc.invert_yaxis()
    axc.set_xlabel("arc cycles (accelerator, ideal memory)")
    axc.set_title("per-capsule arc cycle count", fontsize=10, pad=8)
    S.caption(
        fig,
        "20 bench capsules · arc output vs exact-int golden · isolated @Gemmini, ideal memory · single run. ✓ = bit-exact (0 mismatches).",
    )
    _save(fig, "bitexact")
...
def fig_latency():
    ...
    L = [s["latency"] for s in sw]
    C = [s["cycles"] for s in sw]
    ...
    ax.set_xlabel("modelled memory read latency (cycles)")
    ax.set_ylabel("A2 matmul cycles")
    ax.set_title("Timing realism: arc cycles scale with memory latency (knob closes the ideal-mem gap)", pad=12)
    S.badge(
        ax,
        L[0],
        C[0],
        "latency 0 = ideal mem\n(238 cyc, bit-exact at every point)",
        color=S.COLOR["merlin_targetgen"],
        fontsize=8,
    )
    S.caption(
        fig,
        "A2 single-tile matmul only (illustrative); x = simulator memory-read latency (an arc knob, not measured DRAM). Output bit-exact at every latency.",
    )
    _save(fig, "latency")
...
def fig_checks():
    ...
    for r in range(2):
        for c in range(2):
            ...
            axm.text(
                ...
                fontsize=13,
                fontweight="bold",
                color="white" if cols[r][c] != "#E8E2D4" else S.INK,
            )
    ...
    axm.set_title(
        "RTL-derived checks vs oracle\n383 real agent runs — 0 false positives",
        fontsize=10.5,
        fontweight="bold",
        pad=10,
    )
    # recall lift bars
    axr.bar(
        ["base checks", "+ fence_bracket\n+preload/config"],
        [91 / 141 * 100, 124 / 141 * 100],
        color=[S.COLOR["baseline"], S.COLOR["merlin_targetgen"]],
        edgecolor=S.INK,
        lw=1,
    )
    ...
    axr.set_ylim(0, 100)
    axr.set_ylabel("recall on real RTL-tier-class failures (%)")
    axr.set_title("recall lift from added general invariants\n(FP stays 0)", fontsize=10, pad=10)
    S.caption(
        fig,
        "383 REAL agent-run traces (biased sample, not random). 0/242 false positives. FN=17 = 9 numerical functional_mismatch (out of static-check scope by design) + 8 other (tool-crash / mode-specific).",
    )
    _save(fig, "checks")
...
def fig_mutation():
    ...
    ax.set_title(
        "Pre-screen catches RTL failures in ~ms vs verilator's ~minutes (0 false-positive on 'original')", pad=28
    )
    ...
    S.caption(
        fig,
        "5 mutants on ONE capsule (A2), illustrative not statistical. verilator wall is boot-dominated + noisy (110-198 s). pre-screen ms is the static-check time. 1 numerical mutant missed by design.",
    )
    _save(fig, "mutation")
...
def fig_hostcomm():
    ...
    metrics = [
        ("rocc cmds", "cmds", 1),
        ("mvin KB", "mvin_B", 1 / 1024),
        ("mvout KB", "mvout_B", 1 / 1024),
        ("busy %", "busy_pct", 1),
    ]
    x = np.arange(len(metrics))
    w = 0.8 / max(len(caps), 1)
    ...
    for j, c in enumerate(caps):
        ...
        b = ax.bar(
            ...
            w,
            label=c.split("_")[0],
            color=[S.COLOR["golden"], S.COLOR["merlin_targetgen"]][j % 2],
            edgecolor=S.INK,
            lw=0.8,
        )
    ...
    ax.set_ylabel("value")
    ax.legend(fontsize=9, title="capsule")
    ax.set_title("Host↔accelerator telemetry the arc tier exposes (RoCC control + DMA traffic + utilization)", pad=12)
    S.caption(
        fig,
        "Measured from the arc harness on 2 capsules (A2 single-tile, C0 MLP). Bytes/cmds are exact; busy% = accelerator-active cycles / total.",
    )
    _save(fig, "hostcomm")
