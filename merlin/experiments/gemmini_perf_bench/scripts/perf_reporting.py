"""Legacy Verilator Arm4 report policy; current GSIM admission is package-owned."""
...
from merlin.common import digest as DIGEST
...
def _read_json(path: Path, *, label: str) -> object:
    if path.is_symlink() or not path.is_file():
        raise CURRENT.ReportingGateError(f"required {label} is absent or linked: {path}")
    try:
        return json.loads(path.read_bytes())
    except (OSError, UnicodeDecodeError, ValueError) as exc:
        raise CURRENT.ReportingGateError(f"{label} is unreadable at {path}: {exc}") from exc


def _mapping(path: Path, *, label: str = "Arm4 reporting artifact") -> dict:
    doc = _read_json(path, label=label)
    if not isinstance(doc, dict):
        raise CURRENT.ReportingGateError(f"{label} must be a mapping: {path}")
    return doc


def _load_sealed_campaign(run: Path) -> dict:
    digest_path = run / "campaign_manifest.digest.json"
    CURRENT.require_read_only(digest_path, label="campaign-manifest digest record")
    record = _mapping(digest_path, label="campaign-manifest digest record")
    required = {"path", "sha256", "n_bytes", "verified", "immutable"}
    if (
        set(record) != required
        or record.get("path") != "campaign_manifest.json"
        or record.get("verified") is not True
        or record.get("immutable") is not True
    ):
        raise CURRENT.ReportingGateError("campaign-manifest digest record is incomplete")
    document = CURRENT.read_immutable_json(
        run / "campaign_manifest.json",
        record.get("sha256"),
        label="sealed campaign manifest",
        expected_n_bytes=record.get("n_bytes"),
    )
    if not isinstance(document, dict):
        raise CURRENT.ReportingGateError("sealed campaign manifest must be a mapping")
    return document


def _simple_component(value: object, *, label: str) -> str:
    if not isinstance(value, str) or not value or Path(value).name != value or value in (".", ".."):
        raise CURRENT.ReportingGateError(f"{label} must be a simple non-empty path component")
    return value
...
def identity_tuple(raw: object, *, owner: str) -> Identity:
    ...
    if not isinstance(raw, Mapping):
        raise CURRENT.ReportingGateError(f"{owner} omits its exact identity mapping")
    values = tuple(
    ...
    if values[2] not in _SIMULATOR_POLICY:
        raise CURRENT.ReportingGateError(f"{owner} names unsupported simulator {values[2]!r}")
...
def expected_identities(campaign: Mapping) -> tuple[Identity, ...]:
    ...
    if not isinstance(raw, list) or not raw:
        raise CURRENT.ReportingGateError("Arm4 campaign has no non-vacuous expected identity set")
    identities = tuple(identity_tuple(item, owner="expected cell") for item in raw)
    if len(identities) != len(set(identities)):
        raise CURRENT.ReportingGateError("Arm4 campaign repeats an expected cell identity")
    ...
    if any(simulators != ["spike", "verilator"] for simulators in by_replica.values()):
        raise CURRENT.ReportingGateError(
            "every Arm4 capsule replicate must schedule one Spike L2 screen followed by one Verilator L3 certification"
        )
    return identities
...
def _candidate_handoff(campaign: Mapping):
    ...
    if (
        ...
        or not isinstance(stage.get("record_path"), str)
        or not stage["record_path"]
        or not DIGEST.is_sha256(stage.get("record_sha256"))
    ):
        raise CURRENT.ReportingGateError("campaign lacks the exact sealed candidate-stage attribution")
    record_path = Path(str(stage.get("record_path") or ""))
    CURRENT.read_immutable_json(record_path, stage.get("record_sha256"), label="performance candidate record")
    try:
        from merlin_experiments.phase2 import campaign as campaign_gate

        handoff = VERIFY.verify_candidate_handoff(record_path, verify_authoring_tools=False)
        expected = campaign_gate.candidate_handoff_record(handoff)
    except Exception as exc:
        raise CURRENT.ReportingGateError(f"performance candidate evidence is not consumable: {exc}") from exc
    if dict(stage) != expected:
        raise CURRENT.ReportingGateError("campaign candidate-stage attribution disagrees with its record")
    tools = handoff.tool_evidence
    probes = tools["tool_probe_results"]
    if (
        not handoff.required_actions
        or not isinstance(probes, list)
        or not probes
        or tools["tool_probe_recheck_results"] != probes
        or any(not isinstance(row, Mapping) or row.get("returncode") != 0 for row in probes)
    ):
        raise CURRENT.ReportingGateError("candidate-stage tool evidence is incomplete or changed")
    ...
    if (
        ...
        raise CURRENT.ReportingGateError("candidate-stage audit is not clean")
    if (
        handoff.target_descriptor.is_symlink()
        or not handoff.target_descriptor.is_file()
        or DIGEST.sha256_bytes(handoff.target_descriptor.read_bytes()) != handoff.target_descriptor_sha256
    ):
        raise CURRENT.ReportingGateError("candidate target descriptor bytes changed")
    for plane in ("outer_codex_control_plane", "inner_execution_plane"):
        ...
        if (
            ...
            or not DIGEST.is_sha256(evidence.get("policy_sha256"))
        ):
            raise CURRENT.ReportingGateError("candidate-stage sandbox evidence is incomplete")
    return handoff
...
def _validate_completion_shape(completion: object) -> dict:
    if not isinstance(completion, Mapping) or set(completion) != _COMPLETION_FIELDS:
        raise CURRENT.ReportingGateError("Arm4 campaign completion metrics do not match the exact-cell schema")
    count_fields = _COMPLETION_FIELDS - {"complete"}
    if any(
        isinstance(completion[field], bool) or not isinstance(completion[field], int) or completion[field] < 0
        for field in count_fields
    ):
        raise CURRENT.ReportingGateError("Arm4 campaign completion counts are malformed")
    if completion.get("complete") is not True or completion.get("expected", 0) <= 0:
        raise CURRENT.ReportingGateError("Arm4 campaign completion is incomplete or vacuous")
...
def _declared_analyzer_identity(preflight: object):
    ...
    try:
        ...
        raise CURRENT.ReportingGateError(f"sealed claim preflight names an unusable analyzer: {exc}") from exc
    if identity is None:
        raise CURRENT.ReportingGateError(
...
def _full_model_cert_tier(sentinel: object, handoff: object) -> str:
    ...
    if not isinstance(sentinel, Mapping):
        raise CURRENT.ReportingGateError("campaign has no full-model admission record")
    ...
    staged = [str(value) for value in (dict(getattr(handoff, "e2e_sentinel", {}) or {}).get("required_tiers") or [])]
    if not staged or declared != staged:
        raise CURRENT.ReportingGateError(
            f"full-model admission was graded against tiers {declared} but the sealed staging handoff declares {staged}"
        )
...
def validate_campaign(campaign: Mapping) -> tuple[dict, tuple[Identity, ...], object]:
    ...
    if (
        campaign.get("status") != "GO"
        or campaign.get("measurement_status") != "GO"
        or campaign.get("refusal") is not None
    ):
        raise CURRENT.ReportingGateError("Arm4 measurement campaign is not GO")
    if campaign.get("approach") != "arm4":
        raise CURRENT.ReportingGateError("reporting accepts the Arm4-only campaign, not another approach")
    ...
    if not isinstance(boundary, Mapping) or not isinstance(boundary.get("reason"), str):
        raise CURRENT.ReportingGateError("Arm4 campaign lacks an explicit claim-decision boundary")
    if mode == "measurement_smoke_only":
        if (
            campaign.get("claim_status") != "NOT_ESTABLISHED"
            ...
            raise CURRENT.ReportingGateError("measurement smoke does not preserve its explicit non-claim boundary")
        ...
        if (
            campaign.get("claim_launch_status") != "GO"
            ...
            or boundary.get("promotion_status") != ("PROMOTED" if status == "ESTABLISHED" else "BLOCKED")
        ):
            raise CURRENT.ReportingGateError("formal claim campaign lacks an exact integrated claim decision")
        analyzer = _declared_analyzer_identity(preflight)
        if boundary.get("module") != analyzer.module or not str(boundary.get("identity_bridge") or "").startswith(
            analyzer.function + "("
        ):
            raise CURRENT.ReportingGateError(
        ...
        raise CURRENT.ReportingGateError("Arm4 campaign has an unsupported experiment mode")
    handoff = _candidate_handoff(campaign)

    run_id = campaign.get("functional_run_id")
    digest = campaign.get("functional_submission_sha256")
    if not isinstance(run_id, str) or not run_id or not DIGEST.is_sha256(digest):
        raise CURRENT.ReportingGateError("Arm4 report lacks a functional run ID and submission SHA-256")
    candidate = campaign["candidate_stage"]
    if (
        campaign.get("snapshot_sha256") != candidate["candidate_sha256"]
        or Path(str(campaign.get("snapshot") or "")).resolve() != Path(candidate["candidate_path"]).resolve()
        ...
        raise CURRENT.ReportingGateError("measured candidate does not trace to the pinned functional submission")
    for phase in ("fork_before", "fork_after"):
        check = campaign.get(phase)
        if not isinstance(check, Mapping) or check.get("ok") is not True or check.get("state") != "held":
            raise CURRENT.ReportingGateError(f"functional invariants were not held at {phase}")
    for phase in ("candidate_before", "candidate_after"):
        ...
        if (
            ...
            raise CURRENT.ReportingGateError(f"sealed candidate invariants were not held at {phase}")
    ...
    try:
        ...
        raise CURRENT.ReportingGateError(f"functional host-lane evidence is invalid: {exc}") from exc
    if verified_host != host:
        raise CURRENT.ReportingGateError("functional host-lane evidence changed while being verified")
    if dict(host) != handoff.host_lane:
        raise CURRENT.ReportingGateError("campaign host lane differs from the verified candidate handoff")
    run_snapshot = host.get("run_snapshot")
    if (
        not isinstance(run_snapshot, Mapping)
        or run_snapshot.get("content_sha256") != handoff.functional_bundle_snapshot_sha256
    ):
        raise CURRENT.ReportingGateError("candidate host lane names a different functional bundle snapshot")
    ...
    if (
        ...
        or sentinel.get("kind") != "model"
        or sentinel.get("source") != "functional_bundle_input_snapshot_v2"
        or not DIGEST.is_sha256(sentinel.get("source_sha256"))
        or isinstance(sentinel.get("n_files"), bool)
        or not isinstance(sentinel.get("n_files"), int)
        or sentinel["n_files"] <= 0
        or isinstance(sentinel.get("n_bytes"), bool)
        or not isinstance(sentinel.get("n_bytes"), int)
        or sentinel["n_bytes"] <= 0
        ...
        raise CURRENT.ReportingGateError(
            f"campaign lacks a passing frozen mixed-lane full-model admission sentinel at {cert_tier}"
        )
    staged_sentinel = handoff.e2e_sentinel
    if sentinel.get("capsule") != staged_sentinel.get("capsule") or sentinel.get(
        "source_sha256"
    ) != staged_sentinel.get("capsule_sha256"):
        raise CURRENT.ReportingGateError("measured full-model admission differs from the verified candidate handoff")
    ...
    if (
        ...
        or not DIGEST.is_sha256(frozen_contract.get("sha256"))
    ):
        raise CURRENT.ReportingGateError("campaign has no authenticated frozen measurement contract")
    try:
        contract_record = P2_CONTRACTS.exact_tree_record(Path(str(frozen_contract.get("path") or "")))
    except Exception as exc:
        raise CURRENT.ReportingGateError(f"frozen measurement contract is invalid: {exc}") from exc
    if contract_record["sha256"] != frozen_contract["sha256"]:
        raise CURRENT.ReportingGateError("frozen measurement contract digest changed")

    sandbox = campaign.get("sandbox")
    if (
        not isinstance(sandbox, Mapping)
        or sandbox.get("engine") != "bwrap"
        or sandbox.get("network") != "available"
        or sandbox.get("package_read_only") is not True
        or sandbox.get("answer_surface_coverage_gap") != []
    ):
        raise CURRENT.ReportingGateError(
            "Arm4 campaign did not record an answer-closed, read-only package sandbox with network available"
        )
    ...
    if (
        ...
        or any(not isinstance(row, Mapping) or not isinstance(row.get("label"), str) for row in probes)
    ):
        raise CURRENT.ReportingGateError("Arm4 campaign has incomplete or vacuous tool-probe evidence")
    by_label = {row["label"]: row for row in probes}
    if set(by_label) != set(required) or any(by_label[label].get("returncode") != 0 for label in required):
        raise CURRENT.ReportingGateError("Arm4 campaign did not pass every enforced tool probe")
    ...
    if mode == "formal_claim":
        ...
        if campaign.get("claim_preflight") != handoff.formal_claim:
            raise CURRENT.ReportingGateError("formal PK preflight differs from the verified candidate handoff")
        replicate_contract = handoff.formal_claim.get("declaration", {}).get("replicates", {})
        if handoff.replicates != replicate_contract.get("exact_count") or list(
            handoff.formal_replicate_identities
        ) != replicate_contract.get("identities"):
            raise CURRENT.ReportingGateError("formal replicate identities differ from the frozen acceptance contract")
    else:
        if (
            isinstance(handoff.smoke_replicates, bool)
            ...
            raise CURRENT.ReportingGateError("candidate handoff has no bounded non-claim smoke replicate count")
        smoke_ids = {f"r{index:03d}" for index in range(handoff.smoke_replicates)}
        expected_prompt_cells = tuple(dict(row) for row in handoff.expected_cells if row.get("replicate") in smoke_ids)
    if tuple(dict(row) for row in campaign["expected_identities"]) != expected_prompt_cells:
        raise CURRENT.ReportingGateError("measurement cells differ from the verified candidate prompt")
    completion = _validate_completion_shape(campaign.get("completion"))
    if completion["expected"] != len(identities):
        raise CURRENT.ReportingGateError("completion expected count disagrees with expected identities")
...
def validate_claim_decision(campaign: Mapping, handoff, rows: Sequence[Mapping]) -> None:
    ...
    try:
        from merlin_experiments.phase2.claims.dispatch import load_analyzer_module

        module = load_analyzer_module(analyzer.module)
        ...
        names = sorted(
            name for name in dir(module) if name.startswith("preflight_") and callable(getattr(module, name, None))
        )
        if len(names) != 1:
            raise CURRENT.ReportingGateError(
        ...
        raise CURRENT.ReportingGateError(f"formal claim evidence cannot be recomputed: {exc}") from exc
    if preflight != campaign.get("claim_preflight"):
        raise CURRENT.ReportingGateError("sealed preflight differs from frozen descriptor recomputation")
    if decision != campaign.get("claim_decision"):
        raise CURRENT.ReportingGateError("sealed claim decision differs from exact-row recomputation")
    status = decision.get("status")
    expected_promotion = "PROMOTED" if status == "ESTABLISHED" else "BLOCKED"
    if (
        status not in {"ESTABLISHED", "REFUTED"}
        or campaign.get("claim_status") != status
        or campaign["decision_boundary"].get("promotion_status") != expected_promotion
    ):
        raise CURRENT.ReportingGateError("formal promotion does not match its recomputed decision")


def _exact_tree_record(root: Path) -> dict[str, int | str]:
    if root.is_symlink() or not root.is_dir():
        raise CURRENT.ReportingGateError(f"frozen workload capsule tree is absent or linked: {root}")
    CURRENT.require_read_only(root, label="frozen workload root")
    digest = hashlib.sha256()
    n_files = n_bytes = 0
    for path in sorted(root.rglob("*"), key=lambda item: item.relative_to(root).as_posix()):
        if path.is_symlink() or (not path.is_dir() and not path.is_file()):
            raise CURRENT.ReportingGateError(f"frozen workload contains a link or special file: {path}")
        CURRENT.require_read_only(path, label="frozen workload entry")
        if path.is_dir():
            continue
    ...
    if n_files <= 0 or n_bytes <= 0:
        raise CURRENT.ReportingGateError("frozen workload capsule tree is empty")
...
def _pinned_path(run: Path, raw: object, expected: Path, *, label: str) -> Path:
    if not isinstance(raw, str) or not raw:
        raise CURRENT.ReportingGateError(f"campaign does not pin its {label}")
    candidate = Path(raw)
    if not candidate.is_absolute():
        candidate = run / candidate
    if candidate.resolve() != expected.resolve():
        raise CURRENT.ReportingGateError(f"campaign {label} escapes its run-private canonical path")
    return expected


def validate_frozen_workload(run: Path, campaign: Mapping, identities: Sequence[Identity], handoff) -> dict:
    ...
    if candidate_manifest != candidate_root / "performance_corpus_manifest.json":
        raise CURRENT.ReportingGateError("candidate-stage workload manifest escapes its frozen corpus")
    root = _pinned_path(run, campaign.get("workload_snapshot"), candidate_root, label="frozen workload snapshot")
    manifest_path = _pinned_path(
        run, campaign.get("workload_manifest"), candidate_manifest, label="frozen workload manifest"
    )
    if (
        campaign.get("workload_manifest_sha256") != handoff.corpus_manifest_sha256
        or campaign.get("workload_sha256") != handoff.corpus_sha256
    ):
        raise CURRENT.ReportingGateError("measurement workload digests disagree with the candidate-stage corpus")
    manifest = CURRENT.read_immutable_json(
        manifest_path, campaign.get("workload_manifest_sha256"), label="frozen workload manifest"
    )
    if not isinstance(manifest, dict) or manifest.get("schema_version") != 1:
        raise CURRENT.ReportingGateError("frozen workload manifest schema is unsupported")
    workload_sha = campaign.get("workload_sha256")
    if not DIGEST.is_sha256(workload_sha) or manifest.get("capsules_sha256") != workload_sha:
        raise CURRENT.ReportingGateError("frozen workload aggregate digest is absent or inconsistent")

    capsules = manifest.get("capsules")
    counts = manifest.get("counts")
    if not isinstance(capsules, list) or not capsules or not isinstance(counts, Mapping):
        raise CURRENT.ReportingGateError("frozen workload manifest has a vacuous capsule set")
    seen: set[tuple[str, str]] = set()
    seen_paths: set[Path] = set()
    for item in capsules:
        if not isinstance(item, Mapping):
            raise CURRENT.ReportingGateError("frozen workload manifest contains a malformed capsule row")
        ...
        key = (family, capsule)
        if key in seen:
            raise CURRENT.ReportingGateError(f"frozen workload repeats capsule identity {key}")
        seen.add(key)
        relative = item.get("snapshot_relative_path")
        if not isinstance(relative, str) or not relative:
            raise CURRENT.ReportingGateError(f"frozen workload capsule {key} has no snapshot path")
        capsule_path = root / relative
        try:
            resolved_capsule = capsule_path.resolve()
            resolved_capsule.relative_to((root / "capsules").resolve())
        except ValueError as exc:
            raise CURRENT.ReportingGateError(f"frozen workload capsule {key} escapes its snapshot") from exc
        if resolved_capsule.name != capsule or resolved_capsule in seen_paths:
            raise CURRENT.ReportingGateError(f"frozen workload capsule {key} does not own one unique named snapshot")
        seen_paths.add(resolved_capsule)
        performance = item.get("performance")
        if not isinstance(performance, Mapping) or performance.get("family") != family:
            raise CURRENT.ReportingGateError(f"frozen workload capsule {key} disagrees with its performance family")
        observed = _exact_tree_record(capsule_path)
        if (
            item.get("source_sha256") != item.get("snapshot_sha256")
            or observed["sha256"] != item.get("snapshot_sha256")
            or observed["n_files"] != item.get("n_files")
            or observed["n_bytes"] != item.get("n_bytes")
        ):
            raise CURRENT.ReportingGateError(f"frozen workload capsule digest changed: {key}")

    expected_capsules = {(family, capsule) for family, capsule, _sim, _rep in identities}
    if seen != expected_capsules:
        raise CURRENT.ReportingGateError("frozen workload capsule set disagrees with expected identities")
    aggregate = _exact_tree_record(root / "capsules")
    if (
        aggregate["sha256"] != workload_sha
        or counts.get("capsules") != len(capsules)
        ...
        raise CURRENT.ReportingGateError("frozen workload aggregate bytes or counts changed")
    return manifest


def validate_rows(rows: Sequence[Mapping], expected: Sequence[Identity], expected_counts: Mapping) -> dict:
    """Recompute every exact-cell metric and require the sealed row sequence to match."""
    if not isinstance(rows, list) or not rows:
        raise CURRENT.ReportingGateError("Arm4 perf_results must contain a non-vacuous row list")
    observed = tuple(
        identity_tuple(row.get("identity"), owner="result row")
        if isinstance(row, Mapping)
        else identity_tuple(None, owner="result row")
        for row in rows
    )
    if observed != tuple(expected):
        raise CURRENT.ReportingGateError("sealed result row sequence disagrees with expected identities")
    ...
    for identity, row in zip(observed, rows, strict=True):
        ...
        if row.get("approach") != "arm4" or any(row.get(key) != value for key, value in policy.items()):
            raise CURRENT.ReportingGateError(f"result {identity} violates its simulator policy")
        is_correct = (
            row.get("correct") is True
            and row.get("tier_status") == "pass"
        ...
        if simulator == "spike":
            if cycles is not None:
                raise CURRENT.ReportingGateError(f"Spike L2 result {identity} exposes non-citable cycles")
    ...
    counts = {
        ...
        "complete": (
            ...
            and failed == 0
            and screen_expected > 0
            and citable_expected > 0
            and screen_passed == screen_expected
            and citable_passed == citable_expected
        ),
    }
    if counts != dict(expected_counts):
        raise CURRENT.ReportingGateError(
            f"Arm4 perf_results completion {counts} disagrees with campaign {dict(expected_counts)}"
        )
    return counts
...
def _load_sealed_results(run: Path, campaign: Mapping) -> list[dict]:
    ...
    required = {"path", "sha256", "n_bytes", "records", "verified", "immutable", "digest_record_sha256"}
    if not isinstance(record, Mapping) or set(record) != required:
        raise CURRENT.ReportingGateError("campaign has no complete sealed-result digest record")
    if (
        record.get("path") != "perf_results.json"
        or record.get("verified") is not True
        or record.get("immutable") is not True
    ):
        raise CURRENT.ReportingGateError("campaign does not identify verified immutable perf_results.json")
    rows = CURRENT.read_immutable_json(
        run / "perf_results.json",
        record.get("sha256"),
        label="sealed perf_results.json",
        expected_n_bytes=record.get("n_bytes"),
    )
    if not isinstance(rows, list) or record.get("records") != len(rows):
        raise CURRENT.ReportingGateError("sealed perf_results row count disagrees with its digest record")

    digest_record = CURRENT.read_immutable_json(
        run / "perf_results.digest.json", record.get("digest_record_sha256"), label="perf_results digest record"
    )
    expected_record = dict(record)
    expected_record.pop("digest_record_sha256")
    if digest_record != expected_record:
        raise CURRENT.ReportingGateError("perf_results digest-record contents disagree with the campaign")
    return rows
...
def load_reportable_run(run: Path) -> tuple[dict, list[dict], dict]:
    ...
    if manifest.is_file() and _mapping(manifest).get("schema") == "paired_arm4_performance_campaign_v2":
        raise CURRENT.ReportingGateError(
            "current paired campaigns require their parent experiment manifest, its expected hash, "
            "and verified candidate handoffs; the legacy Verilator report cannot consume GSIM evidence"
        )
