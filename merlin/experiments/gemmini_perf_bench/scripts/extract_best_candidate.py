...
* The ratio is not the verdict. `candidate_decision.compare` decides whether the best total is a
  result at all -- a cohort whose best equals its baseline is NO_EFFECT rather than a 1.0000x win,
  and an engine that is not the certified timing engine is handed to `compare` with the axes it
  cannot see, so it is excluded rather than ranked on.
"""
...
from merlin.perf.candidate_comparison import (  # noqa: E402
    FUNCTIONAL_SIMULATOR_BLIND_AXES,
    decide_measured_totals,
)

#: The engine whose cycle counts may ORDER two candidates, and the ones that may not. A functional
#: ISA simulator prices every accelerator command at one cycle, so its degradation correlates with
#: success -- it flatters exactly the offloading this search is trying to do (measured: a retarget
#: that nearly doubled mesh instructions moved it 0.2%). `validate_redacted_feedback` already
#: refuses a feedback document from any other engine; this names the same fact where the ranking
#: happens, and hands the blind axes to `compare` rather than silently trusting the number.
CERTIFIED_TIMING_ENGINE = "gsim"
FUNCTIONAL_ONLY_ENGINES = frozenset({"spike"})
...
def choose_best(measurements: "Sequence[Mapping[str, Any]]") -> dict[str, Any]:
    ...
    # THE RATIO IS NOT THE VERDICT. `speedup_vs_baseline` is printed and copied into the exported
    # tree's provenance, and until this call nothing decided whether it was a result at all: a
    # cohort whose best total equals its baseline still produced a confident "1.0000x", and an
    # engine that prices every accelerator command at one cycle would have been ranked on exactly
    # like the certified one. `candidate_decision.compare` is the named instrument for that, and
    # it is given the engine's blind axes rather than trusted to ignore them.
    engines = {str(r.get("engine") or "") for r in cohort}
    engine = engines.pop() if len(engines) == 1 else ""
    decision = decide_measured_totals(
        instrument=engine,
        baseline_cycles=baseline,
        candidate_cycles=lowest,
        blind_to=(FUNCTIONAL_SIMULATOR_BLIND_AXES if engine in FUNCTIONAL_ONLY_ENGINES else ()),
        basis=(
            f"summed measured cycles over the {len(cohort_key)} member(s) every measurement in this "
            f"cohort covered, on engine {engine or 'UNRECORDED'}"
        ),
    )
    return {
        ...
        "cohort_engine": engine or None,
        "certified_timing_engine": engine == CERTIFIED_TIMING_ENGINE,
        "decision": decision,
...
def main(argv: "Sequence[str] | None" = None) -> int:
    ...
    print(f"  {decision.get('why', 'no decision was reached')}")
