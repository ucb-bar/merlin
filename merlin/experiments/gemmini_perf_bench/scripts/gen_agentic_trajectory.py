    ...
    for arm, (sub, label) in ARM_META.items():
        ...
        for r in cands:
            ...
            if (d / "rounds").is_dir() and any((d / "rounds").glob("round_*.transcript.jsonl")):
                picks.append((sub, r["run_id"], label))
                break
    return picks
...
BUCKET = {
    T.KIND_READ: "Reading",
    T.KIND_RESULT: "Reading",
    T.KIND_EDIT: "Writing code",
    T.KIND_BASH: "Running (Bash)",
    T.KIND_THINK: "Thinking",
    T.KIND_TEXT: "Thinking",
}
...
def _panel(ax, run, run_dir, title):
    tl = run["timeline"]
    if not tl:
        ax.text(0.5, 0.5, f"{run['run_id']}\n(no transcript)", ha="center", va="center")
        ax.axis("off")
        return
    n = len(tl)
    ...
    tot = np.sum(list(series.values()), axis=0)
    tot[tot == 0] = 1.0
    ax.stackplot(
        x, *[series[b] / tot for b in ORDER], colors=[CO[b] for b in ORDER], labels=ORDER, alpha=0.92, zorder=2
    )
    ax.set_ylim(0, 1)
    ax.set_xlim(0, 100)
    ...
    axr = ax.twinx()
    axr.set_ylabel("cumulative spend ($)", color=SPEND, fontsize=9)
    axr.tick_params(colors=SPEND, labelsize=8)
    axr.spines["top"].set_visible(False)
    if sx is not None:
        axr.plot(sx, sy, color=SPEND, lw=3.0, zorder=8, solid_capstyle="round")
        axr.set_ylim(0, max(sy) * 1.15)
        axr.annotate(
            f"  ${total_cost:.2f} · {total_tok / 1e6:.1f}M tok",
            xy=(sx[-1], sy[-1]),
            xytext=(-4, 6),
...
def main():
    ...
    picks = _select_runs()
    if not picks:
        print("  (no valid runs with transcripts yet — skipping trajectory)")
        return 0
    ...
    fig.legend(handles=handles, loc="lower center", ncol=5, fontsize=9.5, frameon=False, bbox_to_anchor=(0.5, -0.02))
    fig.suptitle(
        "Authoring trajectory & cumulative spend — baseline vs merlin vs merlin+CIRCT (PILOT)",
        fontsize=15,
        fontweight="bold",
        y=1.0,
    )
    ...
    for run, _rd, lab in runs:
        ...
        totals.append(
            f"{short} ${p.get('estimated_cost_usd', 0):.2f}/{(p.get('tokens_total') or 0) / 1e6:.1f}M ({run['run_id']})"
        )
    S.caption(
        fig,
        "Smoothed activity share (Read/Write/Bash/Think) over each agent's transcript; bold line = "
