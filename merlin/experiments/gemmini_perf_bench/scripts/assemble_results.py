...
def main(argv: list[str] | None = None) -> int:
    ...
    while offset < len(expected):
        ...
        if len(pair_expected) != 2 or any((row[0], row[1], row[3]) != base for row in pair_expected):
            raise CURRENT.ReportingGateError(f"expected identities do not form one simulator pair: {base}")
        path = _pair_file(run, base)
        if path in used_paths:
            raise CURRENT.ReportingGateError(f"multiple identities resolve to per-replicate file {path}")
        used_paths.add(path)
        if path.is_symlink() or not path.is_file():
            raise CURRENT.ReportingGateError(f"per-replicate result is absent or linked: {path}")
        try:
            pair = json.loads(path.read_bytes())
        except (OSError, UnicodeDecodeError, ValueError) as exc:
            raise CURRENT.ReportingGateError(f"per-replicate result is unreadable at {path}: {exc}") from exc
        if not isinstance(pair, list) or len(pair) != 2:
            raise CURRENT.ReportingGateError(f"per-replicate result must contain exactly two cells: {path}")
        observed = tuple(
            PR.identity_tuple(row.get("identity"), owner="per-replicate result")
            if isinstance(row, dict)
            else PR.identity_tuple(None, owner="per-replicate result")
            for row in pair
        )
        if observed != pair_expected:
            raise CURRENT.ReportingGateError(f"per-replicate result {path.name!r} disagrees with expected identities")
    ...
    print(
        f"verified {len(assembled)} sealed Arm4 cells from {len(used_paths)} "
