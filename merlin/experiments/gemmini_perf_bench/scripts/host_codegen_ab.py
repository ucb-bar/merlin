...
def validate_buffers(signature: list, arrays: list) -> None:
    ...
    for index, ((shape, dtype), array) in enumerate(zip(signature, arrays)):
        if (
            ...
            or np.dtype(types[dtype]) != array.dtype
...
def probe(bundle: Path, root: Path, variants: dict, *, int8: bool, vlen: int) -> dict:
    ...
    if any(not isinstance(v, list) or any(not isinstance(f, str) for f in v) for v in variants.values()):
    ...
    baseline = None
    report = {
        "schema": "merlin.host-codegen-ab.v1",
        "bundle": str(bundle),
        "int8_compute": int8,
        "vlen": vlen,
        "variants": {},
    ...
    for name, requested in variants.items():
        ...
        prepared, features = prepare_for_lowering(
            selected / "model.mlir", work, int8_compute=int8, features=frozenset(requested), vlen=vlen
        )
        ...
        ll.write_text(
            lower_to_llvm_ir(
                upstream, workdir=work, vectorize=True, transform_schedule=RVV_TRANSFORM_SCHEDULE, features=features
            )
        )
        obj = compile_ll(ll, work / "model.rvv.o", extra_flags=(f"-march=rv64gcv_zvl{vlen}b",))
        dis = subprocess.run(
            [str(toolchain.objdump()), "-d", "--no-show-raw-insn", str(obj)], capture_output=True, text=True, check=True
        ).stdout
        ...
        row = {
            ...
            "concrete_features": sorted(features),
            "prepared_op_counts": counts,
            "instructions": sum(mnemonics.values()),
            "vector_instructions": sum(n for m, n in mnemonics.items() if m.startswith("v")),
            "mnemonics": dict(mnemonics),
            "bit_identical_to_baseline": same,
        ...
        print(
            json.dumps(
                {
                    "variant": name,
                    **{k: row[k] for k in ("instructions", "vector_instructions", "bit_identical_to_baseline")},
                }
            ),
            flush=True,
        )
