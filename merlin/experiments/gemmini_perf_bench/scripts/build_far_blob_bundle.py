...
def compose_harness(fragments: Path, out: Path, *, target: str) -> Path:
    ...
    from merlin.perf.warm_profile_harness import WarmProfileContract, render_target_warm_then_measure_main
    ...
    bracket = H.occupancy_partition_bracket(Path(counters["header"]).read_text(encoding="utf-8"), slots=8)
    if not bracket["closes_accelerator_busy"]:
        raise SystemExit("refusing a partial counter partition: busy would be a lower bound reported as a total")

    main = render_target_warm_then_measure_main(
        target=target,
        arguments=args,
        prepare_input=frag("reseed"),
        validate_outputs=frag("validate").strip(),
        contract=WarmProfileContract(),
        counter_bracket=bracket,
    )
    prologue = "\n".join(["#include <stdint.h>", "#include <stdio.h>", f'#include "include/{target}_testutils.h"', ""])
...
def main() -> int:
    ...
    ap.add_argument(
        "--bundle", type=Path, required=True, help="dir holding the harness_*.c.frag set, const_blob.bin, plan.json"
    )
    ap.add_argument("--llvm", type=Path, required=True, help="package-emitted llvm-dialect MLIR")
    ap.add_argument(
        "--reference", type=Path, required=True, help="raw reference bytes for merlin_reference (no container header)"
    )
    ...
    data_s = write_bundle_data(bundle, arena_bytes=plan["mutable_bytes"], reference_bin=args.reference.resolve())
    ...
    run("assemble const_blob.S", [cc, *march, "-c", str(bundle / "const_blob.S"), "-o", str(work / "const_blob.o")])
    run("assemble bundle_data.S", [cc, *march, "-c", str(data_s), "-o", str(work / "bundle_data.o")])
    run("compile harness.c", [cc, *cflags, *includes, "-c", str(bundle / "harness.c"), "-o", str(work / "harness.o")])
    support: list[Path] = []
    for source in recipe.support_sources:
        source = Path(source)
        if source.suffix in (".c", ".S", ".s"):
            unit = work / f"{source.stem}.o"
            run(f"compile {source.name}", [cc, *cflags, *includes, "-c", str(source), "-o", str(unit)])
            support.append(unit)
        else:
    ...
    run(
        ...
        [
            ...
            *includes,
            "-T",
            str(recipe.link_script),
            "-o",
            str(elf),
            *[str(o) for o in support],
            str(work / "harness.o"),
            str(obj),
            str(work / "const_blob.o"),
            str(work / "bundle_data.o"),
            *ldflags,
        ],
    )
