...
def claim_groups(descriptor: Path, certificate: Path, certificate_sha256: str, members: list[str]) -> list[dict]:
    ...
    for name in members:
        ...
        decision = GATE.plan_evaluation(
            cert, PAIRED._gsim_workload(member), phase="development_correctness", gsim_available=True
        )
...
def run(root: Path) -> list[dict]:
    ...
    with (root / "launcher.lock").open("a+") as lock:
        ...
        check = subprocess.run(
            [sys.executable, str(snapshot / SCRIPTS / "perf_suite.py"), "preflight", "--root", str(root)],
            cwd=snapshot,
            env=env,
        )
        if check.returncode:
        ...
        def launch(campaign):
            ...
            with (attempt / "console.log").open("xb") as log:
                result = subprocess.run(
                    argv, cwd=snapshot, env=env, stdout=log, stderr=subprocess.STDOUT, pass_fds=(lock.fileno(),)
                )
