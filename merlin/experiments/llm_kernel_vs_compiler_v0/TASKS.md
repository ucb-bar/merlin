...
| 1.2 | `kernel_library.py` — signature-keyed store + reuse ladder L0–L3 | **OPEN** — blocks 4.F and the "new kernel variants" curve |
| 1.3 | `task_basis.py` — weighted set cover over a census + eligibility map | **OPEN** — blocks all of Phase 3 |
...
| 3.3 | Task basis: signature key, eligibility filter, cost weight, greedy cover ≥95%, family floor | **OPEN** — blocked on 1.3. **Until this lands we are not measuring TinyLlama**, only three pre-existing pilot capsules |
| 3.4 | `basis_certificate.json` | **OPEN** |
...
| 4.6 | Multi-seed repeats (≥3) | **RUNNING** — 27-job matrix (3 methods × 3 tasks × 3 seeds) |
...
| 4.9 | Arm F kernel-library reuse + generalization matrix | **OPEN** — blocked on 1.2. This is what makes Arm A *fair*; without it kernel-gen pays full price for every config |
...
| 6.4 | Same pilot on every arm, in parallel and blind | **DONE** — 27-job matrix running sandboxed |
...
1. **1.3 → 3.3 → 3.5** — a real task basis from the TinyLlama census. *Everything measured so far is
   three pilot capsules, not TinyLlama.*
2. **1.2 → 4.9** — the kernel library, so kernel-gen is not charged full price for every config.
   Without it the comparison is unfair **to the kernel arm** and the crossover arrives too early.
3. **4.10 → 4.13** — Merlin-Base, frozen. There is currently **no compiler arm at all**; only the
   kernel-generation side exists.
...
except the multi-seed runs (4.6) which cost provider spend and are running now.

## Standing risks

- **A harness limit reads as a model defect.** Six occurrences on this project, three found in the
  last two days (kernel misroute, zero token accounting, cross-arm reads). Any new number gets an
  adversarial pass over the *trace* before it is quoted, not just the score.
