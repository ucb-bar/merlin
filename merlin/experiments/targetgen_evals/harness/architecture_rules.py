...
def _rule(rule_id: str, name: str, passed: bool, severity: str, message: str) -> dict:
    return {"rule_id": rule_id, "name": name, "passed": passed, "severity": severity, "message": message}
...
def check_r1(run_dir: Path, manifest: dict) -> dict:
    ...
    if expected.exists():
        return _rule("R1", "generated-repo-naming", True, "info", f"generated/{target}-mlir/ exists")
    return _rule(
        "R1",
        "generated-repo-naming",
        False,
        "error",
        f"generated/{target}-mlir/ does not exist; all generated output must live under this directory",
    )
...
def check_r2(run_dir: Path, manifest: dict) -> dict:
    ...
    if xdsl_dir.exists() and any(xdsl_dir.iterdir()):
        return _rule("R2", "xdsl-before-tablegen", True, "info", f"xdsl/ directory exists and is non-empty")
    return _rule(
        "R2",
        "xdsl-before-tablegen",
        False,
        "warning",
        "xdsl/ directory is absent or empty; xDSL artifacts must exist before TableGen/C++ promotion",
    )
...
def check_r3(run_dir: Path, manifest: dict) -> dict:
    if manifest.get("promotion_flag", False):
        return _rule(
            "R3", "no-premature-tablegen", True, "info", "promotion_flag is set; TableGen/C++ generation is permitted"
        )
    target = manifest["target"]
    ...
    if violations:
        ...
        return _rule(
            "R3", "no-premature-tablegen", False, "error", f"TableGen/C++ files found before promotion: {files}"
        )
    return _rule(
        "R3", "no-premature-tablegen", True, "info", "No TableGen/C++ files found (correct for xDSL-first workflow)"
    )
...
def check_r4(run_dir: Path, manifest: dict, repo_root: Path) -> dict:
    ...
    try:
        ...
        return _rule("R4", "merlin-core-immutable", True, "info", "No Merlin core files modified since run init")
    except subprocess.CalledProcessError as e:
        return _rule("R4", "merlin-core-immutable", False, "warning", f"Could not run git diff: {e.stderr.strip()}")
...
def _check_dialect_plan_rules(run_dir: Path, manifest: dict) -> list[dict]:
    ...
    try:
        ...
        return [
            _rule(f"R{i}", name, False, "error", msg)
            for i, name in enumerate(
                [
                    "op-evidence",
                    "op-verifier-coverage",
                    "op-lowering-exit",
                    "no-scheduling-in-semantics",
                    "no-runtime-in-types",
                    "unsupported-fails-early",
                ],
                start=5,
            )
        ]
    ...
    results.append(
        _rule(
            ...
            f"Ops missing evidence: {missing_evidence}" if missing_evidence else "All ops have evidence",
        )
    )
    ...
    results.append(
        _rule(
            ...
            f"Ops missing verifier: {missing_verifier}" if missing_verifier else "All ops have verifier coverage",
        )
    )
    ...
    results.append(
        _rule(
            "R7",
            "op-lowering-exit",
            not missing_lowering,
            "error" if missing_lowering else "info",
            f"Ops missing lowering exit: {missing_lowering}" if missing_lowering else "All ops have a lowering exit",
        )
    )
    ...
    missing_unsupported = [
        op.get("name", "?") for op in ops if op.get("has_unsupported_cases") and not op.get("unsupported_handling")
    ]
    results.append(
        _rule(
            "R10",
