...
from merlin.agentreport.corpus_coverage import owner_map, read_required_tiers, read_roster_paths
...
def _declared_rosters():
    """Current declared recipes and explicit runtime aliases; never reconstruct old run inputs."""
    from merlin_experiments.phase0.declarations import all_declarations
    from merlin_experiments.spec import SpecError, read_yaml

    declarations, aliases = {}, {}
    for declaration in all_declarations():
        prior = declarations.get(declaration.profile)
        if prior is not None and prior.profile_inputs() != declaration.profile_inputs():
            raise SpecError(f"conflicting corpus declarations for profile {declaration.profile!r}")
        declarations[declaration.profile] = declaration
        descriptor = read_yaml(declaration.descriptor)
        names = {declaration.target, declaration.profile, str(descriptor.get("target") or "")}
        for name in names - {""}:
            if name in aliases and aliases[name] != declaration.profile:
                raise SpecError(f"ambiguous runtime corpus alias {name!r}")
            aliases[name] = declaration.profile
...
def cmd_coverage(a) -> int:
    ...
    declarations, profile_of = _declared_rosters()
    ...
    all_profiles = sorted(set(declarations) | {profile_of.get(r["target"], r["target"]) for r in refs})

    rosters, reasons = {}, {}
    for name in all_profiles:
        declaration = declarations.get(name)
        if declaration is None:
            rosters[name], reasons[name] = {}, f"no current corpus declaration for {name!r}"
            continue
        # Preserve the report's existing lane scope/order: SMT and performance sweeps were not
        # part of this roster reader. Relocation must not silently change its denominator.
        paths = [
            (declaration.recipe, "public"),
            (declaration.synth_profile, "synth"),
            (declaration.hidden_profile, "hidden"),
        ]
        rosters[name], reasons[name] = read_roster_paths((path, lane) for path, lane in paths if path is not None)
    ...
    for name in all_profiles:
        ...
        row = cov.to_dict()
        declaration = declarations.get(name)
        row["roster_scope"] = "current-declared-corpus" if declaration is not None else "unavailable"
        row["roster_definition"] = str(declaration.definition) if declaration is not None else None
        row["roster_recipe"] = str(declaration.recipe) if declaration is not None else None
