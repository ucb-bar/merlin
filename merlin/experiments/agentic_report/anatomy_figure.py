...
def _cost(a):
    ...
    if notional is not None or kind == "subscription_notional":
        return (f"~${notional:,.0f} notional" if notional is not None else "cost not reported"), "~$"
    return "", "$"
...
def render(a: dict, out: Path, *, until_best: bool = True) -> Path:
    ...
    best_at = min(v["t_s"] for v in verdicts_all if v["n_passed"] == best) / 60.0 if verdicts_all else None
    ...
    keep = xs <= span
    xs = xs[keep]
    stack = [_smooth(np.minimum(sum(acc[k] for k in ks) / width, 1.0))[keep] for _, ks, _ in BANDS]
    ...
    if has_rate:
        ...
        rate.set_yscale("log")
        rate.set_ylabel(f"token rate (tok/min, log)   ·   {len(token_curve)} usage samples", labelpad=3)
        ...
        notes.append(
            "no token rate: this driver reports usage once per run, not per message"
            + (f" ({n_rep} sample{'s' if n_rep != 1 else ''} in this record)" if n_rep else "")
        )
    ...
    if sim:
        ...
        gx = grid / 60.0
        ax.fill_between([0, span], RAIL_LO, RAIL_HI, transform=marg, clip_on=False, color="#F2EEE8", lw=0, zorder=2)
        ...
        ax.plot([0, span], [RAIL_LO] * 2, transform=marg, clip_on=False, color=INK, lw=0.5, alpha=0.35, zorder=4)
        n_by = Counter(e["tier"] for e in sim)
        grades = sorted({(min(e["start_s"] for e in sim if e["grade"] == g), g) for g in {e["grade"] for e in sim}})
    ...
    for v in improved:
        ...
        ax.annotate(
            ...
            zorder=8,
            annotation_clip=False,
            bbox=dict(boxstyle="round,pad=0.26", fc="#FDF7EF", ec=GOLD, lw=0.9, alpha=0.96),
        )
    ...
    fig.text(
        0.052,
        0.958,
        f"{a['target']} · {a['arm']} — an agent writing a compiler for unseen hardware",
        ha="left",
        va="center",
        fontsize=14.0,
        color=INK,
    ...
    bits = [
        ...
        f"{len(calls_all)} tool calls",
        f"{counts['selfcheck']} self-checks",
    ...
    if best_at is not None and wall_min - best_at > 1:
        facts.append(
            f"then {(wall_min - best_at) / 60:.1f} h more"
            + (f", on {len(blocked)} tests that could not move" if blocked else " without moving")
        )
    # The run's headline bottleneck, when there is one: a self-check that is mostly a rebuild.
    costs = a.get("grade_costs") or []
    heavy = max(
        (g for g in costs if g["sim_active_s"] > 0 and g["build_s"] > g["sim_active_s"]),
        key=lambda g: g["build_s"] / g["sim_active_s"],
        default=None,
    )
    if heavy:
        notes.insert(
            0,
            f"a self-check ≈ {heavy['adapter_wall_s'] / 60:.0f} min: "
    ...
    if facts:
        fig.text(0.052, 0.900, "   ·   ".join(facts), ha="left", va="center", fontsize=8.8, color=INK, alpha=0.9)
    if notes:
        fig.text(
            0.938,
            0.900,
    ...
    handles = [
        Patch(facecolor=c, alpha=0.55, edgecolor="white", label=f"{lab} · {sum(whole[k] for k in ks)}×")
        for lab, ks, c in BANDS
    ]
    handles.append(Patch(facecolor=THINK_COLOUR, alpha=0.55, edgecolor="white", label=THINK_LABEL))
    if has_rate:
        handles += [
            Line2D([0], [0], color=SAGE, lw=2, marker="o", ms=3.4, label="rate input"),
    ...
    if sim:
        handles += [
            Patch(
                facecolor=TIER_COLOUR[t],
                alpha=0.9,
                label=f"sim {t} · {'/'.join(engines[t])} · {n_by[t]}× · peak {int(peaks[t])}",
            )
            for t in tiers
        ]
    fig.legend(
...
def main(argv=None) -> int:
    ...
    ap.add_argument(
        "--full-span", action="store_true", help="draw the whole run rather than stopping where the score did"
    )
