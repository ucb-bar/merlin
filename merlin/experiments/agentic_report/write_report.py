...
ARM_LABEL = {
    "arm1": "arm 1 · raw C++",
    "arm2": "arm 2 · C++ & Merlin infra",
    "arm3": "arm 3 · Merlin Python tooling",
    "arm4": "arm 4 · Merlin & CIRCT tooling",
    "eqsat": "e-graph seam",
...
def write(facts: list[dict], out: Path) -> Path:
    ...
    A(
        "Generated from `run_facts.json`. Every number here is read from that file at render time; "
        "none is typed in, so no sentence can drift from the run that produced it.\n"
    )

    A("## What is on disk\n")
    A(
        f"- **{len(facts)} run directories** indexed across the configured roots "
        f"({len(p1)} functional, {len(facts) - len(p1)} performance)."
    )
    A(f"- **{len(graded)}** produced a graded verdict on a sized corpus.")
    A(
        f"- **{len(sel)}** are selected for this report — the best run in each target × arm cell, plus "
    ...
    A(
        f"Across the graded runs: **{tot_h:,.0f} active hours**, **{tot_tok / 1e9:.2f} B tokens**, "
    ...
    split = [f for f in graded if f.get("availability", {}).get("token_split", {}).get("kind") == "measured"]
    ...
    A(
        f"Token mix over all graded runs: **{100 * ci / tot:.0f}% cached input**, "
    ...
    if split:
        ...
        A(
            f"Cache reads and cache writes are billed roughly an order of magnitude apart, and only "
    ...
    for key in sorted(ladders, key=lambda k: -max(x["capsules"] or 0 for x in ladders[k])):
        ...
        A(
            f"**{target} · `{tag}`**{tag_label} — {corpora[0] if len(corpora) == 1 else corpora} "
        ...
        for f in members:
            ...
            A(
                f"| {ARM_LABEL[f['arm']]} | {f['passed']}/{f['capsules']} | "
    ...
    for phase in sorted(by_phase):
        ...
        A(
            f"- **{phase}**: {len(over)} of {len(rows)} run(s) show two or more tool calls in flight at "
            f"once"
            + (
                f"; median overlap among those is "
                f"{statistics.median([f['overlap_share'] for f in over]):.0%} of the wall, peak "
                f"{max(f['max_concurrent'] for f in over)} simultaneous calls."
                if over
                else "."
            )
        )
    A("")
    A(
        "Overlap is only reported where it survives a flush check: the readings are recomputed with "
    ...
    A("")
    A(
        "Passing capsules only. A failing capsule aborts in hundredths of a second, so a median over "
    ...
    if p2:
        ...
        for f in p2:
            for a, d in (f.get("broker_totals") or {}).items():
                ...
                callsn[a] += d["calls"]
        A(
            f"Phase 1 gives the agent a shell and a simulator it drives itself. Phase 2 gives it a "
    ...
    if conflicts:
        A(
            f"{len(conflicts)} run(s) carry a bundle and a run-id that disagree about which arm they "
