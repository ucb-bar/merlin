...
def _build(features, bundle):
    ...
    build = apply_rvv_package(_pkg(features), bundle, work, board="spike_riscv64", harts=1, arena_mb=64)
...
def _gens():
    ...
    return {"gelu": workloads.gen_gelu_f32, "sigmoid": workloads.gen_sigmoid_f32, "silu": workloads.gen_silu_f32}
...
def test_softmax_exp_stays_libm_gelu_vectorizes():
    ...
    ll = P.lower_to_llvm_ir(_SOFTMAX_PLUS_GELU_MLIR, vectorize=True, features=frozenset([FEATURE]), timeout=900)
...
def test_realistic_activation_module_lowers_without_pipeline_error():
    ...
    ll = P.lower_to_llvm_ir(_REALISTIC_ACTIVATION_MLIR, vectorize=True, features=frozenset([FEATURE]), timeout=900)
...
def _n_vector_fp(stream):
    ...
    return sum(stream.count(mn) for mn in ("vfmul.vv", "vfadd.vv", "vfsub.vv", "vfmacc.vv", "vfmacc.vf", "vfmadd.vv"))
...
def test_activation_polynomials_accurate_on_realistic_ranges():
    ...
    def poly_exp(x):
        ...
        x = np.maximum(x, np.float32(-87.0))
        x = np.minimum(x, np.float32(88.0))
    ...
    def poly_erf(x):
        x = x.astype(np.float32)
        ax = np.abs(x)
        s = np.where(x >= 0, np.float32(1.0), np.float32(-1.0))
