...
def _registrar_modules() -> set[str]:
    ...
    for path in sorted(llvm.glob("*.py")):
        ...
        if "def ensure_registered" not in path.read_text(encoding="utf-8"):
            continue
        fn = getattr(importlib.import_module(f"merlin.llvmlower.{path.stem}"), "ensure_registered", None)
        if fn is not None and any(
            p.default is inspect.Parameter.empty and p.kind in (p.POSITIONAL_ONLY, p.POSITIONAL_OR_KEYWORD)
