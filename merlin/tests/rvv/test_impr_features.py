...
def test_two_full_schedule_replacements_refuse_to_compose():
    ...
    with pytest.raises(F.CompositionError):
        F.apply_schedule(P.RVV_TRANSFORM_SCHEDULE, frozenset(["fused_vfmacc_tiled", "accumulator_resident_ntail"]))
    with pytest.raises(F.CompositionError):
        F.apply_schedule(
            P.RVV_TRANSFORM_SCHEDULE, frozenset(["accumulator_resident_mtail", "accumulator_resident_ntail"])
        )
...
def test_additive_edit_layers_on_full_replacement():
    ...
    out = F.apply_schedule(P.RVV_TRANSFORM_SCHEDULE, frozenset(["fused_vfmacc_tiled", "lmul_widen_n"]))
...
def test_single_full_replacement_still_allowed():
    ...
    for nm in ("fused_vfmacc_tiled", "accumulator_resident_mtail", "accumulator_resident_wholemodel"):
        on = F.apply_schedule(P.RVV_TRANSFORM_SCHEDULE, frozenset([nm]))
        assert on != P.RVV_TRANSFORM_SCHEDULE
...
def test_memory_traffic_facet_quantifies_loads_per_fma():
    ...
    def vi(addr, mn, *ops):
        ...
        return VInsn(
            raw=RawInsn(addr=addr, mnemonic=mn, operands=list(ops)), is_vector=is_vec, vtype=vt if is_vec else None
        )
...
def test_only_the_mr_gt_1_register_blocks_imply_the_tile_epilogue_hygiene():
    ...
    for mr_gt_1 in ("accumulator_resident_wholemodel_vf_mr4", "accumulator_resident_wholemodel_vf_mrpad"):
...
def test_implied_hygiene_reaches_the_runner_argv_gate():
    ...
    from merlin.llvmlower.selfcopy import FEATURE as SELF_COPY
    from merlin.llvmlower.selfcopy import needs_canonicalize
...
def test_the_implies_closure_terminates_and_validates_what_it_adds():
    ...
    try:
        F.register(
            F.ImprFeature(name="_t_cycle_a", action_class="PASS", description="t", implies=frozenset({"_t_cycle_b"}))
        )
        F.register(
            F.ImprFeature(name="_t_cycle_b", action_class="PASS", description="t", implies=frozenset({"_t_cycle_a"}))
        )
        F.register(
            F.ImprFeature(name="_t_chain_1", action_class="PASS", description="t", implies=frozenset({"_t_chain_2"}))
        )
        F.register(
            F.ImprFeature(
                name="_t_chain_2", action_class="PASS", description="t", implies=frozenset({"erase_self_copy"})
            )
        )
        F.register(
            F.ImprFeature(name="_t_bad", action_class="PASS", description="t", implies=frozenset({"_t_does_not_exist"}))
        )

        assert F.normalize(["_t_cycle_a"]) == frozenset({"_t_cycle_a", "_t_cycle_b"})
        assert F.normalize(["_t_chain_1"]) == frozenset({"_t_chain_1", "_t_chain_2", "erase_self_copy"})
...
def test_every_mr_gt_1_recipe_implies_the_tile_epilogue_hygiene():
    ...
    mr_gt_1 = {
        ...
        "ensure_perop_block": lambda: F.ensure_perop_block({"linalg.matmul:128x128:128": (4, 16)}, 16),
    }
    mr_eq_1 = {
        ...
        "ensure_perop_block": lambda: F.ensure_perop_block({"linalg.matmul:17x128:128": (1, 16)}, 16),
    }
...
def test_the_statically_registered_mr_gt_1_features_imply_it_too():
    ...
    for name in (
        "accumulator_resident_wholemodel_vf_mr4",
...
def test_a_sized_promotion_variant_names_its_cap_in_the_emitted_pass():
    ...
    at_default = F.apply_pipeline(base, frozenset({F.ensure_promote_stack(F._PROMOTE_STACK_DEFAULT_BYTES)}))
    plain = F.apply_pipeline(base, frozenset({F.PROMOTE_STACK_NAME}))
    assert [p for p in at_default if "promote-buffers" in p] == [p for p in plain if "promote-buffers" in p]
...
def test_the_int8_register_block_tile_is_searchable_not_inherited_from_f32():
    ...
    pads = {get(n).edit_schedule("").split("pad_to_multiple_of")[1].split("]")[0] for n in MRPAD_INT8_TILES}
