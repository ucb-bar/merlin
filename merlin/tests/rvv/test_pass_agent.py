...
def test_our_measured_value_comes_off_the_divergence_not_the_action():
    ...
    d = Divergence(
        axis="compute.activation_vectorization", expert="vectorized_polynomial", ours="scalar_libm_call", backend="rvv"
    )
...
def test_a_successful_turn_yields_a_proposal_and_keeps_the_prose_as_rationale(tmp_path):
    att = pa.propose_pass(
        _Action(),
        module="merlin.llvmlower.act_poly",
...
def test_an_unsandboxed_proposer_is_REFUSED_by_default(tmp_path, monkeypatch):
    ...
    # The refusal NAMES the condition: absent, inoperable, or undetermined are three different
    # operator actions, and the message that collapsed them is what let a dead sandbox read as a
    # missing one for as long as it did.
    assert "sandbox is not usable" in att.error
    assert called == [], "the turn must not run at all without the sandbox"


@pytest.mark.skipif(_NO_SANDBOX is not None, reason=f"sandbox unusable: {_NO_SANDBOX}")
...
def test_the_sandbox_really_cannot_read_the_repo_or_scratch(tmp_path):
    ...
    probe = "import os;print('repo', os.path.exists(%r));print('scratch_proj', os.path.exists(%r))" % (
...
def test_proposer_for_resolves_the_module_from_the_seam_and_collects_attempts(tmp_path):
    propose, attempts = pa.proposer_for(
        _Action(),
        current_source="x=1\n",
        workspace=tmp_path / "ws",
        require_sandbox=False,
...
def test_the_workspace_file_is_the_primary_proposal_channel(tmp_path):
    ...
    att = pa.propose_pass(
        _Action(), module="m", current_source="old\n", workspace=ws, require_sandbox=False, runner=_runner
    )
