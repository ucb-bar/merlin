...
def _run(tmp_path, ours_objd, **kw):
    ...
    return run_beam(
        ...
        certify_fn=_certify_with_objdump(ours_objd),
        **kw,
    )
...
def test_an_unforkable_escalation_reaches_the_slot(tmp_path, ours_objd):
    ...
    def slot(action, *, parent_run_id=None):
        seen.append((getattr(action, "divergence_axis", None), getattr(action, "action_class", None), parent_run_id))
...
def test_the_slot_gets_the_FORK_that_produced_the_residual_not_the_seed(tmp_path, ours_objd):
    ...
    seen = []
    _run(tmp_path, ours_objd, pass_slot_fn=lambda a, *, parent_run_id=None: seen.append(parent_run_id) or {})
...
def test_make_pass_slot_fn_reports_an_unactionable_seam_instead_of_dropping_it(tmp_path):
    ...
    fn = make_pass_slot_fn(
        frozen_pkg=tmp_path / "seed",
        model_dir=tmp_path / "wl",
        runs_root=tmp_path / "runs",
...
def test_make_pass_slot_fn_runs_the_turns_and_records_each_one(tmp_path):
    ...
    def certify(env, *, package_dir, model_dir, runs_root, run_id, targets, timeout):
        ...
        (gen / "objdump.txt").write_text(f"0000000000000000 <forward>:\n   0:\t02b7f0d7          \t{mnem}\n")
        return {
            "status": "ok",
            "correctness": {"gate_ok": True, "fp32_cos": 1.0},
            "measurement": [],
            "_imported": ["merlin.llvmlower.act_poly"],
        }
    ...
    fn = make_pass_slot_fn(
        ...
        max_turns=2,
        certify=certify,
        propose_fn=lambda a, *, feedback=None: PassProposal(module="merlin.llvmlower.act_poly", source="X = 1\n"),
    )
