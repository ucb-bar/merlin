...
from merlin.llvmlower.lmul_group import (
    LMUL_LADDER,
    LMUL_OPTION,
    LmulDerivationError,
    extent_ceiling,
    group_elements,
    group_lmul,
    group_lmul_for_elem_types,
    group_lmul_for_shapes,
    lmul_cflags,
)
...
def test_vlen_enters_as_the_ceiling_not_the_floor():
    ...
    assert (
        group_lmul(operand_bits=8, acc_bits=32, vlen=128, max_group_elems=4096)
        == group_lmul(operand_bits=8, acc_bits=32, vlen=512, max_group_elems=4096)
        == 4
    )
...
def test_the_sentinel_resolves_from_the_prepared_irs_own_element_types():
    ...
    i8 = ContractionShape(op="linalg.matmul", parallel=(128, 256), reduction=(256,), dtypes=("i8", "i8", "i32"))
    f32 = ContractionShape(op="linalg.matmul", parallel=(128, 256), reduction=(256,), dtypes=("f32", "f32", "f32"))
    ...
    narrow = ContractionShape(op="linalg.matmul", parallel=(1, 8), reduction=(64,), dtypes=("i8", "i8", "i32"))
...
def test_the_emitted_vsetvli_lmul_field_actually_moves(tmp_path):
    ...
    def build(features) -> Path:
        obj = (tmp_path / (("f_" + "_".join(sorted(features))) if features else "baseline")).with_suffix(".o")
        flags = F.apply_cflags(list(RISCV_FLAGS), F.normalize(features))
        proc = subprocess.run(
            [str(clang), *flags, "-c", str(src), "-o", str(obj)], capture_output=True, text=True, timeout=300
        )
    ...
    assert wide_lmul > base_lmul, f"register group did not widen: baseline m{base_lmul:g} -> feature m{wide_lmul:g}"
