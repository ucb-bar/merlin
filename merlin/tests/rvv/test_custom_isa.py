...
def test_pass_lowers_marker_to_llvm_inline_asm():
    ...
    assert all(getattr(op, "op_name", None) is None or op.op_name.data != "merlin.inline_asm" for op in m.walk())
...
def test_custom_instruction_emitted_without_llvm_fork(tmp_path):
    ...
    obj = build_rvv_object("merlin_vcix", ".insn r 0x0b, 0, 0, $0, $1, $2", "=r,r,r", ["i32", "i32"], "i32", tmp_path)
    dis = disassemble(obj)
...
def test_inline_asm_emits_named_instruction(tmp_path):
    ...
    obj = build_rvv_object("merlin_xor", "xor $0, $1, $2", "=r,r,r", ["i32", "i32"], "i32", tmp_path)
    dis = disassemble(obj)
