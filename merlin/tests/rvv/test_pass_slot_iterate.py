...
def _checks(verdicts):
    ...
    return {"frozen_baseline_ok": frozen, "bit_exact_ok": bit_exact, "lift_cca": lambda _p: None}
...
def test_the_feedback_is_the_measurement_and_never_a_suggested_fix():
    v = PassVerdict(
        False,
        "inert",
        "byte-identical to the unpatched build (digest 0358237f); the pass was imported but its matching never fired",
...
def test_outstanding_axes_are_reported_when_the_facet_check_refused():
    v = PassVerdict(False, "facet", "promised X but did not achieve Y", residual=("compute.activation_vectorization",))
...
def test_a_refusal_is_fed_into_the_next_turn():
    ...
    iterate_pass_slot(_Action(), propose=propose, max_turns=2, **_checks(["numerics moved", "numerics moved again"]))
...
def test_the_loop_stops_the_moment_a_proposal_is_accepted():
    ...
    achieved = cca.CCA(
        op="matmul",
        backend=["rvv"],
        compute=cca.ComputeFacet(op="matmul", activation_vectorization="vectorized_polynomial"),
    )
    calls = {"n": 0}
...
def test_two_identical_verdicts_stop_the_loop():
    ...
    out = iterate_pass_slot(_Action(), propose=propose, max_turns=5, **_checks(["same failure"] * 5))
...
def test_a_verdict_that_moved_to_a_LATER_stage_is_progress_and_does_not_stop_the_loop():
    ...
    iterate_pass_slot(
        _Action(), propose=propose, max_turns=3, **_checks(["numerics moved", "still inert", "facet unmet"])
    )
...
def test_every_turn_is_returned_including_the_refusals():
    ...
    out = iterate_pass_slot(_Action(), propose=propose, max_turns=3, **_checks(["a", "b", "c"]))
    assert len(out) == 3
...
def test_no_proposal_ends_the_loop_honestly():
    out = iterate_pass_slot(_Action(), propose=lambda a, *, feedback=None: None, max_turns=3, **_checks([None]))
...
def test_a_proposer_that_predates_the_feedback_argument_still_works():
    ...
    out = iterate_pass_slot(_Action(), propose=old_style, max_turns=2, **_checks(["a", "b"]))
...
def test_the_turn_hook_sees_each_turn_as_it_lands():
    ...
    iterate_pass_slot(
        ...
        max_turns=2,
        on_turn=lambda n, p, v: seen.append((n, v.stage)),
        **_checks(["a", "b"]),
    )
