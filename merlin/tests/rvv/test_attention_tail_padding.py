...
def test_113_by_113_attention_lowers_without_a_surviving_vector_mask():
    ...
    Inspecting the saved dialect edge makes the condition explicit as well.
    """
...
def test_15_by_113_by_113_attention_survives_the_eight_hart_pipeline():
    """Exercise the exact text-attention batch and the out-of-process OMP edge.

    Batch 15 is large enough for the eight-hart schedule to split.  This catches
    both allocation-scope damage around the padded tensor and residual vector
    operations before ``mlir-translate``; a serial B=1 test cannot cover either.
    """
    from merlin.llvmlower.lower import lower_model_file
    from merlin.mining import workloads
    from merlin.runtime.backends import zephyr_model as zm
...
def test_divisible_12_by_1024_attention_is_not_padded_and_still_lowers():
    ...
    Vision attention is already divisible by MR=4/NR=8.  It must take the
    ``merlin.bmm_full_tile`` arm, never the allocating pad arm, while retaining
    its eight-hart lowering.  K=64 matches the vision head width.
    """
    from merlin.llvmlower.lower import lower_model_file
    from merlin.mining import workloads
    from merlin.runtime.backends import zephyr_model as zm
    ...
    assert "@llvm.fmuladd.v8f32" in result.ll_path.read_text(encoding="utf-8")
    # A padded destination allocates 12*1024*1024 f32 bytes.  The full-tile arm
    # must not materialize that buffer at all.
