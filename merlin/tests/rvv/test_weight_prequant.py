...
def _real_bundle(name: str) -> Path:
    ...
    return next((path for path in candidates if (path / "model.mlir").is_file()), candidates[0])


REAL_BUNDLE = _real_bundle("lstmnetvit_int8_command_grouped_direct_consistent_pretransposed")
...
def _write_safetensors(path: Path, tensors: dict[str, np.ndarray]) -> None:
    ...
    for name, value in tensors.items():
        ...
        data = value.tobytes()
        header[name] = {"dtype": spell[value.dtype], "shape": list(value.shape), "data_offsets": [off, off + len(data)]}
        payload.append(data)
        off += len(data)
...
def _bundle(root: Path, *, weight_name: str = "net.lstm.weight_ih_l0") -> tuple[Path, np.ndarray]:
    ...
    (root / "weights.safetensors.manifest.json").write_text(
        json.dumps(
            {
                "0": {"kind": "param", "weight": weight_name, "dtype": "float32", "shape": [2, 3]},
                "1": {"kind": "param", "weight": "other", "dtype": "float32", "shape": [2]},
            }
        )
    )
    _write_safetensors(root / "weights.safetensors", {weight_name: weight, "other": other})
...
""".replace("SRC", str(src))
    ...
    (src / "weights.safetensors.manifest.json").write_text(
        json.dumps(
            {
                "0": {"kind": "param", "weight": weight_name, "dtype": "float32", "shape": [2, 3, 2, 2]},
...
@pytest.mark.skipif(not (REAL_RESNET_BUNDLE / "model.mlir").is_file(), reason="ResNet-50 W8A8 bundle absent")
...
def test_integer_preparation_reaches_the_existing_static_weight_fast_path(tmp_path):
    ...
    prepared, _ = prepare_for_lowering(
        dst / "model.mlir", work, int8_compute=True, features=frozenset(), blocking=False
    )
