...
from merlin.llvmlower import int8_contractions as PO
from merlin.llvmlower.declaration_access import patch_declaration_arg_attrs, unpatched_declarations

SYMBOL_PREFIX = "synthetic_int8_contract"
SIDECAR_NAME = "synthetic_signatures.json"


@pytest.mark.parametrize(
    "operation,missing",
    [
        (lambda: PO.rewrite_contractions(None), "symbol_prefix"),
        (lambda: PO.rewrite_prepared_file(None, None, select=None, sidecar_name="explicit.json"), "symbol_prefix"),
        (lambda: PO.rewrite_prepared_file(None, None, select=None, symbol_prefix="explicit"), "sidecar_name"),
        (lambda: PO.load_sidecar(None), "sidecar_name"),
    ],
)
def test_caller_must_name_symbol_and_sidecar_before_any_io(operation, missing):
    # None would fail inside Path or the IR routines; signature binding must refuse first.
...
class TestItIsInertWithoutADecision:
    def test_no_selector_routes_nothing(self):
        ...
        got = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX)
    ...
    def test_a_selector_that_declines_everything_routes_nothing(self):
        mod = _module(_INT8_MM)
        got = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=lambda _s: False)
        assert got.count == 0
...
class TestWhatItRefuses:
    def test_a_float_contraction_is_not_a_candidate(self):
        ...
        assert PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL).count == 0
    ...
    def test_a_batch_contraction_is_routed_as_a_loop_over_slices(self):
        ...
        rewrite = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=lambda _sh: True)
    ...
    def test_a_zero_fill_init_is_routed(self):
        ...
        assert PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL).count == 1
...
class TestWhatItEmits:
    def test_a_caller_can_supply_a_distinct_symbol_namespace(self):
        mod = _module(_INT8_MM)
        got = PO.rewrite_contractions(mod, select=_ALL, symbol_prefix="merlin_outlined_gemm_i8")
    ...
    def test_the_contraction_becomes_a_call_to_the_kernel(self):
        mod = _module(_INT8_MM)
        got = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL)
        assert got.count == 1
        from merlin.xdsl_dialects._common import text as to_text

        out = to_text(mod)
        assert f"{SYMBOL_PREFIX}_0" in out
    ...
    def test_the_printer_drops_declaration_arg_attrs(self):
        ...
        assert unpatched_declarations(raw, got.signatures) == tuple(got.signatures)
    ...
    def test_the_patch_restores_them(self):
        ...
        fixed = patch_declaration_arg_attrs(to_text(mod), got.signatures, argument_access=("read", "read", "write"))
        assert fixed.count('bufferization.access = "read"') == 2
        assert fixed.count('bufferization.access = "write"') == 1
        assert unpatched_declarations(fixed, got.signatures) == ()
    ...
    def test_the_patch_is_idempotent(self):
        ...
        once = patch_declaration_arg_attrs(to_text(mod), got.signatures, argument_access=("read", "read", "write"))
        twice = patch_declaration_arg_attrs(once, got.signatures, argument_access=("read", "read", "write"))
    ...
    def test_the_declaration_is_private(self):
        mod = _module(_INT8_MM)
        PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL)
        from merlin.xdsl_dialects._common import text as to_text
    ...
    def test_the_report_serialises(self):
        ...
        mod = _module(_INT8_MM)
        json.dumps(PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL).to_dict())
...
class TestOnTheRealPreparedModel:
    ...
    def test_a_tile_filling_selector_splits_the_work_dominant_shapes_out(self, prepared):
        ...
        got = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=lambda s: min(s.parallel) >= 32)
    ...
    def test_the_real_tile_filling_selector_takes_the_batched_shapes_too(self, prepared):
        ...
        mod = parse_mlir_file(_PREPARED)
        got = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=PO.tile_filling_selector(64))
        assert got.count == 57
...
class TestTheFileSeam:
    ...
    def test_it_routes_and_records(self, tmp_path):
        ...
        got = PO.rewrite_prepared_file(
            prepared, tmp_path, symbol_prefix=SYMBOL_PREFIX, sidecar_name=SIDECAR_NAME, select=_ALL
        )
        assert got.count == 1
        text = prepared.read_text()
        assert f"func.call @{SYMBOL_PREFIX}_0" in text
        assert 'bufferization.access = "read"' in text
        assert PO.load_sidecar(tmp_path, sidecar_name=SIDECAR_NAME) == {f"{SYMBOL_PREFIX}_0": (64, 16, 32)}
    ...
    def test_custom_sidecar_does_not_clobber_the_matrix_unit_record(self, tmp_path):
        ...
        assert PO.load_sidecar(tmp_path, sidecar_name="outlined.json") == {"merlin_outlined_gemm_i8_0": (64, 16, 32)}
    ...
    def test_nothing_selected_leaves_the_module_byte_identical(self, tmp_path):
        ...
        assert (
            PO.rewrite_prepared_file(
                prepared, tmp_path, symbol_prefix=SYMBOL_PREFIX, sidecar_name=SIDECAR_NAME, select=lambda _s: False
            ).count
            == 0
        )
        assert prepared.read_bytes() == before
        assert PO.load_sidecar(tmp_path, sidecar_name=SIDECAR_NAME) == {}
    ...
    def test_a_malformed_sidecar_is_an_error_not_an_empty_set(self, tmp_path):
        ...
        (tmp_path / SIDECAR_NAME).write_text('{"signatures": {"s": [1, 2]}}', encoding="utf-8")
        with pytest.raises(ValueError, match="triple"):
            PO.load_sidecar(tmp_path, sidecar_name=SIDECAR_NAME)
...
class TestTheRoutingRuleIsRecorded:
    ...
    def test_the_shape_that_shipped_is_flagged_as_sub_tile(self):
        ...
        rw = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL, tile_edge=64)
        assert rw.count == 1
    ...
    def test_the_default_selector_would_have_refused_that_shape(self):
        ...
        mod = _module(_int8_mm(8, 2048, 2304))
        rw = PO.rewrite_contractions(
            mod, symbol_prefix=SYMBOL_PREFIX, select=PO.tile_filling_selector(64), tile_edge=64
        )
        assert rw.count == 0
    ...
    def test_without_an_edge_the_rule_is_UNKNOWN_not_assumed(self):
        ...
        mod = _module(_int8_mm(8, 2048, 2304))
        rw = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=_ALL)
    ...
    def test_the_edge_survives_a_decline(self):
        ...
        mod = _module(_int8_mm(8, 2048, 2304))
        rw = PO.rewrite_contractions(mod, symbol_prefix=SYMBOL_PREFIX, select=lambda _s: False, tile_edge=64)
        assert rw.count == 0
