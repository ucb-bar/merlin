...
def test_generic_file_rewrite_does_not_import_legacy_matrix_pass(tmp_path, monkeypatch):
    import builtins
    import subprocess
    import sys
    ...
    from merlin.llvmlower.device_offload import rewrite_prepared_file
    from merlin.system import derive, offload
    from merlin.targetgen import target_experiment

    # Synthetic device facts only; the real parse, selection, rewrite, printer and sidecar run.
    monkeypatch.setattr(offload, "device_dtype_triples", lambda device: frozenset({("i8", "i8", "i32")}))
    monkeypatch.setattr(offload, "device_contraction_ranks", lambda device: frozenset({2}))
    monkeypatch.setattr(
        target_experiment, "load_capability_manifest", lambda device: SimpleNamespace(endpoint_kind=None)
    )
    monkeypatch.setattr(derive, "link_for", lambda *a: SimpleNamespace(command_transport="host_instruction"))
    monkeypatch.setattr(subprocess, "Popen", lambda *a, **k: pytest.fail("generic rewrite launched a process"))
    original_import = builtins.__import__

    def without_legacy(name, globals=None, locals=None, fromlist=(), level=0):
        if "passes_opu" in name or "passes_opu" in (fromlist or ()):
            pytest.fail("generic device rewrite imported legacy passes_opu")
    ...
    prepared.write_text(_TWO_SHAPES)
    with IR_LOCK:
        result = rewrite_prepared_file(prepared, tmp_path, "synthetic_device", select=lambda shape: True)
    assert result.moved == 2
    assert set(result.signatures.values()) == {(16, 16, 32), (8, 64, 128)}
    text = prepared.read_text()
    assert "linalg.matmul" not in text and text.count("func.call") == 2
    assert text.count('bufferization.access = "read"') == 4
    assert text.count('bufferization.access = "write"') == 2
    assert load_sidecar(tmp_path)["device"] == "synthetic_device"
