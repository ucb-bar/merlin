...
from merlin.xdsl_dialects import interface as i
...
def test_commit_rejects_output_dtype_mismatch():
    ...
    with pytest.raises(VerifyException, match="does not match result element type"):
        _commit({"epilogue": ArrayAttr([]), "output_dtype": StringAttr("i8")}, result_elem="i32").verify()
...
def test_matmul_rejects_incompatible_shapes():
    ...
    a, b = blk.args
    op = i.MatmulOp(operands=[a, b], result_types=[i.AccumulatorType(TensorType(i32, [4, 4]))])
...
def test_use_after_evict_is_flagged():
    from xdsl.dialects.builtin import FunctionType, ModuleOp, StringAttr, TensorType, i8, i32
    ...
    mm = i.MatmulOp(
        operands=[a, pack.res],  # use AFTER evict
