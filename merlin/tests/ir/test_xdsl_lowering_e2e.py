...
def test_intermediates_roundtrip(lowered):
    ...
    extra = get_all_dialects() + [_specs()["toy_npu"].dialect_module.get_dialect(), Linalg, TensorDialect, Arith]
    for mod in lowered.modules():
...
def test_no_reuse_means_no_residency():
    ...
    mod = lower_to_schedule(lower_to_contract(build_input_module(reuse=1), load_curated_contract("toy_npu")))
