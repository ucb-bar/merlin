...
def test_the_console_script_is_registered():
    ...
    owner = tomllib.loads((repo_root() / "packages/merlin-analysis/pyproject.toml").read_text())
    core = tomllib.loads((repo_root() / "pyproject.toml").read_text())
    assert owner["project"]["scripts"]["merlin-bundle-pretranspose"] == "merlin.baselines.pretranspose_cli:main"
    assert "merlin-bundle-pretranspose" not in core["project"]["scripts"]
