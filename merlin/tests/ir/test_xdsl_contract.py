...
def test_check_rejects_mismatched_proof():
    ...
    check = c.CheckOp(operands=[v, [prove.proof]], properties={"requirement": StringAttr("rhs_immutable")})
...
def test_capability_rejects_bad_feature_identifier():
    ...
    op = c.CapabilityOp(
        ...
        properties={"sym_name": StringAttr("t"), "features": ArrayAttr([StringAttr("not a name!")])},
    )
