...
def test_max_pool_reads_the_tile_its_load_padded(golden) -> None:
    ...
    attrs = {"kernel_size": (3, 3), "stride": (2, 2), "padding": (0, 0), "dilation": (1, 1),
             "ceil_mode": False, "input_pad_before": (0, 1, 1, 0), "input_pad_value": float("-inf")}
    got = _run("quantized_ops::max_pool2d", {"input": x}, attrs, want.shape, "bfloat16")
    assert np.array_equal(got, want)
    zero_padded = _run("quantized_ops::max_pool2d", {"input": x},
                       {**attrs, "input_pad_value": 0.0}, want.shape, "bfloat16")
    assert not np.array_equal(zero_padded, want)
    with pytest.raises(UnsupportedConstruct, match="ceil_mode"):
        _run("quantized_ops::max_pool2d", {"input": x}, {**attrs, "ceil_mode": True}, want.shape,
             "bfloat16")
...
def test_adaptive_average_pool_matches_voyager(golden, key: str, size: tuple) -> None:
    ...
    got = _run("quantized_ops::adaptive_avg_pool2d", {"input": x}, {"output_size": size},
               want.shape, "bfloat16")
    assert np.array_equal(got, want)
...
def test_the_classifier_linear_rounds_once_to_bfloat16(golden) -> None:
    ...
    small = _run("aten::linear", {"input": x, "weight": w, "bias": golden["linear_b_small"]}, {},
                 golden["linear_y_small"].shape, "bfloat16")
    ...
    big = _run("aten::linear", {"input": x, "weight": w, "bias": golden["linear_b_big"]}, {},
               golden["linear_y_big_fp32"].shape, "bfloat16")
