...
def _dev():
    ...
    return r.DeviceGetOp(
        result_types=[r.DeviceType()],
        properties={"device": StringAttr("toy_npu0"), "backend": r.BackendAttr(r.Backend.SIMULATOR)},
    )
...
def test_buffer_alloc_rejects_nonpositive_bytes():
    ...
    op = r.BufferAllocOp(operands=[_dev().dev], result_types=[r.BufferType()], properties={"bytes": IntegerAttr(0, 64)})
...
def test_metrics_read_rejects_unknown_metric():
    ...
    op = r.MetricsReadOp(
        operands=[_dev().dev], result_types=[r.MetricsType()], properties={"metrics": ArrayAttr([StringAttr("vibes")])}
    )
...
def test_metrics_read_allows_target_specific_prefix():
    ...
    op = r.MetricsReadOp(
        ...
        properties={"metrics": ArrayAttr([StringAttr("target_specific.rocc_stalls")])},
    )
    op.verify()
...
def test_append_arg_values_must_be_names():
    ...
    cb = r.CommandBufferCreateOp(
        operands=[dev.dev], result_types=[r.CommandBufferType()], properties={"target": StringAttr("toy_npu")}
    )
    op = r.CommandBufferAppendOp(
        operands=[cb.cb],
        properties={"opcode": StringAttr("RES_PACK"), "args": DictionaryAttr({"src": IntegerAttr(3, 64)})},
    )
...
def test_unsupported_queue_kind_flagged_by_analysis():
    from xdsl.dialects.builtin import DictionaryAttr, FunctionType, ModuleOp, StringAttr
    ...
    ap = r.CommandBufferAppendOp(
        ...
        properties={
            ...
            "queue": r.QueueKindAttr(r.QueueKind.HOST),
        },
    )  # simulator has no host queue
