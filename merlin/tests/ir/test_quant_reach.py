...
def test_select_reaches_every_pass(monkeypatch):
    ...
    for fn in _REAL_FNS:
        monkeypatch.setattr(Q, fn, lambda _m, _f=fn, **kw: (seen.append((_f, kw.get("select"))), 0)[1])
...
def test_run_model_forwards_the_restricted_reach(monkeypatch):
    ...
    with pytest.raises(_Reached) as ei:
        DR.run_model(
            str(bundle),
            tmp_path_unused := "/nonexistent-workdir-never-created",
            int8_compute=True,
            quant_passes=["contraction_int8"],
            quant_select=pred,
        )
...
def test_contraction_reach_is_restrictable_by_provenance():
    ...
    n_linear = lower_contraction_int8(_prepared_module(bundle), select=lambda op: aten(op) in _LINEAR_ATEN)
