...
MODEL_CAPSULE = repo_root() / "merlin" / "contract" / "capsules" / "model" / "M2_microvit_gemmini"
...
def test_normalizations_are_not_laundered_into_target_obligations():
    ...
    assert all(not item.discharges() and not item.is_required() for item in PS.normalization_catalog())
...
def _write_log(path, *, installed, invoked):
    lines = [json.dumps({"kind": "install", "passes": {name: "instrumented" for name in installed}})]
    lines += [
        json.dumps({"kind": "invoke", "pass": name, "capsule": capsule, "requirements": requirements})
...
def test_wrong_requirement_class_does_not_certify_a_pass(tmp_path):
    log = tmp_path / "pass.jsonl"
    item = PS.catalog()[0]
    _write_log(log, installed=[item.name], invoked=[(item.name, "UNRELATED_CAPSULE", ["different-capstone"])])
...
def test_uninstrumented_is_not_dead(tmp_path):
    ...
    log.write_text(json.dumps({"kind": "install", "passes": {name: "failed: ImportError: x"}}) + "\n", encoding="utf-8")
    report = PS.exercise_report(logs=[log])
...
def test_ratchet_entries_do_not_leak_across_pass_or_axis():
    gate = _gate()
    first = PS.PassInfo("first", "stage", "", "x.y", input_dialect="linalg", output_dialect="linalg")
    second = PS.PassInfo("second", "stage", "", "x.y", input_dialect="linalg", output_dialect="linalg")
...
def test_prototype_pipeline_is_exercised_only_in_its_own_registry(tmp_path, monkeypatch):
    ...
    assert {name for name, value in report["per_pass"].items() if value["status"] == "exercised"} == {
...
def test_real_model_capstone_exercises_every_production_pass(tmp_path, monkeypatch):
    ...
    assert all(value["capsules"] == [capsule["name"]] for value in report["per_pass"].values())
    assert all(value["required_hits"] == [PS.MODEL_BOUNDARY_CAPSTONE] for value in report["per_pass"].values())
