...
was never the code that ran. The checkout's canonical ``src`` and optional ``packages/*/src``
roots now go on ``sys.path`` FIRST, before anything imports ``merlin``. This pins both core and
extension implementations to the tree beside the tests, in a worktree as much as in a clone.
...
# Pin every distribution before importing the shared namespace; a sibling editable
...
# Historical replay deliberately tests a frozen composite source tree against these tests/data.
# Never prepend the live checkout over that tree: doing so silently erases the historical defect.
_SOURCE_ROOTS = (
    [_REPLAY_ROOT] if _REPLAY_ROOT else [_PACKAGE_ROOT, *sorted((_CHECKOUT_ROOT / "packages").glob("*/src"))]
)
...
def _checkout_containing(start: Path) -> Path | None:
    ...
    for d in [start, *start.parents]:
        if (d / "src" / "merlin" / "__init__.py").is_file() or (
            d / "merlin" / "python" / "merlin" / "__init__.py"
        ).is_file():
            return d
    return None
...
def _assert_library_is_this_checkout() -> None:
    ...
    if _REPLAY_ROOT:
        if not pkg_file or not Path(pkg_file).resolve().is_relative_to(_REPLAY_ROOT):
            raise RuntimeError("historical replay imported merlin outside its declared frozen shadow")
        return
    ...
    raise RuntimeError(
        ...
        f"Fix   : PYTHONPATH={tests_checkout}/src .venv/bin/python -m pytest ...\n"
        "Set MERLIN_ALLOW_FOREIGN_PACKAGE=1 only to test a deliberately installed package."
    )


if _REPLAY_ROOT or not os.environ.get("MERLIN_ALLOW_FOREIGN_PACKAGE"):
