...
def test_spatial_kind_routes_to_the_spatial_extractor(monkeypatch, tmp_path):
    ...
    # Production routes on the kind SET now, not a single primary kind — a hybrid runs one extractor
    # per datapath family — so the stub declares the set and `_resolve_kind` derives from it.
    monkeypatch.setattr(mlc_bridge, "_resolve_kinds", lambda t: ("spatial",))
    called: list[str] = []
...
def test_an_extraction_that_grounds_nothing_records_why(monkeypatch, tmp_path):
    ...
    # per datapath family — so the stub declares the set and `_resolve_kind` derives from it.
    monkeypatch.setattr(mlc_bridge, "_resolve_kinds", lambda t: ("spatial",))
    monkeypatch.setattr(SI, "spatial_facts", lambda target: {"facts": {}, "inputs": {"target": target}})
    out = tmp_path / "facts.json"
...
def test_facts_empty_carries_the_recorded_reason():
    ...
    doc = {
        "inputs": {"target": "t", "hw_sha": "missing"},
        "facts": {},
...
def test_an_empty_cached_artifact_is_not_a_cache_hit(monkeypatch, tmp_path):
    ...
    stale.write_text(json.dumps({"schema_version": "2.0", "inputs": {"hw_sha": "missing"}, "facts": {}}))
    good = tmp_path / "good" / "facts.json"
    good.parent.mkdir(parents=True)
    good.write_text(json.dumps({"facts": {"arrays": [{"name": "mesh"}]}}))

    monkeypatch.setattr(F, "rtl_facts_path", lambda t, explicit=None: stale if t == "variant" else good)
    monkeypatch.setattr(F, "facts_alias", lambda t: "base" if t == "variant" else t, raising=False)
...
def test_an_artifact_written_by_another_family_is_regenerated(monkeypatch, tmp_path):
    ...
    p.write_text(
        json.dumps(
            {
                "generator": {"name": circt_introspect.__name__},
...
def test_a_same_family_artifact_is_still_a_cache_hit(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(mlc_bridge, "_resolve_kinds", lambda t: ("systolic",))
...
def test_facts_alias_is_identity_when_nothing_declares_one(monkeypatch):
    ...
    monkeypatch.setattr(target_registry, "resolve", lambda t: (_ for _ in ()).throw(KeyError(t)))
    F._FACTS_ALIAS_CACHE.clear()
    try:
...
def _bundle(**overrides):
    fields = {
        ...
        "element_widths": {
            "value": {"operand_bits": 8, "accumulator_bits": 32},
            "derived": True,
            "source": "s",
            "evidence": "ev-w",
        },
        "dtypes": {
            "value": [{"name": "int8", "operand": "i8", "accumulator": "i32", "path": "p"}],
            "derived": True,
            "source": "s",
            "evidence": "ev-dt",
        },
        "fma_latency": {"value": {"int8_mac_cycles": 0}, "derived": True, "source": "s", "evidence": "ev-lat"},
...
def test_spatial_facts_records_an_ungrounded_field_as_unknown(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "build_fact_bundle",
        lambda t: _bundle(
            mrf_depth={"value": None, "derived": False, "source": None, "evidence": "no regs_* banks in the manifest"}
        ),
    )
...
def test_spatial_facts_leaves_the_body_empty_when_nothing_is_grounded(monkeypatch):
    monkeypatch.setattr(
        SI, "build_fact_bundle", lambda t: SI._unavailable(t, "the OPU state manifest is not reachable")
    )
    doc = SI.spatial_facts("t")
    assert doc["facts"] == {}
