...
def _fc(cap, trace):
    return RR.run_filecheck(
        _FC,
        CC.compile_trace_checks(_FACTS, cap, target="gemmini"),
        RR.render_trace(trace, _FACTS, target="gemmini"),
        "TRACE",
    )
...
def test_phantom_funct_25_rejected_without_verilator():
    ...
    rendered = RR.render_trace(tr, _FACTS, target="gemmini")
...
def test_real_funct_126_is_legal():
    ...
    assert "ILLEGAL_FUNCT_COUNT 0" in RR.render_trace(tr, _FACTS, target="gemmini")
    ok, diag = _fc(cap, tr)
    assert ok, diag
...
def test_compiled_checks_observe_changed_capsule_and_provider(monkeypatch):
    """A retained capsule name cannot hide changed geometry or selected check semantics."""
    cap = {"name": "same-name", "fixture_geometry": 1}
    provider_revision = ["first"]

    def compile_checks(facts, capsule, target, *, checks=None):
        assert target == "gemmini"
        assert checks is None
        return {"geometry": capsule["fixture_geometry"], "provider": provider_revision[0]}

    monkeypatch.setattr(CC, "compile_checks", compile_checks)
    assert RR.compiled_checks(_FACTS, cap, "gemmini") == {"geometry": 1, "provider": "first"}
    cap["fixture_geometry"] = 2
    assert RR.compiled_checks(_FACTS, cap, "gemmini") == {"geometry": 2, "provider": "first"}
    provider_revision[0] = "replacement"
