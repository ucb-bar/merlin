...
def _fake_simt_introspect():
    ...
    m.build_facts = lambda: {
        ...
        "facts": {
            ...
            "simt": {"lanes_per_warp": 8, "warps_per_core": 4, "cores": 1, "state": "derived", "evidence": "synthetic"},
            "isa": {
                "encoding_bits": 32,
                "instruction_classes": ["FOO_INVOKE", "OP"],
                "state": "derived",
                "evidence": "synthetic",
            },
        },
...
def test_g1_second_simt_runner_config_tracks_the_contract():
    ...
    m = CapabilityManifest(
        ...
        suite=f"{FAKE}-capsule-bench",
        dtype="fp8",
        fourth_output_name=None,
        tier_sim={"L2": "wc_sim", "L3": "wc_rtl"},
        rtl_tiers=("L3",),
        perf_fields=("flops",),
        trace_gate=None,
...
def test_g3_second_simt_oracle_routes_to_its_registered_sim(monkeypatch):
    ...
    CR.register_sim_oracle(
        "wc_sim", adapters=lambda t: sentinel, available=lambda t: (True, f"{t}: wc_sim ready"), exclusive=True
    )
    try:
