...
def test_the_command_buffer_schema_accepts_a_decline():
    schemas.validate_command_buffer(
        _cb(declined={"reason": "no M-tiling: this backend lowers M=32 only", "shape": [64, 32, 32], "op": "matmul"})
    )
...
def _declined_op(name, shape):
    return _op(
        name, status="declined", declined={"reason": "this backend lowers M=32 only", "shape": shape, "op": "matmul"}
    )
...
def test_the_declines_are_named_with_their_shapes(monkeypatch):
    ...
    assert s["declined"] == [
        {"capsule": "A1", "reason": "this backend lowers M=32 only", "shape": [64, 32, 32], "op": "matmul"}
    ]
...
def test_the_headline_separates_a_decline_from_a_wrong_answer(monkeypatch):
    ...
    s = _score(monkeypatch, [_op("A0"), _declined_op("A1", [64, 32, 32]), _declined_op("A2", [64, 64, 32])])
...
def test_the_round_verdict_carries_the_decline_to_the_agent(monkeypatch, tmp_path):
    ...
    sys.path.insert(
        0,
        str(__import__("merlin.common.paths", fromlist=["x"]).repo_root() / "merlin/experiments/capsule_bench/harness"),
    )
    from merlin_experiments.phase1.feedback import qa as qa_check

    rr = tmp_path / "runs" / "atlas-capsule-bench" / "A1"
    rr.mkdir(parents=True)
    (rr / "capsule_result.json").write_text(
        json.dumps(
            {
                "capsule": "A1",
                "status": "declined",
