...
def _whole_program_cb(cap: dict) -> dict:
    ...
    return {
        ...
        "kernel_abi": {
            "kind": "whole_program",
            "args": (
                [{"tensor": spec["name"], "access": "read"} for spec in inputs] + [{"tensor": out, "access": "write"}]
            ),
            "outputs": [out],
        },
    }
...
def test_whole_program_grades_the_complete_elf_against_the_capsule_golden(tmp_path, monkeypatch):
    cap = load_capsule(CAPSULE, contract="merlin/contract")
    cb = _whole_program_cb(cap)
    monkeypatch.setattr(CR, "run_entrypoints", lambda *args, **kwargs: (object(), cb, "module {}"))
    monkeypatch.setattr(
        "merlin.runtime.reference.reference_outputs",
        lambda _cb: (_ for _ in ()).throw(
            AssertionError("an accelerator command projection is not the mixed program's semantic oracle")
        ),
    )
    monkeypatch.setattr(
        "merlin.runtime.simulator.simulate",
        lambda _cb: (_ for _ in ()).throw(
            AssertionError("an accelerator command projection is not the mixed program's semantic oracle")
        ),
    )
    ...
    result = CR.run_capsule(
        ...
        "unused-package",
        runs_root=tmp_path,
        run_id="whole_program_pass",
        config=_config(),
        oracle_adapters=_oracles(CG.golden(cap), seen),
    )
...
def test_whole_program_fails_when_the_complete_elf_disagrees_with_the_golden(tmp_path, monkeypatch):
    cap = load_capsule(CAPSULE, contract="merlin/contract")
    cb = _whole_program_cb(cap)
    monkeypatch.setattr(CR, "run_entrypoints", lambda *args, **kwargs: (object(), cb, "module {}"))
    bad = CG.golden(cap)
    ...
    result = CR.run_capsule(
        ...
        "unused-package",
        runs_root=tmp_path,
        run_id="whole_program_fail",
        config=_config(),
        oracle_adapters=_oracles(bad),
    )
...
def test_recomputed_integer_whole_program_ignores_stale_recorded_operands(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(CR, "run_entrypoints", lambda *args, **kwargs: (object(), cb, "module {}"))
    stale = {
        spec["name"]: {"shape": list(spec["shape"]), "values": [0] * __import__("math").prod(spec["shape"])}
    ...
    result = CR.run_capsule(
        ...
        "unused-package",
        runs_root=tmp_path,
        run_id="whole_program_stale_inputs",
        config=_config(),
        oracle_adapters=_oracles(CG.golden(cap), seen),
    )
