...
def test_source_only_ci_mode_keeps_missing_contracts_unqualified_and_regressions_fatal(monkeypatch):
    gate = _gate()
    base = {
        "n_admitted_capsules_checked": 1,
        "unresolved_targets": {"generated_target": "contract not generated"},
        "unmatched_requested_targets": [],
        "weakened": [],
    }
    monkeypatch.setattr(gate, "audit", lambda _targets: base)
    assert gate.main([]) == 2
    assert gate.main(["--allow-unresolved"]) == 0

    base["weakened"] = [
        {
            "target": "resolved_target",
            "capsule": "new_weakening",
            "hand_authored": False,
            "family": "contraction",
            "dtype": "i8",
    ...
    assert gate.main(["--allow-unresolved"]) == 1

    base["weakened"] = []
    base["n_admitted_capsules_checked"] = 0
    assert gate.main(["--allow-unresolved"]) == 2


def test_misspelled_target_cannot_pass_even_in_source_only_mode():
    gate = _gate()
    assert gate.main(["--target", "no_such_target", "--allow-unresolved"]) == 2
