...
def _declared_policy(target):
    from merlin.targetgen.corpus_spec import profile_datapath

    profile = for_target(target).recipe
    return profile_datapath(yaml.safe_load(profile.read_text()), numeric_only=True)
...
def test_two_targets_derive_two_different_datapaths():
    ...
    got = {t: mesh_datapath(t, numeric_policy=_declared_policy(t)) for t in TARGETS}
...
def test_the_datapath_carries_the_targets_declared_facts_not_dataclass_defaults():
    ...
    for t in TARGETS:
        declared = _declared_policy(t)
        if not declared:
            continue
        b = mesh_datapath(t, numeric_policy=declared)
