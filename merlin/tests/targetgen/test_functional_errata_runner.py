...
def test_funct3_and_vector_funct7_corrections_are_not_target_special_cases() -> None:
    ...
    applied = apply_reviewed_errata(
        module,
        {
            "CSRRCI": {"authoritative": "rtl", "declared": "0x00004073", "hardware": "0x00007073"},
            "VCUBE_BF16": {"authoritative": "rtl", "declared": "0x9e000057", "hardware": "0x8e000057"},
        },
    )
    assert csr.funct3 == 7
...
def test_stale_review_fails_closed_instead_of_patching_a_different_model() -> None:
    ...
    with pytest.raises(ValueError, match="does not match reviewed declared value"):
        apply_reviewed_errata(
            module, {"OP": {"authoritative": "rtl", "declared": "0x0200007f", "hardware": "0x0000007f"}}
        )
...
def test_non_rtl_or_missing_class_corrections_do_not_silently_apply() -> None:
    ...
    assert (
        apply_reviewed_errata(
            module, {"UNRESOLVED": {"authoritative": "unresolved", "declared": "0x1", "hardware": "0x2"}}
        )
        == []
    )
    with pytest.raises(ValueError, match="names no model class"):
        apply_reviewed_errata(module, {"MISSING": {"authoritative": "rtl", "declared": "0x1", "hardware": "0x2"}})
...
def test_vmem_word_address_overlay_changes_only_named_engine_operations() -> None:
    ...
    applied = apply_reviewed_model_errata(
        module,
        {
            "addressing": {
                "authoritative": "rtl",
                "correction": "vmem_base_unit_bytes",
                "model_classes": ["EngineOp"],
