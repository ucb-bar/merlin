...
def test_the_synth_role_matches_what_the_synthesizer_actually_stamps():
    ...
    assert WRITER.SYNTH_ROLE == CS.SOURCE_ROLE
...
def test_an_unprovable_forbid_is_its_own_exception_type():
    ...
    repo forbids."""
    assert issubclass(WRITER.UnprovableForbid, ValueError)
...
def test_a_hand_authored_capsule_still_aborts(tmp_path):
    ...
    with pytest.raises(WRITER.UnprovableForbid):
        WRITER._verify_a_forbidden_lane_is_provable(tmp_path, cap, "gemmini")
...
def test_a_capsule_forbidding_nothing_is_not_examined(tmp_path):
    ...
    WRITER._verify_a_forbidden_lane_is_provable(tmp_path, {"name": "X", "lanes": {}}, "gemmini")
    WRITER._verify_a_forbidden_lane_is_provable(tmp_path, {"name": "X"}, None)
...
def test_the_manifest_records_the_dropped_lane_rather_than_omitting_it(tmp_path):
    ...
    PROVENANCE.update_provenance_manifest([], cap_root=tmp_path, target="gemmini", unprovable_forbids=dropped)
    ...
    PROVENANCE.update_provenance_manifest([], cap_root=tmp_path, target="gemmini")
