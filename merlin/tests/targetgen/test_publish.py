...
def test_dry_run_makes_no_network_calls(out_root, monkeypatch):
    ...
    res = pub.publish("rvv", dry_run=True, gate=False)
...
def test_publish_commits_tags_and_is_idempotent(out_root, monkeypatch):
    ...
    res = pub.publish("rvv", dry_run=False, gate=False)
    ...
    res2 = pub.publish("rvv", dry_run=False, gate=False)
...
def test_real_push_to_github_refused_without_confirmation(out_root):
    ...
    with pytest.raises(pub.PublishError, match="REFUSED|confirm"):
        pub.publish("rvv", dry_run=False, remote=remote, gate=False)
    # dry-run still plans fine (no confirmation needed, no network)
    res = pub.publish("rvv", dry_run=True, remote=remote, gate=False)
...
def test_host_schedule_stays_data_and_support_does_not_become_compiler(out_root, monkeypatch):
    troot = out_root / "artifacts" / "targets"
    source = _make_rvv_package(troot)
    _make_gemmini_package(troot)
    _bare_remote(out_root, "rvv", monkeypatch)
    _bare_remote(out_root, "gemmini", monkeypatch)
    rvv = pub.publish("rvv", dry_run=True, gate=False)
    assert (rvv.repo_dir / "schedule.mlir").read_bytes() == (source / "schedule.mlir").read_bytes()
    assert (rvv.repo_dir / "manifest.yaml").read_bytes() == (source / "manifest.yaml").read_bytes()
    for forbidden in ("CMakeLists.txt", "tools", "include", "lib", "payload"):
        assert not (rvv.repo_dir / forbidden).exists()
    assert "host schedule" in (rvv.repo_dir / "MERLIN_PUBLICATION.md").read_text()
    with pytest.raises(pub.PublishError, match="not support"):
        pub.publish("gemmini", dry_run=True, gate=False)
...
def test_promote_sets_single_champion(out_root):
    ...
    _make_rvv_package(troot, "hand_v0")
    _make_rvv_package(troot, "cand_b")
    pub.promote("rvv", "hand_v0", gate=False)

    m1 = pub.package_records.read_record(troot / "rvv" / "hand_v0")
    ...
    pub.promote("rvv", "cand_b", gate=False)
    m1b = pub.package_records.read_record(troot / "rvv" / "hand_v0")
    m2 = pub.package_records.read_record(troot / "rvv" / "cand_b")
...
def test_unbound_record_certification_does_not_grant_new_authority(out_root, tmp_path):
    ...
    with pytest.raises(pub.PublishError, match="input identity"):
        pub.promote("gemmini", "oot_pkg")
    sel = pub.select_champion("gemmini", package_id="oot_pkg")
    assert sel.cert_status == "unverified"


def test_payload_change_with_same_ids_changes_publish_fingerprint(out_root, monkeypatch):
    """Local Git plumbing only: unverified export does not silently no-op after drift."""
    troot = out_root / "artifacts" / "targets"
    ...
    schedule = package / "schedule.mlir"
    schedule.write_text(schedule.read_text() + "\n// changed input bytes\n")
