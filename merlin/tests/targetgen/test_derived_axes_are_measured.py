...
def test_a_capsule_declaring_no_compute_dtype_still_reports_its_operands(tmp_path):
    ...
    _capsule(
        ...
        inputs=[
            {"role": "input", "dtype": "bf16", "shape": [16, 16]},
...
def _epi(*stages):
    return [
        {"stage": s, "family": "elementwise_map", "isa_classes": [], "evidenced_by": ["manifest_composed_with"]}
        for s in stages
    ]
...
def test_an_epilogue_stage_no_capsule_demands_is_reported_uncovered(tmp_path):
    ...
    assert gap["uncovered"] == ["requant"], (
        "a stage the ABI declares and no capsule demands means a backend that cannot emit it fails nothing here"
    )
