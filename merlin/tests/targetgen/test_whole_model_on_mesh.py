...
def test_whole_model_matmuls_run_on_any_targets_mesh(target, sizing):
    ...
    policy, _ = load_declared_numeric_policy(load_target_experiment(descriptor_path(target)), repo=repo_root())
    try:
        _b = mesh_datapath(target, numeric_policy=policy)
    ...
    edge = _mesh_tile_binding(target, None, None, numeric_policy=policy).tile_dim
    if sizing == "sub_tile":
        edge = 4
    res = compile_cli.run_whole_model_on_mesh(
        target,
