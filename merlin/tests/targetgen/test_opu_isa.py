...
def _derive(root):
    return OI.derive(
        consts=root / "Consts.scala",
        instructions=root / "Instructions.scala",
        params=root / "Parameters.scala",
        funct6_enum="OPMFunct6",
        consts_container="HasThings",
        insn_seq="unitInsns",
...
class TestUnitInstructionSequence:
    def test_reads_each_object_with_its_instantiation_form(self):
        assert OI.unit_instruction_forms(_PARAMS, "unitInsns") == [("ACC", "VV"), ("MOVEIN", "VX"), ("OUT", "VX")]
...
class TestDerivation:
    ...
    def test_a_missing_source_file_is_a_gap_not_an_exception(self, tmp_path):
        d = OI.derive(
            consts=tmp_path / "nope.scala",
            instructions=tmp_path / "nope.scala",
            params=tmp_path / "nope.scala",
            funct6_enum="E",
            consts_container="C",
            insn_seq="s",
            opcode_name="o",
    ...
    def test_an_unmapped_form_gaps_that_instruction_rather_than_defaulting_a_funct3(self, sources):
        d = OI.derive(
            consts=sources / "Consts.scala",
            instructions=sources / "Instructions.scala",
            params=sources / "Parameters.scala",
            funct6_enum="OPMFunct6",
            consts_container="HasThings",
            insn_seq="unitInsns",
            opcode_name="opcVector",
            form_funct3={"VV": "OPMVV"},
        )  # VX omitted
...
class TestCrosscheck:
    def test_agreement_on_every_field_makes_the_derivation_usable(self, sources):
        d = OI.crosscheck(
            _derive(sources), sources / "hdr.h", pairs={"ACC": "ACCUM", "MOVEIN": "MOVE_IN", "OUT": "READ_OUT"}
        )
        assert d.ok
    ...
    def test_a_disagreement_fails_closed_and_names_the_field(self, sources):
        ...
        d = OI.crosscheck(
            _derive(sources), sources / "bad.h", pairs={"ACC": "ACCUM", "MOVEIN": "MOVE_IN", "OUT": "READ_OUT"}
        )
...
@pytest.mark.skipif(not _env("MERLIN_CHIPYARD"), reason="needs the hardware checkout ($MERLIN_CHIPYARD)")
...
class TestVectorUnitParams:
    ...
    def test_a_package_qualified_call_site_is_still_read(self):
        assert OI.vector_unit_params(_CONFIGS, "OPUV256D128ShuttleConfig", mixin_text=_MIXINS)["vLen"] == 256
    ...
    def test_a_nested_argument_does_not_split_the_argument_list(self):
        ...
        assert OI.vector_unit_params(text, "OPUV256D128ShuttleConfig", mixin_text=_MIXINS) == {"vLen": 256, "dLen": 128}


class TestScalaReaders:
    def test_parameter_names_are_read_in_declaration_order(self):
        assert OI.scala_param_names(_MIXINS, "class WithRocketVectorUnit") == ["vLen", "dLen", "params"]
