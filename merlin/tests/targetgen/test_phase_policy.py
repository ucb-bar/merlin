...
def test_an_operand_past_the_measured_range_is_refused_for_unknown_cost_not_for_size():
    big = _capsule(
        inputs=[
            {"name": "A", "role": "input", "shape": [4096, 4096]},
...
def test_an_unpriceable_member_says_why_it_costs_more_than_itself():
    odd = _capsule(
        operation={"op": "mystery", "attributes": {"out": "Y"}},
        semantic={"semantic_family": "contraction"},
...
def test_a_transposed_weight_is_priced_on_the_axis_it_actually_shares():
    ...
    t = _capsule(
        inputs=[{"name": "A", "role": "input", "shape": [8, 32]}, {"name": "W", "role": "weight", "shape": [16, 32]}]
    )
...
def test_operands_that_share_no_axis_refuse_rather_than_guess():
    bad = _capsule(
        inputs=[{"name": "A", "role": "input", "shape": [8, 32]}, {"name": "W", "role": "weight", "shape": [7, 5]}]
    )
...
def test_a_weight_stationary_member_prices_every_activation_sharing_the_weight():
    ...
    rr = _capsule(
        operation={"op": "resident_reuse", "attributes": {"weight": "W"}},
        inputs=[
            {"name": "W", "role": "weight", "shape": [16, 16]},
...
def test_a_convolution_is_priced_from_its_declared_geometry():
    ...
    conv = _capsule(
        ...
        inputs=[
            {"name": "W", "role": "weight", "shape": [36, 16]},
...
def test_a_geometry_leaving_no_output_position_refuses():
    tiny = _capsule(
        ...
        inputs=[
            {"name": "W", "role": "weight", "shape": [324, 16]},
...
def test_two_activations_sharing_an_axis_are_a_contraction_without_any_weight():
    ...
    qk = _capsule(
        operation={"op": "attention_qk", "attributes": {"q": "Q", "k": "K", "out": "Y"}},
        inputs=[{"name": "Q", "role": "input", "shape": [16, 32]}, {"name": "K", "role": "input", "shape": [16, 32]}],
    )
...
def test_an_anchor_is_never_drawn_from_a_different_obligation():
    ...
    att = _capsule(
        ...
        inputs=[
            {"name": "Q", "role": "input", "shape": [256, 64], "dtype": "i8"},
...
def test_the_verified_anchor_is_the_one_the_capsule_DECLARES(tmp_path):
    ...
    a = PP.anchors(
        [declared, in_group, member], target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path]
    )
...
def test_a_declared_extends_naming_an_uncertified_sibling_stays_unverified(tmp_path):
    ...
    a = PP.anchors([declared, member], target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path])
    row = next(r for r in a["paired"] if r["member"] == "member")
...
def test_the_computed_anchor_prefers_a_sibling_that_WAS_certified(tmp_path):
    ...
    a = PP.anchors(
        [attested, never_run, member], target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path]
    )
    row = next(r for r in a["paired"] if r["member"] == "member")
...
def test_a_member_that_ITSELF_certified_is_not_demanded_an_anchor(tmp_path):
    ...
    a = PP.anchors([sibling, member], target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path])
...
def test_verification_does_not_depend_on_how_the_corpus_was_ENUMERATED(tmp_path):
    ...
    caps = [
        _sized("attested_a", 32, 32, 32),
        _sized("attested_b", 32, 32, 32),
        _sized("member", 512, 512, 512, capped="L2"),
    ]
    _result(tmp_path, "attested_a")
    _result(tmp_path, "attested_b")

    forward = PP.anchors(caps, target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path])
    backward = PP.anchors(
        list(reversed(caps)), target="t", cycle_accurate_available=True, verify=True, roots=[tmp_path]
    )
...
def test_the_reach_report_separates_no_claim_from_an_unreachable_one():
    ...
    caps = [
        _capsule(name="functional"),
        _capsule(name="wired", performance=_perf()),
        _capsule(name="unwired", performance=_perf(acceptance={})),
        _capsule(name="contradicted", performance=_perf(comparand={"demand_equal": ["separation_regime"]})),
    ]
...
class TestNecessityDoesNotCondemnOnPartialEvidence:
    """`necessary` sees ONE of eleven requirement axes, and must not answer as though it saw them all.

    The requirement asks for cells, composition shapes, conv windows, epilogue stages, memory regimes,
    aspect ratios, host-lane and host-only pairs, adjacency chains, stage groups and carried state.
    `covers_cell` reads a capsule's family and dtype, which reaches exactly the first. A capsule can
    therefore be the only witness of a required conv window while witnessing no required cell at all.

    Measured on gemmini when this returned NO: 48 of 189 capsules were condemned, among them
    `GQ0..GQ3` -- the quantized-convolution capsules that are the target's only conv evidence. A NO a
    caller reads as "safe to delete" is how a corpus loses the evidence it was built for, so a capsule
    outside the cell set is UNDECIDED here rather than unnecessary.
    """

    @staticmethod
    def _capsule(family: str, dtype: str) -> dict:
        return {"semantic": {"semantic_family": family}, "inputs": [{"dtype": dtype}]}

    def test_a_capsule_outside_the_cell_set_is_undecided_not_unnecessary(self):
        verdict = PP.necessary(self._capsule("convolution", "i8"), {("contraction", ("i8",))})
        assert verdict.value == PP.UNKNOWN, (
            "a capsule witnessing no required CELL was reported unnecessary, but ten other requirement "
            "axes are invisible from its family and dtype -- this is the verdict that condemns a conv "
            "capsule for not being a contraction"
        )
        assert "eleven requirement axes" in verdict.reason

    def test_a_capsule_inside_the_cell_set_is_still_necessary(self):
        verdict = PP.necessary(self._capsule("contraction", "i8"), {("contraction", ("i8",))})
        assert verdict.value == PP.YES

    def test_an_underived_requirement_is_still_undecidable(self):
        """Unchanged: an underived requirement is not an empty one."""
        assert PP.necessary(self._capsule("contraction", "i8"), None).value == PP.UNKNOWN

    def test_nothing_in_the_shipped_corpus_is_condemned(self):
        """The regression this exists for, over the real corpus rather than a fixture."""
        import yaml

        from merlin.common.paths import merlin_dir

        spec_path = merlin_dir() / "contract" / "capsules" / "conformance" / "gemmini.yaml"
        if not spec_path.is_file():
            pytest.skip("no tracked gemmini conformance spec in this checkout")
        spec = yaml.safe_load(spec_path.read_text(encoding="utf-8")) or {}
        required = {
            (str(c["family"]), (str(c["dtype"]),))
            for c in (spec.get("cells") or ())
            if c.get("family") and c.get("dtype")
        }
        root = merlin_dir() / "contract" / "capsules"
        condemned = []
        for sub in ("isa", "layers", "_perf"):
            for p in sorted((root / sub).rglob("capsule.yaml")):
                cap = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
                if PP.necessary(cap, required).value == PP.NO:
                    condemned.append(cap.get("name"))
        assert not condemned, f"capsules reported unnecessary on cell evidence alone: {condemned[:8]}"
