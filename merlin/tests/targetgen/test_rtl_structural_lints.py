...
def _screen(trace, capsule):
    return RC.screen(trace, capsule, _FACTS, target="gemmini")
...
def test_multiple_declared_outputs_are_each_checked():
    ...
    cap = {
        ...
        "operation": {
            ...
            "attributes": {
                "weight": "W",
                "matmuls": [
                    {"lhs": "A0", "out": "Y0", "epilogue": [], "output_dtype": "i32"},
    ...
    both["instructions"].insert(
        -1,
        {
            "index": 99,
            "class": "MVOUT",
            "funct": 3,
            "decoded": {"rows": 16, "cols": 16, "dram": {"kind": "argbase", "arg_index": 4, "offset": 0}},
        },
    )
...
def test_legalization_is_skipped_when_the_array_geometry_is_unknown():
    ...
    rep = RC.screen(_trace([(0, 16, 16)]), _matmul_capsule(M=15, K=16, N=17), facts, target="gemmini")
    c = _check(rep, "T0.extent_tile_legalization")
    assert c.status == "skipped" and "UNKNOWN" in c.message
...
def test_a_conv_is_accepted_when_the_target_names_a_fused_conv_loop_and_the_stream_uses_it():
    ...
    try:
        RC._vocabulary_classes = lambda target, stems: (
            ({"LOOP_CONV"} if "conv" in stems else {"LOOP_WS", "LOOP_CONV"}),
            "test vocabulary",
        )
...
def test_conv_depth_is_unknown_not_failed_when_no_compute_class_is_recognized():
    cap = _conv_capsule()
    t = _rename_compute(_trace([(0, 16, 8), (512, 16, 8), (1024, 4, 8)], n_compute=3, out_stride_bytes=32))
    c = _check(_screen(t, cap), "T0.conv_lowering")
    assert c.status == "skipped" and "UNKNOWN" in c.message
