...
class TestASystemExitTracebackIsNotADiagnosis:
    """A worker that ends `raise SystemExit(main())` names nothing in its traceback.

    The rule this function already implements -- prefer the last traceback over the tail -- is right
    for a process that DIED of an exception and wrong for one that chose its exit code. The latter
    prints its reason itself, earlier in stderr, and then exits; preferring the traceback reports the
    four-line `SystemExit` wrapper and discards the message.

    Measured 2026-09-21 on the m2m capture worker: three capsules failed to generate with `rc=1` and
    the recorded cause was `raise SystemExit(main())`, which names neither the capsule's problem nor
    the worker's. The generation failure was undiagnosable from its own error message.
    """

    def test_the_workers_own_message_survives_a_systemexit_wrapper(self):
        stderr = (
            "loading model...\n"
            "ERROR: could not import torch backend: libcudart.so.12: cannot open shared object file\n"
            "Traceback (most recent call last):\n"
            '  File "/x/_m2m_capture_worker.py", line 399, in <module>\n'
            "    raise SystemExit(main())\n"
            "SystemExit: 1\n"
        )
        cause = _stderr_cause(stderr)
        assert "libcudart" in cause, (
            "the worker printed why it failed and exited deliberately; reporting only the SystemExit "
            "wrapper discards the one line that names the problem"
        )

    def test_a_real_exception_traceback_is_still_preferred_alone(self):
        """The original guard must not regress: a benign warning after a real traceback stays out."""
        stderr = (
            "Traceback (most recent call last):\n"
            '  File "a.py", line 1, in <module>\n'
            "    x()\n"
            "ValueError: bad shape\n"
            "UserWarning: tensor attributes are not part of a single contiguous chunk of memory\n"
        )
        cause = _stderr_cause(stderr)
        assert "ValueError: bad shape" in cause
        assert "stderr tail" not in cause, "a traceback that names its exception needs no tail"
