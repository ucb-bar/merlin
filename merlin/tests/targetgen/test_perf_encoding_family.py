...
_PERF = for_target("gemmini").performance_template
...
def _ladder_levels() -> set[str]:
    ...
    return set(PROFILES._PERFORMANCE_LEVELS)


def _template() -> dict:

    return PROFILES.load_profile("gemmini", **for_target("gemmini").profile_inputs()).get("_performance_template") or {}
...
def _expanded(target: str):
    ...
    errors: list = []
    entries = SWEEPS.expand_sweeps(prof, binding, skipped=[], blocked_unimplemented=[], errors=errors)
...
def test_the_encoding_family_compares_two_of_the_targets_own_encodings(target):
    ...
    declared = set(SWEEPS.target_encodings(target))
    groups: dict[str, set] = {}
    for e in members:
...
def test_an_encoding_index_past_the_end_raises_rather_than_repeating_the_last():
    ...
    with pytest.raises(ValueError, match="encoding"):
        SWEEPS.resolve_encoding("encoding1", ["i8"])
    assert SWEEPS.resolve_encoding("encoding1", ["bf16", "i8"]) == "i8"
...
def test_a_group_reduced_to_one_member_is_dropped_rather_than_shipped():
    ...
    assert hasattr(SWEEPS, "expand_sweeps")
