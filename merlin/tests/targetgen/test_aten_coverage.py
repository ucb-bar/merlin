...
def _captures() -> dict:
    ...
    if not root.is_dir():
        return {}
    return {
        d.name.replace("_fp32_consistent", "").replace("_consistent", ""): d / "model.mlir"
...
def test_the_census_separates_core_from_non_core(opset):
    ...
    core = set(opset["ops"])
    assert not (set(cen["non_core_observed"]) & core), "an op cannot be both core and non-core"
