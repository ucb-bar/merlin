...
def test_explicit_descriptor_needs_no_target_and_beats_the_process_descriptor(tmp_path, monkeypatch):
    desc = _target_dir(tmp_path, "MERLIN_FAUX_DIRECT=selected\n")
    monkeypatch.setenv("MERLIN_TARGET_EXPERIMENT", str(tmp_path / "wrong.yaml"))
    monkeypatch.delenv("MERLIN_FAUX_DIRECT", raising=False)
    assert source_experiment_env(descriptor=desc) == ["MERLIN_FAUX_DIRECT"]
    assert os.environ["MERLIN_FAUX_DIRECT"] == "selected"
    with pytest.raises(ValueError, match="requires a target or explicit descriptor"):
        source_experiment_env()
...
def test_selected_resource_environment_never_falls_back_to_descriptor_siblings(tmp_path, monkeypatch, present):
    import yaml

    descriptor = _target_dir(tmp_path, "MERLIN_FAUX_SELECTED=wrong-sibling\n")
    resources = tmp_path / "selected"
    resources.mkdir()
    descriptor.write_text(yaml.safe_dump({"target": "faux", "resources_root": str(resources)}))
    (resources / "tooling.env.example").write_text("MERLIN_FAUX_TEMPLATE=not-live\n")
    if present:
        (resources / "experiment.env").write_text("MERLIN_FAUX_SELECTED=selected\n")
    monkeypatch.delenv("MERLIN_FAUX_SELECTED", raising=False)
    monkeypatch.delenv("MERLIN_FAUX_TEMPLATE", raising=False)

    assert source_experiment_env(descriptor=descriptor) == (["MERLIN_FAUX_SELECTED"] if present else [])
    assert os.environ.get("MERLIN_FAUX_SELECTED") == ("selected" if present else None)
    assert "MERLIN_FAUX_TEMPLATE" not in os.environ
