...
Three surfaces decide independently today, each written for its own caller:
  ...
  * `llvmlower.int8_contractions` carries its own dtype triple and op table as module literals.
...
def _literal(src):
    from merlin.llvmlower.int8_contractions import routable_contractions
...
def test_the_derived_surface_is_not_simply_echoing_the_literal():
    ...
    floaty = [t for t in ("atlas", "radiance") if any(a not in ("i32",) for _l, _r, a in (triples(t) or ()))]
    if not floaty:
...
def test_asking_an_underivable_device_routes_nothing_rather_than_falling_back():
    ...
    with IR_LOCK:
        assert routable_contractions(mq.parse(_CORPUS["i8_square"]), device="definitely_not_a_target") == []
...
def test_an_explicit_datapath_overrides_the_literal():
    ...
    with IR_LOCK:
        assert routable_contractions(mq.parse(_CORPUS["i8_square"]), dtypes=("f32", "f32", "f32")) == []
    with IR_LOCK:
        assert len(routable_contractions(mq.parse(_CORPUS["f32_square"]), dtypes=("f32", "f32", "f32"))) == 1
