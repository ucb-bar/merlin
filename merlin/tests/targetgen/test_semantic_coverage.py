...
def test_eligibility_truth_table():
    ...
    # FAIL-CLOSED on an op the vocabulary does not know. It used to be spelled `gather` here, which the
    # taxonomy now names (`movement` -- an indexed row move is data motion, and it is one of the
    # prov.op spellings `op_profile` classifies, so it has to resolve; see
    # test_capture_gap_is_demandable). A named op is the wrong probe for this assertion: the property
    # is that an UNNAMEABLE op yields no family, so the probe has to be an op nothing names.
    assert sf.from_op("gather") == "movement"
    v = el.is_eligible(D(op="a_computation_no_primitive_describes", in_dtype="fp16"), cm)
...
def test_suite_acceleratable_coverage():
    ...
    caps = [
        ...
        },
        {
            "name": "SORT",
            "operation": {"op": "reduce"},
            "numeric_policy": {"dtype": "fp32"},
            "semantic": {"eligible": False, "fallback_allowed": True},
        },
    ]
    results = [
        {"capsule": "A2", "kind": "isa", "label": "public", "status": "pass", "tiers": {"L2": {"status": "pass"}}},
        {
            "capsule": "A7",
            "kind": "isa",
            "label": "public",
...
def test_semantic_capabilities_differ_across_targets():
    ...
    gem = _residual_caps("examples/gemmini/target/contracts/residual.yaml")
...
def test_fuzzer_is_deterministic_and_in_closure():
    ...
    assert [tuple(r.source for r in p.regions) for p in a] == [tuple(r.source for r in p.regions) for p in b]
...
def test_composed_with_blocks_standalone_and_intersects_on_merge():
    ...
    epi = cu.SemanticCapability(family="elementwise_map", dtypes=("int8",), composed_with=("contraction",))
    ...
    assert el.is_eligible(el.RegionDescriptor(op="gelu", in_dtype="int8"), {"elementwise_map": merged}).eligible
    ...
    doc = cr.render_markdown(
        {
            ...
            "acceleratable_coverage": {
                "n_eligible": 3,
                "n_eligible_accelerated": 3,
                "acceleratable_region_recall": 1.0,
                "acceleration_precision": 1.0,
                "n_undetermined": 0,
                "n_unclassified": 0,
                "by_family": {
                    "contraction": {"n_regions": 3, "n_eligible": 3, "n_eligible_accelerated": 3, "recall": 1.0},
                    "elementwise_map": {"n_regions": 2, "n_eligible": 0, "n_eligible_accelerated": 0, "recall": None},
                },
