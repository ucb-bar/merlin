...
def test_the_pilot_corpus_demands_a_cycle_accurate_tier(target, monkeypatch):
    ...
    context = load_context(desc, repo=repo_root())
    public = authoring._grading_public_root(context)()
    caps = list(public.rglob("capsule.yaml"))
    ...
    from merlin_experiments.phase1.feedback import certification, loop_grading

    inputs = loop_grading.GradingInputs(context, "raw_baseline", public, authoring.policy_roots(context), None)
    armed, why = certification._cycle_accurate_checkpoint_enabled(inputs=inputs)
