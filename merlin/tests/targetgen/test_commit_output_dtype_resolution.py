...
def _binding(target: str = "gemmini"):
    ...
    try:
        descriptor = generation._descriptor_for(target)
        generation._ensure_contract_on_path(descriptor)
        te = load_target_experiment(descriptor)
        return CS.derive_binding(
            te, profiles.load_profile(target, **for_target(target).profile_inputs()).get("datapath", {})
        )
