...
def _screened(passed: int, failed: int = 0, unavailable: int = 0) -> dict:
    ...
    return {"n_screened": n, "n_screen_passed": passed, "n_screen_failed": failed, "n_screen_unavailable": unavailable}
...
def test_a_declared_screen_tier_is_never_dropped_from_the_tier_map():
    ...
    for tile_exec in (
        _screened(12),
        _screened(11, failed=1),
        _screened(0, unavailable=12),
        {"n_screened": 0, "n_screen_passed": 0},
        {},
        None,
    ):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"n_screened": 0, "n_screen_passed": 0, "n_screen_failed": 0, "n_screen_unavailable": 0}, "skipped"),
...
def test_an_unrecorded_screen_is_unavailable_never_a_pass():
    ...
    tiers = _model_tier_map(DECLARED, "gemmini", ON_MESH, {"n_screened": None, "per_tile": [{"oracle_result": "pass"}]})
...
def test_no_tiles_is_a_skip_and_verified_tiles_without_a_screen_is_a_hole():
    ...
    unscreened = _model_tier_map(DECLARED, "gemmini", ON_MESH, {"per_tile": [{"oracle_result": "pass"}]})["L2"]
...
def test_a_partial_screen_tally_does_not_pass():
    ...
    tiers = _model_tier_map(DECLARED, "gemmini", ON_MESH, {"n_screened": 12, "n_screen_passed": 10})
    assert tiers["L2"].status == "unavailable"
...
def test_the_model_grade_refuses_a_cert_with_no_screen_record():
    ...
    for token in (
        "n_screened",
        "not_all_model_tiles_screened",
        "model_tile_screen_failed",
