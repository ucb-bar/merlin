...
def _stub_worker(monkeypatch, seen: dict, *, meta_extra: dict | None = None):
    ...
    def fake_run(cmd, **kwargs):
        ...
        # The identity model has no parameters, but the capture ABI still ships
        # an empty safetensors container and argument manifest.
        (d / "weights.safetensors").write_bytes((8).to_bytes(8, "little") + b"{}      ")
        (d / "weights.safetensors.manifest.json").write_text("{}\n", encoding="utf-8")
        meta = {
            "ok": True,
            "opaque": 0,
            "opaque_detail": {},
...
def _write(
    tmp_path, monkeypatch, *, workload="acme_net_v2", capture_toml="", meta_extra=None, entry_model=None
) -> tuple:
...
def test_declared_loader_env_reaches_the_capture_worker(tmp_path, monkeypatch):
    ...
    home.mkdir()
    d, seen = _write(tmp_path, monkeypatch, capture_toml=f'[env]\nACME_HOME = "{home}"\nACME_LAYERS = "2"\n')
...
def test_synthetic_inputs_are_recorded_and_withhold_the_accuracy_claim(tmp_path, monkeypatch):
    d, _ = _write(
        ...
        meta_extra={
            ...
            "loader_provenance": {
                "input_source": "synthetic_seed_20260830",
                "synthetic_inputs": True,
                "checkpoint": "vendor/acme_net/V2",
...
def test_real_attributed_inputs_do_support_the_claim(tmp_path, monkeypatch):
    ...
    d, _ = _write(
        ...
        meta_extra={
            ...
            "loader_provenance": {
                "input_source": "curated_validation_split",
                "synthetic_inputs": False,
                "preprocessing": "VENDOR_V2",
                "full_checkpoint": True,
            },
        },
    )
...
@pytest.mark.skipif(
    not (CSrc._m2m_dir() / "workloads").is_dir(), reason="model2MLIR checkout unavailable; set MERLIN_M2M_DIR"
)
