...
def test_the_generated_capsules_match_their_profile_declaration():
    ...
    declarations = {declaration.profile: declaration for declaration in all_declarations()}
    for root in PERF_ROOTS:
        declaration = declarations.get(root.parent.name)
        if declaration is None:
            continue
        doc = PROFILES.load_profile(declaration.profile, include_holdouts=False, **declaration.profile_inputs())
