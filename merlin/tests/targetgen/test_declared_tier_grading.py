...
def _stub_front_half(monkeypatch):
    """Isolate exception routing, not numeric interpretation or access to a stored float golden."""
    ...
    # These tests inject an endpoint that raises before producing numeric evidence. Keep the
    # unrelated front-half numeric engines deterministic, instead of depending on whether this
    # checkout has an independently stored float golden (which decides whether L0/L1 run at all).
    outputs = {"Y0": [0] * (32 * 32)}
...
def test_did_not_halt_is_its_own_plane_not_oracle_unavailable(tmp_path, monkeypatch):
    ...
    command_buffer = _stub_front_half(monkeypatch)
    tier = "L3"
    oracle_calls = []

    def hung(cb, llvm_text, workdir, timeout):
        oracle_calls.append((cb, llvm_text, workdir, timeout))
    ...
    assert len(oracle_calls) == 1, res.get("failure")
    assert {key: oracle_calls[0][0][key] for key in command_buffer} == command_buffer
    assert oracle_calls[0][1] == "# kernel.S (stub)\n"
