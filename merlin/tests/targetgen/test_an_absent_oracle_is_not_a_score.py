...
def _score_from(rows, *, no_oracle=False):
    ...
    score = {"per_capsule": rows, "n_capsules": len(rows), "n_passed": sum(1 for r in rows if r["status"] == "pass")}
    graded = [r for r in rows if r["status"] in ("pass", "fail", "error", "incomplete", "not_gradeable_no_oracle")]
...
def test_a_missing_oracle_makes_the_run_not_gradeable():
    ...
    rows += [_row(f"REAL{i}", "incomplete", "no derived ISA encoding fact for target 'radiance'") for i in range(29)]
    s = _score_from(rows)
...
def test_incomplete_is_not_confused_with_a_withheld_verdict():
    ...
    s = _score_from([_row("A", "not_gradeable_no_oracle"), _row("B", "not_gradeable_no_oracle")], no_oracle=True)
    assert s["n_incomplete"] == 0
    assert "measurement_incomplete" not in s
