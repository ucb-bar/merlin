...
def _result(name: str, *, accurate_s=None, accurate_cycles=None, func_cycles=None) -> dict:
    ...
    if accurate_s is not None:
        tiers["L3"] = {
            "timing": {"sim_active_s": accurate_s},
            "cycles": accurate_cycles,
            "cycle_accurate": True,
...
def _linear_corpus(tmp_path, *, intercept=100.0, per_cycle=0.145, ratio=6.0, n=6):
    ...
    for i in range(n):
        ...
        func = 100 * (i + 1)
        cyc = int(func * ratio)
        docs.append(_result(f"C{i}", accurate_s=intercept + per_cycle * cyc, accurate_cycles=cyc, func_cycles=func))
...
def test_it_refuses_to_scale_when_no_capsule_ran_both_tiers(tmp_path):
    ...
    docs = [
        _result(f"C{i}", accurate_s=100.0 + 0.145 * (500 * (i + 1)), accurate_cycles=500 * (i + 1)) for i in range(6)
    ]
    _write(tmp_path, docs)
    fit = CC.fit_cycles_for("t", timing_root=tmp_path)
...
def test_too_few_points_is_no_fit(tmp_path):
    ...
    assert CC.fit_cycles_for("t", timing_root=tmp_path) is None, "two points define a line through anything"
...
def test_the_deepest_accurate_tier_is_the_cost_when_several_ran(tmp_path):
    ...
    doc = {
        "capsule": "multi",
        "tiers": {
            "L3": {"timing": {"sim_active_s": 100.0}, "cycles": 1000, "cycle_accurate": True, "derived_from_rtl": True},
            "L4": {"timing": {"sim_active_s": 900.0}, "cycles": 1000, "cycle_accurate": True, "derived_from_rtl": True},
        },
    }
    _write(tmp_path, [doc])
        ...
        docs.append(
            {
                ...
                "tiers": {
                    "L3": {
                        "timing": {"sim_active_s": 50.0 + 0.23 * cyc},
                        "cycles": cyc,
                        "cycle_accurate": True,
                        "derived_from_rtl": True,
        ...
        docs.append(
            {
                ...
                "tiers": {
                    "L3": {
                        "timing": {"sim_active_s": 15.0 + 0.0035 * cyc},
                        "cycles": cyc,
                        "cycle_accurate": True,
                        "derived_from_rtl": True,
                        "engine": "gsim",
