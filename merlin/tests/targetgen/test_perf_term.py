...
def _validity() -> Validity:
    return Validity(
        validated_regime="the one run this value came from",
        expected_error="exact",
        weak_regime="anything else",
        escalate_when="a different shape",
    )


def _term(value, **kw) -> PerformanceTerm:
    return PerformanceTerm(name="t", value=value, unit="cycles", provenance=_prov(), validity=_validity(), **kw)
...
def test_unknown_term_value_is_never_zero_in_arithmetic():
    t = PerformanceTerm.unknown(
        "overlap_cycles",
        "cycles",
        _prov("structural_bound"),
        _validity(),
