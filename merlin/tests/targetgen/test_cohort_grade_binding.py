...
def test_grade_rejects_cohort_materialized_from_another_descriptor(monkeypatch, tmp_path):
    ...
    (capsules / ".cohort_admission.json").write_text(json.dumps(record), encoding="utf-8")
    ...
    monkeypatch.setattr(CG, "load_package", lambda *_args, **_kwargs: SimpleNamespace(integrity_exempt=False))
    ...
    monkeypatch.setattr(
        ...
        lambda path: SimpleNamespace(path=path, target="gemmini", descriptor_sha256="b" * 64),
    )

    with pytest.raises(ValueError, match="does not match the target descriptor used for grade"):
        CG.grade(
            tmp_path / "package", capsules_root=capsules, runs_root=tmp_path / "runs", target="gemmini", max_workers=1
        )
