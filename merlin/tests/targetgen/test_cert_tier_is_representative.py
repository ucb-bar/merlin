            ...
            yaml.safe_dump(
                {
                    ...
                    "inputs": [
                        {"name": f"A{i}", "role": "input", "shape": [4, 4], "dtype": dt} for i, dt in enumerate(dts)
                    ],
                }
            ),
            encoding="utf-8",
...
def test_a_redundant_corpus_collapses_to_one_per_cell(tmp_path):
    ...
    root = _corpus(
        tmp_path, [("A", "contraction", ["f32"]), ("B", "contraction", ["f32"]), ("C", "contraction", ["f32"])]
    )
...
def test_every_cell_is_covered_and_none_is_dropped_silently(tmp_path):
    ...
    assert r["uncovered"] == []
    assert set(r["cells"]) == {"contraction/f32", "contraction/mxfp4", "normalization/f32", "attention/bf16"}
...
def test_both_compute_units_survive_the_cover(tmp_path):
    ...
    root = _corpus(
        ...
        [
            ("F1", "contraction", ["f32"]),
            ("F2", "contraction", ["fp16"]),
            ("M1", "contraction", ["mxfp8"]),
            ("M2", "contraction", ["mxfp4"]),
        ],
    )
...
def test_a_capsule_with_no_declared_dtype_is_skipped_not_guessed(tmp_path):
    ...
    (root / "X" / "capsule.yaml").write_text(
        yaml.safe_dump({"name": "X", "label": "public", "semantic": {"semantic_family": "contraction"}, "inputs": []}),
        encoding="utf-8",
    )
...
def test_non_public_capsules_are_not_in_the_public_cover(tmp_path):
    ...
    (root / "H").mkdir()
    (root / "H" / "capsule.yaml").write_text(
        yaml.safe_dump(
            {
                "name": "H",
                "label": "hidden",
                "semantic": {"semantic_family": "reduction"},
                "inputs": [{"name": "A0", "shape": [4], "dtype": "f32"}],
            }
        ),
        encoding="utf-8",
    )
...
def test_the_shipped_corpus_covers_every_cell():
    ...
    roots = [
        repo_root() / "merlin/contract/capsules/radiance/isa",
...
def _ladder(tiers_recorded):
    ...
    return next((t for t, r in tiers_recorded.items() if r.get("mandatory") and r.get("status") == "fail"), None)
...
def test_a_partial_tile_is_its_own_cell(tmp_path):
    ...
    root = _corpus_shaped(
        tmp_path, [("Aligned", "contraction", "f32", [32, 32]), ("Ragged", "contraction", "f32", [32, 33])]
    )
