...
def _patch(monkeypatch, *, manifest=None, manifest_raises=False, taxonomy=None, taxonomy_raises=False):
...
def test_declared_semantic_class_wins_and_derivation_never_runs(monkeypatch):
    ...
    _patch(
        monkeypatch,
        manifest=_FakeManifest(encoding={"semantic_class": {"0": "COMPUTE_PRELOADED", "1": "MVIN"}}),
...
def test_a_target_declaring_nothing_falls_back_to_its_own_isa_definition(monkeypatch):
    ...
    _patch(monkeypatch, manifest=_FakeManifest(), taxonomy={"by_class": {"Alpha": [{}], "Beta": [{}]}})
