...
def _mesh(monkeypatch, *, ran, fell, status="verified", gate=True, verify=None, unrouted=0):
    ...
    out = {
        "status": status,
        "verify": verify if verify is not None else {"gate_ok": gate},
        "mesh_execution": {
            "target": "radiance",
...
def test_a_host_mismatch_is_also_withheld_not_blamed_on_the_submission(monkeypatch):
    ...
    monkeypatch.setattr(cc, "compile_rvv", lambda *a, **k: {"status": "run_mismatch", "verify": {"gate_ok": False}})
...
def test_quantized_drop_within_tolerance_still_passes_when_it_ran_on_the_mesh(monkeypatch):
    ...
    monkeypatch.setattr(
        TargetExperiment,
        "resolve_host_lane",
        lambda self, *, root=None, dtype=None: (
            Path("/frozen/int8"),
            {"package_sha256": "a" * 64, "dtype_strategy": "int8_w8a8"},
        ),
    )
    _mesh(monkeypatch, ran=15, fell=0, status="run_mismatch", verify={"gate_ok": False, "fp32_cos": 0.962})
...
def test_quantized_gross_defect_still_fails(monkeypatch):
    ...
    _mesh(monkeypatch, ran=15, fell=0, status="run_mismatch", verify={"gate_ok": False, "fp32_cos": 0.42})
    r = R._grade_model_capsule(_CAP_INT8, target="radiance", timeout=60)
    assert r["status"] == "fail" and r["failure"]["category"] == "FUNCTIONAL_MISMATCH"
