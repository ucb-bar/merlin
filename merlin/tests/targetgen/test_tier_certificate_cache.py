...
def test_every_declared_grading_module_exists():
    ...
    missing = [name for name in TC._GRADING_MODULES if not module_source_path(name).is_file()]
    assert not missing, f"grading-path members have moved: {missing}"
    assert "merlin.targetgen.capsule_runner" in TC._GRADING_MODULES, (
        "the ladder decides what a tier verdict means; its bytes must be in the instrument"
    )
    assert "merlin.targetgen.tier_cache" in TC._GRADING_MODULES, (
        "this module decides what a HIT means; an edit to it must not be carried across"
    )
    assert "merlin.targetgen.oracle_policy" in TC._GRADING_MODULES, (
        "extracted engine selection must remain part of the judge, not disappear during relocation"
    )
    assert "merlin.targetgen.program_values" in TC._GRADING_MODULES, (
        "extracted output decoding still decides the numeric verdict"
    )
    assert "merlin.targetgen.program_engine_policy" in TC._GRADING_MODULES, (
        "extracted engine admission and provenance still decide which instrument produced a verdict"
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "merlin.targetgen.program_engine_policy",
        "merlin.integrations.modelir",
        "merlin.targetgen.rtl.mlc_bridge",
        "merlin.targetgen.runner_config",
        "merlin.targetgen.capsule_golden",
        "merlin.targetgen.golden_provenance",
        "merlin.targetgen.oracle_helpers.program_cosim",
        "merlin.targetgen.oracle_helpers.functional_errata_runner",
    ],
)
def test_extracted_policy_and_worker_bytes_remain_in_instrument_identity(tmp_path, monkeypatch, module):
    from merlin.common import paths

    sources = {}
    for name in TC._GRADING_MODULES:
        source = tmp_path / (name + ".py")
        source.write_text("# synthetic instrument source\n")
        sources[name] = source
    monkeypatch.setattr(paths, "module_source_path", sources.__getitem__)
    monkeypatch.setattr(TC, "_INSTRUMENT_MEMO", {})
    before = TC.instrument_digest(None, "L2", rtl_tier=False)
    cache = tmp_path / "certificates"
    assert TC.record("synthetic", "L2", DIGEST_A, before, status="pass", tier_result={}, root=cache)
    assert TC.lookup("synthetic", "L2", DIGEST_A, before, root=cache, ledgers=()) is not None
    policy = sources[module]
    policy.write_text("# changed engine admission policy\n")
    TC._INSTRUMENT_MEMO.clear()
    after = TC.instrument_digest(None, "L2", rtl_tier=False)
    assert before and after and before != after
    assert TC.lookup("synthetic", "L2", DIGEST_A, after, root=cache, ledgers=()) is None
    policy.unlink()
    TC._INSTRUMENT_MEMO.clear()
    assert TC.instrument_digest(None, "L2", rtl_tier=False) is None


@pytest.mark.parametrize(
    ("report", "expected"),
    [
        ({"available": True, "engine": " selected-engine "}, "selected-engine"),
        ({"available": False, "engine": "ignored-engine"}, None),
        ({"available": True}, None),
        (None, None),
    ],
)
def test_engine_identity_keeps_loaded_legacy_report_overrides(monkeypatch, report, expected):
    calls = []

    def describe(target, sim_via=None):
        calls.append(target)
        return report

    monkeypatch.setattr(CR, "describe_l3_engine", describe)
    assert TC._engine_token("synthetic", "L3", True) == expected
    assert calls == ["synthetic"]
    assert TC._engine_token("synthetic", "L2", False) == ""
    assert TC._engine_token(None, "L3", True) is None
    assert calls == ["synthetic"]
...
def test_the_ledger_key_has_one_spelling():
    ...
    src = (feedback_source("tier_promote")).read_text()
...
def _ladder(monkeypatch, tmp_path):
    ...
    schemas.validate(cb, "command_buffer", contract="merlin/contract")
    # The cache assertions exercise the actual ladder and adapter invocations, not numeric
    # engines or the presence of independently stored float goldens in this checkout.
    outputs = {"Y0": [0] * (32 * 32)}
    monkeypatch.setattr(CR.CG, "golden", lambda *_args, **_kwargs: outputs)
    monkeypatch.setattr(reference, "reference_outputs", lambda *_args, **_kwargs: outputs)
    monkeypatch.setattr(simulator, "simulate", lambda *_args, **_kwargs: {"outputs": outputs})
