...
import pytest
import yaml
from merlin_experiments.phase0 import generation as GENERATION
from merlin_experiments.phase0 import profiles as PROFILES
from merlin_experiments.phase0 import provenance as PROVENANCE
...
from merlin.targetgen import corpus_spec as SPEC
...
def test_mx_recipe_authors_a_future_acceleration_witness_without_rewriting_legacy_capsules():
    ...
    semantic = SPEC._semantic_block(witness, SimpleNamespace(semantic_defaults={}))
    assert semantic["semantic_family"] == "contraction" and semantic["must_accelerate"] is True
...
def _performance_block(*, family="PG", traits=None, emitter_status="existing"):
    ...
    return {
        ...
        "comparand": {"kind": "paired", "against": "control", "cancels": ["shape"], "demand_equal": ["M", "K"]},
        "falsifier": {"observation": "cycles", "fires_when": "delta_is_zero", "negative_control": "control"},
        "gate": {
            "traits": list(traits or ["explicit_dma"]),
            "instrument": "cycles",
            "capacity": "two_points",
            "on_missing": "skip_with_evidence",
        },
        "regime": {"separation": "fixed", "layout": "identical"},
        "emitter": {"status": emitter_status, "entry": "merlin.targetgen.corpus_spec.build", "knobs": {}},
        "cost": {"tier": 1, "runs": 2, "projected_cycles": "preflight", "basis": "two_members"},
    }
...
def test_extent_tokens_resolve_against_the_tile_edge(token, tile, expected):
    assert SWEEPS.resolve_extent(token, tile) == expected
...
def test_the_same_sweep_tracks_a_different_tile_edge():
    ...
    assert [SWEEPS.resolve_extent(t, 16) for t in tokens] == [15, 16, 17]
    assert [SWEEPS.resolve_extent(t, 64) for t in tokens] == [63, 64, 65]
...
def test_a_sweep_expands_to_the_cross_product_of_its_axes():
    entries = SWEEPS.expand_sweeps(_sweep_profile(), _binding(16))
...
def test_hand_written_entries_are_kept_verbatim_and_come_first():
    ...
    hand = {
        ...
        "N": 1,
        "K": 3,
        "source_reference": "worth writing by hand",
    }
    profile["capsules"] = [hand]

    entries = SWEEPS.expand_sweeps(profile, _binding(16))
...
def test_a_profile_without_sweeps_is_returned_untouched():
    ...
    assert SWEEPS.expand_sweeps({"capsules": hand}, _binding(16)) == hand
...
def test_a_single_K_point_is_refused_because_it_cannot_price_a_tiled_unit():
    ...
    with pytest.raises(ValueError, match="TWO distinct K points"):
        SWEEPS.expand_sweeps(profile, _binding(16))
...
def test_every_axis_declared_as_fitted_needs_two_distinct_points():
    profile = _sweep_profile(axes={"M": ["tile"], "N": ["tile", "2*tile"], "K": [64, 256]}, fit_axes=["M", "N"])
...
def test_a_sweep_without_an_id_or_axes_raises():
    with pytest.raises(ValueError, match="id"):
        SWEEPS.expand_sweeps({"sweeps": [{"axes": {"M": ["tile"]}}]}, _binding(16))
    with pytest.raises(ValueError, match="no axes"):
        SWEEPS.expand_sweeps({"sweeps": [{"id": "X"}]}, _binding(16))


def test_expansion_needs_a_tile_edge():
    with pytest.raises(ValueError, match="tile edge"):
        SWEEPS.expand_sweeps(_sweep_profile(), SimpleNamespace(tile_dim=0))
...
def test_a_sweep_reproduces_the_tile_edge_family_of_the_existing_corpus():
    ...
    """
    profile = _sweep_profile(axes={"M": ["tile-1", "tile", "tile+1"], "N": ["tile"], "K": [64, 256]})
    entries = SWEEPS.expand_sweeps(profile, _binding(16))
    ...
    entries64 = SWEEPS.expand_sweeps(profile, _binding(64))
...
def test_variants_cross_with_the_extents():
    entries = SWEEPS.expand_sweeps(_variant_profile(), _binding(16))
...
def test_a_variant_field_with_no_placeholder_is_copied_through_untouched():
    ...
    entries = SWEEPS.expand_sweeps(prof, _binding(16))
...
def test_a_malformed_variants_block_raises_rather_than_being_ignored():
    with pytest.raises(ValueError):
        SWEEPS.expand_sweeps(_variant_profile(variants="fused_matmul_bias"), _binding(16))
    with pytest.raises(ValueError):
        SWEEPS.expand_sweeps(_variant_profile(variants=["fused_matmul_bias"]), _binding(16))
...
def test_the_declared_blocks_are_carried_onto_the_capsule():
    entry = {"performance": {"level": "L3_inter_layer"}, "comparison_group": {"name": "g", "role": "fused"}}
    cap: dict = {"name": "X"}
    assert WRITER._carry_declared_blocks(entry, cap) is True
...
def test_a_block_already_on_the_capsule_is_never_overwritten():
    ...
    assert WRITER._carry_declared_blocks({"comparison_group": {"name": "g"}}, cap) is False
    assert cap["comparison_group"] == "authored_by_hand"


def test_a_block_the_entry_does_not_declare_is_never_invented():
    cap: dict = {"name": "X"}
    assert WRITER._carry_declared_blocks({}, cap) is False
        ...
        "sweeps": [
            {
                ...
                "base": {
                    ...
                    "kind": "isa",
                    "op": "matmul",
                    "performance": _performance_block(traits=traits, emitter_status=emitter_status),
                },
...
def test_a_met_gate_generates_the_family() -> None:
    ...
    entries = SWEEPS.expand_sweeps(
        _gated_profile(["explicit_dma"]), _binding(16), trait_facts=_trait_facts(explicit_dma=True), skipped=skips
    )
...
def test_a_target_lacking_the_trait_generates_nothing_and_says_so() -> None:
    skips = []
    entries = SWEEPS.expand_sweeps(
        _gated_profile(["explicit_dma"]), _binding(16), trait_facts=_trait_facts(explicit_dma=False), skipped=skips
    )
    assert entries == []
    ...
    assert skips[0]["gate"]["facts"]["explicit_dma"] == {
        "satisfied": False,
        "tier": "test_tier",
        "evidence": "test evidence for explicit_dma=False",
        "missing": [],
    }
...
def test_an_undeterminable_trait_is_not_the_same_as_an_absent_one() -> None:
    ...
    skips = []
    assert (
        SWEEPS.expand_sweeps(
            _gated_profile(["explicit_dma"]), _binding(16), trait_facts=_trait_facts(explicit_dma=None), skipped=skips
        )
        == []
    )
    ...
    skips2 = []
    SWEEPS.expand_sweeps(_gated_profile(["explicit_dma"]), _binding(16), trait_facts={"traits": {}}, skipped=skips2)
...
def test_gate_requires_and_unknown_trait_vocabularies_are_refused() -> None:
    ...
    block["gate"] = {
        "requires": ["explicit_dma"],
        "instrument": "cycles",
        "capacity": "two",
        "on_missing": "skip_with_evidence",
    }
    with pytest.raises(ValueError, match="requires is not accepted"):
        PROFILES._validate_performance_block(block, owner="fixture")
...
def test_execution_capability_is_a_separate_evidence_bearing_gate() -> None:
    ...
    facts["execution_capabilities"] = {
        "whole_program_kernel_abi": {
            ...
            "evidence": "fixture backend declares the generic whole-program ABI",
            "missing": [],
        }
    }

    skips = []
    entries = SWEEPS.expand_sweeps(prof, _binding(), trait_facts=facts, skipped=skips)
    assert len(entries) == 2 and skips == []
    ok, decision = SWEEPS.evaluate_gate(gate, facts)
    assert ok is True
    ...
    skipped = []
    assert SWEEPS.expand_sweeps(prof, _binding(), trait_facts=facts, skipped=skipped) == []
    assert skipped[0]["gate"]["outcome"] == "refuted"
    assert skipped[0]["gate"]["refuted_execution_capabilities"] == ["whole_program_kernel_abi"]
...
def test_a_true_gate_with_an_unimplemented_emitter_is_blocked_not_generated() -> None:
    blocked = []
    entries = SWEEPS.expand_sweeps(
        _gated_profile(["explicit_dma"], emitter_status="new:test_emitter"),
        _binding(),
    ...
    assert blocked == [
        {
            ...
            "emitter": {"status": "new:test_emitter", "entry": "merlin.targetgen.corpus_spec.build", "knobs": {}},
            "fit_axes": ["K"],
            "comparison_roles": [],
        }
    ]
...
def test_gating_is_optional_so_existing_profiles_are_untouched() -> None:
    assert len(SWEEPS.expand_sweeps(_sweep_profile(), _binding(16))) > 0
...
def test_a_comparison_group_of_one_is_refused() -> None:
    ...
    prof = {
        "sweeps": [
            {
                "id": "PF",
                "base": {"cat": "isa", "kind": "isa"},
                "axes": {"M": ["tile"], "K": ["tile", "2*tile"]},
                "name": "PF{i:02d}",
                "variants": [{"op": "fused_matmul_bias", "comparison_group": {"name": "fmb_{M}x{K}", "role": "fused"}}],
            }
        ]
    }
    with pytest.raises(ValueError, match="single member"):
        SWEEPS.expand_sweeps(prof, _binding(16))
        ...
        "sweeps": [
            {
                ...
                "name": "PF{i:02d}",
                "variants": [
                    {"op": "fused_matmul_bias", "comparison_group": {"name": "fmb_{M}x{K}", "role": "fused"}},
                    {"op": "matmul", "comparison_group": {"name": "fmb_{M}x{K}", "role": "part"}},
                    {"op": "bias_add", "comparison_group": {"name": "fmb_{M}x{K}", "role": "part"}},
                ],
            }
        ]
    }
...
def test_performance_facts_are_the_canonical_derived_profile_with_a_digest() -> None:
    ...
    assert set(facts["traits"]) == set(SWEEPS.TRAITS)
...
def test_pb_materializes_from_generic_execution_capabilities_and_skips_without_them() -> None:
    ...
    assert all(entry["performance"]["emitter"]["resolved"]["source"] == "direct" for entry in entries)
    ...
    skipped: list[dict] = []
    assert SWEEPS.expand_sweeps({"sweeps": [sweep]}, binding, trait_facts=denied, skipped=skipped) == []
    assert skipped[0]["gate"]["refuted_execution_capabilities"] == ["whole_program_kernel_abi"]
...
def _minimal_shared_perf():
    return {
        "sweeps": [
            {
                "id": "PX",
                "base": {"cat": "_perf", "kind": "isa", "op": "matmul", "performance": _performance_block(family="PX")},
                "fit_axes": ["K"],
                "axes": {"M": ["tile"], "K": ["tile", "2*tile"]},
            }
        ],
...
def test_load_profile_merges_one_shared_perf_template_for_every_target(tmp_path, monkeypatch):
    (tmp_path / "_perf.yaml").write_text(yaml.safe_dump(_minimal_shared_perf()), encoding="utf-8")
    functional = {"id": "FX", "base": {"cat": "isa"}, "axes": {"M": ["tile"], "K": ["tile", "2*tile"]}}
    for target in ("alpha", "beta"):
        (tmp_path / f"{target}.yaml").write_text(
            yaml.safe_dump({"capsules": [], "sweeps": [functional]}), encoding="utf-8"
        )
    alpha = PROFILES.load_profile("alpha", include_holdouts=False, profiles_root=tmp_path)
    beta = PROFILES.load_profile("beta", include_holdouts=False, profiles_root=tmp_path)
    assert [s["id"] for s in alpha["sweeps"]] == ["FX", "PX"]
    assert [s["id"] for s in beta["sweeps"]] == ["FX", "PX"]
    assert PROFILES.profile_targets(profiles_root=tmp_path) == ["alpha", "beta"]
...
def test_load_profile_refuses_every_target_local_perf_template(tmp_path, monkeypatch, local):
    ...
    with pytest.raises(ValueError, match="target-local performance template"):
        PROFILES.load_profile("target", include_holdouts=False, profiles_root=tmp_path)
...
def test_shipped_targets_all_consume_the_same_claim_separated_perf_template():
    ...
    assert all("source" not in s["base"] and "operand_dtype" not in s["base"] for s in shared["sweeps"])
...
@pytest.mark.parametrize("missing", sorted(PROFILES._PERFORMANCE_FIELDS))
...
def test_gemmini_admits_the_runnable_families_and_records_the_refuted_one():
    profile = PROFILES.load_profile("gemmini", include_holdouts=False, **for_target("gemmini").profile_inputs())
    ...
    entries = SWEEPS.expand_sweeps(
        ...
        binding,
        trait_facts=SWEEPS._performance_facts("gemmini"),
        skipped=skips,
        blocked_unimplemented=blocked,
        errors=errors,
    )
    ...
    assert {entry["operand_dtype"] for entry in entries} == {"int8"}
    assert {entry["performance"]["emitter"]["resolved"]["accum_dtype"] for entry in entries} == {"i32"}
    assert {entry["source"] for entry in entries} == {"direct"}
    assert {entry["cat"] for entry in entries} == {"_perf"}
    assert {entry["label"] for entry in entries} == {"dev"}
    assert {entry["cat"] for entry in entries}.isdisjoint({"isa", "layers", "model", "model_slices", "hidden"})
    ...
    template_blocked = {row["family"] for row in profile["_performance_template"]["blocked_unimplemented"]}
    ...
    capsule, mlir = GENERATION.CS.build(entries[0], binding)
...
def test_generation_persists_complete_performance_record(tmp_path):
    ...
    path = PROVENANCE.update_provenance_manifest([], cap_root=tmp_path, target="fixture", performance_record=record)
...
def test_generate_target_passes_gate_skips_to_provenance(tmp_path, monkeypatch):
    ...
    profile["_performance_template"] = {
        "path": "profiles/_perf.yaml",
        "sha256": "a" * 64,
        "families": [{"family": "PG", "claim": "DIFFERENTIAL", "fit_axes": ["K"], "comparison_roles": []}],
        "blocked_unimplemented": [],
    }
    binding = _binding(16)
    ...
    monkeypatch.setattr(GENERATION, "load_profile", lambda _target, **kwargs: profile)
    monkeypatch.setattr(GENERATION.CS, "derive_binding", lambda _te, _datapath: binding)
    monkeypatch.setattr(
        GENERATION,
        "_performance_facts",
...
def test_generate_target_persists_family_and_member_counts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(GENERATION, "_write_capsule", write)
    monkeypatch.setattr(GENERATION, "_scrub_capsule_dir", lambda _path: None)
