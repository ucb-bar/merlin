...
``contract.matmul_interface.emit_interface_mlir`` seam so either a dropped attribute or an unpooled result
...
def _entry(**over) -> dict:
    entry = {
        ...
        "source_reference": "pool emission test",
    }
    entry.update(over)
    return entry
...
def test_matmul_builder_emits_ragged_pool_extent_without_folding_the_tail():
    ...
    _, mlir = CS.build_matmul(_entry(M=25, N=17, pool_in_dims=[5, 5]), _binding())
