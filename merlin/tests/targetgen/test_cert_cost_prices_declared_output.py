...
_CONV = (
    ...
    '  %W_res = merlin_iface.resident_pack %W {layout = "packed_conv_rhs"} : '
    "(tensor<1024x16xi8>) -> !merlin_iface.resident\n"
    "  %Y0 = merlin_iface.conv2d %IFM, %W_res {kernel = [16, 16, 4, 16], stride = [16, 16], "
    'padding = [0, 0, 0, 0], dilation = [1, 1], name = "Y0", epilogue = [], output_dtype = "i32", '
    'layout = "nhwc"} : (tensor<1x64x64x4xi8>, !merlin_iface.resident) -> tensor<16x16xi32>\n'
...
def test_a_capsule_is_not_advised_a_cap_its_target_cannot_take():
    ...
    for d in sorted(descs.iterdir()):
        if not (d / "target_experiment.yaml").is_file():
            continue
        tiers = [str(t) for t in (constructed_oracle_tiers(d.name) or ())]
