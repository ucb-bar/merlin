...
def _entry(name: str, opcode: int, fields: dict) -> dict:
    ...
    for bits in fields.values():
        for b in bits:
            variable |= 1 << b
    return {"class": name, "fields": fields, "fixed_value": opcode, "fixed_mask": (~variable) & 0xFFFFFFFF}
...
def run_probe(kernel_s: str) -> dict:
    ...
    """
    words = [int(line.split(".word")[1].split()[0], 16) for line in kernel_s.splitlines() if ".word" in line]
    x = [0] * _SCALAR_SLOTS
    mrf = [0] * _TENSOR_SLOTS
    acc = [0] * _ACC_SLOTS
...
def test_an_instruction_with_no_observable_effect_is_unknown_not_empty(model):
    ...
    assert all("nothing about it is established" in v.reason or "no dependence" in v.reason for v in verdicts.values())
...
def test_a_zero_settle_is_refused_because_it_makes_every_write_invisible():
    with pytest.raises(ID.DirectionError):
        ID.ProbeOps(scalar_imm="ADDI", scalar_upper="UPPER", stall="WAIT", halt="STOP", settle=0).validate(FIXTURE)


def test_an_undefined_probe_op_is_refused_before_anything_is_emitted():
    with pytest.raises(ID.DirectionError):
        ID.ProbeOps(scalar_imm="NOPE", scalar_upper="UPPER", stall="WAIT", halt="STOP").validate(FIXTURE)
...
def test_merging_two_preambles_keeps_the_stronger_evidence():
    ...
    a = ID.DirectionModel(
        ...
        by_mnemonic={
            "OP": {
                "x": ID.OperandDirection("OP", "x", ID.UNKNOWN, None, reason="not seen"),
    ...
    b = ID.DirectionModel(
        ...
        by_mnemonic={
            "OP": {
                "x": ID.OperandDirection("OP", "x", ID.USE, "f", reason="seen"),
