...
def _cb_resident(m=_EDGE, k=_EDGE, n=_EDGE, *, epilogue=None, out_dtype="i32", acc_scale=None):
    ...
    return {
        ...
        "commands": [
            ...
            {"opcode": "MATMUL_RESIDENT", "operands": {"lhs": "A0", "rhs": "W_res", "dst": "acc_Y0"}},
            {"opcode": "COMMIT", "operands": {"src": "acc_Y0", "dst": "Y0"}, "attributes": attrs},
...
def _cb_whole_op(m=_EDGE, n=_EDGE, d=_EDGE):
    ...
    return {
        ...
        "target": "gemmini",
        "tensors": {
            "Q": {"shape": [m, d], "dtype": "i8", "role": "input"},
...
def _trace(*, loads=None, stores=None, ld_pitch=_EDGE, st_pitch=_EDGE * 4, relu=False, acc_scale=1.0, readout="i32"):
    """A well-formed stream. ``loads`` / ``stores`` are ``(arg_index, byte_offset, rows, cols)``."""
    ins = [
        {"index": 0, "class": "FENCE", "funct": None, "decoded": {}},
        {"index": 1, "class": "CONFIG_EX", "funct": 0, "decoded": {"subtype": "EX"}},
        {
            "index": 2,
            "class": "CONFIG_ST",
            "funct": 0,
            "decoded": {"subtype": "ST", "out_stride_bytes": st_pitch, "relu": relu, "acc_scale": acc_scale},
        },
        {"index": 3, "class": "CONFIG_LD", "funct": 0, "decoded": {"subtype": "LD", "stride": ld_pitch}},
    ]
    for load in loads if loads is not None else [(0, 0, _EDGE, _EDGE), (1, 0, _EDGE, _EDGE)]:
        arg, off, rows, cols = load[:4]
        pitch = load[4] if len(load) > 4 else ld_pitch  # a real kernel configures each operand's own
        ins.append({"index": len(ins), "class": "CONFIG_LD", "funct": 0, "decoded": {"subtype": "LD", "stride": pitch}})
        ins.append(
            {
                "index": len(ins),
                "class": "MVIN",
                "funct": 2,
                "decoded": {"rows": rows, "cols": cols, "dram": {"kind": "argbase", "arg_index": arg, "offset": off}},
            }
        )
...
def _screen(trace, capsule, cb):
    return RC.screen(trace, capsule, _FACTS, target="gemmini", command_buffer=cb)
...
def test_a_movement_buffer_resolves_to_its_own_two_pointer_order():
    cb = {
        ...
        "target": "gemmini",
        "tensors": {
            "X": {"shape": [_EDGE, _EDGE], "dtype": "i8", "role": "input"},
            "Y0": {"shape": [_EDGE, _EDGE], "dtype": "i8", "role": "output"},
        },
        "commands": [
            {"opcode": "MOVEMENT", "operands": {"src": "X", "dst": "Y0"}, "attributes": {"output_dtype": "i8"}}
        ],
    }
...
def _permuted_trace(k, n, m):
    ...
    """
    return _trace(loads=[(0, 0, m, k, k), (1, 0, k, n, n)], stores=[(2, 0, m, n)], st_pitch=n * 4)
...
def test_the_conformant_direction_passes_on_the_contract_order():
    ...
    k, n, m = _EDGE, _EDGE * 2, _EDGE
    c = _check(
        _trace(loads=[(0, 0, k, n, n), (1, 0, m, k, k)], stores=[(2, 0, m, n)], st_pitch=n * 4),
        _capsule(m, k, n),
        _cb_resident(m, k, n),
    )
...
def test_a_ragged_extent_moved_through_its_tile_padded_buffer_is_not_a_finding():
    ...
    m = k = n = _EDGE - 1
    c = _check(
        _trace(loads=[(0, 0, k, n), (1, 0, m, k)], stores=[(2, 0, m, n)], ld_pitch=_EDGE, st_pitch=_EDGE * 4),
        _capsule(m, k, n),
        _cb_resident(m, k, n),
    )
    assert c.status == "pass", c.message
...
def test_a_store_activation_the_declared_epilogue_does_not_name_is_detected():
    ...
    assert not [
        f
        for f in _check(_trace(relu=True), _capsule(), _cb_resident(epilogue=["relu"])).evidence["findings"]
...
def test_an_accumulator_scale_other_than_the_declared_one_is_detected():
    ...
    assert not [
        f
        for f in _check(
            _trace(acc_scale=0.5), _capsule(), _cb_resident(epilogue=["acc_scale"], acc_scale=0.5)
        ).evidence["findings"]
        if f["field"] == "config_scale"
    ]
...
def test_a_pitch_matching_neither_row_convention_is_reported_only_when_nothing_else_explains_it():
    ...
    c = _check(
        _trace(loads=[(0, 0, k, n, short), (1, 0, m, k, k)], stores=[(2, 0, m, n)], st_pitch=_EDGE * 4),
        _capsule(m, k, n),
        _cb_resident(m, k, n),
    )
...
def test_either_pitch_convention_is_accepted():
    ...
    for pitch in (n, _EDGE):
        c = _check(
            _trace(loads=[(0, 0, k, n, pitch), (1, 0, m, k, k)], stores=[(2, 0, m, n)], st_pitch=_EDGE * 4),
            _capsule(m, k, n),
            _cb_resident(m, k, n),
        )
...
def test_without_a_command_buffer_the_check_is_skipped_not_passed():
    ...
    c = next(x for x in RC.screen(_trace(), _capsule(), _FACTS, target="gemmini").checks if x.id == _CID)
...
def test_without_a_derived_mesh_the_check_is_skipped_not_passed():
    c = next(
        x
        for x in RC.screen(
            _trace(), _capsule(), dict(_FACTS, mesh=None), target="gemmini", command_buffer=_cb_resident()
        ).checks
        if x.id == _CID
    )
...
def test_a_buffer_matching_two_contract_shapes_is_unknown_not_screened():
    ...
    cb["commands"].append(
        {"opcode": "MOVEMENT", "operands": {"src": "Xm", "dst": "Y0"}, "attributes": {"output_dtype": "i8"}}
    )
    names, _shape, why = RC.resolve_kernel_arg_order(cb)
    assert names == []
...
def test_a_multi_row_move_with_no_configured_pitch_is_undecidable_not_a_finding():
    ...
    assert [
        u
        for u in c.evidence["fields_not_derivable"]
        if u["field"] == "dram_base_pointer" and "no DRAM row pitch" in u["reason"]
    ]
    assert not [
        f
        for f in c.evidence["findings"]
...
def test_two_declared_commits_sharing_one_store_configuration_compare_neither_field():
    ...
    assert {"store_activation", "config_scale"} <= {u["field"] for u in c.evidence["fields_not_derivable"]}
    assert not [f for f in c.evidence["findings"] if f["field"] in ("store_activation", "config_scale")]
...
def test_a_dram_operand_that_is_not_a_kernel_argument_is_undecidable():
    ...
    c = _check(tr, _capsule(), _cb_resident())
    assert [u for u in c.evidence["fields_not_derivable"] if "not a kernel argument" in u["reason"]]
