...
def test_materialize_installs_the_package_and_records_where_it_came_from(tmp_path):
    ...
    assert man["package_id"] == "radiance-xdsl-oot"
    assert (dst / "manifest.yaml").read_bytes() == (src / "manifest.yaml").read_bytes()
    man = PB.package_records.read_record(dst)
    assert man["package_slot"] == "agent_spec_v1_mlir_oot"
    ...
    assert man["payload"]["sha256"], "a compiler install records exact copied bytes"
...
def test_materialize_does_not_claim_a_certification_rank_it_lacks(tmp_path):
    ...
    assert PB.package_records.read_record(dst)["publication"]["champion"] is False
