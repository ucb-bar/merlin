...
def _conv_entry(**over) -> dict:
    entry = {
        ...
        "source_reference": "conv golden test",
        "op": "conv2d",
        "out": "Y0",
        "ifm": "IFM",
        "weight": "W",
...
def test_float_engine_materializes_a_conv2d_golden():
    ...
    _specir_or_skip()
    outputs, prov = NUMERICS._float_golden(_conv_entry(), _float_binding())
    y = outputs["Y0"]
...
def test_pointwise_conv_is_the_matmul_golden_on_the_same_operands():
    ...
    conv = _conv_entry(
        kh=1, kw=1, ci=TILE, N=TILE, Himg=4, Wimg=4, stride=[1, 1], padding=[0, 0, 0, 0], dilation=[1, 1]
    )
    matmul = {**_conv_entry(), "op": "matmul", "lhs": "IFM", "weight": "W", "M": 4 * 4, "K": TILE, "N": TILE}
    conv_out, _ = NUMERICS._float_golden(conv, binding)
    mm_out, _ = NUMERICS._float_golden(matmul, binding)
...
def test_padding_changes_the_extent_and_the_values():
    ...
    _specir_or_skip()
    binding = _float_binding()
    unpadded, _ = NUMERICS._float_golden(_conv_entry(), binding)
    padded, _ = NUMERICS._float_golden(_conv_entry(padding=[1, 1, 1, 1]), binding)
...
def test_declared_conv_perf_family_generates_on_a_float_binding():
    ...
    shared = yaml.safe_load((repo_root() / "experiments/templates/phase0/performance.yaml").read_text(encoding="utf-8"))
    convs = [s for s in (shared.get("sweeps") or []) if (s.get("base") or {}).get("op") == "conv2d"]
    ...
    for sweep in convs:
        ...
        for point in points:
            entry = {
                **{k: v for k, v in base.items() if k != "performance"},
                "name": f"{sweep['id']}_{axis}{point}",
                "source_reference": "template",
                axis: SWEEPS.resolve_extent(point, binding.tile_dim),
            }
            outputs, _ = NUMERICS._float_golden(entry, binding)
