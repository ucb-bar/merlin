...
# --- the engine is a MACHINE asset, not a per-run artifact ---------------------------------------
#
# These redirect the INSTALLED home by patching `paths.tracked_build_dir`, never by moving
# MERLIN_REPO_ROOT. That root is also where `paths.env` finds `.env`, and `paths.env` CACHES it -- so
# moving it empties the external-path table for every test that runs afterwards in the same session,
# which is a failure in a LATER file and nearly impossible to attribute back to here.


@pytest.fixture()
def installed_home(monkeypatch, tmp_path):
    """Point the repo-anchored engine home at a tmp dir, and hand back an installer for it."""
    from merlin.common import paths as _paths

    home = tmp_path / "installed"
    monkeypatch.setattr(_paths, "tracked_build_dir", lambda: home)

    def install(target: str, *, executable: bool = True):
        where = GE.installed_engine_home(target)
        where.mkdir(parents=True, exist_ok=True)
        emu = where / GE.BINARY_NAME
        emu.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
        if executable:
...
def test_an_installed_engine_is_found_from_a_run_root_that_has_none(monkeypatch, tmp_path, installed_home):
    """The defect this closes. The derived home follows the RUN's output root, which is right for what
    a run builds and wrong for a tool it uses: a cert whose ``project_root`` is a per-invocation temp
    directory looked for its engine under that temp directory. Measured on one host: 0 of 2295 such
    roots held an engine home, so the elaborated-RTL tier took the slow engine on every one of them and
    installing the fast one could not change it."""
    installed_home("phantomsim")
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "per_run_tmp"))  # a run root with nothing
    resolved = GE.resolve("phantomsim")
...
def test_the_run_relative_home_still_wins_when_it_has_one(monkeypatch, tmp_path, installed_home):
    """A run that builds its own engine must not be overridden by the installed one."""
    installed_home("phantomsim")
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "run"))
    _install_binary("phantomsim")  # and one in the run's own root
    resolved = GE.resolve("phantomsim")
    assert resolved.ok and resolved.source == "derived"
    assert str(resolved.path).startswith(str(tmp_path / "run"))


def test_the_two_homes_are_different_places(monkeypatch, tmp_path, installed_home):
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "elsewhere"))
    assert GE.engine_home("phantomsim") != GE.installed_engine_home("phantomsim")
    assert str(GE.installed_engine_home("phantomsim")).startswith(str(tmp_path / "installed"))


def test_absence_names_both_homes_so_the_fix_is_actionable(monkeypatch, tmp_path, installed_home):
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "elsewhere"))
    reason = GE.resolve("phantomsim").reason
    assert str(tmp_path / "installed") in reason and str(tmp_path / "elsewhere") in reason, reason


def test_an_installed_engine_still_passes_through_the_provenance_gate(monkeypatch, tmp_path, installed_home):
    """Being found in the installed home is not a reason to trust it: a receipt that describes other
    bytes must still refuse, or the fallback becomes a way around the gate."""
    installed_home("phantomsim")
    (GE.installed_engine_home("phantomsim") / GE.RECEIPT_NAME).write_text(
        json.dumps({"artifacts": {"binary": {"sha256": "0" * 64}}}), encoding="utf-8"
    )
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "per_run_tmp"))
    resolved = GE.resolve("phantomsim")
    assert not resolved.ok and resolved.refused, resolved.reason
