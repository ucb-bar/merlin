...
def test_layers_on_mesh_versus_host_fallback_reaches_the_score_file(monkeypatch):
    s = _score(
        ...
        [
            _model_result(
                mesh_execution={
                    "matmul_layers_on_mesh": 35,
                    "matmul_layers_host_fallback": 2,
                    "status": "partial",
                    "per_layer": ["dropped-detail"] * 40,
                }
            )
        ],
    )
        ...
        [
            {
                ...
                "failure": {
                    "plane": "gate",
                    "category": "GATED",
                    "detail": "whole-model capsule deferred: op pass fraction 0.67 < gate 0.8",
                },
            }
        ],
    )
...
def test_the_headline_names_capsules_that_were_never_certified(monkeypatch):
    ...
    results = [
        ...
        for i in range(6)
    ] + [{"capsule": f"s{i}", "status": "screened_only", "kind": "op", "tiers": {}} for i in range(7)]
