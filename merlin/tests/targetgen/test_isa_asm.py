...
def _branch_model() -> IsaModel:
    fields = {"rs1": [7, 8, 9, 10, 11], "rs2": [12, 13, 14, 15, 16], "imm": list(range(24, 32))}
    by = {
        "BNE": {"class": "Branch", "role": "scalar", "fixed_value": 0x63, "fixed_mask": 0x00FE007F, "fields": fields},
        "NOP": {"class": "Nullary", "role": "scalar", "fixed_value": 0x13, "fixed_mask": 0xFFFFFFFF, "fields": {}},
    }
...
def test_relative_label_uses_target_declared_pc_units():
    model = _branch_model()
    words = A.assemble_text(model, "loop: NOP\nBNE rs1=1,rs2=0,imm=loop\n", schedule_contract=_BRANCH_CONTRACT)
    assert words == [
        A.assemble_line(model, "NOP", {}),
