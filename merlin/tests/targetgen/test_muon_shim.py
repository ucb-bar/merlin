...
def test_muon_config_carries_the_simt_constants():
    ...
    assert c.perf_fields[:3] == ("flops", "gflops", "pct_fp_peak")
    assert len(c.perf_fields) == len(set(c.perf_fields))
    assert {"warp_occupancy", "tensor_util", "smem_conflict_rate"} <= set(c.perf_fields)
