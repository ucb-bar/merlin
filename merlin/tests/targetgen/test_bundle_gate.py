...
def _resnet_gate(**kw):
    base = dict(
        model="resnet50",
        datapath="w8a8",
        reference_kind="pt2e_integer",
        comparison="exact_elementwise",
        atol=2e-5,
        rtol=2e-5,
...
class TestTheReferenceMustMeanWhatTheGateClaims:
    def test_a_w8a8_datapath_cannot_be_graded_against_the_weight_only_reference(self):
        ...
        with pytest.raises(GateError) as exc:
            BG.gate_for(
                model="tiny_llama",
                datapath="w8a8",
                reference_kind="weight_only_int8",
                comparison="tolerance_and_topk",
                atol=0.03125,
                rtol=0.02,
    ...
    def test_an_fp32_datapath_graded_against_a_quantized_reference_is_refused(self):
        with pytest.raises(GateError, match="measuring the reference's quantization"):
            BG.gate_for(
                model="m",
                datapath="fp32",
                reference_kind="w8a8_independent",
                comparison="tolerance_and_topk",
                atol=1e-3,
                rtol=1e-3,
                output_elements=10,
    ...
    def test_the_execution_reference_requires_its_scope_declared(self):
        ...
        got = BG.gate_for(
            ...
            rtol=1e-4,
            output_elements=10,
            scope_note="device reproduces the host compiler only",
        )
    ...
    def test_a_missing_reference_names_its_generator_instead_of_falling_back(self):
        ...
        with pytest.raises(GateError, match="make_w8a8_independent_golden.py"):
            BG.gate_for(
                ...
                rtol=0.02,
                output_elements=256000,
                available_references={"weight_only_int8": True, "w8a8_independent": False},
            )
    ...
    def test_a_single_step_may_not_be_declared_a_trajectory(self):
        with pytest.raises(GateError, match="must not be reported as the trajectory"):
            BG.gate_for(
                model="smolvla",
                datapath="w8a8",
                reference_kind="eager_same_precision",
                comparison="trajectory",
                atol=1e-4,
                rtol=1e-4,
                output_elements=1600,
                steps=1,
                session_key="actions",
            )
...
class TestTheHarnessReadsItsOutputFromThePlan:
    ...
    def test_a_second_write_argument_is_read_at_ITS_offset_not_zero(self):
        ...
        got = BH.render_bundle_harness(self._plan(), self._gate(), entry_symbol="k", output_tensor="Y1")
    ...
    def test_an_output_that_is_not_a_write_argument_is_refused(self):
        with pytest.raises(BH.BundleHarnessError, match="correct arithmetic on the wrong bytes"):
            BH.render_bundle_harness(self._plan(), self._gate(), entry_symbol="k", output_tensor="arg0")
    ...
    def test_one_pointer_per_abi_argument_in_declaration_order(self):
        got = BH.render_bundle_harness(self._plan(), self._gate(), entry_symbol="k", output_tensor="Y0")
    ...
    def test_the_validation_is_one_expression_the_profile_harness_accepts(self):
        ...
        from merlin.perf.warm_profile_harness import TargetInvocationHooks, render_warm_then_measure_main
    ...
    def test_a_bad_entry_symbol_is_refused(self):
        for bad in ("", "not an identifier", "0k"):
            with pytest.raises(BH.BundleHarnessError, match="plain C identifier"):
                BH.render_bundle_harness(self._plan(), self._gate(), entry_symbol=bad, output_tensor="Y0")
...
class TestTheGateActuallyFails:
    ...
    def _build(self, tmp_path, mutate):
        ...
        buf = {
            "tensors": {"arg0": {"shape": [4, 4], "dtype": "i8"}, "Y0": {"shape": [1, 8], "dtype": "f32"}},
            "kernel_abi": {"args": [{"tensor": "arg0", "access": "read"}, {"tensor": "Y0", "access": "write"}]},
            "params": {"host_lane_program_emitted": True},
        }
        ...
        main = render_warm_then_measure_main(
            ...
            validate_outputs=h["validate"],
            reset_after_warm="/* arena */",
        )
...
class TestTheGateIsDerivedFromTheCapturesOwnContract:
    ...
    def _contract(self, **over):
        base = {
            ...
            "quality": {
                "scope": "trajectory",
                "golden": "session_quality_fp32.npz",
                "key": "actions",
                "output_index": 0,
    ...
    def _bundle(self, tmp_path, steps=4, per_step=(2, 3)):
        ...
        for field, name in (("correctness", "session_goldens.npz"), ("quality", "session_quality_fp32.npz")):
    ...
    def test_it_builds_the_gate_the_contract_declares(self, tmp_path):
        ...
        gate = G.gate_from_session_contract(
            self._bundle(tmp_path), model="m", datapath="w8a8", bundle_dir=tmp_path, atol=self.FIXTURE_FLOOR, rtol=0.0
        )
    ...
    def test_the_digest_subject_is_the_keyed_array_not_the_container(self, tmp_path):
        ...
        assert G.gate_from_session_contract(
            contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=self.FIXTURE_FLOOR, rtol=0.0
        )
    ...
    def test_a_golden_that_is_not_the_declared_one_is_REFUSED(self, tmp_path):
        ...
        with pytest.raises(G.GateError) as excinfo:
            G.gate_from_session_contract(
                contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=1e-4, rtol=1e-4
            )
...
class TestTheTwoChannelsAreDifferentClaims:
    ...
    def test_the_recorded_block_always_says_which_claim_it_is(self, tmp_path):
        ...
        blocks = [
            G.gate_from_session_contract(
                contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=0.6, rtol=0.0, channel=ch
            ).to_dict()
...
class TestPrecisionNeedsTwoAxesNotOne:
    ...
    def test_a_reference_missing_an_axis_is_UNKNOWN_and_refused(self, monkeypatch):
        ...
        kinds["undeclared_someday"] = {"file": "x.npy", "quantization": "?", "scope": "?", "generator": "?"}
        monkeypatch.setattr(G, "REFERENCE_KINDS", kinds)
        with pytest.raises(G.GateError) as excinfo:
            G.gate_for(
                model="m",
                datapath="w8a8",
                reference_kind="undeclared_someday",
                comparison="tolerance_and_topk",
                atol=1e-4,
                rtol=1e-4,
                output_elements=10,
...
class TestItAgreesWithEveryContractOnDisk:
    ...
    def test_every_declared_digest_verifies(self):
        ...
        for path, contract in self._contracts():
            for field in ("correctness", "quality"):
                ...
                assert G.reference_digest(golden, str(spec["key"])) == spec["reference_sha256"], f"{path}:{field}"
    ...
    def test_every_capture_yields_a_gate_from_its_own_DERIVED_tolerance(self):
        ...
        for path, contract in self._contracts():
            ...
            spread = G.reference_spread(
                path.parent / contract["correctness"]["golden"],
    ...
    def test_a_FIXED_tolerance_is_refused_wherever_the_spread_exceeds_it(self):
        ...
        for path, contract in self._contracts():
            ...
            with pytest.raises(G.GateError, match="tighter than this capture"):
                G.gate_from_session_contract(
                    contract, model=path.parent.name, datapath=datapath, bundle_dir=path.parent, atol=1e-4, rtol=1e-4
                )
            refused += 1
...
class TestTheTrajectoryGateActuallyFails:
    ...
    def _build(self, tmp_path, mutate, *, step_expression=None):
        ...
        if step_expression is not None:
            ...
            declarations = declarations.replace(before, f"merlin_reference + (long)({step_expression}) *")
...
class TestFreestandingSupportIsDeclaredNotPatchedIn:
    ...
    def test_several_distinct_symbols_are_all_reported(self):
        report = (
            "a.c:(.text+0x1): undefined reference to `__errno'\n"
    ...
    def test_the_emitted_shim_COMPILES(self, tmp_path):
        ...
        source.write_text(BH.render_freestanding_support(tuple(BH.FREESTANDING_SHIMS)), encoding="utf-8")
        build = subprocess.run(
            [
                _CC,
                "-std=c11",
...
class TestAFarConstBlobIsAddressedAbsolutelyNotByRelocation:
    ...
    def _plan(self, *, const_rows, mutable_rows):
        ...
        return BP.plan(
            {"tensors": tensors, "kernel_abi": {"args": args}, "params": {"host_lane_program_emitted": True}},
    ...
    def test_a_far_blob_is_reached_from_a_compile_time_literal(self):
        ...
        h = BH.render_bundle_harness(
            plan, self._gate(128), entry_symbol="k", output_tensor="Y0", const_blob_base=0x200000000
        )
    ...
    def test_the_reseed_uses_the_SAME_addressing_as_the_arguments(self):
        ...
        plan = BP.plan(
            buffer, row_pitch_elements=16, session_states=(BP.SessionState("s", input_arg=1, output_index=0),)
        )
        gate = BG.gate_for(
            model="m",
            datapath="w8a8",
        ...
        h = BH.render_bundle_harness(plan, gate, entry_symbol="k", output_tensor="Y0", const_blob_base=0x200000000)
    ...
    def test_a_negative_allowance_and_a_bad_base_are_refused(self):
        ...
        with pytest.raises(BH.BundleHarnessError):
            BH.render_bundle_harness(
                plan, self._gate(128), entry_symbol="k", output_tensor="Y0", near_additional_bytes=-1
            )
        for bad in (0, -1):
            with pytest.raises(BH.BundleHarnessError, match="positive absolute address"):
                BH.render_bundle_harness(
                    plan, self._gate(128), entry_symbol="k", output_tensor="Y0", const_blob_base=bad
                )
...
class TestAConsoleWithNoFloatSupportMustNotBeAskedForOne:
    ...
    def _plan(self):
        return BP.plan(
            {
                "tensors": {"arg0": {"shape": [1, 16], "dtype": "i8"}, "Y0": {"shape": [1, 8], "dtype": "f32"}},
                "params": {"host_lane_program_emitted": True},
                "kernel_abi": {"args": [{"tensor": "arg0", "access": "read"}, {"tensor": "Y0", "access": "write"}]},
            },
            row_pitch_elements=16,
    ...
    def test_no_rendered_fragment_asks_printf_for_a_float(self):
        h = BH.render_bundle_harness(self._plan(), self._gate(), entry_symbol="k", output_tensor="Y0")
    ...
    def test_the_bit_dump_round_trips_through_a_real_run(self, tmp_path):
        ...
        stub = (
            "#include <string.h>\n"
            "const float merlin_reference[8] = {" + ",".join(f"{v!r}f" for v in values) + "};\n"
        ...
        for line in run.stdout.splitlines():
            ...
            if len(parts) == 4 and parts[0] == "MERLIN_OUT":
                word = int(parts[3], 16)
                printed[int(parts[2])] = struct.unpack("<f", struct.pack("<I", word & 0xFFFFFFFF))[0]
        ...
        for index, want in enumerate(values):
            ...
            assert printed[index] == as_f32, f"element {index}: decoded {printed[index]!r} from bits, stored {as_f32!r}"
...
class TestTheConsoleBudgetIsATotalNotAPerStepFigure:
    ...
    def test_the_budget_is_a_SUBSTRATE_property_and_can_be_raised(self):
        ...
        cheap = BH.render_bundle_harness(plan, gate, entry_symbol="k", output_tensor="Y0", console_line_budget=32768)
    ...
    def test_a_negative_budget_is_refused(self):
        with pytest.raises(BH.BundleHarnessError):
            BH.render_bundle_harness(
                self._plan(100), self._gate(100, 10), entry_symbol="k", output_tensor="Y0", console_line_budget=-1
            )
...
class TestATOLERANCEMustBeDerivableNotMerelyDeclaredEarly:
    ...
    def _paths(self, tmp_path, gap):
        ...
        contract = {
            ...
            "correctness": {
                ...
                "output_index": 0,
                "reference": "eager_same_precision",
                "reference_sha256": G.reference_digest(tmp_path / "session_goldens.npz", "actions"),
            },
            "quality": {
                "scope": "trajectory",
                "golden": "session_quality_fp32.npz",
                ...
                "output_index": 0,
                "reference": "eager_fp32",
                "reference_sha256": G.reference_digest(tmp_path / "session_quality_fp32.npz", "actions"),
            },
        }
        return contract
    ...
    def test_the_spread_is_measured_from_the_two_references(self, tmp_path):
        ...
        spread = G.reference_spread(tmp_path / "session_goldens.npz", tmp_path / "session_quality_fp32.npz", "actions")
    ...
    def test_a_tolerance_AT_the_spread_is_accepted(self, tmp_path):
        ...
        contract = self._paths(tmp_path, 0.25)
        gate = G.gate_from_session_contract(
            contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=0.25, rtol=0.0
        )
    ...
    def test_atol_and_rtol_are_counted_TOGETHER_against_the_floor(self, tmp_path):
        ...
        contract = self._paths(tmp_path, 0.25)
        assert G.gate_from_session_contract(
            contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=0.10, rtol=0.15
        )
    ...
    def test_a_capture_with_only_ONE_reference_cannot_be_checked_and_is_not_pretended_to_be(self, tmp_path):
        ...
        gate = G.gate_from_session_contract(
            contract, model="m", datapath="w8a8", bundle_dir=tmp_path, atol=1e-9, rtol=0.0
        )
    ...
    def test_the_real_smolvla_floor_is_the_number_that_refuted_my_gate(self):
        ...
        spread = G.reference_spread(capture / "session_goldens.npz", capture / "session_quality_fp32.npz", "actions")
...
class TestALanguageModelsRankingIsPerTokenNotGlobal:
    ...
    def _plan(self, elements):
        return BP.plan(
            {
                "tensors": {"arg0": {"shape": [1, 16], "dtype": "i8"}, "Y0": {"shape": [elements], "dtype": "f32"}},
                "params": {"host_lane_program_emitted": True},
                "kernel_abi": {"args": [{"tensor": "arg0", "access": "read"}, {"tensor": "Y0", "access": "write"}]},
            },
            row_pitch_elements=16,
    ...
    def test_each_declared_row_is_ranked_separately(self):
        ...
        h = BH.render_bundle_harness(self._plan(256000), gate, entry_symbol="k", output_tensor="Y0")
    ...
    def test_a_single_ROW_disagreement_fails_the_gate(self, tmp_path):
        ...
        for r in range(rows):
            ...
            ref.extend(row)
        stub = [
            "const float merlin_reference[%d] = {%s};" % (n, ",".join(f"{v!r}f" for v in ref)),
            "const unsigned char merlin_const_blob_start[64];",
...
class TestOnlyTheHARNESSDecidesWhatIsPrinted:
    ...
    def test_a_raised_budget_CAN_dump_what_the_gates_own_cap_would_refuse(self):
        ...
        h = BH.render_bundle_harness(
            self._plan(256000), gate, entry_symbol="k", output_tensor="Y0", console_line_budget=300000
        )
        assert h["dumps_values"] is True and h["total_value_lines"] == 256000

    def test_the_default_budget_still_refuses_it(self):
        h = BH.render_bundle_harness(self._plan(256000), self._gate(256000), entry_symbol="k", output_tensor="Y0")
...
class TestAnAnalysisEmissionIsNotAProgram:
    ...
    def test_a_buffer_making_NO_claim_is_REFUSED(self):
        ...
        assert "offload, lane_cost, the roofline" in message, "the refusal must say what an analysis buffer IS good for"
    ...
    def test_a_DECLINED_program_is_refused_and_quotes_the_compiler(self):
        ...
        buffer["declined"] = {
            "reason": "needs about 3216234988 straight-line element evaluations, past this backend's 400000 budget",
            "op": "host_lane",
        }
        with pytest.raises(BH.BundleHarnessError) as excinfo:
            self._render(buffer)
...
# ---------------------------------------------------------------------------------------------
# What the reference can RESOLVE, which is a different claim from what it says.
#
# MEASURED on tiny_llama (2026-09-09/10): its W8A8 logits differ from a torch-eager
# dynamic-quantization reference by 3.6e-2 relative L2 on every token row. The divergence is 5e-7
# through two whole decoder layers, jumps at one activation quantizer sitting 1.9e-6 from a
# round-half-even tie while the reference sits 2.2e-5 on the other side, and compounds to 3.6e-2 by
# the logits. Forcing that code back makes the layer exact again (7.3e-7 -- reproduced), so the
# arithmetic agrees; 485 of 2,809,856 quantized activations lie within 1e-4 of a tie, so a
# population of such disagreements is the EXPECTED outcome and no tolerance recovers an elementwise
# contract. ResNet-50 is bit-exact against the same kind of reference and its bar does not move.
# ---------------------------------------------------------------------------------------------
from merlin.targetgen.bundle_gate import RESOLVING_POWERS, gate_for, unadjudicable_from_reference  # noqa: E402

_MARGIN_QUALIFIED = dict(
    model="tiny_llama",
    datapath="w8a8",
    reference_kind="w8a8_independent",
    comparison="tolerance_and_topk",
...
def test_a_gate_is_bit_exact_unless_it_says_otherwise() -> None:
    gate = gate_for(
        model="resnet50",
        datapath="w8a8",
        reference_kind="w8a8_independent",
    ...
    assert gate.resolving_power == "bit_exact"
    assert gate.noise_floor is None and gate.unadjudicable_rows == ()
    assert gate.to_dict()["resolving_power"] == "bit_exact"


def test_a_margin_qualified_gate_needs_a_measured_floor_and_says_how_it_was_measured() -> None:
    with pytest.raises(GateError, match="POSITIVE measured noise floor"):
        gate_for(**_MARGIN_QUALIFIED, resolving_power="margin_qualified")
    with pytest.raises(GateError, match="no stated derivation"):
        gate_for(**_MARGIN_QUALIFIED, resolving_power="margin_qualified", noise_floor=0.153)


def test_a_bit_exact_gate_refuses_fields_that_would_read_as_an_excuse() -> None:
    with pytest.raises(GateError, match="inert fields"):
        gate_for(**_MARGIN_QUALIFIED, noise_floor=0.153, noise_floor_derivation="measured single-tie spread")


def test_a_gate_where_every_row_is_unadjudicable_is_refused() -> None:
    """A check that cannot fail reports success; refuse it rather than ship it."""
    with pytest.raises(GateError, match="cannot fail"):
        gate_for(
            **_MARGIN_QUALIFIED,
            resolving_power="margin_qualified",
            noise_floor=0.153,
            noise_floor_derivation="measured single-tie spread",
            unadjudicable_rows=tuple(range(8)),
        )


def test_the_declared_gate_records_both_populations_so_neither_is_read_as_the_other() -> None:
    gate = gate_for(
        **_MARGIN_QUALIFIED,
        resolving_power="margin_qualified",
        noise_floor=0.153,
        noise_floor_derivation="max |delta logit| from forcing one unresolved tie",
        unadjudicable_rows=(0, 5, 6),
        max_bad=51215,
        max_bad_derivation="the gated run's own count plus one tie's movement",
    )
    recorded = gate.to_dict()
    assert recorded["unadjudicable_rows"] == [0, 5, 6]
    assert recorded["adjudicable_rows"] == [1, 2, 3, 4, 7]
    assert recorded["noise_floor"] == 0.153
    assert recorded["max_bad"] == 51215
    assert "RANKING only" in recorded["what_the_margin_rule_covers"]
    assert "never counted as agreement" in recorded["resolving_power_licence"]
    assert set(RESOLVING_POWERS) == {"bit_exact", "margin_qualified"}


def test_the_unadjudicable_set_is_derived_from_the_reference_not_the_candidate() -> None:
    """MEASURED margins: row 6 is 0.0054, 26x tighter than the next (0.142)."""
    reference = [[1.0, 1.0 - 0.470], [1.0, 1.0 - 0.0054], [1.0, 1.0 - 0.313]]
    assert unadjudicable_from_reference(reference, noise_floor=0.153) == (1,)
    assert unadjudicable_from_reference(reference, noise_floor=1e-9) == ()


# ---------------------------------------------------------------------------------------------
# The device side of a margin-qualified gate.
#
# The row partition is BAKED, not recomputed in C: which rows the reference can decide depends only
# on the reference and the measured floor, both known before the run, so the host derives it once
# and the device carries the answer. A C reimplementation of the rule could drift from the Python
# and the drift would be silent and in the permissive direction.
# ---------------------------------------------------------------------------------------------


def _margin_gate(**over):
    fields = dict(
        _MARGIN_QUALIFIED,
        resolving_power="margin_qualified",
        noise_floor=0.145557,
        noise_floor_derivation="measured single-tie spread",
        unadjudicable_rows=(5, 6),
        max_bad=51215,
        max_bad_derivation="the gated run's own 47,854 plus the measured 3,361-element movement of one unresolved tie",
    )
    fields.update(over)
    return gate_for(**fields)


def test_the_device_carries_the_partition_rather_than_recomputing_the_rule() -> None:
    c = BH._gate_check(_margin_gate(), output_offset=0, output_ctype="float")
    assert "static const char merlin_unadjudicable[8] = { 0, 0, 0, 0, 0, 1, 1, 0 };" in c
    assert "MERLIN_GATE_RESOLVING" in c


def test_the_device_prints_three_statuses_and_counts_them_apart() -> None:
    c = BH._gate_check(_margin_gate(), output_offset=0, output_ctype="float")
    for status in ('"agrees"', '"reference_cannot_discriminate"', '"disagrees"'):
        assert status in c
    assert "++merlin_rows_indeterminate" in c and "++merlin_rows_disagreeing" in c
    # The two counters are never added together, and only disagreement fails the gate.
    assert "const int merlin_argmax_ok = (merlin_rows_disagreeing == 0);" in c
    assert "merlin_rows_disagreeing + merlin_rows_indeterminate" not in c


def test_a_margin_qualified_gate_asserts_the_elementwise_term_it_counts() -> None:
    """It used to only COUNT it. That is what let a wrong program through."""
    c = BH._gate_check(_margin_gate(), output_offset=0, output_ctype="float")
    assert "MERLIN_GATE_ELEMENTWISE asserted=1" in c
    assert "merlin_bad > merlin_max_bad" in c
    assert "return (merlin_nonfinite != 0 || !merlin_argmax_ok) ? 1 : 0;" not in c
    assert "++merlin_bad" in c


def test_a_bit_exact_gate_still_asserts_every_element() -> None:
    """ResNet-50's bar does not move: bit-exact stays bit-exact."""
    gate = gate_for(
        model="resnet50",
        datapath="w8a8",
        reference_kind="w8a8_independent",
    ...
    c = BH._gate_check(gate, output_offset=0, output_ctype="float")
    assert "return (merlin_bad != 0 || merlin_nonfinite != 0 || !merlin_argmax_ok) ? 1 : 0;" in c
    assert "merlin_unadjudicable" not in c
    assert "reference_cannot_discriminate" not in c


def test_a_disagreement_on_an_adjudicable_row_is_the_one_thing_that_fails_the_gate() -> None:
    """The partition decides which rows can fail. Row 0 is adjudicable, so it can."""
    c = BH._gate_check(_margin_gate(unadjudicable_rows=(6,)), output_offset=0, output_ctype="float")
    assert "static const char merlin_unadjudicable[8] = { 0, 0, 0, 0, 0, 0, 1, 0 };" in c


# ---------------------------------------------------------------------------------------------
# THE VALUE ASSERTION, and why it is not optional.
#
# MEASURED 2026-09-10. A margin-qualified gate that asserted only the per-row ranking PASSED a
# wrong program: one changed compiler constant (fp.fconst(1.0) -> 1.01 in the rsqrt reciprocal, so
# every RMSNorm scales 1% high) left all eight token rows agreeing on top-1 -- it even "fixed" the
# unadjudicable row by accident -- so the gate returned 0 and published a METRIC, while the
# elementwise count went 47,854 -> 128,041 of 256,000 and the digest changed completely.
#
# The margin rule answers the RANKING question. A row being unadjudicable in RANK says nothing
# about whether its VALUES moved, and the same model's own numbers prove it: with all eight rows'
# top-1 recovered by the tie counterfactual, 44,493 elements still exceeded tolerance, spread over
# every row including the six the forced element cannot causally reach.
# ---------------------------------------------------------------------------------------------


def test_a_margin_qualified_gate_must_declare_a_value_bound() -> None:
    fields = dict(
        _MARGIN_QUALIFIED,
        resolving_power="margin_qualified",
        noise_floor=0.145557,
        noise_floor_derivation="measured",
        unadjudicable_rows=(5, 6),
    )
    with pytest.raises(GateError, match="must ALSO declare max_bad"):
        gate_for(**fields)
    with pytest.raises(GateError, match="no stated derivation"):
        gate_for(**fields, max_bad=51215)


def test_a_value_bound_at_or_above_the_element_count_is_refused() -> None:
    """A bound that admits every possible output is a check that cannot fail."""
    with pytest.raises(GateError, match="admits every possible output"):
        _margin_gate(max_bad=256000)


def test_a_bit_exact_gate_refuses_a_value_bound_that_would_loosen_it() -> None:
    with pytest.raises(GateError, match="loosen it silently"):
        gate_for(
            model="resnet50",
            datapath="w8a8",
            reference_kind="w8a8_independent",
...
def test_the_device_asserts_the_population_not_just_the_ranking() -> None:
    c = BH._gate_check(_margin_gate(), output_offset=0, output_ctype="float")
    assert "const int merlin_max_bad = 51215;" in c
    assert "MERLIN_GATE_ELEMENTWISE asserted=1" in c
    assert "merlin_bad > merlin_max_bad" in c
    # and the ranking term is still there, still margin-qualified
    assert "!merlin_argmax_ok" in c and "merlin_unadjudicable" in c


def test_the_1_percent_miscompile_that_passed_would_now_fail() -> None:
    """Its measured counts, replayed against the emitted rule."""
    gate = _margin_gate()
    assert 47854 <= gate.max_bad, "the gated run itself must still pass"
    assert 128041 > gate.max_bad, "the miscompile that passed before must now fail"


def test_a_declared_digest_is_asserted_and_an_undeclared_one_says_so() -> None:
    fixed = BH._gate_check(_margin_gate(expected_digest="092303a0648d31af"), output_offset=0, output_ctype="float")
    assert "0x092303a0648d31afULL" in fixed
    assert "MERLIN_GATE_DIGEST asserted=1" in fixed and "!merlin_digest_ok" in fixed
    candidate = BH._gate_check(_margin_gate(), output_offset=0, output_ctype="float")
    assert "MERLIN_GATE_DIGEST asserted=0" in candidate
    assert "merlin_digest_ok" not in candidate
