...
def _trace(instructions):
    return {"source": "synthetic", "abi": {}, "instructions": instructions, "summary": {"class_histogram": {}}}
...
def test_dram_argbase_is_clean():
    tr = _trace(
        [
            _ins(0, "MVIN", funct=2, spad_addr=0, rows=1, dram={"kind": "argbase", "arg_index": 0, "offset": 0}),
            _ins(1, "FENCE", funct=1),
        ]
    )
    findings, _ = simulate(tr, _facts(), address_model="pointer_args")
...
def test_a_named_float_datapath_is_not_read_as_a_width_of_its_digits():
    ...
    facts = {
        "datapaths": [
            {"name": "input", "dtype": "fp8_e4m3", "elem_bits": 8},
