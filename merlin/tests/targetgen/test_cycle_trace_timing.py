...
def test_busy_idle_and_overlap_are_counted_off_the_rows():
    ...
    assert (
        got["busy_cycles.a.in_program"]
        + got["busy_cycles.b.in_program"]
        - got[OBS.OVERLAP_OBSERVED]
        + got[OBS.IDLE_QUANTITY]
    ) == got[OBS.SAMPLED_QUANTITY]
...
def test_idle_is_declared_an_upper_bound_when_anything_is_unread():
    ...
    idle = next(
        o
        for o in CT.block_from_rows(_rows(aComp=[0], bMove=[0]), decl)[OBS.TIMING_OBSERVATIONS_KEY]
        if o["quantity"] == OBS.IDLE_QUANTITY
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            {"schema_version": CT.DECLARATION_SCHEMA, "busy_columns": {}, OBS.UNMEASURED_UNITS_KEY: []}, id="no-columns"
        ),
        pytest.param({"schema_version": CT.DECLARATION_SCHEMA, "busy_columns": {"a": {}}}, id="no-unmeasured-units"),
        pytest.param(
            {"schema_version": "some.other.v9", "busy_columns": {"a": {}}, OBS.UNMEASURED_UNITS_KEY: []},
            id="wrong-schema-tag",
        ),
    ],
)
...
def _run(monkeypatch, tmp_path, engine_dir):
    ...
    monkeypatch.setattr(
        PO, "_resolve_out_specs", lambda t, cb, b: {"Y": {"base": 0, "shape": [1], "dtype": "i32", "physical": "i32"}}
    )
    monkeypatch.setattr(PO, "_out_nbytes", lambda s: 4)
    # Mirrors the real signature, INCLUDING the keyword-only ``name`` the decode carries so a refusal
    # can say which tensor it is about. A stub that drops a parameter turns a caller change into a
    # TypeError inside the code under test, which reads as a defect in the thing being measured.
    monkeypatch.setattr(
        PO,
        "_decode_output",
        lambda raw, shape, dtype, phys, *, name="output": __import__("numpy").zeros(1, dtype="int32"),
    )
