...
_ROLES = {
    LW.OPS.dma_load: "movement",
    LW.OPS.dma_wait: "movement",
    LW.OPS.contract: "matmul",
    LW.OPS.contract_accumulate: "matmul",
}
...
def _directions():
    ...
    def od(mn, op, direction, sf="x"):
        return ID.OperandDirection(
            mnemonic=mn, operand=op, direction=direction, state_file=sf, written_slots=(0,), reason="synthetic"
        )

    by: dict = {}
    for mn in (LW.OPS.add, LW.OPS.add_imm, LW.OPS.load_upper):
        by[mn] = {"rd": od(mn, "rd", ID.DEF), "rs1": od(mn, "rs1", ID.USE), "rs2": od(mn, "rs2", ID.USE)}
    ...
    for mn in (
        ...
        by[mn] = {
            "vd": od(mn, "vd", ID.DEF, "v"),
            "vs1": od(mn, "vs1", ID.USE, "v"),
            "vs2": od(mn, "vs2", ID.USE, "v"),
            "rs1": od(mn, "rs1", ID.USE),
        }
    return ID.DirectionModel(target="synthetic", by_mnemonic=by, refused={}, provenance="synthetic probe (test)")
...
def test_with_both_evidences_the_driver_returns_a_verdict():
    ...
    a, b = _pair()
    v = RC.hoist_verdict(a, b, issue=_ISSUE, roles=_ROLES, directions=_directions(), separations=_PRICES)
