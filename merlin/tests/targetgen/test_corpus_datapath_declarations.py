...
def test_exact_decode_is_the_default_and_delegates_untouched():
    d = _D()
    dec = NUMERICS._operand_decoder(d, _Fmt(exp_bits=4, mant_bits=3), flush_subnormals=False)
...
def test_declared_flush_sends_every_zero_exponent_code_to_zero():
    d = _D()
    dec = NUMERICS._operand_decoder(d, _Fmt(exp_bits=4, mant_bits=3), flush_subnormals=True)
...
def test_the_subnormal_boundary_is_derived_from_the_format_not_a_byte_layout():
    ...
    d = _D()
    e4m3 = NUMERICS._operand_decoder(d, _Fmt(exp_bits=4, mant_bits=3), flush_subnormals=True)
    e5m2 = NUMERICS._operand_decoder(d, _Fmt(exp_bits=5, mant_bits=2), flush_subnormals=True)
