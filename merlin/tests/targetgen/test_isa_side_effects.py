...
def test_no_window_when_nothing_is_loaded_or_nothing_computed(model):
    ...
    only_moves = _words(model, [("VLOAD", {"rs1": 21, "imm": 0, "vd": 0}), ("VSTORE", {"rs1": 21, "imm": 0, "vd": 0})])
...
def test_an_instruction_that_changes_the_result_is_reported_as_writing_beyond_its_field(model):
    ...
    destructive = SE.probe_instruction(
        model, w, "VLI_ALL", {"imm": 0, "vd": 63}, run=lambda words: "clobbered" if len(words) > len(w) else "good"
    )
...
def test_an_instruction_that_respects_its_destination_is_reported_clean(model):
    ...
    clean = SE.probe_instruction(model, w, "VLI_ALL", {"imm": 0, "vd": 63}, run=lambda words: "same-every-time")
...
def test_a_settle_word_is_placed_after_the_injection(model):
    ...
    seen = []
    SE.probe_instruction(
        model, w, "VLI_ALL", {"imm": 0, "vd": 63}, run=lambda words: seen.append(list(words)) or "x", settle=0xDEAD
    )
...
def test_no_window_yields_no_verdict_rather_than_a_clean_bill(model):
    ...
    r = SE.probe_instruction(
        model,
        _words(model, [("VADD_BF16", {"vd": 8, "vs1": 0, "vs2": 4})]),
        "VLI_ALL",
        {"imm": 0, "vd": 63},
        run=lambda w: "x",
    )
