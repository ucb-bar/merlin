...
def _derivation(
    names=("OPMACC", "OPMVIN", "OPMVINBCAST", "OPMVOUT"), *, ok=True, gaps=(), crosschecks=({"agrees": True},)
):
    return SimpleNamespace(
        encodings={n: _Enc(n) for n in names}, gaps=tuple(gaps), crosschecks=tuple(crosschecks), ok=ok
    )
...
def _contract(roles=None):
    return SimpleNamespace(
        kernel_roles=roles
        or {"accumulate": "OPMACC", "broadcast": "OPMVINBCAST", "readout": "OPMVOUT", "operand_load": "OPMVIN"}
    )
...
def unit(monkeypatch):
    ...
    def _install(derivation=None, contract=None):
        ...
        def selected(target, key):
            assert target == "synthetic_support"
            assert key == "matrix_lowering"
            return fake

        monkeypatch.setattr(plugins, "load_declared", selected)
...
def test_the_classes_are_the_units_own_instruction_names(unit):
    unit()
    assert IT.matrix_unit_role_classes("u", support_target="synthetic_support") == {
        "matmul": ["OPMACC"],
        "weight_load": ["OPMVINBCAST"],
        "memory": ["OPMVIN"],
        "acc_readout": ["OPMVOUT"],
    }
...
def test_a_matmul_capsule_requires_load_push_multiply_and_readout(unit):
    unit()
    got = IT.matrix_unit_classes_for("u", support_target="synthetic_support")(op="matmul", output_dtype="i32")
    assert got == ["OPMVIN", "OPMVINBCAST", "OPMACC", "OPMVOUT"]


def test_a_movement_capsule_requires_only_the_memory_op(unit):
    unit()
    classify = IT.matrix_unit_classes_for("u", support_target="synthetic_support")
    assert classify(op="movement", movement=True) == ["OPMVIN"]
...
def test_an_epilogue_adds_nothing_because_this_unit_has_no_unary_instruction(unit):
    ...
    unit()
    f = IT.matrix_unit_classes_for("u", support_target="synthetic_support")
...
def test_an_ungrounded_derivation_refuses(unit):
    ...
    with pytest.raises(ValueError, match="not fully derived"):
        IT.matrix_unit_role_classes("u", support_target="synthetic_support")
...
def test_no_mappable_role_refuses_rather_than_returning_empty(unit):
    ...
    with pytest.raises(ValueError, match="no declared kernel role"):
        IT.matrix_unit_classes_for("u", support_target="synthetic_support")
...
def test_a_declared_unit_is_read_from_the_compute_unit(unit):
    unit()
    f = CS._classes_source(
        SimpleNamespace(target="synthetic_support"),
        {"name": "synthetic_support", "compute_units": [{"name": "u", "matrix_unit": "some_unit"}]},
    )
...
def test_the_rocc_encoding_regime_is_unchanged():
    ...
    contract = {
        "encoding": {
            "semantic_class": {"1": "MVIN", "2": "MVOUT", "3": "CONFIG", "4": "COMPUTE_PRELOADED", "5": "PRELOAD"},
            "config_subtype": {"a": "CONFIG_EX", "b": "CONFIG_LD"},
            "corpus_issue_order": ["CONFIG_EX", "CONFIG_LD", "MVIN", "PRELOAD", "COMPUTE_PRELOADED", "MVOUT"],
        }
    }
...
def test_command_issue_order_is_owned_by_each_contract_and_refuses_missing_classes():
    ...
    assert CS._classes_source(None, contract)(op="matmul") == ["BARRIER", "LAUNCH"]
    contract["encoding"]["corpus_issue_order"] = ["BARRIER", "UNDECLARED"]
    with pytest.raises(ValueError, match="undeclared"):
        CS._classes_source(None, contract)
    del contract["encoding"]["corpus_issue_order"]
    with pytest.raises(ValueError, match="corpus_issue_order"):
        CS._classes_source(None, contract)
