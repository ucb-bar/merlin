...
def test_an_interpreted_package_is_not_told_to_run_cmake(out_root, monkeypatch):
    ...
    monkeypatch.setenv("MERLIN_PUBLISH_REMOTE_GEMMINI", "file:///dev/null")
    res = pub.publish("gemmini", dry_run=True, package_id="graded_only_v0", gate=False)
    readme = (res.repo_dir / "MERLIN_PUBLICATION.md").read_text(encoding="utf-8")
    assert "cmake" not in readme.lower(), "a python package has no CMakeLists.txt to build"
    assert "gemmini-opt" in readme and "commands:" in readme
    assert "./gemmini-opt --help" not in readme
...
def test_historical_certified_status_cannot_grant_new_authority(out_root, monkeypatch):
    troot = out_root / "artifacts" / "targets"
    ...
    with pytest.raises(pub.PublishError, match="input identity"):
        pub.publish("gemmini", dry_run=True, package_id="certified_v0", gate=True)
    res = pub.publish("gemmini", dry_run=True, package_id="certified_v0", gate=False)
    readme = (res.repo_dir / "MERLIN_PUBLICATION.md").read_text(encoding="utf-8")
    assert "NOT CERTIFIED" in readme
    assert "certified champion codegen package" not in readme
    assert d.is_dir()
