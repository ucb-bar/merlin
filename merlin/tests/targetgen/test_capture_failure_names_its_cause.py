...
def test_retry_cannot_accept_metadata_from_an_earlier_attempt(tmp_path, monkeypatch, returncode, cached):
    from merlin.targetgen import capsule_source as source

    output = tmp_path / "capture"
    output.mkdir()
    # This incomplete cache entry must be retried, not accepted as a hit. Its
    # old successful-looking metadata is not a verdict from the new worker.
    (output / "meta.json").write_text(json.dumps({"ok": True, "opaque": 0}))
    (output / "linalg.mlir").write_text("module {}")
    (output / "inputs.json").write_text("[]")
    (output / "golden.json").write_text("[]")
    monkeypatch.setattr(source.PytorchRefSource, "_cache_slot", lambda *args, **kwargs: output if cached else None)
    calls = []

    def failed_worker(command, **kwargs):
        calls.append(command)
        return SimpleNamespace(returncode=returncode, stderr="current worker failed before publishing metadata")

    monkeypatch.setattr(source.subprocess, "run", failed_worker)
    capture = source.PytorchRefSource(m2m_dir=tmp_path / "upstream", python=tmp_path / "python")
    with pytest.raises(source.M2MUnavailable, match="wrote no meta.json|worker exited non-zero"):
        capture._run(tmp_path / "loader.py", "model", "f32", workdir=output, src="loader")
    assert len(calls) == 1
...
def test_a_semantically_normalizable_capture_reaches_the_shared_zero_opaque_gate(tmp_path, monkeypatch):
  ...
  func.func @forward(%x: tensor<1x32xf32>) -> tensor<1x32xi8> {
    ...
    with pytest.raises(CS.M2MUnavailable, match="no i8×i8→i32 integer contraction"):
        src.capture_loader(loader, "i8", workdir=tmp_path / "cap", scheme="int8_dyn_act_int8_weight")

    # Normalization still reaches zero opaque, but that alone cannot certify a
    # W8A8 test when this synthetic program has no integer contraction.
    meta = json.loads((tmp_path / "cap/meta.json").read_text())
    assert meta["opaque"] == 0
    receipt = meta["capture_normalization"]
    ...
    normalized = (tmp_path / "cap/linalg.mlir").read_text()
    assert "func.call @torchao" not in normalized
    assert "linalg.generic" in normalized
