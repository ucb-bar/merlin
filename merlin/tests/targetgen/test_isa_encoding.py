...
from merlin.targetgen import isa_asm, isa_disasm
...
def test_agent_chain_encode_assemble_disasm_lint(tmp_path):
    ...
    words = [
        isa_asm.encode_mem_op(m, "LOAD", "shared", {"rd": 5, "rs1": 6}),
...
def _text_words(elf: str, objcopy: str, objdump: str, tmp: Path) -> list[int]:
    ...
    secs = [p[1] for p in (l.split() for l in hdr.splitlines()) if len(p) >= 5 and p[0].isdigit() and p[-1] == "TEXT"]
    words: list[int] = []
    for sec in secs:
        b = tmp / f"{sec.strip('.')}.bin"
        subprocess.run([objcopy, "-O", "binary", f"--only-section={sec}", elf, str(b)], capture_output=True)
