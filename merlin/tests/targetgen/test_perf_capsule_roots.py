...
def test_absent_target_descriptor_never_falls_back_to_shared_historical_corpus(monkeypatch, tmp_path):
    from merlin.targetgen import corpora

    monkeypatch.setattr(corpora, "descriptor_path", lambda target: tmp_path / target / "missing.yaml")
    assert corpora.graded_capsule_roots("unknown") == []
    assert corpora.graded_capsule_roots("unknown", hidden=True) == []
