...
def test_unidentified_float_datapath_does_not_become_a_declared_format():
    """A bit-width observation is not an FP8 encoding identity or an eligibility claim."""
    unit = {
        "name": "tile",
        "ops": ["matmul"],
        "dtypes": ["int8"],
        "unnamed_float_datapaths": ["float8"],
        "accumulate": [
            {"in": "int8", "weight": "int8", "acc": "i32"},
            {"in": "float8", "weight": "float8", "acc": "f32"},
        ],
    }
    derived = cd.DerivedCapabilities()
    cd._from_unit_intent({"compute_units": [unit]}, derived)
    assert derived.supported["contraction"].dtypes == ("int8",)
    declared = {"contraction": SemanticCapability(family="contraction", dtypes=("int8",))}
    assert not [f for f in cd.reconcile(declared, derived) if f.get("axis") == "dtypes"]
    # A specifically identified format in another rule is still admitted as evidence.
    unit["accumulate"].append({"in": "mxfp8", "weight": "mxfp8", "acc": "f32"})
    assert cd._unit_dtypes(unit) == ("int8", "mxfp8")
