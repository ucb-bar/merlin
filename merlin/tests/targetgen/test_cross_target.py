...
def test_tool_blocks_are_target_agnostic():
    ...
    def tools(manifest):
        return {p for p in _paths(manifest) if p.startswith("merlin/python/")}
...
def test_radiance_cyclotron_adapter_requires_explicit_oot_muon_support():
    """Check the real oracle only when its selected, host-private provider exists."""
    from merlin.runtime.backends.base import get_backend
    from merlin.targetgen.target_registry import explicit_targets

    if "muon" not in explicit_targets():
        pytest.skip("requires explicitly selected OOT Muon support")
    muon_oracles = get_backend("muon").muon_oracles
    assert callable(muon_oracles.cyclotron_adapter())
    assert muon_oracles.default_adapters()  # selected provider supplies real L-tier adapters
