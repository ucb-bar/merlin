...
def test_store_name_is_derived_from_the_declaration_site():
    ...
    assert introspect._store_name("generators/w/src/main/scala/w/VMEM.scala 62:16", "banks_3") == "vmem.banks"
...
def test_host_command_port_is_classified_by_instruction_shape():
    ...
    """
    rocc = (
        "output io : { flip cmd : { flip ready : UInt<1>, valid : UInt<1>, "
                ...
                "instances": [
                    {
                        "instance_name": f"u{i}",
                        "module_name": "Unit",
                        "instances": [{"instance_name": "b", "module_name": "Bank", "instances": []}],
                    }
                    for i in range(2)
                ],
            }
...
def test_real_elaboration_yields_sized_stores(target):
    ...
    src = arts["source"]
    facts = introspect.extract_facts(
        arts["fir"], arts["hierarchy"], target=target, generator=src.generator, config=src.config
    )
