...
def _score(results):
    ...
    return {
        "n_capsules": len(graded),
        "n_passed": n_pass,
        "n_not_graded_ineligible": len(ungraded),
        "n_gated_deferred": len(deferred),
...
def test_the_measured_atlas_shape_becomes_reachable():
    results = (
        [{"capsule": f"p{i}", "status": "pass"} for i in range(14)]
...
def test_all_pass_is_reachable_once_the_gated_are_excluded():
    ...
    results = [{"capsule": f"p{i}", "status": "pass"} for i in range(22)] + [
        {"capsule": "M0", "status": "gated"},
        {"capsule": "M1", "status": "gated"},
    ]
    s = _score(results)
