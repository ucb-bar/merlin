...
def test_implicit_model_loader_remains_model2mlir_relative(tmp_path):
    assert (
        CSrc.resolve_model_loader({"model": "small_llama"}, m2m_dir=tmp_path / "model2MLIR")
    ...
    cap = CSrc._fused_capsule_yaml(
        {
            ...
            "source_reference": "sha256 capture region",
            "out": "Y0",
        },
        _float_binding(),
        Art(),
...
_needs_m2m = pytest.mark.skipif(
    not _M2M.available(), reason="m2m venv (torch) unavailable; set MERLIN_M2M_PYTHON/MERLIN_M2M_DIR"
)
...
def test_linalg_to_iface_fails_closed_on_missing_op():
    ...
    bad = _MATMUL_LINALG.replace('prov.family = "contraction"', 'prov.family = "normalization"').replace(
        'prov.op = "matmul"', 'prov.op = "rmsnorm"'
    )
        ...
        (
            {
                ...
                "cat": "model_slices",
                "source_role": "handauthored_compiler_test",
                "source_reference": "pytorch matmul",
                "op": "matmul",
                "M": 16,
                "K": 16,
                "N": 16,
        ...
        (
            {
                ...
                "cat": "model_slices",
                "source_role": "handauthored_compiler_test",
                "source_reference": "pytorch rmsnorm",
                "op": "rmsnorm",
                "M": 16,
                "K": 16,
            },
...
def test_args_from_cb_linalg_positional():
    ...
    cb = {
        ...
        "arg_order": ["X", "W", "B", "Y0"],
        "tensors": {
            "X": {"role": "input", "shape": [4, 4]},
            "W": {"role": "input", "shape": [4]},
            "B": {"role": "input", "shape": [4]},
            "Y0": {"role": "output", "shape": [4, 4]},
        },
        "canonical_inputs": {
            "X": {"values": [float(i) for i in range(16)]},
            "W": {"values": [1.0, 1.0, 1.0, 1.0]},
            "B": {"values": [0.0, 0.0, 0.0, 0.0]},
        },
    }
...
def test_write_model_capsule_small_llama(tmp_path):
    ...
    entry = {
        "name": "M0_small_llama_fp32",
        "cat": "model",
        "model": "small_llama",
        "operand_dtype": "f32",
        "source_reference": "tiny full LLaMA",
        "label": "public",
...
def test_generate_corpus_pytorch_route_fails_closed_on_int(tmp_path):
    ...
    ib = dataclasses.replace(
        _float_binding(), operand_dtype="int8", accum_dtype="i32", integer=True, compare="exact_int"
    )
    entry = {
        "name": "WBAD",
        "kind": "isa",
...
def test_extent_scan_matches_the_op_name_exactly():
    """`linalg.matmul_transpose_b` also starts with "linalg.matmul". Accepting it records the operand
    types of a TRANSPOSED pair as a plain matmul's (M, K, N). The extents used to be joined to the
    demands positionally, so one spurious entry mis-shaped every later layer of the model; they are now
    read per tag (see test_generic_contraction_extents), and the exact-name rule is what keeps the
    transposed form from contributing one at all."""
    from merlin.targetgen.capsule_source import _carrier_extents

    tag = '{prov.op = "matmul", prov.family = "contraction"}'
    plain = f"  %0 = linalg.matmul {tag} ins(%a, %b : tensor<8x16xf32>, tensor<16x32xf32>) outs(%c : tensor<8x32xf32>)"
    tb = (
        f"  %1 = linalg.matmul_transpose_b {tag} "
        "ins(%a, %b : tensor<8x16xf32>, tensor<32x16xf32>) outs(%c : tensor<8x32xf32>)"
    )
    assert _carrier_extents(plain) == [(1, 8, 16, 32)]
    assert _carrier_extents(tb) == [None], "a transpose-b matmul must not masquerade as a plain one"
    assert _carrier_extents(plain + "\n" + tb) == [(1, 8, 16, 32), None]
