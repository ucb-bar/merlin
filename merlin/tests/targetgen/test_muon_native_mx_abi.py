...
if "muon" not in explicit_targets():
    pytest.skip("requires explicitly selected OOT Muon support", allow_module_level=True)
...
CONTRACT = repo_root() / "examples/radiance/phase1/kernel_library/selection.yaml"
...
def test_native_program_and_both_controls_execute_on_cyclotron(tmp_path):
    ...
    for index, actual in enumerate(actuals):
        ...
        except MUON.MuonUnavailable as exc:
            if "cycle cap before completion" not in str(exc):
                raise
            pytest.skip(f"Cyclotron bounded execution produced no numerical verdict: {exc}")
