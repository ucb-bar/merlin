...
def test_synthetic_search_seal_executes_with_only_its_installed_source_owner(tmp_path, _working_search_engine):
    """Exercise sealing, not just importing, without sibling extensions or real corpus approval."""
    root = repo_root()
    owner = root / "packages/merlin-experiments/src"
    blocked = {"merlin.targetgen.capsule_runner", "merlin.targetgen.capsule_golden"}
    for sibling in (root / "packages").glob("*/src"):
        if sibling == owner:
            continue
        for path in sibling.rglob("*.py"):
            parts = list(path.relative_to(sibling).with_suffix("").parts)
            if parts[-1] == "__init__":
                parts.pop()
            blocked.add(".".join(parts))
    capsule = tmp_path / "corpus/fixture"
    capsule.mkdir(parents=True)
    (capsule / "capsule.yaml").write_text("label: dev\nsource_role: model_derived\n")
    descriptor = tmp_path / "target_experiment.yaml"
    descriptor.write_text("target: synthetic\n")
    _candidate(tmp_path)
    score = tmp_path / "search-score.json"
    _passing_search_score(score, SimpleNamespace(graded_include=("fixture",)))
    # The existing host-only engine fixture supplies ordinary bytes, never an executable run.
    binding = copy.deepcopy(_working_search_engine)
    binding["target"] = "synthetic"
    binding["binding_sha256"] = _canonical_sha(
        {key: value for key, value in binding.items() if key != "binding_sha256"}
    )
    score_doc = json.loads(score.read_text())
    score_doc["per_capsule"][0]["barrier_engine_binding"] = binding
    score.write_text(json.dumps(score_doc))
    script = """
...
def forbidden(name):
    return name in blocked or name.split('.')[0] in {'aet', 'torch', 'torchao'}
assert not any(forbidden(name) for name in sys.modules)
class OwnerOnly(importlib.abc.MetaPathFinder):
    def find_spec(self, fullname, path=None, target=None):
        assert not forbidden(fullname), f'non-owner import: {fullname}'
sys.meta_path.insert(0, OwnerOnly())
sys.path[:0] = sys.argv[1:3]
def no_execution(*args, **kwargs):
    raise AssertionError('sealing fixture must not execute a backend')
subprocess.Popen = no_execution
from merlin.targetgen import evaluation_cohort as cohort
assert pathlib.Path(cohort.__file__).is_relative_to(sys.argv[2])
...
seal, candidate = work / 'seal.json', work / 'candidate'
record = cohort.create_search_pass_seal(seal, te, candidate, work / 'search-score.json')
assert record['n_capsules'] == record['n_passed'] == 1
assert record['l2_cycles'] == {'fixture': 1000}
assert record['capsules'][0]['name'] == 'fixture'
assert record['claim_scope'] == 'admitted_search_covering_set_not_e2e_readiness'
assert cohort.validate_search_pass_seal(seal, te, candidate)['seal_sha256']
# Actual content checks must still execute through the isolated owner, not a fake seal validator.
(candidate / 'schedule.mlir').write_text('module { func.func private @changed() }')
try:
    cohort.validate_search_pass_seal(seal, te, candidate)
except ValueError as exc:
    assert 'candidate content digest mismatch' in str(exc)
else:
    raise AssertionError('changed candidate accepted')
assert not any(forbidden(name) for name in sys.modules)
"""
    result = subprocess.run(
        [sys.executable, "-I", "-c", script, str(root / "src"), str(owner), str(tmp_path)],
        cwd=tmp_path,
        input=json.dumps({"blocked": sorted(blocked), "binding": binding, "descriptor_sha256": _sha(descriptor)}),
        capture_output=True,
        text=True,
        timeout=30,
    )
