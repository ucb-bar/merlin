...
def frozen_capsule() -> dict:
    ...
    if missing:
        pytest.skip(
            f"frozen model capsule at {_FROZEN_MODEL_DIR} is missing {missing} "
...
def _snapshot_package(tmp_path, package_rel: str, descriptor: Path):
    ...
    paths = [package_rel, str(descriptor), experiment.numeric_profile]
    bundle = {"allowed": [], "host_inputs": [{"path": p} for p in paths if p is not None]}
    manifest = bwrap.materialize_bundle_inputs(ws, bundle, repo=repo_root())
    root = bwrap.bundle_snapshot_root(ws)
...
def test_targeted_model_grade_passes_exact_descriptor_package_and_records_it(monkeypatch, frozen_capsule):
    seen = {}
...
def test_bwrap_model_grade_executes_run_snapshot_not_live_package(monkeypatch, tmp_path, frozen_capsule):
    ...
    monkeypatch.setenv(
        "MERLIN_MODEL_HOST_LANE_SNAPSHOT_RECORD", json.dumps(bwrap.snapshot_record(tmp_path / "workspace"))
    )
    monkeypatch.setenv("MERLIN_MODEL_GRADE_RUN", "host")
    seen = {}
...
def test_qa_loop_exports_verified_snapshot_only_to_host_grading():
    ...
    assert "verify_snapshot_binding" in source
    for key in ("ROOT", "REQUIRED", "RECORD"):
        assert f"MERLIN_MODEL_HOST_LANE_SNAPSHOT_{key}" in source
        assert f"_MODEL_HOST_SNAPSHOT_{key}_ENV" in inspect.getsource(execution.sandbox_command)


def test_model_grade_refuses_descriptor_package_of_the_wrong_datatype(monkeypatch, tmp_path, frozen_capsule):
    called = False
...
def test_model_grade_refuses_host_package_drift_during_compile(monkeypatch, frozen_capsule):
    ...
    monkeypatch.setattr(
        ...
        lambda target, dtype: (Experiment(), repo_root() / "frozen-test-host", before),
    )
    monkeypatch.setattr(
        compile_cli,
        "compile_model",
