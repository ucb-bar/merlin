...
def test_explicit_numeric_policy_reaches_reference(monkeypatch, flush):
    seen = []

    def reference(a, w, **kwargs):
        seen.append(kwargs)
        return a @ w

    monkeypatch.setattr(WG, "accumulate_reference", reference)
    ...
    WG.plan_matmul(
        ...
        A=values,
        W=values,
        numeric_policy={"compare": "tolerance_float", "subnormal_operand_flush": flush},
    )
    assert seen[0]["subnormal_operand_flush"] is flush


def test_missing_float_reference_policy_refuses():
    values = np.ones((4, 4), dtype=np.float32)
    with pytest.raises(WG.WorkloadError, match="explicit declared numeric_policy"):
        WG.plan_matmul(synthetic_facts(), OPS, m=4, k=4, n=4, control_flow=CF, settle=SETTLE, A=values, W=values)
