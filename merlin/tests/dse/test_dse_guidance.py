...
from merlin.dse_guidance import aet_ingest, axes, calibration, pipeline, representation, study, synth
...
from merlin.dse_guidance.reference_data import case_study_dir
...
def test_temporal_stated_deadline_disagreement_warns():
    doc = {
        "workload": "w",
        "timing": {"K": 2, "H": 10, "control_rate_hz": 30, "replan_deadline_ms": 999.0},
        "regions": [],
    }
    md = T.parse(doc)
...
def test_autonomous_k_loop_illegal_when_K_le_1():
    ...
    baseline = BC.parse(
        {
            "workload": "no_reuse_matmul",
            "baseline": {"total_ms": 10, "components": {"cpu_dispatch_ms": 6, "sync_ms": 4}},
        }
    )
...
def test_no_target_reports_baseline_share_only():
    baseline = BC.parse({"workload": "w", "baseline": {"total_ms": 100, "components": {"compute_ms": 40}}})
...
def test_negative_control_does_not_recommend_residency():
    region = load_yaml(study.paths.merlin_dir() / "benchmarks" / "semantic_memory" / "no_reuse_matmul.yaml")
    ...
    assert rows["resident_packed_weights"]["priority_score"] in (None, 0) or rows["resident_packed_weights"][
        "gap_closure"
    ] in (None, 0)
...
def test_cpu_coupling_yaml_matches_aet_run():
    ...
    assert via_yaml.regimes[aet_ingest.OP_LEVEL].host_submit_ns == via_run.regimes[aet_ingest.OP_LEVEL].host_submit_ns
...
def test_component_sum_residualized_not_silently_dropped():
    baseline = BC.parse(
        {"workload": "w", "baseline": {"total_ms": 100, "components": {"compute_ms": 40, "dma_memory_ms": 30}}}
    )
...
def test_untagged_component_defaults_to_assumed():
    baseline = BC.parse({"workload": "w", "baseline": {"total_ms": 10, "components": {"compute_ms": 10}}})
...
def test_artifacts_validate_against_schemas(tmp_path):
    ...
    assert schemas.validate(load_yaml(FIX / "smolvla_action_head_temporal.yaml"), "temporal_workload_metadata") == []
    assert schemas.validate(load_yaml(FIX / "smolvla_action_head_cost.yaml"), "baseline_cost") == []
...
def test_study_synthesized_baseline_is_analytical():
    region = load_yaml(study.paths.merlin_dir() / "benchmarks" / "semantic_memory" / "vla_action_chunk_decode.yaml")
...
def test_residency_is_component_specific_and_unquantified_without_attribution():
    baseline = BC.parse(
        {
            "workload": "w",
            "baseline": {"total_ms": 100, "components": {"packing_ms": 20, "dma_memory_ms": 60, "compute_ms": 20}},
        }
    )
    facts = {
        "K": 8,
        "has_k_loop": True,
        "has_repeated_head": True,
        "head_weights_immutable": True,
        "visible_weight_reuse": 8,
    ...
    base_head = BC.parse(
        {
            ...
            "baseline": {
                "total_ms": 100,
                "components": {"packing_ms": 20, "dma_memory_ms": 60, "compute_ms": 20},
                "repeated_head": {"packing_ms": 8, "weight_memory_ms": 10},
            },
        }
    )
...
def test_model_zoo_discovery_and_specs():
    ...
    for spec in specs:
        assert (
            schemas.validate(  # temporal + baseline docs are well-formed
                {"workload": spec.name, "timing": {"K": spec.temporal.K}, "regions": []}, "temporal_workload_metadata"
            )
            == []
        )
        assert spec.baseline.unit == "cycles"
...
def test_model_xr0_calibration_anchor_is_measured_and_honest():
    ...
    rows = M.calibration_rows(M.MODEL_ARCH["xr0"], M.capture_facts(M._prefer_capture(captures["xr0"])))
...
def _head_rec(i):
    return ATTR.MatmulRecord(
        i, f"matmul_{i}", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, (28 * 1024 + 28 * 1024) * 4, "f32"
    )
...
def _records():
    ...
    return (
        ATTR.MatmulRecord(
            0, "matmul_0", "matmul", False, 1, 2048, 9216, 2048 * 9216 * 4, (1 * 2048 + 1 * 9216) * 4, "f32"
        ),
        _head_rec(1),
        _head_rec(2),
        _head_rec(3),
        ATTR.MatmulRecord(
            4, "matmul_4", "matmul", False, 28, 512, 512, 512 * 512 * 4, (28 * 512 + 28 * 512) * 4, "f32"
        ),  # unmapped -> unknown
    )
...
def _topo(K=5):
    doc = {
        ...
        "regions": [
            {"name": "bb", "role": "backbone_once"},
            {"name": "hd", "role": "repeated_head", "invocation_count": K, "loop_invariant_state": ["weights"]},
        ],
    }
...
@pytest.mark.skipif(not (_RDT_RECAP / "model.mlir").is_file(), reason="no fresh rdt prov.fqn capture")
...
@pytest.mark.skipif(not (_RECAP_LOOP / "openvla" / "model.mlir").is_file(), reason="no loop-preserving capture")
...
@pytest.mark.skipif(not (_RECAP_NATIVE / "bitvla" / "model.mlir").is_file(), reason="no native low-bit bitvla capture")
...
def test_dse_readiness_accuracy_status_wired():
    ...
    recs = (
        ATTR.MatmulRecord(
            0, "m0", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, 1, "f32", fqn="model.action_expert.denoise.0"
        ),
    )
    attr = ATTR.attribute_records(recs, topo)
    r_pass = CON.dse_readiness(topo, attr, None, cpu_coupling_available=False, accuracy_status="pass")
    ...
    r_na = CON.dse_readiness(topo, attr, None, cpu_coupling_available=False, accuracy_status="unavailable")
...
def test_case_study_emits_presentation_package(tmp_path):
    ...
    for f in (
        ...
        "workload_contract_table.csv",
        "abstraction_pressure_table.csv",
        "dse_readiness_summary.csv",
        "dtype_capacity_table.csv",
        "accuracy_gate_report.md",
        "accuracy_gate_results.csv",
...
def test_design_envelope_requirements_and_dtype_scaling():
    ...
    env = DE.derive(
        ...
        K=5,
        deadline_s=64 / 30.0,
        deadline_evidence="assumed_reference",
        macs_per_step=39_432_486_912,
        weight_bytes=391_118_848,
        activation_bytes_per_step=10_000_000,
        dispatches_per_step=20,
        dispatches_evidence="derived_requirement",
        captured_dtype="f32",
    )
...
def test_design_envelope_missing_deadline_keeps_capacity_drops_rate():
    ...
    env = DE.derive(
        ...
        K=5,
        deadline_s=None,
        deadline_evidence="unavailable",
        macs_per_step=1000,
        weight_bytes=4000,
        activation_bytes_per_step=0,
...
def test_design_envelope_feasibility_and_command_gate():
    ...
    design = {
        ...
        "macs_per_cycle": 4096,
        "local_memory_mb": 256,
        "dram_bandwidth_gb_s": 256,
        "supported_dtypes": ["bf16", "int8"],
    }  # no submit_ns
    env = DE.derive(
        "rdt",
        K=5,
...
def test_contract_abstraction_candidates_and_readiness():
    ...
    recs = (
        ...
        ATTR.MatmulRecord(
            1, "m1", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, 1, "f32", fqn="model.action_expert.denoise.1"
        ),
    )
    attr = ATTR.attribute_records(recs, topo)
    ...
    assert (
        "speedup" not in blob.replace("not_claimed", "").replace("what_is_not_claimed", "")
        or "speedup" in str(rw.what_is_not_claimed).lower()
    )  # only as a "not claimed" entry
...
@pytest.mark.skipif(
    not (_OUTPUT / "small_llama_int8_consistent" / "model.mlir").exists(),
...
def test_numerical_contract_fp32_emits_lowbit_candidate_from_real_bytes():
    ...
    c = NC.audit(
        str(_RDT_RECAP),
        workload="rdt",
        workload_class="diffusion/denoise_steps",
...
def test_numerical_contract_yaml_has_no_speedup_or_accuracy_number():
    ...
    if not (_RDT_RECAP / "model.mlir").is_file():
        pytest.skip("no rdt recapture")
    obj = NC.to_yaml_obj(NC.audit(str(_RDT_RECAP), workload="rdt", repeated_head_weight_bytes=391_118_848))
    blob = str(obj).lower()
...
def test_attribution_auto_recovers_roles_from_prov_fqn():
    ...
    recs = (
        ATTR.MatmulRecord(
            0, "matmul_0", "matmul", False, 1, 2048, 9216, 2048 * 9216 * 4, 1, "f32", fqn="model.vision_backbone.proj"
        ),
        ATTR.MatmulRecord(
            1, "matmul_1", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, 1, "f32", fqn="model.action_expert.denoise.0"
        ),
        ATTR.MatmulRecord(
            2, "matmul_2", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, 1, "f32", fqn="model.action_expert.denoise.1"
        ),
    )
...
def test_candidates_carry_attributed_facts_or_block_on_attribution():
    topo = _topo(K=5)
    rules = {"rules": [{"role": "repeated_head", "match": {"shape_signature": [28, 1024, 1024]}}]}
...
def test_model_capture_residency_is_unquantified_not_fabricated():
    ...
    captures = M.discover_model_captures()
    spec = study.spec_from_model("openvla", captures["openvla"])
    res = pipeline.run_guidance(
        spec.temporal, spec.baseline, overrides=spec.overrides, capture_facts=spec.capture_facts
    )
    row = next(r for r in res.triage_multirate["axes"] if r["axis"] == "resident_packed_weights")
...
def _pkg_topo_attr(K=5, fqn="model.action_expert.denoise.0"):
    ...
    topo = _topo(K=K)
    recs = (ATTR.MatmulRecord(0, "m0", "addmm", True, 28, 1024, 1024, 1024 * 1024 * 4, 1, "f32", fqn=fqn),)
...
def test_state_lifetime_marks_unknown_bytes_unavailable():
    ...
    doc = {
        ...
        "regions": [
            {"name": "hd", "role": "repeated_head", "invocation_count": 4, "loop_invariant_state": ["prefix_kv"]}
        ],
    }
    topo = TOP.from_temporal(T.parse(doc))
...
def test_case_study_emits_contract_completeness_package(tmp_path):
    ...
    for f in (
        ...
        "workload_family_table.csv",
        "measurement_priority_table.csv",
        "torchao_integration_plan.md",
    ):
        assert (tmp_path / f).is_file(), f"missing {f}"
...
def test_dtype_certificates_accuracy_gated():
    ...
    assert "speedup" in str(obj["numerical_candidate_certificates"]["certificates"][0]["what_is_not_claimed"])
    assert "gap_closure" not in str(obj).lower()
...
def test_case_study_emits_envelope_and_certificate_tables(tmp_path):
    ...
    CS.run_case_study(tmp_path)
    for f in ("traffic_table.csv", "dispatch_granularity_table.csv", "accuracy_gated_dtype_candidates.csv"):
        assert (tmp_path / f).is_file(), f"missing {f}"
...
def test_independent_verification_harness_passes():
    ...
    r = subprocess.run([sys.executable, str(script), "--check-only"], capture_output=True, text=True, cwd=str(root))
...
def _mm(M, N, K, fqn=None, op="matmul", idx=0):
    ...
    return ATTR.MatmulRecord(
        idx, f"matmul_{idx}", op, op == "addmm", M, K, N, K * N * 4, (M * K + M * N) * 4, "f32", fqn=fqn
    )
...
def test_operator_geometry_extracts_macs_bytes_aspect():
    ...
    s = OG.operator_shape(_mm(67, 2048, 2048, fqn="model.blocks.0.ffn.fc1", op="addmm"), "rdt", "repeated_head")
...
def test_primitive_coverage_aggregation_and_regret():
    ...
    shapes = [
        OG.operator_shape(_mm(8, 344, 128, idx=0), "wa", "r"),
    ...
    for r in regret:
        assert abs(r.max_regret - (r.best_workload_coverage_10 - r.worst_workload_coverage_10)) < 1e-9
...
def test_p5_artifacts_have_no_speedup_or_forbidden_fields():
    ...
    regret = PC.aggregate_regret(cov, per)
    blobs = [
        str(OG.to_yaml_obj({"rdt": shapes}, conv_visible=False)).lower(),
...
def test_contract_graph_data_dependencies_recovered_from_ir():
        ...
        T.parse(
            {
                "workload": "m",
                "class": "diffusion/denoise_steps",
                "timing": {"K": 3, "H": 3, "control_rate_hz": 30},
                "regions": [
                    {
                        "name": "head",
                        "role": "repeated_head",
    ...
    recs = (_mm(8, 128, 128, fqn="model.denoise.0", idx=0), _mm(8, 128, 128, fqn="model.denoise.1", idx=1))
    attr = ATTR.attribute_records(recs, topo)
    shapes = OG.operator_shapes(recs, "m", attr)
...
def test_contract_graph_no_forbidden_wording():
    ...
    g = _graph_fixture(K=5)
    blobs = [
        str(CG2.to_yaml_obj([g])).lower(),
        CG2.summary_md([g]).lower(),
        CG2.rate_mismatch_report_md([g]).lower(),
        str(CG2.multi_rate_contract_yaml([g])).lower(),
        CG2.phase_rate_csv([g]).lower(),
    ]
    for b in blobs:
...
def _toy_graph():
    ...
    N = CG2.Node
    nodes = [
        N(id="w:op:0", workload="w", kind="operator", shape_summary={"macs": 10}),
        N(id="w:op:1", workload="w", kind="operator", shape_summary={"macs": 10}),
        N(id="w:op:2", workload="w", kind="operator", shape_summary={"macs": 100}),
    ]
    edges = [CG2.Edge("w:op:0", "w:op:2", "data_dependency"), CG2.Edge("w:op:1", "w:op:2", "data_dependency")]
...
def test_parallelism_conservative_fallback_when_no_data_edges():
    ...
    nodes = [
        CG2.Node(id="w:op:0", workload="w", kind="operator", shape_summary={"macs": 5}),
...
def test_resource_hierarchy_schema_and_unavailable_units():
    ...
    shapes = [
        OG.operator_shape(_mm(4096, 4096, 2048, idx=0), "w", "r"),  # dense_gemm
        OG.operator_shape(_mm(1, 2048, 256, idx=1), "w", "r"),
    ]  # gemv -> skinny
    ...
    units = {
        u["unit"]: u
        for u in RH.processing_unit_candidates_yaml(RH.processing_unit_candidates(shapes, pressure))[
...
def test_structural_hierarchy_hints_cover_vocabulary():
    ...
    shapes = [
        OG.operator_shape(_mm(4096, 4096, 2048, idx=0, op="addmm"), "w", "repeated_head"),
    ...
    assert {"reduction_tree", "epilogue_unit", "DMA_engine", "loop_controller", "multi_engine_cluster"} <= set(hints)
    ...
    obj = RH.hierarchy_hints_yaml(RH.cluster_hierarchy(shapes), list(hints.values()))["parallel_hierarchy_hints"]
...
def test_p8_reports_have_no_forbidden_wording():
    ...
    blobs = [
        PE.overlap_report_md({"m": phases}, cands).lower(),
...
def test_p9_reports_have_no_forbidden_wording():
    ...
    mem = {"m": rm}
    blobs = [
        ME.memory_envelope_report_md(mem).lower(),
...
def test_epilogue_pattern_label_and_certificates():
    ...
    assert (
        FE._pattern_label({"bias": True, "activation": True, "scale": False, "clamp": False, "cast": False})
        == "matmul->bias->activation"
    )
    assert (
        FE._pattern_label({"bias": False, "activation": False, "scale": False, "clamp": False, "cast": False})
        == "matmul"
    )
    ...
    pat = FE.EpiloguePattern(
        0, "addmm", "matmul->bias->activation", True, True, False, False, False, 2, False, "recovered_from_ir"
    )
        ...
        T.parse(
            {
                ...
                "class": "diffusion/denoise_steps",
                "timing": {"K": 5, "H": 64, "control_rate_hz": 30},
                "regions": [{"name": "h", "role": "repeated_head", "invocation_count": 5}],
            }
        )
    )
...
def test_p10_no_forbidden_wording_or_false_measured_pass():
    ...
    pat = FE.EpiloguePattern(
        0, "addmm", "matmul->bias", True, False, False, False, False, 1, False, "recovered_from_ir"
    )
    a = FE.AccumulatorContract(
        "r",
        1,
    ...
    blobs = [
        FE.fusion_report_md({"m": [pat]}, {"m": [a]}, {"m": certs}).lower(),
...
def test_case_study_emits_p10_artifacts(tmp_path):
    ...
    accs = list(csv.DictReader(io.StringIO((tmp_path / "accumulator_contract_table.csv").read_text())))
    for r in accs:
...
def test_case_study_emits_p9_artifacts(tmp_path):
    ...
    for r in dm:
        assert int(r["avoidable_weight_reload"]) == int(r["weight_bytes"]) * max(int(r["invocations"]) - 1, 0)
    ...
    for c in certs.values():
        for b in c.boundary_levels:
            for fld in (
                ...
                "software_manages",
                "hardware_manages",
                "required_compiler_proof",
                "required_runtime_support",
                "required_isa_semantics",
                "required_hw_support",
                "metadata_crossing",
                "risk",
                "missing_evidence",
...
def test_boundary_grounded_supporting_workloads():
    ...
    assert certs["skinny_gemm_or_gemv_engine"].supporting_workloads == ["openvla", "rdt", "small_llama", "tiny_llama"]
...
def test_boundary_erased_abstractions_blocked():
    ...
    c = _bp_certs()
    for name in ("packed_lowbit_tensor", "scale_object", "fused_dequant_matmul", "native_lowbit_matmul"):
...
def test_responsibility_matrix_schema():
    ...
    assert len(rows) == 17
    for r in rows:
        for col in ("compiler", "runtime_hal", "command_processor", "accelerator_isa", "device_microcode", "datapath"):
...
def test_boundary_no_forbidden_wording():
    ...
    blobs = [
        BP.boundary_report_md(certs, BP.responsibility_rows()).lower(),
    ...
    for b in blobs:
        for term in ("optimal", "best design", "faster", "predicted", "performance improvement", "gap_closure"):
            assert term not in b, f"forbidden term {term!r}"
        for ln in b.splitlines():
            if "speedup" in ln:
                assert "no speedup" in ln or "not a speedup" in ln
...
def test_dse_contract_manifest_and_query(tmp_path):
    ...
    for q in ("summary", "knobs", "boundary", "missing", "index", "boundary:resident_weight_object"):
...
def test_case_study_emits_p12_artifacts(tmp_path):
    ...
    for f in (
        ...
        assert (tmp_path / f).is_file(), f"missing {f}"
    contracts = load_yaml(tmp_path / "boundary_candidate_contracts.yaml")["boundary_candidate_contracts"][
        "certificates"
    ]
...
@pytest.mark.skipif(not (_CS_DIR / "dse_contract.json").is_file(), reason="case_study package not present")
...
def test_insight_consistency_and_no_forbidden_wording():
    ...
    assert all(ok for ok, _ in b["consistency_checks"]), [m for ok, m in b["consistency_checks"] if not ok]
...
def test_insight_mining_is_deterministic():
    ...
    assert IM.presentation_findings(f, IM.usefulness(_CS_DIR, "rdt", f)) == IM.presentation_findings(
        f, IM.usefulness(_CS_DIR, "rdt", f)
    )
...
def test_p14_main_findings_corroborated_and_measured_lowbit_surfaced():
    ...
    assert main
    for f in main:
        check = any(x["verifying_check"] for m in f["relevant_metrics"] for x in b["facts"] if x["metric_name"] == m)
...
def test_p14_signal_vs_context_candidacy():
    ...
    ctx = IM._fact(
        "F0",
        workload="w",
        metric="n_runtime_objects",
        value=3,
        unit="count",
        artifact="runtime_object_candidates.yaml",
        phase="P12",
        evidence="recovered_from_ir",
        implication="objects crossing the HAL",
        corroborated_by=2,
    )
...
def test_p15_abstraction_coverage_recomputes_and_flags_overfit():
    ...
    for r in cov:
        ...
        assert r["overfit_risk"] == ("high" if len(supp) <= 1 else "low" if len(supp) == nwl else "medium")
...
def test_p15_emit_writes_study_deliverables_and_plot_captions(tmp_path):
    ...
    for f in (
        "canonical_signal_table.csv",
        "per_operator_hotspots.csv",
        "abstraction_coverage_table.csv",
        "workload_family_summary.md",
        "corpus_expansion_plan.md",
        "signal_findings_report.md",
    ...
    for section in (
        "Headline metrics",
        "Top ops by MACs",
        "Abstraction necessity (strict",
        "Decision-impact plots",
        "What a DSE tool ingests",
...
def test_p15_canonical_table_has_entity_discriminator_and_no_dups(tmp_path):
    ...
    keys = [(r["dse_question"], r["metric"], r["workload"], r["entity"], str(r["value"])) for r in canon]
...
def test_p16_necessity_recomputes_from_source():
    ...
    shape = list(_csv.DictReader(_io.StringIO((_CS_DIR / "shape_summary_by_workload.csv").read_text())))
    nec = IM.abstraction_necessity(_CS_DIR)
...
def test_p16_operator_pareto_and_leave_one_out():
    ...
    assert sorted(dom["collapses_if_removed"]) == sorted(w for w in wls if dom["micro_loo"][w] < 0.2 <= dom["micro"])
...
def test_p16_capture_fidelity_and_emit(tmp_path):
    ...
    IM.emit_run(b, tmp_path, [])
    for f in (
        "abstraction_necessity_table.csv",
        "primitive_set_frontier.csv",
        "operator_pareto_hotspots.csv",
        "capture_fidelity_matrix.csv",
        "decision_question_scorecard.md",
        "leave_one_workload_out.md",
...
def test_p17_gemv_abstraction_renamed_and_split():
    ...
    assert cells and all(
        "true_gemv=" in r["predicate_inputs"] and "skinny_gemm=" in r["predicate_inputs"] for r in cells
    )
...
def test_p17_frontier_robustness_recompute_monotone_and_extra_tiles():
    ...
    ops = {
        (o["workload"], o["op_index"]): o
        for o in _csv.DictReader(_io.StringIO((_CS_DIR / "operator_shape_table.csv").read_text()))
    }
    mism = 0
...
def test_p17_adversarial_audit_and_emit(tmp_path):
    ...
    IM.emit_run(b, tmp_path, [])
    for f in (
        "predicate_audit_table.csv",
        "predicate_audit_table.md",
        "conclusion_validity_table.csv",
        "adversarial_audit_report.md",
        "primitive_frontier_robustness.csv",
        "primitive_frontier_robustness.md",
        "uncovered_ops_by_primitive_set.csv",
        "macro_micro_influence_table.csv",
        "leave_one_out_delta_table.csv",
...
def test_p17_timing_envelope_is_derived_requirement():
    ...
    assert env["rows"]
    req = {
        (r["workload"], r["region"], r["requirement"]): r["value"]
        for r in _csv.DictReader(_io.StringIO((_CS_DIR / "requirements_table.csv").read_text()))
    }
    for r in env["rows"]:
...
@pytest.mark.skipif(
    not (_CS_DIR / "dse_contract.json").is_file() or not _any_flat_capture_present(),
...
@pytest.mark.skipif(
    not (_CS_DIR / "capture_level_ablation.csv").is_file(), reason="capture-level ablation summary not present"
)
...
@pytest.mark.skipif(not (_CS_DIR / "timeloop_problem_shapes.yaml").is_file(), reason="mapspace seeds not present")
...
@pytest.mark.skipif(not (_CS_DIR / "operand_locality_table.csv").is_file(), reason="operand locality not present")
def test_p20_operand_locality_and_quant_metadata():
    ...
    dm = {
        ...
        for r in _csv.DictReader(_io.StringIO((_CS_DIR / "data_movement_table.csv").read_text()))
    }
...
def test_agent_citation_gate_keeps_grounded_rejects_ungrounded(tmp_path):
    ...
    (tmp_path / "DSE_FINDINGS.md").write_text("# digest\nmatrix_engine is necessary only for rdt (corpus-narrow).\n")
    (tmp_path / "table.csv").write_text("metric,value\navoidable_weight_reload,1564475392\n")
    items = [
        {
            "claim": "matrix_engine presented as general",
            "severity": "high",
            "cite": "matrix_engine is necessary only for rdt",
            "suggested_fix": "say rdt-only",
        },
        {
            "claim": "invented speedup",
            "severity": "high",
            "cite": "the design is 3x faster than baseline",
        ...
        {"claim": "no citation", "severity": "low", "cite": "", "suggested_fix": "x"},
        {
            "claim": "bad severity",
            "severity": "URGENT",
            "cite": "avoidable_weight_reload,1564475392",
            "suggested_fix": "x",
        },
    ]
...
@pytest.mark.skipif(not (_CS_DIR / "loop_aware_contract.csv").is_file(), reason="case_study package not present")
def test_loop_aware_seeds_multiply_repeated_head_by_recovered_K():
    ...
    for r in rows:
        outer = facts.get(r["workload"], {}).get("K", 1) if r["region_role"] in ("repeated_head", "decode_lm") else 1
    ...
    assert all(
        int(r["outer_loop_K"]) == 1 for r in rows if r["workload"] == "openvla" and r["region_role"] == "backbone_once"
    )
