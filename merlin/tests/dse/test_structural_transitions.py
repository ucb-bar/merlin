...
AXES = (
    StructuralAxis("copy_payload", "bytes"),
    StructuralAxis("storage", "bytes"),
...
class LayoutAdapter:
        ...
        return DispatchProgram(
            ...
            [0],
            {
                name: Buffer(
                    ...
                    list(self.shape),
                    self.dtype,
                    "arg" if name == "x" else "intermediate",
                    0 if name == "x" else None,
                )
                for name in ("x", "a", "y")
            },
            [
                Node("dispatch", "producer", ["x"], ["a"], captures=[]),
                Node("dispatch", "consumer", ["a"], ["y"], captures=[]),
            ],
            ["y"],
        )
    ...
    def region_alternatives(self, program):
        def region(name, index, inputs, outputs):
            return RegionAlternative(
                ...
                name,
                self.placement,
                CycleInterval.unknown("no timing calibration"),
                inputs=inputs,
                outputs=outputs,
            )

        return (
            region("producer", 0, (BufferRepresentation("x", self.native),), (BufferRepresentation("a", self.native),)),
            region(
                "consumer_native",
                1,
            ...
            region(
                "consumer_encoded",
                1,
                (BufferRepresentation("a", self.packed),),
                (BufferRepresentation("y", self.native),),
            ),
        )
    ...
    def structural_transition(self, program, *, buffer, producer, consumer, source, destination):
        return strided_copy_transition(
            id="pack",
            buffer=buffer,
            producer=producer.id,
            consumer=consumer.id,
            source_layout=self.source,
            destination_layout=self.destination,
            dtype=self.dtype,
    ...
    def evaluate_structure(self, program, selected, transitions, axes):
        ...
        return StructuralEvaluation(
            (
                quantities.get("scalar_load_payload", 0) + quantities.get("scalar_store_payload", 0),
                quantities.get("destination_storage", 0),
                2 if transitions else 5,
            ),
...
@pytest.mark.parametrize(
    ...
    [
        (
            StaticStridedLayout((2, 3), (3, 1), 12),
            StaticStridedLayout((2, 3), (4, 1), 16),
            "i16",
            "shared_sram",
            24,
            16,
        ),
        (StaticStridedLayout((3, 5), (5, 1), 60), StaticStridedLayout((3, 5), (1, 3), 60), "f32", "host_heap", 120, 60),
    ],
)
...
@pytest.mark.parametrize(
    "layout",
    [
        StaticStridedLayout((2, 3), (2, 1), 12),
        StaticStridedLayout((2, 3), (4, 1), 12),
        StaticStridedLayout((2, -1), (3, 1), 12),
    ],
)
...
def test_legacy_transition_unknown_cost_still_refuses():
    ...
    with pytest.raises(ValueError, match="unresolved"):
        TransitionAlternative("copy", "copy", "x", None, "consumer", rep, rep, CycleInterval.unknown("uncalibrated"))
...
def test_no_implicit_dtype_quantization_or_transport_conversion():
    ...
    transition = strided_copy_transition(
        id="t",
        buffer="a",
        producer="p",
        consumer="c",
        source_layout=a,
        destination_layout=b,
        dtype="i16",
        placement="memory",
        provenance=("static contract",),
    )
    for representation in (
        replace(transition.destination, dtype="i8"),
...
def test_unknown_cycle_transition_uses_existing_exact_emission_accounting():
    adapter = LayoutAdapter(
        (2, 3), "i16", StaticStridedLayout((2, 3), (3, 1), 12), StaticStridedLayout((2, 3), (4, 1), 16), "memory"
    )
    ...
    dispatch = DispatchProgram(
        ...
        [
            ...
            Node("dispatch", "consumer_encoded", ["packed"], ["y"], captures=[]),
        ],
        ["y"],
    )
    receipt = GlobalPlanEmission(
        dispatch,
        plan.digest,
        dispatch_digest(logical),
        (
    ...
    bypassed = replace(receipt, regions=(receipt.regions[0], replace(receipt.regions[1], inputs=(("a", "a"),))))
