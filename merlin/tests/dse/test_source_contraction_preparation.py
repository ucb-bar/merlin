...
def fixture(tmp_path, monkeypatch):
    ...
    def artifact(arm):
        cb = {
            "params": {
                "global_program_plan": {"tasks": [{"task_index": 0, "kind": "contraction", "source_op_indices": [0]}]}
            }
        }
        ...
        return {
            ...
            "candidate_sha256": arm * 64,
            "candidate_lowered_sha256": text_digest(llvm),
            "candidate_command_buffer_sha256": text_digest(raw),
        }
    ...
    def proof(artifact):
        return {
            ...
            "plan_digest": document_digest(P.program_plan(artifact["command_buffer"])),
        }
    ...
    baseline_binding = {
        ...
        "source_sha256": text_digest(SOURCE),
        "compiler_sha256": before["candidate_sha256"],
        "lowered_sha256": before["candidate_lowered_sha256"],
        "command_buffer_sha256": before["candidate_command_buffer_sha256"],
        "host_verifier_policy_sha256": policy,
        "evidence_sha256": document_digest(prior_proof),
    }
    calls, checks = [], []
            ...
            cb = {
                ...
                "params": {
                    "global_program_plan": {
                        ...
                        "entry_bindings": ["A", "B", "C"],
                        "output_bindings": ["Y"],
                    }
                },
            }
            return {
                "lowered": subprocess.CompletedProcess([], 0, arm + " short llvm", ""),
                "command_buffer_emission": subprocess.CompletedProcess([], 0, "", ""),
                "command_buffer": cb,
            }
    ...
    probes = SimpleNamespace(
        ...
        compile_optimization_baseline_probe_candidate=compile("baseline"),
        compile_probe_candidate=compile("current"),
        analysis=SimpleNamespace(
            session=SimpleNamespace(
                journal=SimpleNamespace(
                    iterations=[
                        {"analysis": {"diagnostics": {"verified_global_plan_emission": prior_proof}}},
                        {
                            "analysis": {
                                "diagnostics": {
                                    "verified_global_plan_emission": current_proof,
                ...
                inputs=SimpleNamespace(host_policy={"sha256": policy}),
                current_artifacts=lambda _: after,
                previous_artifacts=lambda _: before,
                ...
                optimization_baseline_artifacts=lambda _: {
                    **before,
                    "compiler_sha256": before["candidate_sha256"],
                    "lowered_sha256": before["candidate_lowered_sha256"],
                    "command_buffer_sha256": before["candidate_command_buffer_sha256"],
    ...
    return probes, calls, checks


def prepare(tmp_path, probes, **kwargs):
    return P.prepare_source_contraction(
        candidate=tmp_path,
        probes=probes,
        comparison_arm=kwargs.pop("comparison_arm", "previous"),
        source_op_index=kwargs.pop("source_op_index", 0),
        entry="work",
        max_m=2,
        max_n=3,
        max_k=5,
...
def test_exact_normal_arms_three_input_abi_and_independent_typed_oracle(tmp_path, monkeypatch, arm):
    probes, calls, checks = fixture(tmp_path, monkeypatch)
    if arm == "optimization_baseline":
        probes.analysis.session.journal.iterations = probes.analysis.session.journal.iterations[-1:]
        probes.analysis.session.previous_artifacts = lambda _: pytest.fail("baseline is not predecessor")
    result = prepare(tmp_path, probes, comparison_arm=arm)
...
def test_invalid_full_pair_never_compiles_or_reverifies_model(tmp_path, monkeypatch, mutation):
    ...
    kwargs = {}
    if mutation == "proof":
        probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"]["verified_global_plan_emission"][
            "status"
        ] = "UNKNOWN"
    elif mutation == "nonplan_buffer":
        probes.analysis.session.current_artifacts(None)["command_buffer"]["tensors"] = {"forged": True}
    elif mutation == "ownership":
        artifact = probes.analysis.session.current_artifacts(None)
        ...
        proof = probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"][
        ...
        probes.analysis.session.inputs.host_policy["sha256"] = "changed"
        kwargs["comparison_arm"] = "optimization_baseline"
    result = prepare(tmp_path, probes, **kwargs)
    assert result["status"] == "UNKNOWN" and calls == checks == []


@pytest.mark.parametrize(
    "mutation",
    ["missing_initializer_arg", "wrong_owner", "wrong_abi", "failed_proof", "compile_timeout", "changed_source"],
)
def test_short_boundary_and_compilation_fail_closed(tmp_path, monkeypatch, mutation):
    probes, calls, _ = fixture(tmp_path, monkeypatch)
    original = probes.compile_previous_probe_candidate
...
def test_changed_current_binding_never_returns_prepared(tmp_path, monkeypatch):
    probes, _, _ = fixture(tmp_path, monkeypatch)
    original = probes.compile_probe_candidate

    def change(*args, **kwargs):
        result = original(*args, **kwargs)
        probes.analysis.session.current_probe_binding = lambda _: object()
        return result

    probes.compile_probe_candidate = change
    assert prepare(tmp_path, probes)["status"] == "UNKNOWN"


def test_explicit_host_to_contraction_route_is_reproduced_by_short_arms(tmp_path, monkeypatch):
    probes, _, _ = fixture(tmp_path, monkeypatch)
    before = probes.analysis.session.previous_artifacts(None)
    ...
    proof = probes.analysis.session.journal.iterations[0]["analysis"]["diagnostics"]["verified_global_plan_emission"]
    ...
    compile_before = probes.compile_previous_probe_candidate
    ...
    probes.compile_previous_probe_candidate = host_short
    result = prepare(tmp_path, probes, expected_route_transition=("host", "contraction"))
    assert result["status"] == "prepared", result
...
def test_selected_secondary_member_binds_source_pair_without_primary_artifacts(tmp_path, monkeypatch):
    probes, _, _ = fixture(tmp_path, monkeypatch)
    before, after = (
        probes.analysis.session.previous_artifacts(None),
        probes.analysis.session.current_artifacts(None),
    )
    ...
    selected = {
        ...
        "previous": {
            "artifacts": before,
            "analysis": probes.analysis.session.journal.iterations[0]["analysis"],
            "member_binding": {"arm": "previous", "source_sha256": text_digest(SOURCE)},
        },
        "current": {
            "artifacts": after,
            "analysis": probes.analysis.session.journal.iterations[1]["analysis"],
            "member_binding": {"arm": "current", "source_sha256": text_digest(SOURCE)},
        },
    }
    probes.analysis.session.selected_changed_portfolio_context = lambda _candidate, supplied: (
        selected if supplied == selection else pytest.fail("substituted member")
    )
    probes.analysis.session.current_artifacts = lambda _: pytest.fail("primary current artifact read")
    probes.analysis.session.previous_artifacts = lambda _: pytest.fail("primary prior artifact read")
    result = prepare(tmp_path, probes, portfolio_member=selection)
    assert result["status"] == "prepared", result
    assert result["comparison_binding"] == {
        "schema": "selected_portfolio_source_program_pair_v1",
        "selection": selection,
...
def test_separate_initializer_admission_keeps_declared_refusal_and_requires_exact_pins(tmp_path, monkeypatch, mutation):
    ...
    def admit(**kwargs):
        result = {
            ...
            "source_sha256": text_digest(kwargs["source_text"]),
            "lowered_sha256": text_digest(kwargs["lowered_text"]),
            "command_buffer_sha256": document_digest(kwargs["command_buffer"]),
            "candidate_sha256": kwargs["candidate_sha256"],
            "declared_proof_sha256": document_digest(kwargs["declared_proof"]),
        }
        if mutation == "unproved":
            result["status"] = "UNKNOWN"
        elif mutation is not None:
            key = {
                "source": "source_sha256",
                "llvm": "lowered_sha256",
                "buffer": "command_buffer_sha256",
                "compiler": "candidate_sha256",
                "declared_proof": "declared_proof_sha256",
            }[mutation]
            result[key] = "stale"
        return result
    ...
    if mutation is None:
        ...
        assert all(
            arm["short_execution_admission"]["status"] == "source_bound_numerical_probe"
...
def test_invalid_budget_not_silently_clamped(tmp_path, monkeypatch, timeout):
    ...
    with pytest.raises(ValueError, match="budget"):
        prepare(tmp_path, probes, timeout_s=timeout)
