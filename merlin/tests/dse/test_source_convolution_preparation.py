...
def fixture(tmp_path, monkeypatch):
    ...
    def artifact(sha, kind):
        ...
        return {
            "interface": source,
            "lowered_text": "module " + kind,
            "command_buffer": cb,
            "command_buffer_text": raw,
            "candidate_sha256": sha,
            "candidate_lowered_sha256": P._sha("module " + kind),
    ...
    def proof(artifact):
        return {
            "status": "verified",
            "source_sha256": P._sha(source.read_text()),
            ...
            "plan_digest": P._digest(P._plan(artifact["command_buffer"])),
        }
    ...
    evidence_binding = {
        ...
        "compiler_sha256": before_sha,
        "host_verifier_policy_sha256": policy_sha,
        "evidence_sha256": P._digest(before_proof),
    }
            ...
            cb = {
                ...
                "params": {
                    "global_program_plan": {
                        ...
                        "entry_bindings": ["input", "weight"],
                        "output_bindings": ["output"],
                    }
                },
            }
            return {
                "lowered": subprocess.CompletedProcess([], 0, "short " + kind, ""),
    ...
    probes = SimpleNamespace(
        ...
        compile_previous_probe_candidate=compile_arm("host"),
        compile_probe_candidate=compile_arm("convolution"),
        analysis=SimpleNamespace(
            session=SimpleNamespace(
                journal=SimpleNamespace(
                    iterations=[
                        {"analysis": {"diagnostics": {"verified_global_plan_emission": before_proof}}},
                        {
                            "analysis": {
                                "diagnostics": {
                                    "captured_logical_graph": {"logical_dispatch_digest": graph_sha},
                                    "verified_global_plan_emission": after_proof,
                                    "verified_baseline_global_plan_emission": before_proof,
                                    "baseline_global_plan_evidence_binding": evidence_binding,
                                }
                            }
                        },
                    ]
                ),
                inputs=SimpleNamespace(host_policy={"sha256": policy_sha}),
                current_artifacts=lambda _: after,
                previous_artifacts=lambda _: before,
                current_probe_binding=lambda _: current_binding,
                previous_probe_binding=lambda _: previous_binding,
    ...
    monkeypatch.setattr(
        ...
        "observe_model_macs",
        lambda text, **kwargs: [
            (None, SimpleNamespace(source_op_index=6 if text == source.read_text() else 2, status="derived", macs=1000))
        ],
    )
    extraction = {
        "schema": "actual_source_convolution_witness_v1",
        "input_layout": "nchw",
        "weight_layout": "oihw",
        "output_layout": "nchw",
        "input_shape": [1, 1, 2, 2],
    ...
    monkeypatch.setattr(
        P, "extract_source_convolution", lambda *args, **kwargs: ("short source", copy.deepcopy(extraction))
    )
    verified = []
...
def test_prepares_only_short_normal_manifest_arms_and_preserves_reference(tmp_path, monkeypatch, comparison_arm):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    if comparison_arm == "optimization_baseline":
        ...
        probes.analysis.session.previous_artifacts = lambda _: pytest.fail(
            "initial comparison is not a previous iteration"
        )
        probes.compile_previous_probe_candidate = lambda *a, **kw: pytest.fail("wrong arm")
    else:
        probes.analysis.session.optimization_baseline_artifacts = lambda _: pytest.fail("wrong arm")
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
...
def test_stale_full_plan_refuses_without_any_compilation(tmp_path, monkeypatch, field):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    proof = probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"]["verified_global_plan_emission"]
    proof[field] = "stale"
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
        comparison_arm="previous",
        entry="compute",
        output=tmp_path / "prepared",
    )
...
def test_missing_baseline_cached_policy_proof_is_not_recomputed(tmp_path, monkeypatch):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    del probes.analysis.session.journal.iterations[-1]["analysis"]["diagnostics"][
        "baseline_global_plan_evidence_binding"
    ]
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
...
def test_nonplan_command_buffer_mutation_refuses_before_compilation(tmp_path, monkeypatch, field):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    probes.analysis.session.current_artifacts(tmp_path)["command_buffer"][field] = {"tampered": True}
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
        comparison_arm="previous",
...
def test_original_raw_and_canonical_hashes_are_not_interchangeable(tmp_path, monkeypatch):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    artifact = probes.analysis.session.current_artifacts(tmp_path)
...
def test_short_route_must_match_changed_full_source_owner(tmp_path, monkeypatch):
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    probes.compile_probe_candidate = probes.compile_previous_probe_candidate
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
        comparison_arm="previous",
...
def test_short_compiler_timeout_retains_unknown_preparation_receipt(tmp_path, monkeypatch):
    ...
    probes.compile_previous_probe_candidate = timeout
    result = P.prepare_source_convolution(
        candidate=tmp_path,
        probes=probes,
        comparison_arm="previous",
...
def test_shared_pair_binder_reuses_exact_model_evidence_without_running_compiler(tmp_path, monkeypatch, comparison_arm):
    ...
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    pair = bind_source_program_pair(candidate=tmp_path, probes=probes, comparison_arm=comparison_arm)
    ...
    expected = (
        probes.compile_optimization_baseline_probe_candidate
        if comparison_arm == "optimization_baseline"
        else probes.compile_previous_probe_candidate
    )
...
def test_shared_pair_binder_refuses_invalid_or_exceeded_source_bounds(tmp_path, monkeypatch, limit):
    ...
    with pytest.raises(ValueError, match="bound"):
        bind_source_program_pair(candidate=tmp_path, probes=probes, comparison_arm="previous", max_source_bytes=limit)
    assert calls == verified == []
...
def test_shared_pair_binder_does_not_substitute_baseline_for_missing_previous(tmp_path, monkeypatch):
    ...
    probes, calls, verified = fixture(tmp_path, monkeypatch)
    probes.analysis.session.journal.iterations = probes.analysis.session.journal.iterations[-1:]
    probes.analysis.session.optimization_baseline_artifacts = lambda _: pytest.fail("implicit comparator substitution")
    with pytest.raises(ValueError, match="preceding analyzed iteration"):
        bind_source_program_pair(candidate=tmp_path, probes=probes, comparison_arm="previous")
    assert calls == verified == []
