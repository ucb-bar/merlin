...
def emitter(*, looped=96, prologue=12, k_step=24, epilogue=18) -> PF.EmitterShape:
    return PF.EmitterShape(
        prologue_words=prologue,
        k_step_words=k_step,
        tile_epilogue_words=epilogue,
        looped_words=looped,
...
class TestRefusesTheOversizedShape:
    ...
    def test_a_footprint_that_wraps_the_window_is_refused(self):
        ...
        pf = PF.preflight_matmul(
            "oversized", m=512, k=1024, n=512, budget=budget(), rate=SINGLE_POINT, emitter=emitter()
        )
        assert not pf.ok
    ...
    def test_the_largest_fitting_shape_is_not_refused(self):
        ...
        pf = PF.preflight_matmul(
            "at_the_ceiling", m=416, k=832, n=416, budget=budget(), rate=SINGLE_POINT, emitter=emitter()
        )
    ...
    def test_no_oracle_and_no_target_are_touched(self):
        ...
        pf = PF.preflight_matmul("free", m=64, k=64, n=64, budget=budget(), rate=SINGLE_POINT, emitter=emitter())
...
class TestImemRefusal:
    def test_an_unrolled_program_over_capacity_is_refused(self):
        ...
        pf = PF.preflight_matmul(
            "unrolled_layer", m=416, k=832, n=416, budget=budget(), rate=SINGLE_POINT, emitter=emitter(), loops=False
        )
        assert not pf.ok
    ...
    def test_the_same_shape_is_fine_when_the_emitter_loops(self):
        looped = PF.preflight_matmul(
            "looped_layer", m=416, k=832, n=416, budget=budget(), rate=SINGLE_POINT, emitter=emitter(), loops=True
        )
    ...
    def test_an_unknown_capacity_refuses_as_UNCHECKED_not_as_a_pass(self):
        pf = PF.preflight_matmul(
            "no_imem_fact", m=64, k=64, n=64, budget=budget(imem=None), rate=SINGLE_POINT, emitter=emitter()
        )
        assert not pf.ok
...
class TestUncheckedIsNeverAPass:
    def test_an_unknown_dram_window_refuses(self):
        pf = PF.preflight_matmul(
            "no_window_fact", m=64, k=64, n=64, budget=budget(window=None), rate=SINGLE_POINT, emitter=emitter()
        )
        assert not pf.ok
    ...
    def test_a_partial_tile_is_refused_before_a_footprint_is_even_computed(self):
        pf = PF.preflight_matmul("ragged", m=33, k=64, n=64, budget=budget(), rate=SINGLE_POINT, emitter=emitter())
        assert not pf.ok
...
class TestRateHonesty:
    ...
    def test_a_projection_reports_how_far_past_the_evidence_it_reaches(self):
        ...
        rendered = PF.render(
            PF.preflight_matmul("big", m=416, k=832, n=416, budget=budget(), rate=SINGLE_POINT, emitter=emitter())
        )
...
class TestProjectedWall:
    ...
    def law(self, *, per_cycle: float, per_word: float, fixed: float = 1.0) -> CostLaw:
        return CostLaw(
            substrate="fixture",
            concurrency=1,
            fixed=Term("fixed", fixed, "s", Provenance.MEASURED, "floor probe", n=3, domain=(1.0, 1.0)),
            per_cycle=Term(
                "per_cycle", per_cycle, "s/cycle", Provenance.MEASURED, "cycle probe", n=4, domain=(0.0, 4_000_000.0)
            ),
            per_word=Term(
                "per_word", per_word, "s/word", Provenance.MEASURED, "halt-first probe", n=4, domain=(0.0, 200_000.0)
            ),
        )
    ...
    def test_both_terms_reach_the_projection(self):
        ...
        pf = PF.preflight_matmul(
            "priced", m=416, k=832, n=416, budget=budget(), rate=SINGLE_POINT, emitter=emitter(), laws={"L3": law}
        )
        est = pf.wall["L3"]
    ...
    def test_a_law_missing_the_word_term_projects_a_LOWER_BOUND(self):
        law = CostLaw(
            ...
            per_cycle=Term(
                "per_cycle", 1e-4, "s/cycle", Provenance.MEASURED, "cycle probe", n=4, domain=(0.0, 4_000_000.0)
            ),
            per_word=Term("per_word", None, "s/word", Provenance.UNKNOWN, "no load probe was run"),
        )
        pf = PF.preflight_matmul(
            "half_priced", m=64, k=64, n=64, budget=budget(), rate=SINGLE_POINT, emitter=emitter(), laws={"L3": law}
        )
    ...
    def test_the_cheapest_tier_is_the_one_a_budget_should_buy_first(self):
        laws = {"cheap": self.law(per_cycle=1e-6, per_word=1e-5), "dear": self.law(per_cycle=1e-3, per_word=1e-3)}
        pf = PF.preflight_matmul(
            "two_tiers", m=64, k=64, n=64, budget=budget(), rate=SINGLE_POINT, emitter=emitter(), laws=laws
        )
...
class TestUsefulBytesFromDeclaredShapes:
    ...
    def test_a_declaration_with_no_shape_raises_rather_than_sizing_it_as_nothing(self):
        with pytest.raises(ValueError):
            PF.operands_from_declaration(
                [{"name": "X", "role": "input", "dtype": "bf16"}], element_bytes_of=lambda d: 2
            )
...
class TestSerialization:
    def test_the_record_says_out_loud_that_its_cycles_are_an_extrapolation(self):
        d = PF.preflight_matmul(
            "rec", m=64, k=64, n=64, budget=budget(), rate=SINGLE_POINT, emitter=emitter()
        ).as_dict()
    ...
    def test_a_refusal_survives_serialization_with_its_proven_flag(self):
        d = PF.preflight_matmul(
            "rec", m=512, k=1024, n=512, budget=budget(), rate=SINGLE_POINT, emitter=emitter()
        ).as_dict()
