...
from merlin.perf.source_convolution_witness import evaluate_source_convolution, extract_source_convolution
...
def test_reference_refuses_wrong_operand_width():
    ...
    with pytest.raises(ValueError, match="signed i8"):
        evaluate_source_convolution(
            record, np.zeros(record["input_shape"], dtype=np.uint8), np.zeros(record["weight_shape"], dtype=np.int8)
        )
