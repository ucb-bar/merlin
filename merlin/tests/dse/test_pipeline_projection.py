...
def _stage(name, resource, kind, cycles, **kwargs):
    return PipelineStage(name, resource, kind, CycleInterval.point(cycles, f"warm reduced {name} witness"), **kwargs)


def test_full_repetition_uses_fill_plus_initiation_interval_not_full_simulation() -> None:
    projection = project_pipeline(
        (
            _stage("load", "reader", "movement", 40, moved_bytes_per_item=4096, commands_per_item=1),
            _stage("execute", "array", "compute", 80),
            _stage("store", "writer", "movement", 20, moved_bytes_per_item=1024, commands_per_item=1),
        ),
        repetitions=513,
    )
    ...
    assert projection.occupancy.busy == {"array": 513 * 80, "reader": 513 * 40, "writer": 513 * 20}
...
def test_large_projection_keeps_exact_latency_but_marks_overlap_unknown() -> None:
    projection = project_pipeline(
        ...
        repetitions=100,
        policy=PipelineProjectionPolicy(max_overlap_intervals=10),
    )
...
def test_buffer_ownership_distinguishes_serialization_from_latency_hiding() -> None:
    stages = (_stage("load", "reader", "movement", 40), _stage("execute", "array", "compute", 80))
    single = project_pipeline(
        stages, 10**12, buffers=(PipelineBuffer("load", "execute", 1, "one source allocation", 4096),)
    )
    double = project_pipeline(
        stages, 10**12, buffers=(PipelineBuffer("load", "execute", 2, "two disjoint source slots", 4096),)
    )
    assert single.cycles.hi == 120 * 10**12
    assert double.cycles.hi == 120 + (10**12 - 1) * 80
    assert single.occupancy.compute_utilization == 2 / 3
    assert double.occupancy.compute_utilization > 0.99
...
def test_powered_completion_matches_small_explicit_slot_schedule(durations, slots) -> None:
    ...
    stages = tuple(_stage(str(i), f"resource{i}", "compute", p) for i, p in enumerate(durations))
    buffers = tuple(PipelineBuffer(str(i), str(i + 1), count, "fixture allocation") for i, count in enumerate(slots))
    completions = []
    for item in range(9):
        row = []
        for j, duration in enumerate(durations):
            ready = [row[j - 1] if j else 0, completions[-1][j] if completions else 0]
...
def test_interval_floor_uses_lower_cost_and_idealization_is_explicit() -> None:
    stages = (
        PipelineStage("a", "r1", "movement", CycleInterval(2, 10)),
