...
class TestTheSameProgramIsNotAComparison:
    ...
    def test_it_is_checked_BEFORE_any_metric_however_good_the_ratio(self):
        ...
        got = CD.compare(BASE, same, axes=[Axis("wall_cycles", True)], metrics=[_metric(base=1000.0, cand=1.0)])
        assert got.verdict == CD.IDENTICAL_EMISSION
    ...
    def test_absent_digests_do_not_count_as_matching(self):
        ...
        got = CD.compare(_arm("b"), _arm("c"), axes=[Axis("host_work", True)], metrics=[_metric()])
        assert got.verdict != CD.IDENTICAL_EMISSION
...
class TestAnInstrumentBlindToTheChangeCannotDecide:
    def test_a_metric_blind_to_every_moved_axis_is_excluded_and_the_change_is_BLIND(self):
        ...
        got = CD.compare(
            BASE,
            CAND,
            axes=[Axis("traffic", True, "169.52 MiB on both arms")],
            metrics=[
                _metric("host_instructions", base=914191598.0, cand=556761305.0, blind=("traffic",), instrument="spike")
            ],
        )
    ...
    def test_a_sighted_metric_beside_a_blind_one_still_decides(self):
        got = CD.compare(
            BASE,
            CAND,
            axes=[Axis("traffic", True)],
            metrics=[
                _metric("host_instructions", blind=("traffic",)),
                _metric("bytes_moved", base=200.0, cand=100.0, instrument="lane_cost"),
            ],
        )
    ...
    def test_a_duplicated_axis_is_refused(self):
        with pytest.raises(ValueError, match="declared twice"):
            CD.compare(BASE, CAND, axes=[Axis("traffic", True), Axis("traffic", False)], metrics=[_metric()])
...
class TestBandsEliminateAndNeverCertify:
    def _band(self, *, b, c, name="band"):
        return Metric(
            name=name,
            instrument="compose_estimate.band",
            baseline=sum(b) / 2,
            candidate=sum(c) / 2,
            baseline_interval=b,
            candidate_interval=c,
        )

    def test_overlapping_bands_are_UNKNOWN_not_a_tie(self):
        got = CD.compare(
            BASE, CAND, axes=[Axis("offload", True)], metrics=[self._band(b=(100.0, 5000.0), c=(120.0, 6000.0))]
        )
    ...
    def test_a_disjoint_band_ABOVE_the_baseline_eliminates_the_candidate(self):
        got = CD.compare(
            BASE, CAND, axes=[Axis("offload", True)], metrics=[self._band(b=(100.0, 200.0), c=(300.0, 400.0))]
        )
    ...
    def test_a_disjoint_band_FAVOURING_the_candidate_is_still_not_a_win(self):
        ...
        got = CD.compare(
            BASE, CAND, axes=[Axis("offload", True)], metrics=[self._band(b=(300.0, 400.0), c=(100.0, 200.0))]
        )
        assert got.verdict == CD.UNKNOWN
    ...
    def test_a_point_metric_beside_a_favouring_band_does_decide(self):
        got = CD.compare(
            BASE,
            CAND,
            axes=[Axis("offload", True)],
            metrics=[self._band(b=(300.0, 400.0), c=(100.0, 200.0)), _metric("cycles", instrument="gsim")],
        )
    ...
    def test_the_real_band_width_cannot_separate_two_close_arms(self):
        ...
        got = CD.compare(
            BASE, CAND, axes=[Axis("offload", True)], metrics=[self._band(b=(1000.0, 211500.0), c=(500.0, 105750.0))]
        )
        assert got.verdict == CD.UNKNOWN
...
class TestTheVerdictQuotesTheWeakestImprovement:
    def test_agreeing_instruments_yield_BETTER_quoting_the_weakest(self):
        got = CD.compare(
            BASE,
            CAND,
            axes=[Axis("wall_cycles", True)],
            metrics=[
                _metric("a", base=100.0, cand=10.0, instrument="flattering"),
    ...
    def test_disagreeing_instruments_yield_UNKNOWN_and_name_both(self):
        got = CD.compare(
            ...
            axes=[Axis("wall_cycles", True)],
            metrics=[
                _metric("a", base=100.0, cand=50.0, instrument="one"),
    ...
    def test_every_instrument_regressing_yields_WORSE(self):
        got = CD.compare(
            ...
            CAND,
            axes=[Axis("wall_cycles", True)],
            metrics=[_metric(base=100.0, cand=105.0), _metric("b", base=100.0, cand=200.0)],
        )
    ...
    def test_a_measured_zero_is_NO_EFFECT_and_says_it_is_not_unmeasured(self):
        got = CD.compare(BASE, CAND, axes=[Axis("wall_cycles", True)], metrics=[_metric(base=100.0, cand=100.0)])
    ...
    def test_no_axis_moving_is_NO_EFFECT_rather_than_a_ratio(self):
        got = CD.compare(
            BASE, CAND, axes=[Axis("traffic", False), Axis("offload", False)], metrics=[_metric(base=100.0, cand=10.0)]
        )
    ...
    def test_a_non_positive_reading_yields_no_ratio_and_no_verdict(self):
        got = CD.compare(BASE, CAND, axes=[Axis("wall_cycles", True)], metrics=[_metric(base=0.0, cand=50.0)])
...
class TestTheRecordIsReadable:
    def test_the_decision_serialises_with_its_licence_and_moved_axes(self):
        got = CD.compare(
            ...
            metrics=[_metric("bytes_moved", base=169.52, cand=84.0, instrument="lane_cost")],
        )
        d = got.to_dict()
