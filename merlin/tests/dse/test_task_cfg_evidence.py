...
def _check(body, tasks, *, globals_=""):
    ...
    module = parse_mlir_text(
        "builtin.module {" + globals_ + " llvm.func @kernel(%p: !llvm.ptr, %c: i1) {" + body + "} }", context
    )
...
def test_unreachable_cfg_keeps_legacy_refusal_evidence():
    ...
    assert analyze_task_cfg(function, [0], prepared_cfg=prepared) == analyze_task_cfg(function, [0])
    assert (
        "owned kernel contains unreachable blocks" in analyze_task_cfg(function, [0], prepared_cfg=prepared)["problems"]
    )
