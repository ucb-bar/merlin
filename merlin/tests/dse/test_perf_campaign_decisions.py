...
def _bound(partial: float, *, demand_unknown: bool = False) -> Composed:
    ...
    return Composed(
        cycles=UNKNOWN if unresolved else partial,
...
def _eligible():
    return assess_eligibility(_declaration("explicit_dma"), traits={"explicit_dma": True}, emitter_implemented=True)
...
def _screen_evidence(*, fired: bool | None = True, reason: str = "control rejected"):
    ...
    falsifier = (FalsifierEvidence(identity=expected[0], negative_control=True, fired=fired, reason=reason),)
...
def test_trait_false_unknown_and_missing_are_distinct_from_eligible() -> None:
    ...
    refuted = assess_eligibility(
        declaration, traits={"explicit_dma": False, "explicit_completion": None}, emitter_implemented=True
    )
    assert refuted.state == SKIPPED_INAPPLICABLE_FALSE
    assert refuted.details["traits"] == {"explicit_dma": False, "explicit_completion": None}

    unknown = assess_eligibility(declaration, traits={"explicit_dma": True}, emitter_implemented=True)
    ...
    eligible = assess_eligibility(
        declaration, traits={"explicit_dma": True, "explicit_completion": True}, emitter_implemented=True
    )
...
def test_unknown_trait_names_and_non_tri_state_values_fail_closed() -> None:
    with pytest.raises(CampaignDeclarationError, match="unknown performance trait"):
        assess_eligibility(_declaration("made_up"), traits={"made_up": True}, emitter_implemented=True)
    with pytest.raises(CampaignEvidenceError, match="True, False, or None"):
        assess_eligibility(_declaration("explicit_dma"), traits={"explicit_dma": 1}, emitter_implemented=True)


def test_an_applicable_family_with_no_emitter_is_blocked_not_inapplicable() -> None:
    decision = assess_eligibility(
        _declaration("explicit_dma"), traits={"explicit_dma": True}, emitter_implemented=False
    )
...
def test_measurements_require_exact_replica_identities_and_two_points_per_fit() -> None:
    ...
    assert validate_measurements(rows, expected_identities=expected, fitted_parameters=("depth",)) == rows

    with pytest.raises(CampaignEvidenceError, match="missing replica"):
        validate_measurements(rows[:1], expected_identities=expected, fitted_parameters=("depth",))
    with pytest.raises(CampaignEvidenceError, match="duplicate replica"):
        validate_measurements((rows[0], rows[0], rows[1]), expected_identities=expected, fitted_parameters=("depth",))
    with pytest.raises(CampaignEvidenceError, match="at least two distinct measurements"):
        validate_measurements(
            (_measurement(expected[0], 1), _measurement(expected[1], 1)),
...
def test_missing_or_undetermined_negative_control_evidence_is_refused() -> None:
    ...
    common = dict(expected_identities=expected, fitted_parameters=("depth",), base=_bound(10), candidate=_bound(8))
    ...
    mislabelled = decide_promotion(
        _eligible(), rows, falsifier_evidence=(FalsifierEvidence(expected[1], True, True, "wrong member"),), **common
    )
...
def test_fired_control_still_refuses_when_unpriced_demands_do_not_cancel() -> None:
    ...
    decision = decide_promotion(
        ...
        fitted_parameters=("depth",),
        falsifier_evidence=evidence,
        base=_bound(10, demand_unknown=True),
        candidate=_bound(8, demand_unknown=True),
        demands_base={"unpriced": 1},
...
def test_existing_eta_decision_converts_to_a_negative_control_reading() -> None:
    ...
    fired = FalsifierEvidence.from_ab_decision(
        _identity("control", "r0"),
        ab_decision(base, unchanged, bit_exact=True, invariants_held=True),
        negative_control=True,
    )
    did_not_fire = FalsifierEvidence.from_ab_decision(
        _identity("candidate", "r0"),
        ab_decision(base, improved, bit_exact=True, invariants_held=True),
...
def test_completion_requires_promotion_exact_certifying_replicas_and_a_pass() -> None:
    ...
    cert_expected = (_identity("candidate", "c0", "certify"), _identity("candidate", "c1", "certify"))
    cert_rows = (_measurement(cert_expected[0], 1), _measurement(cert_expected[1], 2))

    done = complete_family(
        promoted,
        cert_rows,
        expected_identities=cert_expected,
        fitted_parameters=("depth",),
