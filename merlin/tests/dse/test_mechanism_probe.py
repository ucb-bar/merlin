...
from merlin.perf.mechanism_probe import (
    ...
    ProbeBinding,
    ProbeObservation,
    TimingMeasurementIdentity,
    authorize_probe_timing,
    derive_mechanism_signature,
    extract_mechanism_evidence,
...
def signature(*, dtype="i8", encoding="blocked", shared=False, regime="fits_double", edges=("aligned",), dep=True):
    return derive_mechanism_signature(
        ...
        capacity_regime={"local-store": regime},
        tile_shape=(8, 8, 16),
        edge_cases=edges,
        repetition_semantics="independent output tiles with resident weights",
        instruction_semantics=[
            {"operation": "operand-load", "operand": "activation"},
...
def timing_authority(**changes):
    ...
    measured_identity = TimingMeasurementIdentity(
        *(digest(name) for name in ("synthetic-engine", "synthetic-configuration", "synthetic-timer-counter-semantics"))
    )
    document = {
        "schema": "host_probe_timing_validation_v1",
        "measurement_identity": measured_identity.__dict__,
        "target_digest": binding().target_digest,
        "mechanism_digest": signature().digest,
        "repetition_domain": [1, 10],
        "systematic_error_cycles": 2,
    }
    document.update(changes)
    reference = {
        "scope": "synthetic analytical fixture; not physical hardware qualification",
        "identity": document["measurement_identity"],
        "target_digest": document["target_digest"],
        "mechanism_digest": document["mechanism_digest"],
        "points": [
            {"repetitions": n, "reference_cycles": 16 + 3 * n, "engine_cycles": 17 + 3 * n} for n in range(1, 11)
        ],
    }
    ...
    return authorize_probe_timing(
        validation_record=document,
        reference_evidence=payload,
        host_validator=validate,
        provenance="host-checked synthetic analytical test fixture",
    )
...
def observation(repetitions, *, cycles=None, sig=None, identity=None, authority=None):
    ...
    receipt = WarmComputeReceipt(
        f"probe-{repetitions}", cycles or (17 + 3 * repetitions), WarmProfileContract(), f"receipt-{repetitions}"
    )
...
def test_fixed_and_rate_fit_returns_empirical_interval_only_within_domain():
    ...
    fitted = fit_probe_calibration(
        [observation(n, authority=authority) for n in (2, 4, 6, 8)],
        current_binding=binding(),
    ...
    interval = fitted.interval(
        binding=binding(), signature=signature(), repetitions=5, current_timing_authority=authority
    )
    ...
    assert not fitted.interval(
        binding=binding(), signature=signature(), repetitions=9, current_timing_authority=authority
    ).resolved  # authority wider than actual samples
    assert not fitted.interval(
        binding=binding(), signature=signature(shared=True), repetitions=4, current_timing_authority=authority
    ).resolved
    assert not fitted.interval(
        binding=replace(binding(), compiler_digest=digest("new")),
        signature=signature(),
        repetitions=4,
        current_timing_authority=authority,
    ).resolved
...
def test_mixed_stale_and_missing_authority_refused():
    ...
    for stale in (
        timing_authority(systematic_error_cycles=3),
        timing_authority(
            measurement_identity=replace(
                authority.measurement_identity, engine_configuration_sha256=digest("other configuration")
            ).__dict__
        ),
    ):
    ...
    with pytest.raises(ValueError, match="missing observation"):
        fit_probe_calibration(
            rows[:3] + [observation(8)], current_binding=binding(), current_timing_authority=authority
        )
    with pytest.raises(ValueError, match="stale timing authority"):
        fit_probe_calibration(
            rows,
            current_binding=binding(),
            current_timing_authority=timing_authority(target_digest=digest("other target")),
        )
    narrow = timing_authority(repetition_domain=[1, 7])
    with pytest.raises(ValueError, match="validity domain"):
        fit_probe_calibration(
            [observation(n, authority=narrow) for n in (2, 4, 6, 8)],
            current_binding=binding(),
            current_timing_authority=narrow,
        )
...
def test_reference_hash_alone_is_not_host_validation():
    ...
    with pytest.raises(ValueError, match="reference evidence changed"):
        authorize_probe_timing(
            ...
            reference_evidence=b"actual reference",
            host_validator=None,
...
def test_fit_rejects_capacity_domain_or_stale_evidence():
    ...
    with pytest.raises(ValueError, match="equivalence domains"):
        fit_probe_calibration(rows + [observation(8, sig=signature(regime="spills"))], current_binding=binding())
    with pytest.raises(ValueError, match="stale"):
        fit_probe_calibration(
            rows + [observation(8, identity=replace(binding(), target_digest=digest("different RTL")))],
            current_binding=binding(),
        )
...
def test_host_extraction_does_not_invent_resource_topology_from_command_counts():
    cb = {"tensors": {"x": {"dtype": "i8", "shape": [8, 8]}}, "commands": [{"opcode": "compute"}]}
    ...
    result = extract_mechanism_evidence(
        artifact=b"actual lowered artifact",
        command_buffer=cb,
        binding=binding(),
        repetitions=4,
        artifact_analyzer=analyzer,
        extraction_provenance="host analyzer",
    )
...
def test_host_extractor_bound_to_actual_bytes_and_decline_blocks_extraction():
    args = dict(
        ...
        binding=binding(),
        repetitions=4,
        artifact_analyzer=lambda _: {"instruction_count": 2, "encoding_resolution": {"status": "resolved"}},
        motif_extractor=lambda artifact, cb: signature(),
        extraction_provenance="host",
    )
