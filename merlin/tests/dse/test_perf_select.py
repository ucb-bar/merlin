...
KINDS = {"move": ResourceKind.MOVEMENT, "arith": ResourceKind.COMPUTE, "unattributed": ResourceKind.FIXED}
...
def _source(*, total=1000, move=700, arith=200, fixed=100, completion=True):
    return activity_from_busy(
        "w",
        total,
        {"move": move, "arith": arith, "unattributed": fixed},
        KINDS,
        partitioned=False,
        completion_observable=completion,
...
def _amp(*, moved=8192, useful=2048, transfers=32):
    return amplification(
        MovementObservation("w", moved_bytes=moved, useful_bytes=useful, transfers=transfers), trait=MOVEMENT_TRAIT
    )
...
def test_dma_axis_propagates_an_unavailable_amplification():
    amp = amplification(MovementObservation("w", moved_bytes=0, useful_bytes=2048, transfers=8), trait=MOVEMENT_TRAIT)
...
def test_candidates_from_does_not_multiply_the_axes_together():
    ...
    assert len(cands) == len(axes[sel.Axis.DMA_TILING].candidates) + len(axes[sel.Axis.OVERLAP].candidates)
...
def test_rank_retains_unscorable_candidates_but_sorts_them_last():
    good = sel.Candidate(
        sel.Axis.OVERLAP, "w", (("group_a", "arith"), ("group_b", "move")), 1000, 200.0, 0.0, "ceiling"
    )
    blind = sel.Candidate(sel.Axis.DMA_TILING, "w", (("transfers", 4),), 1000, 500.0, UNKNOWN, "interval not split")
...
def test_the_ranking_is_denominated_in_the_scarce_unit_so_cost_can_flip_it():
    ...
    big = sel.Candidate(
        sel.Axis.OVERLAP, "w", (("group_a", "a"), ("group_b", "b")), 1000, 300.0, 0.0, "big but expensive"
    )
    small = sel.Candidate(
        sel.Axis.OVERLAP, "w", (("group_a", "c"), ("group_b", "d")), 1000, 100.0, 0.0, "small but cheap"
    )
    flat = sel.rank([big, small], reference_cycles=1000, budget=_budget())
    assert [v.candidate_id for v in flat] == [big.id, small.id]
    priced = sel.rank([big, small], reference_cycles=1000, budget=_budget(), cost_units={big.id: 10.0, small.id: 1.0})
...
def test_predicted_remaining_below_fires_when_nothing_left_promises_three_percent():
    v = sel.predicted_remaining_below(_state(best_cycles=1000.0, predicted_best_cycles=980.0), POLICY)
    assert v.fired and "2.00%" in v.reason


def test_predicted_remaining_below_does_not_fire_while_a_candidate_promises_more():
    v = sel.predicted_remaining_below(_state(best_cycles=1000.0, predicted_best_cycles=800.0), POLICY)
    assert not v.fired


def test_predicted_remaining_below_does_not_fire_when_no_candidate_carries_a_prediction():
    v = sel.predicted_remaining_below(_state(best_cycles=1000.0, predicted_best_cycles=UNKNOWN), POLICY)
    assert not v.fired
    assert v.missing
...
def _cands(n: int, *, hi: float = 300.0, lo: float = 0.0) -> list[sel.Candidate]:
    return [
        sel.Candidate(
            sel.Axis.OVERLAP,
            "w",
            (("group_a", f"g{i}"), ("group_b", "move")),
            1000,
            hi - i,
            lo,
            f"fixture candidate {i}",
        )
        for i in range(n)
    ]
...
def test_the_loop_stops_once_the_conservative_attainable_target_is_reached():
    ...
    res = sel.search(cands, evaluate=ev, budget=_budget(limit_items=10), baseline_cycles=1000, attainable_cycles=800.0)
...
def test_the_loop_stops_when_nothing_remaining_predicts_three_percent():
    strong = sel.Candidate(
        sel.Axis.OVERLAP, "w", (("group_a", "big"), ("group_b", "move")), 1000, 300.0, 0.0, "worth a query"
    )
    weak = [
        sel.Candidate(
            sel.Axis.OVERLAP, "w", (("group_a", f"t{i}"), ("group_b", "move")), 1000, 5.0, 0.0, "barely moves"
        )
        for i in range(3)
    ]
    ev = FakeEvaluator({strong.id: 700.0})
    res = sel.search([strong, *weak], evaluate=ev, budget=_budget(limit_items=10), baseline_cycles=1000)
...
def test_an_identical_candidate_is_served_from_cache_and_charged_nothing():
    ...
    twin = sel.Candidate(
        ...
        a.saving_hi,
        a.saving_lo,
        "a different rationale, the same point",
    )
...
def test_write_convergence_emits_the_curve_and_reports_a_missing_plot_as_not_run(tmp_path):
    ...
    res = sel.search(_cands(3), evaluate=FakeEvaluator({}), budget=_budget(limit_items=2), baseline_cycles=1000)
...
def test_the_observed_command_count_comes_from_the_amplification_split():
    ...
    amp = amplification(
        MovementObservation(
            "w", moved_bytes=1088, useful_bytes=512, transfer_bytes=(256, 256, 256, 64, 64, 64, 64, 64)
        ),
