...
linalg.yield %v : i8
    ...
    return (
        "builtin.module {func.func @source(%arg:tensor<3x3xi8>)->tensor<3x3xi8>{\n"
        + "\n".join(rows)
...
def declaration(shape=(3, 3), reverse=True):
    return {
        "id": "copy",
        "kind": "static_strided_copy",
        "source_edge": {
            "producer_op_index": 1,
            "producer_result_index": 0,
            "consumer_op_index": 3,
        ...
        "destination_layout": {
            "shape": list(shape),
            "strides_elements": [1, shape[0]] if reverse else [shape[1], 1],
            "offset_elements": 0,
        },
    }
...
def test_dispatch_never_falls_back_after_marked_copy_refusal(marked):
    ...
    if marked == "declaration":
        artifact["command_buffer"]["params"]["global_program_plan"]["physical_transitions"] = [declaration()]
    elif marked == "llvm":
        artifact["lowered_text"] = '"merlin.transition_source" = "copy"'
    elif marked == "malformed":
        artifact["command_buffer"]["params"]["global_program_plan"]["physical_transitions"] = None
    calls = []
    ...
    probes = SimpleNamespace(
        analysis=SimpleNamespace(
            session=SimpleNamespace(previous_artifacts=lambda _: {}, current_artifacts=lambda _: artifact)
        )
    )
    assert dispatch(candidate=Path("candidate"), probes=probes, timeout_s=60)["status"] == "UNKNOWN"
...
def test_dispatch_routes_from_selected_secondary_member_not_primary_artifacts():
    selection = {"schema": "global_changed_portfolio_member_selection_v1", "portfolio_index": 1, "capsule": "secondary"}
    artifact = {
        "command_buffer": {"params": {"global_program_plan": {"physical_transitions": [declaration()]}}},
        "lowered_text": "",
    }
    selected = {
        "selection": selection,
        "previous": {"artifacts": {"command_buffer": {"params": {"global_program_plan": {}}}, "lowered_text": ""}},
    ...
    probes = SimpleNamespace(
        analysis=SimpleNamespace(
            session=SimpleNamespace(
                selected_changed_portfolio_context=lambda _candidate, supplied: (
                    selected if supplied in (None, selection) else (_ for _ in ()).throw(ValueError("substituted"))
                ),
    ...
    assert (
        dispatch(candidate=Path("candidate"), probes=probes, timeout_s=60, portfolio_member=selection)["status"]
        == "UNKNOWN"
    )
...
def experiment_fixture(tmp_path, monkeypatch, *, mode="match"):
    ...
    def cb(rows):
        return {
            "params": {"global_program_plan": {"physical_transitions": rows}},
            "kernel_abi": {
                "kind": "whole_program",
    ...
    before, after = (
        {"interface": source, "lowered_text": llvm, "candidate_lowered_sha256": sha(llvm), "command_buffer": cb(rows)}
        for rows in ([], [declaration()])
    )
    ...
    if mode == "stale":
        iterations[-1]["analysis"]["diagnostics"]["verified_global_plan_emission"]["candidate_lowered_sha256"] = (
            "0" * 64
        )
    ...
    monkeypatch.setattr(
        Q,
        "verify_physical_transitions",
        lambda **kw: {
            "status": "verified"
            if kw["command_buffer"]["params"]["global_program_plan"]["physical_transitions"]
            else "not_declared"
        },
    )
    calls = []
    ...
    def compile_arm(arm, candidate, interface, scratch, **kwargs):
        ...
        if mode == "edge" and rows:
            rows[0]["source_edge"]["consumer_operand_index"] = 1
        return {
            "lowered": SimpleNamespace(returncode=0, stdout=llvm, stderr=""),
            "command_buffer_emission": SimpleNamespace(returncode=0),
            "command_buffer": cb(rows),
        }
    ...
    probes = SimpleNamespace(
        ...
        analysis=SimpleNamespace(
            session=SimpleNamespace(
                ...
                journal=SimpleNamespace(iterations=iterations),
            )
        ),
    )
    return probes, calls
...
def test_both_compilers_must_reproduce_the_changed_source_mechanism(tmp_path, monkeypatch, mode):
    probes, calls = experiment_fixture(tmp_path, monkeypatch, mode=mode)
    provider = Q.HostPhysicalTransitionQualifier(
        native_layout=lambda _: {},
        expected_symbol="kernel",
        abi_provenance={"fixture": "native"},
    ...
    receipt = provider(candidate=Path("candidate"), probes=probes, timeout_s=60)
