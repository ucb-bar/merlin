...
def _serial_tiles():
    return schedule_activity(
        (
            ActivityEvent("load0", "move", "movement", 40, movement_bytes=4096, movement_commands=1),
            ActivityEvent("compute0", "array", "compute", 80, depends_on=("load0",)),
            ActivityEvent(
                "load1", "move", "movement", 40, depends_on=("compute0",), movement_bytes=4096, movement_commands=1
            ),
...
def _pipelined_tiles():
    return schedule_activity(
        (
            ...
            ActivityEvent(
                "load1", "move", "movement", 40, depends_on=("load0",), movement_bytes=4096, movement_commands=1
            ),
            ActivityEvent("compute1", "array", "compute", 80, depends_on=("compute0", "load1")),
        )
    )
...
def test_shared_serial_group_expresses_a_target_that_cannot_overlap_engines() -> None:
    timeline = schedule_activity(
        (
            ...
            ActivityEvent("compute", "array", "compute", 80, depends_on=("load",), serial_group="shared"),
            ActivityEvent("next_load", "move", "movement", 40, depends_on=("load",), serial_group="shared"),
        )
    )
...
def test_encoding_conversion_is_visible_as_movement_not_free_metadata() -> None:
    timeline = schedule_activity(
        (
            ActivityEvent(
                "pack", "move", "encoding", 25, movement_bytes=2048, movement_commands=1, encoding_transition=True
            ),
...
def test_non_topological_dependency_is_refused_instead_of_assumed_ready() -> None:
    try:
        schedule_activity((ActivityEvent("compute", "array", "compute", 1, depends_on=("missing_load",)),))
