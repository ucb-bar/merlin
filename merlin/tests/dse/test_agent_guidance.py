...
def _report():
    occupancy = OccupancySummary(
        total_cycles=200,
        busy_cycles=(("array", 100), ("dma", 100)),
        compute_resources=("array",),
        movement_resources=("dma",),
        overlap_cycles=20,
        overlap_available_cycles=100,
        idle_cycles=0,
        critical_path_cycles=200,
        movement_bytes=1000,
        movement_commands=4,
    )
    return evaluate_whole_models((ModelPerformance("model", 1000, 200, 190, 150, True, occupancy),))
...
def test_manifest_surface_resolves_to_real_symbol_and_ranked_problem(tmp_path) -> None:
    ...
    movement = next(row for row in brief["ranked_actions"] if row["opportunity"]["kind"] == "exposed_movement")
...
def test_edit_contract_names_actual_symbols_and_explicit_helper_extensions(tmp_path):
    inventory = inspect_compiler_package(_package(tmp_path))
    contract = build_compiler_edit_contract(
        inventory,
        helper_extensions=[
            {
                "directory": "lowering",
                "surface_ids": ["overlap-policy"],
...
@pytest.mark.parametrize(
    ...
    [
        (".", ["overlap-policy"]),
        ("../escape", ["overlap-policy"]),
        ("evaluator", ["overlap-policy"]),
...
def test_macro_optimization_order_requires_global_work_before_local_cleanup(tmp_path):
    ...
    assert "exact_quantized_epilogues_and_residuals" in tiers["whole_program_work_deletion"]["gaps"]
    assert tiers["global_dataflow_and_representation"]["tier"] < tiers["operator_layer_and_tile_efficiency"]["tier"]
    assert tiers["global_execution_and_latency_hiding"]["tier"] < tiers["local_scalar_cleanup"]["tier"]
    assert "cannot bypass" in order["prohibited_shortcut"]
    assert (
        guidance_for_emission_analysis({}, inspect_compiler_package(_package(tmp_path)))["optimization_order"] == order
    )
...
def test_host_payload_guides_edits_without_claiming_cycles(tmp_path) -> None:
    ...
    analysis = {
        "verified_global_plan_emission": {
            ...
            "host_activity": {
                "status": "derived",
                "load_payload_bytes": 120,
                "store_payload_bytes": 80,
...
def test_static_materialization_hotspot_visible_without_known_dynamic_cost(tmp_path) -> None:
    inventory = inspect_compiler_package(_package(tmp_path))
    allocation = {
        "buffer_root": "alloca:31",
        "allocation_operation_index": 31,
        "allocation_task": "2",
        "static_allocation_payload_bytes": 123456,
        "load_payload_bytes": None,
        "store_payload_bytes": None,
...
def test_surface_cannot_point_at_a_symbol_that_does_not_exist(tmp_path) -> None:
    ...
    manifest = package / "manifest.yaml"
    manifest.write_text(
        manifest.read_text(encoding="utf-8").replace("Scheduler.choose", "Scheduler.missing"), encoding="utf-8"
    )
...
def test_surface_rejects_metric_or_unknown_cca_axes(tmp_path) -> None:
    ...
    manifest.write_text(original.replace("dispatch.dma_overlap", "dispatch.n_dispatches"), encoding="utf-8")
    with pytest.raises(ValueError, match="non-editable CCA axes"):
        inspect_compiler_package(package)

    manifest.write_text(original.replace("dispatch.dma_overlap", "dispatch.not_an_axis"), encoding="utf-8")
...
def test_emission_brief_maps_global_evidence_to_exact_edit_coordinate(tmp_path) -> None:
        ...
        "arms": {
            ...
            "candidate": {
                ...
                "representation_activity": {
                    "emitted_encoding_transitions": {"status": "UNKNOWN", "declared_directives": 2},
    ...
    movement = next(row for row in brief["ranked_actions"] if row["kind"] == "movement_regression")
    assert movement["status"] == "actionable"
    assert movement["edit_surfaces"][0]["path"] == "lowering/schedule.py"
    assert movement["edit_surfaces"][0]["symbol"] == "Scheduler.choose"
    assert movement["edit_surfaces"][0]["line"] == 2
...
def test_emitted_artifact_and_trace_evidence_join_the_same_edit_loop(tmp_path) -> None:
        ...
        "target_artifact_activity": {
            "baseline": {
                ...
                "issued": {
                    ...
                    "synchronization_instructions": 2,
            ...
            "candidate": {
                "status": "decoded",
                "issued": {
                    "movement_instructions": 5,
                    "configuration_instructions": 2,
                    "synchronization_instructions": 3,
                    "loop_descriptor_instructions": 0,
                },
                "encoding_resolution": {"status": "complete"},
                "artifact_opportunities": [
                    {
                        "axis": "dispatch.dma_overlap",
                        "observation": "DMA is immediately awaited",
                        "change": "overlap its consumer",
                        "status": "forkable",
                        "forkable_now": True,
                    }
                ],
    ...
    opportunity = next(row for row in brief["ranked_actions"] if row["kind"] == "artifact_cca_opportunity")
...
def test_declined_full_model_is_the_first_named_gap_not_zero_work(tmp_path) -> None:
    ...
    assert brief["mechanism_coverage"]["declared_movement_volume"] == "unavailable: lowering declined"
...
def test_bound_compiler_owned_plan_is_not_confused_with_shared_solver_or_costs(tmp_path):
    ...
    plan.update(
        status="verified",
        source_operations=19,
        tasks=3,
        emitted_operations_by_task={"0": 4, "1": 10, "2": 2},
        compiler_temporaries=["temporary"],
        proof_scope="source/task/ABI/CFG only",
        numeric_equivalence="requires reduced witness",
    )
...
def test_declined_or_refused_gap_evidence_never_becomes_ready(tmp_path):
    ...
    for status in ("declined", "refused", "unsupported", "unavailable: compiler declined"):
        analysis = {
            "target_artifact_activity": {"candidate": {"status": status}},
