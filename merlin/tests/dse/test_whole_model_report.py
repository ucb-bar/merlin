    ...
    report = evaluate_whole_models(
        (
            ModelPerformance(
                ...
                provenance=("test measured A/B receipt",),
                cycle_basis="measured",
            ),
            ModelPerformance(
                "model_b",
                2000,
...
def test_favorable_estimates_cannot_pass_measured_performance_gate(basis):
    row = ModelPerformance(
        ...
        150,
        True,
        _occupancy(200, 180, 60, 50),
        provenance=("cost-model source",),
        cycle_basis=basis,
    )
...
def test_plan_conversion_preserves_estimate_basis():
    ...
    result = SimpleNamespace(
        ...
        legal_floor=SimpleNamespace(cycles=190),
        physical_floor=SimpleNamespace(cycles=150),
        occupancy=_occupancy(200, 180, 60, 50),
    )
    row = performance_from_plan("model", 1000, result, heldout=True, provenance=("calibrated short probes",))
...
def test_measured_label_without_provenance_does_not_earn_a_verdict():
    row = ModelPerformance("model", 1000, 200, 190, 150, True, _occupancy(200, 180, 60, 50), cycle_basis="measured")
    assert not evaluate_whole_models((row,)).passed


def test_micro_result_cannot_substitute_for_missing_heldout_whole_model() -> None:
    report = evaluate_whole_models(
        (ModelPerformance("training_model", 1000, 100, 90, 80, False, _occupancy(100, 90, 20, 15)),)
    )
...
def test_unknown_roofline_blocks_gap_closure_instead_of_reading_as_zero() -> None:
    report = evaluate_whole_models(
        (ModelPerformance("model", 1000, 300, None, None, True, _occupancy(300, 250, 100, 60)),)
    )
...
def test_fast_geomean_does_not_hide_one_model_regression() -> None:
    report = evaluate_whole_models(
        (
            ModelPerformance("fast", 1000, 100, 95, 80, True, _occupancy(100, 95, 10, 5)),
            ModelPerformance("regressed", 1000, 1021, 900, 800, True, _occupancy(1021, 900, 200, 79)),
        )
    )
...
def test_speedup_cannot_pass_without_whole_model_occupancy_evidence() -> None:
    report = evaluate_whole_models((ModelPerformance("model", 1000, 100, 95, 80, True, None),))
...
def test_aggregate_resource_counters_do_not_imply_transfer_elapsed_union() -> None:
    occupancy = OccupancySummary(
        10,
        (("compute", 10), ("load", 4), ("store", 6)),
        compute_resources=("compute",),
        movement_resources=("load", "store"),
        overlap_cycles=6,
    )
    report = evaluate_whole_models((ModelPerformance("model", 100, 10, 10, 10, True, occupancy),))
...
def test_missing_compute_counter_is_not_zero_busy_or_ranked_as_idle_capacity():
    occupancy = OccupancySummary(10, (("first", 10),), compute_resources=("first", "second"))
...
def test_explicit_zero_busy_is_known_idle_capacity_not_missing_evidence():
    occupancy = OccupancySummary(10, (("first", 10), ("second", 0)), compute_resources=("first", "second"))
...
def test_missing_declared_movement_engine_cannot_pass_complete_occupancy_gate():
    ...
    report = evaluate_whole_models(
        (
            ModelPerformance(
                "model", 1000, 200, 190, 150, True, occupancy, provenance=("counter fixture",), cycle_basis="measured"
            ),
        )
    )
    assert not dict(report.gates)["whole_model_occupancy_evidence"]
