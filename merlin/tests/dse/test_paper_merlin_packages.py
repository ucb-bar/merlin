...
def test_compiler_identity_includes_optional_distribution_sources(tmp_path, monkeypatch):
    core = tmp_path / "src"
    extension = tmp_path / "packages/merlin-analysis/src"
    (core / "merlin").mkdir(parents=True)
    extension.mkdir(parents=True)
    (core / "merlin/__init__.py").write_text("")
    (tmp_path / "pyproject.toml").write_text("[project]\n")
    implementation = extension / "producer.py"
    implementation.write_text("value = 1\n")
    monkeypatch.setattr(packages, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(packages, "python_import_roots", lambda: (core, extension))
    original = packages._compiler_source_identity()
    implementation.write_text("value = 2\n")
    assert packages._compiler_source_identity() != original


def test_compiler_identity_refuses_foreign_installed_sources(tmp_path, monkeypatch):
    (tmp_path / "src/merlin").mkdir(parents=True)
    (tmp_path / "src/merlin/__init__.py").write_text("")
    monkeypatch.setattr(packages, "repo_root", lambda: tmp_path)
    monkeypatch.setattr(packages, "python_import_roots", lambda: (tmp_path.parent,))
    with pytest.raises(ValueError, match="active Merlin source checkout"):
        packages._compiler_source_identity()
...
def _staged(tmp_path: Path) -> tuple[Path, Path]:
    ...
    for model in raw["models"]:
        for precision, artifact in model["artifacts"].items():
            ...
            (root / "capture-byte").write_text(f"{model['name']}/{precision}\n", encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "complete": True,
                "study": str(staged),
                "study_sha256": packages._sha(staged),
                "captures": captures,
            }
        ),
        encoding="utf-8",
...
def test_explicit_authority_command_emits_closed_receipt_and_rejects_tampering(tmp_path):
    authority, receipt = packages.issue_toolchain_authority(
        output=tmp_path / "authority.json", authority_id="reviewed-k1-compiler", target="k1", build_tool="/bin/true"
    )
...
def test_missing_backend_producers_block_all_publication_and_report_every_package(tmp_path, monkeypatch):
    ...
    with pytest.raises(packages.MerlinPackageSetNotReady) as caught:
        packages.materialize(
            staged, registration, promoted, producer_inputs, runtime, authority, receipt, execute=True, product=product
        )
