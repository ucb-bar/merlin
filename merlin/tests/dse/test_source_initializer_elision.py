...
def no_external_execution(monkeypatch):
    def refuse(*args, **kwargs):
        raise AssertionError("initializer evidence tests must not launch processes or listeners")
...
def test_batched_contraction_does_not_expand_initializer_admission():
    source = """module {
func.func @work(%c:tensor<2x2x3xi8>,%a:tensor<2x2x5xi8>,%b:tensor<2x5x3xi8>)->tensor<2x2x3xi8>{
%z=arith.constant 0:i8
%init=linalg.fill ins(%z:i8) outs(%c:tensor<2x2x3xi8>)->tensor<2x2x3xi8>
%r=linalg.generic {indexing_maps=[affine_map<(b,m,n,k)->(b,m,k)>,
affine_map<(b,m,n,k)->(b,k,n)>,affine_map<(b,m,n,k)->(b,m,n)>],
iterator_types=["parallel","parallel","parallel","reduction"]}
ins(%a,%b:tensor<2x2x5xi8>,tensor<2x5x3xi8>) outs(%init:tensor<2x2x3xi8>){
^bb0(%x:i8,%y:i8,%acc:i8):
%p=arith.muli %x,%y:i8
%v=arith.addi %p,%acc:i8
linalg.yield %v:i8
}->tensor<2x2x3xi8>
func.return %r:tensor<2x2x3xi8>
}}"""
    ...
    assert "unbatched rank-2 contraction" in proof["reason"]
    assert proof["emitted_machine_initialization"] == proof["numeric_equivalence"] == "UNPROVEN"


@pytest.mark.parametrize("geometry_change", [{"batch_shape": (1,)}, {"output_rank": 3}])
def test_initializer_checks_batch_and_rank_independently(monkeypatch, geometry_change):
    from merlin.perf import source_initializer_elision as initializer

    original_contract = initializer._contract

    def changed_contract(operation):
        info, dimensions = original_contract(operation)
        return info, {**dimensions, **geometry_change}

    monkeypatch.setattr(initializer, "_contract", changed_contract)
    proof = initializer.prove_implicit_zero_initializer(source_text=SOURCE, command_buffer=buffer(), source_op_index=2)
    assert proof["status"] == "UNKNOWN"
    assert "unbatched rank-2 contraction" in proof["reason"]
...
def test_fill_observed_as_second_return_is_not_single_use_elidable():
    ...
    source = source.replace("func.return %r:tensor<2x3xi8>", "func.return %r,%init:tensor<2x3xi8>,tensor<2x3xi8>")
    proof = prove_implicit_zero_initializer(source_text=source, command_buffer=buffer(source), source_op_index=2)
...
def test_nonidentity_or_stale_source_refuses(mutation):
    ...
    assert (
        prove_implicit_zero_initializer(source_text=source, command_buffer=cb, source_op_index=2)["status"] == "UNKNOWN"
    )
...
def structural_artifact():
    ...
    for name, shape in zip(names, ([2, 3], [2, 5], [5, 3], [2, 3], [2, 3])):
        cb["tensors"][name] = {
            "shape": shape,
            "dtype": "i32" if name == "Y_acc" else "i8",
            "role": "input" if name in {"C", "A", "B"} else "output" if name == "Y" else "intermediate",
        }
    cb["kernel_abi"]["args"] = [
        {"tensor": name, "access": "read" if i < 3 else "write"} for i, name in enumerate(names)
    ]
    plan = cb["params"]["global_program_plan"]
    plan.update(
...
def test_supplement_is_numerical_probe_permission_not_a_forged_plan_pass():
    ...
    proof = verify_compiler_global_plan(
        ...
        command_buffer_sha256=document_digest(cb),
    )
...
def test_wrapper_task_keys_survive_receipt_transport_without_changing_proof():
    ...
    llvm = llvm.replace(
        ...
        "%wrapper=llvm.getelementptr %y[%z] {merlin.global_task=-1:i64}:(!llvm.ptr,i64)->!llvm.ptr,i8\n%p=llvm.getelementptr",
    )
    llvm = llvm.replace(
        "llvm.return",
        "%end=llvm.getelementptr %y[%z] {merlin.global_task=-2:i64}:(!llvm.ptr,i64)->!llvm.ptr,i8\nllvm.return",
    )
    proof = verify_compiler_global_plan(
        source_text=SOURCE,
        lowered_text=llvm,
