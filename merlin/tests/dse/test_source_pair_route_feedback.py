...
from merlin.perf.source_program_pair import document_digest as digest
...
def scenario(tmp_path, monkeypatch, comparison_arm="optimization_baseline"):
    ...
    for arm, compiler, instruction in (("before", "a" * 64, "load_execute"), ("after", "b" * 64, "loop_launch")):
        for domain, text in (("full", source), ("short", probe)):
            cb = {
                "params": {
                    "global_program_plan": {
                        "source_sha256": text_digest(text),
                ...
                "tasks": [
                    {
                        ...
                        "class_counts": {instruction: 1},
                        "role_counts": {"compute": 1},
                        "owned_instruction_payload_sha256": text_digest(llvm),
                    }
                ],
            }
            if domain == "full":
                artifacts[arm] = {
                    ...
                    "lowered_sha256": text_digest(llvm),
                    "command_buffer_sha256": text_digest(raw),
                }
                ...
                prepared["arms"][arm] = {
                    "lowered_path": str(lp),
                    "command_buffer_path": str(cp),
                    "compiler_sha256": compiler,
                    "lowered_sha256": text_digest(llvm),
                    "command_buffer_canonical_sha256": text_digest(raw),
                    "source_task_cfg_proof": proof,
                    "task_instruction_evidence": summary,
                    "task_instruction_evidence_sha256": digest(summary),
                }
    current = {
        "verified_global_plan_emission": full_proofs["after"],
    ...
    previous = {
        "verified_global_plan_emission": full_proofs["before"],
        "task_instruction_evidence": {"candidate": summaries["before"]},
    }
    probes = SimpleNamespace(
        analysis=SimpleNamespace(
            session=SimpleNamespace(
                inputs=SimpleNamespace(target="test_target", host_policy={"sha256": policy}),
    ...
    pair = SimpleNamespace(
        source=source, source_sha256=text_digest(source), graph_sha256=text_digest(source), artifacts=artifacts
    )
    return dict(candidate=tmp_path, probes=probes, pair=pair, prepared=prepared)
...
def test_join_uses_correct_full_arm_without_any_parser(tmp_path, monkeypatch, comparison_arm):
    ...
    monkeypatch.setattr(
        P, "verify_compiler_global_plan", lambda **k: pytest.fail("no full verification in cached join")
    )
...
def test_independent_artifact_and_policy_binding_refuses_rehashed_claims(tmp_path, monkeypatch, mutation):
    ...
    if mutation.endswith("_claim"):
        key = {
            "compiler_claim": "compiler_sha256",
            "facts_claim": "target_facts_sha256",
            "policy_claim": "host_verifier_policy_sha256",
            "source_claim": "source_sha256",
        }[mutation]
