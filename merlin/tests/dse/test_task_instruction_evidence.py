...
from merlin.perf.task_instruction_evidence import _sha, digest, summarize_task_instructions
...
def fixture():
    ...
    operations = [
        op("llvm.inline_asm", -1),
        op("llvm.add", 0),
        op("llvm.inline_asm", 0),
        op("llvm.inline_asm", 0),
        op("llvm.store", 1),
        op("llvm.inline_asm", 1),
    ]
    rows = [
        {"index": i, "class": klass, "funct": selector, "decoded": fields}
        for i, klass, selector, fields in [
            (0, "BARRIER", None, {}),
            (1, "COMPUTE", 91, {"source": 3}),
            (2, "UNKNOWN", 92, {}),
            (3, "SEQUENCER", 93, {}),
        ]
    ]
    ...
    proof = {
        ...
        "candidate_lowered_sha256": _sha("llvm"),
        "candidate_command_buffer_sha256": _sha(raw),
        "plan_digest": digest(cb["params"]["global_program_plan"]),
    }
    kwargs = dict(
        source_text="source",
        lowered_text="llvm",
        ...
        command_buffer=cb,
        verified_plan=proof,
        parsed_module=SimpleNamespace(walk=lambda: iter(operations)),
        decoded_trace={"instructions": rows},
        target_facts={
            "isa": {"revision": "synthetic-a"},
            "roles_by_selector": {"91": ["accumulate"], "93": ["sequence"]},
        },
        host_policy_sha256="c" * 64,
        decode_module=lambda _: {"instructions": rows},
    )
...
def test_stale_or_unowned_records_fail_closed(change):
    ...
    if change == "source":
        ...
        kwargs["lowered_text"] += "changed"
    elif change == "buffer":
        kwargs["command_buffer"]["kernel_abi"] = {"changed": True}
    elif change == "proof":
        kwargs["verified_plan"]["status"] = "UNKNOWN"
    elif change == "policy":
        kwargs["host_policy_sha256"] = None
    elif change == "trace":
        kwargs["decoded_trace"] = {"instructions": deepcopy(rows)}
        kwargs["decoded_trace"]["instructions"][1]["class"] = "OTHER"
    elif change == "owner":
        ops[-1].attributes.clear()
    elif change == "module":
        kwargs["parsed_module"] = None
    with pytest.raises(ValueError):
        summarize_task_instructions(**kwargs)
...
def test_target_roles_are_data_and_change_evidence_identity():
    ...
    kwargs["target_facts"] = {
        "isa": {"revision": "synthetic-b"},
        "roles_by_selector": {"91": ["multiply", "accumulate"]},
    }
...
def test_exact_payload_change_changes_owned_digest_without_claiming_route_change():
    ...
    second = summarize_task_instructions(**kwargs)
    assert (
        first["tasks"][0]["owned_instruction_payload_sha256"] != second["tasks"][0]["owned_instruction_payload_sha256"]
    )
...
def test_short_initializer_permission_is_labeled_separately_not_full_coverage(mutation):
    ...
    proof.update(
        status="refused",
        problems=["source operations are not fully covered: [0, 1]"],
        control_flow={"status": "verified"},
    )
    admission = {
        "schema": "short_initializer_execution_admission_v1",
        "status": "source_bound_numerical_probe",
        "source_sha256": _sha(kwargs["source_text"]),
        "lowered_sha256": _sha(kwargs["lowered_text"]),
        "command_buffer_sha256": digest(kwargs["command_buffer"]),
        "candidate_sha256": proof["candidate_sha256"],
        "declared_proof_sha256": digest(proof),
        ...
        "initializer_closure": {
            ...
            "source_sha256": _sha(kwargs["source_text"]),
            "command_buffer_sha256": digest(kwargs["command_buffer"]),
            "implicit_source_op_indices": [0, 1],
        },
    }
    if mutation == "unrelated_omission":
        admission["initializer_closure"]["implicit_source_op_indices"] = [0, 1, 4]
    elif mutation == "stale_llvm":
        admission["lowered_sha256"] = "f" * 64
    elif mutation == "stale_proof":
        admission["declared_proof_sha256"] = "f" * 64
    elif mutation == "unverified_cfg":
        proof["control_flow"]["status"] = "UNKNOWN"
        admission["declared_proof_sha256"] = digest(proof)
