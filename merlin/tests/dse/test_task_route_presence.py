...
def envelope(source, compiler, index, classes, *, roles=()):
    binding = {
        ...
        "lowered_sha256": "a" * 64,
        "command_buffer_sha256": "b" * 64,
        "compiler_sha256": compiler * 64,
        "logical_dispatch_digest": source * 64,
        "plan_digest": "c" * 64,
        "target_facts_sha256": "d" * 64,
        "host_verifier_policy_sha256": "e" * 64,
    }
    task = {
        "task_index": 3,
        "source_op_indices": [index],
        "declared_task_kind": "opaque",
        "instruction_indices": list(range(sum(classes.values()))),
...
def scenario(*, second_family=False):
    before, after = (
        ("load_and_execute", "descriptor_launch") if not second_family else ("vector_dispatch", "fused_stream")
    )
    full = {
        "before": envelope("1", "3", 48, {before: 80}, roles=[before]),
        "after": envelope("1", "4", 48, {after: 6}, roles=[after]),
    }
    short = {
        "before": envelope("2", "3", 2, {before: 2}, roles=[before]),
        "after": envelope("2", "4", 2, {after: 1}, roles=[after]),
    }
    extraction = {"source_sha256": "1" * 64, "probe_source_sha256": "2" * 64, "source_op_index": 48}
    return dict(
        full=full, short=short, extraction=extraction, extraction_sha256=digest(extraction), short_source_op_index=2
    )
...
def test_stale_or_ambiguous_four_arm_join_refuses(mutation):
    ...
    if mutation in {"source", "compiler", "policy", "facts"}:
        key = {
            ...
            "policy": "host_verifier_policy_sha256",
            "facts": "target_facts_sha256",
        }[mutation]
        record["summary"]["binding"][key] = "f" * 64
        record["expected_binding"][key] = "f" * 64
    elif mutation == "source_index":
        record["summary"]["tasks"][0]["source_op_indices"] = [0]
    elif mutation == "multiowner":
        record["summary"]["tasks"][0]["source_op_indices"] = [2, 5]
    elif mutation == "count":
        record["summary"]["tasks"][0]["class_counts"] = {"descriptor_launch": 99}
    elif mutation == "payload":
        record["summary"]["tasks"][0]["owned_instruction_payload_sha256"] = "changed"
    elif mutation == "extraction":
        kwargs["extraction"]["source_op_index"] = 5
    else:
        kwargs["short"].pop("before")
    if mutation != "payload":
...
def test_initializer_exception_is_scoped_to_short_source_and_never_relabels_full_plan(domain):
    ...
    record["summary"].update(
        status="short_admitted_static_ownership",
        declared_source_plan_status="refused",
        implicit_initializer_source_op_indices=[0, 1],
        short_execution_admission_sha256=digest(admission),
    )
    record["summary_sha256"] = digest(record["summary"])
    result = compare_task_route_presence(**kwargs)
    if domain == "full":
