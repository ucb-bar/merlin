...
def row(name, value=1):
    return {"class": name, "funct": 3, "rs1": {"kind": "const", "raw": value}, "rs2": {"kind": "const", "raw": 0}}
...
def artifact(rows, suffix=""):
    ...
    return (
        {
            "emission": {"candidate_lowered_sha256": digest},
            "diagnostics": {"captured_logical_graph": {"logical_dispatch_digest": "same-graph"}},
        },
        {"lowered_text": text, "candidate_lowered_sha256": digest, "decoded_trace": {"instructions": rows}},
    )


def signature(name, value=1, *, mixed=False):
    events = (
        [ActivityEvent("transfer", "port", "movement", 0), ActivityEvent("body", "array", "compute", 0, ("transfer",))]
        if mixed
        else [ActivityEvent("body", "whole_motif", "compute", 0)]
    )
    return derive_mechanism_signature(
        representations=[ValueRepresentation("local", "rows", "i8")],
        events=events,
        capacity_regime={"local": "fits"},
        tile_shape=[3, 5],
        edge_cases=["tail"],
...
def classify(before, after, sampled):
    ...
    return classify_probe_relevance(
        previous_analysis=left, current_analysis=right, previous_artifacts=la, current_artifacts=ra, signature=sampled
    )
...
def test_reordered_traffic_requires_context_even_with_multi_resource_probe():
    result = classify([row("packet"), row("array")], [row("array"), row("packet")], signature("packet", mixed=True))
...
def test_unchanged_trace_does_not_hide_host_encoding_change():
    ...
    result = classify_probe_relevance(
        previous_analysis=left,
        current_analysis=right,
        previous_artifacts=la,
        current_artifacts=ra,
