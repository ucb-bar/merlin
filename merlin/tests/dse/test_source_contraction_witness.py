...
from merlin.perf.source_contraction_witness import evaluate_source_contraction, extract_source_contraction
...
def arrays(record):
    return [
        np.full(shape, -128 if shape == [2, 5] else 127 if shape == [5, 3] else 123, dtype="int" + dtype[1:])
...
def test_rank_general_contraction_preserves_batch_and_broadcast_semantics(batch, lhs_broadcast, rhs_broadcast):
    original = rank_general_source(batch=batch, lhs_broadcast=lhs_broadcast, rhs_broadcast=rhs_broadcast)
    probe, record = extract_source_contraction(
        original, 0, entry="work", max_m=2, max_n=3, max_k=4, max_batch_extent=2, max_macs=1000
    )
    ...
    values = []
    for index, (shape, dtype) in enumerate(zip(record["input_shapes"], record["input_dtypes"], strict=True)):
        fill = (1, 2, 5)[index]
        values.append(np.full(shape, fill, dtype="int" + dtype[1:]))
    result = evaluate_source_contraction(probe, record, values)
    np.testing.assert_array_equal(result, np.full(record["output_shape"], 5 + 4 * 2, dtype=np.int32))
