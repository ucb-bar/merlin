...
def contraction(*, batches, lhs_broadcast=False, suffix="0"):
    ...
    return (
        f"""%r{suffix}=linalg.generic {{indexing_maps=[affine_map<({domain})->({lhs_map})>,affine_map<({domain})->({rhs_map})>,affine_map<({domain})->({out_map})>],iterator_types=[{iterators}]}}
...
def source():
    ...
    for suffix, shapes in (("0", one), ("1", two)):
        args.extend(
            f"%{name}{suffix}:tensor<{shape}xi{dtype}>"
            for name, shape, dtype in zip(("a", "b", "c"), shapes, ("8", "8", "32"), strict=True)
        )
    return (
        "module {func.func @work("
        + ",".join(args)
        + ")->(tensor<7x4x5xi32>,tensor<2x3x4x5xi32>){\n"
        + first
        + "\n"
        + second
...
def artifact(path, tasks):
    return {
        "interface": path,
        "lowered_text": "",
        "command_buffer": {"params": {"global_program_plan": {"tasks": tasks}}},
    }
...
def test_qualifier_binds_selected_member_and_runs_both_arms_per_class(tmp_path, monkeypatch):
    ...
    probes = SimpleNamespace(
        analysis=SimpleNamespace(session=SimpleNamespace(selected_changed_portfolio_context=context))
    )
    rows = {row["source_op_index"]: row for row in Q.contraction_lane_migrations(path.read_text(), before, after)}
    prepared = []
    ...
    def runtime(**kwargs):
        ...
        return {"status": "passed", "correct": True, "arms": {arm: {"correct": True} for arm in ("before", "after")}}

    qualifier = Q.LaneMigrationContractionQualifier(
        target="test", runtime_provider=runtime, abi_provenance={"host": "pinned"}, output=tmp_path / "receipts"
    )
    receipt = qualifier(candidate=tmp_path, probes=probes, timeout_s=60, portfolio_member=selection)
...
def test_dispatch_never_falls_back_from_lane_migration(tmp_path):
    ...
    selected = {"selection": {"portfolio_index": 0}, "previous": {"artifacts": before}, "current": {"artifacts": after}}
    probes = SimpleNamespace(
        analysis=SimpleNamespace(session=SimpleNamespace(selected_changed_portfolio_context=lambda _c, _s: selected))
    )
    dispatch(candidate=tmp_path, probes=probes, timeout_s=60)
    assert calls == ["lane"]
