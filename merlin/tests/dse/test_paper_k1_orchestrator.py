...
def _contract(tmp_path: Path, *, run_id: str = "run-0", model: str = "gemma2_2b") -> Path:
    ...
    root.mkdir()
    cell = {"model": model, "backend": "merlin_frozen", "precision": "w8a8", "core_count": 4}
    result_identity = {
    ...
    contract = {
        ...
        "frozen_provenance": {"study_sha256": "a" * 64, "compiler_policy_sha256": "8" * 64},
        "build": {"tool": {"path": "build-tool", "sha256": orchestrator._sha_file(tool)}},
    }
...
def _plan(tmp_path: Path, monkeypatch) -> tuple[Path, dict]:
    ...
    path = orchestrator.create_matrix_plan(
        [_contract(tmp_path)], tmp_path / "matrix.json", controller_root=_controller_root(tmp_path)
    )
...
def test_systemd_command_serializes_board_and_runs_controller_on_remote_host(tmp_path: Path) -> None:
    ...
    transport = orchestrator.K1SSHSystemdTransport(orchestrator.SSHConfig(host="192.0.2.8"), runner=runner)

    evidence = transport.run_cell(
        remote_controller="/cache/controller",
        remote_contract="/cache/contract",
        remote_run_root="/runs/cell",
        contract_tree_sha256="b" * 64,
        unit_name="merlin-paper-matrix-000",
        timeout_seconds=30,
    )
    ...
    assert "PYTHONPATH=/cache/controller/repo/src:" in command
    assert "/cache/controller/repo/packages/merlin-analysis/src" in command
    assert evidence["unit"] == "merlin-paper-matrix-000"


def test_ssh_stage_names_cache_by_tree_and_verifies_uploaded_archive_digest(tmp_path: Path) -> None:
    commands: list[list[str]] = []
    ...
    remote = transport.stage(archive, archive_sha256=archive_sha, tree_sha256=tree_sha, kind="contract")
...
def _environment_receipt(
    matrix_sha256: str, controller_tree_sha256: str, runtime_requirements_sha256: str, required_core_count: int
) -> bytes:
    file_row = {"path": "/identity", "resolved_path": "/identity", "sha256": "1" * 64, "size": 1}
    modules = {
        name: {**file_row, "name": name, "import_name": imported}
        for name, imported in orchestrator._REMOTE_MODULES.items()
    }
    tools = {
        name: {**file_row, "path": path, "resolved_path": path} for name, path in orchestrator._REMOTE_TOOLS.items()
    }
    frequencies = [
        {"core_id": core, "governor": "performance", "current_khz": 1600000, "max_khz": 1600000}
        for core in range(required_core_count)
    ]
    document = {
        "schema_version": 1,
        "kind": "paper_k1_environment_preflight_v1",
        "status": "ready",
        ...
        "runtime": {
            ...
            "systemd_version_sha256": "2" * 64,
            "openssl_ed25519": True,
            "board_probe_source_sha256": "3" * 64,
            "board_probe_executable_sha256": "4" * 64,
            "board_probe": {"kind": "merlin_board_probe_v1", "vlen_source": "csr", "governor": "performance"},
            "core_frequencies": frequencies,
            "mapped_libraries": [{"path": "/lib/ld-linux-riscv64-lp64d.so.1", "sha256": "5" * 64, "size": 1}],
        },
    }
...
class _FakeTransport:
    ...
    def stage(self, _archive: Path, *, archive_sha256: str, tree_sha256: str, kind: str) -> str:
    ...
    def environment_preflight(
        ...
        *,
        ...
        return _environment_receipt(
            matrix_sha256, controller_tree_sha256, runtime_requirements_sha256, required_core_count
        )
    ...
    def retrieve(self, _remote_run_root: str, destination: Path) -> str:
        ...
        with tarfile.open(destination, "w") as stream:
            for name in ("terminal.json", "result.yaml", "output", ".paper-controller-issuance-v1"):
...
def _verified(_transport_root: Path, planned, *, retrieval_sha256: str, final_dir: Path):
    result = {
        ...
        "measurement_receipt": {
            "path": str(final_dir / "transport/output/receipt.yaml"),
            "sha256": "c" * 64,
        },
    }
    return {
...
def test_matrix_is_sequential_atomic_and_resume_never_contacts_board_for_terminal_cell(
    ...
    output = orchestrator.run_matrix(plan_path, transport=first, output_dir=tmp_path / "run")
...
def test_host_recovers_board_terminal_cell_without_rerunning_measurement(tmp_path: Path, monkeypatch) -> None:
    plan_path, _ = _plan(tmp_path, monkeypatch)
    monkeypatch.setattr(orchestrator, "_validate_retrieved", _verified)
...
def test_corrupt_retrieval_never_becomes_a_terminal_cell_or_final_notary(tmp_path: Path, monkeypatch) -> None:
    plan_path, _ = _plan(tmp_path, monkeypatch)
...
def test_plan_rejects_host_cross_compiler_for_board_local_rebuild(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(orchestrator, "_contract", _fake_contract)
    ...
    with pytest.raises(ValueError, match="cross-compiler cannot execute"):
        orchestrator.create_matrix_plan([contract], tmp_path / "plan.json", controller_root=_controller_root(tmp_path))
...
def test_invalid_remote_dependency_receipt_blocks_before_any_cell_contact(tmp_path: Path, monkeypatch) -> None:
    plan_path, _ = _plan(tmp_path, monkeypatch)
...
def test_prepare_materializes_frozen_order_with_one_aet_parent_and_never_executes(tmp_path: Path, monkeypatch) -> None:
    ...
    cells = [
        SimpleNamespace(model=model, backend=backend, precision="fp32", core_count=1, key=f"{model.name}/b0/fp32/1c")
        for model in models
    ]
    ...
    handle = SimpleNamespace(run_id="aet-parent", run_dir=aet_root, timestamp="20260831T000000Z", git_sha="deadbee")
    finished = []
    ...
    monkeypatch.setattr(orchestrator, "finish_run", lambda _handle, status, summary: finished.append((status, summary)))
    monkeypatch.setattr(orchestrator, "_contract", _fake_contract)
    ...
    monkeypatch.setattr(
        orchestrator, "produce_receipt", lambda *_args, **_kwargs: pytest.fail("prepare must not execute a measurement")
    )
    ...
    assert [row["run_id"] for row in plan["cells"]] == ["aet-parent__cell000", "aet-parent__cell001"]
    assert [row["cell"]["model"] for row in plan["cells"]] == ["m0", "m1"]
    assert all((Path(row["contract_root"]) / "measurement_contract.yaml").is_file() for row in plan["cells"])
    ...
    assert finished == [("ok", {"prepared": True, "n_cells": 2, "matrix_sha256": plan["matrix_sha256"]})]
...
def _finalization_fixture(tmp_path: Path, monkeypatch):
    ...
    matrix_state = {
        ...
        "terminal_cells": 2,
        "expected_cells": 2,
        "environment_preflight_sha256": orchestrator._sha_file(run / "environment-preflight.json"),
    ...
    ordered_cells = [
        SimpleNamespace(
            ...
            precision=row["cell"]["precision"],
            core_count=row["cell"]["core_count"],
        )
    ...
    def seal(_spec, results, *, trusted_issuance_fingerprints):
        sealed_calls.append(([result["run_id"] for result in results], dict(trusted_issuance_fingerprints)))
        return {
            "schema_version": 3,
            "study_sha256": "a" * 64,
            "results": results,
...
def test_finalize_seals_exact_plan_order_and_publishes_results_last(tmp_path: Path, monkeypatch) -> None:
    ...
    assert [result["run_id"] for result in document["results"]] == [row["run_id"] for row in plan["cells"]]
...
def test_finalize_rejects_partial_extra_or_identity_tampered_cells(tmp_path: Path, monkeypatch, mutation: str) -> None:
    plan_path, plan, run, study_path, sealed_calls = _finalization_fixture(tmp_path, monkeypatch)
    cells = run / "cells"
    if mutation == "missing":
...
def test_finalize_rejects_notary_or_prerequisite_tampering_before_seal(tmp_path: Path, monkeypatch) -> None:
    plan_path, _plan_value, run, study_path, sealed_calls = _finalization_fixture(tmp_path, monkeypatch)
...
def test_finalize_rejects_plan_order_that_differs_from_frozen_study(tmp_path: Path, monkeypatch) -> None:
...
def test_remote_payload_refuses_non_riscv_host(tmp_path: Path, monkeypatch) -> None:
    ...
    with pytest.raises(ValueError, match="locally on a RISC-V K1"):
        orchestrator.run_remote_cell(contract, tmp_path / "remote-run", contract_tree_sha256="f" * 64)

    assert not (tmp_path / "remote-run").exists()


def test_remote_environment_preflight_refuses_non_riscv_before_writing_receipt(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(orchestrator.platform, "machine", lambda: "x86_64")
    ...
    with pytest.raises(ValueError, match="must execute on RISC-V"):
        orchestrator.create_remote_environment_receipt(
            output,
            matrix_sha256="a" * 64,
            controller_tree_sha256="b" * 64,
            runtime_requirements_sha256="c" * 64,
