...
from merlin.kernels.framework_contracts import available_frameworks, load_contract
...
def _nk(text, src="xnnpack", op="gemm", dt="f32"):
    return NormalizedKernel(source=src, target="rvv", path=f"{op}_{dt}.c", op=op, dtype=dt, raw_text=text)
...
def test_clustering_groups_equivalent_kernels():
    doss = [
        build_dossier(_nk(_GEMM_1X4)),
        build_dossier(_nk(_GEMM_1X4_B)),
        build_dossier(_nk(_GEMM_1X4, op="conv")),
    ]  # different op -> different cluster
