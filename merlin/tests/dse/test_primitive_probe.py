...
_SPEC.loader.exec_module(primitive_probe)
...
def source(monkeypatch, *, completion=True, ordered=True):
    ...
    # This unit tests the example IR transform's completion handling; the target decoder's
...
def test_context_keeps_loads_in_body_and_restores_setup_after_warmup(monkeypatch):
    ...
    def names(fn):
        return [op.asm_string.data for op in fn.body.block.ops if op.name == "llvm.inline_asm"]
