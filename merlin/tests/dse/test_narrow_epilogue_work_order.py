...
def test_main_retains_lazy_cli_and_policy_patch_boundary(monkeypatch):
    from merlin.perf import narrow_epilogue_work_order as policy
    from merlin.perf import narrow_epilogue_work_order_cli as cli

    seen = []
    monkeypatch.setattr(cli, "main", lambda argv: seen.append(argv) or 23)
    assert policy.main(["--help"]) == 23
    assert seen == [["--help"]]
    assert cli._core is policy
...
def _buffer(name, dtype, *, shape=(4, 4), encoding="dense", layout="row_major", kind="intermediate"):
    return {
        "id": name,
        "shape": list(shape),
        "dtype": dtype,
...
def _stage(index: int, stage: str, source: str, output: str, *, extras=(), roles=(), operation=None) -> dict:
    return {
        "kind": "dispatch",
...
def _source_plan_metadata(program: dict, tasks: list[dict]) -> tuple[dict, dict]:
    ...
    for name, buffer in program["buffers"].items():
        ...
        for extent in reversed(shape):
            ...
            stride *= extent
        contract = {
            "schema": "test_grouped_storage_v1",
            "logical_shape": shape,
            "dtype": buffer["dtype"],
            "axis_groups": [[axis] for axis in range(len(shape))],
            "physical_shape": shape,
            "strides_elements": strides,
            "storage_elements": stride,
            "offset_elements": 0,
        }
        checked = {
            "contract": contract,
            "proof_scope": "test bounded address map",
        ...
        layout_contract = {
            key: contract[key]
            for key in ("axis_groups", "physical_shape", "strides_elements", "storage_elements", "offset_elements")
        }
        ...
        rows.append(
            {
                ...
                "physical_tensor": {"shape": shape, "dtype": buffer["dtype"], "role": buffer["kind"]},
    ...
    def stage(source_index: int, stage_name: str, operation: str, inputs: list[tuple[str, str]], output: str) -> dict:
    ...
    specifications = {
        0: [
            stage(1, "acc_scale", "acc_scale", [("acc0", "accumulator"), ("scale0", "scale")], "scaled0"),
            stage(2, "requant", "requant", [("scaled0", "accumulator")], "out0"),
        ],
        3: [],
        4: [
            stage(5, "bias", "bias", [("acc4", "accumulator"), ("bias4", "bias")], "biased4"),
        ...
        roots.append(
            {
                ...
                "accumulator": {
                    "shape": program["buffers"][accumulator]["shape"],
                    "dtype": program["buffers"][accumulator]["dtype"],
                },
                "source_operation_ids": [source_index, *[item["source_operation_id"] for item in stages]],
                "stages": stages,
                "reasons": reasons.get(source_index, []),
                "classification": classification,
            }
        )
    epilogues = {
        "schema": "source_integer_epilogue_ownership_v1",
        "status": "verified",
        "contraction_roots": len(roots),
        "classification_counts": {
            "complete_integer_epilogue": 2,
    ...
    storage = {
        "schema": "source_buffer_physical_storage_v1",
        "status": "complete",
        "materialized_source_values": len(rows),
        "exact_physical_representations": len(rows),
        "rows": rows,
        "unknown": [],
        "proof_scope": "test exact source/storage join",
        "not_proven": ["runtime residency"],
    }
    return (
        {
            ...
            "problems": [],
            "proof_scope": "test source-bound evidence",
        },
        encodings,
    )
...
def _record(candidate: Path, contract: dict) -> dict:
    ...
    nodes = [
        ...
        _root(7, "acc7"),
        _stage(8, "bias", "acc7", "res7", extras=("skip7",), roles=("bias",), operation="add"),
        ...
        _root(10, "acc10"),
        {
            ...
            "regions": 0,
            "captures": [],
            "prov": {"prov.family": "cast", "prov.op": "dtype_cast", "prov.region_id": "f10"},
        },
        _root(12, "acc12"),
        {
            "kind": "view",
    ...
    workload = {field: identity[field] for field in ("capsule", "capsule_sha256", "required_lanes", "required_tiers")}
    analysis = {
...
def _capability(record: dict, form: dict) -> dict:
    ...
    document = {
        ...
        "status": "verified",
        "target_descriptor_sha256": record["cross_run_static_analysis_binding"]["target_descriptor_sha256"],
        "target_instruction_facts_sha256": record["analysis"]["diagnostics"]["task_instruction_evidence"]["candidate"][
            "binding"
        ]["target_facts_sha256"],
        "proofs": [proof],
    }
...
def test_inventory_distinguishes_every_narrow_readout_outcome(tmp_path):
    ...
    assert [row["producer_source_operation_id"] for row in member["residual_second_operand"]] == [7]
    assert [row["producer_source_operation_id"] for row in member["float_or_unsupported_stage"]] == [10]
...
def test_capability_must_bind_exact_target_facts_form_and_completion(tmp_path):
    ...
    for mutation in ("target", "facts", "layout", "operand", "width", "completion", "citation", "raw_pin"):
        ...
        if mutation != "raw_pin":
            changed["sha256"] = _digest({key: value for key, value in changed.items() if key != "sha256"})
        result = inventory_portfolio_narrow_epilogues(
            record, capability_evidence=changed, capability_evidence_file_sha256=raw_pin
        )
...
def test_incomplete_source_ownership_is_a_named_failure_not_an_opportunity(tmp_path):
    candidate = _candidate(tmp_path)
    record = _record(candidate, _contract())
    record["analysis"]["diagnostics"]["task_instruction_evidence"]["candidate"]["tasks"][0]["source_op_indices"].remove(
        0
    )
...
def test_missing_source_plan_metadata_fails_closed(tmp_path):
    candidate = _candidate(tmp_path)
    record = _record(candidate, _contract())
    record["analysis"]["diagnostics"]["verified_global_plan_emission"].pop("source_plan_metadata")
...
def test_missing_physical_encoding_is_an_explicit_noneligible_class(tmp_path):
    ...
    storage["unknown"].append(
        {
            ...
            for key in ("source_buffer", "source_origin", "materialized_tensor", "logical", "physical_tensor")
        }
    ...
    member = result["members"][0]
    assert [row["producer_source_operation_id"] for row in member["missing_representation"]] == [0]
...
def test_logical_epilogue_hints_cannot_replace_source_plan_semantics(tmp_path):
    candidate = _candidate(tmp_path)
    record = _record(candidate, _contract())
    metadata = record["analysis"]["diagnostics"]["verified_global_plan_emission"]["source_plan_metadata"]
    root = next(
        item for item in metadata["integer_epilogue_ownership"]["roots"] if item["producer_source_operation_id"] == 0
    )
    root["stages"] = []
    ...
    member = result["members"][0]
    assert [row["producer_source_operation_id"] for row in member["missing_capability"]] == [4]
    refused = next(row for row in member["float_or_unsupported_stage"] if row["producer_source_operation_id"] == 0)
...
def test_changed_source_plan_semantic_digest_fails_closed(tmp_path):
    candidate = _candidate(tmp_path)
    record = _record(candidate, _contract())
    roots = record["analysis"]["diagnostics"]["verified_global_plan_emission"]["source_plan_metadata"][
        "integer_epilogue_ownership"
    ]["roots"]
    ...
    assert "integer-epilogue root 0 stage 0 identity is invalid" in result["members"][0]["problems"]
...
def test_builder_emits_validator_accepted_one_mechanism_documents(tmp_path):
    ...
    authority = FrozenEditAuthority(tmp_path)
    authority.freeze(candidate, documents["edit_contract"], has_iterations=False)
    portfolio = {
        "schema": "full_model_optimization_portfolio_v1",
    ...
    validator = MechanismProgram(
        tmp_path,
        authority,
        portfolio_identity=portfolio,
        portfolio_identity_sha256=_digest(portfolio),
    )
    catalog_path = tmp_path / "catalog.json"
    catalog_path.write_text(json.dumps(documents["catalog"]))
    catalog_path.chmod(0o444)
    validator.freeze_catalog(catalog_path, hashlib.sha256(catalog_path.read_bytes()).hexdigest(), has_iterations=False)
    validated = validator.validate_work_order(documents["work_order"], candidate_sha256=hash_tree(candidate)["sha256"])
...
def test_cli_requires_readonly_pins_and_writes_once(tmp_path, capsys):
    ...
    args = [
        ...
        str(output.resolve()),
        "--iteration-sha256",
        hashlib.sha256(iteration.read_bytes()).hexdigest(),
        "--candidate-sha256",
        hash_tree(candidate)["sha256"],
        "--capability-sha256",
        hashlib.sha256(capability.read_bytes()).hexdigest(),
    ]
    assert main(args) == 0
