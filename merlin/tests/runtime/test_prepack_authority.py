...
def authority_fixture(tmp_path):
    ...
    manifest.write_text(json.dumps({"0": {"weight": "fixed", "kind": "param", "dtype": "int8", "shape": [2, 3]}}))
    constant = verify_capture_constant(
        manifest_path=manifest,
        manifest_sha256=hashlib.sha256(manifest.read_bytes()).hexdigest(),
        safetensors_path=blob,
        safetensors_sha256=hashlib.sha256(blob.read_bytes()).hexdigest(),
        entry_argument_index=0,
        source_shape=(2, 3),
        source_dtype="i8",
        max_payload_bytes=6,
    )
    ...
    bridge = replay_argument_identity(
        raw_text=raw,
    ...
    cb = {
        ...
        "kernel_abi": {
            "kind": "whole_program",
            "args": [{"tensor": "W", "access": "read"}, {"tensor": "Y", "access": "write"}],
            "outputs": ["Y"],
        },
        "params": {
            "storage_encodings": {"W": encoding.to_dict(), "Y": output.to_dict()},
            "global_program_plan": {
                "source_sha256": bridge.normalized_sha256,
                "entry_bindings": ["W"],
                "tasks": [{"writes": ["Y"]}],
            },
        },
...
def test_missing_or_contradictory_host_binding_refuses(tmp_path, change):
    ...
    if change == "readwrite":
        ...
        cb["params"]["global_program_plan"]["tasks"][0]["writes"].append("W")
    elif change == "index":
        cb["params"]["global_program_plan"]["entry_bindings"] = ["Y"]
    elif change == "source":
        cb["params"]["global_program_plan"]["source_sha256"] = "0" * 64
    elif change == "raw":
        kwargs["raw_source_text"] += "\n"
    elif change == "normalized":
        kwargs["normalized_source_text"] += "\n"
    elif change == "serialized_constant":
        kwargs["constant"] = kwargs["constant"].to_evidence()
    elif change == "serialized_bridge":
        kwargs["bridge"] = kwargs["bridge"].to_evidence()
    elif change == "encoding":
        cb["params"]["storage_encodings"]["W"]["logical_shape"] = [3, 2]
    elif change == "dtype":
        cb["tensors"]["W"]["dtype"] = "i16"
