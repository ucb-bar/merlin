...
from merlin.runtime import COMMON_METRIC_NAMES, Tensor, outputs_match, reference_outputs, simulate
...
def _cb(reuse=3, tensors_extra=None, params=None):
    ...
    cmds = [{"opcode": "RES_PACK", "operands": {"src": "W", "dst": "W_res"}, "attributes": {"layout": "packed_rhs"}}]
    for i in range(reuse):
        tensors[f"A{i}"] = {"shape": [5, 8], "dtype": "i8", "role": "input"}
        cmds.append({"opcode": "MATMUL_RESIDENT", "operands": {"lhs": f"A{i}", "rhs": "W_res", "dst": f"acc{i}"}})
        cmds.append(
            {
                "opcode": "COMMIT",
    ...
    return {
        ...
        "tensors": tensors,
        "commands": cmds,
        "params": params or {"requant_shift": 4},
    }
...
def test_end_to_end_known_values():
    ...
    cb = {
        ...
        "target": "toy_npu",
        "backend": "simulator",
        "tensors": {"W": {"shape": [2, 2], "dtype": "i8"}, "A0": {"shape": [2, 2], "dtype": "i8"}},
        "commands": [
