...
def _plan(program: DispatchProgram) -> GlobalPlan:
    ...
    fused = RegionAlternative(
        ...
        "fused_kernel",
        "engine",
        CycleInterval.point(10, "fixture"),
    ...
    def emit_global_plan(self, program: DispatchProgram, plan: GlobalPlan) -> GlobalPlanEmission:
        ...
        emitted = DispatchProgram(
            ...
            [0],
            {
                ...
                **({"x2": Buffer("x2", [4], "i8", "intermediate")} if self.hidden_node else {}),
            },
            nodes,
            ["x1"],
        )
        return GlobalPlanEmission(
            ...
            (),
            (BoundaryMapping("input", "b0", "x0"), BoundaryMapping("output", "b2", "x1")),
...
def test_global_plan_emitter_replaces_two_logical_dispatches_with_one_fused_dispatch() -> None:
    ...
    program = build_program(logical, capability="engine", global_plan=plan, global_plan_emitter=_Emitter())
...
def test_emitter_cannot_hide_unaccounted_executable_work() -> None:
    ...
    with pytest.raises(ValueError, match="unaccounted"):
        build_program(logical, global_plan=_plan(logical), global_plan_emitter=_Emitter(hidden_node=True))
...
def _split_emission():
    ...
    transition = TransitionAlternative("unpack", "encoding", "b1", "producer", "consumer", packed, plain, cost)
    ...
    emitted = DispatchProgram(
        ...
        [
            ...
            Node("dispatch", "consumer_kernel", ["x2"], ["x3"], captures=[]),
        ],
        ["x3"],
    )
...
def test_owned_nodes_cannot_hide_a_bypassed_encoding_transition() -> None:
    ...
    emission = replace(emission, regions=(emission.regions[0], replace(emission.regions[1], inputs=(("b1", "x1"),))))
...
def test_owned_nodes_cannot_hide_a_missing_model_input() -> None:
    ...
    assert any("boundary differs" in issue for issue in verify_global_plan_emission(logical, plan, emission))
...
def test_mapping_is_required_even_when_node_ownership_is_complete() -> None:
    ...
    assert any("complete logical boundary" in issue for issue in verify_global_plan_emission(logical, plan, emission))
...
def test_encoding_declaration_must_agree_with_the_actual_plan_edge() -> None:
    ...
    assert any("source encoding" in issue for issue in verify_global_plan_emission(logical, plan, emission))
