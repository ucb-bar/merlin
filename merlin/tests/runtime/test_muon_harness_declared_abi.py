...
def test_a_slot_may_hold_one_name_or_several(mh):
    ...
    assert mh.declared_abi_order(_cb(kernel_abi={"weight": ["W0", "W1"], "lhs": "A0", "outputs": "Y0"})) == (
        ["W0", "W1", "A0"],
        ["Y0"],
    )
...
def test_the_opcode_path_is_tried_first_and_its_answer_is_kept(mh, monkeypatch):
    ...
    got = mh.args_from_cb(
        _cb(kernel_abi={"lhs": ["A0"], "outputs": ["Y0"]}, operand_shapes={"A0": [2, 2], "Y0": [2, 2]})
    )
...
def test_a_rank_1_operand_is_a_row_vector(mh, monkeypatch):
    ins, _ = _derive(
        ...
        monkeypatch,
        _cb(
            kernel_abi={"lhs": ["A0"], "outputs": ["Y0"]}, operand_shapes={"A0": [16], "Y0": [16]}, leaves={"A0": [16]}
        ),
    )
    ...
    assert (
        _derive(
            ...
            _cb(
                ...
                canonical_inputs={"A0": {"values": [1.0, 2.0]}},
            ),
        )
        is None
    )
...
def test_an_input_with_no_resolvable_shape_is_refused(mh, monkeypatch):
    assert (
        _derive(mh, monkeypatch, _cb(kernel_abi={"lhs": ["A0"], "outputs": ["Y0"]}, operand_shapes={"Y0": [2, 2]}))
        is None
    )
...
def _mx_cb(scale_role="scale"):
    return _cb(
        kernel_abi={"weight": ["W", "W_scale"], "lhs": ["A0", "A0_scale"], "outputs": ["Y0"]},
        operand_shapes={"W": [2, 2], "A0": [2, 2], "W_scale": [1, 2], "A0_scale": [1, 2], "Y0": [2, 2]},
...
def test_declared_scales_are_fed_even_when_the_opcode_path_answered(mh, monkeypatch):
    ...
    monkeypatch.setattr(
        mh,
        "_args_from_cb_by_opcode",
        lambda cb: (
            [mh.TensorArg("W", 2, 2, [0.0] * 4, "f32"), mh.TensorArg("A0", 2, 2, [0.0] * 4, "f32")],
            [mh.TensorArg("Y0", 2, 2, [0.0] * 4, "f32")],
        ),
    )
    ins, _ = mh.args_from_cb(_mx_cb())
    assert [a.name for a in ins] == ["W", "W_scale", "A0", "A0_scale"], (
        "a kernel fed fewer operands than its capsule declares is not given a smaller answer, it is given the wrong one"
    )
...
def test_a_cb_with_no_declared_tensors_says_exactly_that(mh):
    cb = _cb(
        commands=[{"opcode": "MATMUL", "operands": {"lhs": "A0", "rhs": "W", "dst": "16x16"}}],
...
def test_an_unresolved_operand_name_is_named_with_its_slot(mh):
    cb = _cb(
        commands=[{"opcode": "MATMUL", "operands": {"lhs": "A0", "rhs": "W", "dst": "tensor<16x16xf32>"}}],
        leaves={"A0": [2, 2], "W": [2, 2]},
        canonical_inputs={"A0": {"values": [0.0]}},
    )
    msg = mh.why_no_operands(cb)
...
def test_a_well_formed_cb_keeps_the_shape_reduction_message(mh):
    ...
    cb = _cb(
        commands=[{"opcode": "MATMUL", "operands": {"lhs": "A0", "rhs": "W", "dst": "Y0"}}],
