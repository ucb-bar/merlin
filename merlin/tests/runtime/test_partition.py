...
from merlin.runtime.partition import Partition, PartitionError, equal_partitions, load_partition_map, parse_size
...
def test_duplicate_names_are_refused(tmp_path):
    path = _write(
        tmp_path, {"dram_bytes": "512M", "partitions": [{"name": "a", "bytes": "128M"}, {"name": "a", "bytes": "128M"}]}
    )
...
def test_an_unaligned_explicit_base_is_rounded_up_not_accepted_raw(tmp_path):
    ...
    path = _write(tmp_path, {"dram_bytes": "512M", "partitions": [{"name": "a", "bytes": "1M", "base": hex(BASE + 1)}]})
    pm = load_partition_map(path)
