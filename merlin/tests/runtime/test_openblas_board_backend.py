"""Tests for the BOARD (RVV) OpenBLAS kernel backend (default-off, additive).

Mirrors test_xnnpack_board_backend.py: the routable-matmul classifier and the MLIR
matmul->external-call rewrite are faithful and default-off. The board-build/run leg (needs the
K1 + SpacemiT toolchain) is exercised by the e2e path, not here.
"""
...
def test_routable_filter_plain_2d_f32_only():
    assert ob._is_routable("32x256xf32", "256x256xf32", "32x256xf32", "32x256xf32")
    # rank != 2 (batched) -> not routed
    assert not ob._is_routable("2x32x256xf32", "256x256xf32", "2x32x256xf32", "2x32x256xf32")
    # non-f32 -> not routed
    assert not ob._is_routable("32x256xbf16", "256x256xbf16", "32x256xbf16", "32x256xbf16")
    # dynamic dim -> not routed (static only)
    assert not ob._is_routable("?x256xf32", "256x256xf32", "?x256xf32", "?x256xf32")
...
def test_rewrite_routes_plain_matmul_to_external_call():
    ...
    assert "call @merlin_openblas_gemm_f32_0(%a, %b, %0)" in out
    # one private decl per signature, with read/read/write access annotations + inside the body
    assert 'func.func private @merlin_openblas_gemm_f32_0(' in out
    ...
    assert out.index("private @merlin_openblas_gemm_f32_0") < out.index("func.func @forward")
