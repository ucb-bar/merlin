...
def _fake_backend(monkeypatch, *, ceiling: int, seen: list | None = None):
    ...
    monkeypatch.setitem(
        ...
        type(
            ...
            {
                "exclusive": True,
                "adapters": staticmethod(lambda t: {}),
...
def test_recursive_rescale_and_column_tiling_preserve_declared_policy(monkeypatch):
    ...
    monkeypatch.setattr(CC, "_mesh_tile_binding", binding)
    a = np.ones((2, 4)) * 0.25
    w = np.ones((4, 64)) * 0.25
    out = CC.run_matmul_on_mesh(
        "t", a.tolist(), w.tolist(), operand_dtype="fp32", accum_dtype="f32", numeric_policy=policy
    )
    np.testing.assert_allclose(out, a @ w)
