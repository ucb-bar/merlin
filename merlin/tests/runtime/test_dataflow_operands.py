...
def test_returns_none_when_dataflow_genuinely_cannot_answer():
    ...
    assert (
        dataflow_operands(
            _cb({"A": {"shape": [1], "role": "input"}}, [{"opcode": "OP", "operands": {"src": "A", "dst": "nowhere"}}])
        )
        is None
    )
    # consumes nothing declared -> no leaves
    assert (
        dataflow_operands(_cb({"Y": {"shape": [1], "role": "output"}}, [{"opcode": "OP", "operands": {"dst": "Y"}}]))
        is None
    )
...
def test_leaf_order_is_deterministic():
    ...
    cb = _cb(
        {
            "b": {"shape": [2, 2], "role": "weight"},
            "a": {"shape": [2, 2], "role": "input"},
