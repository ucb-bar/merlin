...
def test_console_parser_has_one_implementation():
    from merlin.baselines import executorch_console

    for name in ("_ConsoleReading", "_leading_ms", "_read_console_ms"):
        assert getattr(et, name) is getattr(executorch_console, name)
...
def _load_et_export():
    ...
    p = module_source_path("merlin.baselines._et_export")
...
def test_extract_fqn_map_picks_deepest_module_and_skips_non_compute():
    ...
    nodes = [
        _FakeNode(
            ...
            "lin_q",
            "aten.linear.default",
            {"nn_module_stack": {"a": ("model.layers.0", "M"), "b": ("model.layers.0.self_attn", "Attn")}},
        ),
        _FakeNode(
            "call_function",
            "act",
            "aten.silu.default",
            {"nn_module_stack": {"a": ("model.layers.0", "M"), "b": ("model.layers.0.mlp", "MLP")}},
        ),
...
def _load_et_inspect():
    p = module_source_path("merlin.baselines._et_inspect")
...
def test_et_region_json_to_profiles_feeds_the_compare(tmp_path):
    ...
    merlin = [RegionProfile(name="attention", fqn="layers.0.self_attn", role="repeated_head", wall_ns=60000, cos=1.0)]
...
def test_audit_labels_portable_kernel_fallback(tmp_path, monkeypatch):
    ...
    fallbacks = [
        __import__("merlin.baselines.contract", fromlist=["ScalarFallback"]).ScalarFallback(
            symbol=sym, reason="no XNNPACK RVV ukernel (portable/scalar kernel)", region=et._region_of_symbol(sym)
        )
...
def test_present_but_unreadable_time_is_unparseable_not_absent():
    ...
    """
    drifted = (
        "I 00:00:06.075701 executorch:executor_runner.cpp:714] Model executed successfully 1 time(s) in <n/a> ms.\n"
    )
...
def test_board_run_refuses_concrete_artifact_that_exceeds_current_ram(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(et.k1_exec, "push", lambda *a, **k: pytest.fail("RAM refusal must happen before transfer"))
...
def test_reduced_capture_is_not_refused_by_a_full_model_name(monkeypatch, tmp_path):
    ...
    r = et.run_model("openvla", "int8", write=False, run_board=True, runner_override=fake_runner)
    assert called
...
def _session_manifest(tmp_path, **overrides):
    ...
    for name, data in {
        ...
        "frame0.bin": b"\0" * 12,
        "frames.bin": b"\1" * 24,
        "correctness.npz": b"correct",
        "quality.npz": b"quality",
    }.items():
    ...
    doc = {
        ...
        "stage_schedule": [
            {"name": "recurrent_step", "steps": 2, "execution": "compiled_recurrent", "timed": True},
        ],
        "parameters": {},
        "provenance": {
...
def _session_package(tmp_path):
    ...
    package_text = "\n".join(packages) + "\n"
    identity_digest = ets.session_identity_sha256(ets.plan_session_identity(ets.load_plan(manifest)))
    invocation = {
        "MERLIN_K1_TOOLCHAIN": "/exact/tc",
    ...
    build_environment = {
        ...
        "toolchain_identity": {
            "root": "/exact/tc",
            "c_compiler": {"path": "/exact/tc/bin/clang", "sha256": "1" * 64, "version": "clang test"},
            "cxx_compiler": {"path": "/exact/tc/bin/clang++", "sha256": "2" * 64, "version": "clang test"},
        },
        "model2mlir_identity": {
    ...
    (tmp_path / "session_package.json").write_text(
        json.dumps(
            {
                ...
                "runner": "executorch_session_runner",
                "observations": 2,
                "warmups": 1,
                "measurement_repeats": 3,
            }
...
def test_paper_producer_refuses_a_non_riscv_runner_before_issuing_authority(tmp_path):
    ...
    with pytest.raises(ets.ExecuTorchSessionError, match="ELF64 little-endian RISC-V"):
        ets.write_paper_producer_receipt(root)
...
def test_paper_executorch_recipe_reproduces_the_sealed_runner_without_invoking_a_compiler(tmp_path):
    ...
    result = regenerate_model_object(
        ...
        target="k1",
        compiler_input=root / ets.PAPER_COMPILER_INPUT,
        tool=tmp_path / "must-not-run",
        output=output,
        source_identity_sha256="b" * 64,
        capture_sha256="a" * 64,
        runtime_artifact_sha256=hashlib.sha256((root / "session_package.json").read_bytes()).hexdigest(),
    )
...
def test_paper_controller_materializes_sealed_executorch_runner_without_cross_compiler_execution(tmp_path, monkeypatch):
    import hashlib
    import json
    import shutil
    ...
    authority = write_toolchain_authority(
        root / "toolchain-authority.json", authority_id="sealed-executorch-test", target="k1", build_tool=tool
    )
...
def test_continuous_session_package_rejects_exporter_runtime_source_mismatch(tmp_path):
    ...
    doc["build_environment"]["executorch_identity"].update(source_git_sha="8" * 40, matches=False)
...
def test_session_build_requires_explicit_toolchain_environment_not_dotenv(monkeypatch, tmp_path):
...
def test_continuous_session_build_blocks_identity_mismatch_before_precision_dispatch(monkeypatch, tmp_path, variant):
    def mismatch():
        raise ets.ExecuTorchSessionError("ExecuTorch exporter/runtime source identity mismatch")
    ...
    with pytest.raises(ets.ExecuTorchSessionError, match="identity mismatch"):
        ets.build_session_package(
            "unused",
            variant,
            tmp_path / "session_contract.yaml",
            tmp_path / "package",
            observations=1,
            warmups=1,
...
def test_continuous_session_parser_requires_exact_stage_sum_and_affinity(tmp_path):
    ...
    log = "\n".join(
        [
            ...
            "ET_SESSION_STAGE 0 recurrent_step 10",
            "ET_SESSION_REPEAT 0 10",
            "ET_SESSION_STAGE 1 recurrent_step 11",
            "ET_SESSION_REPEAT 1 11",
            "ET_SESSION_STAGE 2 recurrent_step 12",
            "ET_SESSION_REPEAT 2 12",
            "ET_SESSION_RSS 4096",
            "ET_SESSION_DONE 2 3",
        ]
    )
    ...
    one_core_log = log.replace("AFFINITY 4 sched_getaffinity", "AFFINITY 1 sched_getaffinity").replace(
        "THREADS 4 extension_threadpool", "THREADS 1 extension_threadpool_no_pool_guard"
    )
    ...
    with pytest.raises(ets.ExecuTorchSessionError, match="exact sum"):
        ets.parse_session_console(log.replace("REPEAT 1 11", "REPEAT 1 99"), plan, requested_cores=4)
    with pytest.raises(ets.ExecuTorchSessionError, match="worker threads"):
        ets.parse_session_console(
            one_core_log.replace("extension_threadpool_no_pool_guard", "extension_threadpool"), plan, requested_cores=1
        )
...
def test_continuous_session_paper_sections_use_full_trajectory_and_opaque_stage_truth(tmp_path):
    ...
    doc = json.loads(path.read_text())
    doc.update(
        correctness_key="commands",
        quality_key="commands",
    ...
    log = "\n".join(
        [
            ...
            "ET_SESSION_THREADS 2 extension_threadpool",
            "ET_SESSION_VLEN 256 csr",
            "ET_SESSION_STAGE 0 recurrent_step 10",
            "ET_SESSION_REPEAT 0 10",
            "ET_SESSION_STAGE 1 recurrent_step 11",
    ...
    run = ets.parse_session_console(log, plan, requested_cores=2, trajectory=tmp_path / "trajectory.bin")
    conditions = {"governor": "performance", "current_khz": 1600000, "max_khz": 1600000, "max_thermal_millic": 42000}
    run = replace(run, board_conditions={"before": conditions, "after": conditions})
    sections = ets.paper_sections(
        plan, run, requested_cores=2, quality_metric="output_cosine", quality_min=0.99, framework_source_sha256="a" * 64
    )
    assert sections["correctness"]["gate_ok"] is True
    assert sections["quality"]["value"] > 0.999999
    assert sections["timing"]["samples"] == [10, 11, 12]
    assert sections["timing"]["timed_stages"] == ["visual_encode", "recurrent_step", "predict"]
...
def test_plan_routes_ops_to_the_library_that_owns_them(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(et, "pte_operators", lambda pte, **kw: {"aten::mul.out": 3, "aten::_fft_r2c.out": 4})
    plan = et.plan_kernels(tmp_path / "model.pte")
...
def test_plan_names_an_operator_executorch_does_not_implement(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        et, "pte_operators", lambda pte, **kw: {"aten::mul.out": 3, "aten::view_as_complex_copy.out": 4}
    )
    plan = et.plan_kernels(tmp_path / "model.pte")
...
def test_run_model_reports_the_missing_operator_instead_of_running_the_board(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        et,
        "export_pte",
        lambda model, b, work, **kw: et.ExportResult(
...
def test_kernel_options_reach_cmake_and_get_their_own_build_dir(monkeypatch, tmp_path):
    ...
    opt = et.cross_compile_runner(tmp_path / "w", kernel_options=("EXECUTORCH_BUILD_KERNELS_OPTIMIZED",))
