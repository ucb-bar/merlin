...
def test_int8_glue_is_weight_only_and_uses_exo_f32_gemm():
    ...
    glue = Path(exo.__file__).with_name("exo_kernels").joinpath("llama_glue_int8.c").read_text()
    ...
    fnk = Path(exo.__file__).with_name("exo_kernels").joinpath("fgemm_nk.c").read_text()
...
def test_autosched_glue_uses_autoscheduled_kernel_and_is_transpose_free():
    ...
    glue = (
        module_source_path("merlin.baselines.exo")
        .with_name("exo_kernels")
        .joinpath("llama_glue_autosched.c")
        .read_text()
    )
