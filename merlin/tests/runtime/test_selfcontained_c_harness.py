...
from merlin.runtime.selfcontained_c_harness import TensorArg, build_program, program_from_cb
...
_MODEL = IsaModel(
    target="synth", runtime_abi={"special_csrs": {"mhartid": 0xF14}, "apertures": {"console_mmio": 0xFF080000}}
)
...
def _prog():
    return build_program(
        _KFN,
        [TensorArg("W", 2, 2, [1.0, 2.0, 3.0, 4.0], "f32"), TensorArg("A0", 2, 2, [0.5, 0.25, 0.125, 1.0], "f32")],
...
def test_integer_output_uses_base10_not_float_print():
    p = build_program(
        "void k(int32_t* X, int32_t* Y){Y[0]=X[0];}",
        [TensorArg("X", 1, 1, [7], "i32")],
        [TensorArg("Y", 1, 1, [0], "i32")],
        kernel_symbol="k",
        model=_MODEL,
    )
...
_CB = {
    ...
    "canonical_inputs": {
        "W": {"shape": [2, 2], "values": [1.0, 2.0, 3.0, 4.0]},
