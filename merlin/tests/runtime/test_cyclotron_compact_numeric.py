...
def test_compact_expected_is_private_and_changes_the_linked_bounds_only() -> None:
    ...
    assert positive.blobs["_merlin_private_n_test_lo_0"] != perturbed.blobs["_merlin_private_n_test_lo_0"]
...
def test_host_dump_harness_has_named_output_but_no_answer_or_device_comparator() -> None:
    ...
    assert 'for(;;)__asm__ volatile("nop" ::: "memory");' not in harness.source
...
def test_gsim_host_dump_can_suppress_console_completion_without_embedding_answers() -> None:
    harness = H.build_external_kernel_main(
        [H.TensorArg("X", 1, 1, [0.0], "f32")],
        [H.TensorArg("Y", 1, 1, [0.0], "f32")],
        kernel_symbol="kernel",
        model=MODEL,
        host_dump_outputs=True,
        host_dump_done_marker=False,
    )

    assert '_ps("DONE\\n")' not in harness.source
    assert "MERLIN_NUMERIC" not in harness.source
    assert "_merlin_private_" not in harness.source
...
def test_legacy_predictable_answer_symbol_cannot_name_compact_key() -> None:
    ...
    assert all("n_4c02f13a2b8d4e61" in symbol for symbol in harness.blobs if symbol.startswith("_merlin_private_"))


@pytest.mark.parametrize(
    ("marker", "status", "mismatches"),
    [
...
def test_compact_parser_rejects_partial_output_check() -> None:
    with pytest.raises(MU.MuonError):
        MO._compact_numeric_from_console("MERLIN_NUMERIC PASS 1 0\nDONE\n", expected_elements=256)


def test_cyclotron_adapter_dumps_actual_output_for_ordinary_negative_control(monkeypatch, tmp_path) -> None:
    compiled = {}
    ...
    report = CG.compare({"Y": [1.0]}, result["outputs"], {"compare": "tolerance_float", "atol": 0.0, "rtol": 0.0})
    assert report["status"] == "fail"
    assert report["first_mismatch"]["observed"] == 2.0


def test_gsim_adapter_dumps_actual_output_for_ordinary_negative_control(monkeypatch, tmp_path) -> None:
    """L3 uses the same answer-free output ELF and lets the host grader fail it."""
    ...
    monkeypatch.setenv("MERLIN_MUON_TRUSTED_COMPACT_NUMERIC", "1")
    monkeypatch.setenv("GSIM_DUMP_FILE", "/tmp/poisoned-by-caller")
    monkeypatch.setenv("GSIM_DUMP_ADDR", "deadbeef")
    monkeypatch.setenv("GSIM_DUMP_LOCAL_ADDR", "badc0de")
    monkeypatch.setenv("GSIM_DUMP_LEN", "99")
    monkeypatch.setattr(MO, "gsim_status", lambda target: (True, "stub"))
    monkeypatch.setattr(GE, "emulator_path", lambda *args, **kwargs: emulator)
    monkeypatch.setattr(MU, "is_mlir_artifact", lambda source: True)
    ...
    monkeypatch.setattr(MU, "compile_mlir_forkfree", compile_stub)
    monkeypatch.setattr(MU, "fuse_soc_elf", lambda elf, workdir, **kwargs: elf)
    monkeypatch.setattr(
        RP,
        "symbol_layouts",
        lambda elf, names: {
            "_out_Y": {"address": 0x884040, "size": 4},
        },
    )

    def run_stub(*args, **kwargs):
        env = kwargs["env"]
        assert env["GSIM_DUMP_ADDR"] == "0x110884040"
        assert env["GSIM_DUMP_LOCAL_ADDR"] == "0x884040"
        assert env["GSIM_DUMP_LEN"] == "4"
        assert env["GSIM_STOP_ON_FINISH"] == "1"
        dump = Path(env["GSIM_DUMP_FILE"])
        assert dump == tmp_path / "gsim.output.bin"
        dump.write_bytes(struct.pack("<f", 2.0))
        kwargs["stdout"].write(
            b"[gsim-emu] FINISHED: cycles=456 wall=0.1s model_finished=1\n[gsim-emu] BINARY_DUMP complete bytes=4\n"
        )
        return subprocess.CompletedProcess(args[0], 0)

    monkeypatch.setattr(subprocess, "run", run_stub)

    result = MO.gsim_muon_adapter("radiance")(_cb(), "builtin.module { llvm.func @k() }", tmp_path, 60)

    assert compiled["host_dump_outputs"] is True
    assert compiled["host_dump_done_marker"] is False
    assert "compact_expected" not in compiled
    assert result["outputs"] == {"Y": [[2.0]]}
    assert result["cycles"] == 456
    assert "numeric_verdict" not in result
    assert result["host_gmem_dump"]["output"]["muon_address"] == 0x884040
    assert result["host_gmem_dump"]["output"]["soc_address"] == 0x110884040
    assert result["host_gmem_dump"]["trust_scope"] == "frozen_non_adversarial_derived_evaluation_only"
    report = CG.compare({"Y": [1.0]}, result["outputs"], {"compare": "tolerance_float", "atol": 0.0, "rtol": 0.0})
    assert report["status"] == "fail"
    assert report["first_mismatch"]["observed"] == 2.0
...
def test_gsim_host_dump_refuses_nonlocal_path(tmp_path) -> None:
    manifest = {
        "output": {"name": "Y", "dtype": "f32", "rows": 1, "cols": 1, "elements": 1, "byte_length": 4},
    }
    with pytest.raises(MU.MuonUnavailable, match="directly inside the GSIM work directory"):
        MO._gsim_host_dump_outputs(
            "[gsim-emu] FINISHED: cycles=1 model_finished=1\n[gsim-emu] BINARY_DUMP complete bytes=4\n",
            tmp_path.parent / "outside.bin",
            manifest,
            workdir=tmp_path,
        )
...
@pytest.mark.parametrize(
    ...
    [
        "[gsim-emu] BINARY_DUMP complete bytes=4\n",
        "[gsim-emu] FINISHED: cycles=1 model_finished=0\n",
        "[gsim-emu] FINISHED: cycles=1 model_finished=1\n"
        "[gsim-emu] FINISHED: cycles=2 model_finished=1\n"
        "[gsim-emu] BINARY_DUMP complete bytes=4\n",
        "[gsim-emu] FINISHED: cycles=1 model_finished=1\n[gsim-emu] BINARY_DUMP complete bytes=3\n",
    ],
)
def test_gsim_host_dump_requires_exact_completion_and_size_marker(console, tmp_path) -> None:
    dump = tmp_path / "gsim.output.bin"
    dump.write_bytes(b"\x00" * 4)
    manifest = {
        "output": {"name": "Y", "dtype": "f32", "rows": 1, "cols": 1, "elements": 1, "byte_length": 4},
    }
    with pytest.raises((MU.MuonError, MU.MuonUnavailable)):
        MO._gsim_host_dump_outputs(console, dump, manifest, workdir=tmp_path)


def test_cyclotron_compact_path_requires_explicit_non_adversarial_opt_in(monkeypatch, tmp_path) -> None:
    compiled = {}
...
def test_host_dump_symbol_lookup_failure_is_honest_unavailable(monkeypatch, tmp_path) -> None:
    ...
    with pytest.raises(MU.MuonUnavailable, match="could not bind.*_out_Y"):
        MO._cyclotron_host_dump_plan(elf, [H.TensorArg("Y", 1, 1, [0.0], "f32")], tmp_path)
...
def test_symbol_layout_reads_local_symbol_size(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args[0], 0, stdout="  123: 10884040 4194304 OBJECT  LOCAL  DEFAULT  6 _out_Y0\n", stderr=""
        ),
    )
...
"""
    ...
    rendered, record = MU._cyclotron_run_config(source, capacity, timeout_cycles=20_000_000)
...
@pytest.mark.parametrize(
    ("produced_bytes", "error"),
    [
        (4, None),
        (3, "truncated or oversized GMEM dump"),
    ],
)
def test_cyclotron_gmem_dump_is_explicit_confined_and_exact_size(monkeypatch, tmp_path, produced_bytes, error) -> None:
    elf = tmp_path / "kernel.elf"
    elf.write_bytes(b"elf")
...
""",
    ...
    if error:
        ...
        console, cycles, _summary = MU._run_cyclotron(elf, 60, gmem_dump=(0x10884040, 4, dump))
...
def test_cyclotron_gmem_dump_refuses_path_outside_elf_workdir(monkeypatch, tmp_path) -> None:
    ...
    with pytest.raises(MU.MuonError, match="directly inside the ELF work directory"):
        MU._run_cyclotron(elf, 60, gmem_dump=(0x10884040, 4, tmp_path.parent / "outside.bin"))
...
""",
    ...
    monkeypatch.setattr(
        subprocess,
        "run",
        lambda *args, **kwargs: subprocess.CompletedProcess(args[0], 1, stdout="", stderr="Error: 0\n"),
    )

    with pytest.raises(MU.MuonUnavailable, match=("configured 20000000-cycle cap.*correctness is unknown")):
        MU._run_cyclotron(elf, 60)


@pytest.mark.parametrize(
    ("returncode", "stderr", "summary_cycles"),
    [
        (2, "Error: 0\n", 20_000_000),
...
def test_cyclotron_does_not_mask_non_cap_failures(monkeypatch, tmp_path, returncode, stderr, summary_cycles) -> None:
    elf = tmp_path / "kernel.elf"
    elf.write_bytes(b"elf")
    config = tmp_path / "config.toml"
    config.write_text(
...
""",
    ...
    monkeypatch.setattr(
        subprocess,
        "run",
        lambda *args, **kwargs: subprocess.CompletedProcess(args[0], returncode, stdout="", stderr=stderr),
    )
