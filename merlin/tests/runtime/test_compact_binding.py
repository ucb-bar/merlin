...
def fixture():
    ...
    cb = {
        "tensors": {
            name: {"shape": list(e.physical_shape), "dtype": e.dtype, "role": "output" if name == "out" else "input"}
            for name, e in encodings.items()
        },
        "params": {"storage_encodings": {name: e.to_dict() for name, e in encodings.items()}},
        "kernel_abi": {
            "kind": "whole_program",
            "args": [{"tensor": name, "access": "write" if name == "out" else "read"} for name in encodings],
            "outputs": ["out"],
        },
    }
    contract = {
        ...
        "original_argument_count": 3,
        "compact_argument_count": 2,
        "bases": [asdict(BaseBuffer("inputs", 64, 32)), asdict(BaseBuffer("outputs", 64, 32))],
        "bindings": [
            asdict(PointerBinding(0, 0, 0, 12)),
            asdict(PointerBinding(1, 0, 16, 4)),
            asdict(PointerBinding(2, 1, 16, 32)),
        ],
        "binding_provenance": "explicit test allocation",
...
def test_malformed_or_unsupported_contracts_refuse(mutation):
    ...
    if mutation == "missing":
        ...
        contract["bindings"][1]["argument_index"] = 0
    elif mutation == "extent":
        contract["bindings"][0]["byte_extent"] = 8
    elif mutation == "bounds":
        contract["bindings"][2]["byte_offset"] = 48
    elif mutation == "overlap":
        contract["bindings"][1]["byte_offset"] = 4
    elif mutation == "unused":
        contract["bases"].append(asdict(BaseBuffer("unused", 1, 32)))
        contract["compact_argument_count"] = 3
    elif mutation == "width":
        facts = replace(facts, pointer_index_bits=(64, 64))
    elif mutation == "offset_alignment":
        contract["bindings"][1]["byte_offset"] = 17
    elif mutation == "base_alignment":
        facts = replace(facts, base_alignments=(2, 16))
    elif mutation == "access":
        facts = replace(facts, base_access=("readwrite", "write"))
    elif mutation == "reuse":
        contract["storage_reused"] = True
    elif mutation == "same_symbol":
        contract["compact_symbol"] = "original"
    elif mutation == "payload":
        payloads["a"] = payloads["a"][:-1]
    elif mutation == "missing_input":
        del payloads["scale"]
    elif mutation == "write_initializer":
        payloads["out"] = bytes(16)
    elif mutation == "missing_encoding":
        del cb["params"]["storage_encodings"]["scale"]
    elif mutation == "budget":
        contract["bases"][0]["byte_extent"] = 65
    elif mutation == "bool_index":
        contract["bindings"][0]["base_index"] = False
...
def test_host_authorization_rechecks_exact_cb_encoding_and_payload():
    ...
    bridge = ArgumentIdentityBridge(digest(b"raw"), digest(b"normalized"), "original", ("tensor<2x2xi16>",), (), ())
    grant = HostPrepackAuthorization(
        "a",
        digest(json.dumps(cb, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()),
        encoding,
        constant,
        bridge,
    )
    ...
    assert (
        bytes(result.original_argument_views()[0])
        == payload[:2] + payload[4:6] + b"\0\0" + payload[2:4] + payload[6:] + b"\0\0"
    )
...
""")
    ...
    built = subprocess.run(
        [compiler, "-shared", "-fPIC", "-O0", str(source), "-o", str(library)], capture_output=True, timeout=20
    )
