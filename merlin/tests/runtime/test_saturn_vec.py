...
import pytest
from vector_workloads import RUNGS
from vector_workloads import build as build_vector

from merlin.runtime import outputs_match, reference_outputs, simulate
from merlin.runtime.backends import rvv_vec_mlir
from merlin.runtime.backends.rvv_vec_codegen import generate_driver
from merlin.runtime.commandbuffer import materialize_inputs

build = partial(build_vector, target="saturn_vec", backend="spike")
...
def saturn_vec():
    ...
    selected = resolve_support("saturn")
    backend = base.get_backend("saturn_vec")
    from pathlib import Path

    assert Path(backend.__file__).resolve().is_relative_to(selected.base.resolve())
    return backend
...
def test_selected_saturn_backend_reuses_generic_codegen(request, monkeypatch):
    import subprocess

    monkeypatch.setattr(subprocess, "run", lambda *a, **k: pytest.fail("unexpected native execution"))
    monkeypatch.setattr(subprocess, "Popen", lambda *a, **k: pytest.fail("unexpected native execution"))
    saturn_vec = request.getfixturevalue("saturn_vec")
    assert saturn_vec.saturn_vec_codegen.generate_driver is generate_driver
    assert saturn_vec.saturn_vec_mlir.emit_mlir is rvv_vec_mlir.emit_mlir
    assert saturn_vec.parse_output("VOUT y 2 3 4\nDONE\n")[0] == {"y": [3, 4]}


@pytest.mark.parametrize("rung", sorted(RUNGS))
def test_vector_spike_rv64gcv_cert(rung, tmp_path, saturn_vec):
    """RVV vector kernel runs bit-exact on spike rv64gcv vs the Merlin reference."""
    if not saturn_vec.available():
        pytest.skip("spike rv64gcv toolchain unavailable")
    cb = build(rung)
...
def test_vector_mlir_emitter_is_linalg_not_c():
    ...
    emit_mlir = rvv_vec_mlir.emit_mlir
...
@pytest.mark.parametrize("rung", sorted(RUNGS))
def test_vector_mlir_host_cert(rung, tmp_path, saturn_vec):
...
def test_vector_native_rvv_emitted(rung, expect, tmp_path, saturn_vec):
