...
def test_generated_harnesses_have_one_implementation():
    from merlin.baselines import buddy_harness

    for name in ("_native_harness_c", "_m2m_harness_c", "_buddy_model_call_c"):
        assert getattr(buddy, name) is getattr(buddy_harness, name)
