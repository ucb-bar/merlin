...
@pytest.mark.skipif(
    not __import__("merlin.runtime.backends.spike_model", fromlist=["x"]).available()
    if hasattr(__import__("merlin.runtime.backends.spike_model", fromlist=["x"]), "available")
    else False,
...
def test_the_uart_backend_refuses_to_build_without_derived_facts():
    ...
    assert "#error" in src
    for macro in ("MERLIN_UART_BASE", "MERLIN_UART_DIV_OFF", "MERLIN_SYS_CLK_HZ", "MERLIN_UART_TX_FULL_BIT"):
...
def test_the_model_harness_brings_the_console_up_before_printing():
    ...
    for unit in ("model_main.c", "vlen_probe.c"):
        ...
        assert body.index("console_init();") < body.index("htif_put"), f"{unit} prints before console_init()"
