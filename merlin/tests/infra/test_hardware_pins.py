...
class TestAConfigNameIsNotADevice:
    """The mistake, measured 2026-09-19: a whole session's cycle numbers were quoted against an hwdb
    name that appears nowhere in the registry, so a reader resolving them by configuration name would
    have compared them against references measured on different silicon.

    Two bitstreams in this repo share the configuration name ``FireSimGemminiRocketConfig`` and the
    same U250 platform, and they are different devices: 43,881,284 bytes against 44,668,362, different
    digests, different Gemmini revisions. A cycle number from one is not a divisor for a cycle number
    from the other. These tests hold that distinction in place, because the tempting cleanup -- "these
    are both the gemmini FireSim bitstream, merge them" -- is precisely the wrong-device hazard.
    """

    def _bitstreams(self):
        return {n: a for n, a in P.load_artifacts().items() if a.role == "firesim_bitstream"}

    def test_every_bitstream_declares_a_digest_so_it_can_be_checked_by_content(self):
        found = self._bitstreams()
        assert found, "the registry must declare at least one bitstream to check"
        for name, a in found.items():
            assert len(a.digest) == 64, f"{name} must declare a full sha256; a name is not an identity"

    def test_one_configuration_name_covers_more_than_one_device(self):
        by_config: dict[tuple[str, str], set[str]] = {}
        for name, a in self._bitstreams().items():
            by_config.setdefault((a.target, a.config), set()).add(a.digest)
        shared = {k: v for k, v in by_config.items() if len(v) > 1}
        assert shared, (
            "no (target, config) pair maps to two digests any more. Either a bitstream lost its "
            "digest, or two distinct devices were merged into one entry -- check before deleting "
            "this test, because the registry exists for exactly that confusion."
        )

    def test_the_two_gemmini_u250_bitstreams_are_not_the_same_bytes(self):
        found = self._bitstreams()
        a, b = found.get("firesim_gemmini_rocket_u250"), found.get("firesim_gemmini_rocket_u250_30mhz")
        assert a and b, "both gemmini U250 bitstreams must stay registered separately"
        assert a.config == b.config, "the point is that they agree on the configuration NAME"
        assert a.digest != b.digest, "...and disagree on the bytes, which is what makes them two devices"
