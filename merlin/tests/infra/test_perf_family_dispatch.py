...
* ``exact_count=2``, not 3.  ``experiments/templates/phase0/performance.yaml`` freezes PK's cohort at two
...
* ``phase2.corpus.PerformanceCapsule``: one canonical view shared by all consumers.
...
_PROFILE = repo_root() / "experiments/templates/phase0/performance.yaml"
_SHA = "a" * 64


def _capsules_named(prefix: str) -> tuple[P2_CORPUS.PerformanceCapsule, ...]:
    ...
    for source in sorted(_PERF_ROOT.iterdir()):
        ...
        rows.append(
            P2_CORPUS.PerformanceCapsule(
                str(descriptor["performance"]["family"]),
                str(descriptor["name"]),
                source,
                f"_perf/{source.name}",
                descriptor,
...
def _rewritten(capsules, edit) -> tuple[P2_CORPUS.PerformanceCapsule, ...]:
    rows = []
...
def _pr_capsules() -> tuple[P2_CORPUS.PerformanceCapsule, ...]:
...
def _install_synthetic_analyzer() -> ModuleType:
    ...
    def preflight_zz_claim(descriptors, *, replicates):
        ...
        return {
            ...
            "expected_identities": [
                {"family": "ZZ", "capsule": name, "simulator": simulator, "replicate": replicate, "tier": tier}
                for name in names
...
def _synthetic_capsules(n: int = 3) -> tuple[P2_CORPUS.PerformanceCapsule, ...]:
    rows = []
    for index in range(n):
        ...
        rows.append(
            P2_CORPUS.PerformanceCapsule(
                "ZZ", name, Path(f"/frozen/_perf/{name}"), f"_perf/{name}", descriptor, _SHA, 2, 100
            )
        )
    return tuple(rows)
...
def test_a_replicate_floor_family_produces_a_candidate_handoff() -> None:
    ...
    claim = RECORD.prepare_formal_claim(capsules)
    ...
    assert RECORD.preflight_cohort(claim) == ("r000", "r001")
    assert len(claim["expected_identities"]) == len(capsules) * 2 * 2

    families = INPUTS.family_declarations(capsules, claim)
    assert [row.family for row in families] == ["PR"]
    assert families[0].acceptance == claim["declaration"]


def _sealing_inputs(capsules, claim):
    identities = list(RECORD.preflight_cohort(claim))
    simulator = str(claim["declaration"]["evidence"]["timing_simulator"])
    cells = [
        {"family": row.family, "capsule": row.capsule, "simulator": row.simulator, "replicate": row.replicate}
        for row in P2_CORPUS.expected_perf_cells(capsules, len(identities), simulator)
    ]
    families = json.loads(
        json.dumps(
            [
                {
                    ...
                    "fitted_parameters": list(row.fitted_parameters),
                    "acceptance": row.acceptance,
                }
                for row in INPUTS.family_declarations(capsules, claim)
            ]
        )
    )
...
def test_the_handoff_survives_its_own_sealed_record_validation() -> None:
    ...
    RECORD._validate_formal_claim_facts(claim, len(identities), identities, 1, cells, families)
    ...
    with pytest.raises(PAS.StageGateError, match="formal replicates drift"):
        RECORD._validate_formal_claim_facts(claim, len(identities), ["r000", "r009"], 1, cells, families)
    with pytest.raises(PAS.StageGateError, match="drift from expected cells"):
        RECORD._validate_formal_claim_facts(claim, len(identities), identities, 1, cells[:-2], families)
...
def test_every_shipped_claim_family_now_reaches_a_handoff() -> None:
    ...
    for prefix in prefixes:
        ...
        try:
            ...
            continue
        ready[prefix] = RECORD.preflight_cohort(claim)
    assert {"PK", "PQ", "PM", "PV"} <= set(ready), (ready, refused)
    for prefix, reason in refused.items():
        assert "declares no acceptance.analyzer" in reason or "replicate count" in reason, (prefix, reason)


def test_a_family_with_no_code_written_for_it_is_dispatched_from_its_declaration(synthetic_analyzer) -> None:
    ...
    assert RECORD.preflight_cohort(claim) == ("r000", "r001")
    assert INPUTS.family_declarations(capsules, claim)[0].family == "ZZ"

    # Its floor is honoured as a floor, and a request below it is refused.
    raised = RECORD.prepare_formal_claim(capsules, 4)
    assert RECORD.preflight_cohort(raised) == ("r000", "r001", "r002", "r003")
    with pytest.raises(PAS.StageGateError, match="minimum_count=2"):
        RECORD.prepare_formal_claim(capsules, 1)
...
def test_pk_handoff_is_exactly_its_own_analyzers_preflight() -> None:
    ...
    dispatched = RECORD.prepare_formal_claim(capsules)

    assert PAS._canonical_json(dispatched) == PAS._canonical_json(direct)
    assert dispatched["declaration"] == PK.supported_acceptance()
    assert RECORD.preflight_cohort(dispatched) == ("r000", "r001")
    assert INPUTS.family_declarations(capsules, dispatched)[0].acceptance == PK.supported_acceptance()
    ...
    with pytest.raises(PAS.StageGateError, match="exact_count=2"):
        RECORD.prepare_formal_claim(capsules, 3)
...
def test_a_family_declaring_no_analyzer_is_refused_by_name() -> None:
    ...
    capsules = tuple(
        P2_CORPUS.PerformanceCapsule(
            "PT",
            f"PT{index:02d}",
            Path(f"/frozen/_perf/PT{index:02d}"),
            f"_perf/PT{index:02d}",
            {
                "name": f"PT{index:02d}",
                "kind": "model_slice",
                "label": "dev",
    ...
    with pytest.raises(PAS.StageGateError, match="declares no acceptance.analyzer"):
        RECORD.prepare_formal_claim(mixed)
...
def test_removing_the_declared_analyzer_is_what_makes_the_positive_stop_passing() -> None:
    ...
    capsules = _pr_capsules()
    assert RECORD.prepare_formal_claim(capsules)["status"] == "READY"

    stripped = _rewritten(capsules, lambda d: d["performance"]["acceptance"].pop("analyzer"))
    with pytest.raises(PAS.StageGateError, match="declares no acceptance.analyzer"):
        RECORD.prepare_formal_claim(stripped)

    malformed = _rewritten(capsules, lambda d: d["performance"]["acceptance"].__setitem__("analyzer", "not-a-path"))
    with pytest.raises(PAS.StageGateError, match="unusable"):
        RECORD.prepare_formal_claim(malformed)

    absent = _rewritten(
        capsules, lambda d: d["performance"]["acceptance"].__setitem__("analyzer", "perf_no_such_analyzer.analyze/v1")
    )
    with pytest.raises(PAS.StageGateError, match="is unavailable"):
        RECORD.prepare_formal_claim(absent)
...
def test_the_refusal_comes_from_the_analyzer_the_declaration_NAMES(synthetic_analyzer) -> None:
    ...
    with pytest.raises(PAS.StageGateError, match="frozen ZZ formal claim preflight refused: ZZ has not measured"):
        RECORD.prepare_formal_claim(_synthetic_capsules())

    # Naming a real module is not enough either: the contract it freezes must be this family's.
    borrowed = _rewritten(
        _synthetic_capsules(),
        lambda d: d["performance"]["acceptance"].__setitem__("analyzer", "perf_pk_claim.analyze_pk_claim/v3"),
    )
    with pytest.raises(PAS.StageGateError, match="frozen ZZ formal claim preflight refused"):
        RECORD.prepare_formal_claim(borrowed)
...
def test_a_declaration_stating_no_replicate_count_is_refused() -> None:
    ...
    with pytest.raises(PAS.StageGateError, match="neither an exact nor a minimum"):
        RECORD.prepare_formal_claim(_rewritten(_pr_capsules(), drop_counts))
...
def test_run_facts_a_stage_cannot_supply_fail_closed() -> None:
    ...
    with pytest.raises(PAS.StageGateError, match="requires run facts this stage cannot supply"):
        RECORD.analyzer_kwargs(needs_counters, {"replicates": lambda: ["r000", "r001"]}, label="probe")
    ...
    assert RECORD.analyzer_kwargs(needs_replicates, {"replicates": lambda: ["r000", "r001"]}, label="probe") == {
...
def test_every_declared_analyzer_publishes_exactly_one_preflight_entry_point() -> None:
    ...
    for module_name, families in sorted(declared.items()):
        ...
        module = load_analyzer_module(module_name)
        entries = sorted(
            name for name in dir(module) if name.startswith("preflight_") and callable(getattr(module, name))
        )
