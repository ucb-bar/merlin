...
from merlin_experiments.phase2 import portfolio_sandbox as PS
...
from merlin_experiments.phase2 import transcript_audit as TA
...
SHA = {name: sha256_bytes(name.encode()) for name in ("target", "graph", "plan", "llvm", "buffer")}


def _synthetic_sandbox_inputs(root, *, surfaces=()):
    return PC.PackageSandboxInputs(
        AW.TC.ToolchainPaths(root, *[str(root / name) for name in ("venv", "llvm", "compat", "clang", "uv")]),
        AW.TC.SimToolchain(),
        "",
        tuple(surfaces),
    )
...
def _portfolio_authoring(experiment, *, stage_root, run_round=None):
    owner = PAuthor.PortfolioAuthoring(
        experiment,
        target_experiment=SimpleNamespace(),
        stage_root=stage_root,
        agent_inputs=SimpleNamespace(),
        frozen_functional=SimpleNamespace(),
        frozen_corpus_manifest=stage_root / "manifest.json",
        sandbox_inputs=_synthetic_sandbox_inputs(stage_root),
        model="test",
        resolved_model="test",
        effort="high",
        codex_binary=Path("codex"),
        max_tool_calls=3,
        declared_instruction_evidence={},
    )
    if run_round is not None:
        owner.run_round = run_round
    return owner
...
def test_agent_brief_joins_existing_object_comparison_without_upgrading_claims(case, expected):
    ...
    before = {
        "emission": {"candidate_lowered_sha256": "a" * 64},
        "diagnostics": {
            "captured_logical_graph": {"status": "verified", "logical_dispatch_digest": "b" * 64},
            "verified_global_plan_emission": {
                "status": "verified",
                "host_activity": {"status": "derived", "load_payload_bytes": 100, "store_payload_bytes": 100},
            },
            "machine_artifact_activity": {
                "candidate": {"status": "compiled", "source_sha256": "a" * 64, "object_sha256": "c" * 64}
            },
        },
    }
    ...
    view = AV.agent_analysis_view(record, complete_evidence="/perf-control/full.json")
...
def test_agent_brief_missing_comparison_does_not_invent_object_equality():
    ...
    view = AV.agent_analysis_view(record, complete_evidence="complete.json")
    priority = view["analysis"]["optimization_brief"]["machine_code_evidence_priority"]
...
def test_metadata_only_edit_does_not_erase_cumulative_object_change(baseline_state, expected):
    ...
    analysis = {
        "emission": {"baseline_lowered_sha256": "a" * 64, "candidate_lowered_sha256": "b" * 64},
        "diagnostics": {
    ...
    original = copy.deepcopy(record)
    brief = AV.agent_analysis_view(record, complete_evidence="full.json")["analysis"]["optimization_brief"]
...
def setup_experiment(tmp_path, *, verified=True, primary_interface_bytes=9, **experiment_options):
    ...
    def analyzer(base, current, objective, **kwargs):
        ...
        plan = {
            ...
            "source_sha256": P2_CONTRACTS.sha256_file(Path(objective.frozen_source_path) / "capsule.interface.mlir"),
        ...
        return {
            ...
            "emission": {"candidate_lowered_sha256": SHA["llvm"], "candidate_command_buffer_sha256": SHA["buffer"]},
            "diagnostics": {
    ...
    def analyzer(base, current, objective, **kwargs):
        ...
        analysis = {
            ...
            "diagnostics": {
                ...
                "arms": {"candidate": {"status": "emitted", "macs": 0, "exact": True, "movement": {"known_bytes": 0}}},
            },
        ...
        kwargs["artifact_sink"](
            {
                ...
                "command_buffer": json.loads(command_text),
...
def _portfolio_selection_analyzer(calls, *, changing_capsules, include_host_activity=True):
    ...
    def analyzer(base, current, objective, **kwargs):
        ...
        command_buffer = {
            "commands": [],
            "tensors": {},
            "params": {"portfolio_fixture": {"capsule": objective.capsule, "revision": revision}},
        }
        ...
        plan = {
            "status": "verified",
            "plan_digest": P2_CONTRACTS.document_sha256({"capsule": objective.capsule, "revision": revision}),
            "candidate_sha256": candidate_sha,
            "logical_dispatch_digest": P2_CONTRACTS.document_sha256({"capsule": objective.capsule}),
        ...
        if include_host_activity:
            plan.update(
                tasks=2 + int(bool(revision_work)),
                host_activity={
                    "status": "derived",
...
def _portfolio_selection_experiment(
    tmp_path, monkeypatch, *, changing_capsules, name="selection", include_host_activity=True
):
    snapshot, snapshot_sha = _make_test_source_snapshot(tmp_path, name + "_policy", "POLICY = 1\n")
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: _test_host_policy(snapshot))
    extra_model = tmp_path / (name + "_secondary_model")
    extra_model.mkdir()
    (extra_model / "capsule.yaml").write_text("interface_mlir: capsule.interface.mlir\n")
    (extra_model / "capsule.interface.mlir").write_text("module { func.func @secondary() }\n")
    extra = INPUTS.StageE2ESentinel(
        "secondary-model",
        str(extra_model),
        str(extra_model),
        P2_CONTRACTS.exact_tree_record(extra_model)["sha256"],
        (),
        (),
    )
    calls = []
    analyzer = _portfolio_selection_analyzer(
        calls, changing_capsules=changing_capsules, include_host_activity=include_host_activity
    )
...
def _install_fixture_isolated_worker(experiment, tmp_path):
    ...
    def sandbox_factory(baseline, package, scratch):
        return {
            arm: {
                ...
                "compiler_dependencies": experiment.inputs.compiler_dependencies(Path(selected)),
    ...
    experiment.analysis.analyzer = FixtureIsolatedWorker(
    ...
    experiment.analysis._member_analyzer = lambda: experiment.analysis.analyzer


def _make_test_source_snapshot(tmp_path, name, policy_text):
    snapshot_tool = importlib.import_module("merlin_experiments.source_snapshot")
    ...
    snapshot_tool.create(
        source,
        snapshot,
        output_root=tmp_path / "unused-out",
        source_roots=("policy",),
        python_roots=(),
        legacy_roots=(),
    )
    receipt = snapshot_tool.verify(snapshot)
    return snapshot, P2_CONTRACTS.document_sha256(receipt["files"])


def _test_host_policy(snapshot):
    source = snapshot / "policy/verifier.py"
    sources = {str(source.resolve()): P2_CONTRACTS.sha256_file(source)}
    relative = {"policy/verifier.py": P2_CONTRACTS.sha256_file(source)}
    return {
        "schema": "global_host_verification_policy_v1",
        "sources": sources,
        "sha256": P2_CONTRACTS.document_sha256(relative),
        "location_sha256": P2_CONTRACTS.document_sha256(sources),
    }
...
def _make_static_cache_experiment(tmp_path, name, snapshot, snapshot_sha, analyzer, **options):
    ...
    sentinel = INPUTS.StageE2ESentinel(
        "cache-model", str(model), str(model), P2_CONTRACTS.exact_tree_record(model)["sha256"], (), ()
    )
    experiment = G.GlobalPerfExperiment(
        baseline=baseline,
        baseline_sha256=hash_tree(baseline)["sha256"],
        ...
        source_snapshot_files_sha256=snapshot_sha,
        analyzer=analyzer,
        **options,
    )
...
def test_exact_cross_run_static_checkpoint_hit_recomputes_current_state_without_measurements(tmp_path, monkeypatch):
    first_snapshot, first_snapshot_sha = _make_test_source_snapshot(tmp_path, "first", "POLICY = 1\n")
    second_snapshot, second_snapshot_sha = _make_test_source_snapshot(tmp_path, "second", "POLICY = 1\n")
    active_policy = [_test_host_policy(first_snapshot)]
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: copy.deepcopy(active_policy[0]))
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer(seed_calls := []))
    seed, seed_candidate = _make_static_cache_experiment(
        tmp_path, "seed", first_snapshot, first_snapshot_sha, EA.analyze_whole_model_emission
    )
    seed.analysis.analyze(seed_candidate, hypothesis="produce exact static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)

    active_policy[0] = _test_host_policy(second_snapshot)
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer(new_calls := []))
    current, current_candidate = _make_static_cache_experiment(
        tmp_path, "current", second_snapshot, second_snapshot_sha, EA.analyze_whole_model_emission
    )
    receipt = current.static_analysis_import.import_checkpoint(
        current_candidate, checkpoint=checkpoint, checkpoint_sha256=P2_CONTRACTS.sha256_file(checkpoint)
    )

    assert receipt["status"] == "hit"
    assert len(seed_calls) == 1 and new_calls == []
    assert len(current.revisions.iterations) == 1
    row = current.revisions.iterations[0]
    ...
    assert Path(current.revision_session.current_artifacts(current_candidate)["interface"]).is_file()
    assert list(current.revision_session.current_portfolio_artifacts(current_candidate)) == [
        current.inputs.sentinel.capsule_sha256
    ]
    assert Path(row["submitted_snapshot"]).stat().st_mode & 0o222 == 0


def test_v11_v13_style_host_policy_byte_mutation_is_an_explicit_cache_miss(tmp_path, monkeypatch):
    first_snapshot, first_snapshot_sha = _make_test_source_snapshot(tmp_path, "v11", "POLICY = 11\n")
    second_snapshot, second_snapshot_sha = _make_test_source_snapshot(tmp_path, "v13", "POLICY = 13\n")
    active_policy = [_test_host_policy(first_snapshot)]
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: copy.deepcopy(active_policy[0]))
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer(seed_calls := []))
    seed, seed_candidate = _make_static_cache_experiment(
        tmp_path, "v11_run", first_snapshot, first_snapshot_sha, EA.analyze_whole_model_emission
    )
    seed.analysis.analyze(seed_candidate, hypothesis="v11 static analysis")
    checkpoint = seed.revision_session.seal(seed_candidate)

    active_policy[0] = _test_host_policy(second_snapshot)
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer(new_calls := []))
    current, current_candidate = _make_static_cache_experiment(
        tmp_path, "v13_run", second_snapshot, second_snapshot_sha, EA.analyze_whole_model_emission
    )
    miss = current.static_analysis_import.import_checkpoint(
        current_candidate, checkpoint=checkpoint, checkpoint_sha256=P2_CONTRACTS.sha256_file(checkpoint)
    )
    assert miss["status"] == "miss"
    assert miss["reason"] == "host_verification_policy_content_changed"
    assert current.revisions.iterations == [] and new_calls == []
    current.analysis.analyze(current_candidate, hypothesis="cold v13 analysis after safe miss")
    assert len(new_calls) == 1


def test_cross_run_static_checkpoint_direct_api_rejects_raw_symlink_and_relative_path(tmp_path, monkeypatch):
    snapshot, snapshot_sha = _make_test_source_snapshot(tmp_path, "direct_path", "POLICY = 1\n")
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: _test_host_policy(snapshot))
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer([]))
    seed, seed_candidate = _make_static_cache_experiment(
        tmp_path, "direct_path_seed", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
    seed.analysis.analyze(seed_candidate, hypothesis="produce exact static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)
    ...
    linked_current, linked_candidate = _make_static_cache_experiment(
        tmp_path, "direct_path_linked", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
    with pytest.raises(ValueError, match="absent, mutable, linked, or changed"):
        linked_current.static_analysis_import.import_checkpoint(
            linked_candidate, checkpoint=linked, checkpoint_sha256=P2_CONTRACTS.sha256_file(checkpoint)
        )

    relative_current, relative_candidate = _make_static_cache_experiment(
        tmp_path, "direct_path_relative", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
    with pytest.raises(ValueError, match="absent, mutable, linked, or changed"):
        relative_current.static_analysis_import.import_checkpoint(
            relative_candidate,
            checkpoint=Path("relative-checkpoint.json"),
            checkpoint_sha256=P2_CONTRACTS.sha256_file(checkpoint),
        )


def test_cross_run_static_checkpoint_direct_api_rejects_linked_snapshot_root(tmp_path, monkeypatch):
    snapshot, snapshot_sha = _make_test_source_snapshot(tmp_path, "snapshot_real", "POLICY = 1\n")
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: _test_host_policy(snapshot))
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer([]))
    seed, seed_candidate = _make_static_cache_experiment(
        tmp_path, "snapshot_seed", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
    seed.analysis.analyze(seed_candidate, hypothesis="produce exact static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)
    ...
    current, candidate = _make_static_cache_experiment(
        tmp_path, "snapshot_current", linked_snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
...
def test_changed_region_selects_secondary_when_primary_emission_is_unchanged(tmp_path, monkeypatch):
    experiment, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}
    )
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="change secondary lowering mechanism")

    selection = experiment.revision_session.select_changed_portfolio_member(candidate)
    assert selection["portfolio_index"] == 1
    assert selection["capsule"] == "secondary-model"
    assert selection["changed_artifact_fields"] == ["plan_digest", "lowered_sha256", "command_buffer_sha256"]
    ...
    receipt = experiment.probes.qualify_changed_region(candidate, provider=provider, timeout_s=30)
    ...
    assert consumed["semantic_receipts"] == experiment.revisions.iterations[-1]["semantic_receipts"]


def test_changed_region_selects_primary_when_primary_is_the_changed_member(tmp_path, monkeypatch):
    experiment, candidate, _ = _portfolio_selection_experiment(tmp_path, monkeypatch, changing_capsules={"cache-model"})
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="change primary lowering mechanism")
...
def _selected_member_semantic_provider(*, candidate, probes, timeout_s, portfolio_member):
    selected = probes.analysis.session.selected_changed_portfolio_context(candidate, portfolio_member)
    return {
        "status": "passed",
        "portfolio_member_binding": {
            "selection": selected["selection"],
            "previous": selected["previous"]["member_binding"],
            "current": selected["current"]["member_binding"],
        },
...
def _qualified_secondary_checkpoint(tmp_path, monkeypatch):
    ...
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="changed secondary")
    semantic = experiment.probes.qualify_changed_region(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "changed_prior",
        "cross_run",
        "prior_portfolio_digest",
        "invented_previous_binding",
        "ranking_delta",
        "cross_member",
        "legacy",
    ],
...
def test_semantic_consumer_rederives_pinned_prior_portfolio(tmp_path, monkeypatch, mutation):
    ...
    document = P2_CONTRACTS.mapping_file(checkpoint)
    ...
    reference["sha256"] = P2_CONTRACTS.sha256_file(semantic_path)
...
def test_secondary_semantic_supplement_binds_both_portfolios(tmp_path, monkeypatch):
    ...
    semantic_path = Path(experiment.revisions.iterations[-1]["semantic_receipts"][0]["path"])
    ...
    with pytest.raises(ValueError, match="supplement preceding iteration"):
        G.write_semantic_supplement(
            original_candidate_receipt=checkpoint,
            previous_iteration_receipt=wrong_previous,
            semantic_receipt=semantic_path,
            output=tmp_path / "wrong_supplement.json",
        )
...
def test_structural_selector_treats_exact_empty_operation_maps_as_known_zero():
    before = {"diagnostics": {"verified_global_plan_emission": {"host_activity": {"dynamic_operations": {}}}}}
    after = copy.deepcopy(before)
    from merlin_experiments.phase2.portfolio_checkpoint import known_structural_host_work_delta

    delta = known_structural_host_work_delta(before, after)
...
def test_changed_region_selector_records_stable_fallback_when_all_host_work_is_unknown(tmp_path, monkeypatch):
    experiment, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"cache-model", "secondary-model"}, include_host_activity=False
    )
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="same mechanism changes both portfolio members")

    selection = experiment.revision_session.select_changed_portfolio_member(candidate)
    assert selection["portfolio_index"] == 0
    assert selection["selection_basis"] == "stable_portfolio_order_no_known_structural_host_work"
    assert selection["known_ranking_metrics"] == []
    assert selection["stable_portfolio_order_tie_break_applied"] is True
    assert selection["performance_inference"] == "none"


def test_changed_region_member_artifact_tamper_refuses_before_provider(tmp_path, monkeypatch):
    experiment, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}
    )
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="change secondary lowering mechanism")
    secondary = experiment.inputs.portfolio_sentinels[1].capsule_sha256
    experiment.revisions.portfolio_artifacts[secondary]["candidate_lowered_sha256"] = "0" * 64
    called = []

    with pytest.raises(ValueError, match="binding changed"):
        experiment.probes.qualify_changed_region(
            candidate, timeout_s=30, provider=lambda **kwargs: called.append(kwargs) or {"status": "passed"}
        )
    assert called == []


def test_changed_region_action_bounds_integrity_checks_around_both_compiler_arms(tmp_path, monkeypatch):
    ...
    experiment, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name="bounded_action_integrity"
    )
    ...
    clock = [0.0]
    integrity_checks = []
    original_check_inputs = experiment.revision_session.check_inputs
    ...
    monkeypatch.setattr(experiment.revision_session, "check_inputs", check_inputs)
    monkeypatch.setattr(PA.time, "monotonic", lambda: clock[0])
    ...
    assert (
        experiment.revision_session.selected_changed_portfolio_context(candidate)["selection"]["capsule"]
        == "secondary-model"
    )
    ...
    def provider(*, candidate, probes, timeout_s, portfolio_member):
        started = PA.time.monotonic()
        selected = probes.analysis.session.selected_changed_portfolio_context(candidate, portfolio_member)
        for index, compile_method in enumerate(
            (probes.compile_previous_probe_candidate, probes.compile_probe_candidate)
        ):
            result = compile_method(
                candidate,
                selected["current"]["interface"],
                tmp_path / f"bounded_action_probe_{index}",
                timeout_s=timeout_s - (PA.time.monotonic() - started),
            )
    ...
    receipt = experiment.probes.qualify_changed_region(candidate, provider=provider, timeout_s=60)
...
def test_changed_region_compiler_action_rechecks_revision_after_external_execution(tmp_path, monkeypatch, tamper):
    ...
    experiment, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name=f"compiler_action_tamper_{tamper}"
    )
    _install_fixture_isolated_worker(experiment, tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="change secondary lowering mechanism")
    original_dependencies = experiment.inputs.compiler_dependencies
    ...
    def compile_entrypoint(*args, **kwargs):
        if tamper == "candidate":
            ...
            submitted = Path(experiment.revisions.iterations[-2]["submitted_snapshot"])
    ...
    monkeypatch.setattr(experiment.inputs, "compiler_dependencies", dependencies)
    monkeypatch.setattr(oot_runner, "load_package", lambda path: object())
    monkeypatch.setattr(worker, "run_sandboxed_entrypoint", compile_entrypoint)

    selected = experiment.revision_session.selected_changed_portfolio_context(candidate)
    with pytest.raises(ValueError, match=("candidate changed|compiler dependencies changed|compiler identity changed")):
        experiment.probes.compile_previous_probe_candidate(
            candidate, selected["current"]["interface"], tmp_path / f"tampered_action_probe_{tamper}", timeout_s=30
        )
...
def test_changed_region_cross_member_artifact_substitution_refuses(tmp_path, monkeypatch):
    ...
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="change secondary lowering mechanism")
    primary, secondary = (member.capsule_sha256 for member in experiment.inputs.portfolio_sentinels)
    experiment.revisions.portfolio_artifacts[secondary] = experiment.revisions.portfolio_artifacts[primary]

    with pytest.raises(ValueError, match="binding changed"):
        experiment.revision_session.select_changed_portfolio_member(candidate)


def test_changed_region_refuses_when_no_portfolio_emission_changed(tmp_path, monkeypatch):
    experiment, candidate, _ = _portfolio_selection_experiment(tmp_path, monkeypatch, changing_capsules=set())
    experiment.analysis.analyze(candidate, hypothesis="initial portfolio emission")
    (candidate / "source.txt").write_text("candidate-v2")
    experiment.analysis.analyze(candidate, hypothesis="metadata-only compiler revision")
    called = []
...
def test_imported_static_portfolio_bundle_can_be_previous_changed_member_context(tmp_path, monkeypatch):
    ...
    seed.analysis.analyze(seed_candidate, hypothesis="produce portfolio static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)
    ...
    assert calls == []
    assert (
        current.revision_session.current_portfolio_member_context(candidate, index=1)["member_binding"]["capsule"]
        == "secondary-model"
    )

    (candidate / "source.txt").write_text("candidate-v2")
    current.analysis.analyze(candidate, hypothesis="change secondary after imported static seed")
    selection = current.revision_session.select_changed_portfolio_member(candidate)
    assert selection["portfolio_index"] == 1
    assert selection["capsule"] == "secondary-model"
    semantic = current.probes.qualify_changed_region(
        candidate, provider=_selected_member_semantic_provider, timeout_s=30
    )
    assert Path(semantic["previous_iteration_record"]) == current.output / "iteration_0000.json"
    assert semantic["previous_iteration_record_sha256"] == P2_CONTRACTS.sha256_file(
        current.output / "iteration_0000.json"
    )
    assert G.consume_global_candidate(current.revision_session.seal(candidate))["semantic_receipts"]


def test_imported_secondary_member_can_compile_with_exact_previous_production_policy(tmp_path, monkeypatch):
    import subprocess
    ...
    seed, seed_candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name="probe_import_seed"
    )
    seed.analysis.analyze(seed_candidate, hypothesis="produce portfolio static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)

    current, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name="probe_import_current"
    )
    delegate = current.analysis.analyzer
    factory_calls = []
    ...
    def sandbox_factory(baseline, package, scratch):
        ...
        for arm, selected in (("baseline", Path(baseline)), ("candidate", Path(package))):
            result[arm] = {
                ...
                "scratch_path": str(Path(scratch).resolve()),
                "compiler_dependencies": current.inputs.compiler_dependencies(selected),
                "command_prefix": ["bwrap", "--clearenv", "PAYLOAD"],
                "bwrap_argv_length": 2,
                "answer_surfaces": [],
                "overlay_trees": {},
    ...
    current.analysis.analyzer = FixtureIsolatedWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
        sandbox_factory=sandbox_factory,
    ...
    current.analysis._member_analyzer = lambda: current.analysis.analyzer
    imported = current.static_analysis_import.import_checkpoint(
        candidate, checkpoint=checkpoint, checkpoint_sha256=P2_CONTRACTS.sha256_file(checkpoint)
    )
    reconstruction = current.revisions.iterations[0]["analysis_reuse"]["compiler_sandbox_reconstruction"]
    ...
    assert current.revisions.compiler_sandbox_sha256[0] == reconstruction["policy_set_sha256"]
    imported_submission = Path(current.revisions.iterations[0]["submitted_snapshot"])
    assert factory_calls[0][0] == imported_submission

    (candidate / "source.txt").write_text("candidate-v2")
    current.analysis.analyze(candidate, hypothesis="change secondary after imported static seed")
    assert current.revision_session.select_changed_portfolio_member(candidate)["portfolio_index"] == 1
    loaded, observed = [], []
    monkeypatch.setattr(oot_runner, "load_package", lambda path: loaded.append(path) or object())
    monkeypatch.setattr(
        worker,
        "run_sandboxed_entrypoint",
        lambda *args, **kwargs: (
            observed.append(kwargs["sandbox"]) or subprocess.CompletedProcess([], 0, "module {}", "")
        ),
    )
    secondary_interface = current.revision_session.selected_changed_portfolio_context(candidate)["previous"][
        "interface"
    ]
    result = current.probes.compile_previous_probe_candidate(
    ...
    assert observed[0]["compiler_dependencies"] == current.revisions.iterations[0]["compiler_dependencies"]
...
def test_imported_previous_probe_refuses_reconstructed_policy_tamper(tmp_path, monkeypatch):
    ...
    seed, seed_candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name="policy_tamper_seed"
    )
    seed.analysis.analyze(seed_candidate, hypothesis="produce portfolio static seed")
    checkpoint = seed.revision_session.seal(seed_candidate)
    current, candidate, _ = _portfolio_selection_experiment(
        tmp_path, monkeypatch, changing_capsules={"secondary-model"}, name="policy_tamper_current"
    )
    ...
    def sandbox_factory(baseline, package, scratch):
        return {
            arm: {
                "package_path": str(Path(selected).resolve()),
                "scratch_path": str(Path(scratch).resolve()),
                "compiler_dependencies": current.inputs.compiler_dependencies(Path(selected)),
                "command_prefix": ["bwrap", "--clearenv", "PAYLOAD"],
                "bwrap_argv_length": 2,
                "answer_surfaces": [],
                "overlay_trees": {},
    ...
    current.analysis.analyzer = IsolatedAnalysisWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
        sandbox_factory=sandbox_factory,
    ...
    current.revisions.compiler_sandboxes[0]["candidate"]["command_prefix"].append("tampered")
    scratch = tmp_path / "tampered_policy_probe"
    scratch.mkdir()
    with pytest.raises(ValueError, match="policy changed identity"):
        current.probes._probe_sandbox(candidate, scratch)
...
def test_cross_run_static_checkpoint_bundle_tamper_fails_closed(tmp_path, monkeypatch):
    ...
    seed, candidate = _make_static_cache_experiment(
        tmp_path, "tamper_seed", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
    row = seed.analysis.analyze(candidate, hypothesis="produce exact static seed")
    checkpoint = seed.revision_session.seal(candidate)
    bundle = Path(row["static_analysis_bundle"]["path"])
    bundle.chmod(0o644)
    current, current_candidate = _make_static_cache_experiment(
        tmp_path, "tamper_current", snapshot, snapshot_sha, EA.analyze_whole_model_emission
    )
...
def test_cross_run_static_checkpoint_analysis_option_change_is_a_cold_miss(tmp_path, monkeypatch):
    first_snapshot, first_snapshot_sha = _make_test_source_snapshot(tmp_path, "option_first", "POLICY = 1\n")
    second_snapshot, second_snapshot_sha = _make_test_source_snapshot(tmp_path, "option_second", "POLICY = 1\n")
    active_policy = [_test_host_policy(first_snapshot)]
    monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: copy.deepcopy(active_policy[0]))
    monkeypatch.setattr(EA, "analyze_whole_model_emission", _static_cache_analyzer([]))
    seed, candidate = _make_static_cache_experiment(
        tmp_path, "option_seed", first_snapshot, first_snapshot_sha, EA.analyze_whole_model_emission, timeout_s=300
    )
    seed.analysis.analyze(candidate, hypothesis="produce exact static seed")
    checkpoint = seed.revision_session.seal(candidate)
    active_policy[0] = _test_host_policy(second_snapshot)
    current, current_candidate = _make_static_cache_experiment(
        tmp_path,
...
def test_cross_run_identity_is_path_neutral_but_content_and_order_strict(tmp_path):
    dep = {
        ...
        "shared_sources": {"planner.py": SHA["plan"]},
        "selected_lazy_exports": {"merlin.plan": "merlin.planner"},
    }
    left_opt = {
        "schema": "global_optimization_baseline_v1",
        "selection": "explicit_host_seed",
        "path": "/old/run/optimization_baseline",
        "sha256": SHA["graph"],
    ...
    assert SI.portable_optimization_baseline_binding(left_opt) == SI.portable_optimization_baseline_binding(right_opt)
    ...
    assert SI.portable_edit_authority(edit) == SI.portable_edit_authority(moved_edit)
    assert SI.portable_historical_reference(
        {"path": "/old/history", "sha256": SHA["graph"], "summary": {"rows": 1}}
    ) == SI.portable_historical_reference({"path": "/new/history", "sha256": SHA["graph"], "summary": {"rows": 1}})
    assert SI.portable_phase1_binding(
        {"run_dir": "/old/phase1", "run_id": "p1", "evidence_sha256": {"freeze.json": SHA["buffer"]}}
    ) == SI.portable_phase1_binding(
        {"run_dir": "/new/phase1", "run_id": "p1", "evidence_sha256": {"freeze.json": SHA["buffer"]}}
    )
    ...
    digest = P2_CONTRACTS.sha256_file(first_tool)
    left_policy = {
        "schema": "machine_artifact_policy_identity_v1",
        "compiler": {"path": str(first_tool), "resolved_path": str(first_tool), "sha256": digest},
        "flags": ["-O2"],
    }
    right_policy = copy.deepcopy(left_policy)
    right_policy["compiler"].update(path=str(second_tool), resolved_path=str(second_tool))
    assert SI.portable_machine_build_policy(left_policy, verify_files=True) == SI.portable_machine_build_policy(
        right_policy, verify_files=True
    )
    second_tool.write_bytes(b"different tool")
    with pytest.raises(ValueError, match="executable or implementation changed"):
        SI.portable_machine_build_policy(right_policy, verify_files=True)

    first = GI.full_model_portfolio_identity(
        [
            SimpleNamespace(capsule="a", capsule_sha256=SHA["graph"], required_lanes=(), required_tiers=()),
            SimpleNamespace(capsule="b", capsule_sha256=SHA["plan"], required_lanes=(), required_tiers=()),
        ]
    )
    second = GI.full_model_portfolio_identity(
        [
            SimpleNamespace(capsule="b", capsule_sha256=SHA["plan"], required_lanes=(), required_tiers=()),
            SimpleNamespace(capsule="a", capsule_sha256=SHA["graph"], required_lanes=(), required_tiers=()),
        ]
    )
    assert P2_CONTRACTS.document_sha256(first) != P2_CONTRACTS.document_sha256(second)
...
def test_host_only_full_graph_and_authoring_budgets_are_distinct_from_probe_ceiling(tmp_path):
    ...
    record = experiment.analysis.analyze(candidate, hypothesis="Exercise the host-only static ceiling")
    ...
    with pytest.raises(ValueError, match="authoring bounds"):
        _portfolio_authoring(
            experiment, stage_root=tmp_path / "stage", run_round=lambda *_args, **_kwargs: {}
        ).run_sequence(candidate, max_rounds=1, total_authoring_seconds=1201, round_seconds=1201)
...
def test_launcher_and_controller_share_the_canonical_resume_portfolio_identity(tmp_path):
    ...
    (training_source / "capsule.yaml").write_text("interface_mlir: capsule.interface.mlir\n", encoding="utf-8")
    (training_source / "capsule.interface.mlir").write_text("module {}\n", encoding="utf-8")
    training = INPUTS.StageE2ESentinel(
        "training-model",
        str(training_source),
        str(training_source),
        P2_CONTRACTS.exact_tree_record(training_source)["sha256"],
        (),
        (),
    )
    experiment, _candidate, _calls = setup_experiment(tmp_path, portfolio_sentinels=(training,))
    launcher = importlib.import_module("launch_global_agent_experiment")

    expected = GI.full_model_portfolio_identity(experiment.inputs.portfolio_sentinels)

    assert launcher.full_model_portfolio_identity is GI.full_model_portfolio_identity
    assert experiment.inputs.portfolio_identity == expected
    assert experiment.inputs.portfolio_identity_sha256 == P2_CONTRACTS.document_sha256(expected)
...
def _portfolio_sentinel(tmp_path, name, *, interface_bytes=9):
    ...
    return INPUTS.StageE2ESentinel(
        name, str(source), str(source), P2_CONTRACTS.exact_tree_record(source)["sha256"], (), ()
    )
...
def test_portfolio_recompiles_every_full_graph_on_one_candidate_snapshot(tmp_path):
    ...
    first = experiment.analysis.analyze(candidate, hypothesis="cross-model global planning")
    assert len(calls) == 3
    ...
    assert all(
        row.get("analysis", first["analysis"])["candidate_sha256"] == first["candidate_sha256"]
        for row in first["portfolio"]["members"]
    )

    (candidate / "source.txt").write_text("second global proposal")
    second = experiment.analysis.analyze(candidate, hypothesis="cross-model data movement")
    assert len(calls) == 6
    assert second["portfolio"]["members_ready"] == 3
    assert all(row["static_comparison"]["previous_iteration"] == 0 for row in second["portfolio"]["members"][1:])


def test_one_worker_portfolio_uses_lpt_shared_deadline_and_declared_record_order(tmp_path, monkeypatch):
    extras = [
    ...
    base_analyzer = experiment.analysis.analyzer
    clock = [0.0]
    monkeypatch.setattr(PA.time, "monotonic", lambda: clock[0])
    budgets = []
    durations = {"real-model": 5.0, "large-second": 20.0, "small-third": 5.0, "large-fourth": 10.0}
    ...
    record = experiment.analysis.analyze(candidate, hypothesis="bounded generic portfolio allocation")
    assert [name for name, _ in budgets] == ["large-second", "large-fourth", "real-model", "small-third"]
    assert [budget for _, budget in budgets] == pytest.approx([100.0, 80.0, 70.0, 65.0])
    ...
    assert all(row["policy"] == record["portfolio"]["analysis_allocation_policy"] for row in allocations)
    assert [row["allocated_seconds"] for row in allocations] == pytest.approx([70.0, 100.0, 65.0, 80.0])
...
def test_measured_portfolio_allocation_preserves_floors_and_rolls_surplus(tmp_path):
    ...
    first = PR.portfolio_member_analysis_allocation(300.0, sentinels, emission_seconds_by_capsule_sha256=measurements)
    ...
    second = PR.portfolio_member_analysis_allocation(
        280.0, sentinels[1:], emission_seconds_by_capsule_sha256=measurements
    )
    last = PR.portfolio_member_analysis_allocation(
    ...
    assert all(
        row["policy"] == "bounded_equal_chance_floor_plus_measured_emission_cost_with_rolling_surplus"
        for row in (first, second, last)
    )


def test_portfolio_concurrency_is_memory_admitted_and_refuses_pressure():
    admitted = PR.portfolio_analysis_concurrency(
        requested_workers=4,
        members=4,
        memory_available_bytes=112 * (1024**3),
        minimum_memory_available_bytes=48 * (1024**3),
    )
    assert admitted["admitted_workers"] == 4
    bounded = PR.portfolio_analysis_concurrency(
        requested_workers=4,
        members=4,
    ...
    with pytest.raises(TimeoutError, match="below the portfolio analysis admission floor"):
        PR.portfolio_analysis_concurrency(
            requested_workers=4,
            members=4,
            memory_available_bytes=47 * (1024**3),
            minimum_memory_available_bytes=48 * (1024**3),
        )


def test_lpt_portfolio_schedule_is_deterministic_and_projects_two_safe_workers():
    schedule = PR.portfolio_concurrent_schedule([64.9, 213.5, 30.5, 241.7], workers=2)
    ...
    assert sum(schedule["worker_estimated_seconds"]) == pytest.approx(sum([64.9, 213.5, 30.5, 241.7]))
...
def test_mandatory_reserve_uses_resource_admitted_lpt_makespan(tmp_path, monkeypatch):
    ...
    experiment, _, _ = setup_experiment(
        tmp_path,
        portfolio_sentinels=extras,
        portfolio_analysis_workers=4,
        minimum_memory_available_bytes=64 * (1024**3),
    )
    observations = {
        sentinel.capsule_sha256: {"emission_wall_seconds": 1.0, "observed_analysis_wall_seconds": seconds}
        for sentinel, seconds in zip(experiment.inputs.portfolio_sentinels, [64.9, 213.5, 30.5, 241.7], strict=True)
    }
    monkeypatch.setattr(experiment.analysis, "_baseline_emission_observations", lambda: observations)
    monkeypatch.setattr(experiment.analysis, "_portfolio_analysis_cost_estimates", lambda: [64.9, 213.5, 30.5, 241.7])
    monkeypatch.setattr(PA, "_host_memory_available_bytes", lambda: 96 * (1024**3))
    reserve = experiment.analysis.mandatory_analysis_reserve_seconds(420)
...
def test_concurrent_portfolio_restores_declared_result_order(tmp_path, monkeypatch):
    ...
    experiment, candidate, _ = setup_experiment(
        ...
        portfolio_sentinels=extras,
        portfolio_analysis_workers=4,
        minimum_memory_available_bytes=16 * (1024**3),
    )
    delegate = experiment.analysis.analyzer
    ...
    experiment.analysis.analyzer = analyzer
    monkeypatch.setattr(PA, "_host_memory_available_bytes", lambda: 80 * (1024**3))
    record = experiment.analysis.analyze(candidate, hypothesis="bounded concurrent portfolio")
    ...
    assert (
        record["portfolio"]["analysis_allocation_policy"]
        == "shared_portfolio_deadline_with_measured_cost_lpt_admission"
    )


def test_one_worker_observed_case_does_not_starve_long_member_with_local_slice(tmp_path, monkeypatch):
    ...
    durations = {"real-model": 96.336, "tiny": 213.509303, "lstm": 30.469780, "smol": 241.742972}
    delegate = experiment.analysis.analyzer
    clock = [0.0]
    execution = []
    monkeypatch.setattr(PA.time, "monotonic", lambda: clock[0])
    monkeypatch.setattr(PA, "_host_memory_available_bytes", lambda: 80 * (1024**3))
    monkeypatch.setattr(experiment.analysis, "_portfolio_analysis_cost_estimates", lambda: costs)
    ...
    experiment.analysis.analyzer = analyzer
    record = experiment.analysis.analyze(candidate, hypothesis="share the exact portfolio deadline")
    assert [name for name, _ in execution] == ["smol", "tiny", "real-model", "lstm"]
    assert dict(execution)["real-model"] == pytest.approx(600 - durations["smol"] - durations["tiny"])
...
def test_exact_baseline_emission_cache_reuses_identity_and_refuses_corruption(tmp_path):
    binding = {"root": str((tmp_path / "cache").resolve()), "compiler_dependencies_sha256": SHA["plan"]}
    identity = EA.baseline_emission_cache_identity(
        baseline_sha256=SHA["graph"],
        capsule_sha256=SHA["buffer"],
        source_sha256=SHA["llvm"],
        target="test",
    ...
    cold = EA.store_baseline_emission_cache(
        binding,
        identity,
        lowered_text="module {}\n",
    ...
    warm = EA.load_baseline_emission_cache(binding, identity)
    assert warm is not None and warm["key"] == cold["key"]
    assert warm["lowered_text"] == "module {}\n"
    changed = dict(identity)
    changed["source_sha256"] = SHA["target"]
    assert EA.load_baseline_emission_cache(binding, changed) is None
    lowered = Path(binding["root"]) / warm["key"] / "lowered.mlir"
    lowered.chmod(0o644)
    lowered.write_text("module { func.func @tampered() }\n")
    with pytest.raises(P2_CONTRACTS.StageGateError, match="artifact digest changed"):
        EA.load_baseline_emission_cache(binding, identity)


def test_timed_out_worker_can_seed_only_its_completed_baseline_emission(tmp_path, monkeypatch):
    ...
    dependencies = {
        "candidate_sha256": hash_tree(baseline)["sha256"],
        "shared_sources": {},
        "selected_lazy_exports": {},
    }
    dependency_sha = SI.compiler_dependency_content_sha256(dependencies)
    cache_binding = {"root": str((tmp_path / "cache").resolve()), "compiler_dependencies_sha256": dependency_sha}
    seed = tmp_path / "seed"
    ...
    (seed / "global_iterations" / "experiment.json").write_text(
        json.dumps(
            {
                "target": "test-target",
                "optimization_baseline_sha256": hash_tree(baseline)["sha256"],
    ...
    (worker / "baseline_emission.json").write_text(
        json.dumps(
            {
    ...
    (worker / "request.json").write_text(
        json.dumps(
            {
                "baseline": str(baseline.resolve()),
                "kwargs": {"target": "test-target"},
    ...
    monkeypatch.setattr(OR, "analysis_emission_entrypoints", lambda _package: ("emit_analysis_bundle",))
    monkeypatch.setattr(ED, "validate_whole_program_schema", lambda *_args, **_kwargs: None)
    schema = {"path": "/compiler-api/command_buffer.schema.json", "sha256": SHA["target"]}
    imported = EA.seed_baseline_emission_cache_from_run(
...
def test_portfolio_action_digest_resolves_primary_and_filters_exact_authority(tmp_path):
    ...
    record = experiment.analysis.analyze(candidate, hypothesis="inspect every member")
    surface = {"id": "global", "path": "compiler.py", "symbol": "Planner.run", "scope": "pass", "effects": ["movement"]}
    action = {
        "rank": 1,
        "kind": "movement",
        "status": "actionable",
        "detail": "delete materialization",
        "evidence": {"bytes": 128},
        "required_effects": ["movement"],
        "edit_surfaces": [surface, {**surface, "id": "unapproved", "symbol": "Other.run"}],
    }
    optimization_order = {
        "schema": "macro_optimization_order_v1",
        "tiers": [{"tier": 1, "name": "whole_program_work_deletion"}],
    }
    record["analysis"]["optimization_brief"] = {"ranked_actions": [action], "optimization_order": optimization_order}
    record["analysis"]["diagnostics"]["arms"]["candidate"].update(
        {
            "macs": 8192,
            "movement": {"known_bytes": 128},
            "representation_activity": {"command_counts": {"MATMUL": 2}, "placement": {"lane_counts": {"on_mesh": 2}}},
        }
    )
    digest = AV.portfolio_action_digest(
        record,
        complete_evidence="INITIAL_FULL_MODEL_EVIDENCE.json",
        edit_contract={"existing_symbols": [{"surface_id": "global", "path": "compiler.py", "symbol": "Planner.run"}]},
    )
    assert [row["identity"]["capsule"] for row in digest["members"]] == ["real-model", "second"]
...
def test_portfolio_member_failure_blocks_current_revision_and_seal(tmp_path):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path, portfolio_sentinels=[extra])
    primary_analyzer = experiment.analysis.analyzer
    ...
    experiment.analysis.analyzer = analyzer
    record = experiment.analysis.analyze(candidate, hypothesis="proposal must cover every training model")
    assert record["readiness"]["status"] == "blocked"
    assert record["portfolio"]["members_ready"] == 1
    assert any(value.startswith("portfolio:large-transformer:") for value in record["readiness"]["blockers"])
    with pytest.raises(ValueError, match="portfolio:large-transformer"):
        experiment.revision_session.seal(candidate)
...
def test_blocked_portfolio_has_exact_non_promotable_authoring_checkpoint(tmp_path):
    ...
    experiment.analysis.analyzer = analyzer
    record = experiment.analysis.analyze(candidate, hypothesis="repair unsupported portfolio member")
    assert record["readiness"]["status"] == "blocked"

    checkpoint = experiment.revision_session.checkpoint_authoring(candidate, name="blocked_seed")
...
def test_timeout_only_blocked_portfolio_retains_all_ready_authoring_checkpoint(tmp_path, monkeypatch):
    extra = _portfolio_sentinel(tmp_path, "large-transformer")
    experiment, candidate, _ = setup_experiment(tmp_path, portfolio_sentinels=[extra], timeout_s=10)
    primary_analyzer = experiment.analysis.analyzer
    now = [0.0]
    ...
    experiment.analysis.analyzer = analyzer
    record = experiment.analysis.analyze(candidate, hypothesis="retain completed portfolio evidence")
    ...
    checkpoint = experiment.revision_session.checkpoint_authoring(candidate, name="timeout_seed")
    consumed = G.consume_authoring_checkpoint(checkpoint)
...
def test_sustained_sequence_can_repair_blocked_initial_portfolio(tmp_path):
    ...
    def analyzer(base, current, objective, **kwargs):
        if objective.capsule == "large-transformer" and (current / "source.txt").read_text() != "portfolio repaired":
            return {
                "candidate_sha256": hash_tree(current)["sha256"],
                "workload": {"capsule_sha256": objective.capsule_sha256},
                "diagnostics": {"arms": {"candidate": {"status": "emission_failed"}}},
    ...
    def author(current, *, round_index, round_timeout_s):
        ...
        repaired = experiment.analysis.analyze(current, hypothesis="implement missing model support")
    ...
    result = _portfolio_authoring(experiment, stage_root=tmp_path / "stage", run_round=author).run_sequence(
        candidate, max_rounds=1, total_authoring_seconds=30, round_seconds=30, on_round_failure="resume-last-checkpoint"
    )
    assert inherited == [(0, "candidate")]
    assert result["checkpoints"][0]["role"] == "initial_blocked_authoring_seed"
    assert G.consume_authoring_checkpoint(Path(result["checkpoints"][0]["path"]))["portfolio_members_ready"] == 1
    assert G.consume_global_candidate(Path(result["last_good_checkpoint"]["path"]))["portfolio"]["members"]
...
def test_sustained_sequence_refuses_regression_of_ready_portfolio_member(tmp_path):
    ...
    def analyzer(base, current, objective, **kwargs):
        if objective.capsule == "real-model" and (current / "source.txt").read_text() == "regress primary":
            return {
                "candidate_sha256": hash_tree(current)["sha256"],
                "workload": {"capsule_sha256": objective.capsule_sha256},
                "diagnostics": {"arms": {"candidate": {"status": "emission_failed"}}},
    ...
    def author(current, *, round_index, round_timeout_s):
        ...
        experiment.analysis.analyze(current, hypothesis="bad trade between model families")
        return {"status": "authored"}

    with pytest.raises(ValueError, match="regressed previously verified portfolio members"):
        _portfolio_authoring(experiment, stage_root=tmp_path / "stage", run_round=author).run_sequence(
            candidate, max_rounds=1, total_authoring_seconds=30, round_seconds=30, on_round_failure="stop"
        )
...
def test_agent_view_compacts_secondary_portfolio_analysis(tmp_path):
    extra = _portfolio_sentinel(tmp_path, "large-transformer")
    experiment, candidate, _ = setup_experiment(tmp_path, portfolio_sentinels=[extra])
    record = experiment.analysis.analyze(candidate, hypothesis="inspect every full graph")
    secondary = record["portfolio"]["members"][1]["analysis"]
    secondary["optimization_brief"] = {
        "ranked_actions": [
            {
                "rank": 1,
                "kind": "movement",
                "status": "actionable",
                "detail": "reuse encoding",
                "evidence": {"bytes": 10},
                "required_effects": ["movement"],
                "edit_surfaces": [
                    {
                        "id": "layout",
                        "path": "compiler/layout.py",
                        "symbol": "plan",
                        "scope": "planner",
                        "effects": ["movement"],
    ...
    assert compact["optimization_brief"]["ranked_actions"][0]["edit_surfaces"] == [
        {"id": "layout", "path": "compiler/layout.py", "symbol": "plan", "scope": "planner", "effects": ["movement"]}
    ]
...
def test_optimization_baseline_requires_exact_explicit_pair(tmp_path, missing):
    ...
    options = {"optimization_baseline": seed, "optimization_baseline_sha256": hash_tree(seed)["sha256"]}
    if missing == "path":
...
def test_optimization_baseline_is_snapshot_not_phase1_or_previous_probe(tmp_path):
    ...
    class Phase1:
        def verify(self, baseline):
            ...
            return {"submission_sha256": hash_tree(baseline)["sha256"], "run_dir": str(tmp_path), "evidence_sha256": {}}

    experiment, candidate, _ = setup_experiment(
        tmp_path,
        phase1=Phase1(),
        optimization_baseline=seed,
        optimization_baseline_sha256=seed_sha,
        optimization_baseline_reason="external objective needs the explicit compiler feature",
    )
    original_analyzer = experiment.analysis.analyzer
    compiled_bases = []
    ...
    experiment.analysis.analyzer = analyze
    # Only the host-captured copy is an input after construction.
    (seed / "compiler.py").write_text("VALUE=2\n")
    first = experiment.analysis.analyze(candidate, hypothesis="initial objective seed")
    (candidate / "source.txt").write_text("actual proposal")
    second = experiment.analysis.analyze(candidate, hypothesis="next proposal")
    assert all(path == experiment.inputs.baseline for path in verified)
    assert compiled_bases == [experiment.inputs.optimization_baseline] * 2
    assert experiment.inputs.optimization_baseline != seed
    assert hash_tree(experiment.inputs.optimization_baseline)["sha256"] == seed_sha
    assert not experiment.inputs.optimization_baseline.stat().st_mode & 0o222
    assert first["baseline_sha256"] == experiment.inputs.baseline_sha256 != seed_sha
    ...
    assert consumed["baseline_sha256"] == experiment.inputs.phase1_binding["submission_sha256"]
    ...
    brief = AV.agent_analysis_view(second, complete_evidence="complete.json")["analysis"]["optimization_brief"]
    ...
    frozen_source = experiment.inputs.optimization_baseline / "compiler.py"
...
def test_optimization_baseline_shared_dependency_mutation_is_rejected(tmp_path):
    ...
    experiment, _, _ = setup_experiment(
        tmp_path,
        compiler_shared_source_root=root,
        optimization_baseline=seed,
...
def _historical_bundle(tmp_path):
    ...
    raw = json.dumps(
        {
            ...
            "target": "test-target",
            "tiers": {
                "L3": {"status": "pass", "cycles": 20, "engine": "test-engine", "sim_provenance": {"sha256": "e" * 64}}
            },
        }
    ).encode()
    row = bind_historical_reference(
        Observation("submission", "public-work", "L3", "console", "rtl", "total_cycles", 20, "cycles", status="pass"),
        target="test-target",
        receipt=raw,
        receipt_sha256=sha256_bytes(raw),
        receipt_path="public/public-work/capsule_result.json",
        public_workloads={"public-work"},
        artifacts={"kernel.elf": b"elf", "harness.c": b"caller"},
        artifact_sha256={"kernel.elf": sha256_bytes(b"elf"), "harness.c": sha256_bytes(b"caller")},
        executable_names=("kernel.elf",),
        harness_name="harness.c",
    )
    bundle = {
        "schema": "historical_reference_bundle_v1",
        "records": [row],
    ...
    path.write_bytes(P2_CONTRACTS.canonical_json(bundle))
    return path, P2_CONTRACTS.sha256_file(path), bundle
...
@pytest.mark.parametrize(
    "mutation", ["bundle_sha", "record_sha", "summary", "authority", "target", "symlink", "candidate_writable"]
)
def test_historical_reference_explicit_bundle_fails_closed(tmp_path, mutation):
    ...
    if mutation == "bundle_sha":
        ...
        record["reference_sha256"] = sha256_bytes(
            json.dumps(
                {k: v for k, v in record.items() if k != "reference_sha256"}, sort_keys=True, separators=(",", ":")
            ).encode()
        )
    ...
    if mutation in ("record_sha", "summary", "authority"):
        path.write_bytes(P2_CONTRACTS.canonical_json(bundle))
        digest = P2_CONTRACTS.sha256_file(path)
    with pytest.raises(ValueError):
        CHECKPOINT.load_historical_reference(
            path, digest, target="other-target" if mutation == "target" else "test-target", candidate_roots=roots
        )
...
def test_historical_reference_compact_feedback_and_readonly_control_copy(tmp_path):
    path, digest, bundle = _historical_bundle(tmp_path)
    experiment, candidate, _ = setup_experiment(
        tmp_path, historical_reference_path=path, historical_reference_sha256=digest
    )
    row = experiment.analysis.analyze(candidate, hypothesis="Inspect actual graph with historical context")
    original = copy.deepcopy(row)
    view = AV.agent_analysis_view(row, complete_evidence="full.json")
    ...
    sealed = experiment.revision_session.seal(candidate)
    assert G.consume_global_candidate(sealed)["historical_reference"] == experiment.inputs.historical_reference
    retained = Path(experiment.inputs.historical_reference["path"])
...
def test_absent_historical_reference_preserves_legacy_brief(tmp_path):
    ...
    row = experiment.analysis.analyze(candidate, hypothesis="Legacy no historical data")
    view = AV.agent_analysis_view(row, complete_evidence="full.json")
...
def test_historical_reference_cannot_originate_in_candidate_or_author_workspace(tmp_path):
    ...
    path.rename(moved)
    experiment.inputs.historical_reference_source = moved
    with pytest.raises(ValueError, match="candidate-writable"):
        experiment.analysis.analyze(candidate, hypothesis="Must not read candidate-selected reference")
    assert not calls
    moved.rename(path)
    experiment.inputs.historical_reference_source = path
...
def test_historical_reference_interpretation_sources_are_policy_pinned():
    ...
    for leaf in ("historical_reference.py", "harvest.py", "work_volume.py"):
        ...
        assert sources[str(path.resolve())] == P2_CONTRACTS.sha256_file(path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "merlin_experiments.phase2.contracts",
        "merlin_experiments.phase2.corpus",
        "merlin_experiments.phase2.reporting",
        "merlin_experiments.phase2.statistics",
        "merlin_experiments.phase2.broker_policy",
        "merlin_experiments.phase2.corpus_feedback",
        "merlin_experiments.phase2.whole_model",
        "merlin_experiments.phase2.static_identity",
        "merlin_experiments.phase2.static_cache",
        "merlin_experiments.phase2.edit_authority",
        "merlin_experiments.phase2.agent_view",
        "merlin_experiments.phase2.agent_workspace",
        "merlin_experiments.phase2.stage_prompt",
        "merlin_experiments.phase2.development_feedback",
        "merlin_experiments.phase2.feedback_metrics",
        "merlin_experiments.phase2.calibration",
        "merlin_experiments.phase2.capsule_verdict",
        "merlin_experiments.phase2.measurement_support",
        "merlin_experiments.phase2.paired_measurement",
        "merlin_experiments.phase2.candidate_record",
        "merlin_experiments.phase2.candidate_verification",
        "merlin_experiments.phase2.stage_inputs",
        "merlin_experiments.phase2.emission_diagnostics",
        "merlin_experiments.phase2.emission_analysis",
        "merlin_experiments.phase2.paired_inputs",
        "merlin_experiments.phase2.revealed_corpus",
        "merlin_experiments.phase2.authoring",
        "merlin_experiments.phase2.authoring_cli",
        "merlin_experiments.phase2.gsim_certificate",
        "merlin_experiments.phase2.heldout_qualification",
        "merlin_experiments.phase2.functional_cohort",
        "merlin_experiments.phase2.functional_coverage",
        "merlin_experiments.phase2.functional_qualification",
        "merlin_experiments.phase2.qualification_policy",
        "merlin_experiments.phase2.holdout_corpus",
        "merlin_experiments.phase2.checkpoint_admission",
        "merlin_experiments.phase2.checkpoint_controller",
        "merlin_experiments.phase2.checkpoint_cli",
        "merlin_experiments.phase2.chia_launch",
        "merlin_experiments.phase2.paired_cli",
        "merlin_experiments.phase2.gsim_workload",
        "merlin_experiments.phase2.claims.dispatch",
        "merlin_experiments.phase2.portfolio_resources",
        "merlin_experiments.phase2.revision_journal",
        "merlin_experiments.source_snapshot",
    ],
)
def test_shared_and_relocated_policy_sources_remain_byte_pinned(tmp_path, monkeypatch, module):
    from merlin.common.paths import module_source_path

    provider = module_source_path(module).resolve()
    if module in {"merlin.perf.source_program_pair_provider", "merlin.perf.analysis_worker"}:
        assert provider.is_relative_to(repo_root() / "packages/merlin-experiments/src")
    policy = G.host_verification_policy_record()
    experiment, candidate, calls = setup_experiment(tmp_path)
    original_hash = P2_CONTRACTS.sha256_file
    assert policy["sources"][str(provider)] == original_hash(provider)
    assert SI.host_policy_content_sha256(policy, source_root=repo_root()) == policy["sha256"]
    monkeypatch.setattr(
        P2_CONTRACTS, "sha256_file", lambda path: "0" * 64 if path.resolve() == provider else original_hash(path)
    )
    monkeypatch.setattr(SI, "_sha256_file", P2_CONTRACTS.sha256_file)
    changed = G.host_verification_policy_record()
    assert changed["sha256"] != policy["sha256"]
    assert changed["location_sha256"] != policy["location_sha256"]
    assert SI.host_policy_content_sha256(changed, source_root=repo_root()) != policy["sha256"]
    with pytest.raises(ValueError, match="host verification policy changed"):
        experiment.analysis.analyze(candidate, hypothesis="Must not reuse evidence interpreted by changed source")
    assert calls == []


@pytest.mark.parametrize("resource", ["gate_phases.yaml", "hardware_pins.yaml"])
def test_analysis_contract_resource_drift_refuses_host_policy_reuse(tmp_path, monkeypatch, resource):
    selected = (repo_root() / "merlin/contract" / resource).resolve()
    policy = G.host_verification_policy_record()
    original_hash = P2_CONTRACTS.sha256_file
    assert policy["sources"][str(selected)] == original_hash(selected)
    experiment, candidate, calls = setup_experiment(tmp_path)
    monkeypatch.setattr(
        P2_CONTRACTS, "sha256_file", lambda path: "0" * 64 if path.resolve() == selected else original_hash(path)
    )
    monkeypatch.setattr(SI, "_sha256_file", P2_CONTRACTS.sha256_file)
    changed = G.host_verification_policy_record()
    assert changed["sha256"] != policy["sha256"]
    assert changed["location_sha256"] != policy["location_sha256"]
    with pytest.raises(ValueError, match="host verification policy changed"):
        experiment.analysis.analyze(candidate, hypothesis="Changed contract resources require new qualification")
    assert calls == []


@pytest.mark.parametrize("change", ["add", "remove"])
def test_phase2_package_membership_invalidates_host_policy(tmp_path, monkeypatch, change):
    from merlin.common import source_membership
    from merlin.common.paths import module_source_path

    package = module_source_path("merlin_experiments.phase2").parent
    original_members = source_membership.python_members
    expected = original_members(package)
    policy = G.host_verification_policy_record()
    assert all(str(path) in policy["sources"] for path in expected.values())
    experiment, candidate, calls = setup_experiment(tmp_path)

    def changed_members(directory, **kwargs):
        members = original_members(directory, **kwargs)
        if directory != package:
            return members
        if change == "remove":
            del members["corpus.py"]
        else:
            # Simulate discovery of an additional ordinary source member without
            # changing the working tree. The shared walker has real file tests.
            members["new_policy.py"] = module_source_path("merlin_experiments.phase1.options")
        return members

    monkeypatch.setattr(G.HP, "python_members", changed_members)
    changed = G.host_verification_policy_record()
    assert changed["sha256"] != policy["sha256"]
    assert changed["location_sha256"] != policy["location_sha256"]
    with pytest.raises(ValueError, match="host verification policy changed"):
        experiment.analysis.analyze(candidate, hypothesis="Changed package membership requires new qualification")
    assert calls == []
...
def test_analysis_only_uses_controller_once_without_probe_or_seal(tmp_path, monkeypatch, verified):
    launcher = importlib.import_module("launch_global_agent_experiment")
    from merlin_experiments.phase2 import portfolio_worker

    experiment, candidate, calls = setup_experiment(tmp_path, verified=verified)
    configured = []
    monkeypatch.setattr(portfolio_worker, "configure_global_analysis", lambda *a, **kw: configured.append((a, kw)))
    monkeypatch.setattr(
        experiment.revision_session, "seal", lambda *_a, **_kw: pytest.fail("analysis-only attempted sealing")
    )
    monkeypatch.setattr(portfolio_worker, "PortfolioAuthoring", lambda *_a, **_kw: pytest.fail("authoring launched"))
    status = launcher.run_analysis_only(
        experiment,
        candidate,
        stage_root=tmp_path,
    ...
    receipt = P2_CONTRACTS.mapping_file(tmp_path / "analysis_only.json")
    assert all(
...
def test_input_graph_alone_does_not_admit_probe_or_seal(tmp_path):
    ...
    row = experiment.analysis.analyze(candidate, hypothesis="Improve layout propagation")
    ...
    with pytest.raises(ValueError, match="global iteration is not ready"):
        experiment.revision_session.seal(candidate)
    with pytest.raises(ValueError, match="global iteration is not ready"):
        experiment.probes.measure_probe(candidate, admission_inputs={}, execute=lambda **_: pytest.fail("ran"))
...
def test_host_guidance_surfaces_reach_full_graph_feedback_without_expanding_authority(tmp_path, case, monkeypatch):
    ...
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    declarations = [
        {
            "id": "overlap",
            "scope": "codegen",
    ...
    experiment.freeze_edit_scope(candidate, contract, host_surface_declarations=declarations)
    if case == "tampered":
        experiment.edit_authority.guidance_inventory = None
        with pytest.raises(ValueError, match="guidance.*changed"):
            experiment.analysis.analyze(candidate, hypothesis="Changed host guidance")
        assert not calls
        return
    ...
    current = experiment.revision_session.inspect_optimization_surfaces(candidate)
    assert [item["id"] for item in current["surfaces"]] == ["overlap"]
    row = experiment.analysis.analyze(candidate, hypothesis="Expose the full approved global surface")
    ...
    assert experiment.edit_authority.contract == contract
    assert len(calls) == 1
    ...
    read_mapping = P2_CONTRACTS.mapping_file
    monkeypatch.setattr(
        P2_CONTRACTS,
        "mapping_file",
        lambda path, **kwargs: (
            {"entry": "frozen_entry"} if Path(path).name == "capsule.yaml" else read_mapping(path, **kwargs)
        ),
    )
    monkeypatch.setattr(
        source_convolution_preparation, "prepare_source_convolution", lambda **kwargs: {"status": "prepared"}
    )
    preparation = experiment.probes.prepare_source_convolution(
        candidate, comparison_arm="optimization_baseline", timeout_s=30
    )
...
def test_launcher_loads_only_host_pinned_guidance_inventory(tmp_path, case):
    ...
    receipt = {"guidance_inventory_sha256": sha256_bytes(raw)}
    if case == "legacy_pin":
        receipt = {"unchanged_catalog_file_sha256": {"inventory.json": sha256_bytes(raw)}}
    elif case == "unpinned":
        receipt = {}
...
def test_host_scope_checked_before_compile_and_again_by_sealed_consumer(tmp_path):
    ...
    contract = {
        "schema": "compiler_edit_contract_v1",
        "existing_symbols": [{"surface_id": "lowering", "path": "compiler.py", "symbol": "allowed"}],
        "helper_extensions": [],
    }
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    experiment.freeze_edit_scope(candidate, contract)
    experiment.analysis.analyze(candidate, hypothesis="Initial frozen scope")
    ...
    with pytest.raises(ValueError, match="host-frozen authority"):
        experiment.analysis.analyze(candidate, hypothesis="Unauthorized change")
    assert len(calls) == 1
    assert list(experiment.output.glob("edit_scope_refusal_*.json"))
    compiler.write_text("def allowed():\n return 7\ndef protected():\n return 2\n")
    experiment.analysis.analyze(candidate, hypothesis="Approved lowering change")
    sealed = experiment.revision_session.seal(candidate)
    assert len(calls) == 2
    assert G.consume_global_candidate(sealed)["global_speedup_proven"] is False
    # A candidate cannot replace the host-frozen contract after successful compilation.
    experiment.edit_authority.contract["existing_symbols"].append(
...
def _freeze_test_mechanism_catalog(experiment, candidate, tmp_path, mechanisms):
    contract = {
        "schema": "compiler_edit_contract_v1",
        "existing_symbols": [
            {"surface_id": name, "path": "compiler.py", "symbol": name} for name in ("optimize", "schedule")
        ],
        "helper_extensions": [],
    }
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    ...
    catalog["sha256"] = P2_CONTRACTS.document_sha256(catalog)
    path = tmp_path / "mechanism_catalog.json"
    path.write_bytes(P2_CONTRACTS.canonical_json(catalog))
    path.chmod(0o444)
    binding = experiment.freeze_mechanism_catalog(path, P2_CONTRACTS.sha256_file(path))
    return path, binding
...
def _test_mechanism_work_order(experiment, candidate, *, mechanism_id="epilogue"):
    ...
    rows = []
    for index, sentinel in enumerate(experiment.inputs.portfolio_sentinels):
        rows.append(
            {
                "capsule": sentinel.capsule,
                "capsule_sha256": sentinel.capsule_sha256,
                "compiler_sha256": candidate_sha256,
                "source_sha256": P2_CONTRACTS.sha256_file(Path(sentinel.frozen_source_path) / "capsule.interface.mlir"),
                "plan_digest": SHA["plan"],
                "candidate_command_buffer_sha256": SHA["buffer"],
                "candidate_lowered_sha256": SHA["llvm"],
    ...
    document = {
        ...
        "catalog_sha256": experiment.mechanism_program.catalog["sha256"],
        "contract_sha256": experiment.edit_authority.contract["sha256"],
        "initial_candidate_sha256": candidate_sha256,
        "round_start_candidate_sha256": candidate_sha256,
        "ordered_portfolio": experiment.inputs.portfolio_identity["members"],
        "portfolio_sha256": experiment.inputs.portfolio_identity_sha256,
        "source_operation_ids": [],
        "portfolio_site_bindings": rows,
    }
    document["sha256"] = P2_CONTRACTS.document_sha256(document)
    return document


def _freeze_test_mechanism_work_order(experiment, candidate, tmp_path, *, mechanism_id="epilogue"):
    document = _test_mechanism_work_order(experiment, candidate, mechanism_id=mechanism_id)
    path = tmp_path / "mechanism_work_order.json"
    path.write_bytes(P2_CONTRACTS.canonical_json(document))
    path.chmod(0o444)
    return path, experiment.freeze_mechanism_work_order(path, P2_CONTRACTS.sha256_file(path), candidate=candidate)


def test_mechanism_catalog_gates_before_analysis_and_binds_iteration_checkpoint(tmp_path):
    experiment, candidate, calls = setup_experiment(tmp_path)
    (candidate / "compiler.py").write_text("def optimize():\n    return 1\n\ndef schedule():\n    return 1\n")
    _path, binding = _freeze_test_mechanism_catalog(
        experiment,
        candidate,
        tmp_path,
        [
            {
                "id": "epilogue",
                "selectors": [{"kind": "function", "path": "compiler.py", "symbol": "optimize"}],
            }
        ],
    )
    seed = experiment.analysis.analyze(candidate, hypothesis="bind exact seed")
    ...
    gate = experiment.revision_session.finalize_mechanism_round(candidate, round_index=0)
    assert gate["status"] == "allowed"
    assert gate["selected_mechanism_id"] == "epilogue"
    assert len(calls) == 1  # Attribution itself never compiles a model.
    row = experiment.analysis.analyze(candidate, hypothesis="delete exact epilogue materialization")
    assert len(calls) == 2
...
def test_multi_mechanism_candidate_is_refused_before_full_model_analyzer(tmp_path):
    experiment, candidate, calls = setup_experiment(tmp_path)
    (candidate / "compiler.py").write_text("def optimize():\n    return 1\n\ndef schedule():\n    return 1\n")
    _freeze_test_mechanism_catalog(
        experiment,
        candidate,
        tmp_path,
        [
            {"id": "epilogue", "selectors": [{"kind": "function", "path": "compiler.py", "symbol": "optimize"}]},
            {"id": "latency", "selectors": [{"kind": "function", "path": "compiler.py", "symbol": "schedule"}]},
        ],
    )
    experiment.analysis.analyze(candidate, hypothesis="bind exact seed")
    experiment.revision_session.begin_mechanism_round(candidate, round_index=0)
    compiler = candidate / "compiler.py"
    compiler.write_text(compiler.read_text().replace("return 1", "return 2"))

    with pytest.raises(ValueError, match="one-mechanism policy"):
        experiment.analysis.analyze(candidate, hypothesis="attempt two mechanisms")
    assert len(calls) == 1
    refusal = P2_CONTRACTS.mapping_file(next(experiment.output.glob("mechanism_analysis_refusal_*.json")))
...
def test_final_mechanism_gate_records_and_refuses_semantic_noop(tmp_path):
    ...
    with pytest.raises(ValueError, match="refused compiler mechanism round"):
        experiment.analysis.analyze(candidate, hypothesis="must not compile no-op round")
    assert len(calls) == 1
...
def test_mechanism_catalog_requires_raw_absolute_readonly_exact_file(tmp_path):
    ...
    contract = {
        "schema": "compiler_edit_contract_v1",
        "existing_symbols": [{"surface_id": "optimize", "path": "compiler.py", "symbol": "optimize"}],
        "helper_extensions": [],
    }
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    experiment.freeze_edit_scope(candidate, contract)
    ...
    real.write_bytes(P2_CONTRACTS.canonical_json(catalog))
    digest = P2_CONTRACTS.sha256_file(real)
    ...
    assert P2_CONTRACTS.mapping_file(Path(binding["frozen_path"])) == catalog
    assert P2_CONTRACTS.mapping_file(experiment.output / "compiler_mechanism_catalog_receipt.json") == binding
...
def test_launcher_requires_mechanism_catalog_path_sha_and_edit_contract(flag, tmp_path):
    ...
    with pytest.raises(SystemExit) as caught:
        launcher.main(
            ["--campaign-config", "not-read.json", "--candidate", "candidate", "--output", "not-created", flag, value]
        )
    assert caught.value.code == 2
...
def test_mechanism_work_order_is_immutable_and_binds_current_member_artifacts(tmp_path):
    ...
    _path, binding = _freeze_test_mechanism_work_order(experiment, candidate, tmp_path)
    initial = experiment.analysis.analyze(candidate, hypothesis="bind exact work-order artifacts")
    ...
    analysis_binding = experiment.analysis.bind_mechanism_work_order_analysis(initial)
    bound_seed = experiment.analysis.record_bound_mechanism_work_order_seed(candidate, initial)
    assert bound_seed["iteration"] == initial["iteration"] + 1
    assert bound_seed["mechanism_work_order_analysis"] == analysis_binding
    seed_checkpoint = experiment.revision_session.seal(candidate, name="bound_seed")
    assert G.consume_global_candidate(seed_checkpoint)["mechanism_work_order_analysis"] == analysis_binding
    assert analysis_binding["candidate_sha256"] == hash_tree(candidate)["sha256"]
    assert analysis_binding["members"][0]["site_binding_sha256"] == P2_CONTRACTS.document_sha256(
        experiment.mechanism_program.work_order["portfolio_site_bindings"][0]
    )
    assert P2_CONTRACTS.mapping_file(Path(binding["frozen_path"])) == binding["work_order"]
    assert len(calls) == 1
    experiment.revision_session.begin_mechanism_round(candidate, round_index=0)
    compiler = candidate / "compiler.py"
    compiler.write_text(compiler.read_text().replace("return 1", "return 2", 1))
    assert (
        experiment.revision_session.finalize_mechanism_round(candidate, round_index=0)["selected_mechanism_id"]
        == "epilogue"
    )
    row = experiment.analysis.analyze(candidate, hypothesis="execute exact assigned mechanism")
    assert experiment.analysis.bind_mechanism_work_order_analysis(row) == analysis_binding
...
def test_sustained_sequence_bootstraps_work_order_before_first_authoring_round(tmp_path):
    ...
    def author(current, *, round_index, round_timeout_s):
        experiment.revision_session.begin_mechanism_round(current, round_index=round_index)
        current_analysis = experiment.analysis.analyze(current, hypothesis="inspect bound work-order seed")
        assert (
            experiment.analysis.bind_mechanism_work_order_analysis(current_analysis)
            == experiment.mechanism_program.analysis_binding
        )
        compiler = current / "compiler.py"
        compiler.write_text(compiler.read_text().replace("return 1", "return 2", 1))
        assert (
            experiment.revision_session.finalize_mechanism_round(current, round_index=round_index)["status"]
            == "allowed"
        )
        experiment.analysis.analyze(current, hypothesis="execute assigned epilogue mechanism")
        return {"status": "authored"}
    ...
    assert len(calls) == 2
    assert experiment.revisions.iterations[0]["mechanism_work_order_analysis"] is None
    assert (
        experiment.revisions.iterations[1]["mechanism_work_order_analysis"]
        == experiment.mechanism_program.analysis_binding
    )
    consumed = G.consume_global_candidate(Path(result["last_good_checkpoint"]["path"]))
    assert consumed["mechanism_work_order_analysis"] == experiment.mechanism_program.analysis_binding
...
def test_sustained_sequence_reuses_immutable_current_analysis_across_work_order_rounds(tmp_path):
    ...
    def author(current, *, round_index, round_timeout_s):
        experiment.revision_session.begin_mechanism_round(current, round_index=round_index)
        current_analysis = experiment.analysis.analyze(
        ...
        experiment.analysis.analyze(current, hypothesis=f"execute epilogue round {round_index}")
        if round_index == 0:
            ...
            experiment.probes.charge_probe_preparation(current, 0.25)
        return {"status": "authored"}

    result = _portfolio_authoring(experiment, stage_root=tmp_path / "stage", run_round=author).run_sequence(
        candidate, max_rounds=2, total_authoring_seconds=60, round_seconds=30, on_round_failure="stop"
    )
    ...
    assert len(calls) == 3
    assert experiment.revisions.iterations[-1]["candidate_sha256"] == result["last_good_checkpoint"]["candidate_sha256"]
    assert (
        experiment.revisions.iterations[-1]["mechanism_work_order_analysis"]
        == experiment.mechanism_program.analysis_binding
    )
...
def test_work_order_continuation_static_analysis_bindings_still_fail_closed(tmp_path, mutation):
    ...
    _freeze_test_mechanism_work_order(experiment, candidate, tmp_path)
    initial = experiment.analysis.analyze(candidate, hypothesis="bind initial exact analysis")
    experiment.analysis.bind_mechanism_work_order_analysis(initial)
    experiment.revision_session.begin_mechanism_round(candidate, round_index=0)
    (candidate / "compiler.py").write_text("def optimize():\n    return 2\n\ndef schedule():\n    return 1\n")
    experiment.revision_session.finalize_mechanism_round(candidate, round_index=0)
    current = experiment.analysis.analyze(candidate, hypothesis="analyze edited compiler")

    if mutation == "returned_analysis":
        current["analysis"]["diagnostics"]["verified_global_plan_emission"]["plan_digest"] = "0" * 64
    else:
        iteration_path = experiment.output / f"iteration_{current['iteration']:04d}.json"
        document = P2_CONTRACTS.mapping_file(iteration_path)
        document["hypothesis"] = "tampered after the immutable iteration was sealed"
        iteration_path.chmod(0o644)
        iteration_path.write_bytes(P2_CONTRACTS.canonical_json(document))
        iteration_path.chmod(0o444)

    with pytest.raises(ValueError, match="no immutable current portfolio analysis"):
        experiment.analysis.bind_mechanism_work_order_analysis(current)


@pytest.mark.parametrize("mutation", ["candidate", "catalog", "portfolio", "self_hash", "flat_sites", "missing_member"])
def test_mechanism_work_order_identity_mutations_fail_closed(tmp_path, mutation):
    ...
    if mutation != "self_hash":
        document["sha256"] = P2_CONTRACTS.document_sha256(
    ...
    with pytest.raises(ValueError, match="mechanism work order|mechanism work-order"):
        experiment.freeze_mechanism_work_order(path, P2_CONTRACTS.sha256_file(path), candidate=candidate)
...
def test_mechanism_work_order_current_analysis_hash_drift_refuses_before_authoring(tmp_path):
    ...
    experiment.freeze_mechanism_work_order(path, P2_CONTRACTS.sha256_file(path), candidate=candidate)
    experiment.revision_session.begin_mechanism_round(candidate, round_index=0)
    initial = experiment.analysis.analyze(candidate, hypothesis="current exact analysis")
...
def test_mechanism_work_order_keeps_repeated_source_ids_bound_per_four_members(tmp_path):
    ...
    for index in range(3):
        ...
        portfolio.append(
            INPUTS.StageE2ESentinel(
                f"training-{index}", str(source), str(source), P2_CONTRACTS.exact_tree_record(source)["sha256"], (), ()
            )
        )
    experiment, candidate, calls = setup_experiment(tmp_path, portfolio_sentinels=tuple(portfolio))
    (candidate / "compiler.py").write_text("def optimize():\n    return 1\n\ndef schedule():\n    return 1\n")
    _freeze_test_mechanism_catalog(
        experiment,
        candidate,
    ...
    experiment.freeze_mechanism_work_order(path, P2_CONTRACTS.sha256_file(path), candidate=candidate)
    experiment.revision_session.begin_mechanism_round(candidate, round_index=0)
    initial = experiment.analysis.analyze(candidate, hypothesis="four exact graphs")
    binding = experiment.analysis.bind_mechanism_work_order_analysis(initial)
    assert len(binding["members"]) == 4
    assert len(calls) == 4
    assert [member["capsule"] for member in binding["members"]] == [
        member.capsule for member in experiment.inputs.portfolio_sentinels
    ]
...
def test_mechanism_work_order_requires_raw_absolute_readonly_exact_file(tmp_path):
    ...
    digest = P2_CONTRACTS.sha256_file(path)
    with pytest.raises(ValueError, match="immutable absolute"):
        experiment.freeze_mechanism_work_order(Path("work-order.json"), digest, candidate=candidate)
...
def test_exact_active_v14_work_order_matches_retained_four_model_analysis(tmp_path):
    root = repo_root()
    artifact = root / (
        "out/artifacts/perf-bench/gemmini/development_v14_epilogue_site_bindings_20260907/active_t01_01_work_order.json"
    )
    catalog_path = root / (
    ...
    contract_path = root / (
        "out/artifacts/perf-bench/gemmini/development_v14_retained_surface_catalog_20260907/edit_contract.json"
    )
    retained = root / (
        "out/artifacts/perf-bench/gemmini/"
    ...
    candidate, iteration_path, checkpoint_path = (
        retained / "round_0000_candidate_submission",
        retained / "iteration_0001.json",
    ...
    work, catalog, contract = (P2_CONTRACTS.mapping_file(path) for path in (artifact, catalog_path, contract_path))
    checkpoint, iteration = P2_CONTRACTS.mapping_file(checkpoint_path), P2_CONTRACTS.mapping_file(iteration_path)
    assert P2_CONTRACTS.sha256_file(artifact) == ("6f8b11ba12091f4e5936a57b3524c61ba60c366aa42f6b088a9c0e967a358f64")
    ...
    validator.edit_authority = FrozenEditAuthority(tmp_path)
    validator.edit_authority.contract = contract
    validator.inputs = SimpleNamespace(
        portfolio_identity=checkpoint["portfolio"],
        portfolio_identity_sha256=P2_CONTRACTS.document_sha256(checkpoint["portfolio"]),
    )
    from merlin_experiments.phase2.mechanism_program import MechanismProgram

    validator.mechanism_program = MechanismProgram(
        tmp_path,
        validator.edit_authority,
        portfolio_identity=validator.inputs.portfolio_identity,
        portfolio_identity_sha256=validator.inputs.portfolio_identity_sha256,
    )
    validator.mechanism_program.catalog = catalog
    validated = validator.mechanism_program.validate_work_order(work, candidate_sha256=hash_tree(candidate)["sha256"])
    ...
    for index, site in enumerate(validated["portfolio_site_bindings"]):
        analysis = iteration["analysis"] if index == 0 else iteration["portfolio"]["members"][index]["analysis"]
...
@pytest.mark.parametrize("flag", ["--mechanism-work-order", "--mechanism-work-order-sha256"])
def test_launcher_requires_mechanism_work_order_path_sha_and_catalog(flag, tmp_path):
    launcher = importlib.import_module("launch_global_agent_experiment")
    value = str((tmp_path / "work-order.json").resolve()) if flag.endswith("work-order") else "a" * 64
    with pytest.raises(SystemExit) as caught:
        launcher.main(
            ["--campaign-config", "not-read.json", "--candidate", "candidate", "--output", "not-created", flag, value]
        )
...
def test_context_provider_installation_is_not_current_motif_applicability():
    analysis = {
        ...
        "diagnostics": {
            ...
            "queued_movement_context": {
                "schema": "queued_movement_context_candidates_v1",
                "artifact_sha256": SHA["llvm"],
                "motifs": [],
            },
        },
    }
    empty = AV.controlled_context_capability(analysis, provider_installed=True)
    ...
    present = AV.controlled_context_capability(analysis, provider_installed=True)
    assert present["available"] and present["admission_verified"] is False
    assert not AV.controlled_context_capability(analysis, provider_installed=False)["available"]
    analysis["diagnostics"]["queued_movement_context"]["artifact_sha256"] = SHA["buffer"]
    assert AV.controlled_context_capability(analysis, provider_installed=True)["current_candidate_status"] == "UNKNOWN"
...
def test_paired_context_same_work_binding_and_scoped_receipt(tmp_path, bad_contract, counter_mode):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Before schedule")
    (candidate / "source.txt").write_text("reordered schedule")
    experiment.analysis.analyze(candidate, hypothesis="After schedule")
    for index, field in ((0, "previous_artifacts"), (1, "artifacts")):
        setattr(
            experiment.revisions,
            field,
            {
                "candidate_sha256": experiment.revisions.iterations[index]["candidate_sha256"],
                "candidate_lowered_sha256": SHA["llvm"],
    ...
    calls = []
    work = {
        "timed_command_count": 3,
        "timed_command_multiset": ["load", "compute"],
        "source_task_index": 7,
        "source_op_indices": [18],
    }
    input_contract = {"wrapper_sha256": SHA["llvm"], "input_bytes": 16}
    digest = P2_CONTRACTS.document_sha256(work)
    input_digest = P2_CONTRACTS.document_sha256(input_contract)
    ...
    def provider(**kwargs):
        ...
        for arm in ("before", "after"):
            ...
            source = {
                "source_artifact_sha256": SHA["llvm"],
                "slice_source_sha256": SHA["buffer"],
                "work_contract_sha256": digest,
                "timed_source_instruction_indices": [1, 2, 3],
                "source_instruction_indices": [0, 1, 2, 3],
            ...
            prepared = {
                "source_artifact_sha256": SHA["buffer"],
                "measurement_scope": "controlled_fixed_work_slice",
                "simulator_executed": False,
                "timed_instruction_count": 3,
                "host_input_bytes": 16,
                "output_storage_bytes": 16,
                "workdir": str(directory),
                "elf_sha256": P2_CONTRACTS.sha256_file(directory / "primitive.elf"),
                "wrapper_sha256": SHA["llvm"],
                "primitive_mlir_sha256": SHA["buffer"],
        ...
        def execute(*, arm, timeout_s):
            ...
            if counter_mode != "missing":
                result["engine_provenance"] = {
                    "binary_sha256": SHA["target"] if arm == "before" or counter_mode == "same" else SHA["plan"]
                }
                result["counter_profile"] = {
                    "kind": "joint_engine_busy_cycles",
                    "partition_proof": {"status": "proved"},
                    "layout": {"complete": True},
                    "active_union_cycles": 90,
                    "idle_cycles": 10 if arm == "before" else 9,
        ...
        return {
            ...
            "paired_context_inputs": {
                "before_binding": experiment.revision_session.previous_probe_binding(candidate),
                "after_binding": experiment.revision_session.current_probe_binding(candidate),
                "before_model_artifact_sha256": SHA["llvm"],
                "after_model_artifact_sha256": SHA["llvm"],
                "arms": arms,
                "work_contract_sha256": digest,
                "deterministic_input_contract": input_contract,
                "deterministic_input_contract_sha256": input_digest,
                "projection_proof": {
                    ...
                    "before_artifact_sha256": SHA["llvm"],
                    "after_artifact_sha256": SHA["llvm"],
                    "before_timed_indices": [1, 2, 3],
                    "after_timed_indices": [1, 2, 3],
                    "work_contract": work,
                    "work_contract_sha256": digest,
                },
            },
    ...
    if bad_contract:
        with pytest.raises(ValueError, match="changed source identity, work"):
            experiment.probes.compare_controlled_context(candidate, provider=provider, timeout_s=60)
        assert calls == []
    else:
        receipt = experiment.probes.compare_controlled_context(candidate, provider=provider, timeout_s=60)
        ...
        assert feedback["status"] == (
            "no_observed_overlap_or_busy_work_change" if counter_mode == "same" else "counter_evidence_unknown"
        )
        assert feedback["source_op_indices"] == [18]
        assert feedback["pipeline_projection_admitted"] is False
        view = AV.agent_analysis_view(experiment.revisions.iterations[-1], complete_evidence="complete.json")
        ...
        assert "scoped_mechanism_coverage" not in experiment.revisions.iterations[-1]["analysis"].get(
            "optimization_brief", {}
        )
        sealed = experiment.revision_session.seal(candidate)
        ...
        with pytest.raises(ValueError, match="not bound"):
            CHECKPOINT.paired_context_decision_feedback(
                experiment.revisions.iterations[-1], stale, target_sha256=SHA["target"]
            )
        (candidate / "source.txt").write_text("next schedule")
        following = experiment.analysis.analyze(candidate, hypothesis="Use prior measurement as search history")
        assert (
            following["static_comparison"]["previous_revision_mechanism_feedback"]["applies_to_current_revision"]
            is False
        )
...
def test_exact_unchanged_analysis_reuses_identity_without_compiling(tmp_path):
    experiment, candidate, calls = setup_experiment(tmp_path)
    first = experiment.analysis.analyze(candidate, hypothesis="Inspect graph")
    second = experiment.analysis.analyze(candidate, hypothesis="Read the same evidence")
    ...
    assert P2_CONTRACTS.sha256_file(Path(receipt["path"])) == receipt["sha256"]
    evidence = P2_CONTRACTS.mapping_file(Path(receipt["path"]))
...
def test_reverted_candidate_reuses_ready_static_analysis_as_new_iteration(tmp_path):
    ...
    experiment, candidate, calls = setup_experiment(tmp_path, portfolio_sentinels=[extra])
    original = (candidate / "source.txt").read_text()
    first = experiment.analysis.analyze(candidate, hypothesis="Inspect original revision")
    ...
    experiment.revisions.iterations[0]["probe_receipts"].append({"path": "timed.json", "sha256": "a" * 64})
    experiment.revisions.iterations[0]["decision_feedback"] = {"status": "measured_elsewhere"}
    experiment.revisions.iteration_artifacts[0] = {
    ...
    experiment.revisions.compiler_sandboxes[0] = copy.deepcopy(source_policy)

    (candidate / "source.txt").write_text("rejected speculative revision")
    second = experiment.analysis.analyze(candidate, hypothesis="Inspect speculative revision")
    experiment.revisions.iteration_artifacts[1] = {
        "candidate_sha256": second["candidate_sha256"],
        "candidate_lowered_sha256": SHA["llvm"],
        "marker": "speculative",
    }
    experiment.revisions.artifacts = copy.deepcopy(experiment.revisions.iteration_artifacts[1])
    (candidate / "source.txt").write_text(original)

    revisited = experiment.analysis.analyze(candidate, hypothesis="Return to qualified checkpoint")

    assert len(calls) == 4  # two models for each of the two novel revisions
    assert len(experiment.revisions.iterations) == 3
    ...
    assert (
        revisited["portfolio"]["analysis_allocation_policy"] == "exact_immutable_ready_iteration_reuse_no_compilation"
    )
    assert revisited["portfolio"]["analysis_concurrency"]["admitted_workers"] == 0
    assert all(member["analysis_allocation"]["allocated_seconds"] == 0 for member in revisited["portfolio"]["members"])
    ...
    assert experiment.revision_session.current_artifacts(candidate)["marker"] == "original"
    assert experiment.revision_session.previous_artifacts(candidate)["marker"] == "speculative"
    assert experiment.revisions.compiler_sandboxes[2] == source_policy
    stored = P2_CONTRACTS.mapping_file(experiment.output / "iteration_0002.json")
    assert stored == experiment.revisions.iterations[2]
    sealed = G.consume_global_candidate(experiment.revision_session.seal(candidate, name="revisited"))
    assert sealed["iteration"] == 2


@pytest.mark.parametrize("unsafe_source", ["blocked", "wall_timeout"])
def test_reverted_candidate_recompiles_an_unsafe_prior_result(tmp_path, monkeypatch, unsafe_source):
    experiment, candidate, _ = setup_experiment(tmp_path, timeout_s=10 if unsafe_source == "wall_timeout" else 300)
    delegate = experiment.analysis.analyzer
    original = (candidate / "source.txt").read_text()
    invocations = []
    fail_first = [True]
    ...
    experiment.analysis.analyzer = analyzer
    unsafe = experiment.analysis.analyze(candidate, hypothesis="Initial unsafe result")
    ...
    experiment.analysis.analyze(candidate, hypothesis="Compile intervening revision")
    (candidate / "source.txt").write_text(original)

    retried = experiment.analysis.analyze(candidate, hypothesis="Retry original bytes")
...
def test_reverted_candidate_does_not_reuse_across_compiler_dependency_change(tmp_path, monkeypatch):
    experiment, candidate, calls = setup_experiment(tmp_path)
    original = (candidate / "source.txt").read_text()
    experiment.analysis.analyze(candidate, hypothesis="Initial dependency closure")
    (candidate / "source.txt").write_text("intervening revision")
    experiment.analysis.analyze(candidate, hypothesis="Changed candidate")
    (candidate / "source.txt").write_text(original)
    dependencies = experiment.inputs.compiler_dependencies

    def changed_dependencies(package):
        record = dependencies(package)
        if (
            package.resolve() == candidate.resolve()
    ...
    monkeypatch.setattr(experiment.inputs, "compiler_dependencies", changed_dependencies)
    retried = experiment.analysis.analyze(candidate, hypothesis="New shared dependency closure")
    assert len(calls) == 3
    assert "analysis_reuse" not in retried
...
def test_reverted_candidate_recompiles_if_prior_snapshot_lost_immutability(tmp_path):
    experiment, candidate, calls = setup_experiment(tmp_path)
    original = (candidate / "source.txt").read_text()
    first = experiment.analysis.analyze(candidate, hypothesis="Capture immutable original")
    (candidate / "source.txt").write_text("intervening revision")
    experiment.analysis.analyze(candidate, hypothesis="Compile intervening revision")
    (candidate / "source.txt").write_text(original)
    ...
    retried = experiment.analysis.analyze(candidate, hypothesis="Do not trust writable cache source")
...
def test_analysis_passes_frozen_host_verifier_policy_to_worker(tmp_path):
    ...
    def analyzer(*args, **kwargs):
        assert kwargs["host_verifier_policy_sha256"] == experiment.inputs.host_policy["sha256"]
        return delegate(*args, **kwargs)

    experiment.analysis.analyzer = analyzer
    experiment.analysis.analyze(candidate, hypothesis="Verify the host-bound baseline plan")
...
def test_reverted_candidate_reuses_earlier_exact_verified_analysis_for_sealing(tmp_path):
    ...
    experiment.analysis.analyze(candidate, hypothesis="Inspect speculative revision")
    (candidate / "source.txt").write_text(original)

    current = experiment.revision_session.current(candidate)
    assert current["candidate_sha256"] == first["candidate_sha256"]
    assert len(calls) == 2
    sealed = G.consume_global_candidate(experiment.revision_session.seal(candidate, name="reverted"))
...
def test_sustained_sequence_checkpoints_and_explicit_recovery(tmp_path, failure):
    ...
    def author(current, *, round_index, round_timeout_s):
        ...
        experiment.analysis.analyze(current, hypothesis="actual full graph test analysis")
        if failure and round_index == (0 if failure == "author_first" else 1):
            broker_complete = failure not in ("integrity", "capacity")
            experiment._write(
                f"agent_round_{round_index:04d}.json",
                {
            ...
            if failure == "capacity":
                rounds = owner.stage_root / "rounds"
                rounds.mkdir(parents=True, exist_ok=True)
                (rounds / f"round_{round_index:02d}.codex_summary.json").write_text(
                    json.dumps(
                        {
                            "exit_code": 1,
                            "timed_out": False,
    ...
    owner = _portfolio_authoring(experiment, stage_root=tmp_path / "stage", run_round=author)
    if failure == "integrity":
        with pytest.raises(ValueError, match="integrity failure"):
            owner.run_sequence(candidate, **kwargs)
        assert len(invoked) == 2
        return
    result = owner.run_sequence(candidate, **kwargs)
    assert invoked == [(0, 30), (1, 30), (2, 10)]
    assert inherited == [
        "candidate",
        "candidate" if failure == "author_first" else "draft round 0",
...
@pytest.mark.parametrize(("budget", "reserve"), [(30, 0), (120, 30), (299, 30), (300, 100), (600, 180)])
def test_macro_round_reserves_a_bounded_final_response_window(budget, reserve):
    assert PAuthor._agent_finalization_reserve_seconds(budget) == reserve
...
def test_clean_reverted_round_can_continue_without_admitting_a_candidate(tmp_path):
    ...
    assert PAuthor._retryable_unchanged_round_failure(terminal, checkpoint_sha256=SHA["graph"])
    terminal["candidate_sha256"] = SHA["target"]
    assert not PAuthor._retryable_unchanged_round_failure(terminal, checkpoint_sha256=SHA["graph"])
...
def test_sequence_restarts_clean_unchanged_round_from_consumed_checkpoint(tmp_path, exit_code):
    ...
    def author(current, *, round_index, round_timeout_s):
        ...
        experiment.analysis.analyze(current, hypothesis="second round full graph")
        return {"status": "authored"}
...
@pytest.mark.parametrize(
    "failure", ["changed_draft", "dirty_audit", "binding_refusal", "missing_checkpoint_digest", "other_exit"]
)
def test_unchanged_timeout_recovery_does_not_admit_unbound_or_unsafe_rounds(failure):
    ...
    if failure == "changed_draft":
        ...
        terminal["audit"] = {"clean": False, "hits": [{"kind": "invalid_broker_invocation"}]}
...
def test_timeout_recovery_rechecks_checkpoint_and_host_bindings(tmp_path, monkeypatch, mutation):
    ...
    def author(current, *, round_index, round_timeout_s):
        ...
        if mutation == "baseline":
            (experiment.inputs.baseline / "source.txt").write_text("mutated baseline")
        elif mutation == "checkpoint":
            checkpoint = experiment.output / "initial_seed_candidate.json"
            checkpoint.chmod(0o644)
            checkpoint.write_text("{}")
        else:
            monkeypatch.setattr(GI.HP, "build_record", lambda **kwargs: {"sha256": "changed"})
    ...
    assert invoked == [0]
    failure = P2_CONTRACTS.mapping_file(experiment.output / "continuation_failure_0000.json")
...
def test_prior_round_context_carries_agent_memory_but_labels_it_untrusted(tmp_path):
    ...
    (audits / "agent_round_0000.json").write_text(
        json.dumps(
            {
                "status": "refused",
                "candidate_sha256": SHA["graph"],
                "agent_exit_code": 0,
                "refusal_reasons": ["analysis deadline"],
            }
        )
    )
    context = PAuthor._prior_round_context(tmp_path, 1)
...
def test_concurrent_identical_analysis_waits_and_reuses_one_snapshot(tmp_path):
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        first = pool.submit(experiment.analysis.analyze, candidate, hypothesis="First request")
        assert entered.wait(2)
        duplicate = pool.submit(experiment.analysis.analyze, candidate, hypothesis="Duplicate request")
        release.set()
...
def test_probe_reuses_exact_prepared_policy_and_cannot_unmask_answers(tmp_path):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    row = experiment.analysis.analyze(candidate, hypothesis="Inspect graph")
    answer, scratch = tmp_path / "answers", tmp_path / "public_probe"
    answer.mkdir()
    scratch.mkdir()
    analyzer = IsolatedAnalysisWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
    ...
    experiment.analysis.analyzer = analyzer
    policy = experiment.probes._probe_sandbox(candidate, scratch)
    ...
    with pytest.raises(ValueError, match="masked answer"):
        experiment.probes._probe_sandbox(candidate, answer)


def test_native_semantic_probe_requires_existing_isolated_policy(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Inspect graph")
    with pytest.raises(ValueError, match="production isolated compiler policy"):
        experiment.probes.run_native_probe(candidate, tmp_path, ["/usr/bin/true"], timeout_s=1)
...
def _comparison_arm_fixture(tmp_path):
    ...
    experiment, candidate, _ = setup_experiment(
        tmp_path, optimization_baseline=seed, optimization_baseline_sha256=hash_tree(seed)["sha256"]
    )
    experiment.analysis.analyze(candidate, hypothesis="Initial optimization comparison")
    row = experiment.revisions.iterations[0]
    interface = Path(experiment.inputs.sentinel.frozen_source_path) / "source.txt"
    row["analysis"]["diagnostics"]["captured_logical_graph"]["source_sha256"] = P2_CONTRACTS.sha256_file(interface)
    lowered, buffer = "module {}", '{"commands": [], "tensors": {}}'
    lower_sha, buffer_sha = sha256_bytes(lowered.encode()), sha256_bytes(buffer.encode())
    row["analysis"]["emission"].update(baseline_lowered_sha256=lower_sha, baseline_command_buffer_sha256=buffer_sha)
    experiment.revisions.artifacts = {
        "candidate_sha256": row["candidate_sha256"],
        "candidate_lowered_sha256": SHA["llvm"],
        "interface": interface,
    }
    experiment.revisions.baseline_artifacts = {
        "identity": {
            "baseline_sha256": experiment.inputs.optimization_baseline_sha256,
            "capsule_sha256": experiment.inputs.sentinel.capsule_sha256,
            "target": experiment.inputs.target,
        },
        "lowered_text": lowered,
        "lowered_sha256": lower_sha,
        "command_buffer_text": buffer,
    ...
    prefix = ["bwrap", "--clearenv", "--tmpfs", str(answer), "bash", "-c", 'exec "$@"', "perf-tool"]
    policy = {
        "package_path": str(experiment.inputs.optimization_baseline),
        "compiler_dependencies": copy.deepcopy(
            experiment.inputs.optimization_baseline_binding["compiler_dependencies"]
        ),
        "command_prefix": prefix,
        "bwrap_argv_length": 4,
        "answer_surfaces": [{"path": str(answer), "kind": "dir"}],
    }
    experiment.revisions.optimization_baseline_sandbox = copy.deepcopy(policy)
    experiment.revisions.optimization_baseline_sandbox_sha256 = P2_CONTRACTS.document_sha256(policy)
    experiment.analysis.analyzer = IsolatedAnalysisWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
        output=tmp_path / "worker",
...
def test_initial_comparison_accessor_is_not_a_previous_iteration_or_qualification(tmp_path):
    ...
    artifact = experiment.revision_session.optimization_baseline_artifacts(candidate)
    binding = experiment.revision_session.optimization_baseline_artifact_binding(candidate)
    assert (
        artifact["compiler_sha256"]
        == experiment.inputs.optimization_baseline_sha256
        != experiment.inputs.baseline_sha256
    )
    ...
    assert len(experiment.revisions.iterations) == 1
    with pytest.raises(ValueError, match="two submitted"):
        experiment.revision_session.previous_artifacts(candidate)
    scratch = tmp_path / "public_scratch"
    scratch.mkdir()
    got = experiment.probes._probe_sandbox(candidate, scratch, optimization_baseline=True)
    ...
    with pytest.raises(ValueError, match="masked answer"):
        experiment.probes._probe_sandbox(candidate, answer, optimization_baseline=True)


@pytest.mark.parametrize(
    "mutation", ["llvm", "buffer", "cache_identity", "analyzed_hash", "source", "policy", "policy_missing", "closure"]
)
def test_comparison_arm_rejects_stale_artifacts_and_recorded_policy(tmp_path, monkeypatch, mutation):
    ...
    if mutation == "llvm":
        experiment.revisions.baseline_artifacts["lowered_text"] += " "
    elif mutation == "buffer":
        experiment.revisions.baseline_artifacts["command_buffer_text"] += " "
    elif mutation == "cache_identity":
        experiment.revisions.baseline_artifacts["identity"]["baseline_sha256"] = experiment.inputs.baseline_sha256
    elif mutation == "analyzed_hash":
        experiment.revisions.iterations[0]["analysis"]["emission"]["baseline_lowered_sha256"] = "a" * 64
    elif mutation == "source":
        outside = tmp_path / "unfrozen.mlir"
        outside.write_text("model")
        experiment.revisions.artifacts["interface"] = outside
    elif mutation == "policy":
        experiment.revisions.optimization_baseline_sandbox["command_prefix"].append("--unshare-all")
    elif mutation == "policy_missing":
        experiment.revisions.optimization_baseline_sandbox = None
    elif mutation == "closure":
        original = experiment.inputs.compiler_dependencies
        monkeypatch.setattr(
            experiment.inputs,
            "compiler_dependencies",
            lambda package: (
                {**original(package), "changed": True}
                if package == experiment.inputs.optimization_baseline
    ...
    scratch.mkdir()
    with pytest.raises(ValueError):
        experiment.probes._probe_sandbox(candidate, scratch, optimization_baseline=True)
...
def test_comparison_probe_compiles_normal_manifest_entrypoints_with_same_masks(tmp_path, monkeypatch):
    ...
    result = experiment.probes.compile_optimization_baseline_probe_candidate(
        candidate, source, tmp_path / "probe", timeout_s=10, emit_command_buffer=True
    )
    assert loaded == [experiment.inputs.optimization_baseline]
...
def test_comparison_compilation_detects_mutation_during_entrypoint(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="artifact bytes changed"):
        experiment.probes.compile_optimization_baseline_probe_candidate(
            candidate, source, tmp_path / "probe", timeout_s=10
        )
...
def test_successful_analysis_records_baseline_policy_without_relabeling_candidate(tmp_path):
    ...
    baseline_policy = {
        "package_path": str(experiment.inputs.optimization_baseline),
        "compiler_dependencies": experiment.inputs.optimization_baseline_binding["compiler_dependencies"],
        "command_prefix": ["bwrap", "--clearenv", "bash"],
        "bwrap_argv_length": 2,
        "answer_surfaces": [],
    }
    ...
    def analyze(base, current, *args, **kwargs):
        ...
        analyze.completed_sandboxes = {
            "baseline": baseline_policy,
            "candidate": {
                "package_path": str(current),
                "compiler_dependencies": experiment.inputs.compiler_dependencies(current),
            },
        }
        return result

    experiment.analysis.analyzer = analyze
    experiment.analysis.analyze(candidate, hypothesis="Retain exact arm policies")
    assert experiment.revisions.optimization_baseline_sandbox == baseline_policy
    assert experiment.revisions.optimization_baseline_sandbox is not baseline_policy
    assert experiment.revisions.optimization_baseline_sandbox_sha256 == P2_CONTRACTS.document_sha256(baseline_policy)
    assert experiment.revisions.compiler_sandboxes[0]["candidate"]["package_path"] != baseline_policy["package_path"]
...
def test_comparison_probe_requires_finite_inner_budget(tmp_path, timeout):
    ...
    with pytest.raises(ValueError, match="bounded timeout"):
        experiment.probes.compile_optimization_baseline_probe_candidate(
            candidate, source, tmp_path / "probe", timeout_s=timeout
        )
...
def test_native_policy_accessor_reuses_exact_policy_but_returns_private_copy(tmp_path, monkeypatch):
    ...
    scratch.mkdir()
    calls = []
    monkeypatch.setattr(experiment.probes, "_probe_sandbox", lambda *args: calls.append(args) or policy)
    first = experiment.probes.native_probe_policy(candidate, scratch)
    first["command_prefix"].append("not-a-grant")
    second = experiment.probes.native_probe_policy(candidate, scratch)
...
def test_source_contraction_prepare_and_execute_are_separate_hash_bound_actions(tmp_path, monkeypatch, route_status):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    original = experiment.analysis.analyze(candidate, hypothesis="Source pair")
    original_mapping = P2_CONTRACTS.mapping_file
    monkeypatch.setattr(P2_CONTRACTS, "mapping_file", lambda *args, **kwargs: {"entry": "host_entry"})
    calls = []
    declared_refusal = {"status": "refused", "problems": ["source operations are not fully covered: [0, 1]"]}
    nested_evidence = {
        "status": "prepared",
        "emitted_route_correspondence": "UNKNOWN",
        "arms": {
            "before": {
                "source_task_cfg_proof": declared_refusal,
                "short_execution_admission": {
                    "schema": "short_initializer_execution_admission_v1",
    ...
    if route_status:
        nested_evidence["task_route_feedback"] = {
            "status": route_status,
            "timing_calibration_admissible": False,
            "scope": "known static class presence only",
        }
    monkeypatch.setattr(
        source_contraction_preparation,
        "prepare_source_contraction",
        lambda **kwargs: calls.append(kwargs) or nested_evidence,
    )
    prepared = experiment.probes.prepare_source_contraction(
        candidate, comparison_arm="optimization_baseline", source_op_index=3, max_m=2, max_n=4, max_k=5, timeout_s=100
    )
    ...
    def provider(**kwargs):
        ...
        return {
            "status": "short_pair_passed",
            "route_relevance": "UNKNOWN",
            **({"task_route_feedback": nested_evidence["task_route_feedback"]} if route_status else {}),
        }

    receipt = experiment.probes.qualify_source_contraction(
        candidate, preparation_sha256=prepared["preparation_sha256"], provider=provider, timeout_s=100
    )
    ...
    assert experiment.revisions.iterations[-1]["readiness"] == original["readiness"]
    assert len(experiment.revisions.iterations[-1]["source_pair_receipts"]) == 1
    monkeypatch.setattr(P2_CONTRACTS, "mapping_file", original_mapping)
    sealed = experiment.revision_session.seal(candidate)
    assert len(G.consume_global_candidate(sealed)["source_pair_receipts"]) == 1
    reference = experiment.revisions.iterations[-1]["source_contraction_preparation_receipts"][0]
    stored = P2_CONTRACTS.mapping_file(Path(reference["path"]))
    ...
    with pytest.raises(ValueError, match="changed"):
        experiment.probes.qualify_source_contraction(
            candidate, preparation_sha256=prepared["preparation_sha256"], provider=provider, timeout_s=10
        )
...
def test_source_entry_fallback_is_unique_and_frozen_not_a_named_model(tmp_path, monkeypatch, case):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Source entry")
    source = tmp_path / "source.mlir"
    text = "module {func.func @arbitrary_name(){func.return}}"
    if case == "ambiguous":
        text = "module {func.func @a(){func.return} func.func @b(){func.return}}"
    source.write_text(text)
    row = experiment.revisions.iterations[-1]
    row["analysis"]["diagnostics"]["captured_logical_graph"]["source_sha256"] = (
        sha256_bytes(text.encode()) if case != "stale" else "0" * 64
    )
    monkeypatch.setattr(P2_CONTRACTS, "mapping_file", lambda *args, **kwargs: {})
    monkeypatch.setattr(experiment.revision_session, "current_artifacts", lambda _: {"interface": source})
    calls = []
    monkeypatch.setattr(
        source_contraction_preparation,
        "prepare_source_contraction",
    ...
    action = lambda: experiment.probes.prepare_source_contraction(
        candidate, comparison_arm="previous", source_op_index=0, max_m=1, max_n=1, max_k=1, timeout_s=10
    )
    if case == "unique":
        action()
...
def test_source_pair_refusal_charges_elapsed_but_does_not_call_provider(tmp_path, monkeypatch):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Source pair")
    start = experiment.revisions.iterations[-1]["elapsed_seconds"]
    with pytest.raises(ValueError, match="current-iteration"):
        experiment.probes.qualify_source_contraction(
            candidate,
            preparation_sha256="0" * 64,
            provider=lambda **_: pytest.fail("unbound preparation cannot execute"),
            timeout_s=10,
        )
    assert experiment.revisions.iterations[-1]["elapsed_seconds"] > start
...
def test_source_preparation_is_feedback_not_semantic_or_runtime_pass(tmp_path, monkeypatch, preparation_status):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    row = experiment.analysis.analyze(candidate, hypothesis="Inspect generic source placement")
    before = experiment.revisions.iterations[-1]["elapsed_seconds"]
    descriptor_reads = []
    monkeypatch.setattr(
        P2_CONTRACTS, "mapping_file", lambda path, **kwargs: descriptor_reads.append(path) or {"entry": "frozen_entry"}
    )
    seen = []
    ...
    def prepare(**kwargs):
        ...
        return {
            ...
            "source_convolution_opportunities": [{"source_op_index": 6, "source_macs": 1024}],
        }

    monkeypatch.setattr(source_convolution_preparation, "prepare_source_convolution", prepare)
    result = experiment.probes.prepare_source_convolution(
        candidate, comparison_arm="optimization_baseline", timeout_s=100
    )
    assert descriptor_reads == [Path(experiment.inputs.sentinel.frozen_source_path) / "capsule.yaml"]
    ...
    actual = experiment.revisions.iterations[-1]
...
def test_preparation_exposes_only_effect_matched_frozen_ast_authority(tmp_path, monkeypatch):
    ...
    contract = {
        "schema": "compiler_edit_contract_v1",
        "existing_symbols": [{"surface_id": "authorized_lower", "path": "compiler.py", "symbol": "lower"}],
        "helper_extensions": [],
    }
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    experiment.freeze_edit_scope(candidate, contract)
    experiment.analysis.analyze(candidate, hypothesis="Inspect generic lowering")
    monkeypatch.setattr(P2_CONTRACTS, "mapping_file", lambda *args, **kwargs: {"entry": "frozen_entry"})
    monkeypatch.setattr(
        source_convolution_preparation, "prepare_source_convolution", lambda **kwargs: {"status": "prepared"}
    )
    inspected = []

    def inventory(path, *, contract):
        assert contract == experiment.inputs.contract_root
    ...
    monkeypatch.setattr(PP, "inspect_compiler_package", inventory)
    result = experiment.probes.prepare_source_convolution(candidate, comparison_arm="previous", timeout_s=30)
    assert inspected == [experiment.edit_authority.seed]
    assert [row["surface_id"] for row in result["allowed_edit_surfaces"]] == ["authorized_lower"]
    assert result["edit_authority_sha256"] == experiment.edit_authority.binding["contract_document_sha256"]
...
def test_preparation_failure_charges_elapsed_without_semantic_receipt(tmp_path, monkeypatch):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Inspect source")
    elapsed = experiment.revisions.iterations[-1]["elapsed_seconds"]
    monkeypatch.setattr(P2_CONTRACTS, "mapping_file", lambda *args, **kwargs: {"entry": "frozen_entry"})
    ...
    with pytest.raises(ValueError, match="missing baseline proof"):
        experiment.probes.prepare_source_convolution(candidate, comparison_arm="optimization_baseline", timeout_s=30)
    assert experiment.revisions.iterations[-1]["elapsed_seconds"] > elapsed
    assert not experiment.revisions.iterations[-1].get("semantic_receipts")


def test_preparation_rejects_implicit_or_invalid_comparison_arm(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    with pytest.raises(ValueError, match="explicitly"):
        experiment.probes.prepare_source_convolution(candidate, comparison_arm="default", timeout_s=30)


def test_preparation_oracle_helpers_are_exact_host_policy_inputs():
    policy = G.host_verification_policy_record()
    for module in (
        "perf.source_convolution_preparation",
        "perf.source_convolution_witness",
        "targetgen.capsule_golden",
        "targetgen.conv_geometry",
        "runtime.commandbuffer",
        "runtime.tensor",
    ):
        path = module_source_path("merlin." + module)
        assert policy["sources"][str(path.resolve())] == P2_CONTRACTS.sha256_file(path)
...
def test_seal_uses_exact_analyzed_snapshot_not_later_python_cache(tmp_path):
    ...
    record = json.loads(experiment.revision_session.seal(candidate).read_text())
...
def test_probe_pair_compiles_both_artifacts_under_one_sandbox(tmp_path, monkeypatch):
    ...
    experiment.analysis.analyzer = worker.IsolatedAnalysisWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
        sandbox_factory=lambda *_: {"candidate": sandbox},
        output=tmp_path / "worker",
    )
    ...
    source = tmp_path / "tiny.mlir"
    source.write_text("module {}")
    result = experiment.probes.compile_probe_candidate(
        candidate, source, tmp_path / "probe", timeout_s=10, emit_command_buffer=True
    )
...
def test_previous_probe_uses_only_retained_predecessor_policy(tmp_path, monkeypatch):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    previous = experiment.analysis.analyze(candidate, hypothesis="Before")
    (candidate / "source.txt").write_text("next compiler")
    experiment.analysis.analyze(candidate, hypothesis="After")
    experiment.revisions.previous_artifacts = {
        "candidate_sha256": previous["candidate_sha256"],
        "candidate_lowered_sha256": SHA["llvm"],
    }
    prefix = ["bwrap", "--clearenv", "bash", "-c", 'exec "$@"', "perf-tool"]
    old = {
        "package_path": previous["submitted_snapshot"],
        "compiler_dependencies": previous["compiler_dependencies"],
        "command_prefix": prefix,
        "bwrap_argv_length": 2,
        "answer_surfaces": [],
    }
    experiment.revisions.compiler_sandboxes[0] = {"candidate": old}
    experiment.analysis.analyzer = worker.IsolatedAnalysisWorker(
        analysis_source=Path(EA.__file__),
        contract_root=merlin_dir() / "contract",
        output=tmp_path / "worker",
        sandbox_factory=lambda *_: pytest.fail("must reuse prior policy"),
    )
    experiment.analysis.analyzer.completed_sandboxes = {"candidate": {"package_path": str(candidate)}}
    loaded, observed = [], []
    monkeypatch.setattr(oot_runner, "load_package", lambda path: loaded.append(path) or object())
    ...
    source = tmp_path / "tiny.mlir"
    source.write_text("module {}")
    experiment.probes.compile_previous_probe_candidate(candidate, source, tmp_path / "probe", timeout_s=10)
    assert loaded == [Path(previous["submitted_snapshot"])]
    assert observed[0]["compiler_dependencies"] == previous["compiler_dependencies"]
    experiment.revisions.compiler_sandboxes.clear()
    with pytest.raises(ValueError, match="no retained successful"):
        experiment.probes.compile_previous_probe_candidate(candidate, source, tmp_path / "no_policy", timeout_s=10)
...
def test_submitted_snapshot_survives_concurrent_agent_edits_but_cannot_seal(tmp_path):
    ...
    experiment.analysis.analyzer = analyzer
    row = experiment.analysis.analyze(candidate, hypothesis="Inspect submitted revision")
...
def test_frozen_baseline_artifacts_reused_but_each_edit_still_compiles(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(EA, "analyze_whole_model_emission", analyzer)
    experiment.analysis.analyzer = analyzer
    experiment.analysis.analyze(candidate, hypothesis="First fullmodel compilation")
    (candidate / "source.txt").write_text("new transformation")
    experiment.analysis.analyze(candidate, hypothesis="Recompile changed candidate only")
    assert len(calls) == 2 and len(baseline_compiles) == 1
    (experiment.inputs.baseline / "source.txt").write_text("changed baseline")
    with pytest.raises(ValueError, match="frozen compiler changed"):
        experiment.analysis.analyze(candidate, hypothesis="Refuse stale baseline reuse")


def test_macro_receipts_allow_completion_order_but_reject_duplicate_indices(tmp_path):
    action = PB.BrokerAction(BP.E2E_ANALYSIS_ACTION, (BP._HOST_E2E_ANALYSIS_SENTINEL,), (), "full-model analysis", True)
    rows = [
        {
            "index": index,
    ...
    assert WM.verify_global_broker_receipts(path, actions=[action], audit=audit)["count"] == 2
    rows[0]["index"] = 0
    path.write_text("\n".join(json.dumps(row) for row in rows))
    with pytest.raises(ValueError, match="schema is invalid"):
        WM.verify_global_broker_receipts(path, actions=[action], audit=audit)
...
def test_new_shared_helper_overlays_older_readonly_grant(tmp_path):
    ...
    dependencies = {
        "shared_source_root": str(current),
        "shared_sources": {"new_helper.py": P2_CONTRACTS.sha256_file(current / "new_helper.py")},
    }
    command = PS.compiler_dependency_mounts(argv, dependencies, tmp_path / "overlay")
    observed = subprocess.run(
        [*command, "/usr/bin/cat", str(current / "existing.py"), str(current / "new_helper.py")],
        capture_output=True,
        text=True,
    )
...
def test_changed_candidate_requires_new_graph_plan_analysis(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Delete representation round trip")
    (candidate / "source.txt").write_text("another compiler")
    with pytest.raises(ValueError, match="recompile its full graph"):
        experiment.revision_session.seal(candidate)
    experiment.analysis.analyze(candidate, hypothesis="Re-evaluate next compiler")
    assert experiment.revision_session.seal(candidate).is_file()


@pytest.mark.parametrize(
    "field",
    ["candidate_sha256", "logical_dispatch_digest", "candidate_lowered_sha256", "candidate_command_buffer_sha256"],
)
def test_plan_must_bind_actual_candidate_graph_and_artifacts(tmp_path, field):
    experiment, candidate, _ = setup_experiment(tmp_path)
    row = experiment.analysis.analyze(candidate, hypothesis="Test artifact identity binding")
    analysis = copy.deepcopy(row["analysis"])
    analysis["diagnostics"]["verified_global_plan_emission"][field] = SHA["target"]
    assert "candidate_global_plan_binding_mismatch" in EA.global_iteration_readiness(analysis)["blockers"]


def test_frozen_model_and_baseline_are_rechecked(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Optimize complete graph")
    (Path(experiment.inputs.sentinel.frozen_source_path) / "source.txt").write_text("different model")
    with pytest.raises(ValueError, match="complete-model objective changed"):
        experiment.analysis.analyze(candidate, hypothesis="Cannot substitute an easier graph")


def test_macro_broker_refuses_unrelated_micro_sweep_without_running_evaluator(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    action = PB.BrokerAction(
        BP.DEVELOPMENT_FEEDBACK_ACTION, (BP._HOST_FEEDBACK_SENTINEL,), (), "legacy micro feedback", False
    )
    ...
    broker = PB.Broker(
        AW.AgentSandboxPolicy(
            ...
            True,
            env_prefix="",
            required_tools=(AW.TC.ToolProbe("synthetic", "true"),),
            process_cwd=tmp_path.resolve(),
            frozen_inputs=None,
        ),
        None,
        candidate,
        (action,),
        tmp_path / "broker" / "receipts.jsonl",
        ...
        workflow=BP.select_workflow(
            ...
            receipt_path=tmp_path / "broker" / "receipts.jsonl",
            services=BP.BrokerServices(
                whole_model_analysis=partial(EA.analyze_whole_model_emission, contract_root=merlin_dir() / "contract"),
                command_buffer_analysis=ED.analyze_command_buffers,
                global_analysis_view=AV.agent_analysis_view,
            ),
    ...
    result = broker.execute({"action": BP.DEVELOPMENT_FEEDBACK_ACTION, "bindings": {}})
    assert result["returncode"] == 125
    assert broker.workflow.stop_verdict is None


def test_macro_broker_reserves_deadline_for_required_whole_model_analysis(tmp_path):
    candidate = tmp_path / "candidate"
    candidate.mkdir()
    inspect = PB.BrokerAction("inspect", ("unused",), (), "inspect", False)
    analysis = PB.BrokerAction(BP.E2E_ANALYSIS_ACTION, (BP._HOST_E2E_ANALYSIS_SENTINEL,), (), "analysis", True)
    broker = PB.Broker(
        AW.AgentSandboxPolicy(
            ("bwrap",),
            (),
    ...
    with pytest.raises(P2_CONTRACTS.StageGateError, match="reserved for mandatory whole-model analysis"):
    ...
    assert broker.execute({"action": BP.E2E_ANALYSIS_ACTION, "bindings": {}})["returncode"] == 125
...
def test_macro_handoff_detects_post_seal_candidate_edit(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Global plan revision")
    record = experiment.revision_session.seal(candidate)
...
def test_optional_probe_is_bound_to_current_graph_and_remains_probe_only(tmp_path, monkeypatch, outcome):
    ...
    from merlin.perf.mechanism_probe import (
        ...
        ProbeBinding,
        ProbeObservation,
        derive_mechanism_signature,
    )
    from merlin.xdsl_dialects.lowering.global_plan import ValueRepresentation

    experiment, candidate, _ = setup_experiment(tmp_path)
    row = experiment.analysis.analyze(candidate, hypothesis="Overlap model loads with contraction work")
    binding = ProbeBinding(
        SHA["graph"], SHA["plan"], row["compiler_dependencies"]["compiler_implementation_sha256"], SHA["target"]
    )
    ...
    probe = MechanismEvidence(binding, signature, sha256_bytes(b"probe"), 4, "host extracted probe")
    inputs = dict(
        model=model,
        probe=probe,
    ...
    if outcome == "runner_failed":
        experiment.analysis.timeout_s = 8.0
    ...
    def execute(*, timeout_s):
        ...
        return ProbeObservation(
            probe,
            WarmComputeReceipt("short-independent-probe", 100, WarmProfileContract(), "warm counter receipt"),
            0.1,
            1,
    ...
    if outcome != "passed":
        error, message = (
            (TimeoutError, "bounded runner stopped")
            if outcome == "runner_failed"
            else (ValueError, "does not match" if outcome == "wrong_observation" else "exceeded its bounded")
        )
        with pytest.raises(error, match=message):
            experiment.probes.measure_probe(candidate, admission_inputs=inputs, execute=execute)
        assert experiment.revisions.iterations[-1]["elapsed_seconds"] == initial_elapsed + execution_seconds
        assert not experiment.revisions.iterations[-1]["probe_receipts"]
        if outcome == "runner_failed":
            with pytest.raises(ValueError, match="exceeds iteration wall budget"):
                experiment.probes.measure_probe(
                    candidate,
                    admission_inputs=inputs,
                    execute=lambda **_: pytest.fail("retry exceeded the remaining iteration budget"),
                )
        return
    receipt = experiment.probes.measure_probe(candidate, admission_inputs=inputs, execute=execute)
    assert experiment.revisions.iterations[-1]["elapsed_seconds"] == initial_elapsed + execution_seconds
    assert ran == [30]
    assert receipt["full_model_cycles"] is None
...
def test_agent_cannot_replace_host_probe_bindings(tmp_path):
    ...
    experiment, candidate, _ = setup_experiment(tmp_path)
    experiment.analysis.analyze(candidate, hypothesis="Try an uncertain mechanism")
    with pytest.raises(ValueError, match="current probe bindings"):
        experiment.probes.measure_probe(
            candidate,
            admission_inputs={"current_binding": {}},
...
def test_shared_compiler_helper_edit_invalidates_the_iteration(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(G, "merlin_dir", lambda: tmp_path / "shared")
    experiment, candidate, _ = setup_experiment(tmp_path)
    (candidate / "compiler.py").write_text("from merlin.helper import transform\n")
    row = experiment.analysis.analyze(candidate, hypothesis="Use a shared generalized transform")
...
def test_real_macro_round_transport_compiles_each_revision_without_micro_feedback(tmp_path, monkeypatch):
    ...
    contract = {
        "schema": "compiler_edit_contract_v1",
        "existing_symbols": [{"surface_id": "lowering", "path": "compiler.py", "symbol": "lower"}],
        "helper_extensions": [],
    }
    contract["sha256"] = P2_CONTRACTS.document_sha256(contract)
    experiment.freeze_edit_scope(candidate, contract)
    mechanism_catalog = {
        ...
        "contract_sha256": contract["sha256"],
        "mechanisms": [
            {"id": "lowering", "selectors": [{"kind": "function", "path": "compiler.py", "symbol": "lower"}]}
        ],
    }
    mechanism_catalog["sha256"] = P2_CONTRACTS.document_sha256(mechanism_catalog)
    mechanism_path = tmp_path / "active_mechanism_catalog.json"
    mechanism_path.write_bytes(P2_CONTRACTS.canonical_json(mechanism_catalog))
    mechanism_path.chmod(0o444)
    experiment.freeze_mechanism_catalog(mechanism_path, P2_CONTRACTS.sha256_file(mechanism_path))
    _freeze_test_mechanism_work_order(experiment, candidate, tmp_path, mechanism_id="lowering")
    # Qualification IO is tested separately; this fixture exercises the authoring transport.
    experiment.inputs.phase1_binding = {"test_fixture": "verified existing qualification"}
    action = PB.BrokerAction(BP.E2E_ANALYSIS_ACTION, (BP._HOST_E2E_ANALYSIS_SENTINEL,), (), "full-model analysis", True)
    original_broker = PB.Broker
    brokers = []
    ...
    def codex_round(*args, **kwargs):
        (candidate / "compiler.py").write_text("def lower():\n return 2\n")
        result = brokers[-1].execute({"action": BP.E2E_ANALYSIS_ACTION, "bindings": {}})
        assert result["returncode"] == 0
        transcript = tmp_path / "transcript.jsonl"
        transcript.write_text("{}\n")
        return 0, transcript, None

    monkeypatch.setattr(PB, "Broker", broker)
    monkeypatch.setattr(AUTHORING, "_codex_round", codex_round)
    monkeypatch.setattr(BP, "action_registry", lambda *_args, **kwargs: (action,))
    monkeypatch.setattr(original_broker, "serving", lambda _self: nullcontext(("127.0.0.1", 1)))
    monkeypatch.setattr(AW, "verify_answer_free_agent_inputs", lambda *_: None)
    monkeypatch.setattr(
        AW,
        "inner_execution_policy",
        lambda *_, **_kwargs: AW.AgentSandboxPolicy(
            ("bwrap",),
            (),
            "available_not_an_isolation_claim",
            True,
    ...
    monkeypatch.setattr(AW, "run_required_tool_probes", lambda *_: [])
    ...
    monkeypatch.setattr(PAuthor, "inspect_compiler_package", inventory)
    monkeypatch.setattr(TEL, "collect_round", lambda *_args, **_kwargs: {"complete": True})
    monkeypatch.setattr(TEL, "read_preflight", lambda *_args: {})
    monkeypatch.setattr(
        TA,
        ...
        lambda *_: {
            "clean": True,
            "broker_invocations": [
                {"action": BP.E2E_ANALYSIS_ACTION, "bindings_sha256": sha256_bytes(P2_CONTRACTS.canonical_json([]))}
            ],
        },
    )
    result = PAuthor.PortfolioAuthoring(
        experiment,
        ...
        agent_inputs=SimpleNamespace(),
        frozen_functional=SimpleNamespace(),
        frozen_corpus_manifest=tmp_path / "manifest.json",
        model="test",
        resolved_model="test",
        effort="high",
        codex_binary=Path("codex"),
        max_tool_calls=3,
        declared_instruction_evidence={},
    ).run_round(candidate, round_index=0, round_timeout_s=30)
    ...
    assert (
        P2_CONTRACTS.mapping_file(Path(result["mechanism_attribution"]["receipt"]["path"]))["candidate_sha256"]
        == result["candidate_sha256"]
    )
    assert result["global_speedup_proven"] is False
    assert len(calls) == 2 and calls[0] != calls[1]
    assert result["broker_evidence"]["successful_actions"] == [BP.E2E_ANALYSIS_ACTION]
    assert brokers[0].workflow.stop_verdict is None
    ...
    assert context["host_post_authoring_validation"]["maximum_seconds"] == experiment.analysis.timeout_s
    assert context["host_post_authoring_validation"]["full_model_simulation_allowed"] is False
    assert context["host_frozen_mechanism_work_order"]["work_order"]["mechanism_id"] == "lowering"
    assert context["mechanism_work_order_analysis"]["candidate_sha256"] != result["candidate_sha256"]


def test_macro_round_validates_final_bytes_after_authoring_with_independent_host_budget(tmp_path, monkeypatch):
    ...
    experiment.inputs.phase1_binding = {"test_fixture": "verified existing qualification"}
    action = PB.BrokerAction(
        BP.E2E_ANALYSIS_ACTION, (BP._HOST_E2E_ANALYSIS_SENTINEL,), (), "optional in-round full-model analysis", False
    )
    original_broker = PB.Broker
    brokers = []
    analysis_timeouts = []
    original_analyze = experiment.analysis.analyze
    ...
    experiment.analysis.analyze = analyze
    monkeypatch.setattr(PB, "Broker", broker)
    monkeypatch.setattr(AUTHORING, "_codex_round", codex_round)
    monkeypatch.setattr(BP, "action_registry", lambda *_args, **kwargs: (action,))
    monkeypatch.setattr(original_broker, "serving", lambda _self: nullcontext(("127.0.0.1", 1)))
    ...
    monkeypatch.setattr(TEL, "collect_round", lambda *_args, **_kwargs: {"complete": True})
    monkeypatch.setattr(TEL, "read_preflight", lambda *_args: {})
    monkeypatch.setattr(TA, "audit_codex_transcript", lambda *_: {"clean": True, "broker_invocations": []})
    ...
    result = PAuthor.PortfolioAuthoring(
        ...
        max_tool_calls=3,
        declared_instruction_evidence={},
    ).run_round(candidate, round_index=0, round_timeout_s=600)

    assert result["status"] == "authored"
    assert analysis_timeouts == [None, 1200]
    assert len(calls) == 2 and calls[0] != calls[1]
    assert brokers[0].deadline - time.monotonic() <= 420
...
def test_cached_policy_rebind_preserves_masks_and_rejects_identity_drift(tmp_path):
    old, new, scratch, next_scratch, answers, overlay = (
        tmp_path / name for name in ("old", "new", "scratch", "next_scratch", "answers", "overlay")
    )
    ...
    cached = {
        "package_path": str(old),
        "scratch_path": str(scratch),
        "compiler_dependencies": G.compiler_dependency_record(old),
        "command_prefix": prefix,
        "bwrap_argv_length": 10,
        "overlay_trees": {str(overlay): P2_CONTRACTS.exact_tree_record(overlay)["sha256"]},
        "answer_surfaces": [{"path": str(answers), "kind": "dir"}],
    }
    rebound = PS.rebind_compiler_sandbox(cached, package=new, scratch=next_scratch, dependencies=dependencies)
    ...
    with pytest.raises(ValueError, match="masked answer"):
        PS.rebind_compiler_sandbox(cached, package=answers, scratch=next_scratch, dependencies=dependencies)
    (new / "compiler.py").write_text("def lower():\n return 3\n")
    with pytest.raises(ValueError, match="source identity"):
        PS.rebind_compiler_sandbox(cached, package=new, scratch=next_scratch, dependencies=dependencies)
    (new / "compiler.py").write_text("def lower():\n return 2\n")
    ...
    with pytest.raises(ValueError, match="closure changed"):
        PS.rebind_compiler_sandbox(cached, package=new, scratch=next_scratch, dependencies=forged)


@pytest.fixture
def sandbox_contract_root(tmp_path):
    contract_root = tmp_path / "sandbox-contract"
    for name in (
        "gate_phases.yaml",
        "hardware_pins.yaml",
        ...
        destination.write_bytes((repo_root() / "merlin/contract" / name).read_bytes())
...
def test_factory_reuses_exact_policy_without_rebuilding_masks_for_each_revision(
    tmp_path, monkeypatch, separate_comparison, explicit_contract, sandbox_contract_root
):
    shared = tmp_path / "selected-compiler-sources"
    shared.mkdir()
    (shared / "__init__.py").write_text("")
    (shared / "helper.py").write_text("VALUE = 1\n")
    contract_root = sandbox_contract_root if explicit_contract else None
    experiment, candidate, _ = setup_experiment(
        tmp_path, compiler_shared_source_root=shared, contract_root=contract_root
    )
    ...
    historical_sha = P2_CONTRACTS.sha256_file(historical_schema)
    calls = []

    def inner(*args, **kwargs):
        calls.append(str(args[1]))
        assert args[4] == experiment.inputs.baseline  # /perf-functional-base keeps qualification identity.
        return AW.AgentSandboxPolicy(
            ("bwrap", "--clearenv", "--ro-bind", str(frozen_contract), "/historical-contract"),
            (),
            "test-only",
            True,
            True,
    ...
    monkeypatch.setattr(AW, "verify_answer_free_agent_inputs", lambda *_: None)
    monkeypatch.setattr(AW, "inner_execution_policy", inner)
    monkeypatch.setattr(
        PB,
        "inner_command",
        lambda policy, *_: [*policy.argv, "bash", "-c", 'exec "$@"', "perf-tool", "PAYLOAD_MARKER"],
    )
    monkeypatch.setattr(BW, "apply_answer_masks", lambda argv, _: [*argv, "--tmpfs", str(answers)])
    monkeypatch.setattr(BW, "coverage_gap", lambda *_: [])
    monkeypatch.setattr(PS, "compiler_dependency_mounts", lambda argv, *_: argv)
    factory = PS.PortfolioSandboxFactory(
        experiment.analysis,
        sandbox_inputs=_synthetic_sandbox_inputs(tmp_path, surfaces=[SimpleNamespace(path=answers, kind="dir")]),
        target_experiment=SimpleNamespace(path=config),
        agent_inputs=SimpleNamespace(manifest_path=config),
        frozen_functional=SimpleNamespace(marker=config),
        frozen_corpus_manifest=config,
    )
    comparison = experiment.inputs.baseline
    ...
    for arm in ("baseline", "candidate"):
        assert first[arm]["compiler_dependencies"]["shared_source_root"] == str(shared.resolve())
    assert "helper.py" in first["candidate"]["compiler_dependencies"]["shared_sources"]
    schema = ED.whole_program_schema_record(
        (contract_root or repo_root() / "merlin/contract") / "schemas/command_buffer.schema.json"
    )
    prefix = first["candidate"]["command_prefix"]
    schema_index = prefix.index(schema["path"])
    assert prefix[schema_index - 1 : schema_index + 2] == [
        "--ro-bind",
        schema["path"],
    ...
    assert G.host_verification_policy_record(contract_root=contract_root)["sources"][schema["path"]] == schema["sha256"]
    assert prefix[prefix.index(str(frozen_contract)) + 1] == "/historical-contract"
    assert P2_CONTRACTS.sha256_file(historical_schema) == historical_sha
    ...
    assert (
        first["candidate"]["compiler_dependencies"]["candidate_sha256"]
        != second["candidate"]["compiler_dependencies"]["candidate_sha256"]
    )
    assert second["candidate"]["scratch_path"] == str(second_scratch)
    if explicit_contract:
        selected_schema = contract_root / "schemas/command_buffer.schema.json"
        document = json.loads(selected_schema.read_text())
        document["$comment"] = "selected schema revision changed"
        selected_schema.write_text(json.dumps(document))
        third_scratch = tmp_path / "scratch3"
        third_scratch.mkdir()
        third = factory(comparison, candidate, third_scratch)
        assert len(calls) == 4
        assert "policy_reuse" not in third["candidate"]
        assert P2_CONTRACTS.sha256_file(historical_schema) == historical_sha
...
def test_pre_action_scope_failure_has_terminal_receipt_and_http_refusal(tmp_path, monkeypatch, failure):
    ...
    if failure == "scope":
        ...
        (experiment.inputs.baseline / "source.txt").write_text("changed immutable baseline")
    action = PB.BrokerAction("candidate-test", ("must-not-execute",), (), "test", False)
    ...
    monkeypatch.setattr(PB, "inner_command", lambda *_: pytest.fail("candidate execution reached"))
    ...
    monkeypatch.setattr(PB, "ThreadingHTTPServer", FakeServer)
    with broker.serving():
        for index in range(2):
            payload = json.dumps({"action": action.name, "bindings": {}}).encode()
            ...
            handler.headers = {"Content-Length": str(len(payload)), "X-Perf-Token": broker.token}
...
def test_unexpected_scope_checker_fault_is_not_normalized_as_a_policy_refusal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(experiment.revision_session, "validate_candidate_scope", broken_checker)
    action = PB.BrokerAction("candidate-test", ("must-not-execute",), (), "test", False)
    broker = PB.Broker(
        AW.AgentSandboxPolicy(
            ("bwrap",),
...
@pytest.mark.parametrize(
    "relative",
    [
        "perf/storage_encoding.py",
        "perf/structural_transitions.py",
        "perf/physical_transition_evidence.py",
        "runtime/storage_binding.py",
        "runtime/prepack_authority.py",
        "runtime/captured_constants.py",
        "frontends/argument_identity.py",
        "perf/host_physical_transition_qualifier.py",
        "perf/model_placement.py",
        "perf/model_macs.py",
    ],
)
def test_storage_interpreter_content_change_invalidates_frozen_segment(tmp_path, monkeypatch, relative):
    original = module_source_path("merlin." + relative.removesuffix(".py").replace("/", "."))
    copied = tmp_path / "host_interpreter.py"
    copied.write_bytes(original.read_bytes())
    sha_file = P2_CONTRACTS.sha256_file
    # Route just this source read to test-owned bytes; production sources remain untouched.
    monkeypatch.setattr(
        P2_CONTRACTS,
        "sha256_file",
        lambda path: sha_file(copied if Path(path).resolve() == original.resolve() else path),
    )
    experiment, candidate, calls = setup_experiment(tmp_path)
    assert str(original.resolve()) in experiment.inputs.host_policy["sources"]
    experiment.analysis.analyze(candidate, hypothesis="Bind original storage interpretation")
    copied.write_bytes(copied.read_bytes() + b"\n# changed storage interpreter\n")
    with pytest.raises(ValueError, match="host verification policy changed"):
        experiment.analysis.analyze(candidate, hypothesis="Must not reuse stale interpretation")
    assert len(calls) == 1
...
def test_agent_storage_view_compacts_maps_and_keeps_bound_unresolved_obligations(tmp_path, bound):
    experiment, candidate, _ = setup_experiment(tmp_path)
    record = experiment.analysis.analyze(candidate, hypothesis="Typed storage plan")
    plan = record["analysis"]["diagnostics"]["verified_global_plan_emission"]
    contracts = {
        f"tensor_{index}": {
            "contract": {"logical_shape": [2, 3], "physical_shape": [3, 2]},
    ...
    original = copy.deepcopy(record)
    view = AV.agent_analysis_view(record, complete_evidence="/perf-control/full_model_analysis_0000.json")
    assert record == original
    ...
    assert summary["details"]["canonical_record_sha256"] == P2_CONTRACTS.document_sha256(record)
    assert summary["details"]["encoding_map_sha256"] == P2_CONTRACTS.document_sha256(contracts)
    ...
    assert transition_summary["details"]["evidence_sha256"] == P2_CONTRACTS.document_sha256(
        plan["physical_transition_evidence"]
    )
...
def test_baseline_storage_view_keeps_separate_binding_and_details(tmp_path, bound):
    experiment, candidate, _ = setup_experiment(tmp_path)
    record = experiment.analysis.analyze(candidate, hypothesis="Separate baseline storage evidence")
    analysis = record["analysis"]
    analysis["emission"]["baseline_lowered_sha256"] = sha256_bytes(b"baseline LLVM")
    analysis["emission"]["baseline_command_buffer_sha256"] = sha256_bytes(b"baseline CB")
    baseline = copy.deepcopy(analysis["diagnostics"]["verified_global_plan_emission"])
    baseline["candidate_sha256"] = experiment.inputs.optimization_baseline_sha256
    ...
    baseline["storage_encodings"] = {
        f"baseline_tensor_{i}": {"caller_materialization": "requires baseline evidence"} for i in range(393)
    }
    if not bound:
    ...
    assert summary["source_plan_binding_verified"] is bound
    assert (
        summary["details"]["json_pointer"]
        == "/analysis/diagnostics/verified_baseline_global_plan_emission/storage_encodings"
    )
    assert summary["details"]["canonical_record_sha256"] == P2_CONTRACTS.document_sha256(record)
...
def test_four_model_fast_evaluator_is_serialized_bound_and_agent_visible(tmp_path):
    from merlin.perf.phase2_portfolio import (
        FastEvaluationPolicy,
        standard_four_model_quality_schema,
    )

    baseline, candidate = tmp_path / "fast-base", tmp_path / "fast-candidate"
    for path in (baseline, candidate):
        path.mkdir()
        (path / "source.txt").write_text(path.name)
    sentinels = []
    for index in range(4):
        source = tmp_path / f"fast-model-{index}"
        source.mkdir()
        (source / "capsule.yaml").write_text("interface_mlir: capsule.interface.mlir\n")
        (source / "capsule.interface.mlir").write_text(f"module {{ // fixture {index}\n}}\n")
        sentinels.append(
            INPUTS.StageE2ESentinel(
                f"fast-model-{index}",
                str(source),
                str(source),
                P2_CONTRACTS.exact_tree_record(source)["sha256"],
                ("lane",),
                ("L2",),
            )
        )
        ...
        return {
            ...
            "diagnostics": {
                ...
                "verified_global_plan_emission": {
                    ...
                    "candidate_sha256": candidate_sha,
                    "logical_dispatch_digest": SHA["graph"],
                    "source_sha256": P2_CONTRACTS.sha256_file(Path(objective.capsule_path) / "capsule.interface.mlir"),
                    "candidate_lowered_sha256": SHA["llvm"],
                    "candidate_command_buffer_sha256": SHA["buffer"],
    ...
    def metrics(cycles, placed, islands, boundaries):
        return {
            ...
            "occupancy": {
                "total_cycles": cycles,
                "busy_cycles": {"compute": cycles * (0.6 if cycles < 100 else 0.5), "move": cycles * 0.5},
            ...
            "roofline": {
                ...
                "composition": "explicit adapter max",
                "provenance": ["host roofline"],
            },
    ...
    def provider(**kwargs):
        ...
        try:
            ...
            assert kwargs["provider_binding"]["resource_admission"] == "serialized_one_model_at_a_time"
            if kwargs["sentinel"].capsule_sha256 == sentinels[0].capsule_sha256:
                baseline_quality = {"top1_degradation_percentage_points": 0.0}
                candidate_quality = {"top1_degradation_percentage_points": 0.4}
            else:
                baseline_quality = {"cosine_similarity": 1.0, "normalized_root_mean_square_error": 0.0}
                candidate_quality = {"cosine_similarity": 0.995, "normalized_root_mean_square_error": 0.01}
            return {
                "baseline": metrics(100, 70, 2, 10),
                "candidate": metrics(80, 80, 1, 8),
                "baseline_quality": {
                    "values": baseline_quality,
                    "complete": True,
                    "provenance": ["independent reference"],
                },
                "candidate_quality": {
                    "values": candidate_quality,
                    "complete": True,
                    "provenance": ["independent reference"],
                },
            }
    ...
    corpora = {
        member.capsule_sha256: sha256_bytes(f"held-out-{index}".encode()) for index, member in enumerate(sentinels)
    }
    quality = standard_four_model_quality_schema(
        [member.capsule_sha256 for member in sentinels],
    ...
    record = experiment.analysis.analyze(candidate, hypothesis="Evaluate fast global candidate")
    digest = AV.portfolio_action_digest(record, complete_evidence="complete.json", edit_contract=None)

    assert provider_calls == [member.capsule_sha256 for member in sentinels]
    assert maximum_active == 1
    assert record["fast_evaluation"]["status"] == "retain"
    assert record["fast_evaluation"]["resource_admission"] == "serialized_one_model_at_a_time"
    assert record["fast_evaluation"]["provider_evaluation_order"] == provider_calls
    assert digest["fast_accuracy_bounded_evaluation"] == record["fast_evaluation"]
    assert (
        P2_CONTRACTS.mapping_file(experiment.output / "experiment.json")["fast_evaluation_binding_sha256"]
        == experiment.fast_evaluation.binding_sha256
    )
    experiment.edit_authority.contract = {
        "existing_symbols": [{"surface_id": "allowed", "path": "compiler.py", "symbol": "schedule"}]
    }
    filtered = experiment.fast_evaluation.authorized_surfaces(
        {
            "optimization_brief": {
                "ranked_actions": [
                    {
    ...
    assert [surface["id"] for surface in filtered] == ["allowed"]
    over_budget_binding = {
        **experiment.fast_evaluation.provider_binding,
        "maximum_model_seconds": 61,
    }
    with pytest.raises(ValueError, match="exact implementation binding"):
        G.GlobalPerfExperiment(
            baseline=baseline,
            baseline_sha256=hash_tree(baseline)["sha256"],
            sentinel=sentinels[0],
            portfolio_sentinels=sentinels[1:],
            target="test-target",
            target_sha256=SHA["target"],
            output=tmp_path / "over-budget-fast-run",
            analyzer=analyzer,
            fast_evaluation_provider=provider,
            fast_evaluation_policy=FastEvaluationPolicy(),
            quality_budgets=quality.budget_map,
            fast_evaluation_provider_binding=over_budget_binding,
        )


def test_unconfigured_fast_evaluator_records_exact_only_fallback(tmp_path):
    experiment, candidate, _ = setup_experiment(tmp_path)

    record = experiment.analysis.analyze(candidate, hypothesis="Keep exact semantics without corpus")

    assert record["fast_evaluation"]["status"] == "exact_only_fallback"
    assert record["fast_evaluation"]["approximation_allowed"] is False
    assert record["fast_evaluation"]["maximum_parallel_model_evaluations"] == 1
