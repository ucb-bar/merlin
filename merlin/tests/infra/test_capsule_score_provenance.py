...
_spec = importlib.util.spec_from_file_location("cp", str(repo_root() / "build_tools/scripts/check_provenance.py"))
...
def test_a_capsule_score_is_a_verdict_claim():
    assert (
        cp._claims_a_verdict({"task": "atlas-mlir-oot-capsule", "functional_pass": 0, "n_passed": 14, "n_capsules": 26})
        is True
    )
...
def test_a_zero_score_over_a_real_suite_still_claims():
    ...
    assert cp._claims_a_verdict({"task": "t", "functional_pass": 0, "n_passed": 0, "n_capsules": 26}) is True
...
def test_a_per_round_qa_verdict_is_not_a_published_verdict():
    ...
    assert (
        cp._claims_a_verdict({"qa_gate": "capsule_bench_v0_pilot", "all_pass": False, "n_passed": 14, "n_capsules": 29})
        is False
    )
