    ...
    for name, (n, wall) in obs.items():
        ...
        if wall is not None:
            (d / "capsule_result.json").write_text(
                json.dumps({"capsule": name, "tiers": {tier: {"status": "pass", "timing": {"adapter_wall_s": wall}}}})
            )
    return root
...
def test_cost_model_is_fitted_from_the_runs_own_passing_walls(tmp_path):
    ...
    obs = loop.CERT._l3_cost_observations(root, "L3")
    assert sorted(obs) == [(7, 111.9), (2310, 1819.1)]
    rate, fixed, basis = loop.CERT._l3_cost_fit(obs)
        ...
        json.dumps(
            {
                "capsule": "timedout",
                "tiers": {
                    "L3": {"status": "fail", "reason": "tool_crash: timeout", "timing": {"adapter_wall_s": 2400.0}}
                },
            }
        )
    )
    assert loop.CERT._l3_cost_observations(root, "L3") == [(100, 200.0)]
    # its SIZE is still known — the capsule is priced, it just does not calibrate the model
    assert loop.CERT._l3_instruction_counts(root)["timedout"] == 3084
...
def test_a_large_capsule_is_not_failed_for_being_large(tmp_path):
    ...
    flat = loop.CERT._verilator_per_capsule_timeout(SimpleNamespace(target="fixture", experiment=tmp_path))
    budget = loop.CERT._verilator_l3_budget(
        root, ["tiny", "gm0", "gm1"], "L3", context=SimpleNamespace(target="fixture", experiment=tmp_path)
    )
...
def test_the_budget_still_bounds_a_hang(tmp_path):
    ...
    root = _run_tree(tmp_path, {"tiny": (7, 111.9), "big": (2310, 1819.1), "monster": (10_000_000, None)})
    flat = loop.CERT._verilator_per_capsule_timeout(SimpleNamespace(target="fixture", experiment=tmp_path))
    budget = loop.CERT._verilator_l3_budget(
        root, ["tiny", "big", "monster"], "L3", context=SimpleNamespace(target="fixture", experiment=tmp_path)
    )
    assert budget == flat * loop.CERT.L3_BUDGET_MAX_MULT
...
def test_no_observations_falls_back_to_the_unscaled_bound_and_says_so(tmp_path):
    ...
    flat = loop.CERT._verilator_per_capsule_timeout(SimpleNamespace(target="fixture", experiment=tmp_path))
    assert (
        loop.CERT._verilator_l3_budget(
            root, ["a", "b"], "L3", context=SimpleNamespace(target="fixture", experiment=tmp_path)
        )
        == flat
    )
    rec = json.loads((root / "l3_timeout_derivation.json").read_text())[-1]
...
def test_an_unsized_capsule_is_recorded_not_assumed_small(tmp_path):
    ...
    root = _run_tree(tmp_path, {"tiny": (7, 111.9), "big": (2310, 1819.1)})
    loop.CERT._verilator_l3_budget(
        root, ["tiny", "big", "never_emitted"], "L3", context=SimpleNamespace(target="fixture", experiment=tmp_path)
    )
    rec = json.loads((root / "l3_timeout_derivation.json").read_text())[-1]
