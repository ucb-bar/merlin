...
def test_direct_emission_pair_uses_optional_bundle_without_fallback(tmp_path, monkeypatch, returncode):
    ...
    monkeypatch.setattr(oot_runner, "run_entrypoint", entrypoint)
    result = stage._emit_pair(package, tmp_path / "interface.mlir", tmp_path, "candidate", 7)
    assert calls == ["emit_analysis_bundle"]
...
def test_whole_model_analysis_stops_after_failed_baseline_pair(tmp_path, monkeypatch):
    ...
    sentinel = stage.StageE2ESentinel(
        "test-model", str(source), str(source), stage._exact_tree_record(source)["sha256"], (), ()
    )
    calls = []
    ...
    with pytest.raises(stage.StageGateError, match="baseline printer failed") as caught:
        stage.analyze_whole_model_emission(
            baseline,
            candidate,
            sentinel,
            timeout_s=10,
...
def test_whole_model_analysis_reuses_exact_seed_and_cached_baseline(tmp_path, monkeypatch):
    ...
    def audit(text, *, arm, timeout_s):
        ...
        return {"status": "verified", "source_sha256": stage._sha256(text.encode()), "build_policy_identity": policy}
...
def test_worker_deadline_kills_analysis_and_child(tmp_path):
    ...
    worker = IsolatedAnalysisWorker(stage_path=stage, sandbox_factory=lambda *args: {}, output=tmp_path / "worker")
