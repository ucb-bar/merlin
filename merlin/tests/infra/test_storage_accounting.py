...
def test_product_destinations_and_derived_folds_match_the_concern_roster():
    from merlin.common.artifacts import declared_product_roots

    concerns = set(SC.declared_concerns())
    for name, destination in declared_product_roots(SC.contract()).items():
        assert destination.parts[0] in concerns
        if str(destination) != name:
            assert name not in concerns, f"{name!r} is both declared and folded away"
