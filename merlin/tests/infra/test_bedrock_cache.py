...
def bedrock_agent():
    ...
    from merlin_experiments.phase1.providers import (
        bedrock_agent,  # noqa: PLC0415 — loaded off the import-isolated harness path
    )
...
def _run(bedrock_agent, tmp_path, monkeypatch, script, **kw):
    ...
    rc, tpath = bedrock_agent.run_round(ws, run_dir, "glm5", {}, te, "none", 1, timeout=100, max_iters=5, **kw)
...
def test_static_and_rolling_cachepoints(bedrock_agent, tmp_path, monkeypatch):
    ...
    reads = [e["message"]["usage"]["cache_read_input_tokens"] for e in events if e.get("type") == "assistant"]
    assert 150 in reads
...
_REAL_REJECT = (
    "An error occurred (AccessDeniedException) when calling the Converse operation: You invoked "
