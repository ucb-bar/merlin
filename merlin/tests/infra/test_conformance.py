...
from merlin_experiments.phase1 import conformance as C

from merlin.common.paths import repo_root

sys.path.insert(0, str(repo_root() / "merlin" / "experiments" / "capsule_bench" / "harness"))
from merlin_experiments.phase1 import authoring as LOOP  # noqa: E402
from merlin_experiments.phase1.feedback import certification  # noqa: E402
...
def test_byte_identical_vendored_xdsl_regex_is_attributed_not_charged(tmp_path, monkeypatch: pytest.MonkeyPatch):
    vendored = "import re\nx = re.search('a', 'b')\n"
    ...
    v = C.compute(tp, sub, "merlin_assisted", "inline_asm_insn", resolved_tools={"xdsl_kit"})
...
def test_xdsl_directory_name_does_not_exempt_modified_regex(tmp_path, monkeypatch: pytest.MonkeyPatch):
    upstream = "import re\nx = re.search('a', 'b')\n"
...
def test_full_selfcheck_counts_complete_nonzero_grade_response(tmp_path):
    ...
    sub = _submission(tmp_path, {})
    calls = [("bash", {"command": "python3 agent_selfcheck.py --submission submission --sim spike --capsules all"})]
    complete = json.dumps(
        {
            "sim": "spike",
            "n_capsules": 2,
...
def test_arm4_is_selected_by_resolved_tools_not_the_stale_arm_name(tmp_path):
    ...
    v = C.compute(
        _transcript(tmp_path / "t.jsonl", calls),
        sub,
        "merlin_assisted",
        "inline_asm_insn",
        resolved_tools={
...
def test_fully_conformant_arm4_requires_successful_rtl_generator_and_readback_evidence(tmp_path):
    ...
    calls = [
        ...
        (
            "bash",
            {
                "command": 'python -c "from merlin.targetgen import rtl_backend as R; '
        ...
        (
            "bash",
            {
                "command": 'python -c "from merlin.targetgen.rtl.facts import load_facts; '
        ...
        (
            "bash",
            {
                "command": 'python -c "from merlin.targetgen.generate import mlir_scaffold; '
...
def test_native_codex_shell_wrapper_preserves_arm4_semantic_evidence(tmp_path):
    ...
    calls = [
        ...
        ("Bash", {"command": codex("python action_catalog.py escalation-ladder spatial.dataflow gemmini")}),
...
def test_native_codex_shell_wrapper_does_not_enable_spoof_composition_or_null_readback(tmp_path):
    ...
    null_v = C.compute(null_read, sub, "merlin_assisted", "inline_asm_insn", resolved_tools=tools)
...
def test_arm4_required_commands_fail_closed_on_error_missing_result_and_stale_readback(tmp_path):
    ...
    calls = [
        (
            "bash",
            {
                "command": 'python -c "import merlin.targetgen.rtl_backend as R; '
                "print(R.derived_levers(R.target_profile('gemmini')))\""
            },
        ),
        (
    ...
    tp = _transcript(tmp_path / "t.jsonl", calls, failed={0, 2}, missing={1}, outputs={3: '{"all_pass": false}'})
    v = C.compute(
        tp,
        sub,
        "merlin_assisted",
...
def test_partial_rtl_tool_surface_cannot_silently_downgrade_to_arm3(tmp_path):
    ...
    v = C.compute(tp, sub, "merlin_assisted", "inline_asm_insn", resolved_tools={"rtl_generators"})
    assert v["arm4_tooling_required"] is True
...
def test_l3_fix_recompute_replaces_an_earlier_conformant_verdict(tmp_path):
    ...
    good_calls = [
        ...
        ("bash", {"command": "python -c \"from merlin.targetgen.rtl.facts import load_facts; load_facts('gemmini')\""}),
        (
            "bash",
            {
                "command": 'python -c "from merlin.targetgen.generate import mlir_scaffold; '
    ...
    _, was_conformant = LOOP._workflow_conformance(good_tp, sub, "merlin_assisted", "inline_asm_insn", tools)
    ...
    final, is_conformant = LOOP._workflow_conformance(fix_tp, sub, "merlin_assisted", "inline_asm_insn", tools)
...
def test_l3_fix_verdict_keeps_arm4_rtl_readback_available():
    ...
    verdict = certification._l3_fix_verdict(vv, 4)
    ...
    tp = _transcript(
        ...
        [
            (
                "bash",
                {
                    "command": "/bin/bash -lc \"python cca_contract.py check-bijection t; python - <<'PY'\n"
