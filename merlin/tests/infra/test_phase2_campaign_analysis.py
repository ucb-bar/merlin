...
class TestARefusalIsNotAFailure:
    ...
    def test_refusals_and_failures_are_counted_separately(self):
        budget = PC.summarize_broker_calls(
            [
                _Call("measure", 100.0, 0),
                _Call("measure", 100.0, 125),
                _Call("measure", 100.0, 126),
                _Call("measure", 100.0, 1),
            ]
        )
    ...
    def test_refused_wall_time_is_reported_as_its_own_fraction(self):
        budget = PC.summarize_broker_calls([_Call("cheap", 1.0, 0), _Call("dear", 99.0, 125)])
...
class TestTheTierTableIsTheShapeOfTheCampaign:
    ...
    def test_the_measure_tier_can_dominate_from_a_minority_of_calls(self):
        ...
        budget = PC.summarize_broker_calls(
            [*[_Call("emit", 3.0, 0) for _ in range(100)], *[_Call("sim", 100.0, 0) for _ in range(20)]]
        )
        tiers = budget.by_tier()
...
class TestTheAttemptSummaryGroupsTheThreeWaysAReaderAsks:
        ...
        return Ledger(
            ...
            attempts=[
                ...
                Attempt(
                    ...
                    found_by="i2",
                    verdict="refuted",
                    hypothesis="the branch was believed",
                    deltas=(Delta("w", "m", 1.0, 2.0, "inst"),),
                ),
                Attempt(
                    mechanism="c",
                    scope="frontend",
    ...
    def test_integrity_problems_are_REPORTED_not_dropped(self):
        ...
        bad = Ledger(target="t", attempts=[Attempt(mechanism="x", scope="global", found_by="", verdict="helped")])
...
class TestTheWholeAnalysisSaysWhatItCannotDecide:
    ...
    def test_the_score_falls_as_gaps_appear(self):
        ...
        gapped = PC.build_analysis(**self._facts(stages=0, tool_spans=0, outcomes=[])).to_dict()["availability_score"]
        assert gapped < full
...
class TestItRunsOnThisTreesRealCampaign:
    ...
    def _analysis(self):
        ...
        roots = [runs_dir() / "gemmini" / "perf-bench" / "agent_stages", artifacts_dir() / "perf-bench" / "gemmini"]
        stages = [
            c
            for r in roots
            if r.is_dir()
            ...
            if c.is_dir()
            and ((c / "agent" / "tools.jsonl").is_file() or (c / "control").is_dir() or (c / "global_control").is_dir())
        ]
        if not stages:
        ...
        return PC.build_analysis(
            target="gemmini",
            stages=len(stages),
            tool_spans=spans,
            point_events=points,
            broker_calls=calls,
            ledger=Ledger("gemmini"),
