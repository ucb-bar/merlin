...
# =====================================================================================
def _stage(name: str, *, launches: bool) -> CTRL.ChildStage:
    return CTRL.ChildStage(name, (lambda: None) if launches else None, lambda: name)
...
def test_the_lead_wave_covers_every_corpus_before_anything_fans_out():
    ...
    lead = CTRL.baseline_lead_prefix(stages, phases)
...
def test_nothing_left_to_launch_needs_no_lead_wave():
    ...
    assert CTRL.baseline_lead_prefix(stages, ["tuning", "held_out"]) == 0
...
def _measure_waves(
    ...
    monkeypatch.setattr(CTRL, "run_child_stages", run_child_stages)
    monkeypatch.setattr(CTRL, "_uncheckpointed_state", lambda root, artifact, *, label: "absent")
    monkeypatch.setattr(AD, "child_environment", lambda config, certificate: {})
    monkeypatch.setattr(CTRL, "_run_checked", lambda runner, command, **k: commands.append(list(command)))
    monkeypatch.setattr(CTRL, "_verify_measurement_manifest", lambda path, **kwargs: {"path": str(path)})
    monkeypatch.setattr(CTRL, "_verify_saved_file", lambda saved, output, *, label: Path(output))
    ...
    state = _Checkpoints()
    CTRL._measure_cells(cells, config, state, command_runner=lambda *a, **k: None, workers=workers)
...
class _Engine:
    ...
    def __init__(self, tmp_path: Path, monkeypatch, name: str = "engineA"):
        ...
        self.activate()
        monkeypatch.setattr(PME.CERTPROD, "encode_declared_outputs", lambda outputs, cb: ("s" * 64, {}))
    ...
    def activate(self):
        ...
        self._monkeypatch.setattr(PME.OOT, "run_on_oracle", run_on_oracle)

    def measure(self, store, cb=CB, llvm="module {}"):
        evidence: dict = {}
        result = PME._gsim_l3_adapter("t", evidence, self.certificate, reuse_scope=SCOPE, store=store)(
...
@pytest.fixture()
def store(tmp_path):
    return PME.L3MeasurementStore(tmp_path / "l3_cache")
...
def test_an_identical_program_is_carried_and_every_record_says_so(tmp_path, monkeypatch, store):
    ...
    assert PME.is_sha256(stamp["measured_program_sha256"])
    # ... and it reaches the campaign-level accounting, which is what a reader reads
    report = PME.reuse_report([_row(False), _row(stamp)])
...
def test_a_different_engine_or_pin_is_measured_again(tmp_path, monkeypatch, store):
    ...
    key = PME._l3_memo_key(CB, "module {}", engine.pin, SCOPE)
...
def test_a_cell_that_states_neither_refuses_the_campaign():
    ...
    report = PME.reuse_report([_row(False), _row(None), _row({"measured_program_sha256": "nope"})])
...
def test_the_correctness_screen_is_not_counted_as_a_cited_cell():
    ...
    report = PME.reuse_report([_row(None, simulator="spike", citable=False), _row(False)])
...
def test_the_headline_pairs_say_which_half_of_the_ratio_was_carried():
    ...
    pair = PME.paired_cycle_rows([{**row, "cycles": 10, "correct": True} for row in rows])[0]
...
def _plan(tmp_path: Path, members: int = 3):
    ...
    workloads = [PME.gsim_workload(member) for member in loaded]
    certificate = SimpleNamespace(
        sha256="d" * 64,
        unresolved={},
        pins={},
        members={PME.GATE.workload_sha256(workload): {} for workload in workloads},
    )
    ...
    return PME.build_measurement_plan(inputs)
...
def _run(plan, out_dir: Path, *, width: int, hardware_counters: bool = False, hook=None):
    ...
    fanout = PME.schedule_fanout(width, plan, hardware_counters=hardware_counters)
    rows, roofline = PME.execute_schedule(
        plan,
        out_dir,
        contract_root=out_dir / "explicit-contract",
        timeout=1,
...
def test_a_wide_run_records_exactly_what_the_serial_run_records(tmp_path):
    ...
    assert PME.ME.completion_report(wide_rows, plan.expected)["complete"] is True
...
def test_hardware_counters_refuse_to_fan_out(tmp_path):
    ...
    plan = _plan(tmp_path)
    fanout = PME.schedule_fanout(16, plan, hardware_counters=True)
...
def test_a_relative_input_path_refuses_to_fan_out(tmp_path, monkeypatch):
    ...
    plan = _plan(tmp_path)
    relative = [PME.copy.copy(spec) for spec in plan.schedule]
    object.__setattr__(relative[0], "package", Path("baseline"))
    narrowed = PME.MeasurementPlan(
        plan.phase, tuple(relative), plan.expected, plan.declaration, plan.declaration_sha256
    )
    fanout = PME.schedule_fanout(16, narrowed, hardware_counters=False)
...
def test_a_width_below_one_is_refused_rather_than_rounded(tmp_path):
    with pytest.raises(PME.PC.CampaignGateError):
        PME.schedule_fanout(0, _plan(tmp_path), hardware_counters=False)
...
def test_an_unset_width_leaves_the_campaign_serial():
    source = module_source_path("merlin_experiments.phase2.paired_cli").read_text(encoding="utf-8")
    declarations = [
        node
        ...
        and node.args[0].value == "--sim-workers"
    ]
    assert len(declarations) == 1
    keywords = {item.arg: item.value for item in declarations[0].keywords}
    assert isinstance(keywords["type"], ast.Name) and keywords["type"].id == "int"
    assert isinstance(keywords["default"], ast.Constant) and keywords["default"].value == 1, (
