...
import pytest
from merlin_experiments.phase1.telemetry import report as TD
from merlin_experiments.phase1.telemetry import timeline as TL
...
def _init(round_=0):
    return {"type": "system", "subtype": "init", "driver": "codex", "round": round_, "started_at": _at(0)}
...
def _use(sec, call_id, name="Bash", input_=None):
    return {
        "type": "assistant",
        "arrived_at": _at(sec),
        "message": {"content": [{"type": "tool_use", "id": call_id, "name": name, "input": input_ or {}}]},
    }
...
def test_a_zero_duration_result_event_is_unknown_not_a_zero_split():
    ...
    rec = TL.decompose([{"type": "result", "subtype": "success", "duration_ms": 0, "duration_api_ms": 0}])
    assert rec["method"] == "unknown"
    assert rec["think_generate_s"] is None
...
def test_claude_duration_fields_are_still_honoured_when_they_are_real():
    ...
    rec = TL.decompose(
        [{"type": "system", "subtype": "init"}, {"type": "result", "duration_ms": 100_000, "duration_api_ms": 40_000}]
    )
...
def test_the_between_round_grading_gap_is_not_charged_to_the_agent():
    ...
    late = 100_000
    evts += [
        {"type": "system", "subtype": "init", "round": 1, "started_at": _at(late)},
        _use(late + 10, "b"),
        _res(late + 20, "b"),
    ]
    rec = TL.decompose(evts)
...
def test_a_tool_call_whose_result_never_arrived_is_counted_not_dropped():
    ...
    rec = TL.decompose([_init(), _use(10, "a"), _text(300)])
...
def test_a_raw_codex_event_stream_decomposes_too():
    ...
    def ev(sec, etype, item_id, status):
        return {
            "seq": 1,
            "arrived_at": _at(sec),
            "event": {"type": etype, "item": {"id": item_id, "type": "command_execution", "status": status}},
        }

    rec = TL.decompose(
        [
            ev(0, "item.started", "i1", "in_progress"),
...
def test_the_split_partitions_the_measured_span():
    ...
    evts = [_init(), _use(5, "a"), _use(7, "b"), _res(9, "b"), _res(40, "a"), _text(55), _use(60, "c"), _res(90, "c")]
    rec = TL.decompose(evts)
...
def test_named_tools_latency_errors_tokens_rates_and_activity_share_are_recorded():
    evts = [
        _init(),
        _use(10, "a", "Bash", {"command": "one"}),
        _res(40, "a", "done"),
        _use(50, "b", "Edit", {"patch": "x"}),
        _use(55, "c", "Bash"),
        _res(60, "b", "bad", True),
        _usage(65, "m1", 100, 20, 300, 40, 25),
        _usage(66, "m1", 100, 20, 300, 40, 25),  # duplicate streaming envelope
        _res(75, "c"),
        _usage(80, "m2", 50, 10, 40, 20, 5),
    ]
    rec = TL.decompose(evts)
...
def test_oracle_timing_keeps_every_l_tier_invocation_and_null_is_not_zero(tmp_path):
    ...
    fixtures = {
        "A": {
            "L2": {
                "status": "pass",
                "engine": "spike",
                "timing": {"build_s": 1, "sim_active_s": 2, "oracle_wait_s": 3, "adapter_wall_s": 6},
            },
            "L3": {
                "status": "pass",
                "engine": "gsim",
                "timing": {"build_s": 4, "sim_active_s": 5, "oracle_wait_s": 6, "adapter_wall_s": 15},
            },
        },
        "B": {
            "L3": {
                "status": "fail",
                "engine": "gsim",
                "timing": {"build_s": 7, "sim_active_s": 8, "oracle_wait_s": 9, "adapter_wall_s": 24},
            }
        },
    ...
    assert l3["totals_s"] == {"build_s": 11.0, "sim_active_s": 13.0, "oracle_wait_s": 15.0, "adapter_wall_s": 39.0}
...
def _complete_codex_run(tmp_path):
    ...
    events = [
        ...
        _use(10, "a"),
        _res(20, "a"),
        _usage(30, "m1", 10, 0, 20, 5, 1),
        {
            "type": "codex_summary",
            "arrived_at": _at(31),
            "turns_started": 1,
            "turns_usage_reported": 1,
            "usage_complete": True,
            "unknown_types": [],
        },
    ]
    ...
    (rounds / "round_00.codex_events.timestamped.jsonl").write_text(
        "\n".join(json.dumps({"seq": i, "arrived_at": _at(i), "event": row}) for i, row in enumerate(raw, 1)) + "\n"
    )
    ...
    (rounds / "round_00.codex_summary.json").write_text(
        json.dumps(
            {"turns_started": 1, "turns_usage_reported": 1, "usage_complete": True, "unknown_types": [], "wall_s": 31}
        )
    )
    ...
    (rounds / "round_00.resource_samples.jsonl").write_text(
        json.dumps({"sampled_at": _at(0), "rss_bytes": 1, "processes": 1, "threads": 1}) + "\n"
    )
    return tmp_path
...
def test_the_run_writer_records_a_derived_split_not_zeros(tmp_path):
    ...
    rows = [
        ...
        },
        {
            "type": "assistant",
            "arrived_at": t % (1, 30),
            "message": {"content": [{"type": "tool_use", "id": "b", "name": "Bash", "input": {"command": "ls"}}]},
        },
        {
            "type": "user",
            "arrived_at": t % (1, 40),
