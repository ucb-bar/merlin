...
SCHEMAS = (
    ...
    "out/build/python/lib/merlin/_data/contract/schemas/command_buffer.schema.json",
)


def _schema(rel: str) -> dict:
    path = repo_root() / rel
    if rel == SCHEMAS[1] and not path.is_file():
        pytest.skip("release bundle not built; source schema is still checked independently")
...
def test_both_schema_copies_stay_byte_identical():
    ...
    _schema(SCHEMAS[1])  # explicit not-built qualification, never compare the source to itself
