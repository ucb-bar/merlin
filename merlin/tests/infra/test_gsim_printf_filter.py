...
_SOURCE = merlin_dir() / "experiments/gemmini_perf_bench/scripts/gsim_printf_filter.cpp"
...
def test_filter_prefix_is_derived_from_unique_firrtl_disassembly_stream(tmp_path: Path) -> None:
...
def test_filter_suppresses_declared_prefix_and_preserves_diagnostics(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            compiler,
            "-std=c++17",
            '-DGSIM_SUPPRESS_PRINTF_PREFIX="TRACE "',
            str(_SOURCE),
            str(driver),
            "-o",
            str(executable),
