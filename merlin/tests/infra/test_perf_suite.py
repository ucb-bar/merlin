...
def test_claims_split_by_declared_family_not_name_prefix_or_analyzer(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(SUITE.STAGE, "discover_performance_corpus", lambda _: SimpleNamespace(capsules=members))
...
def test_suite_environment_pins_warm_measured_only_protocol(tmp_path, monkeypatch):
    ...
    env = SUITE.environment(tmp_path, {"output_root": str(tmp_path / "out"), "trial_workers": 1, "sim_workers": 2})
...
def test_suite_seals_the_evaluator_contract_runner_and_harness_before_authoring():
    ...
    assert set(boundary["files"]) == {"backend_contract", "command_buffer_schema", "gemmini_harness", "paired_runner"}
...
def test_runtime_preflight_executes_every_member_and_refuses_bad_evidence(tmp_path, monkeypatch, failure):
    ...
    monkeypatch.setattr(
        SUITE.STAGE, "prepare_formal_claim", lambda _members: {"declaration": {}, "expected_identities": []}
    )
