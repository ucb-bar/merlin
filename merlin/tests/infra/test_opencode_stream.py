...
sys.path.insert(
    0, str(merlin_dir() / "experiments" / "capsule_bench" / "harness")
)  # sibling imports (model_tiers, ...)


def _load_driver():
    from merlin_experiments.phase1.providers import opencode_agent
...
_STREAM = "\n".join(
    ...
    for x in [
        ...
        {
            "type": "step_finish",
            "part": {"type": "step-finish", "tokens": {"input": 100, "output": 5, "cache": {"read": 3, "write": 1}}},
        },
    ]
)
...
def test_stream_parse_emits_correlated_tool_events():
    ...
    tool_uses = [
        b for e in events if e["type"] == "assistant" for b in e["message"]["content"] if b.get("type") == "tool_use"
    ]
    tool_results = [
        b for e in events if e["type"] == "user" for b in e["message"]["content"] if b.get("type") == "tool_result"
    ]
    assert n == 1
