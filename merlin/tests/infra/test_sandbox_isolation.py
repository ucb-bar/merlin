...
#: A sandbox that is INSTALLED but cannot construct a namespace passes a `which` check and then fails
#: every invocation, so a guard written against presence lets these tests run and then reports the
#: host's policy as a defect in the code under test. Measured on this host: 12 of the sandbox suite's
#: failures were that, and nothing in the output said so. Guard on whether it can actually RUN.
_SANDBOX = probe_sandbox()
_NO_SANDBOX = None if _SANDBOX.usable else _SANDBOX.describe()
...
def _max_exposure_bundle(target: str) -> dict:
    ...
    allowed = [
        ...
        # The target's BACKEND package -- what this docstring has always claimed to bind and did not.
        # Its hand-authored derivations are an answer surface; only its `contracts/` sub-paths are an
        # arm's input, and those are deeper grants that survive the mask.
        {"path": f"merlin/targets/{target}"},
        # The whole generated-output root. The known-answer recovery KEY and the material it was
        # minted from live here, and a bundle that granted a product directory would re-expose them.
        {"path": "out/artifacts"},
    ]
...
def test_expected_instruction_coverage_is_an_answer_surface_and_audit_token(tmp_path):
    ...
    corpus.mkdir()
    expected = corpus / "expected_instruction_coverage.yaml"
    expected.write_text("synthetic structural expectation\n")
    te = replace(load_target_experiment(descriptor), capsule_corpus=corpus)
    surfaces = {surface.path: surface for surface in answer_surfaces(te)}
...
def test_copy_workspace_omits_m2_m3_weights_and_goldens(tmp_path):
    ...
    from merlin_experiments.phase1 import workspace_transport as experiment
...
def test_a_symlinked_toolchain_is_bound_at_its_REAL_path_too(tmp_path):
    ...
    descriptor = repo_root() / "merlin/experiments/capsule_bench/targets/gemmini/target_experiment.yaml"
    paths = replace(TC.ToolchainPaths.from_checkout(), venv=str(snapshot / ".venv"))
    binds = TC.toolchain_binds(load_target_experiment(descriptor), paths=paths, memory_dir="")
...
def test_a_real_toolchain_path_is_bound_exactly_once(tmp_path):
    ...
    descriptor = repo_root() / "merlin/experiments/capsule_bench/targets/gemmini/target_experiment.yaml"
    paths = replace(TC.ToolchainPaths.from_checkout(), venv=str(real))
    binds = TC.toolchain_binds(load_target_experiment(descriptor), paths=paths, memory_dir="")
    pairs = [(binds[i + 1], binds[i + 2]) for i, tok in enumerate(binds) if tok == "--ro-bind"]
    assert sum(1 for a, b in pairs if a == str(real)) == 1


def test_retired_pinned_submission_mode_is_refused_before_launch(monkeypatch):
    """Historical hash-bound resume is unsupported, not qualified by a mount test."""
    from merlin_experiments.phase1.options import parse_options
    from merlin_experiments.phase1.runtime_environment import prepare_runtime_environment

    def forbidden(*args, **kwargs):
        pytest.fail("unsupported mode must not launch authentication or bwrap")

    monkeypatch.setattr(subprocess, "run", forbidden)
    prepared = prepare_runtime_environment(
        parse_options(["--run-id", "unsupported-mode", "--account-config-dir", "/unused"]),
        {"MERLIN_PINNED_SUBMISSION_READ_ONLY": "1"},
        machine_defaults={},
    )
    assert prepared.refusal == 7
    assert "unsupported" in prepared.message
...
def test_answer_surface_is_derived_and_complete(descriptor, tmp_path):
    ...
    # Private grading assets are intentionally not tracked. Supply an explicitly
    # synthetic corpus while retaining each roster descriptor's other policies;
    # requiring a developer's private goldens made this policy test non-hermetic.
    corpus = tmp_path / "corpus"
    nested = corpus / "nested/member"
    nested.mkdir(parents=True)
    golden = nested / "golden.yaml"
    golden.write_text("synthetic numerical expectation\n")
    ...
    assert any(s.path == golden and s.origin == "golden" and s.kind == "file" for s in surfaces)
    ...
    assert any(s.path == hidden and s.origin == "hidden" and s.kind == "dir" for s in surfaces)
    argv = ["bwrap", "--ro-bind", str(tmp_path), str(tmp_path)]
    assert BW.is_exposed(argv, golden) and BW.is_exposed(argv, hidden)
    masked = BW.apply_answer_masks(argv, surfaces)
    assert not BW.is_exposed(masked, golden) and not BW.is_exposed(masked, hidden)
...
def _live_ready(te) -> bool:
    ...
    paths = ToolchainPaths.from_checkout()
    return Path(paths.venv).exists() and Path(paths.llvm).exists()
