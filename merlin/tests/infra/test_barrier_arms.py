...
@pytest.mark.parametrize("bad", [None, 42, "commands", {}, {"commands": []}, {"commands": [{"operands": {}}]}])
...
def test_a_saving_that_grows_with_removed_barriers_is_established():
    out = B.analyze_barrier_claim(
        [
            {"removed": 1, "cycles_saved": 100},
...
def test_barriers_that_cost_nothing_refute_the_claim_rather_than_refusing_it():
    out = B.analyze_barrier_claim([{"removed": 1, "cycles_saved": 5}, {"removed": 15, "cycles_saved": 3}])
    assert out["verdict"] == B.REFUTED
    assert out["verdict"] != B.REFUSED, "a measured null result is evidence, not missing evidence"


@pytest.mark.parametrize(
    "rows", [[], "nope", [{"removed": 1}], [{"cycles_saved": 1}], [{"removed": True, "cycles_saved": 1}]]
)
