...
def test_symlink_blob_is_not_python_but_its_canonical_source_is_checked(tmp_path):
    root = _repo(tmp_path)
    canonical = root / "extension_setup.py"
    canonical.write_text("x = [1, 2, 3]\n")
    linked = root / "packages/example/setup.py"
    linked.parent.mkdir(parents=True)
    linked.symlink_to("../../extension_setup.py")
    subprocess.run(["git", "add", "extension_setup.py", "packages/example/setup.py"], cwd=root, check=True)
    clean = _gate(root)
    assert clean.returncode == 0, clean.stdout + clean.stderr
    canonical.write_text("x=[1,2,\n3]\n")
    subprocess.run(["git", "add", "extension_setup.py"], cwd=root, check=True)
    dirty = _gate(root)
    assert dirty.returncode == 1 and "extension_setup.py" in dirty.stderr, dirty.stdout + dirty.stderr
