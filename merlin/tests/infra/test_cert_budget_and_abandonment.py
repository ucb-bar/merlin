...
@pytest.fixture()
def agg():
    from merlin.agentreport.phase1 import runs
...
def test_the_broker_uses_the_derived_budget(broker, stub_cost):
    ...
    secs, why = broker._per_capsule_timeout(0, context=feedback_context())
    assert secs == pytest.approx(int(1.5 * (41.0 + 0.00024 * 8_000_000)), abs=2), why
...
def test_the_classification_uses_no_regex(agg, broker):
    ...
    text = module_source_path("merlin.agentreport.phase1.runs").read_text()
