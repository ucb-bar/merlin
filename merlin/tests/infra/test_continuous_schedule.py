...
def test_the_flag_exists_and_defaults_to_continuous():
    ...
    defaults = parse_options(["--run-id", "probe"])
    assert defaults.schedule == "continuous"
    assert defaults.max_wall_s == 0
    explicit = parse_options(["--run-id", "probe", "--schedule", "rounds", "--max-wall-s", "120"])
    assert explicit.schedule == "rounds"
    assert explicit.max_wall_s == 120
...
def test_effective_run_shape_and_feedback_health_are_claim_bearing():
    ...
    src = inspect.getsource(session.RunRequest) + inspect.getsource(session.prepare)
...
def test_the_l3_barrier_still_stops_on_budget_in_rounds_mode():
    ...
    assert loop.CERT._l3_barrier_decision(False, rnd=12, max_rounds=12) == "budget"
    assert loop.CERT._l3_barrier_decision(False, rnd=3, max_rounds=12) == "iterate"
    assert loop.CERT._l3_barrier_decision(True, rnd=99, max_rounds=12) == "done"
...
def test_a_pass_is_always_terminal_whatever_the_cap():
    ...
    for cap in (0, 1, 12, 1_000_000):
        assert loop.CERT._l3_barrier_decision(True, rnd=0, max_rounds=cap) == "done"
...
def test_formal_completion_requires_numeric_tooling_and_official_hidden_grade():
    ...
    assert loop.CERT._authoring_completion(True, False) is False
    ...
    assert loop.CERT._formal_completion(True, False, True, True) is False
    assert loop.CERT._formal_completion(False, True, True, True) is False
...
def test_continuous_never_reports_budget_at_the_l3_barrier():
    ...
    for rnd in (0, 5, 50, 500):
        assert loop.CERT._l3_barrier_decision(False, rnd=rnd, max_rounds=rnd + 1_000_000) == "iterate"
...
def test_a_declared_wall_budget_still_ends_the_l3_fix_loop():
    ...
    assert loop.CERT._l3_barrier_decision(False, rnd=7, max_rounds=7) == "budget"
    import inspect
...
def test_promotion_is_not_coupled_to_the_schedule():
    ...
    src = inspect.getsource(loop_grading)
    assert "from merlin_experiments.phase1.feedback.promotion import" in src
    # and the loop's own promotion must not be gated on the schedule flag
    seg = src.split("from merlin_experiments.phase1.feedback.promotion import", 1)[1][:600]
...
def test_the_model_budget_defaults_to_the_qa_timeout():
    ...
    from merlin_experiments.phase1 import runtime_environment

    src = inspect.getsource(runtime_environment.prepare_runtime_environment)
    from merlin_experiments.phase1.options import parse_options

    assert parse_options(["--run-id", "probe"]).model_budget_s is None
    assert parse_options(["--run-id", "probe", "--model-budget-s", "0"]).model_budget_s == 0
    assert "options.qa_timeout if options.model_budget_s is None else options.model_budget_s" in src
    assert 'set_value("MERLIN_MODEL_BUDGET_S", str(budget))' in src
    assert 'remove("MERLIN_MODEL_BUDGET_S")' in src, "0 must CLEAR the ceiling"
