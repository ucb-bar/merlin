...
def _broker():
    ...
    spec = importlib.util.spec_from_file_location("tier_promote", feedback_source("tier_promote"))
...
def _capsule_result(root: Path, elf: bytes = b"program", *, merlin="1" * 40, rtl="2" * 40):
    ...
    cr.write_text(json.dumps({"capsule": "A", "toolchain_shas": {"merlin": merlin, "target_rtl": rtl}}))
...
def test_a_passing_capsule_produces_a_cert_job(tmp_path):
    ...
    ch = ws / ".qa_channel"
    promoted = B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
    assert promoted == ["A"]
...
def test_promotion_launches_the_enqueue_time_snapshot_after_source_moves(tmp_path, monkeypatch):
    ...
    assert B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
    ...
    submission, snapshot_root, error = S.recover_promotion_snapshot(
        ws, req["submission_digest"], req["identity"], token, submission_digest=S._TP._submission_digest
    )
    try:
...
def test_a_failing_capsule_buys_no_cert_time(tmp_path):
    ...
    assert (
        B.promote(
            ws, ws / ".qa_channel", _verdict([("A", False)]), "L2", "L3", None, sys.stderr, context=feedback_context()
        )
        == []
    )
...
def test_the_same_bytes_are_never_certified_twice(tmp_path):
    ...
    ws = _ws(tmp_path)
    ch = ws / ".qa_channel"
    first = B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
    second = B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
...
def test_identical_executable_survives_an_unrelated_source_edit(tmp_path):
    ...
    assert B.promote(ws, ch, verdict, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
    B.record_cert(ws, _execution_verdict([("A", True, "a")]), "L3", sys.stderr)

    (ws / "submission" / "manifest.yaml").write_text("x: 2")
    assert B.promote(ws, ch, verdict, "L2", "L3", None, sys.stderr, context=feedback_context()) == []
...
def test_changed_executable_invalidates_only_its_capsule(tmp_path):
    ...
    assert B.promote(ws, ch, first, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A", "B"]
    B.record_cert(ws, first, "L3", sys.stderr)

    (ws / "submission" / "manifest.yaml").write_text("x: 2")
    changed = _execution_verdict([("A", True, "c"), ("B", True, "b")])
    assert B.promote(ws, ch, changed, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
...
def test_promotion_never_gates_a_run(tmp_path):
    ...
    for junk in ({}, {"per_capsule": None}, {"per_capsule": [{}]}, {"per_capsule": [{"capsule": None}]}):
        assert B.promote(ws, ws / ".qa_channel", junk, "L2", "L3", None, sys.stderr, context=feedback_context()) == []
...
def test_both_brokers_call_promotion():
    ...
    path that matters."""
    for name in ("simjob_broker.py", "selfcheck_broker.py", "loop_grading.py"):
        src = (feedback_source(name.removesuffix(".py"), HARNESS / name)).read_text(encoding="utf-8")
        assert "feedback.promotion" in src, f"{name} does not reach the shared promotion module"
...
def test_the_round_grade_promotes_too():
    ...
    src = Path(loop_grading.__file__).read_text(encoding="utf-8")
    i = src.index("def grade(")
    j = src.index("def _write_stage_ledger(")
    assert "feedback.promotion" in src[i:j], "qa_grade does not promote"


def test_the_policy_is_sourced_not_reimplemented():
    """One policy, one place. The brokers are plumbing; `oracle_schedule` decides."""
    src = (feedback_source("tier_promote")).read_text(encoding="utf-8")
...
def test_a_rejection_names_the_field_and_the_remedy():
    ...
    src = (feedback_source("simjob_broker")).read_text(encoding="utf-8")
...
def _allowed(monkeypatch, sims):
    """Explicit dispatch policy and contract fixtures, without probing a live target."""
    from types import SimpleNamespace

    from merlin_experiments.phase1.feedback import dispatch

    from merlin.targetgen import capsule_runner as CR
    from merlin.targetgen import runner_config as RC
    from merlin.targetgen import target_experiment as TE

    monkeypatch.setattr(dispatch, "allowed_sims", lambda context: tuple(sims))
    monkeypatch.setattr(TE, "load_target_experiment", lambda path: SimpleNamespace(target="fixture"))
    monkeypatch.setattr(TE, "load_capability_manifest", lambda target: object())
    monkeypatch.setattr(
        RC,
        "runner_config_from_manifest",
        lambda manifest: SimpleNamespace(rtl_tiers=("L3",), tier_sim={"L2": "spike", "L3": dispatch._ELABORATED_RTL}),
    )
    monkeypatch.setattr(CR, "chipyard_l3_selection", lambda target: {"engine": "gsim"})
...
def test_the_enqueued_sim_is_one_the_broker_accepts(tmp_path):
    ...
    """
    B = _broker()
    from merlin_experiments.phase1.brokers import simjob as SB

    ws = _ws(tmp_path)
    ch = ws / ".qa_channel"
    B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
    reqs = list(ch.glob("simreq_*.json"))
    assert len(reqs) == 1
    sim = json.loads(reqs[0].read_text())["sim"]
    assert sim in SB._allowed_sims(feedback_context()), (
        f"promotion enqueued --sim {sim!r}, which this broker would reject; "
        f"it accepts {SB._allowed_sims(feedback_context())}"
    )
...
def test_cert_sim_names_the_ladder_sim_when_one_is_declared(tmp_path, monkeypatch):
    ...
    assert B.cert_sim("L2", context=feedback_context()) == "spike"


def _declared_l3_engine():
    """Fixture selected by the same factory seam cert_sim consumes."""
    return "gsim"
...
def test_cert_sim_resolves_the_fidelity_sentinel_to_a_real_engine(monkeypatch):
    ...
    assert B.cert_sim("L3", context=feedback_context()) == engine
    assert B.cert_sim("L3", context=feedback_context()) != "elaborated_rtl", (
        "the fidelity sentinel leaked out as a --sim token"
    )
...
def test_cert_sim_refuses_an_engine_the_broker_would_reject(monkeypatch):
    ...
    assert B.cert_sim("L3", context=feedback_context()) is None
...
def test_cert_sim_keeps_the_sentinel_when_that_is_all_that_is_accepted(monkeypatch):
    ...
    assert B.cert_sim("L3", context=feedback_context()) == B._NEUTRAL_SIM


def test_cert_sim_fails_closed_when_nothing_serves_the_tier(monkeypatch):
    B = _broker()
    _allowed(monkeypatch, ("spike", "verilator", "vcs"))
    assert B.cert_sim("L9", context=feedback_context()) is None, "an unknown tier must not resolve to a guessed sim"
...
def test_no_pending_state_is_written_without_an_enqueued_job(tmp_path, monkeypatch):
    ...
    B = _broker()
    monkeypatch.setattr(B, "cert_sim", lambda tier, **kw: None)
    ws = _ws(tmp_path)
    ch = ws / ".qa_channel"
    promoted = B.promote(ws, ch, _verdict([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
    assert promoted == []
...
def test_recording_preserves_the_pending_execution_digest(tmp_path):
    ...
    ch = ws / ".qa_channel"
    verdict = _execution_verdict([("A", True, "a")])
    B.promote(ws, ch, verdict, "L2", "L3", None, sys.stderr, context=feedback_context())
    before = dict(B._tier_state(ws)["A"]["L3"])
    B.record_cert(ws, _execution_verdict([("A", True, "a")]), "L3", sys.stderr)
    after = B._tier_state(ws)["A"]["L3"]
...
def test_a_cert_for_different_execution_bytes_does_not_resolve_pending(tmp_path):
    ...
    ws = _ws(tmp_path)
    ch = ws / ".qa_channel"
    B.promote(ws, ch, _execution_verdict([("A", True, "a")]), "L2", "L3", None, sys.stderr, context=feedback_context())
    resolved = B.record_cert(
        ws, _execution_verdict([("A", True, "b")]), "L3", sys.stderr, identity="a" * 64, source_identity_verified=True
    )
    assert resolved == []
    assert B._tier_state(ws)["A"]["L3"]["status"] == "pending"
...
def test_unverified_source_cannot_resolve_a_broad_pending_record(tmp_path):
    ...
    resolved = B.record_cert(ws, _execution_verdict([("A", True, "a")]), "L3", sys.stderr, identity=request["identity"])
...
def test_the_broker_reap_actually_calls_the_recorder(tmp_path):
    ...
    assert 'elif j.get("promoted")' in reap[:800], "the recorder is not on the promoted-job branch of the reap"
...
def test_the_round_grade_verdict_schema_enqueues_a_cert_job(tmp_path):
    ...
    ch = ws / ".qa_channel"
    promoted = B.promote(
        ws, ch, _grade_verdict([("A", "pass")]), "L2", "L3", None, sys.stderr, context=feedback_context()
    )
...
def test_a_failing_round_grade_row_buys_no_cert_time(tmp_path):
    ...
    ch = ws / ".qa_channel"
    assert (
        B.promote(ws, ch, _grade_verdict([("A", "fail")]), "L2", "L3", None, sys.stderr, context=feedback_context())
        == []
    )
    assert list(ch.glob("simreq_*.json")) == []
...
def test_a_row_stating_no_verdict_is_not_recorded_as_a_failure(tmp_path, word):
    ...
    ch = ws / ".qa_channel"
    assert (
        B.promote(ws, ch, _grade_verdict([("A", word)]), "L2", "L3", None, sys.stderr, context=feedback_context()) == []
    )
    assert list(ch.glob("simreq_*.json")) == []
    st = json.loads((ws / "qa" / "tier_state.json").read_text())
...
def test_a_promoted_cert_job_keeps_the_harness_interpreter_ahead_of_the_sim_toolchain():
    ...
    S = _harness_module("simjob_broker")
    parts = S._sim_env(context=feedback_context())["PATH"].split(os.pathsep)
    own = os.path.dirname(S.PY)
    assert own in parts, "the harness interpreter's own bin dir is not on the cert job's PATH"
    sim_entries = [i for i, p in enumerate(parts) if p.startswith(str(S._conda_env()))]
...
def test_a_promoted_cert_job_cannot_read_the_campaign_terminal(monkeypatch, tmp_path):
    ...
    with log.open("wb") as stream:
        got = S._spawn_selfcheck(
            ["selfcheck", "--tiers", "L3"], cwd=tmp_path, env={"PATH": "test"}, job_log=stream, timeout_s=900
        )
...
def test_a_promoted_job_runs_from_a_verified_source_snapshot(tmp_path):
    ...
    S = _harness_module("simjob_broker")
    ws = _ws(tmp_path, files=(("submission/manifest.yaml", "x: 1"), ("submission/build/input.txt", "digest-covered")))
    expected = S._TP._submission_digest(ws)

    submission, root, error = S.recover_promotion_snapshot(
        ws, expected, f"submission:{expected}", submission_digest=S._TP._submission_digest
    )
    ...
    missing, missing_root, reason = S.recover_promotion_snapshot(
        ws, expected, f"submission:{expected}", submission_digest=S._TP._submission_digest
    )
...
def test_a_broad_promotion_identity_must_name_the_verified_source_digest(tmp_path):
    ...
    submission, root, reason = S.recover_promotion_snapshot(
        ws, expected, "submission:0000000000000000", submission_digest=S._TP._submission_digest
    )
