...
def nested_tree(tmp_path, monkeypatch):
    ...
    _git(container, "-c", "protocol.file.allow=always", "submodule", "add", "-q", str(nested), "software/tests")
...
def _registry(
    tmp_path, container_rev: str, nested_rev: str, *, local_edits=None, covers=True, content_check=None
) -> Path:
...
def test_modified_file_whose_bytes_are_the_pinned_revisions_is_pinned(nested_tree, tmp_path):
    ...
    pinned_bytes = subprocess.run(
        ("git", "-C", str(live), "cat-file", "blob", f"{rev_pinned}:include/params.h"), capture_output=True
    ).stdout
...
def test_an_undeclared_edit_is_refused(nested_tree, tmp_path):
    ...
    reg = _registry(
        tmp_path, _git(container, "rev-parse", "HEAD"), rev_pinned, local_edits={"include/params.h": declared}
    )
    st = PROV.source_status("headers", "include/params.h", path=reg)
...
def test_verifying_only_the_container_cannot_miss_the_headers(nested_tree, tmp_path):
    ...
    without = _registry(tmp_path / "no_cover", _git(container, "rev-parse", "HEAD"), rev_pinned, covers=False)
...
def test_a_pinned_read_set_verifies_clean(nested_tree, tmp_path):
    ...
    v = PROV.verify("headers", path=reg)
    assert v.ok, v.drift
    assert {s.rel: s.status for s in v.sources} == {"include/isa.h": PROV.PINNED, "include/params.h": PROV.PINNED}
...
def test_nested_in_naming_an_undeclared_pin_is_refused(tmp_path):
    reg = tmp_path / "p.yaml"
    reg.write_text(
        yaml.safe_dump({"version": 1, "pins": {"a": {"commit": "0" * 40, "nested_in": "ghost", "nested_path": "x"}}}),
        encoding="utf-8",
    )
...
def test_nested_in_without_a_path_is_refused(tmp_path):
    ...
    reg.write_text(
        yaml.safe_dump(
            {"version": 1, "pins": {"a": {"commit": "0" * 40}, "b": {"commit": "1" * 40, "nested_in": "a"}}}
        ),
        encoding="utf-8",
    )
...
def test_covers_naming_an_undeclared_pin_is_refused(tmp_path):
    reg = tmp_path / "p.yaml"
    reg.write_text(
        yaml.safe_dump({"version": 1, "pins": {"a": {"commit": "0" * 40, "covers": ["ghost"]}}}), encoding="utf-8"
    )
        ...
        yaml.safe_dump(
            {
                ...
                "pins": {
                    ...
                    "b": {"commit": "1" * 40, "covers": ["a"], "root_env": "NOPE_UNSET_ENV"},
                },
            }
        ),
        encoding="utf-8",
...
def test_content_check_must_be_a_boolean_or_absent(tmp_path):
    ...
    reg = tmp_path / "p.yaml"
    reg.write_text(
        yaml.safe_dump({"version": 1, "pins": {"a": {"commit": "0" * 40, "content_check": "yes"}}}), encoding="utf-8"
    )
...
def test_the_live_registry_loads_and_its_containment_resolves():
    ...
    for name, p in pins.items():
        if p.nested_in:
            ...
            assert p.checks_content, f"{name} is nested; content checking is what catches an off-gitlink header"
...
def test_every_external_isa_source_is_in_some_pins_read_set():
    ...
    assert not missing, (
        "ISA source(s) a dtype claim is derived from that resolve OUTSIDE this repo and "
