...
def test_resume_passes_only_flags_the_subcommand_accepts():
    ...
    cmd = CA.build_resume_cmd(
        Path("/ws"), model="m", effort="high", final_path=Path("/ws/f.txt"), sandbox="bwrap", thread_id="TID"
    )
...
def test_the_session_id_precedes_the_stdin_prompt_marker():
    ...
    cmd = CA.build_resume_cmd(
        Path("/ws"), model="m", effort="", final_path=Path("/f"), sandbox="bwrap", thread_id="TID"
    )
    assert cmd[-2:] == ["TID", "-"]
...
def test_continuation_is_off_unless_the_schedule_is_continuous():
    ...
    loop = module_source_path("merlin_experiments.phase1.authoring").read_text()
...
def test_every_stop_reason_is_named():
    ...
    src = module_source_path("merlin_experiments.phase1.providers.codex_agent").read_text()
    for reason in (
        "continuation not enabled",
        "no thread id to resume",
        "wall budget spent",
