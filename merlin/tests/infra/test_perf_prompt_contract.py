    ...
    return PP.PerfPromptInputs(
        ...
        families=(
            PP.PerfFamily(
                ...
                "fixed_extents",
                "residual bound",
                "identical emitted work; unpriced demands must cancel",
                ("K",),
            ),
        ),
        host_lane=PP.HostLaneGrant(
            "rvv", "rvv_int8", host, host_digest, host_manifest, "compile_model(package=<pinned package>)"
        ),
        tools=(
            PP.ToolGrant(
                "perf-selfcheck", "python3 perf_broker.py request selfcheck", "redacted own-artifact QA", True
            ),
            PP.ToolGrant(
                "cca-contract", "python3 perf_broker.py request cca-contract", "enumerate compiler levers", True
            ),
            PP.ToolGrant("rtl-facts", "python3 perf_broker.py request rtl-facts", "derive target facts", True),
            PP.ToolGrant(
                "gsim", "python3 perf_broker.py request probe-gsim", "harness-owned elaborated-RTL L3 simulator"
            ),
        ),
        allowed_paths=(
            frozen,
...
def test_prompt_is_deterministic_and_carries_every_campaign_boundary() -> None:
    ...
    required = (
        ...
        "undeclared files",
        "Do not\n  `cp`, `install`, or `mv` candidate bytes to `/tmp`",
        "transcript audit rejects the\n  copy-out itself",
        inputs.host_lane.package_id,
        inputs.host_lane.package_sha256,
        "hidden host lowering",
        "`kernel_abi.kind = whole_program`",
        "Host-only operands are never `RES_PACK`",
        "runner was frozen before your authoring began",
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (lambda value: replace(value, direct_candidate_execution_forbidden=False), "requires broker-only bwrap"),
        ...
        (
            lambda value: replace(value, tools=tuple(replace(tool, required=False) for tool in value.tools)),
            "must be enforced",
        ),
        (
            lambda value: replace(
                value, allowed_paths=tuple(path for path in value.allowed_paths if path != value.host_lane.package_path)
            ),
            "omit launch inputs",
        ),
    ),
...
def test_completion_identity_requires_exact_l2_l3_pairs_without_duplicates() -> None:
    ...
    without_l3 = tuple(
        cell
        for cell in inputs.expected_cells
        if not (cell.capsule == "pk_k2" and cell.replicate == "r000" and cell.simulator == "gsim")
    )
    ...
    with pytest.raises(PP.PerfPromptContractError, match="repeats a cell"):
        PP.render_initial_prompt(replace(inputs, expected_cells=inputs.expected_cells + (inputs.expected_cells[0],)))
    uneven = tuple(cell for cell in inputs.expected_cells if not (cell.capsule == "pk_k2" and cell.replicate == "r001"))
...
def test_family_and_tool_contracts_cannot_drift_from_the_cells() -> None:
    ...
    with pytest.raises(PP.PerfPromptContractError, match="bypass"):
        PP.render_initial_prompt(
            replace(
                inputs,
                tools=(replace(inputs.tools[0], command="python3 submission/compiler.py"),) + inputs.tools[1:],
            )
        )
...
def test_prompt_pins_queue_owned_firesim_lifecycle() -> None:
    prompt = PP.render_initial_prompt(_inputs())
    assert "Full-size FireSim is optional post-freeze validation" in prompt
    assert "not a Phase-2 prerequisite" in prompt
    assert "atomic `runworkload-full`" in prompt
    assert ("`firesim kill` ->\n   `firesim infrasetup` -> `firesim runworkload` -> `firesim kill`") in prompt
...
def test_the_prompt_documents_every_summary_field_the_feedback_actually_carries() -> None:
    ...
    for field in (
        "summary.recoverable",
        "recoverable_cycles",
        "share_of_corpus_cycles",
        "recoverable_share_of_corpus",
        "total_recoverable_share",
    ):
