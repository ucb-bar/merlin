...
PAS = G.PAS
SHA = {name: PAS._sha256(name.encode()) for name in ("target", "graph", "plan")}
...
def _revise(experiment, candidate, revision, hypothesis):
    ...
    return experiment.analyze(candidate, hypothesis=hypothesis)
...
def test_without_a_gate_every_iteration_says_so_and_nothing_is_excluded(tmp_path, monkeypatch):
    ...
    record = experiment.analyze(candidate, hypothesis="seed")
...
def test_failed_gate_excludes_the_cheapest_revision_and_records_why(tmp_path, monkeypatch):
    ...
    assert G.GlobalPerfExperiment.authored_host_cost(win["analysis"]) == (500, 1)
    ...
    with pytest.raises(ValueError):
        experiment.seal(candidate, name="should_not_seal")
