...
def _request(ch: Path, rid: str, *, mtime: float, timeout: int = 1800, extra: dict | None = None) -> Path:
...
def test_dead_requester_is_not_replayed_even_when_suite_deadline_is_still_live(tmp_path):
    ...
    abandoned = _request(
        ...
        extra={
            "protocol": 3,
            "requester": {"pid": 999999, "start_ticks": 1, "pid_namespace": broker._pid_namespace()},
    ...
    old_v3 = _request(
        ...
        mtime=103,
        timeout=1800,
        extra={"protocol": 3, "request_id": "999998_456_nonce", "deadline_unix_ns": 10_000_000_000_000},
    )
...
def test_full_suite_channel_budget_cannot_expire_before_per_capsule_work(tmp_path):
    ...
    budget = shim._request_budget_seconds(per_capsule, capsules="all", workers=workers, suite_size=n_capsules)
...
def test_protocol_v3_binds_a_verdict_to_the_graded_submission(tmp_path):
    ...
    doc = broker._decorate_response('{"all_pass": true}', req, "req-1", digest, started_ns=20, completed_ns=30)
...
def test_feedback_health_distinguishes_completed_expired_and_stranded(tmp_path):
    ...
    health = FL.channel_health(tmp_path, now=500)
