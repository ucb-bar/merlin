...
ADD_ARGS = [
    "--arg",
    "x_ptr=*fp32:1025:read",
    "--arg",
    "y_ptr=*fp32:1025:read",
    "--arg",
    "out_ptr=*fp32:1025:write",
    "--arg",
    "n_elements=i32",
    "--assume",
    "n_elements=1025",
...
def test_route_only_reports_without_writing(capsys, tmp_path):
    assert cli.main([VECTOR_ADD, "--target", "saturn", *ADD_ARGS, "--route-only", "--out", str(tmp_path)]) == 0
...
def test_the_vector_add_example_compiles_and_reports(tmp_path):
    ...
    assert (
        cli.main(
            [VECTOR_ADD, "--target", "saturn", *ADD_ARGS, "--emit", "ttir,core-mlir,report", "--out", str(tmp_path)]
        )
        == 0
    )
