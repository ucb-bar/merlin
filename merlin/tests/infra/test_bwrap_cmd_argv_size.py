...
def _compose_source() -> str:
    ...
    return inspect.getsource(execution.sandbox_command)
...
@pytest.mark.parametrize("location", ["argument", "payload"])
def test_composer_refuses_nul_without_creating_a_policy_file(tmp_path, large, location):
    from merlin.targetgen.sandbox import bwrap as BW

    value = "x" * (MAX_ARG_BYTES + 1) if large else "value"
    argv = ["bwrap", "--setenv", "FIXTURE", value]
    payload = " bash -c 'true'"
    ...
    with pytest.raises(ValueError, match="NUL"):
        BW.compose_command(argv, payload, tmp_path / "workspace")
    assert list(tmp_path.iterdir()) == []


def test_composer_refuses_non_bwrap_spill_without_creating_a_policy_file(tmp_path):
    from merlin.targetgen.sandbox import bwrap as BW

    with pytest.raises(ValueError, match="bwrap"):
        BW.compose_command(["printf", "%s", "x" * (MAX_ARG_BYTES + 1)], "", tmp_path / "workspace")
    assert list(tmp_path.iterdir()) == []
...
def test_perf_codex_round_composes_large_policy_through_args_file(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(stage.AW, "outer_codex_policy", lambda *args, **kwargs: policy)
    transcript = tmp_path / "transcript.jsonl"

    def run_round(ws, *args, **kwargs):
        command = kwargs["sandbox_command"]("printf '%s' 'payload'", ws, {})
    ...
    monkeypatch.setattr(stage, "_import_codex_driver", lambda: SimpleNamespace(run_round=run_round))
    result = stage._codex_round(
        ...
        None,
        None,
        sandbox_inputs=stage.PC.PackageSandboxInputs(
