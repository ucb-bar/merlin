...
REPORT_MODULES = {
    "agg_ab_results.py": "merlin.agentreport.phase1.by_treatment",
    "agg_agentic_results.py": "merlin.agentreport.phase1.runs",
    "agg_by_model.py": "merlin.agentreport.phase1.by_model",
}
BOOTSTRAP = f"import sys; sys.path[:0] = {[str(path) for path in python_import_roots()]!r};"
...
def test_no_module_resolves_runs_under_the_retired_experiment_root(rel):
    source = (
        module_source_path("merlin_experiments.phase1.telemetry.report")
        if rel == "timing_decomposition.py"
        else module_source_path(REPORT_MODULES[rel])
        if rel in REPORT_MODULES
        else HARNESS / rel
    )
...
def test_every_arm_bundle_variant_maps_back_to_its_arm():
    ...
    p = subprocess.run(
        [
            sys.executable,
            "-c",
            BOOTSTRAP + "from merlin.agentreport.phase1 import runs as A;"
...
def test_collect_finds_the_runs_that_exist_on_disk():
    ...
    p = subprocess.run(
        [
            sys.executable,
            "-c",
            BOOTSTRAP + "import _common as C; from merlin.agentreport.phase1 import by_treatment as AB;"
            "from merlin.agentreport.phase1.runs import ReportInputs;"
            "n=sum(len(v) for v in AB.collect(None,inputs=ReportInputs(C.TARGET,C.RUNS,C.REPORTS)).values());"
...
def test_scores_are_ranked_by_count_not_by_string():
    ...
    """
    from merlin.agentreport.phase1 import by_model as mod
