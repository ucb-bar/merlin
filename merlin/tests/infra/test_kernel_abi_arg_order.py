...
def test_every_contract_shape_and_token_resolves_without_a_provider():
    problems = _gate().check(static=True)
...
def test_resident_matmul_order_is_group_major_not_command_order():
    ...
    command_order = [c["operands"]["lhs"] for c in cb["commands"] if c["opcode"] in ("MATMUL", "MATMUL_RESIDENT")]
    assert lhs == ["A0", "A2", "A1"], lhs
    assert lhs != command_order, (
        "the probe no longer distinguishes group-major from command order, "
...
def test_whole_op_order_follows_declaration_order_not_role_order():
    ...
    assert gate.resolve_token("interface_external_tensors_in_declaration_order", cb) == ["IFM", "W", "Y0"]
    swapped = {
        "tensors": {"W": cb["tensors"]["W"], "IFM": cb["tensors"]["IFM"], "Y0": cb["tensors"]["Y0"]},
        "commands": cb["commands"],
    }
    assert gate.resolve_token("interface_external_tensors_in_declaration_order", swapped) == ["W", "IFM", "Y0"]


def test_whole_program_order_follows_its_explicit_pointer_boundary():
    gate = _gate()
    cb = gate._probe_whole_program()
    assert gate.resolve_token("declared_whole_program_args", cb) == ["scale", "W", "A0", "mid", "Y0"]
...
def test_gate_catches_a_harness_that_reorders_a_declared_whole_program_boundary():
    gate = _gate()
    target = "selected_test_provider"
    abi = (yaml.safe_load(CONTRACT.read_text(encoding="utf-8")) or {})["kernel_abi"]
    whole_op = next(r for r in abi["arg_order_by_command_shape"] if r["shape"] == "native_whole_op")
    owner = types.SimpleNamespace(_NATIVE_INTERFACE_OPS=frozenset(whole_op["opcodes"]))

    def render(cb, *, target):
        names = [arg["tensor"] for arg in (cb.get("kernel_abi") or {}).get("args", [])][::-1]
...
def test_gate_fails_closed_when_the_whole_op_roster_cannot_be_read():
    ...
    gate = _gate()
    target = "selected_test_provider"
    render = lambda _cb, *, target: f"{target}_kernel();"  # noqa: E731
