...
``merlin/tests/conftest.py`` fixes it by putting the checkout's core and extension roots on ``sys.path``
...
def test_the_imported_library_comes_from_this_checkout():
    ...
    assert got.is_relative_to(here), (
        ...
        f"merlin/tests/conftest.py must put this checkout's source roots on sys.path first."
    )
...
def test_optional_implementations_come_from_this_checkout():
    """Shared namespace packages must not silently use a sibling editable install."""
    import importlib.util

    here = Path(__file__).resolve().parents[3]
    owners = {
        "merlin_experiments": "merlin-experiments",
        "merlin.dse": "merlin-dse",
        "merlin.mining.runner": "merlin-mining",
        "merlin.compare": "merlin-analysis",
    }
    for module, distribution in owners.items():
        spec = importlib.util.find_spec(module)
        assert spec is not None and spec.origin is not None, module
        assert Path(spec.origin).resolve().is_relative_to(here / "packages" / distribution), module
