...
def _grade(tmp_path, monkeypatch, cb, *, run_id: str = "wv") -> dict:
    ...
    monkeypatch.setattr(PROV, "toolchain_shas", lambda _target: {"synthetic": "a" * 64})
...
def test_the_readers_read_the_keys_the_producer_writes(tmp_path, monkeypatch):
    ...
    paired = module_source_path("merlin_experiments.phase2.paired_measurement").read_text(encoding="utf-8")
    for key in ("work_volume", "command_buffer_artifact"):
...
def test_the_receipt_is_the_bytes_the_total_was_counted_from(tmp_path, monkeypatch):
    ...
    assert MS.canonical_sha256(embedded) == work["artifact_sha256"]
...
def test_the_reader_now_names_a_compute_axis(tmp_path, monkeypatch):
    ...
    grade = _grade(tmp_path, monkeypatch, _countable_buffer())
    bindings = MS.resource_bindings(_measurement(grade))
...
def test_the_measurement_identity_no_longer_refuses_the_program(tmp_path, monkeypatch):
    ...
    grade = _grade(tmp_path, monkeypatch, _countable_buffer())
    _, refusals = MS.measurement_identity(
        package_before="a" * 64,
        package_after="a" * 64,
        inputs_before="b" * 64,
        inputs_after="b" * 64,
...
def test_the_reader_refuses_a_compute_axis_for_an_uncountable_member(tmp_path, monkeypatch):
    ...
    assert "compute" not in MS.resource_bindings(_measurement(grade))
...
def test_the_compute_axis_disappears_when_the_evidence_is_corrupted(tmp_path, monkeypatch, corrupt):
    ...
    assert "compute" in MS.resource_bindings(row), "the unmutated row does not bind -- vacuous"
    corrupt(row)
    assert "compute" not in MS.resource_bindings(row)
...
def test_coverage_counts_every_member_and_says_which_carry_no_axis(tmp_path, monkeypatch):
    ...
    coverage = MS.compute_axis_coverage(cells)
...
def test_an_unattributed_member_is_named_with_the_counter_s_own_reason(tmp_path, monkeypatch):
    ...
    coverage = MS.compute_axis_coverage(_cells(tmp_path, monkeypatch, _uncountable_buffer()))
...
def test_a_receiptless_member_is_named_as_such_and_not_confused_with_a_refusal(tmp_path, monkeypatch):
    ...
    coverage = MS.compute_axis_coverage([{"kernel": "legacy", "work_volume": {}, "resource_bindings": {}}])
...
def test_a_fully_attributed_campaign_says_so_positively(tmp_path, monkeypatch):
    ...
    coverage = MS.compute_axis_coverage(_cells(tmp_path, monkeypatch, _countable_buffer()))
...
def test_an_empty_campaign_does_not_claim_coverage_it_does_not_have():
    coverage = MS.compute_axis_coverage([])
...
def test_both_benches_record_the_coverage_and_neither_gates_on_it():
    ...
    paired = module_source_path("merlin_experiments.phase2.paired_cli").read_text(encoding="utf-8")
    assert 'campaign["compute_axis_coverage"] = coverage' in fixed
    assert 'manifest["compute_axis_coverage"] = MS.compute_axis_coverage(' in paired
