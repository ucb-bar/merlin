...
def test_absent_fanout_declaration_means_the_serial_campaign() -> None:
    assert CTRL.declared_fanout({}) == 1
    assert CTRL.declared_fanout({CTRL.FANOUT_ENVIRONMENT_VARIABLE: ""}) == 1
    assert CTRL.declared_fanout({CTRL.FANOUT_ENVIRONMENT_VARIABLE: "   "}) == 1


def test_declared_fanout_is_read_from_the_environment(monkeypatch: pytest.MonkeyPatch) -> None:
    assert CTRL.declared_fanout({CTRL.FANOUT_ENVIRONMENT_VARIABLE: "3"}) == 3
    assert CTRL.declared_fanout({CTRL.FANOUT_ENVIRONMENT_VARIABLE: " 6 "}) == 6
    monkeypatch.setenv(CTRL.FANOUT_ENVIRONMENT_VARIABLE, "4")
    assert CTRL.declared_fanout() == 4
    monkeypatch.delenv(CTRL.FANOUT_ENVIRONMENT_VARIABLE)
    assert CTRL.declared_fanout() == 1
...
def test_an_unparseable_fanout_is_refused_rather_than_guessed(value: str) -> None:
    with pytest.raises(AD.ExperimentError, match="must be a positive integer"):
        CTRL.declared_fanout({CTRL.FANOUT_ENVIRONMENT_VARIABLE: value})
...
def test_commits_run_on_the_calling_thread_in_declared_order_while_launches_overlap() -> None:
    ...
    stages = [
        CTRL.ChildStage(name, lambda name=name: launch(name), lambda name=name: commit(name))
        for name in ("c", "a", "b")
    ]
    assert CTRL.run_child_stages(stages, workers=3) == ["c", "a", "b"]
...
def test_a_single_declared_worker_keeps_every_launch_on_the_calling_thread() -> None:
    ...
    CTRL.run_child_stages(stages, workers=1)
...
def test_an_adopted_stage_is_committed_without_being_launched() -> None:
    launched: list[str] = []
    stages = [
        CTRL.ChildStage("adopted", None, lambda: "adopted"),
        CTRL.ChildStage("fresh", lambda: launched.append("fresh"), lambda: "fresh"),
    ]
    assert CTRL.run_child_stages(stages, workers=2) == ["adopted", "fresh"]
...
def test_a_concurrent_failure_names_every_failure_and_commits_nothing() -> None:
    ...
    def boom(name: str) -> None:
        raise AD.ExperimentError(f"command failed (2): {name}")

    stages = [
        CTRL.ChildStage("candidate:trial_00", lambda: boom("trial_00"), lambda: committed.append("trial_00")),
        CTRL.ChildStage("candidate:trial_01", lambda: None, lambda: committed.append("trial_01")),
        CTRL.ChildStage("candidate:trial_02", lambda: boom("trial_02"), lambda: committed.append("trial_02")),
    ]
    with pytest.raises(AD.ExperimentError) as raised:
        CTRL.run_child_stages(stages, workers=3)
...
def test_serial_mode_still_stops_at_the_first_failure() -> None:
    ...
    def launch(name: str) -> None:
        ...
        if name == "a":
            raise AD.ExperimentError("command failed (1): a")

    stages = [CTRL.ChildStage(name, lambda name=name: launch(name), lambda: None) for name in ("a", "b")]
    with pytest.raises(AD.ExperimentError, match="command failed"):
        CTRL.run_child_stages(stages, workers=1)
...
def test_a_non_positive_worker_count_is_refused() -> None:
    with pytest.raises(AD.ExperimentError, match="at least one"):
        CTRL.run_child_stages([], workers=0)
...
def _context(root: Path) -> AD.ExecutionContext:
    source = root / "source"
    return AD.ExecutionContext(
        source_root=source,
        contract_root=source / "merlin/contract",
        functional_runs_root=root / "functional-runs",
        stage_root=root / "runs" / "agent_stages",
        measurement_root=root / "perf_runs",
        holdout_sources=HOLDOUT.HoldoutSourceContext(
            source,
            source / "experiments/catalog.yaml",
            source / "src/merlin",
            source / "packages/merlin-experiments/src/merlin_experiments",
            source / "packages/merlin-experiments/src/merlin",
        ),
        chia_wrapper=source / "chia_wrapper.py",
        invocation=("python", "synthetic-controller"),
        suite="fixture",
    )
...
def _declaration() -> dict[str, Any]:
    return {
        "trial_contracts": {trial: {"model": "gpt-model", "trial": trial} for trial in AD.TRIALS},
...
def _certificate(root: Path, name: str, digest: str):
    ...
    return SimpleNamespace(
        ...
        path=root / name,
        sha256=digest,
        pins={"gsim_binary": {"path": str(binary), "sha256": AD._sha_file(binary)}},
    )
...
class _Recorder:
    ...
    def __call__(self, argv, cwd, environment):
        args = list(argv)
        run_id = (
            args[args.index("--run-id") + 1] if "--run-id" in args else Path(args[args.index("--stage-root") + 1]).name
        )
        ...
        if run_id in self.failures:
            return AD.CommandResult(3, "", f"child {run_id} refused")
        self.write(run_id)
...
def _author(root: Path, monkeypatch: pytest.MonkeyPatch, *, workers: int, barrier: threading.Barrier | None):
    ...
    stage_root = config.context.stage_root
    telemetry_sha = AD._sha_bytes(AD._canonical(declaration["agent_telemetry"]))
    commit_threads: list[str] = []
    ...
    def handoff(path, _target):
        ...
        return SimpleNamespace(
            ...
            record_sha256=AD._sha_bytes(payload),
            candidate_sha256=AD._sha_bytes(payload + b"candidate"),
    ...
    monkeypatch.setattr(AD, "_verify_live_agent_treatment", lambda *_args: None)
    monkeypatch.setattr(CTRL, "_handoff", handoff)
    runner = _Recorder(barrier=barrier, write=write)
    state = AD.Checkpoints(root / "state", "f" * 64)
    handoffs, evidence = CTRL._author_candidates(
        config,
        state,
        target,
        declaration,
...
def test_concurrent_authoring_writes_the_same_record_as_the_serial_campaign(
    ...
    assert [row["stage"] for row in concurrent.state.load()] == [f"candidate:{trial}" for trial in AD.TRIALS]
    assert serial.evidence == concurrent.evidence
    assert list(concurrent.handoffs) == list(AD.TRIALS)
    ...
    assert serial.runner.order == [f"exp__{trial}" for trial in AD.TRIALS]
...
def test_authoring_adopts_a_checkpointed_trial_without_relaunching_it(
    ...
    assert [row["stage"] for row in resumed.state.load()] == [f"candidate:{trial}" for trial in AD.TRIALS]
...
def _measure(root: Path, monkeypatch: pytest.MonkeyPatch, *, workers: int, barrier: threading.Barrier | None):
    root.mkdir(parents=True, exist_ok=True)
    # This fixture qualifies scheduling/checkpoint identity, not provider setup.
    monkeypatch.setattr(AD, "child_environment", lambda *_: {})
    config = _config(root)
    runs = root / "perf_runs"
    commit_threads: list[str] = []
    ...
    def verify(path, **_kwargs):
        commit_threads.append(threading.current_thread().name)
        return {"path": str(Path(path).resolve()), "sha256": AD._sha_file(Path(path))}
    ...
    handoffs = {
        ...
        for index, trial in enumerate(AD.TRIALS)
    }
    revealed = {
    ...
    monkeypatch.setattr(CTRL, "_verify_measurement_manifest", verify)
    runner = _Recorder(barrier=barrier, write=write)
    state = AD.Checkpoints(root / "state", "f" * 64)
    ...
    manifests = CTRL._measure_cells(cells, config, state, command_runner=runner, workers=workers)
...
def test_the_paired_matrix_is_every_trial_by_every_phase_in_fixed_order(
    ...
    assert [(cell.trial, cell.phase) for cell in result.cells] == [
        (trial, phase) for trial in AD.TRIALS for phase in ("tuning", "held_out")
    ]
...
def test_concurrent_measurement_writes_the_same_record_as_the_serial_campaign(
    ...
    assert [row["stage"] for row in concurrent.state.load()] == [
        f"measurement:{trial}:{phase}" for trial in AD.TRIALS for phase in ("tuning", "held_out")
    ]
    ...
    assert serial.runner.threads == {threading.current_thread().name}
    assert serial.runner.order == [f"exp__{trial}__{phase}" for trial in AD.TRIALS for phase in ("tuning", "held_out")]
    assert set(concurrent.commit_threads) == {threading.current_thread().name}
