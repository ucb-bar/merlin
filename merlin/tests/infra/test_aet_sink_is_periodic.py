...
LOOP = module_source_path("merlin_experiments.phase1.authoring")
...
def test_in_turn_grader_offers_a_tick_hook() -> None:
    ...
    fn = _func(ast.parse(Path(lifecycle.__file__).read_text()), "start_background")
...
def test_the_continuous_call_site_passes_the_sink() -> None:
    ...
    for node in ast.walk(tree):
        if not (
            isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr == "start_background"
        ):
            continue
