...
def _plain_ws(tmp_path, name="ws"):
    ...
    return _ws(
        tmp_path,
        {
            "manifest.yaml": "artifact_type: mlir_oot_target_backend\ntarget: t\n",
            "mlir_oot/opt": "#!/usr/bin/env python3\n",
        },
        name=name,
    )
...
def _certify_x(B, ws, ch, verdict, cover=None):
    ...
    promoted = B.promote(ws, ch, verdict, "L2", "L3", cover, sys.stderr, context=feedback_context())
    for capsule in promoted:
        B.record_cert(
            ws,
            {"per_capsule": [{"capsule": capsule, "pass": True, "execution_digest": by_name[capsule]}]},
            "L3",
            sys.stderr,
...
def _certify(B, ws, ch, verdict, cover=None):
    ...
    for capsule in promoted:
        B.record_cert(
            ws,
            {"per_capsule": [{"capsule": capsule, "pass": True}]},
            "L3",
            sys.stderr,
            identity=_identity_of(ch, capsule),
        )
...
def test_an_edit_that_leaves_a_capsules_program_alone_keeps_its_certificate(tmp_path):
    ...
    assert B.promote(ws, ch, after, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["matmul_tile"], (
...
def test_a_changed_program_invalidates_that_capsules_certificate(tmp_path):
    ...
    ws = _ws(tmp_path)
    ch = ws / ".qa_channel"
    assert _certify_x(B, ws, ch, _vx([("matmul_tile", True, "a"), ("conv_codegen", True, "b")])) == sorted(CORPUS)

    (ws / "submission" / "mlir_oot" / "codegen.py").write_text("cg v2\n")
    after = _vx([("matmul_tile", True, "a"), ("conv_codegen", True, "d")])
    assert B.promote(ws, ch, after, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["conv_codegen"]
...
def test_a_capsule_that_reports_no_program_identity_is_never_spared(tmp_path):
    ...
    assert B.promote(
        ws, ch, _v([(n, True) for n in CORPUS]), "L2", "L3", None, sys.stderr, context=feedback_context()
    ) == sorted(CORPUS), "a row with no execution identity was treated as evidence the program had not changed"
...
def test_the_log_names_the_component_that_invalidated_the_capsule(tmp_path, capsys):
    ...
    capsys.readouterr()
    (ws / "submission" / "mlir_oot" / "lowering" / "tile.py").write_text("tile v2\n")
    B.promote(ws, ch, v, "L2", "L3", None, sys.stderr, context=feedback_context())
...
def test_bytes_no_command_can_read_invalidate_nothing(tmp_path):
    ...
    assert B.promote(ws, ch, v, "L2", "L3", None, sys.stderr, context=feedback_context()) == [], (
...
def test_no_narrower_identity_falls_back_to_the_whole_submission(tmp_path, capsys):
    ...
    more narrowly, so the conservative rule stands. Under-invalidating here would credit an RTL
    certification to bytes that did not earn it, which is strictly worse than re-running."""
    B = _mod()
    ...
    assert B.promote(ws, ch, v, "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
    err = capsys.readouterr().err
...
def test_the_fallback_reason_is_recorded_on_disk_too(tmp_path):
    ...
    B = _mod()
    ws = _plain_ws(tmp_path)
    B.promote(ws, ws / ".qa_channel", _v([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
...
def test_a_passing_promotion_is_recorded_as_certified_not_left_pending(tmp_path):
    ...
    ws = _plain_ws(tmp_path)
    ch = ws / ".qa_channel"
    assert B.promote(ws, ch, _vx([("A", True, "a")]), "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
    ...
    assert B.promote(ws, ch, _vx([("A", True, "b")]), "L2", "L3", None, sys.stderr, context=feedback_context()) == ["A"]
...
def test_a_recorded_certificate_is_reused_when_its_bytes_come_back(tmp_path):
    ...
    ws = _plain_ws(tmp_path)
    ch = ws / ".qa_channel"
    B.promote(ws, ch, _vx([("A", True, "a")]), "L2", "L3", None, sys.stderr, context=feedback_context())
    first = _identity_of(ch, "A")
    (ws / "submission" / "mlir_oot" / "opt").write_text("# edited\n")
    B.promote(ws, ch, _vx([("A", True, "b")]), "L2", "L3", None, sys.stderr, context=feedback_context())
    B.record_cert(ws, _vx([("A", True, "a")]), "L3", sys.stderr, identity=first)

    assert B.promote(ws, ch, _vx([("A", True, "a")]), "L2", "L3", None, sys.stderr, context=feedback_context()) == [], (
...
def test_a_promotion_without_an_artifact_identity_is_still_recorded(tmp_path):
    ...
    ws = _plain_ws(tmp_path)
    ch = ws / ".qa_channel"
    B.promote(ws, ch, _v([("A", True)]), "L2", "L3", None, sys.stderr, context=feedback_context())
...
def test_a_late_result_does_not_move_the_readable_record_onto_its_own_bytes(tmp_path):
    ...
    ch = ws / ".qa_channel"
    B.promote(
        ws, ch, _vx([("A", True, "a")]), "L2", "L3", None, sys.stderr, context=feedback_context()
    )  # job 1 runs bytes "a"
    B.promote(
        ws, ch, _vx([("A", True, "b")]), "L2", "L3", None, sys.stderr, context=feedback_context()
    )  # edit -> job 2 runs "b"
...
@pytest.mark.parametrize(
    "ran",
    [
        "d",  # a well-formed identity for a DIFFERENT program
        "z",
    ],
)  # malformed: the reader could not identify what ran
def test_a_request_identity_never_overrides_the_artifact_the_job_actually_ran(tmp_path, ran):
    ...
    assert B.record_cert(ws, _vx([("A", True, ran)]), "L3", sys.stderr, identity=stale_request) == []
    assert B._tier_state(ws)["A"]["L3"]["status"] == "pending"
...
def test_the_ledger_is_bounded(tmp_path):
    ...
    for i in range(B._LEDGER_KEEP + 8):
        ...
        B.promote(ws, ch, _vx([("A", True, "abcdef"[i % 6])]), "L2", "L3", None, sys.stderr, context=feedback_context())
