...
TOOL = module_source_path("merlin_experiments.phase1.tools.await_verdict")
VERDICT = {
    "n_passed": 82,
    "n_capsules": 96,
    "all_pass": False,
...
def test_it_is_staged_into_the_workspace():
    ...
    from merlin.targetgen.tool_registry import COMMON_CLIENTS

    assert ("merlin_experiments.phase1.tools.await_verdict", "await_verdict.py") in COMMON_CLIENTS
    assert "for client_module, staged_as in _TR.COMMON_CLIENTS:" in inspect.getsource(lifecycle)
    assert "stage_client(ws, module_source_path(client_module), staged_as)" in inspect.getsource(lifecycle)
...
def test_the_agent_is_told_it_exists():
    ...
    from merlin_experiments.phase1 import task_staging

    loop = inspect.getsource(task_staging)
    i = loop.index("def task_runtime_scope_block")
    block = loop[i : i + 4000]
