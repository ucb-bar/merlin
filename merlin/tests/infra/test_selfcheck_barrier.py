...
def score(
    ...
    if bar is None:
        ...
        ran_required = [k for k, v in tiers.items() if k in required and v not in (None, "skipped", "unavailable")]
    ...
    screened = (
        (not certified)
        and blocked_by_selection
        and bar in ("pass", None)
...
def test_the_helper_matches_the_shipped_implementation():
    ...
    src = (feedback_source("agent_selfcheck")).read_text()
