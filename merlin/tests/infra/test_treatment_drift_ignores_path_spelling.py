...
def _record(ws, run_dir, bundle_dir, tools=TOOLS):
    return RI.treatment_snapshot_record(ws, run_dir, bundle_dir, tools)
...
def test_same_bytes_through_a_symlinked_pin_are_not_drift(tmp_path):
    ...
    observed = RI.verify_treatment_snapshot(expected, ws_via_pin, run_dir, bundle_dir, TOOLS)
...
def test_changed_served_bytes_still_fail_closed(tmp_path):
    ...
    with pytest.raises(RuntimeError) as err:
        RI.verify_treatment_snapshot(expected, ws, run_dir, bundle_dir, TOOLS)
...
def test_changed_tool_set_still_fails_closed(tmp_path):
    ...
    expected = _record(ws, run_dir, bundle_dir)
    with pytest.raises(RuntimeError) as err:
        RI.verify_treatment_snapshot(expected, ws, run_dir, bundle_dir, [*TOOLS, "rtl_facts"])
