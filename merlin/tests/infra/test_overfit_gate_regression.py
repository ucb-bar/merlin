...
def test_target_roster_includes_example_only_target_without_runtime_dependencies(tmp_path):
    roster = _load_gate("_target_roster")
    definition = tmp_path / "examples" / "new-device" / "experiment.yaml"
    definition.parent.mkdir(parents=True)
    definition.write_text("schema_version: 1\ntarget: example_only_device\nphases: {}\n")
    assert roster.target_names(tmp_path) == {"example_only_device"}
    assert roster.sources(tmp_path)["examples"] == {"example_only_device"}
