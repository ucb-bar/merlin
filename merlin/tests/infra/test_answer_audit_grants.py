...
def test_importing_the_root_package_is_a_self_scan_not_oracle_use(harness, tmp_path):
    ...
    res = _audit(harness, tmp_path, 'python3 -c "import merlin, os; print(merlin.__file__)"', _arm4_bundle(tmp_path))
    assert res["clean"] is True
    assert res["hits"] == []
...
def test_importing_a_denied_oracle_module_is_still_oracle_use(harness, tmp_path, stmt):
    body = stmt.format(denied=_DENIED_MOD, package=_PACKAGE, reference=_DENIED_MOD.rsplit(".", 1)[-1])
...
def test_a_bundle_denied_module_outside_the_registry_is_oracle_use(harness, tmp_path):
    ...
    res = _audit(harness, tmp_path, f'python3 -c "from {module_name_for(denied_extra)} import adapt"', bundle)
    assert res["clean"] is False
...
def test_a_bundle_without_grant_lists_still_flags_the_declared_oracle(harness, tmp_path):
    ...
    res = _audit(harness, tmp_path, f'python3 -c "import {_DENIED_MOD}"', str(tmp_path / "no_such_bundle"))
    assert res["clean"] is False
...
@pytest.mark.parametrize(
    ("path", "module"),
    [
        ("src/merlin/targetgen/program_oracle.py", "merlin.targetgen.program_oracle"),
        (
...
def test_relocated_oracle_cannot_be_laundered_by_a_grant(harness, tmp_path, path, module):
    bundle = _bundle(tmp_path, allowed=[path], denied=[])
    res = _audit(harness, tmp_path, f'python3 -c "import {module}"', bundle)
    assert res["clean"] is False
    assert [hit["kind"] for hit in res["hits"]] == ["oracle_use"]


def test_src_layout_keeps_the_granted_sibling_distinct_from_the_oracle(harness, tmp_path):
    bundle = _bundle(
        tmp_path,
        allowed=["src/merlin/runtime/commandbuffer.py"],
        denied=["src/merlin/runtime/reference.py"],
    )
    res = _audit(harness, tmp_path, 'python3 -c "from merlin.runtime import commandbuffer"', bundle)
    assert res["clean"] is True
    assert res["hits"] == []
...
def test_authored_python_importing_the_oracle_is_still_flagged(harness, tmp_path):
    ...
    tp = _transcript(
        ...
        tool="Write",
        extra_input={
            "file_path": "submission/selfgrade.py",
...
def test_authored_python_importing_a_granted_sibling_is_clean(harness, tmp_path):
    tp = _transcript(
        ...
        "",
        tool="Write",
        extra_input={"file_path": "submission/emit.py", "content": f"from {_GRANTED_MOD} import dataflow_operands\n"},
    )
    res = harness.audit_transcript(tp, arm="merlin_assisted", bundle=_arm4_bundle(tmp_path))
    assert res["clean"] is True
    assert res["hits"] == []
...
def test_an_oracle_CALL_is_still_flagged_without_any_import(harness, tmp_path):
    ...
    into scope."""
    res = _audit(harness, tmp_path, 'python3 -c "print(reference_outputs(cb))"', _arm4_bundle(tmp_path))
    assert res["clean"] is False
...
def test_grep_pattern_over_a_granted_file_is_not_a_path_read(harness, tmp_path):
    ...
    grader_stem = next(t for t in harness.grader_tokens if "/" not in t and "." not in t)
    ...
    for allowed_f in sorted(targets.glob("*/input_bundles/*/allowed_files.txt")):
        ...
        for line in allowed_f.read_text().splitlines():
            ...
            if not rel or rel.endswith("/"):
                continue
            if any(
                tok in rel for tok in __import__("merlin_experiments.phase1.audit", fromlist=[""])._MERLIN_TOOL_TOKENS
            ):
                out.append(
                    pytest.param(
                        str(allowed_f.parent), arm, rel, id=f"{allowed_f.parent.name}-{rel.rsplit('/', 1)[-1]}"
                    )
                )
    return out
...
def test_a_real_answer_file_read_is_still_a_violation(harness, tmp_path):
    ...
    res = _audit(
        harness,
        tmp_path,
        "cat isa/AT0_config_smoke/golden.yaml",
        _arm4_bundle(tmp_path),
        stdout="expected_out: [1.0, 2.0]",
    )
    assert res["clean"] is False
    assert any(h["kind"] == "path_read" for h in res["hits"])
...
def test_a_granted_directory_never_exempts_a_masked_golden_inside_it(harness, tmp_path):
    ...
    res = _audit(
        harness, tmp_path, "cat merlin/contract/capsules/isa/X/golden.yaml", bundle, stdout="expected_out: [7]"
    )
    assert res["clean"] is False
    assert any(h["kind"] == "path_read" for h in res["hits"])
        ...
        for g in [m for m in allowed if m]:
            for d in [m for m in denied if m]:
                if g != d and g.rsplit(".", 1)[0] == d.rsplit(".", 1)[0]:
                    out.append(
                        pytest.param(str(allowed_f.parent), g, d, id=f"{allowed_f.parent.name}-{g.rsplit('.', 1)[-1]}")
                    )
    return out


@pytest.mark.parametrize("bundle_dir, granted, denied", _bundles_with_a_split())
def test_shipped_bundles_split_grant_and_deny_in_one_package(harness, tmp_path, bundle_dir, granted, denied):
