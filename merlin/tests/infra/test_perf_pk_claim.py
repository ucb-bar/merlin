            ...
            rows += [
                ...
                },
                {
                    **common,
                    ...
                    "citable": True,
                    "cycles": 5 * k + 100,
                    "oracle": {"kind": oracle_kind, "derived_from_rtl": True, "fidelity": "elaborated_rtl"},
                    "timing_identity": {
                        "simulator": timing_simulator,
...
def test_reporting_recomputes_real_frozen_corpus_and_pk_decision(descriptors, tmp_path):
    """Synthetic rows test the report's real admission/analyzer path, not hardware."""
    ...
    path = merlin_dir() / "experiments/gemmini_perf_bench/scripts/perf_reporting.py"
    spec = importlib.util.spec_from_file_location("corpus_reporting_probe", path)
    ...
    for descriptor in descriptors:
        descriptor["source_role"] = "derived_sweep"
        directory = live / "_performance" / descriptor["name"]
        directory.mkdir(parents=True)
        (directory / "capsule.yaml").write_text(yaml.safe_dump(descriptor))
    (live / "MANIFEST.yaml").write_text(
        yaml.safe_dump(
            {
                "generated": [f"_performance/{d['name']}" for d in descriptors],
                "hand_authored": [],
    ...
    frozen = corpus.freeze_performance_corpus(corpus.discover_performance_corpus(target), tmp_path / "frozen")
    handoff = SimpleNamespace(
        corpus_root=frozen.root, corpus_manifest_sha256=frozen.manifest_sha256, corpus_sha256=frozen.capsules_sha256
    )
    rows = _rows(descriptors)
    preflight = PK.preflight_pk_claim(descriptors)
    decision = PK.analyze_pk_claim(descriptors, rows)
    assert preflight["status"] == "READY" and decision["status"] == "ESTABLISHED"
    campaign = {
        "experiment_mode": "formal_claim",
        "claim_preflight": preflight,
        "claim_decision": decision,
        "claim_status": "ESTABLISHED",
        "decision_boundary": {"promotion_status": "PROMOTED"},
    }
    reporting.validate_claim_decision(campaign, handoff, rows)
    wrong = copy.deepcopy(campaign)
    wrong["claim_decision"]["status"] = "REFUTED"
    with pytest.raises(ReportingGateError, match="decision differs"):
        reporting.validate_claim_decision(wrong, handoff, rows)
    member = frozen.capsules[0].source_dir / "capsule.yaml"
    member.chmod(0o644)
    member.write_text("corrupted: true\n")
    with pytest.raises(ReportingGateError, match="capsule bytes changed"):
        reporting.validate_claim_decision(campaign, handoff, rows)
...
def test_verilator_is_supported_only_when_frozen_contract_selects_it(descriptors):
    ...
    assert {row["simulator"] for row in preflight["expected_identities"]} == {"spike", "verilator"}
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda row: row["timing_identity"].__setitem__("simulator_binary_sha256", "not-a-digest"),
            "simulator_binary_sha256",
        ),
        (
            lambda row: row["timing_identity"].__setitem__("elaborated_firrtl_sha256", "d" * 64),
            "one exact elaborated FIRRTL",
        ),
        (lambda row: row["oracle"].__setitem__("kind", "rtl_verilator"), "selected elaborated-RTL oracle"),
    ],
)
def test_timing_identity_is_exact_and_bound_to_selected_gsim(descriptors, mutate, reason):
    rows = _rows(descriptors)
...
def test_exact_affine_positive_l3_evidence_establishes_pk(descriptors):
    ...
    assert result["evidence"] == {
        ...
        "program_elf_sha256_by_observation": [
            {"capsule": descriptor["name"], "replicate": replicate, "sha256": "c" * 64}
            for descriptor in descriptors
            for replicate in REPLICATE_IDS
        ],
        "arm": "arm4",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda ds: ds[0]["performance"]["acceptance"]["thresholds"].__setitem__("r_squared_min_inclusive", 0.9),
            "unsupported acceptance",
        ),
        (
            lambda ds: ds[0]["performance"]["acceptance"]["thresholds"].__setitem__(
                "slope_cycles_per_K_element_min_exclusive", False
            ),
            "unsupported acceptance",
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda rows: _row(rows, "PK00_k16", "spike").__setitem__("correct", False), "not a correct successful"),
        (lambda rows: _row(rows, "PK00_k16", "gsim").__setitem__("cycles", 0), "positive integer cycles"),
        (lambda rows: _row(rows, "PK00_k16", "gsim").__setitem__("cycles", True), "positive integer cycles"),
        (lambda rows: _row(rows, "PK00_k16", "gsim").__setitem__("tier", "L2"), "evidence semantics"),
        (lambda rows: _row(rows, "PK00_k16", "spike").__setitem__("cycles", 999999), "exposes citable cycles"),
        (lambda rows: _row(rows, "PK00_k16", "gsim").__setitem__("approach", "arm3"), "Arm4 L2/L3"),
        (
            lambda rows: _row(rows, "PK00_k16", "gsim", REPLICATE_IDS[-1])["identity"].__setitem__("replicate", "r003"),
            "undeclared identity",
        ),
    ],
)
def test_missing_duplicate_wrong_or_malformed_l2_l3_rows_are_refused(descriptors, mutate, reason):
    rows = _rows(descriptors)
    mutate(rows)
    result = PK.analyze_pk_claim(descriptors, rows)
    assert result["status"] == "REFUSED"
...
def test_rows_for_another_declared_family_are_never_consumed(descriptors):
    rows = _rows(descriptors)
    rows.append(
        {"identity": {"family": "PF", "capsule": "PF00", "simulator": "gsim", "replicate": "r000"}, "cycles": -999999}
    )
    result = PK.analyze_pk_claim(descriptors, rows)
    assert result["status"] == "ESTABLISHED"
