...
def test_preflight_records_fixed_cohort_and_exact_three_replicates(descriptors):
    ...
    assert len(result["expected_identities"]) == 24
...
def test_exact_affine_positive_l3_evidence_establishes_pk(descriptors):
    ...
    assert result["evidence"] == {
        ...
        "l3_positive_cycle_rows_consumed": 12,
...
def test_complete_non_affine_l3_evidence_refutes_instead_of_refusing(descriptors):
    ...
    for replicate in ("r000", "r001", "r002"):
        _row(rows, "PK02_k64", "verilator", replicate)["cycles"] += 300
...
def test_small_integer_cycle_phase_residual_stays_inside_predeclared_floor(descriptors):
    rows = _rows(descriptors)
    for replicate in ("r000", "r001", "r002"):
        _row(rows, "PK01_k32", "verilator", replicate)["cycles"] += 4
    result = PK.analyze_pk_claim(descriptors, rows)
    assert result["status"] == "ESTABLISHED"
...
@pytest.mark.parametrize(
    ("mutate", "reason"),
    [
        (lambda rows: rows.pop(), "exactly 24"),
        ...
        (lambda rows: _row(rows, "PK00_k16", "verilator").__setitem__("cycles", 0),
         "positive integer cycles"),
        (lambda rows: _row(rows, "PK00_k16", "verilator").__setitem__("cycles", True),
         "positive integer cycles"),
        (lambda rows: _row(rows, "PK00_k16", "verilator").__setitem__("tier", "L2"),
         "evidence semantics"),
        (lambda rows: _row(rows, "PK00_k16", "spike").__setitem__("cycles", 999999),
         "exposes citable cycles"),
        (lambda rows: _row(rows, "PK00_k16", "verilator").__setitem__("approach", "arm3"),
         "Arm4 L2/L3"),
        (lambda rows: _row(rows, "PK00_k16", "verilator", "r002")["identity"].__setitem__(
...
def test_rows_for_another_declared_family_are_never_consumed(descriptors):
    ...
    result = PK.analyze_pk_claim(descriptors, rows)
    assert result["status"] == "ESTABLISHED"
    assert result["fit"]["n_observations"] == 12
