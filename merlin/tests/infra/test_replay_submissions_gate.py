...
def test_an_argv_naming_a_missing_script_is_unactionable(tmp_path, rs):
    ...
    _pkg(
        ...
        "bad",
        _GOOD.replace(
            '"mlir_oot/tool.py", "{input_mlir}", "{output_json}"]',
            '"mlir_oot/absent.py", "{input_mlir}", "{output_json}"]',
            1,
        ),
    )
...
def test_an_unknown_placeholder_is_actionable_not_opaque(tmp_path, rs):
    ...
    src = tmp_path / "in.mlir"
    src.write_text("module {}")
    _pkg(tmp_path, "ph", _GOOD.replace('"{input_mlir}", "{output_json}"]', '"{input_json}", "{output_json}"]', 1))
