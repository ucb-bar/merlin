...
#: Every fixture below is ONE program. The cross-identity refusals live in
#: merlin/tests/infra/test_program_identity.py; here the identity is held constant so these
#: tests keep measuring what they were written to measure.
IDENTITY = "static_quantized_recapture_entry"
...
_REFERENCES = {
    "model_fast": {
        ...
        "measured": {
            "whole_model_cycles": 1_000_000,
            "host_share_lower_bound": 0.93,
...
def _buffer(*, commands=10, mesh=4, host=6, prepack=4, args=20, opcodes=("PACK",) * 4 + ("CONV2D",) * 5 + ("COMMIT",)):
    return {
        "commands": [{"opcode": opcode} for opcode in opcodes][:commands],
        "kernel_abi": {"args": [{"tensor": f"t{i}"} for i in range(args)]},
        "params": {
            "mesh_regions": [{}] * mesh,
            "host_lane_regions": [{}] * host,
            "weight_prepack_recipes": [{}] * prepack,
        },
    }


def test_the_tracked_ledger_parses_and_selects_a_measured_destination() -> None:
    """The shipped ledger is the destination; a malformed one silently removes it.

    Deliberately NOT pinned to one entry's cycle count. It used to assert q535's 1,316,619,699 and
    that broke the moment a faster measured point was added, which is the ledger working correctly
    -- a test that fails when the destination improves is a test that discourages improving it. So
    this asserts the SELECTION is well-formed, and the structure assertions below name the entry
    they are about.
    """
    references = load_references()
    name, reference = find_reference(
        "resnet50", DESIGN, references=references, device=DEVICE, program_identity=IDENTITY
    )
    assert isinstance(reference["measured"]["whole_model_cycles"], int)
    assert reference["device"] == DEVICE and reference["program_identity"] == IDENTITY
    best = min(
        cycles
        for entry in references.values()
        if entry.get("model") == "resnet50"
        and entry.get("status") == "achieved"
        and entry.get("device") == DEVICE
        and entry.get("program_identity") == IDENTITY
        for cycles in [(entry.get("measured") or {}).get("whole_model_cycles")]
        if isinstance(cycles, int)
    )
    assert reference["measured"]["whole_model_cycles"] == best, "the selection must be the fewest cycles"
    raw = yaml.safe_load((merlin_dir() / REFERENCE_LEDGER_NAME).read_text())
    assert raw["references"][name]["design"] == DESIGN


def test_the_entry_carrying_the_emitted_structure_is_intact() -> None:
    """The structural census is what lets a candidate be compared without a simulator, and exactly
    one entry carries it. It is named here rather than found, so losing it is a failure and not a
    silently skipped comparison."""
    reference = load_references()["resnet50_w8a8_firesim_q535"]
    assert reference["emitted_structure"]["commands"] == 109
    assert reference["emitted_structure"]["opcodes"]["CONV2D"] == 53
    assert reference["emitted_structure"]["mesh_regions"] == 54
...
def test_a_matching_candidate_reports_every_gap_as_zero() -> None:
    gap = structural_gap(_buffer(), _REFERENCES["model_fast"], program_identity=IDENTITY)
...
def test_gaps_are_signed_and_name_the_field() -> None:
    ...
    gap = structural_gap(_buffer(mesh=0, host=20), _REFERENCES["model_fast"], program_identity=IDENTITY)
...
def test_a_missing_opcode_class_is_a_named_gap() -> None:
    gap = structural_gap(
        _buffer(opcodes=("PACK",) * 4 + ("COMMIT",)), _REFERENCES["model_fast"], program_identity=IDENTITY
    )
...
def test_an_opcode_the_reference_never_emits_is_also_reported() -> None:
    gap = structural_gap(_buffer(opcodes=("MYSTERY",) * 10), _REFERENCES["model_fast"], program_identity=IDENTITY)
...
def test_a_reference_without_structure_yields_no_verdict_rather_than_a_pass() -> None:
    gap = structural_gap(_buffer(), _REFERENCES["model_slow"], program_identity=IDENTITY)
...
def test_a_relative_cycle_estimate_is_anchored_on_the_measurement() -> None:
    ...
    estimate = estimate_cycles(
        500, _REFERENCES["model_fast"], program_identity=IDENTITY, baseline_host_dynamic_operations=1000
    )
...
def test_an_equal_op_count_estimates_the_reference_exactly() -> None:
    estimate = estimate_cycles(
        1000, _REFERENCES["model_fast"], program_identity=IDENTITY, baseline_host_dynamic_operations=1000
    )
...
def test_a_reference_with_no_anchor_says_so_instead_of_guessing() -> None:
    assert estimate_cycles(10, _REFERENCES["model_slow"], program_identity=IDENTITY)["status"] == (
...
def test_every_ledger_capsule_names_an_existing_capsule() -> None:
    ...
    missing = {
        name: [c for c in (entry.get("capsules") or ()) if c not in existing] for name, entry in references.items()
    }
    ...
    for model in {references[n]["model"] for n in claims}:
        for design in {references[n]["design"] for n in claims if references[n]["model"] == model}:
            try:
                ...
                # Nothing MEASURED describes this model on this design -- which is the state a
                # claim whose `design: UNRECORDED` is in. Refusing is the correct answer and it is
                # also "the claim was not selected", so it satisfies what this test is about. The
                # raise must not be swallowed into a pass for the OTHER reason, though, so the
                # by-capsule half below still runs.
                continue
