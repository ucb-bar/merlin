...
def test_stream_stamped_records_real_per_event_times(tmp_path):
    ...
    rc = AS.stream_stamped(
        ["bash", "-c", script], cwd=tmp_path, transcript=tpath, stderr_path=epath, timeout=60, raw_path=raw
    )
    assert rc == 0
...
def test_stream_stamped_kills_the_group_and_keeps_what_arrived(tmp_path):
    ...
    with pytest.raises(subprocess.TimeoutExpired):
        AS.stream_stamped(["bash", "-c", script], cwd=tmp_path, transcript=tpath, stderr_path=epath, timeout=2)
...
def test_claude_launch_sites_stream_instead_of_redirecting():
    ...
    from merlin_experiments.phase1 import authoring as L

    for fn in (EX.launch, L.finalize_report):
...
@pytest.mark.parametrize("finalize", [False, True], ids=["round", "finalize"])
@pytest.mark.parametrize("times_out", [False, True], ids=["exit", "timeout"])
def test_qa_launchers_record_arrival_and_raw_evidence(tmp_path, monkeypatch, finalize, times_out):
    """Exercise both launch sites with a real local child, never a provider or paid agent."""
    ...
    first = '{"type":"assistant","message":{"content":[]}}'
    second = '{"type":"result","num_turns":1}'
    executable.write_text(
        f"#!{sys.executable}\nimport sys, time\n"
        f"print({first!r}, flush=True)\n"
        "print('diagnostic', file=sys.stderr, flush=True)\n"
        f"time.sleep({300 if times_out else 0.05})\n"
        f"print({second!r}, flush=True)\nsys.exit(7)\n"
    )
    executable.chmod(0o755)
    monkeypatch.setenv("PATH", f"{tmp_path}{os.pathsep}{os.environ['PATH']}")
    provider = EX.ProviderConfig(driver="claudecode")
    from types import SimpleNamespace

    context = SimpleNamespace(repo=tmp_path, descriptor=tmp_path / "descriptor.yaml", target="fixture")
    config = EX.ExecutionConfig(context, provider, lambda: (), tmp_path / "timing.json")
    monkeypatch.setattr(bridge, "bridged_name", lambda *args: None)
    monkeypatch.setattr(bridge, "claude_env", lambda model: {})
    monkeypatch.setattr(bridge, "claude_model_name", lambda model, **kwargs: model)
    stopped = []
    monkeypatch.setattr(loop.FL, "stop_brokers", lambda *args, **kwargs: stopped.append(args))
    ws, run = tmp_path / "workspace", tmp_path / "run"
    ws.mkdir()
    run.mkdir()
    (ws / "TASK.md").write_text("Local test only\n")
    timeout = 2 if times_out else 10
    if finalize:
        (ws / "submission").mkdir()
        audit = loop.AnswerAudit((), (), (), tmp_path)
        result = loop.finalize_report(
            ws,
            run,
            "fixture",
            "low",
            ...
            context=context,
            audit=audit,
            grade_callback=lambda *args: {"all_pass": True},
        )
        assert result["agent_rc"] == (124 if times_out else 7)
        assert result["regrade_all_pass"] and not result["restored_after_regression"]
        prefix = "finalize"
    else:
        args = (ws, run, "fixture", "low", "none", {}, 0, timeout)
        if times_out:
            with pytest.raises(subprocess.TimeoutExpired):
                EX.launch(*args, config=config)
        else:
            rc, transcript = EX.launch(*args, config=config)
            assert rc == 7 and transcript == run / "rounds/round_00.transcript.jsonl"
        assert stopped == [(ws, None)], "broker cleanup must survive child timeout"
        prefix = "round_00"
    expected = [first] if times_out else [first, second]
    raw = run / "rounds" / f"{prefix}.stream.raw.jsonl"
    transcript = run / "rounds" / f"{prefix}.transcript.jsonl"
    assert raw.read_text() == "\n".join(expected) + "\n"
    events = [json.loads(line) for line in transcript.read_text().splitlines()]
    stamps = [datetime.fromisoformat(event.pop(AS.ARRIVED_AT)) for event in events]
    assert events == [json.loads(line) for line in expected]
    assert all(stamp.tzinfo is not None for stamp in stamps)
    assert stamps == sorted(stamps)
    if not times_out:
        assert stamps[0] < stamps[1], "separately emitted events need separately observed arrivals"
    assert (run / "rounds" / f"{prefix}.stderr.log").read_text() == "diagnostic\n"
...
def test_the_raw_stdout_tee_is_not_double_counted_as_a_transcript(tmp_path):
        ...
        json.dumps(
            {
                "type": "assistant",
                "message": {
                    "content": [{"type": "tool_use", "name": "Bash", "input": {"command": f"{tokens[0]} --target x"}}]
                },
            }
        )
        + "\n"
    ...
    assert ABC._tool_call_count(tmp_path, tokens) == 1
...
def test_opencode_stream_events_carry_their_line_arrival():
    ...
    from merlin_experiments.phase1.providers import opencode_agent as OA

    stream = "\n".join(
        [
            '{"part": {"type": "text", "id": "t1", "text": "hello"}}',
            '{"part": {"type": "tool", "callID": "c1", "tool": "bash", "state": {"input": {}, "output": "done"}}}',
            '{"part": {"type": "step-finish", "tokens": {"input": 11, "output": 3}}}',
        ]
    )
    stamps = ["2026-09-04T00:00:01+00:00", "2026-09-04T00:00:02+00:00", "2026-09-04T00:00:03+00:00"]
    events: list[dict] = []
...
def test_opencode_capture_fills_arrival_stamps_from_the_tailed_file(tmp_path):
    ...
    stamps: list[str] = []
    script = (
        'printf \'{"part":{"type":"text","id":"a","text":"1"}}\\n\'; sleep 1.5; '
        'printf \'{"part":{"type":"text","id":"b","text":"2"}}\\n\''
    )
    code, stdout, _err = OA._capture(
        ["bash", "-c", script], dict(os.environ), 60, str(tmp_path), stall_seconds=0, stamps=stamps
    )
    assert code == 0
...
def _transcript_without_a_cli_turn_count(path: Path, n_turns: int) -> None:
    ...
    with open(path, "w") as f:
        ...
        f.write(json.dumps({"type": "result", "subtype": "success", "is_error": False, "result": "done"}) + "\n")
...
def test_num_turns_reflects_the_turns_actually_in_the_transcript(tmp_path):
    ...
    ok = AB.emit_to_aet(
        run_dir=tmp_path, run_id="r0", method="arm", model="gpt-5.6-sol", target="t", save_trajectory=False
    )
    if not ok:
        pytest.skip("aet not installed in this environment")
    metrics = [json.loads(x) for x in (tmp_path / "logs" / "metrics.jsonl").read_text().splitlines() if x.strip()]
...
def test_an_authoritative_cli_turn_count_is_never_discarded(tmp_path):
    ...
    with open(tmp_path / "transcript.jsonl", "w") as f:
        ...
        f.write(
            json.dumps(
                {"type": "result", "subtype": "success", "is_error": False, "num_turns": 12, "total_cost_usd": 0.5}
            )
            + "\n"
        )
    ok = AB.emit_to_aet(run_dir=tmp_path, run_id="r1", method="arm", model="claude", target="t", save_trajectory=False)
    if not ok:
        pytest.skip("aet not installed in this environment")
    metrics = [json.loads(x) for x in (tmp_path / "logs" / "metrics.jsonl").read_text().splitlines() if x.strip()]
