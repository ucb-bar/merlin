...
def test_an_undetermined_group_leaves_both_sides_of_the_ratio(monkeypatch):
    ...
    monkeypatch.setattr(
        TB.EL,
        "is_eligible",
        lambda *_a, **_k: TB.EL.EligibilityVerdict(
...
def test_an_ineligible_group_is_excluded_with_its_reason(monkeypatch):
    monkeypatch.setattr(
        TB.EL,
        "is_eligible",
        lambda *_a, **_k: TB.EL.EligibilityVerdict(eligible=False, family="contraction", reason="no unit supports it"),
    )
    b = _basis([_Row(index=0)])
    assert b.entries == ()
...
def test_the_capability_map_cannot_shrink_the_measured_denominator(monkeypatch):
    ...
    monkeypatch.setattr(
        TB.EL,
        "is_eligible",
        lambda *_a, **_k: TB.EL.EligibilityVerdict(eligible=True, family="contraction", reason="ok"),
    )
...
def test_the_cover_stops_once_the_target_is_reached(monkeypatch):
    ...
    rows = [
        _Row(index=i, ticks=t, key=f"k{i}", parallel=(2 ** (i + 3), 4), reduction=(4,))
        for i, t in enumerate((90, 5, 3, 2))
    ]
    b = TB.derive_basis(_Census(rows, model_ticks=100), _Cap(), cover_target=0.9, family_floor=False)
...
def test_the_basis_is_deterministic_across_runs(monkeypatch):
    ...
    rows = [_Row(index=i, ticks=10, key=f"k{i}", parallel=(2 ** (i + 3), 4), reduction=(4,)) for i in range(6)]
    keys = [TB.derive_basis(_Census(rows, model_ticks=100), _Cap()).signature_keys() for _ in range(2)]
...
def test_ties_are_broken_by_signature_not_by_input_order(monkeypatch):
    ...
    rows = [_Row(index=i, ticks=10, key=f"k{i}", parallel=(2 ** (i + 3), 4), reduction=(4,)) for i in range(4)]
...
def test_a_family_evidenced_but_cheap_still_gets_one_task(monkeypatch):
    monkeypatch.setattr(
        TB.EL, "is_eligible", lambda *_a, **_k: TB.EL.EligibilityVerdict(eligible=True, family="f", reason="ok")
    )
    rows = [
        _Row(index=0, family="contraction", ticks=99, key="a"),
        _Row(index=1, family="normalization", ticks=1, key="b"),
    ]
    b = TB.derive_basis(_Census(rows, model_ticks=100), _Cap(("contraction", "normalization")), cover_target=0.9)
...
def test_a_declared_but_unevidenced_family_gets_no_manufactured_task(monkeypatch):
    ...
    b = TB.derive_basis(
        _Census([_Row(index=0, family="contraction", ticks=10, key="a")], model_ticks=10),
...
def test_the_certificate_accounts_for_every_group(monkeypatch):
    ...
    rows = [
        _Row(index=0, family="contraction", parallel=(8, 8), ticks=10, key="a"),
    ...
    c = b.certificate
    accounted = (
        len(b.entries) + len(c["excluded_ineligible"]) + len(c["excluded_undetermined"]) + len(c["eligible_not_chosen"])
    )
...
def test_the_certificate_names_the_census_stage(monkeypatch):
    ...
    monkeypatch.setattr(TB.EL, "is_eligible", lambda *_a, **_k: TB.EL.EligibilityVerdict(True, "contraction", "ok"))
    c = _Census([_Row(index=0)])
    c.stage = "post-quantization"
...
def test_groups_carry_their_shapes_so_a_config_ladder_can_be_built(monkeypatch):
    monkeypatch.setattr(TB.EL, "is_eligible", lambda *_a, **_k: TB.EL.EligibilityVerdict(True, "contraction", "ok"))
    rows = [
        _Row(index=0, parallel=(64, 64), reduction=(64,), ticks=5, key="a"),
...
def test_a_family_the_census_cannot_see_is_unsearched_not_absent(monkeypatch):
    ...
    monkeypatch.setattr(TB.EL, "is_eligible", lambda *_a, **_k: TB.EL.EligibilityVerdict(True, "contraction", "ok"))
    b = TB.derive_basis(
        _Census([_Row(index=0, family="matmul", ticks=1, key="a")], model_ticks=1),
