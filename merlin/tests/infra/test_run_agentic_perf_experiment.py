...
def _mock_preflight_dependencies(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(ORCH, "load_target_experiment", lambda _path: SimpleNamespace(target="gemmini"))
    monkeypatch.setattr(
        ORCH, "_functional_grade_cohort_from_run", lambda _target, _functional: ORCH.FunctionalGradeCohort((), (), 1, 1)
    )
    monkeypatch.setattr(
        ORCH.PAS,
    ...
    monkeypatch.setattr(
        ORCH, "_verify_functional_certificate", lambda *_args: {"public_descriptors": 1, "hidden_descriptors": 1}
    )
    monkeypatch.setattr(
        ORCH, "_verify_functional_certificate_provenance", lambda *_args: {"declaration_sha256": "7" * 64}
    )
    monkeypatch.setattr(
        ORCH,
    ...
    monkeypatch.setattr(
        ORCH, "_verify_tuning_certificate", lambda *_args: {"members": 1, "workload_sha256": ["e" * 64]}
    )
    monkeypatch.setattr(
        ORCH.HOLDOUT, "derive_domain", lambda *_args, **_kwargs: {"target": "gemmini", "legal": [1, 2, 3, 4]}
    )
    monkeypatch.setattr(
        ORCH.HOLDOUT, "verify_rtl_facts_provenance", lambda *_args, **_kwargs: {"replay_sha256": "d" * 64}
    )
    telemetry_sources = {
...
def test_chia_launch_receipt_attests_exact_command_sources_and_resources(
    ...
    command_artifacts = [
        {"index": index, "path": str(Path(command[index]).resolve()), "sha256": ORCH._sha_file(Path(command[index]))}
        for index in (0, 1)
    ]
...
def test_all_three_trial_handoffs_must_match_the_predeclared_treatment() -> None:
    sources = {
        name: f"{index:064x}" for index, name in enumerate(sorted(ORCH.PAS.TELEMETRY_TREATMENT_SOURCES), start=1)
    }
    expected = {
...
def test_stale_stage_must_match_the_complete_predeclared_trial_contract() -> None:
    ...
    ORCH._verify_trial_contract("trial_00", SimpleNamespace(agent_contract=dict(expected)), expected)
    ...
    for field, changed in (
        ...
        with pytest.raises(ORCH.ExperimentError, match="predeclared trial contract"):
            ORCH._verify_trial_contract("trial_00", SimpleNamespace(agent_contract=stale), expected)
...
def _functional_qualification_fixture(tmp_path: Path, *, baseline_sha256: str):
    ...
    tuning = SimpleNamespace(path=tuning_path, sha256=ORCH._sha_file(tuning_path), target="gemmini", pins=pins)
    workload = "8" * 64
    ...
    declaration = {
        ...
        "target_descriptor": {"path": str(descriptor_path.resolve()), "sha256": ORCH._sha_file(descriptor_path)},
        "source_certificate": {
...
def test_functional_certificate_provenance_binds_exact_baseline(tmp_path: Path) -> None:
    ...
    evidence = ORCH._verify_functional_certificate_provenance(certificate, tuning, baseline)
...
def test_measurement_adoption_checks_exact_trial_and_evidence_identities(tmp_path: Path) -> None:
    ...
    manifest = {
        ...
        "measurement_plan_sha256": ORCH.PAIRED._sha256_bytes(ORCH.PAIRED._canonical_bytes(plan)),
        ...
        "fork_before": {"ok": True},
        "fork_after": {"ok": True},
        "raw_results": {"paired_cells": str(cells), "paired_cells_sha256": ORCH._sha_file(cells)},
    }
...
def test_stats_projection_keeps_every_gsim_cell_and_excludes_nonprimary(tmp_path: Path) -> None:
    ...
    cells = [
        ...
        for replicate in ORCH.REPLICATES
        for simulator in ("spike", "gsim", "verilator")
    ]
    cells_path.write_text(json.dumps({"cells": cells}), encoding="utf-8")
    manifest = {
        "status": "GO",
        "raw_results": {"paired_cells": str(cells_path), "paired_cells_sha256": ORCH._sha_file(cells_path)},
    }
...
def test_statistics_predeclaration_projects_out_agent_evidence_hash() -> None:
    ...
    assert ORCH._statistics_trials(full) == [{"trial": "trial_00", "agent_run_id": "run-0"}]
...
def test_functional_certificate_must_cover_exact_public_and_hidden_descriptors(tmp_path: Path) -> None:
    ...
    identities = [ORCH.GATE.workload_sha256(ORCH.PAIRED.CERTPROD.derive_workload(path)) for path in manifests]
    members = tuple(
        ORCH.FunctionalCapsule(
...
def test_frozen_functional_cohort_ignores_later_live_corpus_growth(
    ...
    public_spec, hidden_spec, contract = ORCH._frozen_functional_regrade_inputs(tmp_path / "phase2", cohort)
...
def test_regrade_inputs_are_the_same_canonical_admitted_cohort() -> None:
    ...
    materialized = ORCH.discover_capsules(public_spec, labels={"public", "dev"}, contract=merlin_dir() / "contract")
...
def test_absent_functional_certificate_blocks_launch_unless_explicitly_waived(
    ...
    refused = ORCH.preflight(_uncertified(tmp_path, waived=False), heldout_certificate_provider_available=True)
    assert refused["status"] == "NO_GO"
    assert _certificate_blockers(refused), "control: an absent certificate must block the launch"

    waived = ORCH.preflight(_uncertified(tmp_path, waived=True), heldout_certificate_provider_available=True)
...
def test_the_waiver_records_the_claim_it_withdraws_and_is_not_gate_clean(
    ...
    _mock_preflight_dependencies(tmp_path, monkeypatch)
    declaration = ORCH.preflight(_uncertified(tmp_path, waived=True), heldout_certificate_provider_available=True)
...
def test_the_waiver_suppresses_no_other_blocker(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _mock_preflight_dependencies(tmp_path, monkeypatch)
    declaration = ORCH.preflight(_uncertified(tmp_path, waived=True), heldout_certificate_provider_available=False)
...
def test_the_waiver_never_relaxes_the_tuning_certificate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(ORCH.ExperimentError):
        ORCH.preflight(_uncertified(tmp_path, waived=True), heldout_certificate_provider_available=True)
...
def _identity_by_capsule(mapping, monkeypatch):
    monkeypatch.setattr(ORCH.PAIRED.CERTPROD, "derive_workload", lambda path: {"identity": mapping[path.parent.name]})
...
def test_tuning_corpus_may_measure_one_workload_under_several_family_levers(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(ORCH.PAS, "discover_performance_corpus", lambda *_args, **_kwargs: _fake_corpus(rows))
...
def test_shared_identities_do_not_hide_a_workload_the_certificate_never_covered(
    ...
    with pytest.raises(ORCH.ExperimentError, match="does not cover the derived corpus"):
        ORCH._verify_tuning_certificate(SimpleNamespace(members={"i16": {}}), SimpleNamespace(target="gemmini"))
    with pytest.raises(ORCH.ExperimentError, match="does not cover the derived corpus"):
...
def test_a_narrowed_campaign_may_use_the_whole_corpus_certificate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    coverage = ORCH._verify_tuning_certificate(certificate, SimpleNamespace(target="gemmini"), capsules="PM00,PM01")
