...
def test_the_analysis_costs_no_oracle_and_says_what_it_is_based_on(tmp_path):
    from merlin_experiments.phase2 import broker_policy as BP

    assert BP.ANALYSIS_ACTION == "analyze-command-buffers"
    # It must be OPTIONAL: a required action would force a call the agent may not need.
    import inspect

    src = inspect.getsource(ED.analyze_command_buffers)
...
def test_it_prices_two_buffers_against_the_derived_ceilings(tmp_path):
    ...
    out = ED.analyze_command_buffers(a, b, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0)
...
def test_it_never_claims_an_absolute_cycle_count(tmp_path):
    ...
    a = _buffer(tmp_path, "a.json", (16, 16, 16))
    out = ED.analyze_command_buffers(a, a, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0)
    flat = json.dumps(out)
...
def test_an_unreadable_buffer_refuses_rather_than_crashing(tmp_path):
    ...
    with pytest.raises(Exception):
        ED.analyze_command_buffers(missing, good, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0)
...
def test_declined_whole_model_is_unknown_work_not_a_zero_cost_success(tmp_path):
    ...
    out = ED.analyze_command_buffers(declined, declined, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0)
...
def test_it_offers_no_calibrated_cycle_estimate(tmp_path):
    ...
    out = ED.analyze_command_buffers(a, b, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0, target="gemmini")
...
def test_the_lower_bound_is_a_floor_and_says_so(tmp_path):
    a = _buffer(tmp_path, "a.json", (16, 16, 16))
    out = ED.analyze_command_buffers(a, a, peak_macs_per_cycle=256, achievable_macs_per_cycle=80.0, target="gemmini")
...
def test_an_uncountable_barrier_stream_is_unknown_not_zero(tmp_path):
    ...
    assert barriers["status"] in ("counted", ED.BARRIER_UNKNOWN)
