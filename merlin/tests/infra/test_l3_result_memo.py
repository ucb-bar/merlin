...
def test_the_same_emitted_program_on_the_same_engine_is_one_key():
    assert PME._l3_memo_key(CB, "module {}", PIN_A, SCOPE) == PME._l3_memo_key(CB, "module {}", PIN_A, SCOPE)
...
def test_key_ordering_of_the_command_buffer_does_not_matter():
    ...
    assert PME._l3_memo_key(CB, "m", PIN_A, SCOPE) == PME._l3_memo_key(reordered, "m", PIN_A, SCOPE)
...
def test_a_different_lowered_module_is_a_different_program():
    ...
    assert PME._l3_memo_key(CB, "module { A }", PIN_A, SCOPE) != PME._l3_memo_key(CB, "module { B }", PIN_A, SCOPE)


def test_a_different_command_buffer_is_a_different_program():
    other = {"abi_version": "0.1", "commands": [{"opcode": "CONV2D", "operands": {"dst": "Y"}}]}
    assert PME._l3_memo_key(CB, "m", PIN_A, SCOPE) != PME._l3_memo_key(other, "m", PIN_A, SCOPE)


def test_a_different_engine_shares_nothing():
    """A cycle count is a fact about a program AND the engine that ran it."""
    assert PME._l3_memo_key(CB, "m", PIN_A, SCOPE) != PME._l3_memo_key(CB, "m", PIN_B, SCOPE)
...
def test_each_counter_pass_is_its_own_measurement():
    ...
    """
    keys = {PME._l3_memo_key(CB, "m", PIN_A, f"r000/{name}") for name in ("unprofiled", "occupancy", "physical_bytes")}
    assert len(keys) == 3
...
def test_each_replicate_is_its_own_measurement():
    ...
    assert PME._l3_memo_key(CB, "m", PIN_A, "r000/unprofiled") != PME._l3_memo_key(CB, "m", PIN_A, "r001/unprofiled")


def test_the_memo_starts_empty_so_a_stale_table_cannot_answer_for_a_fresh_stage():
    assert isinstance(PME._L3_MEMO, dict)
...
class _Engine:
    ...
    def __init__(self, tmp_path: Path, monkeypatch, name: str = "engineA"):
        ...
        monkeypatch.setattr(PME.OOT, "run_on_oracle", run_on_oracle)
        monkeypatch.setattr(PME.CERTPROD, "encode_declared_outputs", lambda outputs, cb: ("s" * 64, {}))

    def adapter(self, store, evidence=None):
        return PME._gsim_l3_adapter(
            "t", evidence if evidence is not None else {}, self.certificate, reuse_scope=SCOPE, store=store
        )
...
def test_a_second_measurement_of_the_same_program_returns_the_cached_number(tmp_path, monkeypatch, store):
...
def test_a_different_pass_over_the_same_program_is_a_miss(tmp_path, monkeypatch, store):
    ...
    engine = _Engine(tmp_path, monkeypatch)
    PME._gsim_l3_adapter("t", {}, engine.certificate, reuse_scope="r000/occupancy", store=store)(CB, "m", tmp_path, 60)
    PME._gsim_l3_adapter("t", {}, engine.certificate, reuse_scope="r000/physical_bytes", store=store)(
        CB, "m", tmp_path, 60
    )
    assert engine.calls == 2
...
def test_a_later_process_finds_the_measurement_on_disk(tmp_path, monkeypatch, store):
    ...
    engine = _Engine(tmp_path, monkeypatch)
    first = engine.adapter(store)(CB, "module {}", tmp_path, 60)
    PME._L3_MEMO.clear()  # what a fresh process starts with
    second = engine.adapter(store)(CB, "module {}", tmp_path, 60)
...
def test_an_absent_store_root_is_a_miss_and_the_put_creates_it(tmp_path, monkeypatch):
    fresh = PME.L3MeasurementStore(tmp_path / "never" / "made")
    engine = _Engine(tmp_path, monkeypatch)
