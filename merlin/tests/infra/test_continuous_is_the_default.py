...
def test_the_legacy_single_session_path_is_not_the_default():
    ...
    assert parse_options(["--run-id", "probe"]).continuous is False
    assert parse_options(["--run-id", "probe", "--continuous"]).continuous is True
...
def test_the_certified_continuous_path_still_ignores_the_round_cap():
    ...
    """
    src = (module_source_path("merlin_experiments.phase1.authoring")).read_text(encoding="utf-8")
...
def test_the_cert_sim_is_one_the_broker_accepts():
    ...
    allowed = tuple(SB._allowed_sims(feedback_context()))
    sim = TP.cert_sim("L3", context=feedback_context())
