...
@pytest.mark.parametrize("target", ["gemmini", "foreign-fixture"])
@pytest.mark.parametrize("explicit_root", [False, True])
def test_paired_input_loader_forwards_target_root_and_exact_named_functional_waivers(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, target: str, explicit_root: bool
) -> None:
    observed = {}
    ...
    monkeypatch.setattr(PI.PC, "inspect_functional_run", inspect)
    with pytest.raises(StopAfterFunctionalInspection if explicit_root else TypeError):
        PI.load_paired_inputs(
            ...
            **({"functional_runs_root": tmp_path / "explicit-runs"} if explicit_root else {}),
        )

    if not explicit_root:
        assert observed == {}
        return
    assert observed["args"][0] == tmp_path / "explicit-runs"
    assert observed["kwargs"]["waive"] == frozenset({"score_incomplete", "phase_grade_incomplete"})
...
def _inputs(tmp_path: Path, members: tuple | None = None, *, phase: str = "held_out"):
    descriptor = {
        ...
        "inputs": [{"name": "X", "shape": [16, 16], "dtype": "i8"}, {"name": "W", "shape": [16, 16], "dtype": "i8"}],
    ...
    for member in members:
        ...
        (member.source_dir / "capsule.yaml").write_text(yaml.safe_dump(member.descriptor), encoding="utf-8")
    workloads = [PME.gsim_workload(member) for member in members]
    certificate = SimpleNamespace(
        sha256="d" * 64,
        unresolved={},
        pins={},
        members={GATE.workload_sha256(workload): {} for workload in workloads},
    )
    return PME.PairedInputs(
        SimpleNamespace(run_id="functional", digest="a" * 64),
        SimpleNamespace(record_sha256="b" * 64),
        SimpleNamespace(capsules=members),
        phase,
        tmp_path / "baseline",
        "a" * 64,
        tmp_path / "candidate",
...
def test_pure_plan_is_exact_interleaved_and_deterministic(tmp_path: Path) -> None:
    ...
    first = PME.build_measurement_plan(inputs)
    second = PME.build_measurement_plan(inputs)

    assert first.declaration == second.declaration
    assert first.declaration_sha256 == second.declaration_sha256
    assert len(first.schedule) == 2 * len(PME.REPLICATES) * 2
    for pair_index in range(2 * len(PME.REPLICATES)):
    ...
    assert {item.replicate for item in first.schedule} == set(PME.REPLICATES)
...
def test_pure_workload_identity_matches_certificate_producer(tmp_path: Path) -> None:
    descriptor = {
        ...
        "operation": {
            ...
            "attributes": {
                ...
                "weight": "B",
                "out": "Y",
                "semantic": "ignored-name",
                "output_dtype": "i32",
                "epilogue": [],
            },
        },
        "inputs": [
            {"name": "A", "role": "input", "shape": [7, 11], "dtype": "i8"},
    ...
    member = SimpleNamespace(family="shape", capsule="case", descriptor=descriptor, source_dir=tmp_path)

    assert PME.gsim_workload(member) == WORKLOAD.derive_workload(manifest)


def test_content_addressed_raw_store_is_immutable_and_idempotent(tmp_path: Path) -> None:
    store = PME.ContentAddressedRawStore(tmp_path / "raw")
...
def test_host_only_holdout_reveal_adapter_verifies_exact_manifest(tmp_path: Path) -> None:
    ...
    descriptor = {
        ...
        "operation": {
            "op": "matmul",
            "attributes": {"lhs": "X", "weight": "W", "out": "Y", "epilogue": [], "output_dtype": "i32"},
        },
        "inputs": [
    ...
    root.chmod(0o555)
    try:
        loaded = PI.load_holdout_corpus(
            root,
            manifest,
            manifest_sha256=PME.sha256_file(manifest),
            capsules_sha256=tree["sha256"],
            expected_target="gemmini",
        )
        assert loaded.format == "holdout_reveal_v2"
        assert [(row.family, row.capsule) for row in loaded.capsules] == [("PK", "PKH00_k17")]
    finally:
        root.chmod(0o755)
...
def test_l3_adapter_uses_only_pinned_gsim_and_never_resolves_verilator(
    ...
    certificate = SimpleNamespace(
        pins={
            "gsim_binary": {"sha256": PME.sha256_file(gsim_binary)},
            "gsim_firrtl": {"sha256": "1" * 64},
            "gsim_model": {"sha256": "3" * 64},
        }
    )
    monkeypatch.setattr(PME.OOT, "run_on_oracle", primary)
    ...
    adapter = PME._gsim_l3_adapter(
        "gemmini",
        evidence,
        certificate,
        reuse_scope="r000/unprofiled",
        store=PME.L3MeasurementStore(tmp_path / "l3_cache"),
    )
    result = adapter(
        {"tensors": {"y": {"role": "output", "shape": [1], "dtype": "i32"}}}, "module", tmp_path / "work", 9
    )

    assert result["cycles"] == 123
    assert set(evidence) == {"gsim"}
    assert evidence["gsim"]["output_encoding"] == PME.CERTPROD.OUTPUT_ENCODING
...
@pytest.mark.parametrize(
    "tier", [None, {"status": "skipped", "reason": "not attempted"}, {"status": "error", "reason": "oracle timeout"}]
)
def test_absent_execution_is_not_misreported_as_certificate_rejection(tmp_path, monkeypatch, tier):
    monkeypatch.setattr(PME, "hash_tree", lambda _: {"sha256": "a" * 64})
    monkeypatch.setattr(PME.CR, "load_capsule", lambda *_a, **_k: {})
    monkeypatch.setattr(PME.CR, "simulator_adapter", lambda *_a: object())
    monkeypatch.setattr(PME, "_gsim_l3_adapter", lambda *_a, **_k: object())
    tiers = {"L2": {"status": "pass"}}
    if tier is not None:
        tiers["L3"] = tier
    monkeypatch.setattr(PME.CR, "run_capsule", lambda *_a, **_k: {"status": "error", "numeric": "pass", "tiers": tiers})
...
def test_mocked_execution_has_fresh_workspaces_primary_gsim_and_exact_denominator(tmp_path: Path) -> None:
    ...
    plan = PME.build_measurement_plan(_inputs(tmp_path, (member,)))
    out = tmp_path / "out"
    out.mkdir()
    workspaces = []
    ...
    def executor(spec, workspace, *_args, **_kwargs):
        ...
        return {"schema": "mock", "execution": spec.as_dict(), "measurement": _mock_measurement(spec, cycles)}

    rows, _ = PME.execute_schedule(
        plan,
        out,
        contract_root=tmp_path / "explicit-contract",
        timeout=1,
    ...
    assert len(workspaces) == 2 * len(PME.REPLICATES) and len(set(workspaces)) == len(workspaces)
    # one Spike correctness screen and one GSIM measurement per (member, replicate, arm)
    assert len(rows) == 2 * len(PME.REPLICATES) * 2
    ...
    assert ME.completion_report(rows, plan.expected)["complete"] is True
    pairs = PME.paired_cycle_rows(rows)
    assert len(pairs) == len(PME.REPLICATES) and all(row["simulator"] == "gsim" for row in pairs)
    raw_files = list((out / "raw_results/sha256").glob("*.json"))
    assert len(raw_files) == 2 * len(PME.REPLICATES)
    assert all(path.stem == PME.sha256_file(path) for path in raw_files)


def test_completion_refuses_non_rtl_primary(tmp_path: Path) -> None:
        ...
        _inputs(
            ...
            (
                SimpleNamespace(
                    ...
                    source_sha256="1" * 64,
                    descriptor={
                        "operation": {"op": "movement", "attributes": {"src": "X"}},
                        "inputs": [{"name": "X", "shape": [1], "dtype": "i8"}],
                        "numeric_policy": {"compare": "exact_int"},
                    },
                ),
    ...
    for spec in plan.schedule:
        ...
        rows.extend(PME.result_rows(raw, {"sha256": "f" * 64, "path": "/raw"}))
    primary = next(row for row in rows if row["simulator"] == "gsim")
    primary["provenance"]["derived_from_rtl"] = False
    assert ME.completion_report(rows, plan.expected)["complete"] is False
