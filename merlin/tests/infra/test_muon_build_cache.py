...
def muon_source():
    ...
    if "muon" not in target_registry.explicit_targets():
        pytest.skip("select Muon OOT support explicitly with MERLIN_TARGET_PATH")
    selected = resolve_support("muon")
    backend = selected.plugin()["backend"]
    return contained_resource(selected.base, f"{backend}/muon.py")


@pytest.fixture
def muon(monkeypatch, muon_source):
    ...
    assert Path(m.__file__).resolve() == muon_source.resolve()
    monkeypatch.setattr(
        m,
        "_build_cache_key",
...
def test_installed_frontend_source_membership_changes_cache_key(tmp_path, monkeypatch, muon_source):
    """Exercise the actual key function without importing or registering a provider."""
    import ast
    import subprocess
    from pathlib import Path
    ...
    function = next(node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == "_build_cache_key")
    backend = tmp_path / "provider/backend"
    backend.mkdir(parents=True)
    owner = backend / "muon.py"
    owner.write_text("# provider source\n")
    namespace = {"Path": Path, "__file__": str(owner)}
    exec(compile(ast.Module(body=[function], type_ignores=[]), str(source), "exec"), namespace)
    key = namespace["_build_cache_key"]
    frontend = tmp_path / "installed/merlin/llvmlower"
    frontend.mkdir(parents=True)
    pipeline = frontend / "pipeline.py"
    pipeline.write_text("# installed pipeline\n")
    monkeypatch.setattr(paths, "module_source_path", lambda name: pipeline)
    monkeypatch.setattr(paths, "repo_root", lambda: pytest.fail("checkout fallback used"))
    monkeypatch.setattr(toolchain, "mlir_bin", lambda name: "synthetic-clang")
    monkeypatch.setattr(BC, "toolchain_token", lambda name: "synthetic-toolchain")
    monkeypatch.setattr(subprocess, "Popen", lambda *a, **k: pytest.fail("native launch"))
    baseline = key("fixture", "fixture", {})
    assert baseline
    added = frontend / "helper.py"
    added.write_text("# new helper\n")
    expanded = key("fixture", "fixture", {})
    assert expanded and expanded != baseline
    added.write_text("# changed helper\n")
    assert key("fixture", "fixture", {}) != expanded
    pipeline.unlink()
    assert key("fixture", "fixture", {}) is None
...
def test_the_oracle_build_refuses_an_unstamped_hit(muon_source):
...
def test_both_muon_build_paths_consult_the_cache(muon_source):
