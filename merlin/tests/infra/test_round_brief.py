    ...
    for node in ast.walk(tree):
        for _, statements in ast.iter_fields(node):
            ...
            for index, statement in enumerate(statements):
                ...
                if not isinstance(call.func, ast.Name) or call.func.id != "_launch":
                    continue
                launches += 1
                assert index > 0
                previous = statements[index - 1]
                assert isinstance(previous, ast.Expr) and isinstance(previous.value, ast.Call)
                refresh = previous.value
                assert isinstance(refresh.func, ast.Attribute) and refresh.func.attr == "refresh_before_launch"
                assert [arg.id for arg in refresh.args[:2] if isinstance(arg, ast.Name)] == ["run_dir", "ws"]
                owner = statements[index - 2]
                assert isinstance(owner, ast.ImportFrom)
                assert owner.module == "merlin_experiments.phase1.feedback"
                assert [(alias.name, alias.asname) for alias in owner.names] == [("brief", refresh.func.value.id)]
    assert launches == 3, "new launch paths must join the pre-launch brief refresh contract"
    ...
    for node in ast.walk(tree):
        ...
        for index, statement in enumerate(node.body):
            if not isinstance(statement, ast.Expr) or not isinstance(statement.value, ast.Call):
                continue
            if ast.unparse(statement.value.func) != "brief.write":
                continue
            owner = node.body[index - 1]
            assert isinstance(owner, ast.ImportFrom) and owner.module == "merlin_experiments.phase1.feedback"
            assert [(alias.name, alias.asname) for alias in owner.names] == [("brief", None)]
            assert [ast.unparse(arg) for arg in statement.value.args] == ["run_dir", "ws", "rnd"]
