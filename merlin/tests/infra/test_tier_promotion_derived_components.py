...
def _promote_once(B, ws, ch, log):
    ...
    return B.promote(ws, ch, v, "L2", "L3", None, log, context=feedback_context())
...
# ---------------------------------------------------------------------------------------------
# 4. a DECLARED block says who owns what -- it says nothing about what can be READ
# ---------------------------------------------------------------------------------------------
# The declared branch returned an empty inert set, so every file no prefix claimed became UNATTRIBUTED.
# UNATTRIBUTED is a dependency of every capsule, so one edit to a REPORT.md invalidated the whole corpus.
# Measured on a live run: 253 "L2 invalidated by <unattributed>" against 2 promotion events, while the
# run's own log read "0 file(s) outside every command's reach" -- which was len(frozenset()).
...
def _declared_ws(tmp_path, name="ws"):
    files = dict(FILES)
    files["manifest.yaml"] = MANIFEST + _DECLARED
    return _ws(tmp_path, files, name=name)


def test_a_declared_block_still_knows_which_bytes_nothing_can_read(tmp_path):
    """The same inert set the derivation finds; declaring ownership does not blind us to reachability."""
    B = _mod()
    d = B.decomposition(_declared_ws(tmp_path))
    assert d["source"] == "declared"
    assert set(d["inert"]) == {"REPORT.md", "docs/iteration_notes.md", "scratch_ops.S"}


def test_an_unreadable_file_is_not_a_dependency_of_every_capsule(tmp_path):
    """The defect, stated directly: a report is inert, so it owns nothing and invalidates nothing."""
    B = _mod()
    owners = B.decomposition(_declared_ws(tmp_path))["owners"]
    for rel in ("REPORT.md", "docs/iteration_notes.md", "scratch_ops.S"):
        assert rel not in owners, f"{rel} cannot be read by any command, so it depends on nothing"


def test_declared_ownership_still_wins_outright(tmp_path):
    """The closure is consulted for reachability ONLY; it may never reassign a declared file."""
    B = _mod()
    d = B.decomposition(_declared_ws(tmp_path))
    assert d["owners"]["mlir_oot/frontend.py"] == {"parse"}
    assert d["owners"]["mlir_oot/cmdbuf.py"] == {"emit_command_buffer"}
    # a source file no prefix claims is still UNATTRIBUTED -- reachable, just unowned
    assert d["owners"]["mlir_oot/parse_main.py"] == {UNATTRIBUTED}
    assert UNATTRIBUTED in d["names"]


def test_unattributed_is_absent_when_nothing_actually_fell_through(tmp_path):
    """`names` is what the decomposition FOUND. Announcing UNATTRIBUTED unconditionally put a dependency
    on every capsule that no file backed."""
    B = _mod()
    files = dict(FILES)
    # claim every reachable source file, leaving only the unreadable ones unclaimed
    files["manifest.yaml"] = MANIFEST + "components:\n  parse: [mlir_oot/, run_parse.py]\n"
    d = B.decomposition(_ws(tmp_path, files, name="ws_all"))
    assert d["source"] == "declared"
    fell_through = [r for r, o in d["owners"].items() if o == {UNATTRIBUTED}]
    assert (UNATTRIBUTED in d["names"]) == bool(fell_through), "names must match what was observed"
