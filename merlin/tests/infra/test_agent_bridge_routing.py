...
def test_a_bridged_round_is_metered_whatever_the_driver_declares(BR, monkeypatch):
    ...
    config = EX.ProviderConfig(driver="codex")
    assert EX.billing_mode("nemotron", config=config) == ET.METERED, "bridged codex traffic is real Bedrock spend"
    # the seat model still reports as a seat
    assert EX.billing_mode("gpt-5.6-sol", config=config) != ET.METERED
...
def test_the_driver_name_and_the_wiring_name_route_identically(BR):
    ...
    ``providers.execution.resolve_driver`` returns ``"claudecode"``; the runtime wiring in this module
