...
def no_snapshot(monkeypatch):
    """Create minimal real snapshots while testing final mount order."""
    original = bwrap._snapshot_grants

    def _grants(ws, bundle, repo):
        if not bwrap.bundle_snapshot_root(ws).exists():
            bwrap.materialize_bundle_inputs(ws, bundle, repo=repo)
        return original(ws, bundle, repo)
...
def _order(argv, ws):
    w = str(ws)
    last_mask = max(
        (i for i, a in enumerate(argv) if a == "--tmpfs" and w.startswith(argv[i + 1].rstrip("/"))), default=-1
    )
    last_bind = max((i for i, a in enumerate(argv) if a == "--bind" and argv[i + 1] == w), default=-1)
...
def test_a_deny_containing_the_workspace_does_not_mask_it(no_snapshot, tmp_path):
    ...
    bundle = {
        "allowed": [{"path": "merlin/contract", "mode": "ro"}],
...
def test_a_broad_deny_does_not_eat_a_more_specific_grant(no_snapshot, tmp_path):
    ...
    bundle = {
        "allowed": [{"path": "merlin/experiments/spec", "mode": "ro"}],
        "denied": [{"path": "merlin", "reason": "internals"}],
    }
    argv = bwrap.reapply_bundle_snapshot(["seed"], ws, bundle, repo=repo)
    deny = max(i for i, a in enumerate(argv) if a == "--tmpfs" and argv[i + 1] == str(repo / "merlin"))
    grant = max(
        i
