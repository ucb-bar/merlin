...
def test_the_response_names_both_the_requested_and_the_graded_bytes(broker):
    ...
    doc = broker._decorate_response('{"all_pass": true}', req, "rid", "b" * 64, started_ns=2, completed_ns=3)
...
def test_nothing_is_announced_when_the_bytes_did_not_move(broker):
    ...
    doc = broker._decorate_response('{"all_pass": true}', req, "rid", "a" * 64, started_ns=2, completed_ns=3)
...
def test_a_moved_digest_is_no_longer_refused(broker):
    ...
    src = (feedback_source("selfcheck_broker")).read_text()
    assert "submission changed while the request was queued" not in src, (
        "the stale-request refusal is back; an agent that edits while its check is queued will never receive a check"
    )
...
def _reply(shim, **over):
    v = {"selfcheck_protocol": shim.PROTOCOL_VERSION, "selfcheck_request_id": "rid", "submission_sha256": "a" * 64}
    v.update(over)
    return v
...
def test_a_reply_about_newer_bytes_is_usable_and_announced(shim):
    ok, note = shim._reply_is_usable(
        _reply(
            shim, submission_sha256="b" * 64, graded_newer_bytes=True, graded_newer_note="graded your CURRENT bytes"
        ),
        "rid",
        "a" * 64,
    )
...
def test_the_declaration_alone_cannot_launder_a_foreign_request(shim):
    ...
    ok, _ = shim._reply_is_usable(
        _reply(shim, selfcheck_request_id="other", submission_sha256="b" * 64, graded_newer_bytes=True), "rid", "a" * 64
    )
    assert not ok


def test_the_shim_announces_rather_than_swallowing(shim):
    src = module_source_path("merlin_experiments.phase1.tools.selfcheck").read_text()
