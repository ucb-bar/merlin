...
def _mod(name: str):
    ...
    spec = importlib.util.spec_from_file_location(
        f"functional_gate_{name}", feedback_source(name, HARNESS / f"{name}.py")
    )
...
def test_only_hidden_scoring_requests_capability_admission(monkeypatch):
    ...
    grader._score("pkg", "public", "runs", {"public", "dev"}, False, context=feedback_context())
    grader._score("pkg", "hidden", "runs", {"hidden"}, False, context=feedback_context())
...
def test_arc_routed_formal_model_sim_uses_shared_dynamic_rtl_engine_policy(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _target: {
            "engine": "gsim",
            "fidelity": "elaborated_rtl",
            "reason": "target-derived gsim wrapper",
            "available": True,
            "sim_via": "",
        },
    )
...
def test_formal_model_install_overwrites_ambient_verilator_with_required_gsim(monkeypatch):
    ...
    monkeypatch.setattr(
        grader, "_formal_model_simulator", lambda _target: {"engine": "gsim", "fidelity": "elaborated_rtl"}
    )
...
def test_formal_model_install_refuses_engine_different_from_pin(monkeypatch):
    ...
    monkeypatch.setenv("MERLIN_REQUIRED_RTL_ENGINE", "gsim")
    monkeypatch.setattr(
        grader, "_formal_model_simulator", lambda _target: {"engine": "verilator", "fidelity": "elaborated_rtl"}
    )
...
@pytest.mark.parametrize(
    "mutation,reason",
    [
        (lambda s: s.update({"model_execution_all_pass": False}), "model_execution_evidence_not_complete"),
        (
            lambda s: s["structural_evidence_scope"].update({"n_model_execution_capsules": 1}),
            "structural_evidence_scope_denominator_mismatch",
        ),
        (lambda s: s.update({"structural_evidence_all_pass": False}), "structural_evidence_not_complete"),
    ],
)
...
def test_grader_main_returns_nonzero_and_records_failed_hidden_zero_of_zero(tmp_path, monkeypatch):
    ...
    public = _score()
    hidden = _score(n_capsules=0, n_passed=0, l3=0, rtl_backed=0, functional_pass=0, gradeable=False)
    scores = iter((public, hidden))
    monkeypatch.setattr(grader, "_score", lambda *_args, **_kwargs: next(scores))

    def freeze(rd: Path, *, repo: Path) -> dict:
        sha = grader.hash_tree(rd / "submission")["sha256"]
    ...
    # This test supplies score fixtures, not a hardware simulator. Keep actual simulator
    # selection mandatory in production; the explicit external boundary is controlled here.
    monkeypatch.setattr(
        grader,
        "_install_formal_model_simulator",
        lambda target: ({"engine": "fixture", "fidelity": "elaborated_rtl"}, None),
    )
    rc = grader.main(
        ["--run-dir", str(run_dir), "--arm", "merlin_assisted", "--capsules", "public", "--hidden-capsules", "hidden"],
...
def test_launcher_rejects_nonzero_grader_even_if_manifest_claims_success(tmp_path):
    ...
    result = loop.CERT._official_grade_result(9, tmp_path)
...
def test_launcher_rejects_grader_failed_status_even_with_zero_exit(tmp_path):
    ...
    (tmp_path / "run_manifest.yaml").write_text(yaml.safe_dump(_manifest(_score(), complete=False)), encoding="utf-8")
    result = loop.CERT._official_grade_result(0, tmp_path)
    assert result["complete"] is False
...
def test_arm4_wrapper_rejects_non_rtlcheck_or_ambiguous_bundle_override(monkeypatch, override):
    ...
    called = []
    monkeypatch.setattr(wrapper.L, "main", lambda argv, **kwargs: called.append(argv) or 0)
...
def test_arm4_wrapper_allows_a_generated_realistic_rtlcheck_bundle(monkeypatch):
    ...
    monkeypatch.setattr(wrapper.L, "main", invoke)
    bundle = "merlin_assisted_rtlchecks_hwbringup_v0"
    assert wrapper.main(["--run-id", "test", "--bundle", bundle]) == 0
    assert called[0][called[0].index("--bundle") + 1] == bundle
    assert treatments[0].qa_runner.__module__ == "merlin_experiments.phase1.feedback.rtlchecks"
    assert treatments[0].checkpoint_feedback.__module__ == "merlin_experiments.phase1.feedback.rtlchecks"
