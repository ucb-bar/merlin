...
from merlin.perf.external_objective import OBJECTIVE_DIRECTORY, load_external_objective, objective_directory
...
PAS = importlib.import_module("merlin_experiments.phase2.authoring")
...
def specification(tmp_path, *, source_text=SOURCE, **changes):
    ...
    spec = {
        "schema": "external_full_model_objective_spec_v1",
        "id": "host-model",
        "entry": "forward",
        "normalized_source": str(source),
        "source_sha256": sha(source_text.encode()),
...
def test_stale_pins_and_path_or_authority_surprises_refuse(tmp_path, change):
    ...
    if change == "source_pin":
        changes["source_sha256"] = sha(b"wrong")
    elif change == "relative":
        changes["normalized_source"] = "normalized.mlir"
    elif change == "traversal":
        changes["normalized_source"] = str(tmp_path / ".." / tmp_path.name / "normalized.mlir")
    elif change == "unknown_key":
        changes["normalization_callback"] = "do_not_execute"
    elif change == "unqualified":
        changes["complete_model"] = False
    elif change == "missing_pins":
        changes["provenance_pins"] = {}
    elif change == "path_pin":
        changes["provenance_pins"] = {"/secret/reference": sha(b"x")}
    elif change == "wrong_entry":
        changes["entry"] = "missing"
    path, digest, source = specification(tmp_path, **changes)
    if change == "spec_pin":
        digest = sha(b"other spec")
...
def corpus(tmp_path, monkeypatch):
    ...
    member = P2_CORPUS.PerformanceCapsule("family", "capsule", frozen, "public/capsule", {}, sha(b"a"), 2, 1)
    result = P2_CORPUS.FrozenPerformanceCorpus(
        tmp_path / "frozen", tmp_path / "frozen", tmp_path / "manifest", sha(b"b"), sha(b"c"), (member,)
    )
    target = SimpleNamespace(capsule_corpus=tmp_path / "original" / "public")
    monkeypatch.setattr(AW, "answer_surfaces", lambda _: [])
...
def test_external_sealed_under_existing_readonly_grant_and_legacy_unchanged(tmp_path, monkeypatch):
    ...
    legacy = AW.build_answer_free_agent_inputs(frozen, target, tmp_path / "legacy")
    legacy_record = P2_CONTRACTS.exact_tree_record(legacy.root)
    inputs = AW.build_answer_free_agent_inputs(frozen, target, tmp_path / "inputs", external_objective=objective)
    sentinel = INPUTS.select_external_e2e_sentinel(objective, inputs)
    ...
    assert P2_CONTRACTS.exact_tree_record(legacy.root) == legacy_record
    ...
    monkeypatch.setattr(TC, "toolchain_binds", lambda _, **kwargs: [])
    ...
    inner = AW.inner_execution_policy(target, candidate, inputs, inputs=selected)
    outer = AW.outer_codex_policy(candidate, inputs, [], target, inputs=selected)
    ...
    with pytest.raises(PAS.StageGateError, match="changed"):
        INPUTS.select_external_e2e_sentinel(objective, inputs)
...
def test_multiple_external_models_receive_distinct_readonly_grants(tmp_path, monkeypatch):
    ...
    second_path, second_digest, _ = specification(second_dir, id="language-model", source_text=second_source)
    objectives = [load(first_path, first_digest), load(second_path, second_digest)]
    frozen, target = corpus(tmp_path, monkeypatch)

    inputs = AW.build_answer_free_agent_inputs(frozen, target, tmp_path / "inputs", external_objectives=objectives)
    sentinels = [INPUTS.select_external_e2e_sentinel(objective, inputs) for objective in objectives]
    assert [Path(row.frozen_source_path).relative_to(inputs.root) for row in sentinels] == [
        objective_directory("vision-model"),
        objective_directory("language-model"),
    ]
    assert len({row.capsule_sha256 for row in sentinels}) == 2
    manifest = json.loads(inputs.manifest_path.read_text())
    assert [row["id"] for row in manifest["external_objectives"]] == ["vision-model", "language-model"]
    AW.verify_answer_free_agent_inputs(inputs)

    with pytest.raises(PAS.StageGateError, match="distinct"):
        AW.build_answer_free_agent_inputs(
...
def _isolate_worker_transport(monkeypatch):
    # These tests inspect argv construction, not machine-dependent admission or source copying.
    # Real seal/child import behavior is exercised in test_perf_frozen_python.
    from merlin.perf.host_resources import HostMemorySample

    gateway = importlib.import_module("merlin_experiments.frozen_python")
    snapshot = importlib.import_module("merlin_experiments.source_snapshot")
    provider = {"synthetic_provider": True, "resolved_target": "fixture"}
    frozen = {"synthetic_snapshot": True}
    observed = {}
    ...
    def create(source, destination, **kwargs):
        ...
        assert kwargs["target_name"] == "fixture"
        observed["destination"] = destination
        observed["inputs"] = kwargs["declared_inputs"]

    def verify(destination):
        assert destination == observed["destination"]
        return frozen

    def environment(destination, document):
        assert destination == observed["destination"] and document is frozen
        return {"SYNTHETIC_FROZEN_PROVIDER": "selected"}

    monkeypatch.setattr(snapshot, "selected_provider", select)
    monkeypatch.setattr(snapshot, "create", create)
    monkeypatch.setattr(snapshot, "verify", verify)
    monkeypatch.setattr(snapshot, "provider_environment", environment)
    monkeypatch.setattr(PL, "sample_host_memory", lambda: HostMemorySample(0, 2**50, 2**50, 0, 0))
    monkeypatch.setattr(
        gateway, "python_command", lambda snapshot, argv, **kwargs: ["test-guarded-transport", json.dumps(argv)]
    )
    return observed
...
def test_source_snapshot_worker_receives_exact_external_spec_pin(tmp_path, monkeypatch):
    ...
    frozen = _isolate_worker_transport(monkeypatch)
    seen = {}
    ...
    monkeypatch.setattr(PL, "acquire_host_resource_lease", lambda stage_root, **kwargs: lease)

    def execute(command, environment, **kwargs):
        seen.update(command=command, environment=environment)
        raise WorkerIntercept

    monkeypatch.setattr(PL, "run_resource_guarded_worker", execute)
    with pytest.raises(WorkerIntercept):
        launcher.main(
            [
                ...
                "--external-objective",
                str(path),
                "--external-objective-sha256",
                digest,
            ]
        )
    assert seen["command"][0] == "test-guarded-transport"
    assert frozen["inputs"] == {"descriptor": descriptor}
    assert seen["environment"]["SYNTHETIC_FROZEN_PROVIDER"] == "selected"
    command = json.loads(seen["command"][1])
    ...
    receipts = list((tmp_path / "out.transport").glob("attempt-*/transport_launch.*.json"))
    assert len(receipts) == 1
    receipt = json.loads(receipts[0].read_text())
    assert receipt["native_argv"] == command
    assert receipt["transport_argv"] == seen["command"]
    assert receipt["stage_output"] == str(tmp_path / "out")
    assert receipts[0].stat().st_mode & 0o222 == 0
...
def test_source_snapshot_worker_receives_ordered_portfolio_and_external_member(tmp_path, monkeypatch):
    ...
    path2, digest2, _ = specification(second_dir, id="external-two", source_text=SOURCE.replace("2xi16", "3xi16"))
    frozen = _isolate_worker_transport(monkeypatch)
    ...
    assert seen["environment"]["SYNTHETIC_FROZEN_PROVIDER"] == "selected"
    command = json.loads(seen["command"][1])
    members = [command[index + 1] for index, value in enumerate(command) if value == "--portfolio-capsule"]
    assert members == ["tiny-llama", "lstmnetvit"]
    paths = [command[index + 1] for index, value in enumerate(command) if value == "--portfolio-external-objective"]
    digests = [
        command[index + 1] for index, value in enumerate(command) if value == "--portfolio-external-objective-sha256"
    ]
