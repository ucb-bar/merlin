...
1. **The policy outgrows one argv entry.** The outer bwrap policy is handed to ``bash -c`` as a single
   ...
   The shared :func:`bwrap.compose_command` spills the policy to a NUL-separated file read via
   ``bwrap --args FD``; there is no separate native-stage transport implementation.
...
_ARG_BYTES = 32 * os.sysconf("SC_PAGE_SIZE")
...
def frozen_over_live(tmp_path: Path) -> dict:
    ...
    return {"live": live, "snap": snap, "kept": kept, "postfreeze": postfreeze, "surfaces": surfaces, "argv": argv}
...
@pytest.mark.skipif(not _SANDBOX.usable, reason=_SANDBOX.describe())
def test_masked_launch_succeeds_and_the_unmapped_mask_is_what_broke_it(frozen_over_live):
    ...
    assert "do-not-leak" not in served and "answer:" not in served, f"the masked golden served content: {served!r}"
    ...
    broken = subprocess.run([*overreach, "bash", "-c", read], capture_output=True, text=True, timeout=120)
...
def _oversized_policy() -> list[str]:
    ...
    argv = _system_binds()
    index = 0
    while len(" ".join(argv)) <= _ARG_BYTES:
...
def test_small_policy_is_inlined_byte_for_byte(tmp_path: Path):
    ...
    assert (
        BW.compose_command(argv, " bash -c 'true'", tmp_path / "workspace")
        == "bwrap --ro-bind /usr /usr bash -c 'true'"
    )
    assert list(tmp_path.iterdir()) == [], "the small path must not touch the filesystem"
...
def test_oversized_policy_spills_to_an_args_fd_and_actually_launches(tmp_path: Path):
    ...
    assert len(inline) > _ARG_BYTES, "the fixture must exceed the cap or it proves nothing"
    ...
    command = BW.compose_command(argv, payload, tmp_path / "workspace")
    spilled = list(tmp_path.glob(".workspace.bwrap-args.*"))
    ...
    assert spill.read_bytes().split(b"\0") == [*[v.encode() for v in argv[1:]], b""]


def test_spill_file_refuses_a_policy_it_cannot_represent(tmp_path: Path):
    with pytest.raises(ValueError, match="bwrap executable"):
        BW.compose_command(["notbwrap", "x" * (_ARG_BYTES + 1)], "", tmp_path / "workspace")
    with pytest.raises(ValueError, match="NUL"):
        BW.compose_command(["bwrap", "--setenv", "K", "a\0b"], "", tmp_path / "workspace")
    assert list(tmp_path.iterdir()) == []


def test_spill_files_are_run_local_and_reuse_identical_policy_bytes(tmp_path: Path):
    """The shared transport retains run-owned policy files, not a separate native cleanup list.

    Repeated identical policy is one file; a changed policy must not overwrite the
    bytes an earlier launch used. Run retention, not this composer, owns removal.
    """
    argv = _oversized_policy()
    workspace = tmp_path / "workspace"
    first = BW.compose_command(argv, " bash -c 'true'", workspace)
    [original] = list(tmp_path.glob(".workspace.bwrap-args.*"))
    payload = original.read_bytes()
    assert BW.compose_command(argv, " bash -c 'true'", workspace) == first
    assert list(tmp_path.glob(".workspace.bwrap-args.*")) == [original]
    changed = BW.compose_command([*argv, "--setenv", "DISTINCT_POLICY", "yes"], " bash -c 'true'", workspace)
    assert changed != first
    assert original.read_bytes() == payload
    assert len(list(tmp_path.glob(".workspace.bwrap-args.*"))) == 2
    assert not workspace.exists(), "policy bytes must remain outside the candidate workspace"
...
def test_a_refusing_tool_probe_reports_the_stderr_it_refused_on(monkeypatch, tmp_path: Path):
    ...
    probe = AW.TC.ToolProbe("python3", "python3 -c 'pass'", "/usr/bin/python3")
    monkeypatch.setattr(
        subprocess,
        "run",
    ...
    with pytest.raises(PS.StageGateError) as excinfo:
        AW.run_required_tool_probes(policy, object(), tmp_path)
