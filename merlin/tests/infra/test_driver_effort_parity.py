...
def _load(name):
    src = module_source_path(f"merlin_experiments.phase1.providers.{name}")
    if not src.is_file():
...
def test_opencode_puts_the_effort_on_the_command_line():
    ...
    src = (module_source_path("merlin_experiments.phase1.providers.opencode_agent")).read_text()
...
def test_the_qa_loop_hands_effort_to_every_driver_it_dispatches():
    ...
    body = inspect.getsource(execution.launch)
