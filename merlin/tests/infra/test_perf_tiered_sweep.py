...
class _Feedback:
    ...
    MINIMUM_REFUTING_PREFIX = DF.DevelopmentGsimFeedback.MINIMUM_REFUTING_PREFIX
    _refuted_so_far = DF.DevelopmentGsimFeedback._refuted_so_far
...
def test_the_sweep_orders_cheapest_measured_member_first():
    ...
    ordered, basis = FM.order_members_by_cost(members, cost)
...
def test_an_unpriced_member_sorts_last_never_first():
    ...
    ordered, _ = FM.order_members_by_cost(members, {"cheap": 10.0})
...
def test_without_history_the_declared_order_stands_and_says_so():
    ...
    ordered, basis = FM.order_members_by_cost(members, {})
...
def test_an_unmeasured_member_is_recorded_not_omitted():
    ...
    row = FM.unmeasured_cell(_Member("PM15_m64n64", family="PM"), reason="already behind")
...
def test_the_unmeasured_cell_matches_the_redacted_schema_exactly():
    ...
    row = FM.unmeasured_cell(_Member("x"), reason="r")
    from merlin.common.paths import module_source_path

    source = module_source_path("merlin_experiments.phase2.corpus_feedback").read_text(encoding="utf-8")
...
def test_harvest_reports_no_cost_rather_than_zero_for_an_absent_history(tmp_path):
    assert FM.harvest_member_cost([tmp_path]) == {}
    assert FM.harvest_member_cost([]) == {}
...
def test_harvest_takes_the_median_of_repeated_measurements(tmp_path):
    ...
    assert FM.harvest_member_cost([tmp_path]) == {"K": 20.0}
