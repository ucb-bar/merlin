...
def fixture(tmp_path, monkeypatch, *, unknown=False, task_count=1, compiler_schema=None, tensor_role="output"):
    ...
    if task_count > 1:
        ...
        accesses = "\n".join(
            f"%g{i} = llvm.getelementptr %p{i}[%index] {{merlin.global_task = {i} : i64}} : (!llvm.ptr, i64) -> !llvm.ptr, i32"
    ...
    sentinel = INPUTS.StageE2ESentinel(
        "fixture-model", str(source), str(source), P2_CONTRACTS.exact_tree_record(source)["sha256"], (), ()
    )
    buffer = {
        "commands": [],
    ...
    if task_count > 1:
        ...
        plan.update(
            ...
            schedule_instruction_count=task_count + 2,
            epilogue_instruction_range=[task_count + 1, task_count + 2],
    ...
    monkeypatch.setattr(
        ...
        lambda *a, **kw: {"arms": {"baseline": {"status": "emitted"}, "candidate": {"status": "emitted"}}},
    )
    monkeypatch.setattr(EA, "inspect_compiler_package", lambda *_: None)
    monkeypatch.setattr(EA, "guidance_for_emission_analysis", lambda *_: {})

    def analyze(cache=None, policy="a" * 64):
        retained = {}
        result = EA.analyze_whole_model_emission(
            baseline,
            candidate,
            sentinel,
            timeout_s=10,
            ...
            baseline_artifacts=cache,
            host_verifier_policy_sha256=policy,
            compiler_api_schema=compiler_schema,
...
def test_host_schema_validates_compiler_that_does_not_self_validate(tmp_path, monkeypatch):
    analyze, emits, _, _, _ = fixture(
        tmp_path,
        monkeypatch,
        compiler_schema=ED.whole_program_schema_record(
            repo_root() / "merlin/contract/schemas/command_buffer.schema.json"
        ),
        tensor_role="invented",
    )
    with pytest.raises(CONTRACTS.StageGateError, match="baseline command buffer violates current compiler API schema"):
        analyze()
...
def test_schema_changed_pin_rejected_before_candidate(tmp_path, monkeypatch):
    record = ED.whole_program_schema_record(repo_root() / "merlin/contract/schemas/command_buffer.schema.json")
    record["sha256"] = "0" * 64
    analyze, emits, _, _, _ = fixture(tmp_path, monkeypatch, compiler_schema=record)
    with pytest.raises(CONTRACTS.StageGateError, match="schema binding changed"):
        analyze()
    assert emits == ["baseline"]
...
def test_baseline_real_plan_verified_once_and_cache_reused(tmp_path, monkeypatch):
    ...
    assert cache["global_plan_evidence_binding"]["evidence_sha256"] == P2_CONTRACTS.document_sha256(proof)
...
def test_actual_multitask_proof_survives_worker_json_roundtrip(tmp_path, monkeypatch):
    ...
    assert len(verifies) == 3
    assert (
        second["diagnostics"]["verified_baseline_global_plan_emission"] == transported["verified_global_plan_emission"]
    )
...
def test_baseline_cache_binding_mutation_fails_closed_before_candidate(tmp_path, monkeypatch, mutation):
    ...
    if mutation in ("source", "policy", "compiler"):
        key = {"source": "source_sha256", "policy": "host_verifier_policy_sha256", "compiler": "compiler_sha256"}[
            mutation
        ]
    ...
    with pytest.raises(CONTRACTS.StageGateError, match="baseline.*identity changed|baseline.*binding changed"):
        analyze(cache)
