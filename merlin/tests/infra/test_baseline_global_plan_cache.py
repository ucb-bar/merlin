...
def fixture(tmp_path, monkeypatch, *, unknown=False, task_count=1, compiler_schema=None, tensor_role="output"):
    ...
    sentinel = PAS.StageE2ESentinel(
        "fixture-model", str(source), str(source), PAS._exact_tree_record(source)["sha256"], (), ()
    )
    buffer = {
        "commands": [],
        ...
        "params": {
            "global_program_plan": {
                ...
                "source_sha256": PAS._sha256(source_text.encode()),
                "source_op_count": 1,
                "tasks": [
                    {
                        "task_index": 0,
    ...
    def analyze(cache=None, policy="a" * 64):
        retained = {}
        result = PAS.analyze_whole_model_emission(
            baseline,
            candidate,
            sentinel,
            timeout_s=10,
...
def test_host_schema_validates_compiler_that_does_not_self_validate(tmp_path, monkeypatch):
    analyze, emits, _, _, _ = fixture(
        tmp_path, monkeypatch, compiler_schema=PAS.whole_program_schema_record(), tensor_role="invented"
    )
...
def test_current_schema_validated_with_real_plan_and_json_cache(tmp_path, monkeypatch):
    analyze, emits, _, _, _ = fixture(
        tmp_path, monkeypatch, compiler_schema=PAS.whole_program_schema_record(), task_count=12
    )
    first, cache = analyze()
    second, _ = analyze(json.loads(json.dumps(cache)))
