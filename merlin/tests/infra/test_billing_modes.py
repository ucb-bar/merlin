...
def test_the_codex_driver_declares_a_subscription_seat():
    assert EX.billing_mode("gpt-5.6-sol", config=EX.ProviderConfig(driver="codex")) == ET.SUBSCRIPTION_NOTIONAL


def test_the_claude_cli_driver_is_billed_by_its_provider():
...
def test_a_driver_that_declares_nothing_is_treated_as_metered():
    """Fail-safe direction: guessing "seat" for a metered driver would hide real spend."""
    assert EX.billing_mode("zai.glm-5", config=EX.ProviderConfig(driver="converse")) == ET.METERED
