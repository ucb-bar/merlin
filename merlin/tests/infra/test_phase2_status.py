...
spec = importlib.util.spec_from_file_location(
    "phase2_status", merlin_dir() / "experiments/gemmini_perf_bench/scripts/render_phase2_status.py"
)
...
def test_terminal_receipt_and_failed_draft_do_not_replace_retained(tmp_path, monkeypatch):
    ...
    final = {
        **old,
        ...
        "context_receipts": [{"path": "short.json"}],
    }
    ...
    seal_digest = write(
        ...
        {
            "candidate_sha256": "b" * 64,
            "iteration_record": str(folder / "iteration_0001.json"),
            "iteration_record_sha256": digest,
        },
    )
    sequence = {
        "status": "budget_complete",
        "checkpoints": [{"path": str(seal_path), "candidate_sha256": "b" * 64, "sha256": seal_digest}],
    }
    ...
    called = []
    monkeypatch.setattr(
        structural_delta, "compare_full_model_structure", lambda a, b: called.append((a, b)) or {"status": "compared"}
    )
    status = S.collect_status(tmp_path, process_reader=lambda *_: [])
    ...
    write(
        folder / "iteration_0001.json",
        {
            ...
            "analysis_reuse": {
                ...
                "source_iteration": 0,
                "source_compilation_iteration": 0,
...
def test_only_exact_snapshot_output_process_is_published(tmp_path):
    ...
    for pid, args in (
        (1, ["python", script, "--output", str(run), "--private", "SECRET"]),
...
def test_process_paths_use_own_cwd_without_publishing_arguments(tmp_path, monkeypatch, mode, expected):
    ...
    args = [
        "python",
        str(cwd / script) if mode == "absolute" else str(script),
        "--output",
        str(run) if mode == "absolute" else "run",
        "--private",
        "SECRET",
    ]
...
def test_initial_failure_is_blocked_not_inferred_terminal_and_cause_is_visible(tmp_path):
    ...
    write(
        tmp_path / "global_iterations/iteration_0000.json",
        {
            "schema": "global_perf_iteration_v1",
            "iteration": 0,
...
def test_launcher_preserves_initial_exception_in_terminal_receipt(tmp_path, stage):
    ...
    if stage == "success":
        assert (
            launcher.run_authoring_with_terminal_receipt(tmp_path, configure=configure, sequence=sequence)["status"]
            == "budget_complete"
        )
