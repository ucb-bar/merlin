...
import pytest
import yaml
from merlin_experiments.phase2.claims import paired as P
...
def _contract(**over):
    base = {
        ...
        "analyzer": P.ANALYZER,
        "roles": list(ROLES),
        "expected_faster": "resident",
        "negative_control_capsule": None,
...
DIRECTION_FALSIFIER = {
    "observation": "paired_cycle_difference",
...
GROWTH_FALSIFIER = {
    "observation": BA.PER_UNIT_GROWTH_OBSERVATION,
...
def test_the_predicted_arm_winning_everywhere_is_established():
    out = P.analyze_paired_claim(_descriptors(_contract()), _rows({"A": (100, 150), "B": (200, 260)}))
    assert out["verdict"] == P.ESTABLISHED
    assert len(out["rows"]) == 2


def test_the_predicted_arm_losing_is_refuted_not_refused():
    out = P.analyze_paired_claim(_descriptors(_contract()), _rows({"A": (150, 100), "B": (200, 260)}))
...
def test_arms_that_tie_within_the_band_refute_a_directional_claim():
    out = P.analyze_paired_claim(_descriptors(_contract()), _rows({"A": (100, 100), "B": (200, 200)}))
    assert out["verdict"] == P.REFUTED


def test_a_contract_with_no_predicted_direction_is_refused():
    out = P.analyze_paired_claim(_descriptors(_contract(expected_faster=None)), _rows({"A": (100, 150)}))
...
def test_an_either_direction_family_needs_only_a_separation():
    out = P.analyze_paired_claim(
        _descriptors(_contract(expected_faster=P.EITHER)), _rows({"A": (150, 100), "B": (200, 260)})
    )
    assert out["verdict"] == P.ESTABLISHED


def test_an_either_family_is_refuted_when_the_arms_are_indistinguishable():
    out = P.analyze_paired_claim(_descriptors(_contract(expected_faster=P.EITHER)), _rows({"A": (100, 100)}))
    assert out["verdict"] == P.REFUTED
...
def test_a_negative_control_that_moves_refuses_the_whole_cohort():
    ...
    out = P.analyze_paired_claim(_descriptors(contract), _rows({"A": (100, 180), "B": (200, 260)}))
    assert out["verdict"] == P.REFUSED
...
@pytest.mark.parametrize("bad", [{"roles": ["only_one"]}, {"roles": "resident"}, {"analyzer": "someone.else/v1"}])
...
def test_every_shipped_paired_family_predicts_a_direction():
    ...
    profile = yaml.safe_load(
        (repo_root() / "experiments/templates/phase0/performance.yaml").read_text(encoding="utf-8")
    )
    checked = 0
    for sweep in profile["sweeps"]:
        ...
        assert (performance.get("falsifier") or {}).get("observation"), (
            f"{sweep['id']} is DIFFERENTIAL and names no falsifier observation, so no measurement could contradict it"
        )
...
def test_a_family_may_not_declare_a_key_its_own_analyzer_does_not_implement():
    ...
    for sweep in profile["sweeps"]:
        ...
        identity = analyzer_identity({"acceptance": acceptance})
        if identity is None:
            continue
        module = load_analyzer_module(identity.module)
        if not hasattr(module, "supported_acceptance"):
            continue
        supported = module.supported_acceptance()
        checked += 1
        extra = sorted(set(acceptance) - set(supported))
        assert not extra, (
            f"{sweep['id']} declares {extra} which {identity.module} does not implement; its preflight "
            f"will refuse every descriptor in the family"
        )
    # PK and PR publish reviewed templates; the generic analyzers validate the
    # caller's declaration in preflight instead of publishing a fixed template.
    assert checked >= 2, "the declaration check must reach both actual analyzer templates"
...
def test_a_constant_saving_passes_the_direction_test_and_still_fails_the_growth_claim():
    ...
    """
    direction_only = P.analyze_paired_claim(_descriptors(_contract()), _rows({"A": (100, 150), "B": (200, 250)}))
...
def test_a_growth_family_with_no_per_unit_counts_refuses_rather_than_passing():
    ...
    out = P.analyze_paired_claim(
        _descriptors(_contract(), falsifier=GROWTH_FALSIFIER), _rows({"A": (100, 150), "B": (200, 500)})
    )
    assert out["verdict"] == P.REFUSED
...
def test_a_direction_falsifier_ignores_per_unit_counts_entirely():
    ...
    out = P.analyze_paired_claim(
        _descriptors(_contract()), _rows({"A": (100, 150), "B": (200, 250)}), per_unit={"A": 1, "B": 15}
    )
...
def test_the_shipped_synchronization_family_declares_the_growth_falsifier():
    ...
    declaring = [
        ...
        if (s["base"]["performance"].get("falsifier") or {}).get("observation") == BA.PER_UNIT_GROWTH_OBSERVATION
    ]
    assert declaring, (
...
EVIDENCE = {"timing_simulator": "gsim", "timing_tier": "L3", "correctness_simulator": "spike", "correctness_tier": "L2"}
NOISE_BAND = {
...
def _preflight_descriptors(contract=None, names=("PZ00", "PZ01"), falsifier=None, noise_band=_DEFAULT):
...
def test_the_module_publishes_exactly_one_preflight_entry_point():
    ...
    names = sorted(name for name in dir(P) if name.startswith("preflight_") and callable(getattr(P, name)))
...
def test_a_pair_with_no_predicted_direction_cannot_be_admitted():
    assert P.preflight_paired_claim(_preflight_descriptors(), replicates=SCHEDULE)["status"] == "READY"

    without = _preflight_descriptors(_preflight_contract(expected_faster=None))
    assert "predicts neither" in _preflight_refusal(P.preflight_paired_claim(without, replicates=SCHEDULE))

    foreign = _preflight_descriptors(_preflight_contract(expected_faster="a_third_arm"))
    assert "predicts neither" in _preflight_refusal(P.preflight_paired_claim(foreign, replicates=SCHEDULE))
...
def test_a_symmetric_family_may_predict_only_that_the_two_differ():
    ...
    result = P.preflight_paired_claim(
        _preflight_descriptors(_preflight_contract(expected_faster=P.EITHER)), replicates=SCHEDULE
    )
...
def test_a_cohort_without_two_distinct_roles_is_refused():
    ...
    for roles in ([ROLES[0]], [ROLES[0], ROLES[0]], [ROLES[0], ROLES[1], "third"]):
        broken = _preflight_descriptors(_preflight_contract(roles=list(roles), expected_faster=ROLES[0]))
...
def test_one_replicate_leaves_the_band_undeterminable_and_is_refused():
    assert P.preflight_paired_claim(_preflight_descriptors(), replicates=SCHEDULE)["status"] == "READY"
    reason = _preflight_refusal(P.preflight_paired_claim(_preflight_descriptors(), replicates=("r000",)))
    ...
    floor_of_one = _preflight_descriptors(noise_band={**NOISE_BAND, "minimum_replicate_count": 1})
    assert "UNDETERMINABLE" in _preflight_refusal(P.preflight_paired_claim(floor_of_one, replicates=SCHEDULE))
...
def test_a_negative_control_must_be_declared_and_must_be_in_the_cohort():
    ...
    undeclared = _preflight_descriptors(falsifier={**DIRECTION_FALSIFIER, "negative_control": ""})
    assert "declares no negative control" in _preflight_refusal(
        P.preflight_paired_claim(undeclared, replicates=SCHEDULE)
    )

    elsewhere = _preflight_descriptors(_preflight_contract(negative_control_capsule="PZ99"))
    assert "not in this cohort" in _preflight_refusal(P.preflight_paired_claim(elsewhere, replicates=SCHEDULE))

    bound = P.preflight_paired_claim(
        _preflight_descriptors(_preflight_contract(negative_control_capsule="PZ00")), replicates=SCHEDULE
    )
...
def test_a_growth_falsifier_needs_more_than_one_pair_and_declares_its_missing_counts():
    ...
    assert "cannot show growth" in _preflight_refusal(P.preflight_paired_claim(one_member, replicates=SCHEDULE))

    result = P.preflight_paired_claim(_preflight_descriptors(falsifier=GROWTH_FALSIFIER), replicates=SCHEDULE)
    assert result["status"] == "READY"
...
def test_a_cohort_spanning_two_families_or_two_questions_is_refused():
    ...
    assert "span 2 families" in _preflight_refusal(P.preflight_paired_claim(mixed, replicates=SCHEDULE))
...
def test_a_contract_naming_another_analyzer_or_another_claim_is_refused():
    ...
    assert "not " + repr(P.ANALYZER) in _preflight_refusal(P.preflight_paired_claim(borrowed, replicates=SCHEDULE))

    predicts = _preflight_descriptors()
    for descriptor in predicts:
        descriptor["performance"]["claim"] = "PREDICTS"
    assert "DIFFERENTIAL claims only" in _preflight_refusal(P.preflight_paired_claim(predicts, replicates=SCHEDULE))
...
def test_the_shipped_families_declaring_this_analyzer_reach_a_preflight_verdict():
    ...
    verdicts = {family: P.preflight_paired_claim(rows, replicates=SCHEDULE) for family, rows in by_family.items()}
...
def test_the_synchronization_family_is_admissible_as_frozen():
    ...
    perf_root = _repo_root() / "merlin/contract/capsules/_perf"
    descriptors = [
        yaml.safe_load((source / "capsule.yaml").read_text(encoding="utf-8"))
        for source in sorted(perf_root.iterdir())
    ...
    growth = [
        ...
        if ((row.get("performance") or {}).get("falsifier") or {}).get("observation") == BA.PER_UNIT_GROWTH_OBSERVATION
    ]
