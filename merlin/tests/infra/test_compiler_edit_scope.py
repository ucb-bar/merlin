...
def setup(tmp_path):
    ...
    contract = {
        ...
        "existing_symbols": [{"surface_id": "compile", "path": "compiler/codegen.py", "symbol": "allowed"}],
        "helper_extensions": [
            {"directory": "compiler", "surface_ids": ["compile"], "reason": "explicit host helper scope"}
        ],
    }
    ...
    if change == "owned":
        ...
        file.write_text(
            file.read_text().replace(
                "return 2" if change == "protected" else "LIMIT=16", "return 7" if change == "protected" else "LIMIT=8"
            )
        )
...
def round_setup(tmp_path):
    ...
    (initial / "manifest.yaml").write_text("entrypoints: {compile: old}\noptimization_surfaces: []\n")
    ...
    contract = _seal(
        {
            ...
            "helper_extensions": [
                {"directory": "compiler", "surface_ids": ["a", "b"], "reason": "host-owned shared helper component"}
            ],
        }
    )
...
def catalog(contract, mechanisms):
    return _seal(
        {"schema": "compiler_mechanism_catalog_v1", "contract_sha256": contract["sha256"], "mechanisms": mechanisms}
    )
...
def test_one_mechanism_may_span_multiple_exact_functions_and_paths(tmp_path):
    ...
    (candidate / "compiler/a.py").write_text(
        (candidate / "compiler/a.py").read_text().replace("def tune_a():\n    return 1", "def tune_a():\n    return 3")
    )
    (candidate / "compiler/b.py").write_text((candidate / "compiler/b.py").read_text().replace("return 1", "return 4"))
    ...
    assert {(row["path"], row["symbol"]) for row in result["semantic_ast_units"]} == {
        ("compiler/a.py", "tune_a"),
        ("compiler/b.py", "tune_b"),
    }
...
def test_two_host_catalogued_mechanisms_in_one_round_are_rejected(tmp_path):
    ...
    frozen = catalog(
        contract,
        [
            {"id": "layout", "selectors": [{"kind": "function", "path": "compiler/a.py", "symbol": "tune_a"}]},
            {"id": "latency", "selectors": [{"kind": "method", "path": "compiler/a.py", "symbol": "Planner.choose"}]},
        ],
    )
    ...
    assert any(row["reason"] == "round contains more than one optimization mechanism" for row in result["violations"])
...
def test_broad_class_authority_does_not_hide_unrelated_method_edit(tmp_path):
    ...
    source = candidate / "compiler/a.py"
    source.write_text(
        source.read_text().replace("def unrelated(self):\n        return 2", "def unrelated(self):\n        return 7")
    )

    result = inspect_round_mechanism_edits(initial, start, candidate, contract, frozen)
    assert result["cumulative_candidate_scope"]["status"] == "allowed"
    assert result["status"] == "refused"
    assert [(row["kind"], row["symbol"], row["mechanism_ids"]) for row in result["semantic_ast_units"]] == [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("imports", {"kind": "imports", "path": "compiler/a.py"}, lambda text: "import math\n" + text),
    ],
)
def test_method_class_and_import_selectors_are_explicit_and_narrow(tmp_path, unit, selector, edit):
    initial, start, candidate, contract = round_setup(tmp_path)
...
def test_module_selector_attributes_but_does_not_expand_cumulative_authority(tmp_path):
    ...
    result = inspect_round_mechanism_edits(initial, start, candidate, contract, frozen)
    assert [(row["kind"], row["mechanism_ids"]) for row in result["semantic_ast_units"]] == [
        ("module", ["module-state"])
    ]
...
def test_round_start_cannot_launder_a_prior_out_of_authority_edit(tmp_path):
    ...
    assert result["cumulative_candidate_scope"]["status"] == "refused"
    assert any(row["reason"] == "round start already exceeds cumulative edit authority" for row in result["violations"])
...
def test_catalog_is_host_bound_and_rejects_kind_mismatch_or_cross_group_overlap(tmp_path):
    ...
    wrong_contract = dict(
        catalog(contract, [{"id": "one", "selectors": [{"kind": "imports", "path": "compiler/a.py"}]}])
    )
    wrong_contract["contract_sha256"] = "0" * 64
    wrong_contract = _seal({key: value for key, value in wrong_contract.items() if key != "sha256"})
