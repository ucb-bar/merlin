...
_LOOP = module_source_path("merlin_experiments.phase1.authoring")
...
def test_qa_grade_grades_a_snapshot_not_the_live_workspace():
    ...
    src = inspect.getsource(loop_grading.grade)
...
def test_continuous_flags_are_exposed():
    ...
    options = parse_options(["--run-id", "probe", "--continuous", "--grade-interval", "30"])
    assert options.continuous is True
    assert options.grade_interval == 30


def _continuous_branch() -> ast.If:
    """The `if a.continuous:` branch in main()."""
    for node in ast.walk(_fn("execute")):
...
def _grader_fn() -> ast.FunctionDef:
    ...
    """
    from merlin_experiments.phase1.feedback import lifecycle

    br = ast.parse(inspect.getsource(lifecycle.start_background))
    for node in ast.walk(br):
        if isinstance(node, ast.FunctionDef) and node.name != "start_background" and "grade_callback" in _calls(node):
...
def test_continuous_runs_one_session_and_grades_on_an_interval():
    ...
    background = ast.parse(inspect.getsource(lifecycle.start_background))
    calls = _calls(background)
    ...
    started_names = {
        n.func.attr for n in ast.walk(background) if isinstance(n, ast.Call) and isinstance(n.func, ast.Attribute)
    }
    ...
    assert "grade_callback" in calls, "continuous mode must grade through the same path a round does"
    assert "_launch" in _calls(br), "continuous mode still runs one real agent session"
...
def test_there_is_a_final_authoritative_grade_after_the_session():
    ...
    outer_grades = [
        ...
        if isinstance(n, ast.Call) and id(n) not in inner and isinstance(n.func, ast.Name) and n.func.id == "_grade"
    ]
    assert outer_grades, (
...
def test_per_capsule_tier_promotion_is_untouched_by_this_mode():
    ...
    runner = module_source_path("merlin.targetgen.capsule_runner").read_text(encoding="utf-8")
