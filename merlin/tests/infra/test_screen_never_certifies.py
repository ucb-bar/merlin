...
_SC = feedback_source("agent_selfcheck")
...
def test_the_rule_names_no_engine_and_no_target():
    ...
    # The classification STATEMENTS, located by what they assign rather than by how they are laid
    # out: an exact spelling made this a hostage of the formatter, which reflowed the assignment.

    s = _src()
    tree = ast.parse(s)

    def _line(predicate):
        return next(node.lineno for node in ast.walk(tree) if predicate(node))

    start = _line(
        lambda n: isinstance(n, ast.Assign) and any(getattr(t, "id", "") == "_blocked_by_selection" for t in n.targets)
    )
    end = _line(lambda n: isinstance(n, ast.AugAssign) and getattr(n.target, "id", "") == "nscreened")
    assert start < end, "the classification no longer precedes the count it feeds"
    block = "\n".join(s.splitlines()[start - 1 : end - 1]).lower()
