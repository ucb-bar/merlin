...
def test_in_repo_compatibility_link_inherits_canonical_provenance_ratchet(tmp_path):
    _repo(tmp_path, "seed = 1\n")
    owner = tmp_path / "examples" / "target" / "measured.yaml"
    owner.parent.mkdir(parents=True)
    owner.write_text(f'origin: "{_PERSONAL}/measured.bin"\n', encoding="utf-8")
    legacy = tmp_path / "legacy.yaml"
    legacy.symlink_to(owner.relative_to(tmp_path))
    (tmp_path / "build_tools/scripts/no_local_paths_ratchet.txt").write_text(
        "examples/target/measured.yaml\n", encoding="utf-8"
    )
...
def test_external_link_cannot_inherit_an_in_repo_ratchet(tmp_path):
    _repo(tmp_path, "seed = 1\n")
    outside = tmp_path.parent / f"{tmp_path.name}-external.yaml"
    outside.write_text(f'origin: "{_PERSONAL}/unreviewed.bin"\n', encoding="utf-8")
    (tmp_path / "legacy.yaml").symlink_to(outside)
    (tmp_path / "build_tools/scripts/no_local_paths_ratchet.txt").write_text(
        "examples/target/measured.yaml\n", encoding="utf-8"
    )
    subprocess.run(["git", "add", "-A"], cwd=tmp_path, check=True, capture_output=True)
    ...
    assert result.returncode == 1 and "legacy.yaml" in result.stderr
