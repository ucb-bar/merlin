...
def _run(cwd=None):
    return subprocess.run([sys.executable, str(GATE)], cwd=cwd or ROOT,
...
def _repo(tmp_path, body: str, name: str = "thing.py"):
    ...
    return subprocess.run([sys.executable, str(scripts / GATE.name)], cwd=tmp_path,
...
def test_a_personal_path_fails_and_the_ratchet_excuses_it(tmp_path):
    ...
    (tmp_path / "build_tools" / "scripts" / "no_local_paths_ratchet.txt").write_text(
        "# debt\nthing.py\n", encoding="utf-8")
    r2 = subprocess.run([sys.executable, str(tmp_path / "build_tools/scripts" / GATE.name)],
...
def test_it_refuses_to_pass_when_it_could_not_look(tmp_path):
    ...
    r = subprocess.run([sys.executable, str(scripts / GATE.name)], cwd=tmp_path,
                       capture_output=True, text=True)
