...
def _trials() -> list[dict]:
    return [{"trial": f"t{index}", "agent_run_id": f"agent-{index}"} for index in (1, 2, 3)]


def _evidence() -> list[dict]:
    return [{**trial, "agent_evidence_sha256": str(index) * 64} for index, trial in enumerate(_trials(), 1)]


def _declaration() -> dict:
    return STATS.predeclare(
        trials=_trials(),
        capsules=[{"family": "shape", "capsule": "wide"}, {"family": "reuse", "capsule": "deep"}],
        replicates=("r2", "r0", "r1"),
    )
...
def test_predeclaration_has_every_exact_identity_and_is_deterministic() -> None:
    ...
    assert set(declaration["matrix"][0]) == {"trial", "subject", "family", "capsule", "simulator", "replicate"}
...
def test_all_trials_feed_paired_geometric_means_and_aggregate_median() -> None:
    ...
    assert result["aggregate"]["uncertainty"] == {
        ...
        "minimum": pytest.approx(2),
        "maximum": pytest.approx(8),
        "median_absolute_deviation": pytest.approx(2),
        "n_independent_trials": 3,
    }
    ...
    for family in ("reuse", "shape"):
        assert result["aggregate"]["family_aggregate"][family]["median_speedup"] == pytest.approx(4)
        assert result["aggregate"]["family_aggregate"][family]["all_trial_speedups"] == pytest.approx([2, 4, 8])
...
def test_generalization_family_regression_cannot_be_masked_by_other_family_gain() -> None:
    ...
    assert result["aggregate"]["family_aggregate"]["reuse"]["median_speedup"] == pytest.approx(2.0)
    assert result["aggregate"]["family_aggregate"]["shape"]["median_speedup"] == pytest.approx(0.5)
...
def test_post_run_evidence_must_cover_every_predeclared_trial_exactly() -> None:
    declaration = _declaration()
    missing = STATS.evaluate(declaration, _rows(declaration), trial_evidence=_evidence()[:-1])
    ...
    duplicate = STATS.evaluate(declaration, _rows(declaration), trial_evidence=repeated)
...
def test_declaration_rejects_too_few_or_nonindependent_trials_and_replicates() -> None:
    with pytest.raises(STATS.EvidenceError, match="at least 3 independent"):
        STATS.predeclare(
            trials=_trials()[:2], capsules=[{"family": "f", "capsule": "c"}], replicates=("r0", "r1", "r2")
        )
    repeated = _trials()
    repeated[2]["agent_run_id"] = repeated[1]["agent_run_id"]
    with pytest.raises(STATS.EvidenceError, match="distinct"):
        STATS.predeclare(trials=repeated, capsules=[{"family": "f", "capsule": "c"}], replicates=("r0", "r1", "r2"))
    with pytest.raises(STATS.EvidenceError, match="at least 2 unique paired"):
        STATS.predeclare(trials=_trials(), capsules=[{"family": "f", "capsule": "c"}], replicates=("r0",))
