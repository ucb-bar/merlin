...
def _main_ast() -> ast.FunctionDef:
    ...
    tree = ast.parse(module_source_path("merlin_experiments.phase1.authoring").read_text(encoding="utf-8"))
...
def _agent_loop(main: ast.FunctionDef) -> ast.While:
    ...
    for node in ast.walk(main):
        if isinstance(node, ast.While) and any(
            isinstance(c, ast.Call) and getattr(c.func, "id", "") == "_launch" for c in ast.walk(node)
        ):
            return node
...
def test_the_continuous_schedule_launches_one_session_not_rounds():
    ...
    launch = next(n for n in ast.walk(loop) if isinstance(n, ast.Call) and getattr(n.func, "id", "") == "_launch")
...
def test_the_background_grader_regrades_on_its_interval_while_the_agent_runs(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(LG, "grade", grade)
    _shrink_waits(monkeypatch)

    h = FL.start_background(
        ws,
        run_dir,
        _args(grade_interval=900),
        interval_grades=True,
        grade_callback=LG.grade,
        fast_grade_callback=LG.fast_grade,
    )
    th, _stop = h
    ...
    assert min(ticks) >= FL.FIRST_BACKGROUND_TICK, (
...
def test_in_turn_snapshots_do_not_collide_across_agent_rounds(tmp_path, monkeypatch):
    ...
    FL.stop_background(first)
    for snap, marker in snapshots:
        marker.chmod(0o444)
        snap.chmod(0o555)

    second = FL.start_background(
        ws,
        run_dir,
        _args(grade_interval=900),
        interval_grades=True,
...
def test_the_rounds_schedule_gets_no_interval_grades(tmp_path, monkeypatch):
    ...
    ws, run_dir = _ws(tmp_path, with_verdict=True)
    ticks = []
    monkeypatch.setattr(LG, "grade", lambda *a, **k: ticks.append(a[2]) or {"all_pass": False})
    _shrink_waits(monkeypatch)
...
def test_the_grader_stops_when_the_session_does(tmp_path, monkeypatch):
    ...
    _driver()
    ws, run_dir = _ws(tmp_path, with_verdict=True)
    monkeypatch.setattr(LG, "grade", lambda *a, **k: {"all_pass": False})
...
def test_a_turn_that_lands_no_first_verdict_is_reported_and_not_graded_on(tmp_path, monkeypatch, capfd):
    ...
    """
    _driver()
    monkeypatch.setattr(FL, "FIRST_GRADE_POLL_S", 0.01)
    ...
    monkeypatch.setattr(LG, "fast_grade", fast)
    monkeypatch.setattr(LG, "grade", lambda *a, **k: interval.append(a[2]) or {"all_pass": False})
    ...
    monkeypatch.setattr(LG, "fast_grade", never)
    with monkeypatch.context() as mp:
        _shrink_waits(mp)
        h = FL.start_background(
            ws2,
...
def test_the_certified_schedule_installs_the_grader_around_the_agent_session():
    ...
    starts = [
        n for n in ast.walk(loop) if isinstance(n, ast.Call) and getattr(n.func, "attr", "") == "start_background"
    ]
    assert starts, (
    ...
    stops = [n for n in ast.walk(loop) if isinstance(n, ast.Call) and getattr(n.func, "attr", "") == "stop_background"]
    assert stops, (
    ...
    authoritative = [n for n in ast.walk(loop) if isinstance(n, ast.Call) and getattr(n.func, "id", "") == "_grade"]
...
def test_the_certified_schedule_still_reaches_the_post_freeze_public_hidden_grade():
    ...
    for needle in ("merlin_experiments.phase1.feedback.formal", "_formal_completion", "_verilator_grade"):
        hits = [
            n.lineno
        ...
        if needle in ("merlin_experiments.phase1.feedback.formal", "_formal_completion"):
