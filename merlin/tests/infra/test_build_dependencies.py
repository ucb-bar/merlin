...
def setup(tmp_path):
    ...
    sandbox = {"command_prefix": prefix, "answer_surfaces": [{"path": str(oracle), "kind": "file", "origin": "oracle"}]}
    argv = ("/usr/bin/python3", str(worker), str(request))
    capability = HostBuildDependencies(
        str(source),
        str(frozen),
        argv,
        tuple((str(p), digest(p)) for p in (Path(argv[0]), worker, request)),
        ((str(helper), str(frozen / "pure.py"), digest(helper)),),
        str(request),
        str(worker),
    )
...
def test_only_canonical_format_pair_is_grantable(setup):
    ...
    cap, policy, _, frozen, _ = setup
    registry, schema = (schemas_dir() / name for name in ("quant_formats.registry.yaml", "quant_format.schema.yaml"))
    rows = tuple(
        (str(p), str(frozen / p.relative_to(repo_root())), digest(p), role)
        for p, role in ((registry, "numeric_format_registry"), (schema, "numeric_format_schema"))
    )
...
def test_format_overlay_not_granted(setup, monkeypatch):
    ...
    with pytest.raises(ValueError, match="overlays"):
        replace(
            cap, format_data=((cap.request_path, cap.request_path, digest(Path(cap.request_path)), "unknown"),)
        ).extend(policy, cap.argv)
...
def test_one_pass_visibility_matches_existing_replay(setup):
    ...
    surfaces = [
        AnswerSurface("ref", Path(policy["answer_surfaces"][0]["path"]), "file", "oracle"),
        AnswerSurface("public", frozen / "existing.py", "file", "test"),
        AnswerSurface("absent", frozen / "absent.py", "file", "test"),
    ]
    for additions in ([], ["--tmpfs", str(frozen)], ["--ro-bind", str(helper), str(surfaces[0].path)]):
...
def test_reuses_real_private_overlay_for_missing_readonly_leaf(setup, tmp_path):
    ...
    served = [
        (Path(src), Path(dest)) for state, src, dest in _mounts(actual) if state == "expose" and Path(dest) == frozen
    ]
