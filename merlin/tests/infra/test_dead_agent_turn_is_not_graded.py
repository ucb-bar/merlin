...
Three failure classes, only two of which had guards. `recovery.round_rejected` covers the five-hour
...
from merlin_experiments.phase1 import recovery as RL
    ...
    def test_a_seat_out_of_credits_is_a_dead_turn(self, tmp_path):
        ...
        p = _write(
            ...
            [
                ...
                {
                    "type": "assistant",
                    "message": {"id": "codex_4_1", "model": "gpt-5.6-sol", "content": []},
            ...
            [
                {
                    ...
                    "message": {
                        "model": "claude-opus-5",
                        "content": [{"type": "text", "text": "I reviewed the ledger and have no change to make."}],
                    },
                },
...
class TestTheLoopActsOnIt:
    def test_the_loop_checks_before_grading_and_does_not_grade(self):
        src = (module_source_path("merlin_experiments.phase1.authoring")).read_text()
        i_check = src.index("ROQ.agent_turn_dead(tpath)")
        i_grade = src.index("verdict = _grade(ws, run_dir, rnd, a.no_oracle, a.qa_timeout)")
    ...
    def test_dead_launch_does_not_replace_last_live_conformance_evidence(self, tmp_path):
        ...
        assert RL.latest_live_authoring_transcript([live, dead]) == live
