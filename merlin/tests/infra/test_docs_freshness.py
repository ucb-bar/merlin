...
def test_docs_aggregator_preserves_actionable_failure_details(monkeypatch, tmp_path):
    import importlib.util

    spec = importlib.util.spec_from_file_location("docs_gate_diagnostic_test", SCRIPTS / "check_docs.py")
    gate = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gate)
    monkeypatch.setattr(gate, "ROOT", tmp_path)
    monkeypatch.setattr(gate, "CHECKS", [("docs freshness", "check_docs_freshness.py")])
    monkeypatch.setattr(
        gate.subprocess,
        "run",
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args, 1, stdout="repair: update code_refs\n", stderr="schema FAILED:\n  - guide.md: missing module.py\n"
        ),
    )
    [diagnostic] = gate.collect()
    assert "guide.md: missing module.py" in diagnostic
    assert "repair: update code_refs" in diagnostic


def test_docs_aggregator_bounds_long_failures_and_names_the_checker(monkeypatch, tmp_path):
    import importlib.util

    spec = importlib.util.spec_from_file_location("docs_gate_bounded_test", SCRIPTS / "check_docs.py")
    gate = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(gate)
    monkeypatch.setattr(gate, "ROOT", tmp_path)
    monkeypatch.setattr(gate, "CHECKS", [("docs freshness", "check_docs_freshness.py")])
    monkeypatch.setattr(
        ...
        "run",
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args, 1, stdout="", stderr="\n".join(f"missing source {i}" for i in range(25))
        ),
    )
    [diagnostic] = gate.collect()
    assert "missing source 19" in diagnostic
    assert "missing source 20" not in diagnostic
    assert "5 more lines; run check_docs_freshness.py --check" in diagnostic
...
def test_current_only_ratchet_reports_drafts_without_certifying_them(tmp_path):
    """CI may gate maintained docs while strict release audit still catches stale drafts."""
    import shutil
    ...
    for name, status in (("maintained", "current"), ("work_in_progress", "draft")):
    ...
    for args in (
        ...
        subprocess.run(["git", *args], cwd=tmp_path, check=True, capture_output=True)
    gate = [sys.executable, str(scripts / "check_docs_freshness.py"), "--ratchet"]
    scoped = subprocess.run([*gate, "--current-only"], cwd=tmp_path, capture_output=True, text=True)
    assert scoped.returncode == 1 and "maintained.md" in scoped.stderr
    assert "work_in_progress.md" not in scoped.stderr and "UNVERIFIED" in scoped.stdout
    (docs / "maintained.md").write_text((docs / "maintained.md").read_text().replace("2000-01-01", "2999-01-01"))
    scoped = subprocess.run([*gate, "--current-only"], cwd=tmp_path, capture_output=True, text=True)
    strict = subprocess.run(gate, cwd=tmp_path, capture_output=True, text=True)
    assert scoped.returncode == 0 and "UNVERIFIED" in scoped.stdout
    assert strict.returncode == 1 and "work_in_progress.md" in strict.stderr
