...
def test_host_protection_is_owner_only_never_mode_zero():
    ...
    assert chmod_calls, "host protection must actually set owner-only permissions"
    modes = {
        node.value
...
def _input_lock_fixture(tmp_path, monkeypatch, *, allowed=(), host_inputs=()):
    sys.path.insert(0, str(HARNESS))
    preflight = importlib.import_module("preflight")
    repo = tmp_path / "repo"
    bundle_dir = repo / "bundles" / "arm"
    bundle_dir.mkdir(parents=True)
    (bundle_dir / "input_bundle_manifest.yaml").write_text(
        preflight.yaml.safe_dump(
            {
                "bundle_id": "arm",
                "allowed": [{"path": str(path)} for path in allowed],
                "host_inputs": [{"path": str(path)} for path in host_inputs],
            }
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(preflight.C, "REPO", repo)
    monkeypatch.setattr(preflight.C, "BUNDLES", bundle_dir.parent)
    return preflight, repo, bundle_dir / "bundle_lock.yaml"


def test_bundle_lock_keeps_absolute_external_grant_identity(tmp_path, monkeypatch):
    external = tmp_path / "oot" / "isa.h"
    external.parent.mkdir()
    external.write_bytes(b"external hardware interface")
    preflight, _, lock_path = _input_lock_fixture(tmp_path, monkeypatch, allowed=[external])

    result = preflight.check_bundle_hash_repro()["arm"]
    lock = preflight.yaml.safe_load(lock_path.read_text())

    assert result["reproducible"] is True
    assert result["unresolvable"] == []
    assert lock == {
        "allowed_tree_sha256": {
            str(external): preflight._hash_granted_path(external)["sha256"],
        }
    }


def test_bundle_lock_missing_public_grant_is_not_reproducible(tmp_path, monkeypatch):
    preflight, _, lock_path = _input_lock_fixture(tmp_path, monkeypatch, allowed=["absent.h"])

    result = preflight.check_bundle_hash_repro()["arm"]

    assert result["reproducible"] is False
    assert result["unresolvable"] == ["absent.h"]
    assert preflight.yaml.safe_load(lock_path.read_text())["unresolvable_grants"] == ["absent.h"]


def test_bundle_lock_checks_private_bytes_without_publishing_them(tmp_path, monkeypatch):
    private_name = "private-held-out-inputs"
    preflight, repo, lock_path = _input_lock_fixture(tmp_path, monkeypatch, host_inputs=[private_name])
    private = repo / private_name
    private.mkdir()
    (private / "secret-case-name").write_bytes(b"private values")
    real_hash = preflight._hash_granted_path
    calls = []

    def changing_hash(path):
        calls.append(path)
        record = real_hash(path)
        (private / "secret-case-name").write_bytes(b"changed between independent passes")
        return record

    monkeypatch.setattr(preflight, "_hash_granted_path", changing_hash)
    result = preflight.check_bundle_hash_repro()["arm"]

    assert result["reproducible"] is False
    assert result["n_host_inputs"] == 1
    assert result["unresolvable_host_inputs_count"] == 0
    assert calls == [private, private]
    assert private_name not in str(result) + lock_path.read_text()
    assert "secret-case-name" not in str(result) + lock_path.read_text()
    assert preflight.yaml.safe_load(lock_path.read_text()) == {"allowed_tree_sha256": {}}


def test_bundle_lock_missing_host_input_has_no_shorthand_fallback(tmp_path, monkeypatch):
    private_name = "private-held-out-inputs"
    preflight, repo, lock_path = _input_lock_fixture(tmp_path, monkeypatch, host_inputs=[private_name])
    alternate = repo / "merlin" / private_name
    alternate.mkdir(parents=True)
    (alternate / "secret").write_bytes(b"must not substitute another private tree")

    result = preflight.check_bundle_hash_repro()["arm"]

    assert result["reproducible"] is False
    assert result["n_host_inputs"] == result["unresolvable_host_inputs_count"] == 1
    assert private_name not in str(result) + lock_path.read_text()


def test_bundle_lock_accepts_stable_absolute_host_input(tmp_path, monkeypatch):
    private = tmp_path / "external-private"
    private.write_bytes(b"sealed host input")
    preflight, _, lock_path = _input_lock_fixture(tmp_path, monkeypatch, host_inputs=[private])
    ...
    assert result["n_host_inputs"] == 1
    assert result["unresolvable_host_inputs_count"] == 0
    assert str(private) not in str(result) + lock_path.read_text()
