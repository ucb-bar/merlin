...
from merlin_experiments.phase2 import heldout_qualification as QUAL
...
def _capsule(root: Path, name: str, *, family: str, cohort: str, m: int, n: int, k: int) -> tuple[Path, dict]:
    ...
    return manifest, {"name": name, "path": f"_perf/{name}", "family": family, "cohort": cohort, "M": m, "N": n, "K": k}


def _reveal(root: Path) -> tuple[Path, str, str]:
    _, pk = _capsule(root, "PKH00_k17", family="PK", cohort="PK_predictor", m=16, n=16, k=17)
    _, pkg = _capsule(root, "PKG00_m32k31n48", family="PKG", cohort="PK_MNK_generalization", m=32, n=48, k=31)
    ...
    manifest_sha = RC.sha_file(manifest)
...
def _tuning(tmp_path: Path):
    ...
    for name in sorted(QUAL.GATE.REQUIRED_PINS):
        ...
        pins[name] = {"path": str(artifact.resolve()), "sha256": RC.sha_file(artifact)}
    ...
    return SimpleNamespace(
        target="gemmini",
        path=source.resolve(),
        sha256=RC.sha_file(source),
        pins=pins,
        members={identity: {"workload_sha256": identity}},
        document={
            "members": [{"workload_sha256": identity}],
            "unresolved": [],
            "pins": pins,
            # Relative on purpose: qualification must rebase it to its validated absolute path.
            "build_binding": {"path": receipt.name, "sha256": RC.sha_file(receipt), "commands_sha256": "b" * 64},
        },
    )
...
def _capture_document(manifest: Path, pins: dict, *, target: str = "gemmini") -> dict:
    ...
    output_rows = [{"name": "Y0", "shape": [1], "dtype": "i32", "n_bytes": 4, "sha256": output_sha}]
    ...
    return {
        ...
        "target": target,
        "capsule": manifest.parent.name,
        "capsule_manifest_path": str(manifest.resolve()),
        "capsule_manifest_sha256": RC.sha_file(manifest),
        "workload": workload,
...
def _valid_tuning(tmp_path: Path):
    ...
    model_manifest = QUAL.PRODUCER.write_model_manifest(model, ["ChipTop.h"], tmp_path / "model-manifest.json")
    artifacts = {}
    ...
    paths = QUAL.PRODUCER.ArtifactPaths(
        artifacts["gsim_firrtl"],
        artifacts["verilator_firrtl"],
        model_manifest,
        artifacts["gsim_binary"],
        artifacts["verilator_binary"],
    )
    support = {}
    ...
    receipt = QUAL.PRODUCER.write_build_receipt(
        ...
        emitter=support["emitter"],
        cxx_wrapper=support["wrapper"],
        cxx_compiler=support["compiler"],
        inputs=[("harness", support["harness"])],
        commands=[
            {"stage": "elaborate", "cwd": str(tmp_path), "argv": ["elaborate"]},
            {"stage": "emit", "cwd": str(tmp_path), "argv": [str(support["emitter"])]},
            {"stage": "compile", "cwd": str(tmp_path), "argv": [str(support["wrapper"]), "-c"]},
            {"stage": "link", "cwd": str(tmp_path), "argv": [str(support["compiler"])]},
        ],
    )
    tuning_root = tmp_path / "tuning"
    manifest, _ = _capsule(tuning_root, "PK00_k16", family="PK", cohort="PK_predictor", m=16, n=16, k=16)
    pins = paths.pinned()
...
def test_reveal_loader_uses_both_manifest_declared_cohorts_and_no_globbed_extra(tmp_path: Path) -> None:
    reveal, manifest_sha, corpus_sha = _reveal(tmp_path / "heldout")
    members = RC.load_revealed_members(
        reveal, expected_manifest_sha256=manifest_sha, expected_corpus_sha256=corpus_sha, expected_target="gemmini"
    )
    assert [(row.family, row.cohort) for row in members] == [("PK", "PK_predictor"), ("PKG", "PK_MNK_generalization")]

    # Even a well-formed capsule is not silently admitted when it is absent from the reveal manifest.
    root = reveal.parent
    root.chmod(0o700)
    (root / "_perf").chmod(0o700)
    _capsule(root, "not_declared", family="PKG", cohort="PK_MNK_generalization", m=16, n=16, k=29)
    for path in sorted(root.rglob("*"), key=lambda item: len(item.parts), reverse=True):
        path.chmod(0o500 if path.is_dir() else 0o400)
    root.chmod(0o500)
    with pytest.raises(RC.QualificationError, match="committed tree"):
        RC.load_revealed_members(reveal, expected_manifest_sha256=manifest_sha)
...
def test_offline_provider_lowers_frozen_baseline_and_emits_resume_adoptable_extension(
    ...
    def capturer(**kwargs):
        ...
        return {
            "workload": workload,
            "workload_sha256": identity,
            "capsule_manifest_path": str(manifest),
            "capsule_manifest_sha256": RC.sha_file(manifest),
        }
    ...
    def load_certificate(path, *, expected_sha256):
        assert RC.sha_file(Path(path)) == expected_sha256
    ...
    certificate, digest = QUAL.qualify_revealed_holdout(
        ...
        reveal_manifest_sha256=manifest_sha,
        reveal_corpus_sha256=corpus_sha,
        timeout=37,
        lowerer=lowerer,
        capturer=capturer,
        backend="injected-offline-backend",
    ...
    assert [event[:2] for event in events] == [
        ("lower", "PKH00_k17"),
        ("capture", "PKH00_k17"),
        ("lower", "PKG00_m32k31n48"),
        ("capture", "PKG00_m32k31n48"),
    ]
    ...
    with pytest.raises(RC.QualificationError, match="differs from this experiment"):
...
def test_pinned_runtime_binds_declared_cap_and_restores_ambient_state(
    ...
    class Backend:
        ...
        def pinned_runtime(self, *, binaries, gsim_max_cycles):
            assert binaries == {
                engine: Path(tuning.pins[f"{engine}_binary"]["path"]) for engine in ("gsim", "verilator")
            }
            with monkeypatch.context() as context:
                context.setenv("SYNTHETIC_ENGINE", str(binaries["gsim"]))
                context.setenv("SYNTHETIC_CYCLES", str(gsim_max_cycles))
                yield self
    ...
    monkeypatch.setenv("SYNTHETIC_ENGINE", "/ambient/wrong")
    monkeypatch.setenv("SYNTHETIC_CYCLES", "17")

    with QUAL._pinned_runtime(tuning, gsim_max_cycles=100_000_000) as selected:
        assert isinstance(selected, Backend)
        assert os.environ["SYNTHETIC_ENGINE"] == tuning.pins["gsim_binary"]["path"]
        assert os.environ["SYNTHETIC_CYCLES"] == "100000000"

    assert os.environ["SYNTHETIC_ENGINE"] == "/ambient/wrong"
    assert os.environ["SYNTHETIC_CYCLES"] == "17"

    for invalid in (0, -1, True, "100000000"):
        with pytest.raises(RC.QualificationError, match="positive integer or null"):
...
def test_resume_refuses_partial_qualification_root(tmp_path: Path) -> None:
    ...
    with pytest.raises(RC.QualificationError, match="no unique completion receipt"):
        QUAL.load_completed_qualification(
            root,
            tuning=SimpleNamespace(sha256="a" * 64),
            reveal_manifest_sha256="b" * 64,
            reveal_corpus_sha256="c" * 64,
            functional_base_sha256="d" * 64,
            gsim_max_cycles=100_000_000,
        )
...
def test_extension_merge_is_accepted_by_real_strict_gate(tmp_path: Path) -> None:
    ...
    revealed = RC.load_revealed_members(reveal)
    assert set(extension.members) == set(tuning.members) | {member.workload_sha256 for member in revealed}
    assert extension.document["build_binding"]["sha256"] == tuning.document["build_binding"]["sha256"]
