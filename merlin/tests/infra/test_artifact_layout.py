...
def test_start_run_lands_under_target_suite(tmp_path):
    h = A.start_run(suite="perf-bench", method="m0", seed=2, target="gemmini", project_root=tmp_path)
...
def test_new_products_use_declared_roots_without_rewriting_old_evidence(tmp_path, monkeypatch):
    from merlin.common import storage_cli

    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "out"))
    old = tmp_path / "out/artifacts/perf-ledger/historical/manifest.yaml"
    old.parent.mkdir(parents=True)
    old.write_text("historical evidence\n")
    product = A.new_product("perf-ledger", version=1, target="fixture")
    assert product.path.parent == tmp_path / "out/artifacts/perf-studies/ledger/fixture/v1"
    assert product.topic == "perf-ledger" and product.path.name.startswith("perf-ledger_fixture_")
    assert old.read_text() == "historical evidence\n"
    assert not old.parent.is_symlink()
    assert (product.path.parent / "latest").resolve() == product.path
    folds = {(source, destination) for source, destination, _ in storage_cli.planned_folds()}
    assert (tmp_path / "out/artifacts/perf-ledger", tmp_path / "out/artifacts/perf-studies/ledger") in folds


@pytest.mark.parametrize("destination", ["../escape", "/absolute", "a/../b", "a//b", "", None, "a\\b"])
def test_invalid_product_destination_refuses_before_writes(tmp_path, monkeypatch, destination):
    from merlin.common import storage_cli

    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "out"))

    def declared():
        return {"product_roots": {"fixture": destination}}

    monkeypatch.setattr(A, "storage_contract", declared)
    monkeypatch.setattr(storage_cli, "contract", declared)
    with pytest.raises(ValueError):
        A.new_product("fixture", version=1)
    with pytest.raises(ValueError):
        storage_cli.planned_folds()
...
def test_linter_flags_generated_output_inside_merlin(tmp_path):
    ...
    _run(["git", "init", "-q"], cwd=tmp_path)
    for rel in ("merlin/experiments/e/reports/r.md", "merlin/benchmarks/b/case_study/c.yaml"):
        p = tmp_path / rel
...
def _hook(tool, path):
    ...
    return subprocess.run([sys.executable, str(HOOK)], input=payload, capture_output=True, text=True, cwd=REPO)
...
@pytest.mark.parametrize(
    ...
    [
        "output/foo.png",
        "results/x_dse_analysis/r.csv",
        "selfcheck_out/log.jsonl",
        "merlin/x.svg",
        "artifacts/plots/foo.png",  # retired top-level roots now denied (out/ only)
    ],
)
...
@pytest.mark.parametrize(
    "path",
    [
        "out/artifacts/presentation/set-a/foo.png",
        "out/runs/gemmini/s/r/perf_results.json",
        "merlin/python/merlin/x.py",
        "output/AGENT.md",
        "out/build/x.o",
    ],
)
...
def test_hook_still_allows_writes_when_the_roster_cannot_be_read(tmp_path):
    ...
    payload = json.dumps({"tool_name": "Write", "tool_input": {"file_path": "out/artifacts/anything/a/b.json"}})
    env = {**os.environ, "MERLIN_REPO_ROOT": str(tmp_path)}
    result = subprocess.run(
        [sys.executable, str(HOOK)], input=payload, capture_output=True, text=True, cwd=REPO, env=env
    )
