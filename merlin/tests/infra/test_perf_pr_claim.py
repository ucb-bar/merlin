...
COUNTERS = OccupancyCounters(
    prefix="X",
    engines=("A", "B"),
    by_combination={frozenset({"A"}): "A_CYC", frozenset({"B"}): "B_CYC", frozenset({"A", "B"}): "AB_CYC"},
)
...
def _partition(counters: OccupancyCounters = None, codes: dict = None) -> dict:
    ...
    event = {}
    for index, (combo, _name) in enumerate(sorted(counters.by_combination.items(), key=lambda item: sorted(item[0]))):
        event[combo] = f"%event_{index}"
        operands = [busy[engine] if engine in combo else idle[engine] for engine in counters.engines]
        lines.append(f"  %event_{index} = comb.and bin {', '.join(operands)} : i1")
    ports = [
        f"io_event_io_event_signal_{codes[name]}: {event[combo]}: i1" for combo, name in counters.by_combination.items()
    ]
...
def test_the_real_corpus_preflights_ready_with_three_derived_bands():
    ...
    frozen = [
        yaml.safe_load((path / "capsule.yaml").read_text())
        for path in sorted(root.iterdir())
        if path.is_dir()
        and (path / "capsule.yaml").is_file()
        and yaml.safe_load((path / "capsule.yaml").read_text()).get("performance", {}).get("family") == "PR"
    ]
    assert len(frozen) == 9
...
def test_a_band_of_two_depths_cannot_carry_its_own_negative_control():
    ...
    by_regime = {band: {"points": [{"K": k} for k in depths]} for band, depths in bands.items()}
    descriptors = [
        _descriptor(i, band, k, by_regime)
        for i, (band, k) in enumerate((band, k) for band, depths in bands.items() for k in depths)
    ]
...
def test_established_when_every_boundary_changes_the_rate(descriptors):
    ...
    decision = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
...
def test_refuted_when_one_boundary_leaves_the_rate_unchanged(descriptors):
    ...
    assert [(row["lower_band"], row["upper_band"]) for row in fired] == [("fits_double", "fits_single")]
...
def test_every_band_in_the_transient_leaves_no_boundary_to_compare(descriptors):
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=lambda band, step: _filling(step))
    decision = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    assert decision["status"] == RC.REFUSED
    assert decision["verdict"]["bands_refused_for_transient_reasons"] == ["fits_double", "fits_single", "spills"]
...
def test_absent_counters_refuse_rather_than_skip_the_transient_guard(descriptors):
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=_settled_everywhere)
    decision = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=None, partition=PARTITION)
    assert decision["status"] == RC.REFUSED
...
def test_inert_when_no_band_can_show_its_own_two_ranges_agreeing(descriptors):
    ...
    assert all(band["status"] == RC.BAND_CONTROL_DID_NOT_FIRE for band in decision["verdict"]["bands"])
...
def test_the_decision_does_not_depend_on_the_order_rows_or_descriptors_arrive_in(descriptors):
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=_settled_everywhere)
    forward = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    reverse = PR.analyze_pr_claim(
        list(reversed(descriptors)), list(reversed(rows)), replicates=REPLICATES, counters=COUNTERS, partition=PARTITION
    )
...
def test_the_negative_control_evidence_reaches_the_campaign_judge(descriptors):
    ...
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=_settled_everywhere)
    decision = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    evidence = PR.falsifier_evidence(
        decision, member_of=lambda band: ReplicaIdentity(family="PR", member=band, tier="L3", replica="r000")
    )
...
def test_the_derived_counter_mapping_is_accepted_and_a_useless_one_is_refused(descriptors):
    ...
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=_settled_everywhere)
    live = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    from_dict = PR.analyze_pr_claim(
        descriptors, rows, replicates=REPLICATES, counters=COUNTERS.to_dict(), partition=PARTITION
    )
    assert from_dict == live
    assert from_dict["status"] == RC.ESTABLISHED

    one_engine = {"prefix": "X", "engines": ["A"], "by_combination": {"A": "A_CYC"}}
    refused = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=one_engine, partition=PARTITION)
...
def test_removing_the_transient_guard_would_flip_the_transient_refusal(descriptors, monkeypatch):
    ...
    rows = _rows(descriptors, cycles_of=_affine(RATES), overlap_of=overlap_of)
    guarded = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    assert guarded["verdict"]["bands_refused_for_transient_reasons"] == ["fits_double"]

    real = RC._ft.transient_verdict
    monkeypatch.setattr(RC._ft, "transient_verdict", lambda points: {**real(points), "state": RC._ft.SATURATED})
    relaxed = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
...
def test_introducing_a_tolerance_would_flip_established_into_refuted(descriptors, monkeypatch):
    ...
    rows = _rows(descriptors, cycles_of=cycles_of, overlap_of=_settled_everywhere)
    exact = PR.analyze_pr_claim(descriptors, rows, replicates=REPLICATES, counters=COUNTERS, partition=PARTITION)
    assert exact["status"] == RC.ESTABLISHED
    boundary = next(
        row
        for row in exact["verdict"]["boundaries"]
