...
from merlin.targetgen import rtl_check_compiler
...
from merlin.targetgen.rtl.circt_introspect import (
    _boolean_feature_gate,
    extract_boolean_feature_gate,
...
def test_max_pool_capability_is_read_from_elaborated_firrtl_gate(literal, supported):
    ...
    """
    assert extract_boolean_feature_gate(firrtl, gate=_boolean_feature_gate("gemmini")) == {
        "value": supported,
        "line": 4,
...
def test_isa_pool_fields_without_an_elaborated_gate_do_not_claim_capability():
    assert (
        extract_boolean_feature_gate(
            ...
            gate=_boolean_feature_gate("gemmini"),
        )
        is None
    )
...
def test_pool_gate_in_a_decoy_module_does_not_claim_store_controller_capability():
    ...
    """
    assert extract_boolean_feature_gate(firrtl, gate=_boolean_feature_gate("gemmini")) is None


def test_max_pool_capability_comes_from_the_elaborated_rtl_build_configuration():
    facts = load_facts("gemmini")
    feature = next(i for i in facts["facts"]["interfaces"] if i.get("name") == "elaborated_rtl_features")
...
def test_pool_emitter_preserves_compiled_out_vs_unknown_rtl_evidence(supported, message, monkeypatch):
...
def test_pool_capacity_uses_total_accumulator_rows_across_all_rtl_banks():
    ...
    next(c for c in cb["commands"] if c["opcode"] == "COMMIT")["attributes"]["pool_in_dims"] = [side, side]
    gm._isa.cache_clear()
    ...
    next(c for c in too_large["commands"] if c["opcode"] == "COMMIT")["attributes"]["pool_in_dims"] = [
        overflow_side,
        overflow_side,
    ]
...
def test_conv_pool_reuses_the_im2col_matmul_backend_and_all_l0_engines_agree():
    ...
    normalized = gm._normalize_command_buffer(cb)
    assert [c["opcode"] for c in normalized["commands"]] == ["RES_PACK", "MATMUL_RESIDENT", "COMMIT", "EVICT"]
...
def test_native_pooling_trace_satisfies_each_generated_capsule_contract(name):
    ...
    assert check(trace, expected, cb, address_model="pointer_args") == {"status": "pass", "violations": []}
    ...
    checks = rtl_check_compiler.compile_trace_checks(load_facts("gemmini"), capsule, target="gemmini")
