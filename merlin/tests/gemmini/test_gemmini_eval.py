...
def test_record_legacy_c_backend_still_records_kernel(tmp_path):
    ...
    summ = record_gemmini_run("C0", "spike", runs_root=tmp_path, codegen_backend="legacy_c", result=_result(True))
...
def test_record_fail_writes_failure_record(tmp_path):
    ...
    summ2 = record_gemmini_run(
        "C0",
        "spike",
        runs_root=tmp_path,
        result={**_result(False), "oracle": {"kind": "spike_gemmini_functional", "derived_from_rtl": False}},
    )
    fr2 = json.loads((Path(summ2["run_path"]) / "logs" / "failures.jsonl").read_text().splitlines()[0])
...
def test_dispatcher_cartesian_and_resume(tmp_path):
    ...
    rows = run_sweep(
        ["C0", "C1"], ["spike"], runs_root=tmp_path / "runs", ledger_path=ledger, result_fn=_fake_result_fn(True)
    )
...
def test_dispatcher_reruns_failures(tmp_path):
    ledger = tmp_path / "ledger.jsonl"
    rows = run_sweep(
        ["C0"], ["spike"], runs_root=tmp_path / "runs", ledger_path=ledger, result_fn=_fake_result_fn(False)
    )
