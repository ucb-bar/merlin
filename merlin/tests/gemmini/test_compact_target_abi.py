...
def documents():
    ...
    contract = {
        ...
        "bases": [{}, {}],
        "bindings": [{"argument_index": 0, "base_index": 0}, {"argument_index": 1, "base_index": 1}],
    }
    return cb, contract
...
@pytest.mark.parametrize(
    "layout", ["e-i64:64", "e-p:64:64-p0:64:64", "e-p:64:64:64:128", "e-p:64:no", "e-p:64:64-ni:0", "e-p:64:64-A1"]
)
...
def test_actual_earlier_header_is_selected_and_wrong_element_width_refuses(
    ...
    monkeypatch.setattr(
        backend, "harness_build_recipe", lambda: replace(recipe, include_roots=(include.parent, *recipe.include_roots))
    )
