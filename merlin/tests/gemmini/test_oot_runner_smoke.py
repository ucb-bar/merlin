...
def test_native_package_certifies(ex, tmp_path):
    res = certify(NATIVE, EX / f"{ex}.interface.mlir", runs_root=tmp_path, run_id=f"nat_{ex}", simulator="spike")
...
def test_cpp_package_builds_and_certifies(tmp_path):
    res = certify(
        CPP, EX / "g0_matmul.interface.mlir", runs_root=tmp_path, run_id="cpp_g0", simulator="spike", timeout=1800
    )
...
def test_cpp_verilator_certification(tmp_path):
    res = certify(
        CPP, EX / "g1_relu.interface.mlir", runs_root=tmp_path, run_id="cpp_g1_rtl", simulator="verilator", timeout=1800
    )
    assert res["status"] == "pass"
    assert res["oracle"]["derived_from_rtl"] is True
