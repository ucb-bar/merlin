...
def _f32_kernel() -> str:
    ...
    for r in range(ROWS):
        for c in range(COLS):
            i = r * PITCH + c
            out += [
                f"    %k{i} = llvm.mlir.constant({i} : i64) : i64",
...
def _cb(dtype: str) -> dict:
    ...
    return {
        ...
        "backend": "test",
        "tensors": {
            "arg0": {"shape": [ROWS, COLS], "dtype": dtype, "role": "input"},
...
def test_a_float_host_lane_result_reaches_the_grader_as_values(dtype, kernel, tmp_path):
    ...
    res = oot.run_on_oracle(
        _cb(dtype), kernel(), simulator="spike", target="gemmini", workdir=str(tmp_path), timeout=900
    )
