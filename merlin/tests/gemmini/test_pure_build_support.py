...
def fixture(dtype, explicit):
    tensors = {
        name: {"shape": [2, 3], "dtype": dtype, "role": "input" if name == "A" else "output"} for name in ("A", "Y")
    }
    cb = {
        "abi_version": "0.1",
        "target": "gemmini",
...
def pure_package_path():
    return explicit_targets()["gemmini"] / "build_support/__init__.py"


def pure_package():
    return load_build_package(pure_package_path())
...
def test_pure_default_is_compute_only_warm_one_without_backend_imports():
    ...
    request = json.dumps({"cb": cb, "inputs": inputs, "package": str(pure_package_path())})
...
"""
    result = subprocess.run([sys.executable, "-c", program], input=request, text=True, capture_output=True, timeout=15)
...
def test_pure_legacy_exact_input_path_matches_original_bytes_without_materialization(dtype, monkeypatch):
    ...
    monkeypatch.setattr(
        commandbuffer, "materialize_inputs", lambda *a, **k: pytest.fail("exact inputs cannot invoke materialization")
    )
...
def test_measurement_final_assembly_does_not_discover_or_guess_counters():
    pure = pure_package()
    fragments = pure.assemble_measurement_fragments(
        "  call();  ", requested="warm", include="header\n", prologue="reset\n", epilogue="snapshot\n"
    )
