...
def _codegen():
    ...
    pkg.__path__ = [str(_support_root() / "backend")]
    sys.modules["gback"] = pkg
    module = importlib.import_module("gback.gemmini_codegen_mlir")
    assert Path(module.__file__).resolve() == (_support_root() / "backend/gemmini_codegen_mlir.py").resolve()
    return module
