...
@pytest.fixture
def synthetic_build_dependencies(tmp_path_factory, monkeypatch):
    """Real byte-bound dependencies without requiring or executing compiler tools."""
    from merlin.llvmlower import toolchain
    from merlin.runtime.backends import base

    root = tmp_path_factory.mktemp("synthetic-build-dependencies")
    tools = {}
    for name in ("clang", "mlir-translate", "target-cc"):
        path = root / name
    ...
    include.mkdir()
    (include / "gemmini_params.h").write_text("#define DIM 7\n")
    support, linker = root / "startup.S", root / "link.ld"
    support.write_text("# synthetic startup identity; never assembled\n")
    linker.write_text("/* synthetic linker identity; never linked */\n")
    original = base.harness_build_recipe("gemmini")
    recipe = replace(
        original,
        compiler=tools["target-cc"],
        include_roots=(include,),
        support_sources=(support,),
        link_script=linker,
    )
    monkeypatch.setattr(toolchain, "clang", lambda: tools["clang"])
    monkeypatch.setattr(toolchain, "mlir_translate", lambda: tools["mlir-translate"])
...
def request():
    ...
    cb = {
        ...
        "commands": [],
        "tensors": {
            "a": {"shape": [1], "dtype": "i32", "role": "input"},
            "out": {"shape": [1], "dtype": "i32", "role": "output"},
        },
        "kernel_abi": {
            "kind": "whole_program",
            "args": [{"tensor": "a", "access": "read"}, {"tensor": "out", "access": "write"}],
            "outputs": ["out"],
        },
...
def test_refuses_before_worker(edge, tmp_path, kind):
    ...
    if kind == "missing":
        ...
        kwargs["source_text"] = kwargs["source_text"].replace(
            "1xi32", "999999999xi32" if kind == "renamed_model" else "?xi32"
        )
...
def test_stale_inputs_fail_before_callback(edge, tmp_path, synthetic_build_dependencies):
...
def test_legacy_five_argument_boundary_keeps_initializer_and_accumulator_width(edge):
    ...
    pure = load_build_package(explicit_targets()["gemmini"] / "build_support/__init__.py")
    specs = {
        "seed": ([2, 3], "i8", "read"),
        "left": ([2, 5], "i8", "read"),
        "right": ([5, 3], "i8", "read"),
    ...
    cb = {
        ...
        "kernel_abi": {
            "kind": "whole_program",
            "args": [{"tensor": name, "access": access} for name, (_, _, access) in specs.items()],
            "outputs": ["output"],
        },
    }
    payloads = {
        "seed": struct.pack("<6b", -128, -7, -1, 0, 1, 127),
        "left": struct.pack("<10b", *range(-5, 5)),
        "right": struct.pack("<15b", *range(-7, 8)),
    }
    ...
    assert (
        text.count(
            "gemmini_kernel((void*)T_seed, (void*)T_left, (void*)T_right, (void*)T_output, (void*)T_accumulator);"
        )
        == 2
    )
...
def test_legacy_header_geometry_requires_exact_selected_bytes(edge, tmp_path, problem):
    ...
    if problem == "absent":
        ...
        other.mkdir()
        other = other / "gemmini_params.h"
        other.write_text("#define DIM 9\n")
...
"""
    result = subprocess.run(
        ...
        capture_output=True,
        text=True,
        env=dict(os.environ, PYTHONPATH=str(merlin_dir() / "python")),
        timeout=15,
    )
    assert result.returncode == 0, result.stderr


def test_format_data_is_host_selected_typed_and_schema_bound(edge, tmp_path, synthetic_build_dependencies):
    prepared = edge.prepare_short_program_build(**request(), workdir=tmp_path)
    ...
    assert {Path(row["source"]).name for row in items} == {"quant_formats.registry.yaml", "quant_format.schema.yaml"}
    ...
    bound_tools = {row["source"]: row for row in prepared.dependencies if row["kind"] == "tool"}
    for tool in synthetic_build_dependencies.values():
        assert bound_tools[str(tool)]["sha256"] == hashlib.sha256(tool.read_bytes()).hexdigest()
