...
from merlin.runtime.storage_binding import StoragePrepackRequired, resolve_storage_bindings
...
def fixture():
    ...
    cb = {
        ...
        "commands": [],
        "tensors": {
            name: {"shape": list(enc.physical_shape), "dtype": enc.dtype, "role": "output" if name == "Y" else "input"}
            for name, enc in encodings.items()
        },
        "params": {"storage_encodings": {name: enc.to_dict() for name, enc in encodings.items()}},
        "kernel_abi": {
            "kind": "whole_program",
            "args": [{"tensor": name, "access": "write" if name == "Y" else "read"} for name in encodings],
            "outputs": ["Y"],
        },
    }
...
def test_reference_transpose_pack_and_production_prepack_refusal():
    ...
    reference = resolve_storage_bindings(cb, inputs, max_storage_bytes=100, reference_only_allow_prepack=True)
...
def test_stride_only_transpose_cannot_hide_prepacking():
    cb, inputs = fixture()
    cb["params"]["storage_encodings"]["A"] = GroupedAxesStorage((2, 3), "i8", ((0,), (1,)), (2, 3), (1, 2), 6).to_dict()
...
def test_input_storage_offset_and_nontrivial_float_words_are_preserved():
    cb, inputs = fixture()
    cb["params"]["storage_encodings"]["scale"] = GroupedAxesStorage((), "f32", ((),), (1,), (1,), 3, 1).to_dict()
...
def test_contradictory_storage_refuses_before_materializing(tmp_path, change):
    ...
    if change == "missing":
        ...
        cb["params"]["storage_encodings"]["extra"] = cb["params"]["storage_encodings"]["A"]
    elif change == "dtype":
        cb["tensors"]["A"]["dtype"] = "i32"
    elif change == "shape":
        cb["tensors"]["A"]["shape"] = [3, 2]
    elif change == "bool_shape":
        cb["tensors"]["scale"]["shape"] = [True]
    elif change == "metadata":
        del cb["params"]["storage_encodings"]["A"]["offset_elements"]
    elif change == "input_shape":
        inputs["A"] = [[-3, -2], [-1, 0], [1, 2]]
    elif change == "typed_dtype":
        inputs["A"] = Tensor((2, 3), list(range(6)), "i16")
    elif change == "typed_shape":
        inputs["A"] = Tensor((3, 2), list(range(6)), "i8")
    elif change == "recipe":
        cb["params"]["im2col_recipes"] = [{"source": "A", "target": "Y"}]
...
def test_tiny_native_caller_reads_output_in_declared_logical_order(tmp_path, monkeypatch):
    ...
    include.mkdir()
    (include / "gemmini_testutils.h").write_text(
...
"""
    ...
    built = subprocess.run(
        [compiler, "-std=c11", "-I", str(tmp_path), str(tmp_path / "caller.c"), "-o", str(tmp_path / "caller")],
        capture_output=True,
        text=True,
        timeout=10,
    )
...
def test_rank_zero_output_uses_exact_offset():
    cb, inputs = fixture()
    cb["params"]["storage_encodings"]["Y"] = GroupedAxesStorage((), "i16", ((),), (1,), (1,), 3, 2).to_dict()
