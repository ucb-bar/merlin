...
def test_gsim_is_unavailable_not_an_exception_when_nothing_is_built(monkeypatch, G, tmp_path, toolchain_present):
    ...
    "Nothing is built" means every source empty: the env override, the derived home under the RUN's
    build root, and the repo-anchored home an engine is INSTALLED in. Redirecting both roots is how the
    assertion stays about the code instead of about whether this particular checkout has an emulator
    installed -- which is the whole point of the test, and a third source was added underneath it.

    The installed home is redirected by patching `paths.tracked_build_dir`, NOT by moving
    MERLIN_REPO_ROOT: that root is also where `paths.env` finds `.env`, which it CACHES, so moving it
    empties the external-path table for every test that runs after it in the same session."""
    from merlin.common import paths as _paths

    monkeypatch.delenv("MERLIN_GEMMINI_GSIM_EMU", raising=False)
    monkeypatch.setenv("MERLIN_OUT_ROOT", str(tmp_path / "out"))
    monkeypatch.setattr(_paths, "tracked_build_dir", lambda: tmp_path / "installed")
...
def test_gsim_is_available_when_the_env_points_at_an_executable(monkeypatch, G, tmp_path, toolchain_present):
...
def test_the_derived_home_registers_the_engine_with_no_env_var(monkeypatch, G, tmp_path, toolchain_present):
...
def test_with_no_build_anywhere_the_tier_falls_back_to_verilator_and_says_why(
    monkeypatch, G, tmp_path, toolchain_present
):
    ...
    Both homes are redirected so "no build anywhere" is true of all three sources -- the env override,
    the run's derived home, and the repo-anchored home an engine is installed in."""
    from merlin.common import paths as _paths
    from merlin.targetgen import capsule_runner as CR
    from merlin.targetgen import rtl_engine_policy as POL
