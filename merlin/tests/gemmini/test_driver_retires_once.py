...
def _codegen():
    """Load codegen only from the explicitly selected support package."""
    selected = _support_root() / "backend"
    if "gback" not in sys.modules:
        pkg = types.ModuleType("gback")
        pkg.__path__ = [str(selected)]
        sys.modules["gback"] = pkg
    assert list(sys.modules["gback"].__path__) == [str(selected)], "previous backend selection differs"
    module = importlib.import_module("gback.gemmini_codegen")
    assert Path(module.__file__).resolve() == (selected / "gemmini_codegen.py").resolve()
    return module
...
class TestRetireOnce:
    ...
    def test_a_multi_job_capsule_exists_to_make_that_non_vacuous(self):
        ...
        assert multi, (
            "no capsule in this driver's scope has two jobs, so the per-job-fence "
...
class TestTheOtherArmIsEmittable:
    ...
    def test_the_two_arms_differ_by_one_barrier_per_job_and_nothing_else(self):
        ...
        for cb, jobs in _scoped_command_buffers():
            pair = BA.pair_from_emitter(gc.generate_driver, cb, settings=(gc.RETIRE_ONCE, gc.RETIRE_PER_JOB))
    ...
    def test_a_single_job_capsule_has_no_barrier_to_remove(self):
        ...
        gc = _codegen()
        pair = BA.pair_from_emitter(
            gc.generate_driver, _a_one_job_command_buffer(), settings=(gc.RETIRE_ONCE, gc.RETIRE_PER_JOB)
        )
