...
def _initializer(source: str, name: str) -> list[int]:
    (line,) = [line for line in source.splitlines() if line.startswith("static const") and f"T_{name}[" in line]
...
def test_rank4_slice_order_and_offsets_preserve_each_declared_batch_axis():
    ...
    assert (plan.slice(5).a_byte_offset, plan.slice(5).w_byte_offset, plan.slice(5).dst_byte_offset) == (240, 400, 960)
...
def test_decoded_rank3_trace_has_exact_tail_offsets_and_resets_each_slice_accumulator():
    ...
    assert [
        (item["decoded"]["dram"]["offset"], item["decoded"]["rows"], item["decoded"]["cols"]) for item in w_mvins
    ] == [(0, 16, 4), (256, 1, 4), (272, 16, 4), (528, 1, 4)]
    assert [
        (item["decoded"]["dram"]["offset"], item["decoded"]["rows"], item["decoded"]["cols"]) for item in a_mvins
    ...
    assert [
        (
            item["decoded"]["dram"]["arg_index"],
            item["decoded"]["dram"]["offset"],
            item["decoded"]["rows"],
            item["decoded"]["cols"],
        )
        for item in mvouts
...
def test_rank4_harness_keeps_varying_rhs_storage_and_one_warm_measured_boundary(monkeypatch):
    ...
    w_values = [-60 + 20 * batch + row * 4 + col for batch in range(6) for row in range(5) for col in range(4)]
    monkeypatch.setenv("MERLIN_CACHE_STATE", "warm")
    source = gem.render_harness(cb, target="gemmini", inputs={"A": a_values, "W": w_values})
...
def test_capacity_uses_maximum_one_slice_footprint_not_total_batch_work():
    ...
    rank3 = gm._batched_matmul_plan(_cb(batch_shape=(2,), m=17, k=33, n=33), isa=_fake_isa(scratchpad_rows=160))
    rank4 = gm._batched_matmul_plan(_cb(batch_shape=(2, 3), m=17, k=33, n=33), isa=_fake_isa(scratchpad_rows=160))
    assert rank3.schedule == rank4.schedule == "slice_weight_resident"
    assert (
        gm._batched_matmul_plan(_cb(batch_shape=(2, 3), m=17, k=33, n=33), isa=_fake_isa(scratchpad_rows=32)).schedule
        == "two_tile_streaming"
    )
    assert (
        gm._batched_matmul_plan(_cb(batch_shape=(2, 3)), isa=_fake_isa(scratchpad_rows=32)).schedule
        == "slice_weight_and_a_panel_resident"
    )
...
def test_capacity_selected_nondefault_schedules_emit_their_exact_traffic(
    ...
    plan = gm._batched_matmul_plan(cb, isa=_fake_isa(scratchpad_rows=scratchpad_rows))
    ...
    w_offsets = [item["decoded"]["dram"]["offset"] for item in mvins if item["decoded"]["dram"]["arg_index"] == w_arg]
    one_rhs = [0, 16, 512, 528, 1024, 1040]
    streamed_rhs = [0, 512, 1024, 16, 528, 1040]
    assert w_offsets == (one_rhs if schedule == "slice_weight_resident" else streamed_rhs + streamed_rhs)


@pytest.mark.parametrize(
    "scratchpad_rows,accumulator_rows,fragment",
    [
...
def test_capacity_preflight_fails_closed(scratchpad_rows, accumulator_rows, fragment):
    with pytest.raises(CodegenError, match=fragment):
        gm._batched_matmul_plan(
            _cb(), isa=_fake_isa(scratchpad_rows=scratchpad_rows, accumulator_rows=accumulator_rows)
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (lambda cb: cb["tensors"].update(B={"role": "bias", "shape": [4], "dtype": "i32"}), "exactly its a/w/dst"),
        (lambda cb: cb["tensors"].update(S={"role": "scale", "shape": [1], "dtype": "i8"}), "exactly its a/w/dst"),
        (lambda cb: cb.update(outputs=["A"]), "outputs must be exactly"),
        (lambda cb: cb["commands"][0]["attributes"].update(epilogue=["relu"]), "does not implement an epilogue"),
        (lambda cb: cb["commands"][0]["attributes"].update(batch=3), "disagrees"),
        (lambda cb: cb["params"].update(im2col_recipes=[{"target": "A"}]), "consumes its original a/w slices"),
        (lambda cb: cb["commands"].append({"opcode": "EVICT", "operands": {"handle": "x"}}), "exactly one isolated"),
    ],
)
...
def test_real_rank3_capsule_parses_to_a_declared_output_and_reaches_target_emission(relative):
    ...
    assert cb["tensors"]["Y0"] == {"shape": [2, 16, 16], "dtype": "i32", "role": "output"}
...
def test_registered_compile_path_routes_batched_matmul_through_mlir_harness_link(monkeypatch, tmp_path):
