...
REPO = repo_root()
CONTRACT = REPO / "examples/gemmini/target/contracts/target_contract.yaml"
...
def test_packages_declare_honest_authoring_mode():
    ...
    for pkg in ("gemmini", "saturn_vec"):
        man = yaml.safe_load((REPO / "out/artifacts/targets" / pkg / "hand_v0" / "manifest.yaml").read_text())
...
def test_extracted_facts_reproduce_hand_curated_contract():
    ...
    facts = introspect.extract_facts(_ART["fir"], _ART["hierarchy"], role_target="gemmini")
