...
def bracket_environment(tmp_path, monkeypatch):
    ...
    header = (
        repo_root() / "merlin/experiments/capsule_bench/targets/gemmini/contracts/"
        "hwbringup_gemmini_v0/isa_include/gemmini_counter.h"
    ).read_bytes()
    ...
    discovery = {
        "status": "derived",
        "header": str(roots[0] / "include/gemmini_counter.h"),
        "header_sha256": digest,
        "event_codes": hc.event_codes(header.decode()),
    }
    ...
    monkeypatch.setattr(
        ...
        lambda target: SimpleNamespace(
            counter_partition_inputs=lambda: {
                "status": "available",
                "hw_text": "unused",
                "module": "unused",
                "counter_module": "unused",
                "source": "test",
            }
        ),
    )
    monkeypatch.setattr(
        hc,
        "prove_occupancy_partition_from_circt",
