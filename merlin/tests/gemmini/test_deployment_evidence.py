...
def _profile(tmp_path: Path):
    ...
    return profile_with_sha256(
        contract_path=bindings["contract"][0],
        contract_sha256=bindings["contract"][1],
        config_path=bindings["config"][0],
        config_sha256=bindings["config"][1],
        runtime_header_path=bindings["runtime_header"][0],
        runtime_header_sha256=bindings["runtime_header"][1],
        bitstream_path=bindings["bitstream"][0],
        bitstream_sha256=bindings["bitstream"][1],
...
def _command_buffer(output_dtype: str) -> dict:
    ...
    return {
        ...
        "commands": [
            ...
            {"opcode": "COMMIT", "operands": {"src": "acc", "dst": "Y"}, "attributes": attributes},
        ],
        "kernel_abi": {
            "kind": "whole_program",
            "args": [
...
def _whole_program_loop_command_buffer() -> dict:
    return {
        ...
        "kernel_abi": {
            "kind": "whole_program",
            "args": [
                {"tensor": name, "access": "write" if name.startswith("Y") else "read"}
...
def _evidence(tmp_path: Path, profile_sha: str, output_dtype: str, *, old_wrapper=False):
    ...
    wrapper = derive_wrapper_event_evidence(
        wrapper_path=wrapper_path, wrapper_sha256=wrapper_sha, profile_sha256=profile_sha, emission_identity=identity
    )
...
def test_actual_full_width_readout_is_refused_by_narrow_profile(tmp_path):
    ...
    assert "unsupported_physical_egress" in [row["code"] for row in result["ranked_actionable_diagnostics"]]
...
def test_old_end_before_fence_wrapper_is_refused(tmp_path):
    ...
    assert result["status"] == "refused"
    assert "measured_end_before_completion" in [row["code"] for row in result["ranked_actionable_diagnostics"]]
...
def test_host_event_manifest_must_bind_exact_wrapper_token_stream(tmp_path):
    ...
    manifest_path, manifest_sha = _write(tmp_path / "wrapper_events.json", json.dumps(manifest, sort_keys=True))
...
def test_representative_whole_program_joins_fused_loop_task_writes(tmp_path):
    ...
    evidence = derive_physical_egress_evidence(
        ...
        object_path=object_path,
        object_sha256=object_sha,
        rtl_facts_path=facts_path,
        rtl_facts_sha256=facts_sha,
