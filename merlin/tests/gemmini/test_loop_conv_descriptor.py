...
def contract():
    ...
    return CONV.NativeConvContract(
        header=INCLUDE / "gemmini.h", params=INCLUDE / "gemmini_params.h", facts=FACTS, core_hw=hw
    )
...
def sample():
    return {
        ...
        "W": {"shape": [4, 1], "dtype": "i8"},
        "Y": {"shape": [4, 1], "dtype": "i8"},
    }


def emit(contract, command=None, tensors=None):
    command0, tensors0 = sample()
    return CONV.emit_native_conv(
        command or command0,
        tensors or tensors0,
        contract=contract,
...
def test_actual_header_c_macro_matches_every_emitted_register(contract, tmp_path):
    ...
    defines = "\n".join(
        f"#define {row['name'] if row['name'].startswith('k_') else 'k_' + row['name']} {row['funct']}"
    ...
    source = (
        ...
        + macro.body
        + "\n"
        + "int main(void) { uint64_t x=101,w=203,y=307; "
        + call
        + " return 0; }\n"
    )
    ...
    subprocess.run(
        [cc, "-std=c11", str(path), "-o", str(tmp_path / "packing")], check=True, capture_output=True, timeout=10
    )
    actual = subprocess.run([str(tmp_path / "packing")], check=True, text=True, capture_output=True, timeout=5)
    expected = [
        (row["funct"], numbers.get(row["rs1"], row["rs1"]), numbers.get(row["rs2"], row["rs2"]))
        for row in receipt["instructions"]
    ]
...
def test_stale_hardware_refused_before_descriptor(tmp_path):
    ...
    with pytest.raises(CONV.UnsupportedNativeConv, match="stale core hardware"):
        CONV.NativeConvContract(
            header=INCLUDE / "gemmini.h", params=INCLUDE / "gemmini_params.h", facts=FACTS, core_hw=bad
        )
...
def test_changed_header_field_cannot_pass_pinned_decoder(contract):
    ...
    altered.header = replace(
        altered.header,
        macros=tuple(replace(item, body=wrong) if item.name == macro.name else item for item in altered.header.macros),
    )
...
def test_real_entrypoint_native_selection_and_exact_default_fallback(contract):
    ...
    selection = {}
    text, arguments = codegen.emit_kernel_mlir(
        cb, native_conv_contract=contract, native_conv_selection_receipt=selection
    )
    ...
    refusal = {}
    after = codegen.emit_kernel_mlir(default_cb, native_conv_contract=contract, native_conv_selection_receipt=refusal)
    assert before == after
