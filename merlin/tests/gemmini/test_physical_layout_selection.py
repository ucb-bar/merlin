...
SPEC = importlib.util.spec_from_file_location(
    ...
    _support_root() / "backend/gemmini_physical_layout.py",
)
...
def _request(batch: int = 1):
    return TARGET.NativeConvLayoutRequest("conv", "x", ("w",), ("y",), batch, ("fixture source operation identity",))


def _facts(
    *,
    mesh_rows: int,
    evidence: str | None = "hardware-run-hash",
...
def test_batch_one_transpose_is_rejected_from_supplied_mesh_geometry() -> None:
    ...
    refusal = next(item for item in decision.refusals if item.code == TARGET.UNDERFILLED_TRANSPOSED_INPUT_REFUSAL)
...
def test_guard_is_not_a_fixed_shape_or_fixed_mesh_rule() -> None:
    ...
    assert decision.op is not None
    assert [option.encoding_id for option in decision.op.options] == ["channels_last", "channels_first"]
    assert not any(item.code == TARGET.UNDERFILLED_TRANSPOSED_INPUT_REFUSAL for item in decision.refusals)
...
def test_graph_propagated_non_transposed_layout_wins_when_both_are_legal() -> None:
    ...
    assert dict(result.plan.assignments) == {"w": "packed_parameter", "x": "channels_last", "y": "channels_last"}
...
def test_underfill_with_evidence_from_another_configuration_fails_closed() -> None:
    decision = TARGET.native_convolution_layout_decision(
        ...
        _facts(
            mesh_rows=4,
            evidence_configuration="different-configuration",
            capabilities=("native_conv_transposed_input",),
        ),
    )
    assert decision.op is None
    assert "missing_hardware_cost_guard_evidence" in {item.code for item in decision.refusals}
