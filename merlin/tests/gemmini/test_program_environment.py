...
def fixture(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(support_sources=(harness / "support.c",), link_script=harness / "link.ld"),
    )
    ...
    monkeypatch.setattr(
        gsim_emulator,
        "citation",
        lambda *a, **k: {"available": True, "refused": False, "receipt_status": "bound", "flavour": "binary"},
    )
    prefix = [
        "bwrap",
        "--clearenv",
        ...
        "bash",
        "-c",
        f'export PYTHONPATH={python}${{PYTHONPATH:+:$PYTHONPATH}}; export MERLIN_GEMMINI_HARNESS_DIR={alias}; exec "$@"',
        "host-tool",
    ]
