...
class TestTheEngineDecidesTheFacet:
    ...
    def test_a_simt_endpoint_fills_the_simt_facet(self):
        ...
        c = C.lift_asm_roles(
            _stream(("operand_load",), ("accumulate",), ("sync",), ("readout",)),
            ep,
            op="matmul",
            source="t",
    ...
    def test_a_lane_endpoint_fills_the_vector_facet(self):
        ...
        c = C.lift_asm_roles(_stream(("elementwise",)), ep, op="exp", source="t", geometry={"sew": 8, "lmul": 1.0})
...
class TestDispatchIsFilledForEveryEndpoint:
    def test_the_dispatch_facet_is_engine_agnostic(self):
        ...
        for name in ("gemmini_rocc", "muon_simt", "atlas_vpu"):
            c = C.lift_asm_roles(_stream(("config",), ("accumulate",)), EP.load_endpoint(name), op="x", source="t")
    ...
    def test_config_set_once_is_descriptor_reuse(self):
        ep = EP.load_endpoint("gemmini_rocc")
        reuse = C.lift_asm_roles(_stream(("config",), ("accumulate",), ("accumulate",)), ep, op="m", source="t")
        churn = C.lift_asm_roles(
            _stream(("config",), ("accumulate",), ("config",), ("accumulate",)), ep, op="m", source="t"
        )
...
class TestResidencyIsNeverGuessed:
    ...
    def test_an_unrolled_reduction_with_a_mid_readout_is_not_resident(self):
        ep = EP.load_endpoint("gemmini_rocc")
        c = C.lift_asm_roles(_stream(("accumulate",), ("readout",), ("accumulate",)), ep, op="m", source="t")
        assert c.compute.accumulator_resident is False

    def test_an_unrolled_reduction_committing_once_is_resident(self):
        ep = EP.load_endpoint("gemmini_rocc")
        c = C.lift_asm_roles(_stream(("accumulate",), ("accumulate",), ("readout",)), ep, op="m", source="t")
    ...
    def test_loop_spans_scope_residency_to_the_reduction(self):
        ...
        assert C.lift_asm_roles(s, ep, op="m", source="t", loop_spans=[(90, 200)]).compute.accumulator_resident is True
        assert C.lift_asm_roles(s, ep, op="m", source="t", loop_spans=[(90, 600)]).compute.accumulator_resident is False
...
class TestTextIsaStreams:
    ...
    def test_a_text_stream_lifts_like_a_binary_one(self):
        ...
        d = T.decode_text(
            ["vmatpush.weight.mxu0 0, 1", "vmatmul.mxu0 0,0,0", "vmatmul.mxu0 0,1,0", "vmatpop.mxu0 2"], "atlas", ep
        )
