...
def test_lowering_trace_roundtrips_and_renders():
    ...
    lt = T.LoweringTrace(
        kernel="matmul_f32_64",
        target="rvv",
        source="ours",
        graph=T.GraphRegion(region_id="r0", op="matmul", family="contraction", shape={"M": 64, "N": 64, "K": 64}),
        steps=steps,
        asm=T.AsmRegion(label="k-loop", span=(100, 180), facts={"contraction": "fused_fma", "vfmacc": 64}),
    )
    d = lt.to_dict()
...
def test_asm_region_from_expert_fixture(monkeypatch):
    ...
    monkeypatch.setattr(
        objdump, "disassemble_text", lambda *a, **k: (_ASM_DIR / "openblas_sgemm_rvv.objdump").read_text()
    )
...
def test_expert_steps_from_contract():
    ...
    for fw in ("openblas", "xnnpack"):
        ...
        assert [s.name for s in steps] == ["operand-layout", "operand-prepack", "accumulator-block", "epilogue"]
...
def test_build_expert_trace_graph_none_by_design(monkeypatch):
    tr = T.build_expert_trace(
        "xnnpack", _fix(monkeypatch, "xnnpack_f32_gemm_rvv.objdump"), op="matmul", kernel_id="xnnpack_f32_gemm_rvv"
    )
...
def test_build_our_trace_carries_recognized_region(monkeypatch):
    ...
    tr = T.build_our_trace(_fix(monkeypatch, "ours_baseline_matmul.objdump"), record=rec, recognized_op="attention")
...
def test_emit_trace_versioned(tmp_path, monkeypatch):
    ...
    tr = T.build_expert_trace(
        "openblas", _fix(monkeypatch, "openblas_sgemm_rvv.objdump"), op="matmul", kernel_id="openblas_sgemm_rvv"
    )
