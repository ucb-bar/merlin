...
def _selected_shim():
    ...
    if "saturn" not in explicit_targets():
        pytest.skip("requires Saturn support explicitly selected with MERLIN_TARGET_PATH")
    return load_declared("saturn", "matrix_lowering").shim


def emit_translation_unit(*args, **kwargs):
    return _selected_shim().emit_translation_unit(*args, **kwargs)


def scratch_bytes_for(*args, **kwargs):
    return _selected_shim().scratch_bytes_for(*args, **kwargs)
...
class TestTheLeftOperandPack:
    ...
    def test_the_scratch_is_sized_to_the_largest_signature(self):
        ...
        assert scratch_bytes_for({}) == _selected_shim().DEFAULT_SCRATCH_BYTES
...
class TestTheUnitContract:
    ...
    def test_the_shipped_contract_declares_the_unit(self):
        load_contract = _selected_shim().load_contract
