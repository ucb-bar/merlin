...
def test_promote_emits_expected_rules():
    ...
    assert {
        "packed_rhs_policy",
        "accumulator_commit_policy",
        "vl_agnostic_loop_policy",
        "weight_stationary_dataflow_policy",
    } <= names
...
def test_packed_rhs_policy_fires_on_positive_and_silent_on_negative():
    rule = next(
        r
        for r in policy.promote({"packed_rhs": _stat(["xnnpack", "autocomp"], 900)}).rules
...
def test_accumulator_commit_fires_on_epilogue_workload():
    rule = next(
        r
        for r in policy.promote({"accumulator_commit": _stat(["xnnpack", "autocomp"], 400)}).rules
        if r["policy"] == "accumulator_commit_policy"
    )
    bench = _bench("matmul_bias_requant_relu")
    facts = {
        "op": bench["ops"][0],
        "has_epilogue": len(bench["op_sequence"]) > 1,
