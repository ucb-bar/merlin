...
from merlin.mining.prior import (
    LEDGER_ENV,
    OutcomePrior,
    family_strategy_map,
    ledger_path,
...
class TestLearningFromOurOwnRuns:
    ...
    def test_improvement_needs_to_beat_the_noise_margin(self):
        ...
        assert classify_node(self._node(speedup=1.30, parent_speedup=1.0, margin_improved=True)) == "improved"
        assert classify_node(self._node(speedup=1.001, parent_speedup=1.0, margin_improved=False)) == "correct_no_gain"
        assert classify_node(self._node(speedup=0.80, parent_speedup=1.0, margin_improved=False)) == "regressed"
...
class TestShapeScopeIsAClaimOrSilence:
    ...
    def test_unvalidated_scope_names_the_guesses_being_made(self):
        ...
        actions = [self._action(shape_regimes=("skinny",)), self._action(shape_agnostic=True), self._action()]
