...
def derive_completion_contract() -> CompletionContract:
    ...
    headers = [root / "include/gemmini.h" for root in recipe.include_roots if (root / "include/gemmini.h").is_file()]
    ...
    matches = [
        (index + 1, line)
        for index, line in enumerate(payload.decode().splitlines())
    ...
    return CompletionContract(
        ...
        recognizes_decoded=decoded,
        recognizes_operation=operation,
    )
