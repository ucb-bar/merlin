...
def _layout(text):
    ...
    layouts = [line.split('"')[1] for line in text.splitlines() if line.startswith('target datalayout = "')]
    triples = [line.split('"')[1] for line in text.splitlines() if line.startswith('target triple = "')]
    ...
    fields = pointers[0]
    _require(2 <= len(fields) <= 4 and all(x.isdecimal() and int(x) > 0 for x in fields), "malformed pointer layout")
    _require(parts[0] in ("e", "E"), "missing compiler byte order")
    _require(
        not any(part.startswith("ni:") and "0" in part.split(":")[1:] for part in parts),
        "non-integral address-space-zero pointers unsupported",
    )
    _require(
        not any(part[:1] in ("P", "G", "A") and part[1:] != "0" for part in parts),
    ...
    return {
        "data_layout": layouts[0],
        "triple": triples[0],
        "pointer_bits": size,
        "pointer_alignment_bits": alignment,
        "pointer_index_bits": index,
        "byte_order": "little" if parts[0] == "e" else "big",
        "address_space": 0,
    }
...
def _symbol_sizes(text):
    ...
    for line in text.splitlines():
        ...
        if not fields or not fields[-1].startswith("merlin_abi_"):
            continue
        _require(
            len(fields) == 4 and fields[1].isdecimal() and fields[2] in ("B", "D", "R"), "malformed target fact symbol"
        )
...
def _agree(layout, facts, dtype_widths):
    ...
    _require(
        set(facts) == expected and all(type(x) is int and x > 0 for x in facts.values()),
    ...
    _require(
        facts["pointer_alignment"] * facts["char_bits"] == layout["pointer_alignment_bits"],
        "kernel/harness pointer alignment mismatch",
    )
    _require(
        facts["byte_order"] == (1 if layout["byte_order"] == "little" else 2), "kernel/harness byte order mismatch"
    )
...
def derive_compact_target_abi(command_buffer, compact_contract, *, workdir, timeout_s=30):
    ...
    _require(
        command_buffer.get("kernel_abi", {}).get("kind") == "whole_program" and 0 < len(args) <= 4096,
        "bounded whole-program pointer ABI required",
    )
    ...
    def run(argv):
            ...
            try:
                while process.poll() is None:
                    if (
                        ...
                        or max(stdout.stat().st_size, stderr.stat().st_size) > 4 * 1024 * 1024
                    ):
        ...
        _require(
            stdout.stat().st_size <= 4 * 1024 * 1024 and stderr.stat().st_size <= 4 * 1024 * 1024,
    ...
    pins = {
        str(path): _file_sha(path)
        for path in (
            kernel_compiler,
            gcc,
            source,
            Path(__file__).resolve(),
            Path(gemmini.__file__).resolve(),
            Path(gemmini_codegen_mlir.__file__).resolve(),
            Path(codegen.__file__).resolve(),
    ...
    run([*compile_command, "-MD", "-MF", depfile, "-MT", "merlin_abi_dependencies"])
    _require(
        _dependencies(depfile.read_text(), work) == dependencies, "selected headers changed during ABI compilation"
    )
    facts = _symbol_sizes(run([helpers["nm"], "-S", "--radix=d", "--defined-only", obj]))
    _agree(layout, facts, widths)
    _require(
        all(_file_sha(path) == digest for path, digest in pins.items()),
        "ABI tool/source pins changed during derivation",
    )
    ...
    _require(
        all(
            type(binding.get("argument_index")) is int and type(binding.get("base_index")) is int
            for binding in bindings
        ),
        "non-integer compact binding indices",
    )
    _require(
        sorted(binding["argument_index"] for binding in bindings) == list(range(len(args))),
    ...
    for i in range(len(bases)):
        ...
        _require(
            indices and len(accesses) == 1 and accesses <= {"read", "write", "readwrite"},
    ...
    evidence = {
        "schema": "compiled_compact_target_abi_v1",
        "scope": "original_caller_container_guarantees",
        "command_buffer_sha256": _document_sha(command_buffer),
        "compact_contract_sha256": _document_sha(compact_contract),
        "kernel_layout": layout,
        "harness_facts": facts,
        "dtype_order": unique,
        "argument_dtype_order": dtypes,
        "argument_access_order": [arg["access"] for arg in args],
        "source_pins": pins,
        "kernel_flags": kernel_flags,
        "harness_flags": list(recipe.cflags),
        "harness_include_roots": [str(x) for x in recipe.include_roots],
        "selected_container_header": selected_headers[0],
        "commands": commands,
        "object_sha256": _file_sha(obj),
        "layout_artifact_sha256": _file_sha(work / "target_data_layout.ll"),
        "pointer_address_space_required": 0,
        "emitted_address_space_verified": False,
        "stronger_emitted_access_alignment_verified": False,
        "linker_allocations_verified": False,
        "prepack_authorized": False,
        "target_executed": False,
        "runtime_addresses_verified": False,
        "elapsed_seconds": time.monotonic() - started,
    }
    authority_sha = _document_sha(evidence)
    _require(time.monotonic() < deadline, "ABI fact deadline exhausted")
    target_abi = CompactTargetABI(
        tuple(layout["pointer_index_bits"] for _ in bases),
        tuple(base_alignments),
        alignments,
        tuple(base_access),
        "compiled-target-abi-sha256:" + authority_sha,
    )
        ...
        json.dumps(
            {
                **evidence,
                ...
                "target_abi": {field: getattr(target_abi, field) for field in target_abi.__dataclass_fields__},
            },
            indent=2,
        )
        + "\n"
