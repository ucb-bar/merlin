...
def _counter_bracket() -> tuple[list[str], list[str], list[str]]:
    ...
    Every lookup, the sibling import included, sits inside the guard. It did not, and when the sibling
    replaced its hardcoded slot count with a CIRCT-derived one the stale name went on being imported at
    the top of this function -- so `generate_driver` raised ImportError on EVERY call, opt-in or not,
    and the driver this whole family measures could not be emitted at all. An optional measurement must
    degrade to "counters unavailable"; it must never be able to take the emitter down with it.
    """
    try:
        from .gemmini_codegen_mlir import (_counter_slots, _counters_requested,
                                           _read_discovered_counter_header)
        ...
        if capacity.get("status") != "derived":
            return [], [], []
        text = _read_discovered_counter_header(found)
        codes = _hc.event_codes(text)
        disabled = codes.get("DISABLE")
        if isinstance(disabled, bool) or not isinstance(disabled, int) or disabled < 0:
            return [], [], []
        bracket = _hc.counter_bracket_c(_hc.derive_occupancy_counters(text), codes,
                                        slots=int(capacity["slots"]), padding_code=disabled)
