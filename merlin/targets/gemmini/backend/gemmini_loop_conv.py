...
def _descriptor_decode_proof(hw_text, instructions, parameters):
    ...
    return {
        ...
        "zero_only_absent_fields": sorted(absent),
...
def emit_native_conv(
    ...
    *,
    ...
    _require(
        a.get("layout") == "nhwc"
        and a.get("stride") == [1, 1]
        and a.get("dilation") == [1, 1]
        and a.get("padding") == [0, 0, 0, 0],
    ...
    _require(
        a_rows + b_rows <= contract.capacity["max_spad_rows"] and c_rows <= contract.capacity["max_acc_rows"],
        "single native tile exceeds derived double-buffer capacity",
    )
    ...
    _require(
        set(macro.params) == {name for name, value in v.items() if value != 0} | known_zero | {"activation"},
        ...
        instructions.append(
            {
                ...
                "rs1": _expression(args[1].strip(), v),
                "rs2": _expression(args[2].strip(), v),
            }
        )
        rest = rest[end:]
    ...
    code = (
        f"gemmini_extended_config_st({co} * sizeof(elem_t), {v['activation']}, ACC_SCALE_IDENTITY);\n"
