...
def machine_artifact_policy_identity() -> dict:
    ...
    return {
        ...
        "implementations": {
            ...
            for name, path in (
                ("adapter", __file__),
                ("target_recipe", gemmini.__file__),
                ("recipe_interface", base.__file__),
                ("codegen", codegen.__file__),
                ("toolchain", toolchain.__file__),
                ("instruction_audit", rvv_audit.__file__),
            )
        },
    }


def analyze_machine_artifact(
    lowered_text: str, *, workdir: Path, run_command: Callable, timeout_seconds: float = 60
) -> dict:
    ...
    compiler, translator, disassembler = (policy[name]["path"] for name in ("compiler", "translator", "disassembler"))
    flags = policy["flags"]
    ...
    commands = (
        [str(translator), "--mlir-to-llvmir", str(source), "-o", str(ll)],
    ...
    return {
        "schema": "compiled_machine_activity_v1",
        "status": "compiled",
