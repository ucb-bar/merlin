...
def native_layout(spec: Mapping[str, Any]) -> dict[str, int]:
    ...
    return {"rows": rows, "cols": cols, "row_stride": pcols, "storage_elements": prows * pcols}
...
def derive_native_witness_abi(*, target: str) -> dict[str, Any]:
    ...
    return {
        ...
        "expected_symbol": abi.entry_symbol,
        "abi_provenance": {
            "scope": "existing backend whole_program row-major padded ABI",
            "target": target,
