...
def short_program_environment(sandbox):
    ...
    if not namespace.is_absolute() or tuple(namespace.parts[-2:]) != ("merlin", "python") or not namespace.is_dir():
    ...
    for source in (*recipe.support_sources, recipe.link_script):
        ...
        if not destination.is_file() or not is_exposed(prefix, destination) or _sha(source) != _sha(destination):
    ...
    if (
        ...
        or engine.get("refused") is not False
        or engine.get("receipt_status") != "bound"
        or engine.get("flavour") != "binary"
    ):
        raise ValueError("configured short runtime engine lacks an exact bound build receipt")
    return {
        "schema": "existing_short_program_environment_v1",
        "python_executable": str(interpreter),
        "build_namespace_root": str(project),
        "build_path_bindings": ((str(harness), str(alias)),),
        "engine_provenance": engine,
        "existing_support_pins": evidence,
        "adapter_source_pins": {
            str(Path(__file__).resolve()): _sha(__file__),
            str(Path(gemmini.__file__).resolve()): _sha(gemmini.__file__),
        },
        "sandbox_policy_sha256": hashlib.sha256(
            json.dumps(sandbox, sort_keys=True, separators=(",", ":"), allow_nan=False).encode()
        ).hexdigest(),
        "mount_grants": False,
        "scope": "existing namespace and byte-identical recipe paths only",
    }
