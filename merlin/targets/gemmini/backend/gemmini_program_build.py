...
def _source_bounds(source):
    ...
    if tensor_bytes > MAX_STORAGE_BYTES or points > MAX_DOMAIN_POINTS or scalar_work > 100000 or operation_count > 1024:
    ...
    return {
        ...
        "static_ssa_tensor_bytes": tensor_bytes,
        "linalg_domain_points": points,
        "bounded_scalar_body_operations": scalar_work,
...
def _inputs(cb, payloads, *, legacy_dim=None, legacy_format=None):
    ...
    if (
        cb.get("target") != "gemmini"
        or abi.get("kind") != "whole_program"
        or cb.get("canonical_inputs")
        or not isinstance(abi.get("args"), list)
...
def _legacy_dimension(specification, *, host=False):
    ...
    headers = [
        row
        for row in specification["dependencies"]
        if row["kind"] == "header" and Path(row["source"]).name == "gemmini_params.h"
    ]
    if not headers:
    ...
    return {
        "schema": "pinned_legacy_whole_program_abi_v1",
        "dim": dim,
        "parameter_headers": headers,
        "candidate_storage_metadata_modified": False,
...
def prepare_short_program_build(
    *,
    source_text,
    lowered_text,
    command_buffer,
    ...
    if source_evidence.get("probe_source_sha256") != _digest(source_text.encode()) or source_evidence.get(
        "lowered_sha256"
    ) != _digest(lowered_text.encode()):
    ...
    _, storage = _inputs(
        command_buffer,
        logical_payloads,
        legacy_dim=legacy_abi["dim"] if legacy_abi else None,
        legacy_format=pure.format,
    )
    ...
    signature = _signature(
        lowered_text,
        {"compact_symbol": "gemmini_kernel", "compact_argument_count": len(command_buffer["kernel_abi"]["args"])},
    )
    ...
    files = {
        "source.mlir": source_text.encode(),
        "lowered.mlir": lowered_text.encode(),
        "command_buffer.json": cb_bytes,
        "worker.py": Path(__file__).read_bytes(),
    }
    request = {
        "schema": "short_complete_program_build_v2",
    ...
    return ShortProgramBuild(
        (str(interpreter), "-P", str(work / "worker.py"), str(work / "request.json")),
...
def _prepare_build_service(namespace_root, path_bindings=()):
    ...
    def destination_for(source, kind):
        ...
        selected = [
            (len(base.parts), base, dest)
            for base, dest in bindings
    ...
    roots = (
        ...
        "merlin.targetgen.contract.stack_usage",
        ...
        "merlin.llvmlower.codegen",
        "merlin.llvmlower.toolchain",
    )
...
"""
    ...
    format_paths = {}
    for role, source in (("numeric_format_registry", registry_source), ("numeric_format_schema", schema_source)):
        destination = bind(source, "format_data")
        record = next(item for item in dependencies if item["source"] == str(source))
        record["role"] = role
        format_paths[role] = {"source": str(source), "destination": destination, "sha256": record["sha256"]}
    format_relation = {
        "schema": "canonical_numeric_format_relation_v1",
    ...
    tools = {
        "MERLIN_CLANG": bind(toolchain.clang(), "tool"),
    ...
    def recipe_record(mapped):
        ...
        return {
            "compiler": transform(recipe.compiler, "tool"),
    ...
    return {
        "schema": "host_resolved_pure_build_service_v1",
        "target": "gemmini",
        "source_root": str(root),
        "namespace_root": str(namespace),
        "recipe": mapped_recipe,
        "host_recipe": original_recipe,
        ...
        "pure_package_init": str(pure_destination),
        "dependencies": dependencies,
        "format_data_relation": format_relation,
        "tool_environment": tools,
        "active_source_roots": list(roots),
...
def _worker(request_path):
    ...
    for item in specification["dependencies"]:
        ...
        if not destination.is_absolute() or not destination.is_file() or _file_digest(destination) != item["sha256"]:
    ...
    if (
        relation.get("schema") != "canonical_numeric_format_relation_v1"
        or relation.get("overlays_allowed") is not False
    ):
    ...
    canonical_roles = {
        "numeric_format_registry": "quant_formats.registry.yaml",
    ...
    os.environ.update(
        MERLIN_CACHE_STATE="warm",
        MERLIN_HW_COUNTERS="0",
        MERLIN_ELF_BUILD_CACHE="0",
        MERLIN_CLANG=tool_env["MERLIN_CLANG"],
        MERLIN_MLIR_TRANSLATE=tool_env["MERLIN_MLIR_TRANSLATE"],
    )
    ...
    recipe = HarnessBuildRecipe(
        compiler=Path(row["compiler"]),
        include_roots=tuple(map(Path, row["include_roots"])),
        support_sources=tuple(map(Path, row["support_sources"])),
        link_script=Path(row["link_script"]),
        load_address=row["load_address"],
        cflags=tuple(row["cflags"]),
        ldflags=tuple(row["ldflags"]),
    ...
    inputs, storage = _inputs(
        cb,
        {name: contents[filename] for name, filename in request["payloads"].items()},
        legacy_dim=legacy_abi["dim"] if legacy_abi else None,
        legacy_format=pure.format,
    )
    ...
    service = BuildOnlyService(
        target=specification["target"], recipe=recipe, renderer=render, source_pins=tuple(producers.items())
    )
    recipe_evidence = {
        **row,
        "host_recipe_sha256": specification["host_recipe_sha256"],
    ...
    elf = compile_lowered_to_elf(
        cb, contents["lowered.mlir"].decode(), build, target="gemmini", inputs=inputs, _build_service=service
    )
    for name, expected in request["files"].items():
    ...
    output_names = (elf.name, "kernel.o", "kernel.ll", "kernel.su", "kernel.stack_frame.json", "harness.c")
    receipt = {
        "schema": "short_complete_program_build_receipt_v1",
        "status": "built_not_executed",
        "request_sha256": _digest(request_bytes),
        "source_evidence": request["source_evidence"],
        "source_bounds": request["source_bounds"],
        "storage": storage,
        "legacy_abi": legacy_abi,
        "entry_signature": request["entry_signature"],
        "primary_build_pins": build_pins,
