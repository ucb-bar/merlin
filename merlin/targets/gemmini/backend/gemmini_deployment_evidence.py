...
def build_deployment_profile(
    *,
    ...
    supported_physical_egress: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    ...
    if not capabilities or len({(row["encoding"], row["width_bits"]) for row in capabilities}) != len(capabilities):
        raise ValueError("physical egress capabilities must be non-empty and unique")
    return {"schema": "deployment_profile_v1", "artifacts": artifacts, "supported_physical_egress": capabilities}
...
def derive_physical_egress_evidence(
    *,
    ...
    elaborated_hardware_sha256: str | None = None,
) -> dict[str, Any]:
    """Join actual decoded readouts to exact declared output buffers, failing closed on gaps."""
    command_path = _require_exact_file(command_buffer_path, command_buffer_sha256, role="command_buffer")
    ...
    argument_indices = {name: index for index, name in enumerate(abi_arguments)} if abi_arguments is not None else {}
    actual: dict[str, set[str]] = {}
    unmatched: list[dict[str, Any]] = []
    unknown_instructions: list[int] = []
    actual_readouts: list[dict[str, Any]] = []
    loop_evidence = None
    proof_args = (rtl_facts_path, rtl_facts_sha256, elaborated_hardware_path, elaborated_hardware_sha256)
    if any(value is not None for value in proof_args):
        ...
        facts_exact = _require_exact_file(Path(rtl_facts_path), str(rtl_facts_sha256), role="rtl_facts")
        hardware_exact = _require_exact_file(
            Path(elaborated_hardware_path), str(elaborated_hardware_sha256), role="elaborated_hardware"
        )
        layouts = derive_layouts(
            facts_text=facts_exact.read_text(encoding="utf-8"), hardware_text=hardware_exact.read_text(encoding="utf-8")
        )
    ...
    for index, instruction in enumerate(instructions):
        ...
        if (
            abi_arguments is None
            or not isinstance(arg_index, int)
            or isinstance(arg_index, bool)
            ...
            continue
        actual_readouts.append(
            {"instruction_index": instruction_index, "arg_index": arg_index, "readout": str(readout)}
        )
    ...
    if loop_evidence is not None:
        ...
        for writeback in loop_evidence["writebacks"]:
            ...
            if (
                ...
                unmatched.append(
                    {
                        "instruction_index": writeback.get("instruction_index"),
                        "reason": "fused-loop destination or physical encoding is unresolved",
                    }
                )
                continue
            readout = {"signed_integer": "i", "unsigned_integer": "u", "floating_point": "f"}.get(encoding)
            if readout is None:
                unmatched.append(
                    {
                        "instruction_index": writeback.get("instruction_index"),
            ...
            actual_readouts.append(
                {
                    "instruction_index": writeback["instruction_index"],
                    "arg_index": arg_index,
                    "readout": f"{readout}{width}",
                }
            )
    ...
    for name in names:
        ...
        physical_encoding = _dtype_encoding(next(iter(physical_types))) if len(physical_types) == 1 else None
        verified = (
            expected is not None
            and name in expected
            and declared_encoding is not None
            and physical_encoding is not None
            and not unknown_instructions
            and not unmatched
        )
        rows.append(
            {
                ...
                "actual_readout_instruction_count": sum(
                    1
                    for readout in actual_readouts
                    if name in argument_indices and readout["arg_index"] == argument_indices.get(name)
                ),
            }
        )
    missing = sorted((expected or set()) - set(actual))
    status = "verified" if rows and all(row["status"] == "verified" for row in rows) else "UNKNOWN"
    metadata_gap = None
...
def _standard_wrapper_events(tokens: Sequence[str]) -> tuple[list[dict[str, Any]], str, str | None]:
    ...
    markers.sort()
    return ([{"event": event, "token_index": position} for position, _, event in markers], "verified", None)


def _manifest_wrapper_events(
    *, manifest_path: Path, manifest_sha256: str, wrapper_sha256: str, token_stream_sha256: str, token_count: int
    ...
    events.sort()
    return ([{"event": event, "token_index": position} for position, _, event in events], "verified", None, binding)
    ...
    *,
    ...
    if event_manifest_path is None:
        ...
        events, status, reason, manifest_binding = _manifest_wrapper_events(
            ...
            manifest_sha256=str(event_manifest_sha256),
