...
def runtime_elf_digest(path: str | Path) -> str:
    ...
    with TemporaryDirectory() as temporary:
        ...
        result = subprocess.run([objcopy, "--strip-debug", str(path), str(stripped)], capture_output=True, timeout=10)
...
def isolated_primitive_signature(domain: dict):
    ...
    if len(stores) != 2 or any(
        not all((s.row_bytes, s.total_rows, s.banks, s.row_elems, s.element_dtype)) for s in stores
    ):
    ...
    if any(
        v["address"] < 0 or v["address"] + v["rows"] > operand.total_rows or v["cols"] > operand.row_elems
        for v in operands
    ):
    ...
    if (
        accumulator_row + destination["rows"] > accumulator.total_rows
        ...
        s.name: json.dumps(
            {
                ...
                "banks": s.banks,
                "scope": "drained isolated reference state",
            },
            sort_keys=True,
        )
        for s in stores
    ...
    return derive_mechanism_signature(
        representations=[
            ValueRepresentation(
                s.name,
                "explicit-physical-rows",
                s.element_dtype,
                encoding=domain["target_isa_digest"],
                quantization="exact-execution-config-payload",
            )
            for s in stores
        ],
        events=[ActivityEvent("body", "whole_motif", "compute", 0)],
        capacity_regime=geometry,
        tile_shape=shape,
        edge_cases=[
            "full-operand-tile" if all(n == operand.row_elems for n in shape) else "explicit-partial-operand-tile",
...
def _occupancy_bracket(include_roots, workdir: Path) -> tuple[dict, str, str, str]:
    ...
    resolved_header = next(
        (
            root / include_name
            for root in (workdir, *(Path(p) for p in include_roots))
    ...
    record = {
        ...
        "compiled_header": str(resolved_header.resolve()),
        "include_spelling": include_name,
        "header_sha256": discovery["header_sha256"],
        "layout": counters.to_dict(),
        "event_codes": discovery["event_codes"],
        "slot_of": bracket["slot_of"],
...
def prepare_primitive_probe(
    ...
    *,
    ...
    program = extract_primitive_program(
        module,
        target="gemmini",
    ...
    load_encoding = None
    load_end = (
        next(index for index, row in enumerate(rows) if "acc_addr" in row.get("decoded", {}))
        if fixed_work_slice
        else primitive["instruction_indices"][0]
    )
    for i, row in enumerate(rows[:load_end]):
        ...
        if "spad_addr" not in payload:
            continue
        if (
            not isinstance(operand_store.total_rows, int)
            or not isinstance(operand_store.row_elems, int)
    ...
    data = {arg: [((i * 5 + arg * 3) % 11) - 5 for i in range(size // item_bytes)] for arg, size in capacities.items()}
    ...
    store_config = next(row["decoded"] for row in reversed(rows) if row.get("decoded", {}).get("subtype") == "ST")
    output_stride = store_config.get("out_stride_bytes")
    if (
        output_arg in capacities
        or output_offset != 0
        or not isinstance(output_stride, int)
        or output_stride < n * output_bits // 8
        or output_stride % (output_bits // 8)
        or store_config.get("acc_scale") != 1.0
        or store_config.get("relu") is not False
    ):
    ...
    for arg, values in sorted(data.items()):
        declarations.append(
            f"static int{bits}_t buffer_{arg}[{len(values)}] row_align(1) = {{"
            + ",".join(str(value) for value in values)
            + "};"
        )
    declarations.extend(
        (
            f"static int{output_bits}_t buffer_{output_arg}[{output_elements}] row_align(1);",
    ...
    if profile_counters:
        ...
        if include_operand_movement:
            counter_profile["scope"] = (
                ...
                if fixed_work_slice
                else "controlled source prefix with queued movement and compute; not full model context"
            )
    ...
    command = [
        sys.executable,
        "-c",
        "from merlin.llvmlower.codegen import compile_ll; import sys; compile_ll(sys.argv[1],sys.argv[2],'riscv')",
        str(llvm_file),
        str(obj),
    ]
    ...
    scope = (
        "controlled_fixed_work_slice"
        if fixed_work_slice
        else "controlled_source_prefix"
        if include_operand_movement
        else "isolated_primitive"
    )
    domain = (
        _sha(
            json.dumps(
                {
                    ...
                    "primitive_domain": program.domain_digest,
        ...
        if include_operand_movement
        else program.domain_digest
    )
    prepared = {
        "schema": (f"{scope}_preparation_v1" if include_operand_movement else "isolated_primitive_preparation_v1"),
        "workdir": str(work.resolve()),
        "source_artifact_sha256": _sha(source),
        "domain_digest": domain,
        "primitive_domain_digest": program.domain_digest,
        "measurement_scope": scope,
        ...
        "primitive_mlir_sha256": _sha(mlir.encode()),
        "wrapper_sha256": _sha(wrapper.encode()),
        "elf_sha256": _sha(elf.read_bytes()),
        "computed_tile": {"m": m, "k": k, "n": n},
...
def execute_prepared_primitive(prepared: dict, *, timeout_seconds: float = 600) -> dict:
    ...
    for name, key in (
        ("primitive.mlir", "primitive_mlir_sha256"),
        ("primitive.c", "wrapper_sha256"),
        ("primitive.elf", "elf_sha256"),
    ):
    ...
    result = {
        "schema": (
            f"{prepared['measurement_scope']}_execution_v1" if contextual else "isolated_primitive_execution_v1"
        ),
        "correct": True,
        "target": "gemmini",
        "simulator": "gsim",
        ...
        "elf_sha256": _sha(elf.read_bytes()),
        "console_sha256": _sha(console.encode()),
        "warmup_runs": 1,
        "measured_runs": 1,
        ...
        "measured_scope": (
            "body call, all selected operand/competing loads and config changes, overwrite compute and completion"
            if contextual
            else "body call, original preload/compute pair and completion"
        ),
        "excluded_from_cycles": (
            ["host input initialization", "entry configuration/drain", "warm body", "output readback", "verification"]
            if contextual
            else ["operand initialization/drain", "warm body", "output readback", "verification"]
        ),
        "in_context_cycles": None,
        "licence": (
            "controlled fixed-work slice; future computes omitted symmetrically; model traffic/cache UNKNOWN"
            if prepared.get("measurement_scope") == "controlled_fixed_work_slice"
            else "controlled queued source prefix; future model traffic/cache mapping UNKNOWN"
            if contextual
    ...
    if profile is not None:
        ...
        busy = {
            unit: sum(values[name] for combo, name in layout["by_combination"].items() if unit in combo.split("+"))
            for unit in layout["engines"]
        }
        overlap = sum(values[name] for combo, name in layout["by_combination"].items() if len(combo.split("+")) > 1)
        result["counter_profile"] = {
            **profile,
            "values": values,
            "busy_cycles_by_engine_token": busy,
            "overlap_any_engine_cycles": overlap,
            "active_union_cycles": active,
            "idle_cycles": result["total_compute_cycles"] - active,
            "status": (
                f"measured_{prepared['measurement_scope']}_counter_partition"
                if contextual
...
def run_primitive_probe(
    ...
    *,
    ...
    started = monotonic()
    prepared = prepare_primitive_probe(
        source_artifact,
        workdir,
        timeout_seconds=timeout_seconds,
