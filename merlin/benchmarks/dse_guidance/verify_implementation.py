...
from merlin.dse_guidance.case_study import _CORPUS_SUBDIR, RECAP_MODELS, _recap_dir, available_models
...
DANGEROUS = ("improvement", "optimal", "best design", "predicted cycles", "calibrated future", "gap_closure", "faster")
V0_DOCS = {
...
def verify_workload(w: str) -> dict:
    ...
    check(
        raw_mm == raw["n_records"] == attributed_mm,
        f"[{w}] matmul count: raw_grep={raw_mm} extract_matmuls={raw['n_records']} attributed_sum={attributed_mm}",
    )

    # --- B: recovered head facts match the independent raw recompute ---
    raw_head = raw["roles"].get("repeated_head", {})
    check(
        raw_head.get("weight_bytes") == WB and raw_head.get("matmul_count") == MM,
    ...
    check(
        ...
        f"[{w}] traffic_table: WB={WB}, nonresident=WB*{K}={WB * K}, avoidable=WB*{K - 1}={WB * (K - 1)}",
    )
    ...
    check(
        ok_cap and _eq(cap["avoidable_reload_B"], WB * (K - 1)),
    ...
    check(
        int(dg["commands_per_step_matmul_proxy"]) == MM
        and int(dg["dispatches_per_replan_proxy"]) == MM * K
        and dg["syncs_per_step"] == "unavailable",
        f"[{w}] dispatch proxy: commands/step={MM}, dispatches/replan=MM*{K}={MM * K}, syncs=unavailable",
    )

    # --- F: resident_state weights bytes == WB ---
    rs = [r for r in _csv_rows(CS / "resident_state_table.csv") if r["workload"] == w and r["state"] == "weights"]
    check(
        rs and _eq(rs[0]["bytes"], WB) and rs[0]["bytes_evidence"] == "recovered_from_ir",
    ...
    check(
        _eq(reqs["resident_capacity_required"], WB)
        and _eq(reqs["avoidable_weight_reload_bytes"], WB * (K - 1))
        and _eq(reqs["macs_per_replan"], head.get("macs_total", -1)),
        f"[{w}] design_envelope: resident={WB}, avoidable={WB * (K - 1)}, macs_per_replan={head.get('macs_total')}",
    )

    # --- H: accuracy-gated certificate capacity matches dtype_capacity int8 ---
    certs = [
        r
        for r in _csv_rows(CS / "accuracy_gated_dtype_candidates.csv")
        if r["workload"] == w and r["dtype"] == "int8_w8a8"
    ]
    check(
        certs and _eq(certs[0]["resident_capacity_at_format_B"], exp["int8"]),
    ...
    return {
        ...
        "matmuls": MM,
        "weight_bytes": WB,
        "avoidable_reload": WB * (K - 1),
        "cap_int8": round(exp["int8"]),
...
def verify_global() -> None:
    ...
    non_int8_pass = [r for r in gated if r["dtype"] != "int8_w8a8" and r["accuracy_status"] == "measured_pass"]
    ...
    int8_pass = {r["workload"] for r in gated if r["dtype"] == "int8_w8a8" and r["accuracy_status"] == "measured_pass"}
    expected_pass = {w for w in RECAP_MODELS if w in gate_models and w in _MODELS}
    check(
        int8_pass == expected_pass,
    ...
    for f in CS.rglob("*"):
        ...
        if (
            not f.is_file()
            or f.name in V0_DOCS
            or "manual_validation" in f.parts
        ...
        for ln in f.read_text(errors="ignore").splitlines():
            ...
            if not any(
                ...
                for k in (
                    ...
                    "never a claimed",
                    "- speedup",
                    "no bandwidth/speedup",
                    "claim a speedup",
                    "speedup/accuracy",
                    "speedup, no cycle",
                    "real dispatch/sync",
                    "no file claims a speedup",
                )
            ):
    ...
    from merlin.dse_guidance.primitive_coverage import GEMV_PRIMITIVES, TILE_PRIMITIVES
    ...
    check(
        {"P5", "P7", "P8", "P9", "P10"} <= phases and ev_ok and prim_ok,
    ...
    req_keys = {
        ...
        "boundary_placement",
        "measurements_needed_before_quantitative_dse",
        "artifacts_index",
        "what_is_not_claimed",
    }
    wl_ok = set(man.get("workloads", [])) == {w for w in RECAP_MODELS if w in _MODELS}
    ...
    check(
        req_keys <= set(man) and wl_ok and idx_ok and clean,
...
def verify_p5_workload(w: str) -> dict:
    ...
    for sr in shape_rows:
        ...
        if sr["shape_class"] != _classify_geom_indep(rec.M, rec.N, rec.K):
            cls_ok = False
    p5check(
        geom_ok, f"[{w}] operator_shape_table: {len(shape_rows)} rows == {len(recs)} matmuls, M/N/K match IR primitive"
    )
    ...
    for r in tw:
        ...
        pmacs = pM * pN * rec.K
        if not (int(r["padded_M"]) == pM and int(r["padded_N"]) == pN and int(r["padded_macs"]) == pmacs):
            pad_ok = False
...
def verify_p5_global() -> None:
    ...
    bad = [
        r
        for r in _csv_rows(CS_TILE)
        if r["primitive_kind"] == "gemv_lane" and r["shape_class"] == "squareish_gemm" and r["applicable"] == "True"
    ]
    ...
    for r in _csv_rows(CS_REGRET):
        if not _eq(
            r["max_regret"], float(r["best_workload_coverage_10"]) - float(r["worst_workload_coverage_10"]), tol=1e-4
        ):
            reg_ok = False
    ...
    p5_files = [
        ...
        "operator_geometry.yaml",
        "operator_geometry_report.md",
        "tile_waste_table.csv",
        "primitive_coverage_matrix.csv",
        "primitive_coverage_report.md",
...
def verify_p6_workload(w: str, graphs: dict) -> dict:
    ...
    for n in regions:
        ...
        if rr is not None:
            if (
                int(ws.get("matmul_count") or 0) != rr["matmul_count"]
                or int(ws.get("weight_bytes") or 0) != rr["weight_bytes"]
            ):
                reg_ok = False
        mpi, mpr, inv = (
            ws.get("macs_per_invocation"),
            ws.get("macs_per_replan"),
            (n.get("rate") or {}).get("invocations"),
        )
        if mpi and mpr and inv and int(mpr) != int(mpi) * int(inv):  # derived MACs/replan identity
            reg_ok = False
    p6check(reg_ok, f"[{w}] region nodes match attribution facts; macs_per_replan == macs_per_invocation * invocations")
    ...
    k_ok = (
        bool(loop)
        and int(loop[0]["rate"]["trip_count"]) == K
        and bool(head)
        and int(head[0]["rate"]["invocations"]) == K
    )
    p6check(k_ok, f"[{w}] loop_body trip_count == repeated_head invocations == K == {K}")

    # P6-D: cadence vocabulary + edge evidence labels
    cad_ok = all(
        (n.get("rate") or {}).get("cadence", "unknown") in ALLOWED_CADENCE
        for n in nodes
        if n["kind"] in ("phase", "region", "loop_body")
    )
    ...
    p6check(
        deps_match and dd_ev_ok,
        f"[{w}] {n_dd} data_dependency edges == independent SSA re-trace (recovered_from_ir, can_pipeline=False)",
    )
    ...
    return {
        ...
        "nodes": len(nodes),
        "edges": len(edges),
        "phase": nk.get("phase", 0),
        "region": nk.get("region", 0),
        "operator": nk.get("operator", 0),
        "state": nk.get("state_object", 0),
    }
...
def verify_p6_global(graphs: dict) -> None:
    ...
    p6_files = [
        ...
        "workload_contract_graph_summary.md",
        "phase_rate_table.csv",
        "multi_rate_contract.yaml",
        "rate_mismatch_report.md",
    ]
    leaked = {}
    for fn in p6_files:
        low = (CS / fn).read_text(errors="ignore").lower()
...
def verify_p7_workload(w: str, graphs: dict) -> dict:
    ...
    p7check(
        total_macs == indep_total and total_ops == len(recs),
    ...
    p7check(
        _eq(ap, total_macs / cp_macs, tol=1e-3),
    ...
    return {
        ...
        "max_ready_width": int(crit["max_ready_width"]),
...
def verify_p7_global() -> None:
    ...
    p7check(
        all(r["shape_class"] in known_clusters for r in hier), "hierarchy hints reference only known shape clusters"
    )
    ...
    expect = {"reduction_tree", "epilogue_unit", "DMA_engine", "loop_controller", "multi_engine_cluster"}
    p7check(
        units_ok and expect <= set(su),
    ...
    p7check(
        all(0.0 <= float(r["mac_fraction"]) <= 1.0 for r in comp), "resource-pressure compute MAC fractions in [0,1]"
    )
    # no forbidden performance claims in P7 artifacts
    p7_files = [
        "dag_parallelism_report.md",
        "critical_path_table.csv",
        "concurrency_windows.csv",
        "parallel_region_candidates.yaml",
        "sharding_table.csv",
        "sharding_opportunities.yaml",
        "intra_op_sharding_report.md",
        "operator_cluster_to_hierarchy.csv",
        "parallel_hierarchy_hints.yaml",
        "resource_pressure_table.csv",
...
def verify_p8_workload(w: str, graphs: dict) -> dict:
    ...
    p8check(
        all(p["phase"] in graph_phase_ids for p in phases),
    ...
    p8check(
        all(stage.get(p, rate.get(p)) == rate.get(p) for p in rate) and stage == {p: rate[p] for p in rate},
    ...
    p8check(buf_ok, f"[{w}] overlap buffer counts are positive ints or unavailable (yes-overlaps have >=1 buffer)")

    # P8-F: gated overlaps reflect recovered per-workload structure (not a uniform template)
    bb_ops = sum(
        int((n.get("work_summary") or {}).get("matmul_count") or 0)
        for n in g["nodes"]
        if n["kind"] == "region" and n["region_role"] == "backbone_once"
    )
    bb_yes = any(c["can_overlap"] == "yes" for c in cands if c["source_phase"].startswith("backbone(next"))
    p8check((bb_ops > 0) == bb_yes, f"[{w}] backbone overlap=yes iff recovered backbone compute exists (ops={bb_ops})")
    from merlin.dse_guidance import models as _M

    a = _M.MODEL_ARCH.get(w)
    is_vla = bool(
        a
        and a.family in {"flow_matching", "diffusion", "autoregressive_vla"}
        and a.control_rate_hz
        and a.action_horizon
    )
    ctrl_yes = any(c["can_overlap"] == "yes" for c in cands if c["source_phase"] == "control_tick_consumer")
    p8check(
        is_vla == ctrl_yes, f"[{w}] control-tick overlap=yes iff workload is a VLA with a control loop (vla={is_vla})"
    )
    return {"workload": w, "phases": len(phases), "yes_overlaps": sum(1 for c in cands if c["can_overlap"] == "yes")}
...
def verify_p8_global() -> None:
    ...
    p8check(
        opts == {"one_bigger_unit", "multiple_identical_units", "multiple_specialized_units"},
    ...
    p8_files = [
        ...
        "pipeline_stage_table.csv",
        "pipeline_candidates.yaml",
        "buffering_requirement_table.csv",
        "overlap_opportunities.md",
        "processing_unit_guidance.yaml",
        "heterogeneity_report.md",
    ]
    leaked = {}
    for fn in p8_files:
        low = (CS / fn).read_text(errors="ignore").lower()
...
def verify_p9_workload(w: str) -> dict:
    ...
    K = int(RECAP_MODELS[w]["K"])
    role_weight = {role: facts["weight_bytes"] for role, facts in _head_facts_from_raw(w)["roles"].items()}
    dm = {r["region"]: r for r in _csv_rows(CS_DATAMOVE) if r["workload"] == w}

    # P9-A: region weight bytes match the independent recompute
    wb_ok = all(int(dm[role]["weight_bytes"]) == wb for role, wb in role_weight.items() if role in dm)
    p9check(
        wb_ok and set(role_weight) <= set(dm), f"[{w}] data_movement weight bytes match IR recompute ({role_weight})"
    )
    ...
    for role, row in dm.items():
        ...
        if not (
            _eq(row["resident_int8_B"], n * ELEMENT_BYTES["int8"])
    ...
    una_ok = all(
        row["intermediate_bytes"] == "unavailable"
        and row["scale_bytes"] == "unavailable"
        and row["kv_bytes"] == "unavailable"
    ...
    byte_ok = all((r["bytes"] == "unavailable") or r["bytes"].lstrip("-").isdigit() for r in streams)
    p9check(sref_ok and byte_ok and streams, f"[{w}] DMA streams reference valid regions; bytes int-or-unavailable")
    ...
    buf_ok = all(
        r["region"] in regions
        and int(r["min_input_buffer_count"]) >= 1
        and int(r["min_output_buffer_count"]) >= 1
        and r["double_buffering_needed"] in ("yes", "no", "unknown")
        for r in bufs
    )
    ...
    return {
        "workload": w,
        "regions": len(dm),
        "top_avoidable_reload": max((int(r["avoidable_weight_reload"]) for r in dm.values()), default=0),
    }
...
def verify_p9_global() -> None:
    ...
    p9check(
        all(r["candidate_abstraction"] in ALLOWED_DMA_ABSTRACTIONS for r in streams),
        "DMA candidate abstractions in allowed vocabulary",
    )
    # no bandwidth/speedup/cycle claim leaks into P9 artifacts
    p9_files = [
        "memory_hierarchy_envelope.yaml",
        "data_movement_table.csv",
        "reuse_lifetime_table.csv",
        "memory_abstraction_candidates.yaml",
        "memory_envelope_report.md",
        "dma_stream_table.csv",
        "buffer_requirement_table.csv",
        "dma_pressure_report.md",
    ]
    leaked = {}
    for fn in p9_files:
        low = (CS / fn).read_text(errors="ignore").lower()
...
def verify_p10_workload(w: str) -> dict:
    ...
    p10check(
        len(pats) == len(recs) and all(int(p["op_index"]) in by_idx for p in pats),
    ...
    bias_ok = all(p["has_bias"] == "True" for p in pats if (by_idx[int(p["op_index"])].op == "addmm"))
    ...
    vocab_ok = all(
        r["dequant_location"] in _DEQ_VOCAB
        and r["requant_location"] in _REQ_VOCAB
        and r["accumulator_materialization"] in _ACC_VOCAB
        for r in accs
    )
    # f32 storage -> accumulator == compute (no widening); int8 would be i32 (derived, not here)
    dtype_ok = all(
        r["accumulator_dtype"] == r["compute_dtype"] for r in accs if r["storage_dtype"] in ("f32", "float32", "fp32")
    )
    p10check(
        vocab_ok and dtype_ok and accs,
    ...
    erased_ok = all(r["scale_dtype"] == "unavailable" and r["dequant_location"] == "unavailable" for r in accs)
    ...
    rs_ok = all(
        p["has_bias"] == "False"
        and p["has_activation"] == "False"
        and p["has_scale"] == "False"
        and p["has_clamp"] == "False"
        for p in pats
        if p["reshape_separated_epilogue"] == "True"
    )
    p10check(rs_ok, f"[{w}] reshape-separated matmuls claim no directly-fused epilogue")
    return {"workload": w, "matmuls": len(recs), "bias_ops": sum(1 for p in pats if p["has_bias"] == "True")}
...
def verify_p10_global() -> None:
    ...
    cons_ok = all((c["evidence"] == "recovered_from_ir") == bool(c["present_in_workloads"]) for c in units.values())
    ...
    must_block = {
        ...
        "scale_object",
        "packed_lowbit_tensor",
        "resident_packed_weight_object",
        "structured_sparsity_skip",
    }
...
def verify_p12() -> dict:
    from merlin.dse_guidance.boundary_placement import ABSTRACTIONS, LEVELS, REGION_ROLES, RESP_CELLS, STATUS
    ...
    p12check(
        all(c["abstraction"] in ALL_ABS for c in certs) and len(certs) == len(ALL_ABS),
        f"every boundary candidate references a known abstraction ({len(certs)})",
    )
    # (2) every workload reference exists
    p12check(
        all(w in valid_wl for c in certs for w in c["supporting_workloads"]), "every boundary workload reference exists"
    )
    # (3) every region role reference is real
    p12check(
        all(r in REGION_ROLES for c in certs for r in c["region_roles"]), "every boundary region-role reference is real"
    )
    # (4) every boundary level uses the allowed vocabulary
    lv_ok = all(b["level"] in LEVELS for c in certs for b in c["boundary_levels"]) and all(
        {b["level"] for b in c["boundary_levels"]} == set(LEVELS) for c in certs
    )
    p12check(lv_ok, "every boundary level uses the allowed 6-level vocabulary")
    # (5) every status uses the allowed vocabulary
    p12check(
        all(b["status"] in STATUS for c in certs for b in c["boundary_levels"]),
        "every placement status uses the allowed vocabulary",
    )
    # (6) every required proof references a CP-matrix axis or is unavailable
    proof_ok = all(
        (c["compiler_proof_matrix_axis"] in cp_axes) or (c["compiler_proof_status"] == "unavailable") for c in certs
    )
    ...
    p12check(
        knobs and all(k.get("reason") and k.get("evidence") for k in knobs),
        f"every boundary DSE knob has a reason + evidence ({len(knobs)})",
    )
    # (8) every responsibility-matrix cell uses the allowed vocabulary
    cols = ["compiler", "runtime_hal", "command_processor", "accelerator_isa", "device_microcode", "datapath"]
    resp = _csv_rows(CS / "responsibility_split_matrix.csv")
    p12check(
        resp and all(r[col] in RESP_CELLS for r in resp for col in cols),
    ...
    p12_files = [
        ...
        "boundary_placement_report.md",
        "responsibility_split_matrix.csv",
        "interface_contract_sketches.md",
    ...
    try:
        ...
        partial_ok = (
            len(part) == len(ALL_ABS)
            and all(c.supporting_workloads == [] for c in part)
    ...
    return {"abstractions": len(certs), "top": top["abstraction"], "top_score": top["boundary_pressure_score"]}
...
def verify_p13() -> dict:
    ...
    for scope in scopes:
        ...
        p13check(
            all(ok for ok, _ in b["consistency_checks"]),
        ...
        p13check(
            all(f["evidence_tier"] in ("A", "B") for f in main),
        ...
        p13check(
            all(all(m in IM.SIGNAL_METRICS for m in f["relevant_metrics"]) for f in main),
        ...
        p13check(
            not oa, f"[{scope}] gap_audit: 0 open avoidable gaps (found {len(oa)}: {[g['category'] for g in oa][:4]})"
        )
        # P14: every inherent limit is scoped to a required input (no bare caveats)
        p13check(
            b["required_inputs"] and all(x.get("required_input") for x in b["required_inputs"]),
    ...
    p13check(not missing, f"[all] coverage == all {len(expected_arts)} expected artifacts (unused: {missing[:5]})")
    ...
    p13check(measured and lowbit, f"[all] real measured ({len(measured)}) + low-bit ({len(lowbit)}) evidence surfaced")
    ...
    leaked = [
        t
        for t in ("speedup", "faster", "optimal", "performance improvement", "gap_closure", "predicted cycles")
        if t in blob
    ]
    ...
    p13check(
        part is not None
        and all("exists" in r for r in part["inventory"])
...
def verify_p15(IM, bundle) -> None:
    ...
    ctx_in_canon = [r["metric"] for r in canon if facts_by_metric.get(r["metric"], {}).get("metric_class") == "context"]
    p13check(
        canon and not ctx_in_canon,
        f"[P15] canonical_signal_table is signal/measured only ({len(canon)} rows, context leaks: {ctx_in_canon[:4]})",
    )
    ...
    p13check(
        not unmapped and not canon_noq and not main_noq and not bad_q,
    ...
    p13check(
        refs_real and macs_match and h["n_ops"] == len(ops),
    ...
    pad_ok = all(
        abs(r["best_tile_padding_waste"] - best[(r["workload"], r["op_index"])]) < 1e-9 for r in h["by_padding_waste"]
    )
    ...
    for r in cov:
        ...
        if (
            abs(r["mac_coverage"] - exp_mac) > 1e-6
            or abs(r["byte_coverage"] - exp_byte) > 1e-6
    ...
    p13check(
        cov and cov_ok,
        f"[P15] abstraction coverage (workload/MAC/byte/region) recomputes from source ({len(cov)} rows)",
    )
    ...
    scoped_ok = (
        hs["by_macs"]
        and all(r["workload"] == one for r in hs["by_macs"])
        and hs["n_ops"] == sum(1 for o in ops if o["workload"] == one)
        and {r["workload"] for r in bn["canonical_signal"] if r["workload"] not in ("ALL", "ZOO", "")} == {one}
    )
    withheld_ok = not bn["abstraction_coverage"] and not bn["corpus_plan"] and not bn["family_summary"]["families"]
    p13check(
        scoped_ok and withheld_ok,
    ...
    p13check(
        real and none_captured and cp["n_missing"] == len(missing),
    ...
    plots = bundle["plots"]
    no_cap = [p["plot_id"] for p in plots if p["recommendation"] != "omit" and not p.get("dse_caption")]
    p15_blob = (
        str(canon)
        + str(h)
        + str(cov)
        + str(bundle["family_summary"])
        + str(cp)
        + str([p.get("dse_caption") for p in plots])
    ).lower()
    leaked = [
        t
        for t in (
    ...
    p13check(
        not no_cap and not leaked,
        f"[P15] every presented plot has a DSE caption + no forbidden wording (no_cap {no_cap[:3]}, leaked {leaked})",
    )
...
def verify_p16(IM, bundle) -> None:
    ...
    p13check(
        all_cells_valid and not_all_nec and blocked_ok and has_na_cell,
    ...
    for w in wls:
        dense_by_wl[w] = sum(
            float(r["mac_fraction"]) for r in shape if r["workload"] == w and r["shape_class"] == "squareish_gemm"
        )
    ...
    p13check(
        abs(worst_indep - fr[2]["worst"]) < 1e-6,
    ...
    pok = all(
        r[f"k_macs_{int(t * 100)}"] <= r["n_ops"] and r["k_macs_50"] <= r["k_macs_95"]
        for r in par["rows"]
        for t in par["thresholds"]
    )
    rdt = next(r for r in par["rows"] if r["workload"] == "rdt")
    p13check(
        pok and rdt["k_macs_50"] == 1, f"[P16] operator Pareto monotone + within n_ops; rdt k@50%MAC={rdt['k_macs_50']}"
    )
    ...
    p13check(
        frac_ok and loo_ok and sorted(dom["collapses_if_removed"]) == derived,
    ...
    blob = (
        str(nec)
        + str(bundle["primitive_frontier"])
        + str(par)
        + str(bundle["robustness"])
        + str(cf)
        + str(bundle["decision_scorecard"])
    ).lower()
    leaked = [
        t
        for t in ("speedup", "faster", "optimal", "performance improvement", "predicted cycles", "x speedup")
        if t in blob
    ]
    p13check(
        caps_ok and not leaked, f"[P16] new plots captioned + no forbidden wording (caps_ok={caps_ok}, leaked={leaked})"
    )
...
def verify_p17_audit(IM, bundle) -> None:
    ...
    p13check(
        not k_leak and rwo_has_k and rwo_uses_k,
    ...
    cells_ok = all(
        r["predicate_inputs"] and r["thresholds"] and r["classification"] in IM._NEC_CLASSES for r in pa["rows"]
    )
    susp = [r for r in pa["rows"] if r["suspicious"]]
    p13check(
        cells_ok and susp,
        f"[P17] predicate audit well-formed ({len(pa['rows'])} cells); suspicious flagged ({len(susp)})",
    )
    ...
    split_ok = (
        all("true_gemv=" in r["predicate_inputs"] and "skinny_gemm=" in r["predicate_inputs"] for r in gemv_cells)
        and gemv_cells
    )
    p13check(
        "vector_gemv_engine" not in names and "skinny_gemm_or_gemv_engine" in names and split_ok,
    ...
    p13check(
        rows_ok and flagged and loo_complete,
    ...
    aud_ok = all(
        r["conclusion"] and r["supporting_artifact"] and r["verdict"] and any(v in r["metric_class"] for v in vocab)
        for r in aud
    )
    ...
    p13check(
        aud_ok and not leaked,
        f"[P17] adversarial audit references artifacts + verdicts ({len(aud)}); no forbidden wording (leaked={leaked})",
    )
...
def verify_p17_envelope(IM, bundle) -> None:
    ...
    req = {(r["workload"], r["region"], r["requirement"]): r["value"] for r in _rows("requirements_table.csv")}
    from merlin.dse_guidance.models import MODEL_ARCH, _base_model

    tagged = all(
        r["K_basis"] in ("configured", "sweep") and ("sweep" in r["deadline_basis"] or "derived" in r["deadline_basis"])
        for r in env["rows"]
    )
    recompute_ok = True
    for r in env["rows"][:200]:
        ...
        if abs(expect - r["required_compute_MAC_per_s"]) > max(1.0, expect * 1e-6):
            ...
            break
    p13check(
        env["rows"] and tagged and recompute_ok,
    ...
    cmd_ok = all(
        ("proxy_only" in r["command_rate_basis"]) or (r["workload"] in IM._MEASURED_DISPATCH) for r in env["rows"]
    )
    blob = (str(env) + str(bundle["omitted_ops"])).lower()
    no_hw = "measured hardware" not in blob and "measured performance" not in blob
    p13check(cmd_ok and no_hw, f"[P17] command rate proxy-tagged (ok={cmd_ok}); no measured-hardware label ({no_hw})")
    ...
    p13check(
        om and macs_match and frac_ok and lowbit_erased and attn_recovered,
    ...
    plots_ok = all(
        pid in pm and pm[pid]["recommendation"] != "omit" and pm[pid].get("dse_caption") for pid in new_plots
    )
    ...
    p13check(
        ce and no_lowbit and len(loops) <= 1 and fam_ok,
    ...
    if ab["rows"]:
        ...
        p13check(
            named_attn and named_quant and no_loops,
...
def verify_p20(IM) -> None:
    ...
    dims_ok = all(
        int(s["problem"]["instance"]["M"]) * int(s["problem"]["instance"]["N"]) * int(s["problem"]["instance"]["K"])
        == int(s["macs_per_instance"])
        for s in shapes
    )
    ds_ok = all(
        len(s["problem"]["shape"]["data-spaces"]) == 3 and len(s["problem"]["shape"]["dimensions"]) == 3 for s in shapes
    )
    attn_ok = all(
        (
            "weight_stationary" not in s["dataflow_candidates"]
            and all(d.get("operand_identity") != "weight" for d in s["problem"]["shape"]["data-spaces"])
        )
        for s in shapes
        if s["op_class"] == "attention_contraction"
    )
    lin_ok = all(
        any(d.get("operand_identity") == "weight" for d in s["problem"]["shape"]["data-spaces"])
        for s in shapes
    ...
    p13check(
        dims_ok and ds_ok and attn_ok and lin_ok and cnt_ok and not leaked,
    ...
    dm = {
        (r["workload"], r["region"]): r
        for r in _csv.DictReader(_io.StringIO((CS / "data_movement_table.csv").read_text()))
    }
    ...
    wb_ok = all(
        _i(r["bytes"]) == _i(dm.get((r["workload"], r["region"]), {}).get("weight_bytes"))
        for r in loc
        if r["operand"] == "weight"
    )
    wt_resident = all(
        r["resident_candidate"].startswith("yes") for r in loc if r["operand"] == "weight" and _i(r["bytes"]) > 0
    )
    p13check(
        loc and scope_ok and wb_ok and wt_resident,
    ...
    if qmf.is_file():
        ...
        qm_ok = bool(qm) and all(
            _i(r["n_dequant_ops"]) > 0 and r["scale_granularity"] in GRAN and r["storage_dtype"] for r in qm
        )
        native_ok = any("ternary" in r["native_scheme_gap"].lower() for r in qm if r["workload"] == "bitvla")
        p13check(
            qm_ok and native_ok,
...
def verify_p21(IM) -> None:
    ...
    if not any((_cap_base(w, "recaptures_loop") / "model.mlir").is_file() for w in ("smolvla", "openvla", "pi05")):
    ...
    EXPECT = {"smolvla": (10, "latent", None), "openvla": (7, "kv_cache", 221184), "pi05": (10, "latent", None)}
    re_ok = True
    for w, (k, role, kvb) in EXPECT.items():
        ...
        re_ok = (
            ...
            and lr.present
            and lr.K == k
            and lr.K_source == "recovered_from_ir"
            and role in roles
            and lr.kv_cache_bytes == kvb
            and lr.repeated_region_op_count > 50
        )
    # 1b. corpus-wide: EVERY present loop-preserving capture re-derives a valid loop from the IR
    present = sorted(w for w in RECAP_MODELS if (_cap_base(w, "recaptures_loop") / "model.mlir").is_file())
    corpus_ok = True
    for w in present:
    ...
    if af.is_file():
        ...
        art_ok = all(
            w in ar
            and ar[w]["K_source"] == "recovered_from_ir"
    ...
    flip_ok = (
        all("recovered" in kloop[w] and "IR" in kloop[w] for w in EXPECT)
        and all("recovered" in lcs[w] for w in EXPECT)
        and "recovered" in kv["openvla"]
        and "221184" in kv["openvla"]
    )
    blob = af.read_text().lower() if af.is_file() else ""
    leaked = [t for t in ("speedup", "faster", "optimal", "cycles", "throughput") if t in blob]
    p13check(
        re_ok and art_ok and flip_ok and not leaked,
    ...
    if res_ok:
        ...
        for w in present:
            ...
            res_ok = (
                res_ok
                and rc.present
                and rc.n_loop_invariant_operands > 0
                and rc.n_loop_carried >= 1
                and w in rar
    ...
    arts_ok = (
        mf.is_file()
        and kf.is_file()
        and all(r["evidence"] == "recovered_from_model_config" for r in _csv.DictReader(_io.StringIO(mf.read_text())))
    )
    p13check(
        mag_ok and kv_ir_ok and kv_real_ok and arts_ok,
    ...
    for w, g in RC.REAL_GEOMETRY.items():
        for s in g.stacks:
            ...
            nonnull = nonnull and all(
                isinstance(getattr(s, f), int) and getattr(s, f) > 0
                for f in ("n_layers", "hidden", "interm", "heads", "kv_heads", "head_dim")
            )
    ...
    anchors = (
        6.6e9 < RC.REAL_GEOMETRY["openvla"].total_params() < 6.9e9
        and 1.0e9 < RC.REAL_GEOMETRY["tiny_llama"].total_params() < 1.2e9
        and 0.45e9 < RC.REAL_GEOMETRY["rdt2"].total_params() < 0.55e9
    )  # DiT: 473M GEMM params
    p13check(
        nonnull and anchors,
    ...
    for r in air:
        ...
        ai_ok = (
            ai_ok
            and r["ai_resident_mac_per_byte"] > r["ai_nonresident_mac_per_byte"]
    ...
    hw_free = artf.is_file() and not any(t in blob for t in ("peak_mac", "bandwidth_gb", "latency_ms", "ghz", "cycles"))
    p13check(
        ai_ok and hw_free,
    ...
    if nat_cap.is_file():
        ...
        s4_ok = (
            bv.get("storage") == "int2_packed_in_i8"
    ...
    if lbv.is_file():
        ...
        no_perf = not any(
            t in lbv.read_text().lower() for t in ("speedup", "faster", "latency", "throughput", "cycles")
        )
        no_assumed_lowbit = not any(
            "int4" in r["accuracy_status"] or "fp8" in r["accuracy_status"] for r in got.values()
        )
    ...
    if rtf.is_file():
        ...
        all_req = all(
            "required_from_recovered_structure" in r["evidence"] and "design_target" in r["evidence"] for r in rt
        )
...
def main(write: bool = True) -> int:
    ...
    p6_rows = [verify_p6_workload(w, _graphs) for w in RECAP_MODELS if w in _MODELS]
    verify_p6_global(_graphs)
    p7_rows = [verify_p7_workload(w, _graphs) for w in RECAP_MODELS if w in _MODELS]
    verify_p7_global()
    p8_rows = [verify_p8_workload(w, _graphs) for w in RECAP_MODELS if w in _MODELS]
    verify_p8_global()
    p9_rows = [verify_p9_workload(w) for w in RECAP_MODELS if w in _MODELS]
    verify_p9_global()
    p10_rows = [verify_p10_workload(w) for w in RECAP_MODELS if w in _MODELS]
    verify_p10_global()
    ...
    for r in rows:
        L.append(
            f"| {r['workload']} | {r['K']} | {r['matmuls']} | {r['weight_bytes']:,} | "
    ...
    L.append("|---|---|---|---|")
    for r in p5_rows:
        L.append(f"| {r['workload']} | {r['operators']} | {r['shape_classes']} | {r['semantic_classes']} |")
    L.append("\n### P5 checks\n")
    ...
    for r in p6_rows:
        L.append(
            f"| {r['workload']} | {r['nodes']} | {r['edges']} | {r['phase']} | "
    ...
    L.append(
        "Inter-op concurrency (recomputed from the DAG); low values mean the parallelism "
    ...
    L.append("|---|---|---|---|")
    for r in p7_rows:
        L.append(f"| {r['workload']} | {r['available_parallelism']}× | {r['max_ready_width']} | {r['serialization']} |")
    L.append("\n### P7 checks\n")
    ...
    L.append(
        f"**{p12_passed}/{len(p12_results)} P12 checks passed.** "
    ...
    L.append(
        f"**{p13_passed}/{len(p13_results)} P13 checks passed.** "
