...
- workload class: **autoregressive_decode**  ·  K=32, H=32, control_rate=30.0 Hz
...
- **signal**: temporal_reuse — action-head weights are loop-invariant across K=32 steps; a flat capture sees one use and hides this axis
- **evidence**: `{'K': 32, 'loop_invariant_state': ['weights'], 'head_phases': ['head']}`
...
- **attributed IR facts (Level-1)**: `{'matmul_count': 15, 'macs_per_invocation': 614465536, 'weight_bytes': 614465536, 'activation_bytes_per_invocation': 1691648, 'invocations': 32, 'macs_total': 19662897152, 'activation_bytes_total': 54132736}`
...
## autonomous_K_loop
- **signal**: temporal_reuse — a bounded K=32 loop with loop-carried state can run device-side
- **evidence**: `{'K': 32, 'loop_carried_state': []}`
...
- **evidence**: `{'backbone_phases': ['backbone'], 'head_phases': ['head'], 'K': 32, 'H': 32, 'control_rate_hz': 30.0, 'replan_deadline_ms': 1066.6666666666667}`
...
- **evidence**: `{'H': 32, 'control_rate_hz': 30.0, 'replan_deadline_ms': 1066.6666666666667}`
...
## command_batching
- **signal**: cpu_coupling — the K=32 head steps issue repeated host submits a flat capture hides
- **evidence**: `{'K': 32, 'note': 'K per-step submits are collapsible to one buffer'}`
...
- **evidence**: `{'workload_class': 'autoregressive_decode', 'K': 32}`
