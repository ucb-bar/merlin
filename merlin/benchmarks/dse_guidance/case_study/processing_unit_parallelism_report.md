...
|---|---|---|---|---|
| dense_gemm | True | 158 | 35.0% | compute_macs |
| skinny_gemm_or_gemv | True | 847 | 63.7% | compute_macs |
...
- **Average inter-op parallelism is low (1.28×)** (see `dag_parallelism_report.md`): the dependency DAG is near-sequential, so **many identical units would be hard to keep busy** by inter-op concurrency alone — the parallelism to exploit is *intra-op* sharding.
- **Compute splits across two shapes:** dense GEMM is 35% of MACs (pi05, rdt, rdt2, smolvla) while skinny/GEMV is 64% (groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama). This favors **specialized units** (a matrix engine *and* a GEMV/vector engine) over one universal unit.
...
|---|---|---|
| matrix_engine | serves dense_gemm: 35% of MACs across pi05, rdt, rdt2, smolvla | pi05, rdt, rdt2, smolvla |
| vector_gemv_engine | serves skinny_gemm_or_gemv: 64% of MACs across groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama | groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama |
...
| dma_engine | serves dma_or_memory (structural; structural across groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama) | groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama |
| loop_controller | serves control_or_dispatch (structural; structural across groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama) | groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama |
| scalar_control_unit | serves control_or_dispatch (structural; structural across groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama) | groot_n1d7, molmoact, openvla, pi05, rdt, rdt2, small_llama, smolvla, tiny_llama |
