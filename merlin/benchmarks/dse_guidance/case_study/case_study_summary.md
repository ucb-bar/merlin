...
| openvla | backbone_once:8, repeated_head:15, unknown:3 | 0 GMAC/replan, 0.02 GB avoidable reload | resident bf16 2 MB | resident_weight_object | real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
| small_llama | repeated_head:14, unknown:1 | 0 GMAC/replan, 0.05 GB avoidable reload | resident bf16 1 MB | resident_weight_object | real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
