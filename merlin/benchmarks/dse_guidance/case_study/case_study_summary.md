...
| rdt2 | repeated_head:23 | 5 GMAC/replan, 1.21 GB avoidable reload | resident bf16 151 MB | resident_weight_object | real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
| groot_n1d7 | backbone_once:2, repeated_head:16 | 10 GMAC/replan, 0.89 GB avoidable reload | resident bf16 148 MB | resident_weight_object | quantization accuracy gates (per candidate low-bit format); real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
| molmoact | repeated_head:17 | 61 GMAC/replan, 26.51 GB avoidable reload | resident bf16 1894 MB | resident_weight_object | quantization accuracy gates (per candidate low-bit format); real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
| smolvla | backbone_once:87, repeated_head:19 | 7 GMAC/replan, 0.28 GB avoidable reload | resident bf16 16 MB | resident_weight_object | quantization accuracy gates (per candidate low-bit format); real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
| pi05 | backbone_once:489, repeated_head:288 | 18285 GMAC/replan, 82.90 GB avoidable reload | resident bf16 4605 MB | resident_weight_object | quantization accuracy gates (per candidate low-bit format); real (target) command-submit / sync latency; K / control-rate from the real deployment (currently reference values) |
