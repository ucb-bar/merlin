# Structural DSE candidate axes — groot_n1d7

- workload class: **flow_matching_action_head**  ·  K=4, H=16, control_rate=30.0 Hz
...
- **signal**: temporal_reuse — action-head weights are loop-invariant across K=4 steps; a flat capture sees one use and hides this axis
- **evidence**: `{'K': 4, 'loop_invariant_state': ['weights'], 'head_phases': ['head']}`
...
- **attributed IR facts (Level-1)**: `{'matmul_count': 116, 'macs_per_invocation': 20393361408, 'weight_bytes': 2200436736, 'activation_bytes_per_invocation': 79920128, 'invocations': 4, 'macs_total': 81573445632, 'activation_bytes_total': 319680512}`
...
## autonomous_K_loop
- **signal**: temporal_reuse — a bounded K=4 loop with loop-carried state can run device-side
- **evidence**: `{'K': 4, 'loop_carried_state': []}`
...
- **evidence**: `{'backbone_phases': ['backbone'], 'head_phases': ['head'], 'K': 4, 'H': 16, 'control_rate_hz': 30.0, 'replan_deadline_ms': 533.3333333333334}`
...
- **evidence**: `{'H': 16, 'control_rate_hz': 30.0, 'replan_deadline_ms': 533.3333333333334}`
...
## command_batching
- **signal**: cpu_coupling — the K=4 head steps issue repeated host submits a flat capture hides
- **evidence**: `{'K': 4, 'note': 'K per-step submits are collapsible to one buffer'}`
