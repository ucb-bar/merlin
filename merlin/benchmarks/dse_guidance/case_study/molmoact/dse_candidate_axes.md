# Structural DSE candidate axes — molmoact

- workload class: **autoregressive_decode**  ·  K=8, H=8, control_rate=5.0 Hz
...
- **signal**: temporal_reuse — action-head weights are loop-invariant across K=8 steps; a flat capture sees one use and hides this axis
- **evidence**: `{'K': 8, 'loop_invariant_state': ['weights'], 'head_phases': ['head']}`
...
- **attributed IR facts (Level-1)**: `{'matmul_count': 17, 'macs_per_invocation': 7574913024, 'weight_bytes': 3787456512, 'activation_bytes_per_invocation': 10403840, 'invocations': 8, 'macs_total': 60599304192, 'activation_bytes_total': 83230720}`
...
## autonomous_K_loop
- **signal**: temporal_reuse — a bounded K=8 loop with loop-carried state can run device-side
- **evidence**: `{'K': 8, 'loop_carried_state': []}`
...
- **evidence**: `{'backbone_phases': ['backbone'], 'head_phases': ['head'], 'K': 8, 'H': 8, 'control_rate_hz': 5.0, 'replan_deadline_ms': 1600.0}`
...
- **evidence**: `{'H': 8, 'control_rate_hz': 5.0, 'replan_deadline_ms': 1600.0}`
...
## command_batching
- **signal**: cpu_coupling — the K=8 head steps issue repeated host submits a flat capture hides
- **evidence**: `{'K': 8, 'note': 'K per-step submits are collapsible to one buffer'}`
...
- **evidence**: `{'workload_class': 'autoregressive_decode', 'K': 8}`
