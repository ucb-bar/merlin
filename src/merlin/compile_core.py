...
def compile_core_mlir(
    ...
    **llvm_kwargs: Any,
    ...
    ``workdir`` is required only for the LLVM path (it emits files). ``ir_audit=True``
    writes exact named-stage snapshots under an explicit workdir on either path;
    it is opt-in because dense constants can make snapshots large. ``audit_sidecars``
    binds explicitly declared weight/manifest files without converting or copying them.
    ``ir_audit="compact"`` requests native inspection views with hash-only records
    for unsupported printers; ``"both"`` also retains exact snapshots.
    Other keyword arguments are forwarded to :func:`llvmlower.lower.lower_model`.
    """
    from merlin.common.ir_audit import audit_mode

    audit_mode(ir_audit)  # Reject invalid options before routing or executing passes.
    ...
    res = lower_model(text(module), workdir, ir_audit=ir_audit, audit_sidecars=audit_sidecars, **llvm_kwargs)
