...
- `objdump.py` — Generic, ISA-agnostic disassembly tokenizer.
  Owns normalized hex-column conversion (`word_of`, explicit instruction width)
  and unknown-mnemonic vocabulary shared by semantic decoders. Generic RoCC and
  derived-ISA decoding must not import an accelerator-specific decoder for these.
