...
__all__ = ["Census", "CensusRow", "Legality", "census", "census_bundle", "legality_of", "to_markdown"]
...
class Census:
    ...
    def by_verdict(self, verdict: str) -> tuple[CensusRow, ...]:
        return tuple(r for r in self.rows if r.legality is not None and r.legality.verdict == verdict)
...
def legality_of(shape: ContractionShape, units: Sequence[Any], *, site: str = "", undecidable: str = "") -> Legality:
    ...
    if not units:
        return Legality(UNKNOWN, ROUTING_SCOPE, reason=undecidable or "no compute units to route against")
    if len(shape.dtypes) < 3:
        return Legality(UNKNOWN, ROUTING_SCOPE, reason="operand element types not observed on this op")
    ...
    demand = routing.OpDemand(op=_demand_op(shape.op), in_fmt=in_fmt, weight_fmt=weight_fmt, site=site)
...
def _unit_vocabulary(units: Sequence[Any]) -> str:
    ...
    vocab = [
        f"{u.name}(kind={u.kind}, ops=[{','.join(u.ops) or '<any>'}], "
        f"dtypes=[{','.join(u.dtypes) or '<none declared>'}])"
        for u in units
    ]
...
def _ticks_by_key(
    prof_table: Sequence[Mapping[str, Any]] | None, prof_ticks: Mapping[int, tuple[int, int]] | None
...
def census(
    ...
    *,
    ...
    notes: tuple[str, ...] = (
        (undecidable + "; legality is unknown for every row",) if undecidable else (_unit_vocabulary(units),)
    )
    ...
    if shared:
        notes += (
            f"{shared} row(s) share a provenance key with another profiled op: their ticks are "
...
def census_bundle(
    ...
    *,
    ...
    if not src.is_file():
        return Census(model=model, stage="missing", source=str(src), target=target, notes=(f"no model.mlir at {src}",))
    ...
    if work_dir is not None:
        ...
        try:
            ...
            extra = (
                f"preparation failed ({type(exc).__name__}: {exc}); read the raw capture, so "
                "element types are the CAPTURE's and not the ones a build would compile",
            )
    else:
        extra = ("no work dir given: read the raw capture, whose element types precede the quant rewrite",)
    ...
    return Census(
        ...
        ranked_by=got.ranked_by,
        notes=got.notes + extra,
    )
...
def to_markdown(censuses: Sequence[Census], *, top: int = 20) -> str:
    ...
    for c in censuses:
        ...
        if overall is not None:
            out += [
                f"- measured share of whole-model time, each tick bucket counted once "
            ...
            out += [
                "",
                "The per-row `% model` column below must NOT be summed: rows sharing a bucket "
        ...
        if not c.rows:
            ...
            continue
        out += [
            "| # | family | op class | parallel / reduction | dtypes | work | ticks | % model | verdict | reason |",
            "|---|---|---|---|---|---|---|---|---|---|",
        ]
        for r in c.rows[:top]:
            lg = r.legality
...
def profile_usability(doc: Mapping[str, Any]) -> tuple[bool, str]:
    ...
    if pert and not pert.get("perturbation_ok", True):
        return False, (
            f"instrumentation moved the wall by {pert.get('delta_pct')}%, over the "
...
def load_profile(
    path: "str | Path", *, require_usable: bool = True
...
def main(argv: "Sequence[str] | None" = None) -> int:
    ...
    ap.add_argument(
        "--no-prepare", action="store_true", help="read the raw capture (element types will PRECEDE the quant rewrite)"
    )
    ap.add_argument(
        "--allow-unusable-profile",
        action="store_true",
    ...
    prod = new_product(
        "target-evolution", version=1, target=a.target or None, notes=f"contraction census, dtype={a.dtype}"
    )
    ...
    for name in [m.strip() for m in a.models.split(",") if m.strip()]:
        ...
        if bundle is None:
            ...
            censuses.append(
                Census(model=name, stage="missing", source="", target=a.target or None, notes=("bundle not found",))
            )
            continue
        ...
        print(
            f"{resolved}: {len(got.rows)} contractions at stage={got.stage}, "
    ...
    payload = {
        "dtype": a.dtype,
        "target": a.target or None,
        "legality_scope": ROUTING_SCOPE,
        "models": [c.to_dict() for c in censuses],
    }
    prod.add_artifact("workload_census.json").write_text(json.dumps(payload, indent=1) + "\n", encoding="utf-8")
    prod.add_artifact("workload_census.md").write_text(to_markdown(censuses, top=a.top), encoding="utf-8")
    prod.write_manifest()
    print(f"\nwrote {prod.path}")
    return 0
