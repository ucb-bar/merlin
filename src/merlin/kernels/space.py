...
def space_from_records(records, *, axes, target: str) -> DesignSpace:
    ...
    for axis in axes:
        ...
        if len(levels) > 1:
            ...
            space.notes.append(
                f"axis {axis!r} is constant at {levels[0]!r} across the corpus — not a "
    ...
    for r in recs:
        ...
        space.cells[key] = Cell(coords=coords, observed=True, n_kernels=(prev.n_kernels if prev else 0) + 1)
    return space
...
def to_search_space_doc(
    space: DesignSpace, *, name: str | None = None, candidate_type: str = "compilation_strategy", method: str = "grid"
) -> dict:
    ...
    return {
        ...
        "objective": {
            ...
            "note": (
                "derived from a corpus sample, so the axes are the ones that mattered on real "
        ...
        "provenance": {
            "target": space.target,
            "observed_cells": len(space.observed),
            "unobserved_cells": len(space.unobserved()),
...
def corpus_prior(space: DesignSpace):
    ...
    def prior(point: dict) -> dict:
        ...
        if cell is None:
            return {"observed": False, "n_kernels": 0, "improved": 0, "regressed": 0, "failed": 0, "attempted": 0}
        return {
            "observed": cell.observed,
            "n_kernels": cell.n_kernels,
...
def candidates(space: DesignSpace, *, limit: int | None = None) -> list[dict]:
    ...
    out = [{"cell": c.key, "coords": dict(c.coords), "steps_from_observed": _distance(c)} for c in ranked]
    if limit is not None and len(out) > limit:
        dropped = len(out) - limit
        out = out[:limit]
        out.append(
            {"cell": f"... and {dropped} more unobserved cells not listed", "coords": {}, "steps_from_observed": -1}
        )
    return out
