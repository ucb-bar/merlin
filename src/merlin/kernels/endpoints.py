...
class Endpoint:
    ...
    notes: str = ""
    #: Were the bound roles CONFIRMED against the target's own derived table, or merely DECLARED?
    #: False means no derived table was available, so ``roles`` is the spec's hand declaration passed
    #: through unchanged — indistinguishable, until this field existed, from a binding the RTL
    #: confirmed. It used to be free text inside ``source`` ("[unverified: …]"), which nothing branched
    #: on, so a hand declaration and a derived one read the same to every consumer.
    verified: bool = True
    #: Why verification did not happen. Empty when ``verified`` is True.
    unverified_reason: str = ""

    def roles_if_verified(self) -> dict[str, tuple[str, ...]]:
        """The role bindings, or ``{}`` when nothing confirmed them.

        The honest reading of an unverified endpoint for a consumer that treats ``roles`` as EVIDENCE
        about the hardware. A consumer that wants the declaration anyway reads ``roles`` directly and
        owns the fact that it is a declaration."""
        return dict(self.roles) if self.verified else {}
    ...
    def engines_evidenced(self) -> frozenset[str]:
        ...
        FAIL CLOSED on an UNVERIFIED endpoint: with no derived table to confirm them, the roles are a
        declaration, and a declaration is not evidence. Returning them here would let a target claim a
        vector or spatial engine on the strength of its own yaml — which is exactly the shape of
        defect this method's name promises to avoid. Read :attr:`roles` (or
        :meth:`roles_if_verified`) if the declaration itself is what you want."""
...
def endpoint_names(target: str | None = None) -> tuple[str, ...]:
    ...
    return tuple(sorted(n for n, b in eps.items() if target is None or (b or {}).get("target") == target))
...
def _derived_names(target: str, block: dict) -> tuple[set[str], str]:
    ...
    if source == "rtl_facts":
        ...
        table = next((i for i in body.get("interfaces", ()) if i.get("name") == enc.get("table")), {})
    ...
    if source == "matrix_units":
        try:
            unit = _matrix_unit(block)
        except (OSError, ValueError, KeyError) as exc:
            return set(), f"matrix_units(unavailable: {exc})"
    ...
    if source == "mnemonic_grammar":
        ...
        # vocabulary. Returning the declared set means every role binds, and the loader marks the
        # result ``verified=False`` (a field, not a phrase in a string) so nobody mistakes "declared"
        # for "confirmed against silicon". The asm audit then reports every observed mnemonic no role
        # covers, so the gap stays visible.
...
def _matrix_unit(block: dict) -> dict:
    """One selected provider's matrix declaration, never a checkout-global fallback."""
    from merlin.targetgen.target_registry import load_matrix_contract

    enc = block.get("encoding") or {}
    units = load_matrix_contract(str(block.get("target") or ""))["units"]
    unit = units.get(enc.get("unit"))
    if not isinstance(unit, dict):
        raise ValueError(f"selected matrix contract declares no unit {enc.get('unit')!r}")
    return unit


def _roles_from_matrix_units(block: dict) -> dict[str, tuple[str, ...]]:
    """Roles declared by the selected provider, or none when unavailable."""
    enc = block.get("encoding") or {}
    try:
        unit = _matrix_unit(block)
    except (OSError, ValueError, KeyError):
        return {}
    declared = unit.get(enc.get("roles_from") or "kernel_roles") or {}
...
def load_endpoint(name: str) -> Endpoint:
    ...
    if not declared:
        declared = (
            _roles_from_isa_model(target)
            if enc_source == "isa_model"
            else _roles_from_matrix_units(block)
            if enc_source == "matrix_units"
            else {}
        )
        ...
        if engine:
            declared = {r: v for r, v in declared.items() if _roles.engine_of(r) in (engine, None)}
    ...
    if not names:
        ...
        # missing -- an absent toolchain is not evidence about the hardware. But say it STRUCTURALLY:
        # the marker used to be a phrase appended to ``source``, and grep found no consumer that
        # branched on it, so every one of these hand declarations was consumed as though the target's
        # own table had confirmed it.
        bound = dict(declared)
        verified = False
        unverified_reason = (
            f"no derived instruction table was available for {target!r} via {how}; "
            f"the {len(declared)} declared role binding(s) are UNCONFIRMED"
        )
    ...
    return Endpoint(
        ...
        target=target,
        engine=engine,
        exposure=str(block.get("exposure") or ""),
        source=how,
        roles=bound,
        missing=missing,
...
def _crosscheck_with_pairs(block: dict) -> dict:
    ...
    if enc.get("source") == "matrix_units" and enc.get("unit"):
        try:
            pairs = (_matrix_unit(block).get("declarations") or {}).get("crosscheck_pairs")
            if pairs:
