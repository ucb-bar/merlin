...
# Vector and spatial facets describe architecture classes: geometry is IDENTITY, while admitted
# dataflow/accumulator-residency axes are LEVER. Unimplemented facets remain BACKEND_STUB until
# their lifter and routes land. Operation identity is shared by every compute endpoint.
#
...
FIELD_REGISTRY: dict[str, FieldSpec] = {
    ...
    "compute.op": FieldSpec(
        "compute.op", IDENTITY, ("compute",), "the operation being computed — the join key, not a lever"
    ),
    "compute.contraction_form": FieldSpec(
        "compute.contraction_form", LEVER, ("rvv", "compute"), "fused_fma vs mul_add -> PASS fused_vfmacc_contraction"
    ),
    "compute.accumulator_dtype": FieldSpec(
        "compute.accumulator_dtype", LEVER, ("rvv", "compute"), "accumulate width (i32/f32) -> KNOB dtype_strategy"
    ),
    "compute.widening": FieldSpec(
        "compute.widening", LEVER, ("rvv", "compute"), "i8xi8->i32 widening MAC -> KNOB dtype_strategy=int8_w8a8"
    ),
    "compute.reduction_form": FieldSpec(
        "compute.reduction_form", LEVER, ("rvv", "compute"), "tree/vredsum reduction -> HEURISTIC lower_multi_reduction"
    ),
    "compute.register_block": FieldSpec(
        "compute.register_block", LEVER, ("rvv", "compute"), "MRxNR register block -> KNOB register-block MR"
    ),
    "compute.epilogue": FieldSpec(
        "compute.epilogue",
        LEVER,
        ("rvv", "compute"),
        "requant/narrow fused into store -> PASS fuse-requant-narrowing-store",
    ),
    "compute.accumulator_resident": FieldSpec(
    ...
    "compute.nr_is_vsetvlmax": FieldSpec(
        "compute.nr_is_vsetvlmax", LEVER, ("rvv",), "VL-adaptive output width -> HEURISTIC NR=vsetvlmax"
    ),
    "compute.activation_vectorization": FieldSpec(
        "compute.activation_vectorization",
        LEVER,
        ("rvv", "compute"),
        "vectorized poly vs scalar libm -> PASS vectorized_transcendental_activation",
    ),
    # --- vector (RVV/SIMD) ---
    "vector.sew": FieldSpec(
        "vector.sew", LEVER, ("rvv",), "element width -> KNOB dtype_strategy (sew follows element dtype)"
    ),
    "vector.lmul": FieldSpec(
        "vector.lmul", LEVER, ("rvv",), "register grouping -> KNOB vector_sizes (widen N to raise LMUL)"
    ),
    "vector.vl_strategy": FieldSpec(
        "vector.vl_strategy", LEVER, ("rvv",), "vsetvl loop vs fixed vsetivli -> PASS vl-polymorphic-tail"
    ),
    "vector.tail": FieldSpec(
        "vector.tail", LEVER, ("rvv",), "ta/tu tail policy -> KNOB tail_policy / PASS vl-polymorphic-tail (tu)"
    ),
    # --- memory (data-movement / packing — the #1 expert GEMM lever, lifted from decode.memory) ---
    "memory.access_pattern": FieldSpec(
    ...
    "memory.panel_reuse": FieldSpec(
        "memory.panel_reuse",
        METRIC,
        ("rvv",),
        "loads/FMA amortization — a diagnostic outcome of register-block + packing, not a standalone lever",
    ),
    "memory.a_broadcast_vf": FieldSpec(
        "memory.a_broadcast_vf",
        METRIC,
        ("rvv",),
        "A streamed via vfmacc.vf (no rebuild ladder) — a diagnostic outcome of instruction selection",
    ),
    # --- region (the code AROUND the loop — where the measured expert gap actually lived) ---
    "envelope.calls_in_loop": FieldSpec(
        "envelope.calls_in_loop",
        LEVER,
        ("rvv",),
        "a call inside a loop body is per-iteration overhead whatever it calls -> PASS eliminate-epilogue-copy",
    ),
    "envelope.runtime_calls": FieldSpec(
        "envelope.runtime_calls",
        LEVER,
        ("rvv",),
        "runtime escape (memrefCopy et al) instead of emitted code -> PASS/CODEGEN in-place accumulator writeback",
    ),
    "envelope.work_ins_per_mac": FieldSpec(
        "envelope.work_ins_per_mac", METRIC, ("rvv",), "N^3 coefficient — hot-loop efficiency, a diagnostic outcome"
    ),
    "envelope.overhead_ins_per_output": FieldSpec(
        "envelope.overhead_ins_per_output",
        METRIC,
        ("rvv",),
        "N^2 coefficient — per-tile overhead, a diagnostic outcome that SIZES the region gap",
    ),
    ...
    "spatial.pe_rows": FieldSpec(
        "spatial.pe_rows",
        IDENTITY,
        ("spatial",),
        "fixed systolic-array rows (mesh DIM) — a target constant, not a lever",
    ),
    "spatial.pe_cols": FieldSpec(
        "spatial.pe_cols",
        IDENTITY,
        ("spatial",),
        "fixed systolic-array cols (mesh DIM) — a target constant, not a lever",
    ),
    "spatial.dataflow": FieldSpec(
        "spatial.dataflow", LEVER, ("spatial",), "WS vs OS dataflow selection (a discovered mesh implies this lever)"
    ),
    "spatial.accumulator_resident": FieldSpec(
    ...
    "simt.warps": FieldSpec(
        "simt.warps", BACKEND_STUB, ("simt",), "warps cooperating on the region -> partition lever (routes pending)"
    ),
    "simt.threads_per_warp": FieldSpec(
        "simt.threads_per_warp",
        IDENTITY,
        ("simt",),
        "fixed warp width -- a target constant the compiler targets, not a choice (cf. spatial.pe_rows)",
    ),
    "simt.smem_resident": FieldSpec(
        "simt.smem_resident",
        BACKEND_STUB,
        ("simt",),
        "operand tile staged in shared memory across the reduction -> residency lever (routes pending)",
    ),
    ...
    "simt.barriers_in_loop": FieldSpec(
    ...
    "simt.divergence": FieldSpec(
        "simt.divergence", BACKEND_STUB, ("simt",), "uniform vs divergent control flow -> predication/partition lever"
    ),
    ...
    "memory.onchip_bytes_required": FieldSpec(
        "memory.onchip_bytes_required", METRIC, ("memory",), "METRIC: bytes the region's working set needs on chip"
    ),
...
KNOWN_OPEN: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    # Per-target: the spatial LEVER axes (dataflow, accumulator-residency) are
...
def schema_axes() -> set[str]:
    ...
    return {f"{fname}.{fld.name}" for fname, cls in FACET_CLASSES.items() for fld in dataclasses.fields(cls)}
...
def dump_contract(backend: str, path: str | Path, *, toolchain_version: dict[str, Any] | None = None) -> Path:
        ...
        rows.append(
            {
                ...
                "routes": [
                    {"action_class": r.action_class, "target_seam": r.target_seam, "forkable_now": r.forkable_now}
                    for r in rs
                ],
    ...
    doc = {
        ...
        "report": {
            "orphan_fields": report.orphan_fields,
            "orphan_routes": report.orphan_routes,
            "unclassified": report.unclassified,
            "ladder_errors": report.ladder_errors,
        },
        "axes": rows,
    }
    p = Path(path)
