...
def resolve_extent(extent: "int | str", tile: int) -> int:
    ...
    if not sep:
        ...
        if head.strip() != _TILE:
            raise ValueError(
                f"unsupported extent expression {extent!r}; expected an int, 'tile', "
...
class Case:
    ...
    def resolved(self, tile: int) -> "Case":
        ...
        return Case(
            name=self.name,
            m=resolve_extent(self.m, tile),
            n=resolve_extent(self.n, tile),
            k=resolve_extent(self.k, tile),
            why=self.why,
            bias=self.bias,
    ...
    def operands(self) -> tuple[np.ndarray, np.ndarray, np.ndarray | None]:
        ...
        if not all(isinstance(e, int) for e in (self.m, self.n, self.k)):
            raise ValueError(
                f"case {self.name!r} still has a tile-relative extent "
        ...
        bias = rng.integers(-1000, 1001, size=(self.n,)).astype(np.int32) if self.bias else None
        return lhs, rhs, bias
...
def reference(
    lhs: np.ndarray, rhs: np.ndarray, bias: np.ndarray | None = None, *, acc_bits: int = _ACC_BITS
) -> np.ndarray:
...
_CASES: tuple[Case, ...] = (
    ...
    Case(
        ...
        _TILE,
        64,
        "M=1 vecmat: the prior integration packed this to a single row "
        "and its operand load read past the end of the panel",
        seed=1,
    ),
    Case("narrow_n_1", _TILE, 1, 64, "N=1 matvec: the mirror case, where the column operand is one lane", seed=2),
    Case(
        "narrow_both_1",
        1,
        1,
        64,
        "both parallel extents 1: a rank-1 accumulate reduced to a dot product, where every length in play is 1",
        seed=3,
    ),
    # --- parallel extents around the tile -----------------------------------------------------
    *(
        Case(f"m_{m}", m, _TILE, 64, "M swept below, at and just over a plausible tile edge, and odd", seed=10 + m)
        for m in (2, 3, 7, 15, 16, 17, 31, 32)
    ),
    *(
        Case(f"n_{n}", _TILE, n, 64, "N swept below, at and just over a plausible tile edge, and odd", seed=40 + n)
        for n in (2, 3, 7, 15, 16, 17, 31, 32)
    ),
    # --- reduction extent ---------------------------------------------------------------------
    *(
        Case(
            ...
            "tile/2",
            k,
            "K tiny / odd / exact / one over / long: the reduction tail is harmless only if it is actually handled",
            seed=80 + k,
        )
    ...
    Case(
        ...
        "tile/4",
        24,
        "M != N, so an operand swap in the accumulate is "
        "NOT shape-safe: this is the case that would have caught the reversed operand order",
        seed=200,
    ),
    Case("asymmetric_n_gt_m", "tile/4", _TILE, 24, "the other asymmetry, so a swap fails in both directions", seed=201),
    # --- epilogue -----------------------------------------------------------------------------
    Case(
        "bias",
        "tile/2",
        "tile/2",
        32,
        "bias initialised into the accumulator by broadcast rather "
        "than added afterwards, which is the hardware's own init path",
        bias=True,
        seed=300,
    ),
    Case(
        ...
        "tile/2",
        32,
        "bias broadcast with a single output row: the init writes all rows whether or not they are read",
        bias=True,
        seed=301,
    ),
    ...
    Case(
        ...
        "tile/2",
        255,
        "full-range int8 operands over a long reduction: the largest "
    ...
    Case(
        ...
        "tile/2",
        32,
        "the epilogue on a square tile: the base case for scale/round/clamp/narrow being bit-exact against numpy",
        requant=True,
        seed=600,
    ),
    Case(
        ...
        "tile/2",
        32,
        "epilogue with a single output row -- the readout writes "
        "one row and the narrowing must follow it, not the whole tile",
        requant=True,
        seed=601,
    ),
    Case(
        ...
        1,
        32,
        "epilogue on a single output column: a per-column "
    ...
    Case(
        ...
        33,
        24,
        "epilogue across a short row tile AND a short column tile, so the "
        "narrowing is exercised where the tiling loop's bounds are ragged",
        requant=True,
        seed=603,
    ),
    Case(
        ...
        "tile/2",
        255,
        "full-range operands over a long reduction, so "
        "the accumulator is large and the clamp actually fires -- the cases above mostly do not reach "
        "it, and a clamp that never runs is a clamp that is not tested",
        requant=True,
        seed=604,
        amplitude=127,
    ),
    Case(
        ...
        "tile/2",
        32,
        "bias broadcast into the accumulator AND the "
    ...
    # for spectformer int8 (llvmlower/int8_contractions on the prepared module), i.e. the contractions that
    ...
    Case(
        ...
        1024,
        32,
        "spectformer's FFN up-projection shape (12 of them in the "
        "model, the joint largest contributor): M=196 is not a multiple of any tile edge, so every "
        "tile column ends in a short row tile",
        seed=500,
    ),
    Case(
        "workload_ffn_down",
        ...
        256,
        32,
        "the FFN down-projection (12 of them): the same short M "
        "with a narrower N, so the tail lands at a different place in the tiling loop",
        seed=501,
    ),
    Case(
        "workload_attn_proj",
        ...
        768,
        32,
        "the fused QKV projection (8 of them): N=768 is a whole "
        "number of tiles at every edge, isolating the M tail from the N tail",
        seed=502,
    ),
    Case(
        "workload_im2col",
        ...
        196,
        32,
        "the patch-embedding contraction: the ONLY routed shape whose "
        "N is not a multiple of the operand alignment, so it is the case that would catch a "
        "right-operand row stride the datapath cannot fetch",
        seed=503,
    ),
    Case(
        "workload_classifier",
        ...
        1000,
        32,
        "the classifier head: M=1 at a large N, i.e. a vecmat over "
        "many tile columns. The M=1 probes above use a single tile; this one runs the tiling loop with "
        "one live row throughout, which is where a row-index bug in the readout would show",
        seed=504,
    ),
    ...
    Case(
        "workload_ffn_up_k256",
        196,
        1024,
        256,
        "the FFN up-projection at the model's real reduction "
    ...
    Case(
        "workload_ffn_down_k1024",
        196,
        256,
        1024,
        "the FFN down-projection at its real K -- the "
        "LONGEST reduction in the model. K=1024 is where an accumulator that is re-zeroed, or carried "
        "across a tile boundary it should not be, stops being maskable by a short reduction",
        seed=521,
    ),
    Case(
        "workload_attn_proj_k256",
        196,
        768,
        256,
        "the fused QKV projection at real depth: N=768 is a "
        "whole number of tiles at every edge, so a failure here is the reduction or the pack, never the "
        "N tail -- which is exactly what makes it diagnostic alongside the two above",
        seed=522,
    ),
    Case(
        "workload_im2col_k768",
        256,
        196,
        768,
        "patch embedding at real depth. Its N=196 is still not a "
        "multiple of the operand alignment, so this is the one case that exercises an unaligned "
        "right-operand row stride AND a long reduction at the same time",
        seed=523,
    ),
    Case(
        "workload_classifier_k256",
        1,
        1000,
        256,
        "the classifier head at real depth: M=1 with a "
        ...
        "row), so it is here for correctness, not throughput",
        seed=524,
    ),
    ...
    Case(
        ...
        9216,
        2304,
        "the widest decode projection: 144 column blocks at edge 64 "
        "under the same unfilled row panel, so a fault that needs many tile-column iterations to "
        "show up has room to",
        seed=531,
    ),
    Case(
        "decode_ffn_down_k9216",
        8,
        2304,
        9216,
        "the DEEPEST reduction any model here lowers, at a "
        "partial row panel. K=9216 is also exactly the pad-tail scratch bound, so this is the case "
        "that sits on that boundary rather than comfortably inside it",
        seed=532,
    ),
    Case(
        "decode_ffn_down_bias",
        8,
        2304,
        9216,
        "the same shape with the bias broadcast init, which "
        "writes every tile row whether or not M fills it -- the init and the readout disagree about "
        "how many rows are live, and only a partial row panel can expose that",
        bias=True,
        seed=533,
    ),
    Case(
        "decode_ffn_down_requant",
        8,
        2304,
        9216,
        "the same shape with the full epilogue, which reads back only the rows M covers",
        bias=True,
        requant=True,
        seed=534,
    ),
...
def select(
    tile: int, *, path: "str | Path | None" = None, tiles_mn: bool = True, max_out_elems: int | None = None
    ...
    **The deferral criterion is a property of the kernel, so it is a parameter.** It used to be hardcoded
    ...
    for case in load_corpus(path):
        ...
        if not tiles_mn and (m > int(tile) or n > int(tile)):
            skipped.append((got, f"m={m} n={n} exceeds the {tile}x{tile} tile; needs a kernel that tiles M and N"))
        elif max_out_elems is not None and m * n > int(max_out_elems):
            skipped.append(
                (
                    got,
                    f"output is {m}x{n} = {m * n} int32 elements, over the image's "
            ...
            skipped.append(
                (
                    got,
                    f"k={k} is at or above the accumulator-overflow bound "
...
def write_corpus(path: "str | Path | None" = None) -> Path:
    ...
    payload: dict[str, Any] = {
        "note": (
            "Frozen acceptance corpus for the outer-product delta. Cases are removed only with a "
