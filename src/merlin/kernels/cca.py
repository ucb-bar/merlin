...
def lift_schedule(kernel, *, machine=None, op: str, source: str = "schedule", backend: str) -> CCA:
    """OUR side of an expressiveness measurement: a CCA lifted from a :mod:`merlin.sched` schedule.

    Every other lifter reads a BUILT artifact -- assembly, an object file, a graph record. That left
    the schedule itself, which is the thing the scheduling-IR coverage register makes its claims
    about, as the one description with no CCA of its own, so "does our schedule agree with the corpus
    on this axis" could not be asked at all.

    What it reads is the PAYLOAD, never statement order: dispatch counts come from
    :func:`~merlin.sched.ir.kernel.instances` (the dynamic instances the loop nest denotes, not the
    source sites), asynchrony from ``Call.produces``/``Call.awaits``, placement from ``Call.stages``
    and configuration from ``Call.sets``. Nothing here knows what a target's instructions are called:
    an instruction is a synchronisation instruction exactly when the MACHINE says so, and a machine is
    a parameter.

    THREE-STATE, like the machine model it reads. A field the schedule does not determine is ``None``
    and an agreement report charges it to the DENOMINATOR as undecidable -- never zero, and never a
    plausible default. ``communication.fences`` is 0 only on a machine that DECLARES synchronisation
    instructions and a schedule that issues none; on a machine that declares none it is ``None``,
    because "this schedule has no fences" and "nobody wrote down what a fence is here" are different
    statements and only the first is a measurement.
    """
    from merlin.sched.ir.kernel import instances

    seq = [c for c, _ in instances(kernel)]
    n = len(seq)

    # A command that only ESTABLISHES state: it sets configuration keys and stages nothing. Read off
    # the same fields `hoist_config` acts on, so the two cannot disagree about what a config is.
    n_config = sum(1 for c in seq if c.sets and not c.stages)

    # --- asynchrony: a token produced here and awaited later, with work in between ---------------
    produced_at: dict[str, int] = {}
    gaps: list[int] = []
    for i, c in enumerate(seq):
        if c.produces is not None:
            produced_at[c.produces] = i
        for tok in c.awaits:
            if tok in produced_at:
                gaps.append(i - produced_at[tok] - 1)
    any_token = any(c.produces is not None for c in seq)
    overlap = (max(gaps) > 0) if gaps else (False if any_token else None)
    issue_to_wait = max(gaps) if gaps else None

    # --- placement: banks are counted only where the schedule NAMED one --------------------------
    banked = {(s.memory, s.bank) for c in seq for s in c.stages if s.bank is not None}
    written = {(s.memory, s.bank) for c in seq for s in c.stages if s.bank is not None and s.writes}
    banks_used = len(banked) or None
    double_buffered = len(written) if len(written) > 1 else None

    # --- what the MACHINE declares, never what an instruction is spelled -------------------------
    sync_instrs = {s.instr for s in (getattr(machine, "syncs", ()) or ())}
    fences = sum(1 for c in seq if c.instr in sync_instrs) if sync_instrs else None

    return CCA(
        op=op,
        backend=[backend],
        compute=ComputeFacet(op=op),
        memory=MemoryFacet(banks_used=banks_used),
        dispatch=DispatchFacet(
            n_dispatches=n or None,
            config_fraction=(n_config / n) if n else None,
            double_buffered_banks=double_buffered,
            dma_overlap=overlap,
            dma_issue_to_wait=issue_to_wait,
        ),
        communication=CommunicationFacet(fences=fences) if sync_instrs else None,
        provenance={"level": "schedule", "source": source, "confidence": "high"},
    )
