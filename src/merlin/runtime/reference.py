...
def _attention_epilogue(t: Tensor, attrs: dict[str, Any], default_shift: int, op: str) -> Tensor:
    ...
    if unknown:
        raise ValueError(f"{op} does not implement attribute(s) {unknown}; it implements {sorted(_ATTENTION_ATTRS)}")
...
def _conv2d(
    ...
    if ifm.shape[3] != ci:
        raise ValueError(
            f"CONV2D channel mismatch: {ifm_name}{ifm.shape} has C={ifm.shape[3]} but kernel declares ci={ci}"
        )
    cols = conv_im2col(ifm, kh=kh, kw=kw, ci=ci, stride=stride, padding=padding, dilation=dilation, layout=layout)
    ...
    for stage in attrs.get("epilogue", []):
        if stage in BIAS_STAGES:
            # Per output channel, on the accumulator, before any requantization: the same stage a
            # contraction's commit applies, because a convolution's readout is the same readout.
            if bias is None:
                raise ValueError(f"CONV2D {dst!r}: a bias stage is declared and no bias tensor was resolved")
            result = result.add_bias(bias)
        elif stage == "acc_scale":
            result = result.requant_acc_scale(declared_acc_scale(attrs, op=f"CONV2D {dst!r}"))
        elif stage == "requant":
...
def reference_outputs(cb: dict[str, Any], inputs: dict[str, Any] | None = None) -> dict[str, list]:
    ...
    for cmd in cb.get("commands", []):
        ...
        if op == "RES_PACK":
            ...
            if "scale" in ops:
                resident_dequant[ops["dst"]] = (ops["src"], ops["scale"], int(attrs.get("dequant_axis", 1)))
        elif op == "EVICT":
            ...
            # Two tensors already in (possibly different) quantized domains, each carried into the
            # output's domain by its own multiplier, summed, and rounded ONCE. The definition is
            # `Tensor.residual_add_scaled`; how far a target may lie from it is the capsule's
            # declared `bound_lsb`, which the grade applies and this engine does not.
            lhs_scale, rhs_scale, _bound = residual_add_params(attrs, op=f"RESIDUAL_ADD {ops.get('dst')!r}")
            t = env[ops["lhs"]].residual_add_scaled(env[ops["rhs"]], lhs_scale, rhs_scale)
            ...
            if not isinstance(dst_spec, dict):
                raise ValueError(f"BATCHED_MATMUL {dst!r}: destination must name a declared tensor")
            dst_shape = dst_spec.get("shape")
            geometry = batched_matmul_geometry(at.shape, wt.shape, dst_shape, op=f"BATCHED_MATMUL {dst!r}")
            flat: list = []
            for index in range(geometry.batch_count):
                a0 = index * geometry.lhs_slice_elements
                w0 = index * geometry.rhs_slice_elements
                lhs = Tensor((geometry.m, geometry.k), at.data[a0 : a0 + geometry.lhs_slice_elements], at.dtype)
                rhs = Tensor((geometry.k, geometry.n), wt.data[w0 : w0 + geometry.rhs_slice_elements], wt.dtype)
