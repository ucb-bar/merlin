...
_CONV2D_ATTRS = frozenset(
    {
        ...
        "pool_padding",
        "pool_pad_value",
        # The per-output-channel bias a convolution's readout adds, named the way
        # a contraction's COMMIT names it and resolved by the same function.
        "bias",
    }
)
...
def _validate_attention_attrs(attrs: dict[str, Any], op: str) -> None:
    ...
    if unknown:
        raise SimulationError(
            f"{op} does not implement attribute(s) {unknown}; it implements {sorted(_ATTENTION_ATTRS)}"
        )
...
def _conv_geom(attrs: dict[str, Any], key: str, arity: int, default: list[int]) -> tuple[int, ...]:
    ...
    if not isinstance(v, list) or len(v) != arity:
        raise SimulationError(f"CONV2D {key} must be a list of {arity} int(s) (got {v!r})")
...
def simulate(cb: dict[str, Any], inputs: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for cmd in cb.get("commands", []):
        ...
        if op == "RES_PACK":
            ...
            trace.append({"name": "resident_hit" if rhs in resident else "resident_miss", "object": rhs, "count": 1})
            ...
            # Same definition as the reference engine and the capsule golden, through the same
            # `Tensor.residual_add_scaled`, so the three cannot disagree about where the rounding is.
            lhs, rhs, dst = ops["lhs"], ops["rhs"], ops["dst"]
            try:
                lhs_scale, rhs_scale, _bound = residual_add_params(attrs, op=f"RESIDUAL_ADD {dst!r}")
                t = env[lhs].residual_add_scaled(env[rhs], lhs_scale, rhs_scale)
            except ValueError as e:
                raise SimulationError(str(e)) from e
            t = _narrow_int_readout(t, str(attrs.get("output_dtype", "i32")), "RESIDUAL_ADD")
            if "relu" in (attrs.get("epilogue") or ()):
                t = t.relu()
            env[dst] = t
            outputs[dst] = t.to_list()
            metrics.bytes_read += env[lhs].nbytes + env[rhs].nbytes
            metrics.bytes_written += t.nbytes
            metrics.bytes_moved += env[lhs].nbytes + env[rhs].nbytes + t.nbytes
            metrics.cycles += len(t.data)
            trace.append({"name": "residual_add", "object": dst, "count": len(t.data)})

        elif op == "BATCHED_MATMUL":
            a, w, dst = env[ops["a"]], env[ops["w"]], ops["dst"]
            dst_spec = cb.get("tensors", {}).get(dst)
            if not isinstance(dst_spec, dict):
                raise SimulationError(f"BATCHED_MATMUL {dst!r}: destination must name a declared tensor")
            dst_shape = dst_spec.get("shape")
            try:
                geometry = batched_matmul_geometry(a.shape, w.shape, dst_shape, op=f"BATCHED_MATMUL {dst!r}")
            except ValueError as e:
                raise SimulationError(str(e)) from e
            out: list = []
            for index in range(geometry.batch_count):
                ...
                asl = Tensor((geometry.m, geometry.k), a.data[a0 : a0 + geometry.lhs_slice_elements], a.dtype)
                wsl = Tensor((geometry.k, geometry.n), w.data[w0 : w0 + geometry.rhs_slice_elements], w.dtype)
            ...
            metrics.cycles += geometry.batch_count * base_matmul_cycles(geometry.m, geometry.k, geometry.n)
            trace.append({"name": "batched_matmul", "object": dst, "count": geometry.batch_count})
            ...
            if op == "RMSNORM":
                ...
                if len(g) != cols:
                    raise SimulationError(f"RMSNORM gamma has {len(g)} element(s), expected {cols}")
                ...
                for row in src_rows:
                    out_rows.append([0.5 * v * (1.0 + math.tanh(k * (v + 0.044715 * v * v * v))) for v in row])
            elif op == "SOFTCAP":
                ...
                if cols % 2:
                    raise SimulationError(f"ROPE width {cols} is odd; the half-split convention needs an even width")
                half = cols // 2
                ...
                for p, row in enumerate(src_rows):
                    ...
                    out_rows.append(
                        [
                            row[c] * cos[c % half] + (-row[c + half] if c < half else row[c - half]) * sin[c % half]
                            for c in range(cols)
                        ]
                    )
            ...
            if d != d2:
                raise SimulationError(f"ATTENTION_QK head-dim mismatch: {q}{qt.shape} vs {k}{kt.shape}")
            ...
            if s != s2:
                raise SimulationError(f"ATTENTION_PV key-count mismatch: {p}{pt.shape} vs {v}{vt.shape}")
            t = pt.matmul(vt)
            for stage in attrs.get("epilogue", []):
                if stage == "acc_scale":
                    t = t.requant_acc_scale(float(attrs.get("acc_scale", 1.0)))
            ...
            if not isinstance(kernel, list) or len(kernel) != 4:
                raise SimulationError(f"CONV2D needs kernel = [kh, kw, ci, co]; got {kernel!r}")
            ...
            if len(ifm.shape) != 4:
                raise SimulationError(f"CONV2D activation {ifm_n}{ifm.shape} is not rank-4 NHWC")
            if ifm.shape[3] != ci:
                raise SimulationError(
                    f"CONV2D channel mismatch: {ifm_n}{ifm.shape} has C={ifm.shape[3]} but kernel declares ci={ci}"
                )
            cols = conv_im2col(
                ifm, kh=kh, kw=kw, ci=ci, stride=stride, padding=padding, dilation=dilation, layout=layout
            )
            ...
            for stage in attrs.get("epilogue", []):
                if stage in BIAS_STAGES:
                    try:
                        t = t.add_bias(env[bias_tensor_name(ops, attrs, op=f"CONV2D {dst!r}")])
                    except ValueError as e:
                        raise SimulationError(str(e)) from e
                elif stage == "acc_scale":
                    try:
                        t = t.requant_acc_scale(declared_acc_scale(attrs, op=f"CONV2D {dst!r}"))
                    except ValueError as e:
                        raise SimulationError(str(e)) from e
                elif stage == "requant":
                    t = t.requant(int(attrs.get("requant_shift", default_shift)))
                    ...
                    Ho, Wo = conv_out_dims(ifm.shape[1], ifm.shape[2], kh, kw, stride, padding, dilation)
                    try:
                    ...
                    raise SimulationError(f"CONV2D does not implement epilogue stage '{stage}'")
