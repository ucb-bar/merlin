...
def _run(cmd: list, **kw) -> subprocess.CompletedProcess:
    ...
    return _proc.run_checked(cmd, error=ZephyrModelError, timeout=timeout, timeout_hint=" (pathological compile)", **kw)
...
def _prepare_model_mlir(
    ...
    *,
    ...
    if _qargs:
        print(
            f"[qinner] lifted {len(_qargs)} quant-inner tensor(s) to @forward arguments "
    ...
    if int8_compute:
        ...
        if respect_captured_quantization_scope:
            ...
            print(
                "[quant_scope] respected captured quantization evidence: "
        ...
        if prequant_gather:
            print(
                f"[quant] quantize_before_gather: "
                f"rewrote={_pg.get('prequant_gather_rewrites', 0)} "
                f"erased_f32_chain_ops={_pg.get('prequant_gather_erased_ops', 0)} "
                + " ".join(
                    f"{k}={v}"
                    for k, v in sorted(_pg.items())
                    if k.startswith(("prequant_gather_mode_", "prequant_gather_refused_"))
                )
            )
    ...
    if hoist_weight_invariant_quantize:
        ...
        if bundle_dir is None:
            raise ValueError(
                "hoist_weight_invariant_quantize needs bundle_dir (the weight bytes "
                "are recomputed from the bundle's safetensors)"
            )
        _bd = Path(bundle_dir)
        _man = _json.loads((_bd / "weights.safetensors.manifest.json").read_text())
        _hargs, _hvals, _nch = _qh.apply(module, _qh.activation_arg_indices(_man), _qh.weight_loader(_bd, _man))
        _qh.write_plan(work, _hargs)
        _qh.write_values(work, _hvals)
        print(
            f"[quant_hoist] hoist_weight_invariant_quantize: chains={_nch} "
    ...
    if fuse_quant_round:
        ...
        print("[quant_round] fuse_quantize_round_convert: " + " ".join(f"{k}={v}" for k, v in sorted(_rrep.items())))
    ...
    if vectorize_amax_reduction:
        ...
        _vrep: dict = {}
        _reduce_vec.apply(module, lanes=vec_lanes, min_rank=_VEC_RANK_MIN_RANK, max_rank=vec_max_rank, report_out=_vrep)
        print("[reduce_vec] vectorize_amax_reduction: " + " ".join(f"{k}={v}" for k, v in sorted(_vrep.items())))
    ...
    if tag_vec_ranks or _os.environ.get("MERLIN_VEC_RANK"):
        ...
        for op in module.walk():
            ...
            if body is not None and any(inner.name in ("tensor.extract", "memref.load") for inner in body.ops):
                skip_gather += 1
                continue
            ...
            n_tag += 1
        print(
            f"[vec_rank] tagged {n_tag} all-parallel generics for bounded vectorize "
...
class MatrixRouting:
    ...
    ``support_target`` selects executable OOT support on ``MERLIN_TARGET_PATH``;
    it is not inferred from ``unit`` or ``config``. The provider declares
    ``plugin.matrix_lowering`` with geometry, selector, rewrite_prepared_file,
    load_signatures and build_object callables. Rewrite reports preserve count,
    signatures, skipped and sub_tile(); builds expose object_path, tile_edge,
    scalar_tile, scratch_bytes, parallel_tiles and to_dict().
    """
    ...
    def __post_init__(self) -> None:
        for name, value in self.identity().items():
            if not isinstance(value, str) or not value.strip():
                raise ValueError(f"matrix routing requires an explicit nonempty {name}")

    def provider(self):
        """Resolve executable support explicitly, never from the unit's name."""
        from ...targetgen.plugins import PluginError, load_declared

        provider = load_declared(self.support_target, "matrix_lowering")
        required = ("geometry", "selector", "rewrite_prepared_file", "load_signatures", "build_object")
        missing = [name for name in required if not callable(getattr(provider, name, None))]
        if missing:
            raise PluginError(f"{self.support_target!r}: matrix_lowering lacks callable {missing}")
        return provider

    def identity(self) -> dict[str, str]:
        return {"support_target": self.support_target, "unit": self.unit, "config": self.config}
...
MATRIX_ROUTING_RECORD = "matrix_routing.json"


def load_matrix_signatures(work: Path, matrix: MatrixRouting | None) -> dict[str, tuple[int, ...]]:
    """Bind the provider's signatures to the route selected during preparation.

    This is a local build handoff, not a certificate or a replacement for frozen
    source verification. Legacy sidecars require fresh preparation before reuse.
    """
    record_path = work / MATRIX_ROUTING_RECORD
    if matrix is None:
        if record_path.exists():
            raise ValueError("prepared matrix routing requires the original explicit matrix= selection")
        return {}
    signatures = matrix.provider().load_signatures(work)
    if not record_path.exists():
        if signatures:
            raise ValueError("matrix signatures have no routing record; prepare the model again")
        return {}
    record = json.loads(record_path.read_text(encoding="utf-8"))
    expected = {
        "schema": "merlin.matrix-routing.v1",
        **matrix.identity(),
        "signatures": {name: list(extents) for name, extents in signatures.items()},
    }
    if record != expected:
        raise ValueError("matrix routing record differs from the selected provider, configuration or signatures")
    return signatures
...
def _write_parallel_arms(work: Path, harts: int, table: dict, par_table: dict) -> None:
    ...
    (Path(work) / PARALLEL_ARMS_FILE).write_text(
        json.dumps(
            {
                "harts": int(harts),
...
def prepare_for_lowering(
    ...
    *,
    ...
    from ...llvmlower.impr_features import NAMED_INT8_CONTRACTION_NAME, QUANTIZE_BEFORE_GATHER_NAME
    ...
    prepared = _prepare_model_mlir(
        ...
        hoist_weight_invariant_quantize=(_HOIST_WEIGHT_INVARIANT_QUANTIZE in _closed),
    ...
    def _report_inapplicable_levers() -> None:
        try:
            ...
            for _name, _why in _inapplicable(features, _applicability_source).items():
                print(
                    f"[lever] INAPPLICABLE {_name}: needs {list(_why['needs'])}, module has "
    ...
    if outline_int8:
        ...
        if not routed.count:
            raise ValueError("outline_int8 requested, but no legal rank-2 i8xi8->i32 contraction was routed")
        print(
            f"[outlined-int8] routed {routed.count} rank-2 contraction(s) across "
    ...
    if OPU_MATMUL_NAME in features:
        if matrix is None:
            ...
            raise ValueError(
                ...
                "unit and configuration to route to are unknown; pass "
                "MatrixRouting(support_target=..., unit=..., config=...)"
            )
        routed = matrix.provider().rewrite_prepared_file(
            prepared, work, select=matrix.selector(), tile_edge=matrix.tile_edge()
        )
        (work / MATRIX_ROUTING_RECORD).write_text(
            json.dumps(
                {
                    "schema": "merlin.matrix-routing.v1",
                    **matrix.identity(),
                    "signatures": {name: list(extents) for name, extents in routed.signatures.items()},
                },
                indent=2,
            ),
            encoding="utf-8",
        ...
        print(
            f"[matrix] routed {routed.count} contraction(s) to {matrix.unit} "
        ...
        if sub:
            ...
            print(
                f"[matrix] WARNING: {len(sub)} of {routed.count} routed contraction(s) do NOT fill "
    ...
    if LMUL_GROUP_SENTINEL in features:
        ...
        try:
            features = features | {lmul_group_feature(group_lmul_for_shapes(_cshapes0(prepared), vlen=vlen))}
        except LmulDerivationError as exc:
            print(f"[zephyr_model] register-group width not derivable ({exc}); leaving the compiler's default in place")
    ...
    from ...llvmlower.impr_features import (
        ...
        ensure_perop_block,
        parse_perop_mr_sentinel,
        parse_perop_nr_sentinel,
    )
    ...
    if mr_fill_vlen and _mr_named:
        print(
            f"[zephyr_model] {PEROP_MR_FILL_NAME} ignored: {PEROP_MR_SENTINEL_PREFIX}"
    ...
    if nr_fill_vlen and _nr_named:
        print(
            f"[zephyr_model] {PEROP_NR_FILL_NAME} ignored: {PEROP_NR_SENTINEL_PREFIX}"
    ...
    if _mr_named or _nr_named:
        features = {
            f for f in features if parse_perop_mr_sentinel(f) is None and parse_perop_nr_sentinel(f) is None
        } | {PEROP_BLOCK_NAME}
    if PEROP_BLOCK_NAME in features:
        ...
        _blk = dict(mr_cap=mr_cap, nr_cap=nr_cap, vlen=nr_fill_vlen, mr_vlen=mr_fill_vlen)
        ...
        if _iv.FEATURE in features:
            ...
            print(
                f"[im2col_identity_view] viewed={_views.viewed} "
        ...
        if _ip.FEATURE in features:
            prepared, _pack = _ip.rewrite_prepared_file(
                prepared, _pb.block_table(_cshapes(prepared), **_blk), work, parallel_panels=int(harts) > 1
            )
            packed_entries = {k: (_ip.FEATURE, mr, nr) for k, mr, nr in _pack.entries}
            print(
                f"[im2col_pack] packed={_pack.packed} "
        ...
        if _wpan.FEATURE in features:
            prepared, _wpk = _wpan.rewrite_prepared_file(
                prepared,
                _pb.block_table(_cshapes(prepared), **_blk),
                work,
                bundle=Path(mlir_path).resolve().parent,
                parallel_panels=int(harts) > 1,
            )
            print(
                f"[weight_panel] packed={_wpk.packed} dead_ops_erased={_wpk.dead_ops_erased} "
        ...
        table.update(
            _pb.conv_block_table(
                prepared, features, mr_cap=mr_cap, nr_cap=nr_cap, vlen=nr_fill_vlen, mr_vlen=mr_fill_vlen
            )
        )
        ...
        if table:
            ...
            _par_shapes = [
                s
                for s in _cshapes(prepared)
                if _pb.shape_key(s.op, tuple(int(d) for d in s.parallel), tuple(int(d) for d in (s.reduction or ())))
            ...
            prepared = _pb.tag_prepared_mlir(
                prepared,
                table,
                work=work,
                par_table=par_table,
...
class _DebugHarness:
    ...
    def __init__(self, decls="", boot="", post_init="", pre_run="", post_run="", post_dump="", stacks=""):
...
def _main_c(
    ...
    halt = "sys_reboot(SYS_REBOOT_COLD)" if console == "htif" else "for (;;) k_cpu_idle()"
    omp_include = '#include "libomp_zephyr.h"\n' if n_harts > 1 else ""
    omp_init = (
        f'  int nth = merlin_omp_init({nth});\n  printk("METRIC omp_threads %d\\n", nth);\n' if n_harts > 1 else ""
    )
...
def _prj_conf(cpus: int, backend: str, brd=None, console_facts=None, debug: bool = False) -> str:
    ...
    tick_conf = (
        ""
        if getattr(brd, "tick_hz", None) is None
        else f"""
...
def _cmakelists(
    ...
    ext = (
        f'zephyr_linker_sources(SECTIONS "{weights_section_ld}")\n'
        "zephyr_link_libraries(-Wl,-u,_binary_weights_bin_start)\n"
        if weights_section_ld is not None
        else ""
    )
    ...
    omp_debug = (
        "target_compile_definitions(app PRIVATE MERLIN_OMP_DEBUG_SPLIT=1)\n"
        if omp and os.environ.get("MERLIN_OMP_DEBUG_SPLIT") == "1"
        else ""
    )
    ...
    if omp:
        omp_debug += f"target_compile_definitions(app PRIVATE MERLIN_OMP_MAX_THREADS={max(int(n_harts), 1)})\n"
        ...
        omp_debug += f"target_compile_definitions(app PRIVATE MERLIN_OMP_VECTOR_POOL={1 if backend == 'rvv' else 0})\n"
        ...
        if hart_ids is not None:
            ...
            omp_debug += f'target_compile_definitions(app PRIVATE "MERLIN_OMP_HART_IDS={{{ids}}}")\n'
    ...
    if debug and op_profile:
        ...
        omp_debug += (
            "target_compile_definitions(app PRIVATE MERLIN_PROF_ZEPHYR=1)\n"
    ...
    omp_debug += f'target_compile_definitions(app PRIVATE "MERLIN_BUILD_HASH=\\"{build_hash}\\"")\n'
    ...
    if chip_freq_hz and console_facts is not None:
        ...
        for flag in console_facts.macros(chip_freq_hz=chip_freq_hz):
            omp_debug += f"target_compile_definitions(app PRIVATE {flag[2:] if flag.startswith('-D') else flag})\n"
...
def build_app(
    ...
    *,
    ...
    with IR_LOCK:
        ...
        if debug:
            ...
            try:
                ...
                print(
                    f"[zephyr_model] op profiling unavailable ({exc}); "
        ...
        res = lower_model_file(
            ...
            parallel_harts=(n_harts if n_harts > 1 and backend == "rvv" else None),
    ...
    _run([clang, "--target=riscv64-unknown-elf", *model_cflags, "-c", res.ll_path, "-o", work / "model.o"])
    ...
    if matrix_sigs:
        ...
        matrix_build = matrix.provider().build_object(
            ...
            cc=clang,
            cflags=["--target=riscv64-unknown-elf", *cflags],
            scalar_tile=matrix_scalar_tile,
            parallel_tiles=n_harts > 1,
        )
        ...
        print(
            ...
            f"tile loop {
                'parallel over ' + str(n_harts) + ' hart(s)'
                if matrix_build.parallel_tiles
    ...
    def _cmake(bh: str) -> str:
        return _cmakelists(
            ...
            op_profile=prof_table is not None,
        )
    ...
    def _emit_and_build() -> str:
        ...
        _run(
            [
                build_tool("cmake") or "cmake",
                "-B",
                build_dir,
                "-G",
                "Ninja",
                f"-DBOARD={brd.build_board}",
                *extra,
                "-S",
                app,
            ],
    ...
    if not external and alloc_total:
        ...
        if ram_bytes - image < demand:
            ...
            print(
                f"[zephyr_model] region {ram_bytes / 2**20:.0f} MB leaves "
                ...
                f"Relinking for {grown / 2**20:.0f} MB.",
                flush=True,
            )
            ram_bytes = grown
            overlay = _overlay_for(ram_bytes)
    ...
    if matrix_build is not None:
        ...
        out["matrix_routing"] = matrix.identity()
...
def run_on_verilator(
    ...
    *,
    ...
    if sim is None:
        ...
        how.append(
            f"build it: make -C $MERLIN_CHIPYARD/sims/verilator CONFIG={cfg}"
            if cfg
            else f"declare `rtl_sim_config` for {board!r} in the board registry, or pass config="
        )
        raise ZephyrModelError(f"no Verilator sim for board {board!r} (config {cfg!r}); " + ", or ".join(how))
    proc = subprocess.run([str(sim), str(elf)], capture_output=True, text=True, timeout=timeout)
...
def _parse_console(console: str, rc: int) -> dict[str, Any]:
    ...
    if out_line is None or "DONE" not in console:
        raise ZephyrModelError(f"run did not produce OUT/DONE (rc={rc}):\n{console[-2000:]}")
    parts = out_line.split()
    n = int(parts[1])
    bits = [int(x) for x in parts[2 : 2 + n]]
    flat = np.array([struct.unpack("<f", struct.pack("<I", b & 0xFFFFFFFF))[0] for b in bits], dtype=np.float32)
    ...
    for l in console.splitlines():
        if l.startswith("METRIC "):
            ...
            if len(p_) == 4:
                ...
                values.append(value)
    res = {"outputs": flat, "prefix": flat, "argmax": argmax, "sum": sumval, "metrics": metrics, "console": console}
...
def _gate(prefix: np.ndarray, references, *, max_rel: float | None = None, min_coverage: float | None = None) -> dict:
    ...
    t1 = out.get("w8a8_cos", 0.0) > 0.999 and out.get("w8a8_rel", 1.0) < 1e-2 and _perel_ok("w8a8")
    t2 = out.get("fp32_cos", 0.0) > 0.99 and out.get("fp32_argmax", False) and _perel_ok("fp32")
    # legacy single-reference callers gate at the strict fp32 threshold (+ per-element ceiling)
    legacy = (
        "w8a8" not in out
        and out.get("fp32_cos", 0.0) > 0.9999
        and out.get("fp32_rel", 1.0) < 1e-3
        and _perel_ok("fp32")
    )
    ...
    out["tier_ok"] = "w8a8" if t1 else "fp32" if t2 else "fp32_legacy" if legacy else "fp32_cos_only" if t3 else None
    ...
    out["per_element_basis"] = (
        "relative" if (t1 or t2 or legacy) else "quantization_excess" if (t3 and _excess_applied) else None
    )
    return out
...
def build_and_run(
    ...
    *,
    ...
    b = build_app(
        ...
        cflags_override=cflags_override,
        features=features,
        vlen=vlen,
    )
    result: dict[str, Any] = {
        "elf": str(b["elf"]),
        "app_dir": str(b["app_dir"]),
