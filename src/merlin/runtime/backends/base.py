...
_REGISTRY: dict[str, BackendInfo] = {
    ...
    # Target-owned backends self-register from explicitly selected OOT support;
    # reference metadata does not authorize executable discovery.
...
class PluginOwnershipError(RuntimeError):
    """Selected support no longer owns a plugin already loaded in this process."""
...
_LOADED_PLUGIN_OWNERS: dict[str, _PluginOwner] = {}
_PLUGIN_KEYS = {
    "merlin._oot_backends": ("backend",),
    "merlin._oot_dialects": ("dialect",),
    "merlin._oot_sim_oracles": ("sim_oracle", "sim_oracle_metadata"),
}


def _plugin_source(path: Path) -> Path:
    return (path / "__init__.py" if path.is_dir() else path).resolve()


def _assert_oot_plugin_ownership() -> None:
    """Refuse changed selection without unloading or replacing live plugin objects.

    This is deliberately a global discovery refusal: callers must restore the
    original selection or use a fresh process. Check even on discovery-cache hits
    ...
    with _oot_lock:
        for owner in tuple(_LOADED_PLUGIN_OWNERS.values()):
            try:
                selected = resolve_support(owner.target)
                plugin = selected.plugin()
                root = provider_root(selected.base, plugin.get("path"))
                paths = set()
                ...
                if selected.base.resolve() == owner.provider_root and paths == {owner.module_path}:
                    continue
                reason = f"now selects {selected.base.resolve()} with module paths {sorted(map(str, paths))}"
            except Exception as exc:  # noqa: BLE001 — unresolved identity cannot license old code
                reason = f"selection cannot be verified: {type(exc).__name__}: {exc}"
            raise PluginOwnershipError(
                f"{owner.target}: loaded plugin {owner.namespace} belongs to {owner.provider_root} "
                f"at {owner.module_path}; {reason}. Restore the original selection or start a fresh process; "
                "loaded plugin objects are never replaced."
            )
...
def _oot_plugin_modules(key: str = "backend") -> list[tuple[str, Path]]:
    ...
    support packages explicitly reachable via ``MERLIN_TARGET_PATH``. Reference targets and
    the freshly-generated home never authorize implicit executable discovery. Empty
    ...
    try:
        ...
        return []
    try:
        names = list(target_registry.explicit_targets())
    ...
    for name in names:
        ...
        if path is not None and (path.is_file() or (path.is_dir() and (path / "__init__.py").is_file())):
            out.append((name, path))
        else:
            # DECLARED but pointing nowhere. This used to fall off the end of the loop, which made a
            # misspelled key indistinguishable from a package that never had one -- the exact failure
            # `targetgen.plugins` exists to stop, reproduced here because discovery does its own
            # resolution. Fail closed: no backend, and the reason is retrievable via load_failures().
            _LOAD_FAILURES[name] = (
                f"PluginError: plugin.{key}={rel!r} resolves to nothing under {root} (looked at {path}); "
                f"the package declares the seam but does not ship it"
            )
    return out
...
def _load_oot_backend(name: str, path: Path, *, ns: str = "merlin._oot_backends") -> None:
    """Serialize all backend/dialect/oracle namespace imports and preserve loaded identities."""
    with _oot_lock:
        _load_oot_backend_locked(name, path, ns=ns)


def _load_oot_backend_locked(name: str, path: Path, *, ns: str) -> None:
    ...
    root = provider_root(selected.base, selected.plugin().get("path"))
    try:
        relative = path.absolute().relative_to(root)
    except ValueError:
        # An equivalent selected-root alias is legitimate, an escaping target is not.
        try:
    ...
    if previous is not None and previous != owner:
        raise PluginOwnershipError(
            f"{name}: plugin {modname} is already loaded from {previous.provider_root} at "
            f"{previous.module_path}, not {owner.provider_root} at {owner.module_path}; start a fresh process"
        )
    if modname in sys.modules:
        origin = getattr(getattr(sys.modules[modname], "__spec__", None), "origin", None)
        if origin is None or Path(origin).resolve() != owner.module_path:
            raise PluginOwnershipError(
                f"{modname}: loaded module origin differs from {owner.module_path}; start a fresh process"
            )
        _LOADED_PLUGIN_OWNERS[modname] = owner
        return
    if path.is_dir():
    ...
    try:
        ...
        sys.modules.pop(modname, None)
        # A plugin can register objects before raising. Keep their owner even
        # though the failed module is removed, so changing providers cannot use
        # partial registrations to evade the immutable-ownership check.
...
def _ensure_oot_discovered() -> None:
    ...
    changes; loaded ownership is checked even on cache hits. Changed/removed ownership
    requires a fresh process, never an implicit hot reload of live objects."""
...
def get_backend(name: str):
    ...
    if why and name in _REGISTRY:
        ...
        try:
            declared_backend = selected.plugin().get("backend")
        except TargetContractMissing:
            # Generic built-ins need no target package. A selected external
            # provider disappearing after refusal still cannot license fallback.
            declared_backend = selected.external_root is not None
        if declared_backend:
            # A built-in registration is not a substitute for a selected OOT
            # backend that discovery refused or whose initialization failed.
            module = f"merlin._oot_backends.{name}"
            if module not in sys.modules or module not in _LOADED_PLUGIN_OWNERS:
                raise KeyError(f"{name}: backend module failed to load -- {why}")
