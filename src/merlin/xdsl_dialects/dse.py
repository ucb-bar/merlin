...
if HAS_XDSL:
    ...
    class CandidateOp(IRDLOperation):
        ...
        def verify_(self) -> None:
            ...
            if not self.interface_ops:
                raise VerifyException("dse.candidate must reference at least one interface op")
            for entry in self.interface_ops:
                ...
                if not ref or not ref.startswith("interface."):
                    raise VerifyException(
                        "dse.candidate interface_ops entries must be interface.* op names, got %r" % (ref,)
                    )
        ...
        def verify_(self) -> None:
            ...
            for key, val in self.metrics.data.items():
                if not isinstance(val, IntegerAttr):
                    raise VerifyException("dse.result metric %r must be an integer count/cycles" % key)
    ...
    _ATTR_CLASSES = [InterfaceCandidateType, VariantType, ExploitabilityType, VariantAttr, RegimeAttr]
    ...
    def build_example():
        ...
        base = ResultOp(
            ...
            properties={
                ...
                "metrics": DictionaryAttr({"cycles": IntegerAttr(11008, 64), "bytes_moved": IntegerAttr(73728, 64)}),
            },
        )
        sv = ResultOp(
            operands=[cand.candidate],
            properties={
                ...
                "workload": StringAttr("repeated_rhs_matmul"),
                "backend": StringAttr("simulator"),
                "metrics": DictionaryAttr({"cycles": IntegerAttr(8256, 64), "bytes_moved": IntegerAttr(49152, 64)}),
            },
        )
        tag = RegimeTagOp(
            operands=[cand.candidate],
