...
#: THE DATA-MOVEMENT CLASS, and why it is not covered by ``TILE_SCHEDULE``.
#:
#: The four classes above are about WHERE work goes (partition), how it is cut against the compute
#: edge (tiling), how it is spelled (lowering) and how it leaves the device (seam). None of them is
#: about how much data one transfer carries, and nothing in the vocabulary could demand it — so a
#: backend that tiles correctly, lowers correctly and crosses the seam correctly can still issue one
#: array tile per transfer forever and no capsule can say it should not.
#:
#: That gap was measured, not imagined: across 90 instruction traces of one live agent's compiler
#: (82,985 instructions), the memory-load classes were 19.2% of the stream and NOT ONE of them carried
#: more than a single array tile, while the same backend's own generated package derived a
#: multi-tile transfer payload from the target's declared DMA bound and its dialect already permitted
#: it. The emitter simply never issued a wide transfer, and no obligation could ask.
#:
#: It discharges ``target transformation`` alongside ``TILE_SCHEDULE`` because choosing a transfer
#: width IS a rewrite for the target's execution model in the target-independent plane — but it is a
#: DIFFERENT decision against a DIFFERENT edge: tiling is cut against the compute array, movement is
#: cut against the DMA payload bound, and the two edges are independent (the measured stream tiled
#: correctly at every step while moving one tile at a time).
BLOCK_MOVEMENT = "block-movement"  # target transformation
...
CLASS_OBLIGATION = {
    ...
    BLOCK_MOVEMENT: "target transformation",
...
def _grouping_honoured(recorded, dispatches) -> dict[str, Any]:
    """Did dispatch formation honour the ``schedule.group_dispatch`` decisions that were recorded?

    ``schedule.group_dispatch`` names contraction results that belong in ONE dispatch region, and
    this is the consumer that reads it back. The decision is honoured when every value it grouped is
    inside a dispatch that was FORMED as a region -- a single-op dispatch is the absence of the
    grouping, not a grouping of one.

    ``recorded: 0`` is reported on its own rather than folded into ``honoured``. A decision nothing
    records and a decision nothing contradicts produce the same boolean, and that ambiguity is why
    this op sat write-only: a lowering that stopped emitting it would have looked identical to one
    still emitting it.
    """
    grouped_values = sum(entry.n_values for entry in recorded)
    in_regions = sum(1 for d in dispatches if d.group is not None and d.root_op.startswith("linalg."))
    return {
        "recorded": len(recorded),
        "granularities": sorted({entry.granularity for entry in recorded}),
        "values_grouped": grouped_values,
        "contraction_dispatches_in_a_region": in_regions,
        # None, not True: nothing was decided, so nothing was honoured or contradicted.
        "honoured": None if not recorded else in_regions >= grouped_values,
    }


def run_dialect_plane(
    module,
    ...
    ``target`` makes a dispatch a whole COMPUTE GROUP rather than a single operation: a contraction
    with the stages its unit absorbed, or a fused host region. Without it this plane took the
    single-op baseline, which ``outline_dispatches`` has accepted a ``groups=`` argument to improve
    on all along -- ``compile_cli`` passes it and this path did not, so the two routes through the
    same function formed different dispatch regions for the same model with nothing recording that
    they differed. ``None`` keeps the baseline, byte for byte, for a caller that knows no target.
    """
    if not HAS_XDSL:
    ...
    try:
        ...
        from .pipeline import load_curated_contract
        from .schedule_decisions import lower_to_schedule, recorded_group_dispatch

        contract = analysis_contract or (load_curated_contract(target) if target else None)
        if contract is None:
            raise ValueError("no selected target or analysis_contract; abstraction analysis not run")
        cm = lower_to_schedule(lower_to_contract(module, contract))
        recorded = recorded_group_dispatch(cm)
        analysis = {
    ...
    if target is not None:
        from . import compute_groups as _compute_groups

        groups = _compute_groups.form_groups(module, target, function=forward)
    outlined = outline_dispatches(module, forward=forward, groups=groups)
    analysis["dispatch_grouping"] = _grouping_honoured(recorded, outlined.dispatches)
