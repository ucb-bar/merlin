...
if HAS_XDSL:
    ...
    @irdl_attr_definition
    class DispatchGranularityAttr(EnumAttribute[DispatchGranularity], SpacedOpaqueSyntaxAttribute):
    ...
    class KeepAccumulatorLiveOp(IRDLOperation):
        ...
        def verify_(self) -> None:
            if not self.until.data:
                raise VerifyException("schedule.keep_accumulator_live requires an `until` marker")
    ...
    class VectorStrategyOp(IRDLOperation):
        ...
        def verify_(self) -> None:
            if self.tail is not None and self.tail.data in (VectorStrategy.FIXED_WIDTH, VectorStrategy.SCALABLE_VL):
                raise VerifyException(
                    "schedule.vector_strategy tail must be a tail policy (predicated_tail/masked_tail/scalar_cleanup)"
                )
    ...
    def build_example():
        ...
        from xdsl.dialects.builtin import ArrayAttr, FunctionType, ModuleOp, TensorType, i8
        ...
        bind = BindOp(result_types=[HandleType()], properties={"target": StringAttr("@payload::@main::%matmul0")})
        apply_p = ApplyPolicyOp(operands=[[bind.handle]], properties={"policy": StringAttr("resident_packed_rhs")})
        pres = PreserveLayoutOp(
        ...
        place = PlaceOp(
            operands=[w, [cap.cap]],
            properties={"state": MemoryStateAttr(MemoryState.RESIDENT), "lifetime": StringAttr("region")},
        )
        keep = KeepAccumulatorLiveOp(operands=[w], properties={"until": StringAttr("epilogue_commit")})
        group = GroupDispatchOp(
            operands=[[w, w]], properties={"granularity": DispatchGranularityAttr(DispatchGranularity.COMMAND_BUFFER)}
        )
        vec = VectorStrategyOp(
            operands=[w],
            properties={
        ...
        ret = ReturnOp()
        blk.add_ops([cap, bind, apply_p, pres, hoist, place, keep, group, vec, sel, plan, ret])
