...
def storage_contract() -> dict:
    """Read shared output-layout metadata; absent metadata preserves basic IO.

    Both producers and storage inspection use this declaration. Historical folds
    do not rewrite evidence; product roots only select destinations for new work.
    """
    from .paths import data_path

    try:
        return load_yaml(data_path("contract", "storage.yaml")) or {}
    except (OSError, ValueError):
        return {}


def _product_relative_path(relative: str) -> Path:
    if not isinstance(relative, str) or not relative or "\\" in relative:
        raise ValueError(f"invalid product root: {relative!r}")
    path = Path(relative)
    if path.is_absolute() or any(part in {"", ".", ".."} for part in relative.split("/")):
        raise ValueError(f"product root must stay beneath artifacts: {relative!r}")
    return path


def declared_product_roots(declared: dict) -> dict[str, Path]:
    """Validated relative destinations shared by new writes and legacy migration."""
    roots = declared.get("product_roots", {})
    if not isinstance(roots, dict):
        raise ValueError("storage product_roots must be a mapping")
    for generator in roots:
        if len(_product_relative_path(generator).parts) != 1:
            raise ValueError(f"product name must be a single component: {generator!r}")
    return {name: _product_relative_path(destination) for name, destination in roots.items()}


def product_root(generator: str) -> Path:
    """Resolve a product's declared concern without changing its manifest identity."""
...
def new_product(
    ...
    *,
    ...
    ``product_roots`` in the storage contract may nest a topic under a shared
    concern; its manifest topic and leaf naming remain unchanged.
    """
    ts = utc_stamp()
    sha = git_sha7()
    base = product_root(generator)
    if target:
        base = base / target
