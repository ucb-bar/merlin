...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        ...
        help="retain named stage evidence: exact/both may be large; compact unsupported stages are hash-only",
    )
    p.add_argument(
        "--audit-sidecar",
        action="append",
        default=[],
        metavar="PATH",
        help="bind an existing weights/manifest file in --ir-audit; repeat for each dependency",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.audit_sidecar and not args.ir_audit:
        raise SystemExit("--audit-sidecar requires --ir-audit")
    ...
    result = compile_core.compile_core_mlir(
        ...
        workdir=out / "llvm" if route.kind == "llvm" else (out if args.ir_audit else None),
