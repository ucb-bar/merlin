...
def _m2m_dir() -> Path | None:
    """Use the same configured checkout as capture bundle discovery, including .env."""
    from merlin.integrations.model2mlir import root

    base = root()
    return base if (base / "m2m" / "ir" / "quant" / "dialect.py").is_file() else None


_IMPORT_LOCK = threading.RLock()


def _loaded_from(base: Path) -> bool:
    """Reject a foreign or unverifiable cached m2m owner; never replace its module graph."""
    package = (base / "m2m").resolve()
    for name, module in tuple(sys.modules.items()):
        if name != "m2m" and not name.startswith("m2m."):
            continue
...
def load_dialect() -> Any | None:
    ...
    Temporarily adds the selected checkout to sys.path. Imported classes keep their normal
    module identity; absence is not cached, and a different preloaded checkout is refused.
    This is an optional in-process import, not multi-checkout framework isolation.
    """
    with _IMPORT_LOCK:
        before = list(sys.path)
        try:
            base = _m2m_dir()
            if base is None or not _loaded_from(base):
                return None
            sys.path.insert(0, str(base))
            module = importlib.import_module("m2m.ir.quant")
            return module.Quant if _loaded_from(base) else None
