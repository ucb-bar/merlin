...
L = ["// generated — do not edit", "#pragma once", "#include <stdint.h>", f"#define N_PLACE {len(s['placements'])}"]
...
o = s["outputs"][0]
L += [
    f"#define OUT_ADDR {o['addr']}ULL",
    f"#define OUT_N {len(o['golden_flat'])}",
    f"#define OUT_ROWS {o.get('rows', 0)}",
    f"#define OUT_COLS {o.get('cols', 0)}",
    f"#define OUT_STRIDE {o.get('stride_bytes', 0)}",
    f"#define OUT_ELEM {o.get('elem_bytes', 4)}",
...
def enc(r):
    if not r or r.get("kind") == "unknown":
        return 0, 1  # unknown -> flag
    if r.get("kind") == "const":
        return (r.get("raw") or 0), 0
    if r.get("kind") == "argbase":
        return int(aa[str(r["arg_index"])]) + (r.get("offset") or 0), 0
    return 0, 1
...
print(
    f"{sys.argv[2]}: insns={len(s['insns'])} place={len(s['placements'])} "
