...
def generate(facts: dict, encoding: dict | None = None) -> str:
    ...
    lines.append(
        f"SCRATCHPAD_ROWS = {spad.get('depth')}   # banks={spad.get('banks')} elem_bits={spad.get('elem_bits')}"
    )
    ...
    if encoding:
        ...
        lines.append(
            "SEMANTIC_CLASS = {" + ", ".join(f"{int(k)}: {json.dumps(v)}" for k, v in sorted(sc.items())) + "}"
        )
        cs = encoding.get("config_subtype") or {}
        lines.append(
            "CONFIG_SUBTYPE = {" + ", ".join(f"{int(k)}: {json.dumps(v)}" for k, v in sorted(cs.items())) + "}"
        )
    lines.append('''
...
def generate_header(facts: dict, encoding: dict, target: str) -> str:
    ...
    ns = f"{target}_isa"
    out = [
        f"// GENERATED from RTL facts + the {target} capability manifest by "
...
def main(argv=None):
    ...
    try:
        ...
        pass
    if encoding:
        from ..rocc.decode import _semantics

        semantics = _semantics(a.target)
        complete = getattr(semantics, "encoding_fields", None)
