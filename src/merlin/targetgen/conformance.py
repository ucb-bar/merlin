...
def _application_axis(target: str, *, captures: dict | None = None, budget_s: float | None = None) -> dict:
    ...
    # PRICE THIS AXIS ON THE ENGINE THAT BINDS, the same way `_cert_affordability` does. `fit_for` with
    # no engine draws ONE line through every certification the target has, whatever answered it, and on
    # a target with two engines that line describes neither: measured here, the mixed fit is r2 0.149
    # over a 3..150528-element range and puts the ceiling at 19,245 written elements, while the same
    # history split by engine gives 6,500 on gsim and 923 on verilator. A spec carrying both numbers --
    # `cert_affordability.max_elements: 923` beside `application_shapes.budget_elements: 19245` -- is
    # not two opinions, it is one axis priced at no machine that exists.
    #
    # The binding engine is the SLOWEST the target has certified on, not the cheapest and not an
...
def _epilogue_axis(target: str) -> dict:
            ...
            rejected.append(
                {
                    ...
                    "why": (
                        ...
                        + ("manifest evidence" if by_manifest else "taxonomy evidence")
    ...
    return {
        ...
        "axis_basis": (
            ...
            "tests one stage and calls the fusion capability covered"
        ),
    }
...
def _binding_engine_fit(target: str):
    """The cost fit of the SLOWEST engine this target has certified on, or ``None``.

    One source for "which line prices a capsule here", so the application axis and the certification
    ceiling cannot disagree. ``None`` when no engine has a usable fit, which is a real answer: the
    caller then falls back to the unattributed whole-history fit rather than refusing to size anything.
    """
    from merlin.targetgen import cert_affordability as CA

    try:
        priced = CA.affordability(target, budget_s=_DEFAULT_CERT_BUDGET_S)
    except Exception:  # noqa: BLE001 -- unreadable run history is "no per-engine fit"
        return None
    affordable = {
        engine: row
        for engine, row in (priced.get("engines") or {}).items()
        if row.get("max_elements") and row.get("fit")
    }
    if not affordable:
        return None
    chosen = min(affordable, key=lambda e: (affordable[e]["max_elements"], e))
    try:
        # Ask for the fit OBJECT rather than rebuilding one from the reported dict: the dict is a
        # rendering (it carries `sources`, and its field set is free to change), and a reconstruction
        # that silently fails would drop this axis back onto the mixed fit with nothing said.
        return CA.fit_for(target, engine=chosen)
    except Exception:  # noqa: BLE001 -- an engine we cannot re-fit is one we do not price with
        return None
...
def _cert_affordability(target: str, *, budget_s: float | None) -> dict:
    ...
    # ...AND PRICED PER ENGINE, because the engine IS the cost. This used to call `cert_cost.fit_for`
    # with no `engine=`, which draws one line through every certification the target has, whatever
    # answered it. Measured here (2026-09-21, gemmini): that mixed line is r2 0.149 and puts the
    # ceiling at 19,245 written elements, while the same history split by engine gives 6,500 on gsim
    # (r2 0.230) and 923 on verilator (r2 0.604) -- a 21x spread between the two machines the mixture
    # was averaging, and the corpus's own tracked ceiling of 875 sits next to the verilator figure.
    # The mixed number is not a newer measurement of anything; it is two devices added together.
    #
    # THE BINDING ENGINE IS THE SLOWEST ONE THIS TARGET HAS CERTIFIED ON, not the cheapest and not an
    # average. This ceiling decides whether a capsule may DEMAND certification, and the demand has to
    # be payable on whichever engine the run actually uses -- so the constraint that binds is the
    # engine that takes longest. Choosing the cheapest would emit capsules nobody can afford the
    # moment a run picks the other one, which is the failure this function exists to prevent.
    #
    # `max_elements_within` does the inversion, so the extrapolation clamp (no answer beyond the
    # measured range times the margin) and the "floor alone exceeds the budget" refusal live in one
    # place. The old inline `(budget - intercept) / per_element` here bypassed both.
    engines: dict = {}
    fit = None
    try:
        priced = CA.affordability(target, budget_s=budget)
        engines = {
            engine: {"max_elements": row.get("max_elements"), "n_samples": row.get("n_samples"), "fit": row.get("fit")}
            for engine, row in (priced.get("engines") or {}).items()
        }
        affordable = {e: row["max_elements"] for e, row in engines.items() if row.get("max_elements")}
        if affordable:
            chosen = min(affordable, key=lambda e: (affordable[e], e))
            row = (priced.get("engines") or {})[chosen]
            f = row["fit"] or {}
            lo, hi = (f.get("measured_range_elements") or [0, 0])[:2]
            return {
                "max_elements": int(affordable[chosen]),
                "budget_s": budget,
                "metric": "written_output_elements",
                "engine": chosen,
                "engine_choice": (
                    "the slowest engine this target has measured history on: the ceiling must be "
                    "payable on whichever engine the run picks, so the binding constraint is the "
                    "engine that takes longest"
                ),
                "engines": {e: row["max_elements"] for e, row in engines.items()},
                "law": f"seconds = {f.get('intercept_s')} + {f.get('per_element_s')} * output",
                "calibrated_to_elements": int(hi or 0),
                "extrapolated": bool(int(affordable[chosen]) > int(hi or 0)),
                "basis": (
                    f"fitted on {f.get('n_samples')} cycle-accurate {chosen} certification(s) of THIS "
                    f"target (r2 {f.get('r2')}, {lo}..{hi} written elements); other engine ceilings: "
                    f"{ {e: r['max_elements'] for e, r in engines.items()} }"
                ),
                "why": why,
            }
    except Exception:  # noqa: BLE001 -- unreadable run history is "none"
        engines = {}
    # No engine could be priced. Fall back to the UNATTRIBUTED whole-history fit only to say how
    # little it is worth: it is reported, never returned as the ceiling, because a mixed fit prices a
    # capsule at no machine that exists.
    try:
    ...
    if fit is not None:
        engines.setdefault(
            "<mixed, not used>",
            {
                "max_elements": CC.max_elements_within(fit, budget),
                "n_samples": fit.n_samples,
                "fit": fit.to_dict(),
                "mixed_engines": bool(fit.mixed_engines),
            },
        )
...
def derive_spec(
    ...
    *,
    oracle_tiers: list[str] | Callable[[], list[str]],
    ...
    ``oracle_tiers`` must be the observed constructed inventory, not advertised metadata.
    An empty observation preserves the existing unknown/no-cap policy.
    A caller may supply a zero-argument observer instead of a list. It is called
    ...
    return {
        ...
        # THE GROUPS REAL MODELS FORM. The epilogue axis asks for each stage once; it cannot ask
        # for the COMBINATION a model needs on one contraction, and a backend that fuses each stage
        # alone and no two together passes it. This axis is the compute groups the captures
        # actually form on this target, stated in capsule words.
        "groups": _group_axis(captures, target),
        # STATE CARRIED FROM ONE COMMAND INTO THE NEXT. Every other axis describes one command on a
        # freshly configured unit. A stage that is pure configuration stays switched on until
        # something switches it off, and only a SECOND command, without that stage, can show it.
        "carried_state": _carried_state_axis(target),
        ...
        "oracle_tiers": oracle_tiers() if callable(oracle_tiers) else oracle_tiers,
...
#: Capsule ops that are the same contraction asked for under another name.
_GROUP_OP_CLASS = {"matmul": "matmul", "linear": "matmul", "fused_matmul_bias": "matmul", "conv2d": "conv2d"}


def _group_signature(op: str, epilogue, dtype: str | None) -> str:
    from merlin.runtime.commandbuffer import EPILOGUE_STAGES

    # In the ABI's declared stage order, so two listings of one combination are one signature:
    # what is demanded is that the stages fuse together, which the listing order does not change.
    rank = {stage: index for index, stage in enumerate(EPILOGUE_STAGES)}
    stages = sorted((str(s) for s in epilogue), key=lambda s: (rank.get(s, len(rank)), s))
    return (
        f"{_GROUP_OP_CLASS.get(str(op), str(op))}|{'+'.join(stages) or '-'}|"
        f"{capsule_dtype(str(dtype)) if dtype else '?'}"
    )


def _group_axis(captures: dict[str, str | Path], target: str) -> dict:
    """The compute groups the target's captured models form, as capsule demands.

    Derived by the pass that decides placement (:mod:`compute_groups`), so a demand here is a
    group the compiler will actually try to put on a unit, with the stages it will try to fuse.
    A group whose stages the capsule vocabulary cannot state is counted under ``unstated`` with
    the reason: it is a group no capsule can demand yet, which is a finding about the vocabulary.
    """
    from merlin.common import mlir_query as mq
    ...
    except Exception as exc:  # noqa: BLE001 -- no contract, no groups to derive
        return {
            "required": [],
            "unstated": {},
            "captures_unreadable": {},
    ...
    for model, capture in sorted(captures.items()):
        try:
            # Through the file door: it re-prints a module MLIR wrote in a custom form this parser
            # does not read, so a capture is unreadable here only when it is unreadable.
            demand = CG.demand(
                CG.form_groups(mq.parse(str(capture)), target, oracle=oracle),
                weight_args=stream_plan.weight_args_beside(capture),
            )
        except Exception as exc:  # noqa: BLE001 -- reported, never silently dropped
            unreadable[str(model)] = f"{type(exc).__name__}: {str(exc)[:160]}"
            continue
        for reason, count in demand["unstated"].items():
            unstated[reason] += count
        for entry in demand["entries"]:
            signature = _group_signature(entry["op"], entry["epilogue"], entry.get("operand_dtype"))
            row = required.setdefault(
                signature,
                {
                    "signature": signature,
                    "op": _GROUP_OP_CLASS.get(entry["op"], entry["op"]),
                    "epilogue": list(entry["epilogue"]),
            ...
            row["groups"] += int(entry["count"])
            if str(model) not in row["observed_in"]:
                row["observed_in"].append(str(model))
            granularity = entry.get("scale_granularity")
            if granularity and granularity not in row["scale_granularities"]:
                row["scale_granularities"].append(granularity)
    return {
        "status": "ok",
        "required": sorted(required.values(), key=lambda r: (-r["groups"], r["signature"])),
        "unstated": dict(unstated),
        "captures_unreadable": unreadable,
        "axis_basis": (
            "the compute groups this target's captured models form: a contraction with the stages "
            "its unit absorbs, by op, epilogue COMBINATION and operand format. A corpus that "
            "demands each stage once demands no combination, so a backend that fuses any one stage "
            "and no two passes it; and the model route is only as good as the patterns some capsule "
            "has certified"
        ),
    }


def _carried_state_axis(target: str) -> dict:
    """One requirement per configuration-only stage this target can fuse onto a contraction."""
    from merlin.runtime.commandbuffer import CONFIGURATION_ONLY_STAGES

    fused = (_epilogue_axis(target) or {}).get("required") or ()
    required = [
        {"stage": str(row.get("stage")), "evidenced_by": row.get("evidenced_by")}
        for row in fused
        if str(row.get("stage")) in CONFIGURATION_ONLY_STAGES
    ]
    return {
        "required": required,
        "axis_basis": (
            "a readout stage with no operand and no parameter is configuration a unit stays in; a "
            "program is two commands over one resident weight, the stage on the FIRST and none on the "
            "second, on a stimulus that goes negative, so a backend that does not restore its "
            "configuration returns the second result with the first one's stage applied"
        ),
    }


def _carried_state_gap(required, corpus_roots, *, labels=None, exclude=None) -> dict:
    """Which configuration-only stages no capsule follows with a command that lacks them."""
    import yaml
    ...
    exclude = set(exclude or ())
    roots = [corpus_roots] if isinstance(corpus_roots, (str, Path)) else list(corpus_roots)
    want = [str(r.get("stage")) for r in (required or []) if r.get("stage")]
    ...
    for root in roots:
        for cy in sorted(Path(root).glob("*/capsule.yaml")):
            try:
                ...
                continue
            name = str(cap.get("name") or cy.parent.name)
            if cap.get("label") not in labels or name in exclude:
                continue
            attrs = (cap.get("operation") or {}).get("attributes")
            commands = attrs.get("matmuls") if isinstance(attrs, dict) else None
            if not isinstance(commands, list) or len(commands) < 2:
                continue
            stages = [[str(x) for x in (c.get("epilogue") or ())] for c in commands if isinstance(c, dict)]
            signed = (cap.get(STIMULUS_RANGE_KEY) or [0])[0] < 0
            for stage in want:
                leaks_visible = any(
                    stage in stages[i] and any(stage not in later for later in stages[i + 1 :])
                    for i in range(len(stages))
                )
                if leaks_visible and signed:
                    shown.setdefault(stage, []).append(name)
                elif any(stage in seq for seq in stages):
                    wrong_order.setdefault(stage, []).append(name)
    missing = sorted(set(want) - set(shown))
    return {
        "status": "ok",
        "n_required": len(want),
        "n_covered": len(want) - len(missing),
        "uncovered": missing,
        "demanded_by": {k: sorted(v) for k, v in sorted(shown.items())},
        "cannot_fail": {k: sorted(v) for k, v in sorted(wrong_order.items())},
        "note": (
            "`cannot_fail` lists capsules that carry the stage in a multi-command program without "
            "being able to show it leaking: the stage comes last, or the stimulus cannot go negative"
        ),
    }


def _group_gap(required, corpus_roots, *, labels=None, exclude=None) -> dict:
    """Which required group signatures no capsule in the corpus demands."""
    import yaml
    ...
    for root in roots:
        for cy in sorted(Path(root).glob("*/capsule.yaml")):
            ...
            dtype = next((str(i.get("dtype")) for i in inputs if isinstance(i, dict) and i.get("dtype")), None)
            signature = _group_signature(str(op.get("op") or ""), attrs.get("epilogue") or (), dtype)
            if signature in want:
                demanded.setdefault(signature, []).append(name)
    missing = sorted(set(want) - set(demanded))
    return {
        "status": "ok",
        "n_required": len(want),
        "n_covered": len(want) - len(missing),
        "uncovered": missing,
        "uncovered_groups": sum(int(want[s].get("groups") or 0) for s in missing),
        "required_groups": sum(int(r.get("groups") or 0) for r in want.values()),
        "demanded_by": {k: sorted(v) for k, v in sorted(demanded.items())},
        "note": (
            "an uncovered signature is a group real models form on this target that no "
            "capsule certifies: the model route lowers it with nothing having tested it, or "
            "falls back without anything having required it"
        ),
    }
...
def _scope_gap(required, corpus_roots, *, labels=None, exclude=None) -> dict:
    ...
    # `covered`, `missing`, and no `status` at all -- while every sibling gap function returns
    # `status`/`n_required`/`n_covered`/`uncovered`. Nothing outside `uncovered()` read it, so the
    # difference cost nothing until a caller tried to treat the axes uniformly: the gate's reporter
    # printed `scope: missing` (its default for a dict with no `status`) and its debt accumulator looked
    # for an `uncovered` key that was never there, so an adjacency gap could not reach the verdict
    # however the accumulator was written. An axis that spells its answer differently from its siblings
    # is an axis that will be skipped by anything written to handle them together.
    return {
        "status": "ok",
        "n_required": len(want),
        "n_covered": len(want) - len(missing),
        ...
        # Reported rather than folded into `uncovered`: a capsule with no linalg module cannot be asked
        ...
        "note": (
            ...
            "boundary between two regions is tested by nothing here"
        ),
    }
...
def uncovered(spec_doc: dict, corpus_roots, *, labels=None, tile_dim: int | None = None, exclude=None) -> dict:
    ...
    # THE GROUP AXIS: the stage COMBINATIONS real models form, which the per-stage axis cannot ask for.
    group_req = (spec_doc.get("groups") or {}).get("required")
    if group_req is None:
        out["groups"] = {
            "status": "not_measured",
            "detail": "this spec predates the group axis; regenerate it with --write to derive the requirement",
        }
    else:
        out["groups"] = _group_gap(group_req, corpus_roots, labels=labels, exclude=exclude)

    carried_req = (spec_doc.get("carried_state") or {}).get("required")
    if carried_req is None:
        out["carried_state"] = {
            "status": "not_measured",
            "detail": "this spec predates the carried-state axis; regenerate it with --write to derive the requirement",
        }
    else:
        out["carried_state"] = _carried_state_gap(carried_req, corpus_roots, labels=labels, exclude=exclude)
