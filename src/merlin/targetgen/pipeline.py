...
from .generate import llvm_plan, mlir_scaffold, oot_package, runtime_adapter, target_repo, xdsl, zephyr_module
...
def build(
    ...
    # THE PLUGIN BLOCK. Without it the emitted package resolves through ``target_registry`` and then
    # raises ``KeyError`` from ``get_backend`` — which is what every package this pipeline has ever
    # written did. It is declared only when the ``runtime`` layer is actually emitted: a contract that
    # names a backend the build did not write is a DANGLING pointer, and discovery now records that as
    # a load failure (correctly) rather than ignoring it. Merged rather than assigned, so a curated
    # contract that already declares plugin keys keeps them.
    if "runtime" in selected:
        tc = {**tc, "plugin": {**dict(tc.get("plugin") or {}), **oot_package.plugin_block()}}
    ...
    if not contract_only:
        ...
        if "runtime" in selected:
            artifacts += runtime_adapter.generate(rap, target_contract=tc)
