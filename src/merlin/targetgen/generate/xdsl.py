...
def generate(dialect_plan: dict[str, Any]) -> list[Artifact]:
    ...
    extra_type_specs = [(name, class_name) for name, class_name in type_specs if name not in KNOWN_TYPES]
    dia_tmpl, test_tmpl = (_REAL_DIALECT, _REAL_TEST) if real else (_DECLARED_DIALECT, _DECLARED_TEST)
    return [
        Artifact(
            f"xdsl/{dialect}_dialect.py",
            dia_tmpl.format(
                dialect=dialect,
                ops=ops,
                types=types,
        ...
        Artifact(f"xdsl/test_{dialect}.py", test_tmpl.format(dialect=dialect, ops=ops, types=types)),
    ]
