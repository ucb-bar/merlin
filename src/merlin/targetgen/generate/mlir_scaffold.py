...
def _ops_td(cls: str, dialect: str, extra_ops: list[dict[str, Any]] | None = None) -> str:
    extra = "\n\n".join(_declared_op_definition(cls, dialect, entry) for entry in (extra_ops or []))
...
def _lowering_cpp(
    ...
    for mapping in lowering:
        ...
        if source != target:
            pattern_lines.append(f'    patterns.add<RenameByNamePattern>(context, "{source}", "{target}");')
...
def generate(dialect_plan: dict[str, Any]) -> list[Artifact]:
    ...
    if op_names >= KNOWN:
        return [
            ...
            Artifact(
                f"{ir}/{cls}Ops.td", _ops_td(cls, dialect, [entry for entry in ops if entry["name"] not in KNOWN])
            ),
            ...
            Artifact(f"{libtransforms}/LowerInterface.cpp", _lowering_cpp(cls, pkg, dialect, lowering)),
            Artifact(f"{libtransforms}/CMakeLists.txt", _transforms_cmake(cls)),
            Artifact(f"lib/Dialect/{cls}/CMakeLists.txt", _dialect_cmake()),
            Artifact(f"lib/Dialect/{cls}/README.md", _lib_readme(cls, dialect, lowering_count=len(lowering))),
            Artifact(f"tests/lit/{dialect}/res_pack_roundtrip.mlir", _lit_test(dialect)),
    ...
    return [
        ...
        Artifact(
            ...
            _lib_readme(
                cls, dialect, op_count=len(ops), type_count=len(types), lowering_count=len(lowering), specialized=False
            ),
        ),
        Artifact(f"tests/lit/{dialect}/interface_lowering.mlir", _lowering_lit_test(dialect, lowering)),
    ]
...
def _lowering_lit_test(dialect: str, lowering: list[dict[str, str]]) -> str:
    ...
    operations = "\n".join(f'  "{row["from"]}"() : () -> ()' for row in lowering)
