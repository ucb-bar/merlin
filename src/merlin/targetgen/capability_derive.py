...
def _unit_dtypes(unit: dict) -> tuple[str, ...]:
    """Identified operand formats a unit accepts.

    An accumulate rule can establish a datapath's *width* without identifying its
    numerical format. Such observations are kept in ``unnamed_float_datapaths`` by
    the extractor, but they cannot enter the eligibility dtype axis: treating an
    unresolved ``float8`` as a gradeable FP8 format would turn uncertainty into a
    false missing-axis finding (or, worse, a fabricated compiler obligation).
    """
    unnamed = set(unit.get("unnamed_float_datapaths") or ())
    seen = [value for value in (unit.get("dtypes") or ()) if value not in unnamed]
    for rule in unit.get("accumulate") or ():
        if isinstance(rule, dict):
            for key in ("in", "weight"):
                v = rule.get(key)
                if v and v not in unnamed and v not in seen:
                    seen.append(v)
    return tuple(seen)
