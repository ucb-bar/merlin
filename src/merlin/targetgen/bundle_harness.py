...
__all__ = [
    ...
    "TRAJECTORY_SYMBOL",
    "CONST_BASE_MACRO",
    "PC_RELATIVE_REACH_BYTES",
    "render_reseed",
    "render_session_loop",
    "render_freestanding_support",
    "unresolved_symbols",
    "FREESTANDING_SHIMS",
]
...
def render_session_loop(plan: PackPlan, *, steps: int, call: str, record: Mapping[str, Any] | None = None) -> str:
    ...
    body = [f"for (int merlin_step_index = 0; merlin_step_index < {int(steps)}; ++merlin_step_index) {{"]
...
def _gate_check(
    ...
    *,
    output_offset: int,
    output_ctype: str,
    step_expression: str = "0",
    from_trajectory: bool = False,
    dump_values: bool | None = None,
    line_budget: int = CONSOLE_LINE_BUDGET,
) -> str:
    ...
    out = (
        f"{TRAJECTORY_SYMBOL} + (long)({step_expression}) * {n}"
        if from_trajectory
    ...
    lines = [
        ...
        "    merlin_bits |= (unsigned long long)((const unsigned char *)&merlin_out[i])[b] << (8u * b);",
        "  }",
    ...
    if dump_values if dump_values is not None else gate.prints_values:
        ...
        lines += [
            f'printf("MERLIN_GATE_MODE digest_and_argmax elements=%d steps=%d line_budget=%d\\n",',
    ...
    if gate.expected_argmax is not None:
        ...
        if gate.resolving_power == "margin_qualified":
            # THE PARTITION IS BAKED, NOT COMPUTED HERE. Which rows the reference can decide
            # depends only on the reference and the measured floor, both known before the run, so
            # the host derives it once (bundle_gate.unadjudicable_from_reference ->
            # tie_sensitivity.adjudicable_rows) and the device carries the answer. The rule then
            # exists in ONE place: a C reimplementation of it could drift from the Python and the
            # drift would be silent and in the permissive direction.
            excused = sorted(set(gate.unadjudicable_rows))
            flags = ", ".join("1" if r in excused else "0" for r in range(int(gate.rows)))
            lines += [
                f"/* Rows the reference cannot discriminate at noise floor {gate.noise_floor!r}: {excused}. */",
                f"static const char merlin_unadjudicable[{int(gate.rows)}] = {{ {flags} }};",
                f'printf("MERLIN_GATE_RESOLVING power=%s rows=%d unadjudicable=%d\\n",',
                f'       "{gate.resolving_power}", {int(gate.rows)}, {len(excused)});',
            ]
        lines += [
            f"int merlin_rows_disagreeing = 0;",
            "int merlin_rows_indeterminate = 0;",
            ...
            "      merlin_row_ref = i;",
            "  }",
            "  const int merlin_row_agrees = (merlin_row_argmax == merlin_row_ref);",
        ]
        if gate.resolving_power == "margin_qualified":
            lines += [
                # INDETERMINATE IS NOT PASS: it gets its own counter and its own printed status,
                # and it is never added to the agreeing population.
                '  const char *merlin_row_status = merlin_row_agrees ? "agrees"',
                '      : (merlin_unadjudicable[r] ? "reference_cannot_discriminate" : "disagrees");',
                "  if (!merlin_row_agrees) {",
                "    if (merlin_unadjudicable[r]) ++merlin_rows_indeterminate;",
                "    else ++merlin_rows_disagreeing;",
                "  }",
                '  printf("MERLIN_TOP1 step=%d row=%d got=%d want=%d status=%s\\n", merlin_step,',
                "         r, merlin_row_argmax, merlin_row_ref, merlin_row_status);",
            ]
        else:
            lines += [
                "  if (!merlin_row_agrees) ++merlin_rows_disagreeing;",
                '  printf("MERLIN_TOP1 step=%d row=%d got=%d want=%d\\n", merlin_step, r,',
                "         merlin_row_argmax, merlin_row_ref);",
            ]
        lines += [
            "}",
            f'printf("MERLIN_GATE_EXPECT rows=%d disagreeing=%d indeterminate=%d\\n",',
            f"       {int(gate.rows)}, merlin_rows_disagreeing, merlin_rows_indeterminate);",
    ...
    if gate.resolving_power == "margin_qualified":
        # THE VALUE ASSERTION. `merlin_bad` used to be counted, printed and NOT asserted, on the
        # reasoning that an exact elementwise contract against this reference is unachievable. That
        # reasoning is sound and the conclusion drawn from it was wrong: measured 2026-09-10, a
        # gate asserting only the per-row ranking PASSED a program with 128,041 of 256,000 elements
        # outside tolerance, because one changed constant left every argmax intact. Unachievable as
        # an EQUALITY does not mean unbounded -- so the population is asserted against a declared,
        # derived bound, and the digest is asserted whenever the program is a fixed one.
        assertions = ["merlin_nonfinite != 0", "!merlin_argmax_ok", "merlin_bad > merlin_max_bad"]
        lines += [
            f"const int merlin_max_bad = {int(gate.max_bad)};",
            'printf("MERLIN_GATE_ELEMENTWISE asserted=1 counted=%d of=%d bound=%d\\n",'
            f" merlin_bad, {n}, merlin_max_bad);",
        ]
        if gate.expected_digest:
            # A fixed program must reproduce its own bytes; that is what makes a run on other
            # hardware a REPRODUCTION rather than an unrelated measurement. Compared as an integer
            # against the declared value, never as a formatted string.
            lines += [
                f"const unsigned long long merlin_want_digest = 0x{gate.expected_digest}ULL;",
                "const int merlin_digest_ok = (merlin_digest == merlin_want_digest);",
                'printf("MERLIN_GATE_DIGEST asserted=1 got=%016llx want=%016llx ok=%d\\n",',
                "       merlin_digest, merlin_want_digest, merlin_digest_ok);",
            ]
            assertions.append("!merlin_digest_ok")
        else:
            lines += ['printf("MERLIN_GATE_DIGEST asserted=0 got=%016llx\\n", merlin_digest);']
        lines += [f"  return ({' || '.join(assertions)}) ? 1 : 0;"]
    else:
        lines += [
...
def render_bundle_harness(
    plan: PackPlan,
    gate: CorrectnessGate,
    *,
    entry_symbol: str,
    output_tensor: str,
    output_ctype: str = "float",
    ...
    if reachable_bytes >= PC_RELATIVE_REACH_BYTES:
        detail = (
            "even with the const blob addressed absolutely, the near region "
            if far
            else "the const blob is linked beside the code, so the whole image "
        )
        raise BundleHarnessError(
            ...
            + (
                "Reduce the near region -- the mutable arena and the compiler's static arena are what it holds."
                if far
                else f"Pass const_blob_base to address the {plan.const_bytes}-byte const blob from a "
    ...
    graded = next((t for t in plan.mutable if t.tensor == output_tensor and t.role == "argument"), None)
    if graded is None:
    ...
    record = {"offset": graded.offset, "elements": graded_elements, "ctype": output_ctype} if steps > 1 else None
    ...
    gate_fn = _gate_check(
        gate,
        output_offset=graded.offset,
        output_ctype=output_ctype,
        step_expression="merlin_step_index",
        from_trajectory=steps > 1,
        dump_values=dump_values,
        line_budget=console_line_budget,
    )
    const_declaration = (
        [
        ...
        if far
        else [f"extern const unsigned char {CONST_SYMBOL}[];"]
    )
    declarations = "\n".join(
        [
            *const_declaration,
    ...
    pointers = ",\n    ".join(pointer_expression(t.storage, t.offset, const_is_far=far) for t in args)
    call = f"{entry_symbol}(\n    {pointers});"
    ...
    recorded_bytes = steps * graded_elements * _CTYPE_BYTES.get(output_ctype, 0) if record is not None else 0
    ...
    return {
        ...
        "graded_output": {
            "tensor": graded.tensor,
            "offset": graded.offset,
...
FREESTANDING_SHIMS: Mapping[str, Mapping[str, str]] = {
    "__errno": {
        ...
        "why": (
            "newlib's libm writes errno on a domain or range error, and generated kernel code "
...
def render_freestanding_support(symbols: Sequence[str]) -> str:
    ...
    blocks = [
        "/* Freestanding support. Each definition carries the argument for why it is",
...
def is_executable_emission(command_buffer: Mapping[str, Any], *, artifact_text: str | None = None) -> tuple[bool, str]:
    ...
    if declined:
        ...
        return False, (
            f"the compiler DECLINED this program and said why: {reason[:400]}"
            if reason
            else "the compiler declined this program"
        )
    ...
    if emitted is False:
        return False, (
            f"params.{EMITTED_PROGRAM_KEY} is False: the host-lane builder ran and declined to emit a program"
        )
    ...
    if artifact_text is not None:
        ...
        if not declared:
            return False, (
                "the emitted artifact defines an entry taking "
