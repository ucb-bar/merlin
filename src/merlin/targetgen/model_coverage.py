...
def _contraction_extents(module) -> dict:
    ...
    _windowed_contraction_extents(module, out)
    return out


def _windowed_contraction_extents(module, out: dict) -> None:
    """Extents for the contractions the shape observer does not accept, read off their loops.

    The observer recognises a contraction whose operands are indexed by plain iteration dims. A
    convolution in generic form indexes its input by ``stride * d + tap``, so it is a contraction by
    structure (a reduction whose body multiplies and accumulates) that the observer passes over. On
    a captured CNN that was every convolution: 53 of 54 contractions carried no extents, so offload
    weighted by work could not see the model's work at all.

    Multiply-accumulates are the product of the loop extents; the split follows the iterator kinds
    (``m`` the leading parallel loops, ``n`` the last, ``k`` the reduction loops). An op whose loops
    cannot be read stays absent, which a consumer reports as unknown rather than as zero.
    """
    try:
        from merlin.common import mlir_query as mq
        from merlin.xdsl_dialects.lowering import contraction_coverage as cc
    except Exception:  # noqa: BLE001 - without the classifier there is nothing to add, never a guess
        return
    import math

    for op in mq.walk(module):
        if id(op) in out or mq.op_name(op) != "linalg.generic":
            continue
        try:
            if cc.classify_generic(op) != "contraction":
                continue
            extents = cc.loop_extents(op)
            kinds = op.properties.get("iterator_types") or op.attributes.get("iterator_types")
            kinds = [str(kind) for kind in kinds]
        except Exception:  # noqa: BLE001
            continue
        if not extents or len(extents) != len(kinds) or set(extents) != set(range(len(kinds))):
            continue
        par = [extents[i] for i, kind in enumerate(kinds) if "reduction" not in kind]
        red = [extents[i] for i, kind in enumerate(kinds) if "reduction" in kind]
        if not par or not red:
            continue
        m = math.prod(par[:-1]) if len(par) > 1 else par[0]
        n = par[-1] if len(par) > 1 else 1
        out[id(op)] = (m, math.prod(red), n, len(kinds))
...
#: Parsed captures, keyed by ``(resolved path, mtime_ns, size)`` so an edited capture is never served
#: stale. Bounded LRU, exactly as :data:`merlin.common.mlir_query._PARSE_CACHE` is and for the same
#: reason: a conformance spec derivation reads the SAME captures once per axis -- cells, host lane,
#: conv geometry, boundaries, geometry, memory regime, scope -- and a capture is tens of thousands of
#: ops. Measured 2026-09-21 on this store: 41 captures cost 86 s for ONE axis, and the full 6-axis
#: derivation ran past a 900 s test budget and a 300 s one. Modules are read-only for every consumer
#: here, so sharing one parsed instance is safe.
#:
#: SIZED TO HOLD THE WHOLE STORE, because a per-axis working set is the wrong unit: each axis walks
#: EVERY capture, so a cache smaller than the store evicts the entry it is about to want again and the
#: next axis re-parses nearly all of them. Measured with a cache of 8 against 41 captures: one axis is
#: 89 s and the following one pays it again.
#:
#: 64 rather than a guess, from measuring what holding them costs: the 41 captures in this store are
#: 58.1 MB of MLIR text and 0.72 GB resident once parsed. That is cheap enough that bounding this
#: tightly buys nothing and costs a re-parse per axis; the bound stays only so a store that grows by an
#: order of magnitude degrades to eviction rather than to the OOM killer.
_MODULE_CACHE: OrderedDict[tuple, Any] = OrderedDict()
...
def load_module(path: str | Path):
    ...
    try:
        ...
        key = None  # unstattable: parse it and cache nothing rather than key on a path that may move
    if key is not None:
    ...
    # NORMALIZE THE MULTI-RESULT FORM FIRST, exactly as `linalg_mlir.parse_mlir_text` does. MLIR prints
    # a multi-result `linalg` terminator parenthesised (`} -> (tensor<..>, tensor<..>)`) and xDSL's
    # linalg parser accepts only the bare form, so a module containing one op of that shape -- an
    # `aten.min.dim` lowering, which returns values AND indices -- cannot be parsed here.
    #
    # THE COST OF NOT DOING THIS, measured 2026-09-21: five captures (`bitvla` and four `smolvla`
    # bundles) were recorded as `captures_unreadable` in every target's conformance spec, so the
    # derived requirement was narrower than the evidence supports and said so in a note nobody could
    # act on. The recorded error named `tensor.expand_shape` and pointed at `into`, which reads as a
    # missing-dialect-feature problem and is not one: that is simply the op the parser reached before
    # giving up, several thousand lines past the real cause. Same file, same context, same xDSL --
    # adding this one call parses it.
    module = Parser(ctx, strip_paren_results(p.read_text(encoding="utf-8")), str(path)).parse_module()
    if key is not None:
