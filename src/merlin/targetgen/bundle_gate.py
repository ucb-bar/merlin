...
__all__ = [
    ...
    "CHANNELS",
    "reference_digest",
    "gate_from_session_contract",
    "grades_unquantized_activations",
    "reference_spread",
    "ReferenceSpread",
    "RESOLVING_POWERS",
    "unadjudicable_from_reference",
]
...
REFERENCE_KINDS: Mapping[str, Mapping[str, Any]] = {
    ...
    "weight_only_int8": {
        ...
        "activations": "full",
        "scope": (
            "weight-only int8; grading a W8A8 run against it measures ACTIVATION "
    ...
    "w8a8_execution": {
        ...
        "scope": (
            "whether the DEVICE reproduced what the host compiler computes -- a host run "
    ...
    "w8a8_independent": {
        ...
        "activations": "quantized",
        "scope": (
            "the arithmetic itself; fail-closed on the bundle's quantized weights not "
    ...
    "pt2e_integer": {
        ...
        "activations": "quantized",
        "scope": (
            "an exact integer reference; available ONLY for a TorchAO PT2E Q/DQ graph, so it "
    ...
    "eager_same_precision": {
        ...
        "weights": "follows_datapath",
        "activations": "follows_datapath",
    },
    "eager_fp32": {
        ...
        "activations": "full",
        "scope": (
            "how much accuracy the QUANTIZATION cost, which is a model-quality claim. It is "
...
COMPARISONS: Mapping[str, str] = {
    "exact_elementwise": (
        "every element within an absolute+relative tolerance, with a count of "
        "violations; only meaningful against an exact integer reference"
    ),
    "tolerance_and_topk": (
        "a stated tolerance plus the argmax per row; the right shape for a "
        "language model, where the ranking is the result and the logits are not"
    ),
    "trajectory": (
        "a declared key over a multi-step session, compared step by step; a single step "
...
_ACTIVATION_UNQUANTIZED = frozenset(name for name in REFERENCE_KINDS if grades_unquantized_activations(name))
...
#: What the reference can actually resolve, which decides what "passed" is allowed to mean.
#:
#: ``bit_exact`` is the correct bar wherever it is achievable, and where it is achievable it must
#: stay: ResNet-50's W8A8 output reproduces its reference exactly, so any difference at all is a
#: defect and nothing about it is negotiable.
#:
#: ``margin_qualified`` exists because on one measured model it is NOT achievable. TinyLlama's W8A8
#: logits differ from a torch-eager dynamic-quantization reference by 3.6e-2 relative L2 on every
#: token row, and the mechanism is measured rather than assumed: the divergence sits at 5e-7 for two
#: whole decoder layers, jumps to 2.3e-4 at exactly one activation quantizer whose value is 1.9e-6
#: from a round-half-even tie while the reference's is 2.2e-5 on the other side, and then compounds
#: layer over layer to 3.6e-2. Forcing that one code back makes the layer exact again (7.3e-7), so
#: the arithmetic is not in question -- but 485 of 2,809,856 quantized activations lie within 1e-4
#: of a tie, so an independent implementation disagrees on a POPULATION of them and no tolerance
#: recovers an elementwise contract. A margin-qualified gate says which of its decisions the
#: reference was able to make at all, and reports the rest as unadjudicable -- never as passes.
RESOLVING_POWERS: Mapping[str, str] = {
    "bit_exact": (
        "the reference resolves every element; any difference is a defect. The correct "
        "bar wherever it is achievable, and it must not be relaxed where it is"
    ),
    "margin_qualified": (
        "the reference's own decisions are only trustworthy above a MEASURED "
        "noise floor; decisions at or below it are reported as unadjudicable and "
        "are never counted as agreement"
    ),
}


def unadjudicable_from_reference(reference_rows: Any, *, noise_floor: float) -> tuple[int, ...]:
    """Which rows this reference cannot decide -- derived from the REFERENCE, before any run.

    Delegates to :func:`merlin.perf.tie_sensitivity.adjudicable_rows` so the rule lives in one
    place. Deriving the partition from the reference alone is what stops a candidate from widening
    it: a program cannot move its own disagreement into the excused population.
    """
    from merlin.perf.tie_sensitivity import adjudicable_rows

    return tuple(index for index, ok in enumerate(adjudicable_rows(reference_rows, noise_floor=noise_floor)) if not ok)
...
class CorrectnessGate:
    ...
    #: What the reference can resolve. Recorded on every gate so a reader cannot mistake a
    #: margin-qualified verdict for a bit-exact one -- they are different claims and the weaker one
    #: must say so on its face.
    resolving_power: str = "bit_exact"
    #: The measured divergence, in the reference's own score units, below which the reference's
    #: decisions are not trustworthy. Required by ``margin_qualified`` and refused otherwise.
    noise_floor: float | None = None
    #: How that floor was MEASURED. A floor with no derivation is a guess, and a guessed floor
    #: decides which rows get excused, which is the whole verdict.
    noise_floor_derivation: str = ""
    #: Rows whose reference margin does not exceed the floor. Derived from the reference before the
    #: run by :func:`unadjudicable_from_reference`, never from the candidate's output.
    unadjudicable_rows: tuple[int, ...] = ()
    #: THE VALUE ASSERTION, and the reason it exists: a margin-qualified gate that asserted only the
    #: per-row ranking PASSED A WRONG PROGRAM. Measured 2026-09-10 -- one changed compiler constant
    #: (a 1% error in every RMSNorm) left all eight token rows agreeing on top-1, so the gate
    #: returned 0 and published a METRIC, while the elementwise count went from 47,854 to 128,041 of
    #: 256,000 and the output digest changed completely. The margin rule answers the RANKING
    #: question and nothing else; a row being unadjudicable in RANK says nothing about whether its
    #: VALUES moved. So the population is asserted separately, against a declared bound.
    max_bad: int | None = None
    #: How that bound was derived. Required with ``max_bad`` for the same reason ``noise_floor``
    #: requires one: a bound chosen rather than measured decides what passes.
    max_bad_derivation: str = ""
    #: The output digest this exact program is expected to reproduce, when the gate grades a FIXED
    #: program rather than a candidate. Asserted when declared. It is what makes a re-run on other
    #: hardware a reproduction rather than a fresh measurement.
    expected_digest: str = ""
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            "schema": "merlin_bundle_correctness_gate_v1",
            "model": self.model,
            "datapath": self.datapath,
            "reference_kind": self.reference_kind,
            "reference_file": self.reference_file,
            "reference_scope": kind["scope"],
            "reference_generator": kind["generator"],
            ...
            "channel_asks": CHANNELS[self.channel]["asks"],
            "resolving_power": self.resolving_power,
            "resolving_power_licence": RESOLVING_POWERS[self.resolving_power],
            "noise_floor": self.noise_floor,
            "noise_floor_derivation": self.noise_floor_derivation,
            "unadjudicable_rows": list(self.unadjudicable_rows),
            "max_bad": self.max_bad,
            "max_bad_derivation": self.max_bad_derivation,
            "expected_digest": self.expected_digest,
            "what_the_margin_rule_covers": (
                "the RANKING only. A row the reference cannot discriminate is unadjudicable in "
                "rank; that says nothing about whether its values moved, which is why max_bad "
                "and expected_digest are asserted separately."
            ),
            "adjudicable_rows": [r for r in range(self.rows) if r not in set(self.unadjudicable_rows)],
...
def gate_for(
    *,
    ...
    noise_floor: float | None = None,
    noise_floor_derivation: str = "",
    unadjudicable_rows: Any = (),
    max_bad: int | None = None,
    max_bad_derivation: str = "",
    expected_digest: str = "",
    available_references: Mapping[str, bool] | None = None,
) -> CorrectnessGate:
    ...
    if reference_kind not in REFERENCE_KINDS:
        raise GateError(
            f"reference kind {reference_kind!r} is not one of "
    ...
    if datapath not in ("w8a8", "fp32"):
        raise GateError(
            f"datapath {datapath!r} must be 'w8a8' or 'fp32'; it decides which references can mean anything"
        )
    if atol < 0 or rtol < 0:
    ...
    if channel not in CHANNELS:
        raise GateError(
            f"channel {channel!r} is not one of {sorted(CHANNELS)}; a gate that does "
    ...
    # WHAT THE REFERENCE CAN RESOLVE. A gate that grades against a reference of limited resolving
    # power must SAY so, carry the floor it used, and say how that floor was measured. The refusals
    # below are the ones that keep "margin-qualified" from becoming "looser".
    if resolving_power not in RESOLVING_POWERS:
        raise GateError(
            f"resolving power {resolving_power!r} is not one of "
            f"{sorted(RESOLVING_POWERS)}; a gate that does not say what its reference "
            f"can resolve invites a margin-qualified verdict being read as bit-exact"
        )
    unadjudicable = tuple(sorted({int(r) for r in unadjudicable_rows}))
    if resolving_power == "bit_exact":
        if noise_floor is not None or noise_floor_derivation or unadjudicable:
            raise GateError(
                f"{model}: a bit_exact gate resolves every element, so a noise floor and an "
                f"unadjudicable row set would be inert fields that a reader would take for an "
                f"excuse. Declare resolving_power='margin_qualified' or drop them"
            )
        if max_bad is not None:
            raise GateError(
                f"{model}: a bit_exact gate already asserts every element (max_bad is 0 by "
                f"construction); declaring {max_bad!r} would loosen it silently"
            )
    else:
        if noise_floor is None or not (float(noise_floor) > 0.0):
            raise GateError(
                f"{model}: a margin_qualified gate needs a POSITIVE measured noise floor; it "
                f"decides which decisions are adjudicable at all, and a gate with no floor "
                f"excuses nothing while claiming it might"
            )
        if not noise_floor_derivation.strip():
            raise GateError(
                f"{model}: the noise floor {noise_floor!r} has no stated derivation. A floor "
                f"chosen rather than measured decides which rows get excused, which is the whole "
                f"verdict -- say what measured it"
            )
        if any(r < 0 or r >= rows for r in unadjudicable):
            raise GateError(
                f"{model}: unadjudicable rows {unadjudicable} fall outside the {rows} declared ranking rows"
            )
        if len(unadjudicable) >= rows:
            raise GateError(
                f"{model}: every one of the {rows} rows is unadjudicable, so this gate cannot "
                f"fail. A check that cannot fail reports success -- refuse it rather than ship it"
            )
        # THE REFUSAL THAT THE MEASURED FAILURE BOUGHT. Without a value assertion this gate passed
        # a program with 128,041 of 256,000 elements outside tolerance, because all eight rankings
        # happened to survive. Ranking agreement is necessary and nowhere near sufficient.
        if max_bad is None:
            raise GateError(
                f"{model}: a margin_qualified gate must ALSO declare max_bad. Asserting only the "
                f"per-row ranking passed a wrong program once already -- a 1% miscompile kept all "
                f"eight argmaxes while moving 128,041 of 256,000 elements outside tolerance. The "
                f"margin rule covers the RANKING; the values need their own bound"
            )
        if int(max_bad) < 0 or int(max_bad) >= output_elements:
            raise GateError(
                f"{model}: max_bad {max_bad!r} must be in [0, {output_elements}); a bound at or "
                f"above the element count admits every possible output"
            )
        if not max_bad_derivation.strip():
            raise GateError(
                f"{model}: max_bad {max_bad!r} has no stated derivation. A bound chosen rather "
                f"than measured decides what passes -- say what measured it"
            )
    ...
    return CorrectnessGate(
        ...
        noise_floor_derivation=noise_floor_derivation,
        unadjudicable_rows=unadjudicable,
        max_bad=None if max_bad is None else int(max_bad),
        max_bad_derivation=max_bad_derivation,
...
def reference_digest(golden_path: Any, key: str) -> str:
    ...
    with np.load(golden_path) as data:
        if key not in data.files:
            raise GateError(
                f"{golden_path}: the contract's key {key!r} is absent from the golden "
...
def gate_from_session_contract(
    ...
    *,
    ...
    require_derivable_tolerance: bool = True,
) -> CorrectnessGate:
    ...
    if not CHANNELS[channel]["admits_unquantized_reference"] and grades_unquantized_activations(reference_kind):
        raise GateError(
    ...
    scope_note = (
        f"{CHANNELS[channel]['asks']}; over the contract-declared {steps}-step "
        ...
        + (
            ...
            f"this tolerance was checked against"
    ...
    return gate_for(
        ...
        atol=atol,
        rtol=rtol,
        output_elements=per_step_elements,
        steps=steps,
        session_key=key,
        scope_note=scope_note,
        channel=channel,
