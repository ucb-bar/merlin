...
from .providers import Provider, read_provider
...
class TargetPackage:
    ...
    def provider(self) -> Provider | None:
        """Read-only package role; capability validation remains with the existing loader."""
...
def load_target(package_dir: str | Path) -> TargetPackage:
    ...
    d = Path(package_dir)
    # ASK FIRST. `manifest.yaml` names two disjoint schemas in sibling directories -- this loader's
    # registry format (target/run_id/outputs) and the experiment-ABI format (artifact_type/entrypoints/
    # commands) -- so pointing this function at the wrong one used to surface as a KeyError on
    # `manifest['target']`, or a FileNotFoundError on dialect.py, from inside the load. The classifier
    # answers the question this function was about to assume the answer to, and names the missing file.
    _package.require(d, "dialect")
