...
def validate_materialized_cohort(root: str | Path, te, *, corpus_roots: list[Path] | None = None) -> dict:
    ...
    source_names = sorted(
        p.name for p in _public_capsule_dirs_in(te.graded_roots() if corpus_roots is None else corpus_roots)
    )
    ...
    expected_policy = (
        "descriptor_search_cohort_v1"
        if search_include
        else "descriptor_capability_resource_and_phase_v1"
        if declared_phase is not None
        else "descriptor_capability_and_resource_v1"
        if explicit
        else "descriptor_exclusions_legacy"
        if excluded
        else "all_discovered"
    )
...
def materialize_public_capsules(
    dest: str | Path,
    *,
    tier_ceiling: str = _DEFAULT_CEILING,
    ...
    dest = Path(dest)
    written: list[str] = []
    sources = _public_capsule_dirs_in(corpus_roots) if corpus_roots is not None else _public_capsule_dirs(contract)
...
def descriptor_corpus_roots(te) -> list[Path]:
    """The public materializer's original primary-plus-sibling discovery, in order."""
...
def materialize_public_cohort(
    te, *, tier_ceiling: str, corpus_roots: list[Path] | None = None, destination: Path | None = None
) -> Path:
    """Publish and validate the descriptor cohort at an explicitly selected ceiling.

    This operation does not discover or construct evaluators. ``corpus_roots`` lets
    a workflow retain the same source discovery used to select its ceiling.
    """
    from merlin.common.artifacts import cache_dir

    roots = descriptor_corpus_roots(te) if corpus_roots is None else corpus_roots
    ...
    if destination is not None:
        ver = Path(destination).absolute()
        for source in roots:
            resolved = Path(source).resolve()
            if (
                ver.resolve() == resolved
                or ver.resolve().is_relative_to(resolved)
                or resolved.is_relative_to(ver.resolve())
            ):
                raise ValueError("public cohort destination overlaps its source")
        ver.mkdir(parents=True, exist_ok=False)
    else:
        base = cache_dir("capsule_bench_public")
    ...
    if search_include:
        ...
        if sorted(set(capability_excluded) | set(resource_excluded) | set(phase_excluded)) != declared_excluded:
    ...
    if expected_source is not None and len(source_names) != expected_source:
        raise ValueError(f"source cohort drift: descriptor expects {expected_source}, discovered {len(source_names)}")
    if expected_admitted is not None and len(admitted_names) != expected_admitted:
        raise ValueError(f"admitted cohort drift: descriptor expects {expected_admitted}, wrote {len(admitted_names)}")
    descriptor_sha = _loaded_descriptor_sha256(te)
    cohort_record = {
        "version": 1,
        "policy": policy,
    ...
    (ver / ".cohort_admission.json").write_text(json.dumps(cohort_record, indent=2) + "\n", encoding="utf-8")
    if destination is not None:
        validate_materialized_cohort(ver, te, corpus_roots=roots)
        return ver
...
def cert_capsule_cover(
    ...
    return {
        ...
        "basis": {
            "axes": ["semantic_family", "dtype"] + (["tile_alignment"] if tile_dim else []),
            "tile_dim": tile_dim,
            ...
            "note": (
                "instruction_classes is declared empty by every capsule in this corpus, so it "
                "could not be used as an axis"
                if not n_class
                else ""
            ),
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--tier-ceiling", default=_DEFAULT_CEILING, help=f"highest reachable oracle tier (default {_DEFAULT_CEILING})"
    )
