...
def selected_bundle_tool_paths(
    allowed_paths: set[str], tool_names: tuple[str, ...] | list[str], *, python_source_root: Path | None = None
) -> set[str]:
    """Return only selected tool grants, in their actual declared spelling.

    Legacy bundles use checkout-relative paths. Fresh reviewed releases rewrite
    those same paths to the active installed ``merlin`` package; descriptor
    inputs such as RTL facts must not be mistaken for an arm-gated tool.
    """
    selected: set[str] = set()
    for name in tool_names:
        for logical in spec(name).bundle_paths:
            candidates = {logical}
            if python_source_root is not None and logical.startswith(_PY):
                path = (Path(python_source_root) / "merlin" / logical[len(_PY) :]).resolve()
                candidates.add(str(path) + ("/" if logical.endswith("/") else ""))
            selected.update(candidates & allowed_paths)
    return selected
...
class BrokerSpec:
    ...
    channel dir in the workspace. Canonical host implementations belong to
    ``merlin_experiments.phase1.brokers``; only public clients are staged in-box.
    """
    ...
    shims: tuple[tuple[str, str], ...]  # (canonical Python module, staged workspace filename)
...
_ISA_BROKER = BrokerSpec(
    ...
    "isa_tools_broker.log",
    (("merlin_experiments.phase1.tools.isa", "isa_tools.py"),),
)
_CCA_BROKER = BrokerSpec(
    ...
    "cca_broker.log",
    (
        ("merlin_experiments.phase1.tools.cca", "cca_contract.py"),
        ("merlin_experiments.phase1.tools.cca", "action_catalog.py"),
    ),
)

# Public standalone clients common to every treatment; brokers remain host-owned.
COMMON_CLIENTS = (
    ("merlin_experiments.phase1.tools.selfcheck", "agent_selfcheck.py"),
    ("merlin_experiments.phase1.tools.simjob", "simjob.py"),
    ("merlin_experiments.phase1.tools.await_verdict", "await_verdict.py"),
)

# Common feedback is not an ablatable tool treatment. Its host owners share the
# same mapping shape so staging and launch never guess native filenames.
COMMON_BROKERS = (
    BrokerSpec(".qa_channel", "merlin_experiments.phase1.brokers.selfcheck", "broker.log", (COMMON_CLIENTS[0],)),
    BrokerSpec(".qa_channel", "merlin_experiments.phase1.brokers.simjob", "simjob_broker.log", (COMMON_CLIENTS[1],)),
)
...
TOOLS: dict[str, ToolSpec] = {
    ...
    "verify_seam": ToolSpec(
        ...
        "caught, a sweep of the target's derived extent lattice, and `python -m merlin.xdsl_dialects.opt`, "
        "which runs a "
...
def public_client_modules() -> tuple[str, ...]:
    """Exact standalone client source closure, independent of checkout script names."""
    clients = {module for module, _ in COMMON_CLIENTS}
    clients.update(module for tool in TOOLS.values() if tool.broker for module, _ in tool.broker.shims)
