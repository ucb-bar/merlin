...
class OpDemand:
    ...
    elem_fmt: str | None = None
    #: How many INDEPENDENT contractions the ``m``/``k``/``n`` extents repeat over -- 1 for a plain
    #: matmul, the product of the batch dims for a batched one. ``None`` when there are no extents.
    #:
    #: It is recorded rather than folded into ``m`` because folding it in states something false: a
    #: ``48x1x9 @ 48x9x16`` batched contraction is not a ``48x9 @ 9x16`` matmul -- it has 48 distinct
    #: right-hand operands, and a tile certified at the folded shape would be evidence about an operation
...
def host_is_declared(target_name: str) -> bool | None:
    ...
    try:
        ...
        # Through `corpora`, which owns the experiments/ layout AND honours MERLIN_TARGET_EXPERIMENT.
        # Building the convention path here ignored that override, so a run pointed at an out-of-tree
        # descriptor resolved it for some readers and not others.
        p = descriptor_path(target_name)
        if not p.is_file():
            return None
...
def host_board_gap(target_name: str) -> str | None:
    ...
    * UNKNOWN -- undetermined, and reported as such. Not folded into either: a target whose endpoint
    ...
    try:
        ...
        te = load_target_experiment(descriptor_path(target_name))
