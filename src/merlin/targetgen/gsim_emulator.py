...
def installed_engine_home(target: str, engine: str = "gsim") -> Path:
    """The REPO-anchored engine home, ignoring ``MERLIN_OUT_ROOT``.

    :func:`engine_home` follows the RUN's output root, which is right for something the run builds and
    wrong for a tool it merely uses. A cert whose ``project_root`` is a per-invocation temp directory
    resolves its engine home under that temp directory: measured on this host, 0 of 2295 such roots
    held one, so the elaborated-RTL tier fell to the slow engine on every single one of them and no
    amount of installing could change it. The engine is a machine asset; this is where it is installed.
    """
    from merlin.common.paths import tracked_build_dir

    return tracked_build_dir() / "rtl_engines" / str(target) / str(engine)
...
def resolve(target: str, *, env_var: str | None = None) -> Resolution:
    ...
    if not path.is_file():
        # The run-relative home has no binary. Before giving up, look where the engine is INSTALLED:
        # that home is repo-anchored, because a simulator is a machine asset and a run whose output
        # root is a per-invocation temp directory would otherwise never find one. Only when the caller
