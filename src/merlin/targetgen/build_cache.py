...
#: Build-path digests, keyed by the stat signature of the files they cover, each with the moment its
#: bytes were read. Reading and hashing ~75 source files is cheap once and not cheap per capsule per
#: tier, so a stat walk decides whether the answer is still current.
#:
#: THE SIGNATURE IS NOT THE CONTENT. Size and mtime were held to leave "no stale window", and they do
#: not: a file rewritten with the SAME number of bytes inside the clock tick the memo was taken has an
#: identical signature, so the stale digest is returned and a build identity does not change when the
#: code that performs the build does. Measured on `test_declared_sibling_helper_changes_build_identity`,
#: which rewrites `VALUE=1` as `VALUE=2` -- eight bytes either way -- and failed only when the run was
#: fast enough for both writes to land in one tick. An identity that can miss a change is worse than no
#: cache, so a hit is trusted only for files that have been QUIESCENT: last modified long enough ago
#: that no later write could share their recorded timestamp. Source files in a grading run are hours
#: old and hit every time; a file being edited is read again.
_BUILD_PATH_MEMO: dict = {}

#: How long a file must have been unmodified before its (size, mtime) is allowed to stand in for its
#: content. Not a property of any target -- a bound on the coarsest timestamp a filesystem this repo
#: runs on can record (1 s on HFS+ and some NFS mounts), doubled, so two writes cannot share a
#: timestamp once it has passed. Lowering it trades correctness for a cache hit.
_QUIESCENT_NS = 2_000_000_000
...
def _build_path_digest(files) -> str:
    ...
    hit = _BUILD_PATH_MEMO.get(memo_key)
    if hit is not None and newest + _QUIESCENT_NS <= time.time_ns():
        return hit
    from merlin.common.provenance import source_digest

    digest = source_digest([str(p) for p in files])
    _BUILD_PATH_MEMO[memo_key] = digest
    return digest
