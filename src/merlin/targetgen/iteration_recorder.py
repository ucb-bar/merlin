...
def _git(args: list[str], cwd: str | Path | None = None) -> str:
    try:
        p = subprocess.run(["git", *args], cwd=str(cwd) if cwd else None, capture_output=True, text=True, timeout=60)
...
def first_failure(results: list[dict]) -> dict | None:
    ...
    plane_order = [
        ...
        "command_buffer_simulate",
        "numeric_golden",
        "target_to_instruction_trace",
        "trace_check",
        "target_to_llvm",
        "llvm_compile",
        "runtime_link",
        ...
        "vcs",
        "firesim",
        "program_did_not_halt",
        "declared_tier_unreachable",
        "oracle_unavailable",
        "runner_internal",
    ]
    ...
    fails = [(r["capsule"], r["failure"]) for r in results if r.get("status") not in ("pass",) and r.get("failure")]
    if not fails:
        return None
...
def record_iteration(
    ...
    *,
    ...
    if cost_time_toolcalls is not None:
        (d / "cost_time_toolcalls.yaml").write_text(yaml.safe_dump(cost_time_toolcalls, sort_keys=False))
    if notes:
    ...
    record = {
        ...
        "numeric_diff": numeric_diff or {},
        "instruction_trace_diff": trace_diff or {},
        "profile": profile or {},
        "cost_time_toolcalls": cost_time_toolcalls,
...
def freeze(
    ...
    *,
    results: list[dict],
    repo: str | Path = ".",
    toolchain_shas: dict | None = None,
    hidden_eval_version: str = "v0",
) -> Path:
