...
def necessary(capsule: Mapping[str, Any], required: "set[tuple] | None") -> Verdict:
    ...
    # NOT "UNNECESSARY" -- "not necessary ON THE ONE AXIS THIS CAN SEE". A cell is one of eleven things
    # the requirement asks for; the other ten (composition shape, conv window, epilogue stage, memory
    # regime, aspect ratio, host lane, host-only lane, adjacency chain, stage group, carried state) are
    # invisible here, and a capsule can be the ONLY witness of one of them while witnessing no required
    # cell at all. Measured on gemmini: reading cells alone condemns 48 of 189 capsules, among them
    # `GQ0..GQ3`, the quantized-convolution capsules that are this target's only conv evidence and the
    # only capsules covering four of its ten required conv windows.
    #
    # So this returns UNKNOWN rather than NO. The distinction is the difference between a report and a
    # pruning licence, and the docstring above already promises the weaker one -- a `NO` that a caller
    # reads as "safe to delete" is how a corpus loses the evidence it was built for.
    return Verdict(
        UNKNOWN,
        f"witnesses {cell[0]}/{'+'.join(cell[1])}, which is not a required CELL -- but cells are one "
        f"of eleven requirement axes and the other ten are not visible from a capsule's family and "
        f"dtype alone, so whether this capsule is necessary cannot be decided here",
    )
