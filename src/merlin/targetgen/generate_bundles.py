...
def _shared_allow(te: TargetExperiment, variant: str) -> list[dict]:
    ...
    # The broad contract directory also contains the historical corpus and its
    # goldens. Grant only the public, target-independent ABI documents and
    # schemas; a reviewed release supplies its own corpus via the descriptor.
    public_contract = (
        "VERSION",
    ...
    out = [{"path": f"merlin/contract/{name}", "mode": "ro", "note": "frozen public ABI"} for name in public_contract]
    ...
    try:
        ...
        pass  # Generated prompts do not need a grant to a nonexistent authored task directory.
    else:
        if not task.is_dir():
            raise ValueError("authored task resource must be a directory")
        out.append({"path": te.experiment_resource("task") + "/", "mode": "ro"})
    out.append({"path": "third_party/llvm-install/", "mode": "ro", "note": "LLVM/MLIR 23 toolchain"})
    if te.hwbringup_set:
        out.append(
            {
    ...
    out.append(
        {
            "path": te.experiment_resource("scripts/agent_selfcheck.py"),
...
def _shared_deny(te: TargetExperiment, variant: str) -> list[dict]:
    ...
    out += [
        {"path": te.experiment_resource("input_bundles/grader_private_v0") + "/", "reason": "grader-private"},
        {"path": te.experiment_resource("runs") + "/", "reason": "prior submissions"},
    ]
...
def _python_source_selection(root: Path | None) -> Path | None:
    if root is None:
        return None
    root = Path(root)
    if not root.is_absolute() or root.is_symlink() or root.resolve() != root or not root.is_dir():
        raise ValueError("python_source_root must be an absolute canonical directory")
    return root


def _select_python_grants(manifest: dict[str, Any], root: Path) -> None:
    """Translate only historical Python grants for an explicitly selected new release."""
    prefix = "merlin/python/merlin/"
    package = root / "merlin"
    for role in ("allowed", "denied", "host_inputs"):
        for entry in manifest.get(role, ()):
            original = entry.get("path")
            if not isinstance(original, str) or not original.startswith(prefix):
                continue
            suffix = Path(original[len(prefix) :])
            if suffix.is_absolute() or ".." in suffix.parts:
                raise ValueError("legacy Python grant escapes the selected package")
            selected = (package / suffix).resolve()
            if not selected.is_relative_to(package):
                raise ValueError("legacy Python grant escapes the selected package")
            # Absence is meaningful for deny surfaces; do not drop missing paths.
...
def generate_bundles(
    ...
    *,
    ...
    for arm in wanted:
        ...
        out[bid]["host_inputs"].extend(
            {"path": path, "note": "operator-only frozen input; never an agent grant"} for path in host_inputs
        )
...
def _materialize_prompt_and_grants(
    ...
    *,
    ...
    if cap is not None:
        ...
        allowed = {
            e["path"] for e in (manifest.get("allowed") or []) if isinstance(e, dict) and isinstance(e.get("path"), str)
        }
        granted = TR.selected_bundle_tool_paths(
            allowed, manifest.get("tools") or (), python_source_root=python_source_root
        )
...
def materialize_bundles(
    ...
    *,
    ...
    The caller selects ``dest`` explicitly. The CLI defaults to a fresh artifact product;
    reviewed release preparation supplies its own staging directory. Generating bundles
    does not approve them or change the inputs of an existing experiment."""
    from pathlib import Path
...
def _main(argv: list[str] | None = None) -> int:
    ...
    ``--dest`` defaults to a fresh versioned artifact product, never beside the descriptor."""
    import argparse
    from pathlib import Path
    ...
    ap.add_argument(
        "--dest",
        default=None,
        help="explicit output input_bundles dir (default: fresh versioned capsule-bench artifact)",
    )
    ap.add_argument(
    ...
    if a.dest:
        ...
        if not te.target.strip() or te.target in (".", "..") or any(c in te.target for c in ("/", "\\", "\x00")):
            ap.error("target must be a single safe path component for artifact output")
        product = new_product(
            "capsule-bench",
            version=1,
            target=te.target,
            sources=[{"path": str(te.path.absolute()), "sha256": te.descriptor_sha256}],
            notes="Generated experiment bundles; not an approved release or compiler certification.",
            update_latest=False,
        )
        dest = product.path / "input_bundles"
    ...
    if product is not None:
        for path in written:
            product.add_artifact(path.relative_to(product.path).as_posix())
        product.write_manifest()
    print(f"materialized {len(written)} bundle files under {dest} (target={te.target}):")
    for p in written:
