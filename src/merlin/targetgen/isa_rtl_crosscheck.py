...
def contracts_dir(target: str) -> Path | None:
    """The explicitly selected bring-up set, or legacy contracts when none is declared.

    A missing declared set never falls back to stale sibling hardware evidence.
    """
    import yaml
    ...
    from . import corpora
    from .target_experiment import declared_contracts_root, descriptor_resources_root

    descriptor = corpora.descriptor_path(target)
    try:
        document = yaml.safe_load(descriptor.read_text()) or {}
    except (OSError, yaml.YAMLError):
        document = {}
    hardware = document.get("hardware_spec", {}) if isinstance(document, dict) else {}
    if isinstance(hardware, dict) and "hwbringup_set" in hardware:
        selected = hardware["hwbringup_set"]
        if not isinstance(selected, str) or not selected.strip():
            raise ValueError("hardware_spec.hwbringup_set must be a non-empty path")
        # Bundle paths prefer the repository root; legacy experiments/... shorthand
        # can still name a retained merlin/ resource. Do not search unrelated roots.
        base = repo_root() / selected
        if base.exists() or base.is_symlink():
            return base if base.is_dir() else None
        legacy = repo_root() / "merlin" / selected
        return legacy if legacy.is_dir() else None
    base = declared_contracts_root(document) if isinstance(document, dict) else None
    if base is None:
        base = descriptor_resources_root(descriptor) / "contracts"
    return base if base.is_dir() else None
...
def compare_opcode_constants(model, consts: dict[str, int]) -> list[Finding]:
    ...
    for name, want in sorted(declared.items()):
        ...
        if got is None:
            findings.append(
                Finding(name, RTL_OPCODE_CONSTANTS, NOT_COVERED, "the RTL names no opcode constant for this class")
            )
        elif got == want:
            ...
            findings.append(
                Finding(
                    ...
                    + (
                        ...
                        "which would alias this opcode onto another)"
                        if got & ((1 << max(want.bit_length(), 1)) - 1) == want
                        else ""
                    ),
                    declared=str(want),
                    evidence=str(got),
        ...
        for p in sorted(root.rglob("*.scala")):
            ...
            for raw in text.splitlines():
                ...
                if (
                    ...
                    and (not line.startswith("def ") or line[len("def ") :].partition("=")[0].strip() not in named)
                ):
                    unnamed += 1
...
def _projections(model) -> dict[str, Any]:
    ...
    for f in ("funct7", "funct3", "funct2"):
        ...
        if obits:
            cands[f"{f}<<opcode"] = (
                lambda f: (
                    lambda e: None if e.get("opcode") is None else ((int(e.get(f) or 0) << obits) | int(e["opcode"]))
                )
            )(f)
    return cands
...
def green_card_paths(target: str) -> list[Path]:
    ...
    return sorted(
        {
            p
            for pattern in ("isa_include/*.md", "*/isa_include/*.md")
            for p in base.glob(pattern)
            if "green_card" in p.name
        }
    )
...
class Report:
    ...
    def by_verdict(self, verdict: str, *, usable_only: bool = True) -> list[Finding]:
        ...
        return [f for f in self.findings if f.verdict == verdict and (kinds is None or f.source in kinds)]
    ...
    def outliers(self) -> dict[str, dict]:
        ...
        for f in self.findings:
            if f.mnemonic in out and f.verdict == AGREE:
                (
                    out[f.mnemonic]["hardware_for"] if f.source in HARDWARE_DERIVED else out[f.mnemonic]["authored_for"]
                ).append(f.source)
        return out
...
def compare_bitpats(model, table: dict[str, BitPattern]) -> list[Finding]:
    ...
    for mnem, ent in sorted((model.by_mnemonic or {}).items()):
        ...
        if pat is None:
            ...
            if fam is None:
                findings.append(
                    Finding(mnem, RTL_BITPAT, NOT_COVERED, "no decode pattern in the RTL names this instruction")
                )
                continue
        ...
        if not common:
            findings.append(
                Finding(mnem, RTL_BITPAT, NOT_COVERED, "the model and the pattern pin no bit in common", matched=how)
            )
            ...
            findings.append(
                Finding(
                    ...
                    evidence=f"0x{_corrected(fv, pat):08x}",
                    matched=how,
                )
            )
        else:
...
def compare_decode_table(model, dt: dict[str, Any]) -> tuple[list[Finding], str]:
    ...
    if not fn or cov < MIN_PROJECTION_COVERAGE:
        return [], (
            f"no projection of the model's own fields explains the decode table "
        ...
        if code is None:
            findings.append(
                Finding(
                    ...
                    RTL_DECODE_TABLE,
                    NOT_COVERED,
                    f"the model publishes no code for this instruction under the fitted projection ({name})",
                )
            )
        elif int(code) in legal:
            findings.append(
                Finding(mnem, RTL_DECODE_TABLE, AGREE, declared=str(int(code)), evidence=str(int(code)), matched=name)
            )
            ...
            findings.append(
                Finding(
                    ...
                    declared=str(int(code)),
                    evidence=",".join(str(c) for c in near[:8]),
                    matched=name,
                )
            )
        else:
...
def compare_headers_to_decoder(dt: dict[str, Any]) -> tuple[list[Finding], list[str]]:
    ...
    if only:
        ...
        notes.append(
            f"{RTL_VS_SHIPPED_HEADERS}: {len(only)} code(s) the hardware decodes that the shipped "
...
def compare_green_card(model, card: dict[str, tuple[str, str, str]]) -> list[Finding]:
    ...
    for mnem, ent in sorted((model.by_mnemonic or {}).items()):
        ...
        findings.append(
            Finding(mnem, SHIPPED_GREEN_CARD, v, declared=f"{int(declared):07b}", evidence=f"{card_f7:07b}")
        )
    return findings
...
def crosscheck(target: str, *, rtl_root: str | Path | None = None) -> Report:
    ...
    rep.sources.append(
        Source(
            ...
            len(bitpats),
            note=f"{n_files} scala files read" if roots else "no RTL source tree resolved for this target",
        )
    )
    if unnamed:
        rep.notes.append(
    ...
    rep.sources.append(
        Source(
            ...
            ", ".join(str(r) for r in roots),
            len(consts),
            note="named binary-string opcode constants in the RTL"
            if consts
            else "no named opcode constants found in this target's RTL",
        )
    )
    ...
    rep.sources.append(
        Source(
            RTL_DECODE_TABLE,
            bool(dt.get("legal_funct")),
            str(dt.get("hw_source") or ""),
            len(dt.get("legal_funct") or ()),
            circular=(circular_kind == RTL_DECODE_TABLE),
            note=(
                "the model is BUILT from this table; agreement here is a tautology"
                if circular_kind == RTL_DECODE_TABLE
                else str(dt.get("method") or "")
            ),
        )
    )
    ...
    rep.sources.append(
        Source(
            ...
            note=(
                "the extractor's own decoder-vs-headers comparison; the model is not a "
                "party to it, so it stays usable even when the table itself is circular"
                if has_hdr
                else "this target's facts record no decoder-vs-header comparison"
            ),
        )
    )
    if has_hdr:
    ...
    if not model.by_mnemonic:
        ...
        if bitpats:
            ...
            rep.notes.append(
                "no opcode-level comparison was possible either: this target's RTL states "
...
def _opcode_level_notes(model, bitpats: dict[str, BitPattern]) -> list[str]:
    ...
    if "opcode" not in layout or not bitpats or not model.opcode_table:
        return [
            "no opcode-level comparison is available for this model "
    ...
    return [
        f"opcode field bits [{hi}:{lo}] (derived from the model's own layout): "
...
def erratum(rep: Report) -> dict[str, Any]:
    ...
    return {
        ...
        "coverage": {
            "model_mnemonics": rep.model_mnemonics,
...
def undeclared_disagreements(rep: Report, errata: dict[str, dict] | None = None) -> dict[str, dict]:
    ...
    for mnem, row in rep.outliers().items():
        ...
        if str(e.get("declared", "")) != str(row.get("declared", "")) or (
            ...
            row = dict(row)
            row["note"] = (
                f"the registry records declared={e.get('declared')!r} hardware={e.get('hardware')!r}, "
