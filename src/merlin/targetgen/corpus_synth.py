...
#: The multiplier a synthesized scale stage declares. A property of the synthetic workload, not of
#: any target: under one sixteenth every accumulator that is an odd multiple of eight is an exact
#: tie, so the rounding mode is observable, and a signed accumulator saturates at both ends.
SYNTH_ACC_SCALE = 0.0625


def _signed_stimulus_range() -> tuple[int, int]:
    from merlin.runtime.commandbuffer import SIGNED_STIMULUS_RANGE

    return SIGNED_STIMULUS_RANGE
...
#: The per-operand multipliers a SYNTHESIZED residual add declares. A property of the synthetic
#: workload, exactly like :data:`SYNTH_ACC_SCALE`: unity is the only multiplier pair under which the
#: capsule's golden is the plain integer sum of its two operands, so the capsule tests the ADD rather
#: than a scale the synthesizer chose.
SYNTH_RESIDUAL_SCALE = 1.0

#: ...and how far a target's result may lie from that reference, in output steps. ZERO: with unity
#: multipliers there is no rounding for an implementation to disagree about, so any nonzero bound would
#: admit a wrong answer. `build_residual_add` gives this no default on purpose -- a bound chosen after
#: seeing a result is not a gate -- which is why the synthesizer has to state one.
SYNTH_RESIDUAL_BOUND_LSB = 0


def declare_residual_add_params(entry: dict) -> dict:
    """Give a synthesized two-operand add the arithmetic it must declare, in place. Inert otherwise.

    The sibling of :func:`declare_pool_window`, and it exists for the same reason: an op the chooser
    may pick must be one the synthesizer can produce a COMPLETE entry for. ``build_residual_add``
    requires ``lhs_scale``/``rhs_scale``/``bound_lsb`` and refuses to default any of them, so an entry
    that named the op without them raised inside the builder -- turning a chosen op into a generation
    failure rather than a capsule.

    Latent until now and no longer: ``residual_add`` has been choosable for the elementwise family all
    along and lost the cost ranking to ``bias_add`` on every target measured, so nothing ever hit it.
    A target whose dtype cannot grade ``bias_add`` would have.
    """
    from merlin.targetgen.corpus_spec import BUILDERS, build_residual_add

    if BUILDERS.get(str(entry.get("op", ""))) is not build_residual_add:
        return entry
...
def cap_to_affordable(entry: dict, spec_doc: dict, *, extends: str = "") -> str | None:
    ...
    """
    # The ceiling comes from `conformance._cert_affordability`, which prices PER (target, engine) via
    # `cert_affordability` and returns the slowest engine's figure -- the one that binds, because the
    # cap has to be payable on whichever engine the run picks. It is read here, never recomputed: a
    # second inversion of the cost law is a second place for the extrapolation clamp to be forgotten,
    # which is how the producer's own inline `(budget - intercept) / per_element` came to bypass it.
    ...
    # NAME THE ENGINE THE CEILING BELONGS TO. A certification second is a property of the simulator,
    # not of the capsule: the same ELF answered on two elaborated-RTL engines differs by more than an
    # order of magnitude, and this reason string is written into a TRACKED capsule where it long
    # outlives the run that produced it. Without the engine, a reader comparing two capsules capped
...
def pass_requirements_for(entry: dict, spec_doc: dict) -> list[str]:
    ...
    Derived, never guessed:
      ...
      * ``block-movement`` -- an operand's CONTIGUOUS axis spans more than one tile, so a transfer of
        that operand carries several tiles' worth of adjacent bytes and the emitter has to decide how
        wide to make it.
    """
    ...
    # for the weight and result rows. M only adds ROWS -- more transfers of the same width -- so an
    # entry that exceeds the edge on M alone demands tiling and nothing about movement width. K or N
    # past the edge is the case where one transfer COULD carry the next tile's bytes too, which is the
    # decision `block-movement` exists to demand. Derived from the entry's own extents, exactly like
    # `tile-schedule`; whether the target's DMA admits that width is the target's fact, checked
    # against the emitted stream by `trace_check.movement_findings`, not assumed here.
...
def synthesize(spec_doc: dict, *, workload_spec: dict | None = None, budget: int | None = None) -> dict:
    ...
    # model needs bias, activation and requantization on the SAME contraction. Each required
    # signature gets one member carrying the whole combination; the signature is the compiler's own
    # statement of a group it will try to put on a unit, so a member here and a group in a model are
    # asked for in the same words.
    #
    # Written on the matmul carrier even for a convolution's signature: what this axis tests is that
    # the STAGES compose on one readout, and the window axis below already owns convolution geometry.
    ...
    for _group in (spec_doc.get("groups") or {}).get("required") or ():
        ...
        if len(stages) < 2:
            continue  # a single stage is the epilogue axis's member already
        unknown = [x for x in stages if x not in _stage_names]
        if unknown:
            unexpressable.append(
                f"group axis: {_group.get('signature')} carries {unknown}, which no capsule builder can state"
            )
            continue
        dtype = str(_group.get("dtype") or "") or (kept[0] if kept else narrowest_admitted(admitted_dtypes))
        if not dtype:
        ...
        entry = {
            ...
            "out": "Y0",
            "epilogue": stages,
            # A scale stage with no multiplier is read as 1.0 by every engine, which makes the
            # requantization a bare saturating cast that a backend ignoring the scale passes.
            **({"acc_scale": SYNTH_ACC_SCALE} if "acc_scale" in stages else {}),
            ...
            "label": "public",
            # SIGNED. Every stage in a combination treats a negative accumulator differently from a
            # positive one, and on the non-negative default none of that can be observed.
            "stimulus_range": list(_signed_stimulus_range()),
            "generalization": {"generalization_axis": "groups"},
            **extents_for("aligned", probes, quantum=_quanta.get(dtype)),
        }
        declare_pool_window(entry)
        declare_residual_add_params(entry)
    ...
    # second. The hand-authored two-command capsule runs them in the other order, on a stimulus that
    # cannot go negative, so it cannot show a stage left switched on: the second command WANTS it.
    for _carried in (spec_doc.get("carried_state") or {}).get("required") or ():
        stage = str(_carried.get("stage") or "")
        dtype = kept[0] if kept else narrowest_admitted(admitted_dtypes)
        if not stage or not dtype:
            unexpressable.append(f"carried-state axis: no admitted dtype to build a {stage!r} program at")
            continue
        extents = extents_for("aligned", probes, quantum=_quanta.get(dtype))
        rows = {key: value for key, value in extents.items() if key.startswith("M")}
        entry = {
            "cat": "layers",
            "kind": "layer",
            "name": f"{SYNTH_PREFIX}_carried_{stage}_{dtype}",
            "op": "resident_reuse",
            "operand_dtype": dtype,
            "weight": "W",
            "matmuls": [
                {"lhs": "A0", "out": "Y0", "epilogue": [stage], **rows},
                {"lhs": "A1", "out": "Y1", "epilogue": [], **rows},
            ],
            "source_role": SOURCE_ROLE,
            "source_reference": (
            ...
            "label": "public",
            # SIGNED, or the second result has nothing the leaked stage would change.
            "stimulus_range": list(_signed_stimulus_range()),
            "generalization": {"generalization_axis": "carried_state"},
            **{key: value for key, value in extents.items() if not key.startswith("M")},
        }
        entry["pass_requirements"] = pass_requirements_for(entry, spec_doc)
        entries.append(entry)
    ...
    # and saturates the other way, a pool's padding identity is wrong for it), and the default
    # stimulus is non-negative: on it none of that is observable, and the member cannot fail.
    ...
    for entry in entries:
        ...
        if any("acc_scale" in (item.get("epilogue") or ()) for item in staged):
            entry.setdefault("acc_scale", SYNTH_ACC_SCALE)
