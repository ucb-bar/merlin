...
def _descriptor_for(target: str) -> Path | None:
    ...
    if env:
        ...
        try:
            ...
            pass
    from .corpora import standard_descriptor_path

    # The CONVENTION path, deliberately not `descriptor_path`: the override was already considered
    # above and rejected because it names a different target, so re-applying it here would hand back
    # that other target's descriptor.
    std = standard_descriptor_path(target)
