...
def task_instruction_binding(
    *, source_text, lowered_text, command_buffer_text, verified_plan, target_facts, host_policy_sha256
):
    ...
    return {
        ...
        "lowered_sha256": _sha(lowered_text),
...
#: The ISA constant naming the custom major opcode, as the derived constants spell it. Named so the
#: lookup is a stated convention rather than an anonymous key; the VALUE is never assumed, and a
#: target whose constants omit it reports the opcode as ``None`` rather than substituting one.
CUSTOM_OPCODE_CONSTANT = "CUSTOM_OPCODE"


def declared_instruction_set(target_facts):
    """The instructions the TARGET DECLARES, from the facts the caller already derived.

    WHY THIS IS SEPARATE FROM THE BINDING. :func:`task_instruction_binding` reduces ``target_facts``
    to ``target_facts_sha256`` -- enough to prove two summaries were computed against the same
    machine, and useless to a reader who wants to know what that machine offers. So the declared set
    itself was derived on every analysis and thrown away, and the only view of the ISA that survived
    was a count of the instructions the program had already emitted. A compiler cannot reach for a
    capability it is never told about.

    Nothing here names an instruction, an opcode or a target: every value comes from
    ``target_facts`` (``merlin.perf.task_instruction_evidence.target_instruction_facts``), and facts
    that could not be derived are reported as an explicit UNKNOWN rather than an empty set -- an
    empty declared set would make every emitted instruction undeclared and nothing unused.
    """
    facts = target_facts if isinstance(target_facts, Mapping) else {}
    names = facts.get("instruction_names")
    names = names if isinstance(names, Mapping) else {}
    roles = facts.get("roles_by_selector")
    roles = roles if isinstance(roles, Mapping) else {}
    isa = facts.get("isa")
    isa = isa if isinstance(isa, Mapping) else {}
    if not names:
        return {
            "schema": "declared_instruction_set_v1",
            "status": "UNKNOWN",
            "reason": (
                "this target's derived facts carry no instruction name table, so the DECLARED "
                "instruction set was NOT derived. An empty declared set would report every "
                "instruction as used and none as unreached, which is a clean answer produced by "
                "not looking."
            ),
    ...
    def order(value):
        # By SELECTOR VALUE where the table spells one, so the listing reads in encoding order
        # rather than in the lexical order of its own spelling ("10" before "2").
        try:
            return (0, int(str(value).strip(), 0), "")
        except (TypeError, ValueError):
            return (1, 0, str(value))

    instructions = []
    for selector in sorted(names, key=order):
        declared_roles = roles.get(str(selector))
        instructions.append(
            {
                "funct": selector,
                "name": str(names[selector]),
                # A role the target's endpoints never claimed is UNKNOWN, not "no roles": the two
                # look identical in an empty list and only one of them is a fact.
                "roles": sorted(declared_roles) if isinstance(declared_roles, list) else None,
            }
        )
    return {
        "schema": "declared_instruction_set_v1",
        "status": "derived",
        "target": facts.get("target"),
        "custom_opcode": isa.get(CUSTOM_OPCODE_CONSTANT),
        "declared_count": len(instructions),
        "instructions": instructions,
        "role_sources": list(facts.get("role_sources") or ()),
        "licence": (
            "what the machine OFFERS, derived from its own facts. An instruction listed here that "
            "the program never emits is an OPPORTUNITY, not a defect; it is the only signal that "
            "names a capability the compiler does not reach for."
        ),
    }


def _declared_instruction_use(declared, rows):
    """Declared-versus-emitted instruction use over the decoded rows this summary already walked.

    The emitted side is the decoder's own ``funct`` per instruction -- PARSED to an int and compared
    to the declared selector as data, never string-matched -- so a table spelling its selectors as
    ints and a decoder spelling them as ints agree without any spelling being assumed.
    """
    if declared.get("status") != "derived":
        return {
            "schema": "declared_instruction_use_v1",
            "status": "UNKNOWN",
    ...
    for row in rows if isinstance(rows, list) else []:
        selector = code(row.get("funct")) if isinstance(row, Mapping) else None
        if selector is None:
            undecoded += 1
            continue
        emitted[selector] += 1
    ...
    for entry in declared["instructions"]:
        selector = code(entry["funct"])
        if selector is None:
            unparsed.append(entry["name"])
            continue
        by_code[selector] = entry
    used = {entry["name"]: emitted[selector] for selector, entry in sorted(by_code.items()) if selector in emitted}
    return {
        "schema": "declared_instruction_use_v1",
        "status": "measured",
        "declared_count": len(by_code),
        "used_count": len(used),
        "used": used,
        "unused": [
            {"funct": selector, "name": entry["name"], "roles": entry["roles"]}
            for selector, entry in sorted(by_code.items())
            if selector not in emitted
        ],
        # An emitted selector the facts do not declare is a provenance problem, not an optimization
        # one: the program is using an instruction nobody has vouched for.
        "undeclared_emitted": sorted(selector for selector in emitted if selector not in by_code),
        "instructions_without_parsed_selector": undecoded,
        "declared_without_parsed_selector": unparsed,
        "licence": (
            "an unused declared instruction is an OPPORTUNITY, not a defect -- the program may be "
            "correct without it. It says the compiler never reaches for a capability the hardware "
            "offers, which is where a structural lever hides."
        ),
    }


def summarize_task_instructions(
    *,
    source_text,
    lowered_text,
    command_buffer_text,
    ...
    host_policy_sha256,
    decode_module,
    short_execution_admission=None,
):
    ...
    """
    binding = task_instruction_binding(
    ...
    if short_execution_admission is not None:
        ...
        expected = {
            ...
            "source_sha256": binding["source_sha256"],
            "lowered_sha256": binding["lowered_sha256"],
            "command_buffer_sha256": digest(command_buffer),
            "candidate_sha256": binding["compiler_sha256"],
            "declared_proof_sha256": digest(verified_plan),
            "declared_plan_modified": False,
            "declared_plan_verified": False,
            "target_route_verified": False,
            "global_cost_calibration": False,
        }
        ...
        if (
            ...
            or any(admission.get(k) != v for k, v in expected.items())
            or closure.get("status") != "proved"
            or closure.get("source_sha256") != binding["source_sha256"]
            or closure.get("command_buffer_sha256") != digest(command_buffer)
            or not isinstance(implicit_sources, list)
            or not implicit_sources
    ...
    if (
        (verified_plan.get("status") != "verified" and admission_digest is None)
    ...
    for task in plan.get("tasks", []):
        ...
        tasks[index] = {
            "task_index": index,
            "source_op_indices": sources,
            "declared_task_kind": task.get("kind"),
            "instruction_indices": [],
            "static_operation_counts": Counter(),
    ...
    for position, (op, instruction) in enumerate(zip(assembly, rows, strict=True)):
        ...
        if not isinstance(declared_roles, list) or any(
            not isinstance(role, str) or not role for role in declared_roles
        ):
    ...
    for task in tasks.values():
        ...
        task["owned_instruction_payload_sha256"] = digest(
            [{key: value for key, value in rows[i].items() if key != "index"} for i in task["instruction_indices"]]
        )
    ...
    declared = declared_instruction_set(target_facts)
    return {
        ...
        "binding": binding,
        # WHAT THE MACHINE OFFERS, beside what this program reached for. Both are derived from the
        # same ``target_facts`` the binding hashes; before this, only the hash survived and the
        # program's own emission was the sole view of the ISA any reader downstream ever got.
        "declared_instruction_set": declared,
        "declared_instruction_use": _declared_instruction_use(declared, rows),
        "decoded_instructions_sha256": digest(rows),
        ...
        "limitations": [
            "absence of decoded fields does not distinguish fieldless instructions from unmodeled descriptors",
