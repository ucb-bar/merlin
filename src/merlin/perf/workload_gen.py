...
from .workload_numerics import accumulate_reference, encode_operand_bytes
...
__all__ = [
    ...
    "machine_facts",
    "candidate_ops",
    "instruction_memory_words",
    "scalar_register_count",
    "probe_control_flow",
    "probe_settle",
    "control_flow_probe_kernel",
    "plan_matmul",
    "alias_report",
    "accumulate_reference",
    "encode_operand_bytes",
]
...
def tile_geometry(target: str) -> TileGeometry:
    ...
    return TileGeometry(int(mesh["rows"]), int(mesh["cols"]), str(mesh.get("source") or "rtl_facts.arrays"))
...
def datapath_formats(target: str, *, accum_dtype: str | None = None) -> tuple[str, str]:
    ...
    if accum_dtype is not None:
        for src, acc in combos:
            if acc == accum_dtype:
                return src, acc
        raise WorkloadError(f"{target!r}: no declared datapath accumulates in {accum_dtype!r}; declared: {combos}")
    return combos[0]
...
def _accumulate_from_rtl_datapaths(target: str) -> list[tuple[str, str]]:
    ...
    try:
        ...
        return []
    by_name = {
        str(d.get("name")): str(d.get("dtype") or "") for d in body.get("datapaths") or [] if isinstance(d, dict)
    }
...
def scalar_register_count(target: str, isa, ops: "KernelOps") -> tuple[int, str]:
    ...
    * what the target DECLARES it has -- the scalar-register row of the architectural-parameters table
    ...
    if bits:
        bounds.append(
            (1 << len(bits), f"the {len(bits)}-bit destination field of {ops.add_imm} in the derived encoding")
        )
    for doc in _shipped_spec_docs(target):
        declared = _spec_table_int(doc.read_text(encoding="utf-8", errors="replace"), "scalar", "registers")
        if declared:
...
class _ScalarFile:
    ...
    def __init__(self, target: str, isa, ops: "KernelOps", roles: Sequence[str], *, zero_index: int = 0):
...
def machine_facts(target: str, *, accum_dtype: str | None = None) -> MachineFacts:
    ...
    return MachineFacts(
        ...
        word_bytes=max(1, isa.inst_width // 8),
...
class KernelOps:
    ...
    def validate(self, isa) -> None:
        missing = [f"{role}={name}" for role, name in self.as_dict().items() if isa.resolve(name) is None]
        if missing:
            raise WorkloadError(
    ...
    *,
    ...
    for scale in imm_scales:
        for slots in delay_slot_candidates:
            src = control_flow_probe_kernel(
                facts, ops, trips=trips, body_cycles=body_cycles, imm_scale=scale, delay_slots=slots, entry=entry
            )
            try:
    ...
    raise WorkloadError(
        f"{facts.target!r}: no candidate branch encoding closed a loop on the device. Tried: " + "; ".join(tried)
    )
...
def probe_settle(
    ...
    ops: KernelOps,
    cf: ControlFlow,
    run_matmul: Callable[["MatmulPlan", int], Any],
    *,
    operands,
...
def alias_report(placements: Sequence[Placement], window: int | None) -> AliasReport:
    ...
    for p in placements:
        ...
        hi = lo + p.nbytes
        spans.append(AliasSpan(p.name, p.base, p.nbytes, lo, hi, wraps=bool(window and hi > window)))
        total += p.nbytes
    ...
    if window is None:
        return AliasReport(
            ...
            total,
            False,
            "the runner publishes no DRAM window size, so the footprint COULD NOT BE CHECKED; this is not a pass",
        )
    if wrapped:
        return AliasReport(
            window,
    ...
    return AliasReport(
        ...
        total,
        True,
        f"all {len(spans)} tensor spans ({total} B) fit disjointly inside the {window} B window; no address wraps",
    )
...
_SCALAR_ROLES = (
    "m_index",
    "n_index",
    "k_index",  # the three loop counters
    "dma_rd",
    "dma_rs1",
    "dma_len",  # the DMA endpoint registers
    "stage_addr",  # the tensor load/store base
    "a_tile",
    "w_tile",
    "c_tile",  # the three walking operand pointers
    "m_limit",
    "n_limit",
    "k_limit",  # the three loop bounds
    "a_row",
    "w_col",  # per-M and per-N restart points
    "tile_bytes",
    "a_row_stride",
    "w_k_stride",
    "c_stride",  # the four strides
...
class MatmulPlan:
    ...
    def command_buffer(self) -> dict:
        ...
        tensors = {}
        for p in self.placements:
            tensors[p.name] = {"role": p.role, "shape": list(p.shape), "dtype": p.dtype, "base": p.base}
        return {
            "abi_version": "0.1",
            "target": self.facts.target,
            "tensors": tensors,
            "commands": [{"opcode": "MATMUL", "operands": {"lhs": "A", "rhs": "W", "dst": "C"}}],
        }
    ...
    def unrolled_word_estimate(self) -> int:
        ...
        mt, kt, nt = self.tiles
        return (
            self.section_words.get("prologue", 0)
    ...
    def divergence(self, device_output) -> dict:
        ...
        return {
            ...
            "mismatching": int(np.count_nonzero(got != ref)),
...
def _place(facts: MachineFacts, m: int, k: int, n: int, *, origin: int, align: int) -> tuple[Placement, ...]:
...
def _pack_tiled(values, facts: MachineFacts, rows: int, cols: int, tile_rows: int, tile_cols: int) -> bytes:
    ...
    return encode_operand_bytes(np.concatenate([c.reshape(-1) for c in chunks]), facts.operand_dtype)


def _issue_tile_transfer(
    p: _Program, facts: MachineFacts, ops: KernelOps, x: _ScalarFile, *, vmem_byte: int, dram_reg: int, nbytes: int
) -> None:
...
def _inner_body(
    ...
    *,
    ...
    if hoist_transfers:
        return _inner_body_hoisted(
            p, facts, ops, x, settle, first_k=first_k, v_lhs=v_lhs, v_wt=v_wt, tile_bytes=tile_bytes
        )
...
def plan_matmul(
    ...
    *,
    ...
    align: int = 64,
    entry: str = "_start",
    subnormal_operand_flush: bool | None = None,
    hoist_transfers: bool = False,
    numeric_policy: dict | None = None,
) -> MatmulPlan:
    ...
    **The operand DRAM layout is TILE-MAJOR** -- each tile contiguous, tiles in row-major tile order --
    ...
    for role, value in (
        ...
        ("a_row_stride", kt * tile_bytes),
        ("w_k_stride", nt * tile_bytes),
        ("c_stride", banks * tile_bytes),
        ("w_origin", w_off),
        ("a_row", a_off),
        ("c_tile", c_off),
    ):
    ...
    _before_body = len(p)
    _inner_body(p, facts, ops, x, settle, first_k=True, v_lhs=v_lhs, v_wt=v_wt, hoist_transfers=hoist_transfers)
    _k_step_words = len(p) - _before_body
    if kt > 1:
        p.load_imm(x["k_index"], 1)
        p.label("k_loop")
        _inner_body(p, facts, ops, x, settle, first_k=False, v_lhs=v_lhs, v_wt=v_wt, hoist_transfers=hoist_transfers)
    ...
    for b in range(banks):
        ...
        _tensor_op(p, ops, ops.tile_store, {"vd": _M_ACC + b, "rs1": x["stage_addr"], "imm": 0}, settle.tensor)
    ...
    if A is not None and W is not None:
        ...
        if subnormal_operand_flush is None:
            ...
            if numeric_policy is None and not CS.dtype_info(facts.accum_dtype)[3]:
                raise WorkloadError(
                    "floating reference requires an explicit declared numeric_policy or subnormal_operand_flush"
                )
            subnormal_operand_flush = bool((numeric_policy or {}).get("subnormal_operand_flush", False))
        golden = accumulate_reference(
            A,
            W,
            accum_dtype=facts.accum_dtype,
    ...
    return MatmulPlan(
        ...
        tiles=(mt, kt, nt),
        section_words={
            "prologue": _prologue_words,
            "k_step": _k_step_words,
            "tile_epilogue": _tile_epilogue_words,
            "total": len(p),
        },
        operands=operands,
        golden=golden,
        instructions=p.items(),
    )
