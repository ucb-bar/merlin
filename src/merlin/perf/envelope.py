...
def array_issue_time(
    ...
    *,
    ...
    """Cycles a rectangular compute array is OCCUPIED issuing ``tiles``, or why that is UNKNOWN.

    The other resources in this module are priced as ``demand / peak``: a scalar amount over a rate.
    An array's issue count is not that shape. It is the loop the sequencer runs, and the partial
    block at each edge is charged asymmetrically -- a tile with fewer columns than the array wide
    wastes array columns for the whole tile, while a short row count costs only its own rows. Those
    edges do not survive a division, so the count comes from
    :func:`merlin.perf.mesh_occupancy.mesh_occupancy`, which runs the sequencer's own loop per tile,
    and the result is presented here in the same :class:`ResourceTime` vocabulary as every other
    resource so a composer can carry it without a second grammar.

    UNKNOWN, with the reason, in exactly two cases -- and neither is a number:

    * the array's own dimensions were not resolved from the target's facts. They are a PARAMETER
      here; this module derives no geometry, and there is no default array to fall back to;
    * some tile's geometry could not be read. An unreadable tile contributes nothing to the count,
      so accepting the partial total would report an occupancy that gets BETTER the less of the
      program the caller understood -- the flattering direction, which is the one nobody audits.
    """
    from .mesh_occupancy import mesh_occupancy

    own = [f for f in fixed if f.resource == resource]
    fixed_cycles = sum(f.cycles for f in own)
    prov = "; ".join(p for p in ([provenance] + [f.provenance for f in own]) if p)

    def _unknown(reason: str) -> ResourceTime:
        return ResourceTime(
            resource=resource,
    ...
    if unresolved:
        return _unknown(
            f"the array geometry for {resource!r} is UNKNOWN: {', '.join(unresolved)} was not "
            "resolved from the target's own facts, and this module holds no default array"
        )

    occupancy = mesh_occupancy(tiles, array_rows=int(array_rows), array_cols=int(array_cols))
    if not occupancy["complete"]:
        return _unknown(
            f"{occupancy['tiles_unreadable']} of "
            f"{occupancy['tiles_unreadable'] + occupancy['tiles_read']} tile descriptors could not "
            "be read, so the issue count is a lower bound on an unknown total rather than a time"
        )
    return ResourceTime(
        resource=resource,
        kind=ResourceKind.COMPUTE,
        cycles=float(occupancy["issue_cycles"]) + fixed_cycles,
        unit="issue_cycles",
        basis=Basis.MOVED,
        fixed_cycles=fixed_cycles,
        evidence_kind="structural_bound",
