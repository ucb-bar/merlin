...
def _combine(a, b, coefficient=1):
    ...
    return Affine(
        a.width,
        (a.constant + coefficient * b.constant) % modulus,
        tuple(sorted((atom, amount) for atom, amount in terms.items() if amount)),
        a.poison | b.poison,
    )
...
def _scale(value, amount, poison=frozenset()):
    ...
    return Affine(
        value.width,
        value.constant * amount % modulus,
        tuple(
            (atom, coefficient * amount % modulus)
            for atom, coefficient in value.terms
            if coefficient * amount % modulus
        ),
        value.poison | poison,
    )
...
def _attributes(op):
    return (
        tuple(sorted((key, str(value)) for key, value in op.properties.items())),
...
def _normal_form(module, *, deadline, max_operations, defined_integer_domain=False, pruned=frozenset()):
    ...
    def normalize(op):
        ...
        if op.name == "llvm.mlir.constant":
            ...
            if (
                isinstance(value, IntegerAttr)
                and set(op.properties) == {"value"}
                and not (set(op.attributes) - {"merlin.global_task"})
                and not op.operands
            ):
        ...
        if not a.terms and not b.terms and b.constant and (defined_integer_domain or (not a.poison and not b.poison)):
    ...
    def total_pure(op):
        if (
            op.name not in {"llvm.add", "llvm.sub", "llvm.mul", "llvm.udiv", "llvm.urem"}
            or len(op.results) != 1
            or _width(op.results[0]) is None
            or op.regions
            or op.successors
            or len(op.operands) != 2
            or not safe_arithmetic(op)
        ):
            return False
        if op.name in {"llvm.udiv", "llvm.urem"}:
            divisor = operand(op.operands[1])
            return (
                isinstance(divisor, Affine)
                and not divisor.terms
                and divisor.constant != 0
    ...
    def operation(op, identity):
        ...
        semantic_nodes.append(
            (
                op,
                tuple(identity + f"r{index}" for index in range(len(op.results))),
                set().union(*(references(value) for value in operands)),
                total_pure(op),
            )
        )
        regions = []
        ...
        return (
            ...
            operands,
            tuple(block_ids[block] for block in op.successors),
            regions,
        )
    ...
    return (
        result,
        {
            ...
            "normalized_operations": normalized_counts,
            "pruned_total_integer_operations": len(pruned),
        },
        dead,
    )


def compare_integer_address_identity(
    before_module, after_module, *, timeout_s=30.0, max_operations=1000000, defined_integer_domain=False
) -> dict:
    ...
    """
    result = {
        "schema": "integer_address_identity_v1",
        "status": "UNKNOWN",
        "proof_scope": "ordered LLVM structure modulo unflagged affine bit-vector integer identities",
        "floating_arithmetic_normalized": False,
        "poison_dependencies_preserved": not defined_integer_domain,
        "normalized_operation_metadata_ignored": ["merlin.global_task"],
        "source_numerics_qualified": False,
        "timing_benefit": "UNMEASURED",
    ...
    try:
        if (
            before_module is None
            or after_module is None
            or timeout_s <= 0
            or max_operations <= 0
        ...
        for label, module in (("before", before_module), ("after", after_module)):
            ...
            result[label] = {
                **counts,
                "normal_form_sha256": hashlib.sha256(json.dumps(form, separators=(",", ":")).encode()).hexdigest(),
            }
        if forms[0] == forms[1]:
