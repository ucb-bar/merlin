...
def performance_from_plan(
    model: str, baseline_cycles: float, result: GlobalPlanResult, *, heldout: bool, provenance: tuple[str, ...] = ()
) -> ModelPerformance:
...
class OptimizationOpportunity:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"kind": self.kind, "model": self.model, "cycles": self.cycles, "detail": self.detail}
...
class WholeModelReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "aggregate": {
                "cycle_basis": (
                    self.models[0].cycle_basis
                    if self.models and len({row.cycle_basis for row in self.models}) == 1
...
def _opportunities(rows: tuple[ModelPerformance, ...]) -> tuple[OptimizationOpportunity, ...]:
    ...
    for row in rows:
        ...
        if compute_busy is not None:
            ...
            found.append(
                OptimizationOpportunity(
                    "accelerator_bubbles",
                    row.model,
                    max(0.0, compute_capacity - compute_busy),
        ...
        if occupancy.overlap_cycles is None:
            found.append(
                OptimizationOpportunity(
                    "unknown_latency_hiding", row.model, None, "compute/movement overlap was not observable"
                )
            )
    ...
    return tuple(sorted(found, key=lambda item: (item.cycles is None, -(item.cycles or 0.0), item.model, item.kind)))


def evaluate_whole_models(
    rows: tuple[ModelPerformance, ...], *, policy: WholeModelGatePolicy | None = None
) -> WholeModelReport:
    ...
    if not rows:
        return WholeModelReport((), None, None, None, 0.0, (), (), ("no whole-model results",))
    ...
    for row in rows:
        ...
        if (
            occupancy is None
            or occupancy.compute_utilization is None
            ...
            refusals.append(f"{row.model}: whole-model occupancy/movement/latency-hiding evidence is incomplete")
        ...
        if row.legal_floor_cycles > row.baseline_cycles:
            refusals.append(
                f"{row.model}: legal floor {row.legal_floor_cycles:g} exceeds baseline {row.baseline_cycles:g}"
            )
            continue
        if row.candidate_cycles + 1e-9 < row.legal_floor_cycles:
            refusals.append(f"{row.model}: candidate violates its legal floor; cost model is unsound")
            continue
        compatible.append(row)

    heldout = [row for row in rows if row.heldout]
    geomean = math.exp(sum(math.log(row.speedup) for row in heldout) / len(heldout)) if heldout else None
    if not heldout:
        refusals.append("no held-out whole-model result")

    gap_denom = sum(row.baseline_cycles - float(row.legal_floor_cycles) for row in compatible)
    ...
    attainments = [row.legal_attainment for row in compatible if row.legal_attainment is not None]
    ...
    occupancy_complete = not any("occupancy/movement/latency-hiding" in refusal for refusal in refusals)

    gates = (
        ("measured_whole_model_cycles", all(row.cycle_basis == "measured" and bool(row.provenance) for row in rows)),
        ("heldout_geomean_speedup", geomean is not None and geomean >= pol.minimum_heldout_geomean_speedup),
        ("individual_regression", worst_regression <= pol.maximum_individual_regression),
        ("aggregate_gap_closed", gap_closed is not None and gap_closed >= pol.minimum_aggregate_gap_closed),
        (
            "legal_roofline_attainment",
            worst_attainment is not None and worst_attainment >= pol.minimum_legal_attainment,
        ),
