...
Trait admission and emitter reach are independent requirements. ISA availability is
queried from the selected target's current machine facts, never inferred from its
name or from a historical measurement. This classification does not change the
experiment's declared emitter, claim, or admission policy.
...
``merlin.perf.workload_gen.plan_matmul``
    ...
    maps, so it needs a resolvable ISA definition and the required machine facts.
