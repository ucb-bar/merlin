...
def _buffer_contract(buffer: Mapping[str, Any], *, role: str, relation: str) -> tuple[dict[str, Any], list[str]]:
    problems: list[str] = []
...
def _graph_links(parts: Mapping[str, Any]) -> tuple[dict[str, int], dict[str, list[int]], list[str]]:
    ...
    for name, buffer in buffers.items():
        if (
            ...
            or not name
            or not isinstance(buffer, Mapping)
            or buffer.get("id") != name
            or not isinstance(buffer.get("shape"), list)
            or any(type(extent) is not int or extent < 0 for extent in buffer.get("shape", []))
            or not isinstance(buffer.get("dtype"), str)
            or not buffer.get("dtype")
        ):
...
def _source_plan_adapter(
    parts: Mapping[str, Any],
) -> tuple[dict[str, Any] | None, dict[int, Mapping[str, Any]], list[str]]:
    ...
    for node in nodes:
        ...
        if isinstance(provenance, dict):
            for field in ("prov.epilogue_stage", "prov.epilogue_operation", "prov.epilogue_operand_roles"):
                provenance.pop(field, None)

    rows, unknown = storage.get("rows"), storage.get("unknown")
    storage_encodings = plan.get("storage_encodings")
    if (
        storage.get("schema") != _SOURCE_STORAGE_SCHEMA
    ...
    if (storage.get("status") == "complete") != (not unknown) or (metadata.get("status") == "verified") != (
        storage.get("status") == "complete"
    ):
    ...
    for index, row in enumerate(rows):
        ...
        if (
            ...
            != {
                ...
                "encoding",
                "encoding_sha256",
                "encoding_contract_location",
                "layout",
                "layout_sha256",
                "layout_contract",
                "proof_scope",
                "caller_materialization",
            ...
            or not tensor
            or tensor in represented_tensors
            ...
            or not isinstance(layout_contract, Mapping)
            or not _pin(layout_sha)
    ...
    if (
        epilogues.get("schema") != _SOURCE_EPILOGUE_SCHEMA
        or epilogues.get("status") != "verified"
        or not isinstance(roots, list)
        or epilogues.get("contraction_roots") != len(roots)
        or not isinstance(counts, Mapping)
        or set(counts) != set(classes)
        or any(
            counts[name] != sum(isinstance(root, Mapping) and root.get("classification") == name for root in roots)
            for name in classes
        )
    ):
    ...
    for root_index, root in enumerate(roots):
        ...
        source_index = root.get("producer_source_operation_id") if isinstance(root, Mapping) else None
        if type(source_index) is not int or not 0 <= source_index < len(nodes) or source_index in roots_by_id:
        ...
        accumulator_name = (
            node.get("outputs", [None])[0] if isinstance(node, Mapping) and len(node.get("outputs", [])) == 1 else None
        )
        ...
        if (
            set(root)
            != {
                "producer_source_operation_id",
                "producer_task_index",
                "producer_task_kind",
                "accumulator_source_buffer",
                "accumulator",
            ...
            or root.get("accumulator") != {"shape": accumulator.get("shape"), "dtype": accumulator.get("dtype")}
            or not isinstance(stages, list)
            or not isinstance(source_ids, list)
            or source_ids
            != [source_index, *[stage.get("source_operation_id") for stage in stages if isinstance(stage, Mapping)]]
        ):
        ...
        for stage_index, stage in enumerate(stages):
            ...
            operation = nodes[operation_id] if type(operation_id) is int and 0 <= operation_id < len(nodes) else None
            ...
            semantic = (
                {
                    key: stage.get(key)
                    for key in ("stage", "operation", "inputs", "output", "indexing_maps", "scalar_operations")
                }
            ...
            if (
                ...
                or not isinstance(stage_owner, Mapping)
                ...
                or not isinstance(stage.get("operation"), str)
                or not stage["operation"]
                or not isinstance(inputs, list)
                or not inputs
                or not isinstance(output, Mapping)
                or not isinstance(operation_inputs, list)
                or len(inputs) != len(operation_inputs)
                or [operand.get("operand_index") for operand in inputs if isinstance(operand, Mapping)]
                != list(range(len(operation_inputs)))
                or not isinstance(operation_outputs, list)
                or len(operation_outputs) != 1
                ...
                or any(not isinstance(item, str) or not item for item in stage["indexing_maps"])
                or not isinstance(stage.get("scalar_operations"), list)
                or any(not isinstance(item, str) or not item for item in stage["scalar_operations"])
            ...
            for operand in inputs:
                ...
                if (
                    ...
                    or set(operand) != {"source_buffer", "operand_index", "role", "relation", "shape", "dtype"}
                    or type(operand_index) is not int
                    or not 0 <= operand_index < len(operation_inputs)
                    or operation_inputs[operand_index] != name
                    or not isinstance(buffer, Mapping)
            ...
            if (
                not valid
                or set(output) != {"source_buffer", "shape", "dtype"}
            ...
            provenance["prov.epilogue_operand_roles"] = [operand["role"] for operand in inputs[1:]]
        ...
        completed = (
            bool(stages)
            and accumulator_width is not None
            and final_width is not None
            and final_width < accumulator_width
        )
        expected_classification = (
            "complete_integer_epilogue" if completed else "partial_integer_epilogue" if stages else "unclassified"
        )
    ...
    adapted = dict(parts)
    adapted.update({"nodes": nodes, "buffers": buffers, "source_plan_epilogue_roots": roots_by_id})
...
def _fallback_residual(
    node: Mapping[str, Any], current: str, buffers: Mapping[str, Mapping[str, Any]]
) -> dict[str, Any] | None:
    provenance = node.get("prov") if isinstance(node.get("prov"), Mapping) else {}
    if (
        node.get("kind") != "dispatch"
        or provenance.get("prov.op") != "add"
        or current not in node.get("inputs", [])
...
def _explicit_stage(
    ...
    *,
    ...
    if not isinstance(inputs, list) or inputs.count(current) != 1 or not isinstance(outputs, list) or len(outputs) != 1:
    ...
    float_used = any(_floating(contract.get("dtype")) for contract in [*operand_contracts, output_contract])
    normalized = {
        ...
        "source_input_buffers": [_source_buffer_witness(name, buffers[name]) for name in inputs if name in buffers],
...
def _exact_contract(
    value: Any, *, fields: set[str], label: str, role: str | None = None, relation: str | None = None
) -> list[str]:
...
def _proof_form(form: Mapping[str, Any]) -> list[str]:
    ...
    problems.extend(
        _exact_contract(accumulator, fields={"dtype", "rank", "encoding", "layout"}, label="capability accumulator")
    )
    ...
    if (
        ...
        or sequence != [stage.get("stage") for stage in stages if isinstance(stage, Mapping)]
    ...
    for index, stage in enumerate(stages):
        if (
            ...
            or not isinstance(stage.get("operation"), str)
            or not stage["operation"]
            or not isinstance(stage.get("inputs"), list)
            or not stage["inputs"]
        ...
        if any(
            isinstance(operand, Mapping) and operand.get("relation") not in {"exact", "scalar", "trailing_broadcast"}
            for operand in inputs
        ):
        ...
        problems.extend(
            _exact_contract(
                stage_output, fields={"dtype", "rank", "encoding", "layout"}, label=f"capability stage {index} output"
            )
        )
        if stage_name == "bias":
            if (
    ...
    if stages and isinstance(stages[-1], Mapping) and stages[-1].get("stage") not in {"requant", "narrow_store"}:
    ...
    if (
        ...
        or not output["encoding"]
        or not isinstance(output.get("layout"), str)
        or not output["layout"]
    ):
...
def _capability_forms(
    ...
    if evidence is None:
        return {}, ([] if file_sha256 is None else ["capability file pin exists without capability evidence"])
    problems: list[str] = []
    ...
    if (
        evidence.get("schema") != CAPABILITY_SCHEMA
    ...
    if not _pin(target_descriptor) or evidence.get("target_descriptor_sha256") != target_descriptor:
        problems.append("capability evidence does not match the iteration target descriptor")
    target_facts = set()
    for analysis in analyses:
        try:
            target_facts.add(
                analysis["diagnostics"]["task_instruction_evidence"]["candidate"]["binding"]["target_facts_sha256"]
            )
        except (KeyError, TypeError):
            target_facts.add(None)
    if (
        len(target_facts) != 1
        or not _pin(next(iter(target_facts)))
    ...
    for index, proof in enumerate(proofs):
        if (
            ...
            or not isinstance(proof.get("form"), Mapping)
            ...
            continue
        problems.extend(f"capability proof {index}: {reason}" for reason in _proof_form(proof["form"]))
...
def _source_form(
    ...
    if not isinstance(accumulator, Mapping):
        ...
        accumulator_contract, problems = _buffer_contract(accumulator, role="accumulator", relation="exact")
    ...
    normalized_stages = [
        {
            ...
            if key
            not in {"source_operation_id", "stage_binding_sha256", "source_input_buffers", "source_output_buffer"}
        }
        for stage in stages
    ]
    form = {
...
def _trace(
    root: Mapping[str, Any], parts: Mapping[str, Any], uses: Mapping[str, list[int]], results: set[str]
) -> dict[str, Any]:
    ...
    accumulator_width = _integer_width(accumulator.get("dtype"))
    input_widths = [_integer_width(buffers[name].get("dtype")) for name in node.get("inputs", [])[:2]]
    base = {
        ...
        "producer_input_buffers": [
            _source_buffer_witness(name, buffers[name]) for name in node.get("inputs", [])[:2] if name in buffers
        ],
    ...
    for _step in range(512):
        ...
        if len(consumers) != 1:
            uncaptured.append(
                "epilogue chain has fanout" if consumers else "epilogue chain terminates before an exact narrow readout"
            )
            break
        ...
        stage, reasons, uses_float, second_operand = _explicit_stage(
            consumer, source_index=source_index, current=current, buffers=buffers, first=not stages
        )
    ...
    if residual is not None:
        return {
            **base,
            "classification": "residual_second_operand",
            "second_operand": residual,
            "reasons": ["residual add needs a second full-shape tensor operand"],
        }
    if float_used or any(
        _floating(buffers[name].get("dtype"))
        for row in base["path"]
        for name in parts["nodes"][row["source_operation_id"]].get("inputs", [])
        if name in buffers
    ):
        return {
            **base,
            "classification": "float_or_unsupported_stage",
            "reasons": sorted(set([*unsupported, *uncaptured, "epilogue path enters floating-point arithmetic"])),
        }
    if unsupported:
        return {
            **base,
    ...
    return {
        **base,
        ...
        "capability_form_sha256": _digest(form),
        "reasons": [],
    }
...
def _member_inventory(
    ...
    member_index: int,
    capability_forms: Mapping[str, Mapping[str, Any]],
) -> dict[str, Any]:
    ...
    if not parts:
        ...
        return member
    ownership_broken = bool(
        parts["missing_owners"] or any("owner" in reason or "ownership" in reason for reason in problems)
    )
    if ownership_broken:
        for raw in parts["placement"].get("contractions", []):
            member["ownership_failures"].append(
                {
                    "producer_source_operation_id": raw.get("source_op_index") if isinstance(raw, Mapping) else None,
                    "classification": "ownership_failure",
    ...
    for raw in parts["placement"].get("contractions", []):
        ...
        if not isinstance(raw, Mapping):
            member["uncaptured"].append(
                {
                    "producer_source_operation_id": None,
                    "classification": "uncaptured",
        ...
        if site is None:
            node = (
                parts["nodes"][source_index]
                if type(source_index) is int and 0 <= source_index < len(parts["nodes"])
                else {}
            )
            output_dtypes = [parts["buffers"].get(name, {}).get("dtype") for name in node.get("outputs", [])]
            bucket = "float_or_unsupported_stage" if any(_floating(dtype) for dtype in output_dtypes) else "uncaptured"
        ...
        metadata_reasons = source_metadata.get("reasons", []) if isinstance(source_metadata, Mapping) else []
        representation_reasons = sorted(
            {
                reason
                for reason in traced.get("reasons", [])
                if "encoding is absent" in reason or "layout is absent" in reason
            }
        )
        ...
        elif classification in {"uncaptured", "float_or_unsupported_stage"} and isinstance(metadata_reasons, list):
            traced["reasons"] = sorted(
                set(
                    [
                        *traced.get("reasons", []),
                        *[reason for reason in metadata_reasons if isinstance(reason, str) and reason],
                    ]
                )
            )
        if classification == "capability_pending":
            ...
            if proof is None:
                ...
                traced["reasons"] = ["no pinned target proof matches the exact stage/operand/encoding/readout form"]
            else:
                classification = "eligible"
                traced["capability_proof_id"] = proof["id"]
                traced["capability_proof_sha256"] = _digest(proof)
                traced["proof_scope"] = "exact target form availability; source rewrite equivalence remains mandatory"
                traced["not_proven"] = [
                    "replacement semantic equivalence",
                    "emitted work deletion",
                    "runtime correctness",
                    "cycle improvement",
                ]
        ...
        member["uncaptured"].append(
            {
                ...
                "reasons": [
                    raw.get("mac_basis")
                    if isinstance(raw, Mapping) and isinstance(raw.get("mac_basis"), str)
                    else "contraction observer left the source operation unresolved"
                ],
            }
        )
    for key in (
        ...
        member[key].sort(
            key=lambda row: -1 if row["producer_source_operation_id"] is None else row["producer_source_operation_id"]
        )
    member["source_operation_ids"] = sorted(
        {source_index for chain in member["eligible"] for source_index in chain["source_operation_ids"]}
    )
    member["problems"] = sorted(set(member["problems"]))
    member["status"] = (
        "not_ready" if member["problems"] else "eligible_sites_bound" if member["eligible"] else "no_eligible_sites"
    )
...
def inventory_portfolio_narrow_epilogues(
    ...
    *,
    ...
    for index, (identity, analysis) in enumerate(zip(identities, analyses, strict=True)):
        result["members"].append(_member_inventory(analysis, identity, record["candidate_sha256"], index, forms))
    ...
    result["status"] = (
        "not_ready" if result["problems"] else "ready_for_work_order" if opportunities else "no_eligible_sites"
    )
    result["sha256"] = _digest(result)
    return result
...
def build_narrow_epilogue_mechanism_documents(
    ...
    *,
    ...
    return {"edit_contract": contract, "catalog": catalog, "inventory": inventory, "work_order": work_order}
...
def main(argv: list[str] | None = None) -> int:
    from .narrow_epilogue_work_order_cli import main as invoke
