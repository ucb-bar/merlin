...
__all__ = [
    ...
    "preflight_operands",
    "preflight_matmul",
    "projected_cycle_ceiling",
    "render",
]


def projected_cycle_ceiling(floor_cycles: float, *, slack: float) -> "tuple[int | None, str]":
    """``(ceiling, basis)`` -- the cycle count a program may spend, or ``(None, why not)``.

    THIS IS THE ARITHMETIC THE CAPSULE FIELD IS NAMED AFTER. ``performance.cost.projected_cycles``
    carried the word ``derived_at_preflight`` on 113 capsules and nothing anywhere resolved it; this is
    the function that does, and it is here rather than in the plane that calls it because a projection
    is what this module is for.

    Two inputs and no third. ``floor_cycles`` is the count the array cannot finish the program in fewer
    of -- derived from the target's own array geometry and the work the program's own command buffer
    counted, so it carries every machine fact and this function carries none. ``slack`` is a declared
    POLICY: how many times that floor a submission may spend and still be admitted. It is not a
    measurement and must never be defaulted here; a caller that cannot read a declared one gets
    ``None`` and a reason, because a slack invented at the point of use silently decides which
    submissions pass.

    The ceiling ROUNDS UP. A floor of 4096.2 cycles admits 4096 at slack 1.0 otherwise, which would
    refuse the only schedule that could ever meet it.
    """
    if not isinstance(floor_cycles, (int, float)) or isinstance(floor_cycles, bool):
        return None, f"the derived issue floor is {floor_cycles!r}, which is not a cycle count"
    if floor_cycles <= 0:
        return None, (
            f"the derived issue floor is {floor_cycles}, so there is nothing to scale: a ceiling of "
            f"zero admits no run and a ceiling derived from zero work is not about this program"
        )
    if not isinstance(slack, (int, float)) or isinstance(slack, bool) or slack < 1.0:
        return None, (
            f"the declared slack over the issue floor is {slack!r}; it must be a number >= 1. A slack "
            f"below 1 would demand a program finish faster than its own sequencer's loop, and an "
            f"absent one must not be defaulted -- inventing it here would choose which submissions pass"
        )
    ceiling = int(math.ceil(float(floor_cycles) * float(slack)))
    return ceiling, (
        f"{ceiling} = ceil({float(floor_cycles):.6g} derived issue-floor cycles x {float(slack):g} declared slack)"
    )
