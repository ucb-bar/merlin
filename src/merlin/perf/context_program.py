...
def slice_context_source(
    module: Any, context: Mapping[str, Any], *, target: str, fixed_work_projection: bool = False
) -> tuple[Any, dict]:
    ...
    readout = next(
        (
            index
            for index in range(indices[-1] + 1, len(rows))
            if isinstance(rows[index].get("decoded", {}).get("acc_addr"), int)
            and rows[index]["decoded"]["acc_addr"] & ~mask == destination & ~mask
        ),
        None,
    )
    if readout is None:
    ...
    store_config = next(
        (index for index in range(readout - 1, -1, -1) if rows[index].get("decoded", {}).get("subtype") == "ST"), None
    )
    completion = next((index for index in range(readout + 1, len(rows)) if rows[index].get("class") == "FENCE"), None)
    if store_config is None or completion is None:
        raise ValueError("source readout configuration or completion is absent")
    if asm[completion].has_side_effects is None or "~{memory}" not in asm[completion].constraints.data.split(","):
    ...
    def visit(op):
        ...
        for operand in op.operands:
            ...
            if isinstance(owner, Block):
                ...
                if (
                    parent is None
                    or parent.name != "llvm.func"
                    or parent.body.blocks.first is not owner
    ...
    function = llvm.FuncOp(
        "source_context",
        llvm.LLVMFunctionType([argument.type for argument in arguments]),
    ...
    return result, {
        ...
        "source_op_indices": context["source_op_indices"],
        "source_instruction_indices": selected,
        "timed_source_instruction_indices": indices,
        "source_readback_index": readout,
        "source_completion_index": completion,
        ...
        "scope": (
            "controlled fixed-work projection; omitted future computes symmetric across arms"
            if fixed_work_projection
