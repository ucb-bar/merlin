...
def _artifact_check(role: str, raw: object, *, root: Path) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if not isinstance(declared_path, str) or not declared_path.strip() or not _is_sha256(expected):
        return (
            {**check, "status": "refused"},
            {
                "priority": 10,
                "status": "refused",
                "code": "artifact_binding_malformed",
    ...
    if path.is_symlink():
        return (
            {**check, "status": "refused"},
            {
                "priority": 11,
                "status": "refused",
                "code": "artifact_binding_is_symlink",
    ...
    try:
        ...
        return (
            {**check, "status": "UNKNOWN", "reason": str(exc)},
            {
                "priority": 12,
                "status": "UNKNOWN",
...
def _identity_diagnostic(raw: object, expected: Mapping[str, str], *, source: str) -> dict[str, Any] | None:
    observed = _mapping(raw)
    if observed != expected:
        return {
            "priority": 2,
            "status": "refused",
            "code": "emission_identity_mismatch",
...
def _egress_check(
    profile: Mapping[str, Any], evidence: object, *, profile_sha256: str, expected_identity: Mapping[str, str]
        ...
        diagnostics.append(
            {
                "priority": 13,
                "status": "refused",
                "code": "egress_capabilities_malformed",
    ...
    if not document:
            ...
            diagnostics.append(
                {
                    "priority": 14,
                    "status": "refused",
                    "code": "egress_evidence_schema_mismatch",
            ...
            diagnostics.append(
                {
                    "priority": 30,
                    "status": "UNKNOWN",
                    "code": "egress_derivation_unverified",
            ...
            diagnostics.append(
                {
                    "priority": 30,
                    "status": "UNKNOWN",
                    "code": "egress_coverage_incomplete",
            ...
            diagnostics.append(
                {
                    "priority": 2,
                    "status": "refused",
                    "code": "egress_profile_identity_mismatch",
        ...
        mismatch = _identity_diagnostic(document.get("emission_identity"), expected_identity, source="physical egress")
        ...
        for index, raw in enumerate(raw_egresses):
            ...
            name = declared_name if isinstance(declared_name, str) and declared_name else f"egress[{index}]"
            ...
            row_result = {
                "name": name,
                "emitted_representation": dict(representation),
                ...
                diagnostics.append(
                    {
                        "priority": 3,
                        "status": "refused",
                        "code": "unsupported_physical_egress",
                ...
                diagnostics.append(
                    {
                        "priority": 4,
                        "status": "refused",
                        "code": "duplicate_physical_egress",
    ...
    return (
        {
            "status": "verified" if status == "admitted" else status,
            "supported": [{"encoding": encoding, "width_bits": width} for encoding, width in sorted(supported)],
            "egresses": rows,
        },
        diagnostics,
    )
...
def _wrapper_check(
    evidence: object, *, profile_sha256: str, expected_identity: Mapping[str, str], root: Path
) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    diagnostics: list[dict[str, Any]] = []
    document = _mapping(evidence)
    ...
    if not document:
            ...
            diagnostics.append(
                {
                    "priority": 15,
                    "status": "refused",
                    "code": "wrapper_evidence_schema_mismatch",
            ...
            diagnostics.append(
                {
                    "priority": 2,
                    "status": "refused",
                    "code": "wrapper_profile_identity_mismatch",
        ...
        mismatch = _identity_diagnostic(document.get("emission_identity"), expected_identity, source="wrapper")
        if mismatch is not None:
            diagnostics.append(mismatch)
        artifact, artifact_diagnostic = _artifact_check("wrapper", document.get("wrapper_artifact"), root=root)
            ...
            diagnostics.append(
                {
                    "priority": 41,
                    "status": "UNKNOWN",
                    "code": "wrapper_derivation_unverified",
        ...
        if malformed or not names:
            diagnostics.append(
                {
                    "priority": 42,
                    "status": "UNKNOWN",
                    "code": "wrapper_events_unavailable",
            ...
            positions = {
                name: [index for index, event in enumerate(names) if event == name]
            ...
            if ambiguous:
                diagnostics.append(
                    {
                        "priority": 5,
                        "status": "refused",
                        "code": "wrapper_core_event_ambiguous",
                ...
                warm, reset, start, measured, end = (positions[name][0] for name in _CORE_WRAPPER_EVENTS)
                warm_completions = [index for index in positions["completion"] if warm < index < reset]
                measured_completions = [index for index in positions["completion"] if measured < index < end]
                if not warm_completions:
                    diagnostics.append(
                        {
                            "priority": 5,
                            "status": "refused",
                            "code": "warm_without_completion",
                    ...
                    diagnostics.append(
                        {
                            "priority": 5,
                            "status": "refused",
                            "code": "wrapper_event_order_invalid",
                    ...
                    diagnostics.append(
                        {
                            "priority": 5,
                            "status": "refused",
                            "code": ("measured_end_before_completion" if after_end else "measured_without_completion"),
                    ...
                    diagnostics.append(
                        {
                            "priority": 43,
                            "status": "UNKNOWN",
                            "code": "wrapper_validation_missing",
                    ...
                    diagnostics.append(
                        {
                            "priority": 5,
                            "status": "refused",
                            "code": "validation_inside_measurement_window",
    ...
    return (
        {
            ...
            "events": names,
            "required_order": ["warm", "completion", "reset", "start", "measured", "completion", "end"],
            "validation_scope": "outside_start_end",
        },
        diagnostics,
    )
...
def assess_deployment_admissibility(
    ...
    *,
        ...
        diagnostics.append(
            {
                "priority": 10,
                "status": "refused",
                "code": "deployment_profile_malformed",
                "message": str(exc),
                "action": "provide a complete deployment_profile schema instance",
            }
        )
        ...
        diagnostics.append(
            {
                "priority": 10,
                "status": "refused",
                "code": "deployment_profile_schema_mismatch",
        ...
        diagnostics.append(
            {
                "priority": 1,
                "status": "refused",
                "code": "deployment_profile_identity_mismatch",
    ...
    if not expected_identity or any(
        not isinstance(key, str) or not key or not _is_sha256(value) for key, value in expected_identity.items()
    ):
        diagnostics.append(
            {
                "priority": 2,
                "status": "refused",
                "code": "expected_emission_identity_malformed",
    ...
    egress, egress_diagnostics = _egress_check(
        profile, egress_evidence, profile_sha256=profile_sha256, expected_identity=expected_identity
    )
    wrapper, wrapper_diagnostics = _wrapper_check(
        wrapper_evidence, profile_sha256=profile_sha256, expected_identity=expected_identity, root=wrapper_base
    )
    ...
    return {
        ...
        "promotion_scope": (
            "deployment-specific final readiness; compiler authoring remains allowed "
...
def require_deployment_admissible(result: Mapping[str, Any], *, expected_profile_sha256: str) -> None:
    ...
    if (
        result.get("schema") != "deployment_admissibility_v1"
...
def main(argv: list[str] | None = None) -> int:
    ...
    result = assess_deployment_admissibility(
        ...
        wrapper_root=args.wrapper_evidence.parent,
    )
