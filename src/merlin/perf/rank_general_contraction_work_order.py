...
def _ordered_analyses(
    ...
    if not isinstance(portfolio, Mapping):
        ...
        return [], []
    if (
        portfolio.get("schema") != "full_model_portfolio_iteration_v1"
        or portfolio.get("candidate_sha256") != candidate_sha256
        or portfolio.get("full_model_simulation_allowed") is not False
        or portfolio.get("selection") != "multi_model_pareto_without_invented_static_cycle_total"
    ):
    ...
    if not isinstance(members, list) or not members:
        ...
        return [], []
    if portfolio.get("members_total") != len(members) or portfolio.get("members_ready") != len(members):
    ...
    for index, member in enumerate(members):
        ...
        if (
            ...
            or not isinstance(identity.get("capsule"), str)
            or not identity["capsule"]
        ...
        if not isinstance(analysis, Mapping):
            ...
            analysis = {}
        workload = {
            field: identity.get(field) for field in ("capsule", "capsule_sha256", "required_lanes", "required_tiers")
        }
    ...
    if len(identities) != len(members) or not _pin(declared) or _digest(_portfolio_identity(identities)) != declared:
        problems.append("ordered member identities do not match the exact portfolio hash")
    return identities, analyses


def _analysis_parts(analysis: Mapping[str, Any], candidate_sha256: str) -> tuple[dict[str, Any], list[str]]:
    ...
    if not all(isinstance(value, Mapping) for value in (plan, graph, evidence, placement_pair, emission)):
    ...
    if (
        tasks.get("schema") != "task_instruction_evidence_v1"
    ...
    if (
        placement.get("schema") != "model_contraction_placement_v1"
    ...
    if not isinstance(task_rows, list):
        ...
        task_rows = []
    if (
        graph.get("nodes") != len(nodes)
        or plan.get("source_operations") != len(nodes)
...
def _site(row: Mapping[str, Any], parts: Mapping[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    reasons: list[str] = []
    ...
    if provenance.get("prov.family") != "contraction" or provenance.get("prov.region_id") != row.get("region"):
    ...
    if any(not isinstance(value, Mapping) for value in input_rows) or not isinstance(output_row, Mapping):
    ...
    if (
        len(dtypes) < 2
        or any(not _integer_dtype(dtype) for dtype in dtypes[:2])
    ...
    site = {
        ...
        "map_evidence": {
            "status": "proved",
            "source": "static_affine_domain_observer",
            "symbols": "none",
            "domain_macs": extent_product,
        },
        "ownership_evidence": {
            "status": "proved",
            "task_index": owner["task_index"],
            "declared_task_kind": owner["declared_task_kind"],
            "task_binding_sha256": _digest({field: parts["binding"][field] for field in _PIN_FIELDS}),
        },
        "declared_lane": row.get("lane"),
        "proof_scope": ("source rank/maps/integer recurrence and exact static task ownership; authoring site only"),
        "not_proven": [
            "target eligibility",
...
def _member_inventory(
    analysis: Mapping[str, Any], identity: Mapping[str, Any], candidate_sha256: str, member_index: int
) -> dict[str, Any]:
        ...
        member["uncaptured"].append(
            {
                ...
                "reasons": [
                    ...
                    else "contraction observer left the source domain unresolved"
                ],
            }
        )
    ...
    member["uncaptured"].sort(
        key=lambda row: (-1 if row["source_operation_id"] is None else row["source_operation_id"], row["reasons"])
    )
    member["refusals"].sort(key=lambda row: row["source_operation_id"])
    member["source_operation_ids"] = [row["source_operation_id"] for row in member["host_rejected"]]
    member["status"] = (
        "eligible_sites_bound"
        if member["source_operation_ids"] and not problems
...
def _embedded_edit_contract(
    record: Mapping[str, Any], analyses: Sequence[Mapping[str, Any]], candidate: Path
) -> dict[str, Any]:
    ...
    if not isinstance(contract, Mapping) or _digest(contract) != authority.get("contract_document_sha256"):
...
def build_rank_general_mechanism_documents(
    ...
    *,
    ...
    inventory = inventory_portfolio_rank_general_contractions(record, iteration_record_sha256=iteration_record_sha256)
...
def main(argv: list[str] | None = None) -> int:
    parser = ArgumentParser(description="Seal one rank-general integer-contraction Phase-2 catalog/work order")
    ...
    report = {
        ...
        "mechanism_id": MECHANISM_ID,
        "output": str(output),
        "opportunity_counts": [len(member["source_operation_ids"]) for member in documents["inventory"]["members"]],
        "already_offloaded_counts": [len(member["already_offloaded"]) for member in documents["inventory"]["members"]],
