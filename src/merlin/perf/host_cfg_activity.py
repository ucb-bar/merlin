...
def _real_name(operation: Any) -> str:
    """The operation's own name, including one the parser holds as unregistered.

    An LLVM operation this xDSL does not model parses as ``builtin.unregistered``. Categorising by
    that name filed every such operation under ``other``: a float-to-integer narrowing was not
    counted as a conversion, so a host requantization loop under-reported the very work this
    instrument exists to show.
    """
    if operation.name == "builtin.unregistered":
        real = getattr(operation, "op_name", None)
        if real is not None:
            return str(getattr(real, "data", real))
...
def _category(name: str) -> str:
    ...
    return {
        "llvm.load": "load",
        "llvm.store": "store",
        "llvm.icmp": "comparison",
        "llvm.fcmp": "comparison",
        "llvm.getelementptr": "address",
        "llvm.ptrtoint": "address",
        "llvm.inttoptr": "address",
        "llvm.br": "branch",
        "llvm.cond_br": "branch",
        "llvm.return": "return",
        "llvm.mlir.constant": "constant",
        "llvm.alloca": "allocation",
        "llvm.inline_asm": "opaque_inline_asm",
    }.get(name, "other")
...
def _constant_loop(header, latch, nodes, predecessors, positions):
    ...
    if terminal.name != "llvm.cond_br" or not header.args or len(terminal.successors) != 2:
    ...
    if not isinstance(condition, Operation) or condition.name != "llvm.icmp" or len(condition.operands) != 2:
    ...
    if predicate not in {"slt", "ult"} or induction not in header.args or not isinstance(induction.type, IntegerType):
    ...
    def forwards_induction(value, seen=frozenset()):
        ...
        if value is induction:
            return True
        if value in seen or not isinstance(value, BlockArgument) or value.owner not in nodes or value.owner is header:
            return False
        incoming = []
    ...
    return {
        "header_block": positions[header],
        "latch_block": positions[latch],
...
def _block_signatures(
    blocks: Any, index: Mapping[Any, int], multiplicity: Mapping[Any, Any], *, limit: int = BLOCK_SIGNATURE_LIMIT
) -> list[dict[str, Any]]:
    ...
    for block in blocks:
        names = tuple(_real_name(operation) for operation in block.ops)
    ...
    ranked = sorted(groups.values(), key=lambda row: -sum(row["dynamic_operations"].values()))
    out = []
    for row in ranked[:limit]:
        total = sum(row["dynamic_operations"].values())
        out.append(
            {**row, "dynamic_total": total, "dynamic_operations": dict(sorted(row["dynamic_operations"].items()))}
        )
    return out
...
def analyze_host_cfg_activity(function: Any, *, prepared_cfg: PreparedHostCFG | None = None) -> dict[str, Any]:
    ...
    if not problems:
        ...
        backedges = [
            (header, latch) for header in blocks for latch in predecessors[header] if dominance.dominates(header, latch)
        ]
    ...
    allocations = []
    buffers = defaultdict(
        lambda: {"load_payload_bytes": 0, "store_payload_bytes": 0, "load_operations": 0, "store_operations": 0}
    )
    ...
    for block in blocks:
        ...
        for operation in block.ops:
            category = _category(_real_name(operation))
            ...
            if category == "allocation":
                ...
                size = (
                    ...
                    if elements is not None and elements >= 0 and element_bytes is not None
                    else None
                )
                ...
                allocations.append(
                    {
                        ...
                        "elements": elements,
                        "element_type": str(operation.properties.get("elem_type")),
                        "payload_bytes": size,
                        "execution_count": count,
                        "declared_payload_bytes": declared,
                        "annotation_status": "absent"
                        if declared is None
                        else ("UNKNOWN" if size is None else "matches" if declared == size else "mismatch"),
                    }
                )
    ...
    for owner, row in tasks.items():
        task_rows.append(
            {
                "task": owner,
                "static_operations": dict(row["static"]),
    ...
    top_allocations = sorted(
        (row for row in hotspots if row["allocation_operation_index"] is not None),
        key=lambda row: (
            row["static_allocation_payload_bytes"] is not None,
            row["static_allocation_payload_bytes"] or 0,
        ),
        reverse=True,
    )[:3]
    top_buffers = sorted(
        (row for row in hotspots if row["buffer_root"] in buffers),
        key=lambda row: (
            row["load_payload_bytes"] is not None,
            (row["load_payload_bytes"] or 0) + (row["store_payload_bytes"] or 0),
        ),
        reverse=True,
    )[:3]
    return {
        "schema": "host_cfg_activity_v1",
        "status": "derived" if not problems and not unknown_payload else "UNKNOWN",
        ...
        "tasks": task_rows,
        "buffer_payload": dict(buffers) if payload_known else None,
        ...
        if all(row["payload_bytes"] is not None for row in allocations)
        else None,
        "cpu_cycles": None,
        "dram_bytes": None,
        "cache_traffic_bytes": None,
        "scope": "one invocation, emitted LLVM IR execution counts and scalar memory payload",
        "limitations": [
            "not post-optimization machine instruction counts",
            "not DRAM/cache traffic",
