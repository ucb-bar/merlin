...
def _par_residue_parallel(features=frozenset()) -> bool:
    ...
    return threshold_of(features) is not None or bool(os.environ.get("MERLIN_PAR_RESIDUE_PARALLEL"))
...
def parallel_transform_schedule(
    n_harts: int,
    *,
    matmul_dim: str = "n",
    batch_matmul_dim: str = "b",
    chunks: "list | None" = None,
    tile_aligned: bool = False,
) -> str:
    ...
    for op, dim in (("linalg.matmul", matmul_dim), ("linalg.batch_matmul", batch_matmul_dim)):
        ...
        if dim not in choices:
            raise ValueError(
                f"{op}: parallel dim {dim!r} not in {sorted(choices)} (the reduction dim K is never tileable)"
            )
        tag = op.split(".")[-1]
        body.append(
...
def vec_pre_schedule(features: "frozenset[str]") -> "str | None":
    ...
    """
    from .impr_features import apply_schedule, ensure_vec_noncontraction, vec_noncontraction_lanes
...
def build_rvv_pipeline(
    ...
    head = (
        [preload, f"transform-interpreter{{entry-point={VEC_PRE_ENTRY}}}", "canonicalize", "cse", *specialize]
        if vec
        else [*specialize, preload]
    )
    passes = [
        "canonicalize",
        "cse",
        *head,
        *([f"transform-interpreter{{entry-point={PARALLEL_ENTRY}}}", "canonicalize", "cse"] if par else []),
        ...
        *(["func.func(linalg-fuse-elementwise-ops)", "canonicalize", "cse"] if _fuse_post() else []),
        ...
        # Do not run this hoist inside the parallel route: buffer-hoisting wraps the scratch in
        # ``memref.alloca_scope``, and the subsequent SCF/OpenMP-to-CF lowering gives that scope
        # multiple blocks, which is verifier-invalid. The K1 parallel path therefore keeps the
        # scratch local until a scope-safe late-hoist exists; the serial path retains stack reuse.
        ...
        *(
            ...
            if par
            else ["func.func(convert-linalg-to-loops)"]
        ),  # fallback: any op the vectorizer skipped
        "convert-scf-to-cf",
        "expand-strided-metadata",
        "lower-affine",
...
def apply_passes(mlir_text: str, pipeline: str, timeout: int = 600) -> str:
    ...
    proc = subprocess.run(
        [str(m2m_python()), str(script), str(src), str(dst), pipeline], capture_output=True, text=True, timeout=timeout
    )
...
def dealloc_placement_violations(mlir_text: str, timeout: int = 600) -> list[str]:
    ...
    script.write_text(
        "import json, sys\n"
        "from torch_mlir import ir\n" + DEALLOC_CHECK_PRELUDE + "ctx = ir.Context()\n"
        "with open(sys.argv[1]) as f:\n"
        "    module = ir.Module.parse(f.read(), ctx)\n"
        "bad, n_alloc, n_sunk = _dealloc_placement_violations(module)\n"
    ...
    proc = subprocess.run([str(m2m_python()), str(script), str(src)], capture_output=True, text=True, timeout=timeout)
...
_RUNNER = (
    _RUNNER_SRC.replace("__MERLIN_EMIT__", EMIT_TRANSLATE)
    .replace("__MERLIN_INSPECT_PARSED__", "")
    .replace("__MERLIN_INSPECT_LOWERED__", "")
)
...
_OP_NAME_CHARS = frozenset("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_")
...
def lower_to_llvm_ir(
    ...
    if pipeline is None:
        if vectorize:
            ...
            if parallel_harts is not None:
                ...
                par_sched.write_text(
                    parallel_transform_schedule(
                        parallel_harts, chunks=parallel_chunks, tile_aligned=_tile_aligned_parallel
                    ),
                    encoding="utf-8",
                )
                if parallel_chunks is not None and not parallel_chunks:
                    ...
                    print(
                        "[parallel_harts] WARNING: no contraction could be split without changing "
            ...
            pipeline = build_rvv_pipeline(
                ...
                features=feats,
                par_sched_path=par_sched,
                vec_sched_path=vec_sched,
                perop_parallel=(parallel_chunks is not None or _tile_aligned_parallel),
            )
        elif parallel:
    ...
    from .selfcopy import FEATURE as _SELFCOPY_FEATURE
    from .selfcopy import with_canonicalize as _with_canon
    ...
    if _grain is not None and not omp:
        ...
        print(
            "[parallel_grain] WARNING: a grain threshold is named but this lowering is SERIAL "
            "(no parallel_harts/parallel), so there is no scf.parallel to price; the feature "
            "will serialize nothing.",
            file=_sys.stderr,
            flush=True,
        )
    ...
    _panel_parallel_gate = (
        "1"
        if (
            omp
            and ({_IM2COL_PANEL_FEATURE, _WEIGHT_PANEL_FEATURE} & feats)
    ...
    if _coarsen_gate == "1" and _team_work is not None:
        raise ValueError(
            "coarsen_openmp_regions and parallel_team_cost are incompatible: "
...
def _fix_float_literals(ll_text: str) -> str:
    ...
    return (
        ll_text.replace("bfloat -inf", "bfloat 0xRFF80")
