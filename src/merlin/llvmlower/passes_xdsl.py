...
def collapse_overrank_matmul(module) -> int:
    ...
    from xdsl.dialects.builtin import AffineMapAttr, ArrayAttr, FloatAttr, IntegerAttr, IntegerType, TensorType
    ...
    for op in targets:
        ...
        iters = ArrayAttr([L.IteratorTypeAttr(par)] * (R - 1) + [L.IteratorTypeAttr(par), L.IteratorTypeAttr(red)])
        block = op.parent_block()

        empty = tensor.EmptyOp((), out_t)
        zero = arith.ConstantOp(IntegerAttr(0, et) if isinstance(et, IntegerType) else FloatAttr(0.0, et))
        ...
        gen = L.GenericOp(
            ...
            body=Region(body),
            indexing_maps=ArrayAttr([AffineMapAttr(lhs_map), AffineMapAttr(rhs_map), AffineMapAttr(out_map)]),
...
def lower_quant_ext(module) -> int:
    ...
    for op, kind in dequant_rewrites:
        ...
        maps = ArrayAttr(
            [AffineMapAttr(identity), AffineMapAttr(sz_map), AffineMapAttr(sz_map), AffineMapAttr(identity)]
        )
    ...
    for op, kind in quant_rewrites:
        ...
        maps = ArrayAttr(
            [AffineMapAttr(identity), AffineMapAttr(scalar_map), AffineMapAttr(scalar_map), AffineMapAttr(identity)]
        )
        iters = ArrayAttr([linalg_ops.IteratorTypeAttr(linalg_ops.IteratorType.PARALLEL) for _ in range(rank)])
        ...
        inv_generic = linalg_ops.GenericOp(
            inputs=(scale,),
            outputs=(inv_empty.tensor,),
            body=Region(inv_body),
            indexing_maps=ArrayAttr([AffineMapAttr(scalar_identity), AffineMapAttr(scalar_identity)]),
    ...
    for func in [op for op in module.walk() if op.name == "func.func"]:
        ...
        for op in reversed(list(block.ops)):
            ...
            removable = bool(op.results) and (
                op.name.startswith(_PURE_TENSOR_PREFIXES) or str(name).startswith("quant_ext.")
            )
...
def lower_bf16_matmul_f32acc(module) -> int:
    ...
    from xdsl.dialects.builtin import AffineMapAttr, ArrayAttr, BFloat16Type, Float16Type, FloatAttr, TensorType, f32
    from xdsl.dialects.linalg import ops as L
    from xdsl.ir import Block, Region
    from xdsl.ir.affine import AffineMap
    ...
    for op in module.walk():
        if op.name == "linalg.matmul" and is_half_tensor(op.results[0].type):
            ...
            is_mul_add_contraction(op)
            and is_half_tensor(op.results[0].type)
    ...
    mm_maps = ArrayAttr(
        [
            AffineMapAttr(AffineMap(3, 0, (d0, d2))),
            AffineMapAttr(AffineMap(3, 0, (d2, d1))),
            AffineMapAttr(AffineMap(3, 0, (d0, d1))),
        ]
    )
    mm_iters = ArrayAttr([L.IteratorTypeAttr(par), L.IteratorTypeAttr(par), L.IteratorTypeAttr(red)])
    ...
    for kind, op in targets:
        ...
        trunc = L.GenericOp(
            inputs=(acc_f32.results[0],),
            outputs=(empty_b.results[0],),
            body=Region(body2),
        ...
        carry_provenance(acc_f32, op, "fp16_f32acc" if isinstance(a_e, Float16Type) else "bf16_f32acc")
...
def _dequant_to_generic(op: "_Dequant") -> str:
    ...
    wty, sty, zty, outty = (t.strip() for t in (op.weight_type, op.scale_type, op.zero_type, op.result_type))
    axis = op.axis
...
def _attach_c_interface(text: str) -> tuple[str, int]:
    ...
    while True:
        ...
        return (f"{text[:start]}{signature} {results} attributes {{llvm.emit_c_interface}} {{{text[body_at + 1 :]}"), 1
...
def _slice_op_span(line: str, head_at: int, head: str, n_types: int):
    ...
    return (line[args_at:args_end], line[props_at:props_end], line[attrs_at:colon], types, at)
...
def _fix_extract_slices(line: str) -> str:
    ...
    while True:
        ...
        fixed = props.replace(f"{_SIZES_FIELD}{raw_sizes}>", _SIZES_FIELD + ", ".join(padded) + ">")
        out.append(line[i:at])
        out.append(f"{_EXTRACT_HEAD}{args}{_PROPS_OPEN}{fixed}{_PROPS_CLOSE}{attrs}{_OPERAND_TYPES}{src}>)")
        i = end
...
def _fix_insert_slices(line: str) -> str:
    ...
    while True:
        ...
        strides_field = (
            None if sizes_field is None else _array_field(props[sizes_field[1] :], _STRIDES_FIELD, last=False)
        )
        ...
        offsets = (
            [int(s) for s in props[offsets_field[0] : offsets_field[1]].split(",")]
            if offsets_field is not None
            else [0] * len(sizes)
        )
        repaired = [
            1 if off + (sz - 1) * st >= extent else st for off, sz, st, extent in zip(offsets, sizes, strides, dst_dims)
        ]
        if repaired == strides:
            out.append(line[i:end])
            i = end
            continue
        fixed = props.replace(
            f"{_STRIDES_FIELD}{raw_strides}>", _STRIDES_FIELD + ", ".join(str(s) for s in repaired) + ">", 1
        )
        out.append(line[i:at])
        out.append(
            f"{_INSERT_HEAD}{args}{_PROPS_OPEN}{fixed}{_PROPS_CLOSE}{attrs}{_OPERAND_TYPES}{src}>, tensor<{dst}>)"
        )
        i = end
