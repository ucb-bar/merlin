...
if __name__ == "__main__":
    ...
    parser.add_argument("--height", type=int, default=None, help="Input height (uses model default if not specified)")
    parser.add_argument("--square", action="store_true", help="Resize to square resolution")
        ...
        if args.model_type.startswith("dpt_"):
            if args.model_type == "dpt_beit_large_512":
                ...
                try:
                    ...
                    model = DPTDepthModel(
                        path=None, backbone="levit_384", non_negative=True, head_features_1=64, head_features_2=8
                    )
                    net_w, net_h = 224, 224
                except (AttributeError, Exception) as e:
                    print("ERROR: dpt_levit_224 is not compatible with the current timm/Python version.")
                    print(f"Error: {e}")
                    print("\nThe issue is that:")
