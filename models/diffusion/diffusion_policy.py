...
class DiffusionPolicy(nn.Module):
    ...
    def __init__(self, state_dim=14, action_dim=7, hidden_dim=256, num_actions=16, num_past_states=4):
        super().__init__()
        ...
        self.cross_attention_layers = nn.ModuleList(
            [
                nn.MultiheadAttention(embed_dim=feature_dim, num_heads=8, batch_first=True)
        ...
        self.fusion_blocks = nn.ModuleList(
            [
                SimpleBlock(feature_dim)
                for _ in range(5)  # 5 blocks for depth
            ]
        )
        ...
        self.action_refinement = nn.ModuleList(
            [
                ActionRefinementBlock(action_dim, num_actions)
                for _ in range(2)  # 2 refinement passes
            ]
        )
    ...
    def denoise_step(self, past_states, actions, timestep, image=None):
        ...
        for cross_attn in self.cross_attention_layers:
            attended_feat, _ = cross_attn(attended_feat, state_action_feat, state_action_feat)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--state_dim", type=int, default=14, help="Robot state dimension (default: 14 for 7-DOF arm with pos+vel)"
    )
    parser.add_argument("--action_dim", type=int, default=7, help="Action dimension (default: 7 for 7-DOF arm)")
    parser.add_argument("--num_actions", type=int, default=16, help="Number of actions in sequence (default: 16)")
    parser.add_argument("--num_past_states", type=int, default=4, help="Number of past states to use (default: 4)")
    parser.add_argument(
        "--hidden_dim", type=int, default=256, help="Hidden dimension (default: 256, comparable to depth networks)"
    )
    ...
    # Create model
    print("Creating Diffusion Policy model...")
