...
static Value getUnpromotedInput(
	Type unpromotedType, Type promotedType, Value promotedResult) {
	...
	if (auto extSIOp = promotedResult.getDefiningOp<arith::ExtSIOp>()) {
		...
		if (cast<VectorType>(extInput.getType()).getElementType() ==
			unpromotedType) {
			return extInput;
		}
	}
	...
	// If it wasn't an integer extension or a float extension, we can't handle
	// it.
	return nullptr;
}
...
static Value extract1DSlice(PatternRewriter &rewriter, Location loc,
	...
	return vector::ExtractStridedSliceOp::create(
		rewriter, loc, input, offsets, dstVecType.getShape(), strides);
}

// Helper to extract an element of a 1D vector.
static Value extract(
	PatternRewriter &rewriter, Location loc, Value input, int position) {
...
static Value flatten(PatternRewriter &rewriter, Location loc, Value vector) {
	...
	VectorType dstType = VectorType::get(
		inputVecType.getNumElements(), inputVecType.getElementType());
...
struct MMTKernel {
	...
	int8_t lhsRegSize = 0;
	// Number of RHS elements fitting in the type of register to be used for
	// RHS.
	int8_t rhsRegSize = 0;
	// Number of Accumulator elements fitting  in the type of register to be
	// used for the accumulator.
	int8_t accRegSize = 0;
	...
	// Register operands for the LHS, RHS and Accumulator are to be referenced
	// as
	// $(lhs:<i>), $(rhs:<i>), $(acc:<i>) respectively, where i is a decimal
	// integer specifying the i-th register for each case (numbered
	// independently, so each starts at 0).
	const char *asmImpl = nullptr;
	// If not null, points to the clobbers list, i.e. the list of registers
	// that the compiler will reserve for this inline asm block's use, in
	// addition to the ones implicitly allocated for the declared inputs and
	// outputs. Using C inline_asm syntax: comma-separated list of raw register
	// names e.g. "v14,v15"
...
class MMTKernelGenerator {
  public:
	...
	SmallVector<Value> generate(PatternRewriter &rewriter, Location loc,
		ArrayRef<Value> lhs, ArrayRef<Value> rhs, ArrayRef<Value> acc) {
		...
		// In the future we may have alternate generator paths, e.g. 1D
		// intrinsics or other asm paths with a different interface, e.g.
		// handling also the memory load accesses.
  ...
  private:
	...
	// This avoids stack bouncing by using the backend's native subvector
	// insertion.
	Value castFixedToScalable(PatternRewriter &rewriter, Location loc,
		...
		// FIX: Pass scalableType directly (not TypeRange) and wrap string in
		// StringAttr
		auto callOp = rewriter.create<LLVM::CallIntrinsicOp>(loc,
	...
	Value castScalableToFixed(PatternRewriter &rewriter, Location loc,
		...
		// FIX: Pass fixedType directly (not TypeRange) and wrap string in
		// StringAttr
		auto callOp = rewriter.create<LLVM::CallIntrinsicOp>(loc,
	...
	SmallVector<Value> generateSpacemiT(PatternRewriter &rewriter, Location loc,
		...
		if (isFloat) {
			// --- FP8/FP16 Path ---
			scalableAccType =
				VectorType::get({4}, rewriter.getF16Type(), {true});
			intrName = "llvm.riscv.smt.vfmadot";
			scalableInputType =
				VectorType::get({8}, rewriter.getIntegerType(8), {true});
			vlVal = 16;
		} else {
			// --- Integer Path ---
			scalableAccType =
				VectorType::get({4}, rewriter.getIntegerType(32), {true});
		...
		// If inputs are F8, we bitcast them to I8 *Fixed* vectors first to
		// match the bitwidth expected by the castFixedToScalable helper which
		// expects bits to match.
		if (isFloat) {
			auto i8FixedType =
				VectorType::get(cast<VectorType>(lhsVal.getType()).getShape(),
					rewriter.getIntegerType(8));
			if (lhsVal.getType() != i8FixedType) {
				lhsVal =
					rewriter.create<LLVM::BitcastOp>(loc, i8FixedType, lhsVal);
				rhsVal =
					rewriter.create<LLVM::BitcastOp>(loc, i8FixedType, rhsVal);
			}
		}

		// 3. Cast to Scalable (No Stack Bounce)
		Value lhsScalable =
			castFixedToScalable(rewriter, loc, lhsVal, scalableInputType);
		Value rhsScalable =
			castFixedToScalable(rewriter, loc, rhsVal, scalableInputType);

		// Accumulator handling
		// For FP16 accumulator, standard vector types match.
		// For Int32 accumulator, standard types match.
		Value accScalable =
			castFixedToScalable(rewriter, loc, accVal, scalableAccType);

		// 4. Set VL
		Value vl = rewriter.create<LLVM::ConstantOp>(
			loc, rewriter.getI64Type(), rewriter.getI64IntegerAttr(vlVal));

		// 5. Call SpacemiT Intrinsic
		// FIX: Pass scalableAccType directly (not TypeRange) and wrap string in
		// StringAttr
		auto callOp = rewriter.create<LLVM::CallIntrinsicOp>(loc,
		...
		// 6. Extract Result
		Value resFixed = castScalableToFixed(rewriter, loc, callOp.getResult(0),
			cast<VectorType>(accVal.getType()));

		return {resFixed};
	}

	SmallVector<Value> generateRVVStandardIntrinsics(PatternRewriter &rewriter,
		Location loc, ArrayRef<Value> lhs, ArrayRef<Value> rhs,
		ArrayRef<Value> acc) {
		...
		VectorType scalableI8 = VectorType::get(
			{scalableCount}, rewriter.getIntegerType(8), {true});
		VectorType scalableI16 = VectorType::get(
			{scalableCount}, rewriter.getIntegerType(16), {true});
		VectorType scalableI32 = VectorType::get(
			{scalableCount}, rewriter.getIntegerType(32), {true});

		Value rhsScalable =
			castFixedToScalable(rewriter, loc, rhs[0], scalableI8);
		Value vl = rewriter.create<LLVM::ConstantOp>(
		...
		// (vwadd.wv). This minimizes vtype switching and allows better
		// scheduling.
		for (int64_t i = 0; i < kernel.m0; ++i) {
			...
			auto mulOp = rewriter.create<LLVM::CallIntrinsicOp>(loc,
				scalableI16, rewriter.getStringAttr("llvm.riscv.vwmul"),
		...
		for (int64_t i = 0; i < kernel.m0; ++i) {
			...
			auto addOp = rewriter.create<LLVM::CallIntrinsicOp>(loc,
				scalableI32, rewriter.getStringAttr("llvm.riscv.vwadd.w"),
				ValueRange{accScalable, accScalable, widenedProducts[i], vl});
			Value resultFixed = castScalableToFixed(rewriter, loc,
				addOp.getResult(0), cast<VectorType>(acc[i].getType()));
	...
	class Constraints {
	  ...
	  public:
		...
		void add(Kind kind, const std::string &constraintCode) {
			switch (kind) {
				...
				case Kind::InputOutput:
					// An input represented by a number `i` is a tied input,
					// tied to the i-th output.
		...
		std::string toString() {
			return llvm::join(llvm::concat<std::string>(
								  outputs, inputs, tiedInputs, clobbers),
				",");
		}
	};
	// Helper for generateAsm. Performs some pre-processing of the kernel's
	// asmImpl. Refer to the comment on kernel::asmImpl.
	void generateAsmCodeAndConstraints(
		std::string &code, std::string &constraintsString) {
		assert(code.empty());
		...
		auto processOperands = [&](Constraints::Kind constraintKind,
			...
			for (int unprocessedIdx = 0; unprocessedIdx < count;
				...
				// Example:   $(lhs:1)   =>   $5
				replaceAllSubstrsInPlace(code,
					llvm::formatv("$({}:{})", name, unprocessedIdx),
	...
	SmallVector<Value> generateAsm(PatternRewriter &rewriter, Location loc,
		...
		// First the input operands. Then the input-output operands, which, as
		// far as input constraints are concerned, are *tied* inputs, i.e. refer
		// to the outputs that we list earlier in the constraints string. This
		// is why us passing the inputs BEFORE the input-outputs here actually
		// matches us listing the inputs AFTER the outputs (but BEFORE the
		// tied-inputs) in the constraints string. Not confusing at all!
		...
		LLVM::InlineAsmOp asmOp = LLVM::InlineAsmOp::create(rewriter, loc,
			returnType, inputs, code, constraints,
		...
		for (int i = 0; i < kernel.accRegs; ++i) {
			...
			resVec.push_back(LLVM::ExtractValueOp::create(
				rewriter, loc, asmOp.getRes(), position));
		}
		return resVec;
	}
};
...
class MMTCustomKernelPattern : public OpRewritePattern<vector::ContractionOp> {
  ...
  public:
	...
	LogicalResult matchAndRewrite(vector::ContractionOp contractionOp,
		...
		Value unpromotedLhs = getUnpromotedInput(
			lhsElemType, accElemType, contractionOp.getLhs());
		Value unpromotedRhs = getUnpromotedInput(
			rhsElemType, accElemType, contractionOp.getRhs());
		if (!unpromotedLhs || !unpromotedRhs) {
			return failure();
		}
		// Prepare the dense array attribute that will be used as the
		// initializer for the destination accumulator vector, before actual
		// values are inserted into it. We do this early because here we need to
		// validate that the destination scalar type is one that we know how to
		// handle.
		VectorType flatAccVectorType = VectorType::get(
			{accType.getNumElements()}, accType.getElementType());
		;
		...
		if (accElemType.isSignlessInteger()) {
			...
			resultInitializer =
				DenseFPElementsAttr::get(flatAccVectorType, 0.f);
		} else {
			return failure();
		}
		...
		auto sliceIntoRegVectors = [&](int regsCount, VectorType regVectorType,
			...
			for (int i = 0; i < regsCount; ++i) {
				regVectors.push_back(extract1DSlice(
					rewriter, loc, regVectorType, src, i * regSize));
			}
			return regVectors;
		};
		...
		SmallVector<Value> lhsRegVectors = sliceIntoRegVectors(
			kernel.lhsRegs, lhsRegVectorType, flatLhsForKernel);
		SmallVector<Value> rhsRegVectors = sliceIntoRegVectors(
			kernel.rhsRegs, rhsRegVectorType, flatRhsForKernel);
		...
		Value result = arith::ConstantOp::create(
			rewriter, loc, flatAccVectorType, resultInitializer);
		int accRegNumElements = accRegVectorType.getNumElements();
		for (int i = 0; i < kernel.accRegs; ++i) {
			result = vector::InsertStridedSliceOp::create(rewriter, loc,
				resRegVectors[i], result,
				std::array<int64_t, 1>{accRegNumElements * i},
				std::array<int64_t, 1>{1});
		}
		// Cast the result from 1D to 2D and replace the original
		// vector.contract.
		rewriter.replaceOpWithNewOp<vector::ShapeCastOp>(
			contractionOp, accType, result);
		return success();
	}
};
	...
	LogicalResult matchAndRewrite(vector::ContractionOp contractionOp,
		...
		std::array<Value, 16> accChunks;
		{
			...
			for (int row = 0; row < 8; ++row) {
				auto accRow = vector::ExtractOp::create(
					rewriter, loc, acc, ArrayRef<int64_t>{row});
		...
		auto int8x4x4VType =
			VectorType::get({4, 4}, rewriter.getIntegerType(8));
		auto extract4x4 = [&](Value in, int rowOffset, int colOffset) {
			auto chunk = vector::ExtractStridedSliceOp::create(rewriter, loc,
				in, ArrayRef<int64_t>{rowOffset, colOffset},
				ArrayRef<int64_t>{4, 4}, ArrayRef<int64_t>{1, 1});
			assert(chunk.getType() == int8x4x4VType);
			return chunk;
		};

		std::array<Value, 2> lhsHalves = {
			extract4x4(inLhs, 0, 0), extract4x4(inLhs, 4, 0)};
		std::array<Value, 2> rhsHalves = {
			extract4x4(inRhs, 0, 0), extract4x4(inRhs, 4, 0)};
		...
		auto sdot = [&](Value acc, Value a, Value b, int64_t lane) -> Value {
			auto bReplicatedLane = vector::ShuffleOp::create(rewriter, loc, b,
				int8Zero4x4, ArrayRef<int64_t>{lane, lane, lane, lane});

			return arm_neon::Sdot2dOp::create(
				rewriter, loc, int32x4VType, acc, a, bReplicatedLane);
		};
		...
		// Put the results back in the accumulator
		{
			...
			for (int row = 0; row < 8; ++row) {
				for (int col = 0; col < 8; col += 4) {
					acc = vector::InsertStridedSliceOp::create(rewriter, loc,
						dstChunks[idx++], acc, ArrayRef<int64_t>{row, col},
						ArrayRef<int64_t>{1});
				}
			}
		}
	...
	if (isRISCV(targetConfig)) {
		...
		if (hasFeature(targetConfig, "+xsmtvdot")) {
			patterns.add<MMTCustomKernelPattern>(
				context, MMTKernel_SpacemiT_Int8());
			patterns.add<MMTCustomKernelPattern>(
				context, MMTKernel_SpacemiT_FP8());
		} else if (hasFeature(targetConfig, "+v")) {
			patterns.add<MMTCustomKernelPattern>(
				context, MMTKernel_RVV_8x16x1_Int8());
			patterns.add<MMTCustomKernelPattern>(
				context, MMTKernel_RVV_7x16x1_Int8());
		}
	}
	if (isAArch64(targetConfig)) {
		// TODO: add a "kernel benefit" system whereby if two kernels are
		// available for the same shape and same data types, the fastest one (ie
		// the one using the most powerful available SIMD instructions) is
		// selected. This is incidentally not needed at the moment because
		// currently no two kernels share the exact same shape and data types.
		patterns.add<MMTCustomKernelPattern>(
		...
		if (hasFeature(targetConfig, "+dotprod")) {
			if (preferIntrinsicsOverAsm(targetConfig)) {
				patterns.add<MMT_8x4x8_i8i8i32_Aarch64Dotprod_Intrinsics>(
					context);
			} else {
				patterns.add<MMTCustomKernelPattern>(context,
					MMTKernel_8x4x8_i8i8i32_Aarch64Dotprod_InlineAsm());
				patterns.add<MMTCustomKernelPattern>(context,
					MMTKernel_8x4x1_i8i8i32_Aarch64Dotprod_InlineAsm());
			}
		}
