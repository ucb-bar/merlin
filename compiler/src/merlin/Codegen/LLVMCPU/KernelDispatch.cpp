...
static llvm::cl::opt<int> clDefaultDistTileSize(
	"iree-llvmcpu-distribution-size",
...
enum class VectorPreProcStrategy {
	// Peel iterations from the vector dimensions so that they become multiple
	// of
	// the vector length.
	Peeling,
	// Compute vector dimensions assuming vector masking support. Vector sizes
	// may
	// be rounded up to the nearest power of two and out-of-bounds elements
	// would
	// be masked-out.
	Masking,
...
static llvm::cl::opt<VectorPreProcStrategy> clPProcStrategy(
	...
	llvm::cl::values(
		...
		clEnumValN(VectorPreProcStrategy::Masking, "mask",
		...
		clEnumValN(VectorPreProcStrategy::None, "none",
			"Do not apply any vectorization pre-processing transformation.")));

// TODO(dcaballe): Move operator<< to DebugUtils.h.
static llvm::raw_ostream &operator<<(
	llvm::raw_ostream &os, const VectorPreProcStrategy &strategy) {
	switch (strategy) {
...
template <typename T>
static llvm::raw_ostream &operator<<(
	llvm::raw_ostream &os, const llvm::SmallVectorImpl<T> &vector) {
...
/// fallback.
static int64_t getVectorSize(
	mlir::FunctionOpInterface entryPointFn, unsigned byteWidth) {
	return getNativeVectorSizeInBytes(entryPointFn) / byteWidth;
}
static int64_t getVectorSize(
	mlir::FunctionOpInterface entryPointFn, ShapedType shapedType) {
...
static SmallVector<int64_t> getMinTilingSizesForEachDim(
	...
	for (auto [index, map] : llvm::enumerate(op.getIndexingMapsArray())) {
		// Check the fastest varying dimension of the operand. Set the vector
		// size of the corresponding loop to the vector size.
...
// threads.
static void reduceDistributionWorkgroups(ArrayRef<int64_t> workload,
	...
	while (numWorkgroups > numWorkgroupsLimit && currDim > 0) {
		...
		int64_t vectorSize =
			vectorSizeHints ? vectorSizeHints.value()[index] : 0;

		// Chech if it's the ideal size with vector size hint. And skip if the
		// new size will break the ideal size.
		if (vectorSize > 1 &&
	...
	for (auto i : llvm::seq<unsigned>(0, distributedTileSizes.size())) {
		if (distributedTileSizes[i] == 0 ||
			ShapedType::isDynamic(workload[i])) {
			continue;
		}
...
static SmallVector<int64_t> getDefaultDistributionTileSizes(
	...
	for (auto i : llvm::seq<size_t>(0, numDims)) {
		...
		if (vectorSize > 1) {
			// Pick the factor of dim which is closest to the target tile size
			// and is a multiplier of vector size.
		...
		// Fallback to power of 2 if there's no hint or can't find the ideal
		// size.
		if (vectorSize <= 1 || candidateTileSize == 1) {
			candidateTileSize = std::max(
				static_cast<int64_t>(llvm::bit_floor<uint64_t>(targetSize)),
				minTileSizes[i]);
		}

		// Limit the workload per workgroup to the default being the max to keep
		// the work per invocation reasonable.
		distributedTileSizes[i] = std::min(candidateTileSize, maxTileSizes[i]);
	}

	reduceDistributionWorkgroups(
		workload, distributedTileSizes, maxTileSizes, vectorSizeHints);
...
static int64_t getMaxDistributionTileSize(int64_t lb, int64_t ub,
	int64_t maxSize, int64_t vectorSize, bool allowIncompleteTile = false) {
...
static int64_t getMaxVectorTileSize(int64_t numElem, int64_t tileSize,
	int64_t vectorSize, bool enforcePowerOfTwo = false) {
...
static void limitVectorTileSizes(SmallVectorImpl<int64_t> &vecTileSizes,
	int64_t eachOperandMaxTileBits, int64_t allOperandsMaxTileBits,
	TypeRange operandTypes, ArrayRef<AffineMap> indexingMaps,
	...
	for (auto bits : operandElemBits) {
		if (bits > eachOperandMaxTileBits) {
			...
			// satisfied. Early-returning. This might conceivably happen due to
			// the tail-recursion in this function, where eachOperandMaxTileBits
			// is halved.
			return;
		}
	...
	// For each loop, we must make sure that if it's the inner-most dimension
	// that concerns an operand with a not-multiple-of-8-bit element type, we
	// preserve the requirement that the inner-most tile dimension is a multiple
	// of 8 bits. For example, if the element type is `i2`, the minimum inner
	// tile size is 8/2 = 4. If the element type if i6, since gcd(8, 6) = 2, we
	// are in the same situation and the minimum inner tile size is again 8/2
	// = 4.
		...
		for (int i : llvm::seq<int>(0, numOperands)) {
			if (isInnerMostDimThatMapIsFunctionOf(indexingMaps[i], loopNum)) {
				if (operandElemBits[i] % 8) {
					minVecTileSizes[loopNum] =
						std::max(minVecTileSizes[loopNum],
							8 / std::gcd(8, operandElemBits[i]));
				}
			}
		}
	}
	...
	for (int loopNum : llvm::reverse(llvm::seq<int>(0, numLoops))) {
		for (int i : llvm::seq<int>(0, numOperands)) {
			...
			int64_t maxVal = std::max(
				minVecTileSizes[loopNum], eachOperandMaxTileBits / tileBits[i]);
			int64_t adjustedVal = std::min(oldVal, maxVal);
			// If we are adjusting a tile size, make sure that the adjusted tile
			// size is a power-of-two, as introducing non-power-of-two tile
			// sizes can be a major performance regression, e.g. a 30% e2e
			// regression in MobileNetV2 just from a single occurrence in a
			// DepthwiseConv (example discussed below in this comment).
			//
			...
			// no-operation (e.g. from 8 to 5 back to 8) and then would trigger
			// the tail recursion with halved eachOperandMaxTileBits, which
			// would result in more aggressive tile size limitation. E.g. in
			// MobileNetV2 on AVX-512 we have a DepthwiseConv with
			//   vecTileSizes = [1 1 8 16 1 3],
			// without this adjustment, this becomes
			//   vecTileSizes = [1 1 5 16 1 3].
			// If we round this 5 back to 8, letting this function tail-recurse,
			// we end up with this much narrower final result:
			//   vecTileSizes = [1 1 2 16 1 3].
			// It is best to round this 5 to 4, obtaining
			//   vecTileSizes = [1 1 4 16 1 3].
			//
			// The above example also shows that some non-power-of-two tile
			// sizes are not getting adjusted, and should not be rounded to
			// nearest power-of-two: the inner-most dimension size 3 above.
			...
			if (oldVal == 0) {
				// Skip updating tile sizes of 0, we want to preserve these
				// values.
				if (!bounds.empty()) {
					// If we have enough information about the upper bound of
					// this tile size, use it.
	...
	if (llvm::sum_of(tileBits) > allOperandsMaxTileBits) {
		limitVectorTileSizes(vecTileSizes, eachOperandMaxTileBits / 2,
			allOperandsMaxTileBits, operandTypes, indexingMaps, bounds);
	}
}
...
static void limitVectorTileSizes(Operation *inputOp,
	SmallVectorImpl<int64_t> &vecTileSizes, int64_t eachOperandMaxTileBits,
...
static int getRegisterSpaceBitsIfKnown(IREE::HAL::ExecutableTargetAttr target) {
	...
	if (isX86(targetConfig)) {
		...
		// 32 NEON/SVE registers (at least 128-bit each, returns the base size
		// for SVE).
		return 32 * 128;
	} else {
...
// SVE).
static void limitVectorTileSizes(
	Operation *op, SmallVectorImpl<int64_t> &vecTileSizes) {
	if (int registerSpaceBits = getRegisterSpaceBitsIfKnown(
			IREE::HAL::ExecutableTargetAttr::lookup(op))) {
		limitVectorTileSizes(
			op, vecTileSizes, registerSpaceBits, registerSpaceBits);
	}
}
...
struct DistributionHeuristicConfig {
	...
	// On the dimensions where the hints != 1, it will try to find the tile
	// sizes which are multipliers of the hints.
...
static SmallVector<int64_t> getDefaultDistributedLevelTileSizes(
	Operation *op, const DistributionHeuristicConfig &config) {
	SmallVector<int64_t> lbs, ubs;
	getRangeBounds(cast<TilingInterface>(op), lbs, ubs);
	int64_t numLoops = lbs.size();

	assert((config.minTileSizes.empty() ||
			   config.minTileSizes.size() == numLoops) &&
		"min tile sizes should be empty or equal to the number of loops");
	assert((config.maxTileSizes.empty() ||
			   config.maxTileSizes.size() == numLoops) &&
	...
	SmallVector<int64_t> distributedTileSizes =
		getDefaultDistributionTileSizes(lbs, ubs, adjustedMinTileSizes,
			adjustedMaxTileSizes, adjustedVectorSizeHints);
	...
	for (auto i : llvm::seq<unsigned>(0, distributedTileSizes.size())) {
		...
		distributedTileSizes[i] =
			getMaxDistributionTileSize(lbs[i], ubs[i], distributedTileSizes[i],
				adjustedMinTileSizes[i], config.allowIncompleteTile);
	}
...
/// reduction loops.
static void splitParallelAndReductionTiles(Operation *op,
...
static void setAlwaysVectorizeSizes(
	linalg::LinalgOp op, SmallVectorImpl<int64_t> &vecTileSizes) {
...
class LoweringConfigGenerator {
  public:
	...
	void setVectorTileSizes(
		ArrayRef<int64_t> tileSizes, ArrayRef<bool> scalableFlags = {}) {
	...
	IREE::CPU::LoweringConfigAttr generateCPULoweringConfig() {
		...
		if (!distTileSizes.empty()) {
			appendLoweringConfigLevelAttr(items,
				IREE::CPU::TilingLevel::DistributionTiles, distTileSizes);
		...
		if (!cacheTileSizes.empty()) {
			...
			splitParallelAndReductionTiles(
				rootOp, parallelTileSizes, reductionTileSizes);
			appendLoweringConfigLevelAttr(items,
				IREE::CPU::TilingLevel::CacheParallelTiles, parallelTileSizes);
			appendLoweringConfigLevelAttr(items,
  ...
  private:
	...
	/// `items`, if it is not a NOP config. E.g., if all the tile sizes are
	/// zeros, it means no tiling at all. Only the distribution tiling level is
...
class IterationDimTracker {
  public:
	...
	/// Returns all global dimension indices associated with the given
	/// operation.
  ...
  private:
	...
	void buildDimMapping() {
		...
		for (Operation *op : operations) {
			...
			// - For pack/unpack operations, use an identity mapping, since
			// tiling applies to the outer (unpacked) dimensions.
			// - For all other (unknown) operations, assume an identity mapping
			// for any value whose rank matches the operation’s loop count.
			TypeSwitch<Operation *>(op)
				.Case<IndexingMapOpInterface>([&](auto op) {
					propagateOnIndexingMapOp(
						op, indicesEquivalence, valueToGlobalDimMaps);
				})
				.Case<linalg::PackOp, linalg::UnPackOp>([&](auto op) {
					propagateOnPackUnpackOp(
						op, indicesEquivalence, valueToGlobalDimMaps, numLoops);
				})
				.Default([&](auto op) {
					propagateOnUnknownOp(
						op, indicesEquivalence, valueToGlobalDimMaps, numLoops);
				});
		}
		...
		totalLoopNum = 0;
		for (auto it = indicesEquivalence.begin();
			 it != indicesEquivalence.end(); ++it) {
			if (!(*it)->isLeader()) {
				continue;
			}
	...
	/// d1 * 3 + d3)>`) are ignored because they fall outside the "loop
	/// dimension" concept. Such expressions describe how indices are computed
	/// within the innermost loop body, but they do not directly identify which
	/// loop dimensions correspond or should be tied.
	void propagateOnIndexingMapOp(IndexingMapOpInterface indexingMapOp,
		llvm::EquivalenceClasses<int64_t> &indicesEquivalence,
		llvm::SmallDenseMap<Value, SmallVector<int64_t>>
		...
		for (OpOperand &operand : op->getOpOperands()) {
			...
			for (auto [dim, expr] : llvm::enumerate(map.getResults())) {
				// Stop if the current dimension exceeds the number of mapped
				// ones.
				...
				// Unify the dimension index between the producer and the
				// current op.
	...
	void propagateOnPackUnpackOp(Operation *op,
		llvm::EquivalenceClasses<int64_t> &indicesEquivalence,
		llvm::SmallDenseMap<Value, SmallVector<int64_t>> &valueToGlobalDimMaps,
		int64_t numLoops) {
		for (OpOperand &operand : op->getOpOperands()) {
	...
	/// Ties the dimensions of operations with their operands, if the operand
	/// rank matches the operation’s loop count.
	void propagateOnUnknownOp(Operation *op,
		llvm::EquivalenceClasses<int64_t> &indicesEquivalence,
		llvm::SmallDenseMap<Value, SmallVector<int64_t>> &valueToGlobalDimMaps,
		int64_t numLoops) {
		for (OpOperand &operand : op->getOpOperands()) {
...
static IREE::Codegen::LoweringConfigAttrInterface getNewLoweringConfig(
	MLIRContext *ctx, ArrayRef<IREE::CPU::LoweringConfigLevelInfo> tilingInfo,
	...
	for (auto [level, tileSizes, scalableFlags] : tilingInfo) {
		...
		// definition of root op. An operation that has distribution tile sizes
		// is the root op. Other level can be dropped if all the tile sizes are
		// zeros.
		if (level != IREE::CPU::TilingLevel::DistributionTiles &&
	...
	return IREE::CPU::LoweringConfigAttr::get(
		ctx, DictionaryAttr::get(ctx, newItems));
}
...
static SmallVector<int64_t> getDefaultMatmulCacheSizes(
	linalg::LinalgOp op, bool isQuantized) {
	unsigned numLoops = op.getNumLoops();
...
static LogicalResult setMatmulPeelingRootConfig(
	...
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, op,
		loweringConfig, DispatchLoweringPassPipeline::CPUDoubleTilingExpert,
		/*workgroupSize=*/{}, /*subgroupSize=*/{}, pipelineConfig);
}

static LogicalResult setMatmulRootConfig(mlir::FunctionOpInterface entryPointFn,
	linalg::LinalgOp linalgOp, ArrayRef<int64_t> distTileSizes,
	ArrayRef<bool> inputVecScalableDims, ArrayRef<int64_t> inputVecTileSizes,
	...
	return setOpConfigAndEntryPointFnTranslation(
		entryPointFn, linalgOp, loweringConfig, pipeline);
}

/// Returns default hard-coded vector sizes for a give target. No smartness
/// should be introduced in this utility.
static void getDefaultMatmulVectorSizes(linalg::LinalgOp op, int64_t vectorSize,
	...
	if (targetAttr && isRISCV(targetAttr.getConfiguration())) {
		// RISC-V natively supports scalar x vector operations so we don't have
		// to vectorize dimension k. Vectorizing dimension k results in a vector
		// load and a sequence of vrgather ops to implemement the broadcast
		// explicitly. We should tile and/or unroll that dimension without
		// vectorization, which is not possible right now.
		sizes.append({8, 32, 1});
		return;
	}
...
static FailureOr<Type> nonWideningLinalgElementType(linalg::LinalgOp op) {
	...
	for (Value v : llvm::concat<Value>(
			 op.getRegionInputArgs(), op.getRegionOutputArgs())) {
...
static void getMatmulVectorSizesUsingFullVectorHeuristics(
	...
	for (Type mmType : matmulTypes) {
		...
		if (mmType.isSignlessIntOrFloat()) {
			minSize =
				std::min(minSize, int64_t{mmType.getIntOrFloatBitWidth()});
		}
	}
...
static bool getMatmulSpacemiTVectorSizes(mlir::FunctionOpInterface entryPointFn,
...
static void getMatmulRISCVVectorSizes(mlir::FunctionOpInterface entryPointFn,
	linalg::LinalgOp op, int64_t vectorSize, SmallVectorImpl<int64_t> &sizes,
	SmallVectorImpl<bool> &scalableSizeFlags) {
	if (!sizes.empty()) {
		return;
	}
	...
	bool isWideningI8ToI32 = lhsType.isSignlessInteger(8) &&
		rhsType.isSignlessInteger(8) && outType.isSignlessInteger(32);
	...
	if (isWideningI8ToI32) {
		...
		// Target LMUL2 to keep register pressure lower and allow wider M
		// unroll.
		constexpr int64_t targetLMUL = 2;

		// Calculate N from LMUL target.
		// (32 bytes * 8 bits * 2 LMUL) / 32 bits = 16 elements on VLEN=256.
		int64_t nSize = (nativeVectorSize * kByteSizeInBits * targetLMUL) /
			kAccumulatorBitWidth;
...
/// shapes.
static void getMatmulAArch64SMEVectorSizes(linalg::LinalgOp op,
	SmallVectorImpl<int64_t> &sizes, SmallVectorImpl<bool> &scalableSizeFlags) {
	// Double-check the operation is one that is supported for lowering to
	// ArmSME.
...
static SizesAndScalableFlags getMatmulVectorSizes(
	mlir::FunctionOpInterface entryPointFn, linalg::LinalgOp op,
	...
	if (targetAttr && isAArch64(targetAttr.getConfiguration())) {
		if (isScalableVectorizationEnabled() && !clDisableArmSMETiling &&
			...
			// Note: This may not pick any sizes (which will fallback to the
			// scalable vectorization heuristics below).
			getMatmulAArch64SMEVectorSizes(
				op, matmulTileSizes, matmulScalableFlags);
		}

		// Try to maximize the vector register utilization for all the matmul
		// element types.
		if (matmulTileSizes.empty()) {
			getMatmulVectorSizesUsingFullVectorHeuristics(entryPointFn, op,
				vectorSize, matmulTileSizes, matmulScalableFlags);
		}
	}
	...
	if (targetAttr && isRISCV(targetAttr.getConfiguration()) &&
		...
		if (!isa<linalg::MatmulTransposeBOp, linalg::BatchMatmulTransposeBOp>(
			...
			getMatmulRISCVVectorSizes(entryPointFn, op, vectorSize,
	...
	if (matmulTileSizes.empty()) {
		getDefaultMatmulVectorSizes(
			op, vectorSize, matmulTileSizes, matmulScalableFlags);
	}
	...
	if (numLoops > 3) {
		...
		scalableTileFlags.append(
			matmulScalableFlags.begin(), matmulScalableFlags.end());
	} else {
		tileSizes.append(
			matmulTileSizes.begin() + (3 - numLoops), matmulTileSizes.end());
		scalableTileFlags.append(matmulScalableFlags.begin() + (3 - numLoops),
			matmulScalableFlags.end());
	}

	// For proper 2-D or higher order matmuls, make sure we don't use a tile
	// size greater than the static dim size for dims that are only unrolled,
	// i.e., N and batch dims.
	...
	if (numLoops >= 3) {
		for (int i = 0; i < (numLoops - 2); ++i) {
			...
			// Ad-hoc: Don't attempt to resize scalable tiles when
			// numScalableDims
			// >= 2. For ArmSME (the only current user of 2D scalable vectors),
			// tile sizes must match SME tiles (and cannot be arbitrarily
			// resized).
...
static LogicalResult setContractionRootConfig(
	mlir::FunctionOpInterface entryPointFn, linalg::LinalgOp linalgOp) {
	assert(
		!getLoweringConfig(linalgOp) && "expected lowering_config is not set");
	...
	vectorSize =
		std::min(vectorSize, getVectorSize(entryPointFn, rhsShapedType));
	vectorSize =
		std::min(vectorSize, getVectorSize(entryPointFn, resShapedType));
	bool isQuantized =
	...
	if (targetAttr && isRISCV(targetAttr.getConfiguration()) &&
		hasAnyVFeature(targetAttr.getConfiguration())) {
		LDBG() << "RISC-V Aggressive Distribution: "
		...
		for (auto loopNum :
			...
			if (clEnableRiscvAggressiveDist) {
				...
				distConfig.maxTileSizes[loopNum] =
					std::max(distConfig.maxTileSizes[loopNum],
						distConfig.minTileSizes[loopNum]);
			}
		}
	}
	...
	if (usePeelingPipeline) {
		return setMatmulPeelingRootConfig(entryPointFn, linalgOp, distTileSizes,
			cacheTileSizes, vecScalableFlags, vecTileSizes, vectorSize);
	}

	return setMatmulRootConfig(entryPointFn, linalgOp, distTileSizes,
		vecScalableFlags, vecTileSizes, vectorSize, vecPreProcStrategy);
}
...
static std::optional<SizesAndScalableFlags> getMmt4dInnerTileSizes(
	linalg::LinalgOp op) {
	...
	if (ShapedType::isStaticShape(destShape)) {
		return SizesAndScalableFlags{
			destShape.take_back(2), IREE::Codegen::ScalableTileFlags(2, false)};
	}
	...
	if (!isa_and_present<linalg::FillOp, IREE::TensorExt::DispatchTensorLoadOp>(
			destOp)) {
		LDBG()
			<< "Mmt4d with non-fill or tensor load init buffer, cannot infer "
			   "inner tile sizes!";
		return std::nullopt;
	}
	if (auto fillOp = dyn_cast<linalg::FillOp>(destOp)) {
		...
		if (!emptyOp) {
			LDBG()
				<< "Could not infer inner tile sizes of a scalable mmt4d op!";
			return std::nullopt;
		}
...
static bool adjustVectorSizesForScalableVectorization(linalg::LinalgOp op,
	DictionaryAttr targetConfig, int64_t m0, int64_t n0,
	...
	if (hasAnySVEFeature(targetConfig) && ShapedType::isDynamic(n0)) {
		...
		// dimension, i.e. n1 from the iteration domain ([b, ], m0, n0, k0, m1,
		// n1, k1). The inner M dimension is not considered here, because SVE
		// currently only makes the N dimension scalable.
...
static IREE::Codegen::LoweringConfigAttrInterface getMmt4dLoweringConfig(
	linalg::LinalgOp op, DictionaryAttr targetConfig) {
	DistributionHeuristicConfig distConfig;
	...
	// though that can't be done meaningfully in general, unless specializing
	// the compilation to fine details of the runtime workload including number
	// of threads and cache sizes. Another thing that we need to know and can't
	// really know at compile time is the values of dynamic sizes. Here we have
	// to guess a reasonable default for the reduction dimension size.
	...
	int64_t tileBytes = (M1 == 1 || N1 == 1) ? clNarrowMatmulTileBytes
											 : clGeneralMatmulTileBytes;
	distConfig.maxTileSizes[mmt4dDimBase + 0] = M1 == 1
		? 1
		: getMatmulTileSize(
			  tileBytes, lhsType.getElementTypeBitWidth(), reductionSize, M0);
	distConfig.maxTileSizes[mmt4dDimBase + 1] = N1 == 1
		? 1
		: getMatmulTileSize(
			  tileBytes, rhsType.getElementTypeBitWidth(), reductionSize, N0);
	...
	IREE::Codegen::ScalableTileFlags vecScalableTileFlags(
		mmt4dDimBase + 6, false);
	bool scalableTilesFound = false;
	// If scalable vectorization is enabled, adjust the vector tile sizes and
	// the corresponding scalable flags.
...
static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, linalg::Mmt4DOp mmt4dOp) {
	assert(
		!getLoweringConfig(mmt4dOp) && "expected lowering_config is not set");
	...
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, mmt4dOp,
		getMmt4dLoweringConfig(mmt4dOp, targetConfig),
...
/// root op
static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, linalg::BatchMmt4DOp batchMmt4dOp) {
	...
	DictionaryAttr targetConfig =
		targetAttr ? targetAttr.getConfiguration() : nullptr;
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, batchMmt4dOp,
...
static SmallVector<int64_t> getPackVectorTileSizes(
	mlir::FunctionOpInterface entryPointFn, linalg::PackOp op) {
	...
	if (op.getSourceType().getElementType().isF16()) {
		// We adjust the vector size to half to use the same lowering strategy
		// as f32.
...
static LogicalResult setRootConfig(
	...
	// tiles or if dynamic tile sizes are found, but scalable tile sizes cannot
	// be inferred.
	...
	// The default function aims to returns the number of workload per
	// workgroup, but it does not know that it is working on packed domain. We
	// need to take inner tile sizes into account and adjust the distribution
	// tile sizes.
	...
	// Dynamic inner tiles lead to unbounded stack allocation (which is
	// introduced by tensor.pad op), so we do not decompose the cases. The x86
	// and risc-v backends prefer to not decompose the ops.
	DictionaryAttr pipelineConfig;
	...
	SmallVector<int64_t> vecTileSizes =
		getPackVectorTileSizes(entryPointFn, op);
	LoweringConfigGenerator generator(op);
	generator.setDistributionTileSizes(distTileSizes);
	...
	IREE::CPU::LoweringConfigAttr loweringConfig =
		generator.generateCPULoweringConfig();
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, op,
		loweringConfig, DispatchLoweringPassPipeline::CPUDataTiling,
		/*workgroupSize=*/{},
		/*subgroupSize=*/{}, pipelineConfig);
}

static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, linalg::UnPackOp op) {
	DistributionHeuristicConfig distConfig;
	...
	// Dynamic inner tiles lead to unbounded stack allocation (which is
	// introduced by tensor.pad op), so we do not decompose the cases. The x86
	// and risc-v backends, as well as the aarch64 backend in case of scalable
	// inner tiles, prefer to not decompose the ops.
...
static LogicalResult setRootConfig(mlir::FunctionOpInterface entryPointFn,
	...
	FailureOr<IREE::LinalgExt::AttentionOpDetail> maybeOpInfo =
		IREE::LinalgExt::AttentionOpDetail::get(attnOp.getQueryMap(),
			attnOp.getKeyMap(), attnOp.getValueMap(), attnOp.getOutputMap());
	...
	{
		// TODO: These aren't the best heuristics and we can do much much better
		// if we reuse the matmul heuristics here. limitVectorTileSizes doesn't
		// try to convert the matmul into a square matmul, which is bad.

		// Limit vector sizes based on register space.
		if (int64_t registerSpaceBits =
				getRegisterSpaceBitsIfKnown(targetAttr)) {
			SmallVector<Type> pvOperandTypes(
				{Float32Type::get(attnOp.getContext()),
					attnOp.getValue().getType(),
					Float32Type::get(attnOp.getContext())});
			SmallVector<AffineMap> pvMaps({opInfo.getSMap(),
			...
			SmallVector<Type> qkOperandTypes(
				{attnOp.getQuery().getType(), attnOp.getKey().getType(),
					Float32Type::get(attnOp.getContext())});
		...
		if (targetAttr) {
			...
			// The large vector size check also has a restriction that the
			// entire iteration space of a contraction cannot exceed the
			// maxVectorSizeBits, so add that restriction here.
			//
			// Add restriction on QK matmul.
			auto identity = AffineMap::getMultiDimIdentityMap(
				opInfo.getDomainRank(), attnOp.getContext());
			...
			limitVectorTileSizes(vecTileSizes, maxVectorSizeBits,
				maxVectorSizeBits, operandTypes, maps, vecTileSizeBounds);
		}
	}
	...
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, attnOp,
		loweringConfig,
...
/// root op.
static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, IREE::LinalgExt::FftOp fftOp) {
	...
	SmallVector<int64_t> distTileSizes = getDefaultDistributedLevelTileSizes(
		fftOp, DistributionHeuristicConfig{});
	...
	if (distTileSizes.size() >= rank && distTileSizes[rank - 1] != 0) {
		...
		if (matchPattern(fftOp.getStage(), m_ConstantInt(&value))) {
			...
			distTileSizes[rank - 1] = std::max(distTileSizes[rank - 1],
				static_cast<int64_t>(clDefaultDistTileSize));
		} else {
			return fftOp.emitOpError(
				"non-constant stage might not work for fft op");
		}
	}
	...
	IREE::CPU::LoweringConfigAttr loweringConfig =
		generator.generateCPULoweringConfig();
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, fftOp,
		loweringConfig,
		DispatchLoweringPassPipeline::CPULinalgExtTileAndVectorize);
}
...
static LogicalResult setWinogradRootConfig(
	mlir::FunctionOpInterface entryPointFn, WinogradOp winogradOp) {
	static_assert(
	...
	IREE::CPU::LoweringConfigAttr loweringConfig =
		generator.generateCPULoweringConfig();
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, winogradOp,
		loweringConfig,
		DispatchLoweringPassPipeline::CPULinalgExtTileAndVectorize);
}

static void setVectorTileSizes(linalg::LinalgOp op,
	ArrayRef<int64_t> distTileSizes, ArrayRef<int64_t> minTileSizes,
	...
	for (auto loopNum : llvm::seq<int>(0, numLoops)) {
		if (distTileSizes[loopNum]) {
			vecTileSizes[loopNum] = getMaxVectorTileSize(distTileSizes[loopNum],
				minTileSizes[loopNum], minTileSizes[loopNum],
				/*enforcePowerOfTwo=*/vecPreProcStrategy ==
					VectorPreProcStrategy::Masking);
		} else {
			// If the distribution tile size is zero, and static loop range is 0
			// as well, set the tile sizes here to zero as well.
...
static LogicalResult setDefaultGenericOpRootConfig(
	...
	const TargetMLTransformInfo &targetMLTransInfo) {
	assert(
		!getLoweringConfig(genericOp) && "expected lowering_config is not set");
	...
	if (genericOp.hasPureTensorSemantics()) {
		...
		if (vecPreProcStrategy == VectorPreProcStrategy::Peeling) {
			pipelineConfig =
				getPipelineConfWithPeelingAttr(genericOp.getContext());
		}
	} else {
		passPipeline =
...
static void getTransposeX86VectorSizes(linalg::GenericOp genericOp,
	IREE::HAL::ExecutableTargetAttr targetAttr, ArrayRef<int64_t> minTileSizes,
	...
	if (llvm::count_if(
			minTileSizes, [](int64_t tileSize) { return tileSize > 1; }) != 2) {
		// Transpose patterns are not applicable if vectorizing more or less
		// than two dims.
		return;
	}
	...
	if (hasAVX512fFeature(targetAttr.getConfiguration()) &&
		llvm::all_of(minTileSizes,
			[](int64_t tileSize) { return tileSize == 1 || tileSize >= 16; })) {
		targetVectorSize = 16;
	}
...
static void getTransposeAArch64VectorSizes(linalg::GenericOp genericOp,
	IREE::HAL::ExecutableTargetAttr targetAttr, SmallVectorImpl<int64_t> &sizes,
	SmallVectorImpl<bool> &scalableFlags) {
...
static std::optional<SizesAndScalableFlags> getTransposeVectorSizes(
	mlir::FunctionOpInterface entryPointFn, linalg::GenericOp genericOp,
	const LinalgOpInfo &linalgOpInfo,
	const TargetMLTransformInfo &targetMLTransInfo) {
	SmallVector<int64_t> tileSizes;
	...
	if (targetAttr && isX86(targetAttr.getConfiguration())) {
		SmallVector<int64_t> minTileSizes = getMinTilingSizesForEachDim(
			entryPointFn, genericOp, linalgOpInfo, targetMLTransInfo);
		getTransposeX86VectorSizes(
			genericOp, targetAttr, minTileSizes, tileSizes);
	} else if (targetAttr && isAArch64(targetAttr.getConfiguration())) {
		getTransposeAArch64VectorSizes(
			genericOp, targetAttr, tileSizes, scalableFlags);
	}
	...
	// If scalable flags are empty, assume target doesn't care about
	// scalability.
...
static LogicalResult setElementwiseGenericOpRootConfig(
	...
	// selection logic in getMaxTileSize. We should either port it there or
	// remove it. Adjust the number of workload per workgroup to at least 4096.
	// This prevents the runtime overheads domiating the execution time. The
	// number is derived from experimients. We should be able to make it related
	// to target.
	...
	// Adjust tiling sizes of vector levels to avoid large unroll factors. Most
	// of the cases are f32 and i32, so we divide it by 4.
...
/// CPUDoubleTilingExpert pipeline.
static LogicalResult setGenericRootConfig(
	mlir::FunctionOpInterface entryPointFn, linalg::GenericOp genericOp,
	const LinalgOpInfo &linalgOpInfo,
	const TargetMLTransformInfo &targetMLTransInfo) {
	assert(
...
static LogicalResult setConvRootConfig(mlir::FunctionOpInterface entryPointFn,
	...
	int64_t vectorSize, VectorPreProcStrategy vecPreProcStrategy) {
	...
	SmallVector<int64_t> vecTileSizes(
		targetTileSizes.begin(), targetTileSizes.end());
	for (auto i : llvm::seq<unsigned>(0, vecTileSizes.size())) {
		auto tileSize = distTileSizes[i] ? distTileSizes[i] : shapes[i];
		// If the tile size is intended to be 1, do not adjust it to
		// `vectorSize`. The ops will be decomposed to lower-rank named ops.
		if (vecTileSizes[i] != 1) {
			vecTileSizes[i] =
	...
	IREE::CPU::LoweringConfigAttr loweringConfig =
		generator.generateCPULoweringConfig();
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, convOp,
		loweringConfig,
...
static SmallVector<int64_t> getNhwcConvVectorSizes(
	mlir::FunctionOpInterface entryPointFn, linalg::ConvolutionOpInterface op,
	int64_t vectorSize) {
	bool isSupported =
		is2DConvOp(op) || is2DDepthConvOp(op) || is2DPoolingOp(op);
	(void)isSupported;
...
static LogicalResult setConvInterfaceRootConfig(
	...
	return setConvRootConfig(entryPointFn,
		cast<linalg::LinalgOp>(convOp.getOperation()), targetTileSizes,
		vectorSize, vecPreProcStrategy);
}

static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, tensor::PadOp padOp) {
	SmallVector<int64_t> lbs, ubs;
	...
	if (ShapedType::isStatic(ubs.back())) {
		distConfig.vectorSizeHints.back() =
	...
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, padOp,
		loweringConfig, DispatchLoweringPassPipeline::CPUDoubleTilingExpert);
}
...
/// `TiledOpInterface`.
static LogicalResult setRootConfig(
	mlir::FunctionOpInterface entryPointFn, TilingInterface op) {
	assert(!getLoweringConfig(op) && "expected lowering_config is not set");
	SmallVector<int64_t> distTileSizes =
	...
	LDBG() << "Set lowering_config for tensor.pad op: " << loweringConfig;
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, op,
		loweringConfig, DispatchLoweringPassPipeline::CPUDefault);
}

/// Redirects to methods that set the configuration based on operation type.
static LogicalResult setRootConfigImpl(mlir::FunctionOpInterface entryPointFn,
	Operation *op, const TargetMLTransformInfo &targetMLTransInfo) {
		...
		TypeSwitch<Operation *, LogicalResult>(op)
			.Case<IREE::LinalgExt::CustomOp>([&](auto op) {
				return setDefaultCustomOpLoweringConfig(
					entryPointFn, op, initCPULaunchConfig);
			})
			...
			.Case<IREE::LinalgExt::WinogradFilterTransformOp,
				...
				IREE::LinalgExt::WinogradOutputTransformOp>([&](auto op) {
				return setWinogradRootConfig(entryPointFn, op);
			})
...
static void scaleAndPermutateTilingForPackOp(linalg::PackOp packOp,
	SmallVector<int64_t> &tileSizes, SmallVector<bool> &scalableFlags) {
...
///    sizes.
static void undoScaleAndPermutateTilingForPackOp(linalg::PackOp packOp,
	SmallVector<int64_t> &tileSizes, SmallVector<bool> &scalableFlags) {
	SmallVector<int64_t> innerTiles(packOp.getStaticInnerTiles());
	SmallVector<bool> innerTileScalableFlags(innerTiles.size(), false);
	// Infer scalable tile sizes and flags if present.
	...
	// Then unscale tile sizes by multiplying the inner tile sizes and setting
	// the corresponding scalable flags to true.
	for (auto [pos, size, scalable] :
		llvm::zip_equal(innerDimPos, innerTiles, innerTileScalableFlags)) {
...
class MultiLoweringConfigGenerator {
  public:
	/// Factory method to creates and returns a MultiLoweringConfigGenerator.
	/// May return null on failure.
	static std::unique_ptr<MultiLoweringConfigGenerator> create(
		Operation *rootOperation, ArrayRef<Operation *> computeOps);
	...
	/// Fills the vector parallel tile sizes that haven't been set yet with
	/// values from non-root ops. Pack ops are prioritized for good performance.
	...
	/// parts. Note: the concept of `VectorCommonParallel` is currently abused
	/// to refer to either: (1) a parallel (non-reduction) loop dimension that
	/// appears in all
	///     compute operations,
  ...
  private:
	...
	/// The Pack op requires special vector tile sizes, which are determined
	/// using getPackVectorTileSize to achieve optimal performance (e.g., a
	/// 16x16 tile size on AVX512 to generate efficient transpose code for the
	/// Pack op).
	///
	/// Example:
	/// For a Matmul RHS Pack op with `outer_dims_perm = [1, 0]` and
	/// `inner_tiles = [16, 1]`, `getPackVectorTileSize` initially returns `[1,
	/// 1]`. Since the `MultiLoweringConfigGenerator` propagates tiling of the
	/// outer (unpacked) dimensions, `undoScaleAndPermutateTilingForPackOp`
	/// translates the tile sizes from `[1, 1]` to `[1, 16]`.
	///
	/// As a result, the Pack op expects its producer (potentially the root op)
	/// to use tile sizes `[1, 16]` for those two dimensions, enabling
	/// tile-and-fuse optimizations.
	SizesAndScalableFlags getVecTileSizesForNonRootPackOp(
	...
	SizesAndScalableFlags getVecTileSizesForNonRootUnPackOp(
	...
	SmallVector<int64_t> getVecTileSizesForNonRootGenericOp(
		mlir::FunctionOpInterface entryPointFn, linalg::GenericOp genericOp);
	...
	llvm::SmallDenseMap<Operation *, SmallVector<int64_t>>
		nonRootOpVecTileSizes;

	// Store the vector parallel scalable flags preferred by non-root
	// operations. Operation -> (global loop dimension index -> scalable flag)
	llvm::SmallDenseMap<Operation *, SmallVector<bool>> nonRootOpScalableFlags;
};

std::unique_ptr<MultiLoweringConfigGenerator>
MultiLoweringConfigGenerator::create(
	Operation *rootOperation, ArrayRef<Operation *> computeOps) {
	...
	if (isa<linalg::ConvolutionOpInterface>(rootOperation)) {
		// TODO(dcaballe): We don't know yet how to properly propagate the
		// lowering config of a convolution.
		return nullptr;
	}
	...
	return std::unique_ptr<MultiLoweringConfigGenerator>(
		new MultiLoweringConfigGenerator(
			rootOperation, rootLoweringConfig, computeOps));
}
	...
	auto loadTilingLevel = [&](IREE::CPU::TilingLevel level) {
		...
		if (level == IREE::CPU::TilingLevel::DistributionTiles) {
			assert(rootLoweringConfig.hasWorkgroupTilingLevel() &&
				"Expected root lowering config to have workgroup tiling "
				"level.");
		...
		// `undoScaleAndPermutateTilingForPackOp` to translate tiling
		// information from the packed back to the unpacked dimensions.
		...
		for (auto [pos, size, flag] : llvm::enumerate(sizes, flags)) {
			int64_t globalDimIdx =
				dimTracker.getGlobalDimIdx(rootOperation, pos);
...
void MultiLoweringConfigGenerator::getVecTileSizesForNonRootOps(
	...
	for (auto op : computeOps) {
		...
		if (auto packOp = dyn_cast<linalg::PackOp>(op)) {
			...
			LDBG() << "Ignoring unknown operation type for non-root op: "
				   << *op;
		}
	}
}
...
void MultiLoweringConfigGenerator::adjustTileSizesForRootOp() {
	...
	auto adjustScalableFlags =
		[&](Operation *op, ArrayRef<bool> scalableFlags,
			IREE::CPU::TilingLevel level,
	...
	for (auto &[op, vecTileSize] : nonRootOpVecTileSizes) {
		if (isa<linalg::PackOp>(op)) {
			...
			adjust(op, vecTileSize, IREE::CPU::TilingLevel::DistributionTiles,
				align);
			adjust(op, vecTileSize,
				IREE::CPU::TilingLevel::VectorCommonParallelTiles, overwrite);
			adjustScalableFlags(op, nonRootOpScalableFlags.lookup(op),
				IREE::CPU::TilingLevel::VectorCommonParallelTiles, overwrite);
		} else if (auto unpackOp = dyn_cast<linalg::UnPackOp>(op)) {
			// For unpack op, just overwrite the vector parallel tile size and
			// the scalable flag. However, dimension tracking is expected be
			// broken in the case of `generic -> unpack`, since only the
			// unpacked dimensions are propagated. To correct this, use the
			// generic op result indexing map to update the tracking.
			//
			// Example: If the generic op result has an affine map
			//          (d0, d1, d2, d3) -> (d0, d1, d2, d3),
			//          we use `vecTileSize` to adjust (d2, d3) instead of (d0,
			//          d1).
			auto linalgOp =
			...
			SmallVector<bool> adjustedScalableFlags(
...
void MultiLoweringConfigGenerator::fillTileSizesWithNonRootOps() {
	...
	for (auto &[op, vecTileSize] : opToVecTileSize) {
		for (auto [pos, size] : llvm::enumerate(vecTileSize)) {
			int64_t globalDimIdx = dimTracker.getGlobalDimIdx(op, pos);
			int64_t &tile = globalTileSizes[IREE::CPU::TilingLevel::
					VectorCommonParallelTiles][globalDimIdx];
			// Only set the tile size if it hasn't been assigned yet.
			if (tile == 0 && size > 0) {
				tile = size;
				auto it = nonRootOpScalableFlags.find(op);
				if (it != nonRootOpScalableFlags.end() &&
					pos < it->second.size()) {
					globalScalableTileFlags
						[IREE::CPU::TilingLevel::VectorCommonParallelTiles]
	...
	for (auto &[op, vecTileSize] : nonRootOpVecTileSizes) {
		...
		for (auto [pos, pair] :
			...
			globalTileSizes[IREE::CPU::TilingLevel::VectorReductionTiles]
						   [globalDimIdx] = size;
			globalScalableTileFlags
				[IREE::CPU::TilingLevel::VectorReductionTiles][globalDimIdx] =
					globalScalableTileFlags[IREE::CPU::TilingLevel::
							VectorCommonParallelTiles][globalDimIdx];
		}
	}
}
...
void MultiLoweringConfigGenerator::splitCommonInnerVectorTiles() {
	...
	SmallVector<bool> &innerFlags = globalScalableTileFlags
		[IREE::CPU::TilingLevel::VectorInnerParallelTiles];
	for (auto [globalDimIdx, size, flag] :
		llvm::enumerate(commonSizes, commonFlags)) {
		// "Common" means a parallel loop present either in all compute ops or
		// in the root op.
	...
	for (auto op : computeOps) {
		...
		for (IREE::CPU::TilingLevel level : tilingLevels) {
			...
			for (auto [pos, iterType] : llvm::enumerate(iterTypes)) {
				int64_t globalDimIdx = dimTracker.getGlobalDimIdx(op, pos);
				// Skip dimensions where the reduction iterator type and tiling
				// level mismatch. The XOR(^) means:
				//   - If the loop dimension is a reduction but the current
				//   tiling level is not `VectorReductionTiles`, skip it.
				//   - If the loop dimension is not a reduction but the current
				//   tiling level is `VectorReductionTiles`, skip it.
			...
			if (auto packOp = dyn_cast<linalg::PackOp>(op)) {
				...
				// unpacked dimensions, while for a pack operation,
				// `LoweringConfig` defines tiling on the packed inner
				// dimensions. Therefore, use `scaleAndPermutateTilingForPackOp`
				// to translate the tiling information from the unpacked to the
				// packed dimensions.
				scaleAndPermutateTilingForPackOp(
					packOp, tileSizes, scalableFlags);
			}
...
/// mmt4d ops.
static LogicalResult adjustTileSizesForRootUnPackOp(
	mlir::FunctionOpInterface entryPointFn, Operation *rootOp) {
	...
	for (OpOperand *opOperand : linalgOp.getDpsInputOperands()) {
		...
		IREE::Codegen::ScalableTileFlags scalableFlags(
			innerTiles.size(), false);
		std::optional<SizesAndScalableFlags> sizesAndScalableFlags =
			getScalableTileSizesAndFlags(unpackOp.getMixedTiles());
		if (sizesAndScalableFlags) {
			innerTiles = sizesAndScalableFlags->first;
	...
	// scalable inner tiles, we align the distribution tile sizes with the
	// static constant of the scalable inner tile size and round up to the next
	// power of 2 to ensure alignment.
	...
	for (IREE::CPU::LoweringConfigLevelInfo &info : tilingInfo) {
		...
		for (auto idx : llvm::seq<int64_t>(0, tileSizes.size())) {
			...
			tileSizes[idx] = roundUpToPow2(alignedTileSize,
		...
		if (info.level == IREE::CPU::TilingLevel::VectorCommonParallelTiles) {
			for (auto [idx, scalable] :
				enumerate(vecParallelScalableTileFlags)) {
	...
	return setOpConfigAndEntryPointFnTranslation(entryPointFn, rootOp,
		newLoweringConfig, pipeline, /*workgroupSize=*/{},
		/*subgroupSize=*/{}, pipelineConfig);
}
...
static LogicalResult setLoweringConfigForComputeOps(
	...
	// Step 2: Collect tile sizes favored by non-root ops, and use them to
	// adjust the root operation’s tile sizes.
	...
	// Step 5: Split parallel vector tile sizes into common parts and
	// op-specific parts.
...
/// pipeline.
static LogicalResult lowerUsingDefaultPipeline(
	mlir::FunctionOpInterface entryPointFn) {
...
static LogicalResult setTranslationInfoAndRootConfig(
	...
	auto targetMLTransInfo =
		TargetMLTransformInfo::getTargetMLTransformInfo(targetAttr);
	if (failed(setRootConfigImpl(
			entryPointFn, rootOperation, targetMLTransInfo))) {
		return failure();
	}
	...
	auto pipeline =
		getTranslationInfo(entryPointFn).getPassPipeline().getValue();
	if (pipeline != DispatchLoweringPassPipeline::TransformDialectCodegen) {
		if (failed(
				adjustTileSizesForRootUnPackOp(entryPointFn, rootOperation))) {
			return failure();
		}

		auto prunedComputeOps = llvm::to_vector(
			llvm::make_filter_range(computeOps, shouldSetLoweringConfig));
		if (failed(setLoweringConfigForComputeOps(
				entryPointFn, prunedComputeOps, rootOperation))) {
			return failure();
		}
	}
