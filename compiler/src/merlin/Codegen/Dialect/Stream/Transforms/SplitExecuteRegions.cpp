...
static void collectLocalDeps(
	Operation *root, Block *block, DenseSet<Operation *> &collected) {
	...
	while (!worklist.empty()) {
		...
		for (Value operand : op->getOperands()) {
			...
			if (!defOp || defOp->getBlock() != block)
				continue;
			if (isa<IREE::Stream::AsyncDispatchOp>(defOp))
				continue;
...
static SmallVector<DispatchChunk> buildChunks(
	...
	for (auto [i, chunk] : llvm::enumerate(chunks)) {
		...
		for (Operation *op : chunk.ops) {
			for (Value result : op->getResults()) {
				bool escapes = false;
				if (yieldedValues.contains(result))
					escapes = true;
...
static IREE::Stream::AsyncExecuteOp emitChunkExecute(DispatchChunk &chunk,
	Value awaitTimepoint, IREE::Stream::AffinityAttr affinityAttr,
	IRMapping &valueMapping, OpBuilder &builder) {
	...
	for (Value input : chunk.externalInputs) {
		...
		if (!isa<IREE::Stream::ResourceType>(mapped.getType()))
			continue;
	...
	auto newExecuteOp =
		IREE::Stream::AsyncExecuteOp::create(builder, loc, resultTypes,
			resultSizes, awaitTimepoint, operands, operandSizes, tiedOperands);
	if (affinityAttr) {
...
static LogicalResult splitExecuteOp(IREE::Stream::AsyncExecuteOp executeOp) {
	auto &body = executeOp.getBody();
	...
	LLVM_DEBUG(llvm::dbgs()
		<< "Splitting execute with " << dispatchCount << " dispatches\n");
	...
	for (auto [blockArg, operand] : llvm::zip_equal(
			 block->getArguments(), executeOp.getResourceOperands())) {
	...
	// The original yield tells us which internal values mapped to which
	// results.
	auto yieldOp = cast<IREE::Stream::YieldOp>(block->getTerminator());
	for (auto [originalResult, yieldedValue] : llvm::zip_equal(
			 executeOp.getResults(), yieldOp.getResourceOperands())) {
...
struct SplitExecuteRegionsPass
	...
	void runOnOperation() override {
		...
		parentOp.getCallableRegion()->walk(
			[&](IREE::Stream::AsyncExecuteOp op) { executeOps.push_back(op); });
