...
CudaTileConvPlan extractCudaTileConvPlan(linalg::GenericOp genOp) {
  ...
  llvm::SmallVector<int64_t> kernelLoops(filterLoops.begin(),
  ...
  auto inputChannelExpr = dyn_cast<AffineDimExpr>(
      inputMap.getResult(hasBatchDim ? spatialRank + 1 : spatialRank));
  ...
  plan.isPointwise = llvm::all_of(
      llvm::ArrayRef<int64_t>(filterShape).take_front(spatialRank),
